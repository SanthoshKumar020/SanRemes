...
#
# Wired into the image as /etc/cont-init.d/01-sanremes-setup by the
...
SANREMES_HOME="${SANREMES_HOME:-/opt/data}"
INSTALL_DIR="/opt/sanremes"

# Drop to sanremes via s6-setuidgid, but skip it when already non-root.
as_sanremes() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid sanremes "$@"; }
...
# starts non-root. The baked install tree under /opt/sanremes is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $SANREMES_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass SANREMES_UID/SANREMES_GID — or the PUID/PGID aliases — which the
...
# `id -u sanremes` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# sanremes UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u sanremes)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-sanremes UID).
...
and the baked /opt/sanremes install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e SANREMES_UID=\$(id -u) -e SANREMES_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the sanremes user to that UID/GID at boot and chowns the data
...
# --- Bootstrap SANREMES_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid sanremes mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom SANREMES_HOME paths whose parents only
# root can create (e.g. `SANREMES_HOME=/home/sanremes/.sanremes` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for SANREMES_UID/SANREMES_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  SANREMES_UID/
# SANREMES_GID still win when both are set.  See #15290, salvages #25872.
SANREMES_UID="${SANREMES_UID:-${PUID:-}}"
SANREMES_GID="${SANREMES_GID:-${PGID:-}}"

if [ -n "${SANREMES_UID:-}" ] && validate_uid_gid "$SANREMES_UID" && [ "$SANREMES_UID" != "$(id -u sanremes)" ]; then
    echo "[stage2] Changing sanremes UID to $SANREMES_UID"
    usermod -u "$SANREMES_UID" sanremes
fi
if [ -n "${SANREMES_GID:-}" ] && validate_uid_gid "$SANREMES_GID" && [ "$SANREMES_GID" != "$(id -g sanremes)" ]; then
    echo "[stage2] Changing sanremes GID to $SANREMES_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$SANREMES_GID" sanremes 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised sanremes user (UID 10000)
...
# sanremes process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes sanremes. Idempotent across container
...
#   - sanremes is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G sanremes 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] sanremes already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" sanremes 2>/dev/null; then
        echo "[stage2] Added sanremes to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group sanremes failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When SANREMES_UID is remapped or the top-level $SANREMES_HOME isn't owned by
# the runtime sanremes UID, restore ownership to sanremes — but ONLY for the
# directories sanremes actually writes to. The full $SANREMES_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of sanremes-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_sanremes_uid=$(id -u sanremes)

path_has_symlink_component() {
    path="$1"
    root="${2:-$SANREMES_HOME}"
...
chown_sanremes_tree() {
    ...
    chown -R sanremes:sanremes "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_sanremes_owner() {
    target="$1"
    find "$target" \( ! -user sanremes -o ! -group sanremes \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$SANREMES_HOME" 2>/dev/null)" != "$actual_sanremes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $SANREMES_HOME (targeted) to sanremes ($actual_sanremes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $SANREMES_HOME: chown the directory itself (not its contents)
    # so sanremes can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$SANREMES_HOME"; then
        :
    else
        chown sanremes:sanremes "$SANREMES_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $SANREMES_HOME failed (rootless container?) — continuing"
    fi
    # SanRemes-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by sanremes (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$SANREMES_HOME/$sub" ] && tree_has_non_sanremes_owner "$SANREMES_HOME/$sub"; then
            chown_sanremes_tree "$SANREMES_HOME/$sub"
        fi
    done
fi
...
# sanremes user. Hosted/container instances keep mutable state under
# $SANREMES_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# SANREMES_DISABLE_LAZY_INSTALLS=1. Keeping /opt/sanremes root-owned and
...
# $SANREMES_HOME/lazy-packages dir on the data volume (Dockerfile sets
# SANREMES_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to sanremes
...
# Always reset ownership of $SANREMES_HOME/profiles to sanremes on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> sanremes …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as sanremes and walks
...
if [ -d "$SANREMES_HOME/profiles" ] && tree_has_non_sanremes_owner "$SANREMES_HOME/profiles"; then
    chown_sanremes_tree "$SANREMES_HOME/profiles"
fi

# Always reset ownership of $SANREMES_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged sanremes runtime even
...
# gate as profiles/).
if [ -d "$SANREMES_HOME/cron" ] && tree_has_non_sanremes_owner "$SANREMES_HOME/cron"; then
    chown_sanremes_tree "$SANREMES_HOME/cron"
fi

# Always ensure logs/gateways is sanremes-owned (#45258). Formerly healed by
...
# top-level $SANREMES_HOME is mis-owned, so a warm volume with sanremes-owned
# SANREMES_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid sanremes mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as sanremes.
if [ -d "$SANREMES_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$SANREMES_HOME/logs/gateways"; then
        :
    else
        chown sanremes:sanremes "$SANREMES_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# sanremes pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged sanremes gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $SANREMES_HOME is
...
# and cron/.
if [ -d "$SANREMES_HOME/platforms/pairing" ] && tree_has_non_sanremes_owner "$SANREMES_HOME/platforms/pairing"; then
    chown_sanremes_tree "$SANREMES_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$SANREMES_HOME/pairing" ] && tree_has_non_sanremes_owner "$SANREMES_HOME/pairing"; then
    chown_sanremes_tree "$SANREMES_HOME/pairing"
fi

# Reset ownership of sanremes-owned top-level state files on every boot.
# The targeted data-volume chown above only covers sanremes-owned
# *subdirectories*; loose state files living directly under $SANREMES_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> sanremes …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged sanremes runtime then hits
...
# sweep so host-owned files in a bind-mounted $SANREMES_HOME are never
...
# entries of sanremes_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$SANREMES_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$SANREMES_HOME/$f"; then
            :
        else
            chown sanremes:sanremes "$SANREMES_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the sanremes runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$SANREMES_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$SANREMES_HOME/config.yaml"; then
        :
    else
        chown sanremes:sanremes "$SANREMES_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$SANREMES_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as sanremes user ---
# Run as sanremes via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $SANREMES_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_sanremes mkdir -p \
    "$SANREMES_HOME/backups" \
    "$SANREMES_HOME/cron" \
    "$SANREMES_HOME/sessions" \
    "$SANREMES_HOME/logs" \
    "$SANREMES_HOME/logs/gateways" \
    "$SANREMES_HOME/hooks" \
    "$SANREMES_HOME/memories" \
    "$SANREMES_HOME/skills" \
    "$SANREMES_HOME/skins" \
    "$SANREMES_HOME/plans" \
    "$SANREMES_HOME/workspace" \
    "$SANREMES_HOME/home" \
    "$SANREMES_HOME/pairing" \
    "$SANREMES_HOME/platforms/pairing" \
    "$SANREMES_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/sanremes/.install_method (see Dockerfile), NOT written here into
# $SANREMES_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $SANREMES_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.sanremes:/opt/data) and sometimes shared with a
...
# 'sanremes update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $SANREMES_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/sanremes stamp, so deleting the data-dir copy is safe).
if [ -f "$SANREMES_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$SANREMES_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$SANREMES_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$SANREMES_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$SANREMES_HOME/$dest"; then
            :
        else
            as_sanremes cp "$INSTALL_DIR/$src" "$SANREMES_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/sanremes/.env.example being present
...
# not generate one. SanRemes loads $SANREMES_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$SANREMES_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$SANREMES_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $SANREMES_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$SANREMES_HOME/.env" ] && ! refuse_symlinked_path "clean" "$SANREMES_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$SANREMES_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$SANREMES_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$SANREMES_HOME/.env"; then
        :
    else
        if [ ! -f "$SANREMES_HOME/.env" ]; then
            ...
            (umask 077 && as_sanremes touch "$SANREMES_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$SANREMES_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$SANREMES_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$SANREMES_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $SANREMES_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $SANREMES_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$SANREMES_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$SANREMES_HOME/.env"; then
        :
    else
        chown sanremes:sanremes "$SANREMES_HOME/.env" 2>/dev/null || true
        chmod 600 "$SANREMES_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `sanremes` user — without this it gets EACCES on every
...
# sanremes group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp sanremes /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted sanremes group access to the Fly Machines API socket"
    else
...
# $SANREMES_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `sanremes update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set SANREMES_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$SANREMES_HOME/config.yaml" ]; then
    s6-setuidgid sanremes "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$SANREMES_HOME/auth.json" ] && [ -n "${SANREMES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$SANREMES_HOME/auth.json"; then
        :
    else
        printf '%s' "$SANREMES_AUTH_JSON_BOOTSTRAP" > "$SANREMES_HOME/auth.json"
        chown sanremes:sanremes "$SANREMES_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$SANREMES_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via SANREMES_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$SANREMES_HOME/auth.json" ] && [ -n "${SANREMES_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$SANREMES_HOME/auth.json"; then
        :
    else
        s6-setuidgid sanremes "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$SANREMES_HOME/auth.json" \
...
# can set SANREMES_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# SANREMES_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$SANREMES_HOME/gateway_state.json" ] && \
        [ "${SANREMES_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$SANREMES_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$SANREMES_HOME/gateway_state.json"
        chown sanremes:sanremes "$SANREMES_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$SANREMES_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_sanremes "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/sanremes/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI SanRemes spawns for the
...
# main-wrapper.sh propagates it into the supervised ``sanremes`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-sanremes, dashboard, etc.).
