...
# SanRemes WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: SANREMES_TINI_SHIM_TARGET overrides the `/init` path (and
# SANREMES_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${SANREMES_TINI_SHIM_TARGET:-/init}"
WRAPPER="${SANREMES_TINI_SHIM_WRAPPER:-/opt/sanremes/docker/main-wrapper.sh}"
