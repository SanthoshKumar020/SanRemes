#!/bin/sh
# shellcheck shell=sh
# /opt/sanremes/bin/sanremes — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `sanremes` user (UID 10000). When an operator runs `docker exec <c> sanremes ...`
# the default UID is root (0), and any file the command writes under
# $SANREMES_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> sanremes login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: SanRemes is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> sanremes
...
# Fix
# ---
# This shim sits at /opt/sanremes/bin/sanremes and is placed earliest on PATH.
# When invoked as root, it drops to the sanremes user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $SANREMES_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user sanremes`, kanban subagents, etc. — it short-circuits
...
# (/opt/sanremes/.venv/bin/sanremes), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set SANREMES_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the sanremes CLI. Default is to drop.

set -e

REAL=/opt/sanremes/.venv/bin/sanremes

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "sanremes-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user sanremes`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${SANREMES_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the sanremes user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "sanremes-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "sanremes-shim: re-run with --user sanremes or set SANREMES_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the sanremes user's home before dropping privileges. Without
