...
has to be thrown away or summarized. SanRemes has always done this in one batch:
...
SanRemes folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. SanRemes's normal session flush is
...
With `micro_compact` unset or `false` SanRemes behaves exactly as it always has:
...
Defaults to `$SANREMES_HOME/logs/agent.log`. It reports passes, outcome mix, net
