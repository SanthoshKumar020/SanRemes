...
## 2026-07-13: Scope plugin manager state by SanRemes home/profile (keyed cache)

Status: Accepted

Context:
SanRemes supports multiple profiles via different SanRemes home directories.
Homes are switched two ways in a running process: the `SANREMES_HOME`
environment variable (single-profile CLI/gateway processes), and the
context-local `set_sanremes_home_override()` (`sanremes_constants.py`), which
...
`get_sanremes_home() / "plugins"`, and context-engine plugins (e.g.
`sanremes-lcm`) capture profile-scoped state — such as the LCM database
path — at registration time. A single-slot cache meant:

1. Switching homes via `set_sanremes_home_override()` was invisible to a
   naive "did `SANREMES_HOME` change" check, so the singleton silently kept
...
2. Even when a fresh `PluginManager` *was* created for a new home, plugin
   modules are imported into `sys.modules` as `sanremes_plugins.<slug>` by
   ...
   `sanremes_plugins.<slug>.<submodule>`, and Python's import machinery
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  SanRemes home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
  `get_plugin_manager()` resolves the current home via `get_sanremes_home()`
  (which itself already consults `get_sanremes_home_override()` before
...
- Per-profile LCM instances (and any other context-engine plugin) use
  ...
  through `SANREMES_HOME` or `set_sanremes_home_override()`.
...
- Regression coverage exercises the real production path
  (`set_sanremes_home_override()`) rather than only the env-var path, and
