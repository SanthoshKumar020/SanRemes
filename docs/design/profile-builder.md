...
Supersedes: PR #31781 (prompt_toolkit `sanremes profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `sanremes profile wizard` in the terminal.
...
needs, and a profile is just a SANREMES_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.sanremes/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — SANREMES_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with SANREMES_HOME override |
...
### Seam #1 — hub-skill install cannot use the SANREMES_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = SANREMES_HOME / "skills"` at **module
import time**. The context-local `set_sanremes_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_sanremes_action`
runs `python -m sanremes_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_sanremes_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's SANREMES_HOME bound
...
spawned detached (`_spawn_sanremes_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (synchronous)
2. write model (synchronous, SANREMES_HOME override)
3. write selected MCP servers (synchronous, SANREMES_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `sanremes -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, SANREMES_HOME override
...
page afterward. Mirror `_write_profile_model`'s SANREMES_HOME-override helper for
...
- Backend E2E with isolated SANREMES_HOME: POST a full create body
