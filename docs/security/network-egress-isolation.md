...
When running SanRemes inside Docker, the default `network_mode: host` gives the
...
The SanRemes [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ sanremes-agent │   │ sanremes-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   SANREMES_UID=$(id -u) SANREMES_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=sanremes,sanremes-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://sanremes-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — SanRemes trust model and vulnerability reporting
