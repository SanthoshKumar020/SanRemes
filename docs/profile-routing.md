...
`sanremes config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.sanremes/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `SANREMES_HOME`, secret scope, and
