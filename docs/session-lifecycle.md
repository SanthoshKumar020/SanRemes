...
The canonical transcript store is SQLite via `SessionDB` (from `sanremes_state`). The
...
Counts consecutive restarts via a JSON file (`{SANREMES_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `sanremes update`, `sanremes gateway restart`,
or `/restart`.
