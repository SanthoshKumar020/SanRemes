...
or change arbitrary SanRemes configuration.
...
small host fact requested by the RFC. Settings writes use SanRemes'
...
`SANREMES_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<SANREMES_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  SanRemes home on every operation.
...
The implementation is covered with real temporary-SanRemes-home tests for:
...
- [Issue #64227](https://github.com/NousResearch/sanremes-agent/issues/64227)
- [RFC PR #58542](https://github.com/NousResearch/sanremes-agent/pull/58542) by Topher Ross
