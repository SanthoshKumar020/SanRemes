...
SanRemes middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing SanRemes'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before SanRemes calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving SanRemes retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving SanRemes guardrails,
...
- `telemetry_schema_version`: currently `sanremes.observer.v1`
- `middleware_schema_version`: currently `sanremes.middleware.v1`
...
SanRemes stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, SanRemes runs
them as a nested chain in registration order. Middleware failures are fail-open:
SanRemes logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, SanRemes applies middleware in this order:
...
For each tool call, SanRemes applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal SanRemes pre-execution path against the effective arguments:
...
For isolated local testing, use one `SANREMES_HOME` for plugin enablement and the
agent run:

```bash
export SANREMES_HOME=/tmp/sanremes-middleware-test
mkdir -p "$SANREMES_HOME"
sanremes plugins enable <plugin-name>
sanremes chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run sanremes plugins enable <plugin-name>
uv run sanremes chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["sanremes_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/sanremes-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape SanRemes expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, SanRemes treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, SanRemes preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. SanRemes does not convert downstream
