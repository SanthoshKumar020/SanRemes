...
SanRemes supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.sanremes/config.yaml`:
...
Or set the env var: `SANREMES_KANBAN_DISPATCH_IN_GATEWAY=false`
