...
This document is the formal interface between the **SanRemes gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `sanremes gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`sanremes gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `sanremes` CLI.
