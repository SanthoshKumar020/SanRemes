# SanRemes Observer Hooks

SanRemes observer hooks are the read-only telemetry contract for plugins that
...
They do not replace SanRemes' planner, model providers, memory, tool registry,
...
SanRemes also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "sanremes.observer.v1"
```

Hook callbacks are fail-open. SanRemes catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so SanRemes only builds
...
The native NeMo Relay SDK integration maps SanRemes session, turn, LLM, and tool
