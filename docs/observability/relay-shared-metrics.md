...
SanRemes includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into SanRemes and does not require a SanRemes observability plugin. SanRemes remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
SanRemes execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `sanremes-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the SanRemes `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `SANREMES_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `SANREMES_NEMO_RELAY_ATOF_*` and `SANREMES_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, SanRemes does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

SanRemes requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
SanRemes installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the SanRemes process so configured interceptors can
...
Collection remains off unless SanRemes policy enables it:
...
opt-in. Set `SANREMES_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, SanRemes does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, SanRemes
...
retain the same `session_id` and add `sanremes.session.segment` plus
`sanremes.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a SanRemes
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that SanRemes process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, SanRemes logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this SanRemes process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the SanRemes
process and share the same policy boundary.

Run profiles in separate SanRemes processes when they require different trust
...
SanRemes core owns one Relay host and one isolated Relay session scope per SanRemes
session. Core lifecycle producers use
`sanremes_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require SanRemes plugin registration. Shared-metrics marks must
...
```text
SanRemes turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> SanRemes shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

SanRemes sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that SanRemes used for the logical call, such as
...
New calls use `sanremes.model_route.count`. The previous
`sanremes.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `sanremes.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; SanRemes
version, OS family, architecture, and install method remain bounded package
resources. Concurrent SanRemes processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `sanremes.task_run`, parented to
the owning SanRemes session. The start counter contains only bounded execution
...
provider attempts for the same SanRemes API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their SanRemes tool-call
...
cancellations. Active task ownership follows the task ID if SanRemes rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`sanremes.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. SanRemes
...
tool or plugin names. SanRemes does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `sanremes.tool_approval` marks and
...
Successful skill mutations emit `sanremes.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `sanremes.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. SanRemes derives reuse and patch-generation
...
```text
$SANREMES_HOME/telemetry/shared_metrics/metrics.sqlite3
$SANREMES_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the SanRemes version,
...
schema field name, its current scope is one `SANREMES_HOME`, so it is more
...
Deleting `$SANREMES_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `SANREMES_HOME`. To reset it, stop SanRemes
processes and remove `$SANREMES_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real SanRemes CLI turn against the deterministic local model server:
