...
SanRemes gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by SanRemes's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `sanremes.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `sanremes.platform.up/degraded` with bounded `error_code` attributes |
...
`sanremes.gateway.active_agents`, `sanremes.gateway.background_work`, and
`sanremes.gateway.background_delegations` are complementary. `active_agents`
...
```bash
sanremes monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'sanremes-agent[otlp]'`),
...
a plain `sanremes gateway run`: the exporter lives in the gateway process, so
...
`sanremes.gateway.up`, `sanremes.platform.up`, and `sanremes.platform.degraded`.
...
absent_over_time(sanremes_gateway_up[5m])

# Locally owned bridge is explicitly down.
sanremes_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
sanremes_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
sanremes_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
sanremes_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(sanremes_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `sanremes.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
sanremes.status = failed|unknown
sanremes.delivery_outcome = failed|not_configured
sanremes.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `sanremes.job_key`;
...
SanRemes Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/sanremes_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/sanremes_otel_capture.jsonl --wait 8
...
`sanremes.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
sanremes monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `sanremes monitoring` CLI intentionally exposes `status` only. This first
release covers only SanRemes Agent-owned service-health and operational-diagnostic
signals, including SanRemes Agent-owned Relay transport health. Team Gateway's
