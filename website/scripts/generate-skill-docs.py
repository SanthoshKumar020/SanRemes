...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/sanremes-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    sanremes_meta = (fm.get("metadata") or {}).get("sanremes") or {}
    tags = sanremes_meta.get("tags") or []
    related = sanremes_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `sanremes skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with SanRemes Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "SanRemes ships with a large built-in skill library copied into `~/.sanremes/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "SanRemes also syncs bundled skills on `sanremes update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.sanremes/skills/` tree, it is still shipped with SanRemes; restore it with `sanremes skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with sanremes-agent — install via sanremes skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with sanremes-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "sanremes skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "sanremes skills install official/blockchain/solana",
        "sanremes skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "sanremes skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
