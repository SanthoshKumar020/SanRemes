        ...
        {expanded && (
          <div className={styles.cardDetail}>
            ...
            <div className={styles.installHint}>
              <code>{skill.installCmd || `sanremes skills install ${skill.name}`}</code>
              <CopyButton
                text={skill.installCmd || `sanremes skills install ${skill.name}`}
              />
            </div>
            {onPick ? (
              <button
...
export default function SkillsDashboard() {
  ...
  // app (SanRemes desktop's Bot Mode agent editor) as a skill PICKER. Site
  // chrome is hidden via a CSS class and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'sanremes-skill-pick', name, identifier, installCmd, source }
  ...
  const pickSkill = useCallback(
    (skill: Skill) => {
      ...
      window.parent.postMessage(
        {
          type: "sanremes-skill-pick",
          name: skill.name,
          identifier: skill.identifier || skill.name,
          installCmd: skill.installCmd || `sanremes skills install ${skill.name}`,
          source: skill.source,
        },
        "*"
      );
  ...
  return (
    <Layout
      title="Skills Hub"
      description="Browse all skills and plugins available for SanRemes Agent"
    >
      <div className={`${styles.page} ${pickerMode ? styles.pickerMode : ""}`}>
        <header className={styles.hero}>
          <div className={styles.heroGlow} />
          <div className={styles.heroContent}>
            <p className={styles.heroEyebrow}>SanRemes Agent</p>
