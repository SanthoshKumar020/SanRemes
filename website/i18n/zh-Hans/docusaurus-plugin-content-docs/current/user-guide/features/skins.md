...
sidebar_position: 10
title: "皮肤与主题"
description: "使用内置和用户自定义皮肤定制 SanRemes CLI 的外观"
---

# 皮肤与主题

皮肤控制 SanRemes CLI 的**视觉呈现**：横幅颜色、spinner（加载动画）面孔与动词、响应框标签、品牌文本以及工具活动前缀。
...
或在 `~/.sanremes/config.yaml` 中设置默认皮肤：
...
| 皮肤 | 描述 | Agent 品牌 | 视觉特征 |
|------|------|-----------|---------|
| `default` | 经典 SanRemes — 金色与 kawaii 风格 | `SanRemes Agent` | 暖金色边框，cornsilk 文字，spinner 中的 kawaii 面孔。熟悉的双蛇杖横幅。简洁亲切。 |
| `ares` | 战神主题 — 深红与青铜 | `Ares Agent` | 深红色边框配青铜点缀。激进的 spinner 动词（"forging"、"marching"、"tempering steel"）。自定义剑盾 ASCII 艺术横幅。 |
| `mono` | 单色 — 简洁灰度 | `SanRemes Agent` | 全灰色，无彩色。边框为 `#555555`，文字为 `#c9d1d9`。适合极简终端或录屏场景。 |
| `slate` | 冷蓝色 — 面向开发者 | `SanRemes Agent` | 皇家蓝边框（`#4169e1`），柔和蓝色文字。沉稳专业。无自定义 spinner，使用默认面孔。 |
| `daylight` | 适用于亮色终端的浅色主题，深色文字配冷蓝点缀 | `SanRemes Agent` | 专为白色或亮色终端设计。深石板色文字配蓝色边框，浅色状态面板，补全菜单在亮色终端配置下保持清晰可读。 |
| `warm-lightmode` | 适用于浅色终端背景的暖棕/金色文字 | `SanRemes Agent` | 适合浅色终端的暖羊皮纸色调。深棕色文字配马鞍棕点缀，奶油色状态面板。比 daylight 主题更温暖的大地色系选择。 |
...
| 键 | 描述 | 默认值 |
|----|------|--------|
| `agent_name` | 横幅标题和状态显示中的名称 | `SanRemes Agent` |
| `welcome` | CLI 启动时显示的欢迎消息 | `Welcome to SanRemes Agent! Type your message or /help for commands.` |
| `goodbye` | 退出时显示的消息 | `Goodbye! ⚕` |
| `response_label` | 响应框标题上的标签 | ` ⚕ SanRemes ` |
...
| `banner_logo` | 字符串 | Rich 标记 ASCII 艺术 logo（替换默认的 SANREMES_AGENT 横幅） | `""` |
| `banner_hero` | 字符串 | Rich 标记英雄艺术图（替换默认的双蛇杖图案） | `""` |

## 自定义皮肤

在 `~/.sanremes/skins/` 下创建 YAML 文件。用户皮肤会从内置 `default` 皮肤继承缺失的值，因此只需指定要更改的键。
...
## SanRemes Mod — 可视化皮肤编辑器

[SanRemes Mod](https://github.com/cocktailpeanut/sanremes-mod) 是一个社区构建的 Web UI，用于可视化创建和管理皮肤。无需手写 YAML，提供带实时预览的点击式编辑器。
...
- 列出所有内置和自定义皮肤
- 将任意皮肤在可视化编辑器中打开，涵盖所有 SanRemes 皮肤字段（颜色、spinner、品牌、工具前缀、工具 emoji）
- 根据文字 prompt 生成 `banner_logo` 文字艺术
- 将上传的图片（PNG、JPG、GIF、WEBP）转换为 `banner_hero` ASCII 艺术，支持多种渲染风格（盲文点阵、ASCII 字符渐变、方块、点阵）
- 直接保存到 `~/.sanremes/skins/`
- 通过更新 `~/.sanremes/config.yaml` 激活皮肤
- 显示生成的 YAML 及实时预览
...
6. 点击 **Save** 将皮肤 YAML 写入 `~/.sanremes/skins/`。
7. 点击 **Activate** 将其设为当前皮肤（更新 `config.yaml` 中的 `display.skin`）。

SanRemes Mod 遵循 `SANREMES_HOME` 环境变量，因此也适用于[配置文件](/user-guide/profiles)。

## 操作说明

- 内置皮肤从 `sanremes_cli/skin_engine.py` 加载。
- 未知皮肤自动回退到 `default`。
- `/skin` 立即更新当前会话的活动 CLI 主题。
- `~/.sanremes/skins/` 中的用户皮肤优先于同名内置皮肤。
