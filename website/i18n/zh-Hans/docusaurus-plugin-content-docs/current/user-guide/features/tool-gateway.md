...
运行 `sanremes model` 并选择 Nous Portal 作为提供商时，SanRemes 会主动询问是否启用 Tool Gateway：
...
网关使用 Nous Portal 凭据认证（在 `sanremes model` 完成后写入 `~/.sanremes/auth.json`）。
...
- **选择为具体厂商**（如 `fal`、`firecrawl`）→ 直连；若对应 Key 缺失则报错并提示运行 `sanremes tools`，**不会**静默回退到网关
- **从未配置过的类别** → 按可用凭据自动检测（行为不变）；但一旦存在选择，仅往 `.env` 加 Key 不会改变路由

（旧版的 `use_gateway` 布尔键已废弃：不再写入，读取时 `use_gateway: true` 等同于 `nous`。请改用 `sanremes tools` 选择提供商。）
...
在 `sanremes tools` 中选择非网关提供商时，选择键会被改写为该厂商名（旧的 `use_gateway` 键若存在会被一并移除），避免配置自相矛盾。
...
若使用自建或自定义网关，可在 `~/.sanremes/.env` 中用环境变量覆盖端点：
...
不需要。类别选择为 **Nous Subscription**（`nous`）时，运行时会忽略该类别的直连 Key；Key 仍保留在 `.env`。之后在 `sanremes tools` 里改回直连提供商，Key 即恢复生效。
...
经网关路由的工具会停止工作，直到你 [续订](https://portal.nousresearch.com/manage-subscription) 或通过 `sanremes tools` 改回直连 Key。
...
Modal（无服务器终端后端）可作为 Nous 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `sanremes setup terminal` 或在 `config.yaml` 中配置。
