...
- 用户希望 **SanRemes 本身驱动浏览器** → 使用 SanRemes 内置的浏览器工具（Browserbase / Camofox）。page-agent 是*相反*的方向。
