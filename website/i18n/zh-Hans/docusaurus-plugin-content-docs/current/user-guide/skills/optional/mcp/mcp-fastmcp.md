...
| | |
|---|---|
| 来源 | 可选 — 通过 `sanremes skills install official/mcp/fastmcp` 安装 |
...
| 相关 skill | [`sanremes-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sanremes-agent), [`mcporter`](/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- 除工具外还需暴露资源或 prompt
- 在接入 SanRemes 或其他客户端之前，使用 FastMCP CLI 对服务器进行冒烟测试
- 将服务器安装到 Claude Code、Claude Desktop、Cursor 或类似的 MCP 客户端
- 为 HTTP 部署准备 FastMCP 服务器仓库

若服务器已存在且只需连接到 SanRemes，请使用 `native-mcp`。若目标是对现有 MCP 服务器进行临时 CLI 访问而非构建新服务器，请使用 `mcporter`。
...
若目标是集成到 SanRemes，可选择：

- 使用 `native-mcp` skill，在 `~/.sanremes/config.yaml` 中配置服务器，或
...
### SanRemes 无法看到已部署的服务器

服务器构建部分可能正确，但 SanRemes 配置有误。加载 `native-mcp` skill 并在 `~/.sanremes/config.yaml` 中配置服务器，然后重启 SanRemes。
