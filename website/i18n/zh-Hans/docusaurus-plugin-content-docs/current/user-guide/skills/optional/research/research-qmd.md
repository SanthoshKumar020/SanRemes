...
| | |
|---|---|
| 来源 | 可选 — 通过 `sanremes skills install official/research/qmd` 安装 |
...
| 相关 skill | [`obsidian`](/user-guide/skills/bundled/note-taking/note-taking-obsidian), [`sanremes-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sanremes-agent), [`arxiv`](/user-guide/skills/bundled/research/research-arxiv) |
...
qmd 提供 MCP 服务器，可通过原生 MCP 客户端直接向 SanRemes Agent 提供搜索工具。这是推荐的集成方式 — 配置完成后，agent 无需每次加载此 skill 即可自动获得 qmd 工具。
...
然后配置 SanRemes Agent 通过 HTTP 连接：
