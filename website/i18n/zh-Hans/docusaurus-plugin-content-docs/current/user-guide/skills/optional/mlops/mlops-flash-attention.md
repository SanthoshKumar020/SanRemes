...
| | |
|---|---|
| 来源 | 可选 — 通过 `sanremes skills install official/mlops/flash-attention` 安装 |
...
**与 HuggingFace Transformers 集成**：参见 [references/transformers-integration.md](https://github.com/NousResearch/sanremes-agent/blob/main/optional-skills/mlops/flash-attention/references/transformers-integration.md)，了解如何在 BERT、GPT、Llama 模型中启用 Flash Attention。

**性能基准测试**：参见 [references/benchmarks.md](https://github.com/NousResearch/sanremes-agent/blob/main/optional-skills/mlops/flash-attention/references/benchmarks.md)，查看跨 GPU 和序列长度的详细速度与内存对比。
