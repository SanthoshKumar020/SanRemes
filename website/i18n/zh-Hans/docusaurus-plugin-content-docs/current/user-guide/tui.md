...
sidebar_position: 2
title: "TUI"
description: "启动 SanRemes 的现代终端 UI——支持鼠标操作、丰富的浮层面板和非阻塞输入。"
---

# TUI

TUI 是 SanRemes 的现代前端——一个终端 UI（用户界面），与 [Classic CLI](cli.md) 共享同一 Python 运行时。相同的 agent、相同的会话、相同的斜杠命令；交互界面更简洁、响应更流畅。

这是以交互方式运行 SanRemes 的推荐方式。
...
- **即时首帧** — banner 在应用加载完成前就已渲染，因此 SanRemes 启动时终端不会出现卡顿感。
...
- **Node.js** ≥ 20 — TUI 作为从 Python CLI 启动的子进程运行。`sanremes doctor` 会验证此项。
- **TTY** — 与 classic CLI 一样，通过管道传入 stdin 或在非交互式环境中运行时，将回退到单次查询模式。

首次启动时，SanRemes 会将 TUI 的 Node 依赖安装到 `ui-tui/node_modules`（一次性操作，耗时数秒）。后续启动速度很快。拉取新版 SanRemes 后，若源文件比 dist 更新，TUI bundle 将自动重新构建。

### 外部预构建

发行版若附带预构建 bundle（如 Nix、系统包），可将 SanRemes 指向该 bundle：
...
| `/reload` | 将 `~/.sanremes/.env` 重新读入运行中的 TUI 进程，使新添加的 API 密钥无需重启即可生效 |
...
1. `SANREMES_TUI_THEME` 环境变量——最高优先级。可选值：`light`、`dark`，或原始 6 位背景十六进制色值（如 `ffffff`、`1a1a2e`）。
...
状态栏忙碌指示器可插拔——默认在 agent 工作期间每 2.5 秒轮换一次 SanRemes 的 kawaii 表情调色板。通过配置或 `/indicator` 斜杠命令选择不同样式：
...
默认情况下，`sanremes --tui` 每次启动都会开启新会话。若要自动重新连接到最近的 TUI 会话（在终端或 SSH 连接意外断开时很有用），可选择启用：

```bash
export SANREMES_TUI_RESUME=1          # 最近的 TUI 会话
# 或：
export SANREMES_TUI_RESUME=<session-id>   # 指定会话
```
...
- **工作目录及 git 分支** — `~/projects/sanremes-agent (docs/two-week-gap-sweep)`。在旁边的终端执行 `git checkout` 时，分支后缀会更新（mtime 缓存），TUI 反映的是实际活跃分支，而非启动时的分支。
...
- **`⚠ YOLO`** — 每当 YOLO 模式开启时（`sanremes --yolo`、`/yolo` 或 `SANREMES_YOLO_MODE=1`）显示的可见警告。同一徽章也出现在启动 banner 中，确保你不会在未注意到的情况下启动自动审批会话。

## 配置

TUI 遵循所有标准 SanRemes 配置：`~/.sanremes/config.yaml`、profile、personality、skin、快捷命令、凭证池、内存提供商、工具/skill 启用状态。不存在 TUI 专属配置文件。
...
会话在 TUI 和 classic CLI 之间共享——两者均写入同一个 `~/.sanremes/state.db`。可以在一个界面开始会话，在另一个界面恢复。会话选择器显示来自两个来源的会话，并带有来源标签。
...
你可能会在代码或日志中看到 `SANREMES_TUI_GATEWAY_URL` 环境变量。它是 **Web 仪表板的内部接线细节**，并非面向用户的远程连接开关。当你打开仪表板的 "Chat" 标签页（`sanremes dashboard` → `/chat`）时，仪表板的 Web 服务器会派生一个内嵌的 TUI 子进程，并注入 `SANREMES_TUI_GATEWAY_URL`，让该子进程通过本地回环 WebSocket（`/api/ws`）连接到仪表板自己的进程内 `tui_gateway`。`/api/ws` 端点仅存在于仪表板服务器内部（`sanremes_cli/web_server.py`），并绑定到该进程的生命周期和认证。

不存在通用的"将任意 TUI 指向任意独立 gateway 端口"的模式。特别是，OpenAI 兼容 API 服务器（`sanremes gateway` / `api_server` 平台）**不**提供 `/api/ws`——它是模型后端接口（`/v1/chat/completions`、`/v1/models` 等），并刻意不暴露 TUI 的 JSON-RPC 控制通道。将 `SANREMES_TUI_GATEWAY_URL` 设置为该端口将返回 404。

如果你希望多个界面共享同一组会话，请使用共享的 `~/.sanremes/state.db`（参见[会话](sessions.md)）或 Web 仪表板的内嵌聊天（参见 [Web Dashboard](features/web-dashboard.md#chat)）——而不是手动设置 gateway URL。

## 回退到 Classic CLI

不带 `--tui` 启动 `sanremes` 将继续使用 classic CLI。若要让某台机器默认使用 TUI，在 shell profile 中设置 `SANREMES_TUI=1`。若要回退，取消设置即可。

如果 TUI 启动失败（无 Node、缺少 bundle、TTY 问题），SanRemes 会打印诊断信息并回退——而不是让你陷入困境。
