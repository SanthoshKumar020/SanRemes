...
description: "将 SanRemes Agent 设置为 Microsoft Teams 机器人"
---

# Microsoft Teams 设置

将 SanRemes Agent 作为机器人接入 Microsoft Teams。与 Slack 的 Socket Mode 不同，Teams 通过调用**公开 HTTPS webhook**（钩子）来投递消息，因此你的实例需要一个可公开访问的端点——本地开发时使用开发隧道，生产环境使用真实域名。
...
Teams 将 @提及作为普通消息投递，其中包含 `<at>BotName</at>` 标签，SanRemes 在处理前会自动去除这些标签。
...
devtunnel port create sanremes-bot -p 3978 --protocol http  # 如已修改 TEAMS_PORT，请替换 3978
...
公开隧道 URL 使用 HTTPS，但 SanRemes 的本地 webhook 监听器使用纯 HTTP。隧道会终止 TLS，并将 HTTP 转发到 `3978` 端口；不要将本地隧道端口配置为 HTTPS。
...
也可通过 `~/.sanremes/config.yaml` 进行配置：
...
对于永久服务器，请在反向代理处终止 TLS，并将请求转发到 SanRemes 的纯 HTTP 监听器，通常为 `http://127.0.0.1:3978`。使用该代理的公开 HTTPS 端点注册机器人：
...
确保公开 HTTPS 端点可从互联网访问并使用有效的 TLS 证书。Teams 会拒绝自签名证书。请将 SanRemes 监听器置于代理后方；`3978` 端口本身不提供 HTTPS。
...
| Teams 发送消息时日志显示 `"UNKNOWN / HTTP/1.0" 400` | 隧道或反向代理正在将 HTTPS 转发到 SanRemes 的纯 HTTP 监听器。请在代理处终止 TLS，并将 HTTP 转发到 `3978` 端口 |
| 日志中出现 `KeyError: 'teams'` | 重启容器——此问题已在当前版本中修复 |
| 机器人响应时出现认证错误 | 验证 `TEAMS_CLIENT_ID`、`TEAMS_CLIENT_SECRET` 和 `TEAMS_TENANT_ID` 是否均已正确设置 |
| `No inference provider configured` | 检查 `~/.sanremes/.env` 中是否设置了 `ANTHROPIC_API_KEY`（或其他提供商密钥） |
| 机器人收到消息但忽略它们 | 你的 AAD 对象 ID 可能不在 `TEAMS_ALLOWED_USERS` 中。运行 `teams status --verbose` 查找 |
| 隧道 URL 在重启后变更 | 使用命名隧道（`devtunnel create sanremes-bot`）时，devtunnel URL 是持久的。ngrok 和 cloudflared 每次运行都会生成新 URL（除非你有付费计划）——URL 变更时请用 `teams app update` 更新机器人端点 |
| Teams 显示"此机器人未响应" | Webhook 返回了错误。检查 `docker logs sanremes` 中的错误堆栈 |
...
- 将凭据存储在权限为 `600` 的 `~/.sanremes/.env` 中（`chmod 600 ~/.sanremes/.env`）
