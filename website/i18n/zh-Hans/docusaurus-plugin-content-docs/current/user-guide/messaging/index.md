...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 SanRemes 对话 — 架构与配置概览"
---

# 消息网关

通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Feishu/Lark、WeCom、Weixin、BlueBubbles（iMessage）、QQ、Yuanbao、Microsoft Teams、LINE、ntfy 或浏览器与 SanRemes 对话。网关是一个单一后台进程，连接所有已配置的平台，管理会话，运行 cron 任务，并传递语音消息。

完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with SanRemes](/guides/use-voice-mode-with-sanremes)。
...
```bash
sanremes gateway setup        # 交互式配置所有消息平台
```
...
```bash
sanremes gateway              # 在前台运行
sanremes gateway setup        # 交互式配置消息平台
sanremes gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo sanremes gateway install --system   # 仅 Linux：安装开机启动的系统服务
sanremes gateway start        # 启动默认服务
sanremes gateway stop         # 停止默认服务
sanremes gateway status       # 检查默认服务状态
sanremes gateway status --system         # 仅 Linux：显式检查系统服务
```
...
| `/update` | 将 SanRemes Agent 更新至最新版本 |
| `/help` | 显示可用命令 |
...
**默认情况下会话永不自动重置** —— 上下文会一直保留，直到你手动 `/reset` 或触发上下文压缩。如果你希望会话自动重置，可在 `~/.sanremes/config.yaml` 的 `session_reset` 部分选择启用：
...
在 `~/.sanremes/gateway.json` 中配置各平台的覆盖设置：
...
# 其他配对命令：
sanremes pairing list          # 查看待审核和已批准的用户
sanremes pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，SanRemes 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。

如果你觉得繁忙确认消息过多——尤其是使用语音输入或快速连续发送消息时——可设置 `display.busy_ack_enabled: false`。你的输入仍会正常排队/引导/中断，只是聊天回复被静默。

## 工具进度通知

在 `~/.sanremes/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.sanremes/config.yaml` 中的 `display.background_process_notifications` 控制：
...
sanremes gateway start                 # 启动服务
sanremes gateway stop                  # 停止服务
sanremes gateway status                # 检查状态
journalctl --user -u sanremes-gateway -f  # 查看日志
...
除非你确实有此需要，否则避免同时安装用户和系统网关单元。SanRemes 检测到两者同时存在时会发出警告，因为 start/stop/status 行为会变得不明确。

:::info 多个安装
如果你在同一台机器上运行多个 SanRemes 安装（使用不同的 `SANREMES_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.sanremes` 使用 `sanremes-gateway`；其他安装使用 `sanremes-gateway-<hash>`。`sanremes gateway` 命令会自动针对当前 `SANREMES_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
sanremes gateway install               # 安装为 launchd agent
sanremes gateway start                 # 启动服务
sanremes gateway stop                  # 停止服务
sanremes gateway status                # 检查状态
tail -f ~/.sanremes/logs/gateway.log   # 查看日志
```

生成的 plist 文件位于 `~/Library/LaunchAgents/ai.sanremes.gateway.plist`。它包含三个环境变量：
...
- **SANREMES_HOME** — 将网关限定到你的 SanRemes 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `sanremes gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `SANREMES_HOME` 目录都有自己的 launchd 标签。默认的 `~/.sanremes` 使用 `ai.sanremes.gateway`；其他安装使用 `ai.sanremes.gateway-<suffix>`。
:::
...
| 平台 | 工具集 | 功能 |
|----------|---------|--------------|
| CLI | `sanremes-cli` | 完全访问 |
| Telegram | `sanremes-telegram` | 完整工具，包括终端 |
| Discord | `sanremes-discord` | 完整工具，包括终端 |
| WhatsApp | `sanremes-whatsapp` | 完整工具，包括终端 |
| Slack | `sanremes-slack` | 完整工具，包括终端 |
| Google Chat | `sanremes-google_chat` | 完整工具，包括终端 |
| Signal | `sanremes-signal` | 完整工具，包括终端 |
| SMS | `sanremes-sms` | 完整工具，包括终端 |
| Email | `sanremes-email` | 完整工具，包括终端 |
| Home Assistant | `sanremes-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `sanremes-mattermost` | 完整工具，包括终端 |
| Matrix | `sanremes-matrix` | 完整工具，包括终端 |
| DingTalk | `sanremes-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `sanremes-feishu` | 完整工具，包括终端 |
| WeCom | `sanremes-wecom` | 完整工具，包括终端 |
| WeCom Callback | `sanremes-wecom-callback` | 完整工具，包括终端 |
| Weixin | `sanremes-weixin` | 完整工具，包括终端 |
| BlueBubbles | `sanremes-bluebubbles` | 完整工具，包括终端 |
| QQBot | `sanremes-qqbot` | 完整工具，包括终端 |
| Yuanbao | `sanremes-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `sanremes-teams` | 完整工具，包括终端 |
| API Server | `sanremes-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `sanremes-webhook` | 完整工具，包括终端 |
...
1. **网关日志**（`~/.sanremes/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
