...
通过回调/webhook 模式，将 SanRemes 作为企业自建应用接入企业微信（WeCom）。

:::info WeCom Bot 与 WeCom 回调
SanRemes 支持两种企业微信集成模式：
...
3. SanRemes 解密消息，将其加入 agent 处理队列
...
（仅在通过 `sanremes gateway install` 注册 systemd/launchd 服务后，才使用 `sanremes gateway start`。）
