...
sidebar_position: 5
title: "内置技能目录"
description: "随 SanRemes Agent 附带的内置技能目录"
---

# 内置技能目录

SanRemes 附带一个大型内置技能库，安装时会复制到 `~/.sanremes/skills/`。下方每个技能均链接至专属页面，包含完整定义、配置和用法说明。

SanRemes 在执行 `sanremes update` 时也会同步内置技能，但同步清单会尊重本地删除和用户编辑。如果此处列出的某个技能在你的 `~/.sanremes/skills/` 目录树中缺失，它仍随 SanRemes 一同发布；可通过 `sanremes skills reset <name> --restore` 恢复。
...
| [`sanremes-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sanremes-agent) | 配置、扩展或贡献 SanRemes Agent。 | `autonomous-ai-agents/sanremes-agent` |
...
| [`teams-meeting-pipeline`](/user-guide/skills/bundled/productivity/productivity-teams-meeting-pipeline) | 通过 SanRemes CLI 操作 Teams 会议摘要流水线——汇总会议、检查流水线状态、重放任务、管理 Microsoft Graph 订阅。 | `productivity/teams-meeting-pipeline` |
...
| [`sanremes-agent-skill-authoring`](/user-guide/skills/bundled/software-development/software-development-sanremes-agent-skill-authoring) | 编写仓库内 SKILL.md：frontmatter、验证器、结构规范。 | `software-development/sanremes-agent-skill-authoring` |
...
| [`plan`](/user-guide/skills/bundled/software-development/software-development-plan) | 计划模式：将 Markdown 计划写入 `.sanremes/plans/`，不执行。 | `software-development/plan` |
