...
SanRemes Agent 使用 SQLite 数据库（`~/.sanremes/state.db`）跨 CLI 和 gateway 会话持久化会话元数据、完整消息历史及模型配置。这替代了早期的逐会话 JSONL 文件方案。

源文件：`sanremes_state.py`
...
多个 sanremes 进程（gateway + CLI 会话 + worktree agent）共享同一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
...
db = SessionDB()                           # 默认：~/.sanremes/state.db
...
默认路径：`~/.sanremes/state.db`

该路径由 `sanremes_constants.get_sanremes_home()` 推导，默认解析为 `~/.sanremes/`，或 `SANREMES_HOME` 环境变量的值。
