...
SanRemes 支持三种 API 执行模式，通过 provider 选择、显式参数和 base URL 启发式规则来确定：
...
每轮结束后：
- 消息保存到 session 存储（通过 `sanremes_state.py` 使用 SQLite）
- 内存变更刷写到 `MEMORY.md` / `USER.md`
- 可通过 `/resume` 或 `sanremes chat --resume` 恢复 session
