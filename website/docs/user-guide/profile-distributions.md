...
A **profile distribution** packages a complete SanRemes agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

SanRemes has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `sanremes profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `sanremes profile update` pulls new versions | Re-send the file |
...
| Made with | `sanremes profile install` / `update` | `/export` and `/import`, or `sanremes profile export` / `import` |
...
Before distributions, sharing a SanRemes agent meant sending someone:
...
```bash
sanremes profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `sanremes profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running SanRemes in 2026, that's already true.
...
:::caution
**SanRemes does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `sanremes profile install` or `sanremes profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
sanremes profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.sanremes/profiles/research-bot/SOUL.md
...
Create `~/.sanremes/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
sanremes_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Create `~/.sanremes/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.sanremes/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `sanremes profile update research-bot` will pull the latest.
...
```bash
sanremes profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.sanremes/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
sanremes profile install github.com/you/research-bot

# Full HTTPS
sanremes profile install https://github.com/you/research-bot.git

# SSH
sanremes profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
sanremes profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
sanremes profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
sanremes profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
sanremes profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
sanremes profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this SanRemes distribution.
...
```bash
cp ~/.sanremes/profiles/research-bot/.env.EXAMPLE ~/.sanremes/profiles/research-bot/.env
...
```bash
sanremes profile info research-bot
```
...
`sanremes profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
sanremes profile update research-bot
```
...
```bash
sanremes profile delete research-bot
```
...
Path:    ~/.sanremes/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `sanremes profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.sanremes/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
sanremes profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `sanremes profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.sanremes/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/sanremes-polymarket-trader.git
git push -u origin main --tags

# Anyone
sanremes profile install github.com/you/sanremes-polymarket-trader --alias
```
...
You built SanRemes-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
sanremes profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
sanremes profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `sanremes profile info <name>` and hold off on updates until you're ready.
:::
...
sanremes profile info research-bot | grep Version
...
```yaml
# ~/.sanremes/profiles/research-bot/local/my-overrides.yaml
...
sanremes profile delete research-bot --yes
sanremes profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
sanremes profile update research-bot --force-config --yes
```
...
sanremes profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.sanremes/profiles/forked-research-bot/
...
sanremes profile install ~/.sanremes/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
sanremes profile delete research-bot-test --yes
sanremes profile install ~/.sanremes/profiles/research-bot --name research-bot-test
```
...
```bash
sanremes profile export research-bot
sanremes profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
sanremes profile import ./research-bot.tar.gz
sanremes profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `sanremes -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.sanremes`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.sanremes`) is exported through an allow-list — only known SanRemes artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.sanremes/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `sanremes -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `sanremes profile` — not a parallel command tree.
...
- Reserved profile names (`sanremes`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`sanremes profile export` / `import`](../reference/profile-commands.md#sanremes-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with SanRemes](../guides/use-soul-with-sanremes.md) — authoring personalities
