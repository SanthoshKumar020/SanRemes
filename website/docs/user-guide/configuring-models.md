...
SanRemes uses two kinds of model slots:
...
[Nous Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `sanremes setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `sanremes portal info`.

- Portal subscribers also get **10% off token-billed providers**.
:::

:::note `model:` schema — empty string vs. mapping
On a brand-new install the bundled default config has `model: ""` (an empty string sentinel meaning "not configured yet"). The first time you run `sanremes setup` or `sanremes model`, that key is upgraded in-place to a mapping with `provider`, `default`, `base_url`, and `api_mode` sub-keys — the shape shown throughout this page and in [`profiles.md`](./profiles.md) / [`configuration.md`](./configuration.md). If you ever see an empty string in `config.yaml`, run `sanremes model` (or click **Change** in the dashboard) and SanRemes will write the dict form for you.
:::
...
- **Right** — the curated model list for the selected provider. These are the agentic models SanRemes recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and SanRemes writes it to `~/.sanremes/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.

### Mid-session switches and context warnings

When you switch models **inside an active session** (Herm TUI model picker, `sanremes` CLI, or `/model` on Telegram/Discord), SanRemes estimates whether your **next message** will run **preflight context compression** against the new model's window. If the session is already near or above that model's compression threshold (see [Context Compression](./configuration.md#context-compression)), the switch reply includes a warning — the same `warning_message` path used for expensive-model notices. The switch still applies immediately; compression runs on the **first user message after the switch**, before the model answers.
...
```bash
sanremes config set security.allow_data_training_tiers_noninteractive true
```

SanRemes still prints the full data-policy warning and the acknowledgement key on every unattended startup, so worker logs retain an audit trail. This setting does not approve expensive-model or provider-routing warnings, and it does not replace the interactive confirmation prompt. Revoke it with `sanremes config unset security.allow_data_training_tiers_noninteractive`.
...
Every auxiliary task defaults to `auto` — meaning SanRemes tries your main model for that job too. If that route is unavailable or hits a capacity-style failure, `auto` follows any task-specific `auxiliary.<task>.fallback_chain`, then the main `fallback_providers` / `fallback_model` chain, then SanRemes' built-in auxiliary discovery chain. Override a specific task when you want a cheaper or faster model for a side-job.

### Common override patterns

| Task | When to override |
|---|---|
| **Title Gen** | When title latency or cost matters more than matching the main model. Pin a known-good flash model, or set `auxiliary.title_generation.prefer_fast_model: true` to let SanRemes choose the provider's fast tier. |
...
| **Skills Hub** | `sanremes skills search` uses this. Usually fine at `auto`. |
| **MCP** | MCP tool routing. Usually fine at `auto`. |
| **Triage Specifier** | Routes the Kanban triage specifier (`sanremes kanban specify`) that expands a rough one-liner into a concrete spec. A cheap, capable model works well. |
...
| **Profile Describer** | Routes profile-description generation (`sanremes profile describe --auto` / the dashboard auto-generate button). Short, cheap call. |
...
When you save via the dashboard, SanRemes writes to `~/.sanremes/config.yaml`:
...
`provider: auto` with `model: ''` tells SanRemes to use the main model for that task, while still honoring fallback policy if the main route cannot serve the auxiliary call.
...
Provider entries (`providers.<name>` in the `providers:` dict, or items in the legacy `custom_providers` list) accept two knobs that shape how SanRemes talks to the endpoint:
...
Header values routinely carry credentials — SanRemes never logs them. `extra_headers` applies to OpenAI-compatible routes; the `anthropic_messages` and `bedrock_converse` API modes do not use it.
...
With discovery off, the model picker (`sanremes model`, `/model`) shows the configured list instead of a live probe.
...
SanRemes matches this declaration to the exact provider route and runtime model
...
omitted, SanRemes keeps its normal provider and model capability detection.

:::note Legacy format
Older configs used a top-level `custom_providers:` list (with `base_url` instead of `api`). It still works and is auto-migrated to the `providers:` dict on `sanremes update` (config v12).
:::

## When does it take effect?

- **CLI** (`sanremes chat`): next `sanremes chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`sanremes gateway restart`) if you want to force all sessions to pick up the change.
...
SanRemes lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `sanremes setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but SanRemes switched providers on me
...
Inside any `sanremes chat` session:
...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `sanremes config set` writes scalars and now also parses inline list/mapping literals, though this short alias form still can't carry a custom `base_url`:

```bash
sanremes config set model.aliases.fav anthropic/claude-opus-4.6
sanremes config set model.aliases.grok x-ai/grok-4
```

> `sanremes config set` also accepts inline **list/mapping literals** (JSON/YAML flow style). Quote them so your shell passes them through intact:
>
> ```bash
> sanremes config set platform_toolsets.line '["clarify", "file", "web"]'
> sanremes config set display.tool_progress_overrides '{"terminal": "off"}'
> ```

Both paths feed the same loader (`sanremes_cli/model_switch.py`). Entries declared in `model_aliases:` take precedence over `model.aliases:` entries with the same name.
...
### `sanremes model` subcommand

```bash
sanremes model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`sanremes model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.default` in `~/.sanremes/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `sanremes config get model --json` and `sanremes status`.

### Direct config edit

Edit `~/.sanremes/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
curl -H "X-SanRemes-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-SanRemes-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-SanRemes-Session-Token: $TOKEN" \
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__SANREMES_SESSION_TOKEN__`) if you're scripting against a running dashboard.
