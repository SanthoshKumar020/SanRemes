...
SanRemes Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, SanRemes checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.sanremes/config.yaml`:
...
| `timeout` | `300` | Seconds SanRemes waits for an approval reply before timing out. |
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `SANREMES_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
1. **CLI flag**: Start a session with `sanremes --yolo` or `sanremes chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `SANREMES_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `SANREMES_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, SanRemes shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that SanRemes refuses to run them **regardless** of:
...
Configure the timeout in `~/.sanremes/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.sanremes/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.sanremes/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` sanremes/gateway | Self-termination prevention |
...
The `SANREMES_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.sanremes/config.yaml`:
...
:::tip
Use `sanremes config edit` to review or remove patterns from your permanent allowlist.
:::

### Mining Approval History (`sanremes approvals suggest`)
...
```bash
sanremes approvals suggest            # dry run — prints a numbered proposal
sanremes approvals suggest --apply 1,3  # merge picks into command_allowlist
sanremes approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.sanremes/state.db`) for
...
Before `write_file` or `patch` touches disk, SanRemes checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `SANREMES_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/` (keys, `authorized_keys`), `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| SanRemes credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under SANREMES_HOME (active profile and global root) |
| Project secret files | `.env`, `.env.local`, `.env.production`, `.envrc` anywhere on disk |

Sensitive paths inside the safe root are still blocked — pointing `SANREMES_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

Safe-root violations return `Write denied: '…' is outside SANREMES_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`
...
### SANREMES_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/NousResearch/sanremes-agent) (`SANREMES_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.sanremes/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.sanremes/cron/jobs.json`, profile skills, or other SanRemes state outside that prefix

To allow both a workspace and SanRemes home:

```bash
export SANREMES_WRITE_SAFE_ROOT=/path/to/project:/home/you/.sanremes
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [SANREMES_WRITE_SAFE_ROOT](../reference/environment-variables.md#sanremes_write_safe_root).

### Cron and other SanRemes state

Do not ask the agent to `patch` `~/.sanremes/cron/jobs.json` directly. Use the `cronjob` tool, [`sanremes cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other SanRemes control files when write safety blocks direct edits.
...
When running the messaging gateway, SanRemes controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.sanremes/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.sanremes/.env to allow open access,
...
For more flexible authorization, SanRemes includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `sanremes pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.sanremes/config.yaml`:
...
sanremes pairing list

# Approve a pairing code
sanremes pairing approve telegram ABC12DEF

# Revoke a user's access
sanremes pairing revoke telegram 123456789

# Clear all pending codes
sanremes pairing clear-pending
```

:::tip Docker users: run pairing commands as the `sanremes` user
The official Docker image runs the gateway as the unprivileged `sanremes` user
...
Always pass `-u sanremes`:

```bash
docker exec -u sanremes sanremes-agent sanremes pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/NousResearch/sanremes-agent/issues/10270
:::

**Storage:** Pairing data is stored in `~/.sanremes/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, SanRemes applies strict security hardening to every container.
...
Container resources are configurable in `~/.sanremes/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.sanremes/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `SANREMES_HOME`. Skills declare these in frontmatter:
...
When loaded, SanRemes checks if these files exist in the active profile's `SANREMES_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit SanRemes infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- SanRemes infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
SanRemes integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run SanRemes under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.sanremes/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.sanremes/logs/` for unauthorized access attempts
10. **Keep updated** — run `sanremes update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.sanremes/.env`:
...
TERMINAL_SSH_USER=sanremes
TERMINAL_SSH_KEY=~/.ssh/sanremes_agent_key
```
...
SanRemes ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `sanremes_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `sanremes doctor` for the full remediation.
- **`sanremes doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
sanremes doctor --ack <advisory-id>
```
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. SanRemes installs these **lazily** on first use rather than eagerly under `sanremes-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `sanremes tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `sanremes tools`.
