...
`~/.sanremes/config.yaml`, `~/.sanremes/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/sanremes`:

```text
/etc/sanremes/
├── config.yaml     # managed config layer (wins over ~/.sanremes/config.yaml)
└── .env            # managed env layer (wins over ~/.sanremes/.env + shell)
```
...
The location can be relocated with the `SANREMES_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SANREMES_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by SanRemes.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SANREMES_MANAGED_DIR=/opt/org/sanremes-policy
```

:::warning
A user who can set `SANREMES_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `sanremes doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/sanremes/config.yaml` (managed) | `/etc/sanremes/.env` (managed) |
| 2 | `~/.sanremes/config.yaml` (user) | `~/.sanremes/.env` (user) |
...
```bash
sanremes config        # shows a header naming the managed source + the pinned keys
sanremes doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, SanRemes refuses and names the source:

```bash
$ sanremes config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/sanremes/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `sanremes config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/sanremes

# Pin some config values for every user on this machine
sudo tee /etc/sanremes/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/sanremes/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/sanremes
sudo chmod 0644 /etc/sanremes/config.yaml /etc/sanremes/.env
```

Changes take effect on the next SanRemes start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`sanremes doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs SanRemes as `root`), managed scope is advisory.
