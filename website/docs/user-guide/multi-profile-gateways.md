...
If you only run one SanRemes agent, you don't need this page — see
...
[Connecting Desktop to Many SanRemes Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more SanRemes agents that should all
...
(`ai.sanremes.gateway-<name>.plist`) or systemd user service
(`sanremes-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
sanremes profile create coder
sanremes profile create personal-bot
sanremes profile create research
...
```bash
sanremes config set gateway.multiplex_profiles true
sanremes gateway restart
```

Equivalently, in the default profile's `~/.sanremes/config.yaml`:
...
You do **not** run `sanremes gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `sanremes gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.sanremes/profiles/coder/.env`; the default listener key is rejected.
...
home). `sanremes status` reports the multiplexer and the profiles it serves;
`sanremes status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/sanremes-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: sanremes-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    sanremes gateway "$action"
  else
    sanremes -p "$profile" gateway "$action"
  fi
}
...
```bash
sanremes-gateways start      # start every configured profile
sanremes-gateways stop       # stop every configured profile
sanremes-gateways restart    # restart all
sanremes-gateways status     # status across all
sanremes-gateways list       # delegates to `sanremes gateway list`
```

:::tip
The `default` profile is targeted with `sanremes gateway <action>` (no `-p`),
not `sanremes -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `sanremes -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.sanremes.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/sanremes-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.sanremes.gateway.plist` /
`sanremes-gateway.service`.
...
```bash
# Default profile
tail -f ~/.sanremes/logs/gateway.log
tail -f ~/.sanremes/logs/gateway.error.log

# Named profile
tail -f ~/.sanremes/profiles/<name>/logs/gateway.log
tail -f ~/.sanremes/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.sanremes/logs/gateway.log ~/.sanremes/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
sanremes logs -f                  # follow default profile
sanremes -p coder logs -f         # follow one profile
sanremes logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
sanremes profile list             # profiles + model + gateway state
sanremes-gateways status          # full status across every profile
launchctl list | grep sanremes    # macOS — PIDs and labels
systemctl --user list-units 'sanremes-gateway-*'   # Linux — units
```
...
```
~/.sanremes/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.sanremes/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
sanremes config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.sanremes/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=sanremes --why="gateways running" \
  sleep infinity &
...
`sanremes-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.sanremes/.env ~/.sanremes/profiles/*/.env
```

## Updating the code

`sanremes update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `sanremes gateway start` after a previous `sanremes gateway stop`. The
...
```bash
ps -ef | grep "sanremes_cli.*-p <profile>"
cat ~/.sanremes/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.sanremes.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.sanremes.gateway-<profile>.plist

# Linux
systemctl --user restart sanremes-gateway-<profile>.service
```

### Health check

```bash
sanremes doctor                  # default profile
sanremes -p <profile> doctor     # one profile
```
