...
# Connecting Desktop to Many SanRemes Instances

Register every SanRemes backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and SanRemes Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another SanRemes gateway…"**) deep-links straight to
...
list of SanRemes gateways. Its intro says it plainly: *"Manage this device and
every SanRemes gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The SanRemes runtime managed by this app." | automatic |
| **Remote gateway** | "A SanRemes gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A SanRemes install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **SanRemes Cloud** | "A hosted instance discovered through your SanRemes Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the SanRemes Cloud sign-in/discovery flow at
  the top of the Gateways page — the **SanRemes Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **SanRemes Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **SanRemes Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `sanremes serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated SanRemes agents
...
connection is registered) dispatches `sanremes update` to every eligible
...
- **SanRemes Cloud** instances are skipped with a *"Managed by SanRemes Cloud"*
...
panel, ⌘K **Update SanRemes**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `sanremes serve` is running on the remote host, the
...
- **"Update SanRemes Desktop to chat with agents on other connections"** — the
