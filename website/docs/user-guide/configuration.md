---
sidebar_position: 2
title: "SanRemes Agent Configuration"
description: "Configure SanRemes Agent — config.yaml, providers, models, API keys, and more"
---

# SanRemes Agent Configuration

All settings are stored in the `~/.sanremes/` directory for easy access.

:::tip Easiest path to a working `config.yaml`
Run `sanremes setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
sanremes config              # View current configuration
sanremes config edit         # Open config.yaml in your editor
sanremes config get KEY      # Print a resolved value
sanremes config set KEY VAL  # Set a specific value
sanremes config unset KEY    # Remove a user-set value
sanremes config check        # Check for missing options (after updates)
sanremes config migrate      # Interactively add missing options

# Examples:
sanremes config get model
sanremes config set model anthropic/claude-opus-4
sanremes config set terminal.backend docker
sanremes config unset terminal.backend
sanremes config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `sanremes config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `sanremes chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.sanremes/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.sanremes/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Long-running SanRemes server surfaces (including the gateway and
`sanremes serve --isolated`) apply the configured `RLIMIT_NOFILE` soft limit
...
The default is `4096`. SanRemes clamps the target to the operating system's hard
...
The `database:` section controls how SanRemes opens its SQLite state database
...
database:
  ...
  # live-downgraded — SanRemes keeps WAL and logs an error telling you the
...
SanRemes also warns (once per process per database) when an existing
...
You can set `providers.<id>.request_timeout_seconds` for a provider-wide request timeout, plus `providers.<id>.models.<model>.timeout_seconds` for a model-specific override. Applies to the primary turn client on every transport (OpenAI-wire, native Anthropic, Anthropic-compatible), the fallback chain, rebuilds after credential rotation, and (for OpenAI-wire) the per-request timeout kwarg — so the configured value wins over the legacy `SANREMES_API_TIMEOUT` env var.

You can also set `providers.<id>.stale_timeout_seconds` for the non-streaming stale-call detector, plus `providers.<id>.models.<model>.stale_timeout_seconds` for a model-specific override. This wins over the legacy `SANREMES_API_CALL_STALE_TIMEOUT` env var.

Leaving these unset keeps the legacy defaults (`SANREMES_API_TIMEOUT=1800`s, `SANREMES_API_CALL_STALE_TIMEOUT=90`s, native Anthropic 900s). The non-streaming stale detector is auto-disabled for local endpoints when left implicit and can scale upward for very large contexts. Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/NousResearch/sanremes-agent/blob/main/cli-config.yaml.example).

## Update Behavior

`sanremes update` settings live under `updates` in `config.yaml`:

```yaml
updates:
  pre_update_backup: quick       # quick (state snapshot, default) | full (snapshot + SANREMES_HOME zip) | off
...
`terminal.temp_dir` controls where SanRemes puts session temp artifacts on the
local backend — background-process logs/pid/exit files, code-execution
sandboxes, and spilled tool results. When it's empty (the default), SanRemes
honors an explicit `TMPDIR`/`TMP`/`TEMP` from the environment and otherwise
uses a managed directory on real storage at `~/.sanremes/cache/terminal`
instead of `/tmp` — on many distros (Arch-based setups in particular) `/tmp`
is a small RAM-backed tmpfs that SanRemes session artifacts can fill under
...
`terminal.font_family` controls the embedded terminal in SanRemes Desktop. It accepts either one locally installed family name (for example, `MesloLGS NF`) or a CSS font stack. SanRemes appends its bundled JetBrains Mono stack as a fallback, and an empty value keeps the default. You can edit the same profile-scoped setting in **Settings → Appearance → Terminal Font**; no Google Fonts download or system-font permission is required.

For cloud sandboxes such as Modal, Daytona, and Vercel Sandbox, `container_persistent: true` means SanRemes will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
find the credentials and config they already use in your normal shell. SanRemes
state is still profile-scoped through `SANREMES_HOME`; `HOME` is not how profiles
select config, memory, sessions, or skills.

SanRemes does **not** change your system-wide `HOME`, your shell startup files, or
the operating system account home. This setting only controls the environment
passed to subprocesses that SanRemes launches through tools such as `terminal`,
...
|---|---|---|---|
| `auto` | Keep the real OS-user `HOME` | Use `{SANREMES_HOME}/home` | Recommended default. Host CLIs keep working; container state persists. |
...
| `profile` | Use `{SANREMES_HOME}/home` when it exists | Use `{SANREMES_HOME}/home` when it exists | Strict per-profile CLI config isolation, but normal `~/.ssh`, `~/.gitconfig`, `~/.azure`, `~/.config/gh`, Claude/Codex auth, npm state, etc. will not be visible unless you initialize or link them inside the profile home. |
...
In that mode tool subprocesses use `{SANREMES_HOME}/home` as `HOME`. SanRemes also
sets `SANREMES_REAL_HOME` so scripts can still locate the actual user home when
they need it. Container backends keep using `{SANREMES_HOME}/home` in `auto` mode
because that directory lives on the persistent SanRemes data volume.

Scripts that need to distinguish profile state from the real user home should
prefer `SANREMES_HOME` for SanRemes data and `SANREMES_REAL_HOME` for the account home:

```python
from pathlib import Path
import os

sanremes_home = Path(os.environ["SANREMES_HOME"])
real_home = Path(os.environ.get("SANREMES_REAL_HOME", os.environ["HOME"]))
```

:::warning
The agent has the same filesystem access as your user account. Use `sanremes tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Single persistent container, shared across SanRemes processes.** SanRemes starts ONE long-lived container on first use and routes every terminal, file, and `execute_code` call through `docker exec` into that same container — across sessions, `/new`, `/reset`, and `delegate_task` subagents. Working-directory changes, installed packages, files in `/workspace`, and **background processes** all carry over from one tool call to the next, and from one SanRemes process to the next. When you close a TUI session, run `/quit`, or start a new `sanremes` invocation, the container keeps running and the next SanRemes process reuses it via a labeled lookup. See **Container lifecycle** below for the exact teardown rules.
...
terminal:
  ...
  docker_persist_across_processes: true   # Reuse container across SanRemes restarts
...
**`docker_env`** vs **`docker_forward_env`**: the former injects literal `KEY=value` pairs you specify in the config (the values live in your `config.yaml` or are passed as a JSON dict via `TERMINAL_DOCKER_ENV='{"DEBUG":"1"}'`). The latter forwards values from your shell or `~/.sanremes/.env`, so the actual secret never appears in the config file. Use `docker_forward_env` for tokens and `docker_env` for static knobs the container needs.

**`terminal.docker_extra_args`** (also overridable via `TERMINAL_DOCKER_EXTRA_ARGS='["--gpus=all"]'`) lets you pass arbitrary `docker run` flags that SanRemes doesn't surface as first-class keys — `--gpus`, `--network`, `--add-host`, alternative `--security-opt` overrides, etc. Each entry must be a string; the list is appended last to the assembled `docker run` invocation so it can override SanRemes' defaults if needed. Use sparingly — flags that conflict with the sandbox hardening (capability drops, `--user`, the workspace bind mount) will silently weaken isolation.

**`terminal.docker_network`** (default `true`; env: `TERMINAL_DOCKER_NETWORK`) — set to `false` to run the sandbox container with `--network=none`, cutting off all network egress from agent commands. This applies to the execution container used by `terminal`, `execute_code`, and the file tools. Because containers persist across SanRemes processes, flipping this to `false` while an older networked container exists will remove that container and start a fresh air-gapped one (a warning is logged); background processes running inside it are lost. Prefer this key over passing `--network=none` through `docker_extra_args`.

**Requirements:** Docker Desktop or Docker Engine installed and running. SanRemes probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle). Podman is supported out of the box: set `SANREMES_DOCKER_BINARY=podman` (or the full path) to force it when both are installed.

#### Container lifecycle

Every SanRemes-managed container is tagged with three labels so subsequent processes (and the orphan reaper) can identify it:

- `sanremes-agent=1` — marks it as SanRemes-managed
- `sanremes-task-id=<sanitized task_id>` — keys the per-task reuse probe
- `sanremes-profile=<sanitized profile name>` — scopes reuse and reaping to the active SanRemes profile by default; when `docker_shared_container_key` is set, its sanitized value is used instead

On startup, SanRemes runs `docker ps --filter label=sanremes-task-id=<id> --filter label=sanremes-profile=<identity>` and **attaches to the existing container** when it finds one. The identity is the active profile unless `docker_shared_container_key` explicitly opts trusted profiles into a common value. If the container is `exited` (e.g. after a Docker daemon restart), it's `docker start`'d and reused — filesystem state and any installed packages survive, but in-container background processes do not.

When a SanRemes process exits — `/quit`, closing a TUI session, gateway shutdown, even SIGKILL — the cleanup path is a **no-op for the container in default mode**. The container keeps running. The next SanRemes process attaches to it in milliseconds via the label probe. This is the behavior the "one long-lived container shared across sessions" contract requires: it's the only way background processes (npm watchers, dev servers, long-running pytest) survive across sessions.
...
| Orphan reaper at next startup | Sweeps **Exited** sanremes-labeled containers older than `2 × lifetime_seconds` (default 600s = 10 min), scoped to the current profile. **Running containers are never touched** — sibling-process safety. Set `docker_orphan_reaper: false` to disable. |
...
- **Switching profiles** isolates containers from each other — a container labeled `sanremes-profile=work` is invisible to a SanRemes process running under `sanremes-profile=research`. The orphan reaper is profile-scoped too, so cross-profile containers don't get reaped accidentally, but they also won't get cleaned up automatically until you start SanRemes again under their original profile.
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.sanremes/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
| `SANREMES_DOCKER_BINARY` | _none_ | Force a specific docker/podman binary path |
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.sanremes/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.sanremes/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `sanremes-{task_id}`.

**Disk limit:** Daytona enforces a 10 GiB maximum. Requests above this are capped with a warning.

### Vercel Sandbox Backend

Runs commands in a [Vercel Sandbox](https://vercel.com/docs/vercel-sandbox) cloud microVM. SanRemes uses the normal terminal and file tool surfaces; there are no Vercel-specific model-facing tools.
...
```bash
pip install 'sanremes-agent[vercel]'
```

**Required authentication:** Configure access-token auth with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This is the supported setup for deployments and normal long-running SanRemes processes on Render, Railway, Docker, and similar hosts.

For one-off local development, SanRemes also accepts short-lived Vercel OIDC tokens:

```bash
VERCEL_OIDC_TOKEN="$(vc project token <project-name>)" sanremes chat
```

From a linked Vercel project directory, you can omit the project name:

```bash
VERCEL_OIDC_TOKEN="$(vc project token)" sanremes chat
```

OIDC tokens are short-lived and should not be used as the documented deployment path.

**Runtime:** `terminal.vercel_runtime` supports `node24`, `node22`, and `python3.13`. If unset, SanRemes defaults to `node24`.

**Persistence:** When `container_persistent: true`, SanRemes snapshots the sandbox filesystem during cleanup and restores a later sandbox for the same task from that snapshot. Snapshot contents can include SanRemes-synced credentials, skills, and cache files that were copied into the sandbox. This preserves filesystem state only; it does not preserve live sandbox identity, PID space, shell state, or running background processes.

**Background commands:** `terminal(background=true)` uses SanRemes' generic non-local background process flow. You can spawn, poll, wait, view logs, and kill processes through the normal process tool while the sandbox is alive. SanRemes does not provide native Vercel detached-process recovery after cleanup or restart.

**Disk sizing:** Vercel Sandbox does not currently support SanRemes' `container_disk` resource knob. Leave `container_disk` unset or at the shared default `51200`; non-default values fail diagnostics and backend creation instead of being silently ignored.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/sanremes-agent` (HPC convention) → `~/.sanremes/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `sanremes config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. SanRemes logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `sanremes doctor` to check.
...
- This covers SanRemes state (`~/.sanremes/`), **not** arbitrary working-tree files inside the sandbox — have the agent copy important artifacts out explicitly (e.g. `scp`, `modal volume put`) before the sandbox is destroyed.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.sanremes/cache/documents:/output" # Gateway-visible exports
```
...
`/home/user/.sanremes/cache/documents:/output`.

- Write files inside Docker to `/output/...`
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.sanremes/cache/documents/report.txt`
...
SanRemes resolves each listed variable from your current shell first, then falls back to `~/.sanremes/.env` if it was saved with `sanremes config set`.
...
When enabled, SanRemes appends `--user $(id -u):$(id -g)` to the `docker run` command so files written into bind-mounted directories (`/workspace`, `/root`, anything in `docker_volumes`) are owned by your host user, not root. The trade-off: the container can no longer `apt install` or write to root-owned paths like `/root/.npm` — use a base image whose `HOME` is owned by a non-root user (or add your required tooling at image build time) if you need both.
...
Docker sandboxes stay isolated by default. SanRemes does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch SanRemes from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched SanRemes from
...
```bash
sanremes config set terminal.persistent_shell false
```
...
- `sanremes config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `sanremes config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
sanremes config set skills.config.myplugin.path ~/myplugin-data
```
...
When the agent uses `skill_manage` to create, edit, patch, or delete a skill, SanRemes can optionally scan the new/updated content for dangerous keyword patterns (credential harvesting, obvious prompt injection, exfil instructions). The scanner is **off by default** — real agent workflows that legitimately touch `~/.ssh/` or mention `$OPENAI_API_KEY` were tripping the heuristic too often. Turn it back on if you want the scanner to prompt you before the agent's skill writes land:
...
Controls how much content SanRemes loads from each automatic context file before applying head/tail truncation. This applies to files injected into the system prompt such as `SOUL.md`, `.sanremes.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules`. It does **not** affect the `read_file` tool.
...
Three related caps control how much raw output a tool can return before SanRemes truncates it:
...
- **`max_bytes`** — When a `terminal` command produces more than this many characters of combined stdout/stderr, SanRemes keeps the first 40% and last 60% and inserts a `[OUTPUT TRUNCATED]` notice between them. Default `50000` (≈12-15K tokens across typical tokenisers).
...
SanRemes also flags **provider-side elision**: when an MCP or web tool result embeds its own truncation markers (`...N more items`, `"has_more": true`, "saved to sandbox" notes), a one-line notice is appended to the result warning that the visible data is incomplete and should be paged/fetched before treating any enumeration as complete.
...
`sanremes tools`), so a toolset listed here is always removed — even if a
...
```yaml
worktree: true    # Always create a worktree (same as sanremes -w)
...
SanRemes automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
If another turn still holds the session lease when this budget expires, SanRemes
fails closed: it does not load the transcript or run the model for the waiting
message. The user receives a rejection notice and must resend. SanRemes does not
...
- **Needs-attention escalation.** A platform continuously in the retry queue past `agent.reconnect_attention_after` (default `7200` seconds = 2 hours, `0` disables) gets `needs_attention: true` and a `retrying_since` timestamp in gateway runtime status (`sanremes status`), plus a WARNING log. Retries continue unchanged — this is a signal, not a circuit breaker. The flag clears on successful reconnect.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `sanremes plugins` → Provider Plugins → Context Engine.

See [Memory Providers](/user-guide/features/memory-providers) for the analogous single-select system for memory plugins.

## Iteration Budget

When the agent is working on a complex task with many tool calls, it can burn through its iteration budget (default: 500 turns). SanRemes does **not** inject mid-task pressure warnings — earlier builds warned the model at 70%/90% budget, which caused models to abandon complex tasks prematurely and was removed in April 2026.

Instead, when the budget is actually exhausted (500/500), SanRemes injects one message asking the model to wrap up and allows a single **grace call** so it can deliver a final response. If that grace call still doesn't produce text, the agent is asked to summarise what it accomplished.
...
`verify_on_stop` accepts `true` (on everywhere), `false` (off — the default), or `"auto"` (legacy surface-aware behavior: on for interactive coding surfaces — CLI, TUI, desktop — and programmatic callers; off for messaging surfaces like Telegram/Discord where the verification narrative reads as chat noise). Off is the default everywhere: fresh installs ship `false` and the config migration turned it off on existing installs, so enabling it is an explicit opt-in. The `SANREMES_VERIFY_ON_STOP` env var overrides the config value when set.
...
When a standing goal is active, SanRemes judges whether each assistant response satisfies it. If not, it feeds a continuation prompt back into the same session and keeps working until the goal is done, the turn budget is exhausted, or the user pauses/clears it. The turn budget is the real backstop — judge failures fail **open** (continue) so a flaky judge never wedges progress.

```yaml
goals:
  max_turns: 20   # Max continuation turns before SanRemes auto-pauses the goal (default: 20)
```

`max_turns` caps how many continuation turns a goal can drive before SanRemes auto-pauses it and asks the user to `/goal resume`. It protects against judge false negatives (goal actually done but judge says continue) and unbounded model spend on fuzzy or unachievable goals. See [Goals](/user-guide/features/goals) for the full feature.

### API Timeouts

SanRemes has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.

| Timeout | Default | Local providers | Config / env |
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `SANREMES_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Raised to a 900s ceiling (`agent.local_stream_stale_timeout`) | `SANREMES_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 90s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `SANREMES_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `SANREMES_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so SanRemes raises this to 30 minutes when it detects a local endpoint. If you explicitly set `SANREMES_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.

The **stale stream detection** kills connections that receive SSE keep-alive pings but no actual content. For local providers (which don't send keep-alive pings during prefill) the default is raised to a finite 900-second ceiling instead of the 180s base — configurable via `agent.local_stream_stale_timeout` or the `SANREMES_LOCAL_STREAM_STALE_TIMEOUT` env var.

The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default SanRemes disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `SANREMES_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
SanRemes turns on cross-session prompt caching automatically when the active provider supports it — no user config needed.

For Claude on **native Anthropic**, **OpenRouter**, and **Nous Portal**, SanRemes attaches `cache_control` breakpoints with the 1-hour TTL (`ttl: "1h"`) on the system prompt and skill blocks. The first send within a fresh hour pays full input rates; subsequent sends across any session within the same hour pull from the cache at the discounted cached-read rate. This means the system prompt, loaded skill content, and the early portion of any long-context include get reused across `sanremes` sessions and across forked subagents for the first hour.

The Qwen Cloud (Alibaba DashScope) upstream caps cache TTL at 5 minutes, so SanRemes uses the 5-minute breakpoint TTL there instead. Other Claude-via-third-party paths (AWS Bedrock, Azure Foundry) fall back to the provider's own caching defaults. xAI Grok uses a separate session-pinned conversation-id mechanism — see [xAI prompt caching](/integrations/providers#xai-grok--responses-api--prompt-caching).
...
The one explicit knob is the cache TTL tier SanRemes requests on Anthropic-style breakpoints:
...
Instead of hand-editing YAML, run `sanremes model` and pick **"Configure auxiliary models"** from the menu. You'll get an interactive per-task picker:
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="SanRemes Agent — Auxiliary Models Tutorial"
...
Every model slot in SanRemes — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, SanRemes ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, SanRemes uses that provider's built-in auth and base URL.
...
:::tip MiniMax OAuth
`minimax-oauth` logs in via browser OAuth (no API key needed). Run `sanremes model` and select **MiniMax (OAuth)** to authenticate. Auxiliary tasks use `MiniMax-M2.7-highspeed` automatically. See the [MiniMax OAuth guide](../guides/minimax-oauth.md).
:::

:::tip xAI Grok OAuth
`xai-oauth` logs in via browser OAuth for SuperGrok and X Premium+ subscribers (no API key needed). Run `sanremes model` and select **xAI Grok OAuth (SuperGrok / Premium+)** to authenticate. The same OAuth token is reused for every direct-to-xAI surface (chat, auxiliary tasks, TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md), and if SanRemes is on a remote host see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md).
:::
...
auxiliary:
  ...
  # Kanban triage specifier — `sanremes kanban specify <id>` (or the
...
Each auxiliary task can optionally define a `fallback_chain` — a list of provider/model entries that SanRemes tries when the primary auxiliary provider fails due to rate limits, connectivity issues, or payment restrictions:
...
When the primary auxiliary provider (`openrouter` / `openai/gpt-4o-mini`) returns a rate-limit, connection timeout, or payment-required error, SanRemes walks the `fallback_chain` in order. It skips entries whose provider matches the already-failed provider, and tries each remaining entry until one succeeds or the chain is exhausted. If all fallbacks fail, SanRemes falls back to the main agent model as a final safety net.
...
The shape mirrors what OpenRouter accepts in the chat completions request body. SanRemes forwards the entire `extra_body` verbatim, so any other OpenRouter request-body field documented at [openrouter.ai/docs](https://openrouter.ai/docs) works the same way.
...
Or via environment variable (in `~/.sanremes/.env`):
...
| `"nous"` | Force Nous Portal | `sanremes auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `sanremes model` → ChatGPT or Codex Subscription |
| `"minimax-oauth"` | Force MiniMax OAuth (browser login, no API key). Uses MiniMax-M2.7-highspeed for auxiliary tasks. | `sanremes model` → MiniMax (OAuth) |
| `"xai-oauth"` | Force xAI Grok OAuth (browser login for SuperGrok or X Premium+ subscribers, no API key). Same OAuth token covers chat, TTS, image, video, and transcription. | `sanremes model` → xAI Grok OAuth (SuperGrok / Premium+) |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `sanremes model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, SanRemes uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
```
Run `sanremes model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](../guides/minimax-oauth.md) for the full walkthrough.
...
`provider: "main"` uses whatever provider SanRemes uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `sanremes config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
field — OpenRouter ignores it for them. SanRemes transparently routes your
...
For other models routed through OpenRouter, SanRemes reads the live model
...
automatically without waiting for a SanRemes update; when the catalog is
unreachable or a model isn't listed, SanRemes falls back to its built-in
...
:::note
There is no `sanremes config set` support for `reasoning_overrides` keys — edit the YAML file directly. This is because model names often contain dots (e.g. `claude-opus-4.5`), which conflict with the CLI's dotted-key syntax.
:::
...
SanRemes detects when the agent is stuck in an unproductive tool-calling loop — the same tool call failing repeatedly, the same tool failing over and over, or an idempotent call returning the same result with no progress. By default it injects a **warning** into the tool result so the model self-corrects; it does not hard-stop, since a person watching the CLI/TUI can intervene.
...
- `+X -Y` line deltas appear only when the tool result already reports a diff (currently `patch`). SanRemes never shells out to git to compute them, so a `write_file` edit is counted without a delta.
...
When `display.file_mutation_verifier` is `true` (default), SanRemes appends a one-line advisory to the assistant's final response whenever a `write_file` or `patch` call failed during the turn and was never superseded by a successful write to the same path. This catches the "batch of parallel patches, half silently fail, model summarises success" class of over-claim without requiring you to manually run `git status` after every edit.
...
Set `file_mutation_verifier: false` (or `SANREMES_FILE_MUTATION_VERIFIER=0`) to suppress the footer. The verifier only fires when real failures are outstanding at turn end — a model that retries a failed patch and succeeds within the same turn will not trigger it for that file.
...
- **Write denied** — path is on the credential denylist or outside `SANREMES_WRITE_SAFE_ROOT` (see [File write safety](./security.md#file-write-safety))
...
⚠️ File-mutation verifier: 2 file(s) were NOT modified this turn despite any wording above that may suggest otherwise. Run `git status` or `read_file` to confirm.
  • ~/.sanremes/cron/jobs.json — [patch] Write denied: '…' is outside SANREMES_WRITE_SAFE_ROOT (/path/to/project)
  • ~/.sanremes/scripts/monitor.py — [write_file] Write denied: '…' is outside SANREMES_WRITE_SAFE_ROOT (/path/to/project)
```

If writes to SanRemes state (cron jobs, skills, scripts under `~/.sanremes/`) are failing, check whether `SANREMES_WRITE_SAFE_ROOT` is set in your environment. For cron changes, use the `cronjob` tool or `sanremes cron edit` instead of patching `jobs.json` directly.
...
You can also set this per-session with the `SANREMES_LANGUAGE` env var, which overrides the config value.
...
When `display.runtime_footer.enabled: true`, SanRemes appends a small runtime-context footer to the **final** message of each gateway turn. The current footer can show the model, context-window percentage, and current working directory. Off by default; opt in per-gateway if your team wants every reply to include this provenance.
...
`interim_assistant_messages` is gateway-only. When enabled, SanRemes sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
send `originator: sanremes-agent` and `User-Agent: SanRemesAgent/<version>`.
...
stt:
  ...
  # prompt: "SanRemes, Teknium, Nous Research, kanban"   # Static vocabulary hint (see below)
  local:
...
Language resolution is the same for **every** STT provider (local, groq, openai, mistral, xai, elevenlabs, deepinfra, command providers, and plugins): `stt.<provider>.language` → `stt.language` → `SANREMES_LOCAL_STT_LANGUAGE` env var → provider auto-detect. **The default is `stt.language: "en"`** — Whisper auto-detection frequently misidentifies short or accented clips, which shows up as voice notes transcribed in the wrong language. Non-English speakers should set `stt.language` to their language code once (e.g. `"es"`, `"zh"`, `"uk"`); set it to `""` to restore auto-detection for multilingual use.
...
- `groq` uses Groq's Whisper-compatible endpoint and reads `GROQ_API_KEY`. Pass `stt.groq.language` (or the global `SANREMES_LOCAL_STT_LANGUAGE` env var) to skip auto-detection and reduce latency.
...
An explicitly selected `stt.provider` is honored strictly — if it's unavailable, transcription errors with guidance to run `sanremes tools` rather than switching providers. Only when no provider has ever been selected does SanRemes auto-detect in this order: `local` → `groq` → `openai`.
...
stt:
  provider: "local"
  prompt: "SanRemes, Teknium, Nous Research, kanban, Ollama"
```
...
When the cap is reached, SanRemes returns a direct limit message naming which
surfaces hold the slots. Existing active sessions keep their normal behavior.
Run `sanremes status` to see the current slot usage and every holder.

The canonical key is top-level `max_concurrent_sessions`. SanRemes also accepts
`gateway.max_concurrent_sessions` as a fallback, but the top-level key wins when
both are set.

The cap is enforced with a local runtime lease file and is best-effort: SanRemes
fails open if the registry cannot be read or locked so users are not stranded.
It is intended for a single host/profile runtime, not a shared `$SANREMES_HOME`
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want SanRemes to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. SanRemes still keys DMs by chat/DM ID as usual.
...
Control what SanRemes does when an unknown user sends a direct message:
...
- `pair` is the default for chat-style DM platforms. SanRemes denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  status:
    type: exec
    command: systemctl status sanremes-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.sanremes/sanremes-agent && git pull && uv pip install -e .
  gpu:
    type: exec
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (SanRemes's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
The `web_search` and `web_extract` tools support five backend providers. Configure the backend in `config.yaml` or via `sanremes tools`:
...
web:
  ...
  # Pin Exa/Parallel to a tier (set by the sanremes tools Free/Paid rows).
...
**Backend selection:** The runtime always uses the stored `web.backend` selection (set via `sanremes tools`; `nous` routes through the managed Tool Gateway). Only if no web backend has ever been selected is one auto-detected from available API keys: if only `SEARXNG_URL` is set, SearXNG is used; if only `EXA_API_KEY` is set, Exa; if only `TAVILY_API_KEY` is set, Tavily; if only `PARALLEL_API_KEY` is set, Parallel; if only `KEENABLE_API_KEY` is set, Keenable. With **no selection and no credentials at all**, requests rotate round-robin across the keyless free-tier ring (Exa / Parallel / Tavily / Firecrawl / Keenable) with automatic next-in-line failover on rate limits — see the [Web Search guide](/user-guide/features/web-search) for details. Once a selection exists, adding a key to `.env` does not change the route. Selecting Tavily, Firecrawl, or Keenable in `sanremes tools` also works without a key.
...
**Exa:** Set `EXA_API_KEY` in `~/.sanremes/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.sanremes/browser_recordings/
  # Optional CDP override — when set, SanRemes attaches directly to your own
  ...
  camofox:
    ...
    session_key: ""              # Optional session key sent when SanRemes creates a tab
...
security:
  website_blocklist:
    ...
    shared_files:                # Load additional rules from external files
      - "/etc/sanremes/blocked-sites.txt"
```
...
Control how SanRemes handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `SANREMES_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: sanremes chat --checkpoints). Default: false (opt-in).
...
SanRemes uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.sanremes/SOUL.md` or `$SANREMES_HOME/SOUL.md` |
| `.sanremes.md` / `SANREMES.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, SanRemes falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.sanremes.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- SanRemes automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`sanremes`)** | Current directory where you run the command |
| **Messaging gateway** | `terminal.cwd` from `~/.sanremes/config.yaml`; if unset, home directory `~` |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```yaml
# In ~/.sanremes/config.yaml:
terminal:
  cwd: /home/myuser/projects
```

`MESSAGING_CWD` and direct `TERMINAL_CWD` entries in `~/.sanremes/.env` are legacy compatibility fallbacks. New configurations should use `terminal.cwd`.
...
- `seen` — internal state. SanRemes latches each shown hint here so it never fires again; the profile-build offer is also recorded here once shown. Don't hand-edit it — wipe the whole `onboarding` section if you want to re-see all hints.
...
dashboard:
  ...
  public_url: ""              # Full public authority for OAuth redirect_uri (env: SANREMES_DASHBOARD_PUBLIC_URL)
...
- `oauth` / `basic_auth` / `drain_auth` — auth provider config read by the bundled dashboard-auth plugins. The drain secret itself is **not** set here; it's provisioned via the `SANREMES_DASHBOARD_DRAIN_SECRET` env var. See [Web Dashboard](/user-guide/features/web-dashboard) for full auth setup.
...
- `ws_orphan_reap_grace_s` — how long a WS-detached session waits before the orphan reaper collects it. Raise alongside the keepalive values if clients reconnect slowly. (`SANREMES_TUI_WS_ORPHAN_REAP_GRACE_S` remains as an internal override.)
- `startup_orphan_sweep` (default `true`) — the WS-orphan reap timer above is in-process, so a gateway restart (update, crash, systemd) before it fires leaves the session row open forever — phantom "active" work in `/resume` and dashboards. On every gateway boot — both the stdio TUI (`entry.main`) and the desktop/dashboard WebSocket sidecar (`handle_ws`) — rows with source `tui` / `desktop` / `subagent` whose start time **and** newest message are both older than the session TTL (`SANREMES_TUI_SESSION_TTL_S`, default 6 hours) are closed with `end_reason: startup_orphan_reap`. Messaging-platform sessions (Telegram, Discord, …) are never touched, live in-memory sessions (a client that already resumed) are excluded, and swept sessions remain resumable.
