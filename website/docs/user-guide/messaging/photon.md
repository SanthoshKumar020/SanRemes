...
Connect SanRemes to **iMessage** through [Photon][photon], a managed
...
your first iMessage from SanRemes — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, SanRemes runs it in a
...
sanremes photon setup --phone +15551234567
```
...
2. **Finds or creates** the `SanRemes Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.sanremes/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.sanremes/.env`
...
(device token, dashboard project id) lives in `~/.sanremes/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other SanRemes
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, SanRemes replies with a pairing code. Approve it with:

```bash
sanremes pairing approve photon <CODE>
```

Use `sanremes pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.sanremes/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.sanremes/.env`):
...
By default SanRemes responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `SanRemes` and
`@SanRemes agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and SanRemes will reply.

## Status & troubleshooting

```bash
sanremes photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line SanRemes uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run sanremes photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `sanremes photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `sanremes photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `sanremes photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** SanRemes sends images, voice
  ...
  media.
- **Native polls are supported.** SanRemes sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** SanRemes sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `sanremes send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<sanremes-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when SanRemes is set
...
| `PHOTON_MENTION_PATTERNS` | SanRemes wake words  | JSON list / comma / newline regex patterns for group mentions |
