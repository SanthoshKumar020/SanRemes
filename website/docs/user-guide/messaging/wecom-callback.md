...
Connect SanRemes to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
SanRemes supports two WeCom integration modes:
...
> Run `sanremes gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. SanRemes decrypts the message, queues it for the agent
...
(Use `sanremes gateway start` only after `sanremes gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in SanRemes and the token the
...
in `~/.sanremes/.env` values around `=` will also break signature checks. After
fixing, restart `sanremes gateway run`.
...
Check `sanremes gateway run` logs for the bound host/port. If the adapter bound to
