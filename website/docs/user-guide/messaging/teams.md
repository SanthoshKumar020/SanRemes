...
description: "Set up SanRemes Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect SanRemes Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
> Run `sanremes gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which SanRemes strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create sanremes-bot --allow-anonymous
devtunnel port create sanremes-bot -p 3978 --protocol http  # replace 3978 with TEAMS_PORT if changed
devtunnel host sanremes-bot
...
The public tunnel URL uses HTTPS, but SanRemes' local webhook listener uses plain HTTP. The tunnel terminates TLS and forwards HTTP to port `3978`; do not configure the local tunnel port as HTTPS.
...
**Docker** (must run from the directory that contains `docker-compose.yml` — usually your cloned `sanremes-agent` repo, not `~`):

```bash
cd /path/to/sanremes-agent
SANREMES_UID=$(id -u) SANREMES_GID=$(id -g) docker compose up -d gateway
```

**Native / systemd install** (typical `sanremes` one-liner installer under `~/.sanremes/sanremes-agent`):

```bash
sanremes gateway restart
# or foreground: sanremes gateway run
```

The Teams SDK is optional; when Teams is enabled, the gateway lazy-installs it into SanRemes' own venv on first start (do **not** use system `pip install` on Ubuntu 24.04 — that hits PEP 668 `externally-managed-environment`). To install manually into the SanRemes venv:

```bash
~/.sanremes/sanremes-agent/venv/bin/pip install microsoft-teams-apps aiohttp
...
# Docker:
docker logs -f sanremes
# Native:
sanremes gateway status -l
```
...
Alternatively, configure via `~/.sanremes/config.yaml`:
...
For a permanent server, terminate TLS at a reverse proxy and forward requests to the plain HTTP SanRemes listener, normally `http://127.0.0.1:3978`. Register the proxy's public HTTPS endpoint with Teams:
...
Make sure the public HTTPS endpoint is reachable from the internet and uses a valid TLS certificate. Teams rejects self-signed certificates. Keep the SanRemes listener behind the proxy; port `3978` does not serve HTTPS itself.
...
| Problem | Solution |
|---------|----------|
| `Can't find a suitable configuration file` from `docker compose` | You are not in the repo that has `docker-compose.yml`, or you are on a native install — use `sanremes gateway restart` instead, or `cd` into the clone first |
| `requirements not met` / `Teams SDK missing` / `No adapter available for teams` | Restart gateway so lazy-install can run, or install into the **SanRemes venv**: `~/.sanremes/sanremes-agent/venv/bin/pip install microsoft-teams-apps aiohttp`. System `pip` fails on Ubuntu 24.04 (PEP 668) and would not affect the service anyway |
...
| Logs show `"UNKNOWN / HTTP/1.0" 400` when Teams sends a message | The tunnel or reverse proxy is forwarding HTTPS to SanRemes' plain HTTP listener. Terminate TLS at the proxy and forward HTTP to port `3978` |
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.sanremes/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create sanremes-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs sanremes` / `sanremes gateway status -l` for tracebacks |
...
- Store credentials in `~/.sanremes/.env` with permissions `600` (`chmod 600 ~/.sanremes/.env`)
