---
sidebar_position: 30
title: "SanRemes Relay"
description: "Connect SanRemes to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# SanRemes Relay (Connector)
...
SanRemes Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.sanremes/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.sanremes/.env`:
...
`sanremes gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.sanremes/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `sanremes auth add nous` (or `sanremes setup`) and retry.
...
startup. Restart it (`sanremes gateway restart`).
