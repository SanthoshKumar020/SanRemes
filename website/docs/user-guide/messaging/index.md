...
description: "Chat with SanRemes from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with SanRemes from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with SanRemes](/guides/use-voice-mode-with-sanremes).
...
```mermaid
flowchart TB
    subgraph Gateway["SanRemes Gateway"]
...
For group chats, hooks, and automation flows, SanRemes supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. SanRemes keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; SanRemes does not hide failures just because the text resembles a silence token.
...
```bash
sanremes gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
sanremes gateway              # Run in foreground
sanremes gateway setup        # Configure messaging platforms interactively
sanremes gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo sanremes gateway install --system   # Linux only: install a boot-time system service
sanremes gateway start        # Start the default service
sanremes gateway stop         # Stop the default service
sanremes gateway status       # Check default service status
sanremes gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.sanremes/config.yaml"
gateway:
...
```bash
sanremes gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. SanRemes sends heartbeats
...
| `/update` | Update SanRemes Agent to the latest version |
...
with the `session_reset` section in `~/.sanremes/config.yaml`:
...
Configure per-platform overrides in `~/.sanremes/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.sanremes/gateway-config.yaml`:
...
sanremes pairing approve telegram XKGH5N7P

# Other pairing commands:
sanremes pairing list          # View pending + approved users
sanremes pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, SanRemes appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.sanremes/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.sanremes/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `SANREMES_HOME`:

- `~/.sanremes/status_phrases.yaml` or any `*.yaml` in `~/.sanremes/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to SANREMES_HOME
...
Off by default. When enabled, SanRemes prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.sanremes/config.yaml`:
...
```bash
SANREMES_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
sanremes gateway install               # Install as user service
sanremes gateway start                 # Start the service
sanremes gateway stop                  # Stop the service
sanremes gateway status                # Check status
journalctl --user -u sanremes-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo sanremes gateway install --system
sudo sanremes gateway start --system
sudo sanremes gateway status --system
journalctl -u sanremes-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit SanRemes installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit sanremes-gateway` (or `sudo systemctl edit sanremes-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `sanremes update`. When `sanremes update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart sanremes-gateway` command (it never blocks on an interactive password prompt).
...
```bash
sanremes gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `sanremes update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo sanremes update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/sanremes-gateway`:

```
sanremes ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed sanremes-gateway*, /usr/bin/systemctl --no-ask-password start sanremes-gateway*, /usr/bin/systemctl --no-ask-password restart sanremes-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. SanRemes will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple SanRemes installations on the same machine (with different `SANREMES_HOME` directories), each gets its own systemd service name. The default `~/.sanremes` uses `sanremes-gateway`; other installations use `sanremes-gateway-<hash>`. The `sanremes gateway` commands automatically target the correct service for your current `SANREMES_HOME`.
:::

### macOS (launchd)

```bash
sanremes gateway install               # Install as launchd agent
sanremes gateway start                 # Start the service
sanremes gateway stop                  # Stop the service
sanremes gateway status                # Check status
tail -f ~/.sanremes/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.sanremes.gateway.plist`. It includes three environment variables:
...
- **SANREMES_HOME** — scopes the gateway to your SanRemes installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `sanremes gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `SANREMES_HOME` directory gets its own launchd label. The default `~/.sanremes` uses `ai.sanremes.gateway`; other installations use `ai.sanremes.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `sanremes-cli` | Full access |
| Telegram | `sanremes-telegram` | Full tools including terminal |
| Discord | `sanremes-discord` | Full tools including terminal |
| WhatsApp | `sanremes-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `sanremes-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `sanremes-slack` | Full tools including terminal |
| Google Chat | `sanremes-google_chat` | Full tools including terminal |
| Signal | `sanremes-signal` | Full tools including terminal |
| SMS | `sanremes-sms` | Full tools including terminal |
| Email | `sanremes-email` | Full tools including terminal |
| Home Assistant | `sanremes-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `sanremes-mattermost` | Full tools including terminal |
| Matrix | `sanremes-matrix` | Full tools including terminal |
| DingTalk | `sanremes-dingtalk` | Full tools including terminal |
| Feishu/Lark | `sanremes-feishu` | Full tools including terminal |
| WeCom | `sanremes-wecom` | Full tools including terminal |
| WeCom Callback | `sanremes-wecom-callback` | Full tools including terminal |
| Weixin | `sanremes-weixin` | Full tools including terminal |
| BlueBubbles | `sanremes-bluebubbles` | Full tools including terminal |
| QQBot | `sanremes-qqbot` | Full tools including terminal |
| Yuanbao | `sanremes-yuanbao` | Full tools including terminal |
| Microsoft Teams | `sanremes-teams` | Full tools including terminal |
| API Server | `sanremes-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `sanremes-webhook` | Full tools including terminal |
| Raft | `sanremes-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.sanremes/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
