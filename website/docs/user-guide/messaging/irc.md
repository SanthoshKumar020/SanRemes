...
> Run `sanremes gateway setup` and pick **IRC** for a guided walk-through.

## Prerequisites

- An IRC server to connect to (e.g. `irc.libera.chat`)
- A channel to join (e.g. `#sanremes`) — comma-separate to join several
- A nickname for the bot (default: `sanremes-bot`)
- Optional: a registered nick + NickServ password if your network requires identification

## Configure SanRemes

You can configure IRC two ways — environment variables (for a quick env-only setup) or the `gateway` block in `~/.sanremes/gateway-config.yaml`.
...
| `IRC_NICKNAME` | ✅ | Bot nickname (default: `sanremes-bot`) |
...
By default, only nicks listed in `allowed_users` (or `IRC_ALLOWED_USERS`) may talk to the bot. Leave the list empty **and** set `IRC_ALLOW_ALL_USERS=true` to let anyone in the channel chat with SanRemes — useful for testing, but not recommended on public networks since IRC nicks are not authenticated unless the network enforces NickServ.
...
Check status with `sanremes gateway status` — IRC connection state is reported there, including for env-only setups.
...
- The adapter acquires a scoped credential lock per server+nick, so two SanRemes profiles won't fight over the same IRC identity.
