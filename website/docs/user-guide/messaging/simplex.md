...
> Run `sanremes gateway setup` and pick **SimpleX** for a guided walk-through.
...
| `SANREMES_SIMPLEX_TEXT_BATCH_DELAY` | Optional | Quiet-period seconds (default: `0.8`) used to concatenate rapid-fire inbound text messages into one event |
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `sanremes pairing approve simplex <CODE>`.
...
`simplex:group:12` as a cron `deliver=` target or in a `sanremes send` call.

## Sending with `sanremes send`

SimpleX works as a standalone send target — the daemon must be running,
but a live gateway is not required for plain text:

```bash
sanremes send --to simplex:alice "hello"          # DM by contact display name
sanremes send --to simplex:group:12 "hello"       # group by numeric ID
sanremes send --to simplex "hello"                # SIMPLEX_HOME_CHANNEL
```
...
`sanremes send --list` shows them by name. Before the first gateway run the
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`sanremes send` CLI](/guides/pipe-script-output):

```bash
sanremes send simplex:<contact-id> "Done!"
```

## Privacy notes

- SimpleX never reveals phone numbers or email addresses — contacts use opaque IDs
- The connection between SanRemes and the daemon is local WebSocket (`ws://127.0.0.1:5225`) — no data leaves your machine
