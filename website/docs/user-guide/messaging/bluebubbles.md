...
Connect SanRemes to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between SanRemes and the BlueBubbles server
...
Or set environment variables directly in `~/.sanremes/.env`:
...
By default, SanRemes responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, SanRemes uses conservative defaults for `SanRemes` and `@SanRemes agent` variants.
...
When someone messages your iMessage, SanRemes automatically sends them a pairing code. Approve it with:
```bash
sanremes pairing approve bluebubbles <CODE>
```
Use `sanremes pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.sanremes/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.sanremes/.env`):
```bash
...
SanRemes will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → SanRemes
SanRemes → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** SanRemes sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | SanRemes wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.sanremes/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — SanRemes resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `sanremes logs gateway` for webhook errors (or `sanremes logs -f` to follow in real-time)
