...
sidebar_position: 4
title: "Slack"
description: "Set up SanRemes Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect SanRemes Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your SanRemes instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. SanRemes uses the modern
...
The fastest path is to paste a manifest SanRemes generates for you. It
...
### Option A: From a SanRemes-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   sanremes slack manifest --agent-view --write
   ```
   This writes `~/.sanremes/slack-manifest.json` and prints paste-in
   ...
   ```bash
   sanremes slack manifest --agent-view \
...
4. Enter an app name (e.g., "SanRemes Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, SanRemes can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `sanremes-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a SanRemes configuration issue.
:::
...
SanRemes uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
sanremes gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
sanremes gateway              # Foreground
sanremes gateway install      # Install as a user service
sudo sanremes gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; SanRemes now
...
Every SanRemes command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
SanRemes command with its description.

Under the hood: SanRemes ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/sanremes-agent/blob/main/sanremes_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing SanRemes
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so SanRemes can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. SanRemes only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When SanRemes adds new commands (e.g. after `sanremes update`), regenerate
the manifest and update your Slack app:

```bash
sanremes slack manifest --write
```
...
3. Paste the new contents of `~/.sanremes/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/sanremes <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/sanremes btw run the tests` — SanRemes routes it the same way as `/btw
run the tests`. Free-form questions also work: `/sanremes what's the
...
Slack never delivers them to SanRemes.

As a workaround, SanRemes recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
SanRemes treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@SanRemes !stop`) and with
...
SanRemes falls back to a text prompt, the prompt instructs you to reply
...
fails, SanRemes retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@SanRemes /cmd` — reply as normal visible
messages instead.)
...
```bash
sanremes slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how SanRemes behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@SanRemes Agent what time is it?`). In channels, SanRemes replies in a thread attached to that message. |
| **Threads** | If you @mention SanRemes inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.sanremes/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # If Slack rejects the native stream, SanRemes keeps one editable text
      ...
      # that message itself @mentions SanRemes, and "all" accepts every
      # other bot. SanRemes always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions SanRemes, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default SanRemes sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"SanRemes is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), SanRemes uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — SanRemes seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and SanRemes falls back
...
- If the native stream can't start or update, SanRemes falls back to a single
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to SanRemes in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before SanRemes will respond.
...
By default SanRemes ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several SanRemes instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- SanRemes always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer SanRemes agents are filtered consistently across workspaces.
...
manifest — regenerate with `sanremes slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where SanRemes will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @SanRemes Agent`).
...
Delivery works even when the cron process isn't co-located with the gateway — SanRemes falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
SanRemes can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, SanRemes also loads tokens from an **OAuth token file** at:

```
~/.sanremes/slack_tokens.json
```
...
- When a message arrives, SanRemes uses the correct workspace-specific client to respond.
...
SanRemes supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @SanRemes Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @SanRemes Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. SanRemes now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @SanRemes Agent`)
...
- Tokens should be stored in `~/.sanremes/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your SanRemes config directory
