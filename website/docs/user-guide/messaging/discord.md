...
sidebar_position: 3
title: "Discord"
description: "Set up SanRemes Agent as a Discord bot"
---

# Discord Setup

SanRemes Agent integrates with Discord as a bot, letting you chat with your AI assistant through direct messages or server channels. The bot receives your messages, processes them through the SanRemes Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, voice messages, file attachments, and slash commands.

Before setup, here's the part most people want to know: how SanRemes behaves once it's in your server.

## How SanRemes Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | SanRemes responds to every message. No `@mention` needed. Each DM has its own session. |
| **Server channels** | By default, SanRemes only responds when you `@mention` it. If you post in a channel without mentioning it, SanRemes ignores the message. |
...
| **Threads** | SanRemes replies in the same thread. Mention rules still apply unless that thread or its parent channel is configured as free-response. Threads stay isolated from the parent channel for session history. |
| **Shared channels with multiple users** | By default, SanRemes isolates session history per user inside the channel for safety and clarity. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |
| **Messages mentioning other users** | When `DISCORD_IGNORE_NO_MENTION` is `true` (the default), SanRemes stays silent if a message @mentions other users but does **not** mention the bot. This prevents the bot from jumping into conversations directed at other people. Set to `false` if you want the bot to respond to all messages regardless of who is mentioned. This only applies in server channels, not DMs. |

:::tip
If you want a normal bot-help channel where people can talk to SanRemes without tagging it every time, add that channel to `DISCORD_FREE_RESPONSE_CHANNELS`.
:::

### Discord Gateway Model

SanRemes on Discord is not a webhook that replies statelessly. It runs through the full messaging gateway, which means each incoming message goes through:

1. authorization (`DISCORD_ALLOWED_USERS`)
2. mention / free-response checks
3. session lookup
4. session transcript loading
5. normal SanRemes agent execution, including tools, memory, and slash commands
6. response delivery back to Discord

That matters because behavior in a busy server depends on both Discord routing and SanRemes session policy.
...
So if Alice and Bob both talk to SanRemes in `#research`, SanRemes treats those as separate conversations by default even though they are using the same visible Discord channel.
...
SanRemes tracks running agents by session key.
...
3. Enter a name for your application (e.g., "SanRemes Agent") and accept the Developer Terms of Service.
4. Click **Create**.
...
The bot token is the credential SanRemes Agent uses to log in as your bot. Still on the **Bot** page:
...
After authorizing, the bot will appear in your server's member list (it will show as offline until you start the SanRemes gateway).

## Step 7: Find Your Discord User ID

SanRemes Agent uses your Discord User ID to control who can interact with the bot. To find it:
...
## Step 8: Configure SanRemes Agent
...
Add the following to your `~/.sanremes/.env` file:
...
:::tip
You can run `sanremes gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## Configuration Reference

Discord behavior is controlled through two files: **`~/.sanremes/.env`** for credentials and env-level toggles, and **`~/.sanremes/config.yaml`** for structured settings. Environment variables always take precedence over config.yaml values when both are set.
...
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` — ignore all other bots. `"mentions"` — only accept bot messages that `@mention` SanRemes. `"all"` — accept all bot messages. |
...
| `SANREMES_DISCORD_TEXT_BATCH_DELAY_SECONDS` | No | `0.6` | Grace window the adapter waits before flushing a queued text chunk. Useful for smoothing streamed output. |
| `SANREMES_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS` | No | `2.0` | Delay between split chunks when a single message exceeds Discord's length limit. |

:::warning Bot-to-bot conversation is not supported
`DISCORD_ALLOW_BOTS` exists to accept input from a specific trusted bot (e.g. a relay or webhook bot), not to let two SanRemes profiles talk to each other. The default, `"none"`, ignores all other bots and is the safe setting.

Wiring multiple SanRemes profiles to reply to one another in a shared channel — by setting `"mentions"` or `"all"` across several profiles — is an unsupported topology. Discord auto-`@mentions` the replied-to author on every reply, so under `"mentions"` two bots will satisfy each other's mention gate indefinitely and ack-loop. There is no circuit breaker for this because the supported configuration is simply to leave `DISCORD_ALLOW_BOTS` at `"none"`. If you must accept a particular bot, scope the acceptance narrowly and never to another auto-replying agent.
:::

### Config File (`config.yaml`)

The `discord` section in `~/.sanremes/config.yaml` mirrors the env vars above. Config.yaml settings are applied as defaults — if the equivalent env var is already set, the env var wins.
...
- If a message arrives inside a thread or forum post and that thread has no explicit entry, SanRemes falls back to the parent channel/forum ID.
...
Discord's WebSocket resume window can expire during a restart or network outage. Messages sent during that gap are not delivered as live gateway events. When this option is enabled, SanRemes scans a bounded set of configured channel and thread histories after Discord reconnects, then sends still-unhandled messages through the same authorization, mention, channel, deduplication, and dispatch path as live events.
...
When `true`: Alice and Bob talking in `#research` each have their own separate conversation with SanRemes. When `false`: the entire channel shares one conversation transcript and one running-agent slot.
...
SanRemes automatically registers installed skills as **native Discord Application Commands**. This means skills appear in Discord's autocomplete `/` menu alongside built-in commands.
...
No extra configuration is needed — any skill installed via `sanremes skills install` is automatically registered as a Discord slash command on the next gateway restart.

### Disabling Slash Command Registration

If you run multiple SanRemes gateways against the same Discord application (e.g. staging + production), only one of them should own the global slash-command registration — otherwise the last startup wins and the registrations flap. Turn slash registration off on the "follower" gateway:
...
Discord's per-upload size limit depends on the server's boost tier (25 MB free, up to 500 MB). If SanRemes gets an HTTP 413, the adapter falls back to a link pointing at the local cache path rather than failing silently.

## Receiving Arbitrary File Types

Any file type a user uploads is accepted. Authorization to message the agent is the gate — not the file extension. Every upload is downloaded, cached under `~/.sanremes/cache/documents/`, and surfaced to the agent as a `DOCUMENT`-typed message event so it can inspect the file with `terminal` (`ffprobe`, `unzip`, `file`, `strings`, etc.) or `read_file`.
...
Add these to your `~/.sanremes/.env`:
...
SanRemes Agent supports Discord voice messages:
...
- **Discord voice channels**: SanRemes can also join a voice channel, listen to users speaking, and talk back in the channel.

For the full setup and operational guide, see:
- [Voice Mode](/user-guide/features/voice-mode)
- [Use Voice Mode with SanRemes](/guides/use-voice-mode-with-sanremes)
...
discord.py plays only one audio stream per connection, so SanRemes installs a software mixer on the outgoing stream that sums an ambient loop, acknowledgements, and TTS replies into that single stream — they overlap instead of cutting each other off.
...
- `voice_playback_timeout_seconds` is a floor, not a hard cap for long TTS. SanRemes probes the generated audio duration and waits for `duration + 30s` when that is longer than the configured floor.
...
If the gateway log says Discord is connected and REST API checks work, but every inbound message is silent, look for this warning in `~/.sanremes/logs/gateway.log`:

```text
No Discord access policy configured; inbound Discord messages will be denied by default.
```

SanRemes 0.18 intentionally fails closed on externally reachable adapters. A Discord bot with no `DISCORD_ALLOWED_USERS`, no `DISCORD_ALLOWED_ROLES`, no `DISCORD_ALLOWED_CHANNELS`, and no explicit allow-all flag will connect successfully but deny inbound users before normal message handling.

### "Privileged intents" / `PrivilegedIntentsRequired` error on startup

**Cause**: SanRemes requests privileged Gateway Intents that are not enabled for your bot in the Developer Portal. Discord then rejects the WebSocket connection. SanRemes always requests **Message Content Intent**. It also requests **Server Members Intent** when your allowlist uses usernames (not numeric IDs) or when `DISCORD_ALLOWED_ROLES` is set. Presence Intent is not required.
...
3. Click **Save Changes**, then restart the gateway (`sanremes gateway restart`).

The gateway log should name the exact intent(s) SanRemes requested. Until they are enabled, Discord will keep rejecting the connection — this is a portal configuration error, not a flaky network issue.
...
**Cause**: The SanRemes gateway isn't running, or the token is incorrect.

**Fix**: Check that `sanremes gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `DISCORD_ALLOWED_USERS`.

**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.sanremes/.env` and restart the gateway.
...
**Fix**: Set this in `~/.sanremes/config.yaml` and restart the gateway:
...
```bash
# ~/.sanremes/.env — works alongside or instead of DISCORD_ALLOWED_USERS
...
```bash
# ~/.sanremes/.env — env vars win over config.yaml
...
For more information on securing your SanRemes Agent deployment, see the [Security Guide](../security.md).
