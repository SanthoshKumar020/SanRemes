...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The SanRemes A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your SanRemes over HTTP.

It interoperates with any A2A-compliant peer — another SanRemes, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **SanRemes ↔ SanRemes across machines** — let your desktop agent hand tasks to a SanRemes on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your SanRemes so other frameworks' agents can send it tasks.
...
```bash
sanremes gateway setup      # pick A2A
```
...
```bash
sanremes tools enable a2a --platform cli        # CLI/TUI sessions
sanremes tools enable a2a --platform telegram   # or any messaging platform
sanremes tools enable a2a --platform a2a        # let inbound A2A tasks call peers (agent chaining)
```
...
With the platform enabled, SanRemes serves:
...
- **Audit log** — every exchange appends to `~/.sanremes/a2a_audit.jsonl`.
