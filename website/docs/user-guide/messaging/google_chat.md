...
sidebar_position: 12
title: "Google Chat"
description: "Set up SanRemes Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect SanRemes Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your SanRemes
...
> Run `sanremes gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only SanRemes can read (e.g.,
`~/.sanremes/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a sanremes restart)
...
- Principal: `sanremes-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — SanRemes calls
...
- **App name**: whatever you want users to see ("SanRemes" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/sanremes-chat-events`.
...
`ADDED_TO_SPACE` event that SanRemes uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure SanRemes

Add the Google Chat section to `~/.sanremes/.env`:
...
Send "hola" in the test DM. The bot posts a "SanRemes is thinking…" marker, then
...
`~/.sanremes/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "SanRemes is thinking…").
...
Thread support: when a user replies inside a thread, SanRemes detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate SanRemes session.
...
3. Download the JSON. Move it onto the host that runs SanRemes.
4. Register the client with SanRemes (run under the profile you want it scoped to):
...
sanremes -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's SanRemes home (e.g.
`~/.sanremes/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.sanremes/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.sanremes/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, SanRemes isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `sanremes gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: SanRemes will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.sanremes/google_chat_user_tokens/<sanitized_email>.json` (filesystem
