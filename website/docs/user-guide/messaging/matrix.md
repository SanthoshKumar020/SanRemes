...
sidebar_position: 9
title: "Matrix"
description: "Set up SanRemes Agent as a Matrix bot"
---

# Matrix Setup

SanRemes Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the SanRemes Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

SanRemes works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how SanRemes behaves once it's connected.

## How SanRemes Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | SanRemes responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, SanRemes requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | SanRemes supports Matrix threads (MSC3440). If you reply in a thread, SanRemes keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, SanRemes auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | SanRemes accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; SanRemes normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, SanRemes isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
SanRemes sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
SanRemes now includes the current Matrix room name, room ID, topic, message ID,
...
2. Choose a username like `sanremes` — the full user ID will be `@sanremes:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `sanremes-bot`).

### Option C: Use Your Own Account

You can also run SanRemes as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

SanRemes needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give SanRemes the bot's user ID and password. SanRemes will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@sanremes:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

SanRemes Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure SanRemes Agent
...
# MATRIX_USER_ID=@sanremes:matrix.example.org
...
Bridge and appservice deployments need extra loop protection. SanRemes always
...
Optional behavior settings in `~/.sanremes/config.yaml`:
...
SanRemes supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with sanremes extras
cd ~/.sanremes/sanremes-agent && uv pip install -e ".[matrix]"
```
...
When E2EE is enabled, SanRemes:

- Stores encryption keys in `~/.sanremes/platforms/matrix/store/` (legacy installs: `~/.sanremes/matrix/store/`)
...
In Matrix conversations, SanRemes exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when SanRemes knows who requested the action.

### Media Limits

SanRemes uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. SanRemes rejects arbitrary
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, SanRemes imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If SanRemes bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
If you delete `~/.sanremes/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

SanRemes detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@sanremes:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@sanremes:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@sanremes:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@sanremes:your-server';
   "
   ...
   curl -X DELETE -H "Authorization: Bearer ADMIN_TOKEN" \
     'https://your-server/_synapse/admin/v2/users/%40sanremes%3Ayour-server/devices/DEVICE_ID'
   ```
...
2. Delete the local crypto store and restart SanRemes:
   ```bash
   rm -f ~/.sanremes/platforms/matrix/store/crypto.db*
   # restart sanremes
   ```
...
SanRemes supports the same gateway commands in Matrix that it supports on other
...
SanRemes only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/sanremes-agent/issues/12614).
...
If you previously used SanRemes with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@sanremes:your-server.org"},
       "password": "***",
       "initial_device_display_name": "SanRemes Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.sanremes/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.sanremes/platforms/matrix/store/crypto.db
   rm -f ~/.sanremes/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.sanremes/.env`:
...
a working E2EE setup with a previous version of SanRemes and are upgrading.
...
services:
  sanremes-matrix:
    ...
    environment:
      ...
      MATRIX_DEVICE_ID: "SANREMES_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.sanremes/platforms/matrix/store
```
...
RUN cd ~/.sanremes/sanremes-agent && uv pip install -e ".[matrix]"

CMD ["sanremes", "gateway"]
```
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote SanRemes API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-SanRemes-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: SanRemes uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps SanRemes's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the SanRemes logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The SanRemes gateway isn't running, or it failed to connect.

**Fix**: Check that `sanremes gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.sanremes/.env` and restart the gateway. Use the full `@user:server` format.
...
- **Media support**: SanRemes can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
