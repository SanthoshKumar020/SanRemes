...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="SanRemes Agent — Webhooks Tutorial"
...
1. Enable via `sanremes gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `sanremes webhook subscribe`
...
| `script` | No | Filter/transform script under `~/.sanremes/scripts/`. The webhook payload is passed as JSON on stdin. JSON object stdout replaces the payload before templating; text stdout is exposed as `script_output`; empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. |
| `skills` | No | List of skill names to load for the agent run. |
| `toolsets` | No | List of toolset keys (e.g. `["terminal", "file", "web"]`) that **replaces** the platform-level webhook toolset for runs triggered by this route only. Manual config edit only — not settable via `sanremes webhook subscribe`, so agent-created subscriptions cannot self-grant elevated tools. Names are validated the same way as `platform_toolsets` entries (unknown or platform-restricted names are dropped). See [Per-route toolsets](#per-route-toolsets). |
          ...
          filters:
            ...
            - any:
                ...
                - field: "payload.project_id"
                  in_file: "~/.sanremes/data/todoist/watchlist.json"
...
Use `script` when declarative filters are not enough. Scripts must live under `~/.sanremes/scripts/` for the active profile; relative paths resolve there, and path traversal outside that directory is blocked. `.sh` and `.bash` scripts run with bash, and all other extensions run with the current Python interpreter.

The route payload is sent to stdin as JSON:

```python
# ~/.sanremes/scripts/todoist-sanremes-label.py
import json
import sys

payload = json.load(sys.stdin)
labels = payload.get("payload", {}).get("labels", [])
if "sanremes" not in labels:
    print("[SILENT]")
    raise SystemExit(0)
...
- Empty stdout, exact `[SILENT]`, `{"__sanremes_ignore__": true}`, timeout, missing script, or nonzero exit code returns HTTP 200 with `{"status":"ignored","reason":"script"}`.
...
Add the `github-pr` route to your `~/.sanremes/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, SanRemes processes the event, and posts a review comment on the PR.
...
```bash
sanremes webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `sanremes webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.

### Create a subscription

```bash
sanremes webhook subscribe github-issues \
  --events "issues" \
...
```bash
sanremes webhook list
```

### Remove a subscription

```bash
sanremes webhook remove github-issues
```

### Test a subscription

```bash
sanremes webhook test github-issues
sanremes webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.sanremes/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `sanremes webhook subscribe` command.
...
For dynamic subscriptions, add the `toolsets` key by editing `~/.sanremes/webhook_subscriptions.json` directly:
...
- `sanremes webhook subscribe` deliberately does **not** accept a toolsets flag. Granting elevated tools is a manual config-file edit, so an agent creating its own subscription at runtime cannot self-grant `terminal`.
...
This is the same trust model that applies to everything the agent reads: web pages, files, and tool output are all untrusted input. SanRemes does not — and cannot reliably — sanitize untrusted text with a blocklist; phrasing, encoding, and translation make that trivially bypassable. **The trust boundary is the agent's capability surface, not the input channel.** Harden there:
...
- Run the gateway in foreground to see logs: `sanremes gateway run`
