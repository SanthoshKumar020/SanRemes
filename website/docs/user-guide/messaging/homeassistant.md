...
description: Control your smart home with SanRemes Agent via Home Assistant integration.
...
SanRemes Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "SanRemes Agent"
5. Copy the token
...
SanRemes Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.sanremes/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "SanRemes Agent".
...
The adapter reads credentials from `~/.sanremes/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active SanRemes profile
...
running SanRemes — `curl -H "Authorization: Bearer <token>" <url>/api/` should
