...
title: "Git Worktrees"
description: "Run multiple SanRemes agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

SanRemes Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with SanRemes so each session has a clean, isolated working directory.

## Why Use Worktrees with SanRemes?

SanRemes treats the **current working directory** as the project root:

- CLI: the directory where you run `sanremes` or `sanremes chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.sanremes/config.yaml`
...
SanRemes creates `.worktrees/my-experiment/` inside the repo (branch
`sanremes/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `sanremes-<id>`
...
git worktree add ../repo-feature feature/sanremes-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/sanremes-experiment` checked out in that directory

Now you can `cd` into the new worktree and run SanRemes there:

```bash
cd ../repo-feature

# Start SanRemes in the worktree
sanremes
```
...
git worktree add ../repo-experiment-a feature/sanremes-a
git worktree add ../repo-experiment-b feature/sanremes-b
```
...
Each SanRemes process:

- Works on its own branch (`feature/sanremes-a` vs `feature/sanremes-b`).
...
- SanRemes checkpoint data under `~/.sanremes/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per SanRemes experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/sanremes-checkpoints-docs`, `feature/sanremes-refactor-tests`.
- **Commit frequently**
...
- **Avoid running SanRemes from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `sanremes -w` (Automatic Worktree Mode)

SanRemes has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `sanremes/sanremes-<hash>`).
...
```bash
sanremes -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `sanremes -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each SanRemes session its own clean checkout.
