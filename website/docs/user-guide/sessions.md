...
SanRemes Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.sanremes/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
SanRemes stores session history so it can resume conversations, but it does not
...
SanRemes explicitly injects for that turn.
...
For example, if a user sends an image and asks SanRemes to make a meme from it,
SanRemes may inspect that image once with vision and run an image-processing
...
`sanremes sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `sanremes sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`sanremes` or `sanremes chat`) |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.sanremes/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `sanremes -c` again in the *same* terminal, SanRemes resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
sanremes -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
sanremes -c "my project"   # → resumes "my project #3"
```
...
sanremes --resume 20250305_091523_a1b2c3d4
sanremes -r 20250305_091523_a1b2c3d4

# Resume by title
sanremes --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
sanremes --resume latest

# Or with the chat subcommand
sanremes chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `sanremes sessions list`.
...
sanremes --resume latest --in ./my-project

# Works with the TUI too
sanremes --tui --resume latest --in ./my-project
```
...
```bash
sanremes --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`sanremes sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
sanremes sessions list --workspace my-project
sanremes sessions list --workspace ~/code/sanremes-agent
```

### Conversation Recap on Resume

When you resume a session, SanRemes displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a SanRemes session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.sanremes/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `sanremes -r "<title>"` from the shell) and pick up where the platform left off.
...
SanRemes automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `sanremes sessions list` or `sanremes sessions browse`.
...
```bash
sanremes sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), SanRemes creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`sanremes -c "my project"`), it automatically picks the most recent session in the lineage.
...
SanRemes provides a full set of session management commands via `sanremes sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
sanremes sessions list

# Filter by platform
sanremes sessions list --source telegram

# Show more sessions
sanremes sessions list --limit 50
```
...
`sanremes sessions export` is one surface for every export format, selected with `--format`:
...
sanremes sessions export backup.jsonl

# Export sessions from a specific platform
sanremes sessions export telegram-history.jsonl --source telegram

# Export a single session
sanremes sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
sanremes sessions export backup.jsonl --redact
```
...
sanremes sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
sanremes sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
sanremes sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
sanremes sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `sanremes-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
sanremes sessions export --format trace

# One session to a local trace file
sanremes sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
sanremes sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.sanremes/session-exports`).

```bash
# Export one session to Markdown
sanremes sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
sanremes sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
sanremes sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
sanremes sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
sanremes sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
sanremes sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
sanremes sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
sanremes sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
sanremes sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
sanremes sessions pin 20250305_091523_a1b2c3d4
sanremes sessions pin 20250305 20250306

# Remove the pin
sanremes sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
sanremes sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
sanremes sessions pinned --json > pinned-sessions.json
```
...
sanremes sessions prune

# Custom age threshold — bare numbers are days
sanremes sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
sanremes sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
sanremes sessions prune --newer-than 5h

# Explicit window with absolute timestamps
sanremes sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
sanremes sessions prune --source telegram
sanremes sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
sanremes sessions prune --newer-than 5h --title "smoke test"   # title substring
sanremes sessions prune --older-than 30 --max-messages 3        # tiny sessions
sanremes sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
sanremes sessions prune --model gpt-5 --older-than 1w           # by model (substring)
sanremes sessions prune --provider openrouter --older-than 60   # by billing provider
sanremes sessions prune --branch feature/old-experiment         # by git branch
sanremes sessions prune --user 12345678 --chat-type group       # by messaging origin
sanremes sessions prune --max-tokens 500 --older-than 7         # by token usage
sanremes sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
sanremes sessions prune --newer-than 5h --dry-run

# Skip confirmation
sanremes sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `sanremes sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `sanremes sessions prune` keeps the 90-day cutoff. Every
...
`sanremes sessions archive` takes the same filters as `prune` but soft-hides
...
sanremes sessions archive --newer-than 5h

# Archive by title substring, preview first
sanremes sessions archive --title "dry run" --dry-run
sanremes sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `sanremes sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`sanremes sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
sanremes sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`sanremes insights`](/reference/cli-commands#sanremes-insights).
...
`sanremes sessions repair-routing` finds message-bearing session rows with no
...
sanremes sessions repair-routing
...
sanremes sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
sanremes sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.sanremes/state.db ~/.sanremes/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into SanRemes and
continue it here. SanRemes reads Claude Code's session logs
...
sanremes sessions import

# Limit to one tool, or point at a specific file
sanremes sessions import --from claude
sanremes sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
sanremes --resume @claude
sanremes --resume @codex
```

`sanremes sessions import` creates a new SanRemes session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `sanremes --resume <id>` command.
...
When SanRemes cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, SanRemes uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to SanRemes in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.sanremes/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.sanremes/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.sanremes/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.sanremes/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.sanremes/sessions/sessions.json` is a **legacy mirror** of it,
...
`sanremes sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.sanremes/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `sanremes sessions list`, the cause is
`state.db` not receiving them — run `sanremes sessions repair` and watch for a
...
`*.jsonl` files in `~/.sanremes/sessions/`. They are no longer written or
read by SanRemes. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every SanRemes process in the same `SANREMES_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.sanremes/config.yaml`:
...
sanremes sessions delete <session_id>
