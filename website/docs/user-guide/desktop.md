---
sidebar_position: 3
title: "SanRemes Desktop"
description: "The native SanRemes desktop app — a polished experience for chatting with SanRemes, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# SanRemes Desktop

The SanRemes desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same SanRemes Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `sanremes` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
SanRemes has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`sanremes`) and **[TUI](./tui.md)** (`sanremes --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`sanremes dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Download the app from the [SanRemes Desktop product page](https://sanremes-agent.nousresearch.com/desktop), or follow the [installation instructions for SanRemes Desktop](../getting-started/installation.md).

If you already have SanRemes installed, simply run
...
- **The same conversation history** as every other SanRemes surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a SanRemes instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting SanRemes Desktop to a remote backend](./features/web-dashboard.md#connecting-sanremes-desktop-to-a-remote-backend).

#### Repository discovery

SanRemes Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional SanRemes sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `sanremes desktop --cwd <path>` (or the `SANREMES_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask SanRemes to open PR**. You can also create and switch branches from here.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `sanremes model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader SanRemes management surface so you don't have to drop to a terminal:
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#sanremes-cron).
- **Profiles** — switch between [SanRemes profiles](./profiles.md) (isolated config/skills/sessions).
...
roster where every [SanRemes profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by SanRemes cron). The roster lives in the left
...
each other's Bot Chats directly (`sanremes -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update SanRemes, and more.
...
The [manual update process](https://sanremes-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the SanRemes agent, your config, and your chats stay. (Same as `sanremes uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `sanremes uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `sanremes uninstall --full`.)
...
You can do the same from the terminal — `sanremes uninstall --gui` for the GUI alone, or `sanremes uninstall` / `sanremes uninstall --full` for the agent too.

:::note
Running `sanremes uninstall --gui` from a **source checkout** (a `sanremes desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `sanremes desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `sanremes desktop`

To launch via the CLI, simply run `sanremes desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `sanremes update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `SANREMES_DESKTOP_CWD`)           |
| `--sanremes-root PATH` | Override the SanRemes source root the app uses (sets `SANREMES_DESKTOP_SANREMES_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `sanremes` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the SanRemes Agent runtime into `SANREMES_HOME` (`~/.sanremes`, or `%LOCALAPPDATA%\sanremes` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `SANREMES_DESKTOP_SANREMES_ROOT`, then a completed managed install, then a probed `sanremes` on `PATH` (unless `--ignore-existing` / `SANREMES_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `SANREMES_DESKTOP_SANREMES` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `sanremes serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `sanremes --tui`. The desktop app is **self-contained**: it runs its own `sanremes serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a SanRemes backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.
...
- **Remote gateway** — enter the URL of a `sanremes serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **SanRemes Cloud** — sign in once to SanRemes Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
Further down the same **Settings → Gateways** page, **Registered gateways** manages a named list of every SanRemes gateway the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), SanRemes Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another SanRemes gateway…**) or via **⌘K → Gateways**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many SanRemes Instances](./multi-connection-desktop.md).
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.sanremes/.env` (the secrets file, mode 0600):
...
sanremes serve --host 0.0.0.0 --port 9119
```

Keep that `sanremes serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `sanremes serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.sanremes/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/sanremes` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `SANREMES_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `SANREMES_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateways settings panel.
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.sanremes/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `SANREMES_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting SanRemes Desktop to a remote backend](./features/web-dashboard.md#connecting-sanremes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & SanRemes Desktop](../reference/environment-variables.md#web-dashboard--sanremes-desktop).
...
`$SANREMES_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
- **Open logs** — opens `SANREMES_HOME/logs` in your file manager. On a remote
...
- **Send diagnostics** — uploads a redacted debug bundle to Nous-internal
  ...
  `sanremes debug share --nous`; secrets are always redacted, the bundle is
...
Boot logs land in `SANREMES_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.sanremes/sanremes-agent/.sanremes-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.sanremes/sanremes-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.nousresearch.sanremes
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.sanremes/sanremes-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
```bash
SANREMES_DESKTOP_SANREMES_ROOT=/path/to/clone npm run dev
SANREMES_HOME=/tmp/throwaway npm run dev
...
(Desktop, then Downloads, then Documents, ...) as SanRemes touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with SanRemes' stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **SanRemes.app** if you use Desktop.
3. Fully quit and relaunch them once.

`sanremes doctor` reports whether the current terminal context already has the
grant, and `sanremes setup` shows this tip on macOS when it doesn't.
...
```bash
tccutil reset ScreenCapture com.nousresearch.sanremes   # repeat per service
```

then toggle the fresh entry ON in System Settings and fully quit & relaunch
SanRemes. Grants are stable from then on.
...
certificate once and tell SanRemes to use it. The one-shot command does
...
```bash
sanremes desktop --setup-tcc-identity
```

Or do it manually:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `SanRemes Local Signing`, Identity Type: *Self-Signed Root*,
...
4. `sanremes config set desktop.macos_signing_identity "SanRemes Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `SanRemes Local Signing`). The command is idempotent —
...
`tccutil reset All com.nousresearch.sanremes` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `sanremes --tui` and the dashboard chat tab
