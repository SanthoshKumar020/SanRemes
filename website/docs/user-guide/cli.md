...
sidebar_position: 1
title: "CLI Interface"
description: "Master the SanRemes Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

SanRemes Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `sanremes setup --portal` — and you're ready to `sanremes chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
SanRemes also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `sanremes --tui` — see the [TUI](tui.md) guide.
:::
...
sanremes chat --query-file prompt.txt
sanremes chat --query-file - < prompt.txt

# With a specific model
sanremes chat --model "anthropic/claude-sonnet-4"

# With a specific provider
sanremes chat --provider nous        # Use Nous Portal
sanremes chat --provider openrouter  # Force OpenRouter

# With specific toolsets
sanremes chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
sanremes -s sanremes-agent-dev,github-auth
sanremes chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
sanremes --continue             # Resume the most recent CLI session (-c)
sanremes --resume <session_id>  # Resume a specific session by ID (-r)
sanremes --resume latest        # Resume the most recent session (same as -c)
sanremes --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
sanremes chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
sanremes -w                         # Interactive mode in worktree
sanremes -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`sanremes -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
sanremes worktree list              # audit: age, size, verdict, reason per tree
sanremes worktree prune             # remove safe trees + delete merged branches
sanremes worktree prune --dry-run   # show the plan without changing anything
sanremes worktree prune --trees-only     # leave local branches alone
sanremes worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running sanremes session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.sanremes/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
The `sanremes plugins` commands manage native SanRemes plugins and portable Agent
...
sanremes plugins disable <plugin-name>
sanremes plugins update <plugin-name>
sanremes plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. SanRemes currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the SanRemes CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The SanRemes CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `SANREMES_YOLO_MODE` is on (either `sanremes --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend SanRemes to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.sanremes/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. SanRemes supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for SanRemes replies |
...
```bash
sanremes -s sanremes-agent-dev,github-auth
sanremes chat -s github-pr-workflow -s github-auth
```

SanRemes loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.sanremes/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.sanremes/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain SanRemes..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, SanRemes also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches SanRemes — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while SanRemes is working, SanRemes prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend SanRemes to the background — just like any terminal process. The shell prints a confirmation:

```
SanRemes Agent has been suspended. Run `fg` to bring SanRemes Agent back.
```
...
Resume this session with:
  sanremes --resume 20260225_143052_a1b2c3
...
```bash
sanremes --continue                          # Resume the most recent CLI session
sanremes -c                                  # Short form
sanremes -c "my project"                     # Resume a named session (latest in lineage)
sanremes --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
sanremes --resume "refactoring auth"         # Resume by title
sanremes --resume latest                     # Resume the most recent session (same as -c)
sanremes --resume latest --in ./my-project   # Latest session for ./my-project's workspace
sanremes -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `sanremes sessions rename <id> <title>` from the command line. Use `sanremes sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in SanRemes's SQLite state database under `~/.sanremes/state.db`. The database keeps:
...
SanRemes immediately confirms the task and gives you back the prompt:
