...
Type `@` followed by a reference to inject content directly into your message. SanRemes expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- SanRemes env: `$SANREMES_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$SANREMES_HOME/skills/.hub/`
