...
description: "Customize SanRemes Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

SanRemes Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SANREMES_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who SanRemes is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

SanRemes now seeds a default `SOUL.md` automatically in:

```text
~/.sanremes/SOUL.md
```

More precisely, it uses the current instance's `SANREMES_HOME`, so if you run SanRemes with a custom home directory, it will use:

```text
$SANREMES_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- SanRemes creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- SanRemes loads `SOUL.md` only from `SANREMES_HOME`
- SanRemes does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, SanRemes falls back to a built-in default identity
...
If SanRemes loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SANREMES_HOME`, the personality belongs to the SanRemes instance itself.

That also makes it easier to teach users:
- "Edit `~/.sanremes/SOUL.md` to change SanRemes' default personality."
...
- how SanRemes should handle uncertainty, disagreement, or ambiguity
...
## What SanRemes injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, SanRemes falls back to a built-in default identity ("You are SanRemes Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
SanRemes ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain SanRemes, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives SanRemes its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.sanremes/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.sanremes/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-sanremes)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how SanRemes speaks
- `display.skin` and `/skin` affect how SanRemes looks in the terminal
