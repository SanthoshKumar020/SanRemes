...
SanRemes Agent supports both text-to-speech output and voice message transcription across all messaging platforms.

:::tip Nous Subscribers
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, OpenAI TTS is available through the **[Tool Gateway](tool-gateway.md)** without a separate OpenAI API key. New installs can run `sanremes setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** for just TTS via `sanremes model` or `sanremes tools`.
:::
...
| CLI | Saved to `~/.sanremes/audio_cache/` | MP3 |
...
tts:
  provider: "edge"              # "edge" | "elevenlabs" | "openai" | "minimax" | "mistral" | "gemini" | "xai" | "deepinfra" | "neutts" | "kittentts" | "piper" — or "nous" for the managed Tool Gateway (written when you pick Nous Subscription in `sanremes tools`)
  ...
  piper:
    ...
    # voices_dir: ''                            # default: ~/.sanremes/cache/piper-voices/
...
- If `region` is omitted, `MINIMAX_API_KEY` keeps precedence for backward compatibility. If only `MINIMAX_CN_API_KEY` is configured, SanRemes selects `cn`.
- An explicitly selected region must have its matching credential. SanRemes never borrows the other region's key. A `base_url` override does not change the selected credential, and an override pointing at the other region's official endpoint is rejected.
...
If the file contains `{transcript}` or `{{ transcript }}`, SanRemes replaces that placeholder with the live TTS text. Otherwise, SanRemes appends a labeled `TRANSCRIPT` section automatically. The persona prompt stays local and is not shown in the chat reply.
...
Each provider has a documented per-request input-character cap. SanRemes splits longer replies into ordered, sentence-aware chunks before calling the provider, so the full normalized text is preserved instead of silently truncated:
...
**Install via `sanremes tools`** → Voice & TTS → Piper — SanRemes runs `pip install piper-tts` for you. Or install manually: `pip install piper-tts`.
...
On the first TTS call for a voice that isn't cached locally, SanRemes runs `python -m piper.download_voices <name>` and downloads the model (~20-90MB depending on quality tier) into `~/.sanremes/cache/piper-voices/`. Subsequent calls reuse the cached model.
...
**Supported `output_format` values:** `mp3` (default), `wav`, `ogg`, `flac`, `m4a`, `aac`, `amr`, `opus`. Your command must actually produce that format (e.g. via `ffmpeg`); SanRemes only validates the declared value and names the output file accordingly. An unknown value falls back to `mp3`. The chosen format is also exposed to the command as the `{format}` placeholder.

**Subprocess environment:** command providers (TTS and STT) run with SanRemes secrets scrubbed from the child environment — gateway bot tokens, LLM provider API keys, and internal relay credentials are removed; `PATH`, `HOME`, locale, and other normal variables are kept. If your command template needs its own API key from the environment (e.g. a `curl` one-liner), list the variable names under `env_passthrough` in the provider config:
...
Credentials come from your shell environment (`VOLCENGINE_APP_ID` / `VOLCENGINE_ACCESS_TOKEN`) or `~/.doubao-speech/config.yaml`. Pick a voice by adding `--voice zh-female-warm` (or any other alias from `doubao-speech list-voices`) to the command. `doubao-speech` also bundles streaming ASR — see the [STT section below](#example-doubao--volcengine-asr) for SanRemes integration. Source and full docs: [github.com/Hypnus-Yuan/doubao-speech](https://github.com/Hypnus-Yuan/doubao-speech).

#### Placeholders

Your command template can reference these placeholders. SanRemes substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces and other shell-sensitive characters are safe.

| Placeholder      | Meaning                                              |
|------------------|------------------------------------------------------|
| `{input_path}`   | Path to the temp UTF-8 text file SanRemes wrote        |
...
| `output_format`    | `mp3`   | One of `mp3` / `wav` / `ogg` / `flac`. Auto-inferred from the output extension if SanRemes picks a path.      |
| `voice_compatible` | `false` | When `true`, SanRemes converts MP3/WAV output to Opus/OGG via ffmpeg so Telegram renders a voice bubble.      |
...
Command-type providers run whatever shell command you configure, with your user's permissions. SanRemes quotes placeholder values and enforces the configured timeout, but the command template itself is trusted local input — treat it the same way you would a shell script on your PATH.
...
| A voice-listing API used by `sanremes setup` | **Plugin** (override `list_voices()`) |
...
Drop this in `~/.sanremes/plugins/my-tts/`:
...
Enable it (`sanremes plugins enable my-tts`), point `tts.provider` at it (`tts.provider: my-tts` in `config.yaml`), and the `text_to_speech` tool will route through your plugin.

#### Optional hooks

Override these on your provider class for richer integration:

- `list_voices()` → list of `{id, display, language, gender, preview_url}` dicts shown in `sanremes tools`.
- `list_models()` → list of `{id, display, languages, max_text_length}` dicts.
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power the picker row in `sanremes tools` / `sanremes setup`. Without this, the plugin still works but its row in the picker is minimal.
...
:::info Zero Config
Local transcription works out of the box when `faster-whisper` is installed. If that's unavailable, SanRemes can also use a local `whisper` CLI from common install locations (like `/opt/homebrew/bin`) or a custom command via `SANREMES_LOCAL_STT_COMMAND`.
:::
...
stt:
  ...
  local:
    ...
    language: ""              # optional ISO-639-1 hint; blank = use SANREMES_LOCAL_STT_LANGUAGE if set, else auto-detect
  groq:
    language: ""              # optional ISO-639-1 hint; blank = use SANREMES_LOCAL_STT_LANGUAGE if set, else auto-detect
  openai:
  ...
  xai:
    ...
    language: ""              # optional ISO-639-1 hint; blank = use SANREMES_LOCAL_STT_LANGUAGE if set, else "en"
```
...
**Groq API** — Requires `GROQ_API_KEY`. Good cloud fallback when you want a free hosted STT option. Set `stt.groq.language` (or the global `SANREMES_LOCAL_STT_LANGUAGE` env var) to skip Whisper's auto-detect and reduce latency on known-language audio.
...
**Mistral API (Voxtral Transcribe)** — Requires `MISTRAL_API_KEY`. Uses Mistral's [Voxtral Transcribe](https://docs.mistral.ai/capabilities/audio/speech_to_text/) models. Supports 13 languages, speaker diarization, and word-level timestamps. Install with `cd ~/.sanremes/sanremes-agent && uv pip install -e ".[mistral]"`.
...
**Custom local CLI fallback** — Set `SANREMES_LOCAL_STT_COMMAND` if you want SanRemes to call a local transcription command directly. The command template supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders. SanRemes tokenizes the rendered template into an argument list and executes it without a shell, so operators such as `|`, `>`, `&&`, and `;` are passed as literal arguments. Your command must write a `.txt` transcript somewhere under `{output_dir}`.
...
export VOLCENGINE_APP_ID="your-app-id"
export VOLCENGINE_ACCESS_TOKEN="your-access-token"
export SANREMES_LOCAL_STT_COMMAND='doubao-speech transcribe {input_path} --out {output_dir}/transcript.txt'
```
...
```bash
export SANREMES_LOCAL_STT_COMMAND='sh -c '\''whisper "$1" --output_format txt --output_dir "$2" | tee "$2/whisper.log"'\'' _ {input_path} {output_dir}'
```
...
SanRemes writes the incoming voice message to `{input_path}`, runs the command, and reads the `.txt` file produced under `{output_dir}`. Language is auto-detected by the Volcengine bigmodel endpoint.

### Fallback Behavior

An **explicit** `stt.provider` selection (written in `config.yaml`, e.g. via `sanremes tools`) is honored strictly — if that provider can't run, transcription fails with a clear error (`stt is configured to use <provider> (set via sanremes tools), but <failure>. Run 'sanremes tools' to change it.`) instead of silently switching engines. Note that `stt.provider: local` written in your config counts as an explicit selection.

When **no provider has ever been selected**, SanRemes auto-detects from what's available:
- **Local faster-whisper unavailable** → Tries a local `whisper` CLI or `SANREMES_LOCAL_STT_COMMAND` before cloud providers
...
This complements the legacy `SANREMES_LOCAL_STT_COMMAND` escape hatch via the built-in `local_command` path. Unlike the shell-driven command-provider registry, the legacy template is tokenized into argv and runs without implicit shell interpretation. Use `stt.providers.<name>` when you want **multiple** shell-driven STT engines, a name you can pick via `stt.provider`, or anything that needs per-provider `language` / `model` / `timeout`.

#### STT placeholders

Your command template can reference these placeholders. SanRemes substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces are safe.
...
1. If `{output_path}` exists and is non-empty → SanRemes reads it as UTF-8 text.
2. Otherwise, if the command wrote to stdout → SanRemes uses that.
...
For `format: json` / `srt` / `vtt`, SanRemes returns the raw file content as the `transcript` field. Extracting `.text` from JSON is out of scope for the runner — either configure `format: txt`, or post-process JSON downstream.
...
The shell command runs under the same user as SanRemes with full filesystem access — same trust model as `tts.providers.<name>: type: command` and `SANREMES_LOCAL_STT_COMMAND`. Only declare command providers from sources you trust.
...
| Only the legacy single-command escape hatch is wanted        | `SANREMES_LOCAL_STT_COMMAND` env var (argv-tokenized; no implicit shell) |
...
Drop this in `~/.sanremes/plugins/my-stt/`:
...
Enable it (`sanremes plugins enable my-stt`), set `stt.provider: my-stt` in `config.yaml`, and voice-message transcription will route through your plugin.
...
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power picker rows in `sanremes tools` / `sanremes setup` (the picker category for STT is not yet shipped — this metadata is available to plugins for forward compatibility).
