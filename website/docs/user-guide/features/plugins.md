...
title: "Plugins"
description: "Extend SanRemes with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

SanRemes has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in SanRemes
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a SanRemes Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.sanremes/plugins/` with a `plugin.yaml` and Python code:

```
~/.sanremes/plugins/my-plugin/
...
Start SanRemes — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.sanremes/plugins/hello-world/plugin.yaml`**
...
**`~/.sanremes/plugins/hello-world/__init__.py`**

```python
"""Minimal SanRemes plugin — registers a tool and a hook."""
...
Drop both files into `~/.sanremes/plugins/hello-world/`, restart SanRemes, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but SanRemes does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.sanremes/plugins/` are disabled by default. Enable them only for trusted repositories by setting `SANREMES_ENABLE_PROJECT_PLUGINS=true` before starting SanRemes.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `sanremes <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `sanremes plugins install` |
| Distribute via pip | `[project.entry-points."sanremes_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with SanRemes — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.sanremes/plugins/` | Personal plugins |
| Project | `.sanremes/plugins/` | Project-specific plugins (requires `SANREMES_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `sanremes_agent.plugins` entry_points | Distributed packages |
| Nix | `services.sanremes-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, SanRemes also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.sanremes/plugins/model-providers/<name>/` and `~/.sanremes/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `sanremes plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.sanremes/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
sanremes plugins                    # interactive toggle (space to check/uncheck)
sanremes plugins enable <name>      # add to allow-list
sanremes plugins disable <name>     # remove from allow-list + add to disabled
```

After `sanremes plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
sanremes plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

SanRemes checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `sanremes plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`sanremes plugins install <source> --force --ref <new-commit>`. The
...
Several categories of plugin bypass `plugins.enabled` — they're part of SanRemes' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.sanremes/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.sanremes/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
SanRemes tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. SanRemes runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, SanRemes never
materializes the prompt on another surface.

SanRemes still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of SanRemes that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.sanremes/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 26 lifecycle events currently accepted by `sanremes_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
SanRemes has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.sanremes/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and SanRemes also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a SanRemes Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a SanRemes Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a SanRemes Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `sanremes <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `SANREMES_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. SanRemes auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `sanremes skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.sanremes/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `sanremes plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
sanremes plugins                               # unified interactive UI
sanremes plugins list                          # table: enabled / disabled / not enabled
sanremes plugins search <term>                 # search the community plugin index
sanremes plugins install <name>                # install by index name (resolved to repo @ pinned ref)
sanremes plugins install user/repo             # install from Git, then prompt Enable? [y/N]
sanremes plugins install user/repo --enable    # install AND enable (no prompt)
sanremes plugins install user/repo --no-enable # install but leave disabled (no prompt)
sanremes plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
sanremes plugins remove my-plugin              # uninstall
sanremes plugins enable my-plugin              # add to allow-list
sanremes plugins disable my-plugin             # remove from allow-list + add to disabled
sanremes plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

SanRemes Desktop registers the `sanremes://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
sanremes://plugin/install?repo=owner/repo            # main install link
sanremes://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
sanremes://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens SanRemes and shows a **confirmation dialog** — the repo id,
...
Install from Git**. Legacy `sanremes://plugin-agent/…` and
`sanremes://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `sanremes-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="sanremes://plugin/install?repo=owner/repo&enable=1">Install in SanRemes</a>
```

MCP servers have the equivalent link form — see
[Add to SanRemes link](/reference/mcp-config-reference#add-to-sanremes-link).
...
When a plugin declares capabilities, `sanremes plugins install` (and
`sanremes plugins enable`) shows the list with one-line risk descriptions and
...
granted, `sanremes plugins update` surfaces the additions and asks again. New
...
`sanremes plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
sanremes plugins capabilities             # all plugins with declared/granted capabilities
sanremes plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and SanRemes has not reviewed the plugin's code. Only install
...
`sanremes plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
sanremes plugins search telegram               # fuzzy search
sanremes plugins search                        # browse the whole index
sanremes plugins search --capability platform  # filter by declared capability
sanremes plugins search media --json           # machine-readable output
sanremes plugins search --refresh              # bypass the 24h local cache
```
...
```bash
sanremes plugins install sanremes-media-studio
```
...
(`https://raw.githubusercontent.com/NousResearch/sanremes-plugin-index/main/index.json`,
overridable via `sanremes config set plugins.index_url <url>`). Fetches are
cached under `~/.sanremes/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with SanRemes — so search works fully offline.

**Index entry format.** Each entry is a JSON object:

```json
{
  "name": "sanremes-media-studio",
  "description": "Generative media workspace plugin.",
  "author": "NousResearch",
  "tags": ["media", "image-gen"],
  "repo": "NousResearch/sanremes-media-studio",
  "ref": "<40-char commit SHA>",
  "subdir": null,
  "homepage": "https://github.com/NousResearch/sanremes-media-studio",
...
(`sanremes_cli/data/plugin_index.json` in the repo) is the format reference.

**Submitting a plugin.** The index is maintained as a plain JSON file —
submit a pull request to the
[sanremes-plugin-index](https://github.com/NousResearch/sanremes-plugin-index)
...
A **plugin pack** is a declarative, shareable YAML file (`sanremes-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: sanremes-media-studio            # bare community-index name…
...
config:                                  # optional, non-secret seeds only
  sanremes-media-studio:
    default_model: flux-3
skills: []                               # declared list only (not auto-installed yet)
```

```bash
sanremes plugins pack show ./sanremes-pack.yaml     # dry-run review
sanremes plugins pack install ./sanremes-pack.yaml  # review → confirm → install
sanremes plugins pack export > sanremes-pack.yaml   # snapshot the current install
sanremes plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `sanremes plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `sanremes plugins install`.
...
provenance (installed via `sanremes plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`sanremes skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `sanremes plugins install` and `sanremes plugins update` runs a static
...
On `sanremes plugins update`, a dangerous verdict on the updated tree
...
Running `sanremes plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `sanremes plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- SanRemes reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- SanRemes rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless SanRemes can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. SanRemes drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. SanRemes checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through SanRemes' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
