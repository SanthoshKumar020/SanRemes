...
sidebar_position: 14
title: "API Server"
description: "Expose sanremes-agent as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes sanremes-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to sanremes-agent and use it as a backend.

Your agent handles requests with its full toolset (terminal, file operations, web search, memory, skills) and returns the final response. When streaming, tool progress indicators appear inline so frontends can show what the agent is doing.

:::tip One backend covers models + tools
SanRemes itself needs a configured provider and tool backends for the API server to be useful. A [Nous Portal](/user-guide/features/tool-gateway) subscription handles both — 300+ models plus web/image/TTS/browser via the Tool Gateway. Run `sanremes setup --portal` once before starting the API server and frontends like Open WebUI or LobeChat get a fully tool-equipped backend.
:::
...
# Optional: only if a browser must call SanRemes directly
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "sanremes-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
**Streaming** (`"stream": true`): Returns Server-Sent Events (SSE) with token-by-token response chunks. For **Chat Completions**, the stream uses standard `chat.completion.chunk` events plus SanRemes' custom `sanremes.tool.progress` event for tool-start UX. For **Responses**, the stream uses OpenAI Responses event types such as `response.created`, `response.output_text.delta`, `response.output_item.added`, `response.output_item.done`, and `response.completed`.

**Tool progress in streams**:
- **Chat Completions**: SanRemes emits `event: sanremes.tool.progress` for tool-start visibility without polluting persisted assistant text.
- **Responses**: SanRemes emits spec-native `function_call` and `function_call_output` output items during the SSE stream, so clients can render structured tool UI in real time.
...
Tool calls in the `output` array were already executed server-side by the SanRemes agent — they are replayed with `"status": "completed"` for structured tool UI, never as pending calls for the client to execute.
...
Lists the agent as an available model. The advertised model name defaults to the [profile](/user-guide/profiles) name (or `sanremes-agent` for the default profile). Required by most frontends for model discovery.

`/v1/models` is intentionally the cheap OpenAI-compat surface. It does **not**
enumerate every authenticated provider/model combination SanRemes can route to,
and it does not do pricing or capability enrichment.

### GET /api/model/options

SanRemes-aware clients can request the same curated provider/model inventory used
...
Normal opens are intentionally conservative for custom providers: SanRemes probes
...
authenticated UI needs the richer SanRemes-specific picker metadata.
...
Use this endpoint when integrating dashboards, browser UIs, or control planes so they can discover whether the running SanRemes version supports runs, streaming, cancellation, and session continuity without depending on private Python internals.

## Browser-extension control

SanRemes can route browser tools through an authenticated extension that controls
the browser session associated with the current SanRemes session. The feature is
...
register only for an existing server session. SanRemes derives the controller
...
SanRemes preserves the existing browser backend. Once the gateway binds a
...
SanRemes never retries the same action through another backend.
...
2. SanRemes returns a single-use ticket with a 30-second TTL and the filtered,
   server-bound controller scope.
3. Open `GET /v1/browser-control/ws` with both WebSocket subprotocols:
   `sanremes-browser-control-v1` and
   `sanremes-browser-control-ticket.<ticket>`.
...
SanRemes sends `browser.controller.command` frames containing `command_id`,
...
Authenticated clients can override SanRemes' default model selection per request
by sending:

- `model` — the target model id for this turn
- `provider` — the SanRemes provider slug to resolve credentials/runtime for this turn
...
alias, SanRemes rejects the request with `400` instead of silently remixing route
...
Requests that include an explicit `provider` — and the SanRemes-native
...
Runs accept a simple `input` string and optional `session_id`, `instructions`, `conversation_history`, or `previous_response_id`. When `session_id` is provided, SanRemes surfaces it in the run status so external UIs can correlate runs with their own conversation IDs.
...
Interrupt a running agent turn. The endpoint returns immediately with `{"status": "stopping"}` while SanRemes asks the active agent to stop at the next safe interruption point.
...
Create a new scheduled job. Body accepts the same shape as `sanremes cron` — prompt, schedule, skills, provider override, delivery target.
...
External UIs can manage SanRemes sessions over REST without standing up the dashboard. All endpoints are gated by `API_SERVER_KEY` and live under `/api/sessions/*`.
...
## Long-term memory scoping (`X-SanRemes-Session-Key`)

Multi-user frontends like Open WebUI need a stable per-channel identifier for long-term memory (Honcho, etc.) that is **independent** of the transcript-scoped `X-SanRemes-Session-Id` (which rotates on `/new`). Pass `X-SanRemes-Session-Key` on `/v1/chat/completions`, `/v1/responses`, or `/v1/runs` and SanRemes threads it through to `AIAgent(gateway_session_key=...)`, where the Honcho memory provider uses it to derive a stable scope.

```http
POST /v1/chat/completions HTTP/1.1
Authorization: Bearer ***
X-SanRemes-Session-Id: transcript-alpha
X-SanRemes-Session-Key: agent:main:webui:dm:user-42
```

Rules: max 256 chars, control characters (`\r`, `\n`, `\x00`) are rejected, and the value is echoed back on responses (JSON + SSE). `/v1/capabilities` advertises support via `"session_key_header": "X-SanRemes-Session-Key"`. Without the key, Honcho's `per-session` strategy produces a different scope per `session_id` — exactly the behavior SanRemes had before.

## System Prompt Handling

When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), sanremes-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call SanRemes directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.
...
- Requests to `/p/<profile>/v1/...` must present that profile's own
  `API_SERVER_KEY` (from `~/.sanremes/profiles/<profile>/.env`). The default
...
:::warning Security
The API server gives full access to sanremes-agent's toolset, **including terminal commands**. `API_SERVER_KEY` is **required for every deployment**, including the default loopback bind on `127.0.0.1`. Keep `API_SERVER_CORS_ORIGINS` narrow to control browser access when you explicitly allow browser callers.
:::
...
| `API_SERVER_MODEL_NAME` | _(profile name)_ | Model name on `/v1/models`. Defaults to profile name, or `sanremes-agent` for default profile. |

### config.yaml

The same settings can live in `~/.sanremes/config.yaml` under a nested `gateway.api_server:` section:
...
To give multiple users their own isolated SanRemes instance (separate config, memory, skills), use [profiles](/user-guide/profiles):

```bash
# Create a profile per user
sanremes profile create alice
sanremes profile create bob
...
cat >> ~/.sanremes/profiles/alice/.env <<EOF
API_SERVER_ENABLED=true
API_SERVER_PORT=8643
API_SERVER_KEY=alice-secret
EOF

cat >> ~/.sanremes/profiles/bob/.env <<EOF
API_SERVER_ENABLED=true
...
sanremes -p alice gateway &
sanremes -p bob gateway &
```
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated SanRemes instance. See the [Open WebUI guide](/user-guide/messaging/open-webui#multi-user-setup-with-profiles) for details.
