...
When SanRemes Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.sanremes/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.sanremes/config.yaml` (switchable per session
via `/personality`).
...
If you use SanRemes' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.sanremes/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. SanRemes Agent's deliverable
