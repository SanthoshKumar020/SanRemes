...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `sanremes tools`.
...
SanRemes Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `sanremes tools` and persists in `config.yaml`.
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Your model selection persists across both paths. New installs can run `sanremes setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the image-gen backend via `sanremes tools`.

If the managed gateway returns `HTTP 4xx` for a specific model, that model isn't yet proxied on the portal side — the agent will tell you so, with remediation steps (switch to FAL.ai in `sanremes tools` with your own `FAL_KEY` for direct access, or pick a different model).
:::
...
`image_gen.provider` is the single selection key: `nous` routes through the managed Tool Gateway; a vendor name (`fal`, `openai`, `xai`, `krea`, ...) goes direct with your own key. The runtime always follows this stored selection — a `FAL_KEY` in `.env` is ignored while `provider: nous`, and `provider: fal` without `FAL_KEY` errors with `image_gen is configured to use fal (set via sanremes tools), but FAL_KEY is not set. Run 'sanremes tools' to change it.` rather than silently rerouting. Change providers via `sanremes tools`, not by adding/removing keys. (The old `use_gateway` boolean is legacy — still read as `nous` when `true`, but never written anymore.)

`max_parallel_requests` defaults to `4`. SanRemes clamps it to at least one and
...
new models appear in the picker as soon as OpenRouter serves them; no SanRemes
...
as a tool the chat model may call, and SanRemes cannot force the call — the
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; SanRemes materializes them to the local cache so delivery still works after expiry
