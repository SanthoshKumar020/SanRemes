...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of SanRemes's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple SanRemes instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
sanremes memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$SANREMES_HOME/honcho.json` (profile-local). The setup wizard handles this for you.

### Self-Hosted Honcho with Authentication

When pointing SanRemes at a self-hosted Honcho server, `sanremes honcho setup` (and `sanremes memory setup`) ask for a **local JWT / bearer token** after the base URL. Paste a JWT signed with the server's `AUTH_JWT_SECRET` (the Honcho compose env var) to enable authenticated access; leave it blank for servers running with `AUTH_USE_AUTH=false`. The local token is stored under the host block (`hosts.<host>.apiKey` in `honcho.json`), separate from any cloud `apiKey`, so you can flip the `Cloud or local?` prompt back to `cloud` later without losing either credential.
...
- `per-session` — each `sanremes` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — SanRemes syncs them back at session init.
...
The `sanremes honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `sanremes memory setup honcho` (or run `sanremes memory setup` and pick it from the list); the `sanremes honcho` subcommand then appears on the next invocation.

```bash
sanremes memory setup honcho    # Configure Honcho directly (works before activation)
sanremes honcho status          # Connection status, config, and key settings
sanremes honcho setup           # Redirects to `sanremes memory setup` (post-activation alias)
sanremes honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
sanremes honcho peer            # Show or update peer names + dialectic reasoning level
sanremes honcho mode            # Show or set recall mode (hybrid/context/tools)
sanremes honcho tokens          # Show or set token budget for context and dialectic
sanremes honcho identity        # Seed or show the AI peer's Honcho identity
sanremes honcho sync            # Sync Honcho config to all existing profiles
sanremes honcho peers           # Show peer identities across all profiles
sanremes honcho sessions        # List known Honcho session mappings
sanremes honcho map             # Map current directory to a Honcho session name
sanremes honcho enable          # Enable Honcho for the active profile
sanremes honcho disable         # Disable Honcho for the active profile
sanremes honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `sanremes honcho`

If you previously used the standalone `sanremes honcho setup`:
...
No re-login or re-setup needed. Run `sanremes memory setup` and select "honcho" — the wizard detects your existing config.
