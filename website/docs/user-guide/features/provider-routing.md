...
When using [OpenRouter](https://openrouter.ai) or [Nous Portal](/integrations/nous-portal) as your LLM provider, SanRemes Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.sanremes/config.yaml`:
...
- **CLI mode** — configured in `~/.sanremes/config.yaml`, loaded at startup
