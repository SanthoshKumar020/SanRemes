...
All skills live in **`~/.sanremes/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point SanRemes at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `sanremes update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.sanremes` profile):

```bash
curl -fsSL https://sanremes-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
sanremes profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
sanremes skills opt-out            # stop future seeding — nothing on disk is touched
sanremes skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
sanremes skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `sanremes update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `sanremes skills opt-in`) to re-enable.

:::note Safe by default
`sanremes skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version SanRemes installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling SanRemes to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.sanremes/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
sanremes chat --toolsets skills -q "What skills do you have?"
sanremes chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, SanRemes-tool framing, no invented
commands).
...
/home/user/.sanremes/cache/chart-q4-2025.png
...
When a missing value is encountered, SanRemes asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `sanremes setup` or `~/.sanremes/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `sanremes config migrate` prompts for unconfigured settings, and `sanremes config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.sanremes/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. SanRemes scans the
...
policy above), SanRemes can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
If you maintain skills outside of SanRemes — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell SanRemes to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.sanremes/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.sanremes/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the SanRemes process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, SanRemes ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.sanremes/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Repos can carry their own skills, active only for sessions started inside that project — the same pattern other agent harnesses use for repo-local configuration. When you launch SanRemes inside a git checkout, it looks for skills in:

```text
<project-root>/.sanremes/skills/    # SanRemes-native location
...
Skills are procedure documents the agent follows, so SanRemes does **not** auto-load them from arbitrary cloned repos. The first time you run SanRemes in a repo with project skills, the banner shows a notice:

```text
◆ 3 project skill(s) found in /home/you/myproject but not loaded — run `sanremes skills trust` to enable them.
```

Trust the repo once (from inside it, or by passing the path):

```bash
sanremes skills trust             # trust the current repo
sanremes skills trust ~/myproject # or explicitly
sanremes skills untrust           # revoke
```

Trusted roots are stored in `skills.trusted_project_dirs` in `~/.sanremes/config.yaml`. Set `skills.project_discovery: false` to turn the feature off entirely (no scanning, no notices).

### Precedence

Project skills are the **highest-precedence tier**: `project → local (~/.sanremes/skills/) → external_dirs`. A project skill named `deploy` overrides a same-named profile or bundled skill for sessions inside that repo — that's the point: vendored repo skills win on their home turf, without touching your global profile. Project skills are tagged `[project]` in the agent's skill index so provenance stays visible.

Like external dirs, project skill directories are treated as repo-owned: autonomous skill maintenance (the curator) never modifies them, and new agent-created skills always go to `~/.sanremes/skills/`.

### Scan-time quarantine

Trust is a repo-level decision, but a repo's skill content changes with every `git pull`. To close that gap, every project skill is scanned with the same security scanner used for Skills Hub installs before it enters the index. A skill whose scan verdict is **dangerous** (prompt-injection directives, credential-exfiltration commands, hidden-text tricks) is quarantined: it does not appear in the skill index, `skills_list`, slash commands, and refuses to load by name with an explanatory error. Scans are content-hash cached under `~/.sanremes/cache/project_skill_scans/` (never inside your repo) and re-run automatically when the skill's content changes.
...
Bundles live in **`~/.sanremes/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `sanremes bundles list`.
...
sanremes bundles create research

# Overwrite an existing bundle
sanremes bundles create backend-dev --skill ... --force

# Delete a bundle
sanremes bundles delete backend-dev

# Re-scan ~/.sanremes/skill-bundles/ and report changes
sanremes bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.sanremes/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.sanremes/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.sanremes/pending/skills/` and are
...
```bash
sanremes skills browse                              # Browse all hub skills (official first)
sanremes skills browse --source official            # Browse only official optional skills
sanremes skills search kubernetes                   # Search all sources
sanremes skills search react --source skills-sh     # Search the skills.sh directory
sanremes skills search https://mintlify.com/docs --source well-known
sanremes skills inspect openai/skills/k8s           # Preview before installing
sanremes skills install openai/skills/k8s           # Install with security scan
sanremes skills install official/security/1password
sanremes skills install skills-sh/vercel-labs/json-render/json-render-react --force
sanremes skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
sanremes skills list --source hub                   # List hub-installed skills
sanremes skills check                               # Check installed hub skills for upstream updates
sanremes skills update                              # Reinstall hub skills with upstream changes when needed
sanremes skills audit                               # Re-scan all hub skills for security
sanremes skills uninstall k8s                       # Remove a hub skill
sanremes skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
sanremes skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
sanremes skills publish skills/my-skill --to github --repo owner/repo
sanremes skills snapshot export setup.json          # Export skill config
sanremes skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with SanRemes. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `sanremes skills search <query> --source skills-sh`. SanRemes resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
SanRemes currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the SanRemes repository itself and install with built-in trust.
...
This is Vercel's public skills directory. SanRemes can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
sanremes skills search https://mintlify.com/docs --source well-known
sanremes skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
sanremes skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

SanRemes can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
sanremes skills install openai/skills/k8s
sanremes skills tap add myorg/skills-repo
```
...
[Skills Hub](https://sanremes-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no SanRemes-side changes required.
...
- SanRemes source id: `clawhub`

#### 6. LobeHub (`lobehub`)

SanRemes can search and convert agent entries from LobeHub's public catalog into installable SanRemes skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- SanRemes source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

SanRemes integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with SanRemes' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- SanRemes source id: `browse-sh`
- Trust level: `community`

```bash
sanremes skills search airbnb --source browse-sh
sanremes skills inspect browse-sh/airbnb.com/search-listings-ddgioa
sanremes skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). SanRemes also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- SanRemes source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. SanRemes does not enumerate or copy unrelated files from the host.

```bash
sanremes skills install https://sharethis.chat/SKILL.md
sanremes skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
sanremes skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `sanremes skills update` re-fetches from the same URL automatically when you want to refresh.
...
`sanremes skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with SanRemes | Always trusted |
...
```bash
sanremes skills check          # Report which installed hub skills changed upstream
sanremes skills update         # Reinstall only the skills with updates available
sanremes skills update react   # Update one specific installed hub skill
sanremes skills update react --force   # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `sanremes skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other SanRemes users add with `sanremes skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.sanremes.tags`, `version`, `author`, `platforms`, `metadata.sanremes.config`).
...
SanRemes discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/sanremes-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any SanRemes user can subscribe and install:

```bash
sanremes skills tap add my-org/sanremes-skills
sanremes skills search deploy
sanremes skills install my-org/sanremes-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.sanremes/skills/.hub/taps.json`:
...
The `sanremes skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `sanremes skills tap list` shows the effective path per tap.
...
```bash
sanremes skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.sanremes/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`sanremes skills reset`)

SanRemes ships with a set of bundled skills in `skills/` inside the repo. On install and on every `sanremes update`, a sync pass copies those into `~/.sanremes/skills/` and records a manifest at `~/.sanremes/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, SanRemes recomputes the hash of your local copy and compares it to the origin hash:
...
sanremes skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `SANREMES_HOME`, so `sanremes -p coder skills reset <name>` only affects that profile.
:::
