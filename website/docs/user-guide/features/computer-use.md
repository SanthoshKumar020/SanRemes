...
SanRemes Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended SanRemes integration. It
...
**Fresh installs already have the driver.** The SanRemes installer
...
- **`sanremes tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
sanremes computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `sanremes computer-use
status` to verify the install.

Already have cua-driver? SanRemes reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `sanremes update`, and the first
`computer_use` call of a session, SanRemes checks the local version and
...
selected with `SANREMES_CUA_DRIVER_CMD` stays
under your control, so SanRemes reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. SanRemes autodetection is a
planned cua-driver follow-up, so currently point SanRemes at that directory or
...
interface. The built-in toolset provides SanRemes actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `sanremes computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the SanRemes host identity. |
...
```
sanremes -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.sanremes/config.yaml`.

## Permission modes and logged-in browser profiles

SanRemes maps its existing approval UX onto cua-driver's immutable runtime
...
| SanRemes session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal SanRemes approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit SanRemes risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
SanRemes then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.sanremes/cua-manifest.yaml
```
...
for the format). SanRemes launches a private runtime with
...
identity instead of resetting with every SanRemes build), and SanRemes verifies
...
resetting or closing the SanRemes session, cancellation cleanup, or process exit
closes that transport session. SanRemes also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One SanRemes
...
embedded service under the SanRemes host identity.
...
## `sanremes computer-use doctor` — your first triage stop

`sanremes computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ sanremes computer-use doctor
...
SanRemes run declares a public cua-driver **session name** (something like
`sanremes-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. SanRemes disables it automatically where it is a known failure
...
SanRemes keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the SanRemes-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. SanRemes
autodetection is a planned cua-driver follow-up, so currently point SanRemes at
...
These are **platform deep dives, not duplicates of the SanRemes skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **SanRemes
...
image capture also saves a bounded, shareable copy under SanRemes' image cache
...
SanRemes applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.sanremes/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. SanRemes applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  SanRemes agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the SanRemes agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. SanRemes warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive SanRemes from inside the RDP / console
...
```
SANREMES_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
SANREMES_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **SanRemes disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) SanRemes sets
...
When it's on, `sanremes computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point SanRemes at a binary you built from source instead
of the published release. SanRemes resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`SANREMES_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for SanRemes testing (see notes).
...
### Option B — point SanRemes straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`SANREMES_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
SANREMES_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
SANREMES_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm SanRemes is using your build

- `sanremes computer-use status` prints the resolved binary path and
  version.
- `sanremes computer-use doctor` confirms the binary is reachable and
...
- **SanRemes spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  SanRemes YOLO, SanRemes instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no SanRemes
  change needed either way.
- **Local builds skip the version check.** SanRemes warns when the
...
**First action when anything's off: run `sanremes computer-use doctor`.**
...
Run `sanremes computer-use install` to fetch the cua-driver binary, or
run `sanremes tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `sanremes computer-use doctor` will
...
- **SanRemes-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  SanRemes `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. SanRemes autodetection is a
  planned follow-up; currently point SanRemes at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (SanRemes among them)
