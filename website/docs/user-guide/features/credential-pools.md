...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, SanRemes automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, SanRemes auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
sanremes auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
sanremes auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
sanremes auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  sanremes_pkce          oauth   sanremes_pkce ←
...
Run `sanremes auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `sanremes auth` | Interactive pool management wizard |
| `sanremes auth list` | Show all pools and credentials |
| `sanremes auth list <provider>` | Show a specific provider's pool |
| `sanremes auth add <provider>` | Add a credential (prompts for type and key) |
| `sanremes auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `sanremes auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `sanremes auth remove <provider> <index>` | Remove credential by 1-based index |
| `sanremes auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `sanremes auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `sanremes model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via sanremes model:
sanremes auth list
# Shows:
...
sanremes auth add Together.ai --api-key sk-together-second-key
```
...
SanRemes automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| SanRemes PKCE OAuth | `~/.sanremes/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `sanremes auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `sanremes auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. SanRemes can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and SanRemes-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`sanremes_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`sanremes_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.sanremes/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to SanRemes' credential store, so its token remains persistable.
