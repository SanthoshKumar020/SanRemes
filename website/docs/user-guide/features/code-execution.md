...
The `execute_code` tool lets the agent write Python scripts that call SanRemes tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with SanRemes over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from sanremes_tools import ...`
2. SanRemes generates a `sanremes_tools.py` stub module with RPC functions
3. SanRemes opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to SanRemes
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from sanremes_tools import web_search, web_extract
...
```python
from sanremes_tools import search_files, read_file
import json
...
```python
from sanremes_tools import search_files, read_file, patch
...
```python
from sanremes_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.sanremes/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to SanRemes's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (SanRemes's own python) |
...
### `SANREMES_*` variables in the child

The child process receives only a small, fixed set of operational `SANREMES_*`
...
(plus `SANREMES_RPC_DIR` / `SANREMES_RPC_SOCKET` / `TZ` / `HOME`, which SanRemes
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `SANREMES_`
through to the child. That broad prefix was removed for security hardening: it
could leak `SANREMES_*`-named configuration that doesn't match a secret substring
(for example `SANREMES_BASE_URL`, `SANREMES_KANBAN_DB`, or a `SANREMES_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `SANREMES_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (SanRemes-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `SANREMES_*`
variables, SanRemes emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`sanremes logs --level
DEBUG`, or check `~/.sanremes/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted SANREMES_* var(s)` if a script behaves
as though a `SANREMES_*` variable is missing.

SanRemes always writes the script and the auto-generated `sanremes_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call SanRemes tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.

## Platform Support

Code execution is available on **Linux, macOS, and Windows**. On Linux and macOS the RPC channel uses a Unix domain socket; on Windows, where `AF_UNIX` is unreliable, SanRemes automatically falls back to a loopback TCP socket for the sandbox RPC transport. Remote terminal backends (Docker/SSH/Modal/etc.) use a file-based RPC transport instead and additionally require Python 3 inside the backend.
