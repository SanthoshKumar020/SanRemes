...
It exists so that skills created via the [self-improvement loop](/user-guide/features/skills#agent-managed-skills-skill_manage-tool) don't pile up forever. Every time the agent solves a novel problem and saves a skill, that skill lands in `~/.sanremes/skills/`. Without maintenance, you end up with dozens of narrow near-duplicates that pollute the catalog and waste tokens.

By default (`prune_builtins: true`) the curator can archive **unused bundled built-in skills** (shipped with the repo) after `archive_after_days` of non-use, alongside the agent-created skills it primarily manages. Hub-installed skills (from [agentskills.io](https://agentskills.io)) are always off-limits. Set `curator.prune_builtins: false` to restore the old agent-created-only behavior, where bundled skills are never touched. The curator also **never auto-deletes** — the worst outcome is archival into `~/.sanremes/skills/.archive/`, which is recoverable.

Tracks [issue #7816](https://github.com/NousResearch/sanremes-agent/issues/7816).

## How it runs

The curator is triggered by an inactivity check, not a cron daemon. On CLI session start, and on a recurring tick inside the gateway's cron-ticker thread, SanRemes checks whether:
...
On a brand-new install (or the first time a pre-curator install ticks after `sanremes update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `sanremes curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.sanremes/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
**Easiest — `sanremes model`:**

```bash
sanremes model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`sanremes model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
sanremes curator status         # last run, counts, pinned list, LRU top 5
sanremes curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
sanremes curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
sanremes curator run --background  # fire-and-forget: start the run in a background thread
sanremes curator run --dry-run  # preview only — report without any mutations
sanremes curator backup         # take a manual snapshot of ~/.sanremes/skills/
sanremes curator rollback       # restore from the newest snapshot
sanremes curator rollback --list     # list available snapshots
sanremes curator rollback --id <ts>  # restore a specific snapshot
sanremes curator rollback -y         # skip the confirmation prompt
sanremes curator pause          # stop runs until resumed
sanremes curator resume
sanremes curator pin <skill>    # never auto-transition this skill
sanremes curator unpin <skill>
sanremes curator adopt <skill>    # hand an unmanaged skill to the curator
sanremes curator adopt --all-unmanaged   # hand over every unmanaged skill
sanremes curator list-unmanaged   # itemize skills with no provenance marker
sanremes curator restore <skill>  # move an archived skill back to active
sanremes curator list-archived    # list skills currently in ~/.sanremes/skills/.archive/
sanremes curator archive <skill>  # manually archive a single skill now
sanremes curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
sanremes curator ledger           # list the per-mutation audit ledger (all actors)
sanremes curator ledger --skill <name> --limit 50  # filter/paginate ledger entries
sanremes curator rollback <entry-id>  # undo a single mutation from the ledger
sanremes curator purge [--days N] [--dry-run]  # delete archived skills older than the TTL (explicit only)
```

## Backups and rollback

Before every real curator pass, SanRemes takes a tar.gz snapshot of `~/.sanremes/skills/` at `~/.sanremes/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
sanremes curator rollback        # restore newest snapshot (with confirmation)
sanremes curator rollback -y     # skip the prompt
sanremes curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, SanRemes takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `sanremes curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `sanremes curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`sanremes curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
Whole-run snapshots answer "undo everything the last curator pass did" — but sometimes you want to know *who changed what* and undo exactly one mutation. Every skill mutation — curator auto-transitions, agent `skill_manage` calls, and your own CLI archive/restore/purge — appends one entry to the append-only JSONL ledger at `~/.sanremes/skills/.curator_ledger.jsonl`:
...
- **before/after** — per-file `{path, sha256}` manifests. File contents are stored content-addressed (deduped by hash) under `~/.sanremes/.curator_backups/blobs/`, so a hundred entries touching the same unchanged file cost one blob.

```bash
sanremes curator ledger                  # newest 20 entries
sanremes curator ledger --skill my-skill --limit 50
sanremes curator rollback <entry-id>     # restore that one mutation's before-state
```

Single-entry rollback restores exactly the files that mutation touched (and removes files it created) from the blob store — nothing else in the skills tree moves. Like whole-tree rollback, it takes a safety ledger entry of the current state first and **fails closed**: if the safety capture can't be written, nothing is changed. Because foreground deletes are ledgered too, `sanremes curator rollback <entry-id>` can resurrect a hard-deleted skill.
...
Archived skills are kept forever by default. If you want `~/.sanremes/skills/.archive/` bounded, set a TTL and purge explicitly — purging never runs automatically, and every purged skill is captured into the ledger (with blobs) first, so even a purge leaves an auditable, recoverable trail:
...
```bash
sanremes curator purge --dry-run   # preview what would be deleted
sanremes curator purge             # delete archives older than the TTL (with confirmation)
sanremes curator purge --days 90   # one-off TTL override
```

## What "agent-created" means

The curator only manages skills explicitly marked as **agent-created** in
`~/.sanremes/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.sanremes/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.sanremes/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed SanRemes at an external skill
...
**To see which skills the curator actually manages**, run `sanremes curator status`.
...
`sanremes curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `sanremes curator adopt <name>` hands one over
```
...
```bash
sanremes curator list-unmanaged                    # itemize them, with reasons
sanremes curator adopt <name> [<name> ...]         # hand specific skills over
sanremes curator adopt --all-unmanaged --dry-run   # preview the full list
sanremes curator adopt --all-unmanaged             # hand over everything (prompts)
sanremes curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `sanremes curator adopt` changes the policy, and
...
**wrote** it — SanRemes edits user-authored skills on your behalf constantly. An
...
- Archives are recoverable via `sanremes curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `sanremes curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `sanremes curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
sanremes curator pin <skill>
sanremes curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.sanremes/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.sanremes/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.sanremes/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.sanremes/logs/curator/`:

```
~/.sanremes/logs/curator/
└── 20260429-111512/
...
```bash
sanremes curator restore <skill-name>
```

This moves the skill back from `~/.sanremes/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.sanremes/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `sanremes curator pause` — the pause persists across sessions; use `resume` to re-enable.
...
- [Issue #7816](https://github.com/NousResearch/sanremes-agent/issues/7816) — original proposal and design discussion
