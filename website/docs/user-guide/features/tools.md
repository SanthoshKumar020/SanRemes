...
description: "Overview of SanRemes Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
SanRemes ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, scheduled tasks, Home Assistant, and more.
...
| **X Search** | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool — gated on xAI credentials (SuperGrok OAuth or `XAI_API_KEY`); off by default, opt in via `sanremes tools` → 🐦 X (Twitter) Search. |
...
| **Automation** | `cronjob` | Scheduled tasks with create/list/update/pause/resume/run/remove actions. Outbound delivery is handled by cron's own delivery, the `sanremes send` CLI, and the gateway notifier — not by an agent-callable tool. |
...
Paid [Nous Portal](https://portal.nousresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `sanremes model` to enable it, or configure individual tools with `sanremes tools`.
:::
...
sanremes chat --toolsets "web,terminal"
...
See [Toolsets Reference](/reference/toolsets-reference) for the full set, including platform presets such as `sanremes-cli`, `sanremes-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
**One persistent container, shared across the whole process.** SanRemes starts a single long-lived container on first use (`docker run -d ... sleep infinity`) and routes every terminal, file, and `execute_code` call through `docker exec` into that same container. Working-directory changes, installed packages, environment tweaks, and files written to `/workspace` all carry over from one tool call to the next, across `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the SanRemes process. The container is stopped and removed on shutdown.
...
```
```bash
# Set credentials in ~/.sanremes/.env
...
# Configure
sanremes config set terminal.backend singularity
sanremes config set terminal.singularity_image ~/python.sif
```
...
modal setup
sanremes config set terminal.backend modal
```
...
sanremes config set terminal.backend vercel_sandbox
sanremes config set terminal.vercel_runtime node24
```

Authenticate with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This access-token setup is the supported path for deployments and normal long-running SanRemes processes on Render, Railway, Docker, and similar hosts. Supported runtimes are `node24`, `node22`, and `python3.13`; SanRemes defaults to `/vercel/sandbox` as the remote workspace root.
...
With `container_persistent: true`, SanRemes uses Vercel snapshots to preserve filesystem state across sandbox recreation for the same task. This can include SanRemes-synced credentials, skills, and cache files inside the sandbox. Snapshots do not preserve live processes, PID space, or the same live sandbox identity.

Background terminal commands use SanRemes' generic non-local process flow: spawn, poll, wait, log, and kill work through the normal process tool while the sandbox is alive, but SanRemes does not provide native Vercel detached-process recovery after cleanup or restart.
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.sanremes/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.sanremes/.env`.
:::
