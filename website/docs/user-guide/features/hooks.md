...
SanRemes has four hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.sanremes/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.sanremes/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
| **[Outbound webhooks](#outbound-webhooks)** | `hooks.outbound:` list in `~/.sanremes/config.yaml` | CLI + Gateway | Push signed lifecycle events to external HTTP endpoints — CI, dashboards, other agents |
...
Each hook is a directory under `~/.sanremes/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".sanremes" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.sanremes/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.sanremes/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".sanremes" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.sanremes/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.sanremes/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/sanremes-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.sanremes/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. SanRemes does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.sanremes/BOOT.md` with natural-language startup instructions.
...
Create `~/.sanremes/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `sanremes cron list` and check if any scheduled jobs failed overnight.
...
```text
~/.sanremes/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.sanremes/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.sanremes/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.sanremes/hooks/boot-md/handler.py`**

```python
"""Run ~/.sanremes/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".sanremes" / "BOOT.md"
...
```bash
sanremes logs --follow --level INFO | grep boot-md
```
...
Delete `~/.sanremes/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of SanRemes shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.sanremes/hooks/`
...
- Runtime event-name validity comes from `sanremes_cli.plugins.VALID_HOOKS`. `sanremes hooks list` lists configured shell/outbound hooks, not every available event; `sanremes hooks test <event>` reports the valid set only when an invalid event is supplied.
...
| `session_id` | `str` | Current SanRemes session id |
...
If a `pre_tool_call` callback exceeds `plugins.hook_callback_timeout` (or is still running from a previous timed-out fire), SanRemes **fails closed**: the tool is blocked with a timeout message rather than proceeding without a policy decision.
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is SanRemes's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.

The clean user-message `content` remains unchanged. For replay and prompt-cache stability, SanRemes may persist the exact API-bound message, including plugin-injected context, in the row's `api_content` sidecar.
...
```python
VOCAB = "SanRemes, Teknium, Nous Research, kanban"
...
Declare shell-script hooks in your `~/.sanremes/config.yaml` and SanRemes will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`sanremes_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.sanremes/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.sanremes/agent-hooks/` (by convention) | `~/.sanremes/plugins/<name>/` | `~/.sanremes/hooks/<name>/` |
...
Each time the event fires, SanRemes spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // SanRemes-canonical

// Modify a pre_tool_call — rewrite tool args before dispatch:
{"action": "modify", "args": {"new_string": "fixed content"}}         // SanRemes-canonical
...
hooks:
  pre_tool_call:
    - matcher: "terminal|write_file|patch"
      command: "~/.sanremes/agent-hooks/secret-scan.sh"
      timeout: 10
      fail_closed: true
```
...
`fail_closed` only applies to blocking-capable events (`pre_tool_call` today); setting it on any other event logs a warning at config-parse time and is ignored. `sanremes hooks test` reflects these semantics — the `parsed` line shows exactly the block shape the dispatcher would receive.
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.sanremes/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.sanremes/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.sanremes/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.sanremes/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.sanremes/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.sanremes/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate SanRemes event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.sanremes/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.sanremes/agent-hooks/log-orchestration.sh
log=~/.sanremes/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time SanRemes sees it, then persists the decision to `~/.sanremes/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `sanremes --accept-hooks chat`)
2. `SANREMES_ACCEPT_HOOKS=1` environment variable
3. `hooks_auto_accept: true` in `~/.sanremes/config.yaml`
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `sanremes hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.sanremes/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/sanremes/.sanremes/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `sanremes hooks list`.

### The `sanremes hooks` CLI

| Command | What it does |
|---------|--------------|
| `sanremes hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `sanremes hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `sanremes hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `sanremes hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.sanremes/agent-hooks/` so the path is easy to audit.
- Re-run `sanremes hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
...
Outbound webhooks are the push-side mirror of the [inbound webhook platform](/user-guide/messaging/webhooks): inbound webhooks wake SanRemes when the world changes; outbound webhooks tell the world when SanRemes does something. Configure a list of HTTP endpoints and the lifecycle events they care about, and SanRemes POSTs a signed JSON payload to each endpoint whenever a matching event fires — no polling on the receiving end.
...
- Wake *another* SanRemes instance: point the URL at that instance's inbound webhook

### Configuration

Add a `hooks.outbound:` list to `~/.sanremes/config.yaml`:

```yaml
hooks:
  outbound:
    - name: ci-notify                       # optional label for logs
      url: https://ci.example.com/sanremes-events
      events: [on_session_end, subagent_stop]
      secret_env: SANREMES_OUTBOUND_WEBHOOK_SECRET   # env var holding the HMAC secret
      timeout: 10                           # per-attempt seconds (1–60)

    - name: tool-monitor
      url: https://metrics.example.com/hooks/sanremes
...
Secrets: prefer `secret_env` (the name of an environment variable, typically set in `~/.sanremes/.env`) over an inline `secret:` literal, so the config file stays free of credentials. Entries without a secret are delivered unsigned (flagged as `UNSIGNED` by `sanremes hooks list`).
...
| `X-SanRemes-Event` | The hook event name |
| `X-SanRemes-Delivery` | Unique id per delivery — same value as `delivery_id` in the body |
| `X-SanRemes-Signature-256` | `sha256=<hex>` — HMAC-SHA256 of the raw body, GitHub-style; only present when a secret is configured |
...
- **Dedupe** on `delivery_id` (or the matching `X-SanRemes-Delivery` header) — remember recently seen ids and skip duplicates. SanRemes retries failed deliveries once, so the same id can legitimately arrive twice.
...
- **No consent prompt.** Outbound targets execute no code on your machine — they receive data at a URL you configured. `SANREMES_SAFE_MODE=1` still skips registration, same as plugins and shell hooks. Note that payloads include tool inputs and event metadata, so only point targets at endpoints you trust, and prefer `https://`.

`sanremes hooks list` shows configured outbound targets alongside shell hooks, including whether each target is signed.
