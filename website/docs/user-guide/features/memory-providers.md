...
SanRemes Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
sanremes memory setup      # interactive picker + configuration
sanremes memory status     # check what's active
sanremes memory off        # disable external provider
```

You can also select the active memory provider via `sanremes plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.sanremes/config.yaml`:
...
When a memory provider is active, SanRemes automatically:
...
**Setup Wizard:**
```bash
sanremes memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `sanremes honcho setup` command still works (it now redirects to `sanremes memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$SANREMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$SANREMES_HOME/honcho.json` > `~/.sanremes/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/sanremes-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/sanremes).
...
:::tip Migrating from `sanremes honcho`
If you previously used `sanremes honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per SanRemes profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All SanRemes profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per SanRemes profile. Host key `sanremes` → default; `sanremes.<profile>` for others. |
...
```bash
sanremes profile create coder --clone
```

`--clone` creates a `sanremes.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
sanremes honcho sync
```

Scans every SanRemes profile, creates host blocks for any profile without one, inherits settings from the default `sanremes` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `sanremes memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/sanremes-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/sanremes).
...
`sanremes memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.sanremes/.env`, and for named profiles use
...
`OPENVIKING_AGENT` is SanRemes' peer ID in OpenViking for peer-scoped memories.
SanRemes sends `User-Agent: openviking-memory-sanremes/<version>` on OpenViking
...
**Setup (Platform):**
```bash
sanremes memory setup    # select "mem0" → "Platform"
# Or manually:
sanremes config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.sanremes/.env
```

**Setup (OSS):**
```bash
sanremes memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
sanremes memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
sanremes memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
sanremes memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
sanremes memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.sanremes/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.sanremes/.env
```
...
**Config:** `$SANREMES_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.sanremes/.env`.
...
| `user_id` | `sanremes-user` | User identifier |
| `agent_id` | `sanremes` | Agent identifier |
...
**Switching modes:** Re-run `sanremes memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
sanremes memory setup    # select "hindsight"
# Or manually:
sanremes config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.sanremes/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p sanremes ui start`

**Config:** `$SANREMES_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `sanremes` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between SanRemes Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/sanremes-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
sanremes memory setup    # select "holographic"
# Or manually:
sanremes config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.sanremes-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$SANREMES_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
sanremes memory setup    # select "retaindb"
# Or manually:
sanremes config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.sanremes/.env
```
...
# Then configure SanRemes
sanremes memory setup    # select "byterover"
# Or manually:
sanremes config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$SANREMES_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=sanremes), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
sanremes memory setup    # select "supermemory"
# Or manually:
sanremes config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.sanremes/.env
```
...
Before running `sanremes memory setup`, set `base_url` in
`$SANREMES_HOME/supermemory.json`:
...
Then run `sanremes memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$SANREMES_HOME/supermemory.json`
...
| `container_tag` | `sanremes` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `sanremes-{identity}` → `sanremes-coder`) to isolate memories per SanRemes profile
...
| **Requires** | `pip install sanremes-memori` + `sanremes-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install sanremes-memori
sanremes-memori install
sanremes config set memory.provider memori
sanremes memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `sanremes-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$SANREMES_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$SANREMES_HOME/` so each profile has its own credentials
