...
# SanRemes Web Dashboard

The web dashboard is a browser-based UI for managing your SanRemes Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `sanremes setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
sanremes dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
sanremes dashboard --host 0.0.0.0

# Start without opening browser
sanremes dashboard --no-open
```
...
child with the selected profile's `SANREMES_HOME`, so the conversation runs
...
processes (manage them via `sanremes -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `sanremes-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.sanremes/sanremes-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.sanremes/sanremes-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `sanremes dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `sanremes dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.sanremes` is nearly full
...
- Node.js (same requirement as `sanremes --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.sanremes/sanremes-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.sanremes/.env
ExecStart=/path/to/venv/bin/python -m sanremes_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.sanremes/.env` containing:

```bash
SANREMES_DASHBOARD_BASIC_AUTH_USERNAME=admin
SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
SANREMES_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `sanremes config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated SanRemes instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `sanremes profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.sanremes/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `sanremes skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `sanremes mcp` reads from.
...
`sanremes mcp catalog` / `sanremes mcp install` use.
...
Connect SanRemes to any messaging platform from the browser — full parity with
`sanremes setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.sanremes/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and SanRemes versions, CPU core count + utilization, memory, disk usage of the SanRemes home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The SanRemes version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `sanremes update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `sanremes portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `sanremes curator`.
...
This re-reads `~/.sanremes/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`SANREMES_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.sanremes`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/sanremes/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — SanRemes Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
|-------|-----------|----------|
| `sanremes dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `sanremes dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `sanremes dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `sanremes dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, SanRemes doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `sanremes dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.
...
- **CLI — `sanremes dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `sanremes setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `SANREMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.sanremes/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  sanremes dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes SANREMES_DASHBOARD_OAUTH_CLIENT_ID to ~/.sanremes/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `SANREMES_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `SANREMES_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `sanremes dashboard register` |

Per the SanRemes Agent convention (`~/.sanremes/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: SANREMES_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `sanremes dashboard register` (Nous Portal) or install a
...
From a logged-in SanRemes install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `sanremes dashboard register` uses your existing Nous login to provision an OAuth client and writes `SANREMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.sanremes/.env` for you:

```bash
sanremes setup            # if you're not already logged into Nous Portal
sanremes dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes SANREMES_DASHBOARD_OAUTH_CLIENT_ID to ~/.sanremes/.env
```
...
```bash
sanremes dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://sanremes.example.com/auth/callback` and set `SANREMES_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `SANREMES_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `SANREMES_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `SANREMES_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.sanremes/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.sanremes/.env <<EOF
SANREMES_DASHBOARD_BASIC_AUTH_USERNAME=admin
SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
SANREMES_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.sanremes/.env
```
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/sanremes/   # required
      client_id: sanremes-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `SANREMES_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `SANREMES_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `SANREMES_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-sanremes.json` — it defines a `sanremes` realm, a **public PKCE client** (`sanremes-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "sanremes-dashboard",
      "name": "SanRemes Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-sanremes.json:/opt/keycloak/data/import/realm-sanremes.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/sanremes/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/sanremes`). The admin console is at
...
```bash
export SANREMES_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/sanremes"
export SANREMES_DASHBOARD_OIDC_CLIENT_ID="sanremes-dashboard"
export SANREMES_DASHBOARD_PUBLIC_URL="http://localhost:9119"
sanremes dashboard --host 0.0.0.0 --port 9119 --no-open
```

`SANREMES_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → sanremes-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `SANREMES_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/sanremes"
  trusted_proxies:
    - "172.20.0.5"
```
...
first; without one, SanRemes fails closed at startup. This prevents the local SPA
...
SanRemes always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
sanremes dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `SANREMES_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `SANREMES_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `sanremes_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `sanremes_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `sanremes_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
| `sanremes_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$SANREMES_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.sanremes/plugins/dashboard-auth-myidp/__init__.py
from sanremes_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `SANREMES_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
SANREMES_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  sanremes dashboard --host 0.0.0.0
...
```bash
# 1. Set the dashboard login credentials in ~/.sanremes/.env (secrets file, 0600).
cat >> ~/.sanremes/.env <<'EOF'
SANREMES_DASHBOARD_BASIC_AUTH_USERNAME=admin
SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
sanremes dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.sanremes/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.sanremes/.env`, so the credentials are in the environment at boot.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/sanremes` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `SANREMES_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `SANREMES_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `SANREMES_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `SANREMES_DASHBOARD_BASIC_AUTH_USERNAME` / `SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `SANREMES_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `sanremes_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `sanremes update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `sanremes dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **SanRemes Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **SanRemes Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
