...
SanRemes Agent includes a full browser automation toolset with multiple backend options:
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `sanremes setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the browser provider via `sanremes model` or `sanremes tools`.
:::
...
**This is the default browser mode**: when `browser.backend` is unset and the `browser-use` CLI is runnable (installed, or available through `uvx`), the agent gets the single `browser_exec` tool. If the CLI can't run, SanRemes falls back to the built-in browser tools automatically.

The mode is a **driver** that composes with your configured browser backend: it drives your local Chrome, a Nous-subscription cloud browser, Browserbase, Firecrawl, or Browser Use cloud browsers — whichever browser source is selected in `sanremes tools` → Browser Automation. The one exception is Camofox, which has no CDP endpoint for the harness to attach to; Camofox setups automatically keep the built-in browser tools.
...
When a cloud provider is configured, SanRemes auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`sanremes setup tools → Browser Automation`
...
When enabled, SanRemes copies your default browser's **active** profile — the one
...
`~/.sanremes/browser-profile/<browser>/`, then drives that snapshot with its
...
When you turn the toggle back off, SanRemes deletes the snapshot store
(`~/.sanremes/browser-profile/`) on the next browser use, so the copied
...
an exclusive (deny-all) lock, so SanRemes cannot copy them while the browser is
...
Set `browser.real_profile_autoclose: true` to let SanRemes **offer to close the
browser for you** when it's holding the profile. Even with this on, SanRemes never
closes it automatically — when the profile is locked it always stops and the
agent asks you first; only on your approval does it run `sanremes browser
...
(e.g. a background/tray instance relaunched), SanRemes stays blocked and tells you
...
Or configure via `sanremes tools` → Browser Automation → Camofox.

Camofox is selected like any other browser backend: pick **Camofox** in `sanremes tools` → Browser Automation, which writes `browser.cloud_provider: camofox` to `config.yaml`. `CAMOFOX_URL` is only the server address — setting it no longer selects the backend by itself once a browser selection exists (never-configured setups still autodetect it).

#### Persistent browser sessions

By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.sanremes/config.yaml`:
...
Then fully restart SanRemes so the new config is picked up.

:::warning Nested path matters
SanRemes reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — SanRemes ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, SanRemes silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active SanRemes profile, so different SanRemes profiles get different browser profiles (profile isolation).

##### What SanRemes does not do
- It does not force persistence on the Camofox server. SanRemes only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), SanRemes cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start SanRemes and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted SanRemes after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

SanRemes derives the stable `userId` from the profile-scoped directory `~/.sanremes/browser_auth/camofox/` (or the equivalent under `$SANREMES_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding SanRemes profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure SanRemes to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` SanRemes uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, SanRemes calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- SanRemes skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- SanRemes does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, SanRemes issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, SanRemes adopts the most recently created one in that group.
3. Otherwise, SanRemes adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, SanRemes falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — SanRemes does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want SanRemes to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, SanRemes generates a per-task `session_key` (`task_<id>`) — SanRemes will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and SanRemes can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while SanRemes runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. SanRemes automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.
...
SanRemes drives Lightpanda through `agent-browser` over CDP, the same way it drives local Chrome.

**Automatic Chrome fallback.** Lightpanda doesn't yet cover everything Chrome does, so the integration is non-disruptive: Lightpanda handles the actions it supports, and SanRemes transparently retries on Chrome for anything it doesn't. The supported set covers the core agent workflow — navigate, snapshot, click, type, scroll, back, press, and eval. Screenshots also fall back to Chrome because Lightpanda has no graphical renderer; `browser_vision` is pre-routed straight to Chrome for the same reason.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud provider, you can attach SanRemes browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start SanRemes from the terminal (`sanremes` or `sanremes chat`) and issue `/browser connect` there.
:::
...
brave-browser \
  ...
  --user-data-dir=$HOME/.sanremes/chrome-debug \
  --no-first-run \
...
Then launch the SanRemes CLI and run `/browser connect`.
...
If SanRemes runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects SanRemes itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let SanRemes talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through SanRemes MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, SanRemes can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Extra Chromium launch flags (comma- or newline-separated). SanRemes auto-injects
...
# SanRemes doesn't add automatically; setting it disables the auto-injection.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `sanremes config set toolsets '["sanremes-cli", "browser"]'`.
:::
...
You can also run `sanremes config set browser.snapshot_threshold 30000`. The setting applies to both explicit `browser_snapshot` calls and the automatic snapshot returned after navigation, including the Camofox backend (minimum 1000). Restart the current SanRemes session after changing it so the browser config cache reloads.
...
```
Type "sanremes agent" into the search field @e3
```
...
Screenshots are stored in `~/.sanremes/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.sanremes/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, SanRemes automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
...
- **Snapshot size** — large pages are truncated at `browser.snapshot_threshold` (default 15,000 characters, matching `web_extract`; no LLM summarization); the complete snapshot is saved to `~/.sanremes/cache/web/` and the output points at it for `read_file` paging
