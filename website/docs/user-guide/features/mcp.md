...
description: "Connect SanRemes Agent to external tool servers via MCP — and control exactly which MCP tools SanRemes loads"
---

# MCP (Model Context Protocol)

MCP lets SanRemes Agent connect to external tool servers so the agent can use tools that live outside SanRemes itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted SanRemes to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

:::tip Coming from Claude Code?
The `mcpServers` block in your `~/.claude.json` maps to `mcp_servers` in SanRemes' `config.yaml` — and `sanremes import-agent claude-code` migrates it (along with skills and instructions) automatically. See [Import from Other Agents](../import-from-other-agents.md).
:::

## What MCP gives you

- Access to external tool ecosystems without writing a native SanRemes tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want SanRemes to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.sanremes/config.yaml`:
...
4. Ask SanRemes to use the MCP-backed capability.
...
SanRemes will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

SanRemes ships a curated catalog of MCP servers that Nous staff has reviewed
and merged. They're disabled by default — install only what you actually
want.

```bash
sanremes mcp                # interactive picker (default)
sanremes mcp catalog        # plain-text list, scriptable
sanremes mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from SanRemes
...
`optional-mcps/` in the sanremes-agent repo — presence in that directory means
Nous approval. There is no community submission tier; entries are added by
merging a PR.

Catalog entries can require:

- **API key** — SanRemes prompts at install time and writes the value to
  `~/.sanremes/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — SanRemes points you at
  `sanremes auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, SanRemes probes the MCP server to list every
...
written (if not). Re-run `sanremes mcp configure <name>` once the server is
reachable to refine.
...
the sanremes-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/NousResearch/sanremes-agent/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed SanRemes
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
SanRemes`) for that entry instead of silently hiding it. Run `sanremes update`
to install the latest SanRemes when you see that.
...
from environment variables (which include everything in `~/.sanremes/.env`).
...
MCPs are never auto-updated. Re-run `sanremes mcp install <name>` to refresh
after a SanRemes update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/NousResearch/sanremes-agent/tree/main/optional-mcps).
...
rejected), and SanRemes's bundled `github/*` skills driving the `gh` CLI are a
...
HTTP MCP servers are remote endpoints SanRemes connects to directly.
...
- you do not want SanRemes spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Cloudflare, Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and SanRemes handles discovery, client identification, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.

SanRemes identifies itself with a [Client ID Metadata Document](../../reference/mcp-config-reference.md#client-identification-cimd-and-dcr) on servers that support one, and falls back to Dynamic Client Registration on those that don't. Both are automatic; there is nothing to configure.

:::tip Figma remote MCP
Figma's hosted endpoint (`https://mcp.figma.com/mcp`) allowlists Dynamic Client Registration by **exact `client_name`** — bare `"SanRemes Agent"` 403s, while `"Claude Code"` and `"Codex"` succeed. SanRemes auto-sets `oauth.client_name: "Claude Code"` for `mcp.figma.com` so install/login works without a special trick:
...
Or: `sanremes mcp install figma`, then `sanremes mcp login figma`.
:::
...
On first connect, SanRemes prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.sanremes/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When SanRemes runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:

- **Paste-back (no setup):** on an interactive terminal SanRemes prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.
- **Proxied callback (`redirect_uri`):** when a public HTTPS endpoint forwards to the host (e.g. a Tailscale Funnel or reverse proxy pointed at the callback port), set `oauth.redirect_uri` and the browser redirect reaches SanRemes on its own — no tunnel or paste needed:
...
For fully headless gateways (messaging bot, no interactive terminal at all), the optional [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) walks the agent through completing the flow manually and writing tokens where SanRemes expects them.
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `sanremes mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `sanremes mcp login` can list the tools and look like it worked, but every real tool call later times out. `sanremes mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `sanremes mcp login googledrive` — with the pre-registered client, SanRemes skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.sanremes/config.yaml` from inside a running SanRemes session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `sanremes mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. SanRemes passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
- `value_from: profile` sends the active SanRemes profile name, resolved once at connect time — useful when multiple profiles on one machine talk to the same server and it needs to tell them apart.
...
SanRemes reads MCP config from `~/.sanremes/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, SanRemes skips the server entirely |
...
For well-known MCP servers, `sanremes mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
sanremes mcp add codex --preset codex
```
...
You can pick any local name (`sanremes mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How SanRemes registers MCP tools

SanRemes prefixes MCP tools so they do not collide with built-in names:
...
- SanRemes only registers resource utilities if the MCP session actually supports resource operations
- SanRemes only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to SanRemes, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, SanRemes skips the server completely and does not even attempt a connection.
...
You can also separately disable SanRemes-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, SanRemes does not create an empty runtime MCP toolset for that server.
...
SanRemes discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify SanRemes when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When SanRemes receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, SanRemes does not blindly pass your full shell environment.
...
Then verify your config and restart SanRemes.
...
Because SanRemes now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, SanRemes may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from SanRemes via the `sampling/createMessage` protocol. This allows an MCP server to ask SanRemes to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running SanRemes as an MCP server

In addition to connecting **to** MCP servers, SanRemes can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use SanRemes's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through SanRemes
- You want a single MCP server that bridges to all of SanRemes's connected messaging platforms at once
- You already have a running SanRemes gateway with connected platforms
...
Add SanRemes to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed SanRemes in a specific location:

```json
{
  "mcpServers": {
    "sanremes": {
      "command": "/home/user/.sanremes/sanremes-agent/venv/bin/sanremes",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a SanRemes-specific channel browser:
...
The MCP server includes a live event bridge that polls SanRemes's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
sanremes mcp serve              # Normal mode
sanremes mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from SanRemes's session store — `~/.sanremes/state.db` is the primary source, with `sessions.json` kept only as a legacy fallback. A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `sanremes send` CLI.
...
- The embedded `sanremes mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of SanRemes, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
