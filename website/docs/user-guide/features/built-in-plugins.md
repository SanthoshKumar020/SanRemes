...
description: "Plugins shipped with SanRemes Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

SanRemes ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.sanremes/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a SanRemes Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.sanremes/plugins/<name>/`
3. **Project** — `./.sanremes/plugins/<name>/` (requires `SANREMES_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `sanremes_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `sanremes memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `sanremes plugins list` and the interactive `sanremes plugins` UI), but none load until you explicitly enable them:

```bash
sanremes plugins enable disk-cleanup
```

Or via `~/.sanremes/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer SanRemes. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
sanremes plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `sanremes plugins enable <name>`.
...
| `sanremes-achievements` | dashboard tab | Steam-style collectible badges generated from your real SanRemes session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `sanremes memory` and `sanremes plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SANREMES_HOME` or `/tmp/sanremes-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under SANREMES_HOME | always | Never |
...
**State** — everything lives at `$SANREMES_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `SANREMES_HOME` or `/tmp/sanremes-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `sanremes plugins enable disk-cleanup` (or check the box in `sanremes plugins`).

**Disabling again:** `sanremes plugins disable disk-cleanup`.
...
**Enabling:** `sanremes plugins enable security-guidance` (or check the box in `sanremes plugins`).

**Disabling again:** `sanremes plugins disable security-guidance`.
...
Traces SanRemes turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of SanRemes' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `sanremes logs`.
...
```bash
sanremes tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart SanRemes and the next turn ships a trace.
...
Then put the credentials in `~/.sanremes/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "SanRemes turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `SANREMES_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the SanRemes session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `sanremes chat` session lives under one Langfuse session.

**Verify:**

```bash
sanremes plugins list                 # observability/langfuse should show "enabled"
sanremes chat -q "hello"              # check the Langfuse UI for a "SanRemes turn" trace
```
...
| `SANREMES_LANGFUSE_RELEASE` | — | Release/version tag |
| `SANREMES_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `SANREMES_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `SANREMES_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

SanRemes-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — SanRemes-prefixed wins when both are set.
...
**Disabling:** `sanremes plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled SanRemes plugin. Do not run `sanremes plugins enable observability/nemo_relay`; SanRemes core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `SANREMES_NEMO_RELAY_PLUGINS_TOML` to that file before starting SanRemes. The policy is process-wide for every profile hosted by that SanRemes process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `SANREMES_NEMO_RELAY_ATOF_*` and `SANREMES_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `sanremes doctor` reports these stale settings when no replacement `plugins.toml` is selected.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `sanremes.session.segment` plus `sanremes.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.sanremes/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
sanremes plugins enable google_meet
sanremes meet setup   # preflight: playwright, chromium, auth file
sanremes meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `sanremes plugins disable google_meet`. Any saved transcripts stay in `~/.sanremes/workspace/meetings/` until you remove them.

### sanremes-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real SanRemes session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with SanRemes feature changes.

**How it works:**

- Scans your entire `~/.sanremes/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$SANREMES_HOME/plugins/sanremes-achievements/state.json`
...
| Secret | Hidden until SanRemes detects the first related signal in your history |

**API** — routes mount under `/api/plugins/sanremes-achievements/`:
...
**State files** — live under `$SANREMES_HOME/plugins/sanremes-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across SanRemes updates. |
...
**Enabling:** Nothing to enable — `sanremes-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `sanremes dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/sanremes-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.sanremes/plugins/sanremes-achievements/` that ships no dashboard. The plugin's state files under `$SANREMES_HOME/plugins/sanremes-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other SanRemes plugin — see [Build a SanRemes Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.sanremes/plugins/<name>/`
- Manifest source is reported as `bundled` in `sanremes plugins list`
