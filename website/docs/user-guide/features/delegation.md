...
Top-level model calls run in the background automatically. SanRemes returns a handle immediately so the conversation can continue, then posts the result back as a new message. An orchestrator subagent waits for its own workers so it can synthesize their results before returning.
...
One exception: when the parent has a resolved workspace directory, every subagent's system prompt embeds that workspace's **project context files** (`.sanremes.md` > AGENTS.md chain > CLAUDE.md > `.cursorrules` — the same discovery, priority, and size caps as the main agent's system prompt; SOUL.md is excluded). Subagents working in a repo operate under the repo's own conventions without having to rediscover them.
...
When a top-level agent provides a `tasks` array, SanRemes returns one background handle, runs the subagents in parallel, and posts one consolidated result after every child finishes. An orchestrator subagent waits for its batch in the current turn so it can synthesize the results.
...
When a background delegation finishes, SanRemes stores its completion event in
the active profile's `state.db` before publishing it to the normal fresh-turn
queue. If SanRemes restarts after completion but before delivery, the pending
...
SanRemes cannot prove whether its external side effects happened. Pending and
...
```
<sanremes_home>/cache/delegation/live/<delegation_id>/task-<n>.log
```
...
```bash
tail -f ~/.sanremes/cache/delegation/live/deleg_ab12cd34/task-0.log
```
...
Top-level model-facing `delegate_task` calls run in the background automatically where the session supports later delivery. SanRemes returns a handle immediately, and the result re-enters the conversation after the child or batch finishes. Orchestrator subagents wait for their workers in the current turn because they must synthesize those results before returning. Stateless request/response endpoints fall back to synchronous execution when they cannot deliver a detached result later.
...
- A SanRemes process restart does **not** resume a running child. Its attempt becomes `unknown` because SanRemes cannot prove which side effects happened.
...
- Each child starts its terminal in `<repo>/.worktrees/subagent-<id>` on its
  own branch `sanremes-subagent/subagent-<id>`, and its goal message tells it to
