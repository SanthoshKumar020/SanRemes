...
SanRemes can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SANREMES_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), SanRemes renders the sprite at full fidelity when your
...
sanremes pets list
sanremes pets list cat

# Install a pet and make it active in one step
sanremes pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
sanremes pets show

# Check your setup
sanremes pets doctor
```

## `sanremes pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `sanremes pets list [query] [--limit N]` |
| List installed pets | `sanremes pets list --installed` |
| Install a pet | `sanremes pets install <slug> [--select] [--force]` |
| Set the active pet | `sanremes pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `sanremes pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `sanremes pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `sanremes pets off` |
| Remove an installed pet | `sanremes pets remove <slug>` |
| Diagnose setup | `sanremes pets doctor` |

`sanremes pets show` flags:
...
Beyond installing pre-made pets from the gallery, SanRemes can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `sanremes pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `sanremes tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SANREMES_PET_IMAGE_PROVIDER` env var (e.g. `SANREMES_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while SanRemes is
...
Run `sanremes pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; SanRemes uses its own
  profile-scoped `<SANREMES_HOME>/pets/` instead — install through `sanremes pets`.

## See also

- The [`sanremes-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sanremes-agent.md)
