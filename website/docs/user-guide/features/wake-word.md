...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey SanRemes' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey SanRemes")

The wake word turns SanRemes into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, SanRemes listens in the background for a spoken
trigger phrase. Say it, and SanRemes starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** SanRemes backend (for example a
...
SanRemes supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey sanremes"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey sanremes"** — a model for it ships with SanRemes, so
...
```bash
cd ~/.sanremes/sanremes-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `sanremes` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.sanremes/config.yaml`,
...
wake_word:
  ...
  phrase: "hey sanremes"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_sanremes         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey sanremes" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey sanremes" utterances are missed.
...
default) to let SanRemes choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which SanRemes installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three SanRemes surfaces, and `surface` picks which one
...
| `cli` | Only the classic `sanremes` CLI. |
| `tui` | Only `sanremes --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when SanRemes surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. SanRemes does not silently fail over to another open surface.
...
"Hey SanRemes" works out of the box — the bundled openWakeWord model
(`model: hey_sanremes`) is the default. To wake on something else, the easiest
...
command (`sanremes -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.sanremes/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("sanremes" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey SanRemes" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey sanremes"
  porcupine:
    keyword: ~/.sanremes/wakewords/hey_sanremes.ppn
```

Set your access key in `~/.sanremes/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `sanremes tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `sanremes-agent[wake]`).
...
but the phrase never fires. SanRemes detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the SanRemes
backend (it may appear as your terminal, `python`, or SanRemes), then toggle the
...
```bash
sanremes config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
sanremes config set wake_word.input_device null
```
