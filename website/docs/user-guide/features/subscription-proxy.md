...
chat completions — use your SanRemes-managed provider subscription as their
...
| Use case | "Use SanRemes as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. SanRemes stores
the refresh token in `~/.sanremes/auth.json` — the same place all SanRemes
...
Model:      SanRemes-4-70B    # or SanRemes-4.3-36B, SanRemes-4-405B
```
...
interface in `sanremes_cli/proxy/adapters/`.
...
If you see `not logged in`, run `sanremes portal`. If you see
...
`sanremes_cli/proxy/adapters/<provider>.py` and registering it in
