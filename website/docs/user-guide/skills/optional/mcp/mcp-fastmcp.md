...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/mcp/fastmcp` |
...
| Related skills | [`sanremes-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sanremes-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into SanRemes or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to SanRemes. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.sanremes/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.sanremes/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is SanRemes integration, either:

- configure the server in `~/.sanremes/config.yaml` using the `native-mcp` skill, or
...
### SanRemes cannot see the deployed server

The server-building part may be correct while the SanRemes config is not. Load the `native-mcp` skill and configure the server in `~/.sanremes/config.yaml`, then restart SanRemes.
