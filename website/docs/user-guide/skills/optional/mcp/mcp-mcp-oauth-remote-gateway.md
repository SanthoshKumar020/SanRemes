...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp/mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), SanRemes Agent |
| License | MIT |
| Platforms | linux, macos |
| Tags | `MCP`, `OAuth`, `PKCE`, `Remote-Deployment` |
| Related skills | [`sanremes-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sanremes-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter), [`fastmcp`](/docs/user-guide/skills/optional/mcp/mcp-fastmcp) |
...
# MCP OAuth on a Remote SanRemes Gateway

## Overview

SanRemes' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the SanRemes process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when SanRemes runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches SanRemes.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files SanRemes' token storage expects, so a subsequent `/reload-mcp` finds
...
2. SanRemes is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI SanRemes** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
SanRemes' native MCP OAuth client (`tools/mcp_oauth.py`):

1. Picks a free local port `P`.
2. Registers a dynamic OAuth client with the AS, sending `redirect_uri = http://127.0.0.1:P/callback`.
3. Starts an HTTP server on `127.0.0.1:P` **inside the SanRemes process**.
4. Prints the authorize URL and waits for the code at its local endpoint.

When SanRemes runs remotely, the `127.0.0.1` in the `redirect_uri` is the remote
...
laptop and fails to connect. The callback never reaches the SanRemes process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the sanremes user, an
empty or missing tokens directory (`$SANREMES_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When SanRemes detects a remote session it prints two
...
Both require an interactive terminal to the SanRemes host. The rest of this skill
is for when there is NO interactive TTY — SanRemes running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the SanRemes Dashboard (try this BEFORE manual token surgery)

A remote SanRemes gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `sanremes dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'sanremes dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "SANREMES_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`SANREMES_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$SANREMES_HOME/.env`. Those
...
SanRemes' `SanRemesTokenStorage` would have written, so on `/reload-mcp` SanRemes finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "SANREMES|RAILWAY|CONTAINER"
echo "$DISPLAY $WAYLAND_DISPLAY $SSH_CLIENT"
```

No display + a remote indicator = remote gateway. `tools/mcp_oauth.py::_can_open_browser()`
uses these same env vars, so if SanRemes' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find SANREMES_HOME and the config path

```bash
SANREMES_HOME=$(python3 -c 'from sanremes_constants import get_sanremes_home; print(get_sanremes_home())')
echo "config: $SANREMES_HOME/config.yaml"
echo "tokens: $SANREMES_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "SanRemes Agent (manual OAuth)",
...
### 8. Write tokens in SanRemes' exact schema

`tools/mcp_oauth.py::SanRemesTokenStorage` expects two files under
`$SANREMES_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "sanremes-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though SanRemes (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, SanRemes sees `auth: oauth`, calls `SanRemesTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the SanRemes process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** SanRemes needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (SanRemes does natively).
...
- **Don't claim SanRemes doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in SanRemes. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of SanRemes' config.
