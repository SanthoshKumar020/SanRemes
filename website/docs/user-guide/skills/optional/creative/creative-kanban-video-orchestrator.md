...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a SanRemes Kanban pipeline that
...
3. **Generates** a setup script that creates SanRemes profiles, project workspace, and the initial kanban task
...
For mapping role → which SanRemes skills + toolsets it loads, see
...
3. Creates each SanRemes profile via `sanremes profile create --clone`
...
7. Fires the initial `sanremes kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/NousResearch/sanremes-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
sanremes kanban watch --tenant <project-tenant>     # live events
sanremes kanban list  --tenant <project-tenant>     # board snapshot
sanremes dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/NousResearch/sanremes-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/monitoring.md)**.
...
task graphs. See **[references/examples.md](https://github.com/NousResearch/sanremes-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `sanremes kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${SANREMES_HOME:-~/.sanremes}/.env` or the user's secret store.
