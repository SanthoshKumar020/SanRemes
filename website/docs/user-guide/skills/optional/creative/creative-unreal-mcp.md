...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the SanRemes MCP catalog. The
...
Two halves, in this order: the editor side must be up before SanRemes connects.
...
### One-time, SanRemes side

    sanremes mcp install unreal-engine
...
`~/.sanremes/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for SanRemes — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. SanRemes connects
...
2. Start the SanRemes session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through SanRemes
they appear as:

| SanRemes tool | Purpose |
|---|---|
...
then happens at `sanremes mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — SanRemes runs batched calls
...
- **Start order matters.** Editor + server up first, then the SanRemes
...
- **Timeouts on long operations.** SanRemes' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.sanremes/config.yaml` for
