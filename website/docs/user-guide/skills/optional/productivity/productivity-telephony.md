...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/productivity/telephony` |
...
This optional skill gives SanRemes practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${SANREMES_HOME:-~/.sanremes}/.env`
...
It does **not** turn SanRemes into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to SanRemes memory
...
### 1) "I want SanRemes to own a real phone number"
Use **Twilio**.
...
- pairs well with SanRemes `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${SANREMES_HOME:-~/.sanremes}/.env`
...
```bash
SCRIPT="$(find ~/.sanremes/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
sanremes skills search telephony
sanremes skills install official/productivity/telephony
```
...
Then save credentials into SanRemes:
...
3. Buy it and save it into `${SANREMES_HOME:-~/.sanremes}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is SanRemes calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing SanRemes's existing `text_to_speech` support.

Use this when:
- you want the call to use SanRemes's configured TTS voice rather than Twilio `<Say>`
...
Recommended SanRemes TTS -> Twilio Play workflow:

1. Generate the audio with SanRemes `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask SanRemes to create the message audio with `text_to_speech`
...
Important note:
- SanRemes TTS is great for prerecorded outbound messages
...
- SanRemes STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to SanRemes memory.
...
- Do not store arbitrary third-party phone numbers in SanRemes memory.
...
3. persist that number to `${SANREMES_HOME:-~/.sanremes}/.env`
