...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SANREMES_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SANREMES_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${SANREMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client sanremes
```
...
```bash
PUBLISH="${SANREMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client sanremes
```
...
```bash
DRIVE="${SANREMES_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `sanremes`)    |
