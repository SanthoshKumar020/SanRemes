...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/productivity/siyuan` |
...
3. Store it in `${SANREMES_HOME:-~/.sanremes}/.env`:
   ```
...
```yaml
# In ~/.sanremes/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
