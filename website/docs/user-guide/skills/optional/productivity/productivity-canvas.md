...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/productivity/canvas` |
...
4. Name the token (e.g., "SanRemes Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${SANREMES_HOME:-~/.sanremes}/.env`:
...
```bash
CANVAS="python $SANREMES_HOME/skills/productivity/canvas/scripts/canvas_api.py"
