...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/devops/pinggy-tunnel` |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. SanRemes' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
