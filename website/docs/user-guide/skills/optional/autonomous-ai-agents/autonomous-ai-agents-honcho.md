---
title: "Honcho — Configure and troubleshoot Honcho memory for SanRemes"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for SanRemes"
---
...
Configure and troubleshoot Honcho memory for SanRemes.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for SanRemes

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every SanRemes profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/sanremes#running-honcho-locally-with-sanremes

### Verify

```bash
sanremes honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this SanRemes profile's AI peer
...
Honcho models conversations as interactions between **peers**. SanRemes creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this SanRemes instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each SanRemes run |
| `global` | Single session across all directories |

Manual override: `sanremes honcho map my-project-name`
...
Each SanRemes profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block sanremes.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `sanremes.coder` host block in `honcho.json`
...
```bash
sanremes honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for SanRemes when Honcho memory is active.
...
Config file: `$SANREMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `sanremes honcho setup`. Ensure `memory.provider: honcho` is in `~/.sanremes/config.yaml`.

### Memory not persisting across sessions
Check `sanremes honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `sanremes profile create <name> --clone`. For existing profiles: `sanremes honcho sync`.
...
| Command | Description |
|---------|-------------|
| `sanremes honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `sanremes honcho status` | Show resolved config, connection test, peer info for active profile |
| `sanremes honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `sanremes honcho disable` | Disable Honcho for the active profile |
| `sanremes honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `sanremes honcho peers` | Show peer identities across all profiles |
| `sanremes honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `sanremes honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `sanremes honcho sessions` | List known directory-to-session-name mappings |
| `sanremes honcho map <name>` | Map current working directory to a Honcho session name |
| `sanremes honcho identity` | Seed AI peer identity or show both peer representations |
| `sanremes honcho sync` | Create host blocks for all SanRemes profiles that don't have one yet |
| `sanremes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to SanRemes + Honcho |
| `sanremes memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `sanremes memory status` | Show active memory provider and config |
| `sanremes memory off` | Disable external memory provider |
