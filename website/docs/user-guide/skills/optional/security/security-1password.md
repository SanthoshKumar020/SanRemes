...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by SanRemes Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during SanRemes terminal calls (desktop app flow only)
...
### Service Account (recommended for SanRemes)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${SANREMES_HOME:-~/.sanremes}/.env` (the skill will prompt for this on first load).
...
## SanRemes Execution Pattern (desktop app flow)

SanRemes terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/sanremes-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/sanremes-op.sock"
