...
| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/security/unbroker` |
...
- **Optional upgrades** (the skill works zero-config without these; `setup --auto` turns on every
  one it detects, reading credentials from the shell env **and from `$SANREMES_HOME/.env`** so keys
  SanRemes already loads for its own tools are picked up without re-exporting - each one converts a
  ...
  - Email automation, two credential-free-or-not options:
    - **Browser mode (no password): `setup --email-mode browser`.** The agent sends opt-out/CCPA
      ...
      `browser_*` tools. Nothing is stored. This requires SanRemes to be pointed at the operator's own
      ...
      `chrome --remote-debugging-port=9222 --user-data-dir="$HOME/.sanremes/chrome-debug"` (a dedicated
...
The engine stores data under `$PDD_DATA_DIR` (default `$SANREMES_HOME/unbroker`), written
...
- **Dossiers are plaintext by default** (JSON, `0600` under `SANREMES_HOME`). For at-rest encryption run
  ...
  a full-`SANREMES_HOME` read; set `PDD_AGE_IDENTITY` to a separate volume for real key separation.
