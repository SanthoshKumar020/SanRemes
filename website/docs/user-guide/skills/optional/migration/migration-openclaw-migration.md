---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into SanRemes"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into SanRemes"
---
...
Import an OpenClaw setup (memories, skills) into SanRemes.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sanremes skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | SanRemes Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `SanRemes`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> SanRemes Migration

Use this skill when a user wants to move their OpenClaw setup into SanRemes Agent with minimal manual cleanup.
...
```bash
sanremes claw migrate              # Full interactive migration
...
sanremes claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `sanremes setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_sanremes.py` to:

- import `SOUL.md` into the SanRemes home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into SanRemes memory entries
- merge OpenClaw command approval patterns into SanRemes `command_allowlist`
- migrate SanRemes-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to SanRemes working-directory configuration
- copy OpenClaw skills into `~/.sanremes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen SanRemes workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.sanremes/tts/`
- archive non-secret docs that do not have a direct SanRemes destination
...
- `scripts/openclaw_to_sanremes.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.sanremes/skills/migration/openclaw-migration/scripts/openclaw_to_sanremes.py`

Do not guess a shorter path like `~/.sanremes/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.sanremes/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of SanRemes-compatible secrets, currently:
...
SanRemes CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in SanRemes. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a SanRemes workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing SanRemes skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.sanremes/skills/migration/openclaw-migration/scripts/openclaw_to_sanremes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.sanremes/skills/migration/openclaw-migration/scripts/openclaw_to_sanremes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.sanremes/skills/migration/openclaw-migration/scripts/openclaw_to_sanremes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.sanremes/skills/migration/openclaw-migration/scripts/openclaw_to_sanremes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.sanremes/skills/migration/openclaw-migration/scripts/openclaw_to_sanremes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.sanremes/skills/migration/openclaw-migration/scripts/openclaw_to_sanremes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of SanRemes unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty SanRemes targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean SanRemes destination. Unsupported auth blobs must still be reported as skipped.
...
- SanRemes persona state imported
- SanRemes memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.sanremes/skills/openclaw-imports/`
