...
```
SanRemes Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${SANREMES_HOME:-$HOME/.sanremes}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to SanRemes config (if missing)
...
3. **Restart SanRemes session** to pick up the new MCP server
