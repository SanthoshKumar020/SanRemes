---
title: "SanRemes Agent Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
sidebar_label: "SanRemes Agent Skill Authoring"
description: "Author in-repo SKILL.md files: frontmatter and structure"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# SanRemes Agent Skill Authoring
...
| Path | `skills/software-development/sanremes-agent-skill-authoring` |
| Version | `2.0.0` |
| Author | SanRemes Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `skills`, `authoring`, `sanremes-agent`, `conventions`, `skill-md` |
...
# Authoring SanRemes-Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.sanremes/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the sanremes-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with sanremes-agent
...
- Don't use for: personal skills in `~/.sanremes/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `sanremes skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), SanRemes Agent
license: MIT
...
- Credit the **human first**, then "SanRemes Agent" as secondary collaborator: `Ben Barclay (benbarclay), SanRemes Agent`.
- Never `author: SanRemes Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), SanRemes Agent`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.sanremes/skills/`.
...
|---|---|
| SanRemes tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.sanremes/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: SanRemes Agent` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.sanremes.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through SanRemes tools
