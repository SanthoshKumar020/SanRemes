...
- A long-running process (sanremes gateway, tui_gateway) misbehaves and you can't restart it
...
The sanremes test runner and pytest both support this:
...
For long-lived processes: SanRemes gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source <sanremes-agent-repo>/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside SanRemes you have two practical options:
...
```json
{
  "name": "Attach to SanRemes",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "<sanremes-agent-repo>" }
  ]
}
```
...
## Debugging SanRemes-specific Processes

### Tests
See Recipe 3. The wrapper captures subprocess output, so run pytest directly for interactive pdb.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `sanremes` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `sanremes --tui`)
...
debugpy.wait_for_client()
```
Start `sanremes --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For SanRemes subagents, debug one process at a time.
