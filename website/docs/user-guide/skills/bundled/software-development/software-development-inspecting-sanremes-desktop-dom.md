---
title: "Inspecting SanRemes Desktop Dom — Read the live SanRemes desktop DOM/CSS over CDP"
sidebar_label: "Inspecting SanRemes Desktop Dom"
description: "Read the live SanRemes desktop DOM/CSS over CDP"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Inspecting SanRemes Desktop Dom

Read the live SanRemes desktop DOM/CSS over CDP.
...
| Path | `skills/software-development/inspecting-sanremes-desktop-dom` |
| Version | `1.0.0` |
...
# Inspecting the live SanRemes desktop DOM
...
`debugging-sanremes-desktop`), or anything where the real question is "does this
look right".
...
- **no `SANREMES_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`SANREMES_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${SANREMES_DESKTOP_CDP_PORT:-9222}/json/version
```
...
```bash
cd apps/desktop
SANREMES_HOME=/tmp/cdp-probe-home \
SANREMES_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
SANREMES_DESKTOP_CDP_PORT=9333 \
  npx electron . --user-data-dir=/tmp/cdp-probe-userdata
```

The separate `--user-data-dir` dodges Electron's single-instance lock, so it
cannot collide with a running `hgui`; the separate `SANREMES_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `SANREMES_HOME` baked in, if you
...
- **A throwaway `SANREMES_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `sanremes:api` and may exit on its own. The renderer still mounts and the DOM is
