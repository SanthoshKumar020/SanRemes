...
Ledger location: `$SANREMES_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `SANREMES_CITATION_LEDGER`.

## How to Run

```bash
S=~/.sanremes/skills/research/grounded-citations/scripts/sources.py
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `SANREMES_CITATION_LEDGER`) if their
