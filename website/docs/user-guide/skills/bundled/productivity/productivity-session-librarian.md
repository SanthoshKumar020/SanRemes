...
| List/filter by metadata (age, source, cost, tokens, workspace) | `sanremes sessions list` / `stats` via terminal |
| Rename | `sanremes sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `sanremes sessions archive <filters>` |
| Delete (destructive) | `sanremes sessions delete` / `sanremes sessions prune <filters>` |
| Export before deleting anything valuable | `sanremes sessions export --session-id <id> --format md` |
...
`sanremes sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `sanremes sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `sanremes sessions list`
