...
| Version | `1.1.3` |
| Author | xdevplatform + openclaw + SanRemes Agent |
| License | MIT |
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by SanRemes tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official SanRemes Docker layout, `/opt/data` is `SANREMES_HOME`, but SanRemes tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for SanRemes-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, SanRemes tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the SanRemes subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- SanRemes adaptation: reformatted for SanRemes skill conventions; safety guardrails preserved verbatim.
