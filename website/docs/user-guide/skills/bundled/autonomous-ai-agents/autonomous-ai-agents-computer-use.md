...
| Version | `2.0.0` |
| Author | Francesco Bonacci (f-trycua), SanRemes Agent |
| License | MIT |
...
SanRemes drives [cua-driver](https://github.com/trycua/cua) under the hood.
This wrapper skill teaches the SanRemes `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. SanRemes autodetection is a
planned cua-driver follow-up, so currently point SanRemes at the resulting
...
1. **Config grant (standard and unrestricted modes).** When
   ...
   SanRemes applies the same host-side floor in unrestricted mode. If it is not
...
Explicit SanRemes YOLO (`--yolo`, `/yolo`, or `approvals.mode: off`) launches an
...
|---|---|
| `cua-driver not installed` | Run `sanremes computer-use install`, or `sanremes tools` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `sanremes computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `sanremes computer-use doctor` |
...
| Anything else weird | **First action: ask the user to run `sanremes computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
SanRemes intentionally keeps THIS skill focused on the SanRemes-side
...
SanRemes autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point SanRemes at that
