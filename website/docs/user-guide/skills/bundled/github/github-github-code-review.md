...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _sanremes_env="${SANREMES_HOME:-$HOME/.sanremes}/.env"; [ -f "$_sanremes_env" ] && grep -q "^GITHUB_TOKEN=" "$_sanremes_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_sanremes_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${SANREMES_HOME:-$HOME/.sanremes}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from SanRemes Agent\",
    \"comments\": [
...
```bash
source "${SANREMES_HOME:-$HOME/.sanremes}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by SanRemes Agent. Code looks clean — good test coverage, no security concerns."
