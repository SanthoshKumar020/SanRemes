...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.sanremes/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. SanRemes stores that single token in `~/.sanremes/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `sanremes` (or the gateway, or a cron job) starts, after `~/.sanremes/.env` has loaded, SanRemes calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default SanRemes **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every SanRemes process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.

The `bws` binary is auto-downloaded into `~/.sanremes/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
2. Create or pick a **Project** (e.g. "SanRemes keys").
...
4. **Machine accounts → New machine account → My SanRemes machine** → **Projects** tab → grant Read access to your project.
...
```bash
sanremes secrets bitwarden setup
```

It will:

1. Download and verify `bws v2.0.0` into `~/.sanremes/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.sanremes/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
sanremes secrets bitwarden status
```

From now on, every `sanremes` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `sanremes secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `sanremes secrets bitwarden status` | Show config + binary version + token presence/validation |
| `sanremes secrets bitwarden token` | Rotate the access token: validate the new token against Bitwarden, then store it in `.env` |
| `sanremes secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `sanremes secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `sanremes secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `sanremes secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
Bitwarden Secrets Manager: → Run `sanremes secrets bitwarden token` to paste a fresh access token ...
```

Fix it without re-running the whole wizard:

```bash
sanremes secrets bitwarden token                     # masked prompt
sanremes secrets bitwarden token --access-token 0.…  # non-interactive
```
...
| `encrypted_cache.enabled` | `false` | Store the last successful fetch in an AES-GCM encrypted cache at `~/.sanremes/cache/bws_cache.enc.json`. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.sanremes/bin/` on first use. |

## Failure modes

Bitwarden never blocks SanRemes startup. If anything goes wrong, you'll see a one-line warning in stderr and SanRemes continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `sanremes secrets bitwarden setup` |
| `Bitwarden rejected the machine-account access token … invalid_client` | Token revoked, expired, machine account deleted — or the token belongs to another region (e.g. EU token hitting the US identity endpoint) | Run `sanremes secrets bitwarden token` to paste a fresh token; for region mismatches re-run setup and pick EU/self-hosted (or set `secrets.bitwarden.server_url`) |
| `bws exited 1: invalid access token` | Token revoked or wrong | Run `sanremes secrets bitwarden token` with a new token |
...
- SanRemes will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — SanRemes does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.sanremes/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple SanRemes installations.
