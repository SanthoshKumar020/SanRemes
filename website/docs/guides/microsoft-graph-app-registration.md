...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.sanremes/.env` at the end.
...
4. Fill in:
   - **Name:** `SanRemes Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `sanremes-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.sanremes/.env`:
...
SanRemes ships a Graph auth smoke-test. From your SanRemes install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.sanremes/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `sanremes gateway restart`.
