...
This tutorial walks you through setting up a Telegram bot powered by SanRemes Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **SanRemes Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.sanremes/.env`

:::tip
A $5/month VPS is plenty for running the gateway. SanRemes itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team SanRemes Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_sanremes_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by SanRemes Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.sanremes/.env`:
...
```
[Gateway] Starting SanRemes Gateway...
...
# View live logs
journalctl --user -u sanremes-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `sanremes gateway install` to update the plist.
:::
...
```bash
sanremes gateway stop && sanremes gateway start
```
...
sanremes pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.sanremes/.env`:
...
Control how much detail the bot shows when using tools. In `~/.sanremes/config.yaml`:
...
Customize how the bot communicates by editing `~/.sanremes/SOUL.md`:

For a full guide, see [Use SOUL.md with SanRemes](/guides/use-soul-with-sanremes).
...
```bash
# From the CLI
sanremes cron list          # View all scheduled jobs
sanremes cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u sanremes-gateway` (Linux) or `~/.sanremes/logs/gateway.log` (macOS) |
| Cron job output | `~/.sanremes/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.sanremes/cron/jobs.json` |
| Pairing data | `~/.sanremes/pairing/` |
| Session history | `~/.sanremes/sessions/` |
