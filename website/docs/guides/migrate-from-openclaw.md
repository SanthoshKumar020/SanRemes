...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to SanRemes Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`sanremes claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into SanRemes. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::note
Coming from **Claude Code** or **OpenAI Codex CLI** instead? Use [`sanremes import-agent`](../user-guide/import-from-other-agents.md).
:::

:::tip
If your OpenClaw setup was multi-provider, `sanremes setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.sanremes/` (by default a single restore-point archive is written before apply, under `~/.sanremes/backups/pre-migration-*.zip`; restorable with `sanremes import`). |
...
| What | OpenClaw source | SanRemes destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.sanremes/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.sanremes/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.sanremes/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.sanremes/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | SanRemes destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.sanremes/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.sanremes/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.sanremes/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.sanremes/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing SanRemes skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | SanRemes destination | Notes |
|------|---------------------|-------------------|-------|
| Default model | `agents.defaults.model` | `config.yaml` → `model` | Can be a string or `{primary, fallbacks}` object |
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `sanremes update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
| Provider API keys | `models.providers.*.apiKey` | `~/.sanremes/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | SanRemes config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | SanRemes config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | SanRemes field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.sanremes/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | SanRemes `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | SanRemes path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `SANREMES_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct SanRemes equivalent)

These are saved to `~/.sanremes/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in SanRemes |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | SanRemes has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `sanremes cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `sanremes webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `sanremes honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `sanremes skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use SanRemes profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to SanRemes manually via `sanremes config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.sanremes/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `sanremes status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart sanremes-gateway`

6. **Check session policies** — run `sanremes config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `sanremes whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `sanremes claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `sanremes config set`.

### Skills not appearing after migration

Imported skills land in `~/.sanremes/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `sanremes config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
