---
sidebar_position: 5
title: "Using SanRemes as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines — no CLI required"
---

# Using SanRemes as a Python Library

SanRemes isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.

---

## Installation

Clone SanRemes and create its supported editable development environment:
...
Run your application with `uv run python your_app.py` from that checkout. SanRemes does not publish a supported wheel or source distribution for `requirements.txt` installs.

:::tip
The same environment variables used by the CLI are required when using SanRemes as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use SanRemes is the `chat()` method — pass a message, get a string back:
...
:::warning
Always set `quiet_mode=True` when embedding SanRemes in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
For running many prompts in parallel, SanRemes includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
...
async def on_message(message):
    if message.author == client.user:
        return
    if message.content.startswith("!sanremes "):
...
|-----------|------|---------|-------------|
| `model` | `str` | `""` | Model in OpenRouter format (defaults to empty; resolved from your sanremes config at runtime) |
