...
- **`google-auth`** — installed automatically the first time you select Vertex (lazy install). Run `sanremes setup` to repair a managed install if that fails.
...
- The **credential path** is a pointer to a secret and lives in `~/.sanremes/.env`.
- **Project ID and region** are non-secret routing settings and live in `~/.sanremes/config.yaml`.
...
1. SanRemes resolves credentials in this order: `VERTEX_CREDENTIALS_PATH` → `GOOGLE_APPLICATION_CREDENTIALS` → ADC.
...
4. If a session runs longer than the token lifetime and a request returns `401`, SanRemes re-mints the token and retries automatically. On a long-running gateway, if ADC's refresh token has itself expired, SanRemes falls back to the service-account JSON when one is configured.

## Available Models

Vertex requires the `google/` vendor prefix on model IDs. The `sanremes model` picker offers:
...
`/model` switches among already-configured providers and models; it does not collect new credentials. Configure Vertex with `sanremes model` first.

## Reasoning / Thinking

Vertex exposes Gemini's thinking budget through the OpenAI-compatible surface. SanRemes maps its reasoning-effort setting onto `extra_body.google.thinking_config` automatically, so `reasoning_effort` works the same way it does on other Gemini surfaces.
...
SanRemes found neither a service-account JSON nor working ADC. Either set `VERTEX_CREDENTIALS_PATH` in `~/.sanremes/.env`, or run `gcloud auth application-default login`. If your project isn't embedded in the credentials, set `vertex.project_id` in `config.yaml`.

### `google-auth` not installed

SanRemes lazy-installs it the first time you select the Vertex provider. If that fails, run `sanremes setup` to repair the managed install.
