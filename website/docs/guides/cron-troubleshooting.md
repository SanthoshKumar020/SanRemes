...
description: "Diagnose and fix common SanRemes cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`sanremes gateway` for foreground, or `sanremes gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `sanremes cron tick`.
...
```bash
date
sanremes cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.sanremes/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.sanremes/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.sanremes/.env` |
...
| `local` | Write access to `~/.sanremes/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `sanremes cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (SanRemes Cloud / Team Gateway)
...
```bash
sanremes skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `sanremes skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `sanremes skills list`.
...
2. `~/.sanremes/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `sanremes cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the SanRemes config directory). Verify:
```bash
ls ~/.sanremes/scripts/your-script.py   # Must exist
sanremes cron edit <job_id> --script ~/.sanremes/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `sanremes skills install <skill-name>`.
...
```bash
ps aux | grep sanremes
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.sanremes/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.sanremes/cron/jobs.json
chmod 600 ~/.sanremes/cron/jobs.json   # Your user should own it
```
...
```bash
sanremes cron list                    # Show all jobs, states, next_run times
sanremes cron run <job_id>            # Schedule for next tick (for testing)
sanremes cron edit <job_id>           # Fix configuration issues
sanremes logs                         # View recent SanRemes logs
sanremes skills list                  # Verify installed skills
```
...
1. Run the job with `sanremes cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.sanremes/logs/agent.log` for scheduler messages and `~/.sanremes/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/sanremes-agent](https://github.com/NousResearch/sanremes-agent) with:
