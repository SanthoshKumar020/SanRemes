...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in SanRemes Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

SanRemes Agent supports xAI Grok through a browser-based OAuth device-code login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and SanRemes automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in SanRemes — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → SanRemes opens or prints an accounts.x.ai verification URL
...
After the first login, credentials are stored under `~/.sanremes/auth.json` and refreshed automatically before they expire.
...
```bash
sanremes auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, browser-only consoles (Cloud Shell, Codespaces, EC2 Instance Connect), or SSH sessions where SanRemes cannot open a browser locally, SanRemes prints the xAI verification URL and user code. Open the URL in any browser on your laptop or in the cloud console, enter the code if prompted, and SanRemes will keep polling until xAI approves the login. No SSH tunnel or local callback listener is required.

```bash
sanremes auth add xai-oauth --no-browser
# Open the printed verification URL in your browser.
```

The same device-code flow applies when you sign in from the web dashboard or the desktop app: SanRemes shows the verification URL and user code, then polls in the background until you approve access.

## How the Login Works

1. SanRemes requests a device code from `auth.x.ai`.
2. You open the verification URL, sign in, enter the displayed code if prompted, and approve access.
3. SanRemes polls xAI until approval, then saves tokens to `~/.sanremes/auth.json`.
4. From then on, SanRemes refreshes the access token in the background — you stay signed in until you `sanremes auth logout xai-oauth` or revoke access from your xAI account settings.
...
```bash
sanremes config set model.default grok-4.6
sanremes config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.sanremes/config.yaml` will contain:
...
```bash
sanremes --provider xai-oauth        # canonical
sanremes --provider grok-oauth       # alias
sanremes --provider x-ai-oauth       # alias
sanremes --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `sanremes tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search auto-enables when xAI credentials are present
The `x_search` toolset auto-enables whenever xAI credentials (a SuperGrok / X Premium+ OAuth token or `XAI_API_KEY`) are configured. Disable explicitly via `sanremes tools` → `🐦 X (Twitter) Search` (press space) if you don't want this. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok / X Premium+ OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
To select xAI as the active provider, set `model.provider: xai-oauth` in `config.yaml` (use `sanremes setup` for the guided flow) or pass `--provider xai-oauth` for a single invocation.

## Troubleshooting

### Token expired — not re-logging in automatically

SanRemes refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), SanRemes surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), SanRemes marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `sanremes auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

Device-code approval has a finite expiry window (xAI sets `expires_in` on the device-code response, typically on the order of tens of minutes). If you do not approve the login in time, SanRemes raises a timeout error.

**Fix:** re-run `sanremes auth add xai-oauth` (or `sanremes model`). The flow starts fresh.

### Logging in from a remote server

On SSH or container sessions SanRemes prints the verification URL and user code instead of opening a browser. Open that URL in a browser on your laptop or in a cloud console — no SSH port forward is needed for xAI Grok OAuth.
...
This is **not** a stale-token problem — re-running `sanremes model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/sanremes-agent/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
sanremes config set model.provider xai
```
...
**Fix:** run `sanremes model` and pick the xAI Grok OAuth provider, or run `sanremes auth add xai-oauth`.
...
```bash
sanremes auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `sanremes auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `sanremes auth list xai-oauth` to see them).
