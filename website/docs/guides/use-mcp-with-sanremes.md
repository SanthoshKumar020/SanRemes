---
sidebar_position: 6
title: "Use MCP with SanRemes"
description: "A practical guide to connecting MCP servers to SanRemes Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with SanRemes

This guide shows how to actually use MCP with SanRemes Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native SanRemes tool
- you want SanRemes to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect SanRemes to internal APIs, databases, or company systems without modifying SanRemes core

Do not use MCP when:
- a built-in SanRemes tool already solves the job well
...
- SanRemes remains the agent
- MCP servers contribute tools
- SanRemes discovers those tools at startup or reload time
...
If you installed SanRemes with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- SanRemes banner/status should show MCP integration when configured
- ask SanRemes what tools it has available
...
## WSL2: bridge SanRemes in WSL to Windows Chrome

This is the practical setup when:

- SanRemes runs inside WSL2
...
In this setup, SanRemes does **not** connect to Chrome directly. Instead:

- SanRemes runs in WSL
- SanRemes starts a local stdio MCP server
...
```text
SanRemes (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- SanRemes stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on SanRemes core browser transport
...
```bash
sanremes mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
sanremes mcp test chrome-devtools-win
```

Then start a fresh SanRemes session or run:
...
Once loaded, SanRemes can use the MCP-prefixed browser tools directly. For example:
...
If SanRemes runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start SanRemes from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start SanRemes from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in SanRemes:
...
2. SanRemes-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So SanRemes will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want SanRemes to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want SanRemes to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. SanRemes remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
sanremes mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
sanremes mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `sanremes mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want SanRemes to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize SanRemes to merge, publish, deploy, or spawn runtimes.
...
Now SanRemes can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing SanRemes core.
...
Because SanRemes now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
