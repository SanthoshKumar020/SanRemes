...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `sanremes send`."
---

# Pipe Script Output to Messaging Platforms

`sanremes send` is a small, scriptable CLI that pushes a message to any
messaging platform SanRemes is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | sanremes send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `sanremes
...
sanremes send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
sanremes send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
sanremes send --list

# Filter by platform
sanremes send --list telegram
```
...
Any platform SanRemes ships adapters for works as a target:
...
`sanremes send` resolves the message body in this order:

1. **Positional argument** — `sanremes send --to telegram "hi"`
2. **`--file PATH`** — `sanremes send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | sanremes send --to telegram`

When stdin is a TTY (no pipe), SanRemes does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  sanremes send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `sanremes send` reuses your SanRemes config, the same script works on
any host where SanRemes is installed — no need to export bot tokens into
...
`sanremes send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  sanremes send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | sanremes send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.sanremes/bin/sanremes send \
...
```bash
./train.py --epochs 200 && \
  sanremes send --to telegram "training done" || \
  sanremes send --to telegram "training failed (exit $?)"
```
...
sanremes send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(sanremes send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `sanremes send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `sanremes send` calls
the platform's REST endpoint directly using credentials from
`~/.sanremes/.env` and `~/.sanremes/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `sanremes gateway start` and retry.
...
# Machine-readable
sanremes send --list --json
```

The listing is built from `~/.sanremes/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`sanremes gateway
...
| Approach | Multi-platform | Reuses SanRemes creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `sanremes send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`sanremes send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `sanremes send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `sanremes send` shares with cron delivery.
