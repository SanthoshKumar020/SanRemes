...
description: "Practical advice to get the most out of SanRemes Agent — prompt tips, CLI shortcuts, context files, memory, cost optimization, and security"
---

# Tips & Best Practices

A quick-wins collection of practical tips that make you immediately more effective with SanRemes Agent. Each section targets a different aspect — scan the headers and jump to what's relevant.

:::tip Confused which model to pick?
Run `sanremes setup --portal` — you get 300+ models including Claude, GPT-5, and Gemini under one subscription. See [Nous Portal](/integrations/nous-portal).
:::
...
Forgot something from your last session? Run `sanremes -c` to resume exactly where you left off, with full conversation history restored. You can also resume by title: `sanremes -r "my research project"`.
...
Want SanRemes to have a stable default voice? Edit `~/.sanremes/SOUL.md` (or `$SANREMES_HOME/SOUL.md` if you use a custom SanRemes home). SanRemes now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.

For a full walkthrough, see [Use SOUL.md with SanRemes](/guides/use-soul-with-sanremes).
...
Already have a `.cursorrules` or `.cursor/rules/*.mdc` file? SanRemes reads those too. No need to duplicate your coding conventions — they're loaded automatically from the working directory.

### Discovery

SanRemes loads the top-level `AGENTS.md` from the current working directory at session start. Subdirectory `AGENTS.md` files are discovered lazily during tool calls (via `subdirectory_hints.py`) and injected into tool results — they are not loaded upfront into the system prompt.
...
:::tip
Run `/usage` periodically to see your token consumption. Run `/insights` for a broader view of usage patterns over the last 30 days. To see what your *fixed* per-message cost is before any conversation — system prompt, skills index, memory, tool schemas — run [`sanremes prompt-size`](/reference/cli-commands#sanremes-prompt-size) (works offline).
:::
...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `sanremes sessions list` and resume with `sanremes -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.

### DM Pairing for Team Access

Instead of manually collecting user IDs for allowlists, enable DM pairing. When a teammate DMs the bot, they get a one-time pairing code. You approve it with `sanremes pairing approve telegram XKGH5N7P` — simple and secure.
...
:::tip
By default, messaging sessions never auto-reset — context lives until you `/reset` or compression kicks in. If you want sessions to reset automatically (after idle time or daily at a fixed hour), opt in via the `session_reset` section in `~/.sanremes/config.yaml`.
:::
...
# In your .env:
TERMINAL_ENV=docker
TERMINAL_DOCKER_IMAGE=sanremes-sandbox:latest
```
...
SanRemes checks every command against a curated list of dangerous patterns before execution. This includes recursive deletes, SQL drops, piping curl to shell, and more. Don't disable this in production — it exists for good reasons.
