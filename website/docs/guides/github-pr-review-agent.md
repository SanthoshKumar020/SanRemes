...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  SanRemes Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to SanRemes instantly when PRs are opened or updated.
:::
...
- **SanRemes Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  sanremes gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.sanremes/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure SanRemes can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/sanremes-agent --state open --limit 3
```
...
Still in the chat, ask SanRemes to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/sanremes-agent
```
...
A skill gives SanRemes consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.sanremes/skills/code-review
```

Create `~/.sanremes/skills/code-review/SKILL.md`:
...
Verify it loaded — start `sanremes` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach SanRemes your team's standards:
...
2. Teach SanRemes your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
sanremes gateway status    # Is the gateway running?
sanremes cron list         # Is the job enabled?
```
