...
description: "Find, install, use, and create skills — on-demand knowledge that teaches SanRemes new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach SanRemes how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every SanRemes installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask SanRemes to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with SanRemes but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
sanremes skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.sanremes/skills/`
...
```bash
# Check it's there
sanremes skills list | grep arxiv
...
For how to ship skills in your own plugin, see [Build a SanRemes Plugin → Bundle skills](/developer-guide/plugins#bundle-skills).
...
When a skill with config is first loaded, SanRemes prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
sanremes skills config gif-search

# View all skill config
sanremes config get skills.config --json
```
...
```bash
mkdir -p ~/.sanremes/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.sanremes/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
sanremes chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.sanremes/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, SanRemes may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, SanRemes will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.sanremes/skills/devops/`, `~/.sanremes/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell SanRemes to update the skill with what you learned. Skills that aren't maintained become liabilities.
