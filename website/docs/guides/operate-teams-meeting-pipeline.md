...
```bash
sanremes teams-pipeline validate
```
...
```bash
sanremes teams-pipeline token-health
sanremes teams-pipeline token-health --force-refresh
```
...
```bash
sanremes teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
sanremes teams-pipeline maintain-subscriptions
sanremes teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: SanRemes cron (recommended if you already run the SanRemes gateway)

SanRemes ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.sanremes/scripts/`. First create the script:

```bash
mkdir -p ~/.sanremes/scripts
cat > ~/.sanremes/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec sanremes teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.sanremes/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
sanremes cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/sanremes-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=SanRemes Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=sanremes
EnvironmentFile=/etc/sanremes/env
ExecStart=/usr/local/bin/sanremes teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/sanremes-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run SanRemes Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now sanremes-teams-pipeline-maintain.timer
systemctl list-timers sanremes-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/sanremes teams-pipeline maintain-subscriptions >> /var/log/sanremes/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.sanremes/.env` at the top of a wrapper script that crontab calls.
...
```bash
sanremes teams-pipeline subscriptions   # should show expirationDateTime advanced
sanremes teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
sanremes teams-pipeline list
sanremes teams-pipeline list --status failed
sanremes teams-pipeline show <job-id>
```

### Replay a stored job

```bash
sanremes teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
sanremes teams-pipeline fetch --meeting-id <meeting-id>
sanremes teams-pipeline fetch --join-web-url "<join-url>"
sanremes teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `sanremes teams-pipeline maintain-subscriptions --dry-run`
- inspect `sanremes teams-pipeline list --status failed`
...
- run `sanremes teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `sanremes teams-pipeline run`
...
- [ ] `sanremes teams-pipeline validate` returns an OK snapshot
- [ ] `sanremes teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (SanRemes cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
