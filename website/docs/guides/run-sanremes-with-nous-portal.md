...
3. Sets `model.provider: nous` in `~/.sanremes/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where SanRemes is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
sanremes setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
sanremes auth add nous --type oauth
# Then re-run `sanremes setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "SanRemes Agent release notes" and summarize the top 3 hits.
```

You should see SanRemes call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`sanremes setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
sanremes config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick SanRemes-4 for agent work

SanRemes-4-70B and SanRemes-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them for conversation/research work through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For SanRemes Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a SanRemes-side opinion.
...
These rows appear in `sanremes tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, SanRemes runs the Portal login inline (without changing your inference provider or your other tools).

Verify your mix with:

```bash
sanremes portal tools
```
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and SanRemes will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [SanRemes profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.sanremes/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `sanremes portal info` shows "not logged in" after `sanremes setup --portal`
...
```bash
sanremes config set model.provider nous
```
...
Re-verify with `sanremes portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so SanRemes doesn't replay it endlessly. Just log in again:

```bash
sanremes auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/NousResearch/sanremes-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`sanremes portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple SanRemes profiles where you're using the wrong one (check `sanremes profile list`)

### Want to revoke and start clean

```bash
sanremes auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-SanRemes tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple SanRemes configurations
