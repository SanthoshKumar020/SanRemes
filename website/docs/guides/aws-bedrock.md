...
sidebar_position: 14
title: "AWS Bedrock"
description: "Use SanRemes Agent with Amazon Bedrock — native Converse API, Anthropic SDK routing, OpenAI models via Bedrock Mantle, IAM authentication, Guardrails, and cross-region inference"
---

# AWS Bedrock

SanRemes Agent supports Amazon Bedrock as a native provider. This gives you full access to the Bedrock ecosystem: IAM authentication, Guardrails, cross-region inference profiles, and all foundation models.

SanRemes routes each model family through the API that serves it best:
...
- **boto3** — install with `cd ~/.sanremes/sanremes-agent && uv pip install -e ".[bedrock]"`
...
:::tip EC2 / ECS / Lambda
On AWS compute, attach an IAM role with `AmazonBedrockFullAccess` and you're done. No API keys, no `.env` configuration — SanRemes detects the instance role automatically.
:::
...
cd ~/.sanremes/sanremes-agent && uv pip install -e ".[bedrock]"
...
SanRemes auto-discovers available models via the Bedrock control plane. You can customize discovery:
...
SanRemes automatically applies prompt caching on the Bedrock **Converse API** path by inserting `cachePoint` markers after the system prompt, tool definitions, and the latest message. Because sending a `cachePoint` block to a model that doesn't support it raises a `ValidationException`, markers are only added for models on a known-good allowlist (Anthropic Claude and Amazon Nova model IDs); unknown models default to no cache markers. Claude models normally use the AnthropicBedrock SDK path, which has its own prompt caching — the Converse `cachePoint` path covers Nova and the bearer-token Claude fallback. No configuration needed; cache reads/writes show up in usage accounting.

### Context-window probing

For models whose context window isn't in SanRemes' static table, SanRemes can probe the real limit by sending oversized requests at fixed tiers (~1.3M and ~2.2M tokens) and parsing the `maximum` reported in Bedrock's length-validation error. Probed values feed the same metadata cache as the static table; stale cached entries that under-report a model's window (e.g. entries seeded before a model's 1M window went GA) are dropped automatically in favor of the larger known value.

## Available Models

Bedrock models use **inference profile IDs** for on-demand invocation. The `sanremes model` picker shows these automatically, with recommended models at the top:
...
Bedrock works with all SanRemes gateway platforms (Telegram, Discord, Slack, Feishu, etc.). Configure Bedrock as your provider, then start the gateway normally:
...
SanRemes checks for credentials in this order:
...
You've hit the Bedrock per-model rate limit. SanRemes automatically retries with backoff. To increase limits, request a quota increase in the [AWS Service Quotas console](https://console.aws.amazon.com/servicequotas/).

## One-Click AWS Deployment

For a fully automated deployment on EC2 with CloudFormation:

**[sample-sanremes-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-sanremes-agent-on-aws-with-bedrock)** — creates VPC, IAM role, EC2 instance, and configures Bedrock automatically. Deploy in any region with one click.
