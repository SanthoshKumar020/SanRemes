...
This page covers all commands related to [SanRemes profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `sanremes profile` without a subcommand shows help.
...
## `sanremes profile use`

```bash
sanremes profile use <name>
```

Sets `<name>` as the active profile. All subsequent `sanremes` commands (without `-p`) will use this profile.
...
sanremes profile use default
```

## `sanremes profile create`

```bash
sanremes profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `sanremes profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `sanremes update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.sanremes`), use `sanremes skills opt-out` / `sanremes skills opt-in`. |
...
sanremes profile create work2 --clone-from work

# Clone everything from a specific profile
sanremes profile create work2-backup --clone-from work --clone-all
```

## `sanremes profile describe`

```bash
sanremes profile describe [<name>] [options]
```
...
sanremes profile describe researcher

# Set it explicitly
sanremes profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
sanremes profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
sanremes profile describe --all --auto
```

## `sanremes profile delete`

```bash
sanremes profile delete <name> [options]
```
...
```bash
sanremes profile delete mybot
sanremes profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.sanremes`) cannot be deleted — use `sanremes uninstall` to remove everything.
:::

## `sanremes profile show`

```bash
sanremes profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's SanRemes home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ sanremes profile show work
Profile: work
Path:    ~/.sanremes/profiles/work
...
## `sanremes profile alias`

```bash
sanremes profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your SanRemes installation.
...
```bash
sanremes profile alias work
# Creates/updates ~/.local/bin/work

sanremes profile alias work --name mywork
# Creates ~/.local/bin/mywork

sanremes profile alias work --remove
# Removes the wrapper script
```

## `sanremes profile rename`

```bash
sanremes profile rename <old-name> <new-name>
```
...
```bash
sanremes profile rename mybot assistant
# ~/.sanremes/profiles/mybot → ~/.sanremes/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `sanremes profile export`

```bash
sanremes profile export <name> [options]
```
...
```bash
sanremes profile export work
# Creates work.tar.gz in the current directory

sanremes profile export work -o ./work-2026-03-29.tar.gz
```
...
## `sanremes profile import`

```bash
sanremes profile import <archive> [options]
```
...
```bash
sanremes profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

sanremes profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `sanremes -p telemetry`). |
...
sanremes profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
sanremes profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
sanremes profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
sanremes profile install ./telemetry/
```

### `sanremes profile update`

```bash
sanremes profile update <name> [--force-config] [--yes]
```
...
### `sanremes profile info`

```bash
sanremes profile info <name>
```

Prints the profile's distribution manifest — name, version, required
SanRemes version, author, env var requirements, the source URL/path, and
...
`sanremes profile list` also shows the distribution name and version in a
`Distribution` column, and `sanremes profile show <name>` / `delete <name>`
...
sanremes profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
sanremes profile install https://github.com/your-org/internal-assistant.git
```
...
`sanremes_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
SanRemes version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host SanRemes can clone from.
3. Tell recipients to run `sanremes profile install <your-repo-url>`.
...
## `sanremes -p` / `sanremes --profile`

```bash
sanremes -p <name> <command> [options]
sanremes --profile <name> <command> [options]
```

Global flag to run any SanRemes command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
sanremes -p work chat -q "Check the server status"
sanremes --profile dev gateway start
sanremes -p personal skills list
sanremes -p work config edit
```

## `sanremes completion`

```bash
sanremes completion <shell>
```
...
- `sanremes profile <TAB>` — subcommands (list, use, create, etc.)
- `sanremes profile use <TAB>` — profile names
- `sanremes -p <TAB>` — profile names
