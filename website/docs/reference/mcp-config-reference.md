...
description: "Reference for SanRemes Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Values resolve from the active profile's secret scope (falling back to the process environment), so put the secret in `~/.sanremes/.env`. An unset variable keeps its literal placeholder.
...
SanRemes may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, SanRemes only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, SanRemes does not create an empty MCP runtime toolset for that server.
...
Behavior:
- SanRemes uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, client identification, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.sanremes/mcp-tokens/<server>.json` and reused across sessions
- Token refresh is automatic; re-authorization only happens when refresh fails
- Only applies to HTTP/StreamableHTTP transport (`url`-based servers)

### Client identification: CIMD and DCR

SanRemes identifies itself to authorization servers with a **Client ID Metadata Document** (CIMD), the mechanism the MCP `2026-07-28` spec adopted in place of Dynamic Client Registration. The document is published at
`https://nousresearch.github.io/sanremes-agent/docs/oauth/client-metadata.json`, and that URL *is* the `client_id` — the authorization server fetches it to learn SanRemes' name, logo, and permitted redirect URIs. Nothing is registered per install, and nothing is user-specific.
...
The document declares a fixed set of loopback redirect URIs, and the spec requires the redirect URI in an authorization request to be an *exact string match* against one of them — so a CIMD flow cannot use the random high port SanRemes normally picks. SanRemes therefore pins the callback to one of ports `27890`–`27894`.

That pin has to be chosen before the server's capabilities are known, because the redirect URI is fixed at the start of the flow while the server's metadata only arrives partway through. So SanRemes pins the port for any flow that *could* end up using CIMD, and reverts to a random port for the rest:

- A server SanRemes has connected to before, whose cached metadata does not advertise CIMD, keeps the random port it has always used.
- A server SanRemes has never reached gets a pinned port on that first login, since guessing is the only way CIMD can ever be used.
...
If a server fetches the document and refuses it at the *token* endpoint (`invalid_client`), SanRemes logs the rejection, records it under `~/.sanremes/mcp-tokens/<server>.cimd-off`, and uses DCR for that server from then on.

A server that cannot fetch or validate the document at all aborts at the *authorization* endpoint instead, before any redirect happens. There is no signal SanRemes can observe there, so the browser shows an invalid-client error and the login times out after five minutes. The timeout message names the document and points at `cimd: false`. Running `sanremes mcp login <server>` clears the recorded rejection, so a corrected document gets another chance.
...
## Add to SanRemes link

MCP vendors and docs can offer a one-click **"Add to SanRemes"** button that opens the SanRemes desktop app with a pre-filled server config, mirroring Cursor's `cursor://anysphere.cursor-deeplink/mcp/install` scheme:

```text
sanremes://mcp/install?name=NAME&config=BASE64
```
...
const link = `sanremes://mcp/install?name=example&config=${btoa(JSON.stringify(config))
