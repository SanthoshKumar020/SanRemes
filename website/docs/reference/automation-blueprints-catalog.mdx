...
of fields, and SanRemes schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and SanRemes asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.sanremes.blueprint` block in its
