...
description: "Frequently asked questions and solutions to common issues with SanRemes Agent"
---
...
### What LLM providers work with SanRemes?

SanRemes Agent works with any OpenAI-compatible API. Supported providers include:
...
- **Anthropic** — Claude models (direct API, OAuth via `sanremes auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `sanremes model` or by editing `~/.sanremes/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run SanRemes in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run SanRemes inside WSL2
...
- let SanRemes use the resulting MCP browser tools

This is more reliable than trying to force SanRemes core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with SanRemes](../guides/use-mcp-with-sanremes.md#wsl2-bridge-sanremes-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). SanRemes Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.sanremes/`.

### Can I use it offline / with local models?

Yes. Run `sanremes model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← SanRemes minimum; set this to match your server's actual context window
```
...
SanRemes persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in SanRemes — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
SanRemes auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `SANREMES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

SanRemes Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same SanRemes Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use SanRemes programmatically:
...
#### `sanremes: command not found` after installation
...
**Cause:** SanRemes requires Python 3.11 or newer.
...
**Cause:** SanRemes builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when SanRemes runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** SanRemes auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.sanremes/config.yaml`:
...
sudo rm /usr/local/bin/sanremes
...
#### The agent says "SanRemes policy" or "SanRemes guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden SanRemes runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

SanRemes enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. SanRemes does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual SanRemes tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
3. Retry in a fresh session with another configured model or provider. A refusal that changes with the model is model/provider behavior, not a SanRemes execution control.
4. If an explicit tool error appears, use its exact text when reporting the problem.

See [Security](/user-guide/security) for SanRemes' documented execution controls and [Providers](/integrations/providers) for provider configuration.
...
**Solution:** Exit your session and use `sanremes model` from your terminal to add new providers:

```bash
# Exit the SanRemes chat session first (Ctrl+C or /quit)
...
After adding a new provider via `sanremes model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `sanremes model` (from terminal) |
| Enter/change API keys | `sanremes model` (from terminal) |
...
sanremes config show

# Re-configure your provider
sanremes model

# Or set directly
sanremes config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.sanremes/.env` for conflicting entries.
:::
...
# Set a valid model
sanremes config set SANREMES_MODEL anthropic/claude-opus-4.7

# Or specify per-session
sanremes chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `sanremes chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or SanRemes detected the wrong context length for your model.
...
sanremes chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, SanRemes may have the wrong context length for your model. Check what it detected:
...
**Cause:** SanRemes detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — SanRemes never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `sanremes chat`
...
cat ~/.sanremes/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `sanremes gateway setup`
- Check gateway logs: `cat ~/.sanremes/logs/gateway.log | tail -50`
...
Configure in `~/.sanremes/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.sanremes/sanremes-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `sanremes gateway start` fails
...
tmux new -s sanremes 'sanremes gateway run'
# Reattach later: tmux attach -t sanremes

# Option 3: Background via nohup
nohup sanremes gateway run > ~/.sanremes/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'sanremes gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `sanremes gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
sanremes gateway install    # Re-snapshots your current PATH
sanremes gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.sanremes.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `sanremes chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `sanremes chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`sanremes prompt-size`](/reference/cli-commands#sanremes-prompt-size) to measure it, then trim: disable toolsets you don't use (`sanremes tools`) and uninstall or disable skills you don't need (`sanremes skills`).
...
cd ~/.sanremes/sanremes-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.sanremes/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
sanremes config show | grep -A 12 mcp_servers

# Restart SanRemes or reload MCP after config changes
sanremes chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with SanRemes](/guides/use-mcp-with-sanremes)
...
:::warning
If an MCP server crashes mid-request, SanRemes will report a timeout. Check the server's own logs (not just SanRemes logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `sanremes profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or SanRemes home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](/user-guide/features/memory-providers).

### What happens when I run `sanremes update`?

`sanremes update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `sanremes update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.sanremes/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** SanRemes can route subagents to a different model automatically. Set this in `~/.sanremes/config.yaml`:
...
Now when you tell SanRemes "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can SanRemes do this?

**Current limitation:** SanRemes profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, SanRemes reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `sanremes skills config` settings don't seem to take effect.

**Solution:** Use `sanremes skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`sanremes gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** SanRemes creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting SanRemes to another machine
...
1. Install SanRemes Agent on the new machine:
   ```bash
   curl -fsSL https://sanremes-agent.nousresearch.com/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   sanremes backup
   ```
   This creates a zip of your entire `~/.sanremes/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/sanremes-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/sanremes-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   sanremes import ~/sanremes-backup-<timestamp>.zip
   ```

4. On the new machine, run `sanremes setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
sanremes profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
sanremes profile import ./work-backup.tar.gz work
```
...
### `sanremes backup` vs `sanremes profile export`

| Feature | `sanremes backup` | `sanremes profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.sanremes` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='sanremes-agent' ~/.sanremes/ newmachine:~/.sanremes/
```

:::tip
`sanremes backup` produces a consistent snapshot even while SanRemes is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the SanRemes installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a SanRemes-specific issue — it's a shell config permissions problem.
...
sanremes config show | head -20

# Re-run model selection
sanremes model

# Or test with a known-good model
sanremes chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/sanremes-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), SanRemes version (`sanremes --version`), and the full error message
