...
description: "Authoritative reference for SanRemes built-in tools, grouped by toolset"
---

# Built-in Tools Reference

This page documents SanRemes' built-in tools, grouped by toolset. Availability varies by platform, credentials, and enabled toolsets.
...
:::tip MCP Tools
In addition to built-in tools, SanRemes can load tools dynamically from MCP servers. MCP tools appear with the prefix `mcp__<server>__` (e.g., `mcp__github__create_issue` for the `github` MCP server). See [MCP Integration](/user-guide/features/mcp) for configuration.
:::
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `execute_code` | Run a Python script that can call SanRemes tools programmatically. Use this when you need 3+ tool calls with processing logic between them, need to filter/reduce large tool outputs before they enter your context, need conditional branching (… | — |
...
Scoped to the Feishu document-comment intelligent-reply handler (`gateway/platforms/feishu_comment.py`). Not exposed on `sanremes-cli` or the regular Feishu chat adapter.
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `computer_use` | Background desktop control via cua-driver — screenshots (SOM / vision / AX), click / drag / scroll / type / key / wait, list_apps, focus_app. Does NOT steal the user's cursor or keyboard focus. Works with any tool-capable model. macOS, Windows, and Linux. | `cua-driver` on `$PATH` (install via `sanremes tools`). |
...
Registered when the agent is either (a) spawned by the kanban dispatcher (`SANREMES_KANBAN_TASK` env set) or (b) running in a profile that explicitly enables the `kanban` toolset. Task-scoped workers use lifecycle tools for their assigned task; orchestrator profiles additionally get board-routing tools like `kanban_list` and `kanban_unblock`. See [Kanban Multi-Agent](/user-guide/features/kanban) for the full workflow.

| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `kanban_show` | Show the active kanban task assigned to this worker (title, description, comments, dependencies). | `SANREMES_KANBAN_TASK` or `kanban` toolset |
...
| `kanban_complete` | Mark the current task done with a structured handoff payload (results, artifacts, follow-ups). | `SANREMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_block` | Block the current task on a question for the user — the dispatcher pauses, surfaces the question, and resumes once a human replies. | `SANREMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_request_review` | Hand the implementation to a reviewer with `summary`, optional structured `metadata`, and an optional reviewer profile. Moves the same task to `review`; it is not a block and does not affect block-loop accounting. | `SANREMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_request_changes` | Reviewer verdict for an actively claimed review run. Closes the review run, reapplies parent gating, and routes the task back to the original implementer without using a block. | `SANREMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_heartbeat` | Send a progress heartbeat during a long-running operation so the dispatcher knows the worker is still alive. | `SANREMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_comment` | Add a comment to the task thread without changing its state — useful for surfacing intermediate findings. | `SANREMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_create` | Fan out child tasks from the current task. Used by orchestrators and follow-up-spawning workers. | `SANREMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_link` | Link tasks with a parent → child dependency edge. | `SANREMES_KANBAN_TASK` or `kanban` toolset |
...
| `kanban_attach` | Attach a file to a task by passing its bytes inline (base64). Stored as a real attachment under the task's attachments dir, capped at 25 MB. | `SANREMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_attach_url` | Attach a file to a task by URL — SanRemes downloads it server-side and stores it as a real attachment (capped at 25 MB). Only http/https URLs. | `SANREMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_attachments` | List the files attached to a task: id, filename, content_type, size, uploader, and the absolute on-disk path. | `SANREMES_KANBAN_TASK` or `kanban` toolset |
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `skill_manage` | Manage skills (create, update, delete). Skills are your procedural memory — reusable approaches for recurring task types. New skills go to ~/.sanremes/skills/; existing skills can be modified wherever they live. Actions: create (full SKILL.m… | — |
...
Enabled for sessions whose source is the SanRemes desktop app, on any backend it
is connected to (local, SSH, URL, or SanRemes Cloud). Absent from CLI, TUI,
messaging, and cron sessions.

| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `read_terminal` | Read what's currently shown in the in-app terminal pane of the SanRemes desktop GUI (the embedded shell beside this chat). | — |
| `close_terminal` | Close the read-only terminal tab for a background process in the SanRemes desktop GUI. Does NOT kill the process — only drops the tab/view; use process(action='kill') to stop it. | — |
| `open_preview` | Open a web URL, localhost dev-server URL, or file path in the preview pane beside the chat in the SanRemes desktop app. | — |
...
| `read_preview` | Read what's currently shown in the preview pane of the SanRemes desktop GUI — the in-app Browser's page text (URL + title + rendered text, pageable with `start`/`count`), or a file/artifact tab's identity. | — |
...
| `read_window_below` | Identify the OS window directly underneath the SanRemes desktop window — app name, title, bounds (metadata only, never pixels). On macOS, other apps' titles appear only when Screen Recording is already granted; the tool never prompts for it. | — |
| `focus_pane` | Reveal and focus a pane in the SanRemes desktop app (chat, files, terminal, review, sessions). | — |
...
| `tour` | Give a live guided tour: dim the screen, highlight an element, and attach a narrated popover (driver.js). Works on the SanRemes app's own UI and on any page open in the preview pane; `targets` discovers what's on screen, `show` narrates step-by-step, `start` hands the user Next/Prev controls. | — |
...
idle. It does share the cooldown, so a tip from SanRemes also buys the user six
...
Opt-in toolset (not loaded in the default `sanremes-cli` set). Add via `--toolsets video` or include `video` in your `toolsets:` config.
...
Opt-in toolset (not loaded in the default `sanremes-cli` set). Add via `--toolsets video_gen` or enable it in `sanremes tools` → Video Generation, which also walks you through picking a backend.
...
Registered on the `sanremes-discord` platform toolset (gateway only). Uses the same bot token as the messaging adapter.
...
Registered on the `sanremes-discord` platform toolset. Moderation actions require the bot to hold the matching Discord permissions.
...
Registered by the bundled `spotify` plugin. Requires an OAuth token — run `sanremes auth spotify` once to authorize.
...
## `sanremes-yuanbao` toolset

Registered only on the `sanremes-yuanbao` platform toolset. Yuanbao is Tencent's chat app; these tools drive its DM/group/sticker APIs.
