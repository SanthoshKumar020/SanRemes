---
sidebar_position: 1
title: "SanRemes Agent Quickstart"
description: "Your first conversation with SanRemes Agent — from install to chatting in under 5 minutes"
---

# SanRemes Agent Quickstart

This guide gets you from zero to a working SanRemes setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [SanRemes Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="SanRemes Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up SanRemes for a team, bot, or always-on workflow
...
|---|---|---|
| I just want SanRemes working on my machine | `sanremes setup` | Run a real chat and verify it responds |
| I already know my provider | `sanremes model` | Save the config, then start chatting |
| I want a bot or always-on setup | `sanremes gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `sanremes model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `sanremes model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if SanRemes cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install SanRemes Agent
### With the SanRemes Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the SanRemes Desktop installer](https://sanremes-agent.nousresearch.com/) from our website and run it.

### Without SanRemes Desktop:
For a command-line only install without SanRemes Desktop, run:
...
The single most important setup step. Use `sanremes model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `sanremes setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `sanremes update`. Re-enable anything later with `sanremes tools`, seed skills with `sanremes skills opt-in --sync`, or tune settings with `sanremes setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `sanremes model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `sanremes model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `sanremes model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `sanremes_cli/models.py` may change between releases) | `sanremes model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `sanremes model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `sanremes model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `sanremes model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `sanremes model` (requires `copilot` CLI + `copilot login`) |
...
SanRemes Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `sanremes model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

SanRemes separates secrets from normal config:

- **Secrets and tokens** → `~/.sanremes/.env`
- **Non-secret settings** → `~/.sanremes/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
sanremes config set model anthropic/claude-opus-4.6
sanremes config set terminal.backend docker
sanremes config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
sanremes            # classic CLI
sanremes --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
SanRemes ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `sanremes` vs `sanremes --tui`.
:::
...
- SanRemes replies without error
...
```bash
sanremes --continue    # Resume the most recent session
sanremes -c            # Short form
```
...
```bash
sanremes gateway setup    # Interactive platform configuration
```
...
- `sanremes tools` — tune tool access per platform
- `sanremes skills` — browse and install reusable workflows
...
```bash
sanremes config set terminal.backend docker    # Docker isolation
sanremes config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `sanremes egress setup && sanremes egress start`; `sanremes setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the SanRemes install directory (the curl installer placed it at
# ~/.sanremes/sanremes-agent on Linux/macOS or %LOCALAPPDATA%\sanremes\sanremes-agent on Windows):
cd ~/.sanremes/sanremes-agent
...
Skills are on-demand instruction documents that teach SanRemes how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

SanRemes ships with a catalog of bundled skills already installed in `~/.sanremes/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
sanremes skills browse                      # list everything available
sanremes skills search kubernetes           # find skills by keyword
sanremes skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `sanremes skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let SanRemes ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.sanremes/sanremes-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| SanRemes opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `sanremes model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `sanremes gateway setup` and check `sanremes gateway status` |
| `sanremes --continue` can't find old session | Switched profiles or session never saved | Check `sanremes sessions list` and confirm you're in the right profile |
...
| `sanremes doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `sanremes model`
3. `sanremes setup`
4. `sanremes sessions list`
5. `sanremes --continue`
6. `sanremes gateway status`
...
| `sanremes tools` | Configure which tools are enabled per platform |
| `sanremes setup` | Full setup wizard (configures everything at once) |
| `sanremes doctor` | Diagnose issues |
| `sanremes update` | Update to latest version |
| `sanremes gateway` | Start the messaging gateway |
| `sanremes --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-sanremes-to-another-machine)** — `sanremes backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
