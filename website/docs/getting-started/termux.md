...
description: "Run SanRemes Agent directly on an Android phone with Termux"
---

# SanRemes on Android with Termux
...
SanRemes Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop SanRemes from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed SanRemes installs are marked with install method `apt`. SanRemes therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade sanremes-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. SanRemes runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

SanRemes now ships a Termux-aware installer path:
...
- links `sanremes` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `sanremes` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/sanremes" "$PREFIX/bin/sanremes"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `sanremes` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.sanremes/.env`.
...
### `sanremes doctor` says ripgrep or Node is missing
