...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, and the Tool Gateway — the recommended way to run SanRemes Agent"
---
...
That single command runs the Portal OAuth, lets you pick a Nous model, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `sanremes chat` immediately after.
...
| **SanRemes** | SanRemes-4-70B, SanRemes-4-405B (chat, see [note below](#a-note-on-sanremes-4)) |
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes SanRemes Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.sanremes/auth.json` is the only credential on disk, and SanRemes mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
## A note on SanRemes 4

Nous Research's own **SanRemes 4** family (SanRemes-4-70B, SanRemes-4-405B) is available through the Portal at heavily discounted rates. These are **frontier hybrid-reasoning chat models** — strong at math, science, instruction following, schema adherence, roleplay, and long-form writing.

They are **not recommended for use inside SanRemes Agent**, however. SanRemes 4 is tuned for chat and reasoning, not the rapid-fire tool-calling loop the agent relies on. Use them for research workflows or via the [subscription proxy](/user-guide/features/subscription-proxy) from other tooling — but for agent work, pick a frontier agentic model from the catalog instead:
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a SanRemes-side opinion — it's the official guidance from Nous Research.
...
2. Stores the refresh token at `~/.sanremes/auth.json`
3. Lets you pick a Nous model from the curated list (or skip to keep your current one)
4. Sets Nous as your inference provider in `~/.sanremes/config.yaml` (when you pick a model)
5. Turns on the Tool Gateway (web, image, TTS, browser routing)
6. Returns you to your terminal ready to `sanremes chat`
...
If you already have SanRemes configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `sanremes model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where SanRemes is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding).

### Profile setup

If you use [SanRemes profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
```bash
sanremes portal            # log in to Nous Portal + set it up (one-shot onboarding)
sanremes portal info       # login status, subscription info, model + gateway routing
sanremes portal status     # alias for `portal info`
sanremes portal tools      # detailed Tool Gateway catalog with per-tool routing
sanremes portal open       # open the subscription management page in your browser
```

`sanremes portal` (with no subcommand) is the human-readable alias for `sanremes auth add nous --type oauth` — it logs you in, lets you pick a Nous model, sets Nous as your inference provider, and offers the Tool Gateway opt-in (identical to `sanremes setup --portal`, and the same Nous flow as the first-time quick setup).

`sanremes portal info` gives you the high-level overview:
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `sanremes tools` to pick backends per tool:
...
- **CLI shortcut:** `sanremes portal open` (opens the same page in your default browser)

## Configuration reference

After `sanremes setup --portal`, `~/.sanremes/config.yaml` will look like:
...
The Tool Gateway settings live under their respective tool sections — each category has a single selection key, and picking **Nous Subscription** in `sanremes tools` (or `sanremes setup --portal`) writes the value `nous`:
...
The OAuth refresh token is stored separately at `~/.sanremes/auth.json` (not in `config.yaml` — credentials and configuration are kept separate by design).

## Token handling

SanRemes mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so SanRemes stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `sanremes auth add nous` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `sanremes portal info` shows "not logged in"
...
or use `sanremes model` and re-select Nous Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `sanremes auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/sanremes-agent/issues) — we surface the Portal's catalog to SanRemes and gaps usually mean a routing config we can update.

### Bills not appearing on my Portal account

Check `sanremes portal info` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `sanremes model`, pick Nous Portal, and the next request will route through your subscription.

## See also

- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway tool, per-tool config, and pricing
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-SanRemes tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple SanRemes configurations sharing one Portal login
