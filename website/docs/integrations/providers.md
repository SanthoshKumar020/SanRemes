...
This page covers setting up inference providers for SanRemes Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use SanRemes.

## Inference Providers

You need at least one way to connect to an LLM. Use `sanremes model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `sanremes model` (OAuth, subscription-based) |
| **OpenAI Codex** | `sanremes model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `sanremes model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `sanremes model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `sanremes model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.sanremes/.env` |
| **Fireworks AI** | `FIREWORKS_API_KEY` in `~/.sanremes/.env` (provider: `fireworks`; aliases: `fireworks-ai`, `fw`) |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.sanremes/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.sanremes/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.sanremes/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.sanremes/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.sanremes/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.sanremes/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.sanremes/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **Actual Computer** | `ACTUAL_API_KEY` in `~/.sanremes/.env` for the hosted relay, or `ACTUAL_BASE_URL=http://127.0.0.1:8080` for the local daemon — no key needed on loopback (provider: `actual`; aliases: `actual-computer`, `actualcomputer`, `aci`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.sanremes/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.sanremes/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.sanremes/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `sanremes model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.sanremes/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.sanremes/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.sanremes/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.sanremes/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.sanremes/.env` (provider: `opencode-zen`) |
| **CommandCode** | `COMMANDCODE_API_KEY` in `~/.sanremes/.env` (provider: `commandcode`, alias: `commandcode-chat`; Claude models via `commandcode-anthropic`, alias: `commandcode-claude`). Works with GOAT/Pro/Max/Provider plans (not the $1 Go plan — no API access). |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.sanremes/.env` (provider: `opencode-go`) |
| **OpenCode Free** | Keyless — no API key or account needed (provider: `opencode-free`, aliases: `free`, `opencode_free`). Select via `sanremes model` or `/model free`; requests are sent anonymously |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.sanremes/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.sanremes/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.sanremes/.env` (provider: `gemini`) |
| **Google Vertex AI** | `sanremes model` → "Google Vertex AI" (provider: `vertex`; OAuth2 via service-account JSON or ADC, GCP billing) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.sanremes/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `sanremes model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `sanremes model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.sanremes/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `sanremes model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `sanremes model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `sanremes model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.sanremes/.env` (provider: `stepfun`) |
| **LM Studio** | `sanremes model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `sanremes model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
[Nous Portal](https://portal.nousresearch.com) is Nous Research's unified subscription gateway and **the recommended way to run SanRemes Agent**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) — billed against your Nous subscription instead of separate per-provider accounts.

```bash
sanremes setup --portal     # fresh install — OAuth + provider + gateway in one command
sanremes model              # existing install — pick "Nous Portal" from the list
sanremes portal info        # inspect login + routing at any time
```
...
**For full details:** see the dedicated [Nous Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run SanRemes Agent with Nous Portal guide](/guides/run-sanremes-with-nous-portal).

**Client identification.** Every Portal request from SanRemes Agent carries a `client=sanremes-client-v<version>` tag (e.g. `client=sanremes-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish SanRemes traffic from other clients. No config required; the tag updates automatically when you `sanremes update`.

**JWT auth (automatic).** SanRemes prefers scoped `inference:invoke` JWTs for Portal requests with the legacy opaque session-key path as a fallback. No configuration is required — credentials are managed by the OAuth flow and rotate transparently. Revoked refresh tokens are quarantined to avoid replay loops.


:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). SanRemes stores the resulting credentials in its own auth store under `~/.sanremes/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), SanRemes marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `sanremes auth add openai-codex` (or `sanremes model` → **ChatGPT or Codex Subscription**) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), SanRemes routes these tasks to your **main chat model** — the same model you picked in `sanremes model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `sanremes setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `sanremes model` or per-tool from `sanremes tools`. Inspect routing at any time with `sanremes portal info`.
:::

### Two Commands for Model Management

SanRemes has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`sanremes model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a SanRemes chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `sanremes model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `sanremes model`, complete the provider setup, then start a new session.


### Subscription plans: what your plan pays for

Several providers let you sign in to SanRemes with a **consumer subscription** (Claude Max, ChatGPT, SuperGrok / X Premium+, …) instead of an API key. What that subscription actually pays for — and what it doesn't — differs per provider, and it's the single most common source of billing surprises. The table below is the short version; each provider's own section has the details.

> Cells marked *not currently documented* mean exactly that: SanRemes docs do not yet specify the behavior. Don't assume — check your provider's billing dashboard, and treat these as open questions.

| Plan / path | Can SanRemes use it? | What gets consumed | What does NOT get consumed | Common surprise |
|---|---|---|---|---|
| **Anthropic — Claude Max + OAuth** | ✅ Yes — `sanremes model` → Anthropic OAuth. Requires Max **and** purchased extra usage credits | The **extra/overage credits** you've added on top of the Max plan | The **base Max plan allowance** (the usage included in Claude Code by default) | All SanRemes usage bills as "extra usage" even while your included Max allowance sits untouched |
...
| **OpenAI Codex — ChatGPT plan OAuth** | ✅ Yes — `sanremes model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth device-code login, uses Codex models) | *Not currently documented* | *Not currently documented* | Docs cover auth and token refresh only; plan-quota semantics are not yet documented |
...
| **Google — Gemini consumer plan (Google AI Pro / Ultra)** | ❌ No documented path — the `gemini` provider is API-key only (`GOOGLE_API_KEY` / `GEMINI_API_KEY`); Vertex AI uses GCP billing | Your **API key's quota** (free tier or billing-enabled Google Cloud project) — *consumer-plan consumption not currently documented* | *Not currently documented* | Free-tier keys can be exhausted after a handful of agent turns, because SanRemes may make several model calls per user turn |

**Anthropic.** The OAuth path routes as Claude Code against your Anthropic account and **only works on a Claude Max plan with purchased extra usage credits** — the base Max allowance is never consumed by SanRemes, only the extra/overage credits on top. Claude Pro subscribers cannot use this path; the supported alternative is an `ANTHROPIC_API_KEY`, billed pay-per-token against that key's organization at standard API pricing. See [Anthropic (Native)](#anthropic-native) below.

**OpenAI Codex.** SanRemes authenticates via ChatGPT device-code OAuth, stores credentials in `~/.sanremes/auth.json`, and can import existing Codex CLI credentials from `~/.codex/auth.json`. Which ChatGPT plan tiers are eligible, and how SanRemes usage counts against your plan's Codex limits, are **not currently documented** — the Codex note under [Nous Portal](#nous-portal) covers authentication and token-refresh behavior only.
...
When you authenticate via `sanremes model` → Anthropic OAuth (or via `sanremes auth add anthropic --type oauth`), SanRemes routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by SanRemes — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
sanremes chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `sanremes model`
# SanRemes will use Claude Code's credential store directly when available
sanremes model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
sanremes chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
sanremes chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `sanremes model`, SanRemes prefers Claude Code's own credential store over copying the token into `~/.sanremes/.env`. That keeps refreshable Claude credentials refreshable.
...
SanRemes supports GitHub Copilot as a first-class provider with two modes:
...
```bash
sanremes chat --provider copilot --model gpt-5.4
```
...
If no token is found, `sanremes model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `sanremes model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `sanremes model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in SanRemes
SanRemes sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, SanRemes now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). SanRemes therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
sanremes chat --provider copilot-acp --model copilot-acp
...
| `SANREMES_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `SANREMES_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# Fireworks AI
sanremes chat --provider fireworks --model accounts/fireworks/models/kimi-k2p6
# Requires: FIREWORKS_API_KEY in ~/.sanremes/.env

# NovitaAI Model API
sanremes chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.sanremes/.env

# z.ai / ZhipuAI GLM
sanremes chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.sanremes/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
sanremes chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.sanremes/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
sanremes chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.sanremes/.env

# MiniMax (global endpoint)
sanremes chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.sanremes/.env

# MiniMax (China endpoint)
sanremes chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.sanremes/.env

# Qwen Cloud / DashScope (Qwen models)
sanremes chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.sanremes/.env

# Xiaomi MiMo
sanremes chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.sanremes/.env

# Tencent TokenHub (Hy3 Preview)
sanremes chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.sanremes/.env

# Arcee AI (Trinity models)
sanremes chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.sanremes/.env

# Meta Model API (Muse Spark family)
sanremes chat --provider meta-ai --model muse-spark-1.2
# Requires: MODEL_API_KEY in ~/.sanremes/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
sanremes chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.sanremes/.env
```

Fireworks uses its native slash-form catalog IDs, such as `accounts/fireworks/models/kimi-k2p6`. Run `sanremes model`, choose **Fireworks AI**, and select from the live catalog or enter another Fireworks model ID. The default endpoint is `https://api.fireworks.ai/inference/v1`; configure a different endpoint through `model.base_url` in `config.yaml`, not `.env`.
...
When using the Z.AI / GLM provider, SanRemes automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.sanremes/.env` and pick xAI in `sanremes model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `sanremes model`, or run `sanremes auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if SanRemes runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), SanRemes automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `sanremes tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `sanremes doctor` and `sanremes chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `sanremes migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).

```bash
sanremes migrate xai          # preview replacements
sanremes migrate xai --apply  # rewrite ~/.sanremes/config.yaml in place
```
...
# Short alias
sanremes chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `sanremes model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and SanRemes auto-discovers the available models.
...
sanremes chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 sanremes chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. SanRemes lazy-installs `google-auth` on first use; run `sanremes setup` if the managed install needs repair. See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.

### Qwen Portal (OAuth)

Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `sanremes model`, sign in through the browser, and SanRemes persists the refresh token.

```bash
sanremes model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.sanremes/auth.json

sanremes chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `SANREMES_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), SanRemes exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
sanremes chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `sanremes model`, sign in through the browser, and SanRemes persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
sanremes chat   # uses api.minimax.io/anthropic endpoint
```
...
sanremes chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.sanremes/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 sanremes chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
SanRemes automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
sanremes chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.sanremes/.env
```
...
Your own hardware as a private inference cluster via [Actual Computer](https://actual.inc). Two serving modes, both OpenAI-compatible (SanRemes uses the Responses API transport):
...
- **Local daemon** — on-device at `http://127.0.0.1:8080`, fully offline. No API key needed: SanRemes detects the loopback base URL and authenticates with an internal placeholder automatically.

```bash
# Hosted relay (ACTUAL_API_KEY in ~/.sanremes/.env)
sanremes chat --provider actual --model <model-id-from-your-cluster>

# Local daemon (ACTUAL_BASE_URL=http://127.0.0.1:8080 in ~/.sanremes/.env, no key)
sanremes chat --provider actual --model <installed-model-name>
```
...
Notes:
- Model IDs come from your cluster's `GET /v1/models` — discover with `sanremes model` or `curl -s https://api.actual.inc/v1/models -H "Authorization: Bearer $ACTUAL_API_KEY"`.
...
- Small local models: SanRemes' full default toolset plus the system prompt can exceed a 32k context window, producing an empty-stream error from llama.cpp-family servers. Restrict the toolset (`-t file,web`) or load the model with a larger context. The optional `actual-setup` skill (`sanremes skills install official/devops/actual-setup`) covers setup and troubleshooting in detail.
...
```bash
# StepFun
sanremes chat --provider stepfun --model step-3.5-flash
# Requires: STEPFUN_API_KEY in ~/.sanremes/.env
```
...
```bash
# Use any available model
sanremes chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.sanremes/.env

# Short alias
sanremes chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
SanRemes Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point SanRemes at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`LLM_MODEL` in `.env` is **removed** — `config.yaml` is the single source of truth for model and endpoint configuration. `OPENAI_BASE_URL` is still honored, but **only** for the `openai-api` provider (it overrides the OpenAI endpoint for direct API-key access). For other providers and custom endpoints, use `sanremes model` or set `model.base_url` in `config.yaml` directly. If you have stale entries in your `.env`, they are automatically cleared on the next `sanremes setup` or config migration.
:::
...
:::warning sanremes model vs /model
**`sanremes model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active SanRemes chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `sanremes model`, set up the new provider, then start a new session.
:::
...
When switching providers, SanRemes persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
SanRemes Agent requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like SanRemes.
:::
...
**Context length (`-c`):** Recent builds default to `0` which reads the model's training context from the GGUF metadata. For models with 128k+ training context, this can OOM trying to allocate the full KV cache. Set `-c` explicitly to at least 64,000 tokens for SanRemes. If using parallel slots (`-np`), the total context is divided among slots — with `-c 64000 -np 4`, each slot only gets 16k, which is below SanRemes' minimum per active session.

Then configure SanRemes to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but SanRemes won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), SanRemes 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
SanRemes preserves the context of an already-loaded LM Studio instance. For an unloaded model in the default explicit mode, SanRemes omits `context_length` unless you configured one in SanRemes, so LM Studio can apply its own model setting. SanRemes then uses only the context length LM Studio reports after loading.
...
If you use LM Studio's Just-In-Time loading / Auto-Evict feature and want LM Studio to manage model loading and eviction from normal chat requests, skip SanRemes' explicit preload step:

```bash
sanremes config set model.lmstudio_load_mode jit
```

Set it back to the default explicit preload behavior with:

```bash
sanremes config set model.lmstudio_load_mode explicit
```

**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, SanRemes) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since SanRemes Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your SanRemes config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your SanRemes config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with SanRemes.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running SanRemes inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser sanremes` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. SanRemes's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what SanRemes thinks the context is
...
SanRemes auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), SanRemes uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure SanRemes with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in SanRemes.
...
Then configure SanRemes with `sanremes model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure SanRemes with `sanremes model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `sanremes model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). SanRemes uses this to decide when to compress history and to validate API requests.
...
SanRemes uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`sanremes model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
providers:
  local:
    ...
    # api_key omitted — SanRemes uses "no-key-required" for keyless local servers
  work:
    api: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    transport: chat_completions   # set explicitly by `sanremes model` → Custom Endpoint wizard; auto-detection still happens as a fallback
  anthropic-proxy:
...
:::note Legacy format
Older configs used a top-level `custom_providers:` list instead. It still works — SanRemes reads both — and `sanremes update` auto-migrates it to the `providers:` dict (config v12). Field names differ slightly in the dict format: legacy `model` is `default_model`, and legacy `api_mode` is `transport`.
:::

Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and SanRemes will merge it into each chat-completions request for that endpoint:
...
The `sanremes model` → Custom Endpoint wizard now prompts for the API mode explicitly and persists your answer to `config.yaml` (as `transport` on the provider entry). URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.

**Native vision for custom-provider models.** If your custom endpoint serves a vision-capable model that isn't in models.dev, set `model.supports_vision: true` so SanRemes routes attached images natively (as `image_url` parts) instead of pre-processing them through `vision_analyze`. Single knob — no need to also set `agent.image_input_mode: native`.
...
You can also select named custom providers from the interactive `sanremes model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under the `providers:` dict. Three worked recipes follow. Each drops into `~/.sanremes/config.yaml` and the matching API key goes in `~/.sanremes/.env`.
...
Together's `/v1/models` endpoint works, so `sanremes model` can auto-discover available models.
...
:::tip Troubleshooting
- `sanremes doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `sanremes model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `sanremes model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point SanRemes at your instance (no API key needed):
   ```bash
   sanremes config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.sanremes/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.sanremes/config.yaml`:
...
Configure a chain of backup providers SanRemes tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `sanremes fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
