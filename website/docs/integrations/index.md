...
SanRemes Agent connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what SanRemes can do and where it can run.
...
SanRemes supports multiple AI inference providers out of the box. Use `sanremes model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](/integrations/providers)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. SanRemes auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](/user-guide/features/mcp)** — Connect SanRemes to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native SanRemes tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support eight backend providers, configured via `config.yaml` or `sanremes tools`:
...
If `web.backend` is not set, the backend is auto-detected from whichever API key is available. Self-hosted Firecrawl is also supported via `FIRECRAWL_API_URL`. Selecting Tavily in `sanremes tools` works without a key.

## Browser Automation

SanRemes includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](/user-guide/features/acp)** — Use SanRemes Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains. SanRemes runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](/user-guide/features/api-server)** — Expose SanRemes as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use SanRemes as a backend with its full toolset.
...
SanRemes runs as a gateway bot on 27+ messaging platforms, all configured through the same `gateway` subsystem:
...
| **Slack** | [api.slack.com/apps?new_app=1](https://api.slack.com/apps?new_app=1) | The **Create New App** dialog — pick *From an app manifest* and paste the manifest `sanremes slack manifest --agent-view` generates |
...
- **[Buzz](/integrations/buzz)** — Block's Nostr-based human+agent workspace. Three integration paths: Buzz Desktop spawns SanRemes as a managed ACP runtime, the `buzz-acp` relay bridge hosts a SanRemes identity server-side, or the native gateway platform joins Buzz channels with full SanRemes memory/skills/approvals/cron. The overview page compares all three.
...
- **[Plugin System](/user-guide/features/plugins)** — Extend SanRemes with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.sanremes/plugins/`, project-local `.sanremes/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/developer-guide/plugins)** — Step-by-step guide for creating SanRemes plugins with tools, hooks, and CLI commands.
