...
description: "How to build an image-generation backend plugin for SanRemes Agent"
---
...
:::tip
Image-gen is one of several **backend plugins** SanRemes supports. The others (with more specialized ABCs) are [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a SanRemes Plugin](/developer-guide/plugins).
:::

## How discovery works

SanRemes scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.sanremes/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `sanremes_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `sanremes tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `sanremes tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.sanremes/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `sanremes plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `sanremes tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `sanremes tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `sanremes tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $SANREMES_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `sanremes plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `sanremes tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `sanremes tools` model picker |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to `$SANREMES_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.sanremes/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `sanremes plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export SANREMES_HOME=/tmp/sanremes-imggen-test
mkdir -p $SANREMES_HOME/plugins/image_gen/my-backend
# …copy __init__.py + plugin.yaml into that dir…

export MY_BACKEND_API_KEY=your-test-key
sanremes plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $SANREMES_HOME/config.yaml
echo "  provider: my-backend" >> $SANREMES_HOME/config.yaml

# Exercise it
sanremes -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `sanremes tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
- [Build a SanRemes Plugin](/developer-guide/plugins) — general tools/hooks/slash commands guide
