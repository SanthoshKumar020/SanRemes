...
Users select your engine via `sanremes plugins` → Provider Plugins → Context Engine, or by editing `config.yaml`:
...
When `compression.context_timeout_seconds > 0` (the default), SanRemes runs the
