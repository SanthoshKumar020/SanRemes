...
SanRemes Agent uses a SQLite database (`~/.sanremes/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `sanremes_state.py`


## Architecture Overview

```
~/.sanremes/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `sanremes_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `sanremes_state.py` for the exact SQL.
...
Multiple sanremes processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.sanremes/state.db
...
Default path: `~/.sanremes/state.db`

This is derived from `sanremes_constants.get_sanremes_home()` which resolves to
`~/.sanremes/` by default, or the value of `SANREMES_HOME` environment variable.
