...
description: "How to add a new inference provider to SanRemes Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

SanRemes can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `sanremes model` menu entries
...
1. `sanremes_cli/auth.py` decides how credentials are found.
2. `sanremes_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `sanremes_cli/models.py` and `sanremes_cli/main.py` make the provider show up in the CLI. (`sanremes_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `sanremes_cli/auth.py`
2. `sanremes_cli/models.py`
3. `sanremes_cli/runtime_provider.py`
4. `sanremes_cli/main.py`
...
:::tip
`sanremes_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `sanremes setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$SANREMES_HOME/plugins/model-providers/` both get picked up.
...
7. `sanremes model` menu includes the provider
8. `sanremes setup` wizard delegates to `main.py` automatically
...
User plugins at `$SANREMES_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `sanremes model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `sanremes_cli/auth.py`
- `_PROVIDER_LABELS` in `sanremes_cli/models.py`
- `_PROVIDER_ALIASES` in both `sanremes_cli/auth.py` and `sanremes_cli/models.py`
- CLI `--provider` choices in `sanremes_cli/main.py`
...
## Step 2: Add auth metadata in `sanremes_cli/auth.py`
...
- What env vars should SanRemes check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `sanremes_cli/models.py`
...
## Step 4: Resolve runtime data in `sanremes_cli/runtime_provider.py`
...
Be careful with API-key precedence. SanRemes already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `sanremes_cli/main.py`

A provider is not discoverable until it shows up in the interactive `sanremes model` flow.

Update these in `sanremes_cli/main.py`:
...
:::tip
`sanremes_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `sanremes model` and `sanremes setup` automatically.
:::
...
When you add a native provider, double-check that SanRemes is only sending fields that provider actually understands.
...
- `tests/sanremes_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/sanremes_cli/test_model_switch_custom_providers.py` (and adjacent `tests/sanremes_cli/test_model_switch_*.py`)
- `tests/sanremes_cli/test_setup_model_provider.py`
...
python -m pytest tests/sanremes_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/sanremes_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m sanremes_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m sanremes_cli.main model
...
- [ ] `ProviderConfig` added in `sanremes_cli/auth.py`
- [ ] aliases added in `sanremes_cli/auth.py` and `sanremes_cli/models.py`
- [ ] model catalog added in `sanremes_cli/models.py`
- [ ] runtime branch added in `sanremes_cli/runtime_provider.py`
- [ ] CLI wiring added in `sanremes_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `sanremes model` but not `sanremes setup`
