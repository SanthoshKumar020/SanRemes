...
SanRemes runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.sanremes/plugins/`, registering a backend the user selects
...
| `sanremes setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `sanremes status` / `sanremes doctor` | `doctor_checks()` |
...
```python title="~/.sanremes/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.sanremes"    # where synced cache files land, or None
...
```yaml title="~/.sanremes/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
sanremes plugins enable acmebox
sanremes config set terminal.backend acmebox
```
