...
title: "Desktop Plugin SDK (@sanremes/plugin-sdk)"
description: "Extend the native SanRemes Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [SanRemes Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `SanRemesPlugin`.
It imports one module — `@sanremes/plugin-sdk` — and gets everything: the app's
...
`$SANREMES_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across SanRemes. This page is the **native
desktop app** (`sanremes desktop`) SDK — the `@sanremes/plugin-sdk` module and
`$SANREMES_HOME/desktop-plugins/`. The **web dashboard** (`sanremes dashboard`) has
its own, unrelated plugin system on `window.__SANREMES_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a SanRemes Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$SANREMES_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$SANREMES_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `SanRemesPlugin` contract, appear in **Settings → Plugins**,
...
Create `$SANREMES_HOME/desktop-plugins/hello/plugin.js` (that's `~/.sanremes/...`
by default, or `~/.sanremes/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.sanremes/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@sanremes/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, SanRemes' }),
      jsx('div', {
...
The only importable specifiers are `@sanremes/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `SanRemesPlugin`:

```ts
interface SanRemesPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `sanremes.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@sanremes/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@sanremes/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@sanremes/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@sanremes/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@sanremes/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@sanremes/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@sanremes/plugin-sdk'
...
`window.sanremes.send('get-price eth')` (or a declarative
`<button data-sanremes-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@sanremes/plugin-sdk'
...
for requests; `targetProfile` is the backend SanRemes profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from SanRemes
...
ctx.os.notify({
  ...
  // Body click → focus SanRemes + navigate. Same vocabulary as OS deep links:
  activate: 'sanremes://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'sanremes://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `sanremes://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `sanremes://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@sanremes/plugin-sdk'
...
```javascript
import { queryClient } from '@sanremes/plugin-sdk'
...
a package into `~/.sanremes/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.sanremes/plugins` is not reachable as a filesystem —
...
the `sanremes://` scheme — a plain anchor on your website or README:
...
`sanremes-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular SanRemes plugin and declare it in a
`manifest.json`:
...
sanremes-agent codebase directly (`sanremes_state`, `sanremes_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.sanremes/`)
...
(`sanremes.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `SanRemesPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@sanremes/plugin-sdk` alias.
2. It's still lint-fenced to `@sanremes/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@sanremes/plugin-sdk`, `react`,
...
| Plugin contract | `SanRemesPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `SanRemesOpenTarget`, `Contribution` |
...
### Agents: the `sanremes-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`sanremes-desktop-plugins`** skill — it carries the same contract as this page in
...
`$SANREMES_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `sanremes logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@sanremes/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.sanremes/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.sanremes/logs/errors.log` for
