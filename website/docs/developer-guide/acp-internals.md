...
The ACP adapter wraps SanRemes' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
sanremes acp / sanremes-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.sanremes/.env
  -> configure stderr logging
  -> construct SanRemesACPAgent
...
- `allow_once` -> SanRemes `once`
- `allow_always` -> SanRemes `always`
- reject options -> SanRemes `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps SanRemes tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["sanremes-acp"])
...
Instead it reuses SanRemes' runtime resolver:

- `acp_adapter/auth.py`
- `sanremes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured SanRemes provider/credentials. It also always advertises a terminal setup auth method (`sanremes-setup`, args `--setup`) so first-run ACP clients can open SanRemes' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.sanremes/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `sanremes-acp` toolset definition
- `sanremes_cli/main.py` — `sanremes acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `sanremes-acp` script
