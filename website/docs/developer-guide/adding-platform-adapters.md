...
This guide covers adding a new messaging platform to the SanRemes gateway. A platform adapter connects SanRemes to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.sanremes/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core SanRemes code. Your plugin is a directory with two files:

```
~/.sanremes/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `sanremes config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-sanremes-config) below).
...
With `provides_tools` declared, SanRemes imports only `tools.py` during plugin
...
`sanremes tools enable my_platform --platform cli`, or by listing the toolset
...
| `sanremes config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `sanremes status` | Shows plugin platforms with `(plugin)` tag |
| `sanremes gateway setup` | Plugin platforms appear in setup menu |
| `sanremes tools` / `sanremes skills` | Plugin platforms in per-platform config |
...
direct `sanremes send --to ...` and cron `deliver=platform:...` by declaring send
...
def register(ctx):
    ctx.register_platform(
        ...
        # May be a regular function or async def. SanRemes awaits any awaitable
...
Most users set up a platform by dropping env vars into `~/.sanremes/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `sanremes gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `sanremes cron run` separate from `sanremes gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `sanremes config`

`sanremes_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
version: 1.0.0
description: >
  My Platform gateway adapter for SanRemes Agent.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #sanremes)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the SanRemes core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`sanremes_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`sanremes_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`sanremes_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`sanremes_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`sanremes_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`sanremes_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"sanremes-newplat"` toolset definition with `_SANREMES_CORE_TOOLS`
2. **`toolsets.py`** — Add `"sanremes-newplat"` to the `"sanremes-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | sanremes-newplat toolset |
