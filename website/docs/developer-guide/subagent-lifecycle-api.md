...
Plugins can launch and supervise fresh SanRemes child sessions without importing
...
active agent turn fails closed with `No active SanRemes parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until SanRemes can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; SanRemes's existing unsafe-tool block remains enforced.
