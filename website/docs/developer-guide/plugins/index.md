...
title: "Build a SanRemes Plugin"
description: "Step-by-step guide to building a complete SanRemes plugin with tools, hooks, data files, and skills"
---

# Build a SanRemes Plugin

This guide walks through building a complete SanRemes plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
SanRemes has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `SANREMES_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.sanremes/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `sanremes skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `NousResearch/sanremes-agent`. Users install them into `~/.sanremes/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Nous Research Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/NousResearch/sanremes-agent/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

SanRemes can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components SanRemes already owns. It does not replace native `plugin.yaml` plus
...
```bash
sanremes plugins install owner/repository --no-enable
sanremes plugins list
sanremes plugins enable <plugin-name>
```
...
SanRemes validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by SanRemes.
...
Portable `streamable-http` entries are routed through SanRemes' existing native
...
same full-trust posture as other installed SanRemes plugins.
...
records it as Published. SanRemes keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. SanRemes does not expose a
...
continue to work after a normal SanRemes upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  SanRemes inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older SanRemes releases can therefore load a plugin whose manifest contains
...
SanRemes enforces this contract with frozen external-plugin fixtures discovered
from an isolated `SANREMES_HOME`. Those tests load and invoke the plugin through
...
```bash
mkdir -p ~/.sanremes/plugins/calculator
cd ~/.sanremes/plugins/calculator
```

### Validate with Plugin Doctor

`sanremes plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by SanRemes itself. It reports invalid hook names, callbacks that do
...
```bash
sanremes plugins doctor . --ci
```

Doctor uses a temporary `SANREMES_HOME`, restores plugin registration state after
...
This tells SanRemes: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `sanremes_cli/plugin_capabilities.py` for the
...
`sanremes_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`sanremes_agent.plugins` entry point:

```toml
[project.entry-points."sanremes_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."sanremes_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

SanRemes reads these from installed metadata without importing your code, so
`sanremes plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this SanRemes understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — SanRemes validates them, and `sanremes plugins install` / `sanremes plugins doctor` surface missing ones with a `pip install` hint, but SanRemes **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into SanRemes' shared venv is a conflict and supply-chain
...
[#64165](https://github.com/NousResearch/sanremes-agent/issues/64165) and
[#15220](https://github.com/NousResearch/sanremes-agent/issues/15220). Plugin
...
4. **Accept `**kwargs`:** SanRemes may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `sanremes my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but SanRemes continues fine
...
Use plugin-relative config keys for user-visible behavior. SanRemes resolves them
...
`<SANREMES_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `SANREMES_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
SANREMES_PLUGINS_DEBUG=1 sanremes plugins list
```
...
The same logs are always written to `~/.sanremes/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
sanremes logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `sanremes plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.sanremes/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `sanremes plugins update` / `remove` git-pull or delete it — your
...
# <sanremes home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.sanremes/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.sanremes/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `sanremes plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing SanRemes into installing arbitrary packages — only specs SanRemes itself ships are eligible |
...
auditable in `~/.sanremes/logs/agent.log`. Plugins load after built-in
...
does not ship with SanRemes core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `sanremes plugins list`
...
loader, the plugin is disabled and SanRemes continues. The gate exists
...
You normally never edit this key by hand. `sanremes plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), SanRemes injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: sanremes-agent"}
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$SANREMES_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $SANREMES_HOME/hook_outputs
```
...
- **The system prompt is SanRemes's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `sanremes_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before SanRemes sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`sanremes.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer SanRemes still loads on an older one.

### Register CLI commands

Plugins can add their own `sanremes <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for sanremes my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: sanremes my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for sanremes my-plugin."""
...
After registration, users can run `sanremes my-plugin status`, `sanremes my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `sanremes name` in a terminal |
...
- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `SANREMES_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `sanremes <subcommand>` (e.g. `sanremes kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "sanremes kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive SanRemes from a hook.
...
SanRemes has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.sanremes/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `sanremes gateway status` without SDK instantiation.
...
SanRemes also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into SanRemes without any Python plugin. Declare them in `~/.sanremes/config.yaml`:
...
SanRemes connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.sanremes/hooks/<name>/`:

```yaml
# ~/.sanremes/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.sanremes/hooks/long-task-alert/handler.py
...
sanremes skills search my-workflow --source myorg/skills-repo
sanremes skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install sanremes-plugin-calculator
# Plugin auto-discovered on next sanremes startup
```
...
```python
# Wrong — will break if SanRemes passes extra context
def handler(args):
    ...
