...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to SanRemes Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in SanRemes tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying SanRemes core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a SanRemes Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_SANREMES_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `sanremes_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `sanremes_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `sanremes chat -q "Use the weather tool for London"`
