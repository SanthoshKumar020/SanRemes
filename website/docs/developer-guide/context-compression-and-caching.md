...
SanRemes Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `sanremes plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

SanRemes has two separate compression layers that operate independently:
...
compression:
  ...
  codex_app_server_auto: native  # native|sanremes|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `sanremes`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, SanRemes raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$SANREMES_HOME`
...
```bash
sanremes config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
sanremes config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). SanRemes keeps the
...
`gpt-5.4-900k`). These are SanRemes-side aliases: the suffix is stripped before
...
thread context, so SanRemes' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and SanRemes
  records the resulting compaction events (compression counters, session
  events). Set `sanremes` to let SanRemes' compression threshold initiate
  app-server compaction, or `off` to disable SanRemes-initiated automatic
  compaction entirely (codex may still compact natively).

SanRemes' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep SanRemes' summary compressor.
...
encrypted `compaction` output item. SanRemes captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. SanRemes
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   SanRemes can avoid; user-facing docs for `/model`, fallback providers, and
