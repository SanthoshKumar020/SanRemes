...
description: "How to build a model provider (inference backend) plugin for SanRemes Agent"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that SanRemes can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$SANREMES_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with SanRemes
2. **User plugins** — `$SANREMES_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
3. **Legacy single-file** — `<repo>/providers/<name>.py` — back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$SANREMES_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
The only required file is `__init__.py`. `plugin.yaml` is used by `sanremes plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `sanremes_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `sanremes_cli/main.py` | Accepts `acme-inference` |
| `sanremes model` picker | `sanremes_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `sanremes doctor` | `sanremes_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `sanremes setup` | `sanremes_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `sanremes_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| `display_name` | str | Human label shown in `sanremes model` picker |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.sanremes/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. SanRemes picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but SanRemes' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Point `SANREMES_HOME` at a temp directory so you don't pollute your real config:

```bash
export SANREMES_HOME=/tmp/sanremes-plugin-test
mkdir -p $SANREMES_HOME/plugins/model-providers/my-provider
cat > $SANREMES_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
sanremes -z "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `sanremes plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$SANREMES_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Model providers can ship as a pip package. Expose an entry point in the
`sanremes_agent.plugins` group in your `pyproject.toml`:

```toml
[project.entry-points."sanremes_agent.plugins"]
acme-inference = "acme_sanremes_plugin:register"
```
...
- **Lowest precedence.** Entry-point plugins are discovered **before**
  ...
  bundled or `$SANREMES_HOME` profile of the same name always overrides a
...
See [Building a SanRemes Plugin](/developer-guide/plugins#distribute-via-pip) for the full entry-points setup.
...
- [Building a SanRemes Plugin](/developer-guide/plugins) — general plugin authoring
