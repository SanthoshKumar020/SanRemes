...
| Variable | Meaning |
|----------|---------|
| `SANREMES_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `SANREMES_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `SANREMES_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by SanRemes itself — they're private to these helpers. The variables SanRemes *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `sanremes --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_sanremes_root)" || { echo "htui: not in a SanRemes checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$SANREMES_MAIN_CHECKOUT/.venv/bin/python" -m sanremes_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `SANREMES_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_sanremes_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `SANREMES_TUI_DIR` are mutually exclusive
`SANREMES_TUI_DIR` points SanRemes at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `sanremes --tui --dev` exits with an error. Run `unset SANREMES_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_sanremes_root)" || { echo "hgui: not in a SanRemes checkout" >&2; return 1; }
  deps="${SANREMES_GUI_DEPS_CHECKOUT:-$SANREMES_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _sanremes_link_deps "$desktop" "$deps/apps/desktop"
    _sanremes_link_deps "$root" "$deps"
  else
  ...
  trap '_sanremes_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    SANREMES_DESKTOP_SANREMES_ROOT="$root" \
    SANREMES_DESKTOP_PYTHON="$SANREMES_MAIN_CHECKOUT/.venv/bin/python" \
    SANREMES_DESKTOP_IGNORE_EXISTING=1 \
    SANREMES_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `SANREMES_DESKTOP_SANREMES_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `sanremes`. |
| `SANREMES_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `SANREMES_DESKTOP_IGNORE_EXISTING` | Ignores any `sanremes` on `PATH` so it can't shadow the worktree. |
| `SANREMES_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real SanRemes checkout.
_sanremes_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/sanremes_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_sanremes_gui_cleanup() {
  local root="$1"
  [[ -n "$root" ]] && pkill -TERM -f "${root}/apps/desktop/node_modules/electron" 2>/dev/null
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'sanremes_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `sanremes --tui --dev` and the `SANREMES_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/NousResearch/sanremes-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `SANREMES_*` variable SanRemes reads
