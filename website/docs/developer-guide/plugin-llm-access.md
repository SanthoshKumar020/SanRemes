...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept SanRemes already has applies. The plugin never sees a
...
sanremes-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — SanRemes provider id (e.g. "openrouter")
...
using SanRemes built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different SanRemes provider
        # (must be one SanRemes already knows about — same names as
        # `sanremes model` and config.yaml model.provider).
        ...
        # plugin sends — SanRemes does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.sanremes/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through SanRemes' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing SanRemes subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/sanremes-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/sanremes-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/sanremes-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/sanremes-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
