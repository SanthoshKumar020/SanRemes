...
sidebar_position: 1
title: "Architecture"
description: "SanRemes Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of SanRemes Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ sanremes_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # SanRemesCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── sanremes_state.py           # SQLite session/state database with FTS5
├── sanremes_constants.py       # SANREMES_HOME, profile-aware paths
...
│
├── sanremes_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `sanremes` subcommands (large file)
...
│   ├── skills_config.py      # sanremes skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # sanremes tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # sanremes gateway start/stop
│
...
```text
User input → SanRemesCLI.process_input()
...
Three discovery sources: `~/.sanremes/plugins/` (user), `.sanremes/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `sanremes plugins` or `config.yaml`.
...
Exposes SanRemes as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`sanremes -p <name>`) gets its own SANREMES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
