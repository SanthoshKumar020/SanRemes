...
description: "How to build a memory provider plugin for SanRemes Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give SanRemes Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `sanremes plugins`.
:::

## Installation Layouts

SanRemes discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with SanRemes. Closed to new providers — see [CONTRIBUTING](https://github.com/NousResearch/sanremes-agent/blob/main/CONTRIBUTING.md). |
| User | `$SANREMES_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.sanremes/plugins/<name>/` | Opt-in via `SANREMES_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `sanremes_agent.memory_providers` entry point | `pip install`, nothing to copy. |
...
SanRemes, in `$SANREMES_HOME/plugins/<name>/` when installed by a user, or in
`./.sanremes/plugins/<name>/` for a project-local one:
...
`sanremes_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."sanremes_agent.memory_providers"]
...
package layout. No copy under `$SANREMES_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files SanRemes reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `sanremes <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          sanremes_home (str): Active SANREMES_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `sanremes memory setup` | **Yes** |
| `save_config(values, sanremes_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
The gate binds to every compaction authority, not just the SanRemes
...
required checkpoint cannot be guaranteed there. The checkpoint-aware SanRemes
...
`get_config_schema()` returns a list of field descriptors used by `sanremes memory setup`:
...
Every field in `get_config_schema()` is prompted during `sanremes memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$SANREMES_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, sanremes_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(sanremes_home) / "my-provider.json"
...
the `messages` parameter; SanRemes will continue calling them with the legacy
signature.
...
All storage paths **must** use the `sanremes_home` kwarg from `initialize()`, not hardcoded `~/.sanremes`:

```python
# CORRECT — profile-scoped
from sanremes_constants import get_sanremes_home
data_dir = get_sanremes_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.sanremes/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `sanremes my-provider status`, `sanremes my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `sanremes <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `sanremes --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: sanremes my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the sanremes my-provider argparse tree.
