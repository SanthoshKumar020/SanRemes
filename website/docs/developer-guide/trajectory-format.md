...
SanRemes Agent saves conversation trajectories in ShareGPT-compatible JSONL format
...
It follows the SanRemes function-calling prompt template with:
...
Trajectory saving is a `run_agent.py` / library-level switch — the `sanremes` CLI
