...
Uses a file-based lock (~/.sanremes/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `sanremes update` reloads
# the module) fail with ModuleNotFoundError for sanremes_time et al.
...
from sanremes_time import now as _sanremes_now
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `sanremes fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `sanremes cron pause {job_ref}` "
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running SanRemes, pin it explicitly: "
                f"`sanremes cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
    ...
    # checkout was updated underneath it (interrupted `sanremes update`, manual
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`sanremes gateway restart` to fix it."
            )
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy sanremes_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from sanremes_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``sanremes tools`` config for the ``cron`` platform.
    ...
    try:
        from sanremes_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    """
    from sanremes_constants import parse_reasoning_effort, resolve_reasoning_config
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``SANREMES_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("SANREMES_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid SANREMES_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _sanremes_now().isoformat(),
    }
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_sanremes_home() / "cron" / "inflight_forced_releases.jsonl"
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_sanremes_home())
...
# (SANREMES_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse SANREMES_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("SANREMES_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid SANREMES_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_sanremes_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_sanremes_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.sanremes/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``sanremes update`` / ``sanremes gateway stop`` / systemd restart, or an
...
_sanremes_home: Path | None = None


def _get_sanremes_home() -> Path:
    """Resolve SanRemes home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active SANREMES_HOME at call time
    ...
    """
    return _sanremes_home or get_sanremes_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    sanremes_home = _get_sanremes_home()
    lock_dir = sanremes_home / "cron"
...
def _reclaim_fds_best_effort() -> None:
    ...
    try:
        ...
        pass
    try:
        from sanremes_cli.resource_limits import apply_nofile_soft_limit
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"SanRemes — {task_name}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from sanremes_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    Runs ``sanremes [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing
    ...
    scheduler's SANREMES_HOME) or a validated local profile name.  Returns None
    ...
    sanremes_bin = _shutil.which("sanremes")
    if sanremes_bin:
        argv = [sanremes_bin]
    else:
        try:
            ...
            if _ilu.find_spec("sanremes_cli") is not None:
                argv = [sys.executable, "-m", "sanremes_cli.main"]
            else:
                return "bot-chat delivery failed: sanremes CLI not resolvable"
        except Exception:
            return "bot-chat delivery failed: sanremes CLI not resolvable"
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution in the child; a leftover SANREMES_HOME
        # from THIS scheduler's profile must not shadow it.
        env.pop("SANREMES_HOME", None)
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="sanremes-cron-botchat-",
            delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _resolve_bot_chat_target(job: dict, profile_arg: str) -> Optional[dict]:
    ...
    ``profile_arg`` is ``""`` for the job's own profile (the SANREMES_HOME
    ...
    ``~/.sanremes/profiles/`` tree, so same-named profiles on other gateways
    can never be targeted by accident.
    """
    if not profile_arg:
        # Own profile: chat subprocess inherits SANREMES_HOME, no name needed.
        return {"platform": BOT_CHAT_PLATFORM, "chat_id": "", "thread_id": None}
    try:
        from sanremes_cli.profiles import normalize_profile_name, profile_exists
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            try:
                ...
                # SANREMES_CRON_MEDIA_SEND_TIMEOUT env override).
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # at boot; a standalone process (manual `sanremes cron run` from the CLI,
    ...
    for target in targets:
        ...
        if transport is not None:
            ...
            # (`sanremes cron run`) has no live relay adapter, so surface the
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("SANREMES_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid SANREMES_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _get_media_send_timeout() -> int:
    ...
    SANREMES_CRON_MEDIA_SEND_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("SANREMES_CRON_MEDIA_SEND_TIMEOUT", "").strip()
    if env_value:
        try:
            timeout = int(float(env_value))
            if timeout > 0:
            ...
            logger.warning(
                "Invalid SANREMES_CRON_MEDIA_SEND_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _get_session_db_timeout() -> float:
    ...
    Mirrors the ``script_timeout_seconds`` resolution pattern: the
    SANREMES_CRON_SESSION_DB_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("SANREMES_CRON_SESSION_DB_TIMEOUT", "").strip()
    if env_value:
        try:
            return float(env_value)
        except (ValueError, TypeError):
            logger.warning(
                "Invalid SANREMES_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _run_job_script(
    ...
    Scripts must reside within SANREMES_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other SanRemes-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against SANREMES_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_sanremes_home() / "scripts"
    ...
    # escape — scripts MUST reside within SANREMES_HOME/scripts/.
    try:
...
def _block_and_pause_job(
    ...
    now_iso = _sanremes_now().strftime("%Y-%m-%d %H:%M:%S")
    doc = (
        f"# Cron Job: {job_name}\n\n"
        f"**Job ID:** {job_id}\n"
        f"**Run Time:** {now_iso}\n"
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("SANREMES_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `sanremes setup`), or pin a "
            "working provider via `sanremes cron edit "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`sanremes setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    if job.get("no_agent"):
        ...
        # "no delivery target resolved". load_sanremes_dotenv does not override
        # already-set vars, so the gateway's in-process tick is unaffected.
        try:
            from sanremes_cli.env_loader import load_sanremes_dotenv

            load_sanremes_dotenv(sanremes_home=_get_sanremes_home())
        except Exception:
            logger.debug(
    ...
    if job_has_monitor(job):
        ...
        _mon_now = _sanremes_now().strftime("%Y-%m-%d %H:%M:%S")
        if not _mon.ok:
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_sanremes_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_sanremes_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # gateway message. Do not seed SANREMES_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads SANREMES_SESSION_PLATFORM
    #     and SANREMES_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via SANREMES_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     SANREMES_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read SANREMES_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the SANREMES_CRON_AUTO_DELIVER_* vars set
    # below, so clearing SANREMES_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # SANREMES_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ SANREMES_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "SANREMES_CRON_AUTO_DELIVER_PLATFORM",
        "SANREMES_CRON_AUTO_DELIVER_CHAT_ID",
        "SANREMES_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["SANREMES_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `sanremes chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with SANREMES_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $SANREMES_KANBAN_TASK -- letting
        ...
        # load_sanremes_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this SANREMES_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_sanremes_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from sanremes_cli.env_loader import (
        ...
        if delivery_target:
            _VAR_MAP["SANREMES_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["SANREMES_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["SANREMES_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > SANREMES_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``sanremes cron edit --model``
        ...
        try:
            ...
            _cfg_path = str(_get_sanremes_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"SANREMES_MODEL={os.getenv('SANREMES_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`sanremes cron edit {job_id} --model <name>` or set a "
                "default with `sanremes model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from sanremes_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("SANREMES_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_sanremes_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject SANREMES_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running SanRemes "
                        "pin it explicitly: "
                        f"`sanremes cron edit {job_id} --provider <provider> "
        ...
        #
        # Bounded with its own timeout (separate from SANREMES_CRON_TIMEOUT,
        ...
        agent = AIAgent(
            ...
            # SANREMES_HOME. When a workdir is configured, also inject project
        ...
        # override via SANREMES_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from sanremes_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
**Job ID:** {job_id}
**Run Time:** {_sanremes_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_sanremes_now().strftime('%b %d %H:%M')}"
            ...
            # sanremes_state's session_lifecycle_statuses docstring — keep the
...
def run_one_job(
    ...
    *,
    ...
    if extra_prompt is None:
        # A gateway-forwarded manual run (`sanremes cron run --prompt` /
    ...
    profile_home = _get_sanremes_home().resolve()
    with _running_lock:
    ...
    *,
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_sanremes_home())
        )
        ...
        if delivery_error:
            ...
            # exact signature via `sanremes cron incidents ack`.
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`sanremes pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `sanremes resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `sanremes cron run` that claimed a job
        ...
        if not due_jobs:
            ...
            if verbose:
                logger.info("%s - No jobs due", _sanremes_now().strftime('%H:%M:%S'))
            try:
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _sanremes_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set SANREMES_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("SANREMES_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid SANREMES_CRON_MAX_PARALLEL value; defaulting to unbounded")
