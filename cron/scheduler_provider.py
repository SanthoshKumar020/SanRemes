...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    the ticker runs — via ``sanremes profile delete``, the desktop's DELETE
    ...
    and in ``sanremes profile list`` (#47368). Filtering on directory existence
...
def fire_overdue_jobs(
    ...
    *,
    ...
    from cron.jobs import _ensure_aware, _sanremes_now, is_job_runnable, load_jobs
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global SANREMES_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `sanremes cron
        ...
        # Heartbeat once before the first sleep so `sanremes cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `sanremes cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_sanremes_home_override()`` + ``use_cron_store()``
        ...
        for entry in _existing_profile_homes(profile_homes):
            home = entry[1] if isinstance(entry, tuple) else entry
            home_token = set_sanremes_home_override(str(home))
            try:
            ...
            for entry in _existing_profile_homes(profile_homes):
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `sanremes cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
