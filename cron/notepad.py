...
Write path is the CLI (``sanremes cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_sanremes_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`sanremes cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `sanremes cron notepad {job_id} delete "
