...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``sanremes://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``sanremes://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"sanremes://blueprint/{quote(blueprint.key)}{qs}"
