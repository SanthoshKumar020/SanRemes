...
``sanremes gateway restart`` (or ``launchctl kickstart ai.sanremes.gateway``
or ``systemctl restart sanremes-gateway``).  When the cron fires, the
...
``sanremes cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``sanremes gateway``, ``launchctl ... sanremes-gateway``,
``systemctl ... sanremes-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_SANREMES_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``sanremes gateway
...
The profile-flag form (``sanremes -p <profile> gateway restart|stop``, #78028)
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `sanremes gateway` operations.
    # The destructive operations are restart, stop, and uninstall.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    # The lookbehind (#77173): `sanremes` must not be a path component or a
    # word tail. Excluding `/`, word chars, `.` and `-` keeps file paths
    # with embedded spaces (`/docs/sanremes gateway restart-notes.md`) from
    # matching via the `/sanremes` tail, while every real command position
    # (start of text, whitespace, `;`/`&`/`|`, `$(`, backtick, even a
    # U+FFFD from binary-content decoding) still matches.
    r"(?:(?<![/\w.\-])sanremes\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops on a sanremes-gateway label. macOS launchd
    # labels look like `ai.sanremes.gateway` / `sanremes-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated sanremes services (e.g.
    # `launchctl unload ai.sanremes.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.sanremes.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a sanremes-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bsanremes[.\-]?gateway)"
    # Branch D: pkill / kill targeting the sanremes gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.sanremes.gateway-... \` / `  -- ...`,
...
# selector — `sanremes -p <profile> gateway restart|stop` / `--profile <name>`
# / `--profile=<name>`. The selector token between `sanremes` and `gateway`
...
# `sanremes -p venus gateway restart` operates on a sibling profile's gateway
...
def _current_profile_name() -> Optional[str]:
    ...
    Prefers the explicit ``SANREMES_PROFILE_NAME`` / ``SANREMES_PROFILE`` env
    (set by the profile launcher and kanban worker spawns), falling back to
    ``sanremes_cli.profiles.get_active_profile_name`` (derived from
    ``SANREMES_HOME``, which the gateway process inherits from its launch
    profile). Returns ``None`` when neither source yields a name.
    """
    for env_name in ("SANREMES_PROFILE_NAME", "SANREMES_PROFILE"):
...
# Branch B only catches `launchctl <verb> ... sanremes[.-]?gateway` when the
...
# command — `for item in 'ai.sanremes.gateway-apollo:...' 'ai.sanremes.gateway:...';
...
# is the unexpanded variable `$label`, not the string "sanremes.gateway". This
...
# EXISTING job, so anchoring to the sanremes-gateway label is still correct —
# `test_safe_commands` requires unrelated-label ops (e.g. `launchctl unload
# ai.sanremes.update-checker.plist`) to stay unblocked. The fix is checking
...
_SANREMES_GATEWAY_LABEL_RE = re.compile(r"(?i)\bsanremes[.\-]?gateway\b")
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _SANREMES_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    a ``sanremes``/``gateway`` token) — this function is the single choke
    ...
    # a runbook line "a human can run: sanremes gateway restart" inside such a
    ...
    if _GATEWAY_LIFECYCLE_PATTERN.search(normalized):
        return True
    # Profile-flag form (#78028): `sanremes -p <profile> gateway restart|stop`
    # bypasses Branch A because the selector sits between `sanremes` and
    ...
    if profile_match:
        ...
        if named:
            # Profile ids cannot contain quotes (sanremes_cli.profiles
...
# for `systemctl restart sanremes-gateway` in syslog, a SQL LIKE literal over a
...
# blocking `grep -r 'systemctl restart sanremes-gateway' .` outright. Require a
...
# to execution (`grep 'systemctl restart sanremes-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.sanremes.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart sanremes-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart sanremes-gateway%'"
...
def _references_at(
    ...
    # (e.g. `Path.home() / ".sanremes"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<SANREMES_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.sanremes/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_sanremes_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_sanremes_home() falls back to Path.home(), which raises when
        # neither SANREMES_HOME nor HOME is resolvable (launchd/systemd
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "sanremes gateway restart"
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.sanremes/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `sanremes gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `sanremes gateway restart` from a shell outside "
