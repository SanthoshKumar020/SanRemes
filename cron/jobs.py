...
Jobs are stored in ~/.sanremes/cron/jobs.json
Output is saved to ~/.sanremes/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own SANREMES_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same SANREMES_HOME — so a job authored in profile
# `coder` lives in `~/.sanremes/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_sanremes_home()` (the active profile home), NOT `get_default_sanremes_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever SANREMES_HOME the
...
# `_get_sanremes_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
SANREMES_DIR = get_sanremes_home().resolve()
...
CRON_DIR = SANREMES_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``sanremes cron status`` process share it
...
# threshold in `sanremes cron status` (sanremes_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_sanremes_home()
       (context-local override, then the SANREMES_HOME env var) — so a test
       or embedder that re-points SANREMES_HOME after this module was
    ...
    home = get_sanremes_home().resolve()
    if home == SANREMES_DIR:
        return live_constants
    cron_dir = home / "cron"
...
# the cron inactivity timeout is disabled (SANREMES_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. SANREMES_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``SANREMES_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _jobs_lock():
    ...
    and standalone ``sanremes`` CLI invocations, which previously shared no lock
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec sanremes sanremes cron create ...`` — ``docker exec``
...
def _is_named_profile_path(path: Path) -> bool:
    ...
    Named profiles live under ``<sanremes_home>/profiles/<name>/``.  The
    default profile lives at ``<sanremes_home>`` directly (no ``profiles``
    parent), as do custom ``SANREMES_HOME`` paths outside ``~/.sanremes``.
...
def _ensure_cron_dir(cron_dir: Path) -> None:
    ...
    Default and custom SanRemes homes keep ``parents=True`` so first-run
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED SanRemes timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `sanremes_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                sanremes_tz = _sanremes_now().tzinfo
                dt = dt.replace(tzinfo=sanremes_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _sanremes_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in SanRemes configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured SanRemes timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _sanremes_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If SanRemes/system local time
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall sanremes-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `sanremes cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``sanremes cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `sanremes cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``sanremes cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``sanremes cron
    ...
    try:
        for _attempt in range(5):
            ...
            try:
                with os.fdopen(fd, "w", encoding="utf-8") as f:
                    json.dump(
                        {"jobs": jobs, "updated_at": _sanremes_now().isoformat()},
                        f,
                        indent=2,
                        ensure_ascii=False,
                    )
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_sanremes_home() / "config.yaml"
...
def _normalize_reasoning_effort(value: Any) -> Optional[str]:
    ...
    every other effort surface uses (``sanremes_constants.parse_reasoning_effort``)
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.sanremes/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        monitor_script: Optional path to a cheap monitor source script (same
                ...
                ~/.sanremes/scripts/, .sh/.bash via bash, else Python). Each
        ...
        reasoning_effort: Optional per-job reasoning effort pin. One of the
                canonical SanRemes levels (none|minimal|low|medium|high|xhigh|
    ...
    # covered, not just `sanremes cron create`.
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _sanremes_now().isoformat(),
...
def trigger_job(
    ...
    if is_terminal_job(job):
        ...
        raise ValueError(
            ...
            f"Create a new occurrence with 'sanremes cron resume {name} "
            "--run-now' or '--at <ISO-8601>'."
        )
    manual_run_at = _sanremes_now().isoformat()
    return update_job(
        job["id"],
        {
            "enabled": True,
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    try:
        ...
        text = (
            ...
            f"- removed at: {_sanremes_now().isoformat()}\n\n"
    ...
    with _jobs_lock():
        ...
        for job in jobs:
            ...
            if not isinstance(claim, dict) or claim.get("by") != expected_owner:
                return False
            claim["at"] = _sanremes_now().isoformat()
            save_jobs(jobs)
            return True
    return False
...
def _machine_id() -> str:
    ...
    Uses ``SANREMES_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("SANREMES_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from SANREMES_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                        ...
                        if times is not None and times > 0 and completed >= times:
                            ...
                            if job.get("last_run_at") is not None:
                                ...
                                logger.warning(
                                    ...
                                    "'sanremes cron resume <job> --run-now' "
                                    "(#93524).",
                                    job.get("name", job.get("id", "?")),
                                    completed,
                                    times,
                ...
                # desktop both run in-process 60s tickers on one SANREMES_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from SANREMES_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`sanremes_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``sanremes_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _sanremes_now().strftime("%Y-%m-%d_%H-%M-%S")
...
def _canonical_skill_ref(raw: Any) -> str:
    ...
    A job may store an absolute path under ``SANREMES_HOME/skills`` or an
