...
def _fetch_monitor_url(url: str) -> tuple[bool, str]:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "sanremes-cron-monitor"})
...
def _persist_monitor_state(job_id: str, new_hash: str, output: str) -> None:
    from cron.jobs import _sanremes_now, update_job
        ...
        update_job(
            job_id,
            {
                "monitor_state": {
                    ...
                    "last_changed_at": _sanremes_now().isoformat(),
                }
            },
        )
    except Exception as exc:
