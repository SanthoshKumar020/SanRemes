#!/usr/bin/env python3
"""
SQLite State Store for SanRemes Agent.
...
# transitive dependency via sanremes_state_common). run_agent keeps its own
# predating copy — sanremes_state cannot import run_agent (circular) — guarded
...
from sanremes_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from sanremes_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from sanremes_state_portability import SessionPortabilityMixin
from sanremes_state_schema import SessionSchemaMixin
from sanremes_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``sanremes -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_sanremes_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.sanremes`` even when a test fixture later
    redirects ``SANREMES_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_sanremes_home()`` fresh so a runtime
       ``SANREMES_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_sanremes_home() / "state.db"
...
# ~/.sanremes/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects SANREMES_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without SANREMES_HOME, a stale worktree
# without the re-pin, or a developer shell that exports SANREMES_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "SANREMES_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.sanremes``).  The test conftest injects the pre-sandbox production
#: root here so custom-``SANREMES_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default SanRemes root for the guard.

    Deliberately avoids ``Path.home()`` / ``sanremes_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``sanremes_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "sanremes"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "sanremes"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".sanremes"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``SANREMES_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under SanRemes test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``SANREMES_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "SANREMES_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``SANREMES_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``sanremes`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real SanRemes home *root*.
    ...
    under ``~/.sanremes/sanremes-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``SANREMES_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``SANREMES_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real SanRemes root {root}). "
                "Tests must run against a temporary SANREMES_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "SANREMES_HOME. If this test genuinely needs the live "
...
# sanremes_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``sanremes sessions optimize`` (FTS merge + VACUUM)
    ...
    ``sanremes_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``sanremes_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``sanremes update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"SanRemes-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart SanRemes"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a SanRemes-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `sanremes doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``sanremes_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see sanremes_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with sanremes_cli.config.
        from sanremes_cli.config import cfg_get, load_config_readonly
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``sanremes doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal SanRemes host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``sanremes serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — SanRemes's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``sanremes_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        st = db_path.stat()
        try:
            from sanremes_cli.sqlite_safe_read import (
                LiveConnectionError,
                offline_file_access,
            )
        except ImportError:
            # Scaffold/embed installs ship sanremes_state without sanremes_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``sanremes_cli.sqlite_safe_read``).
    ...
    try:
        from sanremes_cli.sqlite_safe_read import has_live_connection
    except ImportError:
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the SanRemes home tree (``get_sanremes_home()``) — the safe repair scope:
      SanRemes owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``sanremes_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_sanremes_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. SanRemes needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (sanremes_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_sanremes_state.py:600-632)
        ...
        probe_session_id = f"_sanremes_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, SanRemes deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (sanremes gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.sanremes/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``sanremes sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("SANREMES_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_sanremes_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("SANREMES_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship sanremes_state without sanremes_cli). A
    ...
    """
    try:
        from sanremes_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug(
            "sanremes_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch sanremes_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``sanremes_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    if size <= 0:
        return False
    from sanremes_cli.sqlite_safe_read import read_header_bytes_preopen
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `sanremes snapshot list` / "
                "`sanremes snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (sanremes doctor, dashboards) ──────────
...
_SANREMES_CMDLINE_MARKERS = ("sanremes_cli.main", "sanremes_cli/main", "sanremes serve",
                           "sanremes-agent", "sanremes gateway", "sanremes chat")


def _looks_like_sanremes(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a SanRemes process?
    ...
    We only flag processes that look like SanRemes, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _SANREMES_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple sanremes processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling SanRemes
    ...
    # `sanremes update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `sanremes snapshot list` / "
                    f"`sanremes snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`sanremes db optimize`),
            ...
            # ``sanremes_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `sanremes update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `sanremes sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another SanRemes process held the "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a SanRemes process —
                        # only flag uninspectable holders that look like
                        # another SanRemes instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_sanremes(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from sanremes_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    #
    # `optimize_fts_storage()` (the `sanremes sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`sanremes sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``sanremes sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``sanremes --resume`` into a fresh
    ...
    def sweep_orphaned_sessions(
        ...
        *,
        ...
        ``sanremes serve``, desktop WS sidecar), each backend registers a row
    ...
    # of truth: sanremes_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: sanremes_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``sanremes -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``sanremes sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``SANREMES_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade SanRemes, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one SanRemes session.

        A SanRemes session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a SanRemes session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `sanremes sessions vacuum` runs in a transient
