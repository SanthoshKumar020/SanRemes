"""Centralized logging setup for SanRemes Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.sanremes/logs/`` (profile-aware via ``get_sanremes_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``sanremes_cli.web_server``,
    ``sanremes_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in SanRemes (TUI, gateway, ``hy_memory`` server, MCP
...
from sanremes_constants import get_config_path, get_sanremes_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_sanremes_session_injector", False):
    ...
    _session_record_factory._sanremes_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``sanremes logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``sanremes logs --component gateway``.
    "gateway": ("gateway", "sanremes_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("sanremes_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "sanremes_cli.web_server",
        "sanremes_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    sanremes_home: Optional[Path] = None,
    ...
    """Configure the SanRemes logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    sanremes_home
        Override for the SanRemes home directory.  Falls back to
        ``get_sanremes_home()`` (profile-aware).
    log_level
    ...
    home = sanremes_home or get_sanremes_home()
    from sanremes_constants import mkdir_under_sanremes_home
    log_dir = mkdir_under_sanremes_home(home / "logs")
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_sanremes_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._sanremes_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the sanremes group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from sanremes_cli.config import is_managed
...
# the module header): when several SanRemes processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._sanremes_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="sanremes-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_sanremes_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    from sanremes_constants import mkdir_under_sanremes_home
    mkdir_under_sanremes_home(path.parent)
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse sanremes_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when sanremes_cli.config isn't importable (bare
        # sanremes_logging consumers).
        try:
            from sanremes_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
