...
const logDir = join(process.env.SANREMES_HOME?.trim() || join(homedir(), '.sanremes'), 'logs')
...
// a real ~/.sanremes (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('sanremes-tui: parent lifecycle log unavailable\n')
    }
  }
}
