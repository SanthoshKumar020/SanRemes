...
//
// Both gate on SANREMES_DEV_PERF=1 and dump JSON-lines (default ~/.sanremes/perf.log,
// override SANREMES_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// SANREMES_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import { scrollFastPathStats } from '@sanremes/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.SANREMES_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.SANREMES_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.SANREMES_DEV_PERF_LOG?.trim() || join(homedir(), '.sanremes', 'perf.log')
