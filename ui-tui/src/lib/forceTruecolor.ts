...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SANREMES_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep SanRemes on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SANREMES_TUI_TRUECOLOR=1.
