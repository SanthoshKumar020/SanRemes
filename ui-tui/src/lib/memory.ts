...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because SANREMES_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.SANREMES_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.sanremes', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `sanremes-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.SANREMES_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.SANREMES_HEAPDUMP_MAX_BYTES?.trim()
