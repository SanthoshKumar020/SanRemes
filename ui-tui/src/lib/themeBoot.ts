 ...
 * `sanremes-boot-background` / `sanremes-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports SANREMES_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.sanremes matches get_sanremes_home()'s default.
const bootFilePath = () => join(process.env.SANREMES_HOME ?? join(homedir(), '.sanremes'), 'tui-theme-boot.json')

// Never touch the user's real ~/.sanremes from test runs (the TS suite has no
// SANREMES_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into SANREMES_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.SANREMES_TUI_THEME || env.SANREMES_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.SANREMES_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.SANREMES_TUI_BACKGROUND
  ) {
    env.SANREMES_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.SANREMES_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.SANREMES_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into SANREMES_TUI_THEME.
