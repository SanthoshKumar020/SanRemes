import type { InputEvent, Key } from '@sanremes/ink'
import * as Ink from '@sanremes/ink'
 ...
 * but that accepts `key.meta` on macOS — and sanremes-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.SANREMES_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which sanremes-ink decodes as meta+'d'; without this branch it
