import { Box, Text, useInput, useStdout } from '@sanremes/ink'
...
export function ModelPicker({
  ...
  useInput((ch, key) => {
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `sanremes model` to configure'
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run sanremes model)
          return
        }
    ...
    // filter. With Ctrl held, @sanremes/ink reports `ch` as the key name ('g'),
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          Paste your API key below (saved to ~/.sanremes/.env)
        </Text>
