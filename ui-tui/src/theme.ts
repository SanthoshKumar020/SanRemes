import type { SkinBranding, SkinColors } from '@sanremes/shared/skin'
...
// Mirrors @sanremes/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @sanremes/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic SanRemes navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode SanRemes look was never hand-authored: for years the
 ...
 *  answer when known (cached in SANREMES_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.SANREMES_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit SANREMES_TUI_THEME / COLORFGBG signals above still win,
...
// `SANREMES_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `SANREMES_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `SANREMES_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `SANREMES_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default SanRemes palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.SANREMES_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.SANREMES_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.SANREMES_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into SANREMES_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into SANREMES_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (SanRemes navy
  ...
  // the derived value already carries the identity seeds (e.g. SanRemes navy).
  const activeRow =
