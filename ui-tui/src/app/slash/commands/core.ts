import { forceRedraw, type MouseTrackingMode } from '@sanremes/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update SanRemes Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      if (DASHBOARD_TUI_MODE) {
      ...
      // Exit code 42 signals the Python wrapper to exec `sanremes update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('clipboard copy failed — try SANREMES_TUI_FORCE_OSC52=1 to force the escape sequence')
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `SanRemes #${i + 1}`
