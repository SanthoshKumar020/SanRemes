...
import { terminalBackgroundHex } from '@sanremes/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $SANREMES_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see SANREMES_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['SANREMES_TUI_BACKGROUND', process.env.SANREMES_TUI_BACKGROUND ?? '(unset)'],
            ['SANREMES_TUI_THEME', process.env.SANREMES_TUI_THEME ?? '(unset)'],
