import type { RunExternalProcess } from '@sanremes/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`sanremes ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching sanremes: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`sanremes ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
