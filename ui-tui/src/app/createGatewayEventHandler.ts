...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@sanremes/ink'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.SANREMES_TUI_THEME : undefined

  writeBootTheme(theme, process.env.SANREMES_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to SANREMES_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit SANREMES_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the SANREMES_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.SANREMES_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.SANREMES_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a SANREMES_TUI_THEME the user
    ...
    delete process.env.SANREMES_TUI_THEME
  }
 ...
 * is cached into SANREMES_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * SANREMES_TUI_LIGHT / SANREMES_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.SANREMES_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.SANREMES_TUI_THEME || process.env.SANREMES_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.SANREMES_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.SANREMES_TUI_BACKGROUND ||
      process.env.SANREMES_TUI_THEME ||
      process.env.SANREMES_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.SANREMES_TUI_BACKGROUND || process.env.SANREMES_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.SANREMES_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const scheduleStartupPrompt = () => {
    ...
    setTimeout(async () => {
      ...
      // launches, `sanremes --tui -q "…"`) — submit LITERALLY, bypassing the
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey SanRemes" if this surface owns it (server gates on config).
    ...
    // forging a brand-new one.  Mirrors classic CLI's `sanremes -c` /
    // `sanremes --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: GatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'wake.detected': {
        // "Hey SanRemes": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: sanremes -p ${wakeProfile} --tui`)
