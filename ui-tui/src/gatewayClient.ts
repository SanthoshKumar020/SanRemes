...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.SANREMES_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.SANREMES_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
const resolveGatewayAttachUrl = () => {
  const raw = process.env.SANREMES_TUI_GATEWAY_URL?.trim()

  return raw ? raw : null
}

const resolveSidecarUrl = () => {
  const raw = process.env.SANREMES_TUI_SIDECAR_URL?.trim()

  return raw ? raw : null
}

const resolvePython = (root: string) => {
  const configured = process.env.SANREMES_PYTHON?.trim() || process.env.PYTHON?.trim()
...
export class GatewayClient extends EventEmitter {
  ...
  private startSpawnedGateway(root: string) {
    ...
    const cwd = process.env.SANREMES_CWD || root
    const env = { ...process.env }
    const pyPath = env.PYTHONPATH?.trim()

    env.PYTHONPATH = pyPath ? `${root}${delimiter}${pyPath}` : root
    // Tell the gateway child where the SanRemes source root is so its import
    // guard can force it ahead of any same-named package in the launch cwd.
    env.SANREMES_PYTHON_SRC_ROOT = root
  ...
  start() {
    ...
    const root = process.env.SANREMES_PYTHON_SRC_ROOT ?? resolve(import.meta.dirname, '../../')
