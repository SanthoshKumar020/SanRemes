...
// ~/.sanremes. To exercise the real persistence path we clear that gate, point
// SANREMES_HOME at a temp dir, and re-import the module fresh (path + enabled
// flag are captured at module load).
const loadFresh = async (home: string) => {
  vi.resetModules()
  vi.stubEnv('VITEST', '')
  vi.stubEnv('SANREMES_HOME', home)
...
describe('recordParentLifecycle', () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'sanremes-parentlog-'))
  })
