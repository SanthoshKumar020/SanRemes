 ...
 *
 * Root cause: re-exporting `ink-text-input` from `@sanremes/ink`'s
 * entry-exports drags the upstream `ink` package into the bundle. That
 * `ink` graph and our in-tree `@sanremes/ink` graph reference each other
...
function bundleIsFresh(): boolean {
  ...
  try {
    ...
    const sourceMtime = statSync(resolve(uiTuiRoot, 'packages/sanremes-ink/src/entry-exports.ts')).mtimeMs
...
describe('TUI bundle (issue #31227)', () => {
  ...
  it('does not bundle the upstream ink package or ink-text-input', () => {
    ...
    // that #31227 was about. The in-tree fork at @sanremes/ink replaces
    // all of `ink`; nothing in ui-tui imports `TextInput` from
    // `@sanremes/ink` so the re-export is unused dead weight.
  ...
  it('has the @sanremes/ink entry-exports module compiled to sync init', () => {
    // Sanity check that the alias swap to packages/sanremes-ink/src/entry-exports.ts
    // is still active and producing the expected synchronous init shape.
    expect(bundleSrc).toMatch(/var init_entry_exports = __esm\(\{\s*"packages\/sanremes-ink\/src\/entry-exports\.ts"\(\)/)
  })
})
