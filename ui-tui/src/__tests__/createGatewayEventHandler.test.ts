...
describe('createGatewayEventHandler', () => {
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "💾 Self-improvement review: Skill 'sanremes-release' patched" },
      type: 'review.summary'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith("💾 Self-improvement review: Skill 'sanremes-release' patched")
  })
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.sanremes/config.yaml'
      },
  ...
  it('prefers raw text over Rich-rendered ANSI on message.complete (#16391)', () => {
    ...
    const rendered = '\u001b[33mSanRemes here.\u001b[0m\n\n\u001b[2mLine two.\u001b[0m'
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `sanremes model` to select a provider, or run `sanremes setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
  ...
  it('picks the polarity-matching paired palette from gateway.ready skins', async () => {
    ...
    vi.stubEnv('SANREMES_TUI_BACKGROUND', '')
    ...
    vi.stubEnv('SANREMES_TUI_BACKGROUND', '#ffffff')
    createGatewayEventHandler(buildCtx(appended))({ payload: skin, type: 'skin.changed' } as any)
  ...
  it('opens a fresh session before starting voice after wake detection', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey sanremes', start_new_session: true },
      type: 'wake.detected'
    } as any)
  ...
  it('keeps the current session when wake detection disables session creation', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey sanremes', start_new_session: false },
      type: 'wake.detected'
    } as any)
