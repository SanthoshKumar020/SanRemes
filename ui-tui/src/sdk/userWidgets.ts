...
import { Box, Text } from '@sanremes/ink'
...
/**
 * User widget apps — SanRemes authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$SANREMES_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.sanremes/plugins/`: files under SANREMES_HOME execute
...
const widgetsDir = () => join(process.env.SANREMES_HOME?.trim() || join(homedir(), '.sanremes'), 'tui-widgets')
 ...
 *  every change, so a widget SanRemes writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or SanRemes) ever writes must hot-load too — a 10s poll here read
