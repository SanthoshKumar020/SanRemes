...
React + Ink terminal UI for SanRemes. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SANREMES_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `sanremes setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.sanremes/.sanremes_history` or under `SANREMES_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `sanremes setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/sanremes-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external sanremes setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.sanremes/tui-parent.log
    ...
    types/
      sanremes-ink.d.ts            type declarations for @sanremes/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SanRemesCLI subprocess for slash commands
```
