...
storage discipline as the other indexes). On a populated database, run
    ...
    sanremes sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.sanremes/config.yaml` to disable. Override
the .so location with `SANREMES_FTS5_CJK_SO`.
