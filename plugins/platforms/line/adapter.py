"""
LINE Messaging API platform adapter for SanRemes Agent.
...
to SanRemes Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SANREMES_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            sanremes_home = Path(get_sanremes_home()).resolve()
        except Exception:
            sanremes_home = Path.home().joinpath(".sanremes").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``sanremes status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``sanremes status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``sanremes setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.sanremes/.env`` via ``sanremes_cli.config``.
    """
    print()
    ...
    try:
        from sanremes_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("sanremes_cli.config not available; set LINE_* vars manually in ~/.sanremes/.env")
        return
