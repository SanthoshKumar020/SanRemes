...
def _is_allowed_bridge_path(url: str) -> bool:
    ...
    known SanRemes media cache directory.
    ...
    ``<kind>_cache`` layout that ``get_sanremes_dir`` may return.
    """
    try:
...
def check_whatsapp_requirements() -> bool:
    ...
    """
    # Prefer SanRemes-managed Node/npm so Windows installs are not broken by a
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_sanremes_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `sanremes gateway`.",
                retryable=False,
            )
            return False
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Pair from the dashboard or run `sanremes whatsapp`; remove "
                "WHATSAPP_ENABLED from your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — pair from the dashboard or run `sanremes whatsapp`.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `sanremes update` bumps the Baileys pin).  The stamp file records
            # the package.json hash of the last successful install.
            bridge_dir = bridge_path.parent
            _pkg_json = bridge_dir / "package.json"
            _dep_stamp = bridge_dir / "node_modules" / ".sanremes-pkg-hash"
            ...
            if not _deps_fresh:
                ...
                # SanRemes-managed portable Node before falling back to PATH.
                ...
                try:
                    ...
                    if install_result.returncode != 0:
                        ...
                        self._set_fatal_error(
                            ...
                            f"WhatsApp bridge npm install failed. Run `cd {bridge_dir} && {_npm_bin} install` manually, then restart `sanremes gateway`.",
                            retryable=False,
                        )
                        return False
                    ...
                    self._set_fatal_error(
                        "whatsapp_npm_install_failed",
                        f"WhatsApp bridge npm install failed ({e}). Run `cd {bridge_dir} && {_npm_bin} install` manually, then restart `sanremes gateway`.",
                        retryable=False,
                    )
                    return False
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `sanremes update`, so without this check it
            ...
            # with_sanremes_node_path() copies os.environ when called with no arg.
            bridge_env = with_sanremes_node_path()
            ...
            # hardcodes ~/.sanremes/{image,audio,document}_cache, which diverges
            # under SANREMES_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
                get_audio_cache_dir as _get_audio_dir,
                get_document_cache_dir as _get_doc_dir,
                get_image_cache_dir as _get_img_dir,
            )
            bridge_env["SANREMES_IMAGE_CACHE_DIR"] = str(_get_img_dir())
            bridge_env["SANREMES_AUDIO_CACHE_DIR"] = str(_get_audio_dir())
            bridge_env["SANREMES_DOCUMENT_CACHE_DIR"] = str(_get_doc_dir())
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: sanremes whatsapp")
...
# dict in sanremes_cli/gateway.py, and the _send_whatsapp dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in sanremes_cli/gateway.py and the
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``sanremes setup`` even
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via sanremes_cli.gateway.get_env_value (not os.getenv) so setup-status
