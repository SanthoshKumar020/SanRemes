"""ntfy platform adapter (SanRemes plugin).
...
a SanRemes dependency.

This adapter ships as a SanRemes platform plugin under
``plugins/platforms/ntfy/``. The SanRemes plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "sanremes-in"              # subscribe topic (incoming)
          publish_topic: "sanremes-out"     # optional — defaults to topic
...
_ECHO_TAG = "sanremes-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``sanremes cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a SanRemes dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `sanremes gateway status` without
