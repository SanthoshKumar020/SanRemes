...
In single-bot mode (what SanRemes currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_sanremes_home() / "feishu_seen_message_ids.json"
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(
                32,
                env_int("SANREMES_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE),
            ),
            text_batch_delay_seconds=env_float(
                "SANREMES_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS
            ),
            text_batch_split_delay_seconds=env_float(
                "SANREMES_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0
            ),
            text_batch_max_messages=max(
                1,
                env_int("SANREMES_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES),
            ),
            text_batch_max_chars=max(
                1,
                env_int("SANREMES_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS),
            ),
            media_batch_delay_seconds=env_float(
                "SANREMES_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS
            ),
            webhook_host=str(
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10,
                    thread_name_prefix="sanremes-feishu-sdk",
                )
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local SanRemes gateway is already using this Feishu app_id"
    ...
    async def send_exec_approval(
        ...
        The buttons carry ``sanremes_action`` in their value dict so that
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return {
                    ...
                    "type": btn_type,
                    "value": {"sanremes_action": action_name, "approval_id": approval_id},
                }
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return {
                ...
                "type": btn_type,
                "value": {
                    "sanremes_update_prompt_action": answer,
    ...
    def _write_update_prompt_response(answer: str) -> None:
        response_path = get_sanremes_home() / ".update_response"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that SanRemes does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        sanremes_action = action_value.get("sanremes_action") if isinstance(action_value, dict) else None
        update_prompt_action = (
            action_value.get("sanremes_update_prompt_action")
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("sanremes_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        answer = str(action_value.get("sanremes_update_prompt_action", "") or "").strip().lower()
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by SanRemes webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto SanRemes' SessionSource fields.
    ...
    async def _connect_websocket(self) -> None:
        ...
        self._ws_client = FeishuWSClient(
            ...
            # See https://github.com/NousResearch/sanremes-agent/issues/50656
...
# Called by `sanremes gateway setup` via _setup_feishu() in sanremes_cli/gateway.py.
# =============================================================================
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    if "?" in qr_url:
        qr_url += "&from=sanremes&tp=sanremes"
    else:
        qr_url += "?from=sanremes&tp=sanremes"
    return {
...
# dict in sanremes_cli/gateway.py, and the _send_feishu dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    """
    if not await asyncio.to_thread(_load_lark_oapi):
        return {"error": "Feishu dependencies not installed. Run `sanremes setup` to install Feishu support."}
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_feishu in sanremes_cli/gateway.py and the static
    ...
    from sanremes_cli.setup import prompt_choice
    ...
    if access_idx == 0:
        ...
        print_info("Unknown users can request access; approve with `sanremes pairing approve`.")
    elif access_idx == 1:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `sanremes setup` to install Feishu support.",
