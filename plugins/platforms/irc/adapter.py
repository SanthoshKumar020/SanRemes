"""
IRC Platform Adapter for SanRemes Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the SanRemes agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "sanremes-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :SanRemes Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :SanRemes Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: sanremes_, sanremes_1, sanremes_2...
...
def check_requirements() -> bool:
    ...
    # sanremes setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `sanremes gateway setup` flow for the IRC platform.

    Lazy-imports ``sanremes_cli.setup`` helpers so the plugin stays importable
    in non-CLI contexts (gateway runtime, tests).
    """
    from sanremes_cli.setup import (
        prompt,
        prompt_yes_no,
        save_env_value,
        get_env_value,
    ...
    print_info("Connect SanRemes to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #sanremes — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.sanremes/.env")
    print_info("Restart the gateway for changes to take effect: sanremes gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "sanremes-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "sanremes-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :SanRemes Agent (cron)")
