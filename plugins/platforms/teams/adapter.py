"""
Microsoft Teams platform adapter for SanRemes Agent.
...
Requires:
    ...
    manually: ``<sanremes-venv>/bin/pip install microsoft-teams-apps aiohttp``)
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    SanRemes owns dotenv loading; third-party import side effects must not.
    """
    try:
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "SanRemes"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        headers = {"User-Agent": "Mozilla/5.0 (compatible; SanRemesAgent/1.0)"}
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        sanremes_action = data.get("sanremes_action", "")
        session_key = data.get("session_key", "")

        if not sanremes_action or not session_key:
        ...
        choice = choice_map.get(sanremes_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="sanremes_approve",
            data={**btn_data_base, "sanremes_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="sanremes_approve",
                data={**btn_data_base, "sanremes_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="sanremes_approve",
                    data={**btn_data_base, "sanremes_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="sanremes_approve",
            data={**btn_data_base, "sanremes_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"SanRemes\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.sanremes/.env")
...
def _install_hint() -> str:
    ...
    ``feature_install_command(venv_pip=True)`` targets the actual SanRemes
