...
and injects content-free wake hints into SanRemes' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # SanRemes only verifies that wake hints are content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active SanRemes turn."""
        if not self._message_handler:
            return
...
def interactive_setup() -> None:
    """Interactive ``sanremes gateway setup`` flow for the Raft platform.

    Lazy-imports CLI helpers so the plugin stays importable in gateway runtime
    and test contexts. The flow persists ``RAFT_PROFILE`` to the SanRemes env
    ...
    print_info("Connect SanRemes to Raft as an external agent.")
