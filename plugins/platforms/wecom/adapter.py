...
class WeComAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("SANREMES_WECOM_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("SANREMES_WECOM_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        *,
        ...
        if is_final:
            try:
                ...
                # resend.  SanRemes' prior behaviour — raising RuntimeError so
...
_QR_CODE_PAGE = "https://work.weixin.qq.com/ai/qc/gen?source=sanremes&scode="
...
def qr_scan_for_bot_info(
    *,
    ...
    generate_url = f"{_QR_GENERATE_URL}?source=sanremes"

    # ── Step 1: Fetch QR code ──
    print("  Connecting to WeCom...", end="", flush=True)
    try:
        req = urllib.request.Request(generate_url, headers={"User-Agent": "SanRemesAgent/1.0"})
    ...
    while time.monotonic() < deadline:
        try:
            req = urllib.request.Request(query_url, headers={"User-Agent": "SanRemesAgent/1.0"})
...
# wizard + _PLATFORMS["wecom"] static dict in sanremes_cli/gateway.py, and the
...
def interactive_setup() -> None:
    ...
    Replaces sanremes_cli/gateway.py::_setup_wecom and the static
    ...
    if allowed:
        ...
        access_idx = prompt_choice(
            ...
            [
                ...
                "Use DM pairing (unknown users request access, you approve with 'sanremes pairing approve')",
        ...
        if access_idx == 0:
            ...
            print_info("Approve with: sanremes pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("Direct messages disabled.")
        else:
            print_info("Skipped — configure later with 'sanremes gateway setup'")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `sanremes setup` to install WeCom support.",
