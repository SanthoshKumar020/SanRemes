...
# trigger its own recovery path. Refs: NousResearch/sanremes-agent#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: NousResearch/sanremes-agent#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("SANREMES_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "SANREMES_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "SANREMES_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. SanRemes-created private-chat topic lanes are marked with
        ...
        queued follow-ups after a gateway restart) prefer the SanRemes topic's
        ``message_thread_id`` so they stay in the active topic lane (#87051);
        ``direct_messages_topic_id`` is only used when no topic thread
        resolves, since the native DM-topic id does not match the SanRemes
        ...
        if metadata and metadata.get("telegram_dm_topic_reply_fallback"):
            ...
            if reply_to_message_id is None:
                ...
                # active topic lane: prefer the SanRemes topic thread id when it
    ...
    # older PTB/clients. Streaming edits stay on SanRemes' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        SanRemes must skip rich delivery up front and use the legacy MarkdownV2
        ...
        try:
            if app and app.updater and app.updater.running:
                try:
                    ...
                    # Refs: NousResearch/sanremes-agent#58270
                    await _await_with_thread_deadline(
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `sanremes update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other SanRemes or OpenClaw instance is running "
            "with this token, then restart the gateway with 'sanremes gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                from sanremes_cli.commands import telegram_menu_commands, telegram_menu_max_commands
                if not self._bot:
                    return
                # Telegram allows up to 100 commands but has an undocumented
                # payload size limit (~4KB total).  SanRemes defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the SanRemes process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("SANREMES_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("SANREMES_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("SANREMES_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("SANREMES_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("SANREMES_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            ...
            disable_fallback = (
                os.getenv("SANREMES_TELEGRAM_DISABLE_FALLBACK_IPS", "")
                .strip()
                .lower()
                in {"1", "true", "yes", "on"}
            )
            ...
            if not fallback_ips and not disable_fallback:
                discovery_timeout = self._env_float_clamped(
                    "SANREMES_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT",
                    5.0,
                    min_value=0.0,
                )
                logger.warning(
            ...
            _max_connect = 8
            _init_timeout = _env_float("SANREMES_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            ...
                            f"or set SANREMES_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                            f"SANREMES_TELEGRAM_INIT_TIMEOUT to a lower value."
                        )
                    logger.warning(
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or set SANREMES_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
            ...
            if webhook_url:
                ...
                if not webhook_secret:
                    raise RuntimeError(
                        ...
                        "https://github.com/NousResearch/sanremes-agent/"
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``sanremes model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from sanremes_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from sanremes_cli.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.sanremes/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".sanremes" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@sanremes_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several SanRemes bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the SanRemes inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the SanRemes inbox (#52363).
        #
...
# sanremes_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("SANREMES_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in sanremes_cli/gateway.py.
    """
    from sanremes_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `sanremes setup` to install Telegram support.",
