...
# identify SanRemesAgent traffic — matching other SanRemes outbound surfaces
# that already set ``SanRemesAgent/<version>`` for platform-partner attribution.
try:
    from sanremes_cli import __version__ as _SANREMES_VERSION
except Exception:
    _SANREMES_VERSION = "unknown"
_SANREMES_SLACK_USER_AGENT_PREFIX = f"SanRemesAgent/{_SANREMES_VERSION}"
...
def _slack_unfurl_kwargs(extra: Optional[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    ``_slack_unfurl_hints``: ``sanremes config set`` and Railway persist YAML
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from sanremes_cli.commands import is_gateway_known_command
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # After the label, so that ``<@U…|sanremes>`` is stripped like ``<@U…>``.
    if bot_uid:
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `sanremes slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `sanremes gateway setup` "
                "or in the active profile's ~/.sanremes/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `sanremes gateway setup` "
                "or add it to your active profile's ~/.sanremes/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `sanremes gateway setup` "
                "or in the active profile's ~/.sanremes/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `sanremes gateway setup` "
                "or add it to your active profile's ~/.sanremes/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_sanremes_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_SANREMES_SLACK_USER_AGENT_PREFIX,
            )
            self._app = AsyncApp(
                token=primary_token,
            ...
            # SanRemes has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by SanRemes can "
                    "be removed from the Slack app manifest via "
                    "`sanremes slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /sanremes prefix). A single regex
            ...
            # manifest (see `sanremes slack manifest`). In Socket Mode, Slack
            ...
            from sanremes_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/sanremes$")

            @self._app.command(_slash_pattern)
            async def handle_sanremes_command(ack, command):
            ...
            for _action_id in (
                "sanremes_approve_once",
                "sanremes_approve_session",
                "sanremes_approve_always",
                "sanremes_deny",
            ):
            ...
            for _action_id in (
                "sanremes_confirm_once",
                "sanremes_confirm_always",
                "sanremes_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("sanremes_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^sanremes_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("sanremes_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'sanremes slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: SanRemes handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own SanRemes session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # SanRemes owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_sanremes_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_sanremes_no_thread_response"] = True
    ...
    async def _handle_slack_message_impl(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_sanremes_no_thread_response"):
        ...
        force_process = bool(event.get("_sanremes_force_process"))
        ...
        # the shape produced by peer SanRemes agents in Socket Mode on some
        ...
        try:
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "sanremes_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "sanremes_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "sanremes_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "sanremes_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "sanremes_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "sanremes_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``sanremes_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``sanremes_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"sanremes_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "sanremes_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "sanremes_confirm_once": "once",
            "sanremes_confirm_always": "always",
            "sanremes_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "sanremes_approve_once": "once",
            "sanremes_approve_session": "session",
            "sanremes_approve_always": "always",
            "sanremes_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "sanremes_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/sanremes <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/sanremes
        ...
        if slash_name in {"sanremes", ""}:
            # Legacy /sanremes <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from sanremes_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/sanremes   ``).
        ...
        # "Running /cmd…" ack shown by handle_sanremes_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/sanremes <question>" must produce public replies so
...
# ``sanremes_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_sanremes_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``sanremes_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from sanremes_cli.config import get_env_value, remove_env_value, save_env_value
    from sanremes_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under SANREMES_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from sanremes_cli.slack_cli import _build_full_manifest
            from sanremes_constants import get_sanremes_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="SanRemes",
                bot_description="Your SanRemes agent on Slack",
            )
            target = Path(get_sanremes_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `sanremes slack manifest --write` anytime to refresh after "
                "SanRemes adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `sanremes update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://sanremes-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
    ...
    print()
    print_info("📬 Home Channel: where SanRemes delivers cron job results,")
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``sanremes_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    """Plugin entry point — called by the SanRemes plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `sanremes setup` to install Slack support.",
        # Interactive setup wizard — replaces sanremes_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in sanremes_cli/gateway.py.
