# A2A — Agent-to-Agent protocol for SanRemes
...
peer (another SanRemes, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
sanremes gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, SanRemes serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.sanremes/a2a_audit.jsonl`.
- Conversations persist to `~/.sanremes/a2a_conversations/` — they survive context
