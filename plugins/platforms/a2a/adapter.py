"""
A2A inbound platform adapter — exposes SanRemes as an A2A-discoverable agent.
...
def _default_agent_name() -> str:
    ...
    try:
        import socket
        return f"sanremes-{socket.gethostname()}"
    except Exception:
        return "sanremes-agent"
...
def _active_profile_name() -> str:
    try:
        ...
        return os.getenv("SANREMES_PROFILE", "default") or "default"
    ...
    try:
        ...
        if not profile or profile == "default":
            try:
                ...
                return None
        return os.path.expanduser(f"~/.sanremes/profiles/{profile}")
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _load_served_agents(self, extra: dict) -> dict[str, dict]:
        ...
        default_desc = os.getenv(
            ...
            "SanRemes Agent — a general-purpose agent reachable over A2A.",
        )
        agents[""] = {
            "slug": "",
            "path": "",
        ...
        for key, val in items:
            ...
            agents[slug] = {
                ...
                "local": local,
                "name": str(val.get("name") or f"SanRemes {slug}"),
                "description": str(val.get("description") or f"SanRemes profile '{profile or slug}' exposed over A2A."),
    ...
    def _build_card(self, public_url: Optional[str] = None, agent: Optional[dict] = None) -> dict:
        ...
        return protocol.build_agent_card(
            ...
            url=url,
            description=agent.get("description") or "SanRemes Agent — a general-purpose agent reachable over A2A.",
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        """Forward a routed A2A task to another local SanRemes profile.
        ...
        with lock:
            ...
            cmd = ["sanremes", "chat", "-q", framed_text, "-Q", "--source", "a2a"]
            if session_id:
            ...
            home = _profile_home(profile)
            if home:
                env["SANREMES_HOME"] = home
            env["SANREMES_A2A_PEER"] = peer
            start = time.time()
            try:
                proc = subprocess.run(
