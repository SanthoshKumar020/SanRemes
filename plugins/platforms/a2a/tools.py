"""
A2A client tools — let the SanRemes agent talk to *other* agents as a peer.
...
def a2a_history(args: dict, **_: Any) -> str:
    ...
    written to ~/.sanremes/a2a_conversations/<context>.jsonl and can be reloaded
    here.
    """
    context_id = str(args.get("context_id") or args.get("contextId") or "").strip()
    if not context_id:
...
def _a2a_tools_available() -> bool:
    ...
    (a peer-reachable SanRemes plausibly dials back).
