...
def build_agent_card(
    *,
    ...
    card: dict[str, Any] = {
        ...
        "version": "1.0.0",
        "provider": {
            "organization": os.getenv("A2A_PROVIDER_ORG", "SanRemes Agent"),
    ...
    if isinstance(toolsets, dict):
        for ts_name in sorted(toolsets.keys()):
            ...
            skills.append({
                ...
                "name": ts_name,
                "description": f"SanRemes '{ts_name}' capabilities",
        ...
        for ts in sorted(set(toolsets or [])):
            skills.append({
                "id": f"toolset.{ts}",
                "name": ts,
                "description": f"SanRemes '{ts}' capabilities",
                "tags": [ts],
            })
    if not skills:
        skills.append({
...
def _conv_dir() -> Path:
    try:
        ...
        base = Path(get_sanremes_home())
    except Exception:
        base = Path(os.path.expanduser("~/.sanremes"))
