"""SimpleX Chat platform adapter (SanRemes plugin).
...
This adapter ships as a SanRemes platform plugin under
``plugins/platforms/simplex/``. The SanRemes plugin loader scans the
...
Optional environment variables:
    ...
    SANREMES_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``sanremes setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside SanRemes — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("SANREMES_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``sanremes setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.sanremes/.env`` via
    ``sanremes_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "sanremes_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.sanremes/.env"
        )
        return
