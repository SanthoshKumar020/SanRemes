...
under an immutable ``/opt/sanremes`` (read-only for the sanremes user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$SANREMES_HOME/photon/sidecar`` (the durable data volume,
...
def dir_writable(path: Path) -> bool:
    ...
    resolver does.
    """
    probe = path / ".sanremes-write-probe"
    try:
        probe.touch()
        probe.unlink()
        return True
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_sanremes_home() / "photon" / "sidecar"
    try:
