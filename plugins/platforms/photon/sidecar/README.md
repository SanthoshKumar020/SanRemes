...
Small Node helper that bridges SanRemes Agent to Photon's Spectrum SDK
(`spectrum-ts`).  SanRemes is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-SanRemes-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and SanRemes can receive inbound messages
...
The SanRemes plugin's `sanremes photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. SanRemes is Python, so replies go through this sidecar
