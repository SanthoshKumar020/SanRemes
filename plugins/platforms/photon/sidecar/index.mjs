// SanRemes Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> SanRemes): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (SanRemes -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-SanRemes-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `sanremes photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so SanRemes can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "sanremes-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply SanRemes'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local SanRemes allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of SanRemes, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-sanremes-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when SanRemes supplied them so a known-good
