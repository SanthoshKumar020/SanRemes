...
This plugin connects SanRemes Agent to iMessage (and other Spectrum
...
TypeScript-only, SanRemes runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other SanRemes channel,
...
Run `sanremes photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.sanremes/.env` (the same place every other
...
Management metadata lives in `~/.sanremes/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `sanremes photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, SanRemes
  ...
  `.pluginPayloadAttachment` images immediately after the URL; SanRemes coalesces
...
- **Native polls are supported.** SanRemes posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `sanremes send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<sanremes-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `sanremes photon status`, a DM and a group roundtrip,
