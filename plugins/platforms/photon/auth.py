...
Credential storage mirrors every other SanRemes channel:

    * runtime SDK creds  -> ``~/.sanremes/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.sanremes/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a sanremes dependency
...
# registers SanRemes as its own client_id.
...
# Default name of the project SanRemes provisions for the operator.
DEFAULT_PROJECT_NAME = "SanRemes Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of sanremes-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.sanremes/auth.json`` honouring the active SanRemes profile."""
    try:
        from sanremes_constants import get_sanremes_home
        return Path(get_sanremes_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.sanremes")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # sanremes_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from sanremes_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.sanremes/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.sanremes/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.sanremes/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: sanremes_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `sanremes photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `sanremes photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `sanremes photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `sanremes photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `sanremes photon setup`)"
