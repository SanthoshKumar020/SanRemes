"""
``sanremes photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other SanRemes
...
# wiring imports this module for `sanremes --help`).
...
# when called later (gateway start, sanremes status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `sanremes photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'SanRemes Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``sanremes photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "SanRemes Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.sanremes/.env,
    ...
    try:
        from sanremes_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  sanremes gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `sanremes photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`sanremes photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.sanremes/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `sanremes photon telemetry on` / `sanremes photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.sanremes/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  sanremes gateway restart")
    return 0
...
#
# `sanremes gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``sanremes photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `sanremes gateway setup` wizard."""
