"""
Photon Spectrum (iMessage) platform adapter for SanRemes Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a SanRemes dep
...
#
# Out-of-process senders (cron subprocesses, `sanremes send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_sanremes_home() honors profile overrides — never hardcode ~/.sanremes.
    from sanremes_constants import get_sanremes_home

    return get_sanremes_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (sanremes status, test collection, plugin discovery).
...
# artifacts so SanRemes sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `sanremes photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `sanremes photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `sanremes setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, sanremes status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: sanremes photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: sanremes photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `sanremes update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``sanremes photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so SanRemes does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use SanRemes defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-SanRemes-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any SanRemes checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `sanremes photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                ...
                raise PhotonSidecarStartupError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `sanremes photon setup`)",
                    code="SIDECAR_DEPS_MISSING",
                    retryable=False,
                )
        # A `sanremes update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `sanremes send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`sanremes send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the SanRemes gateway (which spawns the sidecar and "
                    "records its address under <sanremes-home>/runtime/"
    ...
    headers = {"X-SanRemes-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the SanRemes plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `sanremes photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: sanremes photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `sanremes gateway setup` alongside every other
    ...
    # Register CLI subcommands — `sanremes photon ...`
