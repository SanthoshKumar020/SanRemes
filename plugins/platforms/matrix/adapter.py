...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable SanRemes command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    SanRemes accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal SanRemes ``/command`` text."""
...
# Uses get_sanremes_home() so each profile gets its own Matrix store.
from sanremes_constants import get_sanremes_dir as _get_sanremes_dir

_STORE_DIR = _get_sanremes_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches SanRemes
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("SANREMES_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("SANREMES_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "sanremes"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "SanRemes Agent" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in sanremes_cli/{setup,gateway}.py, and the
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        txn_id = f"sanremes_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces sanremes_cli/setup.py::_setup_matrix
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where SanRemes delivers cron job results and notifications.")
...
def _is_connected(config) -> bool:
    ...
    configured. Read via sanremes_cli.gateway.get_env_value so setup-status
