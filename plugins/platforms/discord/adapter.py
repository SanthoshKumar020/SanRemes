...
def _needs_server_members_intent(
    ...
    """Return True when SanRemes must request Discord's Server Members intent.
...
def _format_privileged_intents_guidance(*, needs_members: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "Docs: https://sanremes-agent.nousresearch.com/docs/user-guide/messaging/discord",
        ]
    )
    return "\n".join(lines)
...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("SANREMES_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid SANREMES_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.sanremes/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("SANREMES_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("SANREMES_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="SANREMES_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="SANREMES_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        self._discord_recovery_store = DiscordRecoveryStore(get_sanremes_home())
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the SanRemes gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents SanRemes requested (#79430): Message
    ...
    def _platform_events_subscribed() -> bool:
        ...
        try:
            from sanremes_cli.lifecycle import has_hook
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``SANREMES_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("SANREMES_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_sanremes_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage SanRemes response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs SanRemes work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit SanRemes/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:sanremes|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields SanRemes manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "sanremes_voice",
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit SanRemes pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your SanRemes session")
        ...
        @tree.command(name="status", description="Show SanRemes session status")
        ...
        @tree.command(name="stop", description="Stop the running SanRemes agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.sanremes/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update SanRemes Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the SanRemes gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a SanRemes session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to SanRemes in the thread",
        ...
        # sanremes_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from sanremes_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from sanremes_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a SanRemes skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from sanremes_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new SanRemes session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by SanRemes: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        SanRemes has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "SanRemes"
        if len(content) > 80:
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_sanremes_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by SanRemes: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="SanRemes semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "SanRemes session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 SanRemes handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want SanRemes to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ SanRemes needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **SanRemes needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``sanremes update --gateway``
        ...
        *,
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ SanRemes could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_sanremes_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``sanremes pairing approve``).
    ...
    # so users approved via ``sanremes pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``sanremes update`` prompts.
...
# is not in this process (e.g. ``sanremes cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "SANREMES_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "SANREMES_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``sanremes_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `sanremes setup` to install Discord support.",
        # Interactive setup wizard — replaces the central
        # sanremes_cli/setup.py::_setup_discord function.  Same shape as Teams.
