"""
Email platform adapter for the SanRemes gateway.

Allows users to interact with SanRemes by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "sanremes-agent" "version" "{_sanremes_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `sanremes config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `sanremes gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "SanRemes Agent")
        ...
        msg_id = f"<sanremes-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# sanremes_cli/gateway.py, and the _send_email dispatch in
