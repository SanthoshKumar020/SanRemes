...
# ``model_tools`` import time, so ``sanremes status``, ``sanremes chat``, etc.
...
# ``sanremes_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "SanRemes is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a sanremes MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "sanremes-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SANREMES_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from sanremes_constants import get_sanremes_home as _get_sanremes_home
            _sanremes_home = _get_sanremes_home()
        except (ModuleNotFoundError, ImportError):
            _sanremes_home = _Path.home() / ".sanremes"
        self._thread_count_store = _ThreadCountStore(
            _sanremes_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "SanRemes is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SANREMES_HOME", str(_Path.home() / ".sanremes"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SANREMES_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a sanremes MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'SanRemes is thinking…' marker message.
        ...
        "SanRemes is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "SanRemes is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "SanRemes is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "SanRemes is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``sanremes setup``.

    The setup wizard at ``sanremes_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.sanremes/.env`` so the next
    ...
    from sanremes_cli.config import get_env_value, save_env_value
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. sanremes-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.sanremes/.env")
    print_info("Restart the gateway: sanremes gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``sanremes cron`` running as a
    separate process from ``sanremes gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the SanRemes plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `sanremes setup` to install Google Chat support.",
        ...
        # the "SanRemes is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'SanRemes is thinking…' marker message "
