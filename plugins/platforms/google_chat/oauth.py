...
- Per-user tokens (keyed by sender email):
    ``${SANREMES_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${SANREMES_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${SANREMES_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${SANREMES_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${SANREMES_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's SANREMES_HOME helper so the token follows the user's
# profile (e.g. tests can override via SANREMES_HOME=/tmp/...).
try:
    from sanremes_constants import display_sanremes_home, get_sanremes_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where sanremes_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _sanremes_home.py shim).
...
def _sanremes_home() -> Path:
    """Resolve SANREMES_HOME at call time (NOT module import).

    Tests and ``SANREMES_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.sanremes/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _sanremes_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _sanremes_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _sanremes_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _sanremes_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _sanremes_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `sanremes setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        from sanremes_cli.tools_config import _pip_install
        ...
        print("Run `sanremes setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into SANREMES_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_sanremes_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_sanremes_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for SanRemes (native attachment delivery)"
    )
