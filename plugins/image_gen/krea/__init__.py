...
# SanRemes uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
...
def _poll_krea_job(
    ...
    *,
    ...
    headers = {
        ...
        "User-Agent": "SanRemes-Agent/1.0 (krea-image-gen)",
    }
        ...
        **kwargs: Any,
        ...
        if managed is not None:
            ...
            if not auth_token:
                return error_response(
                    error=(
                        "KREA_API_KEY not set. Run `sanremes tools` → Image "
            ...
            if managed is not None and 400 <= status < 500:
                hint = (
                    ...
                    if status == 429
                        ...
                        "`sanremes tools` → Image Generation."
                    )
                )
                return error_response(
                    error=(
