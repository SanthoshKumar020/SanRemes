...
Output is saved as PNG under ``$SANREMES_HOME/cache/images/``. Source images for
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `sanremes auth codex` (or `sanremes setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`sanremes auth codex` (or `sanremes setup` → Codex) to sign in."
                ),
