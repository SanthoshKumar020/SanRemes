...
agent's existing :func:`~sanremes_cli.runtime_provider.resolve_runtime_provider`,
...
# image_gen.model from ``sanremes tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        after this code shipped — is selectable in ``sanremes tools``. Nous
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``sanremes tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `sanremes tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/NousResearch/sanremes-agent",
