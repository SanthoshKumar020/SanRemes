...
three virtual model IDs so the ``sanremes tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `sanremes tools` → Image "
                    "Generation → OpenAI to configure, or `sanremes setup` "
                    "to add the key."
                ),
