"""langfuse — SanRemes plugin for Langfuse observability.

Traces SanRemes conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the SanRemes plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``sanremes plugins enable
observability/langfuse`` or ``sanremes tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``sanremes tools`` or ~/.sanremes/.env):
  SANREMES_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  SANREMES_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  SANREMES_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  SANREMES_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  SANREMES_LANGFUSE_RELEASE     - release/version tag
  SANREMES_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  SANREMES_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  SANREMES_LANGFUSE_CAPTURE     - content capture mode (default: "sanitized")
  ...
  SANREMES_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("SANREMES_LANGFUSE_DEBUG")
...
def _capture_mode() -> str:
    ...
    value = _env("SANREMES_LANGFUSE_CAPTURE").lower()
    if not value:
    ...
    if not _warned_invalid_capture:
        ...
        logger.warning(
            "Langfuse plugin: invalid SANREMES_LANGFUSE_CAPTURE=%r, falling back "
            "to %r (valid: %s)",
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the SanRemes plugin system —
    ...
    with _LANGFUSE_CLIENT_LOCK:
        ...
        if Langfuse is None:
            logger.warning(
                ...
                "tracing is disabled. Run `sanremes tools` and configure Langfuse "
                "Observability to reinstall it."
            )
            _LANGFUSE_CLIENT = _INIT_FAILED
            return None

        public_key = _env("SANREMES_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
        secret_key = _env("SANREMES_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
        ...
        placeholder_issues = [
            msg
            for msg in (
                _validate_langfuse_key("SANREMES_LANGFUSE_PUBLIC_KEY", public_key),
                _validate_langfuse_key("SANREMES_LANGFUSE_SECRET_KEY", secret_key),
            )
            if msg
        ]
        if placeholder_issues:
            logger.warning(
                ...
                "or unset SANREMES_LANGFUSE_PUBLIC_KEY / SANREMES_LANGFUSE_SECRET_KEY to "
        ...
        base_url = _env("SANREMES_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
        environment = _env("SANREMES_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
        release = _env("SANREMES_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
        sample_rate = _env("SANREMES_LANGFUSE_SAMPLE_RATE")
        ...
        if sample_rate:
            try:
                ...
                logger.warning("Invalid SANREMES_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
        ...
        # short-lived-process gap (kanban workers / sanremes chat -q / cron): exit
...
def _trace_key(
    ...
    *,
    ...
    Older SanRemes paths only expose ``task_id``/``session_id``. Newer paths
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("SANREMES_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def _canonical_usage_and_cost(
    ...
    *,
    ...
    """Translate canonical SanRemes usage into Langfuse usage and cost maps."""
    ...
    # SanRemes estimate rather than summing components because it also includes
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="SanRemes turn",
                tags=["sanremes", "langfuse"],
            ):
...
def _finalize_all_traces() -> None:
    ...
    kanban workers, ``sanremes chat -q`` one-shots, cron jobs — can exit while
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older SanRemes branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current SanRemes also has a turn-scoped
    ...
    # messages is a list only for legacy SanRemes branches that fired
    # pre_llm_call with API messages directly. Current SanRemes fires
...
def register(ctx) -> None:
    ...
    # SanRemes versions.  pre_api_request / post_api_request fire per API
