...
``$SANREMES_HOME/workspace/meetings/<meeting-id>/``. The main sanremes process
...
Run standalone for debugging::
    ...
    SANREMES_MEET_URL=https://meet.google.com/abc-defg-hij \\
    SANREMES_MEET_OUT_DIR=/tmp/meet-debug \\
    SANREMES_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``SANREMES_MEET_OUT_DIR``.
...
# and exposes ``window.__sanremesMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__sanremesMeetInstalled) return;
  window.__sanremesMeetInstalled = true;
  window.__sanremesMeetQueue = [];
  ...
  function pushEntry(speaker, text) {
    ...
    window.__sanremesMeetQueue.push({
      ts: Date.now(),
  ...
  window.__sanremesMeetDrain = () => {
    const out = window.__sanremesMeetQueue.slice();
    window.__sanremesMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "sanremes_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("SANREMES_MEET_URL", "").strip()
    out_dir_env = os.environ.get("SANREMES_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("SANREMES_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("SANREMES_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("SANREMES_MEET_GUEST_NAME", "SanRemes Agent")
    duration_s = _parse_duration(os.environ.get("SANREMES_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when SANREMES_MEET_MODE=realtime.
    mode = os.environ.get("SANREMES_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("SANREMES_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("SANREMES_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("SANREMES_MEET_REALTIME_INSTRUCTIONS", "")
    # SANREMES_MEET_REALTIME_KEY is set explicitly by process_manager.start(),
    ...
    realtime_api_key = os.environ.get("SANREMES_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — SANREMES_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: SANREMES_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in SANREMES_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("SANREMES_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__sanremesMeetDrain && window.__sanremesMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__sanremesMeetInstalled) {
