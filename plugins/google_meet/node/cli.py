"""`sanremes meet node ...` subcommand tree.

Wired into the existing ``sanremes meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``sanremes meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="sanremes-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``sanremes meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             sanremes meet node approve <name> ws://<host>:{args.port} {token}")
        try:
