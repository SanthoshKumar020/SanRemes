...
Launched by ``sanremes meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$SANREMES_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``sanremes meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_sanremes_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "sanremes-meet-node",
    ) -> None:
        self.host = host
        self.port = port
