...
Let the sanremes agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where sanremes runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `sanremes meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `sanremes meet` CLI |
...
| `cli.py` | `sanremes meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SANREMES_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `sanremes meet node {run,list,approve,remove,status,ping}` |
...
sanremes meet install                                      # pip + Chromium
sanremes meet setup                                        # preflight
sanremes meet auth                                         # optional
sanremes meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
sanremes meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.sanremes/.env
sanremes meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
sanremes meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
sanremes meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.sanremes/.env
sanremes meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, sanremes will **not** switch your system audio input automatically — the
...
sanremes plugins enable google_meet
sanremes meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
sanremes meet node ping my-mac
