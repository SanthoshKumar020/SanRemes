...
sanremes meet install                 # pip deps + Chromium (transcribe only)
sanremes meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
sanremes meet auth                    # optional; skips guest-lobby wait
sanremes meet setup                   # preflight checks
```

`sanremes meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or SANREMES_MEET_REALTIME_KEY in ~/.sanremes/.env
```
...
pip install playwright websockets && python -m playwright install chromium
sanremes plugins enable google_meet
sanremes meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
sanremes meet node approve my-mac ws://<mac-ip>:18789 <token>
sanremes meet node ping my-mac                   # confirm reachable
```

Run `sanremes meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A SanRemes agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `sanremes meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `SANREMES_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `sanremes_meet_src`). |
...
Local:
```
$SANREMES_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$SANREMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `sanremes meet node approve`.
