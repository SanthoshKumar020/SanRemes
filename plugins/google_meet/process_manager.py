...
session-scoped state file under ``$SANREMES_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $SANREMES_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_sanremes_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this sanremes install, leave it first —
    ...
    env = os.environ.copy()
    env["SANREMES_MEET_URL"] = url
    env["SANREMES_MEET_OUT_DIR"] = str(out)
    env["SANREMES_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["SANREMES_MEET_HEADED"] = "1"
    if auth_state:
        env["SANREMES_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["SANREMES_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if SANREMES_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["SANREMES_MEET_MODE"] = mode
    if realtime_model:
        env["SANREMES_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["SANREMES_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["SANREMES_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    ...
    # meet_bot checks SANREMES_MEET_REALTIME_KEY before OPENAI_API_KEY.
    if not realtime_api_key:
        try:
            from agent.secret_scope import get_secret

            realtime_api_key = (
                get_secret("SANREMES_MEET_REALTIME_KEY")
                or get_secret("OPENAI_API_KEY")
            )
        except ImportError:  # pragma: no cover — secret_scope is in-repo
            pass
    if realtime_api_key:
        env["SANREMES_MEET_REALTIME_KEY"] = realtime_api_key
