...
def _xai_user_agent() -> str:
    try:
        ...
        return sanremes_xai_user_agent()
    except Exception:
        return "sanremes-agent/video_gen"
...
def _raise_if_blocked_local_input(ref: str) -> None:
    """Refuse to read a local media path that SanRemes' read deny-list blocks.
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``sanremes_cli/tools_config.py``) so the picker doesn't blindly
...
def _auth_required_response(prompt: str) -> Dict[str, Any]:
    return error_response(
        error=(
            "No xAI credentials found. Sign in via `sanremes auth add xai-oauth` "
    ...
    *,
    ...
    try:
        ...
        storage_options = build_xai_storage_options(
            "video_gen",
            filename_prefix="sanremes-xai-video",
            extension="mp4",
        )
