    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `sanremes tools` → Video Generation."
            ) from exc
        raise


def _check_fal_video_available() -> bool:
    """True if the FAL video backend selected via `sanremes tools` (or, on a
        ...
        if not _check_fal_video_available():
            ...
            return error_response(
                error=(
                    ...
                    "(run `sanremes tools` → Video Generation → FAL to configure) "
                    "or sign in to Nous (`sanremes setup`) for managed gateway access."
                ),
                error_type="auth_required",
                provider="fal",
                prompt=prompt,
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `sanremes tools` → Video Generation."
                    ),
