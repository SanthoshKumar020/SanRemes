"""Meta Model API (Muse Spark) provider plugin for SanRemes Agent.
...
Bundled from https://github.com/albertodepaola/sanremes-meta-provider. SanRemes'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to sanremes' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map SanRemes' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in sanremes_cli/providers.py and the retention hint in
