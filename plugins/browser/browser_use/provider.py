...
gateway (which SanRemes uses to bill Browser Use sessions to a Nous
...
Auth env vars (one of)::
    ...
    # OR a managed Nous gateway entry (configured via 'sanremes setup')
"""
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when SanRemes only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"sanremes_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        # Hidden from the sanremes tools picker: the "Browser Use" row now
