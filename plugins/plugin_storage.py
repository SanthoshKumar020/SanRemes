...
``<sanremes home>/plugins/<name>/``. That tree is the plugin *install* dir —
``sanremes plugins remove`` deletes it and ``sanremes plugins update`` git-pulls
...
``<sanremes home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in SanRemes.
...
# Mirrors the plugin-name shape `sanremes plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<sanremes home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`sanremes_constants.get_sanremes_home` on every call. Don't cache the
    result across profile switches.
    """
    from sanremes_constants import get_sanremes_home

    root = get_sanremes_home() / "plugin-data" / _validate_name(name)
