...
``sanremes_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__SANREMES_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``sanremes_cli/web_server.py``.

This means ``sanremes dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from sanremes_cli import kanban_db
from sanremes_cli import kanban_diagnostics as kd
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``sanremes_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from sanremes_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``sanremes_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from sanremes_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from sanremes_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``SANREMES_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from sanremes_cli.kanban import _check_dispatcher_presence
                from sanremes_constants import get_sanremes_home

                # Scope the probe to the request's active home. The dashboard
                # backend can run under a different SANREMES_HOME than the
...
from sanremes_cli.kanban_db import (  # noqa: E402
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``sanremes kanban
...
# spawn failures, stuck-blocked). See sanremes_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``sanremes kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``sanremes kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``sanremes kanban specify <task_id>``.
    ...
    # SANREMES_KANBAN_BOARD env var — this endpoint runs in FastAPI's
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from sanremes_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``sanremes kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.sanremes/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current SanRemes profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.sanremes/profiles/*`` on disk and every
...
def model_options():
    ...
    Thin wrapper around ``sanremes_cli.inventory.build_models_payload`` — the
    same substrate the dashboard Models page and the TUI picker use, so the
    dropdown can never offer a model/provider pair the rest of SanRemes
...
def list_kanban_projects():
    """List first-class SanRemes projects for board scoping.
...
async def export_board_endpoint(slug: str, body: ExportBoardBody):
    ...
    from sanremes_cli import kanban_transfer
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from sanremes_constants import get_sanremes_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_sanremes_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``sanremes profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from sanremes_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``sanremes kanban decompose <task_id>``.
    ...
    # SANREMES_KANBAN_BOARD env var would let concurrent requests for
    # different boards race and cross-write (issue #38323).
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from sanremes_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.sanremes/config.yaml.
