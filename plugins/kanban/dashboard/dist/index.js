/**
 * SanRemes Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.sanremes/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SANREMES_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SANREMES_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "sanremes-kanban-dot-triage",
    todo: "sanremes-kanban-dot-todo",
    ready: "sanremes-kanban-dot-ready",
    running: "sanremes-kanban-dot-running",
    blocked: "sanremes-kanban-dot-blocked",
    review: "sanremes-kanban-dot-review",
    done: "sanremes-kanban-dot-done",
    archived: "sanremes-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-sanremes-task";
  ...
  const DOCS_URL = "https://sanremes-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://sanremes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "sanremes.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="sanremes-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])sanremes-kanban-md-code\1/i.test(attrs)) {
      return ' class="sanremes-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "sanremes-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "sanremes-kanban-md",
  ...
  // standard `drop` event and our `sanremes-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("sanremes-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("sanremes-kanban-column--drop");
          if (target) target.classList.add("sanremes-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("sanremes-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("sanremes-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("sanremes-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SANREMES_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "sanremes-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "sanremes-kanban-attention",
        "sanremes-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "sanremes-kanban-attention-bar" },
        h("span", { className: "sanremes-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "sanremes-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "sanremes-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "sanremes-kanban-attention-row",
                  "sanremes-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "sanremes-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "sanremes-kanban-attention-row-id" }, task.id),
                h("span", { className: "sanremes-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "sanremes-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "sanremes-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "sanremes-kanban-diag-action-btn",
      action.suggested ? "sanremes-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " sanremes-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".sanremes-kanban-drawer-comment-row input, .sanremes-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "sanremes-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("sanremes-kanban-diag", sevClass) },
      h("div", { className: "sanremes-kanban-diag-header" },
        h("span", { className: "sanremes-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "sanremes-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "sanremes-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "sanremes-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "sanremes-kanban-diag-data-row" },
                  h("span", { className: "sanremes-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "sanremes-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "sanremes-kanban-diag-data-row" },
                h("span", { className: "sanremes-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "sanremes-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "sanremes-kanban-diag-reassign-row" },
            h("span", { className: "sanremes-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "sanremes-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "sanremes-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "sanremes-kanban-diag-msg",
              msg.ok ? "sanremes-kanban-diag-msg--ok" : "sanremes-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "sanremes-kanban-section" },
      h("div", { className: "sanremes-kanban-section-head-row" },
        h("span", { className: "sanremes-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "sanremes-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "sanremes-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "sanremes-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "sanremes-kanban-docs-link",
      title: "Open SanRemes Kanban docs in a new tab",
      "aria-label": "SanRemes Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `sanremes kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "sanremes-kanban-msg-ok" : "sanremes-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "sanremes-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "sanremes-kanban-boardswitcher" },
      h("div", { className: "sanremes-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "sanremes-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "sanremes-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "sanremes-kanban-dialog-title" },
        ...
        h("div", { className: "sanremes-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned SanRemes profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "sanremes-kanban-bulk" },
      h("span", { className: "sanremes-kanban-bulk-count" },
      ...
      h("div", { className: "sanremes-kanban-bulk-priority",
      ...
      h("div", { className: "sanremes-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different SanRemes profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "sanremes-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("sanremes-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("sanremes-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "sanremes-kanban-trash",
        dragOver ? "sanremes-kanban-trash--drop" : "",
        props.draggingTaskId ? "sanremes-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "sanremes-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "sanremes-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".sanremes-kanban-card")) return true;
      if (target.closest && target.closest(".sanremes-kanban-column-add")) return true;
      if (target.closest && target.closest(".sanremes-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("sanremes-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("sanremes-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".sanremes-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "sanremes-kanban-columns",
        isScrollable ? "sanremes-kanban-columns--scrollable" : "",
        isPanning ? "sanremes-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("sanremes-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("sanremes-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "sanremes-kanban-column",
        dragOver ? "sanremes-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "sanremes-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "sanremes-kanban-col-check",
        ...
        h("span", { className: cn("sanremes-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "sanremes-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "sanremes-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "sanremes-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "sanremes-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "sanremes-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "sanremes-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "sanremes-kanban-lane" },
                  h("div", { className: "sanremes-kanban-lane-head" },
                    h("span", { className: "sanremes-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "sanremes-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "sanremes-kanban-card--stale-red";
    if (age >= tier.amber) return "sanremes-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".sanremes-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "sanremes-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "sanremes-kanban-card",
        props.selected ? "sanremes-kanban-card--selected" : "",
        props.failed ? "sanremes-kanban-card--failed" : "",
        props.draggingSource ? "sanremes-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "sanremes-kanban-card-content" },
          h("div", { className: "sanremes-kanban-card-row" },
            h("label", {
              className: "sanremes-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "sanremes-kanban-card-check",
            ...
            h("span", { className: "sanremes-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or sanremes kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "sanremes-kanban-warning-badge",
                    "sanremes-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "sanremes-kanban-needs-assignee",
          ...
          h("div", { className: "sanremes-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "sanremes-kanban-card-row sanremes-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "sanremes-kanban-assignee",
                            title: `Assigned to SanRemes profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "sanremes-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "sanremes-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "sanremes-kanban-ago",
  ...
  // centered modal reusing the sanremes-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "sanremes-kanban-dialog sanremes-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "SanRemes profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "SanRemes profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "sanremes-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "sanremes-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "sanremes-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "sanremes-kanban-drawer-close",
        ...
        data ? h("div", { className: "sanremes-kanban-drawer-comment-foot" },
          h("div", {
            className: "sanremes-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "sanremes-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "sanremes-kanban-section" },
      h("div", { className: "sanremes-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "sanremes-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "sanremes-kanban-drawer-body" },
      h("div", { className: "sanremes-kanban-drawer-title" },
        h("span", { className: cn("sanremes-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "sanremes-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "sanremes-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "sanremes-kanban-section" },
            h("div", { className: "sanremes-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "sanremes-kanban-section" },
            h("div", { className: "sanremes-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "sanremes-kanban-done-no-result sanremes-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "sanremes-kanban-section" },
            h("div", { className: "sanremes-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "sanremes-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "sanremes-kanban-section" },
        h("div", { className: "sanremes-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "sanremes-kanban-comment" },
            h("div", { className: "sanremes-kanban-comment-head" },
              h("span", { className: "sanremes-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "sanremes-kanban-diag-action-btn",
      ...
      h("div", { className: "sanremes-kanban-section" },
        h("div", { className: "sanremes-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "sanremes-kanban-comment" },
                h("div", { className: "sanremes-kanban-comment-head" },
                  h("span", { className: "sanremes-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "sanremes-kanban-comment-ago" },
      ...
      h("div", { className: "sanremes-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "sanremes-kanban-event",
              isDiag ? "sanremes-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "sanremes-kanban-event-header" },
                  h("span", { className: "sanremes-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "sanremes-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "sanremes-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "sanremes-kanban-event-header-plain" },
                  h("span", { className: "sanremes-kanban-event-kind" }, e.kind),
                  h("span", { className: "sanremes-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "sanremes-kanban-event-phantom-row" },
                  h("span", { className: "sanremes-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "sanremes-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "sanremes-kanban-event-payload" },
    ...
    return h("div", { className: "sanremes-kanban-section" },
      h("div", { className: "sanremes-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "sanremes-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `sanremes-kanban-run--${r.outcome || r.status || "ended"}`
          : "sanremes-kanban-run--active";
        return h("div", { key: r.id, className: cn("sanremes-kanban-run", outcomeClass) },
          h("div", { className: "sanremes-kanban-run-head" },
            h("span", { className: "sanremes-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "sanremes-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "sanremes-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "sanremes-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "sanremes-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "sanremes-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "sanremes-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "sanremes-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "sanremes-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "sanremes-kanban-pre sanremes-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "sanremes-kanban-meta-row" },
      h("span", { className: "sanremes-kanban-meta-label" }, props.label),
      h("span", { className: "sanremes-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "sanremes-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sanremes-kanban-meta-row" },
        h("span", { className: "sanremes-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "sanremes-kanban-meta-value sanremes-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sanremes-kanban-meta-row" },
        h("span", { className: "sanremes-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "sanremes-kanban-meta-value sanremes-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sanremes-kanban-meta-row" },
        h("span", { className: "sanremes-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "sanremes-kanban-meta-value sanremes-kanban-editable",
    ...
    return h("div", { className: "sanremes-kanban-meta-row" },
      h("span", { className: "sanremes-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "sanremes-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "sanremes-kanban-section" },
      h("div", { className: "sanremes-kanban-section-head-row" },
        h("span", { className: "sanremes-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "sanremes-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "sanremes-kanban-section" },
      h("div", { className: "sanremes-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "sanremes-kanban-deps-row" },
        h("span", { className: "sanremes-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "sanremes-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "sanremes-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "sanremes-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "sanremes-kanban-dep-chip-x",
      ...
      h("div", { className: "sanremes-kanban-deps-row" },
        h("span", { className: "sanremes-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "sanremes-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "sanremes-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "sanremes-kanban-msg-ok"
          : "sanremes-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "sanremes-kanban-section" },
      ...
      h("div", { className: "sanremes-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "sanremes-kanban-home-sub sanremes-kanban-home-sub--on"
              : "sanremes-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SANREMES_PLUGINS__ && typeof window.__SANREMES_PLUGINS__.register === "function") {
    window.__SANREMES_PLUGINS__.register("kanban", KanbanPage);
  }
})();
