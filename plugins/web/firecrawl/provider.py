...
def check_firecrawl_api_key() -> bool:
    """Return True when the Firecrawl backend selected via `sanremes tools`
    ...
    existing tests and the ``sanremes tools`` setup flow.
    """
...
def _raise_web_backend_configuration_error() -> "NoReturn":
    ...
    if _wt.managed_nous_tools_enabled():
        message += (
            ...
            "run `sanremes tools` and select Nous Subscription as the web provider."
        )
    else:
