...
two endpoints so a fresh SanRemes install with **zero web credentials** still
...
def provider_tier(name: str) -> str:
    ...
    ``sanremes tools`` picker's Free/Paid rows). ``free`` forces the keyless
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "backend via `sanremes tools` for reliable service."
            ),
        }
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "via `sanremes tools` for reliable service."
            ),
        }
...
_KEENABLE_TITLE = "sanremes-agent"
