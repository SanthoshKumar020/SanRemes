...
``{title, url, description, position}`` rows every other SanRemes web
provider produces.
...
prefers SanRemes-managed xAI Grok OAuth (via ``sanremes auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.sanremes/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``sanremes tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `sanremes auth` to sign in with "
