...
``sanremes tools`` can prompt the user to install it.
...
GIL — so the whole SanRemes process freezes through Ctrl+C/SIGTERM. Each search
...
def _run_ddgs_search_bounded(query: str, safe_limit: int) -> list[dict[str, Any]]:
    ...
    *its* GIL — it only polls a communicator thread and, on timeout/interrupt,
    ...
    """
    # Imported lazily so plugin import stays light for ``sanremes tools`` probes.
    ...
    if _test_hook:
        env["SANREMES_DDGS_ALLOW_TEST_HOOKS"] = "1"
...
class DDGSWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        ``primp`` call cannot freeze the SanRemes process (#36776, #68096).
        """
        try:
