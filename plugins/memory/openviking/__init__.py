...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — SanRemes peer ID in OpenViking (default: sanremes)
...
_DEFAULT_AGENT = "sanremes"
_OPENVIKING_USER_AGENT = f"openviking-memory-sanremes/{_SANREMES_VERSION}"
_AGENT_PROMPT_LABEL = "SanRemes peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "SANREMES_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip SanRemes slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach SanRemes to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; SanRemes refused the connection."
        ) from exc
...
def _load_sanremes_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("SANREMES_HOME", "")).expanduser() if os.environ.get("SANREMES_HOME") else Path.home() / ".sanremes"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. SanRemes did not start "
    ...
    try:
        ...
        # The SanRemes Desktop backend can include the SanRemes venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the SanRemes venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the SanRemes venv,
        # aborting `sanremes update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; SanRemes will retry on a later access or when "
        "the config changes."
    )
...
def _save_sanremes_only_config(
    *,
    config: dict,
    provider_config: dict,
    env_path: Path,
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new SanRemes session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in SanRemes only", "write values only to SanRemes .env"),
    ...
    _save_sanremes_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to SanRemes .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See sanremes-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_sanremes_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "SanRemes peer ID in OpenViking, sent as the actor peer and "
    ...
    def save_config(self, values: Dict[str, Any], sanremes_home: str) -> None:
    ...
    def post_setup(self, sanremes_home: str, config: dict) -> None:
        ...
        from sanremes_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        sanremes_home_path = Path(sanremes_home)
        env_path = sanremes_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_sanremes_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        sanremes_home = str(kwargs.get("sanremes_home") or "").strip()
        if not sanremes_home:
            try:
                from sanremes_constants import get_sanremes_home
                sanremes_home = str(get_sanremes_home())
            except Exception:
                sanremes_home = str(Path.home() / ".sanremes")
        self._sanremes_home = sanremes_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "SanRemes will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.sanremes/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts sanremes (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; SanRemes will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._sanremes_home:
            return None
        return Path(self._sanremes_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._sanremes_home:
            return None
        return Path(self._sanremes_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_sanremes_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of SanRemes' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert SanRemes canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See sanremes-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/sanremes/.overview.md).
