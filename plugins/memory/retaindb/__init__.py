...
Config (env vars or sanremes config.yaml under retaindb:):
...
class _Client:
    ...
    def upload_file(self, data: bytes, filename: str, remote_path: str, mime_type: str, scope: str, project_id: str | None) -> dict:
        ...
        token = self.api_key.replace("Bearer ", "").strip()
        headers = {"Authorization": f"Bearer {token}", "x-sdk-runtime": "sanremes-plugin"}
    ...
    def read_file_content(self, file_id: str) -> bytes:
        ...
        resp = requests.get(url, headers={"Authorization": f"Bearer {token}", "x-sdk-runtime": "sanremes-plugin"}, timeout=30, allow_redirects=True)
        resp.raise_for_status()
        return resp.content
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > config.yaml project > sanremes-<profile> > "default"
        ...
        if explicit:
            ...
            sanremes_home = str(kwargs.get("sanremes_home", ""))
            profile_name = os.path.basename(sanremes_home) if sanremes_home else ""
            project = f"sanremes-{profile_name}" if (profile_name and profile_name not in {"", ".sanremes"}) else "default"

        self._client = _Client(api_key, base_url, project)
        self._session_id = session_id
        self._user_id = kwargs.get("user_id", "default") or "default"
        self._agent_id = kwargs.get("agent_id", "sanremes") or "sanremes"

        from sanremes_constants import get_sanremes_home
        sanremes_home_path = get_sanremes_home()
        db_path = sanremes_home_path / "retaindb_queue.db"
        self._queue = _WriteQueue(self._client, db_path)

        # Seed agent identity from SOUL.md in background
        soul_path = sanremes_home_path / "SOUL.md"
        if soul_path.exists():
