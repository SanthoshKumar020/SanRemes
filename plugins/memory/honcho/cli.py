...
Handles: sanremes honcho setup | status | sessions | map | peer
"""
...
from sanremes_cli.config import cfg_get
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "sanremes" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all SanRemes profiles and creates host blocks for any that don't
    have one yet. Inherits settings from the default host block.
    """
    try:
        from sanremes_cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'sanremes honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'sanremes honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `sanremes update` -- no output, no exceptions.
    """
    try:
        from sanremes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current SanRemes profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $SANREMES_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_sanremes_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        sanremes_host.get("pinUserPeer"),
        sanremes_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in sanremes_host:
        aliases_src = sanremes_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in sanremes_host:
        prefix_src = sanremes_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(sanremes_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom sanremes_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    sanremes_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        sanremes_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(sanremes_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = sanremes_host.get("userPeerAliases")
    prefix = sanremes_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(sanremes_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(sanremes_host)
    sanremes_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        sanremes_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        sanremes_host, current_prefix, prefix_from_root,
...
def _prompt(label: str, default: str | None = None, secret: bool = False) -> str:
    ...
    if secret:
        if sys.stdin.isatty():
            from sanremes_cli.secret_prompt import masked_secret_prompt
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/sanremes (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives SanRemes persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = sanremes_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            sanremes_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(sanremes_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'sanremes honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'sanremes honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'sanremes honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(sanremes_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'sanremes honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = sanremes_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        sanremes_host["peerName"] = new_peer

    current_ai = sanremes_host.get("aiPeer") or cfg.get("aiPeer", "sanremes")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        sanremes_host["aiPeer"] = new_ai

    current_workspace = sanremes_host.get("workspace") or cfg.get("workspace", "sanremes")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        sanremes_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the SanRemes GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, sanremes_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the SanRemes gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = sanremes_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(sanremes_host)
            sanremes_host["pinUserPeer"] = False
            if prior_aliases:
                sanremes_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                sanremes_host, current_prefix, prefix_from_root,
            ...
            if merged:
                sanremes_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                sanremes_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                sanremes_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = sanremes_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        sanremes_host["observationMode"] = new_obs
    else:
        sanremes_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(sanremes_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        sanremes_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        sanremes_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = sanremes_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        sanremes_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = sanremes_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        sanremes_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                sanremes_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(sanremes_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            sanremes_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        sanremes_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        sanremes_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        sanremes_host["dialecticReasoningLevel"] = new_reasoning
    else:
        sanremes_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = sanremes_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        sanremes_host["sessionStrategy"] = new_strat

    sanremes_host["enabled"] = True
    sanremes_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        sanremes_config = load_config()
        sanremes_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(sanremes_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: sanremes config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    sanremes honcho status     -- show full config")
    print("    sanremes honcho mode       -- change recall/observation mode")
    print("    sanremes honcho tokens     -- tune context and dialectic budgets")
    print("    sanremes honcho peer       -- update peer names")
    print("    sanremes honcho map <name> -- map this directory to a session name\n")
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if sanremes_cli internals move."""
    try:
        from sanremes_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(sanremes_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    sanremes_host["apiKey"] = cred.access_token
    sanremes_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        sanremes_host["peerName"] = cred.consent_peer_name


def _active_profile_name() -> str:
    """Return the active SanRemes profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("sanremes.work") stay readable per the README's back-compat
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: sanremes honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'sanremes honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'sanremes honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: sanremes honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        sanremes = hosts.get(_host_key(), {})
        user = sanremes.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = sanremes.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = sanremes.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = sanremes.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    SanRemes' identity in Honcho. Seed with 'sanremes honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: sanremes honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: sanremes honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = sanremes.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = sanremes.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = sanremes.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. SanRemes asks Honcho's AI peer a question")
        ...
        print("\n  Set with: sanremes honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'sanremes honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'sanremes honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    sanremes honcho identity --show        — show both peer representations")
        print("    sanremes honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → SanRemes + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → SanRemes\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives SanRemes persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  sanremes honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'sanremes honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'sanremes honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  sanremes honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'sanremes'")
        print("  with Honcho configured and no prior session history.")
        print("  (SanRemes calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    sanremes honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'sanremes honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In SanRemes, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'sanremes honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    sanremes honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  sanremes honcho identity <path/to/SOUL.md>")
    ...
    print("    SanRemes:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    SanRemes:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    SanRemes:   Honcho observes every message and updates representations")
    ...
    print("    SanRemes:   per-session by default — each run gets its own session")
    print("              Map a custom name:  sanremes honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. sanremes honcho setup              — configure API key (required)")
        print("  2. sanremes honcho migrate            — re-run this walkthrough")
    else:
        print("  1. sanremes honcho status             — verify Honcho connection")
        print("  2. sanremes                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. sanremes honcho identity --show    — verify AI peer representation")
        print("  4. sanremes honcho tokens             — tune context and dialectic budgets")
        print("  5. sanremes honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'sanremes memory setup'...\n")
        from sanremes_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``sanremes honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``sanremes honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to sanremes memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to SanRemes Honcho",
    )
