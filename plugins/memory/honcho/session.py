...
def _auth_error_message(exc: BaseException) -> str:
    return (
        ...
        "Re-authenticate with 'sanremes honcho setup'."
    )
...
class HonchoSessionManager:
    ...
    Runs alongside sanremes' existing SQLite state and file-based memory,
    ...
    def get_or_create(self, key: str) -> HonchoSession:
        ...
        assistant_peer_id = self._sanitize_id(
            self._config.ai_peer if self._config else "sanremes-assistant"
        )
    ...
    def dialectic_query(
        ...
        try:
            ...
            # Only automatic injection uses the SanRemes-side character cap.
            if (
                apply_injection_cap
                and result
    ...
    def get_prefetch_context(self, session_key: str, user_message: str | None = None) -> dict[str, str]:
        ...
        # Also fetch AI peer's own representation so SanRemes knows itself.
        try:
    ...
    def migrate_memory_files(self, session_key: str, memory_dir: str) -> bool:
        ...
        Args:
            ...
            memory_dir: Path to the memories directory (~/.sanremes/memories/).
