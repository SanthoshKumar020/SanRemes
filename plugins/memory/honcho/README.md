...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/sanremes>
...
```bash
sanremes memory setup honcho   # configure Honcho directly (works on a fresh install)
sanremes memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
sanremes config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.sanremes/.env
```

> `sanremes honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `sanremes memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `sanremes model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SANREMES_HOME/honcho.json` | Profile-local (isolated SanRemes instances) |
| 2 | `~/.sanremes/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active SanRemes profile: `sanremes` (default) or `sanremes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `sanremes honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | SanRemes session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`sanremes-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`sanremes`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-sanremes-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening sanremes in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — SanRemes session ID (timestamp + hex). Every `sanremes` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple SanRemes profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`sanremes`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active SanRemes profile: `sanremes` (default) or `sanremes_<profile>` (e.g. `sanremes -p coder` -> host key `sanremes_coder`). Older `sanremes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `SANREMES_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `sanremes-agent`) |
...
|---------|-------------|
| `sanremes memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `sanremes honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `sanremes memory setup`) |
| `sanremes honcho status` | Show resolved config for active profile |
| `sanremes honcho enable` / `disable` | Toggle Honcho for active profile |
| `sanremes honcho mode <mode>` | Change recall or observation mode |
| `sanremes honcho peer --user <name>` | Update user peer name |
| `sanremes honcho peer --ai <name>` | Update AI peer name |
| `sanremes honcho tokens --context <N>` | Set context token budget |
| `sanremes honcho tokens --dialectic <N>` | Set dialectic max chars |
| `sanremes honcho map <name>` | Map current directory to a session name |
| `sanremes honcho sync` | Create host blocks for all SanRemes profiles |
