...
Resolution order for config file:
  1. $SANREMES_HOME/honcho.json  (instance-local, enables isolated SanRemes instances)
...
from sanremes_cli.profiles import _get_default_sanremes_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a SanRemes profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active SanRemes profile.

    Resolution order:
      1. SANREMES_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``sanremes_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"sanremes"`` (default profile)
    """
    explicit = os.environ.get("SANREMES_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named SanRemes
    # profile share that same host.  Keep named profiles isolated; only the
    # default SanRemes profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $SANREMES_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.sanremes/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_sanremes_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_sanremes_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.sanremes
    ...
    # resolve_config_path()/get_sanremes_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        Resolution: $SANREMES_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active SanRemes profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.sanremes block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the SanRemes-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — SanRemes session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. SanRemes session title (from /title command; non-per-session)
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_sanremes_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_sanremes_home) silently
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'sanremes honcho setup' account switches produce a NEW cache identity
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.sanremes_home) if config.sanremes_home is not None else "",
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/sanremes_home) identity reappears with a
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'sanremes honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `sanremes honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                sanremes_cfg = load_config()
                honcho_cfg = sanremes_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
