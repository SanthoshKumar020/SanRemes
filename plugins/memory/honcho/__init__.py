...
Config: Uses the existing Honcho config chain:
  1. $SANREMES_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, sanremes_home):
        """Write config to $SANREMES_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(sanremes_home) / "honcho.json"
        existing = {}
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps SanRemes CLI/gateway startup responsive when Honcho is down,
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "sanremes-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        # Skip under per-session strategy: every SanRemes run creates a fresh
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_sanremes_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "sanremes-default",
    ...
    def _pop_auth_notice(self) -> str:
        ...
        return (
            ...
            "'sanremes honcho setup' to re-authenticate will restore it."
        )
