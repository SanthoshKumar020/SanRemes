...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with sanremes-agent)
2. User-installed providers: ``$SANREMES_HOME/plugins/<name>/``
3. Project-local providers: ``./.sanremes/plugins/<name>/``, opt-in via
   ``SANREMES_ENABLE_PROJECT_PLUGINS``
4. Pip-installed providers: ``sanremes_agent.memory_providers`` entry points
...
ENTRY_POINTS_GROUP = "sanremes_agent.memory_providers"
...
_USER_NAMESPACE = "_sanremes_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_sanremes_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_sanremes_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$SANREMES_HOME/plugins/`` or None if unavailable."""
    try:
        from sanremes_constants import get_sanremes_home
        d = get_sanremes_home() / "plugins"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """Return ``./.sanremes/plugins/`` or None if unavailable or not opted in.

    Gated on ``SANREMES_ENABLE_PROJECT_PLUGINS`` exactly as the general
    ...
    """
    try:
        from sanremes_cli.plugins import _env_enabled

        if not _env_enabled("SANREMES_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".sanremes" / "plugins"
        return d if d.is_dir() else None
    except Exception:
        return None
...
def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    ...
    # 2. User-installed providers ($SANREMES_HOME/plugins/<name>/)
    # 3. Project-local providers (./.sanremes/plugins/<name>/), opt-in
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    ``sanremes <provider>`` subcommands — working, but a second-class citizen next
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from sanremes_cli.plugins import resolve_module_origin
...
def load_memory_provider(
    ...
    *,
    ...
    (``$SANREMES_HOME/plugins/<name>/``), and pip entry-point providers.
...
class _ProviderCollector:
    ...
    def register_skill(self, *args, **kwargs):
        ...
        provider — ``sanremes memory status``, the setup picker — leaves no
    ...
    def _plugin_context(self):
        ...
        plugin manager, which discovery touches on every sanremes startup.
        """
        if self._context is None:
            from sanremes_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _sanremes_user_memory.<name>.cli, usually
