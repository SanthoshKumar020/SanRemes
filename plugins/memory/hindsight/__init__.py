...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: sanremes)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: sanremes)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $SANREMES_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "sanremes").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so SanRemes can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``sanremes memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'sanremes memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $SANREMES_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_sanremes_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "sanremes": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "sanremes"),
...
def _event_timestamp() -> str:
    """Return the configured SanRemes event time with an explicit UTC offset."""
    event_time = _sanremes_now()
    # sanremes_time.now() guarantees an aware datetime. Keep this fallback so a
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this SanRemes config."""
    profile = config.get("profile", "sanremes")
    return str(profile or "sanremes")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the SanRemes .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active SanRemes profile name (from agent_identity)
      {workspace} — SanRemes workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``sanremes-{user}`` with no user becomes ``sanremes``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between SanRemes Agent and the User"
    ...
    def save_config(self, values, sanremes_home):
        """Write config to $SANREMES_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(sanremes_home) / "hindsight"
    ...
    def post_setup(self, sanremes_home: str, config: dict) -> None:
        ...
        from sanremes_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(sanremes_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "sanremes")
        ...
        self.save_config(provider_config, sanremes_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(sanremes_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(sanremes_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a SanRemes starter template (best-effort)."""
        from sanremes_cli.memory_setup import _CANCELLED, _curses_select
        ...
        bank_id = provider_config.get("bank_id", "sanremes")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "sanremes"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: sanremes-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between SanRemes Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "sanremes"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "sanremes"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/sanremes (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "sanremes", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "sanremes")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between SanRemes Agent and the User")
        ...
        # SanRemes emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run SanRemes as a non-root user, or switch "
                    "to cloud / local_external mode via 'sanremes memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # SanRemes get sluggish. (issue #13125)
                try:
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # turn's queued recall. See NousResearch/sanremes-agent#5820.
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See sanremes-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When SanRemes created/used that client on the shared async loop,
