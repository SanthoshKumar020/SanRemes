...
the ``sanremes`` integration, and applies a chosen manifest to the user's bank
...
from sanremes_cli.urllib_security import open_credentialed_url
...
def fetch_sanremes_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``sanremes`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "sanremes" in (e.get("integrations") or [])]
...
def run_template_step(
    *,
    ...
    """
    try:
        entries = fetch_sanremes_templates()
