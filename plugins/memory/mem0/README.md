...
```bash
sanremes memory setup    # select "mem0"
```
...
Behavioral settings live in `$SANREMES_HOME/mem0.json` (set them via `sanremes memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.sanremes/.env`.
...
| `user_id` | `sanremes-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh SanRemes session and call `mem0_search` — it connects to your server.
...
```bash
sanremes memory setup mem0 --mode oss \
...
```bash
sanremes memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$SANREMES_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.sanremes/mem0_qdrant"}}
  }
}
```
...
```bash
sanremes memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$SANREMES_HOME/mem0.json`).
