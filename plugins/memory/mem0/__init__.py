...
Configuration
-------------
Secret (lives in $SANREMES_HOME/.env or the environment):
...
Behavioral settings (live in $SANREMES_HOME/mem0.json, set via `sanremes memory
setup`):
  ...
  agent_id           — Agent identifier (default: sanremes)
...
_DEFAULT_USER_ID = "sanremes-user"
...
def _load_config() -> dict:
    """Load config from env vars, with $SANREMES_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "sanremes"),
        "oss": {},
    }
    ...
    config_path = get_sanremes_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, sanremes_home):
        """Write config to $SANREMES_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, sanremes_home: str, config: dict) -> None:
        ...
        post_setup(sanremes_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $SANREMES_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "sanremes")
