...
def _curses_select(title: str, items: list[tuple[str, str]], default: int = 0) -> int:
    ...
    from sanremes_cli.curses_ui import curses_radiolist
    display_items = [
...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # sanremes_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(sanremes_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(sanremes_home) / "mem0.json"
    existing = {}
...
def _setup_platform(sanremes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "sanremes-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "sanremes"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.sanremes/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.sanremes/.env (or unset it) or SanRemes will keep "
    ...
    provider.save_config(provider_config, sanremes_home)

    if env_writes:
        _write_env(Path(sanremes_home) / ".env", env_writes)
...
def _setup_selfhosted(sanremes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "sanremes-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "sanremes")
...
def _setup_oss(sanremes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(sanremes_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "sanremes-user")
    ...
    if env_writes:
        _write_env(Path(sanremes_home) / ".env", env_writes)
    _save_mem0_json(sanremes_home, {"mode": "oss", "user_id": user_id, "agent_id": "sanremes", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, sanremes_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # sanremes_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "sanremes-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "sanremes"
...
def _setup_oss_interactive(sanremes_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], sanremes_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], sanremes_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'sanremes-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "sanremes-user")

    agent_id = input("  Agent ID [sanremes]: ").strip()
    agent_id = agent_id or "sanremes"
    ...
    if env_writes:
        _write_env(Path(sanremes_home) / ".env", env_writes)
    _save_mem0_json(sanremes_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(sanremes_home: str, config: dict) -> None:
    """Entry point called by sanremes memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(sanremes_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(sanremes_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(sanremes_home, config, flags)
        return
