"""sanremes-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $SANREMES_HOME/config.yaml (profile-scoped):
  plugins:
    sanremes-memory-store:
      db_path: $SANREMES_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "sanremes-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, sanremes_home):
        """Write config to config.yaml under plugins.sanremes-memory-store."""
        from pathlib import Path
        config_path = Path(sanremes_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["sanremes-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_sanremes_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _sanremes_home = str(get_sanremes_home())
        _default_db = _sanremes_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $SANREMES_HOME in user-supplied paths so config values like
        # "$SANREMES_HOME/memory_store.db" or "~/.sanremes/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$SANREMES_HOME", _sanremes_home)
            db_path = db_path.replace("${SANREMES_HOME}", _sanremes_home)
