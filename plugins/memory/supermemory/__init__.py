...
_DEFAULT_CONTAINER_TAG = "sanremes"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=sanremes"
...
def _load_supermemory_config(sanremes_home: str) -> dict:
    config = _default_config()
    config_path = Path(sanremes_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, sanremes_home: str) -> None:
    config_path = Path(sanremes_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "sanremes"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes SanRemes writes into the "SanRemes" Space in the Supermemory
        ...
        merged = {"sm_source": "sanremes", **(metadata or {})}
...
def _resolve_container_tag_for_setup(sanremes_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(sanremes_home)
...
def _probe_supermemory_connection(api_key: str, sanremes_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(sanremes_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(sanremes_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `sanremes memory setup`.
        # All other options are documented for $SANREMES_HOME/supermemory.json
    ...
    def save_config(self, values, sanremes_home):
        ...
        _save_supermemory_config(sanitized, sanremes_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, sanremes_home)
    ...
    def post_setup(self, sanremes_home: str, config: dict) -> None:
        ...
        from sanremes_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(env_writes, sanremes_home=sanremes_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from sanremes_constants import get_sanremes_home
        self._sanremes_home = kwargs.get("sanremes_home") or str(get_sanremes_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._sanremes_home)
