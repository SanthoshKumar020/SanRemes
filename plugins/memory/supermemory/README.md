...
- Hosted: API key from [app.supermemory.ai/integrations?connect=sanremes](http://app.supermemory.ai/integrations?connect=sanremes)
...
Before running `sanremes memory setup`, add the local endpoint to
...
Then run `sanremes memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$SANREMES_HOME/supermemory.json`
...
| `container_tag` | `sanremes` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `sanremes-{identity}` → `sanremes-coder`). |
...
`https://api.supermemory.ai`. SanRemes resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: sanremes`, and document writes stamp
`metadata.sm_source: sanremes`. This is a **functional routing key, not telemetry**:
it groups SanRemes-written memories into a dedicated "SanRemes" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per SanRemes profile:

```json
{
  "container_tag": "sanremes-{identity}"
}
```

For a profile named `coder`, this resolves to `sanremes-coder`. The default profile resolves to `sanremes-default`. Without `{identity}`, all profiles share the same container.
