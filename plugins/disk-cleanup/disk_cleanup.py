"""disk_cleanup — ephemeral file cleanup for SanRemes Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SANREMES_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SANREMES_HOME and /tmp/sanremes-*
Never touches: ~/.sanremes/logs/ or any system directory.
"""
...
try:
    ...
    def get_sanremes_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SANREMES_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".sanremes").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SANREMES_HOME/logs/``."""
    return get_sanremes_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SANREMES_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SANREMES_HOME or ``/tmp/sanremes-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    sanremes_home = get_sanremes_home()
    try:
        path.resolve().relative_to(sanremes_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/sanremes-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("sanremes-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "sanremes-agent", "backups", "profiles", ".worktrees",
...
# Paths under $SANREMES_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so SANREMES_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        sanremes_home = get_sanremes_home()
        for parent in ("cron", "cronjobs"):
            base = sanremes_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SANREMES_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SANREMES_HOME, but never recurse into known
    # durable state trees.  Some installs place the SanRemes checkout, venv,
    # and desktop build under SANREMES_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    sanremes_home = get_sanremes_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in sanremes_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    sanremes_home = get_sanremes_home()
    try:
        rel = path.resolve().relative_to(sanremes_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "sanremes-agent",
        ...
        # Path isn't under SANREMES_HOME (e.g. /tmp/sanremes-*) — fall through.
        pass
