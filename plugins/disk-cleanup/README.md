...
Auto-tracks and cleans up ephemeral files created during SanRemes Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SANREMES_HOME` and `/tmp/sanremes-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SANREMES_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SANREMES_HOME` or `/tmp/sanremes-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SANREMES_HOME/disk-cleanup/` is itself excluded
- `$SANREMES_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
