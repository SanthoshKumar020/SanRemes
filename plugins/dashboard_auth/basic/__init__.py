...
mirroring the Nous provider's precedence convention:
  ...
  Environment overrides::
      ...
      SANREMES_DASHBOARD_BASIC_AUTH_USERNAME
      SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      SANREMES_DASHBOARD_BASIC_AUTH_SECRET
      SANREMES_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "SANREMES_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or SANREMES_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "SANREMES_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "SANREMES_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "SANREMES_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    # SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
    ...
    plaintext_from_env = os.environ.get(
        "SANREMES_DASHBOARD_BASIC_AUTH_PASSWORD", ""
    ).strip()
