...
A standards-compliant OpenID Connect Relying Party for the ``sanremes dashboard``
...
:class:`~sanremes_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``sanremes_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~sanremes_cli.dashboard_auth.Session`.
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.sanremes/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    SANREMES_DASHBOARD_OIDC_ISSUER
    SANREMES_DASHBOARD_OIDC_CLIENT_ID
    SANREMES_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    SANREMES_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "sanremes_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — SanRemes does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``SANREMES_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "SANREMES_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "SANREMES_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("SANREMES_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.sanremes/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "SANREMES_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(SANREMES_DASHBOARD_OIDC_ISSUER + SANREMES_DASHBOARD_OIDC_CLIENT_ID) "
