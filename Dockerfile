...
# node:26 image instead (SanRemes pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/sanremes is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/sanremes/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when sanremes
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# sanremes process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('sanremes')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main sanremes process, the dashboard,
...
# 'SanRemes WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via SANREMES_UID at runtime
RUN useradd -u 10000 -m -d /opt/data sanremes
...
WORKDIR /opt/sanremes

# ---------- Layer-cached dependency install ----------
# Copy only package manifests first so npm install + Playwright are cached
# unless the lockfiles themselves change.
#
# ui-tui/packages/sanremes-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/sanremes-ink/ ui-tui/packages/sanremes-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as sanremes-ink above).
...
# lock on the @sanremes/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by SanRemes when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the SanRemes production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/sanremes/.venv at first
...
# gives the non-root sanremes user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link sanremes-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/sanremes are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the sanremes user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/sanremes/bin && \
    cp /opt/sanremes/docker/sanremes-exec-shim.sh /opt/sanremes/bin/sanremes && \
    chmod 0755 /opt/sanremes /opt/sanremes/bin/sanremes && \
    printf 'docker\n' > /opt/sanremes/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $SANREMES_HOME. $SANREMES_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``sanremes update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the sanremes user via
# `s6-setuidgid sanremes` in its run script. If SANREMES_UID is unset, services
# run as the default sanremes user (UID 10000).
...
# lives outside both /opt/sanremes (which operators sometimes bind-mount as a
# checkout) and /opt/data (the mutable SANREMES_HOME volume).
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `sanremes dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the SANREMES_GIT_SHA build-arg to
# /opt/sanremes/.sanremes_build_sha at build time, and have
# sanremes_cli/build_info.py read it at runtime.  Both `sanremes dump` and
...
ARG SANREMES_GIT_SHA=
RUN set -eu; \
    if [ -n "${SANREMES_GIT_SHA}" ]; then \
        printf '%s\n' "${SANREMES_GIT_SHA}" > /opt/sanremes/.sanremes_build_sha; \
    fi; \
    mkdir -p /etc/sanremes; \
    SANREMES_GIT_SHA="${SANREMES_GIT_SHA}" python3 -c 'import json, os, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/sanremes/pyproject.toml").read_text(encoding="utf-8"))["project"]; marker = pathlib.Path("/etc/sanremes/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": "nousresearch/sanremes-agent", "version": project["version"], "revision": os.environ.get("SANREMES_GIT_SHA") or None}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-sanremes + dashboard.
...
# `exec sanremes`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $SANREMES_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/sanremes/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-sanremes-setup && \
    chmod +x /etc/cont-init.d/01-sanremes-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV SANREMES_WEB_DIST=/opt/sanremes/sanremes_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/sanremes/ui-tui/dist/entry.js`) and skip the
...
ENV SANREMES_TUI_DIR=/opt/sanremes/ui-tui
ENV SANREMES_HOME=/opt/data
ENV SANREMES_WRITE_SAFE_ROOT=/opt/data
ENV SANREMES_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/sanremes (root-owned, read-only) so a runtime
...
# is seeded + chowned to the sanremes user by docker/stage2-hook.sh and lives
...
ENV SANREMES_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> sanremes ...` they default to root, and any file the
# command writes under $SANREMES_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/sanremes/bin/sanremes, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid sanremes`
# when invoked as root. Non-root callers (supervised processes,
# `--user sanremes`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/sanremes/.venv/bin/sanremes). See the shim source for
# the opt-out env var (SANREMES_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/sanremes-exec-shim.sh /opt/sanremes/bin/sanremes
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/sanremes/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> sanremes ...` and any
# subprocess that doesn't activate the venv first still find sanremes.
#
# /opt/sanremes/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/sanremes/bin:/opt/sanremes/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. sanremes
# subcommand vs. no-args), drops to the sanremes user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/sanremes/docker/entrypoint-dispatch.sh" ]
CMD [ ]
