"""
Timezone-aware clock for SanRemes.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``SANREMES_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.sanremes/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — SanRemes never
...
from sanremes_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — SanRemes requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("SANREMES_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``SANREMES_TIMEZONE`` update) to force ``get_timezone()`` /
