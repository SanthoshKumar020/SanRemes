"""
SanRemes MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (SanRemes-specific extra)

Usage:
    sanremes mcp serve
    sanremes mcp serve --verbose
...
logger = logging.getLogger("sanremes.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SANREMES_HOME."""
    try:
        from sanremes_constants import get_sanremes_home
        return get_sanremes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SANREMES_HOME", Path.home() / ".sanremes")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from sanremes_constants import get_sanremes_home
        directory_file = get_sanremes_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SANREMES_HOME", Path.home() / ".sanremes")
...
class EventBridge:
    ...
    This is the SanRemes equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from sanremes_constants import get_sanremes_home
            db_file = get_sanremes_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SANREMES_HOME", Path.home() / ".sanremes")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the SanRemes MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "sanremes",
        instructions=(
            "SanRemes Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the SanRemes MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
