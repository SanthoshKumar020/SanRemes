...
_SANREMES_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (SANREMES_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `sanremes tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``sanremes tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SANREMES_KANBAN_TASK env "
    ...
    # Coding posture (base SanRemes — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in sanremes_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full SanRemes toolsets (CLI + messaging platforms)
    #
    ...
    # the `sanremes send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "sanremes-cli": {
        ...
        "tools": _SANREMES_CORE_TOOLS,
        "includes": []
    },

    "sanremes-cron": {
        # Mirrors sanremes-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `sanremes tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as sanremes-cli; gated by `sanremes tools`",
        "tools": _SANREMES_CORE_TOOLS,
        "includes": []
    },
    ...
    "sanremes-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _SANREMES_CORE_TOOLS,
        "includes": []
    },

    "sanremes-homeassistant": {
    ...
    "sanremes-email": {
        "description": "Email bot toolset - interact with SanRemes via email (IMAP/SMTP)",
        "tools": _SANREMES_CORE_TOOLS,
        "includes": []
    },
    ...
    "sanremes-sms": {
        "description": "SMS bot toolset - interact with SanRemes via SMS (Twilio)",
        "tools": _SANREMES_CORE_TOOLS,
        "includes": []
    },
    ...
    "sanremes-webhook": {
        ...
        "tools": _SANREMES_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "sanremes-gateway": {
        ...
        "tools": [],
        "includes": ["sanremes-telegram", "sanremes-discord", "sanremes-whatsapp", "sanremes-slack", "sanremes-signal", "sanremes-bluebubbles", "sanremes-homeassistant", "sanremes-email", "sanremes-sms", "sanremes-mattermost", "sanremes-matrix", "sanremes-dingtalk", "sanremes-feishu", "sanremes-wecom", "sanremes-wecom-callback", "sanremes-weixin", "sanremes-qqbot", "sanremes-webhook", "sanremes-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``sanremes-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SANREMES_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``sanremes-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (sanremes-<name>).
        # Gives them _SANREMES_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("sanremes-"):
            platform_name = name[len("sanremes-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SANREMES_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
