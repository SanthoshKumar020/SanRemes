#!/usr/bin/env python3
"""Canvas LMS API CLI for SanRemes Agent.
...
def _check_config():
    ...
    if missing:
        sanremes_env = os.path.join(
            os.environ.get("SANREMES_HOME", os.path.expanduser("~/.sanremes")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {sanremes_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for SanRemes Agent"
    )
