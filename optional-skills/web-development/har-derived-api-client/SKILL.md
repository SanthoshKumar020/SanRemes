...
Covers **every SanRemes browser pathway**: the default local `browser_navigate`
...
- For the CDP path (`har_capture_cdp.py`): a reachable CDP endpoint. On SanRemes,
...
| Browser pathway | How SanRemes reaches it | Capturer |
|---|---|---|
...
Rule of thumb: **if SanRemes *launched* the browser, use `har_capture.py`; if it
...
```bash
# 1a. Capture, LOCAL browser (SanRemes launched it)
...
har_capture.py <url> <out.har> [--wait S] [--headed] [--action SPEC ...]
  ...
  use when SanRemes LAUNCHED the browser (local browser_navigate default)
...
0. **Pick the capturer by pathway** (see How to Run table). Launched-locally → `har_capture.py`; reached over CDP → `har_capture_cdp.py`. On SanRemes, `/browser connect` tells you the CDP endpoint when a cloud/remote backend is active.
...
- **Wrong capturer = empty/no HAR.** `har_capture.py` on a cloud/CDP backend records nothing (it launches its own local browser instead of the one you meant). `har_capture_cdp.py` needs the endpoint; on SanRemes get it from `/browser connect` or `BROWSER_CDP_URL`. Match the capturer to the pathway (How to Run table).
...
- **CDP capture doesn't close the browser.** `har_capture_cdp.py` attaches to a browser it doesn't own and leaves it running — correct for cloud/remote sessions SanRemes manages. Don't add a close; let the owning backend tear it down.
