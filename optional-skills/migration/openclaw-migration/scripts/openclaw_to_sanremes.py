#!/usr/bin/env python3
"""OpenClaw -> SanRemes migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into SanRemes Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into SanRemes.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into SanRemes memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into SanRemes memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import SanRemes-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of SanRemes-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into SanRemes command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.sanremes/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.sanremes/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into SanRemes .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into SanRemes .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into SanRemes .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into SanRemes .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into SanRemes .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into SanRemes config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into SanRemes config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into SanRemes.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into SanRemes MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into SanRemes config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'sanremes cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into SanRemes config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into SanRemes session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into SanRemes custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into SanRemes config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into SanRemes config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into SanRemes config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `sanremes config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`sanremes config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.sanremes/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update SanRemes config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with SanRemes in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``SanRemes`` (prose), but lowercase matches
# like ``openclaw`` → ``sanremes`` (so filesystem paths like ``~/.openclaw``
# become ``~/.sanremes`` — the real SanRemes home — not the broken ``~/.SanRemes``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'SanRemes'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'SanRemes'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'SanRemes'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``SanRemes`` but maps ``openclaw`` → ``sanremes`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.sanremes/config.yaml`` (the real SanRemes home) instead of the broken
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with SanRemes.
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION SanRemes memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> SanRemes Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-sanremes/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the SanRemes config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-sanremes/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new SanRemes session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "SanRemes config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the SanRemes
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No SanRemes-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted SanRemes-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: sanremes config set",
                        )
                    continue
        ...
        for oc_key, sanremes_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and sanremes_key not in secret_additions:
                secret_additions[sanremes_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, sanremes_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and sanremes_key not in secret_additions:
                        secret_additions[sanremes_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        sanremes_config = load_yaml_file(destination)
        current_model = sanremes_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = sanremes_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                sanremes_config["model"] = {"default": model_str}
            dump_yaml_file(destination, sanremes_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; SanRemes still uses "edge"
        ...
        sanremes_config = load_yaml_file(destination)
        existing_tts = sanremes_config.get("tts", {})
        ...
        if self.execute:
            ...
            sanremes_config["tts"] = merged_tts
            dump_yaml_file(destination, sanremes_config)
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct SanRemes destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected SanRemes-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected SanRemes-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        sanremes_cfg_path = self.target_root / "config.yaml"
        sanremes_cfg = load_yaml_file(sanremes_cfg_path)
        existing_mcp = sanremes_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in SanRemes config")
                continue

            sanremes_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                sanremes_srv["command"] = srv["command"]
                if srv.get("args"):
                    sanremes_srv["args"] = srv["args"]
                if srv.get("env"):
                    sanremes_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    sanremes_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                sanremes_srv["url"] = srv["url"]
                if srv.get("headers"):
                    sanremes_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    sanremes_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                sanremes_srv["enabled"] = False
            if srv.get("timeout"):
                sanremes_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                sanremes_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                sanremes_srv["tools"] = {}
                if tools_cfg.get("include"):
                    sanremes_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    sanremes_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                sanremes_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = sanremes_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(sanremes_cfg_path)
            sanremes_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(sanremes_cfg_path, sanremes_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'sanremes cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = sanremes_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> SanRemes reasoning_effort
        ...
        if compaction:
            compression = sanremes_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = sanremes_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            sanremes_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = sanremes_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'sanremes gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("SANREMES_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        sanremes_cfg_path = self.target_root / "config.yaml"
        sanremes_cfg = load_yaml_file(sanremes_cfg_path)
        sr = sanremes_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        sanremes_cfg_path = self.target_root / "config.yaml"
        sanremes_cfg = load_yaml_file(sanremes_cfg_path)
        custom_providers = sanremes_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(sanremes_cfg_path)
            sanremes_cfg["custom_providers"] = custom_providers
            dump_yaml_file(sanremes_cfg_path, sanremes_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to SanRemes config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            sanremes_cfg_path = self.target_root / "config.yaml"
            sanremes_cfg = load_yaml_file(sanremes_cfg_path)
            discord_sanremes = sanremes_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_sanremes["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_sanremes["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                sanremes_cfg["discord"] = discord_sanremes
                dump_yaml_file(sanremes_cfg_path, sanremes_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        sanremes_cfg_path = self.target_root / "config.yaml"
        sanremes_cfg = load_yaml_file(sanremes_cfg_path)
        browser_sanremes = sanremes_cfg.get("browser") or {}
        changed = False

        # Map fields that have SanRemes equivalents
        if browser.get("cdpUrl"):
            browser_sanremes["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_sanremes["headless"] = browser["headless"]
            changed = True

        if changed:
            sanremes_cfg["browser"] = browser_sanremes
            if self.execute:
                self.maybe_backup(sanremes_cfg_path)
                dump_yaml_file(sanremes_cfg_path, sanremes_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            sanremes_mode = mode_map.get(mode, "manual")
            sanremes_cfg.setdefault("approvals", {})["mode"] = sanremes_mode
            if self.execute:
                self.maybe_backup(sanremes_cfg_path)
                dump_yaml_file(sanremes_cfg_path, sanremes_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{sanremes_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> SanRemes Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in SanRemes. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing SanRemes Config Not Overwritten)",
                "",
                "These items already existed in your SanRemes config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the SanRemes agent discovers these",
            "directories, it may read/write to them instead of the SanRemes state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `sanremes claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `sanremes claw cleanup` to fix it.",
            "",
            "## SanRemes-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `sanremes claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `sanremes setup` to configure any remaining settings",
            "- Run `sanremes mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `sanremes cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `sanremes cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `sanremes gateway install` if you need the gateway service",
            "- Review `~/.sanremes/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into SanRemes Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("SANREMES_HOME") or str(Path.home() / ".sanremes"), help="SanRemes home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing SanRemes targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of SanRemes-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> SanRemes Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.sanremes/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.sanremes/config.yaml")
        print("    2. Run: sanremes mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: sanremes cron")
