...
The design is **SanRemes-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
sanremes skills install official/security/unbroker
```

Then start a new SanRemes session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a SanRemes session:
...
- **PII handling.** Dossiers live under the SanRemes home directory (`0600`, optionally
