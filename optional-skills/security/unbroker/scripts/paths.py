...
All per-subject data lives under PDD_DATA_DIR (default: $SANREMES_HOME/unbroker),
which is the same trust boundary SanRemes uses for .env and OAuth tokens.
"""
...
def sanremes_home() -> Path:
    return Path(os.environ.get("SANREMES_HOME") or (Path.home() / ".sanremes"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else sanremes_home() / "unbroker"
