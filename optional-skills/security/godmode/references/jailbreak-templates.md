...
## 5. GODMODE FAST — SanRemes 4 405B (Zero Refusal)

**Model:** `nousresearch/sanremes-4-405b`
**Strategy:** SanRemes models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with SanRemes Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.sanremes/config.yaml`:
