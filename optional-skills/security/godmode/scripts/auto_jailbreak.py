...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SANREMES_HOME", os.path.expanduser("~/.sanremes")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SANREMES_HOME", Path.home() / ".sanremes")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# SanRemes config paths
# ═══════════════════════════════════════════════════════════════════

SANREMES_HOME = Path(os.getenv("SANREMES_HOME", Path.home() / ".sanremes"))
CONFIG_PATH = SANREMES_HOME / "config.yaml"
PREFILL_PATH = SANREMES_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/SanRemes models — already uncensored, just needs clean prompt
    "sanremes": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "sanremes" in model_lower or "nous" in model_lower:
        return "sanremes"
...
def _get_current_model() -> tuple:
    """Read current model and provider from SanRemes config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.sanremes/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart SanRemes for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart SanRemes for changes to take effect.")
