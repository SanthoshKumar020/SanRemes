...
the actual `setup.sh` that creates the project workspace, configures SanRemes
...
2. **Create profiles** — `sanremes profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.sanremes/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `sanremes kanban create` assigned to the director
...
```bash
sanremes profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.sanremes/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.sanremes/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most SanRemes
...
Each profile gets a `SOUL.md` at `~/.sanremes/profiles/<name>/SOUL.md` that
...
```bash
sanremes kanban create "Direct production of <video title>" \
    --assignee director \
...
the SanRemes `.env` (`${SANREMES_HOME:-$HOME/.sanremes}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _sanremes_env="${SANREMES_HOME:-$HOME/.sanremes}/.env"
    if grep -q "^${var}=" "$_sanremes_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_sanremes_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_sanremes_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY sanremes ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY sanremes OPENROUTER_API_KEY || exit 1
# ...
```
