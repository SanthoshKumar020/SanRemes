...
Maps each role archetype to the SanRemes skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public sanremes-agent
...
## SanRemes skills relevant to video production

### Visual / rendering skills (`sanremes-agent/skills/creative/`)
...
### Design / pre-production skills (`sanremes-agent/skills/creative/`)
...
### Audio / media skills (`sanremes-agent/skills/creative/` + `skills/media/`)
...
These are **not** SanRemes skills but external CLIs / APIs that profiles invoke.
...
## Built-in SanRemes tools for media review

These are native SanRemes tools — not invoked via terminal but through their own
...
key is present in `${SANREMES_HOME:-~/.sanremes}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every SanRemes profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SANREMES_HOME:-~/.sanremes}/.env` → environment variable.
...
When a new SanRemes-public video skill ships:
