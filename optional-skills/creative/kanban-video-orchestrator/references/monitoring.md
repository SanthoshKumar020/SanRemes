...
sanremes kanban watch --tenant <project-slug>

# Snapshot of the board
sanremes kanban list --tenant <project-slug>
sanremes kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
sanremes kanban stats --tenant <project-slug>

# Visual dashboard (browser)
sanremes dashboard

# Inspect a specific task (includes comments + events)
sanremes kanban show <task-id>

# Follow a single task's event stream
sanremes kanban tail <task-id>
```

Verify available subcommands with `sanremes kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `sanremes kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `sanremes kanban block <id>` then `sanremes kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `sanremes kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `sanremes kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `sanremes profile list` (profile exists?), `sanremes status` (gateway/dispatcher up?), and verify tenant. |
...
sanremes kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
sanremes kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(sanremes kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
sanremes kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
sanremes kanban block <task-id>
sanremes kanban archive <task-id>

# Diagnose what happened
sanremes kanban show <task-id>      # task body, comments, recent events
sanremes kanban tail <task-id>      # follow the live event stream
sanremes kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `sanremes kanban create` for the director
...
while true; do
    clear
    sanremes kanban list --tenant <slug>
    echo "---"
    sanremes kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `sanremes kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `sanremes profile list` and start
