#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `sanremes kanban list` and
...
def sanremes_available() -> bool:
    return shutil.which("sanremes") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed sanremes CLI."""
    try:
        out = subprocess.run(
            ["sanremes", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `sanremes kanban list`
    out = subprocess.run(
        ["sanremes", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["sanremes", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not sanremes_available():
        print("ERROR: 'sanremes' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
