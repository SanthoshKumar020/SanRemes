...
### 1. Start order: editor first, SanRemes second

SanRemes probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW SanRemes session.
...
restart the editor, restart the SanRemes session.
...
server fails to bind (Output Log) or SanRemes' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.sanremes/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for SanRemes

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. SanRemes' connection
lives in `~/.sanremes/config.yaml` via `sanremes mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms SanRemes — just don't
mistake it for the SanRemes setup step.
...
explicitly warns against overlapping calls. SanRemes executes same-turn tool
...
### 9. Timeouts: SanRemes gives up before Unreal does

SanRemes' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If SanRemes runs elsewhere (SSH backend,
