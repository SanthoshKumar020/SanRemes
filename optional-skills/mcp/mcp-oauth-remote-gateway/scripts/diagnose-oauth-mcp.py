...
Usage:
  ...
  <server_name> matches the files in $SANREMES_HOME/mcp-tokens/<server>.json etc.
...
def _sanremes_home():
    # Prefer SanRemes' own resolver (profile-safe); fall back to env then ~/.sanremes.
    try:
        from sanremes_constants import get_sanremes_home
        return str(get_sanremes_home())
    except Exception:
        return os.environ.get("SANREMES_HOME") or os.path.expanduser("~/.sanremes")


def _tokens_dir():
    return os.path.join(_sanremes_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "sanremes-diag", "version": "1.0"}},
        },
        headers={
