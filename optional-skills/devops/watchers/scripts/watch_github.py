...
Usage (via cron with --no-agent):
    ...
    sanremes cron create sanremes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$SANREMES_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name sanremes-issues --repo NousResearch/sanremes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the SanRemes .env file
(``${SANREMES_HOME:-~/.sanremes}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "SanRemes-Watcher/1.0",
    }
