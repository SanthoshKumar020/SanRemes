...
Usage (via cron with --no-agent):
    ...
    sanremes cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SANREMES_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "SanRemes-Watcher/1.0"})
