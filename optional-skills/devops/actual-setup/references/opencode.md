...
is typically only in `~/.sanremes/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike SanRemes,
...
`sanremes-custom-providers` skill, pitfall 2). SanRemes hits this because it forwards
