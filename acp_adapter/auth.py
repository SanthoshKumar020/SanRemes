"""ACP auth helpers — detect and advertise SanRemes authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "sanremes-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active SanRemes runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if SanRemes can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for SanRemes.
    ...
    not have SanRemes provider credentials configured yet, so SanRemes always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate SanRemes using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure SanRemes provider",
            description=(
                "Open SanRemes' interactive model/provider setup in a terminal. "
                "Use this when SanRemes has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
