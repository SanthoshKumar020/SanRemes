"""ACP session manager — maps ACP sessions to SanRemes AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.sanremes/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when SanRemes itself is running in WSL.

    Windows ACP clients can launch ``sanremes acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from sanremes_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch SanRemes from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["sanremes-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed SanRemes agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by SanRemes AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current SanRemes runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.sanremes/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``SANREMES_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_sanremes_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=sanremes_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see sanremes_state.get_messages_as_conversation).
        try:
    ...
    def _make_agent(
        ...
        *,
        ...
        # the automatic late-refresh (see SanRemesACPAgent._schedule_mcp_late_refresh).
        try:
            from sanremes_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
        ...
        # editor/session cwd instead of the SanRemes daemon's process cwd.
