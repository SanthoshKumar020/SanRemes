"""ACP agent server — exposes SanRemes Agent via the Agent Client Protocol."""
...
from tools.approval import (
    reset_sanremes_interactive_context,
    set_sanremes_interactive_context,
)
...
def _named_custom_provider_catalogs() -> list[tuple[str, str, list[tuple[str, str]]]]:
    ...
    try:
        ...
        from sanremes_cli.models import should_use_ollama_native_catalog
        from sanremes_cli.providers import custom_provider_slug
    except ImportError:
        return []
...
try:
    from sanremes_cli import __version__ as SANREMES_VERSION
except Exception:
    SANREMES_VERSION = "0.0.0"
...
# already uses (`sanremes_cli/moa_cmd.py`). This bounds each provider's row, not
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so SanRemes running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; SanRemes cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a SanRemes/OpenAI-compatible user content payload."""
...
class SanRemesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping SanRemes AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show SanRemes version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so SanRemes maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared SanRemes inventory is also used by ``sanremes model``, the TUI,
        ...
        try:
            from sanremes_cli.inventory import build_models_payload, load_picker_context
            from sanremes_cli.models import normalize_provider, provider_label
            ...
            def choice_provider(model_id: str) -> str:
                ...
                if parts[:1] == ["custom"] and len(parts) > 1:
                    from sanremes_cli.models import _configured_custom_provider_ids
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from sanremes_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  SanRemes estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_sanremes_session_id: str,
        previous_sanremes_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.sanremes.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_sanremes_session_id,
                previous_sanremes_session_id=previous_sanremes_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_sanremes_session_id: Optional[str] = None,
        previous_sanremes_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after SanRemes changes it.

        When the internal SanRemes head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_sanremes_session_id`` so the
        attached ``_meta.sanremes.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # sanremes_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_sanremes_session_id or session_id,
            previous_sanremes_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["sanremes-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        """
        try:
            from sanremes_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from sanremes_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="sanremes-agent", version=SANREMES_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # SanRemes' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches SanRemes setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.sanremes`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"sanremes": {"compactionSummary": True}}
        if kind == "merged":
            return {"sanremes": {"containsCompactionSummary": True}}
        return None
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes SanRemes
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run SanRemes on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive SanRemes' local kawaii waiting/status
        ...
        # tools.approval (set_sanremes_interactive_context) rather than
        # os.environ["SANREMES_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue SANREMES_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind SANREMES_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global SanRemes workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.sanremes/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global SANREMES_CRON_SESSION (#37968).
            ...
            interactive_token = set_sanremes_interactive_context(True)
            ...
            previous_session_id = os.environ.get("SANREMES_SESSION_ID")
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_sanremes_interactive_context(interactive_token)
                # Restore SANREMES_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("SANREMES_SESSION_ID", None)
                else:
                    os.environ["SANREMES_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal SanRemes DB session id before the turn so we
            ...
            pre_turn_sanremes_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (SANREMES_SESSION_KEY in
        ...
        # _meta.sanremes.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_sanremes_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_sanremes_id
            and pre_turn_sanremes_id
            and post_turn_sanremes_id != pre_turn_sanremes_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_sanremes_session_id=post_turn_sanremes_id,
                    previous_sanremes_session_id=pre_turn_sanremes_id,
                )
            except Exception:
                logger.debug(
        ...
        # SanRemes' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the SanRemes install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"SanRemes Agent v{SANREMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when SanRemes has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
