...
This is an additive SanRemes extension surfaced under ACP ``_meta.sanremes`` so
...
compression rotates the internal SanRemes head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.sanremes.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_sanremes_session_id: The live internal SanRemes DB session id
            (``state.agent.session_id``).
        previous_sanremes_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"sanremes": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_sanremes_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_sanremes_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_sanremes_session_id}
    ...
    rotated = bool(
        ...
        and previous_sanremes_session_id != current_sanremes_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentSanRemesSessionId": current_sanremes_session_id,
        "rootSanRemesSessionId": root_id,
        "parentSanRemesSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_sanremes_session_id:
        provenance["previousSanRemesSessionId"] = previous_sanremes_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"sanremes": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"sanremes": {"sessionProvenance": prov}}
