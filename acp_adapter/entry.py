"""CLI entry point for the sanremes-agent ACP adapter.

Loads environment variables from ``~/.sanremes/.env``, configures logging
...
try:
    ...
    # shadowing SanRemes's own modules — ``sanremes acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from SANREMES_HOME (default ``~/.sanremes``)."""
    from sanremes_cli.env_loader import load_sanremes_dotenv

    sanremes_home = get_sanremes_home()
    loaded = load_sanremes_dotenv(sanremes_home=sanremes_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", sanremes_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="sanremes-acp",
        description="Run SanRemes Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print SanRemes version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive SanRemes provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.sanremes/node/ "
...
def _print_version() -> None:
    ...
    print(sanremes_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import SanRemesACPAgent  # noqa: F401

    print("SanRemes ACP check OK")


def _run_setup() -> None:
    from sanremes_cli.main import main as sanremes_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "sanremes", "model"]
        sanremes_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting sanremes-agent ACP adapter")
    ...
    import acp
    from .server import SanRemesACPAgent
    ...
    if os.environ.get("SANREMES_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
