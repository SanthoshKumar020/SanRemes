"""ACP tool-call helpers for mapping sanremes tools to ACP ToolKind and building content."""
...
# ---------------------------------------------------------------------------
# Map sanremes tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a sanremes tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some SanRemes tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured SanRemes tool result clearly failed.
    ...
    # SanRemes core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # SanRemes read_file output is line-numbered with `|`. If we send it as raw
...
def build_tool_start(
    ...
    *,
    ...
    """Create a ToolCallStart event for the given sanremes tool invocation.
