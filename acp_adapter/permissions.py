"""ACP permission bridging for SanRemes dangerous-command approvals."""
...
# Maps ACP permission option ids to SanRemes approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True,
    ...
    """Return ACP options that match SanRemes approval semantics."""
    ...
    # Smart DENY override — the editor must not offer a scope SanRemes
    ...
    if not once_only:
        options.append(PermissionOption(
            ...
            # hint while keeping SanRemes semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_sanremes(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into SanRemes approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_SANREMES.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a SanRemes-compatible approval callback that bridges to ACP.
    ...
    def _callback(
        ...
        **_: object,
        ...
        return _map_outcome_to_sanremes(
            response.outcome,
