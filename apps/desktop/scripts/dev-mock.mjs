...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `sanremes-dev-mock-${Date.now()}`))
  ...
  return { root, sanremesHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(sanremesHome, mockUrl) {
  fs.writeFileSync(
    path.join(sanremesHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(sanremesHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.sanremesHome, mock.url)
  console.log(`  SANREMES_HOME: ${sandbox.sanremesHome}`)
  ...
  const env = {
    ...
    SANREMES_DESKTOP_APP_NAME: `SanRemesDevMock-${Date.now()}`,
  }
