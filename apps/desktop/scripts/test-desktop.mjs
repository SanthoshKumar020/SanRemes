...
// no longer bundles the SanRemes Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'SanRemes.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'SanRemes'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'SanRemes.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'SanRemes'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default SANREMES_HOME for non-sandboxed runs -- matches main.ts's
// resolveSanRemesHome(). On Windows it's %LOCALAPPDATA%\sanremes; elsewhere
// it's ~/.sanremes. The fresh-install sandbox launchFresh() sets its own
// SANREMES_HOME and never touches this.
const DEFAULT_SANREMES_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'sanremes')
  }
  return path.join(os.homedir(), '.sanremes')
})()
const VENV_ROOT = path.join(DEFAULT_SANREMES_HOME, 'sanremes-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'sanremes-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.SANREMES_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `SanRemes-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `SanRemes-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `SanRemes-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'SanRemes-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.SANREMES_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.SANREMES_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const sanremesHome = path.join(sandbox, 'sanremes-home')
  ...
  env.SANREMES_DESKTOP_CWD = cwd
  env.SANREMES_DESKTOP_IGNORE_EXISTING = '1'
  env.SANREMES_DESKTOP_TEST_MODE = 'fresh-install'
  env.SANREMES_DESKTOP_USER_DATA_DIR = userDataDir
  env.SANREMES_HOME = sanremesHome
  delete env.SANREMES_DESKTOP_SANREMES
  delete env.SANREMES_DESKTOP_SANREMES_ROOT
  ...
  console.log(`  SANREMES_HOME: ${sanremesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(sanremesHome, 'sanremes-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The SanRemes Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of sanremes_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'sanremes-agent', 'sanremes_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing SanRemes
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SANREMES_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  SANREMES_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
