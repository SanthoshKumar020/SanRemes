#!/usr/bin/env node
// set-exe-identity.mjs — stamp the SanRemes icon + version metadata onto the
// built SanRemes.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked SanRemes.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `sanremes desktop`, the installer's
...
// path (which rebuilds via `sanremes desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-SanRemes.exe>
//
...
// Stamp the SanRemes icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'SanRemes',
      FileDescription: 'SanRemes',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — SanRemes icon + identity stamped')
}
