...
const UPSTREAM_URL = 'https://github.com/NousResearch/sanremes-agent.git'
...
const LEGACY_STORAGE_KEY = 'sanremes.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'sanremes.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.sanremesDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `sanremes-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const sanremesHome = join(sandbox, 'sanremes-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(sanremesHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(sanremesHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        SANREMES_DESKTOP_APP_NAME: `SanRemesShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        SANREMES_DESKTOP_SANREMES_ROOT: target.targetRoot,
        SANREMES_DESKTOP_IGNORE_EXISTING: '1',
        SANREMES_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'sanremes-short-session-ab-'))
  const prepared = []
