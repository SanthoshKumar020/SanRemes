 ...
 * product name (`SanRemes`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/SanRemes'
 *
 * This is a hard failure with no obvious cause for the user — `sanremes desktop`
 ...
 * (the mac-unpacked SanRemes.app bundle) and Windows (win-unpacked), so we
 ...
 * SanRemes.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `sanremes desktop --build-only` (sanremes_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'SanRemes.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'SanRemes'}.exe`
