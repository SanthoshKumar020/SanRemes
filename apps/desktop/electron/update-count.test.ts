...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'sanremes-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'SanRemes Test')
    git('config', 'user.email', 'sanremes@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/NousResearch/sanremes-agent.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/NousResearch/sanremes-agent/compare/${SHA_A}...${SHA_B}`
  )
})

test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    'git@github.com:NousResearch/sanremes-agent.git',
    'ssh://git@github.com/NousResearch/sanremes-agent.git',
    'git@github.com:NousResearch/sanremes-agent'
  ]) {
    assert.equal(
