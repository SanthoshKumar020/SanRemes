...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    sanremesHome: 'C:\\Users\\andre\\.sanremes',
    python: 'C:\\Users\\andre\\.sanremes\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.sanremes-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      sanremesPath: 'C:\\SanRemes\\sanremes.exe',
      sanremesHome: 'C:\\Users\\andre\\.sanremes',
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command sanremes.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          sanremesHome: 'C:\\Users\\alice\\.sanremes',
          sanremesPath: 'C:\\SanRemes\\sanremes.exe',
          python: 'C:\\SanRemes\\python.exe'
        })
      }

      if (script.includes('.sanremes-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('sanremes_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.sanremes\\profiles\\research')
  assert.match(script, /\.sanremes-update-in-progress/)
...
test('Windows probe validates SanRemes and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $sanremesHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('SanRemes is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /SanRemes is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\SanRemes's\\python.exe" }, 'inspect', [
    'C:\\x y\\sanremes.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'sanremes_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /SanRemes''s/)
  assert.match(script, /C:\\x y\\sanremes\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { sanremesPath: lock.sanremesPath, sanremesHome: lock.sanremesHome }
