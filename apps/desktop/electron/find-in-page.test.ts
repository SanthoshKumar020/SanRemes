...
describe('installFoundInPageForwarder', () => {
  test('forwards found-in-page to the sender as a formatted payload', () => {
    ...
    assert.deepEqual(wc.calls.send, [{ channel: 'sanremes:found-in-page', payload: { activeMatchOrdinal: 2, count: 5 } }])
  })
  ...
  test('handles missing fields without throwing', () => {
    ...
    assert.deepEqual(wc.calls.send, [{ channel: 'sanremes:found-in-page', payload: { activeMatchOrdinal: 0, count: 0 } }])
  })
  ...
  test('two forwarders installed on distinct webContents do not cross-fire', () => {
    ...
    assert.deepEqual(wcA.calls.send, [
      { channel: 'sanremes:found-in-page', payload: { activeMatchOrdinal: 1, count: 1 } }
    ])
...
describe('installFindShortcut', () => {
  ...
  test('sends sanremes:open-find-bar on Ctrl+F (Linux/Windows) and prevents default', () => {
    const wc = makeFakeWebContents()
    ...
    assert.deepEqual(wc.calls.send, [{ channel: 'sanremes:open-find-bar', payload: undefined }])
  ...
  test('sends sanremes:open-find-bar on Cmd+F (meta) on macOS and prevents default', () => {
    const wc = makeFakeWebContents()
    const win = makeFakeWindow(wc)
