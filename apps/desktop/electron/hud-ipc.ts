...
export function registerHudIpc({
  ...
  ipcMain.on('sanremes:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('sanremes:hud:windowing', event => {
  ...
  ipcMain.on('sanremes:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('sanremes:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('sanremes:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('sanremes:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('sanremes:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('sanremes:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('sanremes:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('sanremes:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('sanremes:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('sanremes:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('sanremes:hud:close', async () => {
    closeHudWindow()
