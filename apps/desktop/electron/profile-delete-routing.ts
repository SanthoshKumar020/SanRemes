// Profile-delete routing logic for the `sanremes:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_sanremes_home() and
...
/** Parse a `sanremes:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its SANREMES_HOME.
 *
...
/**
 * Route the next `sanremes:api` request away from the primary/window backend
 ...
 * ensure_sanremes_home() recreates the directory the delete just removed.
 */
