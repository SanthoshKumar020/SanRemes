...
function ownedLock(over: any = {}) {
  return {
    ...
    port: 40000,
    profile: '',
    sanremesPath: '~/.local/bin/sanremes',
    sanremesHome: '~/.sanremes',
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing SanRemes', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('SANREMES_HOME')) {
          return '/home/alice/.sanremes\n'
        }

        if (command.includes('.sanremes-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.sanremes/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.sanremes-update-in-progress/)
})
...
test('listRemoteSanRemesProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/SANREMES_HOME/, '/Users/zillajr/.sanremes\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteSanRemesProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteSanRemesProfiles rejects a hostile SANREMES_HOME', async () => {
  const ssh = fakeSsh([[/SANREMES_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteSanRemesProfiles(ssh),
    (err: any) => {
...
test('locateSanRemes prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/sanremes/, 'OK']])
  assert.equal(await locateSanRemes(ssh, '/opt/sanremes'), '/opt/sanremes')
})

test('locateSanRemes throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong sanremes" bug.
  const ssh = fakeSsh([
    [/command -v sanremes/, '/home/u/.local/bin/sanremes\n'],
    [/\[ -x .*\.local\/bin\/sanremes/, 'OK']
  ])

  await assert.rejects(
    () => locateSanRemes(ssh, '/bad/path/sanremes'),
    (err: any) => {
      assert.equal(err.kind, 'sanremes-not-found')
      assert.match(err.message, /\/bad\/path\/sanremes/)
...
test('locateSanRemes falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v sanremes/, '/home/u/.local/bin/sanremes\n'],
    [/\[ -x .*\.local\/bin\/sanremes/, 'OK']
  ])

  assert.equal(await locateSanRemes(ssh, ''), '/home/u/.local/bin/sanremes')
})

test('locateSanRemes preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$SANREMES_BIN" "$SANREMES_ENTRYPOINT" "$@",
  // where $SANREMES_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.sanremes/sanremes-agent/venv/bin/python\n']
  ])

  assert.equal(await locateSanRemes(ssh, ''), '/home/u/.local/bin/sanremes')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateSanRemes must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateSanRemes returns an explicit remoteSanRemesPath unchanged', async () => {
  // The override half of #74411: an explicit remoteSanRemesPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/sanremes changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/sanremes/, 'OK'],
    [/python3 -c/, '/home/u/.sanremes/sanremes-agent/venv/bin/python\n']
  ])
...
test('locateSanRemes falls back to ~/.local/bin/sanremes when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateSanRemes(ssh, ''), '~/.local/bin/sanremes')
})

test('locateSanRemes tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/sanremes/, 'OK']])
  assert.equal(await locateSanRemes(ssh, ''), '~/.sanremes/sanremes-agent/venv/bin/sanremes')
})

test('locateSanRemes throws a sanremes-not-found error with an install hint', async () => {
...
test('locateSanRemes uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v sanremes/, '/x/sanremes'],
    [/\[ -x/, 'OK']
  ])

  await locateSanRemes(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.sanremes/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.sanremes/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.sanremes/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/sanremes'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/sanremes serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/sanremes'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/sanremes', '/Users/cd9c/.sanremes', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /sanremes-agent.*venv.*bin.*sanremes/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'sanremes wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'sanremes')
    const launcher = path.join(temp, 'sanremes launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/sanremes-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/sanremes', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('SANREMES_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/sanremes', 'work', {
    sanremesHome: '~/.sanremes',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.sanremes-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"sanremes-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /sanremes-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'sanremes-update-mutex-'))
  const sanremesPath = path.join(directory, 'sanremes')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *sanremes-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(sanremesPath, '', {
      sanremesHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { sanremesPath: '/x/sanremes', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('SANREMES_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('SANREMES_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nSANREMES_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'SANREMES_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'SANREMES_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.sanremes\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'SanRemes Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'SANREMES_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile sanremesPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ sanremesPath: '/old/stale/sanremes', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteSanRemesPath: '/new/sanremes', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'SANREMES_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes sanremesHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/SANREMES_HOME/, '/home/alice/.sanremes\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'SANREMES_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"sanremesHome":"\/home\/alice\/\.sanremes"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'SANREMES_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'SANREMES_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/sanremes'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.sanremes/sanremes-agent/venv/bin/sanremes'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/sanremes'))
  assert.doesNotThrow(() => validateRemotePath('~/.sanremes/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/sanremes'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('sanremes'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/sanremes'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/sanremes\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/sanremes\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/sanremes\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/sanremes', '/usr/`id`/sanremes', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.sanremes/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/sanremes')
  assert.ok(result.includes('/usr/local/bin/sanremes'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/sanremes')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('SANREMES_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/sanremes', 'work', {
    tokenFilePath: `~/.sanremes/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.sanremes\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/sanremes', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing SanRemes', () => {
  const cmd = buildSpawnCommand('/x/sanremes', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env SANREMES_DESKTOP=1/)
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.sanremes/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.sanremes/unrelated.log' }))
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/sanremes'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/sanremes'), false)
})
