...
beforeEach(async () => {
  dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'sanremes-download-fs-'))
})
...
test('a pre-open EEXIST collision leaves the seeded temp file and the destination untouched', async () => {
  const dest = path.join(dir, 'report.bin')
  const pinnedTemp = path.join(dir, '.sanremes-download-pinned.part')
  ...
  assert.deepEqual(await listing(), ['.sanremes-download-pinned.part', 'report.bin'])
})
