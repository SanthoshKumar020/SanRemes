 ...
 *
 * The staged Tauri `sanremes-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `sanremes update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `sanremes update`, swaps/relaunches
 * the app, and writes .sanremes-update-result.json. With the app gone before
 * the update starts, the SANREMES_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `sanremes update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into SANREMES_HOME on *every* platform
 * (`sanremes-setup.exe` on Windows, `sanremes-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_sanremes_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `sanremes update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(sanremesHome, 'sanremes-setup.exe')
 ...
 *
 * `copy_self_to_sanremes_home` deliberately no-ops during `--update`
 ...
 * "Another SanRemes update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `sanremes update`, which
 ...
 * the installer is written to SANREMES_HOME at install/repair time, making mtime
