 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `sanremes update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.sanremesHome, 'SanRemes home')
  const sanremesPath = validateRemoteValue(target.sanremesPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(sanremesPath)

  const updateCommand =
    `env SANREMES_HOME=${homeWord} ` +
    `SANREMES_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'SANREMES_UPDATE_ORIGIN_PROFILE=default ' +
    `SANREMES_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `SANREMES_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:SANREMES_HOME=${psLiteral(home)}`,
    `$env:SANREMES_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:SANREMES_UPDATE_ORIGIN_PROFILE="default"',
    `$env:SANREMES_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:SANREMES_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:SANREMES_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:SANREMES_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:SANREMES_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(sanremesPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.sanremes-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote SanRemes updated and every managed SSH profile is ready.'
        : restoreOk
