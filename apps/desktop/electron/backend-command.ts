// Backend subcommand routing for the desktop-managed SanRemes process.
//
// The desktop app launches its own headless backend via `sanremes serve` — it
...
// app hasn't updated yet, or an older `sanremes` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional SanRemes profile to pin via `--profile`.
 */
 ...
 * `-m sanremes_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `sanremes_cli/subcommands/dashboard.py` source registers
