...
const SCAN_MODULE = 'sanremes_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/SanRemes processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'sanremes_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another SanRemes process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the SanRemes installation is free.\n' +
    '\n' +
    'Close other SanRemes windows and terminals, then retry.  If the problem\n' +
    'persists, run `sanremes update` in a terminal for detailed diagnostics.'
  )
}
