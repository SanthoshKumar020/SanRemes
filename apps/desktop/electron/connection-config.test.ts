...
test('normalizeRemoteHeaders keeps safe proxy headers and drops transport/auth headers', () => {
  assert.deepEqual(
    normalizeRemoteHeaders({
      ...
      'X-SanRemes-Session-Token': { encoding: 'plain', value: 'token' },
...
test('remoteRequestMatchesBaseUrl treats HTTPS and WSS as the same gateway origin', () => {
  assert.equal(
    remoteRequestMatchesBaseUrl(
      'wss://sanremes.example.com/gateway/api/ws?ticket=abc',
      'https://sanremes.example.com/gateway'
    ),
    true
  )
  assert.equal(remoteRequestMatchesBaseUrl('ws://sanremes.example.com/api/ws', 'http://sanremes.example.com'), true)
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://sanremes.example.com/other/api/ws', 'https://sanremes.example.com/gateway'),
    false
  )
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://other.example.com/gateway/api/ws', 'https://sanremes.example.com/gateway'),
    false
  )
})
...
test('profileRemoteOverride returns the per-profile remote with defaulted auth mode', () => {
  const config = {
    profiles: {
      coder: { mode: 'remote', url: '  https://coder.example.com/sanremes  ', token: { value: 'sek' } }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://coder.example.com/sanremes',
    authMode: 'token',
    token: { value: 'sek' }
  })
})
...
test('localProfileEntry preserves inactive SSH drafts but drops Cloud state', () => {
  const ssh = { mode: 'ssh', host: 'box', user: 'alice', remoteSanRemesPath: '/sanremes' }
...
// --- translateSelfProfileQuery (registry SSH-scoped sanremes:api contract) ---
...
test('resolveProfileApiRequest routes action-status polls with the action-spawning routes', () => {
  ...
  // install): _spawn_sanremes_action registers the dynamic action name only in
...
test('normalizeRemoteBaseUrl strips trailing slashes, hash, and query', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/sanremes/'), 'https://gw.example.com/sanremes')
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/sanremes?x=1#frag'), 'https://gw.example.com/sanremes')
})

test('normalizeRemoteBaseUrl preserves a path prefix', () => {
  assert.equal(normalizeRemoteBaseUrl('https://host/sanremes'), 'https://host/sanremes')
})
...
test('normalizeRemoteBaseUrl auto-prepends http:// for scheme-less host:port input', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('gw.example.com/sanremes/'), 'http://gw.example.com/sanremes')
})
...
test('buildGatewayWsUrl honors a path prefix', () => {
  assert.equal(buildGatewayWsUrl('https://host/sanremes', 't'), 'wss://host/sanremes/api/ws?token=t')
})
...
test('buildGatewayWsUrlWithTicket uses ?ticket= not ?token=', () => {
  const url = buildGatewayWsUrlWithTicket('https://gw.example.com/sanremes', 'tkt-9')
  assert.equal(url, 'wss://gw.example.com/sanremes/api/ws?ticket=tkt-9')
...
test('cookiesHaveSession detects the bare access-token cookie', () => {
  assert.equal(cookiesHaveSession([{ name: 'sanremes_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession detects the __Host- and __Secure- prefixed variants', () => {
  assert.equal(cookiesHaveSession([{ name: '__Host-sanremes_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveSession([{ name: '__Secure-sanremes_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession is false for an empty value', () => {
  assert.equal(cookiesHaveSession([{ name: 'sanremes_session_at', value: '' }]), false)
})
...
test('cookiesHaveSession ignores unrelated cookies (AT-only by design)', () => {
  ...
  assert.equal(cookiesHaveSession([{ name: 'sanremes_session_rt', value: 'x' }]), false)
...
test('AT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(AT_COOKIE_VARIANTS, ['__Host-sanremes_session_at', '__Secure-sanremes_session_at', 'sanremes_session_at'])
})

test('RT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(RT_COOKIE_VARIANTS, ['__Host-sanremes_session_rt', '__Secure-sanremes_session_rt', 'sanremes_session_rt'])
})

// --- cookiesHaveLiveSession (AT or RT — the connectivity check) ---

test('cookiesHaveLiveSession is true for a live access-token cookie', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'sanremes_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-sanremes_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-sanremes_session_at', value: 'x' }]), true)
})
...
test('cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie', () => {
  ...
  assert.equal(cookiesHaveLiveSession([{ name: 'sanremes_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-sanremes_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-sanremes_session_rt', value: 'x' }]), true)
})

test('cookiesHaveLiveSession is true when both AT and RT are present', () => {
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'sanremes_session_at', value: 'a' },
      { name: 'sanremes_session_rt', value: 'r' }
    ]),
    true
  )
})
...
test('cookiesHaveLiveSession is false for empty values', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'sanremes_session_at', value: '' }]), false)
  assert.equal(cookiesHaveLiveSession([{ name: 'sanremes_session_rt', value: '' }]), false)
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'sanremes_session_at', value: '' },
      { name: 'sanremes_session_rt', value: '' }
    ]),
    false
  )
})
...
test('cookiesHavePrivySession does NOT treat sanremes gateway cookies as a portal session', () => {
  // The whole point of Q7: a gateway session cookie is NOT a portal sign-in.
  assert.equal(cookiesHavePrivySession([{ name: 'sanremes_session_at', value: 'x' }]), false)
  assert.equal(cookiesHavePrivySession([{ name: '__Host-sanremes_session_rt', value: 'x' }]), false)
})
...
test('cookiesHavePrivyAccessToken is false for empty values, gateway cookies, and non-arrays', () => {
  ...
  assert.equal(cookiesHavePrivyAccessToken([{ name: 'sanremes_session_at', value: 'x' }]), false)
...
test('gateway WS URL IPC result serializes success and the auth-vs-transport matrix', async () => {
  ...
  for (const error of [
    ...
    new Error('Timed out connecting to SanRemes backend after 8000ms'),
...
// BEFORE waitForSanRemesReady. This mirrors main.ts buildRemoteConnection's
