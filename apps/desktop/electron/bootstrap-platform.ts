 ...
 * null to keep it enabled. `SANREMES_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform } = {}) {
  ...
  const override = String(env.SANREMES_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()

  if (GPU_OVERRIDE_ON.has(override)) {
    return 'override (SANREMES_DESKTOP_DISABLE_GPU)'
  }
 ...
 * gateway tokens. The `sanremes desktop` launcher detects the session keychain
 * (or reads `desktop.password_store` from config.yaml) and bridges the value
 * in via SANREMES_DESKTOP_PASSWORD_STORE.
 *
...
function resolveLinuxPasswordStore(options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform } = {}) {
  ...
  const requested = String(env.SANREMES_DESKTOP_PASSWORD_STORE || '').trim()
  ...
  if (!LINUX_PASSWORD_STORES.has(requested)) {
    return { store: null, warning: `ignoring unknown SANREMES_DESKTOP_PASSWORD_STORE value: ${requested}` }
  }
