export type ParentWatchdogEnv = {
  SANREMES_PARENT_PID: string
  SANREMES_PARENT_START_MARKER?: string
  SANREMES_PARENT_NONCE?: string
  /** Spawn tag consumed by sanremes_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { SANREMES_PARENT_PID: String(pid) }
  env.SANREMES_SPAWN = spawnTag(pid, startMarker)
  ...
  env.SANREMES_PARENT_START_MARKER = startMarker
  env.SANREMES_PARENT_NONCE = nonce

  return env
}

/** Build the `SANREMES_SPAWN` tag mirrored by sanremes_cli/process_identity.py.
