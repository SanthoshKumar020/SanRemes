...
test('SANREMES_GIT_BASH_PATH override takes precedence', () => {
  const result = findGitBash({
    isWindows: true,
    env: { SANREMES_GIT_BASH_PATH: 'D:\\CustomGit\\bin\\bash.exe' },
    fileExists: yes,
    findOnPath: () => null
  })
...
test('SANREMES_GIT_BASH_PATH invalid path falls through to candidates', () => {
  const env = {
    SANREMES_GIT_BASH_PATH: 'X:\\Missing\\bash.exe',
...
test('SANREMES_GIT_BASH_PATH empty string is ignored', () => {
  const result = findGitBash({
    isWindows: true,
    env: { SANREMES_GIT_BASH_PATH: '', LOCALAPPDATA: '' },
    fileExists: no,
