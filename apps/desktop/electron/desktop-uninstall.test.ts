...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'sanremes_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'sanremes_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'sanremes_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/SanRemes.app/Contents/MacOS/SanRemes', 'darwin'),
    '/Applications/SanRemes.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/SanRemes.app/Contents/MacOS/SanRemes', 'darwin'),
    '/Users/x/Applications/SanRemes.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\SanRemes\\SanRemes.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\SanRemes'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\sanremes-desktop\\SanRemes.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\sanremes-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\SanRemes.exe', 'win32'), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/SanRemes.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/SanRemes.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.sanremes/sanremes-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.sanremes/sanremes-agent',
    uninstallArgs: ['-m', 'sanremes_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/sanremes/linux-unpacked',
    sanremesHome: '/home/x/.sanremes'
  })
  ...
  assert.match(script, /'-m' 'sanremes_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/sanremes\/linux-unpacked'/)
  assert.match(script, /export SANREMES_HOME='\/home\/x\/\.sanremes'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.sanremes/sanremes-agent',
    agentRoot: '/home/x/.sanremes/sanremes-agent',
    uninstallArgs: ['-m', 'sanremes_cli.uninstall', '--mode', 'full'],
    appPath: null,
    sanremesHome: '/home/x/.sanremes'
  })

  // System python + source on PYTHONPATH so import sanremes_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.sanremes\/sanremes-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'sanremes_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'sanremes_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    sanremesHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'sanremes_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    pythonPath: 'C:\\sanremes',
    agentRoot: 'C:\\sanremes',
    uninstallArgs: ['-m', 'sanremes_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\SanRemes',
    sanremesHome: 'C:\\Users\\x\\AppData\\Local\\sanremes'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import sanremes_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\sanremes;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "sanremes_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\SanRemes" >nul 2>&1/)
