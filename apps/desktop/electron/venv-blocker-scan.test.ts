...
describe('resolveVenvPython', () => {
  it('returns a real path when a temp venv python file exists', () => {
    const sandbox = fs.mkdtempSync(path.join(os.tmpdir(), 'sanremes-vt-'))
...
describe('formatProbeFailedMessage', () => {
  it('suggests retry and sanremes update', () => {
    const msg = formatProbeFailedMessage()
    assert.ok(msg.includes('sanremes update'))
    assert.ok(msg.includes('retry'))
  })
})
        ...
        processes: [
          {
            pid: 47484,
            name: 'python.exe',
            cmdline: 'C:\\SanRemes\\venv\\Scripts\\python.exe -m http.server 8766 --directory C',
            kind: 'local-preview',
            safeToStop: true,
...
describe('scanVenvBlockers', () => {
  ...
  it('calls subprocess with correct args, cwd and timeout', async () => {
    ...
    assert.deepEqual(c.args, ['-m', 'sanremes_cli._scan_venv_blockers'])
...
describe('stopSafeVenvBlockers', () => {
  it('stops only blockers explicitly classified as safe local previews', async () => {
    ...
    assert.deepEqual(calls, [
      {
        command: 'C:\\SanRemes\\venv\\Scripts\\python.exe',
        args: ['-m', 'sanremes_cli._scan_venv_blockers', '--terminate-safe', '47484', '1722798000.25']
      }
    ])
