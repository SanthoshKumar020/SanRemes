...
const READY_RE = /^SANREMES_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitSanRemesPath = '') {
  const explicit = psLiteral(explicitSanRemesPath)
  ...
  const script = [
    ...
    '$sanremesHome=$env:SANREMES_HOME',
    'if(-not $sanremesHome){$sanremesHome=Join-Path $env:LOCALAPPDATA "sanremes"}',
    'Assert-NoReparse $sanremesHome $true',
    '$candidate=[IO.Path]::Combine($sanremesHome, "sanremes-agent\\venv\\Scripts\\sanremes.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "sanremes-agent\\.venv\\Scripts\\sanremes.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $sanremesHome "sanremes-agent\\venv\\Scripts\\sanremes.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "sanremes-agent\\.venv\\Scripts\\sanremes.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command sanremes.exe -ErrorAction SilentlyContinue',
    ...
    '$sanremes=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$sanremes=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $sanremes){throw "SanRemes is not installed on the remote Windows host."}',
    'Assert-NoReparse $sanremes $false',
    'if($explicit -and $sanremes -ne $explicit){throw "The configured SanRemes path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($sanremes), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;sanremesHome=$sanremesHome;sanremesPath=$sanremes;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(sanremesHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class SanRemesMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(sanremesHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".sanremes-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[SanRemesMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, sanremesHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(sanremesHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitSanRemesPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitSanRemesPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'sanremes_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'sanremes_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'sanremes_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.sanremesHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".sanremes-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};sanremesPath=${psLiteral(reservation.sanremesPath)};sanremesHome=${psLiteral(reservation.sanremesHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.sanremesPath === 'string' &&
    typeof lock.sanremesHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.sanremesPath === runtime.sanremesPath &&
    lock.sanremesHome === runtime.sanremesHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteSanRemesPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.sanremesHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.sanremesPath])

  if (!inspection.supported) {
    const error: any = new Error('Update SanRemes on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.sanremesPath = inspection.path
  const sanremesVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located sanremes at ${runtime.sanremesPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, sanremesPath: runtime.sanremesPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForSanRemes(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="SanRemes SSH"', 'powershell.exe -NoLogo')
