 ...
 * origin=git@github.com:NousResearch/sanremes-agent.git. A background
...
test('canonicalGitHubRemote normalizes SSH and HTTPS forms to the same value', () => {
  assert.equal(canonicalGitHubRemote('git@github.com:NousResearch/sanremes-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('git@github.com:NousResearch/sanremes-agent'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('ssh://git@github.com/NousResearch/sanremes-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('https://github.com/NousResearch/sanremes-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Case-insensitive: an uppercased owner still canonicalizes to the same repo.
  assert.equal(canonicalGitHubRemote('git@github.com:nousresearch/sanremes-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Trailing slashes are stripped.
  assert.equal(canonicalGitHubRemote('https://github.com/NousResearch/sanremes-agent/'), OFFICIAL_REPO_CANONICAL)
})
...
test('isSshRemote detects scp-like and ssh:// forms only', () => {
  assert.equal(isSshRemote('git@github.com:NousResearch/sanremes-agent.git'), true)
  assert.equal(isSshRemote('ssh://git@github.com/NousResearch/sanremes-agent.git'), true)
  assert.equal(isSshRemote('https://github.com/NousResearch/sanremes-agent.git'), false)
...
test('isOfficialSshRemote is true only for the official repo over SSH', () => {
  assert.equal(isOfficialSshRemote('git@github.com:NousResearch/sanremes-agent.git'), true)
  assert.equal(isOfficialSshRemote('git@github.com:NousResearch/sanremes-agent'), true)
  assert.equal(isOfficialSshRemote('ssh://git@github.com/NousResearch/sanremes-agent.git'), true)
...
test('isOfficialSshRemote does NOT match forks, other hosts, or HTTPS', () => {
  ...
  assert.equal(isOfficialSshRemote('git@github.com:someuser/sanremes-agent.git'), false)
  // Same repo name on a different host is not the official repo.
  assert.equal(isOfficialSshRemote('git@gitlab.com:NousResearch/sanremes-agent.git'), false)
  ...
  assert.equal(isOfficialSshRemote('https://github.com/NousResearch/sanremes-agent.git'), false)
