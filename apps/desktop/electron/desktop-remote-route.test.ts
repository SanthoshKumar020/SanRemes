...
test('environment URL without its token keeps the existing error', () => {
  assert.throws(
    ...
    /SANREMES_DESKTOP_REMOTE_TOKEN is not/
  )
})
...
test('profile SSH route fails closed when any dial field differs', () => {
  ...
  const variants = [
    { ...ssh, port: 2200 },
    { ...ssh, keyPath: '/keys/b' },
    { ...ssh, remoteSanRemesPath: '/opt/sanremes' },
...
test('global SSH treats an omitted port as 22 and checks the primary route', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'ssh', remote: { mode: 'ssh', host: 'box.test', user: 'sanremes' } },
    registry: registry('ssh-primary', [
      { id: 'ssh-primary', kind: 'ssh', label: 'SSH primary', host: 'box.test', user: 'sanremes', port: 22 }
    ])
  })
...
test('profile remote wins over a registry-backed global SSH route', () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      mode: 'ssh',
      remote: { mode: 'ssh', host: 'global-box.test', user: 'sanremes' },
      profiles: {
        worker: { mode: 'remote', url: 'https://worker.test', authMode: 'token', token: tokenA }
      }
    },
    ...
    registry: registry('global-ssh', [
      { id: 'global-ssh', kind: 'ssh', label: 'Global SSH', host: 'global-box.test', user: 'sanremes' },
...
test('profile SSH wins over a different registry primary SSH route', () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      ...
      remote: { mode: 'ssh', host: 'global-box.test', user: 'sanremes' },
      profiles: {
        worker: { mode: 'ssh', host: 'worker-box.test', user: 'sanremes' }
      }
    },
    profile: 'worker',
    registry: registry('global-ssh', [
      { id: 'global-ssh', kind: 'ssh', label: 'Global SSH', host: 'global-box.test', user: 'sanremes' },
      { id: 'worker-ssh', kind: 'ssh', label: 'Worker SSH', host: 'worker-box.test', user: 'sanremes' }
    ])
  })
...
test('local route does not inherit an unrelated registry SSH connection', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'local' },
    registry: registry('local', [
      { id: 'unused-ssh', kind: 'ssh', label: 'Unused SSH', host: 'box.test', user: 'sanremes' }
    ])
  })
...
// remote transport, or startSanRemes() spawns a loopback `sanremes serve` the
    ...
    registry: registry('cloud-1', [
      {
        ...
        url: 'https://agent.sanremes.cloud',
        authMode: 'oauth',
        org: 'nous'
      }
    ])
