...
// hardening, SANREMES_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('sanremes:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('sanremes:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('sanremes:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('sanremes:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<SANREMES_HOME>/desktop-plugins`,
  // resolved from the main-process SANREMES_HOME (see resolveSanRemesHome) — NOT from
  // the connected backend. A remote backend reports its own `sanremes_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped sanremes_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(sanremesHome, 'profiles', profile) : sanremesHome
  ...
  ipcMain.handle('sanremes:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<SANREMES_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where SANREMES_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('sanremes:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<SANREMES_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('sanremes:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('sanremes:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('sanremes:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('sanremes:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('sanremes:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('sanremes:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
