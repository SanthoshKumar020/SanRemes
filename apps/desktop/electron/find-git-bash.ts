 ...
 *   1. SANREMES_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\sanremes\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect SANREMES_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.SANREMES_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'sanremes', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'sanremes', 'git', 'usr', 'bin', 'bash.exe'))
  }
