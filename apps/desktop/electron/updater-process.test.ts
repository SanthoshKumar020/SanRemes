...
test('stagedUpdaterSupportsPrewrittenMarker rejects installers predating the self-adopt fix', () => {
  ...
  // refreshed because copy_self_to_sanremes_home no-ops during --update.
  assert.equal(
    stagedUpdaterSupportsPrewrittenMarker('C:\\SanRemes\\sanremes-setup.exe', {
...
test('resolveStagedUpdaterBinary still returns a stale staged updater on Windows', () => {
  ...
  assert.equal(
    resolveStagedUpdaterBinary('C:\\SanRemes', {
      fileExists: () => true,
      isWindows: true,
      stagedMtimeMs: () => MARKER_SELF_ADOPT_EPOCH_MS - 60 * DAY_MS
    }),
    path.join('C:\\SanRemes', 'sanremes-setup.exe')
  )
})
...
test('spawnUpdaterProcess hides the updater console and detaches the child on Windows', () => {
  ...
  const result = spawnUpdaterProcess(
    'sanremes-setup.exe',
    ['--update', '--branch', 'main'],
    { cwd: 'C:\\SanRemes', detached: true, stdio: 'ignore' },
    {
      isWindows: true,
  ...
  assert.deepEqual(calls, [
    {
      ...
      command: 'sanremes-setup.exe',
      options: { cwd: 'C:\\SanRemes', detached: true, stdio: 'ignore', windowsHide: true }
    }
  ])
})
...
test('resolveStagedUpdaterBinary hands Windows the staged installer it finds', () => {
  const home = 'C:\\Users\\sanremes\\AppData\\Local\\sanremes'
  const staged = path.join(home, 'sanremes-setup.exe')
  const probed: string[] = []
...
test('resolveStagedUpdaterBinary returns null off Windows even when sanremes-setup is staged (#74836)', () => {
  const home = '/Users/sanremes/.sanremes'
  let probes = 0

  const resolved = resolveStagedUpdaterBinary(home, {
    // The installer stages sanremes-setup on macOS/Linux too, so "it exists" is
...
test('resolveStagedUpdaterBinary returns null on Windows when nothing is staged', () => {
  const resolved = resolveStagedUpdaterBinary('C:\\Users\\sanremes\\AppData\\Local\\sanremes', {
    fileExists: () => false,
    isWindows: true
  })
...
test('resolveUpdateScriptHandoff prefers the repo script on Windows when present', () => {
  const root = String.raw`C:\Users\sanremes\AppData\Local\sanremes\sanremes-agent`
...
test('resolveUpdateScriptHandoff returns null when the checkout predates the script', () => {
  const handoff = resolveUpdateScriptHandoff(String.raw`C:\Users\sanremes\AppData\Local\sanremes\sanremes-agent`, {
    isWindows: true,
    fileExists: () => false
  })
...
test('resolveUpdateScriptHandoff is Windows-only (POSIX updates in place)', () => {
  const handoff = resolveUpdateScriptHandoff('/home/sanremes/.sanremes/sanremes-agent', {
    isWindows: false,
    fileExists: () => true
  })
...
test('resolvePosixScriptHandoff returns the bash recipe when the script exists', () => {
  const root = '/home/sanremes/.sanremes/sanremes-agent'
...
test('resolvePosixScriptHandoff is null when the checkout predates the script', () => {
  const handoff = resolvePosixScriptHandoff('/home/sanremes/.sanremes/sanremes-agent', {
    isWindows: false,
    fileExists: () => false
  })
...
test('resolvePosixScriptHandoff is null on Windows', () => {
  const handoff = resolvePosixScriptHandoff(String.raw`C:\Users\sanremes\AppData\Local\sanremes\sanremes-agent`, {
    isWindows: true,
    fileExists: () => true
  })
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--no-sandbox',
    'sanremes://open/session/abc',
    '--profile=work'
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'sanremes://open/session/abc', '--profile=work'])
