// Unit tests for the pure Windows `sanremes` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvSanRemesCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `sanremes` shim shadows
//      the real sanremes.cmd/sanremes.exe.
...
//   3. resolveVenvSanRemesCommand() — must probe the venv python via
//      canImportSanRemesCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSanRemes: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSanRemes: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSanRemes: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSanRemes: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvSanRemes: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvSanRemes: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvSanRemesCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportSanRemesCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    sanremesHome: '/fake/sanremes-home',
...
test('resolveVenvSanRemesCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvSanRemesCommand('/root/venv/Scripts/sanremes.exe', [], deps), null)
})

test('resolveVenvSanRemesCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvSanRemesCommand('/root/venv/Scripts/sanremes.cmd', [], deps), null)
})

test('resolveVenvSanRemesCommand: returns null when the basename is not sanremes/sanremes.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSanRemesCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvSanRemesCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSanRemesCommand('/root/venv/bin/sanremes.exe', [], deps), null)
})

test('resolveVenvSanRemesCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvSanRemesCommand('/root/venv/Scripts/sanremes.exe', [], deps), null)
})

test('resolveVenvSanRemesCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportSanRemesCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvSanRemesCommand('/root/venv/Scripts/sanremes.exe', ['serve'], deps)
...
test('resolveVenvSanRemesCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvSanRemesCommand('/root/venv/Scripts/sanremes.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'sanremes_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvSanRemesCommand: is case-insensitive on sanremes.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvSanRemesCommand('/root/venv/Scripts/SANREMES.EXE', [], deps))
  assert.ok(resolveVenvSanRemesCommand('/root/venv/SCRIPTS/sanremes.exe', [], deps))
})
