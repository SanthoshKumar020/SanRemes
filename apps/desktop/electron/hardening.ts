...
// Relative, not `@sanremes/shared`: the electron bundle is built by esbuild with
...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so SanRemes Desktop cannot save remote gateway tokens. ' +
        ...
        'or set SANREMES_DESKTOP_REMOTE_URL and SANREMES_DESKTOP_REMOTE_TOKEN in your environment.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set SANREMES_DESKTOP_REMOTE_URL and SANREMES_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
...
// SANREMES_DESKTOP_REMOTE_* env vars.
