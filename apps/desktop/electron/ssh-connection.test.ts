...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env SANREMES_DASHBOARD_SESSION_TOKEN=abc123deadbeef SANREMES_DESKTOP=1 sanremes dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /SANREMES_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /SANREMES_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-SanRemes-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-SanRemes-Session-Token: hdr_888'), /X-SanRemes-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-SanRemes-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('sanremes', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v sanremes', 15000)
  assert.equal(args[args.length - 1], 'command -v sanremes')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `sanremes-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `sanremes-ssh-test-${process.pid}-${Date.now()}`)
