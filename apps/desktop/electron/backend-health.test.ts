...
test('uses lightweight /api/health for current backends', async () => {
  ...
  await waitForSanRemesReady('http://127.0.0.1:9000/', {
    token: 'secret-token',
...
test('does not fall back to heavyweight /api/status for transient health failures', async () => {
  ...
  await assert.rejects(
    waitForSanRemesReady('http://127.0.0.1:9000', {
      fetchPublicJson: async url => {
        calls.push(['public', url])
        throw new Error('Timed out connecting to SanRemes backend after 15000ms')
      },
      fetchJson: async url => {
        calls.push(['token', url])
      },
...
test('recognizes missing-route shapes only', () => {
  ...
  assert.equal(
    isMissingHealthEndpointError(
      new Error('Expected JSON from /api/health but got HTML. The endpoint is likely missing on the SanRemes backend.')
    ),
    true
  )
  assert.equal(isMissingHealthEndpointError(new Error('Timed out connecting to SanRemes backend after 15000ms')), false)
...
test('anonymous gate-shaped 401 falls back to /api/status (backend predates /api/health)', async () => {
  ...
  await waitForSanRemesReady('http://192.168.1.132:9119', {
    token: null,
    fetchPublicJson: async url => {
      calls.push(['public', url])
...
test('a credentialed 401 fails fast for reauth instead of reporting a dead session ready', async () => {
  ...
  await assert.rejects(
    waitForSanRemesReady('https://gateway.example', {
      token: 'session-token',
      fetchPublicJson: async () => {
...
test('unsigned OAuth is a terminal reauth failure; needsOauthLogin alone is not', () => {
  // The unsigned-in throw must set isReauthRequired so startSanRemes latches.
  ...
  assert.equal(isReauthRequiredError(new Error('Could not reach the remote SanRemes gateway')), false)
})
...
test('a credentialed probe still uses the 404 fallback for a genuinely missing route', async () => {
  ...
  await waitForSanRemesReady('https://gateway.example', {
    token: 'session-token',
    fetchPublicJson: async () => {
      throw new Error('public probe must not be used when credentialed')
    },
...
test('waitForSanRemesReady surfaces actionable error for cloud agent 503', async () => {
  let attempts = 0
  const currentTime = { value: 0 }

  try {
    await waitForSanRemesReady('https://ares-3009.agents.nousresearch.com', {
      fetchPublicJson: async () => {
        attempts++
        // Always return 503
...
test('waitForSanRemesReady does not cloud-wrap non-cloud 503 errors', async () => {
  const currentTime = { value: 0 }
