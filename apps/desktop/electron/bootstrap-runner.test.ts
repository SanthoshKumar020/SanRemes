...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'sanremes-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/sanremes-runner-test',
    sourceRepoRoot: null,
    sanremesHome: '/tmp/sanremes-runner-test',
    logRoot: '/tmp/sanremes-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'sanremes-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'sanremes-agent')
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/sanremes-agent', '--sanremes-home', '/tmp/sanremes', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/sanremes',
      sanremesHome: '/tmp/home'
    }),
    ['--dir', '/tmp/sanremes', '--sanremes-home', '/tmp/home', '--branch', 'main']
  )
})
