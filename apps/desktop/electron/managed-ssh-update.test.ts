...
test('POSIX managed launcher is detached, correlation-scoped, and never publishes handoff exit 75', () => {
  ...
  assert.match(command, new RegExp(`SANREMES_UPDATE_CORRELATION_ID=.*${CORRELATION}`))
...
test('POSIX managed launcher executes the updater command and atomically publishes its status', async () => {
  const home = await mkdtemp(path.join(os.tmpdir(), 'sanremes-managed-launch-'))
...
test('Windows managed launcher starts a hidden child and leaves exit 75 to the external coordinator', () => {
  const command = buildWindowsManagedUpdateLaunch(
    {
      ...
      pythonPath: 'C:\\SanRemes\\python.exe'
    },
    CORRELATION
  )
  ...
  assert.match(wrapper, /SANREMES_UPDATE_WINDOWS_DETACHED/)
  assert.match(wrapper, /SANREMES_UPDATE_TAURI_READY_PATH/)
  assert.match(wrapper, /SANREMES_UPDATE_TAURI_OUTCOME_PATH/)
...
test('POSIX observer reads the exact correlation receipt and terminal marker from disk', async () => {
  const home = await mkdtemp(path.join(os.tmpdir(), 'sanremes-managed-update-'))
    ...
    const command = buildRemoteUpdateObservationCommand(
      {
        ssh: { exec: async () => '' },
        platform: 'Linux',
        sanremesPath: '/opt/sanremes/sanremes',
        sanremesHome: home
      },
      CORRELATION
    )
...
test('managed observer unwraps a named profile home for the install-wide marker', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'sanremes-managed-profile-marker-'))
  const profileHome = path.join(root, 'profiles', 'research')

  try {
    await mkdir(profileHome, { recursive: true })
    await writeFile(path.join(root, '.sanremes-update-in-progress'), `${process.pid}\n1\n`)
