...
describe('buildOpaqueProfileRoutes', () => {
  it('groups SSH aliases by their effective route without exposing endpoint data', async () => {
    const configs = new Map([
      [
        ...
        config({
          mode: 'ssh',
          sshHost: 'lab-a',
          sshRemoteSanRemesPath: '~/.sanremes',
    ...
    const resolveSsh = vi.fn(async () => ({ hostname: 'gateway.example', port: 22, user: 'sanremes' }))
    ...
    expect(JSON.stringify(routes)).not.toContain('.sanremes')
  })

  it('changes opaque IDs when the effective SSH destination changes', async () => {
    const options = {
      getProfileConfig: () => config({ mode: 'ssh', sshHost: 'lab', sshRemoteSanRemesPath: '~/.sanremes' }),
      globalConfig: config(),
      installationId: 'install-a-secret',
      primaryProfile: 'default',
    ...
    const before = await buildOpaqueProfileRoutes({
      ...options,
      resolveSsh: async () => ({ hostname: 'old.example', port: 22, user: 'sanremes' })
    })

    const after = await buildOpaqueProfileRoutes({
      ...options,
      resolveSsh: async () => ({ hostname: 'new.example', port: 22, user: 'sanremes' })
    })
  ...
  it('isolates an SSH resolution failure to its configured route', async () => {
    const options = {
      getProfileConfig: (profile: string) =>
        profile === 'broken'
          ? config({ mode: 'ssh', sshHost: 'unreachable', sshPort: 2222, sshUser: 'sanremes' })
          : config(),
      globalConfig: config(),
      installationId: 'install-a-secret',
      primaryProfile: 'default',
  ...
  it('reports an explicit backend profile inherited from global SSH', async () => {
    const routes = await buildOpaqueProfileRoutes({
      ...
      resolveSsh: vi.fn(async () => ({ hostname: 'gateway.example', port: 22, user: 'sanremes' }))
    })
