 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of SanRemes Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.sanremes-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(sanremesHome) {
  return path.join(sanremesHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.sanremes/sanremes-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(sanremesHome) {
  if (!sanremesHome) {
    return null
  }

  const candidate = path.join(sanremesHome, 'sanremes-agent', 'scripts', installScriptName())
...
function cachedScriptPath(sanremesHome, commit) {
  return path.join(bootstrapCacheDir(sanremesHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/sanremes-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(sanremesHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(sanremesHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, sanremesHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass SANREMES_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          SANREMES_HOME: sanremesHome || process.env.SANREMES_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, sanremesHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, sanremesHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--sanremes-home', sanremesHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, sanremesHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, sanremesHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, sanremesHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(sanremesHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, sanremesHome, emit })
