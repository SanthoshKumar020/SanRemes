 ...
 * the three user-facing uninstall modes to the `sanremes uninstall` CLI flags,
 ...
 *              `sanremes uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `sanremes uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `sanremes uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `sanremes` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m sanremes_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `sanremes_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'sanremes_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/SanRemes.app/Contents/MacOS/SanRemes  → …/SanRemes.app
 *   Windows: …\SanRemes\SanRemes.exe                 → …\SanRemes  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/SanRemes.app/Contents/MacOS/SanRemes → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/SanRemes.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs SanRemes.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]SanRemes$/i.test(dir) || /[\\/]sanremes-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/sanremes
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import sanremes_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, sanremesHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export SANREMES_HOME=${q(sanremesHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import sanremes_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but SanRemes install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "SANREMES_HOME=${String(sanremesHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
