 ...
 *     `X-SanRemes-Session-Token` header; WS uses `?token=`.
...
// sanremes_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - sanremes_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - sanremes_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / sanremes #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-sanremes_session_at', '__Secure-sanremes_session_at', 'sanremes_session_at']
const RT_COOKIE_VARIANTS = ['__Host-sanremes_session_rt', '__Secure-sanremes_session_rt', 'sanremes_session_rt']

// The Nous portal (NAS) does NOT use SanRemes gateway session cookies — it is a
...
// Keep this aligned with sanremes_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['sanremes', 'test', 'tmp', 'root', 'sudo'])
...
/** Which SanRemes profile the remote SSH dashboard should actually run as.
...
// SanRemes Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteSanRemesPath = String(entry.remoteSanRemesPath || '').trim()

  if (remoteSanRemesPath) {
    out.remoteSanRemesPath = remoteSanRemesPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote SanRemes installation. Preserve an explicit mapping
  // when it is a valid SanRemes profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_sanremes_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `SANREMES_HOME` scopes it.
 *
 ...
 * `sanremes:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://sanremes.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `sanremes:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a sanremes session ACCESS-token cookie
 ...
 * the SanRemes gateway session cookies, so cloud sign-in / discovery liveness
