...
test('error page names the failure and carries a Reload button', () => {
  ...
  assert.match(html, /SanRemes couldn.t start the desktop UI/)
  ...
  assert.match(html, /sanremes desktop --force-build/)
...
test('error page reload button targets the real renderer URL when provided', () => {
  const html = buildRendererLoadErrorPage({
    ...
    reloadUrl: 'file:///C:/SanRemes%20Agent/dist/index.html'
  })
  ...
  assert.match(html, /location\.replace\("file:\/\/\/C:\/SanRemes%20Agent\/dist\/index\.html"\)/)
