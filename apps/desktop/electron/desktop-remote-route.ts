...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'SANREMES_DESKTOP_REMOTE_URL is set but SANREMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote SanRemes backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `sanremes serve` the desktop never
