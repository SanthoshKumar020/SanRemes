 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote SanRemes dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the SanRemes install on the remote (login-shell probe),
 ...
 *     `SANREMES_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForSanRemes in.
 *
 * The minted SANREMES_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^SANREMES_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.sanremes/desktop-ssh'
...
// Resolve the remote sanremes executable. An EXPLICIT path is honored strictly
...
async function locateSanRemes(ssh, remoteSanRemesPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The sanremes binary or wrapper script
    ...
    //     the SanRemes version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/NousResearch/sanremes-agent/issues/74411
    return candidate
  }
  ...
  if (remoteSanRemesPath) {
    if (await isExecutable(remoteSanRemesPath)) {
      return resolveLauncher(remoteSanRemesPath)
    }

    const err: any = new Error(
      `The SanRemes path you set is not an executable on the remote host: "${remoteSanRemesPath}". ` +
        'Check the path (it must be the full path to the `sanremes` binary on the remote, e.g. ' +
        '~/sanremes-agent/.venv/bin/sanremes), or clear it to auto-detect.'
    )

    err.kind = 'sanremes-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v sanremes')}`)).trim()
  ...
  candidates.push('~/.local/bin/sanremes')
  candidates.push('/usr/local/bin/sanremes')
  candidates.push('~/.sanremes/sanremes-agent/venv/bin/sanremes')
  ...
  const err: any = new Error(
    'SanRemes is not installed on the remote host (could not find a `sanremes` executable). ' +
      'Install it on the remote with:  curl -fsSL https://sanremes-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the SanRemes path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<sanremes> --version`,
// e.g. "SanRemes Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH sanremes a
// connection uses, so a stale/unexpected install is visible.
async function probeSanRemesVersion(ssh, sanremesPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(sanremesPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". SanRemes Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The SANREMES_HOME the remote dashboard will use (explicit env wins, else
// ~/.sanremes). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteSanRemesHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${SANREMES_HOME:-$HOME/.sanremes}"')).trim().split('\n').pop()

    return out || '~/.sanremes'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote SanRemes home.')
...
marker=home/'.sanremes-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing SanRemes checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, sanremesHome) {
  const home = assertSafeRemoteHome(sanremesHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote SanRemes install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote SanRemes update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote SanRemes update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteSanRemesProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteSanRemesHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote SanRemes profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote SanRemes home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'sanremesPath', 'sanremesHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !sanremesPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(sanremesPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/sanremes`, leaving neither
      // launcher nor SANREMES_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `sanremes_home=os.path.expanduser(${shq(sanremesHome)}) if ${shq(sanremesHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if sanremes_home:\n' +
      ' expected_entries.add(os.path.join(sanremes_home,"sanremes-agent","venv","bin","sanremes"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.sanremesPath)
  const expectedHome = lock.sanremesHome ? expandRemotePath(lock.sanremesHome) : "''"
  ...
  const executableMatch = lock.sanremesHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.sanremesPath === expected.sanremesPath &&
    lock.sanremesHome === expected.sanremesHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.sanremesPath)})
sanremes_home=os.path.expanduser(${py(lock.sanremesHome)})
expected_entries={expected_path,os.path.join(sanremes_home,"sanremes-agent","venv","bin","sanremes")}
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"sanremes-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(sanremesPath, profile, opts: any = {}) {
  const sanremes = expandRemotePath(sanremesPath)
  ...
  const dashCmd =
    ...
    `exec env SANREMES_DESKTOP=1 ${sanremes} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} sanremes-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, sanremesPath) {
  const sanremes = expandRemotePath(sanremesPath)

  const out = await ssh.exec(
    `help="$(${sanremes} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { sanremesPath, profile, token, ownershipId, sanremesHome = '~/.sanremes', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, sanremesPath))) {
    const err: any = new Error(
      'The remote SanRemes install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update SanRemes on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(sanremesPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        sanremesHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForSanRemes, a token-gated
...
async function connect(deps) {
  ...
  const sanremesHome = await probeRemoteSanRemesHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, sanremesHome)
  const sanremesPath = await locateSanRemes(ssh, remoteSanRemesPath)
  log(`located sanremes at ${sanremesPath}`)
  const sanremesVersion = await probeSanRemesVersion(ssh, sanremesPath)

  if (sanremesVersion) {
    log(`remote sanremes version: ${sanremesVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this SanRemes Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.sanremesPath === sanremesPath &&
      lock.sanremesHome === sanremesHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    sanremesHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, sanremesHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForSanRemes(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
