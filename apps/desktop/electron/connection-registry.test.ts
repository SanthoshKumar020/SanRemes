...
test('primary SSH reuse rejects a descriptor with a different remote SanRemes path', async () => {
  const registry = migrateV1ToRegistry({
    mode: 'ssh',
    remote: { mode: 'ssh', host: 'build-host', remoteSanRemesPath: '/srv/sanremes', user: 'alice' },
    profiles: {}
  })
    ...
    await reuseMatchingPrimarySshBackend({
      ...
      ensurePrimary: async () => ({
        ...
        ssh: {
          effectiveConfigFingerprint: 'same-effective-config',
          host: 'build-host',
          remoteSanRemesPath: '/opt/sanremes',
          remoteProfile: '',
          user: 'alice'
        }
      }),
...
test('registry primary reuses a matching primary backend descriptor', () => {
  const registry = normalizeRegistry({
    ...
    primary: 'sanremes-vps',
    launchMode: 'primary',
    lastUsed: 'sanremes-vps',
    connections: [
      { id: LOCAL_CONNECTION_ID, kind: 'local', label: 'This device' },
      { id: 'sanremes-vps', kind: 'ssh', label: 'SanRemes VPS', host: 'sanremes-vps' }
    ]
  })

  const descriptor = {
    connectionId: 'sanremes-vps',
    mode: 'remote' as const,
    remoteKind: 'ssh' as const,
    ssh: { host: 'sanremes-vps' }
  }

  assert.equal(registrySourceOwnsPrimaryBackend(registry, 'sanremes-vps', descriptor), true)
...
test('resolvedConnectionId keeps same-host SSH routes distinct by port, key, path, and profile', () => {
  const base = {
    ...
    keyPath: '/keys/a',
    kind: 'ssh' as const,
    remoteSanRemesPath: '/srv/sanremes',
    remoteProfile: 'alpha',
    user: 'root'
  }
  ...
  const registry: ConnectionRegistry = {
    ...
    connections: [
      ...
      { ...base, id: 'ssh-path', label: 'SSH path', remoteSanRemesPath: '/opt/sanremes' },
  ...
  assert.equal(resolve({ ...base, remoteSanRemesPath: '/opt/sanremes' }), 'ssh-path')
...
// The electron and @sanremes/shared implementations MUST stay byte-identical —
...
test('backendScopeKey: electron and shared implementations agree everywhere', async () => {
  ...
  const shared = (await import(String('@sanremes/shared'))) as {
...
test('roster: source profile metadata follows the connection-qualified row', () => {
  ...
  const vpsMeta = {
    display_name: 'Emma',
    ui_meta: { 'sanremes-bots': { title: 'Emma', shape: 'blobatar::sun', color: '#8b5cf6' } },
    has_avatar: true
  }
...
test('token only persists on token-auth remotes; oauth/cloud drop it', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'C', url: 'https://c.sanremes.cloud', authMode: 'oauth', token: { enc: 'x' } },
    registry
  )
...
test('merge preserves fields the editor does not carry (org, ssh extras)', () => {
  ...
  assert.equal(labelOnly.remoteSanRemesPath, '/opt/sanremes')
...
test('remote input normalizes URL and auth mode; cloud keeps org', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'Cloud', url: 'https://foo.sanremes.cloud', authMode: 'oauth', org: 'nous' },
    registry
  )
  ...
  const input = {
    ...
    connections: [
      ...
      },
      {
        id: 'cloud-1',
        kind: 'cloud',
        label: 'SanRemes Cloud',
        url: 'https://a.sanremes.cloud',
        authMode: 'oauth',
        org: 'nous'
      },
...
test('migrate: v1 cloud keeps cloud provenance + org', () => {
  const registry = migrateV1ToRegistry({
    mode: 'cloud',
    remote: { url: 'https://a.sanremes.cloud', authMode: 'oauth', org: 'nous' }
  })
...
test('Apply remote preserves an existing URL identity and label without duplicates', () => {
  ...
  assert.equal(matches[0].id, 'sanremes-alex')
  assert.equal(matches[0].label, 'Existing gateway')
  assert.equal(matches[0].authMode, 'oauth')
  assert.equal(applied.primary, 'sanremes-alex')
  assert.equal(applied.lastUsed, 'sanremes-alex')
})
