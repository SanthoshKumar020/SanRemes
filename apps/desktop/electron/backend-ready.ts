...
// `sanremes serve` announces SANREMES_BACKEND_READY; the legacy `sanremes dashboard`
// backend announces SANREMES_DASHBOARD_READY. Accept either so the desktop spawn
// works against both the headless backend and old/dashboard runtimes.
const _READY_RE = /^SANREMES_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m

// The announcement clock starts the instant the backend process is spawned —
// before uvicorn binds its socket. On a cold install the child must first
// compile and import the whole `sanremes_cli.main` → `web_server` → FastAPI/
 ...
 * SANREMES_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.SANREMES_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
/**
 * Watch a child process's stdout for the `SANREMES_(BACKEND|DASHBOARD)_READY
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs(), describeOutputTail = () => '') {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`SanRemes backend: exited before port announcement (${signal || code})${describeOutputTail()}`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for SanRemes backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
