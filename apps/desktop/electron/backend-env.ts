...
/**
 * SanRemes-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\sanremes\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$SANREMES_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_sanremes_node_dirs()` in sanremes_constants.py, which the Electron main
 * process cannot import.
 */
function sanremesManagedNodePathEntries(
  sanremesHome,
  ...
  const root = pathModule.join(sanremesHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const sanremesNodeDirs = sanremesManagedNodePathEntries(sanremesHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([sanremesNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeSanRemesHomeRoot(sanremesHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!sanremesHome) {
    return sanremesHome
  }

  const resolved = pathModule.resolve(String(sanremesHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). sanremes_bootstrap sets this inside the child too, but only
