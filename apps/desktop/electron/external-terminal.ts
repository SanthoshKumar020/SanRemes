// Launching the SanRemes TUI in the user's OWN terminal emulator.
//
...
// `sanremes --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's SanRemes runtime is often a venv Python invoked
//     as `python -m sanremes_cli.main`, not a `sanremes` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The SanRemes
...
export function terminalScriptEnv(
  ...
  if (sanremesHome) {
    out.SANREMES_HOME = sanremesHome
  }
