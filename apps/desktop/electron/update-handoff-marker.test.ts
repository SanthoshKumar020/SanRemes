...
function sandbox(tag: string) {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), `sanremes-handoff-marker-${tag}-`))
  const installRoot = path.join(home, 'sanremes-agent')
...
function markerStartedAt(home: string): number {
  const [, startedAt] = fs.readFileSync(path.join(home, '.sanremes-update-in-progress'), 'utf8').split('\n')
...
function runPosix(installRoot: string, startedAt?: string) {
  ...
  if (startedAt === undefined) {
    delete env.SANREMES_UPDATE_STARTED_AT
  } else {
    env.SANREMES_UPDATE_STARTED_AT = startedAt
  }
...
function assertScriptHandoff(run: (installRoot: string, startedAt?: string) => ReturnType<typeof spawnSync>) {
  ...
  fs.writeFileSync(path.join(refreshed.home, '.sanremes-update-in-progress'), '999999\n1\n')
