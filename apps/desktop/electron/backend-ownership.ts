  ...
  return {
    ...
    async reapOrphans(): Promise<number[]> {
      ...
      for (const entry of entries) {
        ...
        // if it reaches reapOrphans (see main.ts startSanRemes + #87295).
...
export function backendCommandMatches(command: unknown): boolean {
  return /(?:^|[\s/\\"])(?:sanremes(?:\.exe)?|sanremes_cli\.main|sanremes_cli[/\\]main\.py)"?(?:\s+(?:--profile|-p)\s+\S+)?\s+(?:serve|dashboard)(?:\s|$)/i.test(
    String(command ?? '')
  )
}
