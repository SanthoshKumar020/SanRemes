...
// registry, and the sanremes:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // SANREMES_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.SANREMES_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip SanRemes' TUI into
    ...
    env.TERM_PROGRAM = 'SanRemes'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a sanremes/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from SANREMES_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.SANREMES_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `sanremes:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('sanremes:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('sanremes:terminal:write', (_event, id, data) => {
  ...
  ipcMain.handle('sanremes:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('sanremes:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('sanremes:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
