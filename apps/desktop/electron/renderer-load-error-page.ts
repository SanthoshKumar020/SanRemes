 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `sanremes update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `sanremes desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'SanRemes couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>sanremes desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>sanremes desktop --force-build</code>, then restart the app.</p>
