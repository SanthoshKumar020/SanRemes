/**
 * windows-sanremes-path.ts
 *
 * Pure, dependency-injected pieces of Windows `sanremes` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvSanRemesCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `sanremes` shim shadowed the real
 *      sanremes.cmd/sanremes.exe; the shim then failed the --version probe and
 ...
 *      and SanRemes launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvSanRemesCommand() — unwrapWindowsVenvSanRemesCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `sanremes`) must
 * not shadow `sanremes.cmd`/`sanremes.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the SanRemes launcher that drives `sanremes update`.
...
export interface ResolveVenvSanRemesCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportSanRemesCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `sanremes`/`sanremes.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/sanremes(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m sanremes_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportSanRemesCli(). A
 * venv whose update died mid-`pip install` still has python.exe + sanremes.exe
 ...
 *
 * Returns null when `command` is not a venv sanremes shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvSanRemesCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvSanRemesCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^sanremes(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv SanRemes at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing SanRemes Python at ${python}`,
    command: python,
