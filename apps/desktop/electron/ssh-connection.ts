...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(SANREMES_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-SanRemes-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.sanremes/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'sanremes-desktop-ssh')
  }

  return path.join(os.homedir(), '.sanremes', 'desktop-ssh')
}
