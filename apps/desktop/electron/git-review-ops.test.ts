...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sanremes-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'sanremes-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'SanRemes Test'], { cwd: dir })
