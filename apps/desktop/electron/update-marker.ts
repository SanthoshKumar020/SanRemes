...
export function markerPath(sanremesHome) {
  return path.join(sanremesHome, '.sanremes-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(sanremesHome)
  let raw
 ...
 *
 * The Tauri-based sanremes-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(sanremesHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(sanremesHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * SanRemes to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(sanremesHome, opts)
