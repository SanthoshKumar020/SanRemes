 ...
 *
 * The desktop main process can pass SANREMES_DASHBOARD_SESSION_TOKEN when it
...
async function fetchPublicText(url, options: any = {}) {
  ...
  if (protocol !== 'http:' && protocol !== 'https:') {
    throw new Error(`Unsupported SanRemes backend URL protocol: ${protocol}`)
  }
  ...
  const res = await fetch(url, { signal: AbortSignal.timeout(timeoutMs) }).catch(error => {
    if (error.name === 'TimeoutError') {
      throw new Error(`Timed out connecting to SanRemes backend after ${timeoutMs}ms`)
    }
...
function extractInjectedDashboardToken(html) {
  const match = /window\.__SANREMES_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")/.exec(String(html || ''))
 ...
 */
async function adoptServedDashboardToken(baseUrl, spawnToken, { childAlive, label = 'SanRemes backend', ...options }) {
