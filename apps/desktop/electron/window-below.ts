// window-below.ts — which OS window sits directly underneath a SanRemes window.
//
...
export function enumerationFailureNote(platform: string, env: NodeJS.ProcessEnv, detail?: string): string {
  ...
  if (env.HYPRLAND_INSTANCE_SIGNATURE) {
    return (
      ...
      'Check that `hyprctl clients` works from the same session SanRemes is ' +
      'running in.'
    )
  }
  ...
  if (wayland) {
    return (
      ...
      'X11/Xorg session, or run SanRemes under XWayland with DISPLAY set.'
    )
  }
 ...
 *
 * Walks past every window owned by our own process (all SanRemes windows share
...
export async function readWindowBelow(
  ...
  if (process.platform === 'darwin' && !titlesAvailable) {
    result.note =
      ...
      'Screen Recording permission, which SanRemes does not request for this.'
  }
