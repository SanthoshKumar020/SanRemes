 ...
 * INITIATED, not completed. A dying `python.exe -m sanremes_cli.main serve`
 * stays in the process table while it unmaps .pyd files (AV / NTFS filter
 * drivers stretch this out), and it need not hold the venv `sanremes.exe` shim
...
export interface ReleaseGateDeps {
  /** Probe the venv sanremes.exe shim (real: O_RDWR open attempt). */
