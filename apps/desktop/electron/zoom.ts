...
export const ZOOM_STORAGE_KEY = 'sanremes:desktop:zoomLevel'
 ...
 * but forgot to emit 'sanremes:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('sanremes:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `sanremes:zoom:changed`, which never fired —
