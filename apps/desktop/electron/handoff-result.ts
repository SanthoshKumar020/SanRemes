 ...
 * console. It writes SANREMES_HOME/.sanremes-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen SanRemes within 30 minutes.
...
export function handoffResultPath(sanremesHome: string): string {
  return path.join(sanremesHome, '.sanremes-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(sanremesHome)
  let raw: string
