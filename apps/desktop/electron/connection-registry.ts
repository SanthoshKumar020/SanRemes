 ...
 * runtime, remote gateways, SanRemes Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@sanremes/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteSanRemesPath || '').trim() !== String(activeSsh.remoteSanRemesPath || '').trim() ||
...
/** Turn `ls ~/.sanremes/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. SanRemes Cloud
 * instances are platform-managed — we never run `sanremes update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteSanRemesPath: input.remoteSanRemesPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/SanRemes-managed names; an empty result stores nothing.
 ...
 * `remoteSanRemesPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'SanRemes Cloud' : 'Remote gateway'),
