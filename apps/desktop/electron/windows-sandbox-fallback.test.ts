...
test('sandbox marker round-trips through the userData file', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sanremes-sandbox-marker-'))
...
test('buildIcaclsGrantArgs targets ALL APPLICATION PACKAGES with inherited RX', () => {
  assert.deepEqual(buildIcaclsGrantArgs('C:\\SanRemes\\win-unpacked'), [
    'C:\\SanRemes\\win-unpacked',
    '/grant',
...
test('grantAllApplicationPackagesAcl is a no-op off Windows and reports exec failures', () => {
  ...
  const ok = grantAllApplicationPackagesAcl('C:\\SanRemes', {
    platform: 'win32',
  ...
  assert.deepEqual(calls[0]?.args, buildIcaclsGrantArgs('C:\\SanRemes'))

  const failed = grantAllApplicationPackagesAcl('C:\\SanRemes', {
    platform: 'win32',
    execFileSync() {
...
test('buildNoSandboxRelaunchArgs appends a single --no-sandbox flag', () => {
  assert.deepEqual(buildNoSandboxRelaunchArgs(['--foo', '--no-sandbox', 'sanremes://x']), [
    '--foo',
    'sanremes://x',
    '--no-sandbox'
  ])
