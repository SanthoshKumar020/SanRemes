...
//   SANREMES_WIN_SSH_HOST   ssh alias/host of the Windows box
//   SANREMES_WIN_SSH_USER   remote user
//   SANREMES_WIN_SSH_SANREMES absolute path to the remote sanremes.exe under test
const liveHost = process.env.SANREMES_WIN_SSH_HOST || ''
const liveUser = process.env.SANREMES_WIN_SSH_USER || ''
const configuredSanRemes = process.env.SANREMES_WIN_SSH_SANREMES || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-SanRemes-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredSanRemes)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredSanRemes.replace('sanremes.exe', 'python.exe')}' -m sanremes_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredSanRemes.replace('sanremes.exe', 'python.exe')
        const terminate = `& '${python}' -m sanremes_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.sanremesPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m sanremes_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m sanremes_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
