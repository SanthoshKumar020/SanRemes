...
const USER_DATA_OVERRIDE = process.env.SANREMES_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.SANREMES_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.SANREMES_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with SANREMES_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[sanremes] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[sanremes] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. SANREMES_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[sanremes] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[sanremes] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via SANREMES_DESKTOP_PASSWORD_STORE, bridged by the
// `sanremes desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[sanremes] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[sanremes] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `sanremes desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose SanRemes sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[sanremes] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[sanremes] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[sanremes] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[sanremes] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[sanremes] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('sanremes:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized SanRemes burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[sanremes] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[sanremes] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[sanremes] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[sanremes] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// SANREMES_HOME — the user-facing root for everything SanRemes-related. Mirrors
// scripts/install.ps1's $SanRemesHome and scripts/install.sh's $SANREMES_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\sanremes (matches install.ps1)
//   macOS / Linux: ~/.sanremes (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.sanremes directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\sanremes yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// SANREMES_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// SANREMES_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.sanremes / %LOCALAPPDATA%\sanremes.
function resolveSanRemesHome() {
  if (process.env.SANREMES_HOME) {
    return normalizeSanRemesHomeRoot(process.env.SANREMES_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'sanremes-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a SANREMES_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\sanremes and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('SANREMES_HOME')

    if (fromRegistry) {
      return normalizeSanRemesHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'sanremes')
    const legacy = path.join(app.getPath('home'), '.sanremes')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.sanremes setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.sanremes')
}

const SANREMES_HOME = resolveSanRemesHome()

function pathWithSanRemesManagedNode(...entries) {
  const managed = sanremesManagedNodePathEntries(SANREMES_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_SANREMES_ROOT — the canonical mutable SanRemes install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_SANREMES_ROOT = path.join(SANREMES_HOME, 'sanremes-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_SANREMES_ROOT, 'venv')
...
// boots, going straight to `resolveSanRemesBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_SANREMES_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_SANREMES_ROOT, '.sanremes-bootstrap-complete')
...
// active-profile.json records which SanRemes profile the desktop launches its
// local backend as. When set, startSanRemes() passes `sanremes --profile <name>
// dashboard …`, which deterministically pins SANREMES_HOME (see
// _apply_profile_override in sanremes_cli/main.py) and bypasses the sticky
// ~/.sanremes/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors sanremes_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// sanremesDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under SANREMES_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by sanremes_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(SANREMES_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (sanremes_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.SANREMES_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.SANREMES_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.SANREMES_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.SANREMES_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.SANREMES_DESKTOP_APP_NAME || 'SanRemes'
...
// 'sanremes:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.sanremes) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.sanremes')
}

// Seed the native About panel with the live SanRemes version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `sanremes update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveSanRemesVersion(),
...
// startSanRemes(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.SANREMES_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.SANREMES_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
//   * WSL2 IPC stall  = the renderer's `sanremes:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.SANREMES_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startSanRemes() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning sanremes serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start SanRemes backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  sanremesLog.push(...lines)

  if (sanremesLog.length > 300) {
    sanremesLog.splice(0, sanremesLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-sanremes-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('sanremes:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the sanremes:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('sanremes:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_SANREMES_ROOT
  })
}
...
// The Tauri updater writes SANREMES_HOME/.sanremes-update-in-progress for the whole
...
// (`force_kill_other_sanremes`, taskkill /IM sanremes.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(SANREMES_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — SanRemes will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(SANREMES_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'SanRemes update did not finish',
        `${result.message}\n\nDetails: ${path.join(SANREMES_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `sanremes` — must not shadow `sanremes.cmd`/`sanremes.exe`.
...
function unwrapWindowsVenvSanRemesCommand(command, backendArgs) {
  return resolveVenvSanRemesCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `sanremes serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `sanremes` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'sanremes_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `sanremes --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, SANREMES_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveSanRemesBackend step 4); execFileSync of a .cmd without
...
function isSanRemesSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'sanremes_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.SANREMES_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several SanRemes deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the sanremes-import
...
// findGitBash — locate bash.exe on Windows. Resolves SANREMES_GIT_BASH_PATH
...
// normal SANREMES_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\sanremes\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'sanremes', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'sanremes', 'git', 'bin', 'git.exe'))
  }
...
function recentSanRemesLog() {
  return sanremesLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's sanremes root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_SANREMES_ROOT.
// SANREMES_DESKTOP_SANREMES_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.SANREMES_DESKTOP_SANREMES_ROOT && path.resolve(process.env.SANREMES_DESKTOP_SANREMES_ROOT),
    !IS_PACKAGED && isSanRemesSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isSanRemesSourceRoot(ACTIVE_SANREMES_ROOT) ? ACTIVE_SANREMES_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_SANREMES_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('sanremes:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'sanremes-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `sanremes update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(SANREMES_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `sanremes update` can write
// fresh entry points. On Windows this is the file the running backend
// `sanremes.exe` holds open; on POSIX it's never mandatory-locked.
function venvSanRemesShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'sanremes.exe')
    : path.join(updateRoot, 'venv', 'bin', 'sanremes')
}
...
// only signals the direct child, so on Windows a backend `sanremes.exe` that
// spawned its own grandchildren (a `sanremes` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `SanRemes backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live SanRemes backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.sanremesBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the SanRemes backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.sanremesBackendIdentity
...
// `sanremesProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `sanremes.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a sanremes
// REPL, a pty terminal, the gateway) keep `sanremes.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const sanremesProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\sanremes.exe, but the
  // backend is `python.exe -m sanremes_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (sanremesProcess && Number.isInteger(sanremesProcess.pid)) {
    initialPids.push(sanremesProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(sanremesProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvSanRemesShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentSanRemesProcess = backendConnectionState.getProcess()

        if (currentSanRemesProcess && Number.isInteger(currentSanRemesProcess.pid)) {
          stragglers.push(currentSanRemesProcess.pid)
        }
...
// `sanremes update`). Instead we spawn the staged SanRemes-Setup binary with
// --update and quit, so it can run `sanremes update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `sanremes update` → relaunch),
      ...
      // in: the SANREMES_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `sanremes update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `sanremes desktop`, never the Tauri installer that self-copies
      // sanremes-setup.exe into SANREMES_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `sanremes` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `sanremes update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `sanremes update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, sanremesRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(SANREMES_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating SanRemes — this window will close and the updater will open. Don’t reopen SanRemes yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(SANREMES_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // sanremes.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running sanremes, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the SanRemes install open ' +
        '(a second SanRemes window or a terminal running sanremes?). Close it and retry.'
    ...
    // SanRemes processes running from this venv.  The updater normally refuses
    ...
    // `sanremes update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `sanremes update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          SANREMES_HOME,
          SANREMES_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithSanRemesManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `sanremes update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(SANREMES_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another SanRemes update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(SANREMES_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. SanRemes will keep running — try again, or run \`sanremes update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvSanRemes = path.join(venvBin, IS_WINDOWS ? 'sanremes.exe' : 'sanremes')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's SanRemes launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the sanremes
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.sanremes-bootstrap-complete')),
      hasVenvSanRemes: fileExists(venvSanRemes),
...
// Python-level pre-update snapshot inside `sanremes update`.
function preflightStateDb(sanremesHome, rememberLog) {
  const stateDbPath = path.join(sanremesHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(sanremesHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(sanremesHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(sanremesHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `sanremes update`, swaps/relaunches the app bundle, and writes
// .sanremes-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'sanremes update', percent: null })

    return { ok: true, manual: true, command: 'sanremes update', sanremesRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      SANREMES_HOME,
      SANREMES_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithSanRemesManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating SanRemes — this window will close. Don’t reopen SanRemes yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_SANREMES_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isSanRemesSourceRoot(ACTIVE_SANREMES_ROOT) &&
    fileExists(venvPython) &&
    canImportSanRemesCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_SANREMES_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `sanremes
...
function resolveWebDist() {
  const override = process.env.SANREMES_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set SANREMES_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: sanremes desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: sanremes desktop --force-build`
  )
...
function resolveSanRemesCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/SanRemes.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.SANREMES_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveSanRemesCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'sanremes_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_SANREMES_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `SanRemes at ${ACTIVE_SANREMES_ROOT}`,
    command,
    args: ['-m', 'sanremes_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      sanremesHome: SANREMES_HOME,
      pythonPathEntries: [ACTIVE_SANREMES_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_SANREMES_ROOT,
    bootstrap: true,
    shell: false
  }
}
...
function resolveSanRemesBackend(backendArgs) {
  ...
  //    installed `sanremes` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isSanRemesSourceRoot.)
  if (!IS_PACKAGED && isSanRemesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `SanRemes source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_SANREMES_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\sanremes\\sanremes-agent (Windows) or ~/.sanremes/sanremes-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active SanRemes runtime at ${ACTIVE_SANREMES_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `sanremes` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    SANREMES_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.SANREMES_DESKTOP_IGNORE_EXISTING !== '1') {
    let sanremesCommand = null
    const sanremesOverride = process.env.SANREMES_DESKTOP_SANREMES

    if (sanremesOverride) {
      const resolvedOverride = findOnPath(sanremesOverride)

      if (resolvedOverride) {
        sanremesCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(sanremesOverride, { isWsl: IS_WSL })) {
        sanremesCommand = sanremesOverride
      } else {
        rememberLog(`Ignoring Windows SanRemes override under WSL: ${sanremesOverride}`)
      }
    } else {
      sanremesCommand = findOnPath('sanremes')
    }

    if (sanremesCommand) {
      if (looksLikeDesktopAppBinary(sanremesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving SanRemes CLI: ${sanremesCommand}`)
        sanremesCommand = null
      }
    }

    if (sanremesCommand) {
      const unwrapped = unwrapWindowsVenvSanRemesCommand(sanremesCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `sanremes` shim
      ...
      const shellForProbe = isCommandScript(sanremesCommand)

      // SANREMES_DESKTOP_SANREMES is an explicit deployment override (used by
      ...
      if (
        ...
        return {
          label: `existing SanRemes CLI at ${sanremesCommand}`,
          command: sanremesCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing SanRemes CLI at ${sanremesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed sanremes_cli module via system Python.
  ...
  if (python) {
    ...
    // having sanremes_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\sanremes\sanremes-agent\venv.
    if (canImportSanRemesCli(python)) {
      return {
        kind: 'python',
        label: `installed sanremes_cli module via ${python}`,
        command: python,
        args: ['-m', 'sanremes_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: sanremes_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveSanRemesBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'SanRemes Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_SANREMES_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveSanRemesBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no SanRemes install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'SanRemes recovery was handed off to SanRemes Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      sanremesHome: SANREMES_HOME,
      logRoot: path.join(SANREMES_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('SanRemes install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `SanRemes bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(SANREMES_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startSanRemes() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // sanremes:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveSanRemesBackend(backend.args))
  }
  ...
  if (!isSanRemesSourceRoot(ACTIVE_SANREMES_ROOT)) {
    throw new Error(
      `SanRemes install at ${ACTIVE_SANREMES_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. SanRemes' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\sanremes\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `sanremes` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for SanRemes on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch SanRemes.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isSanRemesSourceRoot()
    // plus an importable sanremes_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `SanRemes venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `SanRemes at ${ACTIVE_SANREMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'SanRemes runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----sanremes-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported SanRemes backend URL protocol: ${parsed.protocol}`))
        ...
        const req = client.request(
          parsed,
          {
            ...
            headers: {
              ...
              'X-SanRemes-Session-Token': token,
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the SanRemes backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to SanRemes backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-SanRemes-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-SanRemes-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('sanremes:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveSanRemesCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('sanremes:preview-file-changed', payload)
}
...
// waitForSanRemesReady how to read a 401 — rejected session vs gated route.
...
async function waitForSanRemes(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForSanRemesReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('sanremes:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('sanremes:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('sanremes:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('sanremes:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting SanRemes...".
function sendPowerResume() {
  ...
  webContents.send('sanremes:power-resume')
}
...
// 'sanremes:power-battery' push below.
ipcMain.handle('sanremes:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('sanremes:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('sanremes:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('sanremes:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('sanremes:context-menu-spellcheck', {
...
//
// Hosted SanRemes gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``sanremes_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / sanremes #37247).
//     Both are set as HttpOnly cookies (``sanremes_session_at`` ~15 min,
//     ``sanremes_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startSanRemes → the renderer's
// single-shot boot() with no retry) surfaced as the "SanRemes couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "SanRemes
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to SanRemes Cloud agent…' : 'Sign in to SanRemes gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to SanRemes backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in sanremes_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- SanRemes Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:sanremes-remote-oauth`:
...
// convention (sanremes_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every SanRemes surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.SANREMES_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the SanRemes gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for sanremes_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing SanRemes Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a SanRemes Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to SanRemes Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (SanRemes Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to SanRemes Cloud. Open Settings → Gateway, choose SanRemes Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your SanRemes Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your SanRemes Cloud session has expired. Sign in to SanRemes Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the SanRemes Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every sanremes:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteSanRemesPath`/`remoteProfile` —
...
// a valid stored value (pins the root SANREMES_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.SANREMES_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.SANREMES_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted SanRemes Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteSanRemesPath: (ssh || savedSsh)?.remoteSanRemesPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by SANREMES_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the SanRemes Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered SanRemes Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteSanRemesPath: input.sshRemoteSanRemesPath ?? existingBlock.remoteSanRemesPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (sanremes #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote SanRemes gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote SanRemes gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.SANREMES_DESKTOP_REMOTE_TOKEN,
      url: process.env.SANREMES_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      sanremesPath: result.sanremesPath,
      sanremesHome: result.sanremesHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { sanremesPath: result.sanremesPath, sanremesHome: result.sanremesHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteSanRemesPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteSanRemesPath: sshConfig.remoteSanRemesPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForSanRemes: (baseUrl, token) => waitForSanRemes(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        sanremesVersion: result.sanremesVersion || '',
        ...
        // per-profile pools also use bare keys. Only startSanRemes' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.sanremesVersion || 'sanremes (version unknown)'} at ${result.sanremesPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteSanRemesVersion: result.sanremesVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (SANREMES_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.SANREMES_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startSanRemes() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every SanRemes
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteSanRemesPath: input.sshRemoteSanRemesPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteSanRemesPath || '')
          let sanremesPath
          let sanremesVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            sanremesPath = runtime.sanremesPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.sanremesPath])
            sanremesVersion = inspection.version
            supported = inspection.supported
          } else {
            sanremesPath = await remoteLifecycle.locateSanRemes(ssh, sshConfig.remoteSanRemesPath || '')
            sanremesVersion = await remoteLifecycle.probeSanRemesVersion(ssh, sanremesPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, sanremesPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update SanRemes on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteSanRemesPath: sanremesPath,
            remoteSanRemesVersion: sanremesVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startSanRemes())
  ...
  // connect to SanRemes gateway". Mirror the renderer's connect here so the test
...
function resetSanRemesConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const sanremesProcess = backendConnectionState.invalidate()
  stopBackendChild(sanremesProcess)
...
// startSanRemes() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetSanRemesConnection() invalidates it.
  const sanremesProcess = backendConnectionState.getProcess()
  const dying = sanremesProcess && !sanremesProcess.killed ? sanremesProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('sanremes:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('sanremes:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startSanRemes()
  ...
  // spawning, so two children never share one profile's SANREMES_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `SanRemes backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteSanRemesPath: source.remoteSanRemesPath,
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `SanRemes backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. sanremes:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: sanremesLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForSanRemes(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.sanremesPath || !state?.sanremesHome) {
  ...
  return {
    ...
    sanremesPath: state.sanremesPath,
    sanremesHome: state.sanremesHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteSanRemesPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.sanremesPath ? platform : await probeWindowsRemote(ssh, config.remoteSanRemesPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          sanremesPath: runtime.sanremesPath,
          sanremesHome: runtime.sanremesHome,
          pythonPath: runtime.python
        }
      }
    }

    const sanremesPath = await remoteLifecycle.locateSanRemes(ssh, config.remoteSanRemesPath || '')
    const sanremesHome = await remoteLifecycle.probeRemoteSanRemesHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, sanremesPath, sanremesHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { sanremesPath: state.sanremesPath, sanremesHome: state.sanremesHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForSanRemes(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startSanRemes() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForSanRemes(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited SANREMES_HOME env (see _apply_profile_override
  // step 3 in sanremes_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveSanRemesBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const sanremesCwd = resolveSanRemesCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(SANREMES_HOME, 'profiles', key))
  )

  rememberLog(`Starting SanRemes backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: sanremesCwd,
        SANREMES_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        SANREMES_WEB_DIST: webDist,
        ...(readyFile ? { SANREMES_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`SanRemes backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`SanRemes backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `SanRemes backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForSanRemes(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `SanRemes backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `SanRemes backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_sanremes_home() recreates the deleted profile directory.
//
...
async function startSanRemes() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('SanRemes Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startSanRemes() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving SanRemes backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('SanRemes backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote SanRemes backend at ${remote.baseUrl}`, 24)
      await waitForSanRemes(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote SanRemes backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, sanremesLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.sanremes/active_profile file) and
    // resolves SANREMES_HOME the same way `sanremes -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving SanRemes runtime', 28)

        return resolveSanRemesBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting SanRemes backend via ${backend.label}`, 84)
    rememberLog(`Starting SanRemes backend via ${backend.label}`)
    ...
    const sanremesProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin SANREMES_HOME for the child so Python's get_sanremes_home()
          // resolves to the SAME location our resolveSanRemesHome() picked. Without
          // this pin, Python falls back to ~/.sanremes on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\sanremes, which differs from C:\Users\<u>\.sanremes.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets SANREMES_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(sanremesProcess)
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, sanremesProcess)

    if (!processOwner) {
      stopBackendChild(sanremesProcess)
      await waitForBackendExit(sanremesProcess)
      releaseBackendChild(sanremesProcess)
      throw new Error('SanRemes backend start was superseded by a newer connection attempt.')
    }

    sanremesProcess.stdout.on('data', rememberLog)
    sanremesProcess.stderr.on('data', rememberLog)
    ...
    sanremesProcess.once('error', error => {
      releaseBackendChild(sanremesProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale SanRemes backend error: ${error.message}`)
        rejectBackendStart?.(new Error('SanRemes backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`SanRemes backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `SanRemes backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    sanremesProcess.once('exit', (code, signal) => {
      releaseBackendChild(sanremesProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale SanRemes backend exit (${signal || code})`)
      ...
      rememberLog(`SanRemes backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `SanRemes backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `SanRemes backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentSanRemesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for SanRemes backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(sanremesProcess, {
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for SanRemes backend to become ready', 90)
    await Promise.race([waitForSanRemes(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => sanremesProcess.exitCode === null && !sanremesProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local SanRemes backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'SanRemes backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('sanremes:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while SanRemes is
...
// pushes pet state over IPC (sanremes:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via sanremes:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main SanRemes app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the SanRemes icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see sanremes:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep SanRemes in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('sanremes:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so SanRemes can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('sanremes:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('sanremes:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[sanremes] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('sanremes:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `sanremes:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, sanremes:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('sanremes:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('sanremes:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('sanremes:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'sanremes desktop --force-build',
  ...
  startSanRemes().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('sanremes:connection', async (_event, profile) => {
...
ipcMain.handle('sanremes:connection:for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
  const registry = readDesktopConnectionsRegistry()
  const id = String(connectionId || '').trim() || registry.primary
  // Same single-owner claim as 'sanremes:connection', keyed by the composite
...
ipcMain.on('sanremes:connection:active-route', (event, route) => {
...
// "Starting SanRemes…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('sanremes:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetSanRemesConnection({ soft: true }),
...
ipcMain.handle('sanremes:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('sanremes:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('sanremes:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('sanremes:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('sanremes:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('sanremes:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveSanRemesBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'SanRemes is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `sanremes-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, SANREMES_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('sanremes:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('sanremes:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('sanremes:zoom:get', event => {
...
ipcMain.on('sanremes:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('sanremes:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startSanRemes() call restarts the
...
ipcMain.handle('sanremes:bootstrap:repair', async () => {
  // Forceful repair: force the next startSanRemes() through the full installer
  ...
  // "bypass active runtime" path inside startSanRemes() and fall through
  ...
  resetSanRemesConnection()

  return { ok: true }
})
ipcMain.handle('sanremes:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('sanremes:bootstrap:cancel', async () => {
...
ipcMain.handle('sanremes:boot-progress:get', async () => bootProgressState)
ipcMain.handle('sanremes:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('sanremes:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('sanremes:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('sanremes:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('sanremes:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('sanremes:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('sanremes:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('sanremes:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('sanremes:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('sanremes:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('sanremes:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('sanremes:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('sanremes:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('sanremes:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('sanremes:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteSanRemesPath: entry.remoteSanRemesPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startSanRemes()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteSanRemesPath: connection.remoteSanRemesPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteSanRemesProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root SANREMES_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('sanremes:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// sanremes:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('sanremes:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('sanremes:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `sanremes update` to every eligible registered connection at once.
...
ipcMain.handle('sanremes:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/sanremes/update', {}, { timeoutMs: 15_000 })
...
// analogue of the sanremes:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('sanremes:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('sanremes:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startSanRemes() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('sanremes:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- SanRemes Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('sanremes:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('sanremes:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('sanremes:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('sanremes:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of sanremes:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('sanremes:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('sanremes:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new SANREMES_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('sanremes:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('sanremes:requestMicrophoneAccess', async () => {
...
ipcMain.handle('sanremes:window:readBelow', async event => {
...
async function handleSanRemesApiRequest(request) {
  ...
  // backend calls ensure_sanremes_home() which recreates the profile directory,
...
ipcMain.handle('sanremes:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its SANREMES_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleSanRemesApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleSanRemesApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('sanremes:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('sanremes:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('sanremes:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as sanremes://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('sanremes:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('sanremes:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('sanremes:data-url-read-max:get', () => ({
...
ipcMain.handle('sanremes:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('sanremes:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('sanremes:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('sanremes:readFileText', async (_event, filePath) => {
...
// `sanremes:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('sanremes:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('sanremes:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('sanremes:writeClipboard', (_event, text) => {
...
ipcMain.handle('sanremes:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('sanremes:readClipboard', () => clipboard.readText())

ipcMain.handle('sanremes:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('sanremes:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('sanremes:context-menu:edit', (event, command) => {
...
ipcMain.handle('sanremes:context-menu:copy-image', event => {
...
ipcMain.handle('sanremes:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('sanremes:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('sanremes:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('sanremes:saveClipboardImage', async () => {
...
ipcMain.handle('sanremes:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('sanremes:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('sanremes:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('sanremes:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('sanremes:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('sanremes:titlebar-theme', (_event, payload) => {
...
ipcMain.on('sanremes:native-theme', (_event, mode) => {
...
ipcMain.on('sanremes:translucency:support', event => {
  event.returnValue = { glass: GLASS_SUPPORTED, translucency: TRANSLUCENCY_SUPPORTED }
})

ipcMain.on('sanremes:translucency', (_event, payload) => {
...
ipcMain.on('sanremes:keep-awake', (_event, on) => {
...
ipcMain.handle('sanremes:quick-entry:settings:get', async () => {
...
ipcMain.handle('sanremes:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('sanremes:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('sanremes:quick-entry:submit', {
...
ipcMain.on('sanremes:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('sanremes:quick-entry:state', payload)
  }
})

ipcMain.on('sanremes:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('sanremes:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('sanremes:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `sanremes:window:openSession`).
...
ipcMain.handle('sanremes:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('sanremes:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('sanremes:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('sanremes:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveSanRemesCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('sanremes:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveSanRemesCwd()
}))

ipcMain.handle('sanremes:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('sanremes:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('sanremes:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('sanremes:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('sanremes:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('sanremes:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('sanremes:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: sanremesLog.slice(-200) }))
...
ipcMain.on('sanremes:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (sanremes:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('sanremes:updates:check', async () =>
...
ipcMain.handle('sanremes:updates:apply', async (_event, payload) =>
...
ipcMain.handle('sanremes:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('sanremes:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical SanRemes version (the one `release.py` bumps in
// sanremes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real SanRemes version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveSanRemesVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'sanremes_cli', '__init__.py')
...
// Renderer-bundle skew: `sanremes update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live SanRemes version and push it into the native About panel
// just before showing it, so an in-place `sanremes update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveSanRemesVersion()} — app build out of date, update the desktop app`
        : resolveSanRemesVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('sanremes:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveSanRemesVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    sanremesRoot: resolveUpdateRoot(),
...
// the actual removal via `sanremes uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (sanremes_cli/uninstall.py + sanremes_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_SANREMES_ROOT
  ...
  const fallback = () => ({
    sanremes_home: SANREMES_HOME,
    agent_installed: isSanRemesSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'sanremes_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, SANREMES_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no SanRemes agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import sanremes_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_SANREMES_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_SANREMES_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_SANREMES_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `sanremes-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `sanremes-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('sanremes:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('sanremes:uninstall:run', async (_event, payload) => {
...
// sanremes://mcp/install?name=NAME&config=B64 — the vendor "Add to SanRemes"
// button, or sanremes://plugin/install?repo=owner/repo). Dev
// (`SANREMES_DESKTOP_DEV_SERVER`) registers sanremes-dev:// instead — bare
// Electron or a stale OS handler often owns sanremes:// on dev machines.
...
// ---------------------------------------------------------------------------
const SANREMES_PROTOCOL = DEV_SERVER ? 'sanremes-dev' : 'sanremes'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['sanremes-dev', 'sanremes'] : ['sanremes']
...
function handleDeepLink(url) {
  ...
  // sanremes://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('sanremes:deep-link', payload)
...
ipcMain.handle('sanremes:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${SANREMES_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SANREMES_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(SANREMES_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${SANREMES_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `sanremes://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startSanRemes), whose
...
app.whenReady().then(() => {
  ...
  // startSanRemes() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching sanremes:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting SanRemes never keeps another app's chord hostage.
