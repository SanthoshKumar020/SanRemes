...
export interface SanRemesReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForSanRemesReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote SanRemes gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForSanRemesReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed SanRemes Cloud instance
 ...
 * `isReauthRequired` so startSanRemes latches; `needsOauthLogin` alone only
...
export async function waitForSanRemesReady(baseUrl: string, options: SanRemesReadyOptions): Promise<void> {
  ...
  throw new Error(`SanRemes backend did not become ready: ${detail}`)
}
