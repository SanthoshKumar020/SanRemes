...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('sanremes:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('sanremes:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('sanremes:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('sanremes:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('sanremes:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('sanremes:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('sanremes:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('sanremes:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('sanremes:pet-overlay:control', payload)
  })
}
