...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/sanremes', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/sanremes/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/sanremes'), 'https://gw.example.com/sanremes/auth/native/refresh')
})
