...
/**
 * E2E smoke tests for the packaged SanRemes desktop app.
 *
 ...
 * (credential stripping, isolated SANREMES_HOME + userData, unique app name).
 *
...
test('window opens with the SanRemes title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      sanremesDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).sanremesDesktop?.hud?.open({ sessionId: null })
  )
