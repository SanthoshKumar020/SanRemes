 ...
 * packaged binary) with a real `sanremes serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → sanremes serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with SanRemes title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('SanRemes')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { sanremesDesktop?: Record<string, unknown> }).sanremesDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → sanremes serve is spawned → WS connects → config
