...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'SanRemes E2E'], { cwd: repo })
...
function configureRepoCwd(sanremesHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(sanremesHome, mockUrl)
  fs.appendFileSync(path.join(sanremesHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(sanremesHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.sanremesHome, mock.url, repo)
