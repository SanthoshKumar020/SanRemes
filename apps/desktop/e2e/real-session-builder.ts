 ...
 * AIAgent loop, using the E2E mock provider configured in `sanremesHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(sanremesHome: string) {
  ...
  static async start(sanremesHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(sanremesHome)
