/**
 * Shared E2E fixtures for the SanRemes desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → sanremes serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `sanremes desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `sanremes-e2e-${prefix}-${Math.random()}`))
  const sanremesHome = path.join(root, 'sanremes-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(sanremesHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(sanremesHome, 'config.yaml')
 ...
 */
export function writeEnvFile(sanremesHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(sanremesHome, '.env')
 ...
 */
function writeEmptyConfig(sanremesHome: string): void {
  const configPath = path.join(sanremesHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - SANREMES_HOME → sandbox sanremes-home (isolated config/sessions)
 *  - SANREMES_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - SANREMES_DESKTOP_IGNORE_EXISTING=1 → don't pick up `sanremes` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - SANREMES_DESKTOP_SANREMES_ROOT → REPO_ROOT (dev checkout resolution)
 *  - SANREMES_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    SANREMES_HOME: sandbox.sanremesHome,
    SANREMES_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    SANREMES_DESKTOP_IGNORE_EXISTING: '1',
    SANREMES_DESKTOP_SANREMES_ROOT: REPO_ROOT,
    SANREMES_DESKTOP_APP_NAME: `SanRemesE2E-${Date.now()}`,
    ...
    SANREMES_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated SANREMES_HOME + userData
 * @param env      - the process environment (already has SANREMES_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(
    sandbox.sanremesHome,
    mock.url,
    options.extraDisplayConfig,
    options.extraConfig,
    options.modelContextLength,
  )
  writeEnvFile(sandbox.sanremesHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.sanremesHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via SANREMES_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`sanremes serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.sanremesHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { SANREMES_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to SanRemes backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'SanRemes.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'SanRemes.app', 'Contents', 'MacOS', 'SanRemes')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'sanremes')
}
 ...
 * progress without spawning a real SanRemes backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * SANREMES_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    SANREMES_DESKTOP_BOOT_FAKE: '1',
    SANREMES_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + sanremes-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).SANREMES_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).SANREMES_DESKTOP_SANREMES
  delete (env as Record<string, string | undefined>).SANREMES_DESKTOP_SANREMES_ROOT
