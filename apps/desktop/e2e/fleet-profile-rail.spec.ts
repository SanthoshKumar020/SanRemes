 ...
 * second gateway, "Homelab", is a REAL second `sanremes serve` this spec spawns
 * with its own SANREMES_HOME, profiles and session token, registered in the v2
...
function findSanRemesBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'sanremes')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['sanremes'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('sanremes binary not found: create the repo venv (uv sync) or put sanremes on PATH')
}
...
/**
 * Spawn a second, fully real `sanremes serve` as the remote gateway. Its
 * session token is pinned through SANREMES_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        SANREMES_HOME: home,
        SANREMES_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote sanremes serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-SanRemes-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote sanremes serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.sanremesHome, mock.url)
    ...
    seedProfiles(sandbox.sanremesHome, ['research'])
