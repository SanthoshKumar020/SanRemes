...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.sanremesHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.sanremesHome)
  const builder = await RealSessionBuilder.start(sandbox.sanremesHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.sanremesHome, mock.url)
  fs.appendFileSync(path.join(sandbox.sanremesHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.sanremesHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
