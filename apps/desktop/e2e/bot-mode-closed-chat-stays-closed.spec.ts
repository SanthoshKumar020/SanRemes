...
async function seedBot(sanremesHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(sanremesHome, 'profiles', name)
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.sanremesHome, mock.url, 'alpha')
  await seedBot(sandbox.sanremesHome, mock.url, 'beta')
