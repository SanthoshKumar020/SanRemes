...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `SanRemesPlugin` and
...
`$SANREMES_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`sanremes-desktop-plugins` skill.
