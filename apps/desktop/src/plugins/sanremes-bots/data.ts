...
import { atom, host, queryClient, useQuery, useValue } from '@sanremes/plugin-sdk'
...
export const BOT_ATTENTION_HINTS: Record<string, string> = {
  ...
  missing_config: 'Provider not configured — run sanremes model',
 ...
 *  protocol into the system prompt itself (sanremes-agent bot_mode_probe).
...
export function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (sanremes-agent #86875) also expose the union
 ...
 *  callable alias is 'sanremes' — the mention middleware resolves it back to
...
export function botHandle(name: string, bot?: Partial<RosterRow> | null): string {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'sanremes' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`sanremes profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "SanRemes"
 *  can never hijack the primary profile's @sanremes alias. */
export function mentionNameForms(value: null | string | undefined): string[] {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'sanremes'].includes(form)
  )
}
...
export function botFriendlyNames(bot: Partial<RosterRow> | null | undefined): Array<null | string | undefined> {
  ...
  return [bot?.ui_meta?.['sanremes-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update SanRemes Desktop to open another Bot chat.'
    })
...
export function resolveRosterMentions(
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'sanremes') {
      token = byForm.has('sanremes') ? 'sanremes' : token
    }
