...
vi.mock('@sanremes/plugin-sdk', () => ({ host: hostMock, LruCache: UnboundedCache }))

vi.mock('./data', () => ({
  botHandle: (name: string) => (name === 'default' ? 'sanremes' : name),
...
describe('the roster loop pushes the OTHER connections’ agents', () => {
  it('gives each gateway a union roster that excludes its own agents', async () => {
    ...
    expect(syncs[1].params.agents).toEqual([
      expect.objectContaining({ connection_id: 'a', handle: 'sanremes', profile: 'default' })
    ])
