 ...
 * job for a different profile keeps the `sanremes -p <bot> chat` delegation
...
/** Run the delegation command under a `sanremes` stub that prints its argv, so
 *  the assertion is what the SHELL passed — not what the string looks like. */
function argvOf(prompt: string): string[] {
  const command = prompt.slice(prompt.indexOf('sanremes '), prompt.lastIndexOf('\n\nIf the command'))
  const result = spawnSync('sh', ['-c', `sanremes() { printf '%s\\037' "$@"; }\n${command}`], { encoding: 'utf8' })
...
describe('direct vs delegated execution', () => {
  ...
  it('keeps the delegation wrapper for a different active profile', () => {
    ...
    expect(prompt).toMatch(/sanremes -p 'research' chat/)
