...
describe('routing', () => {
  ...
  it('resolves @sanremes to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@sanremes take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @sanremes', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @sanremes,/)
    ...
    expect(peer).toMatch(/group chat with @sanremes/)
  })
