...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "SanRemes". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `sanremes profile rename
  ...
  // never generic "SanRemes" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as SanRemes (the agent it is)
