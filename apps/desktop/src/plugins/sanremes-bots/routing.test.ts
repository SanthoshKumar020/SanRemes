 ...
 * the raw Cloud hostname, or as generic "SanRemes" when Cloud was the only
 * source.
 *
...
vi.mock('@sanremes/plugin-sdk', () => ({ host: hostMock }))
...
describe('alias identity survives the hosted handoff (#89131)', () => {
  ...
  it('renders the sole Cloud-only default as the alias, not "SanRemes"', () => {
  ...
  it('never leaks the alias onto a same-named default on another connection', () => {
    ...
    // Local default while the ACTIVE gateway is local: untouched "SanRemes".
    expect(displayName({ name: 'default' }, null)).toBe('SanRemes')
  })
