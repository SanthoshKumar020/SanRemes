 ...
 * `{type:'sanremes-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
import type * as SanRemesSdk from '@sanremes/plugin-sdk'
import { cleanup, fireEvent, render, screen } from '@testing-library/react'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'

const HUB_ORIGIN = 'https://sanremes-agent.nousresearch.com'
...
vi.mock('@sanremes/plugin-sdk', async importOriginal => {
  const original = await importOriginal<typeof SanRemesSdk>()
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    postPick(
      { identifier: 'nous/web-research', name: 'Web Research', type: 'sanremes-skill-pick' },
      {
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'sanremes-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'nous/web-research', type: 'sanremes-skill-pick' }, { source: frame.contentWindow })
