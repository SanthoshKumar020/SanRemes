...
import { atom, host } from '@sanremes/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'sanremes-bots-groups'
...
/** The v3 envelope stored under the default profile's `sanremes-bots-groups`
 ...
 *  clients (SanRemes Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "SanRemes"
 *  (community report, Aug 21 2026: renamed default still read "SanRemes is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as SanRemes (matching displayName and the
 *  @sanremes handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`sanremes profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'SanRemes' : trimmed
}
