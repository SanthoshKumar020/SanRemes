...
describe('cachedUnionRoster', () => {
  it('reads the entry useRoster wrote under the connection-suffixed key', async () => {
    ...
    seed(['sanremes-bots', 'roster', 'local'], { profiles: [{ name: 'default' }] })
    ...
    expect(cache.has(JSON.stringify(['sanremes-bots', 'roster']))).toBe(false)
  })

  it('falls back to another connection’s entry when the window has moved', async () => {
    const { cachedUnionRoster } = await import('./data')

    seed(['sanremes-bots', 'roster', 'vera'], { profiles: [{ connectionId: 'vera', name: 'default' }] })
    connection.id = 'local'
  ...
  it('prefers the freshest snapshot among several cached connections', async () => {
    ...
    seed(['sanremes-bots', 'roster', 'old'], { fetchedAt: 1_000, profiles: [{ name: 'stale' }] })
    seed(['sanremes-bots', 'roster', 'new'], { fetchedAt: 9_000, profiles: [{ name: 'fresh' }] })
