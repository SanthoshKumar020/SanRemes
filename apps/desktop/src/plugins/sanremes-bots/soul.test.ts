...
vi.mock('./data', () => ({
  botHandle: (name: string) => (String(name).toLowerCase() === 'default' ? 'sanremes' : name),
...
describe('appending the protocol to an existing SOUL', () => {
  ...
  it('points at the real CLI verb, not the plural that does not exist', async () => {
    ...
    expect(soul).toMatch(/run `sanremes profile list` for the LIVE/)
    expect(soul).not.toMatch(/sanremes profiles list/)
  })
