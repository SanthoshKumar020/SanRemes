...
import { host, LruCache } from '@sanremes/plugin-sdk'
...
// to (local, remote URL, SSH, SanRemes Cloud, docker) must be able to find
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['sanremes-bots']?.title || profile?.display_name || ''),
