...
vi.mock('@sanremes/plugin-sdk', async importOriginal => {
  const sdk = await importOriginal<typeof SanRemesSdk>()
...
describe('hiding persists locally and cross-machine', () => {
  it('writes hidden:true to storage and ships it in the server ui_meta', async () => {
    ...
    expect(configure?.[1].ui_meta['sanremes-bots'].hidden).toBe(true)
  })
  ...
  it('clears the flag on unhide, and the server false beats a stale local true', async () => {
    ...
    expect(configure?.[1].ui_meta['sanremes-bots'].hidden).toBe(false)
    ...
    profileOps.mergeServerMeta([
      { name: 'ghost', ui_meta: { 'sanremes-bots': { hidden: false, title: 'Ghost' } } }
  ...
  it('lands a hide done on another machine and persists it to storage', async () => {
    ...
    profileOps.mergeServerMeta([
      { name: 'ghost', ui_meta: { 'sanremes-bots': { hidden: true, title: 'Ghost' } } }
    ] as unknown as RosterRow[])
