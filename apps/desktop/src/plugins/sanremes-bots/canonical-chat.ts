...
import * as sdk from '@sanremes/plugin-sdk'
import { host } from '@sanremes/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['sanremes-bots'].chat) were
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This SanRemes Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (sanremes-agent#89617) — clicking Retry
  ...
  // until an app restart (sanremes-agent#93604). A resume is cheap and
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update SanRemes Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['sanremes-bots']
