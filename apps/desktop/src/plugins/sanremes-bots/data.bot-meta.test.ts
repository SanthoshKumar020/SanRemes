...
vi.mock('./shared', () => ({ getPluginCtx: () => ({ storage: storageMock }), ID: 'sanremes-bots' }))
...
describe('avatar asset sync fires only on a real change', () => {
  ...
  it('never ships the image or the pet icon inside ui_meta', async () => {
    ...
    const uiMeta = (configure?.params.ui_meta as Record<string, Record<string, unknown>>)['sanremes-bots']
...
describe('server metadata reconciliation', () => {
  const serverRow = (name: string, meta: Record<string, unknown>) =>
    ({ name, ui_meta: { 'sanremes-bots': meta } }) as unknown as RosterRow
