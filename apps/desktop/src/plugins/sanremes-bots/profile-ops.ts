...
import { forgetSessionUnread, host, queryClient } from '@sanremes/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['sanremes-bots']
...
/** Permanently delete a bot's SanRemes profile, then remove plugin-local state
 ...
 * directory (sanremes-agent#52279). That is the "can't delete a bot" error. */
