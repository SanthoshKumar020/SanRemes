...
vi.mock('@sanremes/plugin-sdk', async () => {
...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['sanremes-bots-groups']
}
...
describe('speaker labels', () => {
  it('the default profile speaks as SanRemes in transcripts, not @default', async () => {
    ...
    expect(line).toBe('SanRemes: hello room')
    ...
    ).toBe('SanRemes (you): hi')
    expect(
      rounds.formatGroupChatLine({ from: { kind: 'member', name: 'builder' }, text: 'yo' } as GroupMessage, 'research')
    ).toBe('builder: yo')
  })

  it('honor friendly identity: Bot Mode title, then display_name, never a stale SanRemes', async () => {
    const { chat, rounds } = await loadRoom()
    const data = await import('./data')

    // A renamed default (core display_name via `sanremes profile rename`) must
    // read as its new name — the community report was "Lucy" still showing
    // "SanRemes is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → SanRemes, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('SanRemes')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['sanremes-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['sanremes-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['sanremes-bots-groups']).toBe(2)
  ...
  it('fans a room write out to every reachable default-profile gateway', async () => {
    ...
    host.requestProfile = async (route: { connectionId: string }, method: string) => {
      ...
      if (method === 'profiles.configure') {
        return { applied: { ui_meta: true, ui_meta_revisions: { 'sanremes-bots-groups': 1 } } }
      }
