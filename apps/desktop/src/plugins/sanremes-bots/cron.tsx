/**
 * Routines: the SanRemes cron jobs scoped to the bot you're chatting with — the
...
// sanremes -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `sanremes -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// SanRemes-native schedule string; Advanced exposes it raw.
...
/** Compose the SanRemes schedule string from picker state. */
