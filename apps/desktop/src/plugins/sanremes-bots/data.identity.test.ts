...
vi.mock('./shared', () => ({ getPluginCtx: () => null, ID: 'sanremes-bots' }))
...
describe('the @handle a bot answers to', () => {
  it('presents the primary profile as @sanremes — "default" never surfaces in the UI', () => {
    expect(botHandle('default')).toBe('sanremes')
...
describe('renamed bots stay taggable', () => {
  // Discord report, Aug 2026: renaming a bot — Bot Mode title or `sanremes
  ...
  it('drops reserved tokens so a rename cannot hijack a built-in tag', () => {
    expect(mentionNameForms('SanRemes')).toEqual([])
  ...
  it('inserts the renamed slug, falling back to the profile handle', () => {
    ...
    expect(botMentionTag(row({ name: 'default' }))).toBe('sanremes')
    // display_name (`sanremes profile rename`) drives the tag too.
...
describe('resolving @mentions against the roster', () => {
  ...
  it('never treats @sanremes in your own chat as a handoff to yourself', () => {
    expect(resolveRosterMentions('@sanremes do it', roster, { connectionId: 'local', name: 'default' })).toEqual([])
    // From ANOTHER bot's chat the same tag is a real handoff.
    expect(
      resolveRosterMentions('@sanremes do it', roster, { connectionId: 'mac-mini', name: 'dixie' }).map(bot => bot.name)
    ).toEqual(['default'])
  })
...
describe('roster search narrows without re-ranking', () => {
  ...
  it('matches profile handles and preserves roster order', () => {
    ...
    expect(filterBots(roster, meta, '@sanremes').map(bot => bot.name)).toEqual(['default'])
...
describe('needs-attention badge (#93091 item 3)', () => {
  ...
  it('classifies the raw gateway error strings current backends emit', () => {
    ...
    expect(reasonFor('No LLM provider configured. Run sanremes model to pick one.')).toBe('missing_config')
