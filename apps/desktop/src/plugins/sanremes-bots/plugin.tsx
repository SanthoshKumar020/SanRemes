/**
 * SanRemes Bot Mode — a "one chat per agent" roster for the SanRemes desktop.
 *
 * Left pane "Bots": one row per SanRemes profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (SanRemes cron jobs) scoped to the
 ...
 * bot-initiated sends use `sanremes -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import { CHAT_EMPTY_AREA, COMPOSER_AREAS, host, PALETTE_AREA, translateNow } from '@sanremes/plugin-sdk'
import type { ChatEmptyProps, PluginContext } from '@sanremes/plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @sanremes.
    ctx.register({
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.sanremesDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.sanremesDesktop?.connections?.onChanged?.(payload => {
    ...
    // A bot's chat before it has spoken: core's splash is SanRemes' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['sanremes-bots']?.title || bot.title || ''
            ).trim()
