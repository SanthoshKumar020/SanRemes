...
import { Button, host, Input } from '@sanremes/plugin-sdk'
import { useEffect, useRef, useState } from 'react'

import { useBots } from './i18n'

// ── skills hub section: the REAL hub page (docs) embedded as a picker ──────
// https://sanremes-agent.nousresearch.com/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'sanremes-skill-pick', ...} to us (sanremes-agent#86243). We validate
...
export function HubSkillsSection({ forProfile, onInstalled }: HubSkillsSectionProps) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.type !== 'sanremes-skill-pick' || !data.name) {
        return
      }
