...
import { Button, host, Input, useI18n } from '@sanremes/plugin-sdk'
...
// and the row falls back to the "run sanremes mcp / Settings" hint. profile is
...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.sanremesDesktop && window.sanremesDesktop.openExternal) {
        window.sanremesDesktop.openExternal(authUrl)
      } else {
