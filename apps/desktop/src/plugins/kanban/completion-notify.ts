 ...
 *    desktop shell fires only while the user is AWAY from SanRemes. This is the
...
import { host, type PluginOs, type PluginRestOptions, type PluginTranslate } from '@sanremes/plugin-sdk'
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from SanRemes (the toast above covers the foreground case). Isolated:
