...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'sanremes:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.sanremesDesktop?.saveImageFromUrl) {
        if (await window.sanremesDesktop.saveImageFromUrl(src)) {
