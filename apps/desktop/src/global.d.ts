import type { GatewayWsUrlResult } from '@sanremes/shared'
import type { TranslucencyState } from '@sanremes/shared/translucency'
...
declare global {
  interface Window {
    sanremesDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<SanRemesConnection>
      ...
      // Resume this session in the user's own terminal emulator (`sanremes --tui
      ...
      // SanRemes Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new SANREMES_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: SanRemesApiRequest) => Promise<T>
      notify: (payload: SanRemesNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<SanRemesReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<SanRemesReadFileTextResult>
      selectPaths: (options?: SanRemesSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<SanRemesPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<SanRemesPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<SanRemesPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: SanRemesActiveWork) => void
      setTitleBarTheme?: (payload: SanRemesTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<SanRemesReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<SANREMES_HOME>/desktop-plugins),
      ...
      /** LOCAL `<SANREMES_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<SANREMES_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<SanRemesGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<SanRemesGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<SanRemesGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<SanRemesRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: SanRemesReviewScope, baseRef?: null | string) => Promise<SanRemesReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: SanRemesReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<SanRemesReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<SanRemesRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<SanRemesPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: SanRemesTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<SanRemesTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: SanRemesWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: SanRemesPreviewFileChanged) => void) => () => void
...
export interface SanRemesTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface SanRemesTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `sanremes update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  sanremesRoot?: string
...
export interface SanRemesConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface SanRemesTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface SanRemesActiveWork {
  count: number
  titles: string[]
}

export interface SanRemesWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a SanRemes Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted SanRemes Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected SanRemes Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the sanremes gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered SanRemes Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface SanRemesApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface SanRemesNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface SanRemesPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface SanRemesReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface SanRemesPreviewWatch {
  id: string
  path: string
}
...
export interface SanRemesGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface SanRemesGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface SanRemesGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface SanRemesRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface SanRemesRepoStatus {
  branch: null | string
...
// turn began.
export type SanRemesReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface SanRemesReviewFile {
  path: string
  added: number
  removed: number
...
export interface SanRemesReviewList {
  files: SanRemesReviewFile[]
...
export interface SanRemesReviewPr {
  url: string
  state: string
  number: number
}
...
export interface SanRemesBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface SanRemesRepoPullRequests {
  ghReady: boolean
  prs: SanRemesBranchPullRequest[]
}
...
export interface SanRemesPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface SanRemesReviewShipInfo {
  ghReady: boolean
  pr: SanRemesReviewPr | null
}

export interface SanRemesReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface SanRemesReadDirResult {
  entries: SanRemesReadDirEntry[]
  error?: string
}

export interface SanRemesPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface SanRemesSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
