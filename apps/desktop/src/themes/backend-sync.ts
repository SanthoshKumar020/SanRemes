/**
 * Live skin sync from the SanRemes backend.
 *
 * The backend resolves the active skin (built-in or `$SANREMES_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (SanRemes
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { SanRemesSkin } from '@sanremes/shared/skin'
 ...
 */
export function ingestBackendSkin(skin: SanRemesSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as SanRemesSkin)
