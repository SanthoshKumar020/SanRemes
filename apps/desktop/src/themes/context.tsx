...
const SKIN_KEY = 'sanremes-desktop-theme-v2'
const MODE_KEY = 'sanremes-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'sanremes-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'sanremes-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'sanremes-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.sanremesTheme = skinName
  root.dataset.sanremesMode = rendered
  ...
  window.sanremesDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('sanremes-boot-background', chromeBg)
    window.localStorage.setItem('sanremes-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.sanremesThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.sanremesDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (SanRemes authoring/activating a skin from a
