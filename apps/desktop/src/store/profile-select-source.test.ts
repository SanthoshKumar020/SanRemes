...
describe('selectProfile startup preference (#79886)', () => {
  ...
  it('does not replace the local startup preference for a profile SSH override', async () => {
    ...
    selectProfile('macmini-sanremes')

    await vi.waitFor(() => expect(ensureGatewayForProfile).toHaveBeenCalledWith('macmini-sanremes'))
    await vi.waitFor(() => expect(getConnection).toHaveBeenCalledWith('macmini-sanremes'))
