...
import { DEFAULT_GLASS_MATERIAL, DEFAULT_GLASS_SCOPE } from '@sanremes/shared/translucency'
...
const KEY = 'sanremes.desktop.translucency.v2'
const LEGACY_KEY = 'sanremes.desktop.translucency.v1'
...
const glassAttr = () => document.documentElement.hasAttribute('data-sanremes-glass')
const clearAttr = () => document.documentElement.hasAttribute('data-sanremes-clear')
...
describe('window translucency lever', () => {
  ...
  it('mirrors every tick to the desktop bridge so the clear-mode fade tracks the drag', () => {
    vi.useFakeTimers()
    const calls: unknown[] = []
    window.sanremesDesktop = { setTranslucency: (payload: unknown) => calls.push(payload) } as never
...
describe('frost and area', () => {
  ...
  it('publishes the area as an attribute only while glass is on', () => {
    ...
    if (!GLASS_SUPPORTED) {
      expect(document.documentElement.hasAttribute('data-sanremes-glass-scope')).toBe(false)

      return
    }

    setTranslucencyMode('glass')
    expect(document.documentElement.getAttribute('data-sanremes-glass-scope')).toBe('sidebar')

    setTranslucencyScope('window')
    expect(document.documentElement.getAttribute('data-sanremes-glass-scope')).toBe('window')
    ...
    setTranslucency(50)
    expect(document.documentElement.hasAttribute('data-sanremes-glass-scope')).toBe(true)
...
describe('translucency peek', () => {
  it('stays open until every overlapping hold has ended', () => {
    beginTranslucencyPeek()
    beginTranslucencyPeek()
    expect(document.documentElement.hasAttribute('data-sanremes-translucency-peek')).toBe(true)

    endTranslucencyPeek()
    expect(document.documentElement.hasAttribute('data-sanremes-translucency-peek')).toBe(true)

    endTranslucencyPeek()
    expect(document.documentElement.hasAttribute('data-sanremes-translucency-peek')).toBe(false)
  })
...
describe('cross-window sync', () => {
  ...
  it('ignores storage events for other keys', () => {
    ...
    window.dispatchEvent(new StorageEvent('storage', { key: 'sanremes.desktop.zoom.v1', newValue: 'x' }))
...
describe('glass is confined to chat windows', () => {
  ...
  it('does not thin surfaces in a special-purpose window', () => {
    ...
    expect($translucency.get().mode).toBe('glass')
    expect(document.documentElement.hasAttribute('data-sanremes-glass')).toBe(false)
  })
  ...
  // `data-sanremes-glass` would silently never match.
  it('still publishes the live setting and the tint to a special-purpose window', () => {
    setSearch('?win=hud')
    setTranslucency(60)
    setTranslucencyMode('glass')

    expect(document.documentElement.hasAttribute('data-sanremes-glass-on')).toBe(true)
  ...
  it('withdraws both flags when glass is switched off', () => {
    ...
    expect(document.documentElement.hasAttribute('data-sanremes-glass-on')).toBe(false)
  ...
  it('thins them in the primary window', () => {
    ...
    expect(document.documentElement.hasAttribute('data-sanremes-glass')).toBe(true)
  })
})
