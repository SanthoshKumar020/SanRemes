import { LOCAL_CONNECTION_ID } from '@sanremes/shared'
...
import type { ProfileInfo } from '@/types/sanremes'
...
// /api/profiles/active `current`). "default" is the root ~/.sanremes. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'sanremes.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'sanremes.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await sanremesApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new SANREMES_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.sanremesDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<SanRemesConnection | null> {
  const getConnection = window.sanremesDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<SanRemesConnection | null> {
  const getConnectionFor = window.sanremesDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'sanremes.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.sanremesDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.sanremesDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.sanremes) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.sanremesDesktop?.touchBackend?.(target).catch(() => undefined)
}
