...
export function canUseQuickEntry(): boolean {
  return typeof window !== 'undefined' && typeof window.sanremesDesktop?.quickEntry?.getSettings === 'function'
}
...
export async function loadQuickEntrySettings(): Promise<void> {
  ...
  try {
    applyStatus(await window.sanremesDesktop.quickEntry.getSettings())
  } catch {
...
export async function saveQuickEntrySettings(patch: { enabled?: boolean; shortcut?: string }): Promise<void> {
  ...
  try {
    applyStatus(await window.sanremesDesktop.quickEntry.setSettings(patch))
  } catch {
...
export function initQuickEntryBridge(): () => void {
  const api = typeof window === 'undefined' ? undefined : window.sanremesDesktop?.quickEntry
