 ...
 * (`sanremes:data-url-read-max:*`). The default and the clamp are shared with
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.sanremesDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.sanremesDesktop?.dataUrlReadMax) {
