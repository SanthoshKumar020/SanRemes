...
const agentConn = (over: Partial<SanRemesConnection> = {}): SanRemesConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as SanRemesConnection
...
const getConnection = vi.fn<(profile?: string | null) => Promise<SanRemesConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<SanRemesConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { sanremesDesktop: { getConnection, getConnectionFor } })
})
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<SanRemesConnection>(() => undefined))
