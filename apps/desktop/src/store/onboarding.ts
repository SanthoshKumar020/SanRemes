...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/sanremes'
...
const CONFIGURED_CACHE_KEY = 'sanremes-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'sanremes-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'SanRemes is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nous Tool Gateway,
// mirroring sanremes_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
async function fetchProviderDefaultModel(
  ...
  // `sanremes model` does (for Nous it honors the user's free/paid tier, so a
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'SanRemes could not save the selected model.')
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but SanRemes still cannot resolve a usable provider. ${detail}`
    : 'Connected, but SanRemes still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'SanRemes Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
async function openSignInUrl(url: string) {
  if (window.sanremesDesktop?.openExternal) {
    try {
      await window.sanremesDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `SanRemes still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but SanRemes still cannot reach ${url}.` }
    }
