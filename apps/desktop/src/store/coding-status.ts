...
import type { SanRemesGitWorktree, SanRemesRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: SanRemesGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, SanRemesRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, SanRemesGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<SanRemesRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<SanRemesGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<SanRemesRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<SanRemesGitWorktree[]>>()
const $noRepoStatus = atom<SanRemesRepoStatus | null>(null)
const $noWorktrees = atom<SanRemesGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<SanRemesRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<SanRemesGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: SanRemesRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: SanRemesGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<SanRemesRepoStatus | null>
  seq: number
  target: string
}
