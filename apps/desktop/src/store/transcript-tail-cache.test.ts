...
describe('transcript tail cache', () => {
  ...
  it('self-evicts a corrupt entry instead of returning garbage', () => {
    window.localStorage.setItem('sanremes.transcript-tail.v2:sess-bad', '{not json')

    expect(loadTranscriptTail('sess-bad')).toBeNull()
    expect(window.localStorage.getItem('sanremes.transcript-tail.v2:sess-bad')).toBeNull()
  })
  ...
  it('repairs a poisoned persisted tail carrying a duplicate toolCallId (#87857)', () => {
    ...
    window.localStorage.setItem('sanremes.transcript-tail.v2:sess-poisoned', JSON.stringify(poisoned))
  ...
  describe('profile scoping (#94828)', () => {
    ...
    it('purges pre-scoping v1 entries so a stale tail can never paint again', async () => {
      ...
      window.localStorage.setItem('sanremes.transcript-tail.v1:sess-legacy', JSON.stringify(stale))
      window.localStorage.setItem('sanremes.transcript-tail.v1-index', JSON.stringify(['sess-legacy']))
      ...
      try {
        ...
        expect(window.localStorage.getItem('sanremes.transcript-tail.v1:sess-legacy')).toBeNull()
        expect(window.localStorage.getItem('sanremes.transcript-tail.v1-index')).toBeNull()
      } finally {
        vi.resetModules()
      }
    })
