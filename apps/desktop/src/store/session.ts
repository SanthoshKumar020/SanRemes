import type { ConnectionState } from '@sanremes/shared'
...
import type { SessionInfo, UsageStats } from '@/types/sanremes'
...
const COMPOSER_MODEL_KEY = 'sanremes.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'sanremes.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'sanremes.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'sanremes.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'sanremes.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'sanremes.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'sanremes.desktop.lastRoute'
...
function workspaceCwdKey(connection: SanRemesConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.sanremesDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.sanremesDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<SanRemesConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'sanremes.desktop.sessionOwnerHints.v1'
...
export const setConnection = (next: Updater<SanRemesConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useSanRemesConfig.
