...
import type { SanRemesBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/sanremes'
...
export const $pullRequestsByBranch = atom<Record<string, SanRemesBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'sanremes.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('sanremes.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: SanRemesBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
