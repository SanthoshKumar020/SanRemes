...
import type { SanRemesActiveWork } from '@/global'
import { $sessions } from '@/store/session'
import { $workingSessionIds } from '@/store/session-states'

const $activeWork = computed([$workingSessionIds, $sessions], (workingIds, sessions): SanRemesActiveWork => {
...
if (typeof window !== 'undefined') {
  ...
  $activeWork.subscribe(work => {
    ...
    lastSent = next
    window.sanremesDesktop?.setActiveWork?.(work)
  })
}
