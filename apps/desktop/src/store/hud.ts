 ...
 * composer with the reply scrolling above it, so SanRemes can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.sanremesDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.sanremesDesktop?.hud
...
export function resetHudLayout(): void {
  void window.sanremesDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.sanremesDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.sanremesDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
