...
import type { SanRemesGitBaseBranch, SanRemesGitBranch } from '@/global'
import { getSanRemesConfig, sanremesApi, type SanRemesGateway } from '@/sanremes'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/sanremes'
...
const PROJECT_SCOPE_KEY = 'sanremes.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('SanRemes gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active SanRemes profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await sanremesApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<SanRemesGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<SanRemesGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero SanRemes
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getSanRemesConfig(context.profile))
...
// repo, returning where SanRemes should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<SanRemesGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<SanRemesGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.sanremesDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.sanremesDesktop?.writeClipboard?.(path)
  }
}
