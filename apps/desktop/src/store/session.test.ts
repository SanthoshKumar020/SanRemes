...
describe('session owner hints', () => {
  ...
  it('ignores malformed persisted entries and never throws on hydrate', () => {
    window.localStorage.setItem(
      'sanremes.desktop.sessionOwnerHints.v1',
      JSON.stringify([
        'junk',
        ['no-route', null],
...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('sanremes.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('sanremes.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
  ...
  it('does not publish a delayed configured default after ownership is lost', async () => {
    ...
    ;(window as { sanremesDesktop?: unknown }).sanremesDesktop = {
  ...
  it('prefers the configured default over the sticky remembered workspace', () => {
    window.localStorage.setItem('sanremes.desktop.workspace-cwd', '/home/user/sticky')
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('sanremes.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')
...
describe('remembered session id (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    ...
    localStorage.setItem('sanremes.desktop.lastSessionId', 'legacy-session')
    ...
    expect(localStorage.getItem('sanremes.desktop.lastSessionId')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('sanremes.desktop.lastSessionId.profile.research%2Fops')).toBe('ops-session')
...
describe('remembered route (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    localStorage.setItem('sanremes.desktop.lastRoute', '/skills')
    ...
    expect(localStorage.getItem('sanremes.desktop.lastRoute')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('sanremes.desktop.lastRoute.profile.research%2Fops')).toBe('/cron')
    expect(getRememberedRoute('research')).toBeNull()
  })
