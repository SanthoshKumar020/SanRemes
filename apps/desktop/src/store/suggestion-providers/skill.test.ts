...
describe('collidesWithWorkspace', () => {
  it('suppresses a skill named exactly like the cwd folder', () => {
    expect(collidesWithWorkspace('sanremes-agent', '/Users/b/www/sanremes-agent')).toBe(true)
  })

  it('suppresses inside worktree-suffixed folders too', () => {
    expect(collidesWithWorkspace('sanremes-agent', '/Users/b/www/sanremes-agent-suggest')).toBe(true)
  })
  ...
  it('does not suppress on substring-only overlap', () => {
    ...
    expect(collidesWithWorkspace('clean', '/Users/b/www/sanremes-agent')).toBe(false)
  })

  it('never collides when detached (empty cwd)', () => {
    expect(collidesWithWorkspace('sanremes-agent', '')).toBe(false)
  })
})
...
describe('skillTouchedInMessages', () => {
  ...
  it('matches qualified skill names (category/name, plugin:name)', () => {
    expect(
      skillTouchedInMessages('sanremes-agent-dev', [toolCall('skill_view', { name: 'github/sanremes-agent-dev' })])
    ).toBe(true)
    expect(
