...
import { getSkills } from '@/sanremes'
 ...
 *  Working in `~/www/sanremes-agent`, the draft says "sanremes-agent" constantly
 *  — a "Use skill: sanremes-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`sanremes-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/sanremes-agent-dev`, `plugin:skill`). */
