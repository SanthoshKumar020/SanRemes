...
const sanremes = await import('@/sanremes')
const getSanRemesConfig = vi.mocked(sanremes.getSanRemesConfig)
...
describe('project scope', () => {
  ...
  it('persists the scope to localStorage', () => {
    enterProject('p_abc')
    expect(window.localStorage.getItem('sanremes.desktop.projectScope')).toBe('p_abc')
  })
})
  ...
  it('names the stale-backend remedy when a remote gateway lacks the worktree route', async () => {
    ...
    desktopGit.mockReturnValue({
      worktreeAdd: vi.fn(async () => {
        throw new Error(
          'Expected JSON from https://vps/api/git/worktree/add but got HTML (status 404). The endpoint is likely missing on the SanRemes backend.'
        )
      })
    } as never)
...
describe('repository discovery policy', () => {
  ...
  it('records disabled policy without invoking the filesystem scanner', async () => {
    ...
    getSanRemesConfig.mockResolvedValue({
      desktop: {
  ...
  it('passes custom roots and exclusions to Electron and records on the origin gateway', async () => {
    ...
    expect(getSanRemesConfig).toHaveBeenCalledWith('default')
  ...
  it('does not scan the local filesystem for remote connections but still refreshes the project tree', async () => {
    ...
    expect(scanRepos).not.toHaveBeenCalled()
    expect(getSanRemesConfig).not.toHaveBeenCalled()
    ...
    // `scan: true`) — repos with zero SanRemes sessions must still surface —
