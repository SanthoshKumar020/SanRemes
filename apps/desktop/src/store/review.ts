...
import type { SanRemesReviewFile, SanRemesReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — SanRemes' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'sanremes.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'sanremes.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'sanremes.desktop.reviewTreeMode'
const SELECTED_KEY = 'sanremes.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<SanRemesReviewFile[]>([])
...
export const $reviewShipInfo = atom<SanRemesReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['sanremesDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: SanRemesReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly SanRemesReviewFile[], path: string): SanRemesReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.sanremesDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.sanremesDesktop?.openExternal?.(url)
    }
