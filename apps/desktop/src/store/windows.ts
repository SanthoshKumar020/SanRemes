...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.sanremesDesktop?.openSessionWindow === 'function'
}

// True when the shell can open a full peer app window (⌘⇧N / "New Window").
export function canOpenNewWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.sanremesDesktop?.openWindow === 'function'
}

// True when the shell can pop the in-app Browser into its own OS window.
export function canOpenBrowserWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.sanremesDesktop?.openBrowserWindow === 'function'
}
...
export function canOpenSessionInTerminal(): boolean {
  return typeof window !== 'undefined' && typeof window.sanremesDesktop?.openSessionInTerminal === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await runWindowOpen(
    () => window.sanremesDesktop.openSessionWindow(sessionId, opts),
...
export async function openNewWindow(): Promise<void> {
  ...
  await runWindowOpen(() => window.sanremesDesktop.openWindow(), 'Could not open a new window')
}
...
export async function openBrowserInNewWindow(tabId: string): Promise<boolean> {
  ...
  return runWindowOpen(() => window.sanremesDesktop.openBrowserWindow(tabId), 'Could not pop out browser')
}
...
export async function openSessionInTerminal(
  ...
  await runWindowOpen(
    () => window.sanremesDesktop.openSessionInTerminal(sessionId, opts),
