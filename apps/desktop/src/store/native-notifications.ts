...
export type { SanRemesOpenTarget }
...
const STORAGE_KEY = 'sanremes:native-notifications'
...
// "Backgrounded" = the user isn't on SanRemes. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `sanremes://index-network/intent/1`.
   */
  activate?: string
...
export function dispatchNativeNotification(input: NativeNotificationInput): boolean {
  ...
  void window.sanremesDesktop?.notify({
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `sanremes://index-network/intent/1`). */
  activate?: SanRemesOpenTarget
 ...
 *  user is away from SanRemes — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveSanRemesOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveSanRemesOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.sanremesDesktop
