...
/**
 * Feature store for backend (agent) plugins — the native SanRemes plugins plus
 ...
 * primitives `sanremes plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native SanRemes. */
  portable?: boolean
}
 ...
 *  SANREMES_HOME. Always refetches (it's a cheap local disk scan on the
