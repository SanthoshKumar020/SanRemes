 ...
 * `@sanremes/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-sanremes-glass]`
 * block in styles.css).
 */
...
} from '@sanremes/shared/translucency'
 ...
 * sets `sanremesDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.sanremesDesktop?.glassSupported === 'boolean'
    ? window.sanremesDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.sanremesDesktop?.translucencySupported === 'boolean'
    ? window.sanremesDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-sanremes-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-sanremes-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-sanremes-glass-on', glassLive)
  root.toggleAttribute('data-sanremes-glass', glassOn)
  root.toggleAttribute('data-sanremes-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-sanremes-glass-scope', scope)
  } else {
    root.removeAttribute('data-sanremes-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.sanremesDesktop?.setTranslucency?.(state)
  })
