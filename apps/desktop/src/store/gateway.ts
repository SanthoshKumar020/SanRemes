import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@sanremes/shared'
import { atom } from 'nanostores'

import type { SanRemesConnection } from '@/global'
import { reconnectBackoffDelayMs } from '@/lib/reconnect-backoff'
import { RECONNECT_ATTEMPT_TIMEOUT_MS, withTimeout } from '@/lib/with-timeout'
import { SanRemesGateway, setApiRequestConnection } from '@/sanremes'
...
const isOpen = (gateway: SanRemesGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: SanRemesConnection) => void
  /**
...
interface Secondary {
  ...
  connection: SanRemesConnection | null
  gateway: SanRemesGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: SanRemesGateway | null
  ...
  $gateway: ReturnType<typeof atom<SanRemesGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('sanremes.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<SanRemesGateway | null>(null),
...
export function setPrimaryGateway(gateway: SanRemesGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<SanRemesConnection, 'connectionId'> | null): void {
...
export function activeGateway(): SanRemesGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the sanremes module (null
...
function publishActiveConnection(connection: SanRemesConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new SanRemesGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: SanRemesGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`SanRemes gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.sanremesDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update SanRemes Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<SanRemesGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "SanRemes gateway is not connected" on the Sessions `+` action (#88880).
