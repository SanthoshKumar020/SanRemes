...
import { getSanRemesConfigRecord, saveSanRemesConfig } from '@/sanremes'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getSanRemesConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveSanRemesConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
