 ...
 *
 * The archive is the CLI's own `sanremes profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/sanremes'
...
import type { ProfileDesktopOverlay } from '@/types/sanremes'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'SanRemes profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.sanremesDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.sanremesDesktop?.selectPaths?.({
