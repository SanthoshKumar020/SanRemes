...
import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@sanremes/shared'
...
const TILES_KEY = 'sanremes.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'sanremes.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'sanremes.desktop.botChatSessions.v1'
 ...
 * restore re-dials the profile's backend, whose ensure_sanremes_home() re-creates
 * the profile directory the delete just removed (sanremes-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (sanremes-agent#94235). Tiles persisted
...
export const $focusedStoredSessionId = computed(
  ...
  (groupId, tree, selected, workspaceMode) => {
    ...
    // it to the sidebar group, whose active pane ('sanremes-bots:pane') is not a
...
// Dev hook for automation (mirrors __SANREMES_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__SANREMES_SESSION_TILES__ = {
