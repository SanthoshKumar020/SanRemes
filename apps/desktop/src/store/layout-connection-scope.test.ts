...
const localConn = {
  ...
  mode: 'local',
  profile: 'default'
} as unknown as SanRemesConnection
...
describe('connection-scoped sidebar lists (#77318)', () => {
  ...
  it('writes remote pins to a connection-scoped key, leaving the legacy key intact', () => {
    ...
    expect(readKey('sanremes.desktop.pinnedSessions')).toBe(JSON.stringify(['local-1']))

    // The remote pin landed under its own gateway scope, not the shared key
    // and not a per-profile fragment.
    const scoped = readKey(`sanremes.desktop.pinnedSessions.remote.${encodeURIComponent('https://vps-a.example:8443')}`)
  ...
  it('keeps pins across a profile switch on the same gateway, but not session order', () => {
    ...
    setConnection({ ...remoteA, profile: 'k9' } as unknown as SanRemesConnection)
