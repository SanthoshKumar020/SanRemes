 ...
 * app's bounds and stays visible while SanRemes is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'sanremes.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'sanremes.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.sanremesDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.sanremesDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.sanremesDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.sanremesDesktop?.petOverlay?.close()
}
