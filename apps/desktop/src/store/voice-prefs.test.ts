...
vi.mock('@/sanremes', () => ({
  getSanRemesConfigRecord: vi.fn(async () => ({})),
  saveSanRemesConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye sanremes', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye sanremes')
  })
