...
export function setZoomPercent(percent: number): void {
  window.sanremesDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.sanremesDesktop?.zoom) {
  void window.sanremesDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.sanremesDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
