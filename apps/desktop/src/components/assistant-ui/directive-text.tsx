...
const SANREMES_REF_TYPES = WIRE_REFERENCE_KINDS
type SanRemesRefType = (typeof SANREMES_REF_TYPES)[number]
...
const SANREMES_DIRECTIVE_RE = referenceRe()
...
export const sanremesDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SANREMES_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...sanremesDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing SanRemes directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.sanremesDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.sanremesDesktop?.readFileDataUrl(id)
