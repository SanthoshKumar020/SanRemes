 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.sanremes.send(prompt)` (or declarative
 * `data-sanremes-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'sanremes-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'sanremes-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `sanremes.send(prompt)`.
 ...
 *  wires `data-sanremes-send` so declarative HTML works with zero script:
 *  `<button data-sanremes-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.sanremes={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-sanremes-send]"):null;' +
    'if(el)send(el.getAttribute("data-sanremes-send")||"")},true)})()</script>'
  )
}
...
function InlineHtmlFrame({
  ...
  useEffect(() => {
    ...
    void Promise.resolve(window.sanremesDesktop?.readFileText(path))
      .then(result => {
        if (!alive) {
          return
        }
