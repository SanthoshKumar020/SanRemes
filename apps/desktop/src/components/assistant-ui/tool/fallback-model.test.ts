...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'SanRemes Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'SanRemes docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('SanRemes Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'SanRemes docs', url: 'https://example.com/docs' }
    ])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://sanremes-agent.nousresearch.com/docs' },
    ...
    expect(view.title).toBe('Failed to open sanremes-agent.nousresearch.com/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://sanremes-agent.nousresearch.com/docs' },
        result: { success: true, url: 'https://sanremes-agent.nousresearch.com/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened sanremes-agent.nousresearch.com/docs')
  })
})
