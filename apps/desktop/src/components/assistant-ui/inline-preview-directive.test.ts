...
describe('frameSizeFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'sanremes-inline-preview-size',
    token: 'tok',
    height: 500,
    width: 300,
    ...over
...
describe('intentFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'sanremes-inline-preview-intent',
    token: 'tok',
  ...
  it('rejects wrong token, wrong type, empty, and hostile shapes', () => {
    ...
    expect(intentFromMessage(msg({ type: 'sanremes-inline-preview-size' }), 'tok')).toBeNull()
...
describe('withInlineChrome intent wiring', () => {
  it('injects sanremes.send and the data-sanremes-send click bridge', () => {
    const framed = withInlineChrome('<html><body><h1>w</h1></body></html>', 'tok', '')

    expect(framed).toContain('window.sanremes={send:send}')
    expect(framed).toContain('data-sanremes-send')
    expect(framed).toContain('sanremes-inline-preview-intent')
  })
})
