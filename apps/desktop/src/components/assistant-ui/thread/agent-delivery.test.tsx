...
describe('delivery command detection', () => {
  it('matches the canonical delivery command', () => {
    const cmd = 'sanremes -p turqoise chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 SanRemes (@sanremes): hi there"'

    expect(deliveryTargetFromCommand(cmd)).toBe('turqoise')
  })

  it('matches with a cd prefix and timeout wrapper', () => {
    const cmd = 'cd ~ && timeout 240 sanremes -p mr-tester chat --in "~" -Q -q "Message from 🤖 SanRemes: hello"'
  ...
  it('ignores ordinary terminal commands', () => {
    ...
    expect(deliveryTargetFromCommand('sanremes -p turqoise chat -q "plain question"')).toBeNull()
    expect(deliveryTargetFromCommand('sanremes sessions list')).toBeNull()
  })
})

describe('reply extraction', () => {
  it('strips session_id bookkeeping and keeps the reply', () => {
    const output = 'session_id: 20260813_220347_f69ac6\nHi SanRemes! Good to hear from you.'

    expect(replyTextFromResult({ output })).toBe('Hi SanRemes! Good to hear from you.')
  })
