...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's SANREMES_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.sanremesDesktop?.logsRoot?.()
      ...
      const result = await window.sanremesDesktop?.openDir?.(root)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {window.sanremesDesktop?.logsRoot && (
