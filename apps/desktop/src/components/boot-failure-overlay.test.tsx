...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to SanRemes gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.sanremesDesktop
  Object.defineProperty(window, 'sanremesDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'sanremesDesktop', { configurable: true, value: original })
}
