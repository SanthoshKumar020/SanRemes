...
function installDesktopMock(state: DesktopBootstrapState) {
  ...
  Object.defineProperty(window, 'sanremesDesktop', {
    configurable: true,
    value: desktop
  })
...
afterEach(() => {
  cleanup()
  vi.useRealTimers()
  Reflect.deleteProperty(window, 'sanremesDesktop')
})
...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: { platform: 'win32', activeRoot: 'C:\\Users\\me\\AppData\\Local\\sanremes\\sanremes-agent' }
      })
    )

    render(<DesktopInstallOverlay />)

    expect(await screen.findByText('Set up SanRemes Desktop')).toBeTruthy()
    expect(screen.getByText('Connect to existing SanRemes')).toBeTruthy()
    expect(screen.getByText('Install SanRemes locally')).toBeTruthy()
    expect(screen.queryByText(/steps complete/i)).toBeNull()
    expect(screen.queryByText(/Fetching installer manifest/i)).toBeNull()
  })

  it('continues local bootstrap only when Install SanRemes locally is selected', async () => {
    ...
    fireEvent.click(await screen.findByText('Install SanRemes locally'))

    expect(desktop.continueBootstrapLocal).toHaveBeenCalledTimes(1)
    expect(screen.getByText('Set up SanRemes Desktop')).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText('Set up SanRemes Desktop')).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText('Install SanRemes locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)

    expect(
      await screen.findByText('Local installation could not start. Restart SanRemes Desktop and try again.')
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent('Install SanRemes locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)
    ...
    expect(screen.queryByText('Local installation could not start. Restart SanRemes Desktop and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click((await screen.findByText('Install SanRemes locally')).closest('button') as HTMLButtonElement)
    expect(
      await screen.findByText('Local installation could not start. Restart SanRemes Desktop and try again.')
    ).toBeTruthy()
    ...
    act(() => {
      desktop.emitBootstrapEvent({
        ...
        active: false,
        platform: 'win32',
        activeRoot: 'C:\\Users\\me\\AppData\\Local\\sanremes\\sanremes-agent-repaired'
      })
    })

    expect(screen.queryByText('Local installation could not start. Restart SanRemes Desktop and try again.')).toBeNull()
  })

  it('opens the remote connection form from the first-run choice', async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: { platform: 'linux', activeRoot: '/home/me/.sanremes/sanremes-agent' }
      })
    )

    render(<DesktopInstallOverlay />)

    fireEvent.click(await screen.findByText('Connect to existing SanRemes'))
  ...
  it('requires a successful token connection test before applying remote config', async () => {
    ...
    desktop.probeConnectionConfig.mockResolvedValue({
      authMode: 'token',
      baseUrl: 'https://gateway.example.com/sanremes',
      error: null,
      providers: [],
      reachable: true,
      version: '0.17.0'
    ...
    fireEvent.click(await screen.findByText('Connect to existing SanRemes'))
    fireEvent.change(await screen.findByPlaceholderText('https://gateway.example.com/sanremes'), {
      target: { value: 'https://gateway.example.com/sanremes' }
    })
    ...
    await waitFor(() => {
      expect(desktop.testConnectionConfig).toHaveBeenCalledWith({
        ...
        remoteUrl: 'https://gateway.example.com/sanremes'
      })
    })

    await screen.findByText('Connected to https://gateway.example.com/sanremes (0.17.0).')
  ...
  it('ignores a completed probe after the gateway URL becomes invalid', async () => {
    ...
    fireEvent.click(await screen.findByText('Connect to existing SanRemes'))
    const urlInput = await screen.findByPlaceholderText('https://gateway.example.com/sanremes')
    fireEvent.change(urlInput, { target: { value: 'https://gateway.example.com/sanremes' } })
  ...
  it('does not enable Apply when credentials change during a connection test', async () => {
    ...
    await act(async () => {
      resolveTest?.({ baseUrl: 'https://gateway.example.com/sanremes', ok: true, version: '0.17.0' })
      await pendingTest
    })

    expect(screen.queryByText('Connected to https://gateway.example.com/sanremes (0.17.0).')).toBeNull()
    expect(apply.disabled).toBe(true)
  })
  ...
  it('signs in, tests, and applies a password-style remote gateway', async () => {
    ...
    await waitFor(() => {
      expect(desktop.oauthLoginConnectionConfig).toHaveBeenCalledWith('https://gateway.example.com/sanremes')
    })
    ...
    await screen.findByText('Connected to https://gateway.example.com/sanremes.')
    const apply = screen.getByText('Apply and reconnect').closest('button') as HTMLButtonElement
    expect(apply.disabled).toBe(false)
    fireEvent.click(apply)
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    const desktop = installDesktopMock(
      bootstrapState({
        unsupportedPlatform: {
          platform: 'darwin',
          activeRoot: '/Users/me/.sanremes/sanremes-agent',
    ...
    expect(await screen.findByText('SanRemes needs a one-time install')).toBeTruthy()
    ...
    fireEvent.change(screen.getByPlaceholderText('https://gateway.example.com/sanremes'), {
      target: { value: 'https://gateway.example.com/sanremes' }
    })
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText('SanRemes needs a one-time install')).toBeNull()
  })
})
