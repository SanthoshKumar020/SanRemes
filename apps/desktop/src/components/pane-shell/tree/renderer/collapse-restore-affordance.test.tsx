...
describe('Sessions/Bots strip — #91223', () => {
  beforeEach(() => {
    ...
    registerPane('sanremes-bots:pane', { hideOnly: true, placement: 'left' }, 'Bots')
  ...
  it('double-clicking the Sessions tab leaves the strip and both chips', () => {
    ...
    expect(tabEl('sanremes-bots:pane')).toBeTruthy()
  ...
  it('double-clicking the Bots tab leaves the strip too', () => {
    ...
    tap(tabEl('sanremes-bots:pane')!)
    doubleTap(tabEl('sanremes-bots:pane')!)
...
describe('docked tool tile — collapsing keeps the restore chip', () => {
  beforeEach(() => {
    registerPane('workspace', { placement: 'main', uncloseable: true }, 'Chat')
    registerPane('sanremes-bots:routines', { placement: 'main', width: '250px' }, 'Cronjobs')
    $layoutTree.set(
      split('row', [
        group(['workspace'], { active: 'workspace', id: 'g-main' }),
        group(['sanremes-bots:routines'], { active: 'sanremes-bots:routines', id: 'g-routines' })
      ])
    )
  })
  ...
  it('clicking the active tab does not collapse the tile or drop its label', () => {
    ...
    expect(tabEl('sanremes-bots:routines')).toBeTruthy()
    tap(tabEl('sanremes-bots:routines')!)

    expect(zoneAt(1).minimized).toBeFalsy()
    expect(tabEl('sanremes-bots:routines')).toBeTruthy()
    expect(tabEl('sanremes-bots:routines')?.textContent).toMatch(/cronjobs/i)
  })
  ...
  it('collapsing via the store does not dismiss the pane from the tree', () => {
    collapseTreePane('sanremes-bots:routines')

    expect(zoneAt(1).minimized).toBe(true)
    expect($dismissedPanes.get().has('sanremes-bots:routines')).toBe(false)
    expect(zoneAt(1).panes).toContain('sanremes-bots:routines')
