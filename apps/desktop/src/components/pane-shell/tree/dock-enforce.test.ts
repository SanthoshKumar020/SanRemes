...
const TREE_KEY = 'sanremes.desktop.layoutTree.v2'
const USER_PLACED_KEY = 'sanremes.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'sanremes.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['sanremes-bots:pane'], active: 'sanremes-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'sanremes-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'sanremes-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","sanremes-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['sanremes-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['sanremes-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'sanremes-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['sanremes-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'sanremes-bots:pane')!
      ...
      children: [
        {
          type: 'group',
          id: 'g-left',
          panes: ['sessions', 'sanremes-bots:pane'],
          active: 'sanremes-bots:pane',
          headerHidden: true
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
  ...
  it('re-homes an edge-enforced pane stranded in the sessions tab strip', async () => {
      ...
      children: [
        {
          type: 'group',
          id: 'g-sessions',
          panes: ['sessions', 'sanremes-bots:pane', 'sanremes-bots:routines'],
          active: 'sanremes-bots:pane'
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
    ...
    const botsGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'sanremes-bots:pane')!
    const routinesGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'sanremes-bots:routines')!

    expect(botsGroup.panes).toEqual(['sessions', 'sanremes-bots:pane'])
    expect(botsGroup.active).toBe('sanremes-bots:pane')
    expect(routinesGroup.panes).toEqual(['sanremes-bots:routines'])
    ...
    const dockedRoutinesTree = {
      ...
      children: [
        ...
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' },
        {
          type: 'group',
          id: 'g-routines',
          panes: ['sanremes-bots:routines'],
          active: 'sanremes-bots:routines'
        }
      ]
    }
