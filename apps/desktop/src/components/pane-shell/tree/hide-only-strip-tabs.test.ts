...
function sessionsBotsTree() {
  ...
  registerPane('sanremes-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'sanremes-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('sanremes-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('sanremes-bots:pane')
    expect($hiddenStripTabs.get()).toContain('sanremes-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('sanremes-bots:pane')

    expect(setStripTabHidden('sanremes-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('sanremes-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('sanremes-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('sanremes-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('sanremes.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('sanremes-bots:pane')
    ...
    revealTreePane('sanremes-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('sanremes-bots:pane')
    expect(window.localStorage.getItem('sanremes.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'sanremes-bots:pane', title: 'sanremes-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'sanremes-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
