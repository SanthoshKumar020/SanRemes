...
export function getMemoryProviderConfig(provider: string, profile?: null | string): Promise<MemoryProviderConfig> {
  return sanremesApi<MemoryProviderConfig>({
...
export function startMemoryProviderOAuth(
  ...
  return sanremesApi<MemoryProviderOAuthStatus>({
    ...profileScoped(profile),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `sanremes memory` / `sanremes curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return sanremesApi<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return sanremesApi<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return sanremesApi<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return sanremesApi<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
export function runCurator(): Promise<ActionResponse> {
  return sanremesApi<ActionResponse>({
    ...profileScoped(),
...
export function updateSanRemes(): Promise<ActionResponse> {
  return sanremesApi<ActionResponse>({
    ...profileScoped(),
    path: '/api/sanremes/update',
    method: 'POST'
  })
}
...
export function checkSanRemesUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return sanremesApi<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/sanremes/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200, profile?: ProfileScope): Promise<ActionStatusResponse> {
  return window.sanremesDesktop.api<ActionStatusResponse>({
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return sanremesApi<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return sanremesApi<AudioSpeakResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(profile?: null | string): Promise<ElevenLabsVoicesResponse> {
  return sanremesApi<ElevenLabsVoicesResponse>({
...
export function getGhAuthStatus(refresh = false): Promise<{ available: boolean; authenticated: boolean }> {
  return sanremesApi<{ available: boolean; authenticated: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `sanremes doctor` / `sanremes security
// audit` / `sanremes backup` / `sanremes debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return sanremesApi<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return sanremesApi<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return sanremesApi<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return sanremesApi<DebugShareResponse>({
