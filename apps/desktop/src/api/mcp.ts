...
export function testMcpServer(name: string, profile?: ProfileScope): Promise<McpTestResult> {
  return window.sanremesDesktop.api<McpTestResult>({
 ...
 *  `saveSanRemesConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function authMcpServer(name: string, profile?: ProfileScope): Promise<McpOAuthFlow> {
  return window.sanremesDesktop.api<McpOAuthFlow>({
    ...capabilityScoped(profile),
...
export function cancelMcpOAuthFlow(flowId: string, profile?: null | string): Promise<{ ok: boolean; status: string }> {
  return sanremesApi<{ ok: boolean; status: string }>({
...
// `sanremes mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveSanRemesConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return sanremesApi<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(body: {
  ...
  return sanremesApi<McpServerSummary>({
    ...profileScoped(),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getMcpCatalog(profile?: ProfileScope): Promise<McpCatalogResponse> {
  return window.sanremesDesktop.api<McpCatalogResponse>({
    ...capabilityScoped(profile),
