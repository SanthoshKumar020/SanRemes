 ...
 *
 *  Exported for tests. */
export async function activeConnection(): Promise<SanRemesConnection> {
  ...
  return withTimeout(
    window.sanremesDesktop.getConnection(profile),
    RECONNECT_ATTEMPT_TIMEOUT_MS,
    `Timed out connecting to profile "${profile}"`
  )
}

/** Options for a plugin REST call — mirrors the app's own `sanremesDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see SanRemesApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.sanremesDesktop?.api) {
    throw new Error('SanRemes desktop bridge unavailable')
  }
