import { JsonRpcGatewayClient } from '@sanremes/shared'

import type { SanRemesApiRequest } from '@/global'
...
// "Timed out connecting to SanRemes backend" that hangs the UI (#48504).
//
...
export class SanRemesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'SanRemes gateway connection closed',
      connectErrorMessage: 'Could not connect to SanRemes gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'SanRemes gateway is not connected',
...
// process to route them to that host (sanremes:api's registry branch). Null
...
export function sanremesApi<T>(request: SanRemesApiRequest): Promise<T> {
  return window.sanremesDesktop.api<T>({ ...connectionScoped(), ...request })
}
