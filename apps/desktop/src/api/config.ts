...
export function getLogs(params: {
  ...
  return sanremesApi<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getSanRemesConfig(profile?: string): Promise<SanRemesConfig> {
  return sanremesApi<SanRemesConfig>({
...
export function getSanRemesConfigRecord(profile?: ProfileScope): Promise<SanRemesConfigRecord> {
  return window.sanremesDesktop.api<SanRemesConfigRecord>({
    ...capabilityScoped(profile),
    path: '/api/config'
  })
}

export function getSanRemesConfigDefaults(): Promise<SanRemesConfigRecord> {
  return sanremesApi<SanRemesConfigRecord>({
    ...profileScoped(),
...
export function getSanRemesConfigSchema(profile?: null | string): Promise<ConfigSchemaResponse> {
  return sanremesApi<ConfigSchemaResponse>({
    ...profileScoped(profile),
    path: '/api/config/schema'
  })
}

export function saveSanRemesConfig(config: SanRemesConfigRecord, profile?: null | string): Promise<{ ok: boolean }> {
  return sanremesApi<{ ok: boolean }>({
    ...profileScoped(profile),
    path: '/api/config',
    method: 'PUT',
    body: { config }
...
export function getEnvVars(profile?: null | string): Promise<Record<string, EnvVarInfo>> {
  return sanremesApi<Record<string, EnvVarInfo>>({
    ...profileScoped(profile),
    path: '/api/env'
  })
}
...
export function setEnvVar(key: string, value: string, profile?: ProfileScope): Promise<{ ok: boolean }> {
  return window.sanremesDesktop.api<{ ok: boolean }>({
    ...capabilityScoped(profile),
    path: '/api/env',
    method: 'PUT',
    body: { key, value }
...
export function revealEnvVar(key: string, profile?: ProfileScope): Promise<{ key: string; value: string }> {
  return window.sanremesDesktop.api<{ key: string; value: string }>({
    ...capabilityScoped(profile),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function validateProviderCredential(
  ...
  return sanremesApi<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return sanremesApi<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return sanremesApi<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return sanremesApi<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return sanremesApi<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return sanremesApi<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string, profile?: ProfileScope): Promise<OAuthStartResponse> {
  return window.sanremesDesktop.api<OAuthStartResponse>({
    ...capabilityScoped(profile),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return sanremesApi<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(
  ...
  return window.sanremesDesktop.api<OAuthPollResponse>({
    ...capabilityScoped(profile),
