...
vi.mock('@/sanremes', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), sanremesApi: vi.fn() }))
...
const { deleteProfile, sanremesApi } = await import('@/sanremes')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its SANREMES_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { sanremesDesktop: unknown }).sanremesDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(sanremesApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(sanremesApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(sanremesApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
