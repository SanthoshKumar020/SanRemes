/**
 * @sanremes/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__SANREMES_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, sanremesApi, type SanRemesGateway } from '@/sanremes'
import {
  $gateway,
...
import type { PaginatedSessions, UsageStats } from '@/types/sanremes'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend SanRemes profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.sanremesDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.sanremesDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and SanRemesGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (sanremes-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.sanremesDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update SanRemes Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.sanremesDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update SanRemes Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update SanRemes Desktop to open another Bot chat.' })
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('SanRemes Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return sanremesApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return sanremesApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('SanRemes gateway unavailable')
    }
 ...
 *  "SANREMES AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { RpcEvent, StatusResponse } from '@/types/sanremes'
