...
export function PetOverlayApp() {
  ...
  const setIgnore = (ignore: boolean) => {
    if (ignoreRef.current !== ignore) {
      ...
      window.sanremesDesktop?.petOverlay?.setIgnoreMouse(ignore)
    }
  }
  ...
  useEffect(() => {
    const off = window.sanremesDesktop?.petOverlay?.onState(payload => {
    ...
    window.sanremesDesktop?.petOverlay?.control({ type: 'ready' })
  ...
  useEffect(() => {
    ...
    window.sanremesDesktop?.petOverlay?.setFocusable(composerOpen)
  ...
  const onPetPointerMove = (e: React.PointerEvent) => {
    ...
    window.sanremesDesktop?.petOverlay?.setBounds({
      height: drag.height,
      width: drag.width,
  ...
  const onPetPointerUp = (e: React.PointerEvent) => {
    ...
    if (drag.moved) {
      ...
      window.sanremesDesktop?.petOverlay?.control({
    ...
    if (e.shiftKey) {
      window.sanremesDesktop?.petOverlay?.control({ type: 'pop-in' })
    ...
    if (clickTimerRef.current) {
      clearTimeout(clickTimerRef.current)
      clickTimerRef.current = undefined
      window.sanremesDesktop?.petOverlay?.control({ type: 'toggle-app' })
  ...
  const send = () => {
    ...
    if (text) {
      window.sanremesDesktop?.petOverlay?.control({ text, type: 'submit' })
    }
  ...
  const openApp = () => {
    ...
    setUnread(false)
    window.sanremesDesktop?.petOverlay?.control({ type: 'open-app' })
  }
  ...
  const onScale = useCallback((next: number, anchor: PetZoomAnchor) => {
    ...
    window.sanremesDesktop?.petOverlay?.control({ scale: next, type: 'scale' })
  }, [])
  ...
  useEffect(() => {
    ...
    window.sanremesDesktop?.petOverlay?.setBounds(bounds)
    window.sanremesDesktop?.petOverlay?.control({ bounds, type: 'bounds' })
