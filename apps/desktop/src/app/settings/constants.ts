...
// lib/personalities (mirrors sanremes_cli/personality.py BUILTIN_PERSONALITIES).
...
export const EMPTY_SELECT_VALUE = '__sanremes_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted SanRemes & Nous-trained models',
  ...
  { prefix: 'SANREMES_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // sanremes_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  fileReadMaxChars: 'Maximum characters SanRemes can read from one file request.',
  approvals: {
    mode: 'How SanRemes handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When SanRemes updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
