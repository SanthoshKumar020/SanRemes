...
import { disconnectOAuthProvider, listOAuthProviders } from '@/sanremes'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/sanremes'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.sanremesDesktop?.terminal)
...
//      in sanremes_cli/provider_catalog.py) — the SAME provider identity
//      `sanremes model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // SanRemes can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider SanRemes can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // SanRemes never deletes creds another tool owns behind a silent API call.
