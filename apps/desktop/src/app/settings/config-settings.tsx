...
import type { ConfigFieldSchema, SanRemesConfigRecord } from '@/types/sanremes'
...
function ConfigSettingsInner({
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['sanremes-config-schema'] : ['sanremes-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getSanRemesConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveSanRemesConfig(config, scopeProfile)
  ...
  const applyConfig = (next: SanRemesConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: SanRemesConfigRecord) => {
