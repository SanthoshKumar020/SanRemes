...
import { sanremesConfigCacheWriter, invalidateSanRemesConfig, useSanRemesConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `sanremes model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in sanremes_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile }: ModelSettingsProps) {
  ...
  const { data: config } = useSanRemesConfigRecord(scopeProfile)
  const setConfig = useMemo(() => sanremesConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateSanRemesConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveSanRemesConfig(next, scopeProfile)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `sanremes model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — SanRemes runs the flow for you.`}
          </p>
        )}
