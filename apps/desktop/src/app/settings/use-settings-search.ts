...
import { getEnvVars, getSanRemesConfigSchema } from '@/sanremes'
...
import { useSanRemesConfigRecord } from '../hooks/use-config-record'
...
export function useSettingsSearchCatalog(enabled: boolean) {
  const { t } = useI18n()
  const configQuery = useSanRemesConfigRecord()
