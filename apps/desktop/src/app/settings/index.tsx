...
import { getSanRemesConfigDefaults, getSanRemesConfigRecord, saveSanRemesConfig } from '@/sanremes'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getSanRemesConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'sanremes-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveSanRemesConfig(await getSanRemesConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
