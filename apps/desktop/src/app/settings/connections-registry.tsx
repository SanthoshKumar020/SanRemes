 ...
 * agent sources (local runtime + any number of remote gateways / SanRemes Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.sanremesDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.sanremesDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `sanremes update` to every eligible source; per-connection results
