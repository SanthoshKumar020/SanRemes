...
import { getProfiles } from '@/sanremes'
...
// User-installed plugins first — mirrors `sanremes plugins list --user`.
...
function reveal(file: string) {
  void window.sanremesDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // sanremes_home breaks against a remote backend (#66899).
    const dir = await window.sanremesDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.sanremesDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's sanremes home (profile-aware), so the
// path comes from the gateway — not from the renderer's local SANREMES_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.sanremesDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'SanRemes (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
