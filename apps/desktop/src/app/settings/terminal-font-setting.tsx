...
import { setSanRemesConfigCache, useSanRemesConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: SanRemesConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useSanRemesConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<SanRemesConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setSanRemesConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="sanremes-terminal-font-families"
          ...
          <datalist id="sanremes-terminal-font-families">
