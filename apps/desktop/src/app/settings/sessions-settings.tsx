...
import type { SanRemesConfigRecord, SessionInfo } from '@/types/sanremes'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveSanRemesConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files SanRemes wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.sanremesDesktop` is
    ...
    const settings = window.sanremesDesktop?.settings
