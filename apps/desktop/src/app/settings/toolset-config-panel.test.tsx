...
import type { ToolsetConfig } from '@/types/sanremes'
...
const getSanRemesConfigSchema = vi.fn()
...
vi.mock('@/sanremes', () => ({
  ...
  getSanRemesConfigSchema: () => getSanRemesConfigSchema(),
...
beforeEach(() => {
  ...
  getSanRemesConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getSanRemesConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveSanRemesConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveSanRemesConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
