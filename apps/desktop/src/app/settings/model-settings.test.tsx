...
const getSanRemesConfigRecord = vi.fn()
const saveSanRemesConfig = vi.fn()
...
vi.mock('@/sanremes', () => ({
  ...
  getSanRemesConfigRecord: () => getSanRemesConfigRecord(),
  saveSanRemesConfig: (config: unknown) => saveSanRemesConfig(config),
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'sanremes-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['sanremes-4', 'sanremes-4-mini'],
        authenticated: true,
        capabilities: { 'sanremes-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'sanremes-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ ok: true, provider: 'nous', model: 'sanremes-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'nous', model: 'sanremes-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getSanRemesConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveSanRemesConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'nous', model: 'sanremes-4' })
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getSanRemesConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveSanRemesConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['sanremes-4'],
          authenticated: true,
          capabilities: { 'sanremes-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'sanremes-4' }]
    })
...
describe('ModelSettings MoA preset editor', () => {
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      expect(saveMoaModels).not.toHaveBeenCalled()
      expect(screen.getByText('nous · sanremes-4')).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'nous', model: 'sanremes-4', enabled: false }),
