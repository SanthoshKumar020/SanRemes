...
// SanRemes Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.sanremesDesktop.setSecretStorageEncryption(on)
  ...
  // --- SanRemes Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.sanremesDesktop
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.sanremesDesktop?.sshConfigHosts) {
      return
    }
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteSanRemesPath: state.sshRemoteSanRemesPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.sanremesDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.sanremesDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'sanremes-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.sanremesDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.sanremesDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.sanremesDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.sanremesDesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.sanremesDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.sanremesDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.sanremesDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.sanremesDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.sanremesDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.sanremesDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* SanRemes Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/sanremes"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteSanRemesPath: event.target.value }))}
                placeholder={g.sshSanRemesPathPlaceholder}
                value={state.sshRemoteSanRemesPath}
              />
            }
            description={g.sshSanRemesPathDesc}
            title={g.sshSanRemesPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          ...
          <ListRow
            action={
              <Button onClick={() => void window.sanremesDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
