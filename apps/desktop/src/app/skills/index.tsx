...
import type { SkillInfo, ToolsetInfo } from '@/types/sanremes'
...
export function SkillsView({
  ...
  const registryBridge = window.sanremesDesktop?.connections
  const rosterBridge = window.sanremesDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'SanRemes (default)' : p.name,
      value: p.name
    }))
