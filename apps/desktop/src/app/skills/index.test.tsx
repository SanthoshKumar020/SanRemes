...
import type * as SanRemesApi from '@/sanremes'
...
vi.mock('@/sanremes', async importOriginal => ({
  ...(await importOriginal<typeof SanRemesApi>()),
...
describe('SkillsView toolset management', { timeout: 60_000 }, () => {
    ...
    await act(async () => {
      window.dispatchEvent(
        new MessageEvent('message', {
          data: { type: 'sanremes-skill-pick', name: 'web-research', identifier: 'web-research' },
          origin: 'https://sanremes-agent.nousresearch.com'
        })
      )
    })
  ...
  it('offers (connection, profile) scope rows on multi-connection desktops', async () => {
    ...
    ;(window as { sanremesDesktop?: unknown }).sanremesDesktop = { connections, getAgentRoster }
