...
interface SanRemesConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useSanRemesConfig({ activeSessionIdRef }: SanRemesConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshSanRemesConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        ...
        // rendering/applying SanRemes' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshSanRemesConfig, sttEnabled, voiceMaxRecordingSeconds }
}
