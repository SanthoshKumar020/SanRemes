...
import { getSanRemesConfig } from '@/sanremes'
...
import { useSanRemesConfig } from './use-sanremes-config'

vi.mock('@/sanremes', () => ({
  getSanRemesConfig: vi.fn(),
  getSanRemesConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'sanremes.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getSanRemesConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getSanRemesConfig>>)

describe('useSanRemesConfig refreshSanRemesConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to SanRemes' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useSanRemesConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshSanRemesConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useSanRemesConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getSanRemesConfig>>>()
    vi.mocked(getSanRemesConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useSanRemesConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshSanRemesConfig(true)
    })
    expect(getSanRemesConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getSanRemesConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getSanRemesConfig>>>()
    vi.mocked(getSanRemesConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useSanRemesConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshSanRemesConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getSanRemesConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getSanRemesConfig>>>()
    vi.mocked(getSanRemesConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshSanRemesConfig(true)
      refreshC = result.current.refreshSanRemesConfig(true)
    })
