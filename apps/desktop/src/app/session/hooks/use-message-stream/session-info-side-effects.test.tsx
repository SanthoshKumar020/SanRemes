...
let refreshSanRemesConfig: ReturnType<typeof vi.fn<() => Promise<void>>>
...
beforeEach(() => {
  sessionStates = null
  refreshSanRemesConfig = vi.fn<() => Promise<void>>(async () => undefined)
...
describe('session.info config refetch gating', () => {
  it('coalesces active-session bursts into one trailing config fetch', async () => {
    ...
    expect(refreshSanRemesConfig).not.toHaveBeenCalled()

    await act(async () => {
      await vi.advanceTimersByTimeAsync(400)
    })

    expect(refreshSanRemesConfig).toHaveBeenCalledTimes(1)
  })
