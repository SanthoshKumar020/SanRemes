...
export function handleStatusEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'notification.show') {
    ...
    // native OS notification when SanRemes is backgrounded; dispatch is gated
  ...
  if (event.type === 'error') {
    const errorMessage = payload?.message || 'SanRemes reported an error'
