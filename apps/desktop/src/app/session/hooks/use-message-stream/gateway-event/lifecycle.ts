...
export function handleLifecycleEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'gateway.ready') {
    ...
    ingestBackendSkin((payload as { skin?: SanRemesSkin } | undefined)?.skin, { apply: false })
  ...
  if (event.type === 'skin.changed') {
    // A runtime skin switch (SanRemes activating an authored skin, or `/skin`
    // on another surface). Only the active source+profile's change repaints.
    if (fromActiveSource()) {
      ingestBackendSkin(payload as SanRemesSkin | undefined, { apply: true })
    }
