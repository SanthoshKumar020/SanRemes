import type { BillingBlock } from '@sanremes/shared'
 ...
 * billing-specific toast — never the generic "SanRemes error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'SanRemes reported an error',
