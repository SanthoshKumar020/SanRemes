import { registryBackendScopeKey } from '@sanremes/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshSanRemesConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshSanRemesConfig — two REST calls (config + defaults) per event, per
