...
describe('renderRpcResult', () => {
  ...
  describe('session.save', () => {
    it('echoes the saved file path', () => {
      expect(renderRpcResult({ file: '/home/user/.sanremes/sessions/saved/x.json' }, 'save')).toBe(
        'Saved transcript to /home/user/.sanremes/sessions/saved/x.json'
      )
    })
  })
  ...
  describe('session.status', () => {
    it('passes through the multi-line plain-text output verbatim', () => {
      const output = 'SanRemes TUI Status\n\nSession ID: s-1\nModel: nous-sanremes-3 (unknown)'
