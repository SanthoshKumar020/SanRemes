import { skillInvocationText } from '@sanremes/shared'
...
// 120s RPC budget (SANREMES_TUI_RPC_TIMEOUT_MS default) instead.
...
const renderWakeStatus = (status: WakeStatusResponse): string => {
  const lines = [
    ...
    `Phrase: "${status.phrase?.trim() || 'hey sanremes'}"`,
  ...
  return useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      const actionHandlers: Record<DesktopActionId, (ctx: SlashActionCtx) => Promise<void>> = {
        ...
        // lease. Sending it through slash.exec creates a separate SanRemesCLI in
