import { JsonRpcGatewayError } from '@sanremes/shared'
...
import { getSession } from '@/sanremes'
...
// it, rather than reaching for SanRemes's own browser and panes.
...
describe('usePromptActions submit / queue drain semantics', () => {
  ...
  it('pins prompt.submit to the active registry connection when the remote session row is untagged', async () => {
    $connection.set({ connectionId: 'sanremes01', mode: 'remote' } as never)
  ...
  it('uploads file bytes via file.attach on a remote gateway and submits the rewritten ref', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/remote/work/.sanremes/desktop-attachments/report.txt',
          ref_text: '@file:.sanremes/desktop-attachments/report.txt',
          uploaded: true
        } as never
      }
    ...
    expect(calls[1]?.params).toEqual({
      session_id: RUNTIME_SESSION_ID,
      text: '@file:.sanremes/desktop-attachments/report.txt\n\nconvert this to epub'
    })
  })
  ...
  it('uploads Windows file bytes when local mode fronts a POSIX WSL/Docker backend', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/root/.sanremes/desktop-attachments/report.txt',
          ref_text: '@file:.sanremes/desktop-attachments/report.txt',
          uploaded: true
        } as never
      }
    ...
    expect(calls[1]).toEqual({
      ...
      params: { session_id: RUNTIME_SESSION_ID, text: '@file:.sanremes/desktop-attachments/report.txt\n\nsummarize' }
    })
  })
  ...
  it('uploads file bytes when the terminal backend is a container (docker)', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/root/.sanremes/attachments/report.txt',
          ref_text: '@file:/root/.sanremes/attachments/report.txt',
          uploaded: true
        } as never
      }
...
describe('usePromptActions eager-upload races', () => {
  ...
  it('joins an in-flight eager upload at submit instead of staging the file twice', async () => {
    ...
    // under .sanremes/desktop-attachments/. Submit must await the in-flight upload
    ...
    const requestGateway = vi.fn(async (method: string) => {
      ...
      if (method === 'file.attach') {
        ...
        return { attached: true, ref_text: '@file:.sanremes/desktop-attachments/doc.pdf', uploaded: true } as never
      }
  ...
  it('still aborts when the user genuinely switches chats after create, during attachment sync (#62805)', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        ...
        return {
          attached: true,
          ref_text: '@file:.sanremes/desktop-attachments/test.pdf',
          uploaded: true
        } as never
      }
...
describe('usePromptActions eager attachment upload (drop-time)', () => {
  ...
  it('uploads a dropped file the moment it lands (active session) and rewrites the chip with the gateway ref', async () => {
    ...
    $connection.set({ mode: 'remote' } as never)
    const readFileDataUrl = vi.fn(async () => 'data:application/pdf;base64,JVBERi0=')
    Object.defineProperty(window, 'sanremesDesktop', { configurable: true, value: { readFileDataUrl } })
    ...
    const requestGateway = vi.fn(async (method: string) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          ref_text: '@file:.sanremes/desktop-attachments/DEVIS_signed.pdf',
          uploaded: true
        } as never
      }
    ...
    expect(chip.refText).toBe('@file:.sanremes/desktop-attachments/DEVIS_signed.pdf')
