...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { sanremesDesktop: { readDir: typeof readDir } }).sanremesDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): SanRemesReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { sanremesDesktop: unknown }).sanremesDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: SanRemesReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SanRemesReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: SanRemesReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: SanRemesReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: SanRemesReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SanRemesReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.sanremesDesktop is missing', async () => {
    delete (window as unknown as { sanremesDesktop?: unknown }).sanremesDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: SanRemesReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: SanRemesReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: SanRemesReadDirResult) => void) | undefined
