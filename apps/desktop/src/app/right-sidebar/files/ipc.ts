...
import type { SanRemesReadDirEntry, SanRemesReadDirResult } from '@/global'
...
export type ProjectTreeEntry = SanRemesReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: SanRemesReadDirEntry) {
...
async function filterIgnored(entries: SanRemesReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SanRemesReadDirResult> {
  if (!window.sanremesDesktop) {
