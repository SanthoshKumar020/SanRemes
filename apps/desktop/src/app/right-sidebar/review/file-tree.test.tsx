...
const file = (path: string): SanRemesReviewFile => ({
  added: 1,
  path,
  removed: 0,
  staged: false,
...
function filesUnderPublish(count: number): SanRemesReviewFile[] {
  return Array.from({ length: count }, (_, i) => file(`publish/file-${String(i).padStart(4, '0')}.so`))
}

function topLevelFiles(count: number): SanRemesReviewFile[] {
