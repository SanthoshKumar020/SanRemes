...
const SANREMES_PATHS_MIME = 'application/x-sanremes-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(SANREMES_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(SANREMES_PATHS_MIME)
  ...
  const getPath = window.sanremesDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.sanremesDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (sanremes --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.sanremesDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[sanremes-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.sanremesDesktop?.terminal?.write(sessionId, `${command}\r`)
