...
describe('BlockerView', () => {
  ...
  it('uses the blocker view for a foreign process instead of the generic update error', async () => {
    ...
    $updateApply.set({
      applying: false,
      stage: 'error',
      message: 'Update aborted: another SanRemes process is using this installation.',
      percent: null,
      error: 'venv-blocked',
      command: null,
      blockers: [
    ...
    expect(screen.getByText('Close other processes to update SanRemes')).toBeTruthy()
  ...
  it('explains safe local previews and offers one-click close-and-update', async () => {
    ...
    expect(screen.getByText('Close local previews to update SanRemes?')).toBeTruthy()
    expect(screen.getByText('Example Preview')).toBeTruthy()
