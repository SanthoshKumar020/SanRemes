...
import type { SessionInfo, SessionMessage } from '@/types/sanremes'
...
describe('collectArtifactsForSession', () => {
  ...
  it('keeps an explicit browser screenshot but ignores page assets', () => {
    const payload = JSON.stringify({
      ...
      screenshot_path: '/tmp/sanremes-browser/screenshot.png'
    })
...
${payload}
    ...
    expect(artifacts[0]).toMatchObject({
      kind: 'image',
      value: '/tmp/sanremes-browser/screenshot.png'
    })
  })
  ...
  it('keeps native browser screenshots without indexing embedded image data', () => {
    const artifacts = collectArtifactsForSession(makeSession({ id: 'native-browser-session' }), [
      {
        content: {
          ...
          meta: { screenshot_path: '/tmp/sanremes-browser/native-screenshot.png' },
      ...
      {
        content: 'Image attached. Screenshot path: /tmp/sanremes browser/summary screenshot.png',
        role: 'tool',
        timestamp: 1_781_774_002,
        tool_name: 'browser_vision'
      },
      {
        content: 'Image attached. Screenshot path: C:\\Users\\Example User\\.sanremes\\screenshot.png',
        role: 'tool',
        timestamp: 1_781_774_003,
        tool_name: 'browser_vision'
      }
    ...
    expect(artifacts.map(artifact => artifact.value)).toEqual([
      '/tmp/sanremes-browser/native-screenshot.png',
      '/tmp/sanremes browser/summary screenshot.png',
      'C:\\Users\\Example User\\.sanremes\\screenshot.png'
    ])
  })
  ...
  it('resolves local file image artifacts through the desktop fs bridge', async () => {
    ...
    const path = '/home/me/.sanremes/cache/image_generate/out.png'
  ...
  it('resolves remote image artifact thumbnails through the desktop fs bridge', async () => {
    ...
    const path = '/Users/me/.sanremes/skills/work-esab/references/images/manual-step03.jpeg'

    await expect(artifactImageSrc(path)).resolves.toBe('data:image/jpeg;base64,cmVtb3Rl')

    expect(api).toHaveBeenCalledWith({
      path: '/api/fs/read-data-url?path=%2FUsers%2Fme%2F.sanremes%2Fskills%2Fwork-esab%2Freferences%2Fimages%2Fmanual-step03.jpeg'
    })
  })
})
