...
function setWorkspaceTransfer(transferring: boolean): void {
  window.sanremesDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.sanremesDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.sanremesDesktop?.hud?.beginMove?.()
}
 ...
 * (see sanremes:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.sanremesDesktop?.hud?.endMove?.()
      }
