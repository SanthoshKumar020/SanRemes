import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@sanremes/shared'
import { useEffect, useRef } from 'react'

import { shouldApplyPostBootProgressError } from '@/components/boot-failure-reauth'
import type { SanRemesConnection } from '@/global'
import { SanRemesGateway } from '@/sanremes'
...
import type { RpcEvent } from '@/types/sanremes'
...
export function primaryRuntimeConnectionId(connection: Pick<SanRemesConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.sanremesDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: SanRemesGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: SanRemesConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting SanRemes...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting SanRemes…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to SanRemes backend'
        )
        ...
        // as an opaque "Could not connect to SanRemes gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshSanRemesConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshSanRemesConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startSanRemes re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new SanRemesGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting SanRemes…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshSanRemesConfig(),
          ...
          // retry it), not brick boot behind the "SanRemes couldn't start"
