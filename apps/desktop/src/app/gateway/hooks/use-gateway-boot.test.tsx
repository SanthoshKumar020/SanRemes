...
// bug that drives the REAL useGatewayBoot hook + REAL SanRemesGateway through a
...
function Harness({
  ...
  refreshSanRemesConfig = async () => undefined,
  refreshSessions
}: {
  beforeConnectionSwitch?: () => void
  refreshSanRemesConfig?: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
beforeEach(() => {
  ...
  ;(window as { sanremesDesktop?: unknown }).sanremesDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { sanremesDesktop?: unknown }).sanremesDesktop
  window.localStorage.removeItem('sanremes.desktop.workspace-cwd')
  $currentCwd.set('')
  $busy.set(false)
  $awaitingResponse.set(false)
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForSanRemes) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startSanRemes()'s remote branch awaits waitForSanRemes() for 45s before it
    ...
    ;(window as { sanremesDesktop?: unknown }).sanremesDesktop = desktop
    ...
    // After ~45s waitForSanRemes gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('SanRemes backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('a superseded Settings switch cannot publish delayed cwd or config work after the winner', async () => {
    ...
    const refreshSanRemesConfig = async (_force = false, shouldPublish?: () => boolean) => {
      if (!shouldPublish) {
        return
      }
    ...
    render(<Harness refreshSanRemesConfig={refreshSanRemesConfig} />)
    await flushAsync()
    expect($gatewayState.get()).toBe('open')
  ...
  it('a getConnection() that hangs on INITIAL boot rejects on its own after the reconnect-attempt timeout, not only when main eventually gives up (#93454)', async () => {
    ...
    // eventual timeout (e.g. waitForSanRemes, ~45s) ever settled it. A wedge
    // that main never resolves (not even a rejection) must not hang
    // "Starting SanRemes…" forever; the renderer needs to own its own bound
  ...
  it('FIX: post-boot ticket-mint boot-progress errors do not lock the UI', async () => {
    ...
    act(() => {
      desktop.emitBootProgress({
        error: 'Could not reach the remote SanRemes gateway while refreshing its WebSocket ticket. Try reconnecting.',
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "SanRemes couldn't start" overlay — the socket is open
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\SanRemes')
    expect($currentCwd.get()).toBe('C:\\SanRemes')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startSanRemes() rejects with a
