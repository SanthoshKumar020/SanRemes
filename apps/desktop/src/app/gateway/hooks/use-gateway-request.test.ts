...
vi.mock('@/sanremes', async importOriginal => {
  const actual = await importOriginal<typeof SanRemesModule>()

  class FakeSanRemesGateway {
  ...
  return { ...actual, SanRemesGateway: FakeSanRemesGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as SanRemesGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as SanRemesGateway, 'default')
  $gateway.set(primary as unknown as SanRemesGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.sanremesDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as SanRemesGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { sanremesDesktop: unknown }).sanremesDesktop = { getConnection }
    $gateway.set(dropped)
