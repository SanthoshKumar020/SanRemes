import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@sanremes/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as SanRemesGateway | null
  const gatewayRef = useRef<SanRemesGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.sanremesDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<SanRemesGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as SanRemesGateway | null
      }),
    []
  )
