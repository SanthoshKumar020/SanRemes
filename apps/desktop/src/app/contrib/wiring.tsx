...
import { getLatestSessionMessages } from '@/sanremes'
...
import { useSanRemesConfig } from '../session/hooks/use-sanremes-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshSanRemesConfig, sttEnabled, voiceMaxRecordingSeconds } = useSanRemesConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('sanremes:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('sanremes:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshSanRemesConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshSanRemesConfig])
