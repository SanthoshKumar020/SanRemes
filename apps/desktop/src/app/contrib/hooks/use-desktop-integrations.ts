...
import { pathFromSanRemesDeepLink, resolveSanRemesOpenPath } from '@/lib/sanremes-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.sanremesDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.sanremesDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.sanremesDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.sanremesDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // sanremes://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.sanremesDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future sanremesDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveSanRemesOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.sanremesDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (sanremes://index-network/intent/1, sanremes://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromSanRemesDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.sanremesDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.sanremesDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.sanremesDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.sanremesDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
