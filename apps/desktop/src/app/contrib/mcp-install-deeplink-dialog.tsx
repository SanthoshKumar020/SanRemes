...
import { getSanRemesConfigRecord, saveMcpServers } from '@/sanremes'
...
import { setSanRemesConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `sanremes://mcp/install` deep links. The payload is
 ...
 * confirming lets SanRemes spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getSanRemesConfigRecord())
      ...
      setSanRemesConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
