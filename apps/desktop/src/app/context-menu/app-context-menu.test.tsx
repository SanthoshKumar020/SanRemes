...
import { ContextMenu, ContextMenuTrigger, SANREMES_CONTEXT_MENU_TRIGGER_ATTR } from '@/components/ui/context-menu'
...
function installBridge(partial: Partial<Window['sanremesDesktop']> = {}) {
...
afterEach(() => {
  ...
  delete desktopWindow.sanremesDesktop
})
...
describe('AppContextMenu', () => {
  ...
  it('offers the resolved copy only for loopback links on a remote gateway', async () => {
    ...
    installBridge({
      reachPreviewUrl: reachPreviewUrl as unknown as Window['sanremesDesktop']['reachPreviewUrl'],
      writeClipboard: writeClipboard as unknown as Window['sanremesDesktop']['writeClipboard']
    })
    mountMenu()
  ...
  it('runs edit verbs after the menu closed, with focus back on the editable', async () => {
    ...
    installBridge({ contextMenuEdit: contextMenuEdit as unknown as Window['sanremesDesktop']['contextMenuEdit'] })
    mountMenu()
  ...
  it('keeps a modal textarea paste menu inside its dialog and restores focus', async () => {
    ...
    installBridge({
      contextMenuEdit: contextMenuEdit as unknown as Window['sanremesDesktop']['contextMenuEdit'],
  ...
  it('keeps paste clickable even when the clipboard probe reports empty', async () => {
    ...
    installBridge({ readClipboard: readClipboard as unknown as Window['sanremesDesktop']['readClipboard'] })
    mountMenu()
    const host = attach('<textarea>text</textarea>')
...
describe('ContextMenuTrigger asChild', () => {
  it('keeps the coordinator marker when the child overwrites data-slot', () => {
    ...
    expect(footer.hasAttribute(SANREMES_CONTEXT_MENU_TRIGGER_ATTR)).toBe(true)
  })
})
