...
import { SANREMES_CONTEXT_MENU_TRIGGER_ATTR } from '@/components/ui/context-menu'
    ...
    [
      ...
      terminal.paste ? (
        <Item
          ...
          onSelect={() =>
            void window.sanremesDesktop?.readClipboard().then(text => (text ? terminal.paste?.(text) : undefined))
          }
        />
      ) : null,
      <Item
...
function domSections(open: Extract<OpenContextMenu, { kind: 'dom' }>, t: Translations): ReactNode[][] {
  ...
  const editableCommand = (command: 'copy' | 'cut' | 'paste') => {
    withEditableFocus(() => void window.sanremesDesktop?.contextMenuEdit?.(command))
  }
  ...
  const spellcheckAction = (action: { kind: 'add' | 'replace'; word: string }) => {
    withEditableFocus(() => void window.sanremesDesktop?.contextMenuSpellcheck?.(action))
  }
    ...
    sections.push(
      [
        ...
        <Item
          ...
          onSelect={() => void window.sanremesDesktop?.contextMenuCopyImage?.()}
        />,
        target.imageUrl ? (
          <Item
            icon="copy"
        ...
        target.imageUrl ? (
          <Item
            ...
            onSelect={() => void window.sanremesDesktop?.saveImageFromUrl?.(target.imageUrl)}
          />
        ) : null
      ].filter(Boolean)
    )
      ...
      [
        ...
        imageUrl ? (
          <Item
            ...
            label={copy.image.saveImageAs}
            onSelect={() => void window.sanremesDesktop?.saveImageFromUrl?.(imageUrl)}
          />
        ) : null
      ].filter(Boolean)
    )
  ...
  return [
    ...
    [
      <Item
        icon="cloud-download"
        key="shell-update"
        label={t.commandCenter.updateSanRemes}
...
export function AppContextMenu() {
  ...
  useEffect(() => {
    ...
    const onContextMenu = (event: MouseEvent) => {
      ...
      if (element?.closest(`[${SANREMES_CONTEXT_MENU_TRIGGER_ATTR}], [data-slot="context-menu-trigger"]`)) {
        return
      }
  ...
  useEffect(() => window.sanremesDesktop?.onContextMenuSpellcheck?.(augmentSpellcheck), [])
