...
import { extractDroppedFiles, partitionDroppedFiles, SANREMES_PATHS_MIME } from '../../hooks/use-composer-actions'
...
export function useComposerDrop({
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, SANREMES_PATHS_MIME)) {
      return
    }
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, SANREMES_PATHS_MIME)) {
      return
    }
