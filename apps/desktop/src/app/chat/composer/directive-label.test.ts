...
import { sanremesDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
function sentLabel(draft: string) {
  return sanremesDirectiveFormatter
    .parse(draft)
...
describe('one label per reference, on every surface', () => {
  ...
  it('an agent mention inserts as plain @name text, never an @simple: chip', () => {
    ...
    expect(sanremesDirectiveFormatter.serialize(item)).toBe('@mr-tester')
  ...
  it('a url still reads host + path on every surface', () => {
    const item = backendRow('@url:https://github.com/NousResearch/sanremes-agent/pull/74533', '', '')
    const { editor, result } = typed('@gith')

    act(() => result.current.replaceTriggerWithChip(item))

    const expected = 'github.com/NousResearch/sanremes-agent/pull/74533'
