...
import type { SessionInfo } from '@/sanremes'
...
// zero SanRemes sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, SanRemesGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, SanRemesGitWorktree[]>>({})
