...
export function ProfileRemoteOverrideDialog({ profileNames }: { profileNames: string[] }) {
  ...
  useEffect(() => {
    ...
    window.sanremesDesktop?.connections
      ?.list()
      .then(registry => {
        if (cancelled) {
          return
  ...
  const performSave = async () => {
    ...
    try {
      await window.sanremesDesktop.applyConnectionConfig({
        mode: 'remote',
        profile,
  ...
  const removeOverride = async () => {
    ...
    try {
      await window.sanremesDesktop.applyConnectionConfig({ mode: 'local', profile })
