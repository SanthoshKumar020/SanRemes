...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__sanremesTourEngine) {
    ${driverIife}
    w.__sanremesTourHolder = {};
    w.__sanremesTourCollect = (${collectTourTargets.toString()});
    w.__sanremesTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__sanremes-tour-style')) {
    var style = document.createElement('style');
    style.id = '__sanremes-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__sanremesTourEngine(
    w.driver.js.driver,
    w.__sanremesTourHolder,
    ${JSON.stringify(action)},
    w.__sanremesTourCollect,
    document
  ));
})()`
}
