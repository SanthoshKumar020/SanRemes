...
const preamble = () => `  var w = window;
  ...
  w.__sanremesActHolder = w.__sanremesActHolder || {};
  w.__sanremesAct = ${actEngineSource()};
  w.__sanremesWatch_fn = (${watchInPage.toString()});
  w.__sanremesWatchTag = ${WATCH_TAG};
  var holder = w.__sanremesActHolder;
  var act = function (a) { return w.__sanremesAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__sanremesWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  w.__sanremesHit = null;
  document.addEventListener('pointerdown', function (e) {
    w.__sanremesHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'SANREMES-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
