...
import { SANREMES_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  try {
    ...
    if (!message.includes("No handler registered for 'sanremes:readFileText'")) {
      throw error
    }
  }
  ...
  const dataUrl = await window.sanremesDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(SANREMES_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
