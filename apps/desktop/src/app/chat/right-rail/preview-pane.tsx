        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.sanremesDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask SanRemes to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.sanremesDesktop?.watchPreviewFile ||
      !window.sanremesDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.sanremesDesktop.onPreviewFileChanged(payload => {
    ...
    void window.sanremesDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.sanremesDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.sanremesDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:sanremes-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.sanremesDesktop?.zoom?.factor?.() || 1
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.sanremesDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.sanremesDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.sanremesDesktop?.openExternal(currentUrl)
                : undefined
            }
