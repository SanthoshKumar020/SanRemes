...
export const zhHant = defineLocale({
  ...
  boot: {
    ready: 'SanRemes Desktop 已就緒',
    desktopBootFailedWithMessage: message => `桌面啟動失敗：${message}`,
    steps: {
      connectingGateway: '正在連線桌面閘道',
      loadingSettings: '正在載入 SanRemes 設定',
      loadingSessions: '正在載入最近工作階段',
      retryingRemoteBackend: '正在重新連線遠端 SanRemes 後端…',
      startingDesktopConnection: '正在啟動桌面連線',
      startingSanRemesDesktop: '正在啟動 SanRemes Desktop…'
    },
    errors: {
      backgroundExited: 'SanRemes 背景程序已結束。',
      backgroundExitedDuringStartup: 'SanRemes 背景程序在啟動期間結束。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '您的 SanRemes 後端早於目前的桌面版本，可能無法正常運作。請更新以保持一致。',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 SanRemes Desktop。',
  ...
  sendDiagnostics: {
    ...
    failedTitle: '上傳失敗',
    failedHint:
      '您也可以在終端機執行 `sanremes debug share --nous`，或執行 `sanremes debug share --local` 在不上傳的情況下檢視報告。',
  ...
  settings: {
    ...
    resetConfirm: '要將所有設定恢復為 SanRemes 預設值嗎？',
    exportFailed: '匯出失敗',
    resetFailed: '重設失敗',
    nav: {
      providers: '提供方',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒僅在 SanRemes 位於背景時觸發。',
      kinds: {
        approval: {
          label: '需要核准',
        ...
        input: {
          label: '需要輸入',
          description: 'SanRemes 提出了問題，或需要密碼或密鑰。'
        },
        turnDone: {
          label: '回覆就緒',
          description: 'SanRemes 在背景時完成了一輪對話。'
        },
        turnError: {
          label: '本輪失敗',
          description: '背景回合錯誤。'
        ...
        plugin: {
          label: '外掛通知',
          description: 'SanRemes 在背景時，桌面外掛傳送了通知。'
        }
      },
      test: '傳送測試通知',
      testTitle: 'SanRemes',
    ...
    appearance: {
      ...
      colorMode: '色彩模式',
      colorModeDesc: '選擇固定模式，或讓 SanRemes 跟隨系統設定。',
      ...
      reactionsTitle: '訊息回應',
      reactionsDesc: 'iMessage 風格的表情回應 — 你可以對訊息做出回應，SanRemes 也能回應你的訊息。',
      tipsTitle: '應用程式內提示',
      tipsDesc: '指向應用程式某處的小氣泡：閒置時偶爾出現，需要時 SanRemes 也會給你一則。關掉一則就不再出現。',
      tipsReset: (count: number) => `復原 ${count} 則已關閉的提示`,
      toursTitle: '導覽',
      toursDesc: '讓 SanRemes 帶你認識應用程式：調暗畫面並逐步標示每個位置。',
      ...
      pet: {
        title: '寵物',
        intro:
          '領養一隻懸浮在應用上的 petdex 動畫寵物，它會根據 SanRemes 的狀態做出反應——工具執行時奔跑、成功時歡呼、出錯時沮喪。',
        restartHint: '寵物功能需要重新啟動——目前執行的應用在此功能加入前啟動。請結束並重新開啟 SanRemes，然後回到此處。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'SanRemes 需要本機時間上下文時使用。留空則使用系統時區。',
      agent: {
        imageInputMode: '控制圖片附件如何傳送給模型。',
        maxTurns: 'SanRemes 停止一次執行前的工具呼叫輪次上限。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'SanRemes 單次檔案讀取可讀取的最大字元數。',
      approvals: {
        mode: 'SanRemes 如何處理需要明確批准的指令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'SanRemes 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'SanRemes 執行環境已更新,但桌面應用程式本身仍是舊建置——在應用程式更新之前,新的介面功能(如 Bot Mode)不會顯示。請執行下方的更新以重新建置應用程式。如果此警告仍未消除,請從最新的桌面安裝程式重新安裝。',
      ...
      automaticUpdatesDesc: 'SanRemes 會在背景自動檢查更新，並在有可用更新時通知你。',
    ...
    quickEntry: {
      enabledTitle: '快速輸入',
      enabledDesc: '用全域快速鍵在任何地方喚出一個小輸入框，無需開啟 SanRemes 即可送出提示。',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'SanRemes Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 SanRemes 後端，請使用遠端閘道。閘道連線屬於本機層級設定；設定檔是從已連線的閘道中探索出來的。',
      envOverrideTitle: '環境變數正在控制此桌面工作階段。',
      envOverrideDesc: '取消設定 SANREMES_DESKTOP_REMOTE_URL 和 SANREMES_DESKTOP_REMOTE_TOKEN 後才會使用下方儲存的設定。',
      localTitle: '本機閘道',
      localDesc: '在 localhost 啟動私有 SanRemes 後端。這是預設方式，可離線使用。',
      remoteTitle: '遠端閘道',
      remoteDesc:
        '將此桌面殼層連線至遠端 SanRemes 後端。託管閘道使用 OAuth 或帳號密碼；自託管閘道也可使用工作階段 Token。',
      remoteUrlTitle: '遠端 URL',
      remoteUrlDesc: '遠端儀表板後端的基礎 URL。支援路徑前綴，例如 /sanremes。',
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'SanRemes Desktop 將使用已儲存的設定重新連線。',
      savedMessage: '已儲存，下次重新啟動後生效。',
      connectedTo: (baseUrl, version) => `已連線至 ${baseUrl}${version ? ` · SanRemes ${version}` : ''}`,
      ...
      sshTitle: '透過 SSH 連線',
      sshDesc:
        'SanRemes 會透過 SSH 在遠端啟動並以通道連線到本應用程式——無需自行啟動或公開任何服務。前提：已具備到該主機的金鑰 SSH 存取。',
      ...
      sshSanRemesPathTitle: 'SanRemes 路徑（選用）',
      sshSanRemesPathDesc: '遠端 sanremes 執行檔的完整路徑。留空 = 自動偵測。',
      sshSanRemesPathPlaceholder: '自動偵測',
      sshTestConnection: '測試 SSH',
      sshConnect: '連線',
      sshButtonsHint: '「儲存」會在下次啟動時生效，「連線」則立即重新連線。',
      sshReachable: (host, platform) => `可連線：${host}（${platform}）——已找到 SanRemes`,
      sshIncompleteHost: '連線前請輸入 SSH 主機。',
      sshErrUnreachable: '無法透過 SSH 連線到該主機。請檢查主機、連接埠和網路。',
      sshErrAuth:
        'SSH 驗證失敗。請將金鑰載入 ssh-agent（ssh-add），或在 ~/.ssh/config 中設定 IdentityFile——SanRemes 以非互動方式執行 ssh。',
      sshErrHostKey: '自上次連線以來主機金鑰已變更。請確認這是預期的，然後執行 ssh-keygen -R <host> 並重新連線。',
      sshErrNotInstalled:
        '遠端主機上未安裝 SanRemes。請在遠端安裝（curl -fsSL https://sanremes-agent.nousresearch.com/install.sh | sh）或設定 SanRemes 路徑。',
      sshErrPlatform: '不支援的遠端平台。SanRemes Desktop 的 SSH 模式支援 Linux、macOS 和 Windows 遠端主機。',
      sshErrTimeout: 'SSH 連線逾時。主機可能無法存取或處於睡眠狀態。',
      sshErrUpdateRequired: '使用 Desktop SSH 連線前，請更新遠端主機上的 SanRemes。',
    ...
    mcp: {
      ...
      deepLinkDescription: '一個連結要求將此 MCP 伺服器加入 SanRemes。請檢查下方的完整設定——它來自該連結，而非 SanRemes。',
    ...
    providers: {
      ...
      intro: '使用訂閱登入，無需複製 API 金鑰。SanRemes 會在應用程式中為您完成瀏覽器登入。',
      connected: '已連線',
      collapse: '收合',
      ...
      localEndpoint: {
        title: '本地 / 自訂端點',
        description: '將 SanRemes 指向任意 OpenAI 相容端點（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在載入提供方...'
    },
    sessions: {
  ...
  skills: {
    ...
    skillArchivedMessage: '可透過 sanremes curator restore 還原。'
  },
  ...
  starmap: {
    ...
    loading: '載入中…',
    emptyTitle: '尚無學習內容',
    emptyDesc: '當 SanRemes 為你的工作建立技能與記憶時，會顯示在這裡。'
  },
  agents: {
    close: '關閉代理',
    title: '派生樹',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '請重新啟動 SanRemes 以使用寵物功能。',
      empty: '沒有符合的寵物。',
      turnOff: '關閉',
      turnOn: '開啟',
      installed: '已安裝',
    ...
    generatePet: {
      ...
      staleBackend: '請更新 SanRemes 以生成寵物。',
      backgroundHint: '你可以關閉此視窗——完成後 SanRemes 會通知你。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: '設定 SanRemes 桌面端' },
    ...
    sanremesActiveSessions: (version, count) => `SanRemes ${version} · 活躍工作階段 ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'Bot 使用者 ID', placeholder: '@sanremes:example.org' },
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: '此設定檔中的工作階段將在你指定的遠端 SanRemes 上執行，而不是這台電腦。',
      urlLabel: '遠端位址',
    ...
    deleting: '刪除中…',
    createDesc: '設定檔是獨立的 SanRemes 環境：各自擁有獨立的設定、技能和 SOUL.md。',
    nameLabel: '名稱',
    cloneFrom: '複製來源',
    cloneFromNone: '無（空白）',
  ...
  cron: {
    ...
    newCron: '新排程工作',
    emptyDescNew: '按 cron 表達式排程一個提示詞。SanRemes 會執行它，並將結果傳送至您選擇的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '無法建立專案',
      staleBackend: '請更新 SanRemes 後端以建立專案——目前後端比桌面應用舊（設定 → 更新 → 後端）。',
      deleteConfirm: '這會從 SanRemes 中移除已儲存的專案。檔案、git 儲存庫和工作樹維持不變。',
      startWork: '新增工作樹',
      ...
      worktreeStaleBackend: '請更新 SanRemes 後端以在此遠端連線上建立工作樹 —— 該後端早於 git 工作樹 API。',
  ...
  composer: {
    ...
    placeholderStarting: '正在啟動 SanRemes...',
    placeholderReconnecting: '正在重新連線至 SanRemes…',
    ...
    attachUrlDesc: 'SanRemes 將擷取該頁面並作為此回合的脈絡。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 SanRemes 無法在應用程式內自行更新。',
    connectionRetry: '請檢查網路連線後重試。',
    latestBody: '您正在執行最新版本。',
    latestBodyBackend: '後端正在執行最新版本。',
    allSetTitle: '已是最新版本',
    availableTitle: '有可用更新',
    availableBody: '新版 SanRemes 已可安裝。',
    availableTitleBackend: '後端有可用更新',
    availableBodyBackend: '已連接的 SanRemes 後端有新版本可安裝。',
    availableBodyNoChangelog: '已有新版本可用。此安裝方式無法顯示更新日誌。',
    updateNow: '立即更新',
    maybeLater: '稍後再說',
    moreChanges: count => `另有 ${count} 項變更。`,
    manualTitle: '從終端機更新',
    manualBody: '您是從命令列安裝的 SanRemes，因此更新也需要在那裡執行。請將此指令貼到終端機：',
    manualPickedUp: '下次啟動 SanRemes 時會使用新版本。',
    guiSkewTitle: '請更新桌面應用程式',
    guiSkewBody:
      '後端已更新，但此桌面應用程式套件未變更。請更新或重新安裝 SanRemes 桌面應用程式（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody:
      'SanRemes 更新程式會在自己的視窗中接管，並在完成後自動重新開啟 SanRemes。更新期間請勿自行重新開啟 SanRemes。',
    applyingBodyBackend: '遠端後端正在套用更新並將重新啟動。恢復後 SanRemes 會自動重新連線。',
    applyingClose: '此視窗會在更新期間關閉，隨後 SanRemes 會自動重新開啟。',
    errorTitle: '更新未完成',
    errorBody: '沒有資料遺失。您可以現在重試。',
    blockerTitle: '關閉本機預覽以更新 SanRemes？',
    blockerBody: 'SanRemes 需要在更新前停止這些本機預覽。這不會修改或刪除您的檔案。',
    foreignBlockerTitle: '關閉其他處理程序以更新 SanRemes',
    foreignBlockerBody:
      'SanRemes 無法安全地自動關閉這些處理程序。請關閉擁有這些處理程序的應用程式、終端機或服務，然後重試更新。',
    mixedBlockerBody: 'SanRemes 可以關閉下方列出的本機預覽。其他處理程序必須手動關閉，更新才能繼續。',
  ...
  install: {
    ...
    setupChoiceTitle: '設定 SanRemes Desktop',
    setupChoiceDesc: '將此應用程式連線到您已執行的 SanRemes 閘道，或在這台電腦上本機安裝 SanRemes。',
    connectExistingTitle: '連線到現有 SanRemes',
    connectExistingShort: '連線現有環境',
    connectExistingDesc: '使用工作階段權杖或瀏覽器登入連線遠端後端。不會啟動本機安裝。',
    installLocalTitle: '本機安裝 SanRemes',
    installLocalDesc: '下載 SanRemes、建立 Python 環境，並在這台電腦上執行後端。',
    localStartUnavailable: '無法啟動本機安裝。請重新啟動 SanRemes Desktop 後再試一次。',
    remoteSetupTitle: '連線到現有 SanRemes',
    remoteSetupDesc: '輸入閘道 URL。SanRemes Desktop 會偵測需要權杖還是瀏覽器登入。',
    remoteUrlTitle: '閘道 URL',
    remoteUrlDesc: '使用 SanRemes 閘道的基礎 URL；遠端位址請包含 https://。',
    remoteUrlPlaceholder: 'https://gateway.example.com/sanremes',
    probing: '正在偵測閘道驗證方式...',
    probeError: '無法連線到該 SanRemes 閘道。',
    ...
    backToSetup: '返回',
    failedTitle: '安裝失敗',
    settingUpTitle: '正在設定 SanRemes Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某個安裝步驟失敗。在 Windows 上，如果另一個 SanRemes CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 SanRemes 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
    activeDesc: '這是一次性設定。SanRemes 安裝程式正在下載相依套件並設定您的電腦。之後啟動會略過此步驟。',
  ...
  onboarding: {
    headerTitle: '開始設定 SanRemes Agent',
    headerDesc: '連線模型提供方即可開始聊天。大多數選項只需一次點擊。',
    preparingInstall: 'SanRemes 正在完成安裝。首次執行通常不到一分鐘。',
    starting: '正在啟動 SanRemes…',
    ...
    recommended: '建議',
    connected: '已連線',
    featuredPitch: '一個訂閱，300+ 前沿模型 — 執行 SanRemes 的建議方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自託管',
        description: '將 SanRemes 指向本機或自託管的 OpenAI 相容端點（vLLM、llama.cpp、Ollama 等）。'
      }
    },
    backToSignIn: '返回登入',
    getKey: '取得金鑰',
    ...
    flowSubtitles: {
      ...
      device_code: '在瀏覽器中開啟驗證頁面 — SanRemes 會自動連線',
    ...
    authorizeThere: '請在那裡授權 SanRemes。',
    copyAuthCode: '複製授權碼並貼到下方。',
    pasteAuthCode: '貼上授權碼',
    reopenAuthPage: '重新開啟授權頁面',
    autoBrowser: provider => `已在瀏覽器中開啟 ${provider}。請在那裡授權 SanRemes，連線會自動完成，無需複製或貼上。`,
  ...
  preview: {
    ...
    largeTitle: '此檔案較大',
    largeBody: (label, size) => `${label} 大小為 ${size}。SanRemes 只會顯示前 512 KB。`,
    previewAnyway: '仍然預覽',
    ...
    web: {
      ...
      failedToLoad: '預覽載入失敗',
      tryAgain: '重試',
      restarting: 'SanRemes 正在重新啟動...',
      askRestart: '請 SanRemes 重新啟動伺服器',
      lookingRestart: taskId => `SanRemes 正在尋找要重新啟動的預覽伺服器 (${taskId})`,
      restartingTitle: '正在重新啟動預覽伺服器',
      restartingMessage: 'SanRemes 正在背景執行。可在預覽主控台查看進度。',
      ...
      blankPageBody: '在上方輸入網址開始瀏覽，或請 SanRemes 開啟頁面。',
      finishedRestarting: message => `SanRemes 已完成預覽伺服器重新啟動${message ? `：${message}` : ''}`,
      failedRestarting: message => `伺服器重新啟動失敗：${message}`,
      unknownError: '未知錯誤',
      restartedTitle: '預覽伺服器已重新啟動',
      reloadingNow: '正在重新載入預覽。',
      restartFailedTitle: '預覽重新啟動失敗',
      restartFailedMessage: 'SanRemes 無法重新啟動伺服器。',
      stillWorking: 'SanRemes 仍在執行，但尚未收到重新啟動結果。伺服器指令可能正在前台執行。',
  ...
  assistant: {
    thread: {
      ...
      showEarlier: '顯示較早的訊息',
      loadingResponse: 'SanRemes 正在載入回覆',
    ...
    approval: {
      gatewayDisconnected: 'SanRemes 閘道未連線',
      sendFailed: '無法傳送核准回應',
      run: '執行',
      command: '指令',
      moreOptions: '更多核准選項',
      ...
      alwaysDescription: pattern =>
        `這會將「${pattern}」模式加入永久允許清單（~/.sanremes/config.yaml）。SanRemes 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
      alwaysAllow: '一律允許'
    },
    clarify: {
      notReady: '澄清請求尚未就緒',
  ...
  prompts: {
    ...
    sudoTitle: '管理員密碼',
    sudoDesc: 'SanRemes 需要您的 sudo 密碼來執行特權指令。它只會傳送給您的本機代理。',
    sudoPlaceholder: 'sudo 密碼',
    secretTitle: '需要密鑰',
    secretDesc: 'SanRemes 需要一個憑證才能繼續。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 SanRemes Desktop 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 SanRemes Desktop 以儲存圖片',
    ...
    handoff: {
      ...
      timedOut: '等待閘道逾時。`sanremes gateway` 是否正在執行？'
    }
  },
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '教一次就夠',
        text: '技能是一組說明檔，SanRemes 會在需要時自行載入。'
      },
      messaging: {
        title: '離開電腦也能用',
      ...
      profiles: {
        title: '設定檔彼此獨立',
        text: '每個都是獨立的 SanRemes — 自己的金鑰、記憶與工作階段。'
      },
      'composer-mentions': {
        title: '附件與指令',
