...
import { getSanRemesConfigRecord, type SanRemesConfigRecord, saveSanRemesConfig } from '@/sanremes'
...
export interface I18nConfigClient {
  getConfig: () => Promise<SanRemesConfigRecord>
  saveConfig: (config: SanRemesConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.sanremesDesktop?.api) {
      return Promise.resolve({})
    }

    return getSanRemesConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.sanremesDesktop?.api) {
    ...
    return saveSanRemesConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: SanRemesConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: SanRemesConfigRecord, locale: Locale): SanRemesConfigRecord {
