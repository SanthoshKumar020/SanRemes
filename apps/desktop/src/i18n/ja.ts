...
export const ja = defineLocale({
  ...
  boot: {
    ready: 'SanRemes Desktop の準備ができました',
    desktopBootFailedWithMessage: message => `デスクトップの起動に失敗しました: ${message}`,
    steps: {
      connectingGateway: 'ライブデスクトップゲートウェイに接続中',
      loadingSettings: 'SanRemes の設定を読み込み中',
      loadingSessions: '最近のセッションを読み込み中',
      retryingRemoteBackend: 'リモート SanRemes バックエンドに再接続中…',
      startingDesktopConnection: 'デスクトップ接続を開始中',
      startingSanRemesDesktop: 'SanRemes Desktop を起動中…'
    },
    errors: {
      backgroundExited: 'SanRemes バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に SanRemes バックグラウンドプロセスが終了しました。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'SanRemes バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    installMethodUnsupportedTitle: 'サポート対象外のインストール方法',
    updateSanRemes: 'SanRemes を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。SanRemes Desktop を再起動してください。',
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'SanRemes が応答を待っています。',
      turnDoneTitle: 'SanRemes が完了しました',
      turnDoneBody: '',
  ...
  sendDiagnostics: {
    ...
    failedHint:
      'ターミナルから `sanremes debug share --nous` を実行するか、`sanremes debug share --local` でアップロードせずにレポートを表示することもできます。',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を SanRemes のデフォルトに戻しますか？',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は SanRemes がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'SanRemes が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'SanRemes がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
        ...
        plugin: {
          label: 'プラグイン通知',
          description: 'SanRemes がバックグラウンドの間に、デスクトッププラグインが通知を送信しました。'
        }
      },
      test: 'テスト通知を送信',
      testTitle: 'SanRemes',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、SanRemes をシステム設定に合わせます。',
      ...
      reactionsDesc:
        'iMessage風の絵文字タップバック — メッセージにリアクションでき、SanRemesもあなたのメッセージにリアクションします。',
      tipsTitle: 'アプリ内ヒント',
      tipsDesc:
        'アプリの一部を指す小さな吹き出し。待機中にときどき、また役に立つときは SanRemes からも表示します。閉じたヒントは二度と表示されません。',
      tipsReset: (count: number) => `閉じた${count}件のヒントを元に戻す`,
      toursTitle: 'ガイドツアー',
      toursDesc: '画面を暗くして各ステップを強調しながら、SanRemes がアプリを案内します。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、SanRemes の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。SanRemes を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'SanRemes がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'SanRemes が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'SanRemes が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを SanRemes がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから SanRemes 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'SanRemes ランタイムは更新されましたが、デスクトップアプリ自体は古いビルドのままです。アプリを更新するまで、新しいインターフェース機能(Bot Mode など)は表示されません。下の更新を実行してアプリを再ビルドしてください。それでもこの警告が消えない場合は、最新のデスクトップインストーラーから再インストールしてください。',
      ...
      automaticUpdatesDesc: 'SanRemes はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    quickEntry: {
      enabledTitle: 'クイック入力',
      enabledDesc:
        'グローバルショートカットで小さな入力欄をどこからでも呼び出し、SanRemes を開かずにプロンプトを送信します。',
    ...
    gateway: {
      ...
      intro:
        'SanRemes Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している SanRemes バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      envOverrideTitle: '環境変数がこのデスクトップセッションを制御しています。',
      envOverrideDesc:
        '保存された設定を使用するには SANREMES_DESKTOP_REMOTE_URL と SANREMES_DESKTOP_REMOTE_TOKEN の設定を解除してください。',
      localTitle: 'ローカルゲートウェイ',
      localDesc:
        'ローカルホストでプライベートな SanRemes バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      remoteTitle: 'リモートゲートウェイ',
      remoteDesc:
        'このデスクトップシェルをリモートの SanRemes バックエンドに接続します。ホスト型ゲートウェイは OAuth またはユーザー名とパスワードを使用します。自己ホスト型はセッショントークンを使用する場合があります。',
      remoteUrlTitle: 'リモート URL',
      remoteUrlDesc:
        'リモートダッシュボードバックエンドのベース URL。/sanremes などのパスプレフィックスもサポートしています。',
      ...
      restartingMessage: 'SanRemes Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · SanRemes ${version}` : ''} に接続しました`,
      ...
      sshTitle: 'SSH で接続',
      sshDesc:
        'SanRemes は SSH 経由でリモート上に起動され、このアプリにトンネルされます。リモート側で何かを起動・公開する必要はありません。ホストへの鍵ベースの SSH アクセスが前提です。',
      ...
      sshSanRemesPathTitle: 'SanRemes パス（任意）',
      sshSanRemesPathDesc: 'リモートの sanremes バイナリへのフルパス。空欄 = 自動検出。',
      sshSanRemesPathPlaceholder: '自動検出',
      sshTestConnection: 'SSH をテスト',
      sshConnect: '接続',
      sshButtonsHint: '「保存」は次回起動時に適用され、「接続」は今すぐ再接続します。',
      sshReachable: (host, platform) => `接続可能: ${host}（${platform}）— SanRemes を検出`,
      sshIncompleteHost: '接続する前に SSH ホストを入力してください。',
      sshErrUnreachable: 'SSH でそのホストに到達できませんでした。ホスト、ポート、ネットワークを確認してください。',
      sshErrAuth:
        'SSH 認証に失敗しました。鍵を ssh-agent に読み込む（ssh-add）か、~/.ssh/config に IdentityFile を設定してください。SanRemes は非対話的に ssh を実行します。',
      sshErrHostKey:
        '前回の接続以降、ホスト鍵が変更されています。想定どおりか確認し、ssh-keygen -R <host> を実行してから再接続してください。',
      sshErrNotInstalled:
        'リモートホストに SanRemes がインストールされていません。リモートでインストールする（curl -fsSL https://sanremes-agent.nousresearch.com/install.sh | sh）か、SanRemes パスを設定してください。',
      sshErrPlatform:
        'サポートされていないリモートプラットフォームです。SanRemes Desktop の SSH モードは Linux、macOS、Windows のリモートホストに対応しています。',
      sshErrTimeout: 'SSH 接続がタイムアウトしました。ホストが到達不能、またはスリープ中の可能性があります。',
      sshErrUpdateRequired: 'Desktop SSH で接続する前に、リモートホストの SanRemes を更新してください。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'リンクがこの MCP サーバーを SanRemes に追加するよう要求しました。下の設定はリンク側から来たものです。内容を必ず確認してください。',
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。SanRemes がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
  ...
  skills: {
    ...
    skillArchivedMessage: 'sanremes curator restore で復元できます。'
  },
  ...
  starmap: {
    ...
    emptyDesc: 'SanRemes がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
    close: 'エージェントを閉じる',
    title: 'スポーンツリー',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには SanRemes を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには SanRemes を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら SanRemes が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'SanRemes デスクトップを設定' },
    ...
    sanremesActiveSessions: (version, count) => `SanRemes ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ボットユーザー ID', placeholder: '@sanremes:example.org' },
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: 'このプロファイルのセッションは、このパソコンではなく指定したリモートの SanRemes で実行されます。',
      urlLabel: 'リモートアドレス',
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した SanRemes 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      saveFailed: 'SanRemes はモデルの変更を保存しませんでした。',
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。SanRemes が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには SanRemes バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'SanRemes から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
      ...
      worktreeStaleBackend:
        'このリモート接続でワークツリーを作成するには SanRemes バックエンドを更新してください — git ワークツリー API 以前のバージョンです。',
  ...
  composer: {
    ...
    placeholderStarting: 'SanRemes を起動中...',
    placeholderReconnecting: 'SanRemes に再接続中…',
    ...
    attachUrlDesc: 'SanRemes がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'SanRemes にコミットと PR を任せる',
  ...
  updates: {
    stages: {
      ...
      pull: 'もうすぐ完了…',
      pydeps: '仕上げ中…',
      update: 'SanRemes を更新中…',
      rebuild: 'デスクトップアプリを再ビルド中…',
      restart: 'SanRemes を再起動中…',
      done: '更新が完了しました',
      manual: 'ターミナルから更新',
    ...
    unsupportedMessage: 'このバージョンの SanRemes はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの SanRemes をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の SanRemes バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'SanRemes をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'SanRemes は次回起動時に新しいバージョンを読み込みます。',
    guiSkewTitle: 'デスクトップアプリを更新してください',
    guiSkewBody:
      'バックエンドは更新されましたが、このデスクトップアプリのパッケージは変更されていません。一致させるために SanRemes デスクトップアプリ（AppImage / .deb / .rpm）を更新または再インストールしてください。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody:
      'SanRemes アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に SanRemes を再度開きます。更新中はご自分で SanRemes を開き直さないでください。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると SanRemes が自動的に再接続します。',
    applyingClose: 'このウィンドウは更新中に閉じ、その後 SanRemes が自動的に再度開きます。',
    errorTitle: '更新が完了しませんでした',
    errorBody: 'ご安心ください。何も失われていません。今すぐ再試行できます。',
    blockerTitle: 'SanRemes を更新するためにローカルプレビューを閉じますか？',
    blockerBody:
      '更新する前に、これらのローカルプレビューを停止する必要があります。ファイルが変更または削除されることはありません。',
    foreignBlockerTitle: '他のプロセスを閉じて SanRemes を更新',
    foreignBlockerBody:
      'SanRemes はこれらのプロセスを安全に自動終了できません。各プロセスを所有するアプリ、ターミナル、またはサービスを閉じてから、もう一度更新してください。',
    mixedBlockerBody:
      'SanRemes は以下のローカルプレビューを閉じることができます。更新を続けるには、他のプロセスを手動で閉じる必要があります。',
  ...
  install: {
    ...
    oneTimeTitle: 'SanRemes には一度限りのインストールが必要です',
    ...
    setupChoiceTitle: 'SanRemes Desktop をセットアップ',
    setupChoiceDesc:
      'すでに実行している SanRemes ゲートウェイに接続するか、このコンピューターに SanRemes をローカルインストールします。',
    connectExistingTitle: '既存の SanRemes に接続',
    connectExistingShort: '既存環境に接続',
    connectExistingDesc:
      'セッショントークンまたはブラウザーサインインでリモートバックエンドを使用します。ローカルインストールは開始されません。',
    installLocalTitle: 'SanRemes をローカルにインストール',
    installLocalDesc: 'SanRemes をダウンロードし、Python 環境を作成して、このコンピューターでバックエンドを実行します。',
    localStartUnavailable:
      'ローカルインストールを開始できません。SanRemes Desktop を再起動して、もう一度お試しください。',
    remoteSetupTitle: '既存の SanRemes に接続',
    remoteSetupDesc:
      'ゲートウェイ URL を入力してください。SanRemes Desktop がトークンとブラウザーサインインのどちらが必要かを検出します。',
    remoteUrlTitle: 'ゲートウェイ URL',
    remoteUrlDesc: 'SanRemes ゲートウェイのベース URL を使用します。リモートの場合は https:// を含めてください。',
    remoteUrlPlaceholder: 'https://gateway.example.com/sanremes',
    probing: 'ゲートウェイ認証方式を検出中...',
    probeError: 'その SanRemes ゲートウェイに到達できませんでした。',
    identityProvider: 'ID プロバイダー',
    authTitle: '認証',
    ...
    settingUpTitle: 'SanRemes Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の SanRemes CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の SanRemes インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。SanRemes インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'SanRemes Agent のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'SanRemes はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'SanRemes を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — SanRemes を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に SanRemes を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — SanRemes が自動接続します',
    ...
    authorizeThere: 'そこで SanRemes を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。SanRemes をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。SanRemes は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'SanRemes を再起動中...',
      askRestart: 'SanRemes にサーバーの再起動を依頼',
      lookingRestart: taskId => `SanRemes は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'SanRemes はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      blankPageBody: '上のアドレス欄に入力するか、SanRemes にページを開くよう頼んでください。',
      finishedRestarting: message =>
        `SanRemes がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'SanRemes がサーバーを再起動できませんでした。',
      stillWorking:
        'SanRemes はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'SanRemes が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'SanRemes ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.sanremes/config.yaml) に追加されます。SanRemes はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    sudoTitle: '管理者パスワード',
    sudoDesc:
      'SanRemes は特権コマンドを実行するために sudo パスワードが必要です。ローカルエージェントにのみ送信されます。',
    sudoPlaceholder: 'sudo パスワード',
    secretTitle: 'シークレットが必要です',
    secretDesc: 'SanRemes は続行するための認証情報が必要です。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには SanRemes Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには SanRemes Desktop を再起動してください',
    ...
    handoff: {
      ...
      timedOut: 'ゲートウェイの待機がタイムアウトしました。`sanremes gateway` は起動していますか？'
    }
  },
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '一度教えれば覚えます',
        text: 'スキルは手順書のフォルダで、必要な場面で SanRemes が自分で読み込みます。'
      },
      messaging: {
        title: 'デスクを離れても SanRemes',
        text: 'Telegram、Discord、Slack などに接続。同じエージェント、同じ記憶のままです。'
      },
      artifacts: {
        title: 'SanRemes が作ったものすべて',
      ...
      profiles: {
        ...
        text: 'それぞれが独自のキー・メモリ・セッションを持つ、別の SanRemes です。'
      },
      'composer-mentions': {
        title: 'ファイルとコマンド',
