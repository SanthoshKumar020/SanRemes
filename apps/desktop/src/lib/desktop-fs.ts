import { sanremesApi } from '@/api/client'
import type {
  SanRemesConnection,
  SanRemesReadDirResult,
  SanRemesReadFileTextResult,
  SanRemesSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: SanRemesConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: SanRemesConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<SanRemesReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<SanRemesReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<SanRemesReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<SanRemesReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.sanremesDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: SanRemesSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
