...
describe('performScopedFind', () => {
  ...
  it('does not re-wrap when the query differs only in case (fast path, triage #81778)', () => {
    ...
    const surface = plantSurface('surface', '<p>SanRemes SanRemes</p>')
    performScopedFind(surface, 'sanremes', { forward: true, findNext: false })

    const before = [...surface.querySelectorAll('mark.find-hit')]
    const result = performScopedFind(surface, 'sanremes', { forward: true, findNext: true })
  ...
  it('re-wraps when a stale mark from an earlier query survives (fast path, #81778 review)', () => {
    ...
    const surface = plantSurface('surface', '<p>sanremes</p>')
    performScopedFind(surface, 'sanremes', { forward: true, findNext: false })
    ...
    surface.insertAdjacentHTML('beforeend', '<mark class="find-hit">SanRemes</mark>')
    surface.insertAdjacentHTML('beforeend', '<p>sanremes</p>')
    ...
    expect(marks.every(mark => mark.textContent.toLowerCase() === 'sanremes')).toBe(true)
  })
