...
function installDesktopBridge(partial: Partial<Window['sanremesDesktop']> = {}) {
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['sanremesDesktop']['fetchLinkTitle'] })
...
describe('external link helpers', () => {
  ...
  it('opens a web link in the in-app browser', async () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['sanremesDesktop']['openExternal'] })
  ...
  it('prefixes a pretty link to a known host with its brand glyph', () => {
    ...
    const url = 'https://github.com/NousResearch/sanremes-agent/pull/123'
