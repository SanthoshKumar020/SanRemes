 ...
 * `sanremes://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `sanremes://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `sanremes://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type SanRemesOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const SANREMES_PROTOCOL = 'sanremes:'
...
export function normalizeSanRemesOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('sanremes://') || trimmed.startsWith(`${SANREMES_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // sanremes://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // sanremes://index-network/intent/1 → /index-network/intent/1
...
export function resolveSanRemesOpenPath(target: SanRemesOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeSanRemesOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeSanRemesOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeSanRemesOpenString(target.path)
...
export function pathFromSanRemesDeepLink(
  ...
  if (kind === 'open') {
    return resolveSanRemesOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveSanRemesOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `sanremes://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromSanRemesDeepLink('open', name, params)
}
