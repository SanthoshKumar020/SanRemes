...
//
// When someone is in a hands-free "Hey SanRemes" voice chat, the natural way to
...
// phrase (optionally addressed to SanRemes), so a real turn that merely contains
...
// Optional address prefixes so "sanremes stop" / "ok stop" / "hey sanremes, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey sanremes', 'hey sanremes,', 'sanremes', 'sanremes,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("sanremes") is not a stop command on its own.
      continue
    }
 ...
 * to SanRemes). Returns false for anything that merely contains "stop" as part of
