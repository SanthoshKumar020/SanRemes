...
// SanRemes is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.sanremesMode
 ...
 * the only carriers of SanRemes' palette — are dropped so the paste target's
