...
import { getApiRequestConnection, getApiRequestProfile, sanremesApi } from '@/sanremes'
...
export async function fetchVoiceClientConfig(): Promise<null | VoiceClientConfig> {
  ...
  const promise = (async () => {
    try {
      // sanremesApi carries connectionScoped(); profileScoped() adds the
      // profile — the same routing every relay audio call uses, so the
      // config comes from the backend the user is actually talking to.
      const response = await sanremesApi<{ ok: boolean } & VoiceClientConfig>({
        ...profileScoped(),
