import type {
  SanRemesGitBaseBranch,
  SanRemesGitBranch,
  SanRemesGitWorktree,
  SanRemesRepoPullRequests,
  SanRemesRepoStatus,
  SanRemesReviewList,
  SanRemesReviewShipInfo
} from '@/global'
import { sanremesApi } from '@/sanremes'
...
// (window.sanremesDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['sanremesDesktop']>['git']>

function desktopApi<T>(path: string, body?: Record<string, unknown>): Promise<T> {
  const desktop = window.sanremesDesktop

  if (!desktop) {
    throw new Error('SanRemes Desktop bridge is unavailable')
  }

  return sanremesApi<T>(
...
const remoteGit: GitBridge = {
  worktreeList: async repoPath =>
    (await gitGet<{ worktrees: SanRemesGitWorktree[] }>('worktrees', { path: repoPath })).worktrees,
  ...
  branchList: async repoPath =>
    (await gitGet<{ branches: SanRemesGitBranch[] }>('branches', { path: repoPath })).branches,

  baseBranchList: async repoPath =>
    (await gitGet<{ branches: SanRemesGitBaseBranch[] }>('base-branches', { path: repoPath })).branches,

  repoStatus: repoPath => gitGet<SanRemesRepoStatus | null>('status', { path: repoPath }),
  ...
  review: {
    list: (repoPath, scope, baseRef) =>
      gitGet<SanRemesReviewList>('review/list', { base: baseRef, path: repoPath, scope }),
    ...
    shipInfo: repoPath => gitGet<SanRemesReviewShipInfo>('review/ship-info', { path: repoPath }),

    prList: (repoPath, branches, numbers) =>
      gitPost<SanRemesRepoPullRequests>('review/pr-list', { branches, numbers: numbers ?? [], path: repoPath }),
...
export function desktopGit(): GitBridge | undefined {
  ...
  return isDesktopFsRemoteMode() ? remoteGit : window.sanremesDesktop?.git
}
