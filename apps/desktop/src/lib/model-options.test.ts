...
import { getGlobalModelOptions } from '@/sanremes'
...
const globalOptions = { model: 'sanremes-4', provider: 'nous', providers: [] }
...
describe('requestModelOptions', () => {
  ...
  it('recovers an empty gateway catalog through profile-scoped REST without replacing the session selection', async () => {
    const gatewayPayload = { model: 'sanremes-local', provider: 'sanremes-local' }

    const restPayload = {
      model: 'profile-default',
      provider: 'openai-codex',
      providers: [{ models: ['sanremes-local'], name: 'SanRemes Local vLLM', slug: 'sanremes-local' }]
    }
    ...
    await expect(requestModelOptions({ gateway: gateway as never, sessionId: 'session-1' })).resolves.toEqual({
      ...restPayload,
      model: 'sanremes-local',
      provider: 'sanremes-local'
    })
  ...
  it('keeps the gateway result when both catalog paths have no selectable models', async () => {
    const gatewayPayload = { model: 'sanremes-local', provider: 'sanremes-local', providers: [] }
  ...
  it('scopes REST recovery to the catalog owner profile', async () => {
    const restPayload = {
      model: 'berry-local',
      provider: 'sanremes-local',
      providers: [{ models: ['berry-local'], name: 'SanRemes Local', slug: 'sanremes-local' }]
    }
...
describe('manualPickRemoved', () => {
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'nous', 'sanremes-4')).toBe(false)
  })
