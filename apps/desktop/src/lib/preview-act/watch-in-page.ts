...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __sanremesWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__sanremesWatch || (win.__sanremesWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('sanremes-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('sanremes-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __sanremesWatchTag?: number }).__sanremesWatchTag
