...
beforeEach(() => {
  ...
  document.querySelector('sanremes-watch')?.remove()
  delete (window as unknown as Record<string, unknown>).__sanremesWatch
})

/** The overlay lives in a closed shadow root, so a test can only see the host. */
const host = () => document.querySelector('sanremes-watch')
...
const drawn = () =>
  (window as unknown as { __sanremesWatch: { parts: Record<string, HTMLElement> & { shadow: ShadowRoot } } })
    .__sanremesWatch.parts
...
describe('watchInPage', () => {
  it('mounts one host on documentElement and reuses it across stages', () => {
    ...
    expect(document.querySelectorAll('sanremes-watch')).toHaveLength(1)
  ...
  it('rebuilds its chrome when the injected source changes underneath it', () => {
    ...
    const stamp = (value: number) => {
      ;(window as unknown as Record<string, unknown>).__sanremesWatchTag = value
    }
