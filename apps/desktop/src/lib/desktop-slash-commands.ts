...
/** Why a known SanRemes command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active SanRemes profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownSanRemesSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * SanRemes' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownSanRemesSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with SanRemes and were never asked for. It is
