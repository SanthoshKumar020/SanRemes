 ...
 *   (`@sanremes/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default SanRemesPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<sanremes home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<sanremes home>/plugins/<name>/desktop/
...
import { createPluginContext, type SanRemesPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.sanremes/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@sanremes/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @sanremes/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: SanRemesPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default SanRemesPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of sanremes-bots predating its adoption in-tree) must
...
//  - `<sanremes home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<sanremes home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
 ...
 *  backend's sanremes_home — #66899). `agentPluginsRoot` is optional: older
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.sanremes/plugins` is installed-but-inert until the
...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update SanRemes Desktop to load larger plugins"
    )
  }
...
async function resolveDiskPluginEntry(
  desktop: Window['sanremesDesktop'],
  folderPath: string,
