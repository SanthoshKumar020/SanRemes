 ...
 *    a `SanRemesPlugin` registers automatically (vite glob — drop a folder in).
 *    `sanremes-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `sanremes-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<sanremes home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<sanremes home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: SanRemesPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default SanRemesPlugin export — skipped`)
