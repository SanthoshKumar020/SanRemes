 ...
 * `SanRemesPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/sanremes'
...
export type { PluginRestOptions } from '@/sanremes'
 ...
 *  under `sanremes.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.sanremesDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from SanRemes — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `sanremes://index-network/intent/1`),
...
export interface SanRemesPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `sanremes.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.sanremesDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.sanremesDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.sanremesDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.sanremesDesktop
