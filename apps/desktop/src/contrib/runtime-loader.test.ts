...
import type { SanRemesReadDirResult } from '@/global'
import type * as SanRemesModule from '@/sanremes'
...
// getStatus would supply the connected backend's sanremes_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ sanremes_home: '/remote/box/.sanremes' }))

vi.mock('@/sanremes', async importActual => ({
  ...(await importActual<typeof SanRemesModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const agentPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<SanRemesReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { sanremesDesktop: unknown }).sanremesDesktop = {
    agentPluginsRoot,
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { sanremesDesktop?: unknown }).sanremesDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend sanremes_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.sanremes/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.sanremes/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.sanremes/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.sanremes/plugins')
    // The remote backend's sanremes_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.sanremes/desktop-plugins')
  })
  ...
  it('treats a package without a Desktop half as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.sanremes/plugins') {
        return { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.sanremes/plugins/my-feature' }] }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.sanremes/plugins/my-feature')
    expect(readDir).not.toHaveBeenCalledWith('/local/.sanremes/plugins/my-feature/desktop')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    agentPluginsRoot.mockResolvedValue('')
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.sanremes/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.sanremes/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.sanremes/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.sanremes/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.sanremesDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.sanremes/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.sanremes/plugins') {
        return { entries: [{ isDirectory: true, name: 'uni', path: '/local/.sanremes/plugins/uni' }] }
      }

      if (dir === '/local/.sanremes/plugins/uni') {
        return {
          entries: [{ isDirectory: true, name: 'desktop', path: '/local/.sanremes/plugins/uni/desktop' }]
        }
      }

      if (dir === '/local/.sanremes/plugins/uni/desktop') {
        return {
            ...
            ? [
                {
                  isDirectory: false,
                  name: 'plugin.js',
                  path: '/local/.sanremes/plugins/uni/desktop/plugin.js'
                }
              ]
            : []
        }
    ...
    try {
      ...
      // ~/.sanremes/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend sanremes_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.sanremes/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.sanremes/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.sanremes/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.sanremes/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.sanremes/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.sanremes/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.sanremesDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.sanremes/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.sanremes/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.sanremes/desktop-plugins/huge/plugin.js'
      })
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'sanremes-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "sanremes-bots", name: "Bot Mode", register() {} }',
        'sanremes-bots',
        { file: '/local/.sanremes/desktop-plugins/sanremes-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['sanremes-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['sanremes-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.sanremes/desktop-plugins/sanremes-bots/plugin.js'
      })
