...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when SanRemes doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in sanremes_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `sanremes model` uses — instead of
...
export interface SanRemesConfig {
  agent?: {
...
export type SanRemesConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `sanremes model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) SanRemes simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (sanremes.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors sanremes_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with SanRemes, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `sanremes tools` active-provider detection). */
  is_active: boolean
 ...
 *  not SanRemes); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** sanremes-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** sanremes-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/sanremes/update/check` — the backend's own update state.
