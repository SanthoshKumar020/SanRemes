...
// (#73044). Exercises the REAL sanremes + store/gateway + store/profile chain:
...
const { pluginSocket, setApiRequestConnection, setApiRequestProfile } = await import('@/sanremes')
...
const conn = (over: Partial<SanRemesConnection> = {}): SanRemesConnection =>
  ({
    authMode: 'oauth',
