...
<p align="center">
  <a href="https://github.com/NousResearch/sanremes-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [SanRemes Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other SanRemes surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to SanRemes and hear it back.</td></tr>
...
### Install with SanRemes (recommended)

Already have the SanRemes CLI? Just run:

```bash
sanremes desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing SanRemes gateway or install SanRemes locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the SanRemes Desktop website.](https://sanremes-agent.nousresearch.com/).
...
```bash
# throwaway SANREMES_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the SanRemes Agent runtime into `SANREMES_HOME`
(`~/.sanremes`, or `%LOCALAPPDATA%\sanremes` on Windows), using the same layout as a
CLI install.
...
- **SanRemes Agent** runs as a headless `sanremes serve` process and exposes the
...
1. `SANREMES_DESKTOP_SANREMES_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `SANREMES_DESKTOP_SANREMES`, or `sanremes` on `PATH`
5. a system Python that can import the SanRemes runtime
...
Desktop supports a managed local backend, explicit remote gateways, and SanRemes
...
screen offers **Connect to existing SanRemes** before starting the local installer.
...
terminal commands, and file operations run against the remote SanRemes host, not
...
```json
{
  "mode": "remote",
  "remote": {
    "url": "https://sanremes.example.com",
    "authMode": "token",
...
transport- or SanRemes-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-SanRemes-Session-Token`.
...
Boot logs land in `SANREMES_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\sanremes\sanremes-agent\.sanremes-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\sanremes\sanremes-agent\venv"
```

> The default SanRemes home on Windows is `%LOCALAPPDATA%\sanremes`. Set the `SANREMES_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://sanremes-agent.nousresearch.com/docs/)
