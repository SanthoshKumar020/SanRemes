...
How to build SanRemes Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another SanRemes surface can also
...
- A **runtime home change** (switching the underlying `SANREMES_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across SanRemes — do not assume
