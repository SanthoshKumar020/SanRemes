...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      throw new Error('This Desktop build cannot refresh OAuth WebSocket tickets. Update SanRemes Desktop and try again.')
    }
...
export interface SanRemesWebSocketUrlOptions {
...
export function buildSanRemesWebSocketUrl(options: SanRemesWebSocketUrlOptions): string {
