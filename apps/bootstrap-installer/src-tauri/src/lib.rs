//! SanRemes Setup — Tauri entrypoint.
//!
...
/// desktop app handing off via `SanRemes-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under SANREMES_HOME/logs/ so install
    ...
    // SanRemes is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "SanRemes installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // SanRemes is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "SanRemes" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "SanRemes" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_sanremes_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::sanremes_home().join("sanremes-agent");
                if bootstrap::sanremes_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_sanremes_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "sanremes already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_sanremes_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_sanremes_home,
            paths::open_log_dir,
        ])
        .run(tauri::generate_context!())
        .expect("error while running SanRemes Setup");
}
