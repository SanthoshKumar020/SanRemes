...
//!
//! Driven when the installer is launched as `SanRemes-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old SanRemes desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `sanremes update`
//!      or repair bootstrap can race locked files),
//!   2. run `sanremes update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in sanremes_cli/main.py),
//!   3. run `sanremes desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `sanremes update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_sanremes) and
...
/// `sanremes update` exit code meaning "another sanremes process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// sanremes_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `sanremes update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `sanremes update` claims
/// the same file (see `sanremes_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `sanremes update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in sanremes_cli/update_lock.py — all three read
...
/// a dashboard-spawned `sanremes update`) still blocks.
...
/// the marker name our PID — because a `sanremes update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    let sanremes_home = crate::paths::sanremes_home();
    let install_root = sanremes_home.join("sanremes-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_sanremes`
    ...
    //
    // The same marker is the cross-process update lock (sanremes_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `sanremes update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another SanRemes update is already running (PID {}, started {} ago). \
    ...
    let sanremes = resolve_sanremes(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the sanremes CLI under {}. Is SanRemes installed? \
    ...
    // async on Windows. If it still holds the venv shim, `sanremes update`
    ...
    // ---- stage 2: sanremes update -----------------------------------------
    // Pass --branch so `sanremes update` targets the branch this installer was
    ...
    // without --branch, `sanremes update` switches the checkout to `main` (a
    ...
    // --force skips `sanremes update`'s Windows running-exe guard (which would
    ...
    // time `sanremes update` runs there is no legitimate sanremes.exe to protect,
    // and the guard would only produce a false "SanRemes is still running" stop.
    //
    ...
    // probe only checks the sanremes.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all SanRemes windows" message.
    ...
    // Retry-once for the update-boundary crash. `sanremes update` lazily imports
    ...
    // second `sanremes update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close SanRemes" state
    // a retry can't fix.
    ...
    // checkout predating the SANREMES_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "SanRemes is still running. Close all SanRemes windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "sanremes update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::sanremes_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: sanremes desktop --build-only ----------------------------
    // `sanremes update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `sanremes update` already does above, and `sanremes update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `sanremes desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch SanRemes manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_sanremes_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for SanRemes to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // SanRemes.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] SanRemes still holding install files ({}); locating backend shims…",
                    format_locked_paths(&locked)
                ),
            );
            let shim = venv_sanremes(install_root);
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_sanremes(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("SanRemes.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("SanRemes.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Find processes running the exact `venv\Scripts\sanremes.exe` shim for this
/// installation. Windows image names are case-insensitive and the desktop is
/// also SanRemes.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
/// Spawn `sanremes <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `sanremes update` is precisely the shape that
...
/// Path to the venv sanremes shim under an install root, regardless of existence.
fn venv_sanremes(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("sanremes.exe")
    } else {
        install_root.join("venv").join("bin").join("sanremes")
    }
}

/// Resolve the sanremes CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `sanremes` on PATH.
fn resolve_sanremes(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_sanremes(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "sanremes.exe" } else { "sanremes" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let sanremes_home = crate::paths::sanremes_home();
    let mut envs = vec![(
        "SANREMES_HOME".to_string(),
        sanremes_home.as_os_str().to_os_string(),
    )];
    // `sanremes update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `sanremes update` child claims that SAME lock (sanremes_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("SanRemes is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in sanremes_cli/update_lock.py.
    envs.push((
        "SANREMES_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        sanremes_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_sanremes_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no SanRemes.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.sanremes-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.sanremes-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::sanremes_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_sanremes_is_under_install_root() {
        let root = Path::new("/x/sanremes-agent");
        let shim = venv_sanremes(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/sanremes-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/sanremes-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "SANREMES_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the sanremes update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_sanremes(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/sanremes-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.sanremes\sanremes-agent\venv\scripts\SANREMES.EXE"),
            Path::new(r"c:\users\tester\.sanremes\sanremes-agent\venv\Scripts\sanremes.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.sanremes\sanremes-agent\apps\desktop\SanRemes.exe"),
            Path::new(r"C:\Users\tester\.sanremes\sanremes-agent\venv\Scripts\sanremes.exe"),
        ));
    }
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".sanremes-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `sanremes update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(sanremesHome, child.pid) races ahead
    ...
    // checkout's `sanremes update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".sanremes-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running SanRemes, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/SanRemes.app"]),
            Some(PathBuf::from("/Applications/SanRemes.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("SanRemes.app");
        let tmp = base.join("SanRemes.app.sanremes-update-new");
        let old = base.join("SanRemes.app.sanremes-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("SanRemes.app");
        let tmp = base.join("SanRemes.app.sanremes-update-new"); // intentionally absent
        let old = base.join("SanRemes.app.sanremes-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("SanRemes.app");
        let tmp = base.join("SanRemes.app.sanremes-update-new"); // absent
        let old = base.join("SanRemes.app.sanremes-update-old");
        write_marker(&target, "OLD");
