...
//!
//! Mirrors `sanremes_constants.get_sanremes_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\sanremes
//!   macOS:   ~/.sanremes
//!   Linux:   ~/.sanremes  (override via $SANREMES_HOME)
//!
//! NOTE (macOS): Python's get_sanremes_home(), scripts/install.sh, and the
//! Electron desktop's resolveSanRemesHome() ALL use ~/.sanremes on macOS — there
...
/// Returns the canonical SanRemes home directory, respecting $SANREMES_HOME if set.
pub fn sanremes_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("SANREMES_HOME") {
    ...
    {
        // %LOCALAPPDATA%\sanremes — matches scripts/install.ps1's $SanRemesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("sanremes");
        }
    }

    // macOS + Linux + fallback: ~/.sanremes (matches Python get_sanremes_home(),
    // install.sh, and the Electron desktop's resolveSanRemesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".sanremes");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".sanremes")
}

pub fn log_dir() -> PathBuf {
    sanremes_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    sanremes_home().join("bootstrap-cache")
}
...
/// SANREMES_HOME so it survives repo checkout deletion (unlike anything under
/// sanremes-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\sanremes\sanremes-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_sanremes`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under SANREMES_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves SANREMES_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    sanremes_home().join(".sanremes-update-in-progress")
}
...
pub fn copy_self_to_sanremes_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SANREMES_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_SANREMES_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".sanremes-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SANREMES_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[sanremes-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SANREMES_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_sanremes_home() -> String {
    sanremes_home().to_string_lossy().into_owned()
}
