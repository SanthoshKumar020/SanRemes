...
//! SANREMES_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for SANREMES_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub sanremes_home: Option<String>,
}
...
/// Spawn the locally-built SanRemes desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_sanremes_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_sanremes_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built SanRemes desktop at {}. The desktop build step \
             may have been skipped or failed. Run `sanremes desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching SanRemes desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/SanRemes
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_sanremes_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "SanRemes.exe"),
            ("win-arm64-unpacked", "SanRemes.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/SanRemes.app/Contents/MacOS", "SanRemes"),
            ("mac-arm64/SanRemes.app/Contents/MacOS", "SanRemes"),
        ]
    } else {
        &[("linux-unpacked", "sanremes")]
    };
...
pub(crate) fn resolve_sanremes_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_sanremes_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../SanRemes.app/Contents/MacOS/SanRemes -> .../SanRemes.app
...
/// path so a bare re-open just opens SanRemes instead of re-running setup.
pub(crate) fn sanremes_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".sanremes-bootstrap-complete").exists()
        && resolve_sanremes_desktop_exe(install_root).is_some()
}
...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    // writeFileAtomic(). sanremes_is_installed() only checks existence, so a
    // partial direct write would incorrectly enable the launcher fast path.
    let tmp_path = install_root.join(".sanremes-bootstrap-complete.tmp");
    {
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_sanremes_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built SanRemes desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_sanremes_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::sanremes_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $SanRemesHome which Stage-Repository clones
    // the repo INTO at $SanRemesHome\sanremes-agent. Mirrors sanremes_constants.
    let sanremes_home = args
        .sanremes_home
        .clone()
        .unwrap_or_else(|| crate::paths::sanremes_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&sanremes_home).join("sanremes-agent");
    ...
    // Copy ourselves to SANREMES_HOME/sanremes-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_sanremes_home() {
        tracing::warn!(?err, "failed to copy installer into SANREMES_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    sanremes_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, sanremes_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("SanRemes"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../SanRemes.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("SanRemes.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("sanremes");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/SanRemes.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_sanremes_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_sanremes_desktop_app(&root)
    ...
    #[test]
    fn resolve_sanremes_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_sanremes_desktop_app(&root).is_none(),
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".sanremes-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".sanremes-bootstrap-complete");
        let tmp_path = root.join(".sanremes-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
    ...
    #[test]
    fn sanremes_is_installed_treats_marker_existence_as_sufficient() {
        ...
        make_release_tree(&root);
        std::fs::write(root.join(".sanremes-bootstrap-complete"), b"").unwrap();
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".sanremes-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".sanremes-bootstrap-complete.tmp").exists(),
