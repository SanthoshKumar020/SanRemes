...
/// read side does not see EOF until the last of them closes it. `sanremes update`
...
/// stranded with it.
///
/// Same bound `Invoke-SanRemesStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `sanremes_home_override` propagates to the child as $SANREMES_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, sanremes_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = sanremes_home_override {
        cmd.env("SANREMES_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, sanremes_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = sanremes_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_sanremes_home() {
    ...
    /// child. `sanremes update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
