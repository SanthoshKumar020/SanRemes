...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating SanRemes' : 'Setting up SanRemes Agent'

  const description = isUpdate
    ? 'SanRemes is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The SanRemes installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="sanremes-fade-in flex h-full flex-col">
