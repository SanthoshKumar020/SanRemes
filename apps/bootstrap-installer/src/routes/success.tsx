...
import { launchSanRemesDesktop } from '../store'

/*
 * Success screen. SANREMES AGENT wordmark stays as the visual anchor
 ...
 * SanRemes.exe doesn't exist). We catch the Tauri error and surface it
 * inline rather than silently doing nothing — the previous version
 * had `onClick={() => void launchSanRemesDesktop()}` which swallowed
...
export default function Success() {
  ...
  return (
    <div className="sanremes-fade-in flex h-full flex-col items-center justify-center gap-8 px-12 py-10">
      <div className="w-full max-w-2xl min-w-0 text-center">
        ...
        >
          <span>
            <span>SanRemes is ready</span>
          </span>
          <span aria-hidden="true">SanRemes is ready</span>
        </p>

        <p className="m-0 text-center text-base leading-normal tracking-tight text-muted-foreground">
          You can launch from here, or any time from your terminal with{' '}
          <code className="font-mono text-sm text-foreground/80">sanremes desktop</code>.
        </p>
      </div>
