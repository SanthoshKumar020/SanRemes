...
export const $sanremesHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.sanremes/logs/bootstrap-installer.log')
    $sanremesHome.set('~/.sanremes')
  ...
  try {
    const [logPath, sanremesHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_sanremes_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $sanremesHome.set(sanremesHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch SanRemes" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (SanRemes-Setup.exe --update);
...
export async function launchSanRemesDesktop(): Promise<void> {
  ...
  await invoke('launch_sanremes_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'SanRemes repository' },
