/**
 * ChatPage — embeds `sanremes --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (sanremes_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "sanremes.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__SANREMES_SESSION_TOKEN__ &&
    !window.__SANREMES_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `sanremes dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("sanremes-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("sanremes-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__SANREMES_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner SanRemes TUI has enabled xterm mouse-events
    ...
    //      payloads upload to SANREMES_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to SANREMES_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[sanremes-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__SANREMES_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets SANREMES_HOME pointed at the
    ...
    // forwarding them into SanRemes. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="sanremes-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
