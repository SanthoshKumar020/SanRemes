import { buildSanRemesWebSocketUrl } from "@sanremes/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/sanremes/). The Python backend
// injects ``window.__SANREMES_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SANREMES_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SANREMES_BASE_PATH = readBasePath();
const BASE = SANREMES_BASE_PATH;
...
declare global {
  interface Window {
    __SANREMES_SESSION_TOKEN__?: string;
    __SANREMES_BASE_PATH__?: string;
    ...
    __SANREMES_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-SanRemes-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__SANREMES_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``sanremes update``, ``sanremes gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__SANREMES_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__SANREMES_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__SANREMES_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``sanremes_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__SANREMES_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-SanRemes-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``sanremes_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__SANREMES_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildSanRemesWebSocketUrl({
...
export const api = {
  ...
  updateSanRemes: () =>
    fetchJSON<ActionResponse>("/api/sanremes/update", { method: "POST" }),
  checkSanRemesUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/sanremes/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** sanremes-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``sanremes update``. */
  can_update_sanremes?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the SANREMES_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the SANREMES_HOME volume — no staleness dimension, so no
 * sampled_at. */
