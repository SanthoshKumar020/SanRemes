...
beforeEach(() => {
  ...
  Object.defineProperty(window, "__SANREMES_SESSION_TOKEN__", {
    configurable: true,
    value: "stale-token",
    writable: true,
  });
  Object.defineProperty(window, "__SANREMES_AUTH_REQUIRED__", {
    configurable: true,
    value: false,
    writable: true,
  });
...
describe("api OAuth helpers", () => {
  it("starts OAuth login in gated mode without requiring an injected session token", async () => {
    vi.stubGlobal("window", { __SANREMES_AUTH_REQUIRED__: true });
  ...
  it("still sends the injected session token for OAuth login in loopback mode", async () => {
    vi.stubGlobal("window", { __SANREMES_SESSION_TOKEN__: "loopback-token" });
    const fetchMock = jsonFetchMock({
      flow: "device_code",
      session_id: "oauth-session",
    });
