...
interface SanRemesConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function SanRemesConsoleModal({ open, onClose }: SanRemesConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("sanremes> ");
  const inputPromptRef = useRef("sanremes> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "sanremes> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to SanRemes Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="sanremes-console-title"
    >
      <div
        className={cn(
          themedBody,
