...
const SIDEBAR_COLLAPSED_KEY = "sanremes-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="sanremes-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="sanremes-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateSanRemes = status?.can_update_sanremes === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'sanremes update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "sanremes update";
    return (
      t.status.updateSanRemesConfirmMessage ??
      `This will run 'sanremes update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateSanRemesConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateSanRemes) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateSanRemes,
      runningLabel: t.status.updatingSanRemes,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the SanRemes gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateSanRemesConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateSanRemesConfirmTitle ?? `${t.status.updateSanRemes}?`}
    />
    </>
  );
}
