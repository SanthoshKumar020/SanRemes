...
export const zh: Translations = {
  ...
  status: {
    ...
    updatingSanRemes: "正在更新 SanRemes…",
  ...
  pluginsPage: {
    ...
    headline: "发现、安装、启用和更新 SanRemes 插件（对齐 `sanremes plugins` CLI）。",
    ...
    removeConfirm: "从 ~/.sanremes/plugins/ 删除此插件？",
    removeHint: "仅可移除用户安装在 ~/.sanremes/plugins 下的插件。",
  ...
  skills: {
    ...
    categories: "分类",
    filters: "筛选",
    noSkills: "未找到技能。技能从 ~/.sanremes/skills/ 加载",
  ...
  env: {
    ...
    hideValue: "隐藏值",
    customTitle: "自定义密钥",
    customHint: "存储在 .env 中、SanRemes 无法识别的任意环境变量。可用于为技能、MCP 服务器或你自己的工具注入环境变量。",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "从真实会话历史中获得的 SanRemes 可收集徽章。已知尚未达成的成就显示为「已发现」；秘密成就在首次出现匹配行为之前保持隐藏。",
      scan_subtitle:
        "正在扫描 SanRemes 会话历史。在历史记录较多时，首次扫描可能需要 5–10 秒。",
    },
    actions: {
      rescan: "重新扫描",
    },
    stats: {
      ...
      latest: "最新",
      latest_hint_empty: "多多运行 SanRemes",
      none_yet: "暂无",
    },
    state: {
      unlocked: "已解锁",
    ...
    guide: {
      ...
      secret_header: "秘密成就",
      secret_body:
        "秘密成就会隐藏其确切触发条件。一旦 SanRemes 检测到相关信号，卡片将变为「已发现」并显示其要求。",
      scan_status_header: "扫描状态",
      scan_status_body:
        "SanRemes 正在对本地历史进行一次扫描，之后卡片会自动出现。即使这需要几秒钟，也没有卡住。",
