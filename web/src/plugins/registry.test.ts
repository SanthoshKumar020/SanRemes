 ...
 * `window.__SANREMES_PLUGIN_SDK__`. Each new key is checked individually so a
...
describe("plugin SDK dialog/toast surface", () => {
  beforeEach(() => {
    ...
    (globalThis as unknown as { window: Record<string, unknown> }).window = {
      __SANREMES_PLUGINS__: undefined,
      __SANREMES_PLUGIN_SDK__: undefined,
    };
  });

  it("exposes Dialog + subcomponents on components", () => {
    exposePluginSDK();
    const sdk = (globalThis as unknown as { window: { __SANREMES_PLUGIN_SDK__: { components: Record<string, unknown>; hooks: Record<string, unknown> } } }).window.__SANREMES_PLUGIN_SDK__;
