...
import { api, SANREMES_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "sanremes:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${SANREMES_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${SANREMES_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?sanremes_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-sanremes-plugin", manifest.name);
