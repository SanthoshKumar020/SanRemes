...
const BACKEND = process.env.SANREMES_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `sanremes dashboard` server injects a one-shot
 * session token into `index.html` (see `sanremes_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__SANREMES_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function sanremesDevToken(): Plugin {
  const TOKEN_RE = /window\.__SANREMES_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__SANREMES_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "sanremes:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[sanremes] Could not find session token in ${BACKEND} — ` +
              `is \`sanremes dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__SANREMES_SESSION_TOKEN__="${match[1]}";` +
              `window.__SANREMES_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[sanremes] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`sanremes dashboard\` or set SANREMES_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@sanremes/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `sanremes dashboard` must serve these; Vite has no
