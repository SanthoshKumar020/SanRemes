...
``sanremes_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import sanremes_state (cycle) — shared
module-level constants live in sanremes_state_common.
"""
...
# Moved methods logged under the "sanremes_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("sanremes_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``sanremes_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `sanremes update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._sanremes_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._sanremes_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`sanremes sessions optimize-storage` with the gateway stopped. "
                        "`sanremes doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from sanremes_constants import get_sanremes_home
            cache_path = get_sanremes_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `sanremes sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `sanremes db optimize`) must keep its EXISTING
    ...
    def _backfill_gateway_metadata_from_sessions_json(
        ...
        """
        sessions_file = get_sanremes_home() / "sessions" / "sessions.json"
