...
(local, remote URL, SSH, SanRemes Cloud, docker) must be able to find every
...
def _(rid, params: dict) -> dict:
    ...
    ``sanremes -p <profile> chat -c "Bot Chat"`` transport local DMs use and
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "sanremes" else profile
        if resolved not in known:
            return _err(rid, 4092, f"no profile '{profile}' on this gateway")

        fd, tmp = tempfile.mkstemp(prefix="sanremes-relay-dm-", suffix=".txt", text=True)
        try:
