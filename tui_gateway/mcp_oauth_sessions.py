...
The underlying token machinery is the *same* one the CLI ``sanremes mcp login``
uses — ``sanremes_cli.mcp_config._probe_single_server`` under
...
def _start_loopback_listener(flow) -> "http.server.HTTPServer":
    ...
    class _Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 — stdlib naming
            ...
            body = b"<h1>Authorization received</h1><p>You can close this tab and return to SanRemes.</p>"
            status = 200
            try:
...
def _worker(session_id: str, sanremes_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    SANREMES_HOME override + secret-scope + force_interactive_oauth +
    ...
    try:
        ...
        home_token = set_sanremes_home_override(sanremes_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(sanremes_home)))
        try:
            with force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                previous_entry = None
                try:
                    previous_entry = manager.remove(server_name, sanremes_home=sanremes_home)
                    ...
                    manager.restore_entry(server_name, previous_entry, sanremes_home=sanremes_home)
                    raise
        finally:
...
def start_flow(
    ...
    *,
    ...
    OAuth-capable). ``sanremes_home`` is the already-resolved profile home dir
    ...
    with _sessions_lock:
        ...
        if any(
            ...
            and r["sanremes_home"] == sanremes_home
    ...
    threading.Thread(
        target=_worker,
        args=(session_id, sanremes_home, server_name, dict(cfg), reconnect_live),
        daemon=True,
