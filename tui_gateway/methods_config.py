...
from sanremes_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    try:
        with _profile_db(params) as db:
            ...
            with pdb.connect_closing() as conn:
                ...
                # zero SanRemes sessions still surface. The desktop's native scan
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from sanremes_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_sanremes_home), "display": display_sanremes_home()})
    if key == "project":
    ...
    if key == "personality":
        ...
        from sanremes_cli.personality import active_personality_name
    ...
    if key == "mtime":
        cfg_path = _sanremes_home / "config.yaml"
        try:
        ...
        if (
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No SanRemes provider is configured.",
                },
            )
...
def _(rid, params: dict) -> dict:
    ...
    collection + force-redaction pipeline as ``sanremes debug share --nous``
    ...
    try:
        ...
        from sanremes_cli.diagnostics_upload import share_to_nous
