...
handlers delegate to the same `sanremes_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List SanRemes profiles (name, path, model, description, skill count).
    ...
    def _canonical_session_row(db, profile_path):
        ...
        describe the same session (sanremes-agent#88200) with no client-side
        pointer involved.
    ...
    def _latest_profile_session_rows(db):
        ...
        sessions never surface in conversation lists (sanremes-agent#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's sanremes-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``sanremes setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (sanremes_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local SANREMES_HOME override — the same
        ...
        try:
            ...
            token = set_sanremes_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from sanremes_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from sanremes_cli.config import load_config_readonly, read_user_config_raw
            from sanremes_cli.web_routers.profiles import _write_profile_model
            from sanremes_constants import (
                reset_sanremes_home_override,
                set_sanremes_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the SANREMES_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            from sanremes_cli.skills_config import get_disabled_skills
            ...
            # Toolsets: the same filtered universe the `sanremes tools`
            ...
            # composites (sanremes-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from sanremes_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `sanremes tools` / Settings is where
            ...
            description = ""
            try:
                from sanremes_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from sanremes_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from sanremes_cli.skills_config import save_disabled_skills
