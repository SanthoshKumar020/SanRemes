...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind SANREMES_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # custom `SANREMES_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        _enable_gateway_prompts()
        home_token = (
            set_sanremes_home_override(str(profile_home)) if profile_home is not None else None
        )
        secret_token = (
        ...
        with _session_resume_lock:
            ...
            try:
                init_home_token = (
                    set_sanremes_home_override(str(profile_home))
                    if profile_home is not None
                    else None
                )
                init_secret_token = (
                ...
                try:
                    ...
                    if init_home_token is not None:
                        reset_sanremes_home_override(init_home_token)
                ...
                if sid in _sessions:
                    ...
                    # Remember the profile home so each turn re-binds SANREMES_HOME (the
        ...
        # atexit.register(_drain_token_queue_at_exit) (sanremes_state.py), which only
...
def _(rid, params: dict) -> dict:
    ...
    # ``sanremes --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    with _profile_db(params) as db:
        ...
        if profile_home is not None:
            ...
            sessions_dir = get_sanremes_home() / "sessions"
        try:
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``sanremes gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from sanremes_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from sanremes_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from sanremes_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sanremes_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from sanremes_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    from sanremes_cli.nous_billing import BillingError, post_subscription_preview
...
def _(rid, params: dict) -> dict:
    ...
    billing:manage.
    """
    from sanremes_cli.nous_billing import BillingError, put_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    """
    from sanremes_cli.nous_billing import BillingError, delete_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    from sanremes_cli.nous_billing import BillingError, post_subscription_upgrade
...
def _(rid, params: dict) -> dict:
    ...
    """
    from sanremes_cli.nous_billing import BillingError, post_charge
    from agent.billing_view import new_idempotency_key
...
def _(rid, params: dict) -> dict:
    ...
    """
    from sanremes_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from sanremes_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sanremes_cli.auth import step_up_nous_billing_scope
        from sanremes_cli.nous_billing import BillingError
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the SanRemes profile home
    # (~/.sanremes/sessions/saved/) rather than the project/workspace CWD, and
    # include the system prompt so the export matches the dashboard save.
    saved_dir = get_sanremes_home() / "sessions" / "saved"
    try:
    ...
    path = saved_dir / f"sanremes_conversation_{timestamp}.json"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if parent_home:
            ...
            branch_owns_db = True
        home_token = (
            set_sanremes_home_override(parent_home) if parent_home else None
        )
