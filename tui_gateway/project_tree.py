...
# cwd at all, or one whose folder can't be promoted (the bare home dir, SANREMES
...
def build_tree(
    ...
    *,
    ...
    bare home dir, the SANREMES_HOME subtree). ``is_junk_cwd`` is the narrower
    policy for non-git session folders: selected descendants may be intentional
    workspaces even when their parent tree contains SanRemes state. User-created
