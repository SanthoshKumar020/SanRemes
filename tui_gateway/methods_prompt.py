...
def _(rid, params: dict) -> dict:
    from sanremes_cli.input_sanitize import sanitize_user_prompt_text
    ...
    # over another app when they are back in SanRemes.
        ...
        if (
            ...
            if not is_truthy_value(params.get("confirm_truncate")):
                ...
                return _err(
                    ...
                    "(update your SanRemes client if a rewind was intended)",
                )
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sanremes_cli.clipboard import has_clipboard_image, save_clipboard_image
...
def _(rid, params: dict) -> dict:
    ...
    def run():
        ...
        try:
            ...
            # SANREMES_HOME override (the ContextVar set on the session-create
            ...
            home_token = (
                set_sanremes_home_override(_profile_home_str)
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not SanRemes Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the SanRemes/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not SanRemes Desktop. If it serves SanRemes/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    def run():
        ...
        try:
            ...
            # the session's SANREMES_HOME override (the ContextVar set on the
...
def _(rid, params: dict) -> dict:
    # `text` is a JSON string describing the OS window underneath the SanRemes
