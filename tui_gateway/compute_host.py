    ...
    def __init__(
        ...
        *,
        ...
        self._heartbeat_secs = (
            ...
            else float(os.environ.get("SANREMES_COMPUTE_HOST_HEARTBEAT_SECS") or "15")
        )
    ...
    def _run_real_turn(self, frame: dict[str, Any]) -> None:
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                import sanremes_undo

                sanremes_undo.on_user_message_appended(session["session_key"])
            except Exception:
                pass
            try:
...
def _default_workers() -> int:
    try:
        return max(2, int(os.environ.get("SANREMES_TUI_RPC_POOL_WORKERS") or "8"))
    except (TypeError, ValueError):
        return 8


def run_host(stdin: Any = None, stdout: Any = None) -> None:
    os.environ["SANREMES_COMPUTE_HOST_CHILD"] = "1"
    ...
    host.emit(
        {
            ...
            "cwd": os.getcwd(),
            "sanremes_home": os.environ.get("SANREMES_HOME", ""),
        }
    )
