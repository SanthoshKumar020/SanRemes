...
def _(rid, params: dict) -> dict:
    ...
    paste_dir = _sanremes_home / "pastes"
...
def _(rid, params: dict) -> dict:
    ...
    def _profile_mention_items(prefix: str) -> list[dict]:
        ...
        under the 'sanremes' alias when no real profile claims that name.
        """
        out: list[dict] = []
        try:
            ...
            if "sanremes".startswith(prefix.lower()) and "sanremes" not in seen:
                out.append(
                    {
                        "text": "@sanremes",
                        "display": "@sanremes",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sanremes_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        from sanremes_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from sanremes_cli.inventory import build_models_payload
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `sanremes model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.sanremes/.env via the unified credential lifecycle
        ...
        from sanremes_cli.credential_lifecycle import save_provider_env_credential
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from sanremes_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from sanremes_cli.credential_lifecycle import remove_provider_env_credential
