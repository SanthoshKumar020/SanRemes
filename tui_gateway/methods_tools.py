...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.sanremes/.env`` into the gateway process via
    ``sanremes_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from sanremes_cli.config import reload_env
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        try:
            from sanremes_cli.plugins import get_plugin_commands
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m sanremes_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "sanremes_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m sanremes_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=sanremes_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "init":
        ...
        from sanremes_cli.init_command import build_init_prompt_for_cwd
    ...
    if name == "moa":
        ...
        try:
            from sanremes_cli.moa_config import moa_usage, normalize_moa_config
    ...
    if name == "loop":
        ...
        if not session:
            return _err(rid, 4001, "no active session")
        try:
            from sanremes_cli.loops import LoopManager, dispatch_loop_command
        except Exception as exc:
        ...
        if result.get("created"):
            try:
                from sanremes_cli.loops import goal_blocks_loop_tick
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind SANREMES_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_sanremes_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("SANREMES_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("SANREMES_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_sanremes_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off SANREMES_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from sanremes_constants import set_sanremes_home_override

            profile_dir = get_profile_dir(profile)
            if not profile_dir or not profile_dir.is_dir():
                return _err(rid, 4064, f"profile '{profile}' not found")
            token = set_sanremes_home_override(str(profile_dir))
        except Exception as e:
    ...
    try:
        ...
        if token is not None:
            try:
                from sanremes_constants import reset_sanremes_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``sanremes journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from sanremes_cli.skills_hub import do_install
        ...
        if action == "browse":
            from sanremes_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from sanremes_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `sanremes mcp` offers, so
    ...
    try:
        ...
        from sanremes_cli import mcp_catalog
...
# (sanremes_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes SANREMES_HOME via set_sanremes_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses sanremes_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sanremes_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sanremes_cli.mcp_config import (
            _apply_mcp_preset,
            _get_mcp_servers,
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sanremes_cli.config import load_config, save_config, save_env_value
        from sanremes_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sanremes_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.sanremesDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``sanremes mcp login`` uses
    ...
    try:
        ...
        sanremes_home = str(get_sanremes_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(sanremes_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``sanremes plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.sanremes/plugins/`` (non-interactive).
    ...
    plugins live under each profile's SANREMES_HOME, so a client can list or
    ...
    if err:
        return err
    try:
        from sanremes_cli.plugins_cmd import (
            _bundled_default_on,
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native SanRemes plugin.
        ...
        if action == "toggle":
            from sanremes_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
        ...
        if action == "install":
            from sanremes_cli.plugins_cmd import dashboard_install_plugin
