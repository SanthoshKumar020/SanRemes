...
# from shadowing SanRemes's own top-level modules.  ``sanremes_bootstrap`` lives at
...
import sanremes_bootstrap

sanremes_bootstrap.harden_import_path()
...
# shared owner in sanremes_cli.mcp_startup — the wait/in-flight/join helpers
...
# sanremes_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `SANREMES_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("SANREMES_TUI_SIDECAR_URL")
...
# tested; override via ``SANREMES_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("SANREMES_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``SANREMES_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``sanremes --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``sanremes_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from sanremes_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → sanremes_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's SANREMES_HOME first), so a launch profile with no
    ...
    if not _mcp_discovery_enabled:
        return
    try:
        from sanremes_cli.mcp_startup import start_background_mcp_discovery
    ...
    try:
        from sanremes_cli.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``sanremes --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``sanremes dashboard`` spawn theirs via
    ``sanremes_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``sanremes_cli.mcp_startup`` thread used by the
    ...
    try:
        from sanremes_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from sanremes_cli.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's SANREMES_HOME override — the shared owner in
    ``sanremes_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    # Live-apply skins SanRemes activates mid-conversation.
    ...
    try:
        from sanremes_cli.model_switch import prewarm_picker_cache_async
