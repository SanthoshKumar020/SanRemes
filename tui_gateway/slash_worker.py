"""Persistent slash-command worker — one SanRemesCLI per TUI session.

Protocol: reads JSON lines from stdin {id, command}, writes {id, ok, output|error} to stdout.
"""

# Stop a ``utils/`` (or ``proxy/``, ``ui/``) package in the launch directory
# from shadowing SanRemes's own top-level modules.  This worker is spawned as
...
# and crash the child in a retry loop (issue #51286).  ``sanremes_bootstrap``
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``SANREMES_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("SANREMES_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("SANREMES_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before SanRemesCLI snapshots tools.

    Each slash_worker child is its own process — the parent ``sanremes serve``
...
def _run(cli: SanRemesCLI, command: str) -> str:
...
def main():
    ...
    os.environ["SANREMES_SESSION_KEY"] = args.session_key
    os.environ["SANREMES_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) SanRemesCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = SanRemesCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
