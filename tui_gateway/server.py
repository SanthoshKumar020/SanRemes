...
from sanremes_constants import (
    DEFAULT_INDICATOR_STYLE,
    INDICATOR_STYLES,
    get_sanremes_home,
    get_sanremes_home_override,
    reset_sanremes_home_override,
    set_sanremes_home_override,
)
from sanremes_cli.env_loader import load_sanremes_dotenv
from utils import is_truthy_value
from tools.environments.local import sanremes_subprocess_env
...
_sanremes_home = get_sanremes_home()
load_sanremes_dotenv(
    sanremes_home=_sanremes_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.sanremes/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_sanremes_home, "logs", "tui_gateway_crash.log")
...
try:
    from sanremes_cli.banner import prefetch_update_check
...
try:
    _slash_timeout = float(os.environ.get("SANREMES_TUI_SLASH_TIMEOUT_S") or "45")
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    ``SANREMES_TUI_WS_ORPHAN_REAP_GRACE_S`` env var is kept as an internal
    override for backward compatibility and wins when set.
    """
    raw = os.environ.get("SANREMES_TUI_WS_ORPHAN_REAP_GRACE_S")
    if raw is None or not str(raw).strip():
        try:
            from sanremes_cli.config import load_config
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("SANREMES_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("sanremes_gateway_runtime_session_record", default=None)
)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar(
    "sanremes_gateway_rpc_method", default=""
)
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend SanRemes' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    SanRemes-managed CLIs (browser-use, uvx, uv) even when the parent
    ...
    try:
        ...
        managed_bin = str(Path(get_sanremes_home()) / "bin")
    except Exception:
        pass
...
class _SlashWorker:
    """Persistent SanRemesCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        self._closed = False
        from sanremes_cli._subprocess_compat import windows_hide_flags

        # slash_worker runs the SanRemes agent → needs provider credentials.
        ...
        # gateway's launch SANREMES_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["SANREMES_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            sanremes_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"SANREMES_HOME": str(profile_home)} if profile_home else None,
        )
        # Prepend the SanRemes venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve SanRemes-managed CLIs
...
def _notify_session_boundary(
    ...
    try:
        from sanremes_cli.lifecycle import finalize_session, invoke_hook
...
_SESSION_OWNERSHIP_UNAVAILABLE = (
    "SanRemes could not safely reserve this session. Try again."
)
...
def _claim_active_session_slot(
    ...
    *,
    ...
    try:
        from sanremes_cli.active_sessions import try_acquire_active_session
...
def _other_runtime_lease_guard(session_id: str, session: dict):
    ...
    lease = session.get("active_session_lease")
    try:
        from sanremes_cli.active_sessions import (
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from sanremes_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from sanremes_cli.lifecycle import invoke_hook
        ...
        with _session_resume_lock:
            with _sessions_lock:
                ...
                if current.get("close_on_disconnect"):
                    ...
                    # `sanremes --tui` keeps real _stdio. UNLESS another window
...
try:
    _SESSION_TTL_S = float(os.environ.get("SANREMES_TUI_SESSION_TTL_S") or 6 * 3600)
...
# A `sanremes serve` killed mid-update used to lose every un-flushed in-memory
...
try:
    _EXIT_FLUSH_BUDGET_S = float(
        os.environ.get("SANREMES_TUI_EXIT_FLUSH_BUDGET_S") or 5.0
    )
except (TypeError, ValueError):
...
try:
    _INCREMENTAL_FLUSH_INTERVAL_S = float(
        os.environ.get("SANREMES_TUI_SESSION_FLUSH_INTERVAL_S") or _REAPER_SCAN_S
    )
except (TypeError, ValueError):
...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    ...
    can never block exit longer than ``SANREMES_TUI_EXIT_FLUSH_BUDGET_S``
    ...
    worker = threading.Thread(target=_run, daemon=True, name="sanremes-exit-flush")
    worker.start()
    worker.join(budget)
...
def install_exit_flush_signal_handlers() -> bool:
    ...
    Called by ``sanremes serve`` / dashboard startup before uvicorn takes over
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `sanremes --tui`, so it must NOT count as dead here (doing
...
def _reap_idle_sessions() -> None:
    ...
    try:
        from sanremes_cli.mem_trim import trim_memory
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from sanremes_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from sanremes_cli.active_sessions import coerce_max_concurrent_sessions
...
def _sweep_orphaned_session_rows() -> list[str]:
    ...
    ago (``SANREMES_TUI_SESSION_TTL_S``). A freshly created row that copied
...
_HEARTBEAT_REFRESH_S = float(
    os.environ.get("SANREMES_GATEWAY_HEARTBEAT_REFRESH_S") or 60.0
)
...
def _start_backend_heartbeat_refresher() -> None:
    ...
    thread = threading.Thread(
        ...
        args=(stop_event,),
        name="sanremes-gateway-heartbeat",
        daemon=True,
    )
    thread.start()
...
def _schedule_startup_orphan_sweep() -> None:
    ...
    reads the DB. ``SANREMES_TUI_WS_ORPHAN_REAP_GRACE_S=0`` (park forever)
    and ``SANREMES_TUI_SESSION_TTL_S=0`` both suppress the sweep; so does
...
def _get_db():
    ...
    if _db is None:
        from sanremes_state import SessionDB
...
# ``profile`` on those calls; we open that profile's db and bind its SANREMES_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if not name:
        return None
    try:
        from sanremes_cli import profiles as profiles_mod

        home = Path(profiles_mod.get_profile_dir(name))
    except Exception:
        return None
    # Already the launch profile? No override needed.
    if home.resolve() == Path(_sanremes_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s SANREMES_HOME around a handler.

    Pets (config + sprites) and projects (projects.db, discovery policy) both
    resolve via ``get_sanremes_home``. The desktop sends ``profile`` so a single
    ...
    def wrapper(rid, params):
        ...
        token = set_sanremes_home_override(home)
        try:
            return handler(rid, params)
        finally:
            reset_sanremes_home_override(token)
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from sanremes_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``sanremes dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("SANREMES_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `sanremes` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # SANREMES_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_sanremes_home_override(profile_home)
                try:
            ...
            if home_token is not None:
                reset_sanremes_home_override(home_token)
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``sanremes`` / ``sanremes --tui`` / CLI) is started from a
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from sanremes_cli.runtime_provider import canonical_custom_identity
    ...
    try:
        ...
        from sanremes_state import is_disk_full_error
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from sanremes_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``sanremes_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg_raw() -> dict:
    ...
    try:
        ...
        # launch profile's _sanremes_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_sanremes_home_override()
        home = override if isinstance(override, str) and override else _sanremes_home
        ...
        if p.exists():
            from sanremes_cli.config import read_user_config_raw
...
def _load_cfg() -> dict:
    ...
    ``sanremes_cli.config.load_config_readonly`` — managed-scope overlay and
    ...
    try:
        from sanremes_cli.config import _expand_env_vars
...
def _apply_managed(cfg: dict) -> dict:
    ...
    sanremes_cli.config.load_config, so without this a managed skin / reasoning_effort
    ...
    """
    try:
        from sanremes_cli import managed_scope
...
def _save_cfg(cfg: dict):
    ...
    path = _sanremes_home / "config.yaml"
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read SANREMES_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # SANREMES_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["SANREMES_GATEWAY_SESSION"] = "1"
    os.environ["SANREMES_EXEC_ASK"] = "1"
    os.environ["SANREMES_INTERACTIVE"] = "1"
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps(
    {
        "success": False,
        "error": (
            "No SanRemes Desktop window answered the tour request. The tour is "
            ...
            "nothing listening. Update the SanRemes Desktop app and start a new "
...
def resolve_skin() -> dict:
    try:
        from sanremes_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``sanremes config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_sanremes_home_override()
    return Path(override if isinstance(override, str) and override else _sanremes_home)
...
def _ensure_skin_watcher() -> None:
    ...
    SanRemes activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="sanremes-change-watcher", daemon=True).start()


def _resolve_model() -> str:
    env = (
        os.environ.get("SANREMES_MODEL", "")
        or os.environ.get("SANREMES_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from sanremes_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``SANREMES_DESKTOP=1`` and ``SANREMES_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``SANREMES_DESKTOP_TERMINAL=1`` → "tui"
        (``sanremes --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``sanremes --tui``.)
    """
    if is_truthy_value(os.environ.get("SANREMES_DESKTOP")) and not is_truthy_value(
        os.environ.get("SANREMES_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads SANREMES_MODEL /
    SANREMES_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`sanremes --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads SANREMES_MODEL /
    # SANREMES_INFERENCE_MODEL, which `sanremes --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("SANREMES_TUI_PROVIDER", "").strip()
    ...
    try:
        from sanremes_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("SANREMES_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
from sanremes_state import _BARE_BILLING_PROVIDERS
...
def _overrides_have_routable_provider(overrides: dict) -> bool:
    ...
    if not provider:
        return False
    try:
        from sanremes_cli.runtime_provider import is_routable_provider
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from sanremes_cli.runtime_provider import (
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # Re-bind SANREMES_HOME to the session's profile so load_soul_md() and
    ...
    profile_home = session.get("profile_home")
    home_token = (
        set_sanremes_home_override(profile_home) if profile_home else None
    )
...
def _load_approval_mode() -> str:
    ...
    canonical ``sanremes_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`sanremes_constants.resolve_reasoning_config` (per-model override >
    global ``agent.reasoning_effort``; YAML boolean False = disabled).
    Closes #21256.
    """
    from sanremes_constants import resolve_reasoning_config
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("SANREMES_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_SANREMES_CORE_TOOLS`` — every other
    ...
    paths run with ``SANREMES_DESKTOP=1``. Keying GUI capability off that env var
    silently stripped every pane/browser tool from URL and cloud gateways while
    the same backend told the model it was "chatting inside the SanRemes desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [
        item.strip()
        for item in os.environ.get("SANREMES_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base SanRemes): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `sanremes --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if unresolved:
            try:
                from sanremes_cli.plugins import discover_plugins
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] SANREMES_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            from sanremes_cli.config import read_raw_config
            from sanremes_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown SANREMES_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in SANREMES_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid SANREMES_TUI_TOOLSETS entries; using configured CLI toolsets"
        )

    try:
        from sanremes_cli.config import load_config
        from sanremes_cli.tools_config import _get_platform_tools
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _SANREMES_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off sanremes-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid SANREMES_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    from sanremes_cli.runtime_provider import resolve_runtime_provider
    ...
    cfg = None
    try:
        from sanremes_cli.config import get_compatible_custom_providers, load_config
    ...
    if agent:
        try:
            from sanremes_cli.context_switch_guard import merge_preflight_compression_warning
    ...
    if not confirm_expensive_model:
        try:
            from sanremes_cli.model_selection_guards import combined_selection_warning
    ...
    #
    # We deliberately do NOT write process-global env vars (SANREMES_MODEL /
    # SANREMES_INFERENCE_MODEL / SANREMES_TUI_PROVIDER / SANREMES_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `sanremes --tui -m`
...
def _compress_session_history(
    ...
    from sanremes_cli.partial_compress import (
...
def _get_usage(agent) -> dict:
    ...
    # SANREMES_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("SANREMES_DEV_CREDITS")):
        try:
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            try:
                from sanremes_cli.personality import available_personalities
...
def _current_profile_name() -> str:
    try:
        from sanremes_cli.profiles import get_active_profile_name
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from sanremes_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        from sanremes_cli import __version__, __release_date__
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from sanremes_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from sanremes_cli.banner import get_update_result
        from sanremes_cli.config import recommended_update_command
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the SanRemes window, and answers
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from sanremes_cli.config import save_env_value_secure
...
def _render_personality_prompt(value) -> str:
    """Delegates to sanremes_cli.personality (single owner of rendering)."""
    from sanremes_cli.personality import render_personality_prompt

    return render_personality_prompt(value)


def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via sanremes_cli.personality (single owner)."""
    from sanremes_cli.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as sanremes_cli.personality.resolve_personality — (name,
    ...
    """
    from sanremes_cli.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to sanremes_cli.personality (single owner).
    """
    from sanremes_cli.personality import prompt_text
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    from sanremes_cli.config import resolve_turn_limit as _resolve_turn_limit
    # Env var override (highest priority)
    env_val = os.environ.get("SANREMES_TUI_MAX_TURNS")
    if env_val:
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("SANREMES_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``SanRemesCLI.__init__`` and ``gateway/run.py``:
    ...
    """
    from sanremes_cli.fallback_config import get_fallback_chain
...
def _resolve_runtime_with_fallback(
    ...
    """
    from sanremes_cli.auth import AuthError
    from sanremes_cli.runtime_provider import resolve_runtime_provider
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from sanremes_cli.fallback_config import resolve_entry_api_key
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses sanremes_cli.mcp_startup; TUI stdio keeps
    # its existing tui_gateway.entry-owned thread.
    try:
        from sanremes_cli.mcp_startup import wait_for_mcp_discovery
    ...
    cfg = _load_cfg()
    from sanremes_cli.config import resolve_ephemeral_system_prompt_from_config
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `sanremes skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("SANREMES_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("SANREMES_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("SANREMES_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("SANREMES_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped SANREMES_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / SANREMES_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The SANREMES_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_sanremes_home)
...
def _finalize_superseded_runtimes(stale: list[tuple[str, dict]]) -> None:
    ...
    sanremes_state_common._RECOVERABLE_END_REASONS so canonical Bot Chat
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    SanRemes has to support both the legacy 8-row petdex atlas and the current
...
def _pet_gen_root():
    ...
    root = get_sanremes_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("SANREMES_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
#
# Layout:  $SANREMES_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_sanremes_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _maybe_fire_tui_loop_tick(sid: str, session: dict) -> None:
    ...
    """
    try:
        from sanremes_cli.loops import LoopManager, goal_blocks_loop_tick
    except Exception:
        return
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=SANREMES_SESSION_KEY`` (see
    ...
    if not session_key or session.get("_finalized"):
        return []
    try:
        from sanremes_cli import kanban_db as _kb
    except Exception:
        return []
    texts: list = []
    try:
    ...
    # DB when SANREMES_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``SANREMES_UI_SESSION_ID`` as
...
def _plan_goal_compression_recovery(
    ...
    *,
    ...
    from sanremes_cli.goals import GoalManager
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn SANREMES_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_sanremes_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from sanremes_cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from sanremes_cli.voice import is_continuous_active
                ...
                try:
                    ...
                    if sid_key:
                        ...
                        if goal_mgr.is_active():
                            try:
                                from sanremes_cli.goals import gather_background_processes as _gather_bg
            ...
            if status == "complete":
                try:
                    from sanremes_cli.loops import LoopManager
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: sanremes_cli.voice unavailable")
                except Exception as e:
            ...
            # Run while any profile-specific SANREMES_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile SANREMES_HOME
    override, so ``get_sanremes_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _sanremes_home
    return base / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile SANREMES_HOME override, while the docker/ssh
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from sanremes_cli.model_switch import parse_model_switch_args
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from sanremes_cli.models import resolve_fast_mode_overrides
    ...
    if key == "focus":
        ...
        from sanremes_cli.focus_view import (
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("SANREMES_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["SANREMES_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("SANREMES_YOLO_MODE", None)
                    nv = "0"
    ...
    if key == "reasoning":
        try:
            from sanremes_constants import parse_reasoning_effort
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through sanremes_cli.personality (single owner) and never
                # touches the user-owned global system prompt.
                from sanremes_cli.personality import persist_personality
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under SANREMES_HOME (~/.sanremes by
    ...
    sanremes_home = os.path.realpath(str(get_sanremes_home()))

    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == sanremes_home
        or real.startswith(sanremes_home + os.sep)
    )
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    SANREMES_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: SANREMES_HOME itself and the dirs in
    ...
    sanremes_home = os.path.normcase(os.path.realpath(str(get_sanremes_home())))
    return real in _non_workspace_dirs() or real == sanremes_home


def _repo_discovery_policy(raw: dict | None = None) -> dict:
    """Return the effective, profile-local Desktop repository scan policy."""
    from sanremes_cli.config import DEFAULT_CONFIG
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    zero SanRemes sessions still appear in the sidebar (#81723). Mirrors the
        ...
        try:
            for dirpath, dirnames, _filenames in os.walk(root):
                ...
                if ".git" in dirnames:
                    ...
                    # Not a repo: skip hidden dirs (e.g. .sanremes) and node_modules.
...
def _discover_repos_payload(
    ...
    repos even with zero sanremes sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (sanremes home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero SanRemes sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with SanRemes
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `sanremes` is interactive — use `/sanremes chat -q …` or run `sanremes` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`sanremes setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`sanremes gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`sanremes sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`sanremes config edit` needs $EDITOR in a real terminal"
    return None


def _resolve_name(name: str) -> str:
    try:
        from sanremes_cli.commands import resolve_command
...
def _model_picker_context(agent):
    ...
    from sanremes_cli.inventory import load_picker_context
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "SanRemes" if role == "assistant" else role.title()
...
def _format_live_help_output() -> str:
    try:
        from sanremes_cli.commands import COMMANDS_BY_CATEGORY
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("SANREMES_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("SANREMES_VOICE_TTS", "").strip() == "1"


def _any_session_running() -> bool:
    """True while any session's agent turn is in flight.

    Registered as the voice busy-probe (``sanremes_cli.voice.set_voice_busy_probe``)
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    os.environ["SANREMES_VOICE"] = "0"
                    os.environ["SANREMES_VOICE_TTS"] = "0"
                    try:
                        from sanremes_cli.voice import stop_continuous
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via sanremes_cli.voice.speak_text with spoken barge-in.
    ...
    """
    from sanremes_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["SANREMES_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["SANREMES_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from sanremes_cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from sanremes_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import sanremes_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from sanremes_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from sanremes_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            from sanremes_cli.browser_connect import (
