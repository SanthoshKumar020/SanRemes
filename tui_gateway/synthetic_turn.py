...
This module is a **test seam**: it is dead unless ``SANREMES_ISO_CERTIFY_SYNTH_TURN``
...
def synth_turn_armed() -> bool:
    ...
    return os.environ.get("SANREMES_ISO_CERTIFY_SYNTH_TURN") == "1"
...
class SyntheticHeavyAgent:
    ...
    def _parse_spec(message: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "duration_s": float(spec.get("duration_s", _env_float("SANREMES_ISO_CERTIFY_DURATION_S", 8.0))),
            ...
            "chunk": int(spec.get("chunk", _env_int("SANREMES_ISO_CERTIFY_CHUNK", 20_000))),
            ...
            "delta_interval_s": float(spec.get("delta_interval_s", _env_float("SANREMES_ISO_CERTIFY_DELTA_S", 0.05))),
            # Notional output tokens attributed per streamed delta (drives the
            # 100K+-token "heavy turn" proxy in usage/metadata).
            "tokens_per_delta": int(spec.get("tokens_per_delta", _env_int("SANREMES_ISO_CERTIFY_TPD", 512))),
