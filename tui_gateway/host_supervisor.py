...
def _default_registry_path() -> Path:
    return get_sanremes_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_sanremes_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_sanremes_home = expected_sanremes_home if expected_sanremes_home is not None else str(get_sanremes_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["SANREMES_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("sanremes_home") or "")
        if got_home and got_home != self.expected_sanremes_home:
            raise RuntimeError(f"compute host SANREMES_HOME mismatch: {got_home} != {self.expected_sanremes_home}")
