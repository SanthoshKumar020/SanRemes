...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SANREMES_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # SanRemes runtime / deployment
    "SANREMES_HOME", "SANREMES_PROFILE", "SANREMES_GATEWAY_LOCK_DIR",
    "SANREMES_MAX_ITERATIONS", "SANREMES_MAX_TOKENS", "SANREMES_API_TIMEOUT",
    "SANREMES_REDACT_SECRETS", "SANREMES_NOUS_TIMEOUT_SECONDS",
    "_SANREMES_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SANREMES_KANBAN_DB", "SANREMES_KANBAN_WORKSPACES_ROOT", "SANREMES_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "SANREMES_KANBAN_",
    "SANREMES_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset SanRemes writes
    ...
    ``sanremes_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical SanRemes parser: save_env_value
    ...
    from sanremes_cli.config import _parse_env_value
...
def build_profile_secret_scope(sanremes_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(sanremes_home)
    secrets = load_env_file(home / ".env")

    try:
        from sanremes_cli.env_loader import get_secret_source_values
