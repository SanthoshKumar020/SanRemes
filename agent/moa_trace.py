...
to ``<sanremes_home>/moa-traces/<session_id>.jsonl``. The record is the TRUE
...
def _traces_enabled_and_dir() -> Optional[Path]:
    ...
    ``moa.trace_dir`` overrides the default ``<sanremes_home>/moa-traces/``.
    """
    try:
    ...
    if override:
        ...
        base = get_sanremes_home() / "moa-traces"
    return base
