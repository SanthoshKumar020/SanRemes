...
See the ``sanremes-agent-dev`` skill (``references/self-improvement-loop.md``)
...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    calls. A provider that IS an autonomous agent reaches SanRemes through a client
    shim, and a shim that cannot carry SanRemes tool calls back turns the fork into
    ...
    exists so a shim that can't declares ``SUPPORTS_SANREMES_TOOL_CALLS = False``
    ...
    for candidate in (client, type(client) if client is not None else None):
        if candidate is None:
            continue
        supported = getattr(candidate, "SUPPORTS_SANREMES_TOOL_CALLS", None)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with SanRemes, e.g. 'sanremes-agent').\n"
    "  • Hub-installed skills (installed via 'sanremes skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'sanremes curator pin'). You are an "
    ...
    "'sanremes curator adopt <name>' — do not try to patch it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • Hub-installed skills (installed via 'sanremes skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'sanremes curator pin'). Pin blocks "
    ...
    "reply and recommend 'sanremes curator adopt <name>' instead.\n"
    "If the only skills that need updating are protected, say\n"
    "'Nothing to save.' and stop.\n\n"
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("SANREMES_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def _run_review_in_thread(
    ...
    # An agent-as-provider whose client can't carry SanRemes tool calls back would
    ...
    if not _parent_can_emit_tool_calls(agent) and not bool(
        ...
        logger.warning(
            "Background review skipped: provider %r cannot emit SanRemes tool calls, "
    ...
    try:
        ...
        with thread_scoped_silence():
            ...
            # scratch (fresh _sanremes_now() timestamp, fresh
