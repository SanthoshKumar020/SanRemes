...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into SanRemes with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.sanremes/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of SanRemes
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``sanremes tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``sanremes setup`` for
