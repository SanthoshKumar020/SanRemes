...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``sanremes logs``.
    ...
    if (
        ...
        # SanRemes only initiates compaction in "sanremes" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping SanRemes preflight compression for codex app-server "
                "(mode=%s); SanRemes will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by SanRemes (via _emit_status), so it always
