...
(https://github.com/NousResearch/sanremes-agent/issues/85125).
...
* :func:`resolve_timeout` — one config-first resolution path for timeout
  ...
  so new surfaces stop inventing ``SANREMES_*_TIMEOUT`` env vars (".env is for
...
* ``None`` timeout means unbounded, and non-positive resolved values are
  ...
  ``SANREMES_CONCURRENT_TOOL_TIMEOUT_S`` convention).
"""
...
class DeadlineExpired(TimeoutError):
    ...
    was SanRemes's own bound — error classification must not attribute it to
...
def clamp_timeout(timeout: Optional[float]) -> Optional[float]:
    ...
    * Non-positive values become ``None`` (unbounded) — matching the existing
      ``SANREMES_CONCURRENT_TOOL_TIMEOUT_S`` "0 disables the bound" convention.
