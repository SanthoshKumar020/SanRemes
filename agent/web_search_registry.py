...
def list_providers(*, scope: Optional[str] = None) -> List[WebSearchProvider]:
    ...
    with _lock:
        ...
        merged.update(_scoped_providers.get(scope or sanremes_home_key(), {}))
...
def get_provider(name: str, *, scope: Optional[str] = None) -> Optional[WebSearchProvider]:
    ...
    with _lock:
        key = name.strip()
        return _scoped_providers.get(scope or sanremes_home_key(), {}).get(key) or _providers.get(key)
...
# in keyless_mcp; an explicit `sanremes tools` pick bypasses this walk
...
def _resolve(configured: Optional[str], *, capability: str) -> Optional[WebSearchProvider]:
    ...
    with _lock:
        ...
        snapshot.update(_scoped_providers.get(sanremes_home_key(), {}))
