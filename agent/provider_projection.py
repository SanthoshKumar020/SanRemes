"""Fold an agent-as-provider's own activity back into SanRemes' turn state.

Most providers are models: they ask SanRemes to run a tool and SanRemes runs it, so
...
time SanRemes sees the response that work is already done.

Those calls must never come back as pending ``tool_calls`` — SanRemes would re-run
...
* the **skill-review nudge**, whose counter (``_iters_since_skill``) only moves
  on SanRemes tool iterations, of which there are none.

So the provider client hands both back on the completion object and this helper
applies them: ``sanremes_projected_messages`` (already-completed
``assistant(tool_calls=[…])`` + ``tool(result)`` history rows) and
``sanremes_provider_tool_iterations`` (how many tool iterations happened inside
...
def splice_provider_projection(
    ...
    """
    projected = getattr(response, "sanremes_projected_messages", None)
    ...
    raw_iters = getattr(response, "sanremes_provider_tool_iterations", 0)
    try:
