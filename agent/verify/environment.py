...
The manifest lives at ``<project>/.sanremes/environment.json`` and is the
...
MANIFEST_VERSION = 1
_MANIFEST_RELPATH = Path(".sanremes") / "environment.json"
...
def load_manifest(root: Path) -> Recipe | None:
    ...
    fresh detection instead of breaking ``sanremes verify``.
    """
