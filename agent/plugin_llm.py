...
Plugins built on SanRemes Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing SanRemes
...
:class:`~sanremes_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors SanRemes' main config
...
handles every provider, fallback chain, and per-task override SanRemes
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`sanremes_cli.plugins.PluginContext`
