...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into SanRemes accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    SanRemes' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, SanRemes should still count that turn
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in SanRemes state.

    The app-server owns the compacted thread context, so SanRemes should not
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior SanRemes deterministic-fallback
...
# ---------------------------------------------------------------------------
# Codex app-server → SanRemes UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal SanRemes tool loop. Without this bridge gateway
...
# Codex item types that map to a SanRemes tool_call in the projector (and
...
# Internal MCP server that wraps SanRemes' native tools for codex. When
# codex calls back through it, the inner dispatch runs in a SEPARATE
# sanremes-tools-mcp-server subprocess that has no access to the parent
...
# the display event for those calls; we strip the mcp.sanremes-tools.*
# namespacing and emit the bare tool name (web_search, browser_navigate,
# vision_analyze, ...) since the user thinks of these as SanRemes tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "sanremes-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic SanRemes tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (SanRemes' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    notifications into SanRemes' gateway UI callbacks.
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into SanRemes'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to SanRemes' standard prompt flow if a
        ...
        # explicitly opted out of SanRemes approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / SANREMES_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing SanRemes UI. Defaults (manual/smart/unset) preserve the
        ...
        # item/agentMessage/delta, ...) into SanRemes' gateway UI callbacks
...
def _is_plain_json_data(value: Any) -> bool:
    ...
    format.  SanRemes assembles Codex payloads from JSON round-trips, so they
...
def _bypass_sdk_request_transform(stream_kwargs: dict) -> dict:
    ...
    the transform.  Set SANREMES_CODEX_SDK_TRANSFORM=1 to restore the pre-fix
    behavior.
    """
    if os.environ.get("SANREMES_CODEX_SDK_TRANSFORM", "").strip().lower() in {
        "1", "true", "yes", "on"
    }:
