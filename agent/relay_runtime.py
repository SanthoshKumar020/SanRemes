"""Profile-scoped NeMo Relay runtimes owned by the SanRemes agent core."""
...
from sanremes_cli.relay_plugin_cutover import (
...
SESSION_SCOPE = "sanremes.session"
TURN_SCOPE = "sanremes.turn"
LOGICAL_LLM_SCOPE = "sanremes.logical_llm_call"
RUNTIME_SCHEMA_KEY = "sanremes.relay.schema_version"
RUNTIME_SCHEMA_VERSION = "sanremes.relay.runtime.v1"
RUNTIME_INSTANCE_KEY = "sanremes.relay.runtime_instance"
RELAY_PLUGINS_EXECUTION_CONSUMER = "sanremes.nemo_relay.plugins"
...
def pop_relay_scope(
    ...
    *,
    ...
    'metadata'`` when SanRemes finalization forwards runtime metadata. Filter to
...
@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a SanRemes session."""
    ...
    def run_in_session(
        ...
        **kwargs: Any,
        ...
        with self._lock:
            ...
            if runtime is None:
                raise RuntimeError("SanRemes Relay operation lease is released")
...
class _ProcessRelayPluginConfiguration:
    ...
    def acquire(
        ...
        with self._lock:
            ...
            if self._active and not self._clear_active():
                logger.warning(
                    "SanRemes Relay plugin cleanup is still pending; refusing to "
            ...
            try:
                ...
                logger.warning(
                    "SanRemes could not determine whether a process-global Relay "
            ...
            if existing_report is not None:
                logger.warning(
                    ...
                    "outside SanRemes native ownership; leaving it unchanged and "
                    "disabling SanRemes-managed Relay middleware for this process"
                )
                return self._remember(
                    owner_id,
            ...
            try:
                ...
                if dynamic_plugins:
                    try:
                        ...
                        raise RuntimeError(
                            "SanRemes Relay dynamic plugin activation failed"
                        ) from exc

                if self._activation is None:
                    # SanRemes only enters Relay's initialization path after an
                ...
                logger.warning(
                    "SanRemes Relay plugin initialization failed: %s",
                    exc,
                    exc_info=True,
                )
                return self._remember(
            ...
            logger.info(
                ...
                "hosted by this SanRemes process."
            )
            return state
    ...
    def _clear_active(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "SanRemes Relay plugin subscriber flush failed",
                exc_info=True,
            )
            return False
        try:
            ...
            logger.warning(
                "SanRemes Relay plugin configuration cleanup failed",
                exc_info=True,
            )
            return False
        self._active = False
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a SanRemes-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
        ...
        with session.lock:
            ...
            try:
                self.run_in_session(
                    ...
                    old_handle,
                    output={"sanremes.session.segment_reason": reason},
                    metadata={
                        RUNTIME_SCHEMA_KEY: RUNTIME_SCHEMA_VERSION,
                        RUNTIME_INSTANCE_KEY: self.runtime_id,
                    },
                ...
                logger.warning(
                    "SanRemes Relay segment close failed (session=%s segment=%d); "
            ...
            scope_metadata = {
                RUNTIME_SCHEMA_KEY: RUNTIME_SCHEMA_VERSION,
                RUNTIME_INSTANCE_KEY: self.runtime_id,
                "sanremes.session.segment": session.segment,
                "sanremes.session.segment_reason": reason,
            }
            parent_handle = None
            if session.parent_session_id:
                with self._sessions_lock:
            ...
            try:
                ...
                logger.warning(
                    "SanRemes Relay segment open failed (session=%s segment=%d); "
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active SanRemes Relay session without creating one."""
        with self._sessions_lock:
            if self._closing:
                return None
    ...
    def get_session_handle(self, session_id: str) -> Any:
        """Return the Relay parent handle for a SanRemes session, if active."""
        ...
        **kwargs: Any,
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("SanRemes Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("SanRemes Relay session context is unavailable")
    ...
    def _begin_operation(self) -> None:
        ...
        with self._sessions_lock:
            if self._closing:
                raise RuntimeError("SanRemes Relay runtime is shutting down")
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the SanRemes session identified by ``event``."""
    ...
    def apply_tool_request_intercepts(
        ...
        *,
        ...
        """Apply Relay request rewriting before SanRemes authorizes a tool call."""
        ...
        def close_with_drain() -> None:
            ...
            for _ in range(drain_limit):
                ...
                try:
                    ...
                    logger.warning(
                        "SanRemes Relay orphaned scope drain failed",
                        exc_info=True,
                    )
                    break
            ...
            if drained_holder["count"]:
                logger.warning(
                    "SanRemes Relay drained %d orphaned scope(s) before closing %s",
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "SanRemes Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        if has_active_operations:
            thread = threading.Thread(
                ...
                name=f"sanremes-nemo-relay-shutdown-{self.runtime_id[:8]}",
                daemon=True,
            )
            try:
                thread.start()
                ...
                with self._sessions_lock:
                    self._shutdown_started = False
                logger.warning(
                    "SanRemes Relay deferred shutdown could not start",
                    exc_info=True,
                )
            return
    ...
    def _finish_shutdown(self) -> None:
        try:
            ...
            with self._sessions_lock:
                self._shutdown_started = False
            logger.warning("SanRemes Relay shutdown failed", exc_info=True)
            return
        with self._sessions_lock:
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("SanRemes Relay runtime operation failed", exc_info=True)
            return None
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical SanRemes profile."""
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "SanRemes Relay runtime initialization failed", exc_info=True
                )
...
class RelayTurnContext:
    """Runtime-only context for one SanRemes turn or top-level task."""
...
_CURRENT_TURN: contextvars.ContextVar[RelayTurnContext | None] = contextvars.ContextVar(
    "sanremes_relay_turn", default=None
)

# Depth of managed Relay callbacks executing on the current logical call path.
# Set >0 while the native Relay pipeline is mid-dispatch of a SanRemes callback
...
_MANAGED_CALLBACK_DEPTH: contextvars.ContextVar[int] = contextvars.ContextVar(
    "sanremes_relay_managed_callback_depth", default=0
)
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for SanRemes core."""
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "SanRemes Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                metadata = {"sanremes.execution_surface": platform or "unknown"}
                ...
                logger.warning(
                    "SanRemes Relay conversation initialization failed",
                    exc_info=True,
                )
    ...
    def begin_turn(
        ...
        *,
        ...
        if lease.released:
            raise RuntimeError("SanRemes Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            ...
            if active:
                ...
                # SanRemes turns would create sibling scopes on that stack, but
                # their completion order is not guaranteed to be LIFO.
                turn.relay_enabled = False
                logger.warning(
                    "Skipping Relay instrumentation for concurrent SanRemes turn "
                    "%s in session %s",
                    turn_id,
                    lease.session_id,
                )
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "SanRemes Relay segment rotation failed", exc_info=True
                )
            try:
                turn.handle = lease.host.run_in_session(
                    ...
                    metadata={
                        ...
                        "sanremes.execution_surface": lease.platform or "unknown",
                    },
                    timeout=_SCOPE_OP_TIMEOUT,
                )
            except Exception:
                logger.warning("SanRemes Relay turn initialization failed", exc_info=True)
            ...
            try:
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        ...
                        if failure:
                            logger.warning(
                                "SanRemes Relay turn finalization failed: %s",
                                failure,
                            )
            finally:
                try:
                ...
                try:
                    ...
                    logger.warning(
                        "SanRemes Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
    ...
    def _consume_deferred_close(self, lease: Any) -> None:
        ...
        try:
            ...
            logger.warning(
                "SanRemes Relay deferred session close failed", exc_info=True
            )
    ...
    def notify_session_compacted(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "SanRemes Relay compaction notification failed", exc_info=True
            )
    ...
    def _finish_logical_calls(
        ...
        *,
        ...
        for index in range(len(logical_calls) - 1, -1, -1):
            ...
            logger.warning("SanRemes Relay logical LLM finalization failed: %s", failure)
            break
...
def resolve_execution_context(
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0:
        ...
        # a SanRemes tool). Nested managed execution here is structurally
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a SanRemes session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("SanRemes Relay mark failed: %s", name, exc_info=True)
        return False
...
def apply_tool_request_intercepts(
    *,
    ...
    """Return Relay-rewritten arguments at SanRemes's authorization boundary."""
    if not session_id:
        return args
    runtime = get_runtime(create=False)
    if runtime is None:
...
def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by SanRemes core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("SanRemes Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared SanRemes session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("SanRemes Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("SanRemes Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared SanRemes session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("SanRemes Relay runtime is unavailable")
    session = runtime.get_session(session_id)
...
def get_runtime(
    *,
    ...
    """Return the Relay host for the active SanRemes profile."""
...
def _configured_plugin_inputs(
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError(
                "SanRemes [[dynamic_plugins]] records are unsupported; use Relay "
        ...
        raise _RelayPluginConfigurationLoadError(
            "SanRemes Relay plugin configuration could not be loaded from "
...
def _resolve_plugin_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="sanremes-nemo-relay-plugin-lifecycle",
        daemon=True,
    )
    thread.start()
    thread.join()
