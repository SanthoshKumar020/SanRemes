"""Vertex AI (Google Cloud) adapter for SanRemes Agent.

Provides authentication and configuration for Vertex AI's OpenAI-compatible
endpoint. This allows SanRemes to use Gemini models via Google Cloud with
...
# who installed plain `sanremes-agent` and only later selected a Gemini model.
try:
...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed SanRemes at Vertex.
    ...
    SanRemes up for Vertex would suddenly see it (and could spend against those
    credentials). So this checks only SanRemes-scoped signals:
      ...
      * a resolvable ``VERTEX_CREDENTIALS_PATH`` service-account file
        (the SanRemes-specific path var — NOT ``GOOGLE_APPLICATION_CREDENTIALS``).
    """
    if _resolve_project_override():
        return True
