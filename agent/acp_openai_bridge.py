"""OpenAI-shape bridge shared by SanRemes' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach SanRemes as
...
the agent's *own* tool notifications. SanRemes' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from SanRemes' perspective).
...
of its own (Copilot) forwards everything SanRemes offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only SanRemes'
agent-level tools, because re-offering the overlapping ones makes SanRemes re-run
...
class StreamChunks(list):
    ...
    SanRemes reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``sanremes_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just SanRemes'
