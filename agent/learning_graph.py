...
def _sanremes_meta(fm: dict[str, Any]) -> dict[str, Any]:
    """``metadata.sanremes`` as a dict, tolerant of the string-valued frontmatter
    ...
    return sanremes if isinstance(sanremes, dict) else {}


def _related(fm: dict[str, Any]) -> list[str]:
    raw = fm.get("related_skills") or _sanremes_meta(fm).get("related_skills")
...
def _category(fm: dict[str, Any], skill_md: Path) -> str:
    cat = fm.get("category") or _sanremes_meta(fm).get("category")
    if cat:
        return str(cat)
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        path = get_sanremes_home() / "skills" / ".usage.json"
        try:
...
def _memory_cards() -> list[dict[str, Any]]:
    ...
    """
    base = get_sanremes_home() / "memories"
...
def _skill_roots() -> list[tuple[str, Path]]:
    ...
    home_skills = get_sanremes_home() / "skills"
