...
MoA-enabled; the normal SanRemes agent loop still owns tool calling and turn
...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from sanremes_cli.moa_config import coerce_privacy_filter
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of SanRemes boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model SanRemes agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "SanRemes agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal SanRemes agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
...
def build_moa_facade(agent, preset_name: Any = None) -> MoAClient:
    ...
    try:
        from sanremes_cli.config import load_config
        from sanremes_cli.moa_config import normalize_moa_config
