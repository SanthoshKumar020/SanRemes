"""Upload a SanRemes session transcript to Hugging Face as an agent trace.

SanRemes stores sessions in its own SQLite store (``sanremes_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``sanremes trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through SanRemes' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "sanremes-traces"
_SANREMES_VERSION = "sanremes-agent"
...
# ---------------------------------------------------------------------------
# Conversion: SanRemes OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses SanRemes' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render SanRemes conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.sanremes/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `sanremes trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/sanremes-traces`` dataset. Returns a
