"""Lightweight internationalization (i18n) for SanRemes static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by SanRemes itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``SANREMES_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``SANREMES_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("SANREMES_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "SANREMES_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a sanremes dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`sanremes_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("SANREMES_LANGUAGE")
    if env_lang:
