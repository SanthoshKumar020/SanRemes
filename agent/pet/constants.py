...
row taxonomy is inferred from the atlas shape so SanRemes can render both legacy
...
# rows (1536x1872), while older SanRemes/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are SanRemes' activity state names. They are not always identical to the
...
# Legacy SanRemes/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical SanRemes activity names -> accepted row-name aliases in descending
