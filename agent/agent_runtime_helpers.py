...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-SanRemes-Session-Id header), and an
        ...
        if env_var_enabled("SANREMES_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from sanremes_cli.config import load_config as _load_moa_cfg
    ...
    # that are canonicalized only after SanRemes sends the request. Honor its
    ...
    if not _supports_anthropic_cache_markers:
        ...
        from sanremes_cli.providers import custom_provider_aliases
    ...
    if _supports_anthropic_cache_markers and (
        ...
        try:
            from sanremes_cli.config import get_custom_provider_model_capability
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to sanremes's outer conversation loop,
    ...
    if agent.provider == "opencode-free":
        from sanremes_cli.models import opencode_zen_free_headers
    ...
    # All primary construction and recovery paths must identify SanRemes to the
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from sanremes_cli.providers import determine_api_mode
    ...
    from sanremes_cli.models import opencode_provider_family
    ...
    try:
        ...
        from sanremes_cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from sanremes_cli.middleware import apply_tool_request_middleware
    ...
    if not pre_tool_block_checked:
        try:
            from sanremes_cli.plugins import _dispatch_pre_tool_call_hooks
    ...
    from sanremes_cli.middleware import run_tool_execution_middleware
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: sanremes' own dispatch loop intentionally keeps an
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    SanRemes' keepalive client (#10324 / ``_build_keepalive_http_client``) and
