...
Storage
-------
Bundles live in ``~/.sanremes/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SANREMES_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``sanremes bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SANREMES_HOME.

    Honors ``SANREMES_BUNDLES_DIR`` for tests; falls back to
    ``<SANREMES_HOME>/skill-bundles``.
    """
    override = os.environ.get("SANREMES_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_sanremes_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `sanremes bundles` CLI subcommand.
# ---------------------------------------------------------------------------
