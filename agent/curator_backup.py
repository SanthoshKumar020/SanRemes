...
A pre-run snapshot of ``~/.sanremes/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.sanremes/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.sanremes/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
from sanremes_cli.sizefmt import format_bytes
...
def _backups_dir() -> Path:
    return get_sanremes_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.sanremes/cron/jobs.json``)."""
    return get_sanremes_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.sanremes/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.sanremes/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.sanremes/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.sanremes/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `sanremes curator rollback --list` to see available snapshots)",
            None,
        )
