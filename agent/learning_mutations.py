...
- **skills** → the skill name (e.g. ``"debugging-sanremes-desktop"``)
...
shared by the CLI (``sanremes journey delete|edit``), the TUI ``/journey`` overlay
(gateway RPCs), and the desktop GUI (REST). Deleting a skill *archives* it
(recoverable via ``sanremes curator restore``); deleting a memory rewrites its
...
def _memories_dir() -> Path:
    ...
    return get_sanremes_home() / "memories"
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    if skill_usage.get_record(name).get("pinned"):
        return {"ok": False, "message": f"'{name}' is pinned — unpin it first (sanremes curator unpin {name})"}

    ok, message = skill_usage.archive_skill(name)
    if ok:
        _clear_skill_cache()

    return {"ok": ok, "message": f"archived '{name}' — restore with: sanremes curator restore {name}" if ok else message}
