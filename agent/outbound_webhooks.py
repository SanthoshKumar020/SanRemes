...
(``gateway/platforms/webhook.py``): inbound wakes SanRemes when the world
changes; outbound tells the world when SanRemes does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-SanRemes-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``SANREMES_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.sanremes/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/sanremes-events
          ...
          secret_env: SANREMES_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `sanremes hooks list`
...
Headers::
    ...
    User-Agent:              SanRemes-Agent-Outbound-Webhook
    X-SanRemes-Event:          <hook event name>
    X-SanRemes-Delivery:       <delivery_id>
    X-SanRemes-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("SANREMES_SAFE_MODE"):
        logger.info("SANREMES_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _parse_single_target(index: int, raw: Any) -> Optional[WebhookTarget]:
    from sanremes_cli.plugins import VALID_HOOKS
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-SanRemes-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-SanRemes-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
