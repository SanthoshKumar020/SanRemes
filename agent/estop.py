...
``sanremes pause`` writes a sentinel file at ``$SANREMES_HOME/ESTOP``;
``sanremes resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "SanRemes is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.sanremes/ESTOP``.
...
def _sanremes_home() -> Path:
    """Resolve the active SANREMES_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active SANREMES_HOME."""
    return _sanremes_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on SANREMES_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ SanRemes is paused ({reason}). New work is on hold; "
            "run `sanremes resume` to pick things back up."
        )
    return (
        "⏸️ SanRemes is paused. New work is on hold; "
        "run `sanremes resume` to pick things back up."
    )
