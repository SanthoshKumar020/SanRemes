...
from sanremes_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.sanremes/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    SANREMES_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``SANREMES_KANBAN_TASK`` / ``SANREMES_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("SANREMES_KANBAN_TASK") or os.getenv("SANREMES_KANBAN_BOARD"):
        ...
        try:
            from sanremes_constants import is_container

            result = is_container()
        except Exception:
            result = False
    elif env == "s6":
        # The SanRemes Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``sanremes_cli.config`` on the
...
# `sanremes-agent` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting SanRemes itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with SanRemes, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"sanremes-agent"})
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``SANREMES_PLATFORM`` or
            ``SANREMES_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("SANREMES_PLATFORM")
        or get_session_env("SANREMES_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``sanremes config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``sanremes`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.sanremes/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``sanremes`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against SANREMES_HOME, not cwd
        if not p.is_absolute():
            p = (sanremes_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.sanremes/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.sanremes/skills`` at the git root) are
...
#   <root>/.sanremes/skills/   — SanRemes-native location
...
# a one-line "run `sanremes skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when SANREMES_HOME itself lives inside a git checkout:
    ``<root>/.sanremes/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``sanremes skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`sanremes skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under SANREMES_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_sanremes_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.sanremes/skills/`` (including via symlinks) or configured
    ...
    # ~/.sanremes/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: SanRemes can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    sanremes = metadata.get("sanremes") or {}
    if not isinstance(sanremes, dict):
        sanremes = {}
    return {
        "fallback_for_toolsets": sanremes.get("fallback_for_toolsets", []),
        "requires_toolsets": sanremes.get("requires_toolsets", []),
        "fallback_for_tools": sanremes.get("fallback_for_tools", []),
        "requires_tools": sanremes.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    sanremes = metadata.get("sanremes")
    if not isinstance(sanremes, dict):
        return []
    raw = sanremes.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes SanRemes metadata, VCS, virtualenv/dependency, cache, and skill
