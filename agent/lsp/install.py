...
appropriate.  All installs go to a SanRemes-owned bin staging dir,
``<SANREMES_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``sanremes lsp
  status``.
...
from sanremes_constants import find_node_executable
...
# ``<SANREMES_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `sanremes lsp status` reports the host's presence.
...
def sanremes_lsp_bin_dir() -> Path:
    """Return the SanRemes-owned bin staging dir for LSP servers."""
    from sanremes_constants import get_sanremes_home

    p = get_sanremes_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(sanremes_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $SANREMES_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that SanRemes installed and
    ...
    if npm is None:
        ...
        return None
    staging = sanremes_lsp_bin_dir().parent  # <SANREMES_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = sanremes_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a sanremes-owned target dir.
    ...
    console script.
    """
    pip_target = sanremes_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = sanremes_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``sanremes lsp status`` CLI to give users a quick
