...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<SANREMES_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<SANREMES_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    session_path = os.path.join(
        sanremes_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(sanremes_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
        ...
        f"-HostName SanRemes -HostProfileId sanremes -HostVersion 1.0.0 "
...
def sanremes_lsp_session_dir() -> str:
