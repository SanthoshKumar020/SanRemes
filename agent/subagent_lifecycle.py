"""Public, plugin-safe lifecycle API for delegated SanRemes subagents.
...
_EXECUTOR = _DaemonExecutor(max_workers=8, thread_name_prefix="sanremes-lifecycle")
_SECRET = secrets.token_bytes(32)
_ACTIVE_PARENT_AGENT: contextvars.ContextVar[Any] = contextvars.ContextVar(
    "sanremes_subagent_lifecycle_parent", default=None
)
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError(
                "No active SanRemes parent session is available."
            )
        ...
        if not subagent_id:
            raise SubagentLifecycleError("SanRemes failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
    ...
    def _validate_request(request: SubagentLaunchRequest, parent: Any) -> None:
        ...
        if request.working_directory is not None:
            raise SubagentLifecycleError(
                "working_directory is not supported because SanRemes delegates use isolated task environments."
            )
        if request.blocked_tools:
            raise SubagentLifecycleError(
                "Per-tool blocking is not supported; use allowed_toolsets. SanRemes always blocks unsafe child tools."
            )
        try:
            metadata_bytes = len(
