...
from sanremes_constants import OPENROUTER_MODELS_URL
...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``SANREMES_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
    ...
    if base_url:
        try:
            from sanremes_cli.config import get_custom_provider_tls_settings
    ...
    for env_var in ("SANREMES_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_sanremes_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from sanremes_constants import get_sanremes_home
    return get_sanremes_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run SanRemes Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent SanRemes from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for SanRemes' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets SanRemes minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from sanremes_constants import get_sanremes_home
    return get_sanremes_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # SanRemes grow conversations past the runtime limit and Ollama
...
# who never asked for it. The variant suffix is a SanRemes-side alias: it is
...
# SanRemes-side picker suffix that opts a Codex slug into the live-verified
...
def strip_codex_context_variant_suffix(model: Optional[str]) -> str:
    ...
    The suffix is a SanRemes picker alias (``gpt-5.6-sol-900k``); the Codex
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    # ``-900k`` variants are SanRemes picker aliases — the Codex catalog only
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from sanremes_cli.models import get_copilot_model_context
    ...
    # ``SanRemes-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets SanRemes sends to providers:
