...
def _diff_ansi() -> dict[str, str]:
    ...
    try:
        from sanremes_cli.skin_engine import get_active_skin
...
def build_status_phrase(tool_name: str, args: dict | None, max_len: int = 49) -> str | None:
    ...
    name ("SanRemes is running …"), so it starts lowercase with "is".
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in SanRemes' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("SANREMES_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
