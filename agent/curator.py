...
def _state_file() -> Path:
    return get_sanremes_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.sanremes/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `sanremes update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``sanremes curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``sanremes update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``sanremes curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `sanremes curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as SanRemes' background skill CURATOR. This is an "
    ...
    "into ~/.sanremes/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: sanremes-config-*, sanremes-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.sanremes/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.sanremes/logs/curator/``)
    ...
    authored skill data in ``~/.sanremes/skills/``.

    ``ensure_sanremes_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_sanremes_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every sanremes install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: sanremes curator status
        keep an umbrella stable: sanremes curator pin document-tools
    ...
    lines.append("full report: sanremes curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: sanremes curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.sanremes/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.sanremes/skills/.archive/` for "
            "safety and can be restored via `sanremes curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.sanremes/skills/.archive/`. "
            "Restore any via `sanremes curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `sanremes curator restore <name>`")
    lines.append("- All archives live under `~/.sanremes/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``sanremes curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `sanremes curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `sanremes update` only if "
        ...
        # recorded in state so `sanremes curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``sanremes model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `sanremes model` → auxiliary
