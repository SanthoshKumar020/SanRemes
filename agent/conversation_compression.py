...
# Mirror sanremes_cli.config.DEFAULT_CONFIG["compression"] keys of the same name.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``sanremes_state.SessionDB`` class (and its live instances) is old.
...
def _adopt_live_compression_child(
    ...
    try:
        ...
        os.environ["SANREMES_SESSION_ID"] = child_session_id
    try:
...
def _direct_messages_for_pre_compress_memory(messages: Any) -> list[dict[str, Any]]:
    ...
    receive one normalized host contract instead of having to infer SanRemes
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `sanremes setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by SanRemes "
        ...
        if aux_context < threshold:
            ...
            if threshold_suggestion_viable:
                ...
                msg += (
                    ...
                    f"SanRemes's small-context floor and output reservation "
...
def compress_context(
    ...
    *,
    ...
    # SanRemes' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|sanremes|off).
    # The memory-provider context handoff below is intentionally SanRemes-only:
    ...
    try:
        ...
        if agent._session_db:
            ...
            try:
                ...
                if in_place:
                    ...
                    # whose thread lacks the SANREMES_HOME context.
                    try:
                    ...
                    agent._session_db.publish_compression_child(
                        ...
                        or os.environ.get("SANREMES_SESSION_SOURCE", "cli"),
                        model=agent.model,
                    ...
                    try:
                        from sanremes_cli.goals import migrate_goal_to_session
                        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
                    except Exception as _goal_err:
                        logger.debug("Could not migrate goal on compression: %s", _goal_err)
                    # Same boundary hazard for /heartbeat state — carry it too.
                    try:
                        from sanremes_cli.heartbeat import migrate_heartbeat_to_session
        ...
        # engines (e.g. sanremes-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See sanremes-lcm#68. Built-in ContextCompressor
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    SanRemes' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep SanRemes' transcript
    unchanged.
    """
    auto_mode = str(
        getattr(agent, "codex_app_server_auto_compaction", "native") or "native"
    ).lower()
    if auto_mode not in {"native", "sanremes", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "sanremes":
        logger.info(
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="sanremes_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
