...
* The ``iron-proxy`` binary is auto-installed into ``<sanremes_home>/bin/iron-proxy``
  on first use.  SanRemes pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<sanremes_home>/proxy/ca.{crt,key}`` is generated on
  first ``sanremes egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<sanremes_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``sanremes egress start``), pidfile
  at ``<sanremes_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<sanremes_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``sanremes egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<sanremes_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "SANREMES_IRON_PROXY_MGMT_KEY"
...
# credential (SanRemes' auth.py keys Google on both GEMINI_API_KEY and
...
def _sanremes_bin_dir() -> Path:
    from sanremes_constants import get_sanremes_home

    return get_sanremes_home() / "bin"
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.sanremes/proxy/`` just to
    check whether a pidfile exists.
    """
    from sanremes_constants import get_sanremes_home

    return get_sanremes_home() / "proxy"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<sanremes_home>/bin/iron-proxy``  (our managed copy — preferred)
    ...
    managed = _sanremes_bin_dir() / _platform_binary_name()
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``sanremes proxy install`` surface lets them
    propagate so the wizard can show a clear error.
    """

    bin_dir = _sanremes_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="sanremes-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "sanremes-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``sanremes egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="sanremes-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
        ...
        subprocess.run(  # noqa: S603
            [
                ...
                "-key", str(tmp_key),
                "-sha256", "-days", "3650",
                "-subj", "/CN=sanremes iron-proxy CA",
...
def mint_proxy_token(prefix: str = "sanremes-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<sanremes_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``SANREMES_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``sanremes egress reload`` reads the same file to authenticate.
    """
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="sanremes-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `sanremes egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `sanremes egress setup` and use "
            "`sanremes egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `sanremes egress setup`, "
            "then `sanremes egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``sanremes egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `sanremes egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<sanremes_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a SanRemes dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``sanremes egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``sanremes egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_SANREMES_IRON_PROXY_NONCE_ENV = "SANREMES_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_SANREMES_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `sanremes egress install`."
        )
    ...
    if not cfg.exists():
        raise RuntimeError(
            ...
            "Run `sanremes egress setup` first."
        )
    ...
    env[_SANREMES_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the SanRemes
    # process.
    #
    ...
    # ``sanremes egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``sanremes egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `sanremes egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`sanremes egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `sanremes secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
