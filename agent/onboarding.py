...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time SanRemes starts and finds ``~/.openclaw/``.

    Points users at ``sanremes claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``sanremes claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to SanRemes, run "
        "`sanremes claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `sanremes claw cleanup` (renames it to ~/.openclaw.pre-migration — "
