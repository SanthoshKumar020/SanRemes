...
from sanremes_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.sanremes/.env``. See NousResearch/sanremes-agent#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'sanremes memory status'. Note: "
        "systemd/gateway services do not inherit ~/.sanremes/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from sanremes_cli.providers import SANREMES_OVERLAYS, get_provider

        overlay = SANREMES_OVERLAYS.get(provider)
    ...
    try:
        ...
        from sanremes_cli.models import normalize_provider as normalize_model_provider
        from sanremes_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: sanremes config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$SANREMES_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_sanremes_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$SANREMES_HOME`` just means the notice
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — SanRemes never even initiates the compaction), so no
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .sanremes.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / SANREMES_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.sanremes/SOUL.md as the primary
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
        ...
        try:
            from sanremes_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
    ...
    try:
        from sanremes_cli.model_normalize import (
    ...
    agent._cache_ttl = "5m"
    try:
        from sanremes_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind SANREMES_DEV_CREDITS.
    ...
    # both live under ~/.sanremes/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from sanremes_logging import setup_logging, setup_verbose_logging
    setup_logging(sanremes_home=_ra()._sanremes_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that sanremes_logging.setup_logging() does not
        ...
        # handler level inside sanremes_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from sanremes_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from sanremes_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = sanremes_xai_default_headers()
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `sanremes model` to "
                        "select a provider, or run `sanremes setup` for first-time "
                        "configuration."
                    )
    ...
    # A multiplexed gateway may enter a different SANREMES_HOME after
    ...
    # worker (kanban_show tool is present iff SANREMES_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["SANREMES_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.sanremes/sessions/ alongside gateway sessions
    sanremes_home = get_sanremes_home()
    agent.logs_dir = sanremes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.sanremes/sessions/session_{sid}.json)
    ...
    try:
        from sanremes_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `sanremes --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from sanremes_cli.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical SanRemes behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "sanremes"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace SanRemes' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "sanremes", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, sanremes, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from sanremes_cli.config import split_model_config_default
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from sanremes_cli.auth import resolve_provider as resolve_auth_provider
        ...
        if not _configured_base_url and _custom_provider_candidate:
            ...
            if isinstance(_user_providers, dict):
                from sanremes_cli.config import is_provider_enabled
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from sanremes_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from sanremes_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by SanRemes Agent.  Choose a model with at least "
    ...
    # Nous SanRemes 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from sanremes_cli.model_switch import _check_sanremes_model_warning

            _sanremes_warn = _check_sanremes_model_warning(agent.model or "")
            if _sanremes_warn:
                _user_msg = (
                    "⚠ Nous Research SanRemes 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_sanremes_warn)
        except Exception:
            pass
