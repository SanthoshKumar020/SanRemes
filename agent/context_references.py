...
from sanremes_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
_SENSITIVE_SANREMES_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    sanremes_home = get_sanremes_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(sanremes_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(sanremes_home / rel for rel in _SENSITIVE_SANREMES_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal SanRemes path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.sanremes/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal SanRemes path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
