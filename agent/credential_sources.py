"""Unified removal contract for every credential source SanRemes reads from.

SanRemes seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.sanremes/.env
    claude_code   — ~/.claude/.credentials.json
    sanremes_pkce   — ~/.sanremes/.anthropic_oauth.json
    ...
    manual        — user ran `sanremes auth add`
...
unify here is **removal**:
    ...
    ``sanremes auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), sanremes_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.sanremes/.env  → clear it
    ...
    """
    from sanremes_cli.config import get_env_path, remove_env_value
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in sanremes_cli/config.py. read_text() with no
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.sanremes/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to SanRemes.",
            f"  The pool entry is now suppressed — SanRemes will ignore "
            f"{env_var} until you run `sanremes auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so SanRemes stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `sanremes auth add anthropic` to re-enable if needed.",
    ])


def _remove_sanremes_pkce(provider: str, removed) -> RemovalResult:
    """~/.sanremes/.anthropic_oauth.json is ours — delete it outright."""
    from sanremes_constants import get_sanremes_home

    result = RemovalResult()
    oauth_file = get_sanremes_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared SanRemes Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `sanremes auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `sanremes auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``sanremes auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `sanremes model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the SanRemes auth store is not enough — _seed_from_singletons() would
    ...
    via ``sanremes auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `sanremes auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `sanremes auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    SanRemes stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `sanremes auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="sanremes_pkce",
        remove_fn=_remove_sanremes_pkce,
        description="~/.sanremes/.anthropic_oauth.json",
    ))
    register(RemovalStep(
