...
Every SanRemes request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to SanRemes Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=sanremes-agent",
        "client=sanremes-client-v<__version__>",
    ]

The version is sourced live from ``sanremes_cli.__version__`` so it auto-aligns
...
``sanremes_cli.__version__`` is the canonical source of truth.
"""
...
def _sanremes_version() -> str:
    """Return the current SanRemes release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``sanremes_cli`` cannot be imported (should
...
def sanremes_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=sanremes-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=sanremes-client-v{_sanremes_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a SanRemes session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    SanRemes conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.sanremes/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific SanRemes
    ...
    """
    tags = ["product=sanremes-agent", sanremes_client_tag()]
