...
``~/.sanremes/.env`` has loaded and BEFORE the rest of SanRemes reads
``os.environ``.
...
* **Startup-time, synchronous.**  ``fetch()`` is called once per process
  (per SANREMES_HOME) by the orchestrator in
...
_SOURCE_ENVIRONMENT = ContextVar("sanremes_secret_source_environment", default=None)
...
class ErrorKind(str, Enum):
    ...
    A fixed vocabulary keeps startup warnings and ``sanremes secrets status``
...
class SecretSource(ABC):
    ...
    Attributes:
        ...
        label: Human-readable name used in startup messages and
            ``sanremes secrets status`` (e.g. ``"Bitwarden Secrets Manager"``).
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        may be malformed.  ``home_path`` is the resolved SANREMES_HOME.
        """
    ...
    def remediation(self, kind: Optional["ErrorKind"], cfg: dict) -> str:
        ...
        Called by the startup status printer (and ``sanremes secrets ...
        ...
        *what to run* next to fix it — not just what broke.  Sources
        ...
        ``sanremes secrets bitwarden token`` for AUTH_FAILED).  Return an
        ...
        generic = {
            ErrorKind.NOT_CONFIGURED: (
                f"Run `sanremes secrets {self.name} setup` to finish configuration."
            ),
            ErrorKind.BINARY_MISSING: (
                f"Run `sanremes secrets {self.name} setup` to install the helper CLI."
            ),
            ErrorKind.AUTH_FAILED: (
                f"Credentials rejected — run `sanremes secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.AUTH_EXPIRED: (
                f"Credentials expired — run `sanremes secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.NETWORK: (
...
# codes that must not reach SanRemes' own startup output.
...
def run_secret_cli(
    ...
    * The child gets ``PATH``/``HOME``/locale basics plus only the env
      ...
      this point holds every credential SanRemes knows about.
    * ``NO_COLOR=1`` is set and stderr/stdout are ANSI-scrubbed so
      helper diagnostics can't smuggle escape sequences into SanRemes
      output.
