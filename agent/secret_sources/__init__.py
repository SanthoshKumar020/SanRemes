...
credentials at process startup, _after_ ~/.sanremes/.env has loaded.
...
Currently bundled:
  ...
  - ``bitwarden`` — Bitwarden Secrets Manager (`bws` CLI).  See
    ...
    ``sanremes_cli.secrets_cli`` for the user-facing setup wizard.
  - ``onepassword`` — 1Password ``op://`` secret references (`op` CLI).
    See ``agent.secret_sources.onepassword`` for the integration and
    ``sanremes_cli.onepassword_secrets_cli`` for the user-facing commands.
