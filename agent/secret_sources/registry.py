...
``sanremes_cli.env_loader._apply_external_secret_sources()``.
...
def get_source(name: str, *, scope: Optional[str] = None) -> Optional[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        return _SCOPED_SOURCES.get(scope or sanremes_home_key(), {}).get(
            name
...
def list_sources(*, scope: Optional[str] = None) -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(scope or sanremes_home_key(), {}))
...
def list_plugin_sources() -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(sanremes_home_key(), {}))
        return list(merged.values())
...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``sanremes`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's SANREMES_HOME is ``~/.sanremes/profiles/<name>``; the
    default profile (``~/.sanremes``) returns "".
    """
...
def apply_all(secrets_cfg: dict, home_path: Path,
    ...
    enabled = _ordered_enabled_sources(
        secrets_cfg, scope=sanremes_home_key(home_path)
    )
    if not enabled:
        return report
