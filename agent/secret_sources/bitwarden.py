...
SanRemes pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.sanremes/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<sanremes_home>/bin/bws`` on
  first use.  SanRemes pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.sanremes/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``sanremes`` invocations don't
  hammer the API.
* Failures NEVER block SanRemes startup.  Missing binary, no network,
...
# In-process cache so repeated load_sanremes_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `sanremes chat -q ...`
...
# <sanremes_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.sanremes/.env (which
...
_ENCRYPTED_CACHE_INFO = b"sanremes-bws-encrypted-cache-v1"
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under sanremes_home/cache/.

    Thin wrapper over the shared DiskCache, kept for tests and any direct
    callers; falls back to `$SANREMES_HOME` / `~/.sanremes` when home is None.
    """
    return _DISK_CACHE.path(home_path)


def _encrypted_disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the encrypted disk cache path under sanremes_home/cache/."""
...
def _sanremes_bin_dir() -> Path:
    """Where SanRemes stores its managed binaries.  Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<sanremes_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``sanremes secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="sanremes-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a SanRemes
...
# ---------------------------------------------------------------------------
# Public entry point — called from sanremes_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_sanremes_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `sanremes secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `sanremes secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `sanremes secrets bitwarden setup` to install."
        )
        return result
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            return (
                "Run `sanremes secrets bitwarden token` to paste a fresh access "
                "token (create one in the Bitwarden web app: Secrets Manager → "
                "Machine accounts → Access tokens).  Wrong region?  Re-run "
                "`sanremes secrets bitwarden setup` and pick EU/self-hosted."
            )
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`sanremes secrets bitwarden token`) so the
