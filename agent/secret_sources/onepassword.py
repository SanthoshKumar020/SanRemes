...
* Authentication is whatever the user's ``op`` CLI already uses — a
  ...
  or a desktop/interactive session (``OP_SESSION_*``).  SanRemes never
...
* Failures NEVER block startup.  A missing ``op`` binary, expired auth, a
  ...
  SanRemes continues with whatever credentials ``.env`` already had.
...
are cached in-process and on disk under ``<sanremes_home>/cache/op_cache.json``
so back-to-back short-lived ``sanremes`` invocations don't re-shell ``op`` for
...
# In-process cache.  The key folds in str(home_path) so a SANREMES_HOME switch
...
def _op_child_env(token_value: str) -> Dict[str, str]:
    ...
    # configured SanRemes to source it from, so normalize to that name here.
    if token_value:
...
def apply_onepassword_secrets(
    *,
    ...
    Called by ``load_sanremes_dotenv()`` after the .env files have loaded.
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            ...
            if isinstance(cfg, dict):
                token_env = str(cfg.get("service_account_token_env") or token_env)
            return (
                "Run `sanremes secrets onepassword token` to paste a fresh "
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`sanremes secrets onepassword token`) so
