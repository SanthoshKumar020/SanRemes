...
cache problem must never block SanRemes startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the SanRemes home used for cache paths.

    ``home_path`` is whatever ``load_sanremes_dotenv()`` already resolved;
    falling back to ``$SANREMES_HOME`` / ``~/.sanremes`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from sanremes_constants import get_sanremes_home

        home_path = get_sanremes_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<sanremes_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if SANREMES_HOME is ever made traversable.
            try:
