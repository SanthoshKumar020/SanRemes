...
``CommandSecretsProvider`` (sanremes-desktop ``src/main/secrets/commandProvider.ts``)
...
* The requested key is passed to the child ONLY via the ``SANREMES_SECRET_KEY``
...
* The startup/apply path runs the helper exactly ONCE (with an empty
  ``SANREMES_SECRET_KEY``) — it is never called per-key in a loop, so a
...
# sanremes_cli.env_loader can consume both providers identically.
...
def _run_helper(
    ...
    The key is passed as DATA via ``SANREMES_SECRET_KEY`` — never interpolated
    ...
    if source_env is os.environ:
        ...
        env = dict(source_env)
    env["SANREMES_SECRET_KEY"] = secret_key
...
def get_command_secret(
    *,
    ...
    ``SANREMES_SECRET_KEY``.  Returns None on any failure — never raises."""
...
def apply_command_secrets(
    *,
    ...
    # SANREMES_SECRET_KEY and parse its stdout as a dotenv blob.
    stdout = _run_helper(command, "", timeout_seconds, max_output_bytes)
    if stdout is None:
...
class CommandSource(SecretSource):
    ...
    Bulk shape: the helper enumerates a KEY=VALUE blob in one run.  Config::
        ...
        secrets:
          command:
            enabled: true
            command: "cat /run/user/1000/sanremes-secrets.env"
    ...
    def remediation(self, kind, cfg: dict) -> str:
        ...
        if kind == ErrorKind.INTERNAL:
            return (
                ...
                "SanRemes discards helper stderr so diagnostics can't leak "
                "secret material."
            )
