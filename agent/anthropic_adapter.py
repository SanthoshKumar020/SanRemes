"""Anthropic Messages API adapter for SanRemes Agent.

Translates between SanRemes's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in sanremes_cli/runtime_provider.py.
    """
...
# SanRemes effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See sanremes-agent#13848, #17057.
    """
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to sanremes's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to sanremes's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + SanRemesAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://sanremes-agent.nousresearch.com")
        headers.setdefault("X-Title", "SanRemes Agent")
        headers.setdefault("User-Agent", f"SanRemesAgent/{_SANREMES_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (SanRemes loads that into the process env from
    # ``~/.sanremes/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    SanRemes notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    SanRemes historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``sanremes models``) never mutates ``~/.sanremes/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by SanRemes)
      ...
      5. Anthropic credential_pool OAuth entry (~/.sanremes/auth.json)
    ...
    # 1. SanRemes-managed OAuth/setup token env var
    ...
    # 5. SanRemes credential_pool OAuth entry.
...
# ── SanRemes-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.sanremes/.anthropic_oauth.json (our own file).
...
def _get_sanremes_oauth_file() -> Path:
    return get_sanremes_home() / ".anthropic_oauth.json"
...
def run_sanremes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run SanRemes-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize SanRemes with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from sanremes_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_sanremes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read SanRemes-managed OAuth credentials from ~/.sanremes/.anthropic_oauth.json."""
    oauth_file = _get_sanremes_oauth_file()
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read SanRemes OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See sanremes-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See sanremes-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See sanremes-agent#13848 (Kimi) and
    sanremes-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("SanRemes Agent", "Claude Code")
                text = text.replace("SanRemes agent", "Claude Code")
                text = text.replace("sanremes-agent", "claude-code")
        ...
        #      a) bare SanRemes-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that SanRemes surfaces in its CLI. We
