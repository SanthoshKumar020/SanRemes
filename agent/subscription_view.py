...
def build_subscription_state(*, timeout: float = 15.0) -> SubscriptionState:
    ...
    Dev override: when ``SANREMES_DEV_SUBSCRIPTION_FIXTURE`` names a fixture state,
...
def dev_fixture_subscription_state() -> Optional[SubscriptionState]:
    """Return a fixture :class:`SubscriptionState` for ``SANREMES_DEV_SUBSCRIPTION_FIXTURE``.
    ...
    Throwaway scaffolding — mirrors ``SANREMES_DEV_CREDITS_FIXTURE``.
    """
    name = (os.getenv("SANREMES_DEV_SUBSCRIPTION_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return SubscriptionState(logged_in=False, error=f"unknown SANREMES_DEV_SUBSCRIPTION_FIXTURE: {name}")
