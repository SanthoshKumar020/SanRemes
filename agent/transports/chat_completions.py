...
def _reasoning_config_for_model(model: str, reasoning_config: dict | None) -> dict | None:
    ...
    SanRemes' internal effort set extends the wire vocabulary with ``ultra``
...
def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    """Translate SanRemes/OpenRouter-style reasoning config to Gemini thinkingConfig."""
    ...
    # Gemini 2.5 accepts thinkingBudget; don't guess a budget from SanRemes'
    ...
    # is stricter (low/high). Clamp SanRemes' wider effort set to what each
...
def _raise_gemini_thinking_max_tokens(
    ...
    global SanRemes cap of 4096 is enough for visible text, but Ultra/high
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(
        ...
        - SanRemes-internal scaffolding markers — any top-level message key
        ...
        for msg_idx, msg in enumerate(messages):
            ...
            # Drop all SanRemes-internal scaffolding markers (``_``-prefixed).
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if message_tool_calls:
            ...
            for tc in message_tool_calls:
                ...
                # preserve an explicit blank name for SanRemes's recovery path.
