...
projection into SanRemes' display, approval bridging, transcript projection into
...
"codex_app_server"`. SanRemes' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 SanRemes secret — gateway
        ...
        spawn_env = sanremes_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("SANREMES_KANBAN_TASK"):
            kanban_db = spawn_env.get("SANREMES_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    os.path.join(
                        spawn_env.get("SANREMES_HOME", os.path.expanduser("~/.sanremes")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "sanremes",
        client_title: str = "SanRemes Agent",
