...
Owns one Codex thread per SanRemes session. Drives `turn/start`, consumes
...
# SanRemes' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SANREMES_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse SanRemes/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per SanRemes session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SANREMES_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SANREMES_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="sanremes",
            client_title="SanRemes Agent",
            client_version=_get_sanremes_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but SanRemes' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into SanRemes' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own sanremes-tools callback we
            # auto-accept — the user already approved SanRemes' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO SanRemes
        approval-mode/timeout logic. The SanRemes-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; SanRemes approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map SanRemes approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    SanRemes returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the SanRemes approval mode/timeout resolution
    ...
    # "prompt expired"; the SanRemes-side messaging already distinguishes them.
    return "decline"
...
def _get_sanremes_version() -> str:
    """Best-effort SanRemes version string for codex's userAgent line."""
    try:
