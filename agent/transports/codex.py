...
# Wire-name used when SanRemes keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through SanRemes's configured provider
# (Firecrawl / Tavily / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "sanremes_web_search"
...
# rename on the wire (sanremes_<name>), map back in normalize_response so
# SanRemes dispatch is unaffected.
_OPENCODE_RESERVED_TOOL_NAMES = ("web_search", "search_files")
_RESERVED_TOOL_ALIAS_PREFIX = "sanremes_"
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs SanRemes web providers.
        #
        ...
        #    keep SanRemes dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``sanremes_web_search`` so Grok cannot hijack the name. The alias
        ...
        kwargs = {
            # ``-900k`` large-context picker variants are SanRemes-side aliases
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so SanRemes dispatches
                ...
                # (sanremes_web_search / sanremes_search_files, #85589).
