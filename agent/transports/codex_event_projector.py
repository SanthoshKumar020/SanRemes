"""Projects codex app-server events into SanRemes' messages list.

The translator that lets SanRemes' memory/skill review keep working under the
...
SanRemes' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape SanRemes
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way SanRemes' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but SanRemes stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way SanRemes already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — SanRemes' messages store text only.
        text_parts: list[str] = []
