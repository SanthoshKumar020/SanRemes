...
def _ensure_file_checkpoint(
    ...
    # which can differ from the SanRemes process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    in config.yaml wins, the legacy ``SANREMES_CONCURRENT_TOOL_TIMEOUT_S`` env var
    ...
    return resolve_timeout(
        ...
        env_var="SANREMES_CONCURRENT_TOOL_TIMEOUT_S",
    )
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current SanRemes process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before SanRemes policy and dispatch exactly once."""
    ...
    def _sanremes_pipeline(relay_args: dict[str, Any]) -> Any:
...
def _resolve_sequential_tool_timeout() -> float | None:
    ...
    ``SANREMES_CONCURRENT_TOOL_TIMEOUT_S`` legacy bridge) so the two executor
