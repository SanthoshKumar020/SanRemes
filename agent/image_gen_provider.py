...
as ``kind: backend``) or ``~/.sanremes/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``sanremes tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``sanremes tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``sanremes tools`` picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``sanremes tools``
...
def _images_cache_dir() -> Path:
    """Return ``$SANREMES_HOME/cache/images/``, creating parents as needed."""
    from sanremes_constants import get_sanremes_home

    path = get_sanremes_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$SANREMES_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$SANREMES_HOME/cache/images/``.
