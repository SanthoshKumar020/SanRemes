...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in sanremes_state.py — kept local
# to avoid importing sanremes_state at module load time (its module-level
# DEFAULT_DB_PATH = get_sanremes_home() / "state.db" breaks tests that
# monkeypatch get_sanremes_home to return a str).
...
from sanremes_constants import PARTIAL_STREAM_STUB_ID
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.sanremes/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for SanRemes tool use: "
    ...
    return (
        ...
        f"context, but SanRemes needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In SanRemes config, set `model.ollama_num_ctx: 65536` "
...
def _nous_entitlement_message(capability: str) -> str:
    try:
        from sanremes_cli.nous_account import (
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            lines = [
                ...
                "Retry with a fresh credential state: `sanremes auth reset anthropic`. Until "
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only SanRemes' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `sanremes fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from sanremes_cli.moa_config import decode_moa_turn
    ...
    # Adopt any ~/.sanremes/.env credential/base-url edits made since the last
    ...
    # 'disk', or 'unknown' — see sanremes_state.classify_persistence_error).
    ...
    # all run inside Codex). Default SanRemes path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for SanRemes internals.
        #
        # SanRemes invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for SanRemes tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from sanremes_cli.middleware import apply_llm_request_middleware
                ...
                try:
                    from sanremes_cli.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("SANREMES_DUMP_REQUESTS"):
                ...
                from sanremes_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a SanRemes/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    from sanremes_constants import display_sanremes_home as _dhh_fn
                    _dhh = _dhh_fn()
                    _body_text = ""
                    try:
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: sanremes auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `sanremes doctor` for credential-chain diagnostics, or")
                    ...
                    from sanremes_constants import display_sanremes_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for SanRemes-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: sanremes config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: sanremes config set ANTHROPIC_API_KEY \"\"")
                ...
                # and omits the field, and SanRemes' local compression takes
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from sanremes_cli.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `sanremes model`."
                        )
                ...
                # providers (DeepSeek, Kimi, MiMo, SanRemes).  A 429 can
                ...
                # which SanRemes' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. SanRemes' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`sanremes",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # SanRemes may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See sanremes-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `sanremes auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `sanremes model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: sanremes portal", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: sanremes setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        sanremes fallback add   (interactive picker — same as `sanremes model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            ...
                            "(not a SanRemes/gateway failure).\n\n"
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.sanremes/config.yaml` to extend the per-call "
                            f"timeout. (SanRemes's built-in floor is 600s for "
        ...
        try:
            ...
            # with the iterations SanRemes never saw. Appended before this turn's
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates SanRemes mid-turn, resume logic still sees the
                    ...
                    from sanremes_state import classify_persistence_error
                ...
                # timeout (SANREMES_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from sanremes_cli.plugins import get_pre_verify_continue_message
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("SANREMES_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
            ...
            if sys.is_finalizing() or _is_interpreter_shutdown_error(e):
                ...
                final_response = (
                    ...
                    "resumed with: sanremes --resume <session-id>"
                )
