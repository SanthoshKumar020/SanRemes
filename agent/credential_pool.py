...
import sanremes_cli.auth as auth_mod
...
# un-throttled log fires on *every* selection. On Windows several SanRemes
...
# readiness handshake ("Timed out connecting to SanRemes backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``sanremes_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
    ...
    # ~/.sanremes/auth.json even when SANREMES_HOME points at a profile path
    # (mirrors the read-side guard in _load_global_auth_store). Uses the
    # unmodified HOME env, not Path.home() which fixtures may monkeypatch.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_env = os.environ.get("HOME", "")
        if real_home_env:
            real_root = Path(real_home_env) / ".sanremes" / "auth.json"
            try:
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``sanremes model`` / ``sanremes auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another SanRemes process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``sanremes`` invocation that defaults to the active provider
        (e.g. setup wizard, ``sanremes auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across SanRemes
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``SANREMES_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "SANREMES_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "SANREMES_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("sanremes_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another SanRemes
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("sanremes_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another SanRemes process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other SanRemes profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `sanremes model` / `sanremes auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `sanremes model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``sanremes auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `sanremes auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `sanremes auth remove <provider> <N>` is stable across all source types.
    try:
        from sanremes_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, SanRemes PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `sanremes setup` ("Claude
        ...
        # Claude Code CLI, sanremes_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.sanremes/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `sanremes setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"sanremes_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_sanremes_oauth_credentials

        for source_name, creds in (
            ("sanremes_pkce", read_sanremes_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `sanremes auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (sanremes auth remove copilot gh_cli) must not pay either on
        ...
        # SanRemes auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from sanremes_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.sanremes/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `sanremes auth remove minimax-oauth <N>` flow works.
        ...
        try:
            from sanremes_cli.auth import get_provider_auth_state
        ...
        # Respect user suppression — `sanremes auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the SanRemes auth store.  Without this gate the removal is instantly
        ...
        # SanRemes owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `sanremes auth openai-codex`.
        ...
        # When the user logs in via ``sanremes model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``sanremes auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from sanremes_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.sanremes/.env over os.environ — the user's config file is the
# authoritative source for SanRemes credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_sanremes_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `sanremes auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.sanremes/.env.
    ...
    try:
        from sanremes_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from sanremes_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.sanremes/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `sanremes auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and SanRemes
