...
upstream prefix cache warm.  See ``sanremes-agent-dev``'s
...
from sanremes_constants import get_default_sanremes_root, get_sanremes_home
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- sanremes-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``SANREMES_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("SANREMES_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the SANREMES_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from sanremes_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from sanremes_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound SANREMES_HOME ContextVar override wins. Surfaces that multiplex
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  SanRemes never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want SANREMES_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the SANREMES_HOME ContextVar
    ...
    # Pointer to the sanremes-agent skill + docs for user questions about SanRemes
    ...
    stable_parts.append(
        SANREMES_AGENT_HELP_GUIDANCE if _has_skill_view
        else SANREMES_AGENT_HELP_GUIDANCE_NO_SKILLS
    )
    ...
    # SANREMES_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base SanRemes, any interactive coding surface in a code
    ...
    # `sanremes -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['sanremes-bots']).
    ...
    # Active-profile hint — names the SanRemes profile the agent is running
    # under so it doesn't conflate ~/.sanremes/skills/ (default profile) with
    # ~/.sanremes/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient SANREMES_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_sanremes_root(): get_sanremes_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_sanremes_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_sanremes_root())
    else:
        _home_str = _root_str = str(get_sanremes_home())
    if active_profile == "default":
        post_workspace_parts.append(
            "Active SanRemes profile: default. Other profiles (if any) live "
        ...
        # data sits at the ROOT (get_default_sanremes_root()), which in ambient
        # profile mode is NOT get_sanremes_home().
        profile_home = _home_str
        default_root = get_default_sanremes_root()
        post_workspace_parts.append(
            f"Active SanRemes profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing SanRemes). Every other surface (desktop chat panel,
    ...
    from sanremes_time import get_timezone as _sanremes_tz, now as _sanremes_now
    now = _sanremes_now()
