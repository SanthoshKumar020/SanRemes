...
token`` all expose exactly this contract. SanRemes already accepts a callable
...
def _mint(command: str, label: str) -> tuple[str, Optional[float]]:
    ...
    if stdout.lstrip().startswith("{"):
        ...
        if isinstance(payload, dict):
            ...
            # Imported lazily: sanremes_cli.auth imports from agent.* at module
            # level, so a top-level import here would risk a cycle.
            from sanremes_cli.auth import _parse_iso_timestamp
