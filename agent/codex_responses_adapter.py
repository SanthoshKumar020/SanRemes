...
def _clamp_responses_call_id(call_id: str) -> str:
    ...
    component the built-in ``sanremes-tools`` server already overflows 64 chars,
...
# output items.  SanRemes injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while SanRemes-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on SanRemes threading encrypted reasoning back across
    ...
    ``convert_messages``). Dropping the checkpoint costs nothing: SanRemes'
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a SanRemes-side metadata key and not part
...
def _normalize_codex_response(
    ...
    *,
    ...
    if tool_calls:
        ...
        # See https://github.com/NousResearch/sanremes-agent/issues/64434
