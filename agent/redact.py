...
# `export SANREMES_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in sanremes_cli/main.py, gateway/run.py, and
# cli.py) or `SANREMES_REDACT_SECRETS=false` in ~/.sanremes/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("SANREMES_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across SanRemes — used by
    ``sanremes config``, ``sanremes status``, ``sanremes dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical sanremes log line
...
# (``security.redact_secrets: false`` / SANREMES_REDACT_SECRETS) applies to
