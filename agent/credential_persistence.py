...
``auth.json``.  They intentionally have no dependency on ``sanremes_cli.auth`` so
...
# Sources SanRemes owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and SanRemes-owned OAuth/device-code state)
