...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `sanremes backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - sanremes_home (str): The active SANREMES_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.sanremes``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "sanremes").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'sanremes memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], sanremes_home: str) -> None:
        ...
        Called by 'sanremes memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``sanremes_home`` is the active SANREMES_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SANREMES_HOME.

        ``sanremes backup`` only walks SANREMES_HOME, so any provider state kept
        ...
        of the archive, and ``sanremes import`` restores them to their original
