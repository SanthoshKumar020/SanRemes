"""OpenAI-compatible shim that forwards SanRemes requests to `copilot --acp`.

This adapter lets SanRemes treat the GitHub Copilot ACP server as a chat-style
backend. Each request starts a short-lived ACP session, sends the formatted
conversation as a single prompt, collects text chunks, and converts the result
back into the minimal shape SanRemes expects from an OpenAI client.
"""
...
def _resolve_command() -> str:
    return (
        os.getenv("SANREMES_COPILOT_ACP_COMMAND", "").strip()
...
def _resolve_args() -> list[str]:
    raw = os.getenv("SANREMES_COPILOT_ACP_ARGS", "").strip()
    if not raw:
        return ["--acp", "--stdio"]
...
def _acp_supported(command: str, args: list[str]) -> bool | None:
    ...
    SANREMES_COPILOT_ACP_ARGS transport is the operator's business.
    """
    if "--acp" not in args:
        return True
...
def _resolve_home_dir() -> str:
    ...
    # subprocess with no HOME; callers can set SANREMES_HOME explicitly if they
...
def _format_messages_as_prompt(
    ...
    sections: list[str] = [
        "You are being used as the active ACP agent backend for SanRemes.",
        "Use ACP capabilities to complete tasks.",
        "IMPORTANT: If you take an action with a tool, you MUST output tool calls using <tool_call>{...}</tool_call> blocks with JSON exactly in OpenAI function-call shape.",
        "If no tool is needed, answer normally.",
    ]
    if model:
        sections.append(f"SanRemes requested model hint: {model}")

    # Copilot has no tools of its own that would collide with SanRemes', so it
    ...
    def _run_prompt(self, prompt_text: str, *, timeout_seconds: float) -> tuple[str, str]:
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                f"SANREMES_COPILOT_ACP_COMMAND / SANREMES_COPILOT_ACP_ARGS "
                f"to a working pair."
            )
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Install GitHub Copilot CLI or set SANREMES_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH."
            ) from exc
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None, reasoning_parts: list[str] | None = None) -> Any:
            ...
            if proc.poll() is not None and stderr_text:
                if _is_gh_copilot_deprecation_message(stderr_text):
                    raise RuntimeError(
                        "SanRemes ACP mode requires the NEW GitHub Copilot CLI "
                        ...
                        "point SanRemes at it explicitly:\n"
                        "  export SANREMES_COPILOT_ACP_COMMAND=/path/to/new/copilot\n\n"
                        "Alternative: use the `copilot` provider (no ACP, hits the Copilot API\n"
                        "directly with a Copilot subscription token) via `sanremes setup`.\n\n"
                        f"Original error:\n{stderr_text}"
                    )
        ...
        *,
        ...
        if method == "session/request_permission":
            ...
            response = _jsonrpc_error(
                message_id,
                -32601,
                f"ACP client method '{method}' is not supported by SanRemes yet.",
            )
