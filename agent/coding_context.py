"""Coding-context awareness — base SanRemes, every interactive surface.

When the user runs SanRemes inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), SanRemes shifts into a **coding posture**. This module is the
...
from sanremes_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("sanremes.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "sanremes", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _SANREMES_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``SANREMES_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether SanRemes should operate in its coding posture right now."""
