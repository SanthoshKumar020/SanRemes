...
Data resolution order:
  ...
  2. Disk cache (~/.sanremes/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: SanRemes ↔ models.dev
# ---------------------------------------------------------------------------

# SanRemes provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; SanRemes' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → SanRemes ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_sanremes_ids(mdev_id: str) -> List[str]:
    """Return the SanRemes provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for sanremes_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(sanremes_id)
...
def _get_cache_path() -> Path:
    ...
    return get_sanremes_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from sanremes_constants import get_sanremes_home
    return get_sanremes_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``sanremes config refresh``, the
...
#
# Provider keys accept the SanRemes provider id (as used elsewhere in
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the SanRemes provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by SanRemes id.
    for sanremes_id in _models_dev_to_sanremes_ids(provider_key):
        if sanremes_id != provider_key:
            candidates.append(sanremes_id)
...
def _get_provider_models(
    ...
    """Resolve a SanRemes provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``sanremes model``), which is an interactive user-facing flow
    where a fresh catalog is worth a short network wait.
    """
    from sanremes_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal SanRemes agent traffic.
...
def get_provider_info(
    ...
    Accepts either a SanRemes provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve SanRemes ID → models.dev ID
...
def get_model_info(
    ...
    Accepts SanRemes or models.dev provider ID.  Tries exact match then
