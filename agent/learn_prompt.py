...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a skill via ``skill_manage`` that follows the SanRemes
     skill-authoring standards (description <=60 chars, the modern section
     order, SanRemes-tool framing, no invented commands). Small sources get one
...
# (HARDLINE)" and the sanremes-agent-dev new-skill salvage reference. Embedded in
# the prompt so the agent authors skills the way a maintainer would by hand.
_AUTHORING_STANDARDS = """\
Follow the SanRemes skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `SanRemes`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as SanRemes.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.sanremes.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through SanRemes tools.
...
SanRemes-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference SanRemes tools by name in backticks: `terminal`, `read_file`,
