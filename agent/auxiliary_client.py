...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.sanremes/auth.json active provider)
...
from sanremes_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``SANREMES_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `sanremes update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    try:
        from sanremes_cli.models import (
    ...
    # SanRemes owns auxiliary retry + provider/model fallback policy (the
    ...
    # SanRemes sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let SanRemes control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="sanremes-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before SanRemes triggers summarization.  Higher values delay
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from sanremes_cli.auth import resolve_api_key_provider_credentials
        from sanremes_cli.models import fetch_models_with_pricing
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://sanremes-agent.nousresearch.com",
    "X-Title": "SanRemes Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``SANREMES_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``SANREMES_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("SANREMES_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("SANREMES_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "SanRemesAgent",
}
...
_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://sanremes-agent.nousresearch.com",
    "X-Title": "SanRemes Agent",
    "User-Agent": f"SanRemesAgent/{_SANREMES_VERSION}",
}
...
# in lockstep with sanremes_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``sanremes_cli.__version__`` is
...
_AUTH_JSON_PATH = get_sanremes_home() / "auth.json"
...
def _codex_cloudflare_headers(
    ...
    the official endpoint always send SanRemes' originator and version. Custom
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (SanRemes Agent)",
    ...
    if _is_official_codex_base_url(base_url):
        ...
        headers.update({
            "User-Agent": f"SanRemesAgent/{__version__}",
            "originator": "sanremes-agent",
        })
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from sanremes_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("SANREMES_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
    except (TypeError, ValueError):
        return 1800
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the SanRemes-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private SanRemes plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``sanremes_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.sanremes/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from sanremes_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from sanremes_cli.auth import _agent_key_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from sanremes_cli.auth import resolve_nous_runtime_credentials

        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("SANREMES_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``sanremes auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``sanremes_cli.auth``'s singleton runtime resolver for older
    auth-store-only logins. Returns ``None`` if the user is not authenticated
    with xAI Grok OAuth.
    """
    try:
        from sanremes_cli.auth import (
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    os.getenv("SANREMES_XAI_BASE_URL", "").strip().rstrip("/")
    ...
    try:
        from sanremes_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from SanRemes auth store.
    ...
    try:
        from sanremes_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from sanremes_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from sanremes_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from sanremes_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if not _aux_probe_active():
        ...
        try:
            from sanremes_cli.models import get_nous_recommended_aux_model
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from sanremes_cli.config import load_config
        from sanremes_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `sanremes setup` or `sanremes model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    from tools.xai_http import sanremes_xai_default_headers

    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=sanremes_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`sanremes_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from sanremes_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second sanremes
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``sanremes config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from sanremes_cli.nous_account import get_nous_portal_account_info
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        if normalized == "copilot":
            from sanremes_cli.copilot_auth import (
                _jwt_cache,
                _token_fingerprint,
        ...
        if normalized == "openai-codex":
            from sanremes_cli.auth import resolve_codex_runtime_credentials
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_sanremes_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``sanremes_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._sanremes_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into SanRemes' built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `sanremes model` but the
    #    old OPENAI_BASE_URL lingers in ~/.sanremes/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: sanremes model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.sanremes/.env",
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from sanremes_cli.runtime_provider import _get_named_custom_provider
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_sanremes_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_sanremes_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from sanremes_cli.copilot_auth import copilot_request_headers
        ...
        async_kwargs["default_headers"] = sanremes_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so SanRemes owns
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from sanremes_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: sanremes auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        from sanremes_cli.providers import nous_api_mode
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: sanremes model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: sanremes model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: sanremes doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("sanremes_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from sanremes_cli.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(sanremes_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from sanremes_cli.models import _should_use_copilot_responses_api
...
def _resolve_task_provider_model(
    ...
    def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
        ...
        if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
            return False
        try:
            from sanremes_cli.providers import get_provider
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`sanremes_cli.plugins.PluginContext.register_auxiliary_task`) the
    ...
    try:
        from sanremes_cli.plugins import get_plugin_auxiliary_tasks
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one SanRemes will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate SanRemes reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional SanRemes reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `sanremes model`."
                    )
