...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``SANREMES_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("SANREMES_VERIFY_ON_STOP")
    if env is not None:
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether the workspace has a runtime verify recipe ``sanremes verify`` can run.

    True when a saved ``.sanremes/environment.json`` manifest exists, or when
    ...
    if verify_commands:
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction += (
                ...
                "start + readiness), prefer `sanremes verify --json` — a passing "
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction = (
                ...
                "project has a runnable verification recipe. Run `sanremes verify "
            ...
            command_instruction = (
                ...
                "`tempfile` path with a `sanremes-verify-` filename prefix, run it "
