...
_SANREMES_MD_NAMES = (".sanremes.md", "SANREMES.md")


def _find_sanremes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.sanremes.md`` or ``SANREMES.md``.
    ...
    # could pick up a .sanremes.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _SANREMES_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are SanRemes Agent, an intelligent AI assistant created by Nous Research. "
...
SANREMES_AGENT_HELP_GUIDANCE = (
    "You run on SanRemes Agent (by Nous Research). When the user needs help with "
    "SanRemes itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://sanremes-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `sanremes-agent` skill with skill_view(name='sanremes-agent') "
...
SANREMES_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on SanRemes Agent (by Nous Research). When the user needs help with "
    "SanRemes itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://sanremes-agent.nousresearch.com/docs is the "
    "authoritative reference and always holds the latest, most up-to-date "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.sanremes/kanban.db`. Your task id is in "
    "`$SANREMES_KANBAN_TASK`; your workspace is `$SANREMES_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SANREMES_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $SANREMES_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${SANREMES_KANBAN_BRANCH:-wt/$SANREMES_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$SANREMES_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`sanremes profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `sanremes kanban <verb>` for board operations. Use "
...
#
# The sanremes-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to sanremes-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that SanRemes "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of SanRemes floating over another application, so the
    user is rarely asking about SanRemes — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating SanRemes "
        ...
        "rather than anything inside SanRemes. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the SanRemes terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the SanRemes desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.sanremes.send(\"prompt\") — or a "
        "data-sanremes-send=\"prompt\" attribute on any clickable element — sends "
    ...
    "webui": (
        "You are in the SanRemes WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where SanRemes itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host SanRemes runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where SanRemes itself is running. The host OS, home, and cwd "
                f"of the SanRemes process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where SanRemes "
    ...
    # Embedder-supplied environment description. Lets a host that wraps SanRemes
    ...
    extra = (os.getenv("SANREMES_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_sanremes_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("SANREMES_PLATFORM") or os.environ.get("SANREMES_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.sanremes/skills/`` directory.  External dirs
    ...
    # SANREMES_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_sanremes_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.sanremes/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_sanremes_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot SanRemes Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `sanremes-agent` skill "
            "first. It has the actual commands (e.g. `sanremes config set …`, `sanremes tools`, "
            "`sanremes setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md(
    ...
    """Load SOUL.md from SANREMES_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the SANREMES_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from sanremes_cli.config import ensure_sanremes_home
        ensure_sanremes_home()
    except Exception as e:
        logger.debug("Could not ensure SANREMES_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_sanremes_home()
...
def _load_sanremes_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.sanremes.md / SANREMES.md — walk to git root."""
    sanremes_md_path = _find_sanremes_md(cwd_path)
    if not sanremes_md_path:
        return ""
    try:
        content = sanremes_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = sanremes_md_path.name
        try:
            rel = str(sanremes_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".sanremes.md", context_length=context_length,
            read_path=str(sanremes_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", sanremes_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .sanremes.md / SANREMES.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SANREMES_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the SanRemes install/source
    ...
    # explicitly configured cwd is honored verbatim — the SanRemes tree is a
    ...
    # their launch dir IS the user's shell cwd (developing SanRemes in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the SanRemes install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_sanremes_md(cwd_path, context_length)
    ...
    # SOUL.md from SANREMES_HOME only — skip when already loaded as identity
    if not skip_soul:
