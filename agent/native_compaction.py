...
SanRemes' support is deliberately narrow (live verification, Aug 2026):
...
Ownership model: SanRemes' local compression stays fully armed as the
...
def _warn_native_compaction_suppressed_by_checkpoint_gate() -> None:
    ...
    logger.warning(
        ...
        "checkpoint-aware SanRemes compressor stays authoritative."
    )
...
def native_compaction_context_management(
    ...
    *,
    ...
    # the server replaces older context. Keep the checkpoint-aware SanRemes
...
def _is_summary_item(item: Any) -> bool:
    """True when *item* is a canonical SanRemes compression-summary message.
...
def prune_pre_checkpoint_items(
    ...
    - Compression summary messages (``_is_summary_item``, the canonical
      ...
      byte/character-sliced: SanRemes summaries carry structural framing
