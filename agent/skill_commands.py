...
#
# When a user invokes a /skill (or /bundle), SanRemes expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_sanremes_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``SANREMES_PLATFORM`` env var and
    ``SANREMES_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("SANREMES_PLATFORM")
            or get_session_env("SANREMES_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("SANREMES_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Return the effective SanRemes home the skill scan should be scoped to.

    A gateway session can switch between profiles that each carry their own
    ``skills.external_dirs`` (via ``set_sanremes_home_override``), but the
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.sanremes.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_sanremes_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.sanremes/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            ...
            for skill_md in _iter:
                ...
                try:
                    ...
                    # with a core SanRemes slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core SanRemes command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's SanRemes home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.sanremes/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``sanremes -s <skill>`` or
    a deployment's ``SANREMES_TUI_SKILLS`` env var could force-load a skill an
