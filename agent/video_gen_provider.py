...
as ``kind: backend``) or ``~/.sanremes/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        backend can't honor are not advertised), and by ``sanremes tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SANREMES_HOME/cache/videos/``, creating parents as needed."""
    from sanremes_constants import get_sanremes_home

    path = get_sanremes_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SANREMES_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$SANREMES_HOME/cache/videos/``.
