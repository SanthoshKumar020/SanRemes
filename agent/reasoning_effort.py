...
SanRemes' internal effort ladder (``sanremes_constants.VALID_REASONING_EFFORTS``
...
# sanremes_constants.VALID_REASONING_EFFORTS ("none" included so an explicit
# disable can be clamped too when a provider publishes it as a level).
EFFORT_LADDER: tuple[str, ...] = (
    "none", "minimal", "low", "medium", "high", "xhigh", "max", "ultra",
)

# ``ultra`` is SanRemes-internal ladder vocabulary (the Codex product tier); no
