...
SanRemes keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for SanRemes's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with SanRemes)
        ...
        with httpx.Client(timeout=timeout) as client:
            resp = client.post(
                ...
                headers={
                    ...
                    "X-Goog-Api-Client": f"sanremes-agent/{_SANREMES_VERSION}",
                },
            )
...
_FREE_TIER_GUIDANCE = (
    ...
    "for Gemini Flash models). SanRemes typically makes 3-10 API calls per user turn, "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "GOOGLE_API_KEY in ~/.sanremes/.env and restart your session. "
    "Details: https://ai.google.dev/gemini-api/docs/api-key"
)


class GeminiAPIError(Exception):
    """Error shape compatible with SanRemes retry/error classification."""
...
def _thinking_requests_output_headroom(thinking_config: Any) -> bool:
    ...
    SanRemes ``max_tokens`` of 4096/16384 is enough for visible text, but
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.sanremes/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `sanremes setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": f"sanremes-agent/{_SANREMES_VERSION} (gemini-native)",
            "X-Goog-Api-Client": f"sanremes-agent/{_SANREMES_VERSION}",
        }
