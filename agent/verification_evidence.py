...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("sanremes-verify-", "sanremes-ad-hoc-")
...
def _db_path() -> Path:
    return get_sanremes_home() / "verification_evidence.db"
...
def record_verify_run(
    *,
    ...
    session_id: str | None = None,
    ok: bool,
    command: str = "sanremes verify",
    scope: str = "full",
    output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``sanremes verify`` run as verification evidence.

    Explicit CLI-side write: unlike :func:`record_terminal_result` there is
    nothing to classify — the caller (the ``sanremes verify`` command) already
    ...
    evidence = VerificationEvidence(
        command=command,
        canonical_command="sanremes verify",
        kind="verify",
