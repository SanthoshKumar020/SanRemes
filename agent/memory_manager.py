...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_SANREMES_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, SanRemes expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile SANREMES_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped SANREMES_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``sanremes_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_sanremes_home()`` themselves.
        """
        if "sanremes_home" not in kwargs:
            from sanremes_constants import get_sanremes_home
            kwargs["sanremes_home"] = str(get_sanremes_home())
        for provider in self._providers:
            try:
