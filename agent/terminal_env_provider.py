...
Providers live in ``~/.sanremes/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.sanremes/cache`` files land
  inside the backend (e.g. ``"~/.sanremes"`` for home-synced backends,
  ``"/root/.sanremes"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_sanremes_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced SanRemes cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``sanremes setup`` after this backend is selected.
