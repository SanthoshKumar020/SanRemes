...
``kind: backend``) or ``~/.sanremes/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`sanremes_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.sanremes/.env``) so credentials set
    through SanRemes' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``sanremes tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``sanremes tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``sanremes_cli/tools_config.py`` to inject this provider as a
