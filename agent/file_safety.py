...
def _sanremes_home_path() -> Path:
    """Resolve the active SANREMES_HOME (profile-aware) without circular imports."""
    try:
        from sanremes_constants import get_sanremes_home  # local import to avoid cycles
        return get_sanremes_home()
    except Exception:
        return Path(os.path.expanduser("~/.sanremes"))


def _sanremes_root_path() -> Path:
    """Resolve the SanRemes root dir (always the parent of any profile, never per-profile)."""
    try:
        from sanremes_constants import get_default_sanremes_root  # local import to avoid cycles
        return get_default_sanremes_root()
    except Exception:
        return Path(os.path.expanduser("~/.sanremes"))
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    sanremes_home = _sanremes_home_path()
    sanremes_root = _sanremes_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(sanremes_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(sanremes_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(sanremes_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(sanremes_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(sanremes_home / "cache" / "bws_cache.enc.json"),
            str(sanremes_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved SANREMES_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("SANREMES_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    sanremes_dirs = []
    for base in (_sanremes_home_path(), _sanremes_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in sanremes_dirs:
                sanremes_dirs.append(real)
        except Exception:
            continue

    for base_real in sanremes_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside SANREMES_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied SanRemes path.

    Three categories are blocked:

      * Internal SanRemes cache files under ``SANREMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under SANREMES_HOME and the global SanRemes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.sanremes/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active SANREMES_HOME (profile-aware) AND the global
    # SanRemes root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (SANREMES_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    sanremes_dirs: list[Path] = []
    for base in (_sanremes_home_path(), _sanremes_root_path()):
        try:
            real = base.resolve()
            if real not in sanremes_dirs:
    ...
    for hd in sanremes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal SanRemes cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # SANREMES_HOME or <root>.
    ...
    for hd in sanremes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a SanRemes credential store "
    ...
    for hd in sanremes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the SanRemes MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a SanRemes MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    for hd in sanremes_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the SanRemes real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the SanRemes real-profile browser "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied SanRemes read (see
...
#
# SanRemes profiles are separate SANREMES_HOME dirs under
...
#
# Reference: May 2026 incident where a sanremes-security profile session
# edited skills under both ``~/.sanremes/profiles/sanremes-security/skills/``
# AND ``~/.sanremes/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under SANREMES_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from SANREMES_HOME.

    ``~/.sanremes``              -> ``"default"``
    ``~/.sanremes/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _sanremes_home_path().resolve()
        root_real = _sanremes_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside SanRemes scope, or is inside
...
#
#   <SANREMES_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.sanremes/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.sanremes/…`` segment and warns
# regardless of which SanRemes profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.sanremes/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.sanremes`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.sanremes/…`` and returns the
    index where the inner SanRemes-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .sanremes / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".sanremes":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative SanRemes state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.sanremes``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.sanremes`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any SanRemes resolver to
    succeed, so it works correctly even when called from contexts where
    SANREMES_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host SanRemes process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real SANREMES_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.sanremes/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.sanremes/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host SANREMES_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative SanRemes state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host SanRemes "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real SANREMES_HOME. Use the "
