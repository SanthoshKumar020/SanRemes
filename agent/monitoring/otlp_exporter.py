...
configured under ``monitoring.export.otlp``. Lets an operator stream SanRemes
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install sanremes-agent[otlp]``), imported lazily so the
...
def _require_sdk(*, auto_install: bool = True, prompt: bool = True):
    ...
    try:
        ...
        raise OTLPUnavailable(
            ...
            "    pip install 'sanremes-agent[otlp]'\n"
            f"(import error: {e})"
        )
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"sanremes.event": kind or "unknown"}
    keep_by_kind = {
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'sanremes-agent[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
