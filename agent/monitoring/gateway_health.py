...
def _base_attrs(*, profile: str, install_id: str, version: str, supervision_mode: str) -> Dict[str, str]:
    ...
    return {
        ...
        "sanremes.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
...
def build_gateway_health_snapshot(
    ...
    *,
    ...
    metrics: list[GatewayMetric] = [
        _metric("sanremes.gateway.up", 1 if gateway_running else 0, base),
        _metric("sanremes.gateway.active_agents", active_agents, base),
        _metric("sanremes.gateway.busy", 1 if busy else 0, base),
        _metric("sanremes.gateway.drainable", 1 if drainable else 0, base),
        _metric("sanremes.gateway.restart_requested", 1 if runtime.get("restart_requested") else 0, base),
    ]
    if gateway_state:
        metrics.append(_metric("sanremes.gateway.state", 1, base, **{"sanremes.gateway.state": str(gateway_state)}))
    ...
    for platform, pdata in platforms.items():
        ...
        metrics.append(_metric(
            ...
            1 if is_up else 0,
            base,
            **{"sanremes.platform": str(platform), "sanremes.platform.state": state},
        ))
        metrics.append(_metric(
            "sanremes.platform.degraded",
            1 if is_degraded else 0,
            base,
            **{"sanremes.platform": str(platform), "sanremes.platform.state": state, "sanremes.error_code": error_code},
        ))
        if is_degraded:
