...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`sanremes_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.sanremes/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``SANREMES_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``sanremes_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # SanRemes-canonical

    # Inject context for pre_llm_call:
    {"context": "Today is Friday"}

    # Modify tool input for pre_tool_call (SanRemes-canonical):
...
from sanremes_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, windows_hide_flags
...
# sanremes_cli.plugins.get_pre_tool_call_block_message / _get_pre_tool_call_
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``sanremes_cli.config.load_config``
    ...
    setting.  ``SANREMES_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / SANREMES_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("SANREMES_SAFE_MODE"):
        logger.info("SANREMES_SAFE_MODE=1 — shell-hook registration skipped")
        return []
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / SANREMES_ACCEPT_HOOKS=1 / "
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``sanremes hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from sanremes_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def _spawn(spec: ShellHookSpec, stdin_json: str) -> Dict[str, Any]:
    ...
    try:
        ...
        from sanremes_cli._subprocess_compat import split_command_line
...
def _evaluate_result(
    ...
    * non-JSON / unparseable stdout on a ``fail_closed`` blocking hook —
    ...
    test helper (:func:`run_once`) so ``sanremes hooks test`` reflects
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a SanRemes wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical SanRemes
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`sanremes_cli.plugins.get_pre_tool_call_block_message`.  This is
    ...
    if event == "pre_verify":
        # "continue" (SanRemes) / "block" (Claude-Code Stop: block the stop) both
...
def allowlist_path() -> Path:
    ...
    return get_sanremes_home() / ALLOWLIST_FILENAME
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / SANREMES_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ SanRemes is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``SANREMES_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("SANREMES_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `sanremes hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``sanremes hooks test`` and ``sanremes hooks doctor``.

    ``kwargs`` is the same dict that :func:`sanremes_cli.plugins.invoke_hook`
    ...
    produce — otherwise scripts tested via ``sanremes hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical SanRemes-wire-shape response — including exit-code-2
    blocking and ``fail_closed`` semantics, so what ``sanremes hooks test``
