"""Core NeMo Relay adapters for physical SanRemes provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when SanRemes knows the API mode."""
...
def _relay_metadata(
    ...
    if not isinstance(relay_metadata, dict):
        relay_metadata = {}
    relay_metadata.setdefault("sanremes.provider", provider_name)
...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited SanRemes turn when present."""
...
def stream_current(
    ...
    *,
    ...
    """Run a provider stream under the inherited SanRemes turn when present.
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from SanRemes's provider worker thread."""
    ...
    def __init__(
        ...
        *,
        ...
        def run_callback(callback: Callable[..., Any], *args: Any) -> Any:
            ...
            def guarded() -> Any:
                # SanRemes-side callbacks run while the native pipeline drives
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by SanRemes."""
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "SanRemes Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
