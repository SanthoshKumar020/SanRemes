...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. SanRemes does
...
class EntraIdentityConfig:
    ...
    Captures the SanRemes-managed Entra knobs we need outside Azure SDK
    ...
    ``sanremes_cli/runtime_provider.py:1310-1377`` for the analogous
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only SanRemes-selected knobs are passed as kwargs. Everything else
    ...
    ``~/.sanremes/.env`` or the deployment environment.
    """
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    SanRemes processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``sanremes doctor`` /
    ``sanremes auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``sanremes doctor`` and the wizard preflight — never
...
def materialize_bearer_for_http(value: Any) -> str:
    ...
    header outside the OpenAI SDK (e.g. ``sanremes_cli/azure_detect.py``).
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `sanremes doctor` or `az login` to recover.",
                exc,
            )
