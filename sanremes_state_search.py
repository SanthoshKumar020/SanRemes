...
class SessionSearchMixin:
    ...
    def search_messages(
        ...
        Threshold: SANREMES_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("SANREMES_SEARCH_SLOW_MS", "1000"))
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another SanRemes surface and jump
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``SANREMES_DISABLE_FTS_TRIGRAM`` or not yet created), so
