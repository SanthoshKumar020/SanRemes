...
SanRemes Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``SANREMES_NIX_BUILD=1`` on the
SanRemes package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("SANREMES_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for sanremes-agent is not supported.\n"
    "SanRemes is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://sanremes-agent.nousresearch.com/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the SanRemes Nix derivation sets SANREMES_NIX_BUILD=1. If it does, file a bug."
)
