 ...
 *
 * The root ``package.json`` is installed by ``sanremes update`` on every user,
 ...
 *   workspace graph entirely. ``sanremes update`` and ``sanremes doctor --fix``
 ...
 *   ``sanremes tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``sanremes update`` for
 ...
 * fails — read the lazy-install guidance in the ``sanremes-agent-dev`` skill
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in sanremes_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`sanremes tools` → Browser Automation → Camofox.'
  )
})
