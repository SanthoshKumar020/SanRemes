# Política de Seguridad de SanRemes Agent

Este documento describe el modelo de confianza de SanRemes Agent, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/NousResearch/sanremes-agent/security/advisories/new)
o **security@nousresearch.com**. No abras issues públicos para
vulnerabilidades de seguridad. **SanRemes Agent no opera un programa de
...
- Detalles del entorno (`sanremes --version`, SHA del commit, SO, versión de Python).
...
SanRemes Agent es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta SanRemes Agent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a SanRemes Agent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de SanRemes Agent
...
SanRemes Agent admite dos posturas de aislamiento a nivel de SO. Abordan
...
SanRemes Agent admite esto de dos maneras:

- **La propia imagen Docker de SanRemes Agent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de SanRemes Agent
...
SanRemes Agent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en SanRemes Agent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de SanRemes Agent
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   SanRemes Agent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de SanRemes Agent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  SanRemes Agent ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de sanremes-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de SanRemes Agent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en SanRemes Agent. Los errores en la ruta de
  instalación de habilidades o plugins de SanRemes Agent que impidan al
...
- SanRemes Agent incluye guardias de cadena de suministro para lanzamientos de servidores
