...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with sanremes-agent)
2. User plugins: ``$SANREMES_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``sanremes_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$SANREMES_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$SANREMES_HOME/plugins/model-providers/`` if it exists."""
    try:
        from sanremes_constants import get_sanremes_home

        d = get_sanremes_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple SANREMES_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_sanremes_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``sanremes_agent.plugins``
    ...
    * **Provider targets only.** The ``sanremes_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$SANREMES_HOME``) keep their
    ...
    try:
        from sanremes_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$SANREMES_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``sanremes_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$SANREMES_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with sanremes-agent.
    ...
    # 2. User plugins — under $SANREMES_HOME/plugins/model-providers/<name>/.
