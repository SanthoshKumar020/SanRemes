...
sanremes current          93.3%*  26.7%* 33.3%    30.0%  45.8%   ~162K
sanremes lean closed-book 40.0%   60.0%  23.3%    36.7%  40.0%   ~49K
sanremes lean+recovery    70.0%   80.0%  43.3%    80.0%  68.3%   ~49K
...
4. **What exact error message is returned when 'gh pr merge --auto' is attempted on the NousResearch/sanremes-agent repository?**  
...
9. **Which two properties shape almost every design decision in SanRemes according to the Development Guide?**  
...
10. **What error message does the live-checkout git guard display when blocking a history-rewriting command?**  
   gold: `Blocked: `git <op>` would rewrite SanRemes's live source checkout (/home/teknium/.sanremes/sanremes-agent) and can mix module `
...
15. **What was the specific ModuleNotFoundError message caused by the wheel subpackage discovery trap in #34701?**  
   gold: `ModuleNotFoundError: No module named 'sanremes_cli.dashboard_auth'`
...
1. **What is the PR number for the authored fix addressing mid-turn message ordering bugs in SanRemes Desktop?**  
   gold: `#86617`
...
3. **What specific file and line number were identified as the cause of an AssertionError (assert 56 == 55) in the Python tests?**  
   gold: `tests/sanremes_cli/test_session_recovery_lost_and_found.py:327`
...
8. **What error message does the SanRemes terminal tool return when a git command is blocked to prevent rewriting the live source checkout?**  
   gold: `Blocked: `git <op>` would rewrite SanRemes's live source checkout`
9. **What is the core design principle regarding 'Narrow Waist' in SanRemes development?**  
...
5. **What specific error message is reported in issue #83562 regarding the Windows Desktop update?**  
   gold: `SanRemes backend exited (0)`
...
6. **What exact error message did the agent receive when attempting to checkout a worktree while in the live source directory?**  
   gold: `Blocked: `git checkout` would rewrite SanRemes's live source checkout (/home/teknium/.sanremes/sanremes-agent) and can mix mod`
...
10. **What specific SQL update was added to the migration path in sanremes_cli/kanban_db.py to prevent losing active wake on upgrade?**  
