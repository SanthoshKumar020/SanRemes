"""Run the read-tool eval through the REAL SanRemes AIAgent.

For each task: fresh temp SANREMES_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    sanremes_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".sanremes"
    ...
    try:
        ...
        shutil.rmtree(sanremes_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.sanremes/.env; set +a  — then relaunch."
        )
