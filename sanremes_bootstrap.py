"""Windows UTF-8 bootstrap for SanRemes entry points.
...
should be imported at the very top of every SanRemes entry point
(``sanremes``, ``sanremes-agent``, ``sanremes-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``sanremes_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``sanremes_bootstrap``, never ``sanremes_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing SanRemes modules.

    SanRemes ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real SanRemes source root
    ...
    """
    root = src_root or os.environ.get("SANREMES_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``SANREMES_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("SANREMES_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import sanremes_bootstrap``
# (or ``from sanremes_bootstrap import apply_windows_utf8_bootstrap``) at
