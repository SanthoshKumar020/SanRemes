"""Shared utility functions for sanremes-agent."""
...
def atomic_replace(tmp_path: Union[str, Path], target: Union[str, Path]) -> str:
    ...
    ``~/.sanremes/`` to a git-tracked profile package or dotfiles repo
    (GitHub #16743).
...
def warn_if_credential_file_broadly_readable(
    ...
    *,
    ...
    Secret-bearing files that users create by hand (or that older SanRemes
...
def atomic_roundtrip_yaml_save(
    ...
    * Non-dict values (lists, scalars) are overwritten wholesale — list
    ...
    Shares the fail-closed contract ``sanremes_cli.config.atomic_config_write``
    ...
    ``sanremes_cli.config`` itself imports from this module.
    """
    from ruamel.yaml import YAML
    from ruamel.yaml.comments import CommentedMap
    from ruamel.yaml.scalarstring import DoubleQuotedScalarString

    from sanremes_cli.config import require_readable_config_before_write
