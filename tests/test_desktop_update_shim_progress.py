...
def progress(tmp_path):
    ...
    status = tmp_path / "sanremes-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen SanRemes to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `sanremes update`, and reports the stage that was on screen
...
n="$(cat "$SANREMES_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$SANREMES_TEST_CALLS"
for f in "$TMPDIR"/sanremes-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$SANREMES_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$SANREMES_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "sanremes-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    sanremes = install_root / "venv" / "bin" / "sanremes"
    sanremes.write_text(FAKE_SANREMES)
    sanremes.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "SANREMES_TEST_CAPTURE": str(capture),
        "SANREMES_TEST_CALLS": str(calls),
        "SANREMES_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".sanremes-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`sanremes update` is the longest wait in the hand-off and the one the
