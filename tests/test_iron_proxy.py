...
def sanremes_home(tmp_path, monkeypatch):
    """Point SANREMES_HOME at a temp dir so install paths don't touch the real $HOME."""
...
def test_load_mappings_handles_corrupt_json(sanremes_home):
...
def test_verify_checksums_signature_skips_without_gpg(sanremes_home, monkeypatch, tmp_path):
...
def test_start_proxy_idempotent_when_already_running(sanremes_home, monkeypatch):
    state = ip._proxy_state_dir()
...
def test_subprocess_env_strips_unrelated_secrets(sanremes_home, monkeypatch):
...
def test_ca_key_created_with_0o600(sanremes_home, monkeypatch):
...
def test_ensure_audit_log_creates_with_0o600(sanremes_home, tmp_path):
...
def test_ensure_audit_log_tightens_existing_perms(sanremes_home, tmp_path):
    audit = tmp_path / "audit.log"
...
def test_proxy_state_dir_is_0o700(sanremes_home):
    state = ip._proxy_state_dir()
...
def test_mappings_roundtrip_preserves_headers_and_aliases(sanremes_home):
    m = ip.TokenMapping(
...
def test_ensure_management_token_persists_and_is_stable(sanremes_home):
    t1 = ip.ensure_management_token()
    t2 = ip.ensure_management_token()
    assert t1 == t2
    assert t1.startswith("sanremes-mgmt-")
...
def test_reload_proxy_refuses_when_not_running(sanremes_home, monkeypatch):
...
def test_reload_proxy_posts_bearer_to_management_endpoint(sanremes_home, monkeypatch):
...
def test_start_proxy_injects_management_key_env(sanremes_home, monkeypatch):
    ...
    cfg = ip.build_proxy_config(
        mappings=[_sample_mapping()],
        ca_cert=sanremes_home / "ca.crt",
        ca_key=sanremes_home / "ca.key",
        http_listen=["127.0.0.1:9090"],
    )
    ip.write_proxy_config(cfg)
    (sanremes_home / "bin").mkdir(parents=True, exist_ok=True)
    fake_bin = sanremes_home / "bin" / "iron-proxy"
    ...
    assert captured_env[ip._MGMT_API_KEY_ENV].startswith("sanremes-mgmt-")
...
def test_docker_egress_node_options_uses_sentinel(sanremes_home, monkeypatch):
    """``_egress_proxy_args_for_docker`` should NOT put NODE_OPTIONS in
    env_overrides directly; it uses a sentinel key
    ``_SANREMES_EGRESS_NODE_OPTIONS_APPEND`` so DockerEnvironment can
    ...
    assert env.get("_SANREMES_EGRESS_NODE_OPTIONS_APPEND") == "--use-openssl-ca"
...
def test_persisted_nonce_roundtrip(sanremes_home, monkeypatch):
...
def test_get_status_probes_configured_bind_host(sanremes_home, monkeypatch):
