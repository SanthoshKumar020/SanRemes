...
SCRIPT_PATH = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_sanremes.py"
)


def load_module():
    spec = importlib.util.spec_from_file_location("openclaw_to_sanremes", SCRIPT_PATH)
...
"""
    ...
    assert "Tyler Williams > Active Projects: SanRemes Agent" in entries
...
def test_unreadable_config_is_refused_not_overwritten(tmp_path: Path):
    ...
    ``sanremes_cli/agent_import.py``.
    """
    mod = load_module()
    migrator, config_path = _allowlist_migrator(
        mod, tmp_path, MALFORMED_SANREMES_CONFIG)
...
def test_unreadable_config_blocks_later_config_steps_instead_of_partial_writes(
    ...
    assert config_path.read_text(encoding="utf-8") == MALFORMED_SANREMES_CONFIG
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.sanremes/config.yaml into a dotfiles repo.
    ...
    real.write_text("model: sanremes-4-405b\ncommand_allowlist:\n  - /usr/bin/*\n",
                    encoding="utf-8")
    ...
    assert "sanremes-4-405b" in real.read_text(encoding="utf-8")
...
def test_unreadable_config_refused_by_model_config_too(tmp_path: Path):
    ...
    config_path = target / "config.yaml"
    config_path.write_text(MALFORMED_SANREMES_CONFIG, encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # user at ~/.sanremes/config.yaml in its post-migration summary — it never
    # writes to either. Under skills-guard-v2 (#92021) these score as
    # informational _ref findings (the old critical agent_config_mod /
    # sanremes_config_mod findings no longer fire for bare mentions), so the
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_ref", "sanremes_config_ref"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``SanRemes``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "SanRemes prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told SanRemes to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "SanRemes config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "SanRemes uses tools well"
    # All-lowercase matches → lowercase ``sanremes``; this preserves the
    # real filesystem path ``~/.sanremes`` (SanRemes home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "sanremes should always respond concisely"
