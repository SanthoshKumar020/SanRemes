...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        sanremes_home.mkdir()
        (sanremes_home / "google_token.json").write_text("{}")
        (sanremes_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.sanremes/google_token.json" in container_paths
            assert "/root/.sanremes/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
