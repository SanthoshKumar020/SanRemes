...
def test_required_frontmatter_fields(p):
    ...
    sanremes = (fm.get("metadata") or {}).get("sanremes") or {}
    if not (sanremes.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.sanremes.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    sanremes = (fm.get("metadata") or {}).get("sanremes") or {}
    dangling = [
        rs for rs in (sanremes.get("related_skills") or []) if rs not in _all_names()
    ]
