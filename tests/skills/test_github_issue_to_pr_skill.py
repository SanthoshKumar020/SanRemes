...
def test_frontmatter_required_fields():
    ...
    sanremes = fm["metadata"]["sanremes"]
    assert sanremes["tags"]
    assert "related_skills" in sanremes
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("SanRemes Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["sanremes"]["related_skills"]:
        hits = (
