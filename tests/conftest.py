"""Shared fixtures for the sanremes-agent test suite.
...
2. **Isolated SANREMES_HOME.** SANREMES_HOME points to a per-test tempdir so
   code reading ``~/.sanremes/*`` via ``get_sanremes_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".sanremes"`` instead of the canonical
   ``get_sanremes_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SANREMES_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox SANREMES_HOME before ANY test module is imported ──────────────────
# `sanremes_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_sanremes_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.sanremes/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes SANREMES_HOME, but fixtures run
...
# the REAL SanRemes root — capture it BEFORE the sandbox rewires SANREMES_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.sanremes (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("SANREMES_KANBAN_HOME", "").strip()
_PRE_SANDBOX_SANREMES_HOME = os.environ.get("SANREMES_HOME", "")


def _sanremes_home_points_at_production(value: str) -> bool:
    """True when a pre-set SANREMES_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    SANREMES_HOME=~/.sanremes``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``sanremes_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.sanremes`` — the escape vector that landed pytest
    ...
    SANREMES_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".sanremes").resolve()
    except Exception:
        return True
...
if _sanremes_home_points_at_production(os.environ.get("SANREMES_HOME", "")):
    _SESSION_SANREMES_HOME = tempfile.mkdtemp(prefix="sanremes-test-home-")
    os.environ["SANREMES_HOME"] = _SESSION_SANREMES_HOME
    atexit.register(shutil.rmtree, _SESSION_SANREMES_HOME, True)
...
# like a real CLI") — which used to disarm sanremes_state's live-DB guard in
# the child at the same moment the child lost the SANREMES_HOME redirect.
# SANREMES_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# sanremes_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export SANREMES_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["SANREMES_TEST_ISOLATION"] = os.environ.get("SANREMES_HOME", "") or "1"

#: SANREMES_HOME as it stood when conftest was imported - i.e. before any test
...
SANREMES_HOME_AT_CONFTEST_IMPORT = os.environ.get("SANREMES_HOME", "")
...
# SANREMES_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SANREMES_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``sanremes_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a SanRemes agent session inherits them and
    # sanremes_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "SANREMES_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "SANREMES_INTERACTIVE",
    "SANREMES_QUIET",
    "SANREMES_TOOL_PROGRESS",
    "SANREMES_TOOL_PROGRESS_MODE",
    "SANREMES_MAX_ITERATIONS",
    "SANREMES_SESSION_PLATFORM",
    "SANREMES_SESSION_CHAT_ID",
    "SANREMES_SESSION_CHAT_NAME",
    "SANREMES_SESSION_CHAT_TYPE",
    "SANREMES_SESSION_THREAD_ID",
    "SANREMES_SESSION_SOURCE",
    "SANREMES_SESSION_KEY",
    "SANREMES_GATEWAY_SESSION",
    "SANREMES_CRON_SESSION",
    "_SANREMES_GATEWAY",
    "SANREMES_PLATFORM",
    "SANREMES_MODEL",
    "SANREMES_INFERENCE_MODEL",
    "SANREMES_INFERENCE_PROVIDER",
    "SANREMES_TUI_PROVIDER",
    "SANREMES_MANAGED",
    "SANREMES_MANAGED_DIR",
    "SANREMES_DEV",
    "SANREMES_CONTAINER",
    "SANREMES_EPHEMERAL_SYSTEM_PROMPT",
    "SANREMES_TIMEZONE",
    "SANREMES_REDACT_SECRETS",
    "SANREMES_BACKGROUND_NOTIFICATIONS",
    "SANREMES_EXEC_ASK",
    "SANREMES_HOME_MODE",
    "SANREMES_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.sanremes/kanban.db instead of the per-test SANREMES_HOME.
    "SANREMES_KANBAN_DB",
    "SANREMES_KANBAN_BOARD",
    "SANREMES_KANBAN_HOME",
    "SANREMES_KANBAN_WORKSPACES_ROOT",
    "SANREMES_KANBAN_LOGS_ROOT",
    "SANREMES_KANBAN_TASK",
    "SANREMES_KANBAN_WORKSPACE",
    "SANREMES_KANBAN_RUN_ID",
    "SANREMES_KANBAN_CLAIM_LOCK",
    "SANREMES_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "SANREMES_DELEGATED_CHILD_CONTEXT",
    "SANREMES_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "sanremes" host.
    "SANREMES_HONCHO_HOST",
    ...
    "SANREMES_DASHBOARD_OAUTH_CLIENT_ID",
    "SANREMES_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SANREMES_HOME to per-test tempdirs so code that
    reads ``~/.sanremes/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SANREMES_* vars that could change test semantics.
    for name in _SANREMES_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("SANREMES_HONCHO_HOST", "sanremes")

    # 3. Redirect SANREMES_HOME to a per-test tempdir. Code that reads
    #    ``~/.sanremes/*`` via ``get_sanremes_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.sanremes/*`` via
    #    ``Path.home() / ".sanremes"`` instead of ``get_sanremes_home()``
    #    is a bug to fix at the callsite.
    fake_sanremes_home = tmp_path / "sanremes_test"
    fake_sanremes_home.mkdir()
    (fake_sanremes_home / "sessions").mkdir()
    (fake_sanremes_home / "cron").mkdir()
    (fake_sanremes_home / "memories").mkdir()
    (fake_sanremes_home / "skills").mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(fake_sanremes_home))
    ...
    # sanremes_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("SANREMES_TEST_ISOLATION", str(fake_sanremes_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("SANREMES_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. sanremes_state computes ``DEFAULT_DB_PATH = get_sanremes_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from sanremes_state import ...``) that
    ...
    sanremes_state_mod = sys.modules.get("sanremes_state")
    if sanremes_state_mod is not None and hasattr(sanremes_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            sanremes_state_mod, "DEFAULT_DB_PATH", fake_sanremes_home / "state.db"
        )
    ...
    monkeypatch.setenv("SANREMES_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.sanremes/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import sanremes_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path SANREMES_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_sanremes_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from sanremes_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.sanremes. This autouse
...
# allow-list because test-level fixtures legitimately move SANREMES_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session SANREMES_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``SANREMES_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) SanRemes root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_SANREMES_HOME and not _sanremes_home_points_at_production(
        _PRE_SANDBOX_SANREMES_HOME
    ):
        # SANREMES_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_sanremes_root().resolve()
    # No pre-existing SANREMES_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".sanremes").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.sanremes`` captured at import time. Hermetic tests that legitimately
    move SANREMES_HOME to sibling tempdirs are unaffected.

    Only patches when ``sanremes_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("sanremes_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import sanremes_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.sanremes. See #69283."
        )
...
# ``sanremes_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL SanRemes root. This fixture wires the
# test-side knobs:
...
#     installs where SANREMES_HOME is not ~/.sanremes) into the guard's
...
# so subprocess children that import sanremes_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("sanremes_state")
    ...
    if _PRE_SANDBOX_SANREMES_HOME and not _sanremes_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_SANREMES_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local SanRemes home override redirects every later
    # ``get_sanremes_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from sanremes_constants import get_sanremes_home_override, set_sanremes_home_override

        if get_sanremes_home_override() is not None:
            set_sanremes_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal sanremes config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``sanremes-gateway`` process
...
# correlated with a ``tests/sanremes_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> sanremes-gateway``
...
def _wal_is_usable() -> bool:
    """True when SanRemes will actually put a database into WAL mode here.

    SanRemes refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running SanRemes can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a SanRemes managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``sanremes_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_sanremes_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_sanremes_home`` fixture
    redirects ``SANREMES_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.sanremes/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``sanremes_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["SANREMES_VOICE_TTS"] = "1"``. The
...
#      ``sanremes_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``sanremes_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``sanremes_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``sanremes_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# SanRemes runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.sanremes``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where SanRemes falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — SanRemes uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart sanremes-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting sanremes/python patterns are also blocked.
    """
    ...
    _SANREMES_TOKENS = (
        "sanremes-gateway",
        "sanremes.service",
        "sanremes_cli.main gateway",
        "sanremes_cli/main.py gateway",
        "gateway/run.py",
        "sanremes gateway",
    )
    _MUTATING_VERBS = (
    ...
    def _matches_sanremes_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SANREMES_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_sanremes_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch sanremes-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m sanremes_cli.main".
                if (
                    "sanremes" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live sanremes-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting sanremes/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `sanremes update` (or the
        # equivalent `python -m sanremes_cli.main update`).  These commands
        ...
        if "update" in low and (
            # sanremes update / sanremes update --gateway / setsid bash -c ... sanremes update
            ("sanremes" in low and "update" in low.split())
            or
            # python -m sanremes_cli.main update --gateway
            ("sanremes_cli" in low and "update" in low.split())
            or
            # venv/bin/sanremes update  (absolute path variant used in tests)
            (".venv/bin/sanremes" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`sanremes update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_SANREMES_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import sanremes_cli.voice as _voice
    except Exception:
