...
symlink ``~/.sanremes/config.yaml`` (and other state files) to a git-tracked
...
def test_atomic_yaml_write_restores_owner_on_real_symlink_target(
    ...
    sanremes-owned /opt/data/config.yaml via atomic replace, leaving the new file
...
def test_atomic_replace_real_cross_device(tmp_path: Path) -> None:
    ...
    other_fs_dir = shm / f"sanremes-exdev-test-{_uuid.uuid4().hex[:8]}"
    other_fs_dir.mkdir()
    try:
