...
- files inside the SanRemes home tree are repaired with ``chmod u+rw``
...
from sanremes_state import SessionDB, preflight_db_writability
...
def sanremes_home(tmp_path, monkeypatch):
    """Isolated SANREMES_HOME so the repair scope covers tmp DBs."""
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, sanremes_home):
        db = sanremes_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, sanremes_home):
        db = sanremes_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, sanremes_home):
        sub = sanremes_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, sanremes_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, sanremes_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, sanremes_home):
        db = sanremes_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, sanremes_home):
        db_path = sanremes_home / "state.db"
    ...
    def test_sessiondb_actionable_error_outside_home(
        ...
        sanremes_state._set_last_init_error(None)
        try:
            with pytest.raises(sqlite3.OperationalError) as exc_info:
                SessionDB(db_path)
            msg = str(exc_info.value)
