"""
Tests for timezone support (sanremes_time module + integration points).
...
import sanremes_time


def _reset_sanremes_time_cache():
    """Reset the sanremes_time module cache (replacement for removed reset_cache)."""
    sanremes_time._cached_tz = None
    sanremes_time._cached_tz_name = None
    sanremes_time._cache_resolved = False


# =========================================================================
# sanremes_time.now() — core helper
# =========================================================================

class TestSanRemesTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_sanremes_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["SANREMES_TIMEZONE"] = "Asia/Kolkata"
        result = sanremes_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["SANREMES_TIMEZONE"] = "UTC"
        result = sanremes_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["SANREMES_TIMEZONE"] = "Europe/London"
        tz = sanremes_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When SANREMES_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) SANREMES_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("SANREMES_TIMEZONE=" + os.environ.get("SANREMES_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "SANREMES_TIMEZONE=NOT_SET" in result["output"], (
            "SANREMES_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When SANREMES_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("SANREMES_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=sanremes_tz) which shifted
        absolute time when system-local tz != SanRemes tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when SanRemes tz
        ...
        # Use a SanRemes timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _sanremes_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["SANREMES_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_sanremes_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of SanRemes timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["SANREMES_TIMEZONE"] = "US/Eastern"
        _reset_sanremes_time_cache()
