    ...
    with patch.dict(
        "sys.modules",
        {
            "sanremes_constants": MagicMock(
                get_sanremes_home=MagicMock(return_value="/tmp/sanremes_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'sanremes-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'sanremes-release' patched"
    }
