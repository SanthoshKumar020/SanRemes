...
``platform="tui"`` / ``chat_id=SANREMES_SESSION_KEY``, but no component ever
...
class TestCollectKanbanNotifications:
    ...
    def test_profile_scoped_session_reads_the_shared_board(self, tmp_path):
        ...
        sanremes_cli/kanban_db.py module docstring): ``kanban_home()`` anchors on
        ``get_default_sanremes_root()``, which resolves the process env and
        ...
        token = set_sanremes_home_override(str(other_profile_home))
        try:
