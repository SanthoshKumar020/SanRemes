...
locally, one reached over SSH, one behind a plain URL+token, or SanRemes Cloud —
and only the first two run with ``SANREMES_DESKTOP=1`` in their environment.
...
model "You are chatting inside the SanRemes desktop app".
...
class TestDesktopUiToolset:
    ...
    def test_stays_off_the_core_tool_list(self):
        ...
        from toolsets import _SANREMES_CORE_TOOLS

        assert GUI_TOOLS.isdisjoint(_SANREMES_CORE_TOOLS)
...
class TestSurfaceResolution:
    ...
    def test_desktop_env_alone_does_not_grant_them(self, no_desktop_env):
        ...
        The embedded terminal pane runs `sanremes --tui` against this same
        backend; env-keyed gating handed it GUI tools it cannot answer.
        """
        no_desktop_env.setenv("SANREMES_DESKTOP", "1")
        assert "desktop_ui" not in server._gui_surface_toolsets("tui")
...
class TestResolverPlumbing:
    ...
    def test_explicit_env_pin_still_wins(self, no_desktop_env):
        """SANREMES_TUI_TOOLSETS is an operator override; surface can't re-add."""
        no_desktop_env.setenv("SANREMES_TUI_TOOLSETS", "web,memory")
