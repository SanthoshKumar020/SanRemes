...
def server():
    ...
    # e.g. sanremes_cli.active_sessions would bind the mocked get_sanremes_home
    ...
    with patch.dict("sys.modules", {
        "sanremes_constants": MagicMock(get_sanremes_home=MagicMock(return_value="/tmp/sanremes_test")),
        "sanremes_cli.env_loader": MagicMock(),
        "sanremes_cli.banner": MagicMock(),
        "sanremes_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `SANREMES_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("SANREMES_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("SANREMES_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_config_roundtrip(server, tmp_path):
    server._sanremes_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/sanremes-agent-dev": {"name": "sanremes-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "sanremes-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient SANREMES_HOME
    ...
    # The gateway's own SANREMES_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `sanremes config set` does)
    ...
    import sanremes_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_sanremes_home", lambda: tmp_path)
    ...
    # Activate midnight, as `sanremes config set display.skin midnight` would.
