...
"No LLM provider configured. Run `sanremes model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real SANREMES home.
...
# found" (the b200/sanremes-ultra-sft report). The stored MODEL NAME is the one
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("sanremes-ultra-sft")
            == "custom:sanremes-ultra"
        )
