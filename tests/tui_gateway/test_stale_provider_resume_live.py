...
def live_home(monkeypatch):
    """A REAL isolated SANREMES_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="sanremes-live-staleprov-"))
    home = tmp / ".sanremes"
    ...
    # sanremes_constants caches the resolved home at first read — the env var
    ...
    import sanremes_cli.config as hconfig
    ...
    # conftest also re-pins sanremes_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(sanremes_state, "DEFAULT_DB_PATH", home / "state.db")
    monkeypatch.setattr(server, "_db", None, raising=False)
    monkeypatch.setattr(server, "_db_error", None, raising=False)
    monkeypatch.setattr(server, "_sanremes_home", str(home), raising=False)
    yield home, server
