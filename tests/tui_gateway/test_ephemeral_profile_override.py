"""Regression tests: profile SANREMES_HOME override in ephemeral agent threads (#50233).

Why: normal prompt turns bind ``session['profile_home']`` via
``set_sanremes_home_override`` before ``run_conversation`` so the turn runs against
...
``SANREMES_HOME`` ContextVar set on the session-create thread does NOT propagate to
...
PROFILE_HOME = "/home/user/.sanremes/profiles/work"
...
def override_calls():
    ...
    with patch("tui_gateway.server.threading.Thread", _InlineThread), \
        patch("tui_gateway.server.set_sanremes_home_override", return_value="TOK") as m_set, \
        patch("tui_gateway.server.reset_sanremes_home_override") as m_reset, \
