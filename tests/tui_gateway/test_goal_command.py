...
def sanremes_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves SANREMES_HOME.
...
@pytest.fixture()
def server(sanremes_home, monkeypatch):
    ...
    # Pin config resolution to the isolated SANREMES_HOME. Sibling test
    ...
    # module-level ``_sanremes_home = get_sanremes_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_sanremes_home", sanremes_home)
...
def test_moa_bare_returns_usage(server, session, sanremes_home):
    _write_moa_config(sanremes_home, """
moa:
