...
``SANREMES_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def sanremes_root(tmp_path, monkeypatch):
    """A temp SANREMES_HOME root with two named profiles: 'work' and 'other'.

    Pointing SANREMES_HOME at a dir outside ~/.sanremes makes it the profile ROOT
    (get_default_sanremes_root's Docker/custom branch), so named profiles live at
    ``<root>/profiles/<name>/`` and the launch/default profile is ``<root>``.
    """
    root = tmp_path / "sanremes_home"
...
def test_add_lands_in_named_profile_only(sanremes_root):
    root = sanremes_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(sanremes_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_writes_env_and_header_to_right_profile(sanremes_root):
    root = sanremes_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(sanremes_root):
    root = sanremes_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(sanremes_root):
    root = sanremes_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(sanremes_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(sanremes_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(sanremes_root):
    root = sanremes_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == SANREMES_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
