...
def test_outbox_drain_returns_each_envelope_once(home):
    ...
    env = bot_relay.enqueue_envelope(
        home, target=target, message="m", sender_profile="default", sender_handle="sanremes"
    )
...
def test_deliver_validates_profile_and_runs_transport(home, monkeypatch):
    ...
    assert argv[0].rsplit("\\", 1)[-1].rsplit("/", 1)[-1] in ("sanremes", "sanremes.exe")
    assert "Bot Chat" in argv and "--query-file" in argv

    # 'sanremes' alias resolves to default
    _result(srv._methods["bot_relay.deliver"](2, {"profile": "sanremes", "message": "x"}))
...
def test_deliver_write_failure_still_removes_tempfile(home, monkeypatch, tmp_path):
    ...
    assert not glob.glob(str(tmp_path / "sanremes-relay-dm-*")), "tempfile leaked"
