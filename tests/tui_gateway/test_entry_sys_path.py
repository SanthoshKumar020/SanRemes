...
SanRemes's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
neutralize this before any non-stdlib import is resolved, by delegating to the
shared ``sanremes_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call sanremes_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call sanremes_bootstrap.harden_import_path()"
    )
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the SanRemes root.  harden_import_path must relocate the
    SanRemes root to the front so ``from utils import ...`` resolves to SanRemes."""
    import sys

    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/sanremes", "/usr/lib"]
        sanremes_bootstrap.harden_import_path(src_root="/opt/sanremes")
        assert sys.path[0] == "/opt/sanremes"
        assert sys.path.index("/opt/sanremes") < sys.path.index(
