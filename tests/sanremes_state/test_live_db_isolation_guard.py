...
developer's REAL ~/.sanremes/state.db, and a pytest-spawned process flipped
...
# Must match the root the guard itself computes.  Hardcoding ``~/.sanremes``
# silently disarmed every assertion below on Windows, where the real root is
# ``%LOCALAPPDATA%\sanremes``: the paths under test were then *correctly*
...
REAL_ROOT = sanremes_state._real_platform_state_root()
...
class TestProductionPathRefused:
    def test_explicit_production_db_path_raises(self):
        """SessionDB pointed at the real ~/.sanremes/state.db must fail hard."""
    ...
    def test_default_resolution_to_production_raises(self, monkeypatch):
        ...
        Simulates the escape vector: SANREMES_HOME leaked/reset to the real
        home (subprocess child, stale worktree, gateway-launched shell) so
        ``_default_db_path()`` resolves the production DB.
        """
        monkeypatch.setenv("SANREMES_HOME", str(REAL_ROOT))
        ...
        monkeypatch.setattr(
            sanremes_state, "DEFAULT_DB_PATH", sanremes_state._IMPORT_DEFAULT_DB_PATH
        )
        with pytest.raises(RuntimeError, match="live-system guard"):
            SessionDB()
...
class TestHermeticPathsAllowed:
    ...
    def test_tmp_sanremes_home_default_resolution_works(self, tmp_path, monkeypatch):
        """Argless SessionDB() under a hermetic SANREMES_HOME must succeed."""
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "hermetic-home"))
        monkeypatch.setattr(
            sanremes_state, "DEFAULT_DB_PATH", sanremes_state._IMPORT_DEFAULT_DB_PATH
        )
        db = SessionDB()
...
class TestBypassMarker:
    ...
    def test_bypass_marker_disables_state_db_guard(self):
        ...
        """
        sanremes_state._ensure_test_isolation(REAL_ROOT / "state.db")
...
class TestSessionStoreLoudFailure:
    def test_guard_error_is_not_swallowed_into_jsonl_fallback(
        ...
        monkeypatch.setattr(sanremes_state, "SessionDB", _boom)
        with pytest.raises(RuntimeError, match="live-system guard"):
...
class TestSubprocessChildCovered:
    def test_child_without_sanremes_home_is_refused(self, tmp_path):
        """A subprocess child of a test (no SANREMES_HOME) must be blocked.
        ...
        env = {
            ...
            if k not in ("SANREMES_HOME", "PYTEST_PLUGINS", "PYTHONPATH")
        }
    ...
    def test_child_with_tmp_sanremes_home_succeeds(self, tmp_path):
        """Same child, hermetic SANREMES_HOME: must work — no false positive."""
        env = {
            k: v
            for k, v in os.environ.items()
        ...
        env["PYTEST_CURRENT_TEST"] = "tests/fake.py::test_child (call)"
        env["SANREMES_HOME"] = str(tmp_path / "child-home")
        env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
        code = (
            "from sanremes_state import SessionDB\n"
            "db = SessionDB()\n"
