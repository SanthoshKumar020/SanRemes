...
entirely on the process environment: ``SANREMES_HOME`` says *where* to write and
...
# hardcoded ~/.sanremes — that root is %LOCALAPPDATA%\sanremes on Windows) and
...
root = sanremes_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        sanremes_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
def _scrubbed_env(**overrides):
    ...
    Also strips ``SANREMES_TEST_ISOLATION`` — the conftest-exported marker
    ...
    env = {
        ...
        and k not in ("SANREMES_HOME", "SANREMES_TEST_ISOLATION")
    }
...
class TestScrubbedChildEnvironment:
    def test_child_without_pytest_env_still_refuses_production_db(self):
        """The #82770 escape: no PYTEST_* and no SANREMES_HOME, yet still a test.

        This is the exact shape of the leak — the child resolves the real
        ``state.db`` because ``SANREMES_HOME`` is gone, and the env-only guard
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{sanremes_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
...
class TestPytestProcessRecognition:
    ...
    def test_recognises_pytest_invocations(self, cmdline):
        assert sanremes_state._process_looks_like_pytest(self._FakeProc(cmdline))

    @pytest.mark.parametrize(
        "cmdline",
        [
            ["sanremes", "gateway", "start"],
            ["/usr/bin/python", "-m", "sanremes_cli.main", "sessions", "list"],
            # A path that merely *contains* "pytest" is not a pytest process:
            # tmp paths like /tmp/pytest-of-dev/... show up in real argv.
            ["sanremes", "run", "--file", "/tmp/pytest-of-dev/test0/input.txt"],
        ],
    )
    def test_ignores_non_pytest_invocations(self, cmdline):
        assert not sanremes_state._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_unreadable_process_is_not_pytest(self):
        ...
        assert not sanremes_state._process_looks_like_pytest(_Denied())
