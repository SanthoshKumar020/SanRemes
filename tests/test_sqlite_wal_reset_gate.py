...
SanRemes must not *enable* multi-process WAL on SQLite builds that still contain
...
def _reset_wal_reset_bug_warnings():
    sanremes_state._wal_reset_bug_warned_paths.clear()
    yield
    sanremes_state._wal_reset_bug_warned_paths.clear()
...
class TestApplyWalWalResetGate:
    def test_fresh_db_uses_delete_when_vulnerable(self, tmp_path, monkeypatch, caplog):
        monkeypatch.setattr(
            sanremes_state, "is_sqlite_wal_reset_vulnerable", lambda version_info=None: True
        )
        conn = sqlite3.connect(str(tmp_path / "fresh.db"))
        with caplog.at_level("WARNING", logger="sanremes_state"):
...
class TestNoDowngradeUnderConcurrentOpeners:
    ...
    def test_configured_delete_refuses_when_probe_blocked(
        ...
        monkeypatch.setattr(sanremes_state, "resolve_journal_mode", lambda: "delete")
...
def test_doctor_warns_without_adding_issues(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sanremes_state, "sqlite_source_id", lambda: "testid-abc")
