...
opts in with --run-e2e or SANREMES_RUN_E2E=1.  This is intentional — the test
...
pytestmark = pytest.mark.skipif(
    os.environ.get("SANREMES_RUN_E2E", "0") != "1",
    reason="E2E proxy test — set SANREMES_RUN_E2E=1 to run (requires network + curl + openssl)",
)
...
def test_iron_proxy_swaps_authorization_header_end_to_end(sanremes_home, monkeypatch):
...
def test_iron_proxy_swaps_x_api_key_header_end_to_end(sanremes_home, monkeypatch):
...
def test_iron_proxy_management_reload_end_to_end(sanremes_home, monkeypatch):
