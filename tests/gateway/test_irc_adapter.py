...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "sanremes"
        assert adapter.channel == "#sanremes-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :sanremes: hello there")
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG sanremes :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        adapter = IRCAdapter(cfg)
        adapter._current_nick = "sanremes"
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :sanremes: hello")
        assert len(dispatched) == 0
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        monkeypatch.setenv("IRC_NICKNAME", "sanremesbot")
        monkeypatch.setenv("IRC_USE_TLS", "false")

        # Server greets us with 001 RPL_WELCOME, then nothing for QUIT drain.
        conn = _FakeIRCConnection([b":server 001 sanremesbot-cron :Welcome"])
        ...
        assert any(line.startswith("NICK sanremesbot-cron") for line in sent_lines)
        assert any(line.startswith("USER sanremesbot-cron 0 * :SanRemes Agent (cron)")
