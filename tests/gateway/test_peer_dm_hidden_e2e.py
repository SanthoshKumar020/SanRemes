"""E2E: ``sanremes peer dm`` against a REAL api_server gateway whose canonical
Bot Chat session is HIDDEN (issue #91583).

Two real SANREMES homes in spirit: the "peer" side is a real
:class:`APIServerAdapter` bound to a real loopback TCP socket over a real
SQLite ``state.db`` (its own tmp SANREMES_HOME) containing a hidden
``Bot Chat`` row — exactly what Bot Mode leaves behind. The "local" side is
the stock ``sanremes peer dm`` client code (``sanremes_cli.subcommands.peer``),
...
from sanremes_cli.subcommands import peer as peer_cmd
...
def peer_gateway(tmp_path, monkeypatch):
    """A real api_server gateway (own SANREMES_HOME + state.db) on a real socket."""
