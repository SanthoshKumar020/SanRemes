...
def _clear_env(monkeypatch):
    monkeypatch.delenv("SANREMES_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", raising=False)
    monkeypatch.delenv("SANREMES_GATEWAY_HTTPX_MAX_KEEPALIVE", raising=False)
...
def test_env_override_rejects_garbage(monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "not-a-number")
    monkeypatch.setenv("SANREMES_GATEWAY_HTTPX_MAX_KEEPALIVE", "-3")
