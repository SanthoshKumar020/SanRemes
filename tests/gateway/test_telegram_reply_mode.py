...
class TestTelegramYamlConfigLoading:
    ...
    def _write_config(self, tmp_path, content: str):
        ...
        sanremes_home.mkdir()
        (sanremes_home / "config.yaml").write_text(content, encoding="utf-8")
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        sanremes_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/NousResearch/sanremes-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via SanRemes-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/NousResearch/sanremes-agent/issues/87051:
    ...
    response in a different chat lane than the SanRemes topic the session runs
    in. The no-anchor branch must prefer the SanRemes topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_sanremes_topic_thread_id(self):
