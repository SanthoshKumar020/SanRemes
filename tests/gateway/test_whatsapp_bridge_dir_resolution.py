...
(/opt/sanremes/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable SANREMES_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_sanremes_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable SANREMES_HOME."""
    ...
    monkeypatch.setattr(
        "sanremes_constants.get_sanremes_home", lambda: sanremes_home
    )
    ...
    expected = sanremes_home / "scripts" / "whatsapp-bridge"
