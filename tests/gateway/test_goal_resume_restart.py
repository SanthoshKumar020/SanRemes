...
def sanremes_home(tmp_path, monkeypatch):
    ...
    # get_sanremes_home() prefers the context-local override over the env
    # var, so a set_sanremes_home_override() leaked by ANY earlier test in
    ...
    token = set_sanremes_home_override(str(home))
...
class TestCliResumeRestartsWork:
    def test_resume_after_budget_exhaustion_queues_continuation(self, sanremes_home):
    ...
    def test_resume_without_goal_queues_nothing(self, sanremes_home):
...
class TestGatewayResumeRestartsWork:
    ...
    @pytest.mark.asyncio
    async def test_resume_without_goal_enqueues_nothing(self, sanremes_home):
        runner, adapter = _make_runner()
