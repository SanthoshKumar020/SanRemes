...
def test_session_store_and_runner_reopen_after_failed_construction(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(sanremes_state, "SessionDB", fail_once_session_db)
    monkeypatch.setattr(sanremes_state, "_default_db_path", lambda: db_path)
    ...
    monkeypatch.setattr(sanremes_state, "SessionDB", runner_fail_once)
    monkeypatch.setattr(sanremes_state, "AsyncSessionDB", lambda db: ("async", db))
...
def test_recovered_db_rows_survive_fallback_structural_save(monkeypatch, tmp_path) -> None:
    ...
    database = sanremes_state.SessionDB(db_path=db_path)
    ...
    real_session_db = sanremes_state.SessionDB
    calls = 0
