...
Proves, on windows-latest:
  1. `GatewayControlServer` binds ``\\\\.\\pipe\\sanremes-gateway-<hash>`` via
...
os.environ["SANREMES_HOME"] = sys.argv[2]
...
def test_named_pipe_identify_status_and_fleet_consumer(live_server, monkeypatch):
    ...
    assert ident["kind"] == "sanremes-gateway"
    ...
    monkeypatch.setattr(
        "sanremes_cli.profiles._get_profiles_root", lambda: home / "no-profiles"
    )
...
def test_pipe_gone_after_kill_falls_back(live_server, monkeypatch):
    ...
    (home / "gateway_state.json").write_text(
        json.dumps(
            {"pid": os.getpid(), "code_sha": "OLD", "kind": "sanremes-gateway"}
        ),
        encoding="utf-8",
    )
    monkeypatch.setattr(
