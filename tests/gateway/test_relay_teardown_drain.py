...
def test_drain_grace_clamps_to_disconnect_budget(monkeypatch):
    ...
    monkeypatch.delenv("SANREMES_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", raising=False)
...
def test_drain_grace_honors_env_budget(monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "12")
    assert wt._disconnect_drain_grace_s() == wt._DISCONNECT_DRAIN_GRACE_S
    monkeypatch.setenv("SANREMES_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "3.5")
    grace = wt._disconnect_drain_grace_s()
    assert 0 <= grace <= 3.5 - 3 * wt._TEARDOWN_AWAIT_TIMEOUT_S
    monkeypatch.setenv("SANREMES_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "1")
