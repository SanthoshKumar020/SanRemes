...
that omits the SanRemes venv bin dir and ~/.local/bin. The spawned
tui_gateway.slash_worker then cannot resolve SanRemes-managed CLIs such as
...
class TestPrependToolPaths:
    def test_prepends_managed_venv_and_user_bin(self, monkeypatch, tmp_path):
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "hh"))
        env = {"PATH": "/usr/bin"}
