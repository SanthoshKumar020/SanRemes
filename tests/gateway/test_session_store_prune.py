...
def test_session_store_default_db_uses_runtime_sanremes_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime SANREMES_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    sanremes_state before a fixture redirected SANREMES_HOME used to pin every
    default SessionDB() at the developer's real ~/.sanremes/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_sanremes_home"
    fake_home.mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "sanremes sessions list" in raw["_README"]
