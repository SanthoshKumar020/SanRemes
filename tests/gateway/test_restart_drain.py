...
async def test_restart_command_while_busy_requests_drain_without_interrupt(monkeypatch):
    ...
    monkeypatch.delenv("SANREMES_GATEWAY_EXTERNAL_SUPERVISOR", raising=False)
...
def test_load_busy_text_mode_follows_input_mode_and_honors_legacy(tmp_path, monkeypatch):
    monkeypatch.setattr(gateway_run, "_sanremes_home", tmp_path)
    monkeypatch.delenv("SANREMES_GATEWAY_BUSY_TEXT_MODE", raising=False)
    monkeypatch.delenv("SANREMES_GATEWAY_BUSY_INPUT_MODE", raising=False)
    ...
    monkeypatch.setenv("SANREMES_GATEWAY_BUSY_TEXT_MODE", "queue")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "queue"

    # Bogus legacy value is ignored → falls through to busy_input_mode (interrupt).
    monkeypatch.setenv("SANREMES_GATEWAY_BUSY_TEXT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "interrupt"
...
async def test_windows_detached_restart_scrubs_gateway_marker(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway_run, "_resolve_sanremes_bin", lambda: ["sanremes"])
    ...
    assert cmd[-3:] == ["sanremes", "gateway", "restart"]
    assert kwargs["env"].get("_SANREMES_GATEWAY") is None
...
async def test_request_restart_skips_wait_when_only_wedged_turns(monkeypatch):
    ...
    Regression: a WhatsApp turn wedged for 30+ min pinned `sanremes update`
    ...
    """
    monkeypatch.delenv("SANREMES_AGENT_TIMEOUT", raising=False)
    runner, _adapter = make_restart_runner()
    runner.stop = AsyncMock()
...
def test_wedged_agent_count_disabled_timeout_counts_nothing(monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_AGENT_TIMEOUT", "0")
    runner, _adapter = make_restart_runner()
