...
``gateway run --replace`` only kills same-SANREMES_HOME PID-file holders.
...
def test_lock_conflict_names_owning_profile(adapter):
    ...
    existing = {
        ...
        "sanremes_home": "/opt/data/profiles/lead-gen-outreach",
    }
    ...
    assert adapter._fatal_error_message == (
        ...
        "(sanremes --profile lead-gen-outreach gateway stop)."
    )
    assert adapter._fatal_error_retryable is True
    assert adapter._fatal_error_code == "telegram-bot-token_lock"


def test_lock_conflict_infers_profile_from_legacy_sanremes_home(adapter):
    """Locks written before the profile field existed still attribute via sanremes_home."""
    existing = {
        "pid": 559,
        "start_time": 123,
        "sanremes_home": "/opt/data/profiles/lead-gen-outreach",
