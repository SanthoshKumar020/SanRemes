...
- The original implementation hardcoded an allow-list of known gateway
  ...
  sources (``acp``, ``webhook``, user-defined ``SANREMES_SESSION_SOURCE``
  ...
  but exist in .sanremes/sessions."
...
def test_session_list_surfaces_all_user_facing_sources(monkeypatch):
    ...
    assert "custom-1" in ids, "custom SANREMES_SESSION_SOURCE values were being hidden"
