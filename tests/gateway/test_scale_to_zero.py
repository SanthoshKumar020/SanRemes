...
# ── scale_to_zero_enabled (the Labs SANREMES_SCALE_TO_ZERO stamp, D11/Q8=A) ────
...
_FLY_ENV = {FLY_APP_NAME_ENV: "sanremes-agent-stg-test", FLY_MACHINE_ID_ENV: "d891234f"}
...
def test_suspend_self_posts_suspend_for_this_machine(tmp_path):
    ...
    assert request.startswith(
        "POST /v1/apps/sanremes-agent-stg-test/machines/d891234f/suspend HTTP/1.1\r\n"
    )
    assert "Host: flaps\r\n" in request
