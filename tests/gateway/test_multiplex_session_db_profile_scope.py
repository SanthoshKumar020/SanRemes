...
redirected ``get_sanremes_home()`` for that turn.  The rows carried the correct
...
def multiplex_homes(tmp_path, monkeypatch):
    """A root home plus a named profile home, with SANREMES_HOME on the root.
    ...
    # The suite-wide fixture in conftest re-points ``sanremes_state.DEFAULT_DB_PATH``
    ...
    # the hatch is closed and resolution goes through ``get_sanremes_home()``,
    # which is what production does.  ``SANREMES_HOME`` above still keeps that
...
def test_primary_handler_enters_routed_profile_scope_before_dispatch(multiplex_homes):
    ...
    async def capture_scope(event):
        seen.append(Path(get_sanremes_home()))
    ...
    assert seen == [profile]
    assert Path(get_sanremes_home()) == root
...
def test_primary_handler_rejected_route_falls_back_and_marks_sentinel(multiplex_homes):
    ...
    async def capture_scope(event):
        seen.append((Path(get_sanremes_home()), event.source.profile_route_rejected))
...
def test_db_handle_follows_the_active_profile_scope(multiplex_homes):
    ...
    token = set_sanremes_home_override(str(profile))
    try:
