...
def test_slack_wake_resumes_the_creators_workspace_scoped_session(tmp_path, monkeypatch):
    monkeypatch.setenv("SANREMES_KANBAN_DB", str(tmp_path / "wake-scope.db"))
    kb.init_db()
...
def test_slack_wake_falls_back_to_the_adapter_channel_workspace_map(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_KANBAN_DB", str(tmp_path / "wake-scope-fallback.db"))
    kb.init_db()
    _completed_subscription(
        platform="slack",
        chat_id=CHANNEL,
...
def test_unknown_channel_keeps_the_previous_unscoped_wake(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_KANBAN_DB", str(tmp_path / "wake-scope-unknown.db"))
    kb.init_db()
    _completed_subscription(
        platform="slack",
        chat_id=CHANNEL,
...
def test_unscoped_platform_wake_key_is_byte_identical(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_KANBAN_DB", str(tmp_path / "wake-scope-telegram.db"))
    kb.init_db()
    _completed_subscription(
        platform="telegram",
        chat_id="chat-dm",
