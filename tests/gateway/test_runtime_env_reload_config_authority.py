...
SANREMES_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env SANREMES_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``sanremes config get terminal.backend`` still says local.
    """
    sanremes_home = tmp_path / ".sanremes"
    sanremes_home.mkdir()
    (sanremes_home / "config.yaml").write_text(
    ...
    (sanremes_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
