...
E2E-style: real profile homes under a temp SANREMES root, real config.yaml
...
def sanremes_root(tmp_path, monkeypatch):
    ...
    # get_default_sanremes_root memoizes per (native_home, env_home) pair, so
...
class TestMultiplexOnToolsetIsolation:
    @pytest.mark.asyncio
    async def test_each_profile_sees_its_own_toolsets(self, sanremes_root):
    ...
    @pytest.mark.asyncio
    async def test_owner_key_does_not_open_secondary_profile(self, sanremes_root):
    ...
    @pytest.mark.asyncio
    async def test_unknown_profile_is_404(self, sanremes_root):
        adapter = _make_adapter(multiplex=True)
        async with TestClient(TestServer(_make_app(adapter))) as cli:
            resp = await cli.get(
                "/p/ghost/v1/toolsets",
...
class TestMultiplexOffPrefixFailsClosed:
    ...
    def test_foreign_prefix_rejected(self, sanremes_root):
    ...
    def test_self_referential_prefix_falls_through(self, sanremes_root):
    ...
    def test_own_named_profile_prefix_falls_through(self, sanremes_root, monkeypatch):
        """A gateway launched FOR profile lokaj accepts /p/lokaj/…"""
        monkeypatch.setenv(
            "SANREMES_HOME", str(sanremes_root / "profiles" / "lokaj")
        )
        adapter = _make_adapter(multiplex=False)
    ...
    @pytest.mark.asyncio
    async def test_foreign_prefix_is_404_end_to_end(self, sanremes_root):
        adapter = _make_adapter(multiplex=False)
        async with TestClient(TestServer(_make_app(adapter))) as cli:
            resp = await cli.get(
                "/p/lokaj/v1/toolsets",
