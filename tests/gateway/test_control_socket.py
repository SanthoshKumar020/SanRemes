...
def test_short_home_binds_in_home(tmp_path: Path):
    ...
    try:
        short_home = short_root / ".sanremes"
        short_home.mkdir()
...
def test_windows_pipe_name_is_stable_and_home_scoped(tmp_path: Path):
    ...
    assert a.startswith(r"\\.\pipe\sanremes-gateway-")
    assert a != b
...
def test_default_identify_payload_shape(home: Path, monkeypatch):
    ...
    for key in ("sanremes_home", "supervisor", "kind", "start_time"):
        assert key in ident
...
def test_collect_fleet_versions_prefers_socket(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.profiles._get_profiles_root", lambda: tmp_path / "no-profiles"
    )
    # stale state file that would report a WRONG pid — socket must win
    (home / "gateway_state.json").write_text(
        json.dumps({"pid": 1, "code_sha": "stalefile", "kind": "sanremes-gateway"})
    )
    monkeypatch.setattr(
...
def test_runtime_inventory_dedupes_same_pid_across_homes(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.profiles._get_profiles_root", lambda: profiles_root
    )
    monkeypatch.setattr(
        "sanremes_cli.gateway._get_service_pids", lambda all_profiles=False: set()
    )
    monkeypatch.setattr(
        "sanremes_cli.gateway.find_profile_gateway_processes", lambda: []
    )
    monkeypatch.setattr(
        "gateway.control_socket.identify_gateway",
