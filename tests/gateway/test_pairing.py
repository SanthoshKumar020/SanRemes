...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_sanremes_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        # A profile's store anchors to the sanremes ROOT, not the current
        # SANREMES_HOME — the current home may itself be a profile, and nesting
        # profiles inside profiles is how a `-p work` CLI and its gateway end
        # up reading different files. Patch that seam, not get_sanremes_home.
        with patch("gateway.pairing.PAIRING_DIR", global_dir), patch(
            "gateway.pairing.get_default_sanremes_root", return_value=home
        ):
            store = PairingStore(profile="alice")
            # Scoped under the mocked root's profile dir, using the same
            # consolidated layout a standalone `sanremes -p alice` resolves —
...
class TestProfileScopedStorage:
    ...
    under each profile's own SanRemes home so a multiplexing gateway can keep
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``sanremes pairing`` CLI continues to work without a profile context."""
        from sanremes_constants import get_sanremes_home
        monkeypatch.setattr("sanremes_constants.get_sanremes_home", lambda: tmp_path)
    ...
    def test_default_store_is_not_frozen_at_first_use(self, tmp_path, monkeypatch):
        ...
        whatever SANREMES_HOME resolved to the first time this module's
        default path was computed. A long-lived process (the gateway,
        started once at container/process boot) can construct a
        PairingStore before SANREMES_HOME/profile context is fully
        ...
        monkeypatch.setattr("sanremes_constants.get_sanremes_home", lambda: first_home)
        first_store = PairingStore()
        assert first_store._dir == first_home / "platforms" / "pairing"

        monkeypatch.setattr("sanremes_constants.get_sanremes_home", lambda: second_home)
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        """Explicit profile stores use that profile's normal SanRemes layout."""
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    ...
    def test_profile_store_matches_profile_cli_home(self, tmp_path, monkeypatch):
        """Gateway and ``sanremes -p`` must resolve the same pairing store."""
