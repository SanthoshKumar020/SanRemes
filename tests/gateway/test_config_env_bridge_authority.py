...
Guards against the 60-vs-500 bug where a stale `.env SANREMES_MAX_ITERATIONS=60`
...
def _run_gateway_import(sanremes_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "SANREMES_AGENT_TIMEOUT",
            "SANREMES_AGENT_TIMEOUT_WARNING",
            "SANREMES_TURN_LEASE_TIMEOUT",
            "SANREMES_SESSION_STALL_TIMEOUT",
            "SANREMES_GATEWAY_BUSY_INPUT_MODE",
            "SANREMES_GATEWAY_BUSY_TEXT_MODE",
    ...
    # otherwise intentionally clean so stale SanRemes settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
@pytest.fixture
def sanremes_home(tmp_path: Path) -> Path:
    home = tmp_path / ".sanremes"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(sanremes_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(sanremes_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
        "session_stall_timeout": 300,
    })
    _write_env(sanremes_home, {
        "SANREMES_AGENT_TIMEOUT": "60",
        "SANREMES_AGENT_TIMEOUT_WARNING": "30",
        "SANREMES_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(sanremes_home, initial_env={})

    assert env.get("SANREMES_AGENT_TIMEOUT") == "1800"
    assert env.get("SANREMES_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("SANREMES_SESSION_STALL_TIMEOUT") == "300"


def test_config_turn_lease_timeout_wins_over_stale_env(sanremes_home: Path) -> None:
    ...
    _write_env(
        sanremes_home,
        {"SANREMES_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(sanremes_home, initial_env={})

    assert env.get("SANREMES_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("SANREMES_TURN_LEASE_TIMEOUT") == "1800"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(sanremes_home: Path) -> None:
    ...
    _write_config(sanremes_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(sanremes_home, initial_env={})

    assert env.get("SANREMES_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(sanremes_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    SANREMES_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        sanremes_home,
        initial_env={"SANREMES_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("SANREMES_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
