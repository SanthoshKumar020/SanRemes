...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "sanremes-gateway"
    ...
    def test_get_running_pid_cached_invalidates_when_pid_file_changes(self, tmp_path, monkeypatch):
        ...
        def _write_record(pid: int, start_time: int) -> None:
            record = {
                ...
                "argv": ["python", "-m", "sanremes_cli.main", "gateway"],
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level SANREMES_HOME.
        ...
        the process-level SANREMES_HOME, not the profile's directory.  See #56986.
        """
        ...
        monkeypatch.setenv("SANREMES_HOME", str(process_home))
...
class TestGatewayRuntimeStatus:
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``sanremes gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "sanremes gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "sanremes -p coder gateway run --replace",
            "/opt/sanremes/.venv/bin/sanremes --profile coder gateway run --replace",
            "sanremes_home=/opt/data/profiles/coder sanremes gateway run --replace",
        ):
    ...
    def test_command_line_belongs_to_profile_normalizes_separators(self):
        """A Windows argv renders SANREMES_HOME with backslashes while the
        ...
        cmdline = r"sanremes_home=c:\opt\data\profiles\coder sanremes gateway run --replace"
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_rejects_live_other_process(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SANREMES_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "sanremes-gateway",
            "argv": ["/Users/user/.sanremes/sanremes-agent/sanremes_cli/main.py", "gateway", "run", "--replace"],
        }))
    ...
    def test_acquire_scoped_lock_self_reacquires_when_disk_start_time_null(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "sanremes-gateway",
            "argv": ["sanremes_cli/main.py", "--profile", "milena", "gateway", "run", "--replace"],
    ...
    def test_acquire_scoped_lock_self_reacquires_when_start_times_differ(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": 111,
            "kind": "sanremes-gateway",
            "argv": ["sanremes_cli/main.py", "gateway", "run", "--replace"],
            "scope": "discord-bot-token",
        }))
    ...
    def test_acquire_scoped_lock_stamps_profile_label(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "profiles" / "lead-gen-outreach"))
    ...
    def test_acquire_scoped_lock_omits_profile_when_not_inferable(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "some-custom-dir"))
...
class TestScopedLockOwnerLabel:
    ...
    def test_profile_label_for_root_home_is_default(self, monkeypatch):
        monkeypatch.setenv("SANREMES_HOME", "/opt/data")
    ...
    def test_owner_label_prefers_explicit_profile_field(self):
        record = {"profile": "coder", "sanremes_home": "/opt/data/profiles/other"}
    ...
    def test_owner_label_ignores_invalid_profile_and_uses_home(self):
        ...
        # sanremes_home fallback — attribution degrades, never corrupts.
        record = {
            "profile": "bad; rm -rf /",
            "sanremes_home": "/opt/data/profiles/zerocool",
        }
        assert status.scoped_lock_owner_label(record) == "zerocool"

    def test_owner_label_falls_back_to_sanremes_home(self):
        # Locks written before the profile field existed still attribute.
        record = {"pid": 559, "sanremes_home": "/opt/data/profiles/zerocool"}
        assert status.scoped_lock_owner_label(record) == "zerocool"
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_sanremes_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's SANREMES_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
        ...
        payload = json.loads((tmp_path / ".gateway-takeover.json").read_text())
        assert payload["replacer_sanremes_home"] == str(tmp_path)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        SANREMES_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.sanremes flap
        ...
        marker_path.write_text(json.dumps({
            ...
            "replacer_pid": 99999,
            "replacer_sanremes_home": str(tmp_path / "profiles" / "other"),
    ...
    def test_consume_accepts_legacy_marker_without_sanremes_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older SanRemes versions have no
        ``replacer_sanremes_home`` field; an absent field is treated as same-home so
...
class TestScopedLockTakeover:
    ...
    def _owner_record(target_home: Path, *, pid: int = 4242, start_time: int = 123):
        ...
        record = {
            "pid": pid,
            "kind": "sanremes-gateway",
            "argv": ["python", "-m", "sanremes_cli.main", "gateway", "run"],
            "start_time": start_time,
            "sanremes_home": str(target_home),
        }
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        replacer_home.mkdir()
        monkeypatch.setenv("SANREMES_HOME", str(replacer_home))
        ...
        monkeypatch.setattr(
            ...
            lambda _pid: "python -m sanremes_cli.main gateway run",
        )
        calls = []
        ...
        def terminate(pid, *, force=False):
            ...
            assert payload["target_sanremes_home"] == str(target_home)
            assert payload["replacer_sanremes_home"] == str(replacer_home)
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``sanremes gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00sanremes_cli/main.py\x00gateway\x00"

        monkeypatch.setattr(status.Path, "read_bytes", fake_read_bytes)
        result = status._read_process_cmdline(12345)
        assert "sanremes_cli/main.py" in result
        assert calls == ["proc"]
...
class TestLaunchdPlistRespawnGovernance:
    def test_plist_has_throttle_interval(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
        from sanremes_cli.gateway import generate_launchd_plist
...
class TestResolveGatewayLiveness:
    ...
    def test_profile_dir_scopes_every_read_to_that_profile(self, tmp_path):
        ...
        ignore the SANREMES_HOME contextvar override (#56986), so the profile
...
def test_strict_gateway_identity_rejects_reused_pid(tmp_path, monkeypatch):
    pid_path = tmp_path / "gateway.pid"
    lock_path = tmp_path / "gateway.lock"
    record = {"pid": 123, "start_time": 10.0, "kind": "sanremes-gateway"}
