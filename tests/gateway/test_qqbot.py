...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.sanremes/.update_response."""
        adapter = self._make_adapter()
        sanremes_home = tmp_path / "sanremes_home"
        sanremes_home.mkdir()
        monkeypatch.setattr(
            "sanremes_constants.get_sanremes_home",
            lambda: sanremes_home,
        )
        ...
        response = sanremes_home / ".update_response"
