...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.sanremes."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /sanremes, /btw, /stop,
        ...
        for expected in ("/sanremes", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``sanremes gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "sanremes_feedback" in created_apps[0].registered_actions
        assert "sanremes_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("sanremes_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("sanremes_clarify_choice")
            for pattern in clarify_choice_patterns
        )
    ...
    async def test_channel_delivery_honors_unfurl_config(self):
        ...
        with patch.object(_slack_mod.aiohttp, "ClientSession", return_value=session):
            result = await _slack_mod._standalone_send(
                config,
                "C123",
                "[SanRemes](https://example.com/sanremes)",
            )

        assert result["success"] is True
        payload = session.post.call_args.kwargs["json"]
        assert payload["text"] == "<https://example.com/sanremes|SanRemes>"
...
class TestSlashCommands:
    ...
    # instead of as /sanremes subcommands. This is the Discord/Telegram parity
    ...
    @pytest.mark.asyncio
    async def test_legacy_sanremes_prefix_still_works(self, adapter):
        """Backward compat: /sanremes btw foo must still route to /btw foo.

        Old workspace manifests only declared /sanremes as the single slash.
...
class TestMessageSplitting:
    ...
    async def test_send_coerces_string_unfurl_options(self, adapter):
        """`sanremes config set` / Railway persist YAML booleans as strings.
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "sanremes gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestTrackingStructureBounds:
    ...
    async def test_slash_command_contexts_bounded(self, adapter):
        ...
        adapter.handle_sanremes_command = AsyncMock(return_value=None)
        for i in range(10):
            command = {
                "command": "/sanremes",
                "text": "/status",
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/sanremes-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/sanremes-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_SANREMES_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_sanremes_slack_user_agent_prefix_format(self):
        """Module constant matches the SanRemesAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._SANREMES_SLACK_USER_AGENT_PREFIX.startswith("SanRemesAgent/")
...
class TestSlackAuthoredTextDeduplication:
    ...
    @pytest.mark.parametrize(
        "flat",
        [
            "hey <@U_BOT|sanremes> please look",
