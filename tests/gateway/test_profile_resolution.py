...
class TestResolutionOrder:
    ...
    def test_source_profile_wins_over_routing(self, mock_runner, discord_source):
        ...
        with patch("sanremes_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("sanremes_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("sanremes_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/sanremes/profiles/from-source")
                    result = mock_runner._resolve_profile_home_for_source(discord_source)
                    
                    assert result == Path("/sanremes/profiles/from-source")
...
class TestMissingProfileWarning:
    ...
    def test_nonexistent_profile_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("sanremes_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("sanremes_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/sanremes/profiles/nonexistent")
                with patch("sanremes_cli.profiles.profile_exists", return_value=False):
                    with patch("sanremes_constants.get_sanremes_home", return_value=Path("/sanremes")):
                        with caplog.at_level(logging.WARNING):
                            result = mock_runner._resolve_profile_home_for_source(discord_source)
                            
                            # Should fall back to global SANREMES_HOME
                            assert result == Path("/sanremes")
...
class TestExceptionHandling:
    ...
    def test_get_profile_dir_exception_logs_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("sanremes_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("sanremes_cli.profiles.get_profile_dir", side_effect=ValueError("Invalid profile name")):
                with patch("sanremes_constants.get_sanremes_home", return_value=Path("/sanremes")):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
...
class TestRoutingConsultation:
    ...
    def test_routing_consulted_when_source_profile_empty(self, mock_runner, discord_source):
        ...
        with patch("sanremes_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("sanremes_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/sanremes/profiles/routed")
...
class TestNonDiscordProfileRouting:
    ...
    def test_telegram_route_resolves(self, mock_runner, telegram_source):
        ...
        with patch(
            "sanremes_cli.profiles.profiles_to_serve",
