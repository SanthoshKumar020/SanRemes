...
from sanremes_cli.dashboard_auth.ws_tickets import _reset_for_tests, mint_ticket
...
def test_dashboard_ticket_identity_is_carried_forward_without_trusting_rpc_params(gated_dashboard):
    ...
    assert ws._sanremes_auth_identity == {
...
def test_dashboard_ticket_subprotocol_carries_the_same_server_identity(gated_dashboard):
    ...
    assert ws._sanremes_ws_subprotocol == web_server._GATEWAY_WS_PROTOCOL
...
def test_cloud_agent_context_binds_registration_principal_and_transport_family():
    ...
    try:
        ...
        assert get_session_env("SANREMES_BROWSER_CONTROL_PRINCIPAL") == _principal_digest(
            identity
        )
        assert (
