...
closure the PR changed, against a real temp ``SANREMES_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "sanremes_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_sanremes_home", sanremes_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_sanremes_home() / config.yaml`` — point it here.
    monkeypatch.setattr("sanremes_constants.get_sanremes_home", lambda: sanremes_home)
    monkeypatch.setattr("sanremes_cli.config.get_sanremes_home", lambda: sanremes_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_sanremes_home", default_home)
    _stub_picker_dependencies(monkeypatch)
