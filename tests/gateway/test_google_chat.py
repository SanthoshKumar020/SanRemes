...
def adapter(tmp_path):
    ...
    ~/.sanremes/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.sanremes/...)
...
class TestOnPubsubMessage:
    ...
    def test_relay_flat_bot_sender_is_filtered_end_to_end(self, adapter):
        ...
        envelope = {
            ...
            "sender_display_name": "SanRemesBot",
            "sender_type": "BOT",
            "text": "reply from bot",
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the SanRemes host. The relay flattens Chat
...
class TestSend:
    ...
    async def test_send_clarify_posts_choice_card(self, adapter):
        ...
        assert buttons[0]["onClick"]["action"]["function"] == "sanremes_clarify"
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'SanRemes is thinking…' stuck in chat. Race fix via
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/sanremes-chat"
            ),
        ...
        monkeypatch.setattr("sanremes_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("sanremes_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("sanremes_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "sanremes_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "sanremes_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "sanremes_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "sanremes_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
