...
Precedence verified here:
    SANREMES_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    """Isolated SANREMES_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``sanremes_cli`` / ``gateway`` so each
    ...
    monkeypatch.delenv("SANREMES_MAX_TOKENS", raising=False)

    _saved = {
        k: v
        for k, v in sys.modules.items()
        if k.startswith(("sanremes_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (sanremes_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("sanremes_cli", "gateway")):
                del sys.modules[mod]
