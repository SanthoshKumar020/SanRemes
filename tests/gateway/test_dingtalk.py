...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "SanRemes"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^sanremes"]}
        )
        assert adapter._message_matches_mention_patterns("sanremes please help") is True
        assert adapter._message_matches_mention_patterns("please sanremes help") is False
