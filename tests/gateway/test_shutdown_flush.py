...
def test_recover_closes_owned_db_when_unexpected_exception_escapes(
    ...
    db = InterruptingDB()
    monkeypatch.setattr("sanremes_state.SessionDB", lambda: db)
...
def test_get_flush_dir_uses_get_sanremes_home(tmp_path, monkeypatch):
    """Flush dir must use get_sanremes_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_sanremes_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "sanremes_constants.get_sanremes_home", fake_get_sanremes_home
    )
