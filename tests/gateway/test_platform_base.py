...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    sanremes_home.mkdir()
    monkeypatch.setattr(base, "_SANREMES_HOME", sanremes_home)
    monkeypatch.setattr(base, "_SANREMES_ROOT", sanremes_home)
    path = sanremes_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestSecretCaptureGuidance:
    def test_gateway_secret_capture_message_points_to_local_setup(self):
        ...
        assert "~/.sanremes/.env" in message
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.sanremes/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("SANREMES_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("SANREMES_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("SANREMES_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("SANREMES_KANBAN_HOME", str(tmp_path / "sanremes"))
        monkeypatch.setenv("SANREMES_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "sanremes" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("SANREMES_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("SANREMES_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("SANREMES_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.sanremes/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        sanremes_dir = fake_home / ".sanremes"
        (sanremes_dir / "mcp-tokens").mkdir(parents=True)
        secret = sanremes_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._SANREMES_HOME",
            sanremes_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._SANREMES_ROOT",
            sanremes_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the SANREMES_HOME root (google_token.json)
        ...
        fake_home = tmp_path / "home"
        sanremes_dir = fake_home / ".sanremes"
        sanremes_dir.mkdir(parents=True)
        token = sanremes_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._SANREMES_HOME", sanremes_dir)
        monkeypatch.setattr("gateway.platforms.base._SANREMES_ROOT", sanremes_dir)

        assert BasePlatformAdapter.validate_media_delivery_path(str(token)) is None


    def test_denylist_blocks_non_cache_file_under_sanremes_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.sanremes
        ...
        sanremes_dir = fake_home / ".sanremes"
        sanremes_dir.mkdir(parents=True)
        artifact = sanremes_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting SANREMES_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose SANREMES_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.sanremes) emits
        ...
        sanremes_root = denied_root / ".sanremes"
        prof_cache = sanremes_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._SANREMES_ROOT", sanremes_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.sanremes/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        sanremes_home = tmp_path / ".sanremes"
        cache = sanremes_home / "cache" / "images"
    ...
    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.sanremes/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "SANREMES_HUMAN_DELAY_MODE": "natural",
            "SANREMES_HUMAN_DELAY_MIN_MS": "oops",
            "SANREMES_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_tolerates_malformed_env_vars(self):
        env = {
            "SANREMES_HUMAN_DELAY_MODE": "custom",
            "SANREMES_HUMAN_DELAY_MIN_MS": "oops",
            "SANREMES_HUMAN_DELAY_MAX_MS": "still-bad",
        }
        with patch.dict(os.environ, env):
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"SANREMES_MEDIA_DELIVERY_STRICT": "1",
                                     "SANREMES_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the SanRemes
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../sanremes/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.sanremes/cache/media/sensitive_host_path_abc123.bin"
...
class TestDockerProfileSandboxMediaTranslation:
    ...
    def test_home_credential_surface_still_refused(self, monkeypatch):
        """The /root/.sanremes exclusion survives profile scoping: translating
        ...
        for task in ("default", f"session:{self.SESSION_KEY}"):
            secrets = self._sandbox_dir(task) / "home" / ".sanremes"
            secrets.mkdir(parents=True, exist_ok=True)
            (secrets / "auth.json").write_text("{}")

        assert (
            BasePlatformAdapter.validate_media_delivery_path(
                "/root/.sanremes/auth.json", session_key=self.SESSION_KEY
            )
            is None
        )
