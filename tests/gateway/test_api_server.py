...
class TestAdapterInit:
    ...
    def test_create_agent_forwards_runtime_config(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.tools_config._get_platform_tools", lambda *_: set())
    ...
    async def test_health_detailed_returns_ok(self, adapter):
        ...
        with patch("gateway.status.read_runtime_status", return_value={
            ...
            async with TestClient(TestServer(app)) as cli:
                ...
                assert data["platform"] == "sanremes-agent"
...
class TestModelsEndpoint:
    @pytest.mark.asyncio
    async def test_models_returns_sanremes_agent(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["data"][0]["id"] == "sanremes-agent"
            assert data["data"][0]["owned_by"] == "sanremes"
    ...
    def test_resolve_model_name_default_profile(self):
        """Default profile falls back to 'sanremes-agent'."""
        with patch("sanremes_cli.profiles.get_active_profile_name", return_value="default"):
            assert APIServerAdapter._resolve_model_name("") == "sanremes-agent"
    ...
    async def test_model_options_returns_shared_inventory(self, adapter, monkeypatch):
        ...
        from sanremes_cli import inventory
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["object"] == "sanremes.api_server.capabilities"
            assert data["platform"] == "sanremes-agent"
            assert data["model"] == "sanremes-agent"
            ...
            assert data["features"]["session_continuity_header"] == "X-SanRemes-Session-Id"
...
class TestToolsetsEndpoint:
    ...
    async def test_toolsets_returns_resolved_tools(self, adapter):
        ...
        with patch(
            "sanremes_cli.tools_config._get_effective_configurable_toolsets",
            return_value=fake_toolsets,
        ), patch(
            "sanremes_cli.tools_config._get_platform_tools",
            return_value={"default"},
        ), patch(
            "sanremes_cli.tools_config.get_nous_subscription_features",
            return_value=feature_snapshot,
        ) as resolve_features, patch(
            "sanremes_cli.tools_config._toolset_has_keys",
            return_value=True,
        ) as has_keys, patch(
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stream_includes_tool_progress(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                ...
                assert "event: sanremes.tool.progress" in body
    ...
    async def test_stream_emits_tool_lifecycle_with_call_id(self, adapter):
        ...
        ``tool.started``-style ``sanremes.tool.progress`` event; clients
        ...
        of the lifecycle pair on the same ``event: sanremes.tool.progress``
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            for i, line in enumerate(lines):
                if line.strip() != "event: sanremes.tool.progress":
                    continue
    ...
    async def test_result_error_fallback_is_redacted(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "sanremes-agent", "input": "Hello"},
                )
    ...
    async def test_stream_task_done_callback_enqueues_eos_for_responses(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with (
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "sanremes-agent", "input": "hi", "stream": True},
                )
                assert resp.status == 200
    ...
    async def test_get_stored_response(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "sanremes-agent", "input": "Hi"},
                )
    ...
    async def test_tool_calls_in_output(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "sanremes-agent", "input": "What is 6*7?"},
                )
    ...
    async def test_hard_failure_redacts_secret_like_error_text(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "sanremes-agent", "messages": [{"role": "user", "content": "hello"}]},
                )
            ...
            body = json.dumps(data)
            assert raw_secret not in body
            assert raw_secret not in resp.headers.get("X-SanRemes-Error", "")
            assert "OPENAI_API_KEY=" in body
            assert data["error"]["sanremes"]["failed"] is True
...
# ---------------------------------------------------------------------------
# X-SanRemes-Session-Id header (session continuity)
# ---------------------------------------------------------------------------
...
class TestSessionIdHeader:
    ...
    async def test_traversal_session_id_header_rejected(self, auth_adapter):
        """Security (#5958): a path-traversal X-SanRemes-Session-Id must be
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                for bad in ("../../../../etc/pwned", "/abs/path", "..\\win"):
                    resp = await cli.post(
                        "/v1/chat/completions",
                        headers={"X-SanRemes-Session-Id": bad, "Authorization": "Bearer sk-secret"},
                        json={"model": "sanremes-agent", "messages": [{"role": "user", "content": "hi"}]},
                    )
    ...
    async def test_provided_session_id_loads_history_from_db(self, auth_adapter):
        """When X-SanRemes-Session-Id is provided, history comes from SessionDB not request body."""
        mock_result = {"final_response": "OK", "messages": [], "api_calls": 1}
        db_history = [
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-SanRemes-Session-Id": "existing-session", "Authorization": "Bearer sk-secret"},
...
# ---------------------------------------------------------------------------
# X-SanRemes-Session-Key header (long-term memory scoping)
# ---------------------------------------------------------------------------
...
class TestSessionKeyHeader:
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_create_agent", side_effect=_fake_create_agent):
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-SanRemes-Session-Key": "agent:main:webui:dm:user-7",
    ...
    async def test_responses_endpoint_accepts_session_key(self, auth_adapter):
        """Responses API honors the same X-SanRemes-Session-Key contract."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    headers={
                        "X-SanRemes-Session-Key": "webui:chan-1",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "sanremes-agent", "input": "hello", "store": False},
                )
            assert resp.status == 200
            assert resp.headers.get("X-SanRemes-Session-Key") == "webui:chan-1"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_capabilities_advertises_session_key_header(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["features"]["session_key_header"] == "X-SanRemes-Session-Key"
...
class TestStoredSessionModelFilter:
    ...
    "no stored model" — replaying "sanremes-agent" upstream 400s. Found live
    (Aug 2026): the first cross-gateway `sanremes peer dm` against a fresh
    api_server failed every turn with "sanremes-agent is not a valid model ID".
    """
...
class TestSessionDbOffEventLoop:
    ...
    async def test_create_session_without_model_does_not_persist_virtual_alias(self, auth_adapter):
        ...
        virtual model alias (self._model_name, e.g. "sanremes-agent") as if it
        ...
        "sanremes-agent" as an invalid model identifier on every turn.
        """
        app = _create_app(auth_adapter)
    ...
    async def test_create_session_with_explicit_virtual_alias_does_not_persist_it(self, auth_adapter):
        """Sending ``model: "sanremes-agent"`` explicitly (the virtual alias
    ...
    async def test_create_session_with_provider_prefixed_virtual_alias_does_not_persist_it(self, auth_adapter):
        ...
        "openrouter::sanremes-agent" never matched self._model_name and leaked
...
class TestApiKeyStartupGuardFailsClosed:
    ...
    def _blocking_auth_import():
        ...
        def _blocked(name, *args, **kwargs):
            if name == "sanremes_cli.auth":
                raise ImportError("simulated: sanremes_cli.auth unavailable")
...
class TestCreateAgentModelRecovery:
    def test_create_agent_defaults_to_provider_catalog_model_when_empty(self, monkeypatch):
        ...
        `sanremes auth add openai-codex` without `sanremes model`) —
        ...
        monkeypatch.setattr(
            "sanremes_cli.models.get_default_model_for_provider",
    ...
    def test_create_agent_does_not_cache_virtual_alias(self, monkeypatch):
        """Write-side guard: the advertised virtual model (``sanremes-agent``)
