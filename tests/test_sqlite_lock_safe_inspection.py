"""POSIX advisory locks must survive SanRemes' own database inspection.
...
SanRemes used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `sanremes sessions
...
out across SanRemes' inspection calls.
"""
...
def clean_registry():
    ...
    import sanremes_cli.sqlite_safe_read as mod
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_sanremes_fail_close", False):
    ...
    conn._sanremes_fail_close = True
    ...
    conn._sanremes_fail_close = False
    conn.close()
    assert not has_live_connection(db)
    assert read_header_bytes_preopen(db, length=16) is not None
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import sanremes_cli.sqlite_safe_read as ssr
