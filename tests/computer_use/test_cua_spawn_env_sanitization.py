...
def _assert_path_preserved(env):
    """Original PATH entries survive sanitization; the sanremes console-script
    dir may be prepended (see _sanitize_subprocess_env, issue #92998) so we
    assert the contract, not byte equality."""
    from tools.environments.local import _resolve_sanremes_bin_dir

    path_val = env.get("PATH", "")
    assert path_val.endswith("/usr/bin:/bin"), path_val
    sanremes_bin = _resolve_sanremes_bin_dir()
    if sanremes_bin and path_val != "/usr/bin:/bin":
        assert path_val.startswith(sanremes_bin + os.pathsep), path_val
...
def test_resolve_mcp_invocation_sanitizes_env(monkeypatch):
    ...
    monkeypatch.delenv("SANREMES_CUA_TELEMETRY", raising=False)
