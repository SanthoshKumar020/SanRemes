"""Guard: SanRemes-owned subprocesses must not resolve managed runtimes by bare PATH.

SanRemes installs runtimes for itself — ``uv`` at ``$SANREMES_HOME/bin/uv``, Node at
``$SANREMES_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in SanRemes's own
...
* when a system copy also exists, the one SanRemes does not own wins — which is
...
``iter_sanremes_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes SanRemes provisions into SANREMES_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not SanRemes-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("sanremes_cli/update_cmd.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    for path in REPO_ROOT.rglob("*.py"):
        ...
        # sanremes_agent-0.20.5/, build/ and *.egg-info dirs). CI jobs that
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a SanRemes-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$SANREMES_HOME/bin (uv) and $SANREMES_HOME/node are not on an "
        ...
        "  node/npm -> sanremes_constants.find_node_executable()\n"
        "  PATH env -> sanremes_constants.iter_sanremes_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(sanremes_constants, helper))


def test_managed_uv_helpers_exist():
    from sanremes_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
