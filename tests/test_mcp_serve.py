"""
Tests for mcp_serve — SanRemes MCP server.
...
def _isolate_sanremes_home(tmp_path, monkeypatch):
    """Redirect SANREMES_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking sanremes_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <SANREMES_HOME>/state.db for its mtime gate; the autouse
        # fixture points SANREMES_HOME at tmp_path.
        db_path = tmp_path / "state.db"
