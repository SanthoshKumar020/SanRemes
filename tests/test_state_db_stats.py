"""Tests for state.db health/stats collection (sanremes doctor section).

Covers:
- ``sanremes_state.collect_state_db_stats``: read-only, best-effort stats
  (page_count, freelist, WAL size, journal mode, row counts, FTS presence,
  pending v23 FTS-rebuild bookkeeping).
- ``sanremes_state.count_db_holders``: /proc-based best-effort probe for how
  many processes hold the DB file open (Linux only; None elsewhere/on error).
- ``sanremes_cli.doctor._render_state_db_stats``: formatting/threshold helper
...
from sanremes_state import SessionDB, collect_state_db_stats, count_db_holders
...
def test_collect_and_render_stale_fts_holder_deferral(populated_db):
    ...
    from sanremes_cli.doctor import _render_state_db_stats
...
def test_render_warns_on_large_db():
    from sanremes_cli.doctor import (
...
def test_render_large_db_with_pending_rebuild_suggests_optimize():
    from sanremes_cli.doctor import STATE_DB_SIZE_WARN_BYTES, _render_state_db_stats
