...
def test_credentials_ready_false_when_no_provider(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.runtime_provider.resolve_runtime_provider", _raise)
...
def test_offer_first_run_setup_routes_into_shared_picker(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.main.select_provider_and_model", _fake_picker)
    monkeypatch.setattr("builtins.input", lambda *a, **k: "y")
    # After the picker "runs", config has a provider and creds resolve.
    monkeypatch.setattr(
        "sanremes_cli.config.load_config",
        lambda: {"model": {"provider": "nous", "default": "sanremes-4-405b"}},
    )
    monkeypatch.setattr(
        "sanremes_cli.runtime_provider.resolve_runtime_provider",
        lambda **kw: {
    ...
    assert shell.model == "sanremes-4-405b"
...
def test_offer_first_run_setup_declined(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.main.select_provider_and_model", _fail_picker)
...
def test_offer_first_run_setup_picker_cancel_is_graceful(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.main.select_provider_and_model", _cancel_picker)
...
def test_empty_key_error_names_actual_provider(monkeypatch, capsys):
    ...
    assert "sanremes model" in out or "sanremes setup" in out
