...
SessionDB.session_gateway_runtime (sanremes_state.py) — the round trip that
makes `sanremes --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare SanRemesCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.SanRemesCLI)
