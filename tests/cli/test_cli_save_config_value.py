...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_sanremes_home(), so
        # point SANREMES_HOME at the temp dir (the _sanremes_home import-time
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "sanremes_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in SANREMES_HOME/config.yaml
    ...
    install whose SANREMES_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read SANREMES_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh SANREMES_HOME with NO config.yaml (managed/desktop first launch).
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
    ...
    def test_does_not_write_repo_cli_config(self, tmp_path, monkeypatch):
        ...
        result = yaml.safe_load((sanremes_home / "config.yaml").read_text())
        assert result["wake_word"]["enabled"] is True
