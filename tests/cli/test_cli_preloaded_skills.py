...
def _make_real_cli(**kwargs):
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), patch.dict(
        ...
        with patch.object(cli_mod, "get_tool_definitions", return_value=[]), patch.dict(
            ...
            return cli_mod.SanRemesCLI(**kwargs)
...
def _real_finalize(cli_obj):
    """Call the real SanRemesCLI.finalize_preloaded_skills on a dummy object."""
    return _REAL_FINALIZE(cli_obj)


def _capture_real_finalize():
    import cli as cli_mod
    return cli_mod.SanRemesCLI.__dict__["finalize_preloaded_skills"]
...
def test_main_applies_preloaded_skills_to_system_prompt(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod, "SanRemesCLI", fake_cli)
    monkeypatch.setattr(
        cli_mod,
        "build_preloaded_skills_prompt",
        lambda skills, task_id=None: ("skill prompt", ["sanremes-agent-dev", "github-auth"], []),
    )

    with pytest.raises(SystemExit):
        cli_mod.main(skills="sanremes-agent-dev,github-auth", list_tools=True)
    ...
    assert cli_obj.preloaded_skills == ["sanremes-agent-dev", "github-auth"]
...
def test_show_banner_does_not_print_skills():
    ...
    cli_obj.preloaded_skills = ["sanremes-agent-dev", "github-auth"]
