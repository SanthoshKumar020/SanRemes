...
def session_db(tmp_path):
    ...
    os.environ["SANREMES_HOME"] = str(tmp_path / ".sanremes")
    os.makedirs(tmp_path / ".sanremes", exist_ok=True)
    from sanremes_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".sanremes" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal SanRemesCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        SanRemesCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        SanRemesCLI._handle_branch_command(cli_instance, "/branch refactor approach")
