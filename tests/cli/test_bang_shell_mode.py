...
from sanremes_cli.bang_shell import (
    USAGE_HINT,
    bang_shell_enabled,
    is_bang_command,
    parse_bang_command,
...
class TestBangContextGating:
    ...
    def test_enabled_in_plain_cli(self, monkeypatch):
        for var in ("SANREMES_GATEWAY_SESSION", "SANREMES_CRON_SESSION",
                    "SANREMES_SESSION_PLATFORM"):
            monkeypatch.delenv(var, raising=False)
        assert bang_shell_enabled() is True

    @pytest.mark.parametrize("var,value", [
        ("SANREMES_GATEWAY_SESSION", "1"),
        ("SANREMES_CRON_SESSION", "true"),
        ("SANREMES_SESSION_PLATFORM", "discord"),
    ])
    def test_disabled_in_non_cli_contexts(self, monkeypatch, var, value):
        for v in ("SANREMES_GATEWAY_SESSION", "SANREMES_CRON_SESSION",
                  "SANREMES_SESSION_PLATFORM"):
...
def _make_cli(history=None):
    """Build a SanRemesCLI shell with only what handle_bang_shell touches."""
...
class TestBangHandlerDispatch:
    ...
    def test_bare_bang_prints_usage_and_runs_nothing(self):
        cli = _make_cli()
        with patch("sanremes_cli.bang_shell.run_bang_command") as runner:
...
class TestBangApprovalGate:
    ...
    def test_approval_gate_is_invoked_for_a_dangerous_command(self):
        ...
        with patch("tools.terminal_tool._check_all_guards", gate), \
             patch("sanremes_cli.bang_shell.run_bang_command", return_value=0):
    ...
    def test_denied_command_is_not_executed(self):
        ...
        with patch("tools.terminal_tool._check_all_guards", gate), \
             patch("sanremes_cli.bang_shell.run_bang_command") as runner:
