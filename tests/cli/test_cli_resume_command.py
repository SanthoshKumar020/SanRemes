...
class TestCliResumeCommand:
    ...
    def test_handle_resume_by_index_switches_to_numbered_session(self):
        ...
        with (
            patch("sanremes_cli.main._resolve_session_by_name_or_id", return_value=None),
            patch("cli._cprint") as mock_cprint,
        ):
...
class TestCliResumeRestoresCwd:
    ...
    session was started — the same contract as a startup ``sanremes -c`` /
    ``--resume``.
    ...
    def test_handle_resume_restores_recorded_cwd(self, tmp_path):
        ...
        with (
            patch("sanremes_cli.main._resolve_session_by_name_or_id", return_value="sess_dir"),
            patch("cli._cprint"),
...
class TestResumeFlushesBeforeEndSession:
    ...
    def test_resume_flushes_when_agent_present(self):
        ...
        with (
            patch("sanremes_cli.main._resolve_session_by_name_or_id", return_value="target"),
            patch("cli._cprint"),
        ):
