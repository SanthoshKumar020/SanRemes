...
@pytest.fixture
def bare_cli():
    """A SanRemesCLI with no __init__ — we only exercise the redraw helper."""
    cli = object.__new__(SanRemesCLI)
    return cli
...
class TestForceFullRedraw:
    ...
    def test_same_width_sigwinch_is_left_untouched(self, bare_cli, monkeypatch):
        ...
        stale-paint crash is handled by _sanremes_call_output_screen_diff's
        retry instead (#83874)."""
        app = MagicMock()
        events = []
        app.renderer.output.erase_screen.side_effect = lambda: events.append("erase")
    ...
    def test_output_screen_diff_retries_on_corrupt_previous_screen(self, bare_cli):
        ...
        result = cli_mod._sanremes_call_output_screen_diff(
            fake_osd,
            app=None,
            output=None,
            screen=screen,
