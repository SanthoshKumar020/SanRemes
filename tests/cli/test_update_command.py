...
Verifies that ``SanRemesCLI._handle_update_command`` correctly:
...
Also verifies that ``sanremes_cli.main._launch_tui`` correctly handles exit
...
def _make_self(modal_response):
    ...
    so we don't need a full ``SanRemesCLI`` construction.
...
def _call(self_):
    ...
    return SanRemesCLI._handle_update_command(self_)
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        patch("sanremes_cli.config.is_managed", return_value=True),
        patch(
            "sanremes_cli.config.format_managed_message",
...
def test_affirmative_answer_sets_pending_relaunch_and_returns_true(answer, capsys):
    ...
    with patch("sanremes_cli.config.is_managed", return_value=False):
        result = _call(self_)
