...
def test_empty_query_returns_all_with_original_indices():
    pairs = SanRemesCLI._filter_model_picker_entries(MODELS, "")
    assert pairs == list(enumerate(MODELS))


def test_filter_narrows_and_preserves_original_index():
    pairs = SanRemesCLI._filter_model_picker_entries(MODELS, "grok")
...
def test_subsequence_match_case_insensitive():
    ...
    pairs = SanRemesCLI._filter_model_picker_entries(MODELS, "CS46")
    assert ("anthropic/claude-sonnet-4.6") in [e for _i, e in pairs]


def test_no_match_returns_empty():
    assert SanRemesCLI._filter_model_picker_entries(MODELS, "zzzznope") == []
...
def test_filter_does_not_reorder_or_pick_a_default():
    ...
    pairs = SanRemesCLI._filter_model_picker_entries(MODELS, "claude")
...
def test_whitespace_query_is_treated_as_empty():
    assert SanRemesCLI._filter_model_picker_entries(MODELS, "   ") == list(enumerate(MODELS))
