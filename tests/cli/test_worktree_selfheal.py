...
Regression for the Aug 2026 `sanremes -w` timeout incident: 39 accumulated packs
...
(lock pid = the live sanremes process), poisoning every retry.
...
class TestCleanupFailedWorktreeAdd:
    def _simulate_timed_out_add(self, repo):
        ...
        wt = repo / ".worktrees" / "sanremes-dead00"
        _git(repo, "worktree", "add", str(wt), "-b", "sanremes/sanremes-dead00")
        # Live-pid lock, exactly what `sanremes -w` writes before the checkout.
        _git(repo, "worktree", "lock", str(wt), "--reason", "sanremes pid=999999")
    ...
    def test_sweeps_dir_admin_entry_and_branch(self, repo):
        ...
        admin = repo / ".git" / "worktrees" / "sanremes-dead00"
        assert admin.exists() and (admin / "locked").exists()

        _cleanup_failed_worktree_add(str(repo), wt, "sanremes/sanremes-dead00")

        assert not wt.exists(), "partial worktree dir must be removed"
        assert not admin.exists(), "LOCKED admin entry must be removed"
        branches = _git(repo, "branch", "--list", "sanremes/sanremes-dead00").stdout
    ...
    def test_retry_succeeds_after_cleanup(self, repo):
        ...
        result = _git(
            repo, "worktree", "add", str(wt), "-b", "sanremes/sanremes-dead00", check=False
        )
    ...
    def test_noop_when_nothing_exists(self, repo):
        ...
        _cleanup_failed_worktree_add(
            str(repo), repo / ".worktrees" / "never-existed", "sanremes/never-existed"
        )  # must not raise
