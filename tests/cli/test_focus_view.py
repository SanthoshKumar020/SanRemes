...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = SanRemesCLI.__new__(SanRemesCLI)
        host.model = "opus"
        ...
        with patch.object(SanRemesCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(SanRemesCLI, "_is_session_yolo_active", return_value=False):
            text = SanRemesCLI._build_status_bar_text(host, width=width)
...
class TestCommandRegistration:
    ...
    def test_verbose_cycle_releases_focus_view(self):
        ...
        with patch("cli.save_config_value", return_value=True), patch("cli._cprint"):
            SanRemesCLI._toggle_verbose(host)
