...
def _make_cli(**kwargs):
    """Create a SanRemesCLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports sanremes_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``SanRemesCLI.run()``'s local ``KeyBindings`` instance, so they are not
