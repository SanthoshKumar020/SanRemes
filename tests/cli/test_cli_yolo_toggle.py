...
Pre-fix bug (issue #33925): ``cli.SanRemesCLI._toggle_yolo`` mutated only
``os.environ["SANREMES_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``sanremes --yolo`` (process-start env), ``SANREMES_YOLO_MODE=1``,
and ``sanremes config set approvals.mode off`` actually bypassed.
...
read (``session_id``). This avoids the heavy ``SanRemesCLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout — ``SanRemesCLI.__init__`` imports a lot of optional
...
def _clear_approval_state(monkeypatch):
    ...
    # SanRemes-driven test runs may inherit SANREMES_YOLO_MODE=1 from the parent
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    to invoking them on a fully-constructed ``SanRemesCLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``SanRemesCLI`` from this test file.
    """
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_enables_session_bypass(self):
        ...
        with patch("cli._cprint"):
            SanRemesCLI._toggle_yolo(stand_in)
    ...
    def test_toggle_yolo_disables_session_bypass_on_second_call(self):
        stand_in = _make_stand_in()
        with patch("cli._cprint"):
            SanRemesCLI._toggle_yolo(stand_in)  # ON
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is True
            SanRemesCLI._toggle_yolo(stand_in)  # OFF
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is False
    ...
    def test_two_independent_sessions_are_isolated(self):
        ...
        try:
            with patch("cli._cprint"):
                SanRemesCLI._toggle_yolo(cli_a)
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert SanRemesCLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            SanRemesCLI._toggle_yolo(stand_in)

        assert SanRemesCLI._is_session_yolo_active(stand_in) is True
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``sanremes --yolo`` sets ``SANREMES_YOLO_MODE`` before tool imports, so
...
class TestToggleYoloEndToEnd:
    ...
    def test_toggle_yolo_bypasses_dangerous_command_check(self):
        ...
        try:
            with patch("cli._cprint"):
                SanRemesCLI._toggle_yolo(stand_in)  # YOLO ON
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            SanRemesCLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        SanRemesCLI._transfer_session_yolo(stand_in, "", "new")
        SanRemesCLI._transfer_session_yolo(stand_in, "old", "")
