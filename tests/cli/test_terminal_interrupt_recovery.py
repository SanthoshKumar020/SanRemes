...
The recovery path lives in ``SanRemesCLI._recover_terminal_after_interrupt()``,
...
@pytest.fixture
def bare_cli():
    """A SanRemesCLI with no __init__ — we only exercise the recovery helper."""
    return object.__new__(SanRemesCLI)


class TestRecoverTerminalAfterInterrupt:
    """Directly exercise SanRemesCLI._recover_terminal_after_interrupt()."""
    ...
    def test_drains_stdin_then_redraws(self, bare_cli):
        ...
        with patch("sanremes_cli.curses_ui.flush_stdin") as mock_flush:
    ...
    def test_redraw_still_runs_when_flush_fails(self, bare_cli):
        ...
        bare_cli._force_full_redraw = MagicMock()
        with patch(
            "sanremes_cli.curses_ui.flush_stdin", side_effect=OSError("no tty")
        ):
    ...
    def test_flush_runs_before_redraw(self, bare_cli):
        ...
        with patch(
            "sanremes_cli.curses_ui.flush_stdin",
    ...
    def test_flush_stdin_is_tty_gated(self):
        ...
        touching termios.
        """
        from sanremes_cli.curses_ui import flush_stdin
...
class TestFinallyBlockWiring:
    ...
    def test_recovery_is_invoked_behind_interrupt_guard(self):
        src = inspect.getsource(SanRemesCLI.run)
    ...
    def test_recovery_helper_exists(self):
        assert hasattr(SanRemesCLI, "_recover_terminal_after_interrupt")
        assert callable(SanRemesCLI._recover_terminal_after_interrupt)
