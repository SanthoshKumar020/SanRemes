...
def _setup_worktree(repo_root):
    ...
    wt_name = f"sanremes-{short_id}"
    branch_name = f"sanremes/{wt_name}"
...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("sanremes/sanremes-")
...
class TestStaleWorktreePruning:
    ...
    def test_prunes_old_clean_worktree(self, git_repo):
        ...
        for entry in worktrees_dir.iterdir():
            if not entry.is_dir() or not entry.name.startswith("sanremes-"):
                continue
            try:
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned sanremes/* and pr-* branches."""

    def test_prunes_orphaned_sanremes_branch(self, git_repo):
        """sanremes/sanremes-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "sanremes/sanremes-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "sanremes/sanremes-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "sanremes/sanremes-deadbeef" in result.stdout
        ...
        orphaned = [
            ...
            and (b.startswith("sanremes/sanremes-") or b.startswith("pr-"))
        ]
        assert "sanremes/sanremes-deadbeef" in orphaned
        ...
        assert "sanremes/sanremes-deadbeef" not in result.stdout
...
class TestWorktreeLockReaping:
    ...
    def _mk(cli, repo, name, pid=None, dirty=False, unpushed=False, age_h=100):
        ...
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", f"sanremes/{name}", "HEAD"],
            cwd=repo, capture_output=True,
        )
        if pid is not None:
            subprocess.run(
                ["git", "worktree", "lock", "--reason", f"sanremes pid={pid}", str(p)],
                cwd=repo, capture_output=True,
            )
        if unpushed:
    ...
    def test_live_locked_survives_at_any_age(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "sanremes-live", pid=os.getpid())
    ...
    def test_dead_locked_clean_is_reaped(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "sanremes-dead", pid=999999)
    ...
    def test_dead_locked_dirty_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "sanremes-deaddirty", pid=999999, dirty=True)
        cli._prune_stale_worktrees(str(git_repo))
    ...
    def test_dead_locked_unpushed_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "sanremes-deadunp", pid=999999, unpushed=True)
        cli._prune_stale_worktrees(str(git_repo))
    ...
    def test_unlocked_clean_stale_is_reaped(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "sanremes-nolock", pid=None)
        cli._prune_stale_worktrees(str(git_repo))
        assert not wt.exists(), "clean unlocked stale worktree should be reaped"

    def test_dirty_survives_over_72h(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "sanremes-dirty72", pid=None, dirty=True, age_h=100)
        cli._prune_stale_worktrees(str(git_repo))
...
class TestWorktreeLockPredicate:
    ...
    def test_unlocked_returns_none(self, git_repo):
        import cli
        p = git_repo / ".worktrees" / "sanremes-x"
        (git_repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", "sanremes/sanremes-x", "HEAD"],
    ...
    def test_foreign_lock_reason_returns_dead(self, git_repo):
        import cli
        p = self._mk_locked(git_repo, "sanremes-foreign", "some other tool")
...
class TestWidenedPruner:
    ...
    Previously only ``sanremes-*`` directories were considered, so salvage/
    ...
    # -- named (non sanremes-*) directories are now covered ------------------
    ...
    def test_squash_merged_tree_is_reaped(self, git_repo):
        import cli
        wt, sha = self._mk(git_repo, "sanremes-merged", commit=True, age_h=100)
    ...
    def test_merged_predicate_fails_safe_without_upstream(self, git_repo_no_remote):
        ...
        p = repo / ".worktrees" / "sanremes-noremote"
        (repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
...
class TestMergeVerdictCache:
    ...
    dominates ``sanremes -w`` startup (~0.2-1.0s per worktree, re-run on every
    ...
    def test_cache_hit_matches_uncached_verdict(self, git_repo):
        ...
        import cli
        wt, sha = self._mk(git_repo, "sanremes-cachehit", commit=True)
        self._merge_upstream(git_repo, sha)
    ...
    def test_new_commit_invalidates_cached_verdict(self, git_repo):
        ...
        """
        import cli
        wt, sha = self._mk(git_repo, "sanremes-moves", commit=True)
        self._merge_upstream(git_repo, sha)
...
class TestPruneParallelEquivalence:
    ...
    def _kinds(survivors):
        ...
        for n in survivors:
            for kind in ("merged", "unpushed", "dirty", "fresh"):
                if n.startswith(f"sanremes-{kind}"):
                    out.add(kind)
        return out
    ...
    def test_pool_failure_falls_back_to_serial(self, git_repo, monkeypatch):
        ...
        wt, sha = self._mk(git_repo, "sanremes-poolfail", commit=True)
        self._merge_upstream(git_repo, sha)
...
class TestShallowCloneDeepening:
    ...
    (real incident: 21 of 25 sanremes-* trees stuck on a default install).
    ...
    def _stuck_worktree(self, tmp_path):
        ...
        wt = clone / ".worktrees" / "sanremes-shallowstuck"
        (clone / ".worktrees").mkdir()
        self._run(
            ["git", "worktree", "add", str(wt), "-b", "sanremes/sanremes-shallowstuck", "HEAD"],
            clone,
        )
...
class TestPrMergedEscapeHatch:
    ...
    def test_merged_pr_tree_is_reaped(self, git_repo, tmp_path, monkeypatch):
        import cli
        wt = self._mk_diverged(git_repo, "sanremes-rebase-merged")
    ...
    def test_no_merged_pr_preserved(self, git_repo, tmp_path, monkeypatch):
        import cli
        wt = self._mk_diverged(git_repo, "sanremes-pr-open")
    ...
    def test_gh_failure_fails_safe(self, git_repo, tmp_path, monkeypatch):
        import cli
        wt = self._mk_diverged(git_repo, "sanremes-gh-down")
    ...
    def test_dirty_tree_never_reaped_even_with_merged_pr(
        ...
        import cli
        wt = self._mk_diverged(git_repo, "sanremes-dirty-merged")
    ...
    def test_merged_verdict_memoized_by_branch_and_head(
        ...
        import cli
        wt = self._mk_diverged(git_repo, "sanremes-memo")
        self._stub_gh(tmp_path, monkeypatch)
        cache: dict = {}
    ...
    def test_negative_verdict_not_cached(self, git_repo, tmp_path, monkeypatch):
        import cli
        wt = self._mk_diverged(git_repo, "sanremes-nocache-neg")
        self._stub_gh(tmp_path, monkeypatch, stdout="[]")
        cache: dict = {}
