...
from sanremes_cli import main as sanremes_main
...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.config.load_config", lambda: config)
    monkeypatch.setattr("sanremes_cli.runtime_provider.load_config", lambda: config)

    shell = cli.SanRemesCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "sanremes_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_sanremes_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in SanRemesCLI.__init__")

    monkeypatch.setattr("sanremes_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("sanremes_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.SanRemesCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("sanremes_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    config_home = tmp_path / "sanremes"
    config_home.mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "sanremes_cli.auth.get_provider_auth_state",
        lambda provider: {
    ...
    monkeypatch.setattr(
        "sanremes_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "sanremes_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("sanremes_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("sanremes_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "sanremes_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "sanremes_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "sanremes_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    sanremes_main._model_flow_nous(stale_config, current_model="glm-5.2")
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex"],
    )

    shell = cli.SanRemesCLI(compact=True, max_turns=1)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "sanremes_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("sanremes_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("sanremes_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("sanremes_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("sanremes_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("sanremes_cli.config.save_config", lambda cfg: None)
    ...
    monkeypatch.setattr("sanremes_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    sanremes_main._model_flow_custom({})
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("sanremes_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "sanremes_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("sanremes_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("sanremes_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "sanremes_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "sanremes_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("sanremes_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    sanremes_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(sanremes_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "sanremes_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "nous"}},
    )
    monkeypatch.setattr("sanremes_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("sanremes_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("sanremes_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("sanremes_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("sanremes_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(sanremes_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("sanremes_cli.auth._login_nous", _fake_login)

    sanremes_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.nousresearch.com",
            inference_url="https://inference.nousresearch.com/v1",
            client_id="sanremes-local",
...
def test_auto_provider_name_localhost():
    from sanremes_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from sanremes_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "sanremes_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("sanremes_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("sanremes_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="SANREMES_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "SANREMES_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from sanremes_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
