"""Tests for cli.SanRemesCLI._confirm_destructive_slash.

Drives the helper directly via __get__ on a SimpleNamespace stand-in so we
don't have to construct a full SanRemesCLI (which requires extensive setup).
"""
...
def _make_self(prompt_response):
    ...
    self_._normalize_slash_confirm_choice = _bound(
        SanRemesCLI._normalize_slash_confirm_choice, self_,
    )
    return self_
...
def test_gate_on_choice_once_returns_once():
    ...
    with patch(
        ...
        result = _bound(SanRemesCLI._confirm_destructive_slash, self_)(
            "clear", "detail",
        )
...
def test_split_destructive_skip_recognized_tokens():
    ...
    assert SanRemesCLI._split_destructive_skip("/reset now") == ("", True)
    assert SanRemesCLI._split_destructive_skip("/clear --yes") == ("", True)
    assert SanRemesCLI._split_destructive_skip("/undo -y") == ("", True)
...
def test_split_destructive_skip_handles_empty_and_none():
    ...
    assert SanRemesCLI._split_destructive_skip(None) == ("", False)
    assert SanRemesCLI._split_destructive_skip("") == ("", False)
    assert SanRemesCLI._split_destructive_skip("   ") == ("", False)
...
def test_confirm_destructive_slash_now_skips_modal():
    ...
    self_._split_destructive_skip = SanRemesCLI._split_destructive_skip  # classmethod
...
def test_confirm_destructive_slash_yes_flag_skips_modal():
    ...
    self_._split_destructive_skip = SanRemesCLI._split_destructive_skip
