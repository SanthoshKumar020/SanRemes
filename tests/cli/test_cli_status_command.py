...
def test_process_command_egress_prints_proxy_status(monkeypatch):
    cli_obj = _make_cli()
    monkeypatch.setattr(
        "sanremes_cli.proxy_cli.format_status_text",
...
def test_show_session_status_prints_gateway_style_summary():
    ...
    with patch("cli.display_sanremes_home", return_value="~/.sanremes"):
        cli_obj._show_session_status()

    printed = "\n".join(str(call.args[0]) for call in cli_obj.console.print.call_args_list)
    assert "SanRemes CLI Status" in printed
    assert "Session ID: session-123" in printed
    assert "Path: ~/.sanremes" in printed
...
def test_profile_command_reports_custom_root_profile(monkeypatch, tmp_path, capsys):
    """Profile detection works for custom-root deployments (not under ~/.sanremes)."""
    cli_obj = _make_cli()
