...
($INSTALL_DIR), not into the shared $SANREMES_HOME.

Background (shared-$SANREMES_HOME bug)
------------------------------------
$SANREMES_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.sanremes:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$SANREMES_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``sanremes update``).
...
``~/.sanremes/sanremes-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$SANREMES_HOME/\.install_method"', text), (
        "install.sh must not stamp $SANREMES_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `sanremes update`"
    )
