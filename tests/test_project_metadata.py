...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"sanremes-agent[{extra}]" in spec
        ]
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `sanremes update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `sanremes update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (sanremes_cli/dingtalk_auth.py)
...
def test_every_lazy_deps_exact_pin_matches_uv_lock():
    ...
    install ships the locked version, and the ``sanremes update`` lazy-refresh
    ...
    assert not drift, (
        ...
        "every package the core lock also ships — otherwise `sanremes update` "
...
def test_huggingface_hub_lazy_pin_matches_uv_lock():
    ...
    a feature from mere package presence, the `sanremes update` lazy-refresh
    ...
    assert lazy_pin == locked, (
        ...
        "huggingface-hub`), or `sanremes update` will churn/downgrade the "
