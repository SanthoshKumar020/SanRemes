...
Incident (2026-08-17): a malformed-SCHEMA state.db sent SanRemes into an
...
def test_repair_aborts_when_backup_refused_for_disk(tmp_path):
    ...
    with patch("shutil.disk_usage", return_value=tight):
        report = sanremes_state.repair_state_db_schema(db)
...
def test_live_connection_keeps_its_write_lock_across_a_repair_pass(tmp_path):
    ...
    POSIX advisory locks, so it is immune. DELETE mode is what SanRemes falls
...
def test_forensic_backup_includes_the_rollback_journal(tmp_path):
    ...
    Rollback-journal mode is SanRemes's fallback on NFS/SMB/FUSE/ZFS and on
