"""Tests for sanremes_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every SanRemes entry point
(sanremes, sanremes-agent, sanremes-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every SanRemes entry point imports sanremes_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported sanremes_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("sanremes_bootstrap", None)
    import sanremes_bootstrap  # noqa: WPS433
    return sanremes_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the SanRemes CLI
...
class TestEntryPointsImportBootstrap:
    """Every SanRemes entry point must import sanremes_bootstrap as its
    ...
    # Entry points that invoke SanRemes as a process.  Each one must
    # import sanremes_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "sanremes_cli/main.py",   # sanremes CLI (console_script)
        "run_agent.py",          # sanremes-agent (console_script)
        "acp_adapter/entry.py",  # sanremes-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import sanremes_bootstrap' and that
        ...
        half-finished ``sanremes update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``sanremes_bootstrap`` as a top-level module) leaves sanremes
        ...
        """
        # Resolve relative to the sanremes-agent repo root.  Tests live
        # at tests/test_sanremes_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # sanremes start even when sanremes_bootstrap hasn't been
        ...
        assert first_import_name == "sanremes_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'sanremes_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import sanremes_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing SanRemes's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SANREMES_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SANREMES_PYTHON_SRC_ROOT"] = env
            elif "SANREMES_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SANREMES_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/sanremes")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SANREMES_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SANREMES_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/sanremes", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # SanRemes root.  The guard must relocate SanRemes to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/sanremes"])
        assert result[0] == "/opt/sanremes"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the SanRemes root.
        assert result.index("/opt/sanremes") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SANREMES_PYTHON_SRC_ROOT"] = "/env/sanremes"
            hb.harden_import_path()
            assert sys.path[0] == "/env/sanremes"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SANREMES_PYTHON_SRC_ROOT", None)
