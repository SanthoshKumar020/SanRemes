...
def test_root_session_no_compression(db):
    ...
    assert prov["currentSanRemesSessionId"] == "root1"
    assert prov["rootSanRemesSessionId"] == "root1"
    assert prov["parentSanRemesSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_sanremes_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentSanRemesSessionId"] == "old"
    assert prov["rootSanRemesSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousSanRemesSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootSanRemesSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"sanremes"}
    assert "sessionProvenance" in meta["sanremes"]
    assert meta["sanremes"]["sessionProvenance"]["currentSanRemesSessionId"] == "root1"
