...
Keeps the ACP server tests offline: ``SanRemesACPAgent._build_model_state``
calls ``sanremes_cli.inventory.build_models_payload``, which (without this
...
def _offline_model_inventory(monkeypatch):
    ...
    import sanremes_cli.inventory as inventory
