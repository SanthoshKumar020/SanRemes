"""Tests for acp_adapter.server — SanRemesACPAgent ACP server."""
...
def agent(mock_manager):
    """SanRemesACPAgent backed by a mock session manager."""
    return SanRemesACPAgent(session_manager=mock_manager)
...
class TestSessionOps:
    ...
    async def test_new_session_returns_authenticated_cross_provider_model_state(self):
        ...
        acp_agent = SanRemesACPAgent(session_manager=manager)
        ...
        with (
            patch("sanremes_cli.inventory.load_picker_context", return_value=picker_context),
            patch("sanremes_cli.inventory.build_models_payload", return_value=payload) as build_payload,
        ):
...
class TestPrompt:
    ...
    async def test_prompt_binds_session_id_into_subprocess_env(self, agent, mock_manager):
        ...
        leaving the ``SANREMES_SESSION_ID`` ContextVar bound to the explicit ""
        ...
        empty ``SANREMES_SESSION_ID`` instead of the session's own id.
        """
        ...
        def _run(*args, **kwargs):
            ...
            captured["child"] = _make_run_env({}).get("SANREMES_SESSION_ID")
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["sanremes-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["sanremes-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["sanremes-acp", "mcp-srv"]
