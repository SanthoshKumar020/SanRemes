...
``Invoke-SanRemesStep``, which starts the step with ``RedirectStandardOutput`` /
...
last of them to close it. ``sanremes update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``sanremes_cli/main.py``), so the
...
closes and ``Invoke-SanRemesStep`` blocks for the life of the gateway.

Everything the hand-off owes the Desktop is downstream of that call:
``.sanremes-update-result.json`` is never written, ``.sanremes-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating SanRemes" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``sanremes update`` is exactly this
...
class TestIdleWatchdogCountsUpdateLogGrowth:
    ...
    Real updates are stdout-silent for 40+ minutes: ``sanremes update`` captures
    the (very loud) Electron/vite build into ``logs/update.log`` — NOT the
    child's stdout (``sanremes_cli/update_cmd.py``, the update-log tee) — so the
    ...
    the ``SANREMES_UPDATE_STEP_IDLE_SECONDS`` override each fails a test here.
    """
    ...
    def test_progress_log_overridable_for_self_test(self):
        assert "SANREMES_UPDATE_PROGRESS_LOG" in self._src()

    def test_idle_override_env_retained(self):
        # The user/test-facing idle override must survive the amendment.
        assert "SANREMES_UPDATE_STEP_IDLE_SECONDS" in self._src()
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *leak* -- a step that spawns a grandchild with ``UseShellExecute = $false``
    ...
    asserts the grandchild was **still alive** when ``Invoke-SanRemesStep``
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``sanremes update`` build: output streams to
    ...
    env = {
        **os.environ,
        ...
        "SANREMES_UPDATE_PIPE_DRAIN_SECONDS": "3",
        "SANREMES_UPDATE_STEP_IDLE_SECONDS": "3",
        "SANREMES_SELFTEST_HOLD_SECONDS": "45",
    }
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "SanRemes' forever) or metered a chatty step (backpressure on the running "
