...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.sanremes/sanremes-agent/utils.py)
...
2. ``sanremes update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``sanremes_cli/main.py`` (the ``sanremes update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``sanremes_constants`` after the
