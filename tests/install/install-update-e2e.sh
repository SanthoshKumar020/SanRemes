...
#
# Installs a real, earlier SanRemes the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `sanremes`.
#
...
# Keep sandbox state out of the default .sanremes-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".sanremes-sandbox-e2e-$ROUTE"
export SANREMES_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/sanremes/.sanremes/sanremes-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/sanremes-agent.git"
...
LOG_DIR="${SANREMES_E2E_LOG_DIR:-$(mktemp -d -t sanremes-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED sanremes accept FLAG on `sanremes update`?
#
...
# that silently rots. `sanremes update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "sanremes update --help 2>&1" | grep -qF -- "$flag"
}
...
require_sanremes_works() {
  local when="$1" out
  out="$(in_sandbox "sanremes --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "sanremes --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "sanremes runs $when"
}

# ── install the earlier SanRemes ─────────────────────────────────────────────
...
require_sanremes_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: sanremes update'
    ...
    # sanremes which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="sanremes update --yes"
    else
      update_cmd="sanremes update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "sanremes update failed ($update_cmd)"
    fi
    require_landed_on_target 'sanremes update'
    require_sanremes_works 'after sanremes update'
    ;;
  installer)
    ...
    require_sanremes_works 'after installer re-run'
    ;;
esac
