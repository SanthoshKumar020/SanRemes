...
# cwd. Without an explicit PYTHONPATH the child resolves ``sanremes_cli`` /
...
def _make_running_kanban_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SANREMES_PROFILE", "parent-worker")
    monkeypatch.setenv("SANREMES_KANBAN_WORKSPACE", str(workspace))
    monkeypatch.setenv("SANREMES_KANBAN_ATTACHMENTS_ROOT", str(attachments_root))
    ...
    monkeypatch.setenv("SANREMES_KANBAN_RUN_ID", str(run_id))
...
def test_delegated_child_context_suppresses_env_gated_kanban_tools(monkeypatch, tmp_path):
    ...
    The parent process may be a dispatcher worker with SANREMES_KANBAN_TASK set;
    the child is only a subagent, not the run owner.
    """
    monkeypatch.setenv("SANREMES_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("SANREMES_KANBAN_RUN_ID", "123")
    home = tmp_path / ".sanremes"
    home.mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(home))
...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    ``os.environ`` and therefore never wrote SANREMES_DELEGATED_CHILD_CONTEXT into
    the sandbox env.
    """
    home = tmp_path / ".sanremes"
    home.mkdir()
    ...
    monkeypatch.setenv("SANREMES_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("SANREMES_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("SANREMES_KANBAN_DB", str(home / "kanban.db"))
    monkeypatch.setenv("SANREMES_KANBAN_WORKSPACE", str(tmp_path / "parent-workspace"))
    monkeypatch.setenv("SANREMES_KANBAN_CLAIM_LOCK", "lock")
    monkeypatch.delenv("SANREMES_DELEGATED_CHILD_CONTEXT", raising=False)
    ...
    with delegated_child_context():
        env = _scrub_child_env(
            dict(os.environ),
            is_passthrough=lambda k: k.startswith("SANREMES_KANBAN_"),
            is_windows=False,
        )

    assert os.environ.get("SANREMES_DELEGATED_CHILD_CONTEXT") is None
    assert env["SANREMES_HOME"] == str(home)
    assert env["SANREMES_DELEGATED_CHILD_CONTEXT"] == "1"
    assert "SANREMES_KANBAN_TASK" not in env
    assert "SANREMES_KANBAN_RUN_ID" not in env
    assert "SANREMES_KANBAN_DB" not in env
    assert "SANREMES_KANBAN_WORKSPACE" not in env
    assert "SANREMES_KANBAN_CLAIM_LOCK" not in env
...
def test_delegate_child_kanban_cli_cannot_delete_parent_board(
    ...
    code = (
        "from sanremes_cli import kanban; "
        "import argparse; "
