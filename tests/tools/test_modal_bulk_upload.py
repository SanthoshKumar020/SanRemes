...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.sanremes/credentials/a.json"),
            (str(src_b), "/root/.sanremes/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.sanremes/credentials/a.json" in names
            assert "root/.sanremes/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.sanremes/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.sanremes/skills/b.py").read()
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.sanremes/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.sanremes/large.bin" in names
