...
def sanremes_home(monkeypatch, tmp_path):
    """Isolate SANREMES_HOME so the tests don't pollute the real config.
...
class TestPatchCRLFPreservation:
    def test_patch_on_crlf_file_stays_pure_crlf(self, sanremes_home, tmp_path):
    ...
    def test_patch_multiline_replacement_on_crlf(self, sanremes_home, tmp_path):
...
class TestWriteFileCRLFPreservation:
    ...
    def test_overwrite_lf_file_stays_lf(self, sanremes_home, tmp_path):
