...
class TestRunBrowserCommandPathConstruction:
    ...
    def test_subprocess_preserves_executable_path_with_spaces(self, tmp_path):
        ...
        browser_path = "/Users/test/Library/Application Support/sanremes/node_modules/.bin/agent-browser"
        sanremes_home = str(tmp_path / "sanremes-home")
 ...
 patch("tools.browser_tool._chromium_installed", return_value=True), \
             ...
             patch("sanremes_constants.Path.home", return_value=tmp_path), \
    ...
    def test_npx_sentinel_resolves_via_resolve_npx_bin_with_pinned_spec(self, tmp_path):
        ...
        could let a broken system npx shadow a healthy SanRemes-managed one) and
        ...
        with patch("tools.browser_tool._find_agent_browser", return_value="npx agent-browser"), \
             patch("tools.browser_tool._resolve_npx_bin", return_value="/opt/sanremes/node/bin/npx"), \
             patch("tools.browser_tool._chromium_installed", return_value=True), \
             patch("tools.browser_tool._get_session_info", return_value=fake_session), \
             patch("tools.browser_tool._socket_safe_tmpdir", return_value=str(tmp_path)), \
             patch("tools.browser_tool._discover_homebrew_node_dirs", return_value=[]), \
        ...
        assert captured_cmd[:5] == [
            "/opt/sanremes/node/bin/npx", "--ignore-scripts", "--prefer-offline", "-y",
            AGENT_BROWSER_NPX_SPEC,
        ]
...
class TestResolveNpxBinPriority:
    ...
    SanRemes-managed one — and each candidate must be validated (actually
    ...
    def test_prefers_managed_extended_path_over_bare_path(self, monkeypatch):
        ...
        monkeypatch.setattr(bt, "_merge_browser_path", lambda _p: "/sanremes/node/bin")
        monkeypatch.setattr(
            bt.shutil, "which",
            lambda cmd, path=None: (
                "/sanremes/node/bin/npx" if path == "/sanremes/node/bin"
                else "/usr/local/bin/npx"
            ),
        )
        monkeypatch.setattr(bt, "node_tool_runnable", lambda p: True)

        assert bt._resolve_npx_bin() == "/sanremes/node/bin/npx"
    ...
    def test_falls_back_to_bare_path_when_extended_dir_has_no_npx(self, monkeypatch):
        ...
        monkeypatch.setattr(
            bt.shutil, "which",
            lambda cmd, path=None: None if path == "/sanremes/node/bin" else "/usr/bin/npx",
        )
        monkeypatch.setattr(bt, "node_tool_runnable", lambda p: True)
