...
A Bot Mode agent invoked as a short-lived ``sanremes -p <bot> chat -Q
...
``sanremes_cli.oneshot``) a bounded linger over every tracked
...
def test_config_reader_falls_back_when_config_unreadable(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.config.read_raw_config", _boom, raising=False
    )
...
def test_oneshot_module_lingers_before_agent_close():
    """sanremes_cli/oneshot.py must wait for pending completions before
    agent.close() (which kill_all()s the task's processes)."""
    src = (REPO_ROOT / "sanremes_cli" / "oneshot.py").read_text(encoding="utf-8")
...
def _run_e2e_parent(tmp_path, *, linger: bool) -> Path:
    ...
    env = dict(os.environ)
    env.setdefault("SANREMES_HOME", str(tmp_path / "sanremes_home"))
    proc = subprocess.run(
