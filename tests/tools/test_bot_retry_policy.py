...
def _is_sanremes_cli(argv) -> bool:
    """Match the delivery CLI by basename — local_delivery_command may
    resolve the venv-relative sanremes next to the interpreter (#93590)."""
    name = str(argv[0]).rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
    return name in ("sanremes", "sanremes.exe")
...
def _transport_calls(calls):
    ...
    return [argv for argv in calls if argv and _is_sanremes_cli(argv)]
...
def test_deliver_retries_same_argv_on_transient_failure(home, monkeypatch):
    ...
    def _fake_run(argv, **kwargs):
        ...
        if not _is_sanremes_cli(list(argv)):
            return _Proc(0)
