...
def test_resolve_stdio_command_falls_back_to_sanremes_node_bin(tmp_path):
    ...
    with patch("tools.mcp_tool.shutil.which", return_value=None), \
         patch.dict("os.environ", {"SANREMES_HOME": str(tmp_path)}, clear=False):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$SANREMES_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the SanRemes Docker image
    ...
    # the other candidates ($SANREMES_HOME/node/bin/npx and ~/.local/bin/npx)
