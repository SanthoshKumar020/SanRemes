...
class TestCheckRequirements:
    ...
    def test_gateway_tool_prompt_gate_uses_active_profile_token(self, monkeypatch):
        ...
        scope_token = secret_scope.set_secret_scope({"UNRELATED_SECRET": "value"})
        try:
            with patch("sanremes_cli.config.load_config") as load_config:
...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in sanremes-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_only_in_sanremes_discord_toolset(self):
        from toolsets import TOOLSETS, _SANREMES_CORE_TOOLS
        assert "discord" in TOOLSETS["sanremes-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["sanremes-discord"]["tools"]
        assert "discord" not in _SANREMES_CORE_TOOLS
        assert "discord_admin" not in _SANREMES_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in {"sanremes-discord", "sanremes-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["sanremes-discord"], quiet_mode=True,
