...
SanRemes exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_sanremes_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    SANREMES_HOME (cache/terminal), NOT tmpfs /tmp."""
    import sanremes_constants  # noqa: F401 — resolves SANREMES_HOME per call
    ...
    assert result == str(tmp_path / ".sanremes" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "sanremes_bg_dead1.log").exists()
