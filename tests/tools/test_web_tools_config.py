...
class TestBackendSelection:
    ...
    ``sanremes tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from sanremes_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.sanremes/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.sanremes/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_tavily_request_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Client-Name"] == "sanremes-agent"
