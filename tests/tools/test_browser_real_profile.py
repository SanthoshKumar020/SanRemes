...
``~/.sanremes/browser-profile/<browser>/`` and launches the user's real binary
on the copy with a devtools port (see sanremes_cli.browser_connect). These tests
...
class TestRealProfileResolvers:
    def test_data_dir_windows(self):
        import sanremes_cli.browser_connect as bc
...
class TestSnapshotRealProfile:
    ...
    def test_fresh_snapshot_copies_auth_and_skips_caches(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(bc, "get_sanremes_home", lambda: home)
    ...
    def test_missing_source_fails_closed(self, tmp_path, monkeypatch):
        import sanremes_cli.browser_connect as bc
        monkeypatch.setattr(bc, "get_sanremes_home", lambda: tmp_path / "hh")
...
class TestRealProfileCdpLaunch:
    ...
    def test_non_chromium_default_fails_closed(self):
        ...
        with patch.object(bt, "_use_real_profile", return_value=True), \
             patch("sanremes_cli.browser_connect.detect_default_chromium", return_value=None):
            cdp, err = bt._real_profile_cdp()
        assert cdp is None
    ...
    def test_snapshot_failure_fails_closed(self):
        ...
        self._reset()
        with patch.object(bt, "_use_real_profile", return_value=True), \
             patch("sanremes_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("sanremes_cli.browser_connect.snapshot_real_profile", return_value=(None, "boom")):
            cdp, err = bt._real_profile_cdp()
        assert cdp is None
    ...
    def test_launch_returns_http_cdp(self, tmp_path):
        ...
        with patch.object(bt, "_use_real_profile", return_value=True), \
             patch("sanremes_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("sanremes_cli.browser_connect.snapshot_real_profile", return_value=(str(tmp_path), None)), \
...
class TestChannelIdentity:
    ...
    def test_channel_sentinel_fails_closed_in_cdp(self):
        ...
        bt._real_profile_cdp_cache.clear()
        with patch.object(bt, "_use_real_profile", return_value=True), \
             patch("sanremes_cli.browser_connect.detect_default_chromium",
                   return_value=bc.UNSUPPORTED_CHANNEL), \
             patch("sanremes_cli.browser_connect.snapshot_real_profile") as snap:
            cdp, err = bt._real_profile_cdp()
        assert cdp is None
...
class TestSnapshotIsCredentialStore:
    """The copied Cookies/Login Data must live inside SanRemes' secret lifecycle."""
    ...
    def test_snapshot_dir_secured(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(bc, "get_sanremes_home", lambda: tmp_path / "hh")
        called = {}
        with patch("sanremes_cli.config._secure_dir",
...
class TestReviewBugFixes:
    ...
    def test_permissions_secured_on_refresh(self, tmp_path, monkeypatch):
        ...
        bc.snapshot_real_profile("chrome", src=str(src))  # fresh
        secured = []
        with patch("sanremes_cli.config._secure_dir", side_effect=secured.append):
    ...
    def test_lightpanda_engine_fails_actionably(self):
        ...
        with patch.object(bt, "_use_real_profile", return_value=True), \
             ...
             patch("sanremes_cli.browser_connect.detect_default_chromium") as det:
            cdp, err = bt._real_profile_cdp()
        assert cdp is None
...
class TestReviewRound3:
    ...
    def test_snapshot_blocks_when_locked_even_with_autoclose(self, tmp_path, monkeypatch):
        ...
        message offers the close (mentions SanRemes can close it)."""
        import sanremes_cli.browser_connect as bc
        src = self._multi(tmp_path / "real")
        home = tmp_path / "hh"
        monkeypatch.setattr(bc, "get_sanremes_home", lambda: home)
    ...
    def test_consent_off_triggers_cleanup(self, tmp_path, monkeypatch):
        ...
        called = {"n": 0}
        with patch.object(bt, "_use_real_profile", return_value=False), \
             patch("sanremes_cli.browser_connect.cleanup_real_profile_snapshots",
    ...
    def test_reuse_skips_snapshot_overlay(self, tmp_path):
        ...
        with patch.object(bt, "_use_real_profile", return_value=True), \
             ...
             patch("sanremes_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("sanremes_cli.browser_connect.real_profile_copy_dir", return_value=str(tmp_path)), \
    ...
    def test_relaunch_path_does_snapshot(self, tmp_path):
        ...
        with patch.object(bt, "_use_real_profile", return_value=True), \
             ...
             patch("sanremes_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("sanremes_cli.browser_connect.real_profile_copy_dir", return_value=str(tmp_path)), \
             patch("sanremes_cli.browser_connect.snapshot_real_profile",
