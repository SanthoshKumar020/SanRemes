...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    token = set_sanremes_home_override(str(tmp_path))
    try:
        env = build_subprocess_env(
    ...
    assert env["SANREMES_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_sanremes_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see SANREMES_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('SANREMES_HOME'), "
    ...
    assert result["home"] == str(sanremes_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `sanremes` on PATH
# ---------------------------------------------------------------------------


def test_e2e_scrubbed_env_resolves_bare_sanremes_under_minimal_parent_path(monkeypatch):
    """Regression for #92998/#93082: a gateway launched by systemd/cron with a
    minimal PATH (no sanremes console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``sanremes``.
    ...
    bin_dir = local_mod._resolve_sanremes_bin_dir()
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "sanremes.exe" if os.name == "nt" else "sanremes")
    ):
        pytest.skip("no real sanremes console-script install available")

    # Simulate the service-manager minimal PATH: sanremes dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("sanremes", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("sanremes", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'sanremes' must resolve from the child PATH {env.get('PATH')!r}"
    )
