...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.sanremes/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_handles_config_error(self):
        ...
        with patch("sanremes_cli.config.load_config", side_effect=RuntimeError("boom")):
    ...
    def test_guard_flag_quoted_false_stays_disabled(self):
        ...
        for quoted in ("false", "False", "0", "no", "off"):
            with patch("sanremes_cli.config.load_config",
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.sanremes/skills/ as a
    workaround.
    """
...
class TestBackgroundOwnershipPolicyConsistency:
    ...
    def test_repeated_identical_write_gets_the_same_answer(self, tmp_path, monkeypatch):
        ...
        (tmp_path / ".sanremes" / "skills").mkdir(parents=True, exist_ok=True)
        with _skill_dir(tmp_path):
...
# come up. The user unpins via `sanremes curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "sanremes curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
...
def _curator_pass(tmp_path, *, monkeypatch):
    ...
    Points SANREMES_HOME at ``tmp_path/.sanremes`` so skill_usage's archive path
    (``get_sanremes_home()``) resolves into the same tree the skill manager
    ...
    skills_root = sanremes_home / "skills"
...
class TestCuratorConsolidationDeleteGuard:
    ...
    def test_background_review_support_file_overwrite_requires_that_file_read(self, tmp_path, monkeypatch):
        ...
        with _curator_pass(tmp_path, monkeypatch=monkeypatch):
            _create_curator_skill("reviewed", _skill_content("reviewed"))
            ref = tmp_path / ".sanremes" / "skills" / "reviewed" / "references"
            ref.mkdir()
