...
# ---------------------------------------------------------------------------
# sanremes tools picker: tier variant rows
# ---------------------------------------------------------------------------
...
class TestPickerTierRows:
    def test_variant_schemas_flatten_to_tier_rows(self, fresh_registry, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.plugins._ensure_plugins_discovered", lambda: None
        )
    ...
    def test_tier_match_highlights_correct_row(self):
        from sanremes_cli.tools_config import _web_tier_matches
