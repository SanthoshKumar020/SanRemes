...
from tools.code_execution_tool import (
    ...
    _uses_sanremes_python_environment,
...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        path stages in sanremes_sandbox_*, the session kernel in
        sanremes_kernel_* — either satisfies strict mode's isolation promise."""
        result = self._run("import os; print(os.getcwd())", mode="strict")
        self.assertEqual(result["status"], "success")
        cwd = result["output"].strip()
        self.assertTrue(
            "sanremes_sandbox_" in cwd or "sanremes_kernel_" in cwd,
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The sanremes-agent venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_sanremes_tools(self):
        """Regression: sanremes_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from sanremes_tools import terminal`.
        """
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            code = (
                "from sanremes_tools import terminal\n"
                "r = terminal('echo x')\n"
    ...
    def test_strict_mode_can_still_import_sanremes_tools(self):
...
# ---------------------------------------------------------------------------
# _python_environment_prefix / _uses_sanremes_python_environment
# ---------------------------------------------------------------------------
...
class TestPythonEnvironmentPrefix(unittest.TestCase):
    ...
    def test_failure_is_not_cached(self):
        ...
        A sticky cached failure would silently drop the sanremes root from
...
class TestUsesSanRemesPythonEnvironment(unittest.TestCase):
    """Unit tests for _uses_sanremes_python_environment."""
    ...
    def test_true_for_current_interpreter(self):
        ...
        self.assertTrue(_uses_sanremes_python_environment(sys.executable))
    ...
    def test_true_for_current_interpreter_without_probe(self):
        ...
        must never drop the sanremes root for the interpreter SanRemes itself runs.
        """
        with patch("subprocess.run",
                   side_effect=subprocess.TimeoutExpired(cmd=[], timeout=5)) as mock_run:
            self.assertTrue(_uses_sanremes_python_environment(sys.executable))
    ...
    def test_false_for_different_prefix(self):
        ...
        with patch("tools.code_execution_tool._python_environment_prefix",
            ...
            self.assertFalse(_uses_sanremes_python_environment("/other/python"))

    def test_false_when_prefix_is_empty(self):
        """If prefix cannot be determined (error path), treat as external."""
        with patch("tools.code_execution_tool._python_environment_prefix",
                   return_value=""):
            self.assertFalse(_uses_sanremes_python_environment("/bad/python"))

    def test_true_when_prefix_matches_sys_prefix(self):
        sanremes_prefix = os.path.realpath(sys.prefix)
        with patch("tools.code_execution_tool._python_environment_prefix",
                   return_value=sanremes_prefix):
            self.assertTrue(_uses_sanremes_python_environment("/same/env/python"))


# ---------------------------------------------------------------------------
# PYTHONPATH composition — sanremes root included only for same-env interpreters
# ---------------------------------------------------------------------------

class TestPythonPathComposition(unittest.TestCase):
    """Verify sanremes root inclusion in PYTHONPATH depends on env match.

    Patches ``_uses_sanremes_python_environment`` directly so these tests are
    ...
    def _capture_pythonpath(self, same_env: bool) -> tuple:
        ...
        def _fake_popen(cmd, **kwargs):
            ...
            # [python, <staging_dir>/sanremes_kernel_runner.py] (session
    ...
    def test_sanremes_root_included_when_same_env(self):
        """When interpreter is in the SanRemes env, sanremes root is in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=True)
        parts = pythonpath.split(os.pathsep)
        self.assertIn(self._sanremes_root(), parts,
                      "sanremes root must be in PYTHONPATH for same-env interpreters")

    def test_sanremes_root_excluded_when_external_env(self):
        """When interpreter is external, sanremes root must NOT be in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=False)
        parts = pythonpath.split(os.pathsep)
        self.assertNotIn(self._sanremes_root(), parts,
                         "sanremes root must not leak into an external interpreter's PYTHONPATH")
