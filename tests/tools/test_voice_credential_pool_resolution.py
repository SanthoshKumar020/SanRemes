...
Keys added via ``sanremes auth add <provider>`` live in the credential pool /
auth store and used to be invisible to the voice tools, which only read
``os.environ`` + ``~/.sanremes/.env`` via ``get_env_value``. The shared
...
def _no_dotenv(monkeypatch):
    """Keep the developer's real ~/.sanremes/.env out of these tests."""
