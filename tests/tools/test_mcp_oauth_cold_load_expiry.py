...
1. ``SanRemesTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``SanRemesMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# SanRemesTokenStorage — absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
        from tools.mcp_oauth import SanRemesTokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# SanRemesMCPOAuthProvider._initialize — seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = SanRemesTokenStorage("srv")  # empty — no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="SanRemes Agent",
    )
