...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_sanremes_cli_expands_web(self):
        """sanremes-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"sanremes-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("sanremes-cli", expanded)


    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['sanremes-cli'] yields ['web']."""
        parent_toolsets = {"sanremes-cli"}
