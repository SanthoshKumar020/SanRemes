...
class TestHeredocMarker:
    ...
    def test_uuid_marker_on_collision(self):
        ...
        assert marker.startswith("SANREMES_PERSIST_")
...
class TestWriteToSandbox:
    def test_success(self):
        ...
        result = _write_to_sandbox("hello world", "/tmp/sanremes-results/abc.txt", env)
    ...
    def test_large_content_via_stdin(self):
        ...
        env.execute.return_value = {"output": "", "returncode": 0}
        big = "x" * 200_000
        _write_to_sandbox(big, "/tmp/sanremes-results/big.txt", env)
        cmd = env.execute.call_args[0][0]
...
class TestResolveStorageDir:
    ...
    def test_uses_env_temp_dir_when_available(self):
        ...
        assert _resolve_storage_dir(env) == "/data/data/com.termux/files/usr/tmp/sanremes-results"
...
class TestBuildPersistedMessage:
    def test_structure(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/sanremes-results/test123.txt",
        )
        ...
        assert "/tmp/sanremes-results/test123.txt" in msg
    ...
    def test_large_size_shows_mb(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/sanremes-results/big.txt",
        )
        assert "MB" in msg
...
class TestMaybePersistToolResult:
    ...
    def test_tool_use_id_cannot_escape_storage_dir(self):
        ...
        assert "Full output saved to: /tmp/sanremes-results/outside_whoami_x_" in result
        assert "/tmp/sanremes-results/../" not in result
        assert target.startswith("/tmp/sanremes-results/outside_whoami_x_")
...
# ── Host-side spillover ($SANREMES_HOME/cache/spillover) ────────────────
...
class TestSpillover:
    ...
    def test_remote_env_probe_failure_falls_back_to_sandbox_write(self):
        ...
        assert PERSISTED_OUTPUT_TAG in result
        assert "/tmp/sanremes-results/tc_remote_2.txt" in result
...
class TestRecoveryHint:
    def test_preview_teaches_recovery_not_refetch(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/sanremes-results/r.txt",
        )
