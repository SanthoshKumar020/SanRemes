...
tests -- see `sanremes-agent-dev` skill, "Why do subagents get their own
...
def test_session_key_scopes_to_its_own_slot(monkeypatch):
    monkeypatch.setenv("SANREMES_SESSION_KEY", "sess-A")
...
def test_distinct_session_keys_get_distinct_slots(monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_SESSION_KEY", "sess-B")
...
#
# The tests above set SANREMES_SESSION_KEY through os.environ, which only
...
# SANREMES_SESSION_KEY absent from os.environ.


def test_session_key_from_contextvar_without_environ(monkeypatch):
    # Prove the fix works on the gateway path: SANREMES_SESSION_KEY is NOT in
...
def test_contextvar_session_key_wins_over_environ(monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_SESSION_KEY", "sess-ENV")
    tokens = set_session_vars(session_key="sess-CTX")
    try:
        assert (
            terminal_tool._resolve_container_task_id(None) == "session:sess-CTX"
...
# long-lived container per SanRemes profile, shared by every session of that
