"""Regression test for the ``SanRemesMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
@pytest.mark.asyncio
async def test_sanremes_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import _SANREMES_PROVIDER_CLS, reset_manager_for_tests

    assert _SANREMES_PROVIDER_CLS is not None, "SDK OAuth types must be available"
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="SanRemes Agent",
    )
    provider = _SANREMES_PROVIDER_CLS(
        server_name="srv",
...
@pytest.mark.asyncio
async def test_sanremes_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
    ...
    assert _SANREMES_PROVIDER_CLS is not None
...
async def test_long_lived_resource_request_does_not_block_concurrent_post(
    ...
    acquiring its Bearer token.  SanRemes narrows that lock around resource I/O
