...
``sanremes_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``SANREMES_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # SANREMES_* handling (#27303): only the operational allowlist passes;
        # every other SANREMES_* is dropped (the broad prefix was removed).
        "SANREMES_HOME": "/home/alice/.sanremes",        # allowlisted → kept
        "SANREMES_PROFILE": "default",                 # allowlisted → kept
        "SANREMES_INTERACTIVE": "1",                   # not allowlisted → dropped
        "SANREMES_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "SANREMES_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``sanremes_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``sanremes_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        """
        from tools.code_execution_tool import generate_sanremes_tools_module
        import tempfile, ast
        stub = generate_sanremes_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_sanremes_tools_module(["terminal"], transport="uds")
...
# let the child import sanremes_tools, a user script that printed non-ASCII
