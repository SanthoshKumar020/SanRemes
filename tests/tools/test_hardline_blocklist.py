...
# boundary, blocking `sanremes send` message bodies, multi-line
# `git commit -m` messages, and heredoc text that merely MENTION
# shutdown/reboot commands.
_QUOTED_NEWLINE_DATA_ALLOW = [
    # sanremes send with a multi-line message body (the reported symptom)
    'sanremes send -t telegram -s "spark1" "console output:\nsudo reboot\ndone"',
    'sanremes send -t telegram "line1\nshutdown -h now\nline3"',
...
_QUOTED_NEWLINE_THREATS_BLOCK = [
    ...
    'sanremes send -t telegram "$(sudo reboot)"',
    'echo "`shutdown -h now`"',
...
def test_quoted_newline_data_not_blocked_by_full_guard_chain(clean_session):
    ...
    command = 'sanremes send -t telegram "status:\nsudo reboot happened at 3am"'
...
def test_yolo_env_var_cannot_bypass_hardline(clean_session, monkeypatch):
    """SANREMES_YOLO_MODE=1 must not bypass the hardline floor."""
...
def test_approvals_mode_off_cannot_bypass_hardline(clean_session, monkeypatch, tmp_path):
    ...
    # _get_approval_mode() reads from sanremes config; simplest path: monkeypatch the helper.
