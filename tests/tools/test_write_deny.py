...
class TestWriteDenyExactPaths:
    ...
    def test_sanremes_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.sanremes/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "sanremes_root"
        ...
        # Sanity check: SANREMES_HOME does point to the profile dir, not the root.
        from sanremes_constants import get_sanremes_home, get_default_sanremes_root
...
class TestWriteAllowed:
    ...
    def test_sanremes_control_files_requested_writable(self):
