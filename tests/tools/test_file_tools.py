...
class TestSensitivePathCheck:
    ...
    def test_sanremes_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools._sanremes_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools._sanremes_config_resolved_loaded", True)

        from tools.file_tools import write_file_tool
        result = json.loads(write_file_tool(str(fake_config), "approvals:\n  mode: off\n"))
        assert "error" in result
        assert "SanRemes config" in result["error"]

    def test_sanremes_config_blocked_via_tilde_path(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools._sanremes_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools._sanremes_config_resolved_loaded", True)
    ...
    def test_system_path_still_blocked(self, monkeypatch):
        monkeypatch.setattr("tools.file_tools._sanremes_config_resolved", "/some/other/path")
        monkeypatch.setattr("tools.file_tools._sanremes_config_resolved_loaded", True)
    ...
    def test_normal_file_not_blocked(self, mock_get, monkeypatch):
        monkeypatch.setattr("tools.file_tools._sanremes_config_resolved", "/home/user/.sanremes/config.yaml")
        monkeypatch.setattr("tools.file_tools._sanremes_config_resolved_loaded", True)
        mock_ops = MagicMock()
        result_obj = MagicMock()
