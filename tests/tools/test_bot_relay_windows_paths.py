...
2. ``local_delivery_command`` hardcoded ``"sanremes"``, relying on PATH —
   ...
   basename so resolved absolute paths (and ``sanremes.exe``) still take
...
def test_waiter_windows_path_compiles_after_backslash_folding():
    ...
    code = _waiter_code("C:\\Users\\joshu\\.sanremes")
...
def test_waiter_raw_prefix_keeps_injection_defense():
    ...
    code = _waiter_code(Path("/tmp/sanremes-home"), inj)
...
def test_local_delivery_resolves_sibling_sanremes(tmp_path, monkeypatch):
    bin_dir = tmp_path / "venv" / "bin"
    bin_dir.mkdir(parents=True)
    sibling = bin_dir / "sanremes"
    sibling.touch()
    sibling.chmod(0o755)
...
def test_local_delivery_uses_shutil_which_when_no_sibling(tmp_path, monkeypatch):
    ...
    which_hit = str(tmp_path / "usr-local-bin" / "sanremes")
    monkeypatch.setattr(
        bot_relay.shutil, "which", lambda name: which_hit if name == "sanremes" else None
    )
...
def test_delivery_lock_recognizes_resolved_cli_paths(tmp_path, monkeypatch):
    ...
    now that argv[0] may be a resolved absolute path (or sanremes.exe)."""
    acquired = []
    ...
    with bot_mode_dm._delivery_lock(
        [str(tmp_path / "venv" / "bin" / "sanremes"), "-p", "ops", "chat"],
        stdin_file=False,
    ):
        pass
    with bot_mode_dm._delivery_lock(["sanremes", "-p", "ops", "chat"], stdin_file=False):
        pass
    with bot_mode_dm._delivery_lock(
        ["C:\\venv\\Scripts\\sanremes.exe", "-p", "ops", "chat"], stdin_file=False
    ):
        pass
