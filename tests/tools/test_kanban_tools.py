...
Verifies:
  - Tools are gated on SANREMES_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `sanremes chat` sessions (no SANREMES_KANBAN_TASK) must have
    ...
    schema = registry.get_definitions(set(resolve_toolset("sanremes-cli")), quiet=True)
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: SANREMES_HOME isolated, SANREMES_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".sanremes"
    home.mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(home))
    monkeypatch.setenv("SANREMES_PROFILE", "test-worker")
    monkeypatch.delenv("SANREMES_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("SANREMES_KANBAN_TASK", tid)
    return tid
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("SANREMES_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated SANREMES_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to SANREMES_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``SANREMES_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``sanremes-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "sanremes-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from SANREMES_PROFILE in the fixture, not the
        # caller-supplied "sanremes-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(home))
    monkeypatch.setenv("SANREMES_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# ---------------------------------------------------------------------------
#
# A worker process has SANREMES_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no SANREMES_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # SANREMES_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no SANREMES_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via SANREMES_KANBAN_BOARD env var,
...
# restarting SanRemes. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated SanRemes home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    SANREMES_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".sanremes"
    home.mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(home))
    # Make sure neither SANREMES_KANBAN_DB nor SANREMES_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("SANREMES_KANBAN_DB", raising=False)
    monkeypatch.delenv("SANREMES_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("SANREMES_KANBAN_TASK", raising=False)
    monkeypatch.setenv("SANREMES_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: SANREMES_SESSION_PLATFORM + SANREMES_SESSION_CHAT_ID set.
# - TUI sessions: SANREMES_SESSION_KEY (or SANREMES_SESSION_ID) set, with
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("SANREMES_SESSION_PLATFORM", "telegram")
    monkeypatch.setenv("SANREMES_SESSION_CHAT_ID", "chat-42")
    monkeypatch.setenv("SANREMES_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("SANREMES_SESSION_USER_ID", "user-9")
    monkeypatch.setenv("SANREMES_SESSION_USER_ID_ALT", "alt-user-9")
    monkeypatch.setenv("SANREMES_SESSION_CHAT_TYPE", "forum")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports SANREMES_SESSION_KEY.
    We should still auto-subscribe, with platform='tui' and
    chat_id=<key>."""
    from tools import kanban_tools as kt
    monkeypatch.delenv("SANREMES_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("SANREMES_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("SANREMES_SESSION_THREAD_ID", raising=False)
    monkeypatch.delenv("SANREMES_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("SANREMES_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("SANREMES_SESSION_ID", raising=False)
...
def test_create_does_not_subscribe_in_cli_session(monkeypatch, worker_env):
    ...
    monkeypatch.delenv("SANREMES_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("SANREMES_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("SANREMES_SESSION_KEY", raising=False)
    monkeypatch.delenv("SANREMES_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.sanremes; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".sanremes"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n  auto_subscribe_on_create: false\n"
    )
    monkeypatch.setenv("SANREMES_HOME", str(home))
    monkeypatch.setenv("SANREMES_SESSION_PLATFORM", "discord")
    monkeypatch.setenv("SANREMES_SESSION_CHAT_ID", "channel-1")
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting SANREMES_ALLOW_PRIVATE_URLS on a private network.
    Resets the url_safety process-lifetime cache on both sides so the
    override neither leaks in nor out of the test.
    """
    from tools import url_safety

    monkeypatch.setenv("SANREMES_ALLOW_PRIVATE_URLS", "true")
...
def default_url_guard(monkeypatch):
    ...
    Clears SANREMES_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
    cache on both sides so a prior test's opt-in can't leak in.
    """
    from tools import url_safety

    monkeypatch.delenv("SANREMES_ALLOW_PRIVATE_URLS", raising=False)
    url_safety._reset_allow_private_cache()
    yield
    url_safety._reset_allow_private_cache()
