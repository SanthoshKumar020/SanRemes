...
def checkpoint_base(tmp_path):
    """Isolated checkpoint base — never writes to ~/.sanremes/."""
...
class TestStoreInit:
    ...
    def test_legacy_migration_archives_prev2_repos(
        ...
        (fake_repo / "SANREMES_WORKDIR").write_text(str(work_dir) + "\n")
...
class TestSafeRestore:
    """Safe restore: preserve user hand-edits, revert only SanRemes-authored changes.
    ...
    def test_safe_restore_skips_file_user_edited_after_agent(self, mgr, work_dir):
        ...
        # SanRemes writes the file, then the user modifies it afterwards.
        (work_dir / "main.py").write_text("agent version\n")
        mgr.record_agent_write(str(work_dir / "main.py"))
        ...
        result = mgr.restore(str(work_dir), base, safe=True)
        assert result["success"] is True
        # Content no longer matches what SanRemes last wrote → preserved.
    ...
    def test_safe_restore_removes_agent_created_file_keeps_user_edit(self, mgr, work_dir):
        ...
        # SanRemes creates a brand-new file after the checkpoint...
        ...
        result = mgr.restore(str(work_dir), base, safe=True)
        assert result["success"] is True
        # User edit preserved; SanRemes-created file removed (not in checkpoint).
    ...
    # shape as a file SanRemes created — and the delete branch treats absence as
...
def _seed_legacy_repo(base: Path, name: str, workdir: Path, mtime: float = None) -> Path:
    ...
    (shadow / "SANREMES_WORKDIR").write_text(str(workdir) + "\n")
    (shadow / "info").mkdir()
...
class TestPruneCheckpointsOrphanAllowlist:
    ...
    def test_end_to_end_timing_change_during_confirmation_prompt(self, tmp_path, monkeypatch):
        ...
        `sanremes checkpoints prune`: the preview shows one pre-v2 orphan; a
        ...
        be deleted.
        """
        import sanremes_cli.checkpoints as checkpoints_cli
