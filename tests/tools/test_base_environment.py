...
class TestWrapCommand:
    def test_basic_shape(self):
        ...
        assert "__sanremes_ec=$?" in wrapped
        ...
        assert "exit $__sanremes_ec" in wrapped
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        import shlex
        snap = str(tmp_path / "sanremes-snap-x.sh")
        _q = shlex.quote
        ...
        writer = (
            ...
            f"__sanremes_snap_tmp=$(mktemp {_tmpl}) && "
            f"{{ export -p > \"$__sanremes_snap_tmp\" && mv -f \"$__sanremes_snap_tmp\" {_q(snap)}; }} "
            f"2>/dev/null || rm -f \"$__sanremes_snap_tmp\" 2>/dev/null || true; "
            "done"
        )
    ...
    def test_failed_export_does_not_destroy_good_snapshot(self, tmp_path):
        ...
        script = (
            f"__sanremes_snap_tmp=$(mktemp {bad_tmp}) && "
            f"{{ export -p > \"$__sanremes_snap_tmp\" && mv -f \"$__sanremes_snap_tmp\" {_q(snap)}; }} "
            f"2>/dev/null || rm -f \"$__sanremes_snap_tmp\" 2>/dev/null || true"
        )
        self._run(script)
...
class TestEmbedStdinHeredoc:
    def test_heredoc_format(self):
        ...
        assert "SANREMES_STDIN_" in result
