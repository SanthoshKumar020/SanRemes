...
Under CIMD the ``client_id`` is the HTTPS URL of a document SanRemes publishes,
...
the conditions under which SanRemes is allowed to present it.
...
def test_document_client_id_is_the_url_sanremes_sends():
    """A CIMD document is only valid when its client_id is its own URL."""
    assert _document()["client_id"] == _CIMD_CLIENT_METADATA_URL


def test_document_declares_every_callback_sanremes_can_build(tmp_path, monkeypatch):
...
def test_generated_redirect_uri_is_registered_in_the_document(tmp_path, monkeypatch):
    ...
    _configure_callback_port(cfg, SanRemesTokenStorage("srv"))
...
def test_eligible_flow_gets_a_pinned_port(tmp_path, monkeypatch, private_ports):
    ...
    result = _maybe_use_cimd({}, SanRemesTokenStorage("srv"))
...
def test_concurrent_servers_get_different_pinned_ports(
    ...
    ports = {
        _maybe_use_cimd({}, SanRemesTokenStorage(f"srv-{i}"))[1]
...
def test_self_hosted_document_url_overrides_the_default(
    ...
    result = _maybe_use_cimd(cfg, SanRemesTokenStorage("srv"))
...
def test_config_that_conflicts_with_the_document_falls_back_to_dcr(
    ...
    assert _maybe_use_cimd(dict(cfg), SanRemesTokenStorage("srv")) is None
...
def test_existing_registration_falls_back_to_dcr(tmp_path, monkeypatch, private_ports):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    storage = SanRemesTokenStorage("srv")
...
def test_exhausted_port_range_falls_back_to_dcr(tmp_path, monkeypatch, private_ports):
    ...
    try:
        cfg: dict = {}
        port = _configure_callback_port(cfg, SanRemesTokenStorage("srv"))
    finally:
        for sock in squatters:
            sock.close()
...
def test_more_servers_than_pinned_ports_all_get_cimd(
    ...
    results = [
        _maybe_use_cimd({}, SanRemesTokenStorage(f"srv-{i}"))
...
def test_cached_metadata_decides_whether_to_pin(
    ...
    long after SanRemes fixes the redirect URI. Metadata cached by an earlier
...
def test_cached_pinned_port_is_not_handed_to_a_sibling_server(
    ...
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    settled = SanRemesTokenStorage("settled")
    ...
    fresh = _maybe_use_cimd({}, SanRemesTokenStorage("fresh"))
...
def test_sdk_chooses_cimd_only_when_the_server_advertises_it(
    ...
    """Closes the loop on the handoff: feed what SanRemes configured into the
...
def test_reauthorizing_clears_the_rejection(tmp_path, monkeypatch, private_ports):
    """`sanremes mcp login` wipes stored state, so a fixed document is retried."""
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    storage = SanRemesTokenStorage("srv")
...
def test_rejected_dcr_client_leaves_cimd_available(
    ...
    assert provider.context.client_metadata_url == _CIMD_CLIENT_METADATA_URL
    assert not SanRemesTokenStorage("srv").cimd_rejected()
...
def test_timeout_on_a_cimd_flow_names_the_document_and_the_escape_hatch(monkeypatch):
    ...
    symptom SanRemes sees is the callback timing out."""
