...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning sanremes PID to write to <session>.owner_pid
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which sanremes process owns a daemon so that
    concurrent sanremes processes don't reap each other's active browser
...
class TestLeakedDaemonWithLiveOwner:
    ...
    long-lived sanremes process, pinning ~5 CPU cores and driving load to 100+.
...
class TestPeriodicOrphanReap:
    ...
    boot — which is exactly what happens in a sanremes process that stays up
    for days.
    """
