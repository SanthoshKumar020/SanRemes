...
Background (#86878): ``sanremes chat -q "..."`` runs one turn and exits. cli.py
exports ``SANREMES_INTERACTIVE=1`` (needed for interactive sudo password
...
class TestSingleQueryApprovalModeParsing:
    def test_default_is_deny(self):
        ...
        with mock_patch("sanremes_cli.config.load_config_readonly", return_value={"approvals": {}}):
            assert _get_single_query_approval_mode() == "deny"

    def test_explicit_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("sanremes_cli.config.load_config_readonly", return_value={"approvals": {"single_query_mode": "deny"}}):
            assert _get_single_query_approval_mode() == "deny"

    def test_explicit_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("sanremes_cli.config.load_config_readonly", return_value={"approvals": {"single_query_mode": "approve"}}):
    ...
    def test_off_maps_to_approve(self):
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch("sanremes_cli.config.load_config_readonly", return_value={"approvals": {"single_query_mode": "off"}}):
            assert _get_single_query_approval_mode() == "approve"

    def test_allow_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("sanremes_cli.config.load_config_readonly", return_value={"approvals": {"single_query_mode": "allow"}}):
            assert _get_single_query_approval_mode() == "approve"

    def test_yes_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("sanremes_cli.config.load_config_readonly", return_value={"approvals": {"single_query_mode": "yes"}}):
            assert _get_single_query_approval_mode() == "approve"
    ...
    def test_unknown_value_defaults_to_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("sanremes_cli.config.load_config_readonly", return_value={"approvals": {"single_query_mode": "maybe"}}):
            assert _get_single_query_approval_mode() == "deny"

    def test_config_load_failure_defaults_to_deny(self):
        """If config loading fails entirely, default to deny (safe)."""
        from unittest.mock import patch as mock_patch
        with mock_patch("sanremes_cli.config.load_config_readonly", side_effect=RuntimeError("config broken")):
            assert _get_single_query_approval_mode() == "deny"

    def test_yaml_boolean_false_maps_to_deny(self):
        """YAML 1.1 parses bare 'off' as False. Ensure it maps to deny."""
        from unittest.mock import patch as mock_patch
        with mock_patch("sanremes_cli.config.load_config_readonly", return_value={"approvals": {"single_query_mode": False}}):
...
class TestSingleQueryContextDetection:
    def test_env_var_marks_single_query(self, monkeypatch):
        monkeypatch.setenv("SANREMES_SINGLE_QUERY_SESSION", "1")
    ...
    def test_env_var_unset_is_not_single_query(self, monkeypatch):
        monkeypatch.delenv("SANREMES_SINGLE_QUERY_SESSION", raising=False)
    ...
    def test_env_var_false_is_not_single_query(self, monkeypatch):
        monkeypatch.setenv("SANREMES_SINGLE_QUERY_SESSION", "0")
        assert approval_module._is_single_query_approval_context() is False
...
class TestSingleQueryDenyMode:
    """When SANREMES_SINGLE_QUERY_SESSION is set and single_query_mode=deny,
    ...
    def test_dangerous_command_blocked_in_single_query_deny_mode(self, monkeypatch):
        monkeypatch.setenv("SANREMES_SINGLE_QUERY_SESSION", "1")
        monkeypatch.setenv("SANREMES_INTERACTIVE", "1")
        monkeypatch.delenv("SANREMES_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("SANREMES_YOLO_MODE", raising=False)
...
class TestSingleQueryApproveMode:
    """When SANREMES_SINGLE_QUERY_SESSION is set and single_query_mode=approve,
...
class TestSingleQueryModeInteractions:
    ...
    def test_yolo_overrides_single_query_deny(self, monkeypatch):
        ...
        commands."""
        monkeypatch.setenv("SANREMES_SINGLE_QUERY_SESSION", "1")
        monkeypatch.setenv("SANREMES_YOLO_MODE", "1")
        monkeypatch.setenv("SANREMES_INTERACTIVE", "1")
        monkeypatch.delenv("SANREMES_GATEWAY_SESSION", raising=False)

        # _YOLO_MODE_FROZEN is frozen at module import time (security: prevents
        # prompt injection from runtime-setting SANREMES_YOLO_MODE). Patch the
        # module attribute directly to simulate process-startup with
        # SANREMES_YOLO_MODE=1.
        from unittest.mock import patch as mock_patch
        with (
