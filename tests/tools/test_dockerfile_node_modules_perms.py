...
Older images made /opt/sanremes/ui-tui and /opt/sanremes/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV SANREMES_TUI_DIR=/opt/sanremes/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R sanremes:sanremes /opt/sanremes/ui-tui" not in text
    assert "chown -R sanremes:sanremes /opt/sanremes/node_modules" not in text
