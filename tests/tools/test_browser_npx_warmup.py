...
warm_agent_browser_npx_cache() is the fire-and-forget helper `sanremes update` /
`sanremes doctor --fix` call to pre-fetch agent-browser via npx so the first real
...
`sanremes update` — not only when a browser tool is actually used), must pass
...
def test_captures_stdout_and_stderr_instead_of_inheriting_parent_fds():
    """The npx registry fetch runs on every `sanremes update` — its stdout/
...
def test_uses_credential_scrubbed_environment():
    ...
    os.environ with every provider/gateway credential SanRemes holds."""
...
def test_merges_extended_path_so_managed_only_npx_can_find_sibling_node():
    """If npx was resolved via the SanRemes-managed/extended search (not the
    ...
    with patch("tools.browser_tool._resolve_npx_bin", return_value="/opt/sanremes/node/bin/npx"), \
         patch("tools.browser_tool._build_browser_env", return_value={"PATH": "/usr/bin"}), \
         patch(
             "tools.browser_tool._merge_browser_path",
             return_value="/opt/sanremes/node/bin:/usr/bin",
         ) as mock_merge, \
         patch("subprocess.Popen", return_value=_mock_proc()) as mock_popen:
        warm_agent_browser_npx_cache()
    ...
    _args, kwargs = mock_popen.call_args
    assert kwargs["env"]["PATH"] == "/opt/sanremes/node/bin:/usr/bin"
...
def test_timeout_cleanup_communicate_itself_raising_does_not_propagate():
    ...
    fire-and-forget caller (sanremes_cli/doctor.py calls this bare)."""
    proc = _mock_proc(
        communicate_side_effect=[
            subprocess.TimeoutExpired(cmd=["npx"], timeout=60.0),
...
def test_returns_false_instead_of_raising_on_unexpected_communicate_exception():
    """Fire-and-forget contract: sanremes_cli/doctor.py calls this bare (no
