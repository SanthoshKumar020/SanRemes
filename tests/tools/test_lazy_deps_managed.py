...
A package-manager install (NixOS, and anything else shipping SanRemes from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.sanremes/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("sanremes_cli.config.get_managed_system", lambda: "nixos")
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets SANREMES_MANAGED *and* a writable target.

    Dockerfile sets SANREMES_LAZY_INSTALL_TARGET and the NixOS container module
    passes SANREMES_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("sanremes_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.config.get_managed_system", _raise)
