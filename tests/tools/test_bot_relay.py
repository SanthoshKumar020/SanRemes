    ...
    return [
        {
            ...
            "connection_label": "SanRemes Cloud",
            "title": "Moxie",
...
def test_roster_roundtrip_and_validation(root):
    rows = _rows() + [
        ...
        "not-a-dict",
        {"profile": "default", "handle": "sanremes", "connection_id": "cloud-1"},  # dupe
    ]
...
def test_resolve_remote_target_forms(root):
    ...
    assert bot_relay.resolve_remote_target("@sanremes", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("sanremes@cloud-1", roster)["profile"] == "default"
    assert bot_relay.resolve_remote_target("sanremes@nope", roster) is None
...
def test_resolve_ambiguous_handle_across_connections(root):
    ...
    assert "sanremes" in forms  # unique handle stays bare
...
def test_relay_route_queues_envelope_and_spawns_waiter(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "sanremes", "connection_id": "cloud-1",
         "connection_label": "SanRemes Cloud", "title": "Moxie"},
    ])
    ...
    agent = _FakeAgent(home)
    out = json.loads(message_agent_tool(target="sanremes", message="ping", agent=agent))
    assert out.get("status") == "sent"
    assert "SanRemes Cloud" in spawned["label"]
    ...
    assert pending[0]["message"].startswith("Message from 🤖 sanremes (@sanremes): ping")
...
def test_protocol_section_lists_remote_teammates(tmp_path):
    ...
    assert "`@sanremes` — on SanRemes Cloud — Moxie" in section
...
def test_cleanup_bot_relay_artifacts_sweeps_stale_plaintext(tmp_path, monkeypatch):
    ...
    stale_env = bot_relay.enqueue_envelope(
        ...
        sender_profile="default", sender_handle="sanremes",
    )
...
def test_cleanup_bot_relay_artifacts_missing_dir_is_zero(tmp_path, monkeypatch):
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "nope"))
...
def test_ttl_config_read_is_lazy_and_defensive(monkeypatch):
    ...
    def _boom(name, *a, **k):
        if name.startswith("sanremes_cli"):
...
def test_message_agent_surfaces_runtime_offline_refusal(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "sanremes", "connection_id": "cloud-1",
         "connection_label": "SanRemes Cloud", "online": False},
    ])
    monkeypatch.setattr(
        "tools.bot_mode_dm._spawn_delivery",
        lambda *a, **k: json.dumps({"status": "sent"}),
