...
class SubagentWorktreeTests(unittest.TestCase):
    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp(prefix="sanremes-sw-test-"))
    ...
    def test_create_makes_isolated_worktree(self):
        ...
        self.assertEqual(info["branch"], "sanremes-subagent/subagent-abc123")
    ...
    def test_context_note_names_path_and_branch(self):
        note = sw.build_worktree_context_note(
            {"path": "/x/wt", "branch": "sanremes-subagent/subagent-1"}
        )
        self.assertIn("/x/wt", note)
        self.assertIn("sanremes-subagent/subagent-1", note)
...
class WorktreePayloadSchemaTests(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp(prefix="sanremes-sw-schema-"))
        self.addCleanup(shutil.rmtree, self.tmp, True)
