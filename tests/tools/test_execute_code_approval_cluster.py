...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SANREMES_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SANREMES_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_sanremes_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SANREMES_HOME": "/h", "SANREMES_PROFILE": "p",
        "SANREMES_CONFIG": "/c.yaml", "SANREMES_ENV": "/e",
        "SANREMES_DELEGATED_CHILD_CONTEXT": "1",
        # other SANREMES_* → dropped (broad prefix removed)
        "SANREMES_BASE_URL": "https://x", "SANREMES_INTERACTIVE": "1",
        "SANREMES_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        "SANREMES_HOME", "SANREMES_PROFILE", "SANREMES_CONFIG", "SANREMES_ENV",
        "SANREMES_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SANREMES_BASE_URL", "SANREMES_INTERACTIVE", "SANREMES_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SANREMES_* vars."""
    import logging
