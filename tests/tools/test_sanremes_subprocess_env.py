"""Tests for sanremes_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_SANREMES_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
_TIER1_SAMPLE = {
    ...
    "SANREMES_DASHBOARD_SESSION_TOKEN": "dash-secret",
}
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return sanremes_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        with patch.dict(
            ...
            {
                **_SAFE_SAMPLE,
                "SANREMES_KANBAN_TASK": "t_parent",
                "SANREMES_KANBAN_RUN_ID": "123",
                "SANREMES_KANBAN_DB": "/tmp/parent-kanban.db",
                "SANREMES_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            },
            clear=True,
        ):
