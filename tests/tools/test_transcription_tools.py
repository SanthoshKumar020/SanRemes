...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("SANREMES_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("SANREMES_LOCAL_STT_LANGUAGE", raising=False)
...
class TestTranscribeLocalCommand:
    def test_command_provider_uses_sanitized_child_env(self, monkeypatch):
        """Salvage of #56332: command STT must not inherit SanRemes secrets."""
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("SANREMES_LOCAL_STT_LANGUAGE", "en")
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.sanremes_xai_user_agent = MagicMock(return_value="sanremes-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("SANREMES_LOCAL_STT_LANGUAGE", "fr")
...
class TestExplicitOpenaiSelectionError:
    ...
    with the ``sanremes tools`` remediation for managed-Nous users), the old
    ...
    def test_dispatch_returns_selection_specific_error(self, monkeypatch):
        ...
        its sanremes tools remediation instead of the all-provider install
        hint."""
        self._no_openai_credentials(monkeypatch)
        monkeypatch.setattr(
            "tools.transcription_tools.managed_nous_tools_enabled", lambda: True
        ...
        with patch("tools.transcription_tools._HAS_OPENAI", True), \
             ...
             patch(
                 ...
                 lambda what: f"managed route down for {what}; run `sanremes tools`",
             ):
        ...
        assert "sanremes tools" in result["error"]
