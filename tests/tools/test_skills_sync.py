...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.sanremes/)``
    ...
    def test_refuses_anything_that_is_not_a_strict_child_of_skills(self, tmp_path):
        """``/``, ``~/.sanremes`` itself, a sibling dir, and the skills root
        ...
        sanremes = tmp_path / "home"
        sanremes.mkdir()
        skills = sanremes / "skills"
        (skills / "keep").mkdir(parents=True)
        sibling = sanremes / "kanban.db"  # any non-skills path
        sibling.mkdir()

        with patch("tools.skills_sync.SKILLS_DIR", skills):
            for target in (Path("/"), sanremes, sibling, skills):
...
class TestGetBundledDir:
    def test_env_var_override_with_default_fallback(self, tmp_path, monkeypatch):
        ...
        custom_dir.mkdir()
        monkeypatch.setenv("SANREMES_BUNDLED_SKILLS", str(custom_dir))
        assert _get_bundled_dir() == custom_dir

        # Empty or unset falls back to the relative path from __file__.
        monkeypatch.setenv("SANREMES_BUNDLED_SKILLS", "")
        assert _get_bundled_dir().name == "skills"
        monkeypatch.delenv("SANREMES_BUNDLED_SKILLS", raising=False)
        assert _get_bundled_dir().name == "skills"
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `sanremes profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.sanremes) rely on so bundled
    skills are never seeded at install time NOR re-injected by `sanremes update`.
    """
    ...
    def test_marker_skips_sync_and_removal_seeds_normally(self, tmp_path):
        ...
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        sanremes_home = tmp_path / "home"
        sanremes_home.mkdir()
        marker = sanremes_home / ".no-bundled-skills"
        ...
        def _patches():
            ...
            stack.enter_context(patch("tools.skills_sync.SKILLS_DIR", skills_dir))
            stack.enter_context(patch("tools.skills_sync.MANIFEST_FILE", manifest_file))
            stack.enter_context(patch("tools.skills_sync.SANREMES_HOME", sanremes_home))
            return stack
...
class TestOptOutToggleAndRemove:
    """`sanremes skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.SANREMES_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.SANREMES_HOME", home):
...
class TestCallTimeDirResolution:
    ...
    SANREMES_HOME at import, so a long-lived dashboard/TUI process serving a
    ...
    The accessors must follow set_sanremes_home_override() at call time, while
    an explicitly patched module global (tests, _profile_scope retargeting)
    still wins.
    """

    def test_accessors_follow_sanremes_home_override(self, tmp_path):
        ...
        try:
            assert ss._sanremes_home() == profile_home
    ...
    def test_explicit_module_patch_wins_over_override(self, tmp_path):
        ...
        token = set_sanremes_home_override(str(tmp_path / "other-profile"))
        try:
