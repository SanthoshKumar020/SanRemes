...
def fake_sanremes(tmp_path, monkeypatch):
    """Build a two-profile SanRemes layout and point SANREMES_HOME at
    the sanremes-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_sanremes):
        from tools.file_tools import write_file_tool
        target = fake_sanremes["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_sanremes):
        ...
        from tools.file_tools import write_file_tool
        target = fake_sanremes["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_sanremes_path_unaffected(self, fake_sanremes, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_sanremes):
    ...
    def test_cross_profile_patch_bypass(self, fake_sanremes):
        from tools.file_tools import patch_tool
        target = fake_sanremes["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_sanremes):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_sanremes, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_sanremes["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up SANREMES_HOME (set in
        ...
        assert "not found in active profile 'sanremes-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.sanremes/profiles/<name>/."""
        # Don't set SANREMES_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_sanremes_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_sanremes_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_sanremes):
        """When active profile is 'sanremes-security', the prompt warns
        ...
        assert "Active SanRemes profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.sanremes/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active SanRemes profile: default" in src
        assert "Active SanRemes profile: {active_profile}" in src
