...
was gated on ``[ -f "$SANREMES_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        "set -eu\n"
        f"{env_setup}"
        f'SANREMES_HOME="{home}"\n'
        # In tests we run unprivileged; as_sanremes is a passthrough then.
        'as_sanremes() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    SanRemes loads $SANREMES_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/sanremes/.env.example -> $SANREMES_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $SANREMES_HOME/.env from it on first boot"
    )
