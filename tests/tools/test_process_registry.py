...
from tools.environments.local import _SANREMES_PROVIDER_ENV_FORCE_PREFIX
...
class TestOrphanedPipeReconciliation:
    ...
    `sanremes update` in Feishu spawned a background subprocess that restarted
...
class TestSpawnEnvSanitization:
    def test_spawn_local_strips_blocked_vars_from_background_env(self, registry):
        ...
        with patch.dict(os.environ, {
            ...
            registry.spawn_local(
                ...
                env_vars={
                    ...
                    f"{_SANREMES_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN": "forced-bot-token",
                },
            )
        ...
        assert f"{_SANREMES_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN" not in env
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with patch("tools.process_registry.time.sleep", return_value=None), \
            ...
            registry._env_poller_loop(
                session,
                env,
                "/path with spaces/sanremes_bg.log",
                "/path with spaces/sanremes_bg.pid",
                "/path with spaces/sanremes_bg.exit",
            )

        assert env.commands[0][0] == "cat '/path with spaces/sanremes_bg.log' 2>/dev/null"
        assert env.commands[1][0] == "kill -0 \"$(cat '/path with spaces/sanremes_bg.pid' 2>/dev/null)\" 2>/dev/null; echo $?"
        assert env.commands[2][0] == "cat '/path with spaces/sanremes_bg.exit' 2>/dev/null"
...
class TestCheckpoint:
    ...
    def test_recover_dead_wrapper_retries_unreaped_systemd_scope(
        ...
        entry = {
            ...
            "systemd_unit": "sanremes-worker-proc_dead_scope.scope",
        }
...
class TestSystemdCgroupIsolation:
    ...
    def test_wraps_in_systemd_scope_when_supervisor_and_available(
        ...
        wrapped in ``systemd-run --user --scope --unit=sanremes-worker-<id>``."""
        ...
        assert argv[unit_idx + 1].startswith("sanremes-worker-"), argv
        assert argv[unit_idx + 1] == f"sanremes-worker-{session.id}", (
            argv
        ...
        assert session.systemd_unit == f"sanremes-worker-{session.id}.scope"
    ...
    def test_inherited_gateway_tree_markers_do_not_scope_child_cli(
        ...
        _SANREMES_GATEWAY is inherited (and set by importing gateway.run), so
    ...
    def test_systemd_post_spawn_failure_never_kills_gateway_process_group(
        ...
        assert stop_unit.call_args.args[0].startswith("sanremes-worker-proc_")
    ...
    def test_pty_spawn_failure_reaps_scope_before_distinct_pipe_fallback(
        ...
        assert stopped_unit == f"sanremes-worker-{session.id}.scope"
        unit_idx = fallback_argv.index("--unit")
        assert fallback_argv[unit_idx + 1] == (
            f"sanremes-worker-{session.id}-pipe-fallback"
        )
        assert session.systemd_unit == (
            f"sanremes-worker-{session.id}-pipe-fallback.scope"
        )
    ...
    def test_kill_recovered_detached_already_exited_stops_persisted_scope(
        ...
        session.systemd_unit = "sanremes-worker-proc_recovered_scope.scope"
        registry._running[session.id] = session
        ...
        assert result["status"] == "already_exited"
        assert stopped == ["sanremes-worker-proc_recovered_scope.scope"]
        assert terminated == []
        assert session.exited is True
    ...
    def test_stop_systemd_unit_treats_absent_unit_as_clean(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda *args, **kwargs: subprocess.CompletedProcess(
                args=args[0],
                returncode=5,
                stderr=b"Unit sanremes-worker-gone.scope not loaded.\n",
            ),
        )

        assert pr._stop_systemd_unit("sanremes-worker-gone.scope") is True
