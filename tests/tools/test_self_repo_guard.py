...
class TestBlocksMutationsInSourceRepo:
    ...
    def test_relative_cd_into_repo(self, repo):
        hit, _ = _detect("cd sanremes-agent && git pull", repo.parent, repo)
        assert hit is True
    ...
    def test_tilde_dash_c_path(self, repo, monkeypatch, tmp_path):
        ...
        hit, _ = _detect("git -C ~/sanremes-agent checkout main", tmp_path, repo)
        assert hit is True
...
class TestBlockMessageGuidance:
    ...
    def test_scratch_hint_honors_sanremes_home(self, repo, monkeypatch):
        monkeypatch.setenv("SANREMES_HOME", "/custom/sanremes-home")
        hit, msg = _detect("git rebase origin/main", repo, repo)
        assert hit is True
        assert "/custom/sanremes-home/scratch" in msg
