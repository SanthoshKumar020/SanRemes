"""Tests for SanRemes-managed Camofox state helpers."""
...
class TestCamofoxStatePaths:
    def test_paths_are_profile_scoped(self, tmp_path):
        state = _load_module()
        with patch.object(state, "get_sanremes_home", return_value=tmp_path):
...
class TestCamofoxIdentity:
    ...
    def test_default_task_id(self, tmp_path):
        ...
        with patch.object(state, "get_sanremes_home", return_value=tmp_path):
            ...
            assert identity["user_id"].startswith("sanremes_")
