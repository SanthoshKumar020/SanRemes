...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("sanremes_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_SESSION_KEY", session_key)
        monkeypatch.setenv("SANREMES_EXEC_ASK", "1")
        monkeypatch.delenv("SANREMES_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("sanremes-verify-", "sanremes-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "sanremes-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/sanremes-verify-example.py",
            "rm -f /tmp/sanremes-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../sanremes-verify-example.py",
            "rm -f /tmp/a/../../tmp/sanremes-verify-example.py",
            "rm -f /var/tmp/sanremes-verify-example.py",
            "rm -f /tmp/sanremes-verify-*",
            "rm -f /tmp/sanremes-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/sanremes-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/sanremes-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\sanremes-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\sanremes-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\sanremes-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"SANREMES_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.sanremes/.env",
            "echo x | tee $SANREMES_HOME/.env",
            'echo x | tee "$SANREMES_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestSanRemesConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.sanremes/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.sanremes/config.yaml",
            "echo '  mode: off' >> ~/.sanremes/config.yaml",
            "echo x | tee ~/.sanremes/config.yaml",
            "echo x | tee $SANREMES_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.sanremes/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-SanRemes absolute config.yaml is
        # handled by the project patterns, not the SanRemes-home rule.
        for cmd in (
            "cat ~/.sanremes/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.sanremes/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / SanRemes-home prefixes must fold to ~/ and
    ~/.sanremes/ in dangerous-command detection.
    ...
    SSH, and SanRemes config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_sanremes_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/SANREMES_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("SANREMES_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.sanremes/sanremes-agent && source venv/bin/activate && python -m sanremes_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m sanremes_cli.main gateway run --replace &",
            "nohup python -m sanremes_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart sanremes-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the SanRemes security config via IFS.
            "sed${IFS}-i ~/.sanremes/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep sanremes) bypasses the pkill/killall name-matching
    ...
    def test_kill_pgrep_expansion_detected(self):
        for cmd in (
            'kill -9 $(pgrep -f "sanremes.*gateway")',
            "kill -9 `pgrep sanremes`",
            "kill $(pgrep gateway)",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof sanremes)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof sanremes_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof sanremes`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the SanRemes service
    label achieves the same effect as `sanremes gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_sanremes_label_detected(self):
        for cmd in (
            "launchctl stop ai.sanremes.gateway",
            "launchctl kickstart -k system/ai.sanremes.gateway",
            "launchctl bootout system/ai.sanremes.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.sanremes.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-SanRemes labels, are
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a sanremes-gateway identifier, so quoted prose and non-gateway sanremes
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "sanremes"/"ai.sanremes" to appear AFTER the verb and missed this
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("SANREMES_GATEWAY_SESSION", "SANREMES_CRON_SESSION",
                      "SANREMES_YOLO_MODE",
                      "SANREMES_SESSION_KEY", "SANREMES_INTERACTIVE")
        }
        os.environ.pop("SANREMES_YOLO_MODE", None)
        os.environ.pop("SANREMES_INTERACTIVE", None)
        # SANREMES_CRON_SESSION takes priority over SANREMES_GATEWAY_SESSION in
        ...
        os.environ.pop("SANREMES_CRON_SESSION", None)
        os.environ["SANREMES_GATEWAY_SESSION"] = "1"
        os.environ["SANREMES_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("sanremes_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"SANREMES_INTERACTIVE": "1"}, clear=False):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.sanremes.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.sanremes.gateway.plist",
    "launchctl stop ai.sanremes.gateway",
    "launchctl restart ai.sanremes.gateway",
    "launchctl bootout gui/501/ai.sanremes.gateway",
    "launchctl remove ai.sanremes.gateway",
    "launchctl disable gui/501/ai.sanremes.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_SANREMES_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other SanRemes ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.sanremes.update-checker",
