...
try:
    ...
    pytest.skip("sanremes-agent tools not importable (missing deps)", allow_module_level=True)
...
class TestCwdHandling:
    ...
    def test_home_path_replaced_for_modal(self, monkeypatch):
        ...
        monkeypatch.setenv("TERMINAL_CWD", "/home/dakota/github/sanremes-agent")
...
class TestDockerHostBindApproval:
    ...
    def _isolate_approval_state(monkeypatch):
        ...
        hermetic SANREMES_HOME fixture runs), so on a dev machine whose real
        ...
        SANREMES_YOLO_MODE off the environment when the module is imported at
        collection time, before conftest's per-test env blanking runs. A test
        run launched from a --yolo SanRemes session (or any shell exporting
        SANREMES_YOLO_MODE=1) freezes True and every guard auto-approves.
