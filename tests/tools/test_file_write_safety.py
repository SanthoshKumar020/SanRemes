"""Tests for file write safety and SANREMES_WRITE_SAFE_ROOT sandboxing.
...
class TestSafeWriteRoot:
    """SANREMES_WRITE_SAFE_ROOT should sandbox writes to a specific subtree."""
    ...
    def test_safe_root_does_not_override_static_deny(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_WRITE_SAFE_ROOT", os.path.expanduser("~"))
        assert _is_write_denied(os.path.expanduser("~/.ssh/id_rsa")) is True


class TestMultipleSafeWriteRoots:
    """SANREMES_WRITE_SAFE_ROOT with multiple colon-separated directories."""
    ...
    def test_write_inside_first_root_allowed(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_WRITE_SAFE_ROOT", f"{root_a}{os.pathsep}{root_b}")
        assert _is_write_denied(str(child)) is False
    ...
    def test_trailing_separator_ignored(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_WRITE_SAFE_ROOT", f"{root}{os.pathsep}")
        assert _is_write_denied(str(inside)) is False
...
class TestGetWriteDeniedError:
    ...
    def test_credential_path_message(self):
        ...
        assert "SANREMES_WRITE_SAFE_ROOT" not in err
    ...
    def test_safe_root_message(self, tmp_path: Path, monkeypatch):
        ...
        assert err is not None
        assert "outside SANREMES_WRITE_SAFE_ROOT" in err
...
class TestSafeRootDenialMessageIntegration:
    ...
    message when SANREMES_WRITE_SAFE_ROOT blocks a path.
    ...
    def test_write_file_safe_root_outside_shows_safe_root_message(
        ...
        assert "outside SANREMES_WRITE_SAFE_ROOT" in res.error
...
class TestAtomicWrite:
    ...
    def test_no_temp_file_leaked_on_success(self, ops, tmp_path: Path):
        ...
        assert [p for p in os.listdir(tmp_path) if ".sanremes-tmp" in p] == []
...
class TestProtectedInstructionFiles:
    ...
    AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project-local .sanremes
    ...
    def test_project_local_sanremes_dir_is_gated(self, tmp_path, approvals):
        proj = tmp_path / "proj" / ".sanremes"
        proj.mkdir(parents=True)
        approvals["answer"] = "deny"
        res = self._write(proj / "config.yaml")
        assert res.get("error") and "BLOCKED" in res["error"]

    def test_checkout_nested_under_sanremes_dir_not_gated(self, tmp_path, approvals):
        """A repo living UNDER a .sanremes dir (e.g. ~/.sanremes/sanremes-agent)
        must not have every write gated — only files directly inside a
        .sanremes dir count as project config."""
        repo = tmp_path / ".sanremes" / "some-repo" / "src"
        repo.mkdir(parents=True)
        res = self._write(repo / "module.py", "x = 1\n")
        assert not res.get("error"), res
        assert approvals["calls"] == []

    def test_real_sanremes_home_not_gated_by_this_check(
        self, tmp_path, approvals, monkeypatch
    ):
        """~/.sanremes itself is governed by existing guards, not this gate."""
        import tools.file_tools as ft
        fake_home = tmp_path / ".sanremes"
        (fake_home / "notes").mkdir(parents=True)
        monkeypatch.setattr(
            ft, "_get_real_sanremes_home", lambda: str(fake_home.resolve())
        )
