...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"SANREMES_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestRegistration:
    ...
    def test_cua_driver_cmd_env_override_is_resolved_dynamically(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_CUA_DRIVER_CMD", str(driver))
...
class TestLazyMcpInstall:
    ...
    def test_start_reports_incompatible_existing_driver_before_mcp_setup(self):
        ...
        with patch.object(
            ...
            with pytest.raises(RuntimeError, match="sanremes computer-use install"):
                cua_backend.CuaDriverBackend().start()
...
class TestContractAutoRepair:
    ...
    installed driver (a state SanRemes' own version-floor bump created),
    ...
    def test_start_auto_repairs_incompatible_driver(self, monkeypatch):
        ...
        with patch.object(
             ...
             patch("sanremes_cli.tools_config.install_cua_driver",
    ...
    def test_explicit_override_is_never_repaired(self, monkeypatch):
        ...
        monkeypatch.setattr(cua_backend, "_contract_repair_attempted", False)
        monkeypatch.setenv("SANREMES_CUA_DRIVER_CMD", "/opt/custom/cua-driver")
        with patch.object(
                 cua_backend,
                 "cua_driver_runtime_contract_status",
                 return_value=self._incompatible(),
             ), \
             patch("sanremes_cli.tools_config.install_cua_driver") as installer, \
             patch("tools.lazy_deps.ensure"):
            with pytest.raises(RuntimeError, match="SANREMES_CUA_DRIVER_CMD"):
                cua_backend.CuaDriverBackend().start()
...
class TestCuaDriverSessionReconnect:
    ...
    def test_reconnect_restores_public_label_before_replaying_read(self):
        ...
        bridge = FakeBridge()
        session = self._make_session(bridge)
        session._declared_session_id = "sanremes-label"
        ...
        assert bridge.calls == [
            ("call", "list_apps", {}),
            ("call", "start_session", {"session": "sanremes-label"}),
            ("call", "list_apps", {}),
        ]
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit SanRemes-managed
...
class TestClickButtonPassthrough:
    """Surface 5 (NousResearch/sanremes-agent#47072) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (NousResearch/sanremes-agent#47072): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (NousResearch/sanremes-agent#47072): instead of hardcoding
    ...
    the MCP subcommand doesn't require a SanRemes patch.
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so SANREMES_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (NousResearch/sanremes-agent#47072): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (NousResearch/sanremes-agent#47072): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (NousResearch/sanremes-agent#47072): trycua/cua#1961 added
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): SanRemes never declared a cua-driver
