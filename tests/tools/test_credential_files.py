...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        sanremes_home.mkdir()
        (sanremes_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"SANREMES_HOME": str(sanremes_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(sanremes_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.sanremes/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        (sanremes_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        sanremes_home = tmp_path / ".sanremes"
        skills_dir = sanremes_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.sanremes/skills"


    def test_custom_container_base(self, tmp_path):
        sanremes_home = tmp_path / ".sanremes"
        (sanremes_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SANREMES_HOME": str(sanremes_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.sanremes")

        assert mounts[0]["container_path"] == "/home/user/.sanremes/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.sanremes/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.sanremes/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SANREMES_HOME."""
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / ".sanremes"))
        (tmp_path / ".sanremes").mkdir()

        # Create a sensitive file one level above sanremes_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside sanremes_home
    ...
    def test_nested_subdir_inside_sanremes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SANREMES_HOME must be allowed."""
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        subdir = sanremes_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SANREMES_HOME pointing outside must be rejected."""
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

        # Create a sensitive file outside sanremes_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside sanremes_home pointing outside
        symlink = sanremes_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SANREMES_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, sanremes_home: Path, cred_files: list):
        import yaml
        config_path = sanremes_home / "config.yaml"
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SANREMES_HOME."""
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
        ...
        sensitive.write_text("{}")
        self._write_config(sanremes_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(sanremes_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SANREMES_HOME via config must still mount."""
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

        (sanremes_home / "oauth.json").write_text("{}")
        self._write_config(sanremes_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        (sanremes_home / "cache" / "documents").mkdir(parents=True)
        (sanremes_home / "cache" / "audio").mkdir(parents=True)
        (sanremes_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.sanremes/cache/documents" in paths
        assert "/root/.sanremes/cache/audio" in paths
        assert "/root/.sanremes/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_sanremes_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        # Use legacy dir name with content — get_sanremes_dir prefers
        # populated old over new.
        legacy_doc = sanremes_home / "document_cache"
        legacy_img = sanremes_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(sanremes_home / "document_cache") in host_paths
        assert str(sanremes_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.sanremes/cache/documents" in container_paths
        assert "/root/.sanremes/cache/images" in container_paths
    ...
    def test_empty_sanremes_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.sanremes/attachments" in container_paths
        assert "/root/.sanremes/images" in container_paths
        assert "/root/.sanremes/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``SANREMES_HOME/images``, not
        ...
        """
        sanremes_home = tmp_path / ".sanremes"
        (sanremes_home / "images").mkdir(parents=True)
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.sanremes/images" in by_container
        assert by_container["/root/.sanremes/images"] == str(sanremes_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / ".sanremes"
        (sanremes_home / "images").mkdir(parents=True)
        upload = sanremes_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.sanremes/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        sanremes_home = tmp_path / ".sanremes"
        img_dir = sanremes_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.sanremes/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(sanremes_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.sanremes/cache/images/x.png"
        assert (sanremes_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    SanRemes cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        sanremes_home = tmp_path / ".sanremes"
        (sanremes_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
        return str(sanremes_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_sanremes(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.sanremes/attachments/drop.zip"

    def test_ssh_maps_to_tilde_sanremes(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.sanremes/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / ".sanremes"
        doc_dir = sanremes_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — SANREMES_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping SANREMES_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"SANREMES_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.sanremes/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.sanremes/google_token.json" in paths
        assert "/root/.sanremes/.env" not in paths
