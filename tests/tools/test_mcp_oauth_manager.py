...
def test_manager_isolates_same_named_servers_by_profile_home(tmp_path, monkeypatch):
    ...
    for home, access_token in ((profile_a, "TOKEN_A"), (profile_b, "TOKEN_B")):
        ...
        try:
            storage = SanRemesTokenStorage("shared")
...
def test_sanremes_provider_subclass_exists():
    """SanRemesMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _SANREMES_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _SANREMES_PROVIDER_CLS is not None
    assert issubclass(_SANREMES_PROVIDER_CLS, OAuthClientProvider)
