...
def test_normal_sanremes_session_maps_to_standard_mode():
...
def test_any_explicit_sanremes_bypass_maps_to_unrestricted_mode():
    from tools.computer_use import tool as computer_use
...
def test_standard_existing_profile_grant_owns_private_macos_runtime():
    ...
    args, socket_path = _standard_runtime_launch_args(
        ...
        platform="darwin",
        socket_path="/tmp/sanremes-cua-test.sock",
    )
    ...
    assert args == [
        ...
        "existing-profile",
        "--socket",
        "/tmp/sanremes-cua-test.sock",
    ]
    assert socket_path == "/tmp/sanremes-cua-test.sock"
