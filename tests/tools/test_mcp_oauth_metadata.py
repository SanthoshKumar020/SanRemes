...
Covers:
- :class:`SanRemesTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.SanRemesMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _SANREMES_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# SanRemesTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = SanRemesTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
        storage = SanRemesTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (SanRemesMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: SanRemesTokenStorage, **context_attrs):
    ...
    provider = _SANREMES_PROVIDER_CLS.__new__(_SANREMES_PROVIDER_CLS)
    provider._sanremes_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
        storage = SanRemesTokenStorage("mgr-srv")
        ...
        with patch.object(
            _SANREMES_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
        storage = SanRemesTokenStorage("flow-srv")
        ...
        with patch.object(
            _SANREMES_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
