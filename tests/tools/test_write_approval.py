...
and the shared slash-command handlers (sanremes_cli/write_approval_commands.py).
...
@pytest.fixture
def sanremes_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="sanremes_wa_test_")
    home = os.path.join(d, ".sanremes")
    os.makedirs(home)
    monkeypatch.setenv("SANREMES_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import sanremes_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(sanremes_home):
...
def test_invalid_subsystem_is_off(sanremes_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(sanremes_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(sanremes_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(sanremes_home, monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.config.load_config", _boom)
...
def test_handle_approve_all(sanremes_home):
...
def test_handle_approval_on(sanremes_home):
    from sanremes_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(sanremes_home):
    from sanremes_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(sanremes_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(sanremes_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(sanremes_home):
