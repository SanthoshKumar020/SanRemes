"""Tests for tools.wake_word — the "Hey SanRemes" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey sanremes"}) == "hey sanremes"
    assert ww.wake_phrase({}) == "hey sanremes"
...
def test_looks_like_path():
    assert ww._looks_like_path("models/hey_sanremes.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "sanremes_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey sanremes"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey sanremes"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_sanremes": object()}

        def predict(self, frame):
            return {"hey_sanremes": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_sanremes.onnx"}}
    )
    assert calls["download"] == [["/models/hey_sanremes.onnx"]]
    assert eng._labels == ["hey_sanremes"]


def test_bundled_hey_sanremes_model_ships_on_disk():
    # The "hey sanremes" wake word works out of the box only if the model is
