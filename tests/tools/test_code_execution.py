...
UDS socket lifecycle, sanremes_tools generation, timeout enforcement,
...
from tools.code_execution_tool import (
    ...
    execute_code,
    generate_sanremes_tools_module,
...
class TestSanRemesToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_sanremes_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)


    def test_empty_list_generates_nothing(self):
        src = generate_sanremes_tools_module([])
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_sanremes_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"sanremes_rpc\")", src)
        self.assertNotIn('os.environ.get("SANREMES_RPC_DIR", "/tmp/sanremes_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_sanremes_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        mkdir_cmd = next(cmd for cmd, _, _ in env.commands
                         if "mkdir -p" in cmd and "sanremes_exec_" in cmd)
        run_cmd = next(cmd for cmd, _, _ in env.commands if "python3 script.py" in cmd)
        cleanup_cmd = next(cmd for cmd, _, _ in env.commands
                           if "rm -rf" in cmd and "sanremes_exec_" in cmd)
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/sanremes_exec_", mkdir_cmd)
        self.assertIn("SANREMES_RPC_DIR=/data/data/com.termux/files/usr/tmp/sanremes_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/sanremes_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/sanremes_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """SANREMES_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"SANREMES_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import sanremes_constants; print(sanremes_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("sanremes_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from sanremes_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from sanremes_tools import json_parse
...
"""
    ...
    def test_retry_helper_all_fail(self):
        ...
        code = """
from sanremes_tools import retry
def always_fail():
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_sanremes_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("SANREMES_RPC_SOCKET", child_env)


    def test_timezone_injected_when_set(self):
        env_backup = os.environ.copy()
        try:
            os.environ["SANREMES_TIMEZONE"] = "America/New_York"
            child_env = self._get_child_env()
    ...
    def test_timezone_not_set_when_empty(self):
        env_backup = os.environ.copy()
        try:
            os.environ.pop("SANREMES_TIMEZONE", None)
            child_env = self._get_child_env()
            if "TZ" in child_env:
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching SANREMES_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated sanremes_tools module reads SANREMES_RPC_TOKEN and sends it."""
        src = generate_sanremes_tools_module(["terminal"], transport="uds")
        self.assertIn("SANREMES_RPC_TOKEN", src)
