"""Contract tests for the Docker image's immutable /opt/sanremes install tree."""
...
def test_dockerfile_makes_opt_sanremes_readonly_for_sanremes_user() -> None:
    ...
    assert "chown -R root:root /opt/sanremes" not in text
    assert "chmod -R a+rX /opt/sanremes" not in text
    assert "chmod -R a-w /opt/sanremes" not in text


def test_dockerfile_does_not_chown_install_trees_to_sanremes() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+sanremes:sanremes\s+/opt/sanremes/\.venv",
        r"chown\s+-R\s+sanremes:sanremes\s+/opt/sanremes/ui-tui",
        r"chown\s+-R\s+sanremes:sanremes\s+/opt/sanremes/gateway",
        r"chown\s+-R\s+sanremes:sanremes\s+/opt/sanremes/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/sanremes must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/sanremes/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $SANREMES_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the sanremes
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/sanremes/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/sanremes/bin && \\\n"
        r"(?:.*\\\n)+?"
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/sanremes tree.
    assert f"ENV SANREMES_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV SANREMES_LAZY_INSTALL_TARGET=/opt/sanremes" not in text

    # The seal flag must still be present — the redirect rides on top of it,
    # it does not replace it.
    assert "ENV SANREMES_DISABLE_LAZY_INSTALLS=1" in text

    # stage2-hook must seed + chown the target dir so first-use installs
    # succeed as the unprivileged sanremes runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$SANREMES_HOME/lazy-packages"' in stage2, (
...
def test_dockerfile_bakes_photon_sidecar_deps() -> None:
    ...
    assert not re.search(
        r"chown\s+-R\s+sanremes:sanremes\s+/opt/sanremes/plugins", text
    )
