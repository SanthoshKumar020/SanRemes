...
class TestIsWriteDenied:
    ...
    def test_oauth_traversal_denied(self, path):
        ...
        full_path = str(sanremes_home / path)
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.sanremes/pairing/{platform}-approved.json as the
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_uses_bash_safe_windows_paths(self, mock_env):
        ...
        assert commands[0] == (
            ...
            "then echo __sanremes_not_regular__; "
            "else exit 1; fi"
        )
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__SANREMES_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__SANREMES_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert result.error is None
        assert "SANREMES_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__SANREMES_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__SANREMES_FENCE_a9f7b3__\n"
        )
...
class TestEscapeNativeToolArg:
    ...
    (/c/Users/...) that _escape_shell_arg produces, but SanRemes sets
