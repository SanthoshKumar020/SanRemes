...
def _restore_tool_modules():
    original_sanremes_home = os.environ.get("SANREMES_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "sanremes_cli"
        or name.startswith("sanremes_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_sanremes_home is None:
            os.environ.pop("SANREMES_HOME", None)
        else:
            os.environ["SANREMES_HOME"] = original_sanremes_home
        _reset_modules(("tools", "sanremes_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    sanremes_cli = types.ModuleType("sanremes_cli")
    sanremes_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["sanremes_cli"] = sanremes_cli
    sanremes_home = tmp_path / "sanremes-home"
    os.environ["SANREMES_HOME"] = str(sanremes_home)
    sys.modules["sanremes_cli.config"] = types.SimpleNamespace(
        get_sanremes_home=lambda: sanremes_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="sanremes-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": sanremes_home / "modal_snapshots.json",
