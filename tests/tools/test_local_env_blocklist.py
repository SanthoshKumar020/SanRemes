...
Verifies that SanRemes-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed SanRemes secrets.

See: https://github.com/NousResearch/sanremes-agent/issues/1002
See: https://github.com/NousResearch/sanremes-agent/issues/1264
"""
...
from tools.environments.local import (
    LocalEnvironment,
    _SANREMES_PROVIDER_ENV_BLOCKLIST,
    _SANREMES_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.sanremes/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a SanRemes inference secret
        ...
        models`` run inside a SanRemes terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _SANREMES_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        SanRemes-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "SANREMES_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
    ...
    def test_bare_sanremes_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve SanRemes even when the gateway PATH cannot."""
        from tools.environments.local import _sanitize_subprocess_env

        with patch(
            "tools.environments.local._resolve_sanremes_bin_dir",
    ...
    def test_bare_sanremes_path_does_not_duplicate_existing_install_dir(self):
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _SANREMES_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_SANREMES_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_SANREMES_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_SANREMES_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_SANREMES_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the SanRemes venv path instead of the project's own ``.venv`` —
    silently clobbering the SanRemes environment (and, when the other project
    pins a different Python, breaking the gateway outright). The SanRemes venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.sanremes/sanremes-agent/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/sanremes",
        })
...
def _physical_repo_root(tmp_path: Path) -> Path:
    ...
    physical_root = tmp_path / "physical-home" / "sanremes-agent"
    physical_root.mkdir(parents=True)
    return physical_root


class TestPythonpathSelectiveStrip:
    """PYTHONPATH SanRemes-owned entry stripping (#74817).

    The Desktop Electron app injects the SanRemes repo root and the SanRemes
    ...
    ``_strip_sanremes_owned_pythonpath`` surgically removes only the
    entries SanRemes itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact SanRemes-owned entries are removed; everything else survives
        verbatim (ordering, duplicates, empty components).

        Covers: the running venv's site-packages, the repo root (computed
        independently via parents[2] so an off-by-one in _sanremes_repo_root
        cannot silently pass), duplicate SanRemes entries, all-owned input
        (PYTHONPATH key removed), and mixed user/SanRemes ordering with an
        empty component preserved.
        """
        from tools.environments.local import _strip_sanremes_owned_pythonpath
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven SanRemes-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the SanRemes venv
        ...
        off-by-one in _sanremes_repo_root cannot silently pass; no launcher
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for SanRemes-owned entries.
        ...
        SanRemes-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and SanRemes venv site-packages entries are stripped
        by the same SanRemes-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local import _strip_sanremes_owned_pythonpath
        import sys
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        sanremes_win = f"C:\\\\Users\\\\u\\\\.sanremes\\\\sanremes-agent\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
            "PYTHONPATH": ";".join([sanremes_win, user_win]),
        }
        ...
        # the SanRemes-owned check cannot match a backslash path here.
        assert sanremes_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_sanremes_owned_paths_stripped(self):
        """On Windows, a SanRemes venv site-packages entry written with
        backslashes is stripped by the same SanRemes-owned check, while a
        ...
        _strip_sanremes_owned_pythonpath(env)
        entries = env["PYTHONPATH"].split(";")
        assert sanremes_win not in entries
        assert user_win in entries
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic SanRemes venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the SanRemes runtime producer contract.
        """
        import tools.environments.local as local

        repo_root = tmp_path / "sanremes-agent"
        ...
        monkeypatch.setattr(local, "_sanremes_repo_root_aliases", (repo_root,))
        monkeypatch.setattr(local, "_in_venv", False)
        monkeypatch.setattr(local, "_sanremes_site_packages", None)
    ...
    def test_unrelated_virtual_env_is_not_runtime_provenance(self, tmp_path, monkeypatch):
        ...
        local._strip_sanremes_owned_pythonpath(env)
    ...
    @pytest.mark.parametrize("builder", [
        ...
        "sanremes_subprocess_env",
    ])
    def test_builders_strip_sanremes_venv_pythonpath(self, builder):
        """Every subprocess env builder applies the same sanitation contract:
        SanRemes venv site-packages is stripped, user entries survive.
        """
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod.sanremes_subprocess_env()
    ...
    def test_scrub_child_env_strips_sanremes_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, SanRemes venv
        site-packages entries should be stripped when
        _strip_sanremes_owned_pythonpath is applied (as the spawn path does),
        ...
        _strip_sanremes_owned_pythonpath(scrubbed)
    ...
    def test_execute_code_composition_strips_inherited_sanremes_entries(self, same_env):
        """Integration: execute_code's real spawn path composes a clean PYTHONPATH.

        Seeds a contaminated inherited PYTHONPATH (SanRemes repo root + SanRemes
        ...
        * inherited SanRemes venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        sanremes_root = str(Path(cet.__file__).resolve().parents[1])
        venv_sp = str(_running_venv_site_packages())
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch("tools.code_execution_tool._uses_sanremes_python_environment",
                   return_value=same_env), \
             patch("subprocess.Popen", side_effect=_fake_popen), \
             patch.dict(os.environ, {
                 "PYTHONPATH": os.pathsep.join(
                     [sanremes_root, venv_sp, user_a, user_b]),
             }):
        ...
        norm_root = os.path.normcase(sanremes_root)
        ...
        assert norm_venv not in norm_parts, \
            "inherited SanRemes venv site-packages must be stripped"
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        import tools.environments.local as local
        from sanremes_cli.gateway_windows import _preserve_sanremes_home_path
        ...
        monkeypatch.setenv("SANREMES_HOME", str(configured_home))

        launcher_entry = Path(_preserve_sanremes_home_path(physical_root))
        aliases = local._build_sanremes_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        )

        assert launcher_entry == configured_home / "sanremes-agent"
        assert launcher_entry in aliases

        monkeypatch.setattr(local, "_sanremes_repo_root_aliases", aliases)
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        The desktop/CLI spawn children with SANREMES_HOME and PYTHONPATH in the
        configured (junction) spelling, but --profile / sticky active_profile
        re-home SANREMES_HOME through resolve_profile_env() before the
        ...
        """
        import tools.environments.local as local
        from sanremes_cli.profiles import resolve_profile_env

        physical_home = tmp_path / "physical-home"
        physical_root = physical_home / "sanremes-agent"
        physical_root.mkdir(parents=True)
        ...
        monkeypatch.setenv("SANREMES_HOME", str(configured_home))
        lexical_root = configured_home / "sanremes-agent"
        ...
        # The sanitizer now runs under the re-homed (profile) SANREMES_HOME.
        aliases = local._build_sanremes_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        (e.g. D:\\sanremes\\sanremes-agent -> C:\\...\\sanremes-agent) while the
        ...
        # repo-level link: <configured-home>/sanremes-agent -> physical repo
        try:
            _make_directory_link(configured_home / "sanremes-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "sanremes-agent").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "sanremes-agent"
        ...
        for lookalike in (configured_home / "sanremes-agent", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        is <root>/profiles/<name> while the repo is a link at <root>/sanremes-agent.
        ...
        try:
            _make_directory_link(configured_root / "sanremes-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "sanremes-agent"
        aliases = local._build_sanremes_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local._same_path(a, lexical_root) for a in aliases)
        assert not any(local._same_path(a, configured_home / "sanremes-agent") for a in aliases)
    ...
    def test_validated_runtime_venv_lexical_after_repo_recovery(self, tmp_path, monkeypatch):
        ...
        local._sanremes_site_packages = None
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the SanRemes runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the SanRemes venv and crashes
    ...
    def test_builders_strip_pythonhome(self, builder):
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod._sanitize_subprocess_env(dict(os.environ))
            elif builder == "sanremes_subprocess_env":
                result = local_mod.sanremes_subprocess_env()
            else:
    ...
    def test_build_subprocess_env_no_scrub_preserves_pythonhome(self):
        ...
        base = {
            ...
            "HOME": "/home/user",
            "PYTHONHOME": "/opt/sanremes-venv",
            "VIRTUAL_ENV": "/opt/sanremes-venv",
            "SERVICE_TOKEN": "s3cr3t",
        }
        result = build_subprocess_env(base, scrub_secrets=False)
        assert result.get("PYTHONHOME") == "/opt/sanremes-venv"
        assert result.get("VIRTUAL_ENV") == "/opt/sanremes-venv"
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_SANREMES_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not SanRemes (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _SANREMES_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _SANREMES_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their SanRemes-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _SANREMES_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to SanRemes, and blocklisting
        ...
        leaked_block = general_chain & _SANREMES_PROVIDER_ENV_BLOCKLIST
    ...
    def test_extra_auth_vars_covered(self):
        ...
        assert extras.issubset(_SANREMES_PROVIDER_ENV_BLOCKLIST)

    def test_claude_code_oauth_token_is_inheritable(self):
        """CLAUDE_CODE_OAUTH_TOKEN is owned by the user's Claude Code install
        (subscription OAuth), not a SanRemes inference credential. Stripping it
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _SANREMES_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _SANREMES_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_sanremes_bin_injection(self):
        """These tests assert the sane-path merge in isolation. Disable the
        sanremes-install-dir prepend (a separate concern, covered by
        TestSanRemesBinDirOnPath) so a real ``sanremes`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._SANREMES_BIN_DIR
        local_mod._SANREMES_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._SANREMES_BIN_DIR = saved
...
class TestSanRemesBinDirOnPath:
    """The sanremes install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``sanremes`` via the terminal tool must work
    even when the gateway was launched without the sanremes install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_sanremes_bin_dir / _prepend_sanremes_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._SANREMES_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/sanremes/bin/sanremes" if name == "sanremes" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/sanremes/bin")
        assert local_mod._resolve_sanremes_bin_dir() == "/opt/sanremes/bin"


    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._SANREMES_BIN_DIR = None
        assert local_mod._prepend_sanremes_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_sanremes_bin_dir(self):
        """A gateway env missing the sanremes dir gets it back in the subshell PATH.

        Platform-agnostic: ``_prepend_sanremes_bin_dir`` uses ``os.pathsep`` on
        every host, so no platform flag is faked here."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._SANREMES_BIN_DIR = "/opt/sanremes/bin"
        with patch.dict(
            os.environ,
            {"PATH": os.pathsep.join(["/usr/bin", "/bin"])},
            clear=True,
        ...
        assert entries[0] == "/opt/sanremes/bin"
        assert "/usr/bin" in entries


class TestSanRemesInternalDynamicSecrets:
    """Dynamically-named SanRemes secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_SANREMES_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    ``_is_sanremes_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``sanremes_subprocess_env``, Docker forward filter, ``env_passthrough``)
    consults it. These tests exercise the terminal execute path + predicate.
    """

    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local import _is_sanremes_internal_secret
        assert _is_sanremes_internal_secret("AUXILIARY_VISION_API_KEY")
        assert _is_sanremes_internal_secret("AUXILIARY_WEB_EXTRACT_API_KEY")
        assert _is_sanremes_internal_secret("AUXILIARY_APPROVAL_API_KEY")
        # plugin-registered task names are covered by the pattern
        assert _is_sanremes_internal_secret("AUXILIARY_MY_PLUGIN_TASK_API_KEY")

    def test_predicate_matches_auxiliary_base_url(self):
        from tools.environments.local import _is_sanremes_internal_secret
        assert _is_sanremes_internal_secret("AUXILIARY_VISION_BASE_URL")
        assert _is_sanremes_internal_secret("AUXILIARY_COMPRESSION_BASE_URL")

    def test_predicate_matches_gateway_relay_auth(self):
        from tools.environments.local import _is_sanremes_internal_secret
        assert _is_sanremes_internal_secret("GATEWAY_RELAY_SECRET")
        assert _is_sanremes_internal_secret("GATEWAY_RELAY_DELIVERY_KEY")
        assert _is_sanremes_internal_secret("GATEWAY_RELAY_SESSION_TOKEN")
    ...
    def test_predicate_allows_auxiliary_non_secrets(self):
        ...
        from tools.environments.local import _is_sanremes_internal_secret
        assert not _is_sanremes_internal_secret("AUXILIARY_VISION_PROVIDER")
        assert not _is_sanremes_internal_secret("AUXILIARY_VISION_MODEL")
        assert not _is_sanremes_internal_secret("GATEWAY_RELAY_URL")
        assert not _is_sanremes_internal_secret("GATEWAY_RELAY_PLATFORMS")
        assert not _is_sanremes_internal_secret("GATEWAY_RELAY_ID")  # not a secret suffix
        # unrelated vars pass through
        assert not _is_sanremes_internal_secret("PATH")
        assert not _is_sanremes_internal_secret("MY_APP_KEY")
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _SANREMES_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _SANREMES_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _SANREMES_PROVIDER_ENV_BLOCKLIST
