...
When ``SANREMES_EXEC_ASK`` leaks into a classic CLI process (historically via
...
class TestExecuteCodeGuardCliApprovalSurvivesExecAskLeak:
    ...
    def test_cli_callback_used_for_platform_marker_leak_without_exec_ask(
        ...
        ``SANREMES_SESSION_PLATFORM`` is set, so the whole-script gate is
        reached with ``SANREMES_EXEC_ASK`` entirely absent. That path must
...
print("EXEC_ASK=" + repr(os.environ.get("SANREMES_EXEC_ASK")))
""" % (str(REPO_ROOT),)
        sanremes_home = tmp_path / "import-test-home"
        proc = subprocess.run(
