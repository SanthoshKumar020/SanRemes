...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        ntfy_cfg = SimpleNamespace(enabled=True, token=None, extra={"topic": "sanremes-in"})
...
class TestSendToPlatformChunking:
    ...
    def test_signal_long_message_is_chunked(self, monkeypatch):
        ...
        The standalone path (sanremes send / cron / MCP) speaks raw JSON-RPC via
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert _call.args[2] == "hello from sanremes"
...
class TestParseTargetRef:
    ...
    def test_explicit_targets_round_trip_chat_and_thread(self):
        cases = [
            ...
            ("matrix", "@sanremes:matrix.org", "@sanremes:matrix.org", None),
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``sanremes cron``
    runs separately from ``sanremes gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``SANREMES_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``SANREMES_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``SANREMES_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
