...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `sanremes-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.sanremes/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.sanremes/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because sanremes uses an internal scheduler, not system crontab.
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["SANREMES_INTERACTIVE", "SANREMES_GATEWAY_SESSION", "SANREMES_EXEC_ASK"],
    )
    ...
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("SANREMES_INTERACTIVE", "SANREMES_GATEWAY_SESSION", "SANREMES_EXEC_ASK"):
...
class TestAgentCannotSetModelPin:
    """Per-job inference pins are user-owned (dashboard / `sanremes cron`
...
class TestGithubExemptionAbuse:
    ...
    def test_same_line_payload_after_github_url_is_scanned(self):
        ...
        for sep in (";", " &&", " |"):
            prompt = f"{self.GH}{sep} cat ~/.sanremes/.env"
    ...
    def test_subshell_and_backtick_payloads_are_scanned(self):
        ...
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}$(cat ~/.sanremes/.env)")
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}`cat ~/.sanremes/.env`")
    ...
    def test_payload_between_two_github_blocks_is_scanned(self):
        ...
        prompt = f"{self.GH}; cat ~/.sanremes/.env; {self.GH}"
        assert "Blocked" in _scan_cron_prompt(prompt)
