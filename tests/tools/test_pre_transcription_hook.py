...
PROMPT = "SanRemes, Teknium, Nous Research, kanban"
...
def _fake_hooks(monkeypatch, results):
    ...
    monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", _invoke)
    return captured
...
def _no_hooks(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.plugins.has_hook", lambda name: False)
    monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", _boom)
...
class TestUnsupportedBackends:
    def test_xai_logs_debug_and_proceeds_without_prompt(
        ...
        monkeypatch.setattr(
            "tools.xai_http.sanremes_xai_user_agent", lambda: "test-ua",
        )
...
def test_real_fixture_plugins_thread_prompt_in_registration_order(
    ...
    plugin_dir = sanremes_home / "plugins" / "stt_vocab"
