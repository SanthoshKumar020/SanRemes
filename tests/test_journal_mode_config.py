...
def _reset_configured_delete_override_warned_paths():
    ...
    sanremes_state._delete_overridden_warned_paths.clear()
    yield
    sanremes_state._delete_overridden_warned_paths.clear()
...
def test_resolve_journal_mode_uses_real_database_config(monkeypatch, tmp_path):
    from sanremes_state import resolve_journal_mode

    _configure_mode(monkeypatch, tmp_path, "DELETE")
    assert resolve_journal_mode() == "delete"


def test_new_nonsecret_sanremes_env_override_is_not_exposed(monkeypatch, tmp_path):
    from sanremes_state import resolve_journal_mode

    _configure_mode(monkeypatch, tmp_path, "wal")
    monkeypatch.setenv("SANREMES_JOURNAL_MODE", "delete")
...
def test_real_db_openers_honor_configured_delete(monkeypatch, tmp_path):
    ...
    from sanremes_cli import kanban_db, projects_db
    ...
    discord = DiscordRecoveryStore(sanremes_home=tmp_path)
