"""Tests for sanremes_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import sanremes_logging
# Use whatever RotatingFileHandler class sanremes_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. sanremes_logging
...
from sanremes_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    sanremes_logging._logging_initialized = False
    ...
    sanremes_logging._reset_queued_handlers()
    ...
    sanremes_logging._install_session_record_factory()
    yield
    ...
    sanremes_logging._logging_initialized = False
    sanremes_logging.clear_session_context()


@pytest.fixture
def sanremes_home(tmp_path, monkeypatch):
    """Provide an isolated SANREMES_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_sanremes_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["SANREMES_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, sanremes_home):
        log_dir = sanremes_logging.setup_logging(sanremes_home=sanremes_home)
        assert log_dir == sanremes_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, sanremes_home):
        sanremes_logging.setup_logging(sanremes_home=sanremes_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in sanremes_logging.rotating_file_handlers()
    ...
    def test_idempotent_no_duplicate_handlers(self, sanremes_home):
        sanremes_logging.setup_logging(sanremes_home=sanremes_home)
        sanremes_logging.setup_logging(sanremes_home=sanremes_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, sanremes_home):
        sanremes_logging.setup_logging(sanremes_home=sanremes_home)

        test_logger = logging.getLogger("test_sanremes_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        sanremes_logging.flush_log_queue()

        agent_log = sanremes_home / "logs" / "agent.log"
    ...
    def test_explicit_params_override_config(self, sanremes_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (sanremes_home / "config.yaml").write_text(yaml.dump(config))

        sanremes_logging.setup_logging(sanremes_home=sanremes_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, sanremes_home):
        sanremes_logging.setup_logging(sanremes_home=sanremes_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, sanremes_home):
        sanremes_logging.setup_logging(sanremes_home=sanremes_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, sanremes_home):
        ...
        gw_log = sanremes_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, sanremes_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, sanremes_home):
        sanremes_logging.setup_logging(sanremes_home=sanremes_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, sanremes_home):
        sanremes_logging.setup_logging(sanremes_home=sanremes_home, mode="gui")

        logging.getLogger("sanremes_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        sanremes_logging.flush_log_queue()

        gui_log = sanremes_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, sanremes_home):
        """When session context is set, log lines include [session_id]."""
        sanremes_logging.setup_logging(sanremes_home=sanremes_home)
        sanremes_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = sanremes_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, sanremes_home):
        sanremes_logging.setup_logging(sanremes_home=sanremes_home)
        sanremes_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_sanremes_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        sanremes_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in sanremes_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
        ...
        sanremes_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("sanremes_cli.config.is_managed", return_value=True):
                sanremes_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = sanremes_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert sanremes_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not sanremes_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, sanremes_home):
        level, max_size, backup = sanremes_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, sanremes_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> sanremes_logging._ManagedRotatingFileHandler:
        handler = sanremes_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that sanremes_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        sanremes_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        sanremes_logging.setup_logging(sanremes_home=sanremes_home, mode="gateway")
        gw_path = sanremes_home / "logs" / "gateway.log"
        rotated = sanremes_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = sanremes_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, sanremes_home):
        sanremes_logging.setup_logging(sanremes_home=sanremes_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_sanremes_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in sanremes_logging.rotating_file_handlers()
        )
