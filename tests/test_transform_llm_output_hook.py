...
from sanremes_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(sanremes_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <sanremes_home>/plugins/<name> and opt it in."""
    plugin_dir = sanremes_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "sanremes_empty"))
