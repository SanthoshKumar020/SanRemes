...
from sanremes_constants import reset_sanremes_home_override, set_sanremes_home_override
from sanremes_cli.active_sessions import active_session_registry_snapshot
from sanremes_cli.browser_connect import ChromeDebugLaunch
...
def test_compute_host_turn_end_updates_metadata_mirror(monkeypatch):
    ...
    import sanremes_cli.banner as _banner
...
def test_profile_scoped_mcp_discovery_uses_target_home(monkeypatch, tmp_path):
    """MCP discovery must start under the selected profile's SANREMES_HOME."""
    from sanremes_cli import mcp_startup
    ...
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "default"))
    token = set_sanremes_home_override(str(profile_home))
    ...
    monkeypatch.setattr(
        ...
        lambda: seen.append(str(get_sanremes_home())),
    )
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["SANREMES_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("SANREMES_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_VOICE", "1")
...
def test_prompt_submit_typed_stop_phrase_ends_voice_chat(monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_VOICE", "1")
    monkeypatch.setenv("SANREMES_VOICE_TTS", "1")
    ...
    assert os.environ["SANREMES_VOICE"] == "0"
    assert os.environ["SANREMES_VOICE_TTS"] == "0"
...
def test_wake_owner_is_sticky_and_routes_detection_to_first_transport(monkeypatch):
    ...
    try:
        ...
        assert emitted == [(
            "wake.detected",
            "first-session",
            {"phrase": "hey sanremes", "profile": None, "start_new_session": True},
            first,
        )]
...
def test_wake_toggle_persists_enabled_flag_only_on_explicit_gesture(monkeypatch):
    ...
    config = {"enabled": False, "phrase": "hey sanremes", "surface": "auto",
              "start_new_session": True}
    persisted = []
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_VOICE", "1")
    # setenv (not delenv) — the handler writes SANREMES_VOICE_TTS directly, and
    ...
    monkeypatch.setenv("SANREMES_VOICE_TTS", "0")
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("SANREMES_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("SANREMES_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "sanremes_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("SANREMES_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("SANREMES_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("SANREMES_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "sanremes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    import sanremes_cli.config as config_mod
    ...
    # _get_platform_tools (a non-configurable platform toolset in sanremes-cli's
    ...
    from sanremes_cli.tools_config import _RECENTLY_SHIPPED_TOOLSETS
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("SANREMES_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "sanremes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("SANREMES_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("SANREMES_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("SANREMES_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("SANREMES_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "sanremes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown SANREMES_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sanremes_state.SessionDB", lambda db_path=None: profile_db)
    monkeypatch.setattr(server, "_get_db", lambda: launch_db)
    monkeypatch.setattr(server, "_enable_gateway_prompts", lambda: None)
    monkeypatch.setattr(server, "_set_session_context", lambda target: [])
    monkeypatch.setattr(server, "_clear_session_context", lambda tokens: None)
...
def test_stored_session_runtime_overrides_skips_bare_billing_provider(monkeypatch):
    ...
    import sanremes_cli.runtime_provider as rp

    monkeypatch.setattr(rp, "load_config", lambda: cfg)
    monkeypatch.setattr("sanremes_cli.config.load_config", lambda: cfg)
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("SANREMES_MODEL", raising=False)
    monkeypatch.setenv("SANREMES_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("SANREMES_MODEL", raising=False)
    monkeypatch.delenv("SANREMES_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/sanremes-test "}}
    )

    assert server._resolve_model() == "nous/sanremes-test"
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set SANREMES_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("SANREMES_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("SANREMES_MODEL", raising=False)
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `sanremes --tui -m <model>` sets SANREMES_MODEL/SANREMES_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("SANREMES_MODEL", "one-shot/model")
    monkeypatch.setenv("SANREMES_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("SANREMES_MODEL", "seed/model")
    monkeypatch.setenv("SANREMES_INFERENCE_MODEL", "seed/model")
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.model_switch.switch_model", lambda **kw: result
    )
    monkeypatch.setattr(
        "sanremes_cli.model_switch.resolve_persist_behavior",
    ...
    monkeypatch.setattr(
        "sanremes_cli.model_cost_guard.expensive_model_warning",
        lambda *a, **k: None,
    )
    session = {"agent": None}
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("SANREMES_MODEL", "nous/sanremes-test")
    monkeypatch.setenv("SANREMES_TUI_PROVIDER", "nous")
    monkeypatch.delenv("SANREMES_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/sanremes-test", "nous")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("SANREMES_MODEL", "nous/sanremes-test")
    monkeypatch.delenv("SANREMES_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("SANREMES_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "sanremes_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nous/sanremes-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("SANREMES_MODEL", "sonnet")
    monkeypatch.delenv("SANREMES_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("SANREMES_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "sanremes_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with SanRemesCLI / gateway: fallback_providers stays first and keeps
...
def test_make_agent_passes_configured_fallback_chain(monkeypatch):
    ...
    monkeypatch.delenv("SANREMES_INFERENCE_MODEL", raising=False)
    monkeypatch.delenv("SANREMES_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("SANREMES_DESKTOP", raising=False)
    monkeypatch.delenv("SANREMES_DESKTOP_TERMINAL", raising=False)
    monkeypatch.setattr(
        server,
        "_load_cfg",
        lambda: {
    ...
    monkeypatch.setattr(
        "sanremes_cli.runtime_provider.resolve_runtime_provider",
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "sanremes_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "sanremes_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_superseded_by_resume_is_recoverable_end_reason():
    from sanremes_state_common import _RECOVERABLE_END_REASONS
...
def test_ensure_session_db_row_records_a_terminal_workspace(monkeypatch, tmp_path):
    ...
    The user cd'd there before running sanremes. Leaving it null stranded the row
...
def test_ensure_session_db_row_stamps_profile_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sanremes_state.SessionDB", _ProfileDB)
    monkeypatch.setattr(server, "_resolve_model", lambda: "test-model")
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_sanremes_home", tmp_path)
...
def test_config_get_approval_mode_uses_smart_default_when_key_is_missing(
    ...
    monkeypatch.setattr(server, "_sanremes_home", tmp_path)
    # Point the canonical resolver (load_config → env SANREMES_HOME) at the
    # temp home too, so the smart default is asserted against THIS config
    # rather than whatever the developer's real ~/.sanremes happens to hold.
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
...
def test_config_get_approval_mode_fails_safe_to_manual_for_invalid_explicit_value(
    ...
    # tools.approval, which reads via sanremes_cli.config.load_config —
    # that path resolves SANREMES_HOME from the environment, not
    # server._sanremes_home.
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_get_approval_mode_normalizes_yaml_off(tmp_path, monkeypatch):
    ...
    # load_config, which resolves SANREMES_HOME from the environment.
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_set_approval_mode_persists_three_way_value_and_emits_live_status(
    ...
    monkeypatch.setattr(server, "_sanremes_home", tmp_path)
    # config.set writes via server._sanremes_home, but the post-write
    # session.info emit resolves the effective mode through the canonical
    # tools.approval resolver (load_config → env SANREMES_HOME).
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    emitted = []
    monkeypatch.setattr(server, "_emit", lambda *args: emitted.append(args))
...
def test_config_set_fast_updates_live_agent_session_scoped(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("SANREMES_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("SANREMES_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("SANREMES_EXEC_ASK", raising=False)
    monkeypatch.delenv("SANREMES_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("SANREMES_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["SANREMES_GATEWAY_SESSION"] == "1"
    assert server.os.environ["SANREMES_EXEC_ASK"] == "1"
    assert server.os.environ["SANREMES_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("sanremes_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("sanremes_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "sanremes_cli.runtime_provider.resolve_runtime_provider",
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "sanremes_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=_Agent())
    monkeypatch.setattr("sanremes_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_recovers_failed_profile_resume_after_build_completes(
    ...
    monkeypatch.setenv("SANREMES_HOME", str(launch_home))
    monkeypatch.delenv("SANREMES_MODEL", raising=False)
    monkeypatch.delenv("SANREMES_INFERENCE_MODEL", raising=False)
    ...
    monkeypatch.setattr("sanremes_cli.model_switch.switch_model", fake_switch_model)
    monkeypatch.setattr(
        "sanremes_cli.model_selection_guards.combined_selection_warning",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr("sanremes_state.SessionDB", FakeDb)
    monkeypatch.setattr(server, "_make_agent", fake_make_agent)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing SANREMES_INFERENCE_PROVIDER on a switch leaked the new
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("SANREMES_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "sanremes_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["SANREMES_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "SANREMES_TUI_PROVIDER" not in os.environ
        assert "SANREMES_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write SANREMES_MODEL / SANREMES_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("SANREMES_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("SANREMES_MODEL", raising=False)
    monkeypatch.delenv("SANREMES_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    try:
        ...
        assert os.environ["SANREMES_TUI_PROVIDER"] == "openai-codex"
        assert "SANREMES_MODEL" not in os.environ
        assert "SANREMES_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("SANREMES_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("SANREMES_MODEL", "old/model")
    monkeypatch.setattr(
        "sanremes_cli.model_switch.switch_model",
    ...
    try:
        ...
        assert os.environ["SANREMES_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["SANREMES_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_config_set_model_session_switch_clears_pending_once_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setattr("sanremes_cli.model_switch.switch_model", lambda **_kwargs: result)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda *args, **kwargs: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_personality_preserves_history_and_returns_info(monkeypatch):
    ...
    monkeypatch.setattr(server, "_emit", lambda *args: emits.append(args))
    # Persistence now flows through the single owner (sanremes_cli.personality),
    # never _write_config_key / agent.system_prompt.
    import sanremes_cli.personality as personality_mod
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        # attachments dir.
        assert not (workspace / ".sanremes" / "desktop-attachments").exists()
...
def test_commands_catalog_includes_plugin_commands(monkeypatch):
    monkeypatch.setattr(
        "sanremes_cli.plugins.get_plugin_commands",
        lambda: {
            "lcm": {
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "SanRemes TUI Status" in out
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("sanremes_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "sanremes_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("sanremes_state")
    ...
    monkeypatch.setitem(sys.modules, "sanremes_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_sanremes_home
    # fixture pins SANREMES_HOME to a temp dir; the handler should append
    # /sessions to it.
...
def test_session_list_honors_params_profile_opens_profile_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("sanremes_state.SessionDB", ProfileDB)
...
def test_session_most_recent_honors_params_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_profile_home", lambda p: profile_home if p == "mlperf" else None)
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("sanremes_state.SessionDB", ProfileDB2)
...
def test_session_branch_installs_parent_profile_secret_scope(monkeypatch, tmp_path):
    ...
    session.branch already binds the parent's SANREMES_HOME and state.db, but the
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `sanremes model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `sanremes model` and the classic CLI /model picker.
    ...
    with patch(
        "sanremes_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("sanremes_cli.models.provider_model_ids") as live_fetch:
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    from sanremes_cli.inventory import ConfigContext

    calls = []

    monkeypatch.setattr(server, "_resolve_model", lambda: "")
    monkeypatch.setattr(
        "sanremes_cli.inventory.load_picker_context",
    ...
    monkeypatch.setattr(
        "sanremes_cli.inventory.build_models_payload",
...
def test_model_options_preserves_canonical_custom_row_after_agent_init(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.runtime_provider.canonical_custom_identity",
        canonical,
    )
    monkeypatch.setattr(
        "sanremes_cli.model_switch.list_authenticated_providers",
    ...
    monkeypatch.setattr(
        "sanremes_cli.auth.is_provider_explicitly_configured",
        lambda _slug: False,
    )
    monkeypatch.setattr("sanremes_cli.inventory._apply_pricing", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("sanremes_cli.inventory._apply_capabilities", lambda *_args, **_kwargs: None)
...
def test_model_save_key_uses_credential_lifecycle_and_picker_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.auth.PROVIDER_REGISTRY",
        {
    ...
    monkeypatch.setattr("sanremes_cli.config.is_managed", lambda: False)
    save_credential = Mock()
    monkeypatch.setattr(
        "sanremes_cli.credential_lifecycle.save_provider_env_credential",
        save_credential,
    )
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``sanremes --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_verification_status_returns_recorded_evidence(tmp_path, monkeypatch):
    ...
    token = set_sanremes_home_override(profile_home)
    project = tmp_path / "project"
    project.mkdir()
    (project / ".git").mkdir()
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"sanremes_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "sanremes_cli.browser_connect.launch_chrome_debug",
                return_value=ChromeDebugLaunch(),
            ),
            patch("sanremes_cli.browser_connect.local_port_in_use", return_value=False),
            patch("sanremes_cli.browser_connect.manual_chrome_debug_command", return_value=None),
            patch(
                "sanremes_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_squatted_port_launches_on_alternate(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with (
            patch("sanremes_cli.browser_connect.launch_chrome_debug", side_effect=_launch),
            patch("sanremes_cli.browser_connect.local_port_in_use", return_value=True),
            patch("sanremes_cli.browser_connect.find_free_debug_port", return_value=9223),
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_sanremes_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.sanremes/.env
    ...
    with patch.dict(sys.modules, {"sanremes_cli.config": fake}):
...
def test_session_save_writes_under_sanremes_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the SanRemes profile
    ...
    Regression: the gateway handler wrote ``sanremes_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are SanRemes.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("sanremes_conversation_*.json"))
    ...
    assert payload["model"] == "sanremes-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are SanRemes."
...
def test_reap_idle_sessions_calls_periodic_trim(monkeypatch):
    ...
    # `from sanremes_cli.mem_trim import trim_memory` at call time.
    import sanremes_cli.mem_trim as mem_trim
...
def test_billing_error_serialization_preserves_server_code(
    ...
    import sanremes_cli.nous_billing as nb
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from get_sanremes_home() (live
    # env var), always targeting SANREMES_HOME/config.yaml — point it at tmp_path.
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    monkeypatch.setattr(cli, "_sanremes_home", tmp_path)
...
def test_tts_stream_begin_and_stop_lifecycle(monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_VOICE_TTS", "1")
    monkeypatch.setenv("SANREMES_VOICE", "0")  # no barge-in monitor (no mic)
    playback_stops: list = []
...
def test_full_duplex_stop_phrase_mid_generation_ends_voice_chat(monkeypatch, tmp_path):
    ...
    assert os.environ.get("SANREMES_VOICE") == "0"  # voice chat ended
...
def test_build_persist_message_quotes_paths_containing_spaces(tmp_path):
    ...
    img_dir = tmp_path / "Application Support" / "SanRemes" / "composer-images"
...
def test_prompt_submit_releases_old_history_before_heap_trim(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(server, "render_message", lambda _t, _c: "")
        monkeypatch.setattr(server, "_emit", lambda *a: None)
        monkeypatch.setattr(server, "set_sanremes_home_override", lambda _home: object())
        monkeypatch.setattr(
            server,
            "reset_sanremes_home_override",
            lambda _token: cleanup_order.append("reset_home"),
        )
        monkeypatch.setattr("sanremes_cli.mem_trim.trim_memory", _inspect_trim_frame)
...
# silently rewrote ~/.sanremes/config.yaml top-to-bottom — top-level keys
...
def test_prompt_submit_truncation_archives_instead_of_deleting(monkeypatch):
    ...
    tests/sanremes_state/test_replace_messages_archive_siblings.py.
    """
...
def test_persist_live_session_system_prompt_uses_profile_home(monkeypatch, tmp_path):
    ...
    SANREMES_HOME to the session's profile before rebuilding the system
    ...
    # to the profile home, not the root ~/.sanremes.
    ...
    assert get_sanremes_home_override() is None
...
def test_persist_live_session_system_prompt_restores_pre_existing_override(tmp_path):
    """reset_sanremes_home_override() restores the previous ContextVar state,
    ...
    class FakeAgent:
        ...
        def _build_system_prompt(self, system_message=None):
            from sanremes_constants import get_sanremes_home
            built_homes.append(str(get_sanremes_home()))
            return "inner prompt"
    ...
    outer_token = set_sanremes_home_override(outer_home)
    try:
        server._persist_live_session_system_prompt(session)
        ...
        # to None.
        assert get_sanremes_home_override() == str(outer_home)
    finally:
        reset_sanremes_home_override(outer_token)
    assert get_sanremes_home_override() is None
