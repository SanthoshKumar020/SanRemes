...
binary download are exercised manually by `sanremes secrets bitwarden
...
def sanremes_home(tmp_path, monkeypatch):
    """Point SanRemes at an isolated home directory."""
    home = tmp_path / ".sanremes"
    home.mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(home))
    # Some modules cache get_sanremes_home; clear if needed.
    import sanremes_constants
    if hasattr(sanremes_constants, "_SANREMES_HOME_CACHE"):
        sanremes_constants._SANREMES_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(sanremes_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from sanremes_cli.env_loader import _apply_external_secret_sources
