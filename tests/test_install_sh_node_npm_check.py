...
def test_check_node_requires_npm_alongside_node() -> None:
    ...
    # falls through to installing the SanRemes-managed Node (which bundles npm).
    assert "node found but npm is not on PATH (stray node symlink?)" in text


def test_check_node_managed_requires_npm() -> None:
    """The SanRemes-managed Node fallback also requires its npm to exist."""
    text = INSTALL_SH.read_text()
    assert (
        '[ -x "$SANREMES_HOME/node/bin/node" ] && [ -x "$SANREMES_HOME/node/bin/npm" ] \\'
        in text
    )
