"""Unit tests for ``sanremes_cli.proxy_cli`` command handlers.
...
from sanremes_cli import proxy_cli


@pytest.fixture
def sanremes_home(tmp_path, monkeypatch):
    """Point SANREMES_HOME at a temp dir so the wizard doesn't touch the
...
def test_cmd_install_success_returns_0(sanremes_home, monkeypatch):
    monkeypatch.setattr(ip, "install_iron_proxy", lambda **kw: sanremes_home / "iron-proxy")
    monkeypatch.setattr(ip, "iron_proxy_version", lambda b: "v0.39.0-test")
    rc = proxy_cli.cmd_install(_args())
    assert rc == 0


def test_cmd_install_failure_returns_1(sanremes_home, monkeypatch):
    def boom(**kw):
...
def test_cmd_setup_from_bitwarden_refuses_on_empty_vault(sanremes_home, monkeypatch):
    ...
    monkeypatch.setattr(ip, "find_iron_proxy", lambda **kw: sanremes_home / "iron-proxy")
    monkeypatch.setattr(ip, "iron_proxy_version", lambda b: "test")
    monkeypatch.setattr(
        ip, "ensure_ca_cert",
        lambda **kw: (sanremes_home / "ca.crt", sanremes_home / "ca.key"),
    )
...
def test_cmd_restart_propagates_start_failure(sanremes_home, monkeypatch):
...
# ---------------------------------------------------------------------------
# _load_env_file_into_environ — setup discovers keys kept only in ~/.sanremes/.env
# ---------------------------------------------------------------------------
...
def test_cmd_status_returns_0(sanremes_home, monkeypatch):
...
def test_cmd_disable_uses_public_status_pid_not_private_read_pid(
    ...
    # mutation only.
    from sanremes_cli.config import load_config as _lc
    cfg2 = _lc()
    assert cfg2["proxy"]["enabled"] is False


def test_cmd_config_returns_0_when_present(sanremes_home, monkeypatch):
    fake = ip.ProxyStatus()
    fake.config_path = sanremes_home / "proxy.yaml"
...
def test_register_cli_uses_egress_command_dest():
    ...
    the inbound OAuth 'sanremes proxy' subparser (dest='proxy_command').
    A future grep-and-refactor on proxy_command should not hit this
    subparser by accident."""

    parser = argparse.ArgumentParser(prog="sanremes egress")
...
def test_cmd_start_refuses_when_bitwarden_mode_but_disabled(sanremes_home, monkeypatch):
...
def test_cmd_setup_audit_log_failure_is_warning_not_abort(sanremes_home, monkeypatch):
    ...
    monkeypatch.setattr(ip, "discover_provider_mappings", lambda **kw: [
        ip.TokenMapping(
            proxy_token="sanremes-proxy-deadbeef",
