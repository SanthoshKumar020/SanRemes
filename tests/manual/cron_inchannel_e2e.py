...
Uses a throwaway SANREMES_HOME so it never touches ~/.sanremes. No real names.
"""
...
def _fresh_home():
    """Point SANREMES_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_sanremes_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["SANREMES_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER SANREMES_HOME is set.
...
# a different get_sanremes_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SANREMES_HOME (throwaway): {HOME}")
