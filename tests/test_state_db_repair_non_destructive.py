...
from sanremes_state import repair_state_db_schema
...
def _probe_repair_lock_from_child(db_path: str, result) -> None:
    ...
    sanremes_state._REPAIR_LOCK_TIMEOUT_SECONDS = 0.5
    with sanremes_state._cross_process_repair_lock(Path(db_path)) as holding:
        result.put(holding)
...
def test_strategies_never_receive_the_live_database(corrupt_db, monkeypatch):
    ...
    seen: list[Path] = []
    real = sanremes_state._run_repair_strategies

    def spy(path, report):
        seen.append(path)
        return real(path, report)

    monkeypatch.setattr(sanremes_state, "_run_repair_strategies", spy)
...
def test_successful_repair_is_promoted_over_the_original(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sanremes_state, "_db_opens_cleanly", lambda path: "forced-unhealthy"
    )
    ...
    monkeypatch.setattr(sanremes_state, "_run_repair_strategies", fake_strategies)
...
def test_committed_writer_after_staging_is_never_lost(
    ...
    monkeypatch.setattr(
        sanremes_state, "_db_opens_cleanly", lambda _path: "forced-unhealthy"
    )
    ...
    monkeypatch.setattr(sanremes_state, "_run_repair_strategies", staged_strategy)
...
def test_environmental_aborts_do_not_burn_repair_ledger(tmp_path, monkeypatch):
    ...
    ledger_path = sanremes_state._repair_ledger_path(db)
    assert not ledger_path.exists(), "environmental aborts must not consume attempts"

    monkeypatch.setattr(sanremes_state, "_repair_scratch_space_error", lambda _path: None)
    ...
    monkeypatch.setattr(sanremes_state, "_run_repair_strategies", successful_strategy)
    report = repair_state_db_schema(db, backup=False)
    assert report["repaired"] is True
...
def test_actual_strategy_failure_still_consumes_one_attempt(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sanremes_state, "_run_repair_strategies", failed_strategy)
    report = repair_state_db_schema(db, backup=False)
    assert report["repaired"] is False
    ledger = sanremes_state._read_repair_ledger(db)
...
def test_repair_outcome_is_recorded_while_cross_process_lock_is_held(
    ...
    real_repair_lock = sanremes_state._cross_process_repair_lock
    ...
    monkeypatch.setattr(
        sanremes_state, "_cross_process_repair_lock", tracking_repair_lock
    )
    ...
    monkeypatch.setattr(sanremes_state, "_record_repair_outcome", record_outcome)
    report = repair_state_db_schema(db, backup=False)
...
def test_exhaustion_is_rechecked_after_acquiring_repair_lock(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sanremes_state, "_persistent_repair_attempts_exhausted", exhaustion_probe
    )
    monkeypatch.setattr(sanremes_state, "_live_writer_holds_db", lambda _path: False)
    surgery_calls = []
    ...
    monkeypatch.setattr(
        sanremes_state, "_repair_state_db_schema_locked", unexpected_surgery
    )
...
def test_scratch_budget_counts_sidecars_in_vacuum_multiplier(tmp_path, monkeypatch):
    ...
    headroom = sanremes_state._repair_backup_headroom_bytes(total)
    ...
    error = sanremes_state._repair_scratch_space_error(db)
...
def test_environmental_promotion_failures_do_not_burn_ledger(
    ...
    monkeypatch.setattr(sanremes_state, "_run_repair_strategies", successful_strategy)
    real_copy = sanremes_state._copy_database_snapshot
    copy_calls = 0
    ...
    monkeypatch.setattr(
        sanremes_state, "_copy_database_snapshot", fail_promotion_three_times
    )
    for _ in range(3):
        report = repair_state_db_schema(db, backup=False)
        assert report["repaired"] is False
    ...
    monkeypatch.setattr(sanremes_state, "_copy_database_snapshot", real_copy)
    report = repair_state_db_schema(db, backup=False)
    assert report["repaired"] is True
...
def test_corrupt_promotion_failure_consumes_one_attempt(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sanremes_state, "_copy_database_snapshot", fail_promotion_with_corruption
    )
    report = repair_state_db_schema(db, backup=False)

    assert report["repaired"] is False
    assert sanremes_state._read_repair_ledger(db)["failed_attempts"] == 1
...
def test_snapshot_includes_committed_wal_frames(tmp_path):
    ...
    try:
        ...
        sanremes_state._copy_database_snapshot(db, scratch)
...
def test_snapshot_deadline_has_a_floor_and_scales_with_source_size(
    ...
    deadline = getattr(sanremes_state, "_repair_snapshot_timeout_seconds", None)
    ...
    monkeypatch.setattr(
        sanremes_state, "_REPAIR_SNAPSHOT_MIN_THROUGHPUT_BYTES_PER_SECOND", 256 * 1024
    )
    ...
    large = deadline(db)
    assert small >= sanremes_state._REPAIR_LOCK_TIMEOUT_SECONDS
    assert large > small
...
def test_transactional_promotion_preserves_a_live_wal_reader(tmp_path):
    ...
    try:
        ...
        sanremes_state._copy_database_snapshot(scratch, live)
...
def test_interrupted_snapshot_rolls_back_destination(tmp_path, monkeypatch):
    ...
    ticks = iter((0.0, sanremes_state._REPAIR_LOCK_TIMEOUT_SECONDS + 1.0))
    monkeypatch.setattr(sanremes_state.time, "monotonic", lambda: next(ticks))

    with pytest.raises(TimeoutError):
        sanremes_state._copy_database_snapshot(source, destination)
...
def test_failed_promotion_returns_failure_and_preserves_original(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sanremes_state, "_copy_database_snapshot", fail_second_copy)
    monkeypatch.setattr(sanremes_state, "_run_repair_strategies", fake_strategies)
...
def test_scratch_space_guard_accounts_for_snapshot_and_vacuum(tmp_path, monkeypatch):
    ...
    headroom = sanremes_state._repair_backup_headroom_bytes(10_000_000_000)
...
def test_stale_scratch_is_removed_before_health_check(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sanremes_state, "_db_opens_cleanly", fake_health)
...
def test_stale_scratch_is_removed_before_space_check(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sanremes_state, "_repair_scratch_space_error", fake_space_check
    )
...
def test_stale_scratch_cleanup_failure_aborts_before_probe(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sanremes_state, "_db_opens_cleanly", fake_health)
    monkeypatch.setattr(
        sanremes_state, "_unlink_db_triple", lambda _path: "scratch is locked"
    )
    report = {
        "repaired": False,
        "strategy": None,
    ...
    result = sanremes_state._repair_state_db_schema_locked(
