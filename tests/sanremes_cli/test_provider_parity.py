...
the SAME provider universe as ``sanremes model`` (the CLI/TUI picker).
...
def test_every_sanremes_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `sanremes model` universe."""
    ...
    assert not missing, (
        "providers shown in `sanremes model` but not configurable in the desktop "
