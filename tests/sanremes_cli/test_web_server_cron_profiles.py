...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    monkeypatch.setattr(scheduler, "_sanremes_home", None)
    captured = {}

    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            captured["job_id"] = job_id
            captured["runtime_home"] = scheduler._get_sanremes_home()
    ...
    outer_token = set_sanremes_home_override(default_home)
    try:
        ...
        assert scheduler._get_sanremes_home() == default_home
    finally:
...
def test_create_registers_scheduler_inside_target_profile(
    ...
    class RecordingProvider(CronScheduler):
        ...
        def register_job(self, job):
            captured["job"] = job
            captured["runtime_home"] = get_sanremes_home()
            captured["jobs_file"] = cron_jobs._current_cron_store().jobs_file
...
async def test_dashboard_cron_noop_inference_fields_keep_existing_snapshots(
    ...
    from sanremes_cli import runtime_provider, web_server
...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    ``~/.sanremes`` when the request carries no explicit ``profile`` (the
    Desktop app's pre-profileScoped clients sent none)."""
    from sanremes_cli import web_server

    monkeypatch.setenv(
        "SANREMES_HOME", str(isolated_profiles["worker_alpha"])
    )
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """SANREMES_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from sanremes_cli import web_server

    monkeypatch.setenv("SANREMES_HOME", str(isolated_profiles["default"]))
