"""Tests for the unified provider catalog (sanremes_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``sanremes model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from sanremes_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
