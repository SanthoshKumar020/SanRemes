...
from sanremes_cli.auth import resolve_provider
...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from sanremes_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # sanremes model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        from sanremes_cli.providers import SANREMES_OVERLAYS

        assert "gmi" in SANREMES_OVERLAYS
        overlay = SANREMES_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    def test_provider_env_hints_include_gmi(self):
        from sanremes_cli.doctor import _PROVIDER_ENV_HINTS
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a SanRemesAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("SanRemesAgent/")

    def test_gmi_profile_declares_sanremes_user_agent(self):
        """The GMI plugin sets a SanRemesAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("SanRemesAgent/"), (
            f"expected GMI profile User-Agent to start with 'SanRemesAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "sanremes_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["sanremes", "chat", "--provider", "gmi"])

        from sanremes_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("sanremes_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("sanremes_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)

        from sanremes_cli.main import select_provider_and_model
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            from sanremes_cli.main import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_sanremes_home() / "config.yaml").read_text()) or {}
