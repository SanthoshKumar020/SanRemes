...
from sanremes_cli import gateway as sanremes_gateway
from sanremes_cli import gateway_windows
from sanremes_cli import main as cli_main
from sanremes_cli import process_identity
...
def test_control_plane_argv_is_not_a_gateway():
    ...
    serve = "C:\\SanRemes\\.venv\\Scripts\\python.exe -m sanremes_cli.main serve --host 127.0.0.1"
    run = "C:\\SanRemes\\.venv\\Scripts\\python.exe -m sanremes_cli.main gateway run"
...
def test_control_plane_classifier_is_token_based_not_substring():
    ...
    py = "C:\\SanRemes\\.venv\\Scripts\\python.exe -m sanremes_cli.main"
...
def test_pause_skips_cold_start_plan_when_desktop_owns_lifecycle(monkeypatch):
    ...
    monkeypatch.setattr(sanremes_gateway, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
        sanremes_gateway, "find_windows_gateway_services", lambda **_k: []
    )
