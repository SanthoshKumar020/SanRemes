"""Tests for the stale-dashboard handling run at the end of ``sanremes update``.

``sanremes update`` detects ``sanremes dashboard`` processes left over from the
...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``sanremes_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``sanremes_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from sanremes_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"sanremes_cli.main.X\")``
    ...
    live = sys.modules.get("sanremes_cli.main")
    if live is None:
        live = importlib.import_module("sanremes_cli.main")
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "sanremesPath": "/opt/sanremes/bin/sanremes",
        "sanremesHome": str(tmp_path),
    ...
    monkeypatch.delenv("SANREMES_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "sanremes_cli.main._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m sanremes_cli.main dashboard"),
                    _ps_line(12345, "sanremes dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("sanremes_cli.main._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "sanremes-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="sanremes-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "sanremes-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "sanremes-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "sanremes-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("sanremes_cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "sanremes-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "sanremes-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "sanremes-dashboard.service"],
            ["systemctl", "--user", "restart", "sanremes-dashboard.service"],
        ]
        assert all(call[:1] != ["sudo"] and call[:2] != ["systemctl"] for call in calls)
        find_pids.assert_not_called()
        kill.assert_not_called()
        assert "✓ restarted sanremes-dashboard.service" in capsys.readouterr().out
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "sanremes_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    `sanremes update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("sanremes_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m sanremes_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("sanremes_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["sanremes_cli.main"]
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(live, "_get_pid_cgroup_path",
                          return_value="/system.slice/sanremes-serve.service"), \
             patch.object(live, "_get_systemd_service_for_pid",
                          return_value="sanremes-serve.service"), \
        ...
        restart.assert_called_once_with(
            "sanremes-serve.service", "/system.slice/sanremes-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service sanremes-serve.service" in out
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: sanremes update's systemd fleet-restart loop may
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"sanremes-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        live = self._live()
        argv = ["sanremes", "dashboard", "--port", "8300"]
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("sanremes_cli.dashboard_procs._sanremes_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["sanremes", "dashboard", "--port", "8300"],
                ["sanremes", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["sanremes", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["sanremes", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["sanremes", "serve"]])

        assert failed == [["sanremes", "serve"]]
        out = capsys.readouterr().out
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from sanremes_cli.dashboard_procs import _filter_dashboard_respawn_candidates
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.sanremes/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["sanremes", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from sanremes_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "sanremes_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "sanremes_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["sanremes", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["sanremes", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["sanremes", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_sanremes_home(self):
        from sanremes_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/sanremes-home-a"
        a = ["sanremes", "dashboard", "--port", "8300"]
        b = ["sanremes", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.sanremes/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["sanremes", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["sanremes", "dashboard", "--port", "8301"]
        home = "/home/u/.sanremes"
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another SANREMES_HOME is never respawned (#94030).
        ...
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, "/home/u/.sanremes"),
                (2, b, "/work/project/.sanremes"),
            ],
            own_home="/home/u/.sanremes",
        )
        assert out == [a]
    ...
    def test_skips_sidecar_fixed_port_serve_on_foreign_home(self):
        ...
        argv = [
            "/Users/u/.sanremes-sidecar/sanremes-agent/venv/bin/python",
            "-m", "sanremes_cli.main",
            "serve", "--host", "127.0.0.1", "--port", "9118", "--skip-build",
        ]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.sanremes-lifeos")],
            own_home="/Users/u/.sanremes",
        )
        assert out == []

    def test_matching_sanremes_home_is_kept(self):
        from sanremes_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        argv = ["sanremes", "serve", "--host", "127.0.0.1", "--port", "9118"]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.sanremes")],
            own_home="/Users/u/.sanremes",
        )
        assert out == [argv]

    def test_symlinked_sanremes_home_compares_equal(self, tmp_path):
        from sanremes_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable SANREMES_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from sanremes_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_sanremes_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            sanremes_constants, "get_sanremes_home", lambda: Path("/home/u/.sanremes")
        )
        argv = ["sanremes", "serve", "--port", "9118"]
        foreign = _filter_dashboard_respawn_candidates([
            (1, argv, "/Users/u/.sanremes-lifeos"),
        ])
        assert foreign == []
        own = _filter_dashboard_respawn_candidates([
            (1, argv, "/home/u/.sanremes"),
        ])
        assert own == [argv]
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["sanremes", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00sanremes_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "sanremes_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``sanremes update`` runs in the PRE-pull Python process. When the update
    adds a new symbol to ``sanremes_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from sanremes_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_cleanup_reloads_before_scanning(self):
        ...
        from sanremes_cli import update_cmd
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("sanremes_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import sanremes_cli._subprocess_compat as compat
        from sanremes_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["sanremes_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["sanremes_cli._subprocess_compat"])
            importlib.reload(sys.modules["sanremes_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "sanremes_cli._subprocess_compat" in reloaded
        assert "sanremes_cli.dashboard_procs" in reloaded
