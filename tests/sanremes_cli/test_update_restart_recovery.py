...
def test_abort_recovery_hands_managed_profiles_to_a_fresh_process(monkeypatch):
    ...
    assert argv[1:4] == ["-m", "sanremes_cli.update_restart_recovery", "--stdin"]
    ...
    assert kwargs["env"]["SANREMES_UPDATE_RESTART_RECOVERY"] == "1"
...
def test_service_matching_is_exact_for_overlapping_profile_names():
    assert update_cmd._gateway_service_matches_profile(
        "foo", "sanremes-gateway-foo.service"
    )
    assert not update_cmd._gateway_service_matches_profile(
        "foo", "sanremes-gateway-foobar.service"
    )
    assert update_cmd._gateway_service_matches_profile(
        "default", "ai.sanremes.gateway"
    )
    assert not update_cmd._gateway_service_matches_profile(
        "default", "ai.sanremes.gateway-foo"
    )


def test_recovery_child_restarts_each_profile_with_a_fresh_main(monkeypatch):
    recovery = importlib.import_module("sanremes_cli.update_restart_recovery")
    calls = []
    ...
    assert [call[0] for call in calls] == [
        [sys.executable, "-m", "sanremes_cli.main", "-p", "coder", "gateway", "restart"],
        [sys.executable, "-m", "sanremes_cli.main", "-p", "default", "gateway", "restart"],
    ]
    for _, kwargs in calls:
        ...
        assert kwargs["check"] is False
        assert kwargs["env"]["SANREMES_UPDATE_RESTART_RECOVERY"] == "1"
        assert "_SANREMES_GATEWAY" not in kwargs["env"]
...
def test_recovery_child_verifies_systemd_profiles_via_is_active(monkeypatch):
    ...
    def fake_run(argv, **kwargs):
        ...
        if argv[0].endswith("systemctl"):
            unit = argv[-1]
            active = unit == "sanremes-gateway.service"
...
def test_recovery_module_empty_payload_is_a_real_clean_process():
    result = subprocess.run(
        [sys.executable, "-m", "sanremes_cli.update_restart_recovery", "--stdin"],
...
def test_recovery_module_end_to_end_in_a_real_fresh_process(tmp_path):
    ...
    grandchild ``sanremes_cli.main … gateway restart`` invocations (recording
    ...
    shim = textwrap.dedent(
        ...
        def _shim_run(argv, *args, **kwargs):
            argv_list = list(argv)
            if "sanremes_cli.main" in argv_list:
            ...
            if argv_list and str(argv_list[0]).endswith("systemctl"):
                unit = argv_list[-1]
                if unit == "sanremes-gateway.service":
    ...
    env["_SANREMES_GATEWAY"] = "1"  # must be scrubbed before the grandchild runs
