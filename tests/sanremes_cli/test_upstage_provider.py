...
Regression guard for the bug where `sanremes model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`sanremes_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in SANREMES_OVERLAYS
        overlay = SANREMES_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `sanremes_cli/main.py` (cmd_model): read
