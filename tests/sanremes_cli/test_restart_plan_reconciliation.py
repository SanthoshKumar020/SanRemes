...
from sanremes_cli.update_inventory import (
    RuntimeRecord,
    UpdatePlan,
    _restart_mechanism,
...
def test_mechanism_ids_are_machine_readable_and_described():
    ...
    assert describe_restart_mechanism("manual", "default") == "sanremes gateway restart"
    assert "sc.exe" in describe_restart_mechanism("windows-service", "default")


def test_windows_service_supervisor_classification():
    from sanremes_cli.update_inventory import _detect_supervisor_for_pid
...
def test_windows_service_runtime_reconciles_via_service_profiles():
    ...
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["sanremes-gateway"], relaunched_profiles=["default"],
...
def test_failed_unit_is_failed():
    outcomes = match_runtime_outcomes(
        ...
        failed_units=["sanremes-gateway-work.service"],
    )
...
def test_restarted_service_unit_matches_profile():
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["sanremes-gateway.service"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
...
def test_untouched_runtime_is_unaccounted_and_escalates(capsys):
    ...
    outcomes = match_runtime_outcomes(
        _plan(_rt("coder", 500)),
        restarted_services=["sanremes-gateway.service"],
