...
- Bare `sanremes setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `sanremes setup --quick` runs the narrower "fill in missing items" flow.
- `sanremes setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("sanremes_cli.setup.ensure_sanremes_home", {}),
        ("sanremes_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("sanremes_cli.config.is_managed", {"return_value": False}),
        ("sanremes_cli.setup.load_config", {"return_value": {}}),
        ("sanremes_cli.setup.save_config", {}),
        ("sanremes_cli.setup.get_env_value", {"return_value": None}),
        ("sanremes_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("sanremes_cli.setup._print_setup_summary", {}),
        ("sanremes_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("sanremes_cli.setup.load_config", {"return_value": {}}),
        ("sanremes_cli.setup.save_config", {}),
        ("sanremes_cli.auth.get_active_provider", {"return_value": None}),
        ("sanremes_cli.setup.get_env_value", {"return_value": None}),
        ("sanremes_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `sanremes setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="sanremes_cli.setup.prompt_choice",
                quick="sanremes_cli.setup._run_quick_setup",
                model="sanremes_cli.setup.setup_model_provider",
                terminal="sanremes_cli.setup.setup_terminal_backend",
                agent="sanremes_cli.setup.setup_agent_settings",
                gateway="sanremes_cli.setup.setup_gateway",
                tools="sanremes_cli.setup.setup_tools",
            )
            from sanremes_cli.setup import run_setup_wizard
...
class TestQuickFlag:
    ...
    def test_quick_flag_runs_quick_setup_only(self, existing_install):
        ...
        with ExitStack() as stack:
            ...
            from sanremes_cli.setup import run_setup_wizard
            from sanremes_cli import setup as setup_mod
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("sanremes_cli.setup.prompt_choice", {"return_value": 0}),
                first="sanremes_cli.setup._run_first_time_quick_setup",
            )
            from sanremes_cli.setup import run_setup_wizard
            from sanremes_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("sanremes_cli.setup.prompt_choice", {"return_value": 2}),
                blank="sanremes_cli.setup._run_blank_slate_setup",
            )
            from sanremes_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "sanremes_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["sanremes", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
