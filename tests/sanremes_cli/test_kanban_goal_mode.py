...
2. Spawn: _default_spawn sets the SANREMES_KANBAN_GOAL_MODE env vars only
...
# ---------------------------------------------------------------------------
# CLI judge gate tests (sanremes kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """sanremes kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from sanremes_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("sanremes_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("sanremes_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("sanremes_cli.kanban.kb.connect_closing", fake_connect_closing)
        monkeypatch.setattr("sanremes_cli.kanban._worker_run_id_for", lambda _: None)
