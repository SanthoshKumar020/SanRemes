"""Regression test: ``sanremes mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``sanremes_cli/_parser.py``).  The dispatcher in ``sanremes_cli/main.py``
...
result, ``sanremes mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the sanremes parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`sanremes mcp add foo --url ...` must keep args.command == "mcp".
