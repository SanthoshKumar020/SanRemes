...
The bug class: on Windows, when `sanremes.exe`/sibling shims could not be
...
import sanremes_cli._install_repair as ir
import sanremes_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("sanremes", "sanremes-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # sanremes.exe cannot be renamed; sanremes-gateway.exe can
    real_rename = Path.rename

    def deny_sanremes(self, target):
        if self.name == "sanremes.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_sanremes)
    monkeypatch.setattr(cli_main, "_sanremes_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "sanremes.exe" in exc_info.value.failed_shims
    # The successful rename (sanremes-gateway.exe) was rolled back.
    assert (scripts / "sanremes-gateway.exe").exists()
    assert not list(scripts.glob("sanremes-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("sanremes",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("sanremes",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = cli_main.ShimQuarantineError(["sanremes.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "sanremes.exe" in out
