...
``sanremes_cli.memory_setup._write_env_vars`` persists provider plugin
credentials to ``~/.sanremes/.env``. It previously called ``Path.write_text``
...
other subprocess-influencing or SanRemes-runtime-location name) could
otherwise plant a value into ``.env`` via the interactive memory-setup
wizard. The next SanRemes process would load it through the
...
from sanremes_cli.config import ensure_sanremes_home, get_env_path, load_env
from sanremes_cli.memory_setup import _write_env_vars


def _env_file_keys() -> set[str]:
    """Parse ``~/.sanremes/.env`` directly and return the set of keys present.
...
def _sanremes_home(tmp_path, monkeypatch):
...
def test_denylisted_key_does_not_block_other_writes(capsys):
    ...
    _write_env_vars({
        ...
        "SANREMES_LANGFUSE_PUBLIC_KEY": "pk-test-123",
        "OPENROUTER_API_KEY": "sk-or-test-456",
    })

    assert "LD_PRELOAD" not in _env_file_keys()
    env = load_env()
    assert env["SANREMES_LANGFUSE_PUBLIC_KEY"] == "pk-test-123"
    assert env["OPENROUTER_API_KEY"] == "sk-or-test-456"


def test_legitimate_sanremes_integration_key_still_writable():
    """``SANREMES_*`` overall is NOT blocked — only the four runtime
    location names (HOME/PROFILE/CONFIG/ENV). Integration credentials
    following the ``SANREMES_*`` convention (SANREMES_LANGFUSE_*,
    SANREMES_SPOTIFY_*, SANREMES_QWEN_BASE_URL, ...) must keep working or
    the memory-setup wizard regresses for every plugin that follows
    the convention."""
    _write_env_vars({
        "SANREMES_LANGFUSE_PUBLIC_KEY": "pk-lf-789",
        "SANREMES_QWEN_BASE_URL": "https://example.com/v1",
    })

    env = load_env()
    assert env["SANREMES_LANGFUSE_PUBLIC_KEY"] == "pk-lf-789"
    assert env["SANREMES_QWEN_BASE_URL"] == "https://example.com/v1"
...
def test_explicit_sanremes_home_writes_to_that_env_file(tmp_path):
    """Plugin ``post_setup`` hooks (e.g. Supermemory) pass an explicit
    SanRemes home; keep that target while still routing through
    ``save_env_value`` validation instead of writing directly."""
    home = tmp_path / "plugin-home"

    _write_env_vars({"MEM0_API_KEY": "m0-test-key-abc"}, sanremes_home=home)
