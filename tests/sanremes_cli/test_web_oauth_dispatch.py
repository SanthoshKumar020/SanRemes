"""Regression tests for the OAuth dispatcher in sanremes_cli.web_server.
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "sanremes_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "sanremes_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "sanremes_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from sanremes_cli import web_server as ws
    ...
    def fake_status():
        observed_homes.append(get_sanremes_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "sanremes auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "sanremes auth" not in detail
    finally:
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `sanremes model` universe) must be offered by /api/providers/oauth. This keeps
    the desktop Accounts tab in lockstep with the CLI picker — no provider the
    CLI can sign into may be missing from the GUI.
    """
    from sanremes_cli.provider_catalog import provider_catalog
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `sanremes model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by SanRemes."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``sanremes auth remove
    xai-oauth``.
    """
    from sanremes_cli import auth as auth_mod
    from sanremes_cli import web_server as ws
    ...
    # Prior `sanremes auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to sanremes_cli.auth.get_auth_status (the
    ...
    with patch("sanremes_cli.auth.get_auth_status", return_value=fake_status):
