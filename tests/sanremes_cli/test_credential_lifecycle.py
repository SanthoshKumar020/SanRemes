...
SANREMES_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def sanremes_home(monkeypatch, tmp_path):
    """Fresh SANREMES_HOME with .env + auth.json + config.yaml fixtures."""
...
def test_delete_clears_provider_models_cache(sanremes_home):
    _write_env(sanremes_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(sanremes_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = sanremes_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(sanremes_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(sanremes_home, OPENAI_API_KEY=old)
    _write_config(
        sanremes_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = sanremes_home.joinpath("config.yaml").read_text(encoding="utf-8")
