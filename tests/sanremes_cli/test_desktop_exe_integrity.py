...
The desktop self-update chain (Desktop → sanremes-setup --update →
``sanremes update`` → ``sanremes desktop --build-only`` → relaunch) rebuilds
SanRemes.exe on the end user's machine. Before this gate, "build succeeded" was
...
# MACHINE_ATTRIBUTES.UserEnabled — mirrors sanremes_cli.main.
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    backup_exe = desktop_dir / "release" / "win-unpacked.bak" / "SanRemes.exe"
    make_pe(backup_exe, PE_AMD64)  # valid old build

    with patch("sanremes_cli.main._windows_native_machine", return_value="AMD64"):
    ...
    assert (desktop_dir / "release" / "win-unpacked.corrupt" / "SanRemes.exe").exists()
...
def test_gate_fails_clearly_without_backup(tmp_path, capsys):
    ...
    with patch("sanremes_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("sanremes_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
...
# ─── end-to-end: `sanremes desktop --build-only` exits nonzero on corrupt exe ─
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    """The updater chain's contract: a rebuild whose SanRemes.exe cannot launch
    must exit nonzero (so sanremes-setup's retry-once kicks in) and must restore
    ...
    make_pe(desktop_dir / "release" / "win-unpacked.bak" / "SanRemes.exe", PE_AMD64)

    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    pack_ok = subprocess.CompletedProcess(["npm", "run", "pack"], 0)

    with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("sanremes_cli.main._resolve_node_runtime_npm", return_value="npm.cmd"), \
         ...
         patch("sanremes_cli.main._stop_desktop_processes_locking_build", return_value=[]), \
         patch("sanremes_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("sanremes_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("sanremes_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("sanremes_cli.main._windows_native_machine", return_value="AMD64"), \
         patch("sanremes_cli.main.subprocess.run", return_value=pack_ok), \
