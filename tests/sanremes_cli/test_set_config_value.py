...
def _isolated_sanremes_home(tmp_path):
    """Point SANREMES_HOME at a temp dir so tests never touch real config."""
    env_file = tmp_path / ".env"
    env_file.touch()
    with patch.dict(os.environ, {"SANREMES_HOME": str(tmp_path)}):
        yield tmp_path
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_sanremes_home):
        set_config_value(key, "test-value-123")
        env_content = _read_env(_isolated_sanremes_home)
        assert f"{key}=test-value-123" in env_content
        # Must NOT appear in config.yaml
        assert key not in _read_config(_isolated_sanremes_home)
...
class TestCatchAllPatterns:
    ...
    def test_api_key_suffix_routes_to_env(self, key, _isolated_sanremes_home):
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_sanremes_home):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_sanremes_home)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_sanremes_home)


    def test_terminal_image_goes_to_config(self, _isolated_sanremes_home):
    ...
    def test_cron_script_timeout_is_recognized(self, _isolated_sanremes_home, capsys):
        ...
        assert "script_timeout_seconds: 600" in _read_config(_isolated_sanremes_home)

    def test_memory_nudge_interval_is_recognized(self, _isolated_sanremes_home, capsys):
        """The documented background-memory review interval is runtime config."""
        set_config_value("memory.nudge_interval", "0")

        assert "not a recognized config key" not in capsys.readouterr().out
        assert "nudge_interval: 0" in _read_config(_isolated_sanremes_home)

    def test_terminal_docker_cwd_mount_flag_goes_to_config_and_env(self, _isolated_sanremes_home):
    ...
    def test_terminal_docker_shared_key_preserves_string_values(
        self, _isolated_sanremes_home, capsys
    ):
        set_config_value("terminal.docker_shared_container_key", "off")

        import yaml

        saved = yaml.safe_load(_read_config(_isolated_sanremes_home))
        assert saved["terminal"]["docker_shared_container_key"] == "off"
        assert "TERMINAL_DOCKER_SHARED_CONTAINER_KEY=off" in _read_env(
            _isolated_sanremes_home
        )
        assert "not a recognized config key" not in capsys.readouterr().out

    def test_terminal_vercel_runtime_goes_to_config_and_env(self, _isolated_sanremes_home):
...
class TestFalsyValues:
    ...
    def test_config_command_accepts_empty_string(self, _isolated_sanremes_home):
...
class TestConfigGetUnset:
    ...
    def test_config_get_prints_resolved_nested_value(self, _isolated_sanremes_home, capsys):
    ...
    def test_config_unset_removes_yaml_key_and_synced_env(self, _isolated_sanremes_home, capsys):
        set_config_value("terminal.backend", "docker")
        assert "TERMINAL_ENV=docker" in _read_env(_isolated_sanremes_home)
        capsys.readouterr()
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_sanremes_home)) or {}
        assert reloaded == {}
        assert "TERMINAL_ENV=" not in _read_env(_isolated_sanremes_home)
        assert "Unset terminal.backend" in capsys.readouterr().out


    def test_config_unset_removes_dotted_token_yaml_key(self, _isolated_sanremes_home, capsys):
        (_isolated_sanremes_home / "config.yaml").write_text(
            "platforms:\n"
            "  teams:\n"
            "    extra:\n"
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_sanremes_home))
...
class TestListNavigation:
    """sanremes config set must preserve YAML list fields when using numeric
    ...
    def test_indexed_set_preserves_sibling_list_entries(self, _isolated_sanremes_home):
        """Setting custom_providers.0.api_key must not destroy entry 1."""
        self._write_config(_isolated_sanremes_home, (
            "custom_providers:\n"
            "- name: provider-a\n"
            "  api_key: old-a\n"
    ...
    def test_indexed_set_preserves_non_targeted_fields(self, _isolated_sanremes_home):
    ...
    def test_deeper_nesting_through_list(self, _isolated_sanremes_home):
...
class TestCronModelDriftConfigWarning:
    ...
    def test_warning_names_the_user_owned_cli_pin_path(
        ...
        assert "sanremes cron edit <job_id> --provider <provider> --model <model>" in warning
...
class TestStringTypedConfigValues:
    ...
    def test_string_typed_values_are_not_coerced(self, _isolated_sanremes_home, value):
    ...
    def test_non_string_defaults_keep_existing_coercion(
        self, _isolated_sanremes_home, key, value, expected
    ):
    ...
    def test_unknown_keys_keep_existing_coercion(self, _isolated_sanremes_home):
...
class TestSecretRedactionInDisplay:
    ...
    def test_redact_config_value_masks_nested_api_key(self):
        from sanremes_cli.config import redact_config_value
    ...
    def test_set_echo_masks_secret_value(self, _isolated_sanremes_home, capsys):
    ...
    def test_set_echo_keeps_nonsecret_value(self, _isolated_sanremes_home, capsys):
...
class TestSchemaValidation:
    """#34067: ``sanremes config set`` must not report bare success for
    ...
    a post-write notice warns that SanRemes may never read it and suggests the
    ...
    def test_desktop_macos_signing_identity_is_accepted(self, _isolated_sanremes_home, capsys):
        """The documented TCC signing identity setting is part of the schema."""
        set_config_value("desktop.macos_signing_identity", "SanRemes Local Signing")
        import yaml
        saved = yaml.safe_load(_read_config(_isolated_sanremes_home))
        assert saved["desktop"]["macos_signing_identity"] == "SanRemes Local Signing"
        assert "not a recognized config key" not in capsys.readouterr().out



    def test_force_suppresses_notice(self, _isolated_sanremes_home, capsys):
        ...
        content = _read_config(_isolated_sanremes_home)
...
class TestValidateConfigKey:
    ...
    def test_known_keys_pass(self, key):
        from sanremes_cli.config import _validate_config_key
...
class TestDisplaySkinTouch:
    ...
    (`sanremes config set display.skin X` while it is already X — the recovery
    ...
    def test_reaffirming_same_skin_moves_the_watcher_signature(self, _isolated_sanremes_home):
        import os as _os
        skins = _isolated_sanremes_home / "skins"
        skins.mkdir()
    ...
    def test_builtin_or_missing_skin_file_is_fine(self, _isolated_sanremes_home):
        """Built-ins have no user file — the set must still succeed cleanly."""
        set_config_value("display.skin", "mono")
        assert "skin: mono" in _read_config(_isolated_sanremes_home)

    def test_touch_preserves_skin_file_contents(self, _isolated_sanremes_home):
        skins = _isolated_sanremes_home / "skins"
        skins.mkdir()
        body = "name: neon\ncolors:\n  ui_accent: '#ff33aa'\n"
...
class TestMappingGuard:
    """``sanremes config set <section> <scalar>`` must not silently destroy an
    ...
    def test_bare_model_shorthand_preserves_siblings(self, _isolated_sanremes_home):
        """sanremes config set model <id> → model.default, siblings survive."""
        self._write_config(_isolated_sanremes_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
        ...
        config_text = _read_config(_isolated_sanremes_home)
        import yaml as _yaml
    ...
    def test_bare_model_shorthand_creates_default_when_none(self, _isolated_sanremes_home):
        """Bare model shorthand still works when config is empty (legacy behaviour)."""
        set_config_value("model", "gpt-5.6-sol")
        assert "gpt-5.6-sol" in _read_config(_isolated_sanremes_home)

    def test_non_model_mapping_is_refused(self, _isolated_sanremes_home):
        """sanremes config set terminal bash → refuse, terminal has sub-keys."""
        self._write_config(_isolated_sanremes_home, {
            "terminal": {
                "backend": "docker",
    ...
    def test_non_model_mapping_force_overwrites(self, _isolated_sanremes_home):
        """sanremes config set --force terminal bash → proceed, section wiped."""
        self._write_config(_isolated_sanremes_home, {
            "terminal": {
                "backend": "docker",
                "shell": "bash",
        ...
        import yaml as _yaml
        parsed = _yaml.safe_load(_read_config(_isolated_sanremes_home))
        assert parsed["terminal"] == "zsh"

    def test_model_default_dotted_path_is_not_guarded(self, _isolated_sanremes_home):
    ...
    def test_model_force_overwrites_entire_section(self, _isolated_sanremes_home):
        """sanremes config set --force model <id> → overwrite entire section."""
        self._write_config(_isolated_sanremes_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
...
class TestScalarModelSubKeyPreservation:
    ...
    def test_scalar_model_id_preserved_after_provider_write(self, _isolated_sanremes_home):
        ...
        raw = _read_config(_isolated_sanremes_home)
        parsed = yaml.safe_load(raw)
        model = parsed["model"]
        assert model["default"] == "gpt-4o", f"model.default lost: {model}"
        assert model["provider"] == "openai"

    def test_scalar_model_id_preserved_after_api_key_write(self, _isolated_sanremes_home):
...
class TestMalformedYAMLConfigPreservation:
    ...
    def test_set_config_value_refuses_broken_yaml(self, _isolated_sanremes_home, capsys):
        """set_config_value must raise, not overwrite the broken config."""
        self._write_broken_config(_isolated_sanremes_home)
    ...
    def test_unset_config_value_refuses_broken_yaml(self, _isolated_sanremes_home, capsys):
        """unset_config_value must raise, not overwrite the broken config."""
        from sanremes_cli.config import unset_config_value
