...
still running after 180.0s — restart may fail") and could deadlock `sanremes
...
def tmp_path():
    ...
    The loop-tick witness tests bind real UNIX sockets under SANREMES_HOME.
...
def _launchd_harness(monkeypatch, tmp_path, pid):
    ...
    patched ``_process_sanremes_home``).
    """
    events = []
    ...
    monkeypatch.setattr(
        "gateway.shutdown_watchdog._process_sanremes_home", lambda: tmp_path
    )
    return events
...
class TestLoopTickWitness:
    ...
    def test_stalled_heartbeat_write_never_escalates_a_running_loop(
        ...
        try:
            ...
            events = []
            monkeypatch.setattr(
                gateway_cli, "get_launchd_label", lambda: "ai.sanremes.gateway"
            )
            monkeypatch.setattr(gateway_cli, "_launchd_domain", lambda: "gui/501")
            monkeypatch.setattr(gateway_cli, "_get_restart_drain_timeout", lambda: 180.0)
            monkeypatch.setattr(
...
def test_default_probe_budget_stays_inside_query_tier():
    ...
    assert worst_case <= 5.0, (
        ...
        "subprocess-timeout doc reference in sanremes_cli/gateway.py"
    )
