...
from sanremes_cli.psutil_android import (
    MARKER,
    REPLACEMENT,
    PSUTIL_URL,
...
def test_install_psutil_android_compat_uses_patched_tree(tmp_path):
    ...
    with patch("urllib.request.urlretrieve", side_effect=fake_urlretrieve), \
         patch.object(sanremes_main, "_run_install_with_heartbeat", side_effect=fake_run_install):
        sanremes_main._install_psutil_android_compat(
            ["uv", "pip"],
            env={"SANREMES_TEST": "1"},
        )

    assert captured["cmd"][:4] == ["uv", "pip", "install", "--no-build-isolation"]
    assert captured["env"] == {"SANREMES_TEST": "1"}
