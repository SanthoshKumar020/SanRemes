...
install``. A locked ``sanremes.exe`` makes the install exit 2, the handler
...
def test_windows_dep_failure_does_not_zip_fallback(monkeypatch):
    monkeypatch.setattr(sanremes_main, "_is_windows", lambda: True)
    exc = _cpe([r"C:\venv\Scripts\uv.exe", "pip", "install", "-e", "."])
...
def test_zip_overlay_blocked_on_modified_file(tmp_path, monkeypatch):
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        update_cmd.subprocess, "run", _porcelain_run(" M sanremes_cli/update_cmd.py\n")
    )
...
def test_update_via_zip_aborts_before_download_when_dirty(
    ...
    monkeypatch.setattr(sanremes_main, "PROJECT_ROOT", fake_root)
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
    ...
    with patch("urllib.request.urlretrieve") as download:
        with pytest.raises(SystemExit) as exc_info:
            sanremes_main._update_via_zip(SimpleNamespace(branch=None))
...
def test_staging_artifact_lines_are_recognized():
    ...
    assert is_artifact("?? agent.sanremes-update-staging/")
    assert is_artifact("?? cli.py.sanremes-update-staging")
    assert is_artifact("?? tools.sanremes-update-old/")
    ...
    assert not is_artifact(" M sanremes_cli/update_cmd.py")
    assert not is_artifact("?? notes.sanremes-update-staging.txt")
...
def test_recheck_ignores_own_staging_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
        _porcelain_run("?? agent.sanremes-update-staging/\n?? cli.py.sanremes-update-old\n"),
    )
    assert (
