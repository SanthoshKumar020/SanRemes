...
``_reexec_dependency_sync_off_windows_shim`` with ``SANREMES_UPDATE_REEXEC=1``)
...
def _run_cmd_update(monkeypatch, impl, *, reexec: bool):
    ...
    monkeypatch.setattr("sanremes_cli.config.detect_install_method", lambda root: "git")
    monkeypatch.setattr("sanremes_cli.update_lock.UpdateLock", lambda: _FakeLock(events["order"]))
    ...
    monkeypatch.setattr("sanremes_cli.update_receipt.finalize_pending_update_receipt", fake_receipt)
    monkeypatch.setattr("os._exit", fake_exit)
    if reexec:
        monkeypatch.setenv("SANREMES_UPDATE_REEXEC", "1")
    else:
        monkeypatch.delenv("SANREMES_UPDATE_REEXEC", raising=False)
