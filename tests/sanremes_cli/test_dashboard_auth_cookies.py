...
def test_session_cookies_use_secure_prefix_when_proxied():
    """HTTPS + /sanremes prefix → __Secure- prefix (__Host- forbids
    Path != "/"; __Secure- keeps the Secure-required hardening)."""
    client = TestClient(_build_app(use_https=True, prefix="/sanremes"))
    r = client.get("/set")
    cookies = r.headers.get_list("set-cookie")
    at = next(c for c in cookies if c.startswith(f"__Secure-{SESSION_AT_COOKIE}="))
    assert "Path=/sanremes" in at
    assert "Secure" in at
...
def test_clear_session_cookies_prefixed_deletions_carry_secure():
    ...
    so an insecure deletion for __Host-sanremes_session_at is silently
