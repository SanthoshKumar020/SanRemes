"""Tests for sanremes_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
def receipt_home(tmp_path, monkeypatch):
    """Isolated SANREMES_HOME for receipt writes."""
    home = tmp_path / ".sanremes"
    home.mkdir()
    monkeypatch.setattr(
        "sanremes_cli.config.get_sanremes_home", lambda: home, raising=False
    )
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["sanremes-gateway"],
        ...
        assert gr["restarted_services"] == ["sanremes-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another sanremes.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "sanremes.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(sanremes_main, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            sanremes_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            sanremes_main,
        ...
        monkeypatch.setattr(
            sanremes_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import sanremes_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "sanremes_cli.profiles._get_default_sanremes_home", lambda: home
        )
        monkeypatch.setattr(
            "sanremes_cli.profiles._get_profiles_root",
    ...
    def test_matrix_returns_true_only_on_stale(self, capsys):
        ...
        assert "STALE" in out
        assert "sanremes -p <profile> gateway restart" in out
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
