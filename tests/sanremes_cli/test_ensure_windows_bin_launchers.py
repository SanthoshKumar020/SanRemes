"""``sanremes`` must survive git operations on the checkout (launcher layout).

The Windows ``sanremes`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``SANREMES_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``sanremes-agent\\bin``) was swept by ``sanremes update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``sanremes`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``sanremes update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``sanremes_constants.venv_bin_dir``), so these
...
from sanremes_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: SANREMES_HOME/sanremes-agent/venv/Scripts + launchers."""
    home = tmp_path / "sanremes"
    root = home / "sanremes-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under SANREMES_HOME gains nothing anywhere."""
    home = tmp_path / "sanremes-home"
    home.mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(home))
    root = tmp_path / "src" / "sanremes-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``sanremes -p <name>`` SANREMES_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("SANREMES_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `sanremes update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "sanremes.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "sanremes.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\sanremes\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/SANREMES/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``sanremes update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/sanremes.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/sanremes.exe is not gitignored — sanremes update's autostash "
