...
``sanremes*.exe`` shims aside (``sanremes.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``sanremes`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("sanremes", "sanremes-agent", "sanremes-acp", "sanremes-gateway"):
...
# ---------------------------------------------------------------------------
# sanremes_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "sanremes.exe" in names, "sanremes.exe must be restored after a no-op install"
    assert "sanremes-acp.exe" in names
    assert "sanremes-gateway.exe" in names
    assert (scripts / "sanremes.exe").read_bytes() == b"MZ-old-sanremes"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "sanremes.exe").read_bytes() == b"MZ-new-sanremes"
...
# ---------------------------------------------------------------------------
# sanremes_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "sanremes.exe" in names, "sanremes.exe must be restored after a no-op recovery install"
    assert (scripts / "sanremes.exe").read_bytes() == b"MZ-old-sanremes"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "sanremes.exe").write_bytes(b"MZ-new-sanremes")
