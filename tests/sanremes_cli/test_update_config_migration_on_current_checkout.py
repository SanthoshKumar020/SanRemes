...
def test_check_and_apply_config_migration_interactive_prompt():
    ...
    with (
        patch.object(update_cmd, "_reload_config_modules"),
        patch.object(update_cmd, "_run_config_check_fresh", return_value=(37, 38)),
        patch("sanremes_cli.config.get_missing_env_vars", return_value=[{"name": "NEW_KEY", "description": "desc"}]),
        patch("sanremes_cli.config.get_missing_config_fields", return_value=[]),
...
def test_check_and_apply_config_migration_assume_yes():
    ...
    with (
        patch.object(update_cmd, "_reload_config_modules"),
        patch.object(update_cmd, "_run_config_check_fresh", return_value=(37, 38)),
        patch("sanremes_cli.config.get_missing_env_vars", return_value=[{"name": "NEW_KEY"}]),
        patch("sanremes_cli.config.get_missing_config_fields", return_value=[]),
        patch.object(
            update_cmd,
            "_run_migrate_config_fresh",
...
def test_check_and_apply_config_migration_non_interactive():
    ...
    with (
        ...
        patch.object(update_cmd, "_run_config_check_fresh", return_value=(37, 38)),
        patch("sanremes_cli.config.get_missing_env_vars", return_value=[]),
        patch("sanremes_cli.config.get_missing_config_fields", return_value=[{"key": "new_setting"}]),
