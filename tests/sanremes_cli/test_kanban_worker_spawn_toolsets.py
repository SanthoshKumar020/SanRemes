...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    SANREMES_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_sanremes_argv", lambda: ["sanremes"])
    ...
    assert pid == 4242
    assert captured["env"]["SANREMES_HOME"] == str(profile)
    assert captured["env"]["SANREMES_KANBAN_TASK"] == "t_spawn_tools"
