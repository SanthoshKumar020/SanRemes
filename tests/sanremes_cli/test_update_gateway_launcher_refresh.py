...
2. ``sanremes_cli.main._refresh_windows_gateway_launchers`` — ``sanremes
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "sanremes_cli.main", "gateway", "run"]
    with mock.patch.object(
        gateway_windows, "_stable_gateway_working_dir", return_value=str(tmp_path)
    ), mock.patch("sanremes_cli.config.get_sanremes_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: sanremes update regenerates launchers
# ---------------------------------------------------------------------------
