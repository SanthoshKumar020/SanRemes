...
A previous ``sanremes update`` whose npm install failed printed "Fix npm and
re-run `sanremes update`" — but re-running hit the "Already up to date!" early
...
def test_current_checkout_repairs_failed_node_deps(capsys):
    ...
    assert "Fix npm and re-run `sanremes update`." in out
