...
from sanremes_cli.platform_actions import CAPABILITY_ID, PlatformActions
from sanremes_cli.plugin_capabilities import CAPABILITY_REGISTRY


def _grant(granted: bool):
    """Patch the capability check the facade performs."""
    return patch(
        "sanremes_cli.plugin_capabilities.plugin_capability_granted",
...
class TestGateDefaultOff:
    ...
    def test_default_config_is_off_via_real_capability_check(self):
        ...
        actions = PlatformActions("some-plugin")
        with patch(
            "sanremes_cli.plugin_capabilities._plugin_entry", return_value={}
        ):
            result = asyncio.run(
    ...
    def test_legacy_allow_platform_actions_key_grants(self):
        ...
        adapter = _telegram_adapter()
        with patch(
            "sanremes_cli.plugin_capabilities._plugin_entry",
