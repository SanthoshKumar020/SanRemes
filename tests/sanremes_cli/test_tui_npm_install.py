...
def _touch_ink(root: Path) -> None:
    ink = root / "node_modules" / "@sanremes" / "ink" / "package.json"
...
def test_make_tui_argv_uses_bundled_tui_when_workspace_missing(
    ...
    sanremes_cli/tui_dist/entry.js but never ships ui-tui/ (that directory only
...
def _write_ws(root: Path, ws_lock: str, hidden_lock: str) -> Path:
    ...
    ``@sanremes/ink`` and the marker live at the workspace root (hoisted);
    ...
    (tui_dir / "package.json").write_text('{"name":"sanremes-tui"}')
    return tui_dir
...
def test_need_install_when_linked_workspace_dep_missing(tmp_path: Path, main_mod) -> None:
    """The closure follows workspace symlinks (@sanremes/ink → ui-tui/packages/…)
    so a linked workspace's own missing dep triggers a reinstall."""
    tui_dir = _write_ws(
        tmp_path,
        '{"packages":{'
        '"ui-tui":{"dependencies":{"@sanremes/ink":"*"}},'
        '"node_modules/@sanremes/ink":{"link":true,"resolved":"ui-tui/packages/sanremes-ink"},'
        '"ui-tui/packages/sanremes-ink":{"dependencies":{"inkdep":"1.0.0"}},'
...
def test_workspace_closure_includes_dev_deps_of_selected_child_workspace(main_mod) -> None:
    ...
    packages = {
        "ui-tui": {"dependencies": {"@sanremes/ink": "*"}},
        "node_modules/@sanremes/ink": {
            "link": True,
            "resolved": "ui-tui/packages/sanremes-ink",
        },
        "ui-tui/packages/sanremes-ink": {"devDependencies": {"child-dev-only": "1"}},
    ...
    termux = main_mod._npm_lock_workspace_closure(
        packages, {"ui-tui", "ui-tui/packages/sanremes-ink"}
    )
...
def test_termux_install_catches_missing_child_workspace_dev_dep(
    ...
    ws_lock = (
        ...
        '"ui-tui":{"dependencies":{"@sanremes/ink":"*"}},'
        '"node_modules/@sanremes/ink":{"link":true,"resolved":"ui-tui/packages/sanremes-ink"},'
        '"ui-tui/packages/sanremes-ink":{"devDependencies":{"child-dev-only":"1.0.0"}},'
    ...
    child = tui_dir / "packages" / "sanremes-ink"
    child.mkdir(parents=True, exist_ok=True)
    (child / "package.json").write_text('{"name":"@sanremes/ink"}')
...
def test_make_tui_argv_decodes_dev_prebuild_with_utf8_replace(
    ...
    ink_dir = tmp_path / "packages" / "sanremes-ink"
    ink_dir.mkdir(parents=True)
...
def test_no_stray_lockfiles_in_workspace_subdirs(main_mod) -> None:
    ...
    # Also sweep ui-tui/packages/* (sanremes-ink etc.)
