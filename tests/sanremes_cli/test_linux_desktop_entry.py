"""Tests for the Linux XDG desktop entry installed by ``sanremes desktop``."""
...
from sanremes_cli import linux_desktop_entry as lde
...
def test_install_writes_entry_with_absolute_exec_and_icon(tmp_path, xdg_home, monkeypatch):
    ...
    sanremes_bin = tmp_path / "bin" / "sanremes"
    sanremes_bin.parent.mkdir()
    sanremes_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "sanremes_cli.relaunch.resolve_sanremes_bin", lambda: str(sanremes_bin)
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)

    assert entry == xdg_home / "applications" / "sanremes.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `sanremes` would not resolve.
    assert values["Exec"] == f"{sanremes_bin} desktop"
    ...
    assert values["Name"] == "SanRemes"
    assert values["Terminal"] == "false"


def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("sanremes_cli.relaunch.resolve_sanremes_bin", lambda: "/usr/bin/sanremes")
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("sanremes_cli.relaunch.resolve_sanremes_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m sanremes_cli.main desktop")
    assert Path(exec_line.split(" ")[0]).is_absolute()


# #90292: the shell installer's bash wrapper makes argv[0] the repo `sanremes`
...
def test_exec_prefixes_interpreter_for_env_shebang_python_script(tmp_path, xdg_home, monkeypatch):
    ...
    sanremes_bin = tmp_path / "bin" / "sanremes"
    sanremes_bin.parent.mkdir()
    sanremes_bin.write_text("#!/usr/bin/env python3\nimport sanremes_cli\n", encoding="utf-8")
    sanremes_bin.chmod(0o755)
    monkeypatch.setattr("sanremes_cli.relaunch.resolve_sanremes_bin", lambda: str(sanremes_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert str(sanremes_bin) in exec_line
...
def test_exec_leaves_shell_wrapper_launchers_alone(tmp_path, xdg_home, monkeypatch):
    ...
    sanremes_bin = tmp_path / "bin" / "sanremes"
    sanremes_bin.parent.mkdir()
    sanremes_bin.write_text('#!/bin/bash\nexec /opt/sanremes/venv/bin/python "$@"\n', encoding="utf-8")
    sanremes_bin.chmod(0o755)
    monkeypatch.setattr("sanremes_cli.relaunch.resolve_sanremes_bin", lambda: str(sanremes_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert exec_line == f"{sanremes_bin} desktop"
...
def test_exec_leaves_venv_shebang_scripts_alone(tmp_path, xdg_home, monkeypatch):
    ...
    sanremes_bin.parent.mkdir()
    interpreter = str(Path(sys.executable).resolve())
    sanremes_bin.write_text(f"#!{interpreter}\nimport sanremes_cli\n", encoding="utf-8")
    sanremes_bin.chmod(0o755)
    monkeypatch.setattr("sanremes_cli.relaunch.resolve_sanremes_bin", lambda: str(sanremes_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when SanRemes is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "sanremes"
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    spaced = tmp_path / "my apps" / "sanremes"
    spaced.parent.mkdir()
    spaced.write_text("", encoding="utf-8")
    monkeypatch.setattr("sanremes_cli.relaunch.resolve_sanremes_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
