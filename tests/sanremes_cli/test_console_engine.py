...
from sanremes_cli.console_engine import SanRemesConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/sanremes-console-test.zip",
    "import /tmp/sanremes-console-test.zip",
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_sanremes_home):
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    monkeypatch.setattr(sanremes_state, "resolved_max_export_messages", lambda: 2)
    materialized = []
    ...
    result = SanRemesConsoleEngine().execute(
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_sanremes_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_sanremes_home):
    ...
    assert code == 0
    assert "SanRemes Console" in stdout.getvalue()
    assert "sanremes>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_capture_output_surfaces_string_exit_code_as_command_error():
    from sanremes_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_execute_handler_string_exit_returns_error_not_crash(_isolate_sanremes_home):
    result = SanRemesConsoleEngine().execute(
...
def test_console_checkpoints_prune_does_not_reprompt_for_orphans(
    _isolate_sanremes_home, monkeypatch
):
    ...
    result = SanRemesConsoleEngine().execute("checkpoints prune", confirmed=True)
