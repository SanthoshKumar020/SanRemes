...
lane). Spawns REAL processes with realistic SanRemes argv shapes and drives
...
the empirical premise-check for each issue:
  ...
  #90778 — holder message mislabels `sanremes dashboard` as the Desktop
...
def _detect() -> list[tuple[int, str, str]]:
    from sanremes_cli.update_cmd import _detect_venv_python_processes
...
class TestDetection:
    def test_detects_sanremes_argv_process(self):
        """Baseline: a live process running `-m sanremes_cli.main serve` with
        cwd under the install root is detected as a venv holder."""
        proc = _spawn(["-m", "sanremes_cli.main", "serve"])
        try:
            matches = _detect()
            ...
            assert "sanremes_cli.main" in cmdline
        finally:
            _kill(proc)

    def test_foreign_python_not_detected(self):
        """A python process with no SanRemes argv and cwd OUTSIDE the install
    ...
    def test_long_runtime_path_gateway_detected_with_full_argv(self):
        ...
        padding = os.path.join("C:\\", "Users", "x" * 90, ".sanremes-runtime")
        proc = _spawn([padding, "-m", "sanremes_cli.main", "gateway", "run"])
        try:
            matches = _detect()
...
class TestClassification:
    def test_pausable_exemption_sees_long_path_gateway(self):
        ...
        from sanremes_cli.update_cmd import _leftover_pausable_gateway_pids

        padding = os.path.join("C:\\", "Users", "y" * 90, ".sanremes-runtime")
        proc = _spawn([padding, "-m", "sanremes_cli.main", "gateway", "run"])
        try:
...
class TestHolderMessage:
    ...
    def test_dashboard_not_labeled_desktop_backend(self):
        from sanremes_cli.update_cmd import _format_venv_python_holders_message

        proc = _spawn(["-m", "sanremes_cli.main", "dashboard"])
        try:
            matches = [m for m in _detect() if m[0] == proc.pid]
            ...
            assert "close the desktop app" not in message.lower(), (
                "standalone `sanremes dashboard` mislabeled as the Desktop "
                f"backend (#90778):\n{message}"
            )
        finally:
            _kill(proc)
    ...
    def test_substring_subcommand_not_mislabeled(self):
        ...
        proc = _spawn(["-m", "sanremes_cli.main", "kanban", "--preserve-cache"])
        try:
            matches = [m for m in _detect() if m[0] == proc.pid]
...
class TestAncestorExclusion:
    ...
    def test_gateway_parent_visible_to_child_scan(self, tmp_path):
        ...
        # `... -m sanremes_cli.main gateway run`, not a multi-line -c blob).
...
class TestConcurrentGateClassification:
    ...
    def test_live_gateway_process_classified_gateway(self):
        """A real process whose argv carries `-m sanremes_cli.main gateway run`
        classifies as ``gateway`` via real psutil against the live table."""
        from sanremes_cli.update_cmd import _classify_concurrent_instance

        proc = _spawn(["-m", "sanremes_cli.main", "gateway", "run"])
        try:
    ...
    def test_live_non_gateway_processes_keep_the_abort(self):
        ...
        repl = _spawn(["-m", "sanremes_cli.main"])
        mgmt = _spawn(["-m", "sanremes_cli.main", "gateway", "status"])
        try:
    ...
    def test_live_filter_drops_only_the_gateway(self):
        ...
        gw = _spawn(["-m", "sanremes_cli.main", "gateway", "run"])
        backend = _spawn(["-m", "sanremes_cli.main", "serve", "--port", "8127"])
        dead = _spawn([])
        _kill(dead)  # reaped → unreadable cmdline → unknown → kept
        try:
            matches = [
                (gw.pid, "sanremes.exe"),
                (backend.pid, "sanremes.exe"),
                (dead.pid, "sanremes.exe"),
            ]
