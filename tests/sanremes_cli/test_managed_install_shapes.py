...
def sanremes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("SANREMES_MANAGED", raising=False)
    return home
...
def test_env_var_names_the_managing_system(sanremes_home, monkeypatch, env_value, expected):
    if env_value is None:
        monkeypatch.delenv("SANREMES_MANAGED", raising=False)
    else:
        monkeypatch.setenv("SANREMES_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    sanremes_home, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the SANREMES_MANAGED of the service."""
    (sanremes_home / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("SANREMES_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(sanremes_home, monkeypatch):
    (sanremes_home / ".managed").write_text("nixos", encoding="utf-8")
    monkeypatch.setenv("SANREMES_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    sanremes_home, monkeypatch, tmp_path, managed_value
):
    """The message names the system, so the user knows what owns the install."""
    monkeypatch.setenv("SANREMES_MANAGED", managed_value)
    ...
    # `sanremes update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "sanremes update"
...
def test_unmanaged_install_offers_no_update_command(sanremes_home, monkeypatch):
    monkeypatch.delenv("SANREMES_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(sanremes_home, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = sanremes_home / ".managed"
