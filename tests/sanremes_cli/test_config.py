"""Tests for sanremes_cli configuration management."""
...
class TestGetSanRemesHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SANREMES_HOME", None)
            home = get_sanremes_home()
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\sanremes — see
                # sanremes_constants._get_platform_default_sanremes_home.
                ...
                assert home == base / "sanremes"
            else:
                assert home == Path.home() / ".sanremes"


class TestEnsureSanRemesHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from sanremes_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``sanremes logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from sanremes_cli import config as cfg_mod
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from sanremes_cli.config import _deep_merge
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unparseable_existing_config(self, tmp_path):
        ...
        from sanremes_cli.config import atomic_config_write
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SANREMES_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SANREMES_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SANREMES_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SANREMES_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("sanremes_cli.config.sanitize_env_file", return_value=2),
            patch(
                "sanremes_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("sanremes_cli.config.read_raw_config", return_value={}),
            patch("sanremes_cli.config.get_missing_env_vars", return_value=[]),
            patch("sanremes_cli.config.get_missing_config_fields", return_value=[]),
            patch("sanremes_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from sanremes_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """SANREMES_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `sanremes setup`) lets a
        ...
        via config.yaml; SANREMES_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from sanremes_cli.config import OPTIONAL_ENV_VARS
        assert "SANREMES_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `sanremes setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `sanremes setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_sanremes_home(self):
        from sanremes_cli.config_migrations import support_floor_message
        from sanremes_constants import display_sanremes_home

        msg = support_floor_message()
        assert f"{display_sanremes_home()}/config.yaml" in msg

    def test_registry_has_no_targets_below_floor(self):
        from sanremes_cli.config_migrations import (
            MIGRATIONS,
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from sanremes_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected SanRemes runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next SanRemes
    process on next startup via the dotenv → ``os.environ`` chain in
    ``sanremes_cli/env_loader.py``.
    ...
    def _sanremes_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "SANREMES_LANGFUSE_PUBLIC_KEY",
            "SANREMES_SPOTIFY_CLIENT_ID",
            "SANREMES_QWEN_BASE_URL",
            "SANREMES_MAX_ITERATIONS",
        ],
    )
    def test_sanremes_integration_keys_still_writable(self, allowed_key):
        """``SANREMES_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        "protected_key",
        [
            "SANREMES_CONFIG_PATH",
            "SANREMES_ENV_PATH",
            "SANREMES_OPTIONAL_MCPS",
            "SANREMES_COPILOT_ACP_COMMAND",
            "SANREMES_COPILOT_ACP_ARGS",
            "SANREMES_YOLO_MODE",
            "SANREMES_ACCEPT_HOOKS",
            "SANREMES_REDACT_SECRETS",
            "SANREMES_INTERACTIVE",
            "SANREMES_EXEC_ASK",
            "SANREMES_GATEWAY_SESSION",
            "SANREMES_CRON_SESSION",
            "SANREMES_SINGLE_QUERY_SESSION",
            "SANREMES_SESSION_KEY",
            "SANREMES_SESSION_PLATFORM",
        ],
    )
    def test_sanremes_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from sanremes_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"SANREMES_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["SANREMES_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("SanRemes_Yolo_Mode", "SANREMES_YOLO_MODE"),
            ("SanRemes_Optional_Mcps", "SANREMES_OPTIONAL_MCPS"),
            ("SanRemes_Copilot_Acp_Command", "SANREMES_COPILOT_ACP_COMMAND"),
            ("SanRemes_Copilot_Acp_Args", "SANREMES_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from sanremes_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from sanremes_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from sanremes_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"SANREMES_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: sanremes\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "sanremes"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import sanremes_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`sanremes config set/unset` must exit cleanly (no traceback) when the
    ...
    def test_config_command_set_exits_cleanly_on_broken_yaml(self, tmp_path, capsys):
        from sanremes_cli.config import config_command
