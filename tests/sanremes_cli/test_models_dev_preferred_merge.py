...
appear in ``/model`` without a SanRemes release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("sanremes_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from sanremes_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("sanremes_cli.main._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("sanremes_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("sanremes_cli.config.get_env_value", return_value=""),
...
class TestOpenRouterAndNousUnchanged:
    ...
    def test_openrouter_does_not_call_merge(self):
        ...
        with patch(
            "sanremes_cli.models._merge_with_models_dev",
