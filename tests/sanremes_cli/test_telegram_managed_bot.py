"""Tests for sanremes_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
from sanremes_cli.telegram_managed_bot import (
    DEFAULT_MANAGER_BOT,
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("sanremes_")
        assert name.endswith("_bot")
        assert len(name) == len("sanremes_") + 16 + len("_bot")
        assert len(name) <= 32
...
class TestGenerateDeepLink:
    ...
    def test_name_url_encoded(self):
        link = generate_deep_link(
            ...
            suggested_name="SanRemes & Friends",
        )
        assert "SanRemes+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "sanremes_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/SanRemesSetupBot/sanremes_abcdefghijklmnop_bot?name=SanRemes+Agent",
            "qr_payload": "https://t.me/newbot/SanRemesSetupBot/sanremes_abcdefghijklmnop_bot?name=SanRemes+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "sanremes_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="SanRemes Agent")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="sanremes_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/SanRemesSetupBot/sanremes_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/SanRemesSetupBot/sanremes_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "sanremes_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "sanremes_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("sanremes_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_eventual_success(self):
        ...
        with patch("sanremes_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("sanremes_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from sanremes_cli.setup import _setup_telegram_auto

        assert callable(_setup_telegram_auto)

    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from sanremes_cli import setup

        seen = {}
        profile_home = tmp_path / ".sanremes" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_sanremes_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "sanremes_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from sanremes_cli.setup import _profile_name_from_sanremes_home

        assert (
            _profile_name_from_sanremes_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\sanremes\profiles\oracle")
            )
            == "oracle"
        )
