...
The update receipt (written by every `sanremes update` run since #91283,
`latest.json` pointer) is the durable outcome record. These tests pin:

- GET /api/sanremes/update/receipt returns the full receipt + summary; 404
  when none exists.
- GET /api/actions/sanremes-update/status attaches the receipt summary, and
...
class TestUpdateReceiptEndpoint:
    ...
    def test_receipt_endpoint_returns_full_receipt_and_summary(self, client, tmp_path, monkeypatch):
        ...
        resp = client.get("/api/sanremes/update/receipt")
...
class TestUpdateStatusReadsReceipt:
    ...
    def test_status_attaches_receipt_summary(self, client, tmp_path, monkeypatch):
        ...
        resp = client.get("/api/actions/sanremes-update/status")
