...
sanremes_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # sanremes_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("SANREMES_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import sanremes_time

    assert sanremes_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (SANREMES_TIMEZONE, SANREMES_REDACT_SECRETS, SANREMES_MAX_ITERATIONS,
    ...
    # writing SANREMES_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
