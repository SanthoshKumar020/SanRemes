"""Tests for sanremes_cli.gateway."""
...
import sanremes_cli.gateway as gateway


_BREAKAWAY_MARKER = "_SANREMES_GATEWAY_BREAKAWAY"
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /sanremes-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``SANREMES_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``SANREMES_HOME`` value, which
...
def _run_native_windows_gateway_start_diag(
    ...
    script = textwrap.dedent(
        ...
        async def start_gateway(*, replace, verbosity):
            assert "_SANREMES_GATEWAY_BREAKAWAY" not in os.environ
            return True
        ...
        diag_path = pathlib.Path(os.environ["SANREMES_HOME"]) / "logs" / "gateway-exit-diag.log"
    ...
    env.update(
        {
            "SANREMES_HOME": str(tmp_path),
            "SANREMES_GATEWAY_DETACHED": "1",
            "SANREMES_GATEWAY_EXIT_DIAG": "1",
            "SANREMES_GATEWAY_MAX_STARTS": "0",
    ...
    from sanremes_cli._subprocess_compat import windows_detach_flags_without_breakaway
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["SANREMES_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "SANREMES_HOME": str(tmp_path),
        "SANREMES_GATEWAY_EXIT_DIAG": "0",
        "SANREMES_TEST_GATEWAY_OUTCOME": outcome,
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("SANREMES_S6_SUPERVISED_CHILD", raising=False)
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sanremes_constants.is_container", lambda: True)
    monkeypatch.setattr("sanremes_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("sanremes_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_spawn_detached_gateway_timestamps_stderr(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "get_sanremes_home", lambda: tmp_path)
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "sanremes-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test SANREMES_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="SANREMES_HOME=/home/alice/.sanremes"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(gateway, "has_legacy_sanremes_units", lambda: True)
    ...
    monkeypatch.setattr(gateway, "remove_legacy_sanremes_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "sanremes-gateway.service"
...
class TestReapUnsupervisedGatewayOrphansMacOS:
    ...
    gateway every time SanRemes Desktop opens (``sanremes serve`` calls
...
class TestReapUnsupervisedGatewayOrphansWindows:
    ...
    Scheduled-Task-supervised gateway every time SanRemes Desktop opens
    (``sanremes serve`` calls ``_reap_unsupervised_gateway_orphans`` during
...
class TestReaperCandidateIsSupervisorOwned:
    ...
    def test_windows_scheduled_task_gateway_spared_without_pidfile(self, monkeypatch):
        ...
        gateway_pid = 52615
        bootstrap_pid = 52616   # Task-launched `sanremes gateway run` bootstrap
        ...
        bootstrap = SimpleNamespace(
            pid=bootstrap_pid, parent=lambda: services, name=lambda: "sanremes-gateway.exe"
        )
        gw = SimpleNamespace(
            pid=gateway_pid, parent=lambda: bootstrap, name=lambda: "sanremes-gateway.exe"
        )
        ...
        orphan = SimpleNamespace(pid=orphan_pid, parent=lambda: None, name=lambda: "sanremes-gateway.exe")
    ...
    def test_windows_backstop_fails_open_when_bootstrap_exited(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        stranded = SimpleNamespace(pid=4242, parent=lambda: None, name=lambda: "sanremes-gateway.exe")
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "sanremes_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        """SanRemes_Gateway_* is Running => reaper returns False, kills nothing."""
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        ...
        # hardcoded "SanRemesGateway" would leave the guard dormant on every
        # standard install (task name is SanRemes_Gateway / SanRemes_Gateway_<p>).
        import sanremes_cli.gateway_windows as gateway_windows

        monkeypatch.setattr(
            gateway_windows, "get_task_name", lambda: "SanRemes_Gateway_testprof"
        )
        queried = []
        ...
        assert killed_pids == []
        assert queried == ["SanRemes_Gateway_testprof"]
    ...
    def test_windows_scheduled_task_running_returns_false_off_windows(self, monkeypatch):
        ...
        assert gateway._windows_scheduled_task_running("SanRemesGateway") is False
        assert gateway._windows_scheduled_task_supervises("SanRemesGateway") is False
        assert gateway._windows_scheduled_task_state("SanRemesGateway") is None
    ...
    def test_supervises_ready_and_queued_but_not_disabled(self, monkeypatch):
        ...
        for state, expected in states.items():
            ...
            assert gateway._windows_scheduled_task_supervises("SanRemes_Gateway") is expected, state
            assert gateway._windows_scheduled_task_running("SanRemes_Gateway") is (state == "Running")
...
def test_find_windows_gateway_services_maps_verified_pid_tree(monkeypatch):
    ...
    fake_psutil = SimpleNamespace(
        win_service_iter=lambda: [
            FakeService("SanRemesGateway", 100),
