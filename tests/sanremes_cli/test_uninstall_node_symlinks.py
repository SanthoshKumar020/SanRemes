"""Tests for sanremes_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $SANREMES_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the SanRemes-managed node dir.
"""
...
def _make_sanremes_node(sanremes_home: Path) -> Path:
    """Create a fake $SANREMES_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = sanremes_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    sanremes_home = fake_home / ".sanremes"
    _make_sanremes_node(sanremes_home)
    ...
    removed = uninstall.remove_node_symlinks(sanremes_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    sanremes_home = fake_home / ".sanremes"
    node_bin = _make_sanremes_node(sanremes_home)
