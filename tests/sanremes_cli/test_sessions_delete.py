...
def test_sessions_delete_accepts_unique_id_prefix(monkeypatch, capsys):
    ...
    monkeypatch.setattr(sanremes_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys,
        "argv",
        ["sanremes", "sessions", "delete", "20260315_092437_c9a6", "--yes"],
    )
...
def _run_prune(monkeypatch, capsys, argv_tail, candidates=None, skipped_open=0):
    """Run `sanremes sessions prune <argv_tail>` against a FakeDB, capturing
    ...
    monkeypatch.setattr(sanremes_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys, "argv", ["sanremes", "sessions", "prune", *argv_tail]
    )
...
def test_sessions_prune_bare_keeps_90_day_default(monkeypatch, capsys):
    """A truly bare `sanremes sessions prune` keeps the implicit 90-day cutoff."""
...
def test_sessions_prune_preview_shows_oldest_newest(monkeypatch, capsys):
    ...
    from sanremes_cli.session_filters import format_epoch
...
def test_sessions_prune_surfaces_matching_open_sessions(monkeypatch, capsys):
    ...
    assert "sanremes sessions delete <id>" in out
