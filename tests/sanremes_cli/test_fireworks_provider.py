...
from sanremes_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS, normalize_provider
...
class TestFireworksAliases:
    ...
    def test_providers_normalize_provider(self, alias):
        from sanremes_cli.providers import normalize_provider as normalize_in_providers
...
class TestFireworksOverlay:
    def test_overlay_exists(self):
        ...
        assert "fireworks" in SANREMES_OVERLAYS
        overlay = SANREMES_OVERLAYS["fireworks"]
...
class TestFireworksAuxiliary:
    ...
    def test_client_sends_attribution_headers(self, monkeypatch):
        ...
        assert headers["HTTP-Referer"] == "https://sanremes-agent.nousresearch.com"
        assert headers["X-Title"] == "SanRemes Agent"
        assert kwargs["base_url"] == "https://api.fireworks.ai/inference/v1"

    def test_client_sends_sanremes_user_agent(self, monkeypatch):
        ...
        headers = kwargs.get("default_headers", {})
        assert headers["User-Agent"].startswith("SanRemesAgent/")
