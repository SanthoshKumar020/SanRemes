...
profile switcher can target any profile's SANREMES_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via SANREMES_HOME.
"""
import json
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_sanremes_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_sanremes_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import sanremes_cli.mcp_config as mcp_config
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same SANREMES_HOME the rest
        ...
        monkeypatch.setattr(
            "sanremes_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_sanremes_home()))
            return None
        ...
        assert resp.json()["sanremes_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_sanremes_home(self, isolated_profiles, monkeypatch):
        import sanremes_cli.web_server as web_server

        monkeypatch.setattr(
            "sanremes_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["SANREMES_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "SANREMES_TUI_GATEWAY_URL" not in env
...
class TestProfileScopedAudio:
    ...
    def test_transcribe_runs_inside_target_profile_home(
        ...
        def _fake_transcribe(path):
            ...
            seen["home"] = str(get_sanremes_home())
