...
Bug class: the checkout's ``.py`` files change (``sanremes update``, manual
...
def _make_pycache(repo: Path, subdir: str = "sanremes_cli") -> Path:
...
def test_sweep_clears_pycache_when_checkout_changed(monkeypatch, tmp_path):
    ...
    (repo / sanremes_main._BYTECODE_FINGERPRINT_FILE).write_text(
        "git:refs/heads/main:" + "a" * 40, encoding="utf-8"
    )

    sanremes_main._sweep_stale_bytecode_if_checkout_changed()

    assert not cache.exists()
    # Stamp updated to the current fingerprint.
    recorded = (repo / sanremes_main._BYTECODE_FINGERPRINT_FILE).read_text(encoding="utf-8")
...
# ---------------------------------------------------------------------------
# Plugin-update sibling site: __pycache__ under ~/.sanremes/plugins/<name>
# ---------------------------------------------------------------------------
