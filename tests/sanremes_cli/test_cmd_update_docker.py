"""Tests for ``sanremes update`` / ``--check`` inside the Docker container.
...
fix, ``sanremes update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side SanRemes, not an update to the running container, so the message
...
from sanremes_cli.main import _cmd_update_check, cmd_update
...
@patch("sanremes_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``sanremes update`` inside Docker → friendly message + exit 2, no git calls.
    ...
    assert "docker pull nousresearch/sanremes-agent:latest" in out
...
def test_format_docker_update_message_contents():
    ...
    """
    from sanremes_cli.config import format_docker_update_message

    msg = format_docker_update_message()

    # Primary command — the entire reason this message exists.
    assert "docker pull nousresearch/sanremes-agent:latest" in msg
    ...
    assert "SANREMES_HOME" in msg or "/opt/data" in msg, (
