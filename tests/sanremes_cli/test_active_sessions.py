...
from sanremes_cli import active_sessions
...
def test_release_orphaned_leases_reclaims_only_unowned_own_pid_entries(tmp_path, monkeypatch):
    ...
    ``_prune_dead`` only fires when the owning pid dies, so a ``sanremes
...
def test_release_under_profile_home_override_targets_acquisition_registry(
    ...
    """Regression for #85431: a lease acquired against the root SANREMES_HOME
