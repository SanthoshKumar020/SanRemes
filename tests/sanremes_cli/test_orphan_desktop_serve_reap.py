"""Orphan Desktop-local ``sanremes serve`` reap at backend start.
...
from sanremes_cli.dashboard_procs import (
...
def test_desktop_local_serve_shape_matches_ephemeral_loopback():
    assert _is_desktop_local_serve_cmdline(
        "python -m sanremes_cli.main serve --host 127.0.0.1 --port 0"
    )
    assert _is_desktop_local_serve_cmdline(
        "sanremes serve --isolated --host 127.0.0.1 --port 0 --ssh-owner-nonce abc"
    )
    assert _is_desktop_local_serve_cmdline(
        "/venv/bin/sanremes serve --host=127.0.0.1 --port=0"
    )


def test_desktop_local_serve_shape_spares_fixed_port_and_non_serve():
    assert not _is_desktop_local_serve_cmdline(
        "sanremes serve --host 100.106.105.2 --port 9119 --skip-build"
    )
    assert not _is_desktop_local_serve_cmdline(
        "sanremes serve --host 127.0.0.1 --port 9119"
    )
    assert not _is_desktop_local_serve_cmdline("sanremes gateway run --replace")
    assert not _is_desktop_local_serve_cmdline(
        "vim notes about sanremes serve --port 0"
    )


def test_reap_only_kills_ppid1_local_serves():
    scanned = [
        (111, "sanremes serve --host 127.0.0.1 --port 0"),  # orphan local
        (222, "sanremes serve --host 127.0.0.1 --port 0"),  # still has parent
        (333, "sanremes serve --host 100.1.2.3 --port 9119"),  # fixed remote
        (444, "sanremes serve --isolated --host 127.0.0.1 --port 0"),  # orphan isolated
    ]
    ...
    with (
        patch(
            "sanremes_cli.dashboard_procs._scan_dashboard_processes",
            return_value=scanned,
        ),
        patch(
            "sanremes_cli.dashboard_procs._process_ppid",
            side_effect=lambda pid: ppids.get(pid),
        ),
        patch("os.kill", side_effect=fake_kill),
        patch("sys.platform", "darwin"),
    ):
        os.environ.pop("SANREMES_DESKTOP_CHILD_PID", None)
...
def test_reap_passes_child_pid_exclude_to_scan():
    with (
        ...
        patch("sys.platform", "darwin"),
        patch.dict(os.environ, {"SANREMES_DESKTOP_CHILD_PID": "999,111"}, clear=False),
    ):
...
def _valid_lock_payload(pid: int, ownership_id: str, spawn_nonce: str) -> dict:
    ...
    return {
        ...
        "logPath": f"~/.sanremes/desktop-ssh/{ownership_id}/{spawn_nonce}.log",
...
def test_valid_lockfile_payload_rejects_wrong_owner_and_shape():
    ...
    bad_log["logPath"] = "~/.sanremes/desktop-ssh/{oid}/other.log".format(oid=oid)
...
def test_reap_spare_lock_owned_ssh_remote_backend_of_foreign_client():
    ...
    scanned = [
        (555, "sanremes serve --host 127.0.0.1 --port 0"),  # lock-owned remote
        (666, "sanremes serve --host 127.0.0.1 --port 0"),  # genuine orphan
    ]
    ppids = {555: 1, 666: 1}
    terms: list[int] = []
    live = {555, 666}
...
def test_reap_spares_young_backend_until_desktop_can_write_lock():
    ...
    scanned = [(777, "sanremes serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
    ...
    with (
        ...
        patch("sanremes_cli.dashboard_procs._process_ppid", return_value=1),
        patch("os.kill", side_effect=fake_kill),
        patch("sys.platform", "darwin"),
    ):
        result = _reap_orphaned_desktop_local_serves(
...
def test_reap_spares_backend_when_process_age_is_unknown():
    scanned = [(778, "sanremes serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
...
def test_reap_age_boundary_makes_180_second_orphan_eligible():
    scanned = [
        (779, "sanremes serve --isolated --host 127.0.0.1 --port 0"),
        (780, "sanremes serve --isolated --host 127.0.0.1 --port 0"),
    ]
    terms: list[int] = []
    live = {779, 780}
...
def test_reap_spare_lock_owned_backend_even_without_exclude_match(tmp_path):
    ...
    spares a matching orphaned serve even when SANREMES_DESKTOP_CHILD_PID is
    ...
    scanned = [(4242, "sanremes serve --host 127.0.0.1 --port 0")]
    terms: list[int] = []
