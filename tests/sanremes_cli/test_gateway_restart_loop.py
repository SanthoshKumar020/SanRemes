...
Covers:
- Defense 1: gateway stop/restart refuse when _SANREMES_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "sanremes gateway uninstall",
        "sanremes  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "SANREMES GATEWAY RESTART",           # uppercase
    ])
    def test_sanremes_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.sanremes.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.sanremes/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l sanremes-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.sanremes.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.sanremes.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.sanremes.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.sanremes/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.sanremes.gateway-apollo:/a.plist' "
            "'ai.sanremes.gateway-cronus:/c.plist' 'ai.sanremes.gateway-plutus:/p.plist' "
            "'ai.sanremes.gateway:/Users/botuser/Library/LaunchAgents/ai.sanremes.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated sanremes note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.sanremes.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.sanremes.gateway',
        "launchctl kick'start' -k gui/501/ai.sanremes.gateway",
        "launchctl kick\\start -k gui/501/ai.sanremes.gateway",
        'launchctl "kickstart" -k gui/501/ai.sanremes.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.sanremes.gateway',
        "launchctl dis\\able gui/501/ai.sanremes.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.sanremes."gateway"',
        # Same class on the systemctl and sanremes-CLI branches.
        'systemctl re"start" sanremes-gateway',
        'sanremes gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.sanremes.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.sanremes.update-checker',
        'systemctl re"start" sanremes-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "sanremes cron list",
        "sanremes update",
        "sanremes config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `sanremes gateway start` is benign — starting a gateway from inside a
        ...
        "sanremes gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway sanremes
        # services must not be falsely blocked (the old `.*sanremes` matched any
        # sanremes token).
        "launchctl unload ai.sanremes.update-checker.plist",
        "launchctl restart ai.sanremes.daemon",
        ...
        "systemctl restart sanremes-meta.service",
        "systemctl restart sanremes-cron-helper",
        ...
        "echo after the sanremes gateway restarted cleanly",
        "the sanremes gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "sanremes skill view gateway-notes && echo sanremes gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `sanremes` is a path component
        # there, not a command.
        "cat '/docs/sanremes gateway restart-notes.md'",
        "less /home/user/notes/sanremes gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "sanremes gateway restart",
        "sanremes gateway restart; echo done",
        "sanremes gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;sanremes gateway restart",
        "true && sanremes gateway stop",
        "echo $(sanremes gateway restart)",
        "echo `sanremes gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.sanremes.gateway"])',
        'subprocess.run(["sanremes", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.sanremes.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["sanremes", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: sanremes gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `sanremes -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry SANREMES_HOME/SANREMES_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("SANREMES_PROFILE", "zeus")
        monkeypatch.delenv("SANREMES_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "sanremes -p zeus gateway stop",
        "sanremes -p zeus gateway restart",
        "sanremes --profile zeus gateway restart",
        "sanremes --profile zeus gateway stop",
        "sanremes --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "sanremes -v -p zeus gateway restart",
        "sanremes -p zeus -v gateway restart",
        "sanremes --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "sanremes -p venus gateway stop",
        "sanremes -p venus gateway restart",
        "sanremes --profile venus gateway restart",
        "sanremes --profile=venus gateway stop",
        "sanremes -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "sanremes -p zeus gateway start",
        "sanremes -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("sanremes gateway restart")
        assert _contains_gateway_lifecycle_command("sanremes gateway stop")

    def test_sanremes_home_derived_profile(self, monkeypatch):
        # Without SANREMES_PROFILE the guard falls back to the SANREMES_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("SANREMES_PROFILE", raising=False)
        monkeypatch.delenv("SANREMES_PROFILE_NAME", raising=False)
        import sanremes_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("sanremes -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("sanremes -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("sanremes -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("sanremes gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_sanremes_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade sanremes then run sanremes gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_sanremes_gateway_once.sh). The script must live under
        # SANREMES_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / ".sanremes"))
        scripts_dir = tmp_path / ".sanremes" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_refuses_inside_gateway(self, monkeypatch):
        ...
        from sanremes_cli.gateway import gateway_command
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SANREMES_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _SANREMES_GATEWAY=1.

    Issue #37453: systemctl --user restart sanremes-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart sanremes-gateway",
        "systemctl --user restart sanremes-gateway",
        "systemctl stop sanremes-gateway.service",
        "sanremes gateway restart",
        "sanremes gateway uninstall",
        "launchctl kickstart gui/501/ai.sanremes.gateway",
        "launchctl bootout gui/501/ai.sanremes.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.sanremes.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.sanremes/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.sanremes.gateway.restart-once.plist",
        "pkill -f sanremes.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart sanremes-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.sanremes.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-sanremes label: label-independent detection is the point
        # (#62891 second reproduction used `ai.sanremes.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.sanremes.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _SANREMES_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _SANREMES_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_SANREMES_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="sanremes gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["sanremes gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.sanremes.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.sanremes.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-sanremes systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run sanremes gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("sanremes gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfesanremes gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under SANREMES_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("sanremes gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/sanremes\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 sanremes gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_sanremes_home()` falls back to Path.home(), which raises
        RuntimeError when neither SANREMES_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart sanremes-gateway' .",
        "grep -rn 'sanremes gateway restart' ./logs",
        "rg 'sanremes gateway restart' ../archive",
        "grep -c 'systemctl stop sanremes-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM sanremes_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell sanremes gateway restart"',
        'sqlite3 ./db ".system systemctl restart sanremes-gateway"',
        'psql ./x -c "\\! systemctl restart sanremes-gateway"',
        "grep -r 'sanremes gateway restart' . | sh",
        "grep -r 'sanremes gateway restart' ./logs | bash",
        "grep -r 'sanremes gateway restart' . | sudo sh",
        "grep -r 'x' . ; sanremes gateway restart",
        "grep -r 'x' . && systemctl restart sanremes-gateway",
        'grep -r "$(sanremes gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart sanremes-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell sanremes gateway restart"',
        'sqlite3 db ".system systemctl restart sanremes-gateway"',
        'psql -c "\\! systemctl restart sanremes-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    sanremes_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run sanremes gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / ".sanremes"))
        (tmp_path / ".sanremes").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run sanremes gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop sanremes-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart sanremes-gateway' /var/log/syslog",
        "rg 'sanremes gateway restart' /home/user/.sanremes/logs/",
        "journalctl -u sanremes-gateway --grep 'systemctl restart sanremes-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'sanremes_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell sanremes gateway restart"',
        'psql -c "\\! systemctl restart sanremes-gateway"',
        "grep 'systemctl restart sanremes-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart sanremes-gateway",
        'grep "$(systemctl restart sanremes-gateway)" f',
        "grep 'restart' log; systemctl restart sanremes-gateway",
        'sqlite3 db "SELECT 1"; sanremes gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "sanremes gateway restart",
        "sudo systemctl stop sanremes-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart sanremes-gateway%'\""
        )
