...
from sanremes_cli.web_server import _SESSION_TOKEN, app


HEADERS = {"X-SanRemes-Session-Token": _SESSION_TOKEN}


@pytest.fixture
def catalog_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, _isolate_sanremes_home):
    ...
    monkeypatch.setenv("SANREMES_OPTIONAL_MCPS", str(catalog))
...
def test_catalog_rejects_undeclared_key_before_any_write_or_install(
    ...
    import sanremes_cli.mcp_catalog as mcp_catalog
...
def test_catalog_cannot_declare_reserved_control_key(
    ...
    response = client.post(
        "/api/mcp/catalog/install",
        headers=HEADERS,
        json={"name": "demo", "env": {"SANREMES_YOLO_MODE": "1"}},
    )
    ...
    assert installs == []
    env_path = catalog_env / ".env"
    assert not env_path.exists() or "SANREMES_YOLO_MODE" not in env_path.read_text(
        encoding="utf-8"
    )
...
def test_rejected_copilot_controls_do_not_change_live_resolvers(
    ...
    monkeypatch.setenv("SANREMES_COPILOT_ACP_COMMAND", "/opt/trusted/copilot")
    monkeypatch.setenv("SANREMES_COPILOT_ACP_ARGS", "--acp --stdio")
    expected_command = _resolve_command()
    expected_args = _resolve_args()

    attempts = {
        "SANREMES_COPILOT_ACP_COMMAND": "/tmp/attacker-command",
        "SANREMES_COPILOT_ACP_ARGS": "--malicious-transport",
    }
...
def test_preexisting_copilot_controls_remain_usable(
    ...
    monkeypatch.setenv("SANREMES_COPILOT_ACP_COMMAND", "parent-placeholder")
    monkeypatch.setenv("SANREMES_COPILOT_ACP_ARGS", "--parent-placeholder")
    (catalog_env / ".env").write_text(
        "SANREMES_COPILOT_ACP_COMMAND=/opt/operator/copilot\n"
        "SANREMES_COPILOT_ACP_ARGS=--acp --stdio --operator-mode\n",
        encoding="utf-8",
    )

    load_sanremes_dotenv(
        sanremes_home=catalog_env,
