"""Regression for #41403 — ``sanremes update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.sanremes.gateway-<profile>`` services kept running pre-update
...
import sanremes_cli.profiles
from sanremes_cli.gateway import (
...
from sanremes_cli.update_cmd import (
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed SANREMES_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.sanremes.gateway",
            "ai.sanremes.gateway-merit-ops",
            "ai.sanremes.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(sanremes_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        gui_loaded = {"ai.sanremes.gateway-a"}
        ...
        assert _locate_launchd_gateway_service("ai.sanremes.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.sanremes.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.sanremes.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.sanremes.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.sanremes.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.sanremes.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.sanremes.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.sanremes.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.sanremes.gateway", "ai.sanremes.gateway-a", "ai.sanremes.gateway-b"],
        )
        located = {
            "ai.sanremes.gateway": (f"gui/{UID}", 100),
            "ai.sanremes.gateway-a": (f"gui/{UID}", 200),
            "ai.sanremes.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through sanremes_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.sanremes.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.sanremes.gateway", current, "ai.sanremes.gateway-user-scoped"],
            located={
                "ai.sanremes.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.sanremes.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.sanremes.gateway",
            f"user/{UID}/ai.sanremes.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.sanremes.gateway",
            f"user/{UID}/ai.sanremes.gateway-user-scoped",
        ...
        assert restarted == [
            current,
            "ai.sanremes.gateway",
            "ai.sanremes.gateway-user-scoped",
        ]
        assert failed == []
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.sanremes.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.sanremes.gateway-a"],
            located={"ai.sanremes.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.sanremes.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.sanremes.gateway",
            labels=["ai.sanremes.gateway", "ai.sanremes.gateway-idle"],
            located={
                "ai.sanremes.gateway": (f"gui/{UID}", 100),
                "ai.sanremes.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.sanremes.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.sanremes.gateway",
            labels=["ai.sanremes.gateway", "ai.sanremes.gateway-dormant"],
            located={
                "ai.sanremes.gateway": (f"gui/{UID}", 100),
                "ai.sanremes.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.sanremes.gateway-dormant"]
        assert restarted == ["ai.sanremes.gateway", "ai.sanremes.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.sanremes.gateway",
            labels=["ai.sanremes.gateway", "ai.sanremes.gateway-a"],
            located={
                "ai.sanremes.gateway": (f"gui/{UID}", 100),
                "ai.sanremes.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.sanremes.gateway-a"]
        assert restarted == ["ai.sanremes.gateway", "ai.sanremes.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.sanremes.gateway",
                "ai.sanremes.gateway-bad",
                "ai.sanremes.gateway-good",
            ],
            located={
                "ai.sanremes.gateway": (f"gui/{UID}", 100),
                "ai.sanremes.gateway-bad": (f"gui/{UID}", 200),
                "ai.sanremes.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.sanremes.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.sanremes.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.sanremes.gateway-good"]
        assert restarted == ["ai.sanremes.gateway", "ai.sanremes.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.sanremes.gateway",
                "ai.sanremes.gateway-wedged",
                "ai.sanremes.gateway-after",
            ],
            located={
                "ai.sanremes.gateway": (f"gui/{UID}", 100),
                "ai.sanremes.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.sanremes.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.sanremes.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.sanremes.gateway-after"]
        assert restarted == ["ai.sanremes.gateway", "ai.sanremes.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.sanremes.gateway": (f"gui/{UID}", 100),
                "ai.sanremes.gateway-wedged": (f"gui/{UID}", 200),
                "ai.sanremes.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.sanremes.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.sanremes.gateway",
            labels=["ai.sanremes.gateway", "ai.sanremes.gateway-zombie"],
            located={
                "ai.sanremes.gateway": (f"gui/{UID}", 100),
                "ai.sanremes.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.sanremes.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.sanremes.gateway"]
        assert failed == ["ai.sanremes.gateway-zombie"]
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.sanremes.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
