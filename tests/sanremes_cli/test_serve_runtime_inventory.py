...
A network-bound `sanremes serve --host <ip>` powering a remote Desktop used to
be invisible to the update pipeline: not in the inventory, a dead-end at the
venv-holder guard, and never relaunched after `sanremes update` killed it. The
...
import sanremes_cli.update_inventory as update_inventory
...
def _ledger_entry(**over):
    entry = {
        ...
        "spawner_create": None,
        "registered_at": 222.0,
        "argv": "sanremes serve --host 100.94.65.93 --port 9119",
        "host": "100.94.65.93",
        "port": 9119,
        "profile": "",
    }
...
def test_register_self_records_structured_detail(tmp_path, monkeypatch):
    from sanremes_cli import process_identity as pi
...
def test_inventory_includes_manual_serve_from_ledger(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "sanremes_cli.process_identity", fake_pi)
...
def test_serve_relaunch_commands_built_from_structured_identity(monkeypatch):
    ...
    assert ["sanremes", "serve", "--host", "100.94.65.93", "--port", "9119"] in cmds
    assert ["sanremes", "--profile", "work", "serve", "--port", "9200"] in cmds
    assert ["sanremes", "dashboard", "--host", "0.0.0.0", "--port", "9300"] in cmds
...
def test_scan_dashboard_processes_includes_ledger_only_serves(monkeypatch):
    """A profiled serve (`sanremes --profile p serve ...`) matches no scan
    pattern; the ledger row must still surface it."""
    import sanremes_cli.dashboard_procs as dp

    profiled = _ledger_entry(
        pid=8123,
        argv="sanremes --profile work serve --host 100.94.65.93 --port 9119",
        profile="work",
    )
