...
@pytest.fixture
def profiles_on_disk(tmp_path, monkeypatch, _isolate_sanremes_home):
...
def client(monkeypatch, profiles_on_disk):
    ...
    from sanremes_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
    from sanremes_constants import get_sanremes_home
...
def _seed_project(home, name, folder):
    from sanremes_cli import projects_db
...
class TestCrossProfileProjectTree:
    ...
    def test_each_profile_contributes_its_own_projects_db(self, client, profiles_on_disk, tmp_path):
        ...
        filters through ``get_sanremes_home()``. If the fan-out failed to rebind
    ...
    def test_a_profile_that_cannot_be_read_does_not_sink_the_rest(
        ...
        def explode_for_worker(db, **kwargs):
            ...
            if get_sanremes_home().name == "worker":
