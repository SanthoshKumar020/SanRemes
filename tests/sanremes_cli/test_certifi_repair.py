...
1. The venv-repair import probes (early recovery + `sanremes update`) must
...
2. ``sanremes doctor`` must fail the certificate check in that state, and
   ``sanremes doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `sanremes update`'s venv repair must apply
    ...
    def _run_probe_script(self, monkeypatch, tmp_path, bundle_path):
        ...
        from sanremes_cli import main as main_mod
...
# =========================================================================
# 2. sanremes doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "sanremes doctor --fix" in message
