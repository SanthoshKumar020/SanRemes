...
option absent from both sets, so every ``sanremes --reasoning high <cmd> …``
invocation mis-classified ``high`` as the first positional and forced eager
plugin CLI discovery (~500-650ms startup tax).

Both sites now share ``sanremes_cli._parser.top_level_value_flag_sets()``,
...
from sanremes_cli.main import _first_positional_argv, _plugin_cli_discovery_needed
...
def test_first_positional_skips_reasoning_value(monkeypatch):
    monkeypatch.setattr("sys.argv", ["sanremes", "--reasoning", "high", "chat", "hello"])
    assert _first_positional_argv() == "chat"
    assert _plugin_cli_discovery_needed() is False


@pytest.mark.parametrize("argv", [
    ["sanremes", "--reasoning", "ultra", "--provider", "openai", "-z", "prompt here"],
    ["sanremes", "-m", "x", "--reasoning=low", "chat"],
])
