"""Tests for the AI_AGENT / SANREMES_AGENT harness-attribution env vars.
...
SanRemes-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal SanRemes' id in the public agent-harness
registry (``sanremes-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the SanRemes process env, and survives the
cross-session leak guard that strips ``SANREMES_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__SANREMES_CWD_testsession0__"
        env._snapshot_path = "/tmp/sanremes-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'SANREMES_AGENT="${SANREMES_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT SANREMES=$SANREMES_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "SANREMES_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} SANREMES=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", SANREMES_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi SANREMES=false" in out.stdout
