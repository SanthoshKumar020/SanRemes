...
from sanremes_cli.auth import AuthError, resolve_spotify_runtime_credentials
...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from sanremes_cli.auth_commands import auth_status_command
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    def _terminal_refresh(_state, **_kw):
        raise AuthError(
            "Spotify token refresh failed. Run `sanremes auth spotify` again.",
            provider="spotify",
