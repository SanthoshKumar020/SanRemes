"""Tests for the top-level `./sanremes` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./sanremes` should use `sanremes_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "sanremes"
    called = []

    fake_main_module = types.ModuleType("sanremes_cli.main")

    def fake_main():
        called.append("sanremes_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "sanremes_cli.main", fake_main_module)
    ...
    assert called == ["sanremes_cli.main"]
