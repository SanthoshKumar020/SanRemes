...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # SANREMES_HOME.  For a standard install (SANREMES_HOME unset) that root is
        # the platform-native default (~/.sanremes), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from sanremes_constants import get_default_sanremes_root
        assert env.get("SANREMES_HOME") == str(get_default_sanremes_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (SANREMES_DESKTOP=1) must NOT
