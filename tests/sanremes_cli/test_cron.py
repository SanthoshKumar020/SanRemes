"""Tests for sanremes_cli.cron command handling."""
...
from sanremes_cli import cron as cron_cli
from sanremes_cli.cron import cron_command
from sanremes_cli.subcommands.cron import build_cron_parser
...
class TestGatewayNotRunningWarning:
    ...
    def test_list_warns_when_gateway_absent(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.gateway.find_gateway_pids", lambda: [])
...
class TestExternalCronProviderStatus:
    ...
    def test_status_reports_provider_not_ticker_for_chronos(
        ...
        monkeypatch.setattr(
            "sanremes_cli.cron._active_cron_provider_name", lambda: "chronos"
        )
...
class TestCronRunBackgroundDispatch:
    """`sanremes cron run` must not report 'failed' when the run was dispatched
