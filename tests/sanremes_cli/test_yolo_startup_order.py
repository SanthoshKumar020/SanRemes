"""Regression tests for #60328: --yolo must set SANREMES_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing SANREMES_YOLO_MODE at the
    ...
    def spy_prepare_startup(args):
        yolo_at_startup["value"] = os.environ.get("SANREMES_YOLO_MODE")

    monkeypatch.setattr(
        "sanremes_cli.main._prepare_agent_startup", spy_prepare_startup
    )
    # Stub cmd_chat so main() returns cleanly without entering chat.
    monkeypatch.setattr("sanremes_cli.main.cmd_chat", lambda args: None)
    monkeypatch.delenv("SANREMES_YOLO_MODE", raising=False)
    monkeypatch.setattr(sys, "argv", argv)

    from sanremes_cli.main import main as cli_main
...
def test_top_level_yolo_flag_sets_env_before_startup(monkeypatch):
    """sanremes --yolo must set SANREMES_YOLO_MODE before
    _prepare_agent_startup imports tools.approval."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["sanremes", "--yolo"]
    )
    assert result == "1", (
        "SANREMES_YOLO_MODE was not '1' when _prepare_agent_startup was "
