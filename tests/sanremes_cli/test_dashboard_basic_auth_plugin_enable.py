...
from sanremes_cli.dashboard_auth import clear_providers, list_providers
from sanremes_cli.plugins import PluginManager, discover_plugins
from sanremes_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
class TestBasicProviderLoadsAfterUnblock:
    def test_disabled_basic_blocks_registration(self, sanremes_home, monkeypatch):
    ...
    def test_unblock_then_rediscover_registers_provider(
        self, sanremes_home, monkeypatch,
    ):
        password_hash = basic_plugin.hash_password("hunter2")
        cfg = {
            "dashboard": {
        ...
        _write_config(sanremes_home, cfg)
