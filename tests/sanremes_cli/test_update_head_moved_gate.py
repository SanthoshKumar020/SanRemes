"""Tests for the post-pull HEAD-movement gate in ``sanremes update``.
...
Before this guard ``sanremes update`` printed "✓ Code updated!" and
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the sanremes_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns sanremes_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/sanremes_cli/test_cmd_update.py).
    """
    ...
    monkeypatch.setattr(
        sanremes_main, "_resolve_update_branch", lambda args: "main"
    )
    monkeypatch.setattr(sanremes_main, "_is_windows", lambda: False)
    monkeypatch.setattr(
        sanremes_main, "_get_origin_url",
    ...
    monkeypatch.setattr(
        sanremes_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(sanremes_main, "_finish_dashboard_update_cleanup", lambda *a: None)
    ...
    monkeypatch.setattr(
        sanremes_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    sanremes_main.cmd_update(args)  # completes normally (no SystemExit)
