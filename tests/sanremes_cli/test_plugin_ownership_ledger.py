...
def _write_plugin(sanremes_home: Path) -> None:
    plugin_dir = sanremes_home / "plugins" / "ledger_probe"
...
def _write_profile_probe(sanremes_home: Path, marker: str) -> None:
    plugin_dir = sanremes_home / "plugins" / "profile_probe"
    plugin_dir.mkdir(parents=True)
    (plugin_dir / "plugin.yaml").write_text(
        yaml.safe_dump(
            {
...
def test_scoped_plugin_cannot_deregister_a_process_global_tool():
    ...
    module_name = "sanremes_plugins.scoped_cleanup"
...
def test_shared_entrypoint_module_uses_the_active_profile_scope(tmp_path):
    ...
    registry = ToolRegistry()
    module_name = "third_party.shared_sanremes_plugin"
    ...
    token = set_sanremes_home_override(home_a)
    try:
        with patch.object(ToolRegistry, "_caller_module", return_value=module_name):
...
def test_entrypoint_policy_uses_the_most_specific_module_prefix(tmp_path):
    ...
    token = set_sanremes_home_override(narrow_scope)
    try:
        registry.register(
...
def test_direct_plugin_platform_registration_infers_immutable_scope(tmp_path):
    ...
    module_name = "company.sanremes.direct_platform_probe"
    ...
    token = set_sanremes_home_override(home_b)
    try:
        platform_registry.register(
            PlatformEntry(
                name=name,
...
def test_same_name_tool_and_platform_are_isolated_by_sanremes_home(
    tmp_path,
    monkeypatch,
):
    ...
    token_a = set_sanremes_home_override(home_a)
    try:
    ...
    token_b = set_sanremes_home_override(home_b)
    try:
...
def test_manager_discovery_uses_its_home_not_the_ambient_profile(
    ...
    ambient = set_sanremes_home_override(home_b)
    try:
        manager_a.discover_and_load()
    finally:
        reset_sanremes_home_override(ambient)
