"""Tests for sanremes_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `sanremes setup` and `sanremes import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("sanremes_constants.is_container", lambda: container)
...
class TestEnsureGatewayService:
    ...
    def test_no_service_manager_is_noop(self, monkeypatch, capsys):
        ...
        assert gateway_mod.ensure_gateway_service() is False
        out = capsys.readouterr().out
        assert "sanremes gateway" in out
