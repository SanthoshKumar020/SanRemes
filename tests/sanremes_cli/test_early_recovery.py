"""Tests for sanremes_cli._early_recovery — the dependency-light bootstrap
repair that runs BEFORE sanremes_cli.main's third-party imports (#57828 / #58004).

Covers:
- entry-point lifecycle: a broken core import (dotenv) crashes the import of
  sanremes_cli.main WITHOUT early recovery, and imports fine when recovery runs
...
def _run_lifecycle_subprocess(tmp_path: Path, *, repair: bool) -> subprocess.CompletedProcess:
    ...
    script.write_text(
        textwrap.dedent(
            ...
            import sanremes_cli._early_recovery as er
    ...
    env = {
        **os.environ,
        ...
        "SANREMES_HOME": str(sanremes_home),
    }
...
def test_broken_dotenv_crashes_main_import_without_repair(tmp_path):
    """Negative control: the shadow really breaks importing sanremes_cli.main,
...
def test_early_recovery_module_is_stdlib_only(tmp_path):
    ...
    script.write_text(
        textwrap.dedent(
            ...
            STDLIB = set(sys.stdlib_module_names) | {"sanremes_cli"}
            ...
            import sanremes_cli._early_recovery  # noqa: F401
...
def test_core_marker_triggers_install_before_any_native_import(
    ...
    import sanremes_cli._install_repair  # noqa: F401  (pre-import for patch)
...
def test_core_marker_marks_attempts_and_keeps_marker_on_install_failure(
    ...
    import sanremes_cli._install_repair  # noqa: F401
...
def test_core_marker_from_dead_updater_is_recovered_on_update_retry(
    ...
    """Retrying ``sanremes update`` must consume a prior deferral marker.
