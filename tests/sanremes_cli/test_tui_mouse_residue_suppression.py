...
The Python launcher (`sanremes --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous SanRemes session
...
from sanremes_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["sanremes", "--tui"])
        monkeypatch.delenv("SANREMES_TUI", raising=False)
        monkeypatch.setenv("SANREMES_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["sanremes", "--tui"])
        monkeypatch.delenv("SANREMES_TUI", raising=False)
        monkeypatch.delenv("SANREMES_TUI_NO_EARLY_DISABLE", raising=False)
