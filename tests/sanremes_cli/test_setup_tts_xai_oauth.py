...
def test_run_xai_oauth_login_from_setup_does_not_hijack_active_provider(
    ...
    `active_provider`), so `sanremes setup tts` OAuth login hijacked the main
    chat provider.
    """
    ...
    monkeypatch.setattr("sanremes_cli.auth._is_remote_session", lambda: True)

    from sanremes_cli.auth import is_source_suppressed, suppress_credential_source
    from sanremes_cli.setup import _run_xai_oauth_login_from_setup
