...
from sanremes_cli.main import _read_ssh_session_token_file, cmd_dashboard
...
def test_token_anchor_is_os_home_not_active_profile(tmp_path, monkeypatch):
    ...
    ``$HOME/.sanremes/desktop-ssh`` (a literal ``~/.sanremes/desktop-ssh`` in
    apps/desktop/electron/remote-lifecycle.ts, expanded against the account's
    $HOME). A non-default sticky profile re-homes ``get_sanremes_home()`` to
    ``<root>/profiles/<name>``, and a Docker-style ``SANREMES_HOME`` can point
    elsewhere entirely — neither must move the validator off
    ``$HOME/.sanremes/desktop-ssh``, or the token is wrongly rejected."""
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    token_dir = home / ".sanremes" / "desktop-ssh" / ("a" * 32)
    token_dir.mkdir(parents=True, mode=0o700)
    token_path = token_dir / "0123456789abcdef.token"

    # A sticky profile and a custom (Docker) root both point get_sanremes_home()
    # away from $HOME/.sanremes; the anchor must ignore both.
    for elsewhere in (home / ".sanremes" / "profiles" / "coder", tmp_path / "opt" / "data"):
        token_path.write_text("b" * 64)
        token_path.chmod(0o600)
        override = set_sanremes_home_override(elsewhere)
        try:
            assert _read_ssh_session_token_file(str(token_path)) == "b" * 64
            assert not token_path.exists()
        finally:
...
def test_token_under_profile_desktop_ssh_is_rejected(tmp_path, monkeypatch):
    ...
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    profile_home = home / ".sanremes" / "profiles" / "coder"
    token_dir = profile_home / "desktop-ssh" / ("a" * 32)
    token_dir.mkdir(parents=True, mode=0o700)
    token_path = token_dir / "0123456789abcdef.token"
    token_path.write_text("b" * 64)
    token_path.chmod(0o600)
    override = set_sanremes_home_override(profile_home)
    try:
...
def test_token_file_rejects_symlink(tmp_path, monkeypatch):
    ...
    override = set_sanremes_home_override(home / ".sanremes")
    try:
...
def test_token_file_rejects_parent_escape(tmp_path, monkeypatch):
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    token_root = home / ".sanremes" / "desktop-ssh"
...
def test_windows_runtime_root_stays_at_machine_root_for_named_profile(tmp_path, monkeypatch):
    from sanremes_cli import windows_ssh_runtime

    machine_root = tmp_path / "custom-sanremes-root"
    monkeypatch.setenv("SANREMES_HOME", str(machine_root / "profiles" / "writer_2"))
