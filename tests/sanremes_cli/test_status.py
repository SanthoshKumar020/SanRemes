...
from sanremes_cli.status import show_status
...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    from sanremes_cli import status as status_mod
    ...
    monkeypatch.setattr(status_mod, "get_sanremes_home", lambda: tmp_path, raising=False)
    ...
    assert "Start with:   sanremes gateway" in output
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in sanremes status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.sanremes/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.sanremes/auth.json" in out
    ...
    def test_status_function_returns_none_does_not_crash(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: sanremes auth add xai-oauth)" in out


def test_show_status_reports_gateway_session_last_activity(monkeypatch, capsys, tmp_path):
    """sanremes status should surface freshest gateway last_active (#72016)."""
    from sanremes_cli import status as status_mod
    import sanremes_cli.auth as auth_mod
    import sanremes_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(sanremes_state, "SessionDB", _FakeDB)
