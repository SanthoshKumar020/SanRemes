...
def main_mod(monkeypatch):
    import sanremes_cli.main as mod
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_sanremes_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import sanremes_cli.oneshot as oneshot
        from sanremes_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """sanremes -z bypasses SanRemesCLI, but recall still needs SessionDB."""
    from sanremes_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "sanremes_state", mod("sanremes_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "sanremes_cli.config",
        mod("sanremes_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "sanremes_cli.models",
        mod("sanremes_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "sanremes_cli.runtime_provider",
        mod(
            "sanremes_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "sanremes_cli.tools_config",
        mod("sanremes_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["SANREMES_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/sanremes-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["SANREMES_MODEL"] == "nous/sanremes-test"
    assert env["SANREMES_INFERENCE_MODEL"] == "nous/sanremes-test"
    assert env["SANREMES_TUI_PROVIDER"] == "nous"
    assert env["SANREMES_INFERENCE_PROVIDER"] == "nous"
    assert env["SANREMES_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["SANREMES_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("sanremes-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_sanremes_ink(monkeypatch, main_mod, tmp_path):
    tui_dir = tmp_path / "ui-tui"
    tsx = tui_dir / "node_modules" / ".bin" / "tsx"
    ink_dir = tui_dir / "packages" / "sanremes-ink"
    ...
    monkeypatch.delenv("SANREMES_TUI_DIR", raising=False)
