"""Tests for the Command Installation check in sanremes doctor."""
...
import sanremes_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal SANREMES_HOME + PROJECT_ROOT for doctor tests."""
    ...
    sanremes_bin = venv_bin_dir / "sanremes"
    sanremes_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    ...
    return home, project, sanremes_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, sanremes_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "sanremes"
        wrong_target = tmp_path / "wrong_sanremes"
        ...
        assert cmd_link.resolve() == sanremes_bin.resolve()
