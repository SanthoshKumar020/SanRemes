...
def test_discover_all_plugins_includes_entrypoint_plugins(monkeypatch, tmp_path):
    ...
    dist = SimpleNamespace(
        version="0.1.0",
        metadata={"Summary": "Karpathy-style LLM Wikis for SanRemes"},
    )
    entry_point = SimpleNamespace(
        name="wiki",
        value="adapters.sanremes.cli_plugin",
    ...
    monkeypatch.setattr(
        "sanremes_cli.plugins.get_bundled_plugins_dir",
        lambda: bundled_dir,
    )
    monkeypatch.setattr(
    ...
    assert entries == [
        (
            "wiki",
            "0.1.0",
            "Karpathy-style LLM Wikis for SanRemes",
            "entrypoint",
            "adapters.sanremes.cli_plugin",
            "wiki",
        )
    ]
...
def test_declared_capabilities_for_entrypoint_uses_distribution_metadata(
    ...
    monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: user_dir)
    monkeypatch.setattr(
        "sanremes_cli.plugins.get_bundled_plugins_dir", lambda: bundled_dir
    )
    monkeypatch.setattr(
        plugins_cmd.importlib.metadata,
        "entry_points",
