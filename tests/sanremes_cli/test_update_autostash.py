...
from sanremes_cli import config as sanremes_config
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(sanremes_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(sanremes_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(sanremes_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(sanremes_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(sanremes_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(sanremes_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(sanremes_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(sanremes_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(sanremes_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    sanremes_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_sanremes_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import sanremes_constants

    monkeypatch.delattr(sanremes_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(sanremes_constants, "apply_subprocess_home_env")

    sanremes_main._reload_updated_runtime_modules()

    assert callable(sanremes_constants.apply_subprocess_home_env)
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sanremes_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        sanremes_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(sanremes_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        sanremes_main.cmd_update(SimpleNamespace())
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        sanremes_main, "_discard_stashed_changes",
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        sanremes_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "sanremes_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    sanremes_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    sanremes_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_update_parser_accepts_keep_stash():
    ...
    from sanremes_cli.subcommands.update import build_update_parser
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``sanremes update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(sanremes_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".sanremes-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by sanremes update (sanremes_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "sanremes-update-autostash")

    assert marker.exists(), (
        ".sanremes-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".sanremes-bootstrap-complete" not in status
