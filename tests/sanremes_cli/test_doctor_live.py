"""Tests for ``sanremes doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from sanremes_cli import doctor_live
from sanremes_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `sanremes doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("sanremes_cli.doctor.SANREMES_HOME", tmp_path / "home")
        monkeypatch.setattr("sanremes_cli.doctor.PROJECT_ROOT", tmp_path / "root")
