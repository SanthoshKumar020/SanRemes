...
External secret backends are useful during normal SanRemes startup, but the
...
""",
    ...
    dispatch = (
        "sanremes_main.cmd_update = lambda _args: 0\n"
        "sanremes_main.main()\n"
        if run_main
        else ""
    )
    probe = (
        "import json, sys\n"
        f"sys.argv = {argv!r}\n"
        "import sanremes_cli.main as sanremes_main\n"
        f"{dispatch}"
    ...
    result = subprocess.run(
        ...
        timeout=120,
        cwd=REPO_ROOT,
        env={**os.environ, "SANREMES_HOME": str(home), "BWS_ACCESS_TOKEN": ""},
    )
...
def test_update_startup_does_not_import_bitwarden_or_cryptography(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["sanremes", "update", "--check"])
...
def test_normal_startup_still_loads_enabled_external_secret_source(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["sanremes", "chat"])
...
def test_dotenv_loading_is_preserved_when_external_secrets_are_skipped(
    ...
    loaded = env_loader.load_sanremes_dotenv(
        sanremes_home=home,
