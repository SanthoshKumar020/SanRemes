"""Tests for the `sanremes memory reset` CLI command.
...
- Profile-scoped reset (uses SANREMES_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SANREMES_HOME with memory files."""
    sanremes_home = tmp_path / ".sanremes"
    memories = sanremes_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nSanRemes repo is at ~/.sanremes/sanremes-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `sanremes memory reset` is run.
    """
    from sanremes_constants import get_sanremes_home

    mem_dir = get_sanremes_home() / "memories"
    files_to_reset = []
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / ".sanremes"
        (sanremes_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
