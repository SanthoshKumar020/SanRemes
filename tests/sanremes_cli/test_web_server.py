"""Tests for sanremes_cli.web_server and related config utilities."""
...
from sanremes_cli.config import (
    reload_env,
    redact_key,
    OPTIONAL_ENV_VARS,
    DEFAULT_CONFIG,
...
@pytest.fixture
def _install_example_plugin(_isolate_sanremes_home):
    """Drop the example-dashboard fixture into the per-test SANREMES_HOME
    ...
    isolated ``SANREMES_HOME``.

    The user-plugin source is preferred over a transient
    ``SANREMES_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, sanremes-achievements, model providers
    ...
    from sanremes_cli import web_server

    user_plugins_dir = get_sanremes_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `sanremes plugins
    # enable example`.
    from sanremes_cli.config import load_config, save_config
    _cfg = load_config()
    ...
    #      and start serving requests against a torn-down SANREMES_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known SanRemes vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints SANREMES_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import sanremes_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("SANREMES_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("SANREMES_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_sanremes_home):
        """Create a TestClient and isolate the state DB under the test SANREMES_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")

        import sanremes_state
        from sanremes_constants import get_sanremes_home
        from sanremes_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(sanremes_state, "DEFAULT_DB_PATH", get_sanremes_home() / "state.db")
    ...
    def test_get_sessions_poll_preserves_pending_wal(self):
        ...
        db_path = get_sanremes_home() / "state.db"
    ...
    def test_get_status_loads_gateway_config_off_event_loop(self, monkeypatch):
        ...
        import sanremes_cli.web_server as web_server
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `sanremes update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `sanremes update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(sanremes_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = sanremes_state.SessionDB.__init__
        ...
        # the class on sanremes_state covers every open the helper makes.
        monkeypatch.setattr(sanremes_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(sanremes_state, "SessionDB", corrupt_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        SANREMES_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from sanremes_constants import get_sanremes_home
        from sanremes_cli.config import load_env
        ...
        config_path = get_sanremes_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from sanremes_constants import get_sanremes_home
        from sanremes_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide SANREMES_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from sanremes_constants import get_sanremes_home

        path = get_sanremes_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_sanremes_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["sanremes"]["workspace"] == "myws"
        assert cfg["hosts"]["sanremes"]["peerName"] == "eri"
        assert cfg["hosts"]["sanremes"]["environment"] == "local"
        assert cfg["hosts"]["sanremes"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["sanremes"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from sanremes_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_sanremes_returns_docker_guidance_without_spawning(self, monkeypatch):
        import sanremes_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn sanremes update")
        ...
        # method through sanremes_cli.config directly.
        monkeypatch.setattr(
            "sanremes_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_sanremes_action", fail_spawn)
        web_server._ACTION_PROCS.pop("sanremes-update", None)
        web_server._ACTION_RESULTS.pop("sanremes-update", None)

        resp = self.client.post("/api/sanremes/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "sanremes-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/sanremes-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/sanremes-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/sanremes-agent:latest" in line for line in status_data["lines"])

    def test_update_sanremes_returns_apt_guidance_without_spawning(self, monkeypatch):
        import sanremes_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn sanremes update")

        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through sanremes_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "sanremes_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        assert data["update_command"] == "pkg upgrade sanremes-agent"
        assert spawned is False

        check = self.client.get("/api/sanremes/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade sanremes-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "sanremes-update.log").write_text(
            "=== sanremes-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== sanremes update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== sanremes-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/sanremes-update/status?lines=2000")
        ...
        assert f"=== sanremes-update completed {action_id} ===" in data["lines"]

    def test_update_sanremes_spawns_with_action_id(self, monkeypatch):
        import sanremes_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_sanremes_action", fake_spawn)
        web_server._ACTION_PROCS.pop("sanremes-update", None)
        web_server._ACTION_RESULTS.pop("sanremes-update", None)
        ...
        assert resp.json() == {
            "ok": True,
            "pid": 12345,
            "name": "sanremes-update",
            "action_id": "a" * 32,
        }
        assert calls == [
            (["update"], "sanremes-update", {"SANREMES_ACTION_ID": "a" * 32})
        ]

    def test_update_sanremes_reuses_running_action(self, monkeypatch):
        import sanremes_cli.web_server as web_server
        ...
        monkeypatch.setattr(
            web_server,
            "_spawn_sanremes_action",
            lambda *_args, **_kwargs: pytest.fail("must not spawn a duplicate update"),
        )
        web_server._ACTION_PROCS["sanremes-update"] = Proc()
        web_server._ACTION_IDS["sanremes-update"] = "b" * 32

        try:
            resp = self.client.post("/api/sanremes/update")
        finally:
            web_server._ACTION_PROCS.pop("sanremes-update", None)
            web_server._ACTION_IDS.pop("sanremes-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/sanremes_cli/test_setup_hidden_env.py.
        """
        from sanremes_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a SanRemes provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from sanremes_cli.config import save_env_value
        from sanremes_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from sanremes_cli.config import save_env_value
        from sanremes_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "sanremes_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/SanRemesSetupBot/sanremes_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/SanRemesSetupBot/sanremes_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "sanremes_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_sanremes_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from sanremes_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from sanremes_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``sanremes model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from sanremes_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_numeric_yaml_provider_key_can_be_activated_and_deleted(self):
        ...
        """
        from sanremes_cli.config import get_config_path, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from sanremes_cli import profiles as profiles_mod
        from sanremes_cli.config import custom_endpoint_key_env
        from sanremes_constants import get_sanremes_home

        default_home = get_sanremes_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from sanremes_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from sanremes_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from sanremes_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from sanremes_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from sanremes_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_sanremes_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from sanremes_constants import get_sanremes_home
        import sanremes_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import sanremes_cli.tools_config as tools_config
        from sanremes_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        """
        import sanremes_cli.tools_config as tools_config
        from sanremes_cli.nous_account import NousPortalAccountInfo

        # Logged out of Nous Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "sanremes_cli.nous_subscription.get_nous_portal_account_info",
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "sanremes"
        save_config(config)
        ...
        assert "sanremes@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from sanremes_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from sanremes_cli.web_server import _denormalize_config_from_web
        from sanremes_cli.config import save_config
...
class TestStatusInstallId:
    ...
    SanRemes home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        import sanremes_cli.web_server as ws
        from sanremes_constants import get_default_sanremes_root
        ...
        id_file = get_default_sanremes_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """SANREMES_HOME=<root> and SANREMES_HOME=<root>/profiles/<name> resolve to
        the same id file — profiles share one physical install identity."""
        import sanremes_cli.web_server as ws

        root = tmp_path / "sanremes-root"
        ...
        monkeypatch.setenv("SANREMES_HOME", str(profile_home))
        monkeypatch.setattr(ws, "_INSTALL_ID_CACHE", {"value": None})
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from sanremes_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.sanremes/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_sanremes_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(sanremes_home, name="ocean"):
        themes_dir = sanremes_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="sanremes-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_sanremes_home):
        ...
        monkeypatch.setattr(
            sanremes_state, "DEFAULT_DB_PATH", get_sanremes_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``sanremes_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in sanremes_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_sanremes_home):
        ...
        # Pin the SessionDB to the isolated SANREMES_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_sanremes_home, _install_example_plugin):
        ...
        installs it into the per-test ``SANREMES_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into SANREMES_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, sanremes-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (sanremes-achievements is loaded by default).
        resp = self.client.get("/api/plugins/sanremes-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        SANREMES_HOME override scopes a request to another profile."""
        from sanremes_constants import (
            reset_sanremes_home_override,
            set_sanremes_home_override,
        )
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets SANREMES_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the sanremes root's plugins/ directory."""
        root = tmp_path / "sanremes-root"
...
# or ``sh -c 'printf …'``) instead of the real ``sanremes --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        import sanremes_cli.main as main_mod

        command = f"sanremes-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "SANREMES_CWD": str(tmp_path),
            "SANREMES_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["SANREMES_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from sanremes_cli.pty_bridge import PtyUnavailableError
        ...
        import sanremes_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from sanremes_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import sanremes_cli.main as cli_main
    import sanremes_cli.web_server as ws
    ...
    gateway_url = env.get("SANREMES_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_sanremes_home, _install_example_plugin):
        ...
        the per-test ``SANREMES_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_sanremes_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("SANREMES_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under SANREMES_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("SANREMES_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHeadlessServeTokenPage:
    """Headless `sanremes serve` must serve the Desktop token handshake page
    at `/` when the dashboard auth gate is off (#94227).

    The Electron renderer boots by fetching `/` and extracting
    ``window.__SANREMES_SESSION_TOKEN__`` for WebSocket auth. Headless serve
    ...
    def _headless_client(monkeypatch, *, gated: bool):
        ...
        monkeypatch.setenv("SANREMES_SERVE_HEADLESS", "1")
        spa_app = FastAPI()
    ...
    def test_root_serves_token_page_when_not_gated(self, monkeypatch):
        ...
        match = re.search(
            r'window\.__SANREMES_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")',
            resp.text,
        )
        ...
        assert "window.__SANREMES_AUTH_REQUIRED__=false" in resp.text
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/sanremes"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/sanremes/ds-assets/bg.png)" in prefixed.text
        assert "url(/sanremes/fonts-terminal/x.woff2)" in prefixed.text
