...
``sanremes auth add copilot`` or by ``_seed_from_env`` when the env var
is set in ``~/.sanremes/.env`` — the picker was silently dropping back to
...
from sanremes_cli.models import _resolve_copilot_catalog_api_key
...
class TestCopilotCatalogApiKeyResolution:
    ...
    def test_falls_back_to_pool_oauth_token(self):
        ...
        with patch(
            ...
            "sanremes_cli.copilot_auth.exchange_copilot_token",
