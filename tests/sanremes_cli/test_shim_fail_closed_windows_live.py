...
holding ``sanremes.exe`` open WITHOUT FILE_SHARE_DELETE, exactly like a
...
def held_shim(tmp_path: Path):
    ...
    shim = scripts / "sanremes.exe"
    shim.write_bytes(b"MZ fake shim")
    (scripts / "sanremes-gateway.exe").write_bytes(b"MZ fake shim")
...
def test_locked_shim_really_cannot_be_renamed(held_shim):
    ...
    with pytest.raises(OSError):
        os.rename(shim, shim.with_name("sanremes.exe.old.premise"))
...
def test_release_then_strict_quarantine_succeeds(tmp_path, monkeypatch):
    ...
    scripts = tmp_path / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    (scripts / "sanremes.exe").write_bytes(b"MZ fake shim")

    holder = subprocess.Popen(
        [sys.executable, "-c", _HOLDER_CODE, str(scripts / "sanremes.exe")],
        stdout=subprocess.PIPE,
        text=True,
    )
