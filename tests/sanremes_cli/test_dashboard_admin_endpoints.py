...
def _client():
    ...
    # Keep the state DB under the isolated SANREMES_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_sanremes_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        env_text = (sanremes_home / ".env").read_text()
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.sanremes/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `sanremes auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `sanremes auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_sanremes_home):
        ...
        (get_sanremes_home() / "memories").mkdir(parents=True, exist_ok=True)
    ...
    def test_reset_targets(self):
        ...
        mem = get_sanremes_home() / "memories"
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_sanremes_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_sanremes_action", fake_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "sanremes_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("sanremes-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # sanremes-index source advertises availability + featured search.
            idx = _Src("sanremes-index")
        ...
        assert {"official", "github", "sanremes-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/sanremes/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/sanremes/update`` runs ``sanremes update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import sanremes_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/sanremes/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/sanremes/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        import sanremes_cli.debug as dbg
        ...
        monkeypatch.setattr("sanremes_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `sanremes tools` configurator."""
    ...
    def test_save_env_writes_key_and_validates_allowlist(self):
        from sanremes_cli.config import get_env_value
...
# ---------------------------------------------------------------------------
# _spawn_sanremes_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_sanremes_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _SANREMES_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    # Isolate the module-global proc registry: _spawn_sanremes_action stores
    ...
    ws._spawn_sanremes_action(["gateway", "restart"], "gateway-restart")

    assert "_SANREMES_GATEWAY" not in captured["env"]
    assert captured["env"]["SANREMES_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    monkeypatch, _isolate_sanremes_home
):
    ...
    startup under SANREMES_DESKTOP=1 so the stale orphan is cleared first.
    """
    import sanremes_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from sanremes_cli.gateway, so patch it
    # on that module.
    import sanremes_cli.gateway as g
