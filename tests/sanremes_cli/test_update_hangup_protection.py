"""Tests for SIGHUP protection and stdout mirroring in ``sanremes update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``sanremes_cli/main.py``.  These exist so
that ``sanremes update`` survives a terminal disconnect mid-install
...
def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("SANREMES_ACTION_ID", "a" * 32)
    ...
    monkeypatch.setattr("sanremes_cli.update_cmd._branch_head_suffix", lambda: "")

    _print_update_completion("✓ Update complete!")

    assert capsys.readouterr().out.splitlines() == [
        "✓ Update complete!",
        f"=== sanremes-update completed {'a' * 32} ===",
    ]


def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("SANREMES_ACTION_ID", "not-safe\nforged")
    monkeypatch.setattr("sanremes_cli.update_cmd._branch_head_suffix", lambda: "")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import sanremes_cli.config as _cfg
        if hasattr(_cfg, "_SANREMES_HOME_CACHE"):
            _cfg._SANREMES_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "sanremes update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_sanremes_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "sanremes_cli.config.get_sanremes_home", _boom, raising=True
        )
