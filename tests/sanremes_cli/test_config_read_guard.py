...
Canonical owners:
  ...
  * ``sanremes_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    "sanremes_cli/config.py",
    "gateway/config.py",
    # _load_gateway_config()'s fallback path for tests that monkeypatch
    # gateway.run._sanremes_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/sanremes/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "sanremes_cli/managed_scope.py",
