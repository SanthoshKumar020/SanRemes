...
`sanremes --tui --resume latest --in ./dir` (and the classic-CLI equivalents)
...
def test_top_level_in_value_not_mistaken_for_subcommand(monkeypatch):
    # `sanremes --in chat` — "chat" is the flag's value, not the subcommand.
    import sys

    import sanremes_cli.main as mod

    monkeypatch.setattr(sys, "argv", ["sanremes", "--in", "chat", "--resume", "latest"])
