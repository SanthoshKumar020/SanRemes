...
Runs a real subprocess with a temp SANREMES_HOME whose config.yaml contains a
``${ENV_VAR}`` reference, plus a managed-scope overlay dir (SANREMES_MANAGED_DIR).
...
Subprocess (not in-process monkeypatching) so module-level ``_sanremes_home``
globals, managed-scope caches, and ``_under_pytest`` guards behave like
production: ``SANREMES_MANAGED_DIR`` is set explicitly, which bypasses the
...
def test_behavioral_read_gets_expansion_and_overlay_while_writeback_stays_raw(
    ...
    code = textwrap.dedent(
        ...
        import os
        from pathlib import Path
        saved = Path(server._sanremes_home, "config.yaml").read_text(encoding="utf-8")
    ...
    out = _run_py(
        code,
        {
            ...
            "SANREMES_MANAGED_DIR": str(managed_dir),
...
def test_writeback_roundtrip_byte_identical_when_unchanged(tmp_path):
    ...
    code = textwrap.dedent(
        ...
        p = Path(__import__('os').environ['SANREMES_HOME']) / 'config.yaml'
    ...
    out = _run_py(code, {"SANREMES_HOME": str(home)}, tmp_path)
