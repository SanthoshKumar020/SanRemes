...
The notice prints the most recent curator run summary on `sanremes update`,
...
session start) so users normally never see the rename map. `sanremes update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(sanremes_main)
