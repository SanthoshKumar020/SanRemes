...
import sanremes_cli.memory_setup as memory_setup
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_sanremes_home", lambda: tmp_path)
    monkeypatch.setattr("sanremes_cli.config.load_config", lambda: {"memory": {}})
...
# behavior is covered by tests/sanremes_cli/test_memory_setup_env_denylist.py,
# which exercises the current save_env_value-routed signature
# (env_writes, sanremes_home=None) \u2014 these three tests pinned the prior direct
...
def test_install_dependencies_force_reinstalls_versioned_specs(tmp_path, monkeypatch):
    ...
    so a downgraded/stripped bridge package is restored on sanremes update."""
...
def test_cmd_status_memory_tool_gate_disabled(capsys, monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.config.load_config", lambda: _cfg)
    # check_memory_requirements() reads the readonly loader, not load_config.
    monkeypatch.setattr(
        "sanremes_cli.config.load_config_readonly", lambda: _cfg, raising=False
    )
