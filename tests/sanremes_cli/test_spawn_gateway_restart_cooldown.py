...
class TestRepeatRestartWithinCooldown:
    ...
    @patch(
        "sanremes_cli.web_server._gateway_subcommand",
        return_value=["gateway", "restart"],
    )
    @patch("sanremes_cli.web_server._spawn_sanremes_action")
    @patch("sanremes_cli.web_server._ACTION_PROCS", {})
    def test_second_request_after_the_child_exits_is_coalesced(
        ...
        from sanremes_cli.web_server import _spawn_gateway_restart

        proc = _exited_proc()
        mock_spawn.return_value = proc

        with patch("sanremes_cli.gateway._reap_unsupervised_gateway_orphans"), patch(
            "sanremes_cli.web_server.time.monotonic", side_effect=[100.0, 103.5]
        ):
    ...
    def test_a_storm_of_requests_produces_exactly_one_restart(
        ...
        with patch("sanremes_cli.gateway._reap_unsupervised_gateway_orphans"), patch(
            "sanremes_cli.web_server.time.monotonic", side_effect=clock
        ):
    ...
    def test_cooldown_survives_the_action_table_being_cleared(
        ...
        with patch("sanremes_cli.gateway._reap_unsupervised_gateway_orphans"), patch(
            "sanremes_cli.web_server.time.monotonic", side_effect=[100.0, 102.0]
        ):
            _spawn_gateway_restart()
...
class TestCooldownReleases:
    ...
    def test_request_after_the_window_starts_a_real_restart(
        ...
        with patch("sanremes_cli.gateway._reap_unsupervised_gateway_orphans"), patch(
            "sanremes_cli.web_server.time.monotonic",
...
class TestExistingBehaviourIsPreserved:
    ...
    def test_live_child_is_still_reused_without_consulting_the_clock(
        ...
        with patch(
            "sanremes_cli.web_server._ACTION_PROCS", {"gateway-restart": live}
        ), patch(
            "sanremes_cli.web_server._ACTION_COMMANDS",
            {"gateway-restart": ("gateway", "restart")},
        ), patch(
            "sanremes_cli.gateway._reap_unsupervised_gateway_orphans"
        ):
