"""Tests for the post-pull syntax guard in ``sanremes update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``sanremes_cli/config.py``), the CLI
becomes unbootable — every ``sanremes`` invocation imports those files at
...
markers in ``sanremes_cli/config.py``; users who ran ``sanremes update`` in
the 7-minute window before #28458 landed could not run any ``sanremes``
command afterward.
"""
...
def test_capture_head_sha_returns_stripped_sha(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sanremes_main.subprocess, "run", fake_run)

    assert sanremes_main._capture_head_sha(["git"], tmp_path) == "deadbeefcafe"
...
def _populate_critical_tree(root: Path, *, broken_file: str | None = None) -> None:
    ...
    for relpath in sanremes_main._UPDATE_CRITICAL_FILES:
        path = root / relpath
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except sanremes_constants.py
    for relpath in sanremes_main._UPDATE_CRITICAL_FILES:
        if relpath == "sanremes_constants.py":
            continue
        path = tmp_path / relpath
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("# stub\n")

    ok, failing_path, error = sanremes_main._validate_critical_files_syntax(tmp_path)
...
# release; if a future ``sanremes update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
