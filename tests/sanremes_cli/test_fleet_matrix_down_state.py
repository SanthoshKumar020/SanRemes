...
def _setup(monkeypatch, tmp_path, record: dict):
    ...
    monkeypatch.setattr("sanremes_cli.profiles._get_default_sanremes_home", lambda: home)
    monkeypatch.setattr(
...
def test_killed_never_replaced_gateway_is_a_down_row(monkeypatch, tmp_path):
    _setup(
        monkeypatch,
        tmp_path,
        {"pid": _DEAD_PID, "gateway_state": "running", "kind": "sanremes-gateway",
         "code_version": "0.20.5"},
    )
...
def test_cleanly_stopped_gateway_is_not_down(monkeypatch, tmp_path):
    for benign_state in ("stopped", "startup_failed"):
        _setup(
            monkeypatch,
            tmp_path,
            {"pid": _DEAD_PID, "gateway_state": benign_state, "kind": "sanremes-gateway"},
        )
