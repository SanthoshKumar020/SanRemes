"""Tests for the per-profile Projects store (sanremes_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="SanRemes Agent", folders=["/tmp/sanremes"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "sanremes-agent"
    assert proj.name == "SanRemes Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/sanremes"
    assert [f.path for f in proj.folders] == ["/tmp/sanremes"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "sanremes-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SANREMES_HOME.
