"""Tests for sanremes_cli/loops.py — /loop recurring in-session wakeups."""
...
def sanremes_home(tmp_path, monkeypatch):
    """Isolated SANREMES_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    from sanremes_cli import goals
...
class TestParseIntervalToken:
    def test_minutes(self):
        from sanremes_cli.loops import parse_interval_token
...
class TestParseLoopArgs:
    def test_fixed_interval(self):
        from sanremes_cli.loops import parse_loop_args
...
class TestFormatInterval:
    def test_render(self):
        from sanremes_cli.loops import format_interval
...
class TestResponseSignalsComplete:
    def test_marker_on_own_line(self):
        from sanremes_cli.loops import response_signals_complete
...
class TestLoopStateSerde:
    def test_round_trip(self):
        from sanremes_cli.loops import LoopState
...
class TestPersistence:
    def test_save_load_clear(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, load_loop
    ...
    def test_list_active_loops(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, list_active_loops
    ...
    def test_migrate_to_session(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, load_loop, migrate_loop_to_session
    ...
    def test_migrate_no_source(self, sanremes_home):
        from sanremes_cli.loops import migrate_loop_to_session

        assert migrate_loop_to_session("nope", "child2") is False
        assert migrate_loop_to_session("same", "same") is False

    def test_migrate_does_not_clobber_child(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, load_loop, migrate_loop_to_session
...
class TestTickLifecycle:
    def test_not_due_immediately(self, sanremes_home):
    ...
    def test_due_after_interval(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_fire_marks_awaiting_and_blocks_refire(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_slash_prompt_returned_raw(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_abandon_tick_rolls_back(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_complete_tick_marker_stops(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_complete_tick_times_cap(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_complete_tick_continues_and_reschedules(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_complete_tick_max_ticks_pauses(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_until_judge_done_stops(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("sanremes_cli.goals.judge_goal", return_value=("done", "suite green", False, None, False)):
            decision = mgr.complete_tick("All 500 tests passed.")
        assert decision["stopped"] is True
        assert decision["status"] == "done"

    def test_until_judge_continue_loops(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("sanremes_cli.goals.judge_goal", return_value=("continue", "3 failures", False, None, False)):
            decision = mgr.complete_tick("3 tests still failing")
        assert decision["stopped"] is False

    def test_until_judge_error_fails_open(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("sanremes_cli.goals.judge_goal", side_effect=RuntimeError("api down")):
            decision = mgr.complete_tick("some output")
        assert decision["stopped"] is False  # fail-open: keep looping


class TestSelfPacedBackoff:
    def test_backoff_doubles_on_unchanged_and_resets_on_change(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_timestamp_only_changes_do_not_reset(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
...
class TestControls:
    def test_min_interval_enforced(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, min_interval_seconds

        mgr = LoopManager(session_id="c1")
        state = mgr.set("poll", interval_seconds=1)
        assert state.interval_seconds >= min_interval_seconds()

    def test_pause_resume(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_paused_mid_tick_clears_awaiting(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
    ...
    def test_status_line_shapes(self, sanremes_home):
        from sanremes_cli.loops import LoopManager
...
class TestGoalMixing:
    def test_active_goal_blocks_tick(self, sanremes_home):
    ...
    def test_no_goal_does_not_block(self, sanremes_home):
        from sanremes_cli.loops import goal_blocks_loop_tick

        assert goal_blocks_loop_tick("g2") is False

    def test_paused_goal_does_not_block(self, sanremes_home):
        from sanremes_cli.goals import GoalManager
        from sanremes_cli.loops import goal_blocks_loop_tick
    ...
    def test_parked_goal_does_not_block(self, sanremes_home):
        from sanremes_cli.goals import GoalManager
        from sanremes_cli.loops import goal_blocks_loop_tick
...
class TestDispatchLoopCommand:
    def test_create_fixed(self, sanremes_home):
    ...
    def test_create_self_paced(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, dispatch_loop_command
    ...
    def test_status_empty(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, dispatch_loop_command
    ...
    def test_pause_resume_stop(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, dispatch_loop_command
    ...
    def test_route_stored(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, dispatch_loop_command, load_loop
    ...
    def test_help(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, dispatch_loop_command
    ...
    def test_bad_times_error(self, sanremes_home):
        from sanremes_cli.loops import LoopManager, dispatch_loop_command
...
class TestListMetaPrefix:
    def test_prefix_scan(self, sanremes_home):
    ...
    def test_wildcards_escaped(self, sanremes_home):
        from sanremes_state import SessionDB
    ...
    def test_empty_prefix(self, sanremes_home):
        from sanremes_state import SessionDB
