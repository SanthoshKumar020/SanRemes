...
import sanremes_cli.backup as backup
...
def profiles(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "sanremes_cli.profiles._get_default_sanremes_home", lambda: default_home
    )
    monkeypatch.setattr(
        "sanremes_cli.profiles._get_profiles_root", lambda: tmp_path / "home" / "profiles"
    )
    monkeypatch.setattr(
        "sanremes_cli.profiles._PROFILE_ID_RE",
...
class TestSiblingEnumeration:
    ...
    def test_never_raises(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("sanremes_cli.profiles._get_default_sanremes_home", _boom)
...
class TestAllProfileSnapshots:
    ...
    def test_one_failing_sibling_does_not_block_others(self, profiles, monkeypatch):
        ...
        def _flaky(label=None, sanremes_home=None, keep=None, max_file_size=None):
            if sanremes_home == profiles["work"]:
                raise OSError("disk full")
            return real(
                label=label, sanremes_home=sanremes_home, keep=keep,
