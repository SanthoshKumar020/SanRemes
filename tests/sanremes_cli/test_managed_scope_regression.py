...
def sanremes_home(tmp_path, monkeypatch):
    ...
    # /etc/sanremes on the dev/CI box can't influence the test.
    monkeypatch.setenv("SANREMES_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(sanremes_home, monkeypatch):
...
def test_env_expansion_in_user_config(sanremes_home, monkeypatch):
    from sanremes_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_sanremes_dotenv(sanremes_home=str(home))
