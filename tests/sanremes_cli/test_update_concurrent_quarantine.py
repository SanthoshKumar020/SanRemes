"""Tests for issue #26670 — concurrent sanremes.exe detection and improved
quarantine retry / reboot-deferred fallback during `sanremes update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_sanremes_instances
# helper (and need the autouse stub in tests/sanremes_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_sanremes_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "sanremes.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "sanremes.exe"),
    ]
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_sanremes_instances(scripts_dir)

    # No crash; helper completes. (Degenerate stub: launcher exe unreadable.)
    assert result == [(launcher_pid, "sanremes.exe")]
...
# ---------------------------------------------------------------------------
# _quarantine_running_sanremes_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "sanremes.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_sanremes_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("sanremes.exe.old.")
...
def test_quarantine_reports_a_lock_it_cannot_break(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_sanremes_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "sanremes_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "sanremes_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_pause_and_resume_windows_gateway_service(
    ...
    assert token == {
        ...
        "unmapped_pids": [],
        "unmapped": [],
        "services": ["SanRemesGateway"],
        "expected_services": ["SanRemesGateway"],
        "restarted_services": [],
        "service_profiles": {"SanRemesGateway": "default"},
    }
    assert stopped == ["SanRemesGateway"]

    cli_main._resume_windows_gateways_after_update(token)
    assert started == ["SanRemesGateway"]
...
def test_pause_windows_gateway_service_failure_restores_every_attempted_service(
    ...
    services = [
        SimpleNamespace(name="SanRemesGateway", service_pid=11, service_create_time=11.0, gateway_pid=101, gateway_create_time=101.0, descendant_identities=()),
        SimpleNamespace(name="SanRemesGatewayPicasso", service_pid=22, service_create_time=22.0, gateway_pid=202, gateway_create_time=202.0, descendant_identities=()),
    ]
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
    ...
    def fake_stop(name, **_kwargs):
        if name == "SanRemesGatewayPicasso":
    ...
    with pytest.raises(RuntimeError, match="SanRemesGatewayPicasso"):
        cli_main._pause_windows_gateways_for_update()

    assert restarted == ["SanRemesGatewayPicasso", "SanRemesGateway"]
...
def test_pause_windows_gateway_service_surfaces_rollback_start_failure(
    ...
    with pytest.raises(RuntimeError, match="rollback failures: SanRemesGateway"):
        cli_main._pause_windows_gateways_for_update()
...
def test_restore_windows_gateway_service_waits_out_stop_pending(monkeypatch):
    ...
    update_cmd._restore_windows_gateway_service("SanRemesGateway")

    assert restarted == ["SanRemesGateway"]
...
def test_resume_windows_gateway_service_failure_stays_retryable(
    ...
    with pytest.raises(RuntimeError, match="SanRemesGateway"):
        cli_main._resume_windows_gateways_after_update(token)

    assert token["resume_needed"] is True
    assert token["services"] == ["SanRemesGateway"]
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m sanremes_cli.main gateway run"
...
# when every concurrent sanremes.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, sanremes.exe shim, sanremes-gateway.exe,
    gateway/run.py, bare `sanremes gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "sanremes_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\sanremes.exe", "gateway", "run"],
        [r"C:\venv\Scripts\sanremes-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["sanremes.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["sanremes.exe", "gateway"],  # bare `sanremes gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["sanremes.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\sanremes.exe"],  # interactive REPL
        [r"C:\venv\Scripts\sanremes.exe", "dashboard"],
        ["sanremes.exe", "gateway", "status"],  # management, not runtime
        ["sanremes.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["sanremes.exe", "gateway", "run"],
                200: ["sanremes.exe"],  # REPL — keep
                300: ["sanremes.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "sanremes.exe"), (300, "sanremes.exe"), (400, "sanremes.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["sanremes.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\sanremes-gateway.exe"],
            }
        ),
    )
    matches = [(111, "sanremes.exe"), (222, "sanremes-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_sanremes_instances",
        return_value=[(1000, "sanremes.exe"), (2000, "sanremes-gateway.exe")],
    ), patch.object(
    ...
    captured = capsys.readouterr().out
    assert "Another sanremes.exe is running" not in captured
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_sanremes_instances",
        return_value=[(1000, "sanremes.exe"), (3000, "sanremes.exe")],
    ), patch.object(
        cli_main,
        "_filter_non_gateway_concurrent_instances",
        return_value=[(3000, "sanremes.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
...
def test_stop_service_refuses_pid_reuse_before_sc_stop(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="identity changed"):
        update_cmd._stop_windows_gateway_service(
            "SanRemesGateway", expected_service_identity=(11, 11.0)
        )
