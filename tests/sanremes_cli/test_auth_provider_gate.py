...
def test_vertex_adc_counts_as_explicit_when_config_present(tmp_path, monkeypatch):
    ...
    SanRemes at it (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH),
    ...
    # vertex.project_id in config.yaml is a deliberate, SanRemes-scoped signal.
    ...
    # No SanRemes-scoped Vertex config at all → stays hidden.
...
def test_vertex_ambient_google_creds_env_does_not_count_as_explicit(tmp_path, monkeypatch):
    ...
    unrelated GCP work) must NOT mark Vertex explicit — only SanRemes-scoped
...
def test_bedrock_not_explicit_without_aws_env(tmp_path, monkeypatch, _clean_aws_env):
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "sanremes"))
    (tmp_path / "sanremes").mkdir(parents=True, exist_ok=True)
...
def test_bedrock_ambient_aws_profile_is_not_explicit(tmp_path, monkeypatch, _clean_aws_env):
    ...
    not an explicit SanRemes provider choice — same principle as gh_cli-seeded
