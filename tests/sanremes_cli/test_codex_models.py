...
from sanremes_cli.codex_models import (
...
def test_curated_codex_fallback_excludes_chatgpt_rejected_pro_slugs(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.codex_models._fetch_models_from_api",
...
def test_picker_never_synthesizes_900k_for_pro_or_unknown_slugs():
    ...
    from sanremes_cli.codex_models import _finalize_codex_models
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in sanremes_cli/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from sanremes_cli.codex_models import get_codex_model_ids as setup_import
...
def test_fetch_from_api_keeps_supported_in_api_false_models(monkeypatch):
    ...
    """
    import sys
    from sanremes_cli import codex_models
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    from sanremes_cli.main import _model_flow_openai_codex
    ...
    monkeypatch.setattr(
        "sanremes_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True, "source": "sanremes-auth-store"},
    )
    monkeypatch.setattr(
        "sanremes_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("sanremes_cli.auth._login_openai_codex", _fake_login)
    monkeypatch.setattr(
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a SanRemesCLI with minimal mocking."""
    ...
    clean_env = {"LLM_MODEL": "", "SANREMES_MAX_ITERATIONS": ""}
    with (
        patch("cli.get_tool_definitions", return_value=[]),
        patch.dict("os.environ", clean_env, clear=False),
        patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
    ):
        cli = SanRemesCLI(model=model, **kwargs)
    return cli
...
class TestNormalizeModelForProvider:
    ...
    def test_default_model_replaced(self):
        ...
        with (
            patch("cli.get_tool_definitions", return_value=[]),
            patch.dict("os.environ", {"LLM_MODEL": "", "SANREMES_MAX_ITERATIONS": ""}, clear=False),
            patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
        ):
            from cli import SanRemesCLI
            cli = SanRemesCLI()
