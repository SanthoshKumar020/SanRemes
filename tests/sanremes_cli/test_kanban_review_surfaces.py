...
from sanremes_cli import kanban as kc
...
def review_worker(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> str:
    ...
    monkeypatch.setenv("SANREMES_PROFILE", "builder")
    ...
    monkeypatch.setenv("SANREMES_KANBAN_TASK", task_id)
    monkeypatch.setenv("SANREMES_KANBAN_RUN_ID", str(task.current_run_id))
    return task_id
...
def test_review_tools_redact_handoff_and_route_changes(
    ...
    monkeypatch.setenv("SANREMES_PROFILE", "reviewer")
    monkeypatch.setenv("SANREMES_KANBAN_RUN_ID", str(review.current_run_id))
...
def test_review_tools_are_gated_and_visible_to_kanban_workers(
    ...
    definitions = registry.get_definitions(
        set(resolve_toolset("sanremes-cli")), quiet=True
    )
    names = {
    ...
    from agent.transports.sanremes_tools_mcp_server import EXPOSED_TOOLS
...
def test_review_cli_round_trip_preserves_handoff(
    ...
    monkeypatch.setenv("SANREMES_KANBAN_TASK", task_id)
    monkeypatch.setenv("SANREMES_KANBAN_RUN_ID", str(implementation.current_run_id))
...
def test_worker_guidance_distinguishes_same_card_and_downstream_review() -> None:
    ...
    from sanremes_cli.config_defaults import DEFAULT_CONFIG
...
def test_goal_mode_review_handoff_cannot_bypass_judge(
    ...
    monkeypatch.setenv("SANREMES_KANBAN_TASK", tool_task)
    monkeypatch.setenv("SANREMES_KANBAN_RUN_ID", str(claimed.current_run_id))
    ...
    monkeypatch.setenv("SANREMES_KANBAN_TASK", cli_task)
    monkeypatch.setenv("SANREMES_KANBAN_RUN_ID", str(cli_claimed.current_run_id))
