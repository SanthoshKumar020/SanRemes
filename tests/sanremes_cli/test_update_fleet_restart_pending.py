...
A ``sanremes update`` killed after git pull advanced HEAD but before the
...
- next ``sanremes update`` with git already up to date still runs the
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(sanremes_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(sanremes_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(sanremes_main, "_resolve_update_branch", lambda args: "main")
    monkeypatch.setattr(sanremes_main, "_is_windows", lambda: False)
    monkeypatch.setattr(
        sanremes_main,
        "_get_origin_url",
        lambda *a, **k: "https://github.com/NousResearch/sanremes-agent.git",
    )
    monkeypatch.setattr(sanremes_main, "_is_fork", lambda *a, **k: False)
    monkeypatch.setattr(
        sanremes_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(sanremes_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        sanremes_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(sanremes_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
        sanremes_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        sanremes_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(sanremes_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(sanremes_main, "_clear_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(
        sanremes_main, "_finish_dashboard_update_cleanup", lambda *a, **k: None
    )
    monkeypatch.setattr(sanremes_main, "_build_web_ui", lambda *a, **k: None)
    monkeypatch.setattr(
        update_cmd, "_venv_core_imports_healthy", lambda: (True, "")
    )
    monkeypatch.setattr(update_cmd, "_update_node_dependencies", lambda: [])

    import sanremes_cli.gateway as sanremes_gateway

    monkeypatch.setattr(
        sanremes_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(sanremes_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
        sanremes_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "sanremes_cli.update_inventory.collect_runtime_inventory",
...
def test_marker_round_trip_under_sanremes_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_sanremes_home()
...
def test_pending_needed_when_unfinished_receipt_runtime_sha_skews(monkeypatch):
    ...
    receipt_dir = get_sanremes_home() / "logs" / "update_receipts"
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "sanremes_cli.gateway.find_gateway_pids", lambda **k: []
    )
    monkeypatch.setattr(sanremes_main, "_purge_stale_sanremes_modules", lambda: None)
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(sanremes_main, "_clear_bytecode_cache", _interrupt)
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "sanremes gateway restart" in err
