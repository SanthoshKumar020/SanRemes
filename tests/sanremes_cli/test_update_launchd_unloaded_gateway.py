"""Regression for #74973 — `sanremes update` must not leave the gateway down.
...
cron stayed dark until someone manually ran ``sanremes gateway restart``.
...
def launchd(monkeypatch):
    """Stub sanremes_cli.gateway so no real launchctl call is made."""
    calls: list[str] = []
    ...
    monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.sanremes.gateway", raising=False)
...
class TestLaunchdRestartAfterUpdate:
    def test_plist_present_always_restarts_without_classifying(self, launchd, capsys):
        ...
        assert update_cmd._restart_launchd_gateway_after_update(supervision_verify=False) == (["ai.sanremes.gateway"], [])
    ...
    def test_restart_failure_warns_that_gateway_is_down(self, launchd, capsys):
        ...
        assert "sanremes gateway restart" in out
...
ai.sanremes.gateway = {
\tactive count = 1
\tpath = /Users/u/Library/LaunchAgents/ai.sanremes.gateway.plist
\tstate = running
\tpid = 59038
\tprogram = /Users/u/.sanremes/bin/sanremes
}
"""
...
class TestServicePidSweepExclusion:
    ...
    def macos_launchd(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.sanremes.gateway")
    ...
    def test_list_failure_falls_back_to_domain_print(self, macos_launchd):
        ...
        from sanremes_cli.gateway import _get_service_pids
...
class TestParseLaunchdPidFromPrintOutput:
    def test_running_service(self):
        from sanremes_cli.gateway import _parse_launchd_pid_from_print_output
