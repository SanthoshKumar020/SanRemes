...
``sanremes gateway enroll --connector-url/--wake-url`` persists GATEWAY_RELAY_URL /
...
from sanremes_cli.gateway_enroll import _warn_if_secondary_multiplex_profile
...
def test_fires_for_secondary_when_default_root_has_multiplex_on(topology, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(topology / "profiles" / "alice"))
...
def test_silent_for_default_profile_even_with_multiplex_on(topology, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(topology))
...
def test_silent_for_unrelated_dir_named_profiles(topology, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(other))
