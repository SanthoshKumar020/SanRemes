"""Tests for sanremes_cli.agent_import — ``sanremes import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real SanRemes stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Uses the profile_env fixture pattern from tests/sanremes_cli/test_profiles.py:
Path.home() and SANREMES_HOME are redirected to tmp_path so nothing touches
the real ~/.sanremes.
"""
...
from sanremes_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and SANREMES_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, SANREMES_HOME -> tmp/.sanremes."""
...
@pytest.fixture()
def sanremes_home(profile_env):
    return profile_env / ".sanremes"
...
def run_import(agent, source, sanremes_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=sanremes_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, sanremes_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, sanremes_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, sanremes_home):
        before = snapshot_tree(sanremes_home)
        report = run_import("claude-code", claude_tree, sanremes_home, execute=False)
        assert snapshot_tree(sanremes_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, sanremes_home):
        preview = run_import("claude-code", claude_tree, sanremes_home, execute=False)
        applied = run_import("claude-code", claude_tree, sanremes_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, sanremes_home):
        return run_import("claude-code", claude_tree, sanremes_home, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, sanremes_home):
        config = yaml.safe_load((sanremes_home / "config.yaml").read_text())
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, sanremes_home):
        return run_import("codex", codex_tree, sanremes_home, execute=True)


    def test_mcp_servers_from_config_toml(self, report, sanremes_home):
        config = yaml.safe_load((sanremes_home / "config.yaml").read_text())
    ...
    def test_skill_copied(self, report, sanremes_home):
        assert (sanremes_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, sanremes_home):
        run_import("claude-code", claude_tree, sanremes_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in sanremes_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, sanremes_home):
        run_import("codex", codex_tree, sanremes_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in sanremes_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, sanremes_home):
        report = run_import("claude-code", claude_tree, sanremes_home, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, sanremes_home):
        run_import("claude-code", claude_tree, sanremes_home, execute=True)
        config = yaml.safe_load((sanremes_home / "config.yaml").read_text())
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, sanremes_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, sanremes_home, execute=True)
        ...
        assert "still importable" in (
            sanremes_home / "memories" / "MEMORY.md").read_text()


    def test_empty_tree_all_skipped(self, profile_env, sanremes_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, sanremes_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_mcp_server_conflicts_without_overwrite(
            self, claude_tree, sanremes_home):
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, sanremes_home):
        dest = sanremes_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, sanremes_home):
        run_import("claude-code", claude_tree, sanremes_home, execute=True)
        first = (sanremes_home / "memories" / "MEMORY.md").read_text()
        report = run_import("claude-code", claude_tree, sanremes_home, execute=True)
        assert (sanremes_home / "memories" / "MEMORY.md").read_text() == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, sanremes_home):
        memory = sanremes_home / "memories" / "MEMORY.md"
        memory.parent.mkdir(parents=True, exist_ok=True)
        memory.write_text(EXISTING_MEMORY, encoding="utf-8")
        return sanremes_home
...
class TestExistingConfigPreserved:
    ...
    @pytest.fixture()
    def config_path(self, sanremes_home):
        return sanremes_home / "config.yaml"
    ...
    def test_malformed_config_is_left_byte_identical(
            self, claude_tree, sanremes_home, config_path):
    ...
    def test_malformed_config_records_an_error_not_an_import(
        ...
        for kind in CONFIG_WRITING_KINDS:
            ...
            assert "sanremes config edit" in reason
    ...
    def test_unreadable_config_is_left_byte_identical(
        ...
        try:
            report = run_import("claude-code", claude_tree, sanremes_home,
                                execute=True)
            statuses = {
                i["status"]
                for kind in CONFIG_WRITING_KINDS
    ...
    def test_import_preserves_every_pre_existing_config_key(
        ...
        assert merged["model"] == "sanremes-4-405b"
    ...
    def test_config_write_is_atomic_and_leaves_no_temp_files(
        ...
        leftovers = [p.name for p in sanremes_home.glob(".tmp*")]
        assert leftovers == []

    def test_symlinked_config_stays_a_symlink(
            self, claude_tree, sanremes_home, tmp_path, config_path):
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
            self, sanremes_home, config_path, monkeypatch):
        """An interrupted dump leaves the previous file complete."""
        from sanremes_cli import agent_import
...
class TestCliWiring:
    def test_parser_builds_and_parses(self):
        import argparse
        from sanremes_cli.subcommands.import_agent import build_import_agent_parser
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, sanremes_home, capsys):
        """End-to-end through import_agent_command with --dry-run."""
        import types
        from sanremes_cli.agent_import import import_agent_command
        ...
        assert not (sanremes_home / "memories" / "MEMORY.md").exists()
        assert not (sanremes_home / "skills" / "claude-code-imports").exists()
        config_text = (sanremes_home / "config.yaml").read_text(encoding="utf-8") \
            if (sanremes_home / "config.yaml").exists() else ""
