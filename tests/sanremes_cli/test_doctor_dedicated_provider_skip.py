"""Regression: sanremes doctor must not run a generic Bearer-auth health
...
with HTTP 404. The dedicated check at sanremes_cli/doctor.py already covers
Anthropic with the right headers, so the pluggable profile must be
skipped by `_build_apikey_providers_list()`.

See: NousResearch/sanremes-agent#22346
"""
...
def test_build_apikey_providers_list_skips_dedicated_check_providers():
    from sanremes_cli import doctor
