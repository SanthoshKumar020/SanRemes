...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SANREMES_HOME with .container-mode file."""
    sanremes_home = tmp_path / ".sanremes"
    sanremes_home.mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
    monkeypatch.delenv("SANREMES_DEV", raising=False)

    container_mode = sanremes_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=sanremes-agent\n"
        "exec_user=sanremes\n"
        "sanremes_bin=/data/current-package/bin/sanremes\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("sanremes_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "sanremes-agent"
    assert info["exec_user"] == "sanremes"
    assert info["sanremes_bin"] == "/data/current-package/bin/sanremes"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from sanremes_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "sanremes"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "sanremes-agent" in cmd
    assert "/data/current-package/bin/sanremes" in cmd
    assert "chat" in cmd
