...
from sanremes_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``sanremes_session_at`` / ``sanremes_session_rt``
...
class _UnreachableProvider(StubAuthProvider):
    ...
    def verify_session(self, *, access_token: str):
        from sanremes_cli.dashboard_auth.base import ProviderError
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["sanremes_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
