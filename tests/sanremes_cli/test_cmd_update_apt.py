"""APT-managed SanRemes installs must never fall through to the git updater."""
...
def test_apt_stamp_is_detected_and_recommends_pkg_upgrade(tmp_path):
    from sanremes_cli.config import detect_install_method, recommended_update_command_for_method

    (tmp_path / ".install_method").write_text("apt\n", encoding="utf-8")
    assert detect_install_method(project_root=tmp_path) == "apt"
    assert recommended_update_command_for_method("apt") == "pkg upgrade sanremes-agent"


@patch("sanremes_cli.config.is_managed", return_value=False)
@patch("sanremes_cli.config.detect_install_method", return_value="apt")
...
def test_cmd_update_apt_prints_pkg_guidance_without_git(
    ...
    assert "pkg upgrade sanremes-agent" in capsys.readouterr().out
