...
``sanremes serve``, the gateway, anything) → ``sanremes serve`` printed only
...
* conflict → single stdout sentinel ``BACKEND_PORT_IN_USE port=<port>`` +
  ...
  transient-condition convention) — and NO ``SANREMES_BACKEND_READY``.
* free explicit port → boots and announces ``SANREMES_BACKEND_READY`` exactly
...
def test_probe_detects_held_socket():
    from sanremes_cli.web_server import _port_bind_conflict
...
def test_addr_in_use_error_classification():
    ...
    from sanremes_cli.web_server import _is_addr_in_use_error
...
def test_exit_code_is_distinct_tempfail():
    from sanremes_cli.web_server import PORT_IN_USE_EXIT_CODE
...
# ---------------------------------------------------------------------------
# E2E: real start_server in a subprocess (temp SANREMES_HOME)
# ---------------------------------------------------------------------------
...
def _spawn_serve(port: int, tmp_path: Path, merge_stderr: bool = True) -> subprocess.Popen:
    ...
    env = dict(os.environ)
    env.update(
        SANREMES_HOME=str(home),
        SANREMES_SERVE_HEADLESS="1",
        PYTHONUNBUFFERED="1",
    )
    # Ensure a stray desktop-parent env can't arm the watchdog/reaper paths.
    for k in ("SANREMES_DESKTOP", "SANREMES_PARENT_PID", "SANREMES_PARENT_START_MARKER",
              "SANREMES_PARENT_NONCE"):
        env.pop(k, None)
    code = (
        "from sanremes_cli.web_server import start_server\n"
...
def test_conflict_emits_sentinel_and_exit_75(tmp_path):
    ...
    assert "SANREMES_BACKEND_READY" not in out  # never claimed ready
...
def test_free_port_boots_and_announces_ready(tmp_path):
    ...
    proc = _spawn_serve(port, tmp_path)
    try:
        ready, lines = _read_until(proc, "SANREMES_BACKEND_READY")
        out = "".join(lines)
        assert ready, f"no READY sentinel; output:\n{out}"
        assert f"SANREMES_BACKEND_READY port={port}" in out
...
def test_ephemeral_port_zero_unaffected(tmp_path):
    ...
    try:
        ...
        ready_line = next(l for l in lines if "SANREMES_BACKEND_READY" in l)
