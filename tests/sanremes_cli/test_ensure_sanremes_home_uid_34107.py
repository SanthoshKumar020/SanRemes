"""Regression tests for #34107 — Docker UID/GID handling in ensure_sanremes_home.

When SanRemes runs in Docker with ``SANREMES_UID=1000`` / ``SANREMES_GID=911``,
the entrypoint chowns the top-level ``SANREMES_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_sanremes_home()`` — especially
...
The fix is a ``_chown_to_sanremes_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_sanremes_uid_gid
# ---------------------------------------------------------------------------


class TestResolveSanRemesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("SANREMES_UID", "1000")
        monkeypatch.setenv("SANREMES_GID", "911")
        from sanremes_cli.config import _resolve_sanremes_uid_gid
        uid, gid = _resolve_sanremes_uid_gid()
        assert uid == 1000
        assert gid == 911
...
# ---------------------------------------------------------------------------
# _chown_to_sanremes_uid
# ---------------------------------------------------------------------------


class TestChownToSanRemesUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SANREMES_UID", "1000")
        monkeypatch.setenv("SANREMES_GID", "911")
        from sanremes_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_sanremes_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_sanremes_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SANREMES_UID", raising=False)
        monkeypatch.delenv("SANREMES_GID", raising=False)
        from sanremes_cli import config as cfg
