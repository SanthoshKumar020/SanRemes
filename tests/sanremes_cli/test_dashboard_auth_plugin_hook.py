...
from sanremes_cli.dashboard_auth import clear_providers, get_provider
from sanremes_cli.dashboard_auth.base import (
    DashboardAuthProvider, LoginStart, Session,
)
from sanremes_cli.plugins import PluginContext, PluginManager, PluginManifest
from sanremes_cli.dashboard_auth import registry as _auth_registry
...
def _real_ctx() -> tuple[PluginManager, PluginContext]:
    manager = PluginManager(scope_key=sanremes_home_key())
...
def test_auth_provider_kept_out_of_manager_teardown_order():
    ...
    # Still attributed to the plugin for `sanremes plugins list`.
...
def test_targeted_unload_disposes_persistent_auth_provider():
    ...
    # `sanremes plugins disable basic` drives a targeted unload of that plugin.
