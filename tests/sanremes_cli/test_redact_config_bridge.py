...
`SANREMES_REDACT_SECRETS` at module-import time. `sanremes_cli/main.py` at
...
in .env), the toggle was silently ignored in both `sanremes chat` and
`sanremes gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `SANREMES_REDACT_SECRETS`
env var in `sanremes_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports sanremes_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("SANREMES_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import sanremes_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('SANREMES_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["SANREMES_HOME"] = str(sanremes_home)
    env.pop("SANREMES_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env SANREMES_REDACT_SECRETS takes precedence over config.yaml."""
    sanremes_home = tmp_path / ".sanremes"
    sanremes_home.mkdir()
    (sanremes_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (sanremes_home / ".env").write_text("SANREMES_REDACT_SECRETS=true\n")
