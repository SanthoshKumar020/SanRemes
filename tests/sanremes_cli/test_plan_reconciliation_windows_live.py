...
Real processes with real SanRemes-shaped argv, real inventory collection
...
reconciliation. No mocks on the components under test.
 ...
 1. Spawn a real process registered as a profile gateway (PID file + state
    file in a temp SANREMES_HOME) — real collect_runtime_inventory() must find
...
def test_plan_reconciliation_live_windows(tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(profiles_mod, "_get_default_sanremes_home", lambda: home)
