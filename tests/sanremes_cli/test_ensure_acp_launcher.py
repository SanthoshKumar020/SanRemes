"""`sanremes update` must self-heal the ``sanremes-acp`` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) resolve the agent by the
``sanremes-acp`` command name on the login-shell PATH. Fresh installs get the
launcher from ``scripts/install.sh``; existing installs get it from
``_ensure_acp_launcher()`` during ``sanremes update``.
"""
...
from sanremes_cli.main import _ensure_acp_launcher
...
def test_does_not_follow_symlink_into_venv(fake_home, tmp_path):
    """#21454 failure mode: never write through a symlinked sanremes-acp."""
    (fake_home / "sanremes").write_text("#!/bin/sh\n", encoding="utf-8")
    console_script = tmp_path / "venv" / "bin" / "sanremes-acp"
    console_script.parent.mkdir(parents=True)
    marker = "#!/usr/bin/env python\n# real console script\n"
    console_script.write_text(marker, encoding="utf-8")
    (fake_home / "sanremes-acp").symlink_to(console_script)

    _ensure_acp_launcher()

    assert console_script.read_text(encoding="utf-8") == marker
    assert (fake_home / "sanremes-acp").is_symlink()
...
def test_unwritable_bin_dir_is_skipped(fake_home):
    ...
    if not hasattr(os, "geteuid"):
        ...
        _ensure_acp_launcher()
        assert not (fake_home / "sanremes-acp").exists()
        return
    if os.geteuid() == 0:
