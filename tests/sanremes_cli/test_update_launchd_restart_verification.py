...
``sanremes update`` on macOS printed ``Update complete!`` and exited 0 while the
``ai.sanremes.gateway`` LaunchAgent sat deregistered for 36 minutes.  The restart
...
from sanremes_cli.update_cmd import _warn_incomplete_gateway_fleet_restart
...
class TestIncompleteFleetWarningIsPlatformCorrect:
    ...
    def test_linux_recovery_instructions_are_unchanged(self, monkeypatch, capsys):
        ...
        _warn_incomplete_gateway_fleet_restart(["sanremes-gateway.service"])
