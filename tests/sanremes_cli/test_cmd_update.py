...
from sanremes_cli.main import cmd_update, PROJECT_ROOT
...
def _patch_managed_uv(request):
    ...
    with patch("sanremes_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("sanremes_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("sanremes_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield
...
def _patch_gateway_discovery():
    ...
    """
    with patch("sanremes_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("sanremes_cli.gateway.supports_systemd_services", return_value=False), \
         patch("sanremes_cli.gateway.find_profile_gateway_processes", return_value=[]):
        yield


class TestCmdUpdateNpmLockfileCache:
    @staticmethod
    def _cache_file(sanremes_root, project_root):
        cache_key = hashlib.sha256(str(project_root).encode()).hexdigest()[:12]
        return sanremes_root / f".npm_lock_hash_{cache_key}"



    def test_record_npm_lockfile_hash(self, tmp_path, monkeypatch):
        from sanremes_cli import main as hm
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `sanremes update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".sanremes"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(sanremes_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            sanremes_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("SANREMES_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("SANREMES_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $SANREMES_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("sanremes_cli.managed_uv.resolve_uv", lambda: None)
...
class TestUpdateManagedPythonEnvIsolation:
    ...
    forces UV_PYTHON_INSTALL_DIR to .sanremes-runtime/python), then re-points
    ...
    def test_update_uv_env_points_venv_and_runtime_store(self):
        ...
        assert ".sanremes-runtime" in uv_env.get("UV_PYTHON_INSTALL_DIR", "")
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/sanremes-agent silently misses updates.
        """
        from sanremes_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/sanremes-agent.git",
    ...
    def test_fork_upstream_sync_that_moves_head_runs_post_update_steps(
        ...
        with patch.object(
            ...
            "sanremes_cli.update_receipt.collect_fleet_versions",
            return_value=[],
        ), patch(
            ...
            "sanremes_cli.gateway.find_gateway_pids",
            return_value=[],
        ), patch(
            "sanremes_cli.gateway.find_profile_gateway_processes",
            return_value=[],
        ), patch(
            "sanremes_cli.gateway._get_service_pids",
            return_value=set(),
        ), patch.object(
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "sanremes_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "sanremes_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch(
            "sanremes_cli.update_cmd._reload_config_modules"
        ), patch(
            "sanremes_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "sanremes_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("sanremes_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "sanremes_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "sanremes_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "sanremes_cli.update_cmd._reload_config_modules"
        ), patch(
            "sanremes_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "sanremes_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "sanremes_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "sanremes_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "sanremes_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "sanremes_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "sanremes_cli.update_cmd._reload_config_modules"
        ), patch(
            "sanremes_cli.update_cmd._run_config_check_fresh", return_value=(1, 24)
        ), patch(
            "sanremes_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("sanremes_cli.main.sys") as mock_sys:
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    Before the fix, ``sanremes update`` ran in the PRE-pull Python process.
    After ``git pull`` updated the source on disk, function-level imports
    returned the OLD cached ``sanremes_cli.config`` module — so
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        sys.modules held the OLD sanremes_cli.config with the OLD
        DEFAULT_CONFIG["_config_version"] after git pull.
        """
        from unittest.mock import patch

        import sanremes_cli.update_cmd as update_cmd
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.sanremes"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.sanremes/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.sanremes/profiles/work"))
        ...
        with (
            patch("sanremes_cli.profiles.list_profiles", return_value=all_profiles),
            patch("sanremes_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("sanremes_cli.profiles.list_profiles", return_value=[default_p]),
            patch("sanremes_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``sanremes update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``sanremes update --check --branch <name>`` honors the branch override.
    ...
    @patch("sanremes_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``sanremes update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from sanremes_cli.main import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_update_rebuilds_desktop_that_disappears_mid_update(self):
        ...
        packaged_exe = desktop_dir / "release" / "win-unpacked" / "SanRemes.exe"
        ...
        desktop_build.assert_called_once_with(
            [hm.sys.executable, "-m", "sanremes_cli.main", "desktop", "--build-only"],
            cwd=PROJECT_ROOT,
            env=ANY,
        )
    ...
    def test_git_failure_zip_fallback_rebuilds_missing_desktop(self, tmp_path, monkeypatch):
        ...
        desktop app (release/win-unpacked/SanRemes.exe) is preserved THROUGH
        ...
        def write_source_zip(_url, destination):
            with zipfile.ZipFile(destination, "w") as archive:
                archive.writestr("sanremes-agent-main/apps/desktop/package.json", "{}")
        ...
        monkeypatch.setattr(update_cmd, "get_sanremes_home", lambda: tmp_path / "sanremes-home")
        ...
        with (
            ...
            patch("sanremes_cli.managed_uv.ensure_uv", return_value="uv"),
            patch("sanremes_cli.managed_uv.update_managed_uv"),
            patch(
            ...
            patch("sanremes_cli.model_catalog.seed_cache_from_checkout", return_value=False),
        ):
