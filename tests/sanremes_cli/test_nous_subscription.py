...
from sanremes_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import sanremes_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "sanremes_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    single Enter during `sanremes model` overwrote it to `web.backend: nous`.
    """
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "sanremes_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `sanremes tools`
# ---------------------------------------------------------------------------
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_sanremes_managed_node_path(
    monkeypatch, tmp_path
):
    """If tools.browser_tool cannot be imported, the managed-Node rung must
    still find a runnable agent-browser under the SanRemes Node dir even when
    ...
    monkeypatch.setattr(
        "sanremes_constants.with_sanremes_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "sanremes_constants.agent_browser_runnable",
