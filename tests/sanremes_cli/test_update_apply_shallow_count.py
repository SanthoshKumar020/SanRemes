"""Shallow-checkout guard on the `sanremes update` apply path (#53479).
...
def _run_count_block(*, shallow: bool, raw_count: str, api_count):
    ...
    with patch.object(update_cmd, "subprocess") as sub:
        ...
        with patch("sanremes_cli.banner._github_compare_behind", return_value=api_count):
            ...
            if commit_count > 0 and apply_is_shallow:
                from sanremes_cli.banner import _github_compare_behind
...
def test_shallow_zero_count_short_circuits_without_api():
    with patch("sanremes_cli.banner._github_compare_behind") as api:
