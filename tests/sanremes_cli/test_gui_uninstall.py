"""Tests for sanremes_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import sanremes_cli.gui_uninstall as gu


def _make_agent(sanremes_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = sanremes_home / "sanremes-agent"
    (agent_root / "sanremes_cli").mkdir(parents=True)
    (agent_root / "sanremes_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(sanremes_home: Path) -> None:
    """Create the source-built GUI artifacts a `sanremes desktop` run produces."""
    desktop = sanremes_home / "sanremes-agent" / "apps" / "desktop"
    ...
    (sanremes_home / "sanremes-agent" / "node_modules").mkdir(parents=True)
    (sanremes_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(sanremes_home: Path) -> None:
    (sanremes_home / "config.yaml").write_text("x: 1\n")
    (sanremes_home / ".env").write_text("KEY=secret\n")
    (sanremes_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(sanremes_home)
    _make_gui_build(sanremes_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(sanremes_home)
    ...
    assert summary["sanremes_home"] == str(sanremes_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `sanremes desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    sanremes_home = tmp_path / ".sanremes"
    _make_agent(sanremes_home)
    icon = lde.icon_path(sanremes_home / "sanremes-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(sanremes_home)
    ...
    assert lde.icon_path(sanremes_home / "sanremes-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (sanremes_home / "sanremes-agent" / "sanremes_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".sanremes")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import sanremes_cli.uninstall as uninstall
