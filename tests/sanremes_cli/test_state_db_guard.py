...
from sanremes_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    SANREMES_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def sanremes_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("sanremes_cli.config") or mod == "sanremes_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, sanremes_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, sanremes_home, capsys, monkeypatch):
