...
Test-pollution class found Aug 2026: ``sanremes_cli.runtime_provider`` is
...
has ``sanremes_cli.config.load_config`` patched. With a module-level
...
silently got the long-dead test's config. Live pair that exposed it:
  ...
  tests/sanremes_cli/test_models.py::TestLocalOllamaModelDiscovery::
...
``sanremes_cli.config`` attributes at call time. These tests pin exactly
that property — a patch on ``sanremes_cli.config.<fn>`` must be visible
...
def test_load_config_patch_is_visible_and_releases():
    ...
    with patch("sanremes_cli.config.load_config", return_value=sentinel):
...
def test_sibling_delegates_are_late_bound_too():
    ...
    with patch(
        "sanremes_cli.config.get_compatible_custom_providers", return_value=marker
    ):
        assert rp.get_compatible_custom_providers({}) is marker
    assert rp.get_compatible_custom_providers({}) is not marker

    hdrs = {"X-Marker": "1"}
    with patch("sanremes_cli.config.normalize_extra_headers", return_value=hdrs):
