...
``venv\\Scripts\\sanremes.exe`` is a launcher that runs the interpreter with the
...
The hand-off sits at the sync boundary, not at the top of ``sanremes update``:
...
SHIM_NAMES = ["sanremes.exe", "sanremes-agent.exe", "sanremes-acp.exe", "sanremes-gateway.exe"]
...
def venv(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["sanremes", "update"])
...
def test_detects_shim_from_zipapp_main_py(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "sanremes.exe" / "__main__.py")])
    assert cli_main._windows_shim_in_process_chain() == venv / "sanremes.exe"


def test_detects_shim_from_main_module_spec_origin(venv, monkeypatch):
    fake_main = types.SimpleNamespace(
        __file__=None,
        __spec__=types.SimpleNamespace(origin=str(venv / "sanremes.exe")),
    )
...
def test_detects_shim_in_ancestor_chain(venv, monkeypatch):
    ...
    _fake_psutil(monkeypatch, [str(venv / "sanremes.exe")])
    assert cli_main._windows_shim_in_process_chain() == venv / "sanremes.exe"


def test_ignores_sanremes_exe_outside_the_project_venv(venv, monkeypatch, tmp_path):
    """A shim from some other install must never trigger a re-exec."""
    other = tmp_path / "other" / "Scripts"
    other.mkdir(parents=True)
    monkeypatch.setattr(sys, "argv", [str(other / "sanremes.exe"), "update"])
    _fake_psutil(monkeypatch, [str(other / "sanremes.exe")])
...
def test_no_shim_off_windows(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "sanremes.exe"), "update"])
    assert cli_main._windows_shim_in_process_chain() is None
...
def test_reexec_falls_through_when_venv_python_is_missing(venv, monkeypatch, capsys):
    ...
    assert cli_main._reexec_dependency_sync_off_windows_shim() is False
    assert "-m sanremes_cli.main update" not in capsys.readouterr().out
...
def test_reexec_falls_through_when_spawn_fails(venv, monkeypatch, capsys):
    ...
    assert cli_main._reexec_dependency_sync_off_windows_shim() is False
    assert "-m sanremes_cli.main update" in capsys.readouterr().out
...
def test_up_to_date_run_never_hands_off(venv, monkeypatch, capsys):
    ...
    The hand-off used to run before the fetch, so every ``sanremes update`` —
    ...
    """
    monkeypatch.setattr(sys, "argv", [str(venv / "sanremes.exe"), "update", "--check"])
    calls = _capture_popen(monkeypatch)
...
def test_pending_rename_filter_drops_only_our_shim_pairs():
    shims = [Path(r"C:\sanremes\venv\Scripts\sanremes.exe")]
    entries = [
        r"\??\C:\other\thing.dll", r"!\??\C:\other\thing.dll.bak",
        r"\??\C:\sanremes\venv\Scripts\sanremes.exe",
        r"!\??\C:\sanremes\venv\Scripts\sanremes.exe.old.1755624735000",
    ]
...
def test_pending_rename_filter_keeps_a_shim_pair_with_a_foreign_target():
    shims = [Path(r"C:\sanremes\venv\Scripts\sanremes.exe")]
    entries = [
        r"\??\C:\sanremes\venv\Scripts\sanremes.exe", r"!\??\C:\somewhere\else.exe",
    ]
    kept, removed = cli_main._filter_pending_shim_renames(entries, shims)
    assert removed == 0
    assert kept == entries
