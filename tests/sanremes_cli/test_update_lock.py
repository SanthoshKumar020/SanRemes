"""Cross-process update mutual exclusion (``sanremes_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``sanremes
...
dashboard-spawned ``sanremes update`` was mid-``npm install``, which then failed
...
from sanremes_cli.update_lock import (
    HANDOFF_PID_ENV,
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".sanremes-update-in-progress"


def test_marker_path_follows_process_sanremes_home(tmp_path, monkeypatch):
    """The lock must land where the Rust updater and Electron gate look.

    All three resolve the *process* SANREMES_HOME; a profile-scoped path would
    put the lock somewhere the other two owners never read.
    """
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    assert update_marker_path() == tmp_path / ".sanremes-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``sanremes update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "SanRemes is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``sanremes-setup`` under ``~/.sanremes`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("sanremes_cli.update_lock._pid_alive", lambda pid: True)
