"""Tests for ``sanremes debug`` CLI command and debug utilities."""
...
@pytest.fixture
def sanremes_home(tmp_path, monkeypatch):
    """Set up an isolated SANREMES_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO sanremes_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from sanremes_cli.debug import _upload_paste_rs
        ...
        with patch("sanremes_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "sanremes_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from sanremes_cli.debug import upload_to_pastebin

        with patch("sanremes_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("sanremes_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, sanremes_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = sanremes_home / "logs" / "agent.log"
        from sanremes_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, sanremes_home):
        """When truncation lands on a line boundary, keep the first full line."""
        from sanremes_cli.debug import _capture_log_snapshot
        ...
        (sanremes_home / "logs" / "agent.log").write_text(line * num_lines)
...
class TestMissingLogNote:
    ...
    `sanremes debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, sanremes_home):
        from sanremes_cli.debug import _capture_log_snapshot

        (sanremes_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, sanremes_home):
        from sanremes_cli.debug import _capture_log_snapshot

        (sanremes_home / "logs" / "desktop.log").unlink()

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.full_text is None
        assert "not on this host" in snap.tail_text
        assert "SanRemes Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(sanremes_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, sanremes_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, sanremes_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from sanremes_cli.debug import _capture_log_snapshot

        (sanremes_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, sanremes_home):
        """The uploaded report — what people paste into support — must explain it."""
        from sanremes_cli.debug import collect_debug_report
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of SANREMES_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def sanremes_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated SANREMES_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".sanremes"
        home.mkdir()
        monkeypatch.setenv("SANREMES_HOME", str(home))
        ...
        monkeypatch.delenv("SANREMES_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, sanremes_home_with_secret):
        from sanremes_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, sanremes_home_with_secret):
        from sanremes_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, sanremes_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when SANREMES_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("SANREMES_REDACT_SECRETS", "false")

        from sanremes_cli.debug import _capture_log_snapshot

        assert os.environ.get("SANREMES_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, sanremes_home_with_secret
    ):
        from sanremes_cli.debug import _capture_log_snapshot

        log_path = sanremes_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, sanremes_home_with_secret):
        from sanremes_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from sanremes_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, sanremes_home):
        from sanremes_cli.debug import collect_debug_report

        with patch("sanremes_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- sanremes dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- sanremes dump ---" in report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, sanremes_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from sanremes_cli.debug import run_debug_share
        ...
        with patch("sanremes_cli.dump.run_dump"), \
             patch("sanremes_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("sanremes_cli.debug.upload_to_pastebin",
    ...
    def test_share_uploads_five_pastes(self, sanremes_home, capsys):
        ...
        with patch("sanremes_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- sanremes dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, sanremes_home_with_secret, capsys
    ):
        ...
        with patch("sanremes_cli.dump.run_dump"), \
             patch("sanremes_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("sanremes_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        from sanremes_cli.debug import run_debug
        ...
        out = capsys.readouterr().out
        assert "sanremes debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, sanremes_home):
        from sanremes_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from sanremes_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from sanremes_cli.debug import delete_paste
        ...
        with patch("sanremes_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.sanremes/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``sanremes debug``
    invocation.
    """


    def test_records_pending_to_json(self, sanremes_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from sanremes_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, sanremes_home):
        """Same URL recorded twice → one entry with the later expire_at."""
        from sanremes_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, sanremes_home):
        from sanremes_cli.debug import (
        ...
        with patch("sanremes_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, sanremes_home):
        from sanremes_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("sanremes_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, sanremes_home):
        ...
        with patch(
            "sanremes_cli.debug.delete_paste",
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, sanremes_home):
        from sanremes_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("sanremes_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from sanremes_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("sanremes_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_shows_privacy_notice(self, sanremes_home, capsys):
        from sanremes_cli.debug import run_debug_share
        ...
        with patch("sanremes_cli.dump.run_dump"), \
             patch("sanremes_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("sanremes_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    Backs both ``sanremes debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, sanremes_home):
        from sanremes_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (sanremes_home / "logs" / "agent.log").write_text(
        ...
        with patch("sanremes_cli.dump.run_dump"), patch(
            "sanremes_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("sanremes_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, sanremes_home):
        from sanremes_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    ...
    def test_no_redact_omits_banner(self, sanremes_home):
        from sanremes_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, sanremes_home):
        from sanremes_cli.debug import collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, sanremes_home):
        import gzip
        import json as _json

        from sanremes_cli.debug import build_nous_bundle
        ...
        assert envelope["format"] == "sanremes-debug-share/1"
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, sanremes_home, capsys):
        from sanremes_cli.debug import run_debug_share
        ...
        with patch("sanremes_cli.dump.run_dump"), patch(
            "sanremes_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, sanremes_home, capsys):
        from sanremes_cli.debug import run_debug_share

        with patch("sanremes_cli.dump.run_dump"), patch(
            "sanremes_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, sanremes_home):
        from sanremes_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("sanremes_cli.dump.run_dump"), patch(
            "sanremes_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("sanremes_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    ``SanRemesCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    which parses an optional destination word and builds the args namespace
    handed to ``run_debug_share``.
    """

    def _handler(self):
        from sanremes_cli.cli_commands_mixin import CLICommandsMixin
    ...
    def _captured(self, cmd_original):
        ...
        with patch("sanremes_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`sanremes debug share` requires explicit consent before uploading.
    ...
    def test_non_interactive_requires_yes(self, sanremes_home, capsys, monkeypatch):
        ...
        with patch("sanremes_cli.dump.run_dump"), \
             patch("sanremes_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, sanremes_home, capsys, monkeypatch):
