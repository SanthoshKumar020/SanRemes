"""Tests for sanremes tools disable/enable/list command (backend)."""
...
from sanremes_cli.tools_config import tools_disable_enable_command
...
class TestToolsDisableBuiltin:
    ...
    def test_disable_removes_toolset_from_platform(self):
        ...
        with patch("sanremes_cli.tools_config.load_config", return_value=config), \
             patch("sanremes_cli.tools_config.save_config") as mock_save:
...
class TestToolsDisableMcp:
    ...
    def test_disable_unknown_server_prints_error(self, capsys):
        config = {"mcp_servers": {}}
        with patch("sanremes_cli.tools_config.load_config", return_value=config), \
             patch("sanremes_cli.tools_config.save_config"):
...
def test_tools_action_accepts_deferred_plugin_without_materializing(action, capsys):
    ...
    try:
        with patch(
            "sanremes_cli.plugins.discover_plugins",
            side_effect=discover_deferred_platform,
        ) as discover, \
             patch("sanremes_cli.tools_config.load_config", return_value=config), \
             patch("sanremes_cli.tools_config.save_config") as save:
