...
def test_get_git_banner_state_reads_origin_and_head(tmp_path):
    ...
    with patch("sanremes_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_via_local_git_ssh_fastpath_ahead_not_behind(tmp_path):
    ...
    nudges the user into `sanremes update`, which can wipe the carried work.
    """
    ...
    def fake_git_stdout(args, *, cwd, timeout=5):
        if args == ["remote", "get-url", "origin"]:
            return "git@github.com:NousResearch/sanremes-agent.git"
