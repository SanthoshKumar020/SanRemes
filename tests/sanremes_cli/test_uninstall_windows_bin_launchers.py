"""Uninstall must not leave a dangling ``sanremes`` command on Windows.

Every uninstall mode deletes the code checkout, but the launchers install.ps1
staged in the managed binary dir (the default SanRemes root's ``bin``, shared
with the managed uv) live outside it. A surviving launcher makes ``sanremes``
...
from sanremes_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
...
def managed_bin(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    (bin_dir / "sanremes.exe").write_bytes(b"MZ launcher")
    (bin_dir / "sanremes-acp.cmd").write_text("@echo off\r\n", encoding="ascii")
...
def test_removes_both_launcher_forms_and_keeps_managed_uv(managed_bin: Path):
    ...
    assert sorted(p.name for p in removed) == ["sanremes-acp.cmd", "sanremes.exe"]
    assert not (managed_bin / "sanremes.exe").exists()
    assert not (managed_bin / "sanremes-acp.cmd").exists()
...
def test_anchors_on_default_root_not_profile_home(
    ...
    """The launcher dir is per-machine; a profile SANREMES_HOME must not
...
def test_noop_on_posix(managed_bin: Path):
    ...
    assert (managed_bin / "sanremes.exe").exists()
...
def test_launcher_names_stay_in_lockstep_with_install_ps1():
    ...
    for name in _WINDOWS_BIN_LAUNCHERS:
        assert name.startswith("sanremes")  # never a generic name it could clobber
...
class TestManagedBinPathMarker:
    ...
    HOME = r"C:\Users\me\AppData\Local\sanremes"
    BIN_ENTRY = r"C:\Users\me\AppData\Local\sanremes\bin"

    def test_keep_data_markers_spare_the_managed_bin(self):
        markers = [m.lower() for m in uninstall._sanremes_path_markers(Path(self.HOME))]
    ...
    def test_full_wipe_markers_take_the_managed_bin(self):
        markers = [
            m.lower()
            for m in uninstall._sanremes_path_markers(
