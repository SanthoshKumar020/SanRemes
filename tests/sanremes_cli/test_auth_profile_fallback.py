...
When ``SANREMES_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.sanremes/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.sanremes            (has its own auth.json fixture)
    * Profile     -> tmp_path/.sanremes/profiles/coder   (active, SANREMES_HOME points here)
...
def test_missing_global_auth_file_is_safe(profile_env):
    ...
    from sanremes_cli.auth import read_credential_pool
...
# ``_load_provider_state`` itself, those helpers raise ``"SanRemes is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (SANREMES_HOME != ~/.sanremes, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    sanremes_home = tmp_path / "classic"
    sanremes_home.mkdir()
