"""Tests for kanban board export / import (``sanremes_cli.kanban_transfer``).
...
from sanremes_cli import kanban_transfer as kt
from sanremes_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("SANREMES_KANBAN_HOME", str(root))
        for var in ("SANREMES_KANBAN_DB", "SANREMES_KANBAN_WORKSPACES_ROOT",
                    "SANREMES_KANBAN_ATTACHMENTS_ROOT", "SANREMES_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this SanRemes"):
