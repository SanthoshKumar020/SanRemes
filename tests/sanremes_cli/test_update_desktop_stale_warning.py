...
#88251: ``sanremes update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_constants.with_sanremes_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "sanremes_constants.display_sanremes_home", lambda: str(tmp_path), raising=False
    )
...
def test_summary_omits_success_banner_when_desktop_rebuild_failed(capsys):
    ...
    assert "sanremes desktop" in out
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_sanremes_home", lambda: tmp_path)
