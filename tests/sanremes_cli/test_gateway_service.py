...
import sanremes_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`sanremes gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `sanremes gateway restart` in the background; when a
        ...
        assert "sanremes gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``SANREMES_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../sanremes_test`` SANREMES_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "sanremes-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir SANREMES_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="SANREMES_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/sanremes_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="SANREMES_HOME=/tmp/sanremes-e2e-41264"\n'
        assert (
            gateway_cli._temp_home_in_service_definition(unit)
            == "/tmp/sanremes-e2e-41264"
        )
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="SANREMES_HOME={tmp_path}/sanremes-home"\n'
...
class TestGetCronDrainTimeout:
    def test_missing_config_falls_back_to_default(self, monkeypatch):
        monkeypatch.delenv("SANREMES_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("SANREMES_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_timeout_stop_sec_covers_default_cron_drain_floor(self, monkeypatch):
        ...
        monkeypatch.delenv("SANREMES_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("SANREMES_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".sanremes" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `sanremes gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import sanremes_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.sanremes.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "<plist>--replace\n<key>SANREMES_HOME</key>"
                "<string>/Users/alice/.sanremes</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_sanremes_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.sanremes.gateway")
        ...
        assert ("observe", "ai.sanremes.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.sanremes.gateway"])
            ),
        )
...
class TestSystemUnitSanRemesHome:
    """SANREMES_HOME in system units must reference the target user, not root."""

    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        self, monkeypatch, tmp_path
    ):
        managed_bin = tmp_path / ".sanremes" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".sanremes")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_sanremes = target_home / ".sanremes"
        root_home = tmp_path / "root"
        root_sanremes = root_home / ".sanremes"
        managed_bin = target_sanremes / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        root_sanremes.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", staticmethod(lambda: root_home))
        monkeypatch.setenv("SANREMES_HOME", str(root_sanremes))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        )
        monkeypatch.setattr(gateway_cli, "get_sanremes_home", lambda: root_sanremes)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "sanremes_constants.iter_sanremes_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "sanremes_constants.sanremes_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        monkeypatch.delenv("SANREMES_HOME", raising=False)
        monkeypatch.setattr(
        ...
        assert 'SANREMES_HOME=/home/alice/.sanremes' in unit
        assert '/root/.sanremes' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's SANREMES_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        sanremes_home = str(gateway_cli.get_sanremes_home().resolve())
        assert f'SANREMES_HOME={sanremes_home}' in unit


class TestSystemUnitRefreshSyncsSanRemesHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.sanremes."""

    def test_refresh_adopts_unit_sanremes_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_sanremes = root_home / ".sanremes"
        alice_sanremes = alice_home / ".sanremes"
        root_sanremes.mkdir(parents=True)
        alice_sanremes.mkdir(parents=True)
        (root_sanremes / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_sanremes / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's SANREMES_HOME + drain timeout).
        monkeypatch.setenv("SANREMES_HOME", str(alice_sanremes))
        ...
        # Simulate sudo without inherited SANREMES_HOME (falls back to root).
        monkeypatch.setenv("SANREMES_HOME", str(root_sanremes))
        ...
        assert os.environ["SANREMES_HOME"] == str(alice_sanremes)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned SANREMES_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_sanremes_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_sanremes_home_from_systemd_unit",
...
class TestSanRemesHomeForTargetUser:
    """Unit tests for _sanremes_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("SANREMES_HOME", raising=False)

        result = gateway_cli._sanremes_home_for_target_user("/home/alice")
        assert result == "/home/alice/.sanremes"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "sanremes gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".sanremes" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SANREMES_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_sanremes_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        ...
        assert f'SANREMES_HOME={target_home / ".sanremes" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".sanremes" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SANREMES_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_sanremes_home", lambda: profile_dir)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".sanremes" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.sanremes.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".sanremes" / "sanremes-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".sanremes" / "sanremes-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".sanremes" / "sanremes-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SANREMES_HOME", str(root_home / ".sanremes"))
        monkeypatch.setattr(gateway_cli, "get_sanremes_home", lambda: root_home / ".sanremes")
        ...
        # WorkingDirectory is anchored at the target user's SANREMES_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.sanremes" in unit
        assert "WorkingDirectory=/home/alice/.sanremes/sanremes-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "sanremes-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "sanremes-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'sanremes gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacySanRemesUnitDetection:
    """Tests for _find_legacy_sanremes_units / has_legacy_sanremes_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``sanremes.service`` unit behind when the service was
    renamed to ``sanremes-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``sanremes.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``sanremes-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=SanRemes Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m sanremes_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (user_dir / "sanremes.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "sanremes.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_sanremes_units()
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m sanremes_cli.main gateway run
          - python path/to/sanremes_cli/main.py gateway run
          - sanremes gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m sanremes_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/sanremes/sanremes_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/sanremes gateway run --replace",
            "ExecStart=/venv/bin/python /opt/sanremes/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "sanremes.service" if i == 0 else "sanremes.service"  # same name
            # Test each variant fresh
            (user_dir / "sanremes.service").write_text(
                f"[Unit]\nDescription=Old SanRemes\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_sanremes_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "sanremes.service" in out
        assert "sanremes gateway migrate-legacy" in out



class TestRemoveLegacySanRemesUnits:
    """Tests for remove_legacy_sanremes_units (the migration action)."""
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        legacy = user_dir / "sanremes.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        removed, remaining = gateway_cli.remove_legacy_sanremes_units(interactive=False)
        ...
        assert any("--user stop sanremes.service" in c for c in cmds_joined)
        assert any("--user disable sanremes.service" in c for c in cmds_joined)
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (sanremes-gateway-coder.service)
        ...
        profile_unit = user_dir / "sanremes-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "sanremes-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `sanremes gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `sanremes gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import sanremes_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "sanremes_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_sanremes_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "sanremes_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_sanremes_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_sanremes_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_sanremes_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_sanremes_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_sanremes_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "sanremes-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "sanremes-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "sanremes-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``sanremes gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "sanremes-gateway")
        ...
        assert "sudo systemctl start sanremes-gateway" in out
        assert "sudo sanremes gateway uninstall --system" in out
        assert "sanremes gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``sanremes gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (SANREMES_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """



    def test_user_unit_workingdirectory_is_sanremes_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".sanremes"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_sanremes_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_sanremes_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.sanremes.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.sanremes.gateway"
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        sanremes = tmp_path / "home" / ".sanremes"
        sanremes.mkdir(parents=True)
        (sanremes / "config.yaml").write_text(config_yaml, encoding="utf-8")
        monkeypatch.setenv("SANREMES_HOME", str(sanremes))
        monkeypatch.delenv("SANREMES_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("SANREMES_CRON_DRAIN_TIMEOUT", raising=False)
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        monkeypatch.setattr(
    ...
    def test_env_override_extends_the_leash(self, tmp_path, monkeypatch):
        unit = self._unit_with_config(
            ...
            monkeypatch,
            "agent:\n  restart_drain_timeout: 0\n",
            env={"SANREMES_CRON_DRAIN_TIMEOUT": "200"},
        )
