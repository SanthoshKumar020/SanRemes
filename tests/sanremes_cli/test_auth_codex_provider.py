"""Tests for Codex auth — tokens stored in SanRemes auth store (~/.sanremes/auth.json)."""
...
def _setup_sanremes_auth(sanremes_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the SanRemes auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_sanremes_auth(sanremes_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (sanremes_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((sanremes_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``sanremes auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``sanremes auth add openai-codex``.
    ...
    (sanremes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `sanremes auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``sanremes auth add openai-codex`` for accounts B and C
    ...
    (sanremes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``sanremes auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to SanRemes auth store, not ~/.codex/."""
    sanremes_home = tmp_path / "sanremes"
    ...
    _save_codex_tokens({"access_token": "sanremes-at", "refresh_token": "sanremes-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches SanRemes store
    assert not (codex_home / "auth.json").exists()

    # SanRemes auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "sanremes-at"


def test_resolve_returns_sanremes_auth_store_source(tmp_path, monkeypatch):
    sanremes_home = tmp_path / "sanremes"
    _setup_sanremes_auth(sanremes_home)
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "sanremes-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run sanremes auth".
    """
    from sanremes_cli.auth import (
    ...
    assert "sanremes auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from sanremes_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from sanremes_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch sanremes_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("sanremes_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
