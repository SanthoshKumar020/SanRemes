...
(remote → cache → seed), `sanremes plugins search --json`, and install-time
...
from sanremes_cli import plugin_index
from sanremes_cli.plugin_index import (
    PluginIndexEntry,
    _parse_entries,
    load_index,
    resolve_name,
...
SAMPLE = _index_doc(
    [
        ...
        {
            "name": "sanremes-telegram-business",
            "description": "Telegram secretary bot with owner approval.",
            "author": "NousResearch",
            "tags": ["telegram", "gateway"],
            "repo": "NousResearch/sanremes-telegram-business",
            "ref": "f" * 40,
        ...
        },
        {
            ...
            "author": "NousResearch",
            "tags": ["example", "llm"],
            "repo": "NousResearch/sanremes-example-plugins",
...
def sanremes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(plugin_index, "get_sanremes_home", lambda: tmp_path)
    return tmp_path
...
class TestParsing:
    def test_parses_object_form(self):
        ...
        assert [e.name for e in entries] == [
            ...
            "sanremes-telegram-business",
            "plugin-llm-example",
        ]
        assert entries[2].subdir == "plugin-llm-example"
        assert entries[2].install_identifier == (
            "NousResearch/sanremes-example-plugins/plugin-llm-example"
        )
        assert entries[0].install_identifier == "NousResearch/sanremes-media-studio"
...
class TestSearch:
    ...
    def test_exact_name_ranks_first(self):
        results = search_index(self.entries, "sanremes-media-studio")
        assert results[0].name == "sanremes-media-studio"

    def test_matches_tags(self):
        results = search_index(self.entries, "telegram")
        assert results and results[0].name == "sanremes-telegram-business"

    def test_matches_description(self):
        results = search_index(self.entries, "secretary")
        assert [e.name for e in results] == ["sanremes-telegram-business"]

    def test_fuzzy_typo_tolerance(self):
        results = search_index(self.entries, "sanremes-media-studo")
        assert results and results[0].name == "sanremes-media-studio"
...
class TestLoadIndex:
    def test_fresh_cache_wins_without_network(self, sanremes_home, monkeypatch):
        _write_cache(sanremes_home, SAMPLE)
    ...
    def test_expired_cache_triggers_remote(self, sanremes_home, monkeypatch):
        _write_cache(sanremes_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_remote_failure_falls_back_to_stale_cache(self, sanremes_home, monkeypatch):
        _write_cache(sanremes_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_no_cache_no_remote_falls_back_to_seed(self, sanremes_home, monkeypatch):
        monkeypatch.setattr(plugin_index, "_fetch_remote", lambda: None)
        entries, source = load_index()
        assert source == "seed"
        assert len(entries) >= 3

    def test_refresh_bypasses_fresh_cache(self, sanremes_home, monkeypatch):
        _write_cache(sanremes_home, SAMPLE)
    ...
    def test_offline_skips_network(self, sanremes_home, monkeypatch):
    ...
    def test_corrupt_cache_ignored(self, sanremes_home, monkeypatch):
        cache = sanremes_home / "cache" / "plugin_index.json"
        cache.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_remote_fetch_writes_cache(self, sanremes_home, monkeypatch):
    ...
    def test_index_url_config_override(self, monkeypatch):
        ...
        from sanremes_cli import config as config_mod
...
class TestResolveName:
    ...
    def test_exact_unique(self):
        entry, candidates = resolve_name(self.entries, "sanremes-media-studio")
        assert entry is not None and entry.repo == "NousResearch/sanremes-media-studio"

    def test_case_insensitive(self):
        entry, _ = resolve_name(self.entries, "SanRemes-Media-Studio")
        assert entry is not None

    def test_unique_partial(self):
        entry, _ = resolve_name(self.entries, "telegram")
        assert entry is not None and entry.name == "sanremes-telegram-business"

    def test_ambiguous_partial(self):
        entry, candidates = resolve_name(self.entries, "sanremes")
...
class TestInstallResolution:
    def test_bare_name_detection(self):
        from sanremes_cli.plugins_cmd import _looks_like_bare_index_name

        assert _looks_like_bare_index_name("sanremes-media-studio")
    ...
    def test_install_resolves_name_and_pins_ref(self, sanremes_home, monkeypatch):
        from sanremes_cli import plugins_cmd
        ...
        with pytest.raises(SystemExit):
            plugins_cmd.cmd_install("sanremes-media-studio", enable=False)
        assert captured["identifier"] == "NousResearch/sanremes-media-studio"
        assert captured["ref"] == "e" * 40

    def test_install_explicit_ref_beats_index_pin(self, sanremes_home, monkeypatch):
        from sanremes_cli import plugins_cmd
        ...
        monkeypatch.setattr(plugins_cmd, "_install_plugin_core", fake_core)
        with pytest.raises(SystemExit):
            plugins_cmd.cmd_install("sanremes-media-studio", enable=False, ref="d" * 40)
        assert captured["ref"] == "d" * 40

    def test_install_ambiguous_name_lists_candidates_and_exits(
        self, sanremes_home, monkeypatch, capsys
    ):
        from sanremes_cli import plugins_cmd
        ...
        with pytest.raises(SystemExit) as exc:
            plugins_cmd.cmd_install("sanremes", enable=False)
        assert exc.value.code == 1
        assert not called
        out = capsys.readouterr().out
        assert "ambiguous" in out
        assert "sanremes-media-studio" in out
        assert "sanremes-telegram-business" in out

    def test_install_unknown_name_exits(self, sanremes_home, monkeypatch, capsys):
        from sanremes_cli import plugins_cmd
    ...
    def test_owner_repo_passthrough_skips_index(self, sanremes_home, monkeypatch):
...
class TestCmdSearch:
    def test_json_output(self, sanremes_home, monkeypatch, capsys):
        from sanremes_cli import plugins_cmd
        ...
        assert payload["results"][0]["name"] == "sanremes-telegram-business"
        assert payload["results"][0]["repo"] == "NousResearch/sanremes-telegram-business"
    ...
    def test_no_results_message(self, sanremes_home, monkeypatch, capsys):
        from sanremes_cli import plugins_cmd
    ...
    def test_parser_accepts_search(self):
        ...
        from sanremes_cli.subcommands.plugins import build_plugins_parser
    ...
    def test_dispatch_routes_search(self, sanremes_home, monkeypatch):
        from sanremes_cli import plugins_cmd
