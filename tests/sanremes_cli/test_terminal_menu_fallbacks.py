...
def test_scoped_numbered_input_handles_navigation_keys(sequence, expected):
    ...
    from sanremes_cli.curses_ui import (
        MenuNavigationStart,
...
def test_prompt_model_selection_requires_expensive_confirmation(monkeypatch, capsys):
    from sanremes_cli.auth import _prompt_model_selection

    monkeypatch.setattr("sanremes_cli.curses_ui.curses_radiolist", _raise_menu)
    monkeypatch.setattr(
...
def test_prompt_model_selection_uses_line_editor_for_custom_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.cli_output.line_input",
        lambda prompt_text: (
...
def test_remove_custom_provider_falls_back_on_menu_runtime_error(tmp_path, monkeypatch):
    from sanremes_cli.main import _remove_custom_provider
