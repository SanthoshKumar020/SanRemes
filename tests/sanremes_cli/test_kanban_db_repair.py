...
and the ``sanremes kanban repair`` CLI verb."""
...
def test_dispatch_tick_runs_wal_checkpoint_at_interval(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_KANBAN_DB", str(db_path))
...
# ---------------------------------------------------------------------------
# repair_db() API + `sanremes kanban repair` CLI verb
# ---------------------------------------------------------------------------

def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `sanremes kanban …`."""
    import argparse
...
def cli_home(tmp_path, monkeypatch):
    """Isolated SANREMES_HOME so kanban_db_path() resolves inside tmp_path."""
