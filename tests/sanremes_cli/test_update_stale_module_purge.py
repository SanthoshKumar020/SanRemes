"""Tests for _purge_stale_sanremes_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `sanremes update`.

Field failure (2026-08-20, Teknium's Linux box): `sanremes update` pulled a
checkout where sanremes_cli/gateway.py newly imports `line_input` from
sanremes_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from sanremes_cli.gateway
...
the SanRemes package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real SanRemes modules from the cache; later
...
def test_purge_evicts_sanremes_prefixed_modules():
    victims = [
        "sanremes_cli.cli_output",
        "sanremes_cli.gateway",
        "gateway.status",
        "tools.ansi_strip",
        "tui_gateway.server",
        "agent.memory_store",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_sanremes_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_sanremes_modules()
    assert sys.modules.get("sanremes_cli.update_cmd") is update_cmd
    assert sys.modules.get("sanremes_cli.main") is cli_main
    assert "sanremes_cli" in sys.modules
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["sanremes_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_sanremes_modules()
    finally:
        sys.modules.pop("sanremes_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from sanremes_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
