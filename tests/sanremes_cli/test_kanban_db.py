"""Tests for the Kanban DB layer (sanremes_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated SANREMES_HOME with an empty kanban DB."""
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import sanremes_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("SANREMES_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("SANREMES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """SanRemes' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` SanRemes' DB/metadata/logs on task completion.
    """
...
# spawned with `sanremes -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's SANREMES_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's SANREMES_HOME."""

    def _set_home(self, monkeypatch, tmp_path, sanremes_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
        monkeypatch.delenv("SANREMES_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.sanremes/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.sanremes/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.sanremes/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # SANREMES_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile SANREMES_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # SANREMES_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is SANREMES_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["SANREMES_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["SANREMES_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["SANREMES_KANBAN_TASK"] == "t_dispatch_env"
        assert env["SANREMES_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "SANREMES_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see sanremes_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    (see sanremes-agent issue #22032).
    ...
    import sanremes_state as _sanremes_state
    monkeypatch.setattr(
        _sanremes_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _sanremes_state._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    sanremes_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("sanremes_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="sanremes_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        sanremes_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "sanremes_cli.kanban_db.sqlite3.connect",
...
def test_sqlite_connect_closes_tracked_conn_on_setup_failure(tmp_path, monkeypatch):
    ...
    from sanremes_cli import sqlite_safe_read
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'sanremes kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_sanremes_argv()
#
# Workers spawned by the dispatcher must use a `sanremes` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["sanremes", ...]`
...
def test_resolve_sanremes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m sanremes_cli.main`.

    Pins the correct module name (NOT `sanremes` — there is no top-level
    `sanremes` package). Regression for #23198: the original PR shipped
    `python -m sanremes` which fails with `No module named sanremes` on every
    invocation.
    """
    import shutil
    import sys
    import sanremes_cli.kanban_db as kb

    monkeypatch.delenv("SANREMES_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_sanremes_argv()
    assert argv == [sys.executable, "-m", "sanremes_cli.main"]


def test_resolve_sanremes_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `sanremes_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m sanremes_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("SANREMES_BIN", None)
    ...
    assert "SanRemes Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="sanremes_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from sanremes_cli.kanban_db import connect
    from sanremes_cli.sqlite_safe_read import file_length_matches_header
