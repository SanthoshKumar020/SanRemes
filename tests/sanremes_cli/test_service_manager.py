"""Tests for sanremes_cli.service_manager — the abstract ServiceManager
...
from sanremes_cli.service_manager import (
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `sanremes_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import sanremes_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("sanremes_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
...
# unprivileged sanremes user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from sanremes_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``sanremes gateway run``, an agent action, the Open WebUI helper) holds
    the per-SANREMES_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root SANREMES_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `sanremes gateway run --replace` (no -p flag).
    assert "sanremes gateway run --replace" in default_text
    assert "sanremes -p default" not in default_text
...
#
# `sanremes gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_sanremes_without_chown(
    s6_scandir, fake_subprocess_run,
) -> None:
    """log/run must not root-chown/unlink volume paths; create leaf as sanremes.

    #45258 parent ownership is stage2's job (``logs/gateways`` seeded as
    sanremes). Restartable log/run must not pathname-chown or pathname-rm a
    sanremes-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on sanremes-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid sanremes mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid sanremes rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_sanremes_idx = log_text.index('s6-setuidgid sanremes mkdir -p "$log_dir"')
    rm_as_sanremes_idx = log_text.index('s6-setuidgid sanremes rm -f "$log_dir/lock"')
    exec_idx = log_text.index("s6-log 1 ")
    assert mkdir_as_sanremes_idx < rm_as_sanremes_idx < exec_idx
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    gateways = sanremes_home / "logs" / "gateways"
    ...
    # Mark the drop so fake rm can refuse unlink outside SANREMES_HOME the way
    # a real sanremes uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'SANREMES_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that sanremes cannot unlink a foreign
        ...
        'if [ -n "$SANREMES_TEST_DROPPED" ]; then\n'
        "  exit 0\n"
        "fi\n"
        f'exec {real_rm} "$@"\n',
        encoding="utf-8",
    ...
    env = os.environ.copy()
    env["SANREMES_HOME"] = str(sanremes_home)
