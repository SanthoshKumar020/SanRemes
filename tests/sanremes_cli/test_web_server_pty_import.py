"""Test the platform-branched PTY bridge import in sanremes_cli.web_server.
...
def test_web_server_uses_posix_pty_bridge_on_posix():
    ...
    from sanremes_cli.pty_bridge import PtyBridge as PosixBridge
    from sanremes_cli.pty_bridge import PtyUnavailableError as PosixErr
...
def test_pty_bridge_import_block_is_platform_branched():
    ...
    #       try:
    #           from sanremes_cli.win_pty_bridge import WinPtyBridge as PtyBridge, ...
    #       except ImportError:
    #           PtyBridge = None
    #           ...
    #   else:
    #       try:
    #           from sanremes_cli.pty_bridge import PtyBridge, PtyUnavailableError
    #       ...
    assert 'sys.platform.startswith("win")' in src or "sys.platform.startswith('win')" in src
    assert "from sanremes_cli.win_pty_bridge import" in src
    assert "from sanremes_cli.pty_bridge import" in src
