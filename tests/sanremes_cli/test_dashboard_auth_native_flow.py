...
Run: pytest tests/sanremes_cli/test_dashboard_auth_native_flow.py
"""
...
from sanremes_cli.dashboard_auth import native_flow
from sanremes_cli.dashboard_auth.base import Session
...
def _walk_native_login(client, *, redirect_uri, challenge, state="cli-state"):
    ...
    assert "sanremes_session_at" not in set_cookie, (
...
@pytest.fixture
def pw_gated_client():
    from sanremes_cli.dashboard_auth.routes import _reset_password_rate_limit
    from tests.sanremes_cli.test_dashboard_auth_password_login import (
        PasswordProvider,
    )
...
def test_password_login_without_broker_still_mints_cookies(pw_gated_client):
    ...
    assert r.json()["next"] == "/"
    set_cookie = r.headers.get("set-cookie", "")
    assert "sanremes_session_at" in set_cookie
