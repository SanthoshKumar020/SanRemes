...
Bug 1 — OpenAI picker dumped the raw ``/v1/models`` catalog
    ...
    moderation, gpt-3.5, …). The ``sanremes model`` CLI shows only the curated
...
Bug 2 — OpenRouter appeared authenticated whenever OPENAI_API_KEY was set
    OpenRouter's SanRemesOverlay carried ``extra_env_vars=("OPENAI_API_KEY",)``.
...
from sanremes_cli import models as M
...
def test_openrouter_overlay_does_not_list_openai_api_key():
    overlay = SANREMES_OVERLAYS["openrouter"]
