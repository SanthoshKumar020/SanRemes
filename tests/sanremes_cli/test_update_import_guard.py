"""Tests for the post-update *import* guard in ``sanremes update``.
...
from sanremes_constants import partial_update_hint
...
def test_syntax_guard_passes_but_import_guard_catches_skew(monkeypatch, tmp_path):
    ...
    # NOTE: patch update_cmd's global, not sanremes_main's. Both modules expose
    ...
    syntax_ok, _, _ = sanremes_main._validate_critical_files_syntax(tmp_path)
    ...
    ok, module, error = sanremes_main._validate_critical_modules_import(tmp_path)
    assert ok is False
...
def test_import_guard_passes_on_consistent_tree(monkeypatch, tmp_path):
    ...
    assert sanremes_main._validate_critical_modules_import(tmp_path) == (True, None, None)
...
def test_import_guard_ignores_non_import_errors(monkeypatch, tmp_path):
    ...
    ok, _, _ = sanremes_main._validate_critical_modules_import(tmp_path)
    assert ok is True
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("sanremes update" in line for line in hint)
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``sanremes update`` can run under a different Python than the install's.
...
@pytest.mark.parametrize("modname", ["agents", "agentops", "toolsets_x", "sanremesx"])
...
def test_probe_and_hint_share_one_first_party_definition():
    ...
    `cli` was first-party to the hint but not the probe, and `sanremesx`
    (third-party) matched the probe's loose `startswith("sanremes")`. A user
    ...
    """
    from sanremes_constants import FIRST_PARTY_MODULE_ROOTS, is_first_party_module
    ...
    for lookalike in ("agents", "agentops", "toolsets_x", "sanremesx"):
