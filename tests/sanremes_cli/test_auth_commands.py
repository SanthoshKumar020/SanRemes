...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (sanremes_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from sanremes_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "sanremes" / "auth.json").read_text())
...
def test_auth_add_configured_provider_uses_canonical_pool_key(tmp_path, monkeypatch):
    ...
    payload = json.loads((sanremes_home / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("sanremes_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from sanremes_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_accepts_non_registry_configured_provider(
    ...
    from sanremes_cli import auth_commands
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.auth._nous_device_code_login",
        lambda **kwargs: {
            "portal_base_url": "https://portal.example.com",
            "inference_base_url": "https://inference.example.com/v1",
            "client_id": "sanremes-cli",
    ...
    # `sanremes auth add nous` must also populate providers.nous so the
    ...
    # raises "SanRemes is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`sanremes auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``sanremes auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``sanremes auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("sanremes_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from sanremes_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from sanremes_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """sanremes auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``sanremes auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "sanremes" / "auth.json").read_text(encoding="utf-8"))
...
def test_clear_provider_auth_removes_provider_pool_entries(tmp_path, monkeypatch):
    ...
    from sanremes_cli.auth import clear_provider_auth
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`sanremes logout --provider openai-codex` must still clear model.provider.
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from sanremes_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source SanRemes reads from has a
...
def test_seed_from_singletons_respects_sanremes_pkce_suppression(tmp_path, monkeypatch):
    """anthropic sanremes_pkce must not re-seed from ~/.sanremes/.anthropic_oauth.json when suppressed."""
    sanremes_home = tmp_path / "sanremes"
    sanremes_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

    import yaml
    (sanremes_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (sanremes_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["sanremes_pkce"]},
    }))

    # Stub the readers so only sanremes_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_sanremes_oauth_credentials", lambda: {
    ...
    # sanremes_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "sanremes_pkce" not in active
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from sanremes_cli.auth import is_source_suppressed
    from sanremes_cli.auth_commands import auth_remove_command

    with patch(
        "sanremes_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "sanremes_cli.copilot_auth.get_copilot_api_token",
