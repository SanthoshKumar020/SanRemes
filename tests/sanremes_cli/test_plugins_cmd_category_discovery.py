...
class TestDiscoverAllPlugins:
    ...
    def test_flat_plugins_still_discovered(self, mock_user_dir, mock_bundled_dir, tmp_path):
        from sanremes_cli.plugins_cmd import _discover_all_plugins
    ...
    def test_bundled_model_providers_skipped(self, mock_user_dir, mock_bundled_dir, tmp_path):
        ...
        Bundled providers should not appear in ``sanremes plugins list`` as
    ...
    def test_user_model_providers_subdir_is_still_scanned(
        ...
        at ``~/.sanremes/plugins/model-providers/<x>/`` is still discovered so
        ``sanremes plugins list`` shows what the user installed."""
        from sanremes_cli.plugins_cmd import _discover_all_plugins
...
class TestPluginStatus:
    ...
    def test_key_in_disabled(self):
        from sanremes_cli.plugins_cmd import _plugin_status
...
class TestFilterPluginEntries:
    def test_enabled_filter_uses_key(self):
        from sanremes_cli.plugins_cmd import _filter_plugin_entries
...
class TestCmdListJson:
    ...
    def test_json_status_uses_key(self, mock_user_dir, mock_bundled_dir, tmp_path, capsys):
        ...
        with patch("sanremes_cli.plugins_cmd._get_enabled_set", return_value={"web/tavily"}):
            args = MagicMock()
            args.json = True
            args.plain = False
            args.no_bundled = False
