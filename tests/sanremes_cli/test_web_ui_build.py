...
(mirroring the desktop build), recorded in a stamp file under $SANREMES_HOME,
NOT mtime comparison — so ``git pull`` / ``sanremes update`` that rewrite
source mtimes without changing content no longer fool it.

Critical invariant: the dashboard Vite build outputs to sanremes_cli/web_dist/
(vite.config.ts: outDir: "../../sanremes_cli/web_dist"), NOT web/dist/.
...
def _isolated_sanremes_home(tmp_path, monkeypatch):
    """Keep web-build-stamp writes inside the test's tmp dir, never the real home."""
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "_sanremes_home"))
...
def _make_web_dir(tmp_path: Path) -> tuple[Path, Path]:
    ...
    dist_dir = tmp_path / "sanremes_cli" / "web_dist"
...
class TestWebUIBuildNeeded:
    ...
    def test_mtime_only_change_is_not_stale(self, tmp_path):
        ...
        ``git pull`` / ``sanremes update`` do) must NOT report stale."""
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_web_install_omits_workspace_and_scrubs_esbuild_override(
        ...
        with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("sanremes_cli.main.subprocess.run", return_value=install_cp) as mock_run, \
             patch("sanremes_cli.main._run_with_idle_timeout", return_value=build_cp) as mock_build:
    ...
    def test_workspace_root_install_names_update_closure(self, tmp_path, monkeypatch):
        ...
        closure as `sanremes update` (ui-tui + web + --include-workspace-root).
        ...
        with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("sanremes_cli.main.subprocess.run", return_value=install_cp) as mock_run, \
             patch("sanremes_cli.main._run_with_idle_timeout", return_value=build_cp):
            result = _build_web_ui(web_dir)
    ...
    def test_web_build_uses_idle_timeout_helper(self, tmp_path):
        ...
        build_cp = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("sanremes_cli.main.subprocess.run", return_value=install_cp), \
             patch("sanremes_cli.main._run_with_idle_timeout", return_value=build_cp) as mock_idle:
            result = _build_web_ui(web_dir)
...
class TestBuildWebUIRetryAndStaleFallback:
    ...
    def test_retries_build_once_on_failure(self, tmp_path):
        ...
        with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("sanremes_cli.main._time.sleep") as mock_sleep, \
             patch("sanremes_cli.main.subprocess.run", return_value=install_ok), \
             patch("sanremes_cli.main._run_with_idle_timeout",
    ...
    def test_falls_back_to_stale_dist_when_retry_also_fails(self, tmp_path, capsys):
        ...
        with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("sanremes_cli.main._time.sleep"), \
             patch("sanremes_cli.main.subprocess.run", return_value=install_ok), \
             patch("sanremes_cli.main._run_with_idle_timeout",
...
class TestBuildWebUIFlock:
    ...
    def test_contended_lock_without_dist_waits_then_skips_fresh_build(self, tmp_path):
        ...
        import fcntl
        import threading
        from sanremes_cli.main import _build_web_ui as build
        ...
        try:
            with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
                 patch("sanremes_cli.main.subprocess.run") as mock_run:
...
class TestBuildRecoversFromMissingToolchain:
    def test_reinstalls_and_retries_when_the_build_cannot_resolve_tsc(self, tmp_path):
        ...
        with patch("sanremes_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             ...
             patch("sanremes_cli.main._run_with_idle_timeout", side_effect=[build_fail, build_ok]) as mock_build, \
             patch("sanremes_cli.main._web_ui_build_needed", return_value=True), \
             patch("sanremes_cli.main._write_web_ui_build_stamp"), \
             patch("sanremes_cli.main._time.sleep"):
            result = _build_web_ui(web_dir)
    ...
    def test_healthy_tree_builds_without_an_extra_install(self, tmp_path):
        ...
        with patch("sanremes_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             patch("sanremes_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
             patch("sanremes_cli.main._run_with_idle_timeout", return_value=build_ok) as mock_build, \
             patch("sanremes_cli.main._web_ui_build_needed", return_value=True), \
             patch("sanremes_cli.main._write_web_ui_build_stamp"):
            result = _build_web_ui(web_dir)
