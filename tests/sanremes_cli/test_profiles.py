"""Comprehensive tests for sanremes_cli.profiles module.
...
from sanremes_cli import profiles
from sanremes_cli.profiles import (
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and SANREMES_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.sanremes/profiles)
    * SANREMES_HOME  -> tmp_path/.sanremes  (so get_sanremes_home() agrees)
    * Creates the bare-minimum ~/.sanremes directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".sanremes"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("SANREMES_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_sanremes_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".sanremes"
...
class TestCreateProfile:
    ...
    def test_fresh_profile_inherits_a_usable_model(self, profile_env):
        ...
        """
        default_home = profile_env / ".sanremes"
...
class TestNoSkillsOptOut:
    """Tests for `sanremes profile create --no-skills` and the opt-out marker."""
    ...
    def test_delete_marker_re_enables_seeding(self, profile_env, monkeypatch):
        ...
        called = []
        stdout_by_call = [
            '{"copied": ["sanremes-agent"], "skipped_opt_out": true}',
            '{"copied": []}',
        ]
        monkeypatch.setattr(
            "subprocess.run",
        ...
        assert r1.get("copied") == ["sanremes-agent"]
        assert len(called) == 1
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `sanremes update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".sanremes" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("sanremes_cli.profiles._cleanup_gateway_service"), \
             patch("sanremes_cli.profiles.time.sleep"), \
             patch("sanremes_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "sanremes_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "sanremes_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "sanremes_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "sanremes_cli.main", "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_shebang_exec_of_sanremes_shim(self, profile_env, monkeypatch):
        """A `sanremes` console-script shim spawned directly (e.g. Electron's
        findOnPath('sanremes') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "sanremes" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "sanremes".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/sanremes", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/sanremes", "--profile", "other", "serve"]),
            # Non-sanremes script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_sanremes_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "sanremes" (e.g.
        sanremes-notes.py, sanremes-unrelated-tool) must NOT be misidentified as
        the console-script shim just because argv[0] is a python interpreter
        and argv[1]'s basename starts with "sanremes" -- only the actual known
        console-script entry points (sanremes, sanremes-agent, sanremes-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/sanremes-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/sanremes-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (sanremes-agent,
        sanremes-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "sanremes"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        self_pid = os.getpid()
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/sanremes-agent",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/sanremes-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestActiveProfile:
    ...
    def test_set_to_default_removes_file(self, profile_env):
        ...
        create_profile("coder", no_alias=True)
        set_active_profile("coder")
        active_path = tmp_path / ".sanremes" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".sanremes" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom SANREMES_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("SANREMES_HOME", str(custom))
        # With Docker-aware roots, a custom SANREMES_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sanremes_cli.profiles.shutil.which", lambda name: "/opt/sanremes/bin/sanremes")
        from sanremes_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/sanremes/bin/sanremes -p mybot" in content


    @pytest.mark.windows_only
    def test_remove_finds_bat_on_windows(self, profile_env):
        from sanremes_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from sanremes_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from sanremes_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".sanremes" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("sanremes_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".sanremes" / "profiles" / "newname"

    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        tmp_path = profile_env
        create_profile("ssi_health", no_alias=True)
        honcho_path = tmp_path / ".sanremes" / "honcho.json"
        ...
        assert "sanremes.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["sanremes_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["sanremes_heimdall"]["peerName"] == "user-peer"
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultSanRemesHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_sanremes_home()."""



    def test_default_sanremes_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_sanremes_home() returns SANREMES_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SANREMES_HOME", str(docker_home))
        home = _get_default_sanremes_home()
        assert home == docker_home



    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under SANREMES_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SANREMES_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        from sanremes_cli.profiles import _check_gateway_running
        ...
        (default_home / "gateway_state.json").write_text(
            json.dumps(
                {
                    "pid": live_pid,
                    "kind": "sanremes-gateway",
                    "argv": ["sanremes", "gateway", "run"],
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="sanremes gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with SANREMES_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("SANREMES_HOME", str(get_profile_dir("coder")))
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_sanremes_home()
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured SANREMES_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        custom = tmp_path / "custom-sanremes"
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("SANREMES_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No SANREMES_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("SANREMES_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_sanremes_home()
