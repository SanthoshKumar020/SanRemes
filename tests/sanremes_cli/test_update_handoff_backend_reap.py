...
(`sanremes update --yes --gateway --force`) left a *swarm* of per-profile `serve`
...
no live sanremes.exe shim). There, any surviving SanRemes `serve`/`dashboard`
...
def test_live_parent_backend_reaped_in_handoff():
    ...
    with patch.dict(sys.modules, {"psutil": fake}):
        holders = [_holder(200, "python.exe -m sanremes_cli.main --profile mr-tester serve")]
...
def test_swarm_of_profile_backends_all_reaped():
    ...
    with patch.dict(sys.modules, {"psutil": fake}):
        holders = [
            _holder(200 + i, f"python.exe -m sanremes_cli.main --profile {p} serve")
...
def test_dashboard_backend_reaped():
    backend = _proc(200, ["python.exe", "-m", "sanremes_cli.main", "dashboard"])
    fake = _fake_psutil({200: backend})
    with patch.dict(sys.modules, {"psutil": fake}):
        holders = [_holder(200, "python.exe -m sanremes_cli.main dashboard")]
        assert cli_main._handoff_reapable_backend_pids(holders) == [200]
...
def test_non_backend_holder_disqualifies_whole_set():
    ...
    backend = _proc(200, _serve_argv("mr-tester"))
    repl = _proc(300, ["python.exe", "-m", "sanremes_cli.main", "chat"])
    fake = _fake_psutil({200: backend, 300: repl})
    with patch.dict(sys.modules, {"psutil": fake}):
        holders = [
            _holder(200, "python.exe -m sanremes_cli.main --profile mr-tester serve"),
            _holder(300, "python.exe -m sanremes_cli.main chat"),
        ]
...
def test_exited_holder_skipped_not_fatal():
    ...
    with patch.dict(sys.modules, {"psutil": fake}):
        holders = [
            _holder(300, "python.exe -m sanremes_cli.main --profile gone serve"),
            _holder(200, "python.exe -m sanremes_cli.main --profile mr-tester serve"),
        ]
        assert cli_main._handoff_reapable_backend_pids(holders) == [200]
...
def test_psutil_unavailable_returns_none():
    ...
    with patch.dict(sys.modules, {}, clear=False):
        ...
        with patch("builtins.__import__", _no_psutil):
            holders = [_holder(200, "python.exe -m sanremes_cli.main --profile x serve")]
            assert cli_main._handoff_reapable_backend_pids(holders) is None
