"""Tests for sanremes_cli.tools_config platform tool persistence."""
...
from sanremes_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'sanremes' instead of 'sanremes-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import sanremes_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["sanremes"]}}

    with caplog.at_level(logging.WARNING, logger="sanremes_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "sanremes" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["sanremes-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["sanremes-cli", "bogus"]}}
...
# via the platform's native composite. The composite ``sanremes-discord``
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["sanremes-telegram", "discord"]}}
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/sanremes-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("sanremes_cli.tools_config.save_config"):
...
def test_first_install_nous_auto_configures_video_gen(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
    ...
    monkeypatch.setattr(
        "sanremes_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"video_gen"},
    )
    monkeypatch.setattr("sanremes_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "sanremes_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
    ...
    configured = []
    monkeypatch.setattr(
        "sanremes_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from sanremes_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """sanremes-gateway includes list should cover all messaging platforms."""
        from sanremes_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["sanremes-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from sanremes_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from sanremes_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/sanremes-agent/issues/6901
    """
    config = {
...
class TestAgentBrowserPostSetup:
    ...
    tools.browser_tool._find_agent_browser (PATH -> Homebrew/SanRemes-managed
    ...
    def _stub_browser_use_install(self):
        ...
        logic under test."""
        with patch("sanremes_cli.tools_config._ensure_browser_use_cli") as stub:
            yield stub

    def test_warns_when_neither_npx_nor_agent_browser_on_path(self):
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch("sanremes_cli.tools_config._print_warning") as warn:
    ...
    def test_chromium_already_installed_skips_subprocess(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "sanremes_cli.tools_config._print_success"
        ) as success:
            _run_post_setup("agent_browser")
    ...
    def test_docker_with_missing_chromium_warns_instead_of_installing(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "sanremes_cli.tools_config._print_warning"
        ) as warn:
            _run_post_setup("agent_browser")
    ...
    def test_installs_chromium_via_npx_resolved_only_through_extended_path(self):
        """SanRemes-managed-Node-only setups: npx resolves via
        ...
        sanremes_npx = "/home/user/.sanremes/node/bin/npx"
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch(
            "tools.browser_tool._chromium_installed", return_value=False
            ...
            "tools.browser_tool._resolve_npx_bin", return_value=sanremes_npx
        ), patch(
            "sanremes_cli.tools_config._print_success"
        ):
            run.return_value = SimpleNamespace(returncode=0, stdout="", stderr="")
        ...
        run.assert_called_once()
        assert run.call_args.args[0] == [
            sanremes_npx, "--ignore-scripts", "-y", AGENT_BROWSER_NPX_SPEC, "install", "--with-deps",
        ]
    ...
    def test_install_failure_prints_stderr_tail_and_does_not_invalidate_cache(self):
        ...
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "sanremes_cli.tools_config._print_info"
        ) as info:
...
class TestBrowserUseCliInstalledForAllNonCamofoxBackends:
    ...
    def test_browser_post_setup_attempts_cli_install(self, key):
        with patch("sanremes_cli.tools_config._ensure_browser_use_cli") as ensure, patch(
    ...
    def test_camofox_post_setup_never_touches_browser_use(self):
        ...
        the CLI in."""
        with patch("sanremes_cli.tools_config._ensure_browser_use_cli") as ensure, patch(
            "sanremes_constants.find_node_executable", return_value=None
        ), patch("subprocess.run"):
    ...
    def test_ensure_helper_always_delegates_to_install_cli(self):
        ...
        $SANREMES_HOME/bin when only side installs exist."""
        with patch(
            "sanremes_cli.tools_config.shutil.which", return_value="/usr/bin/browser-use"
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(True, "browser-use CLI already installed (/managed/bin/browser-use)"),
        ) as install:
            from sanremes_cli.tools_config import _ensure_browser_use_cli
    ...
    def test_ensure_helper_install_failure_is_non_fatal(self):
        ...
        with patch(
            "sanremes_cli.tools_config.shutil.which", return_value=None
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(False, "`uv tool install browser-use` failed:\nboom"),
        ), patch("sanremes_cli.tools_config._print_warning") as warn:
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from sanremes_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from sanremes_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from sanremes_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("sanremes_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("sanremes_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("sanremes_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
    ...
    def test_plugin_picker_falls_back_when_default_is_missing_from_catalog(self):
        ...
        from sanremes_cli.tools_config import _configure_imagegen_model_for_plugin
        ...
        with (
            patch(
                "sanremes_cli.tools_config._plugin_image_gen_catalog",
                return_value=(catalog, "also-missing"),
            ),
            patch("sanremes_cli.tools_config._prompt_choice", return_value=0),
        ):
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `sanremes tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from sanremes_cli.tools_config import _get_effective_configurable_toolsets
...
#    reported as added/removed by `sanremes tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `sanremes tools` printed ``- kanban``
...
def test_vision_picker_custom_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    import sanremes_cli.tools_config as tc
...
# `sanremes tools post-setup <key>` child. On Windows each console child (npm,
...
# ── Toolsets that shipped after a platform's last `sanremes tools` save ────────
#
# Saving the picker (or one toggle in the desktop Toolsets UI) replaces a
# platform's composite (``[sanremes-cli]``) with a frozen explicit list, and
...
def test_saved_list_gains_toolsets_that_shipped_after_it_was_written():
    ...
    on_composite = _get_platform_tools(
        {"platform_toolsets": {"cli": ["sanremes-cli"]}},
        "cli",
...
def test_agent_disabled_toolsets_json_array_string_form_still_wins():
    ...
    `sanremes config set agent.disabled_toolsets '["memory"]'`). It must be
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (sanremes-acp, sanremes-webhook) that never carried the toolset."""
    from toolsets import TOOLSETS, resolve_toolset

    narrow = [
        platform
        for platform in ("acp", "webhook")
        if f"sanremes-{platform}" in TOOLSETS
        and not any(
            set(resolve_toolset(ts, include_registry=False))
            <= set(resolve_toolset(f"sanremes-{platform}"))
...
def test_explicit_plugin_toolset_admitted_in_platform_toolsets(monkeypatch):
    ...
    ``sanremes-cli``), it MUST be admitted as a configurable key instead of
    ...
    import sanremes_cli.tools_config as _tc_mod
    ...
    # standard composite — exactly the "I want sanremes-cli AND a2a in my CLI
    # session" config the issue's user was trying to write.
    config = {"platform_toolsets": {"cli": ["sanremes-cli", "dplat_client"]}}
...
def test_explicit_plugin_toolset_admitted_against_real_a2a_plugin(monkeypatch):
    ...
    a real config like ``platform_toolsets.cli: [sanremes-cli, a2a]``, ``a2a``
    ...
    from sanremes_cli.plugins import discover_plugins, get_plugin_toolsets
    discover_plugins()
    ...
    config = {"platform_toolsets": {"cli": ["sanremes-cli", "a2a"]}}
    enabled = _get_platform_tools(config, "cli")
