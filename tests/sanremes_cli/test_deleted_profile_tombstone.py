...
``sanremes profile delete`` removes the tree. That empty shell then
reappears in ``sanremes profile list`` and Desktop Bot Mode. These tests
...
from sanremes_constants import named_profile_home
from sanremes_logging import setup_logging
...
def _delete(name: str) -> None:
    with patch("sanremes_cli.profiles._cleanup_gateway_service"), patch(
        "sanremes_cli.profiles._stop_profile_backends"
    ):
...
class TestDeletedProfileTombstone:
    def test_delete_then_logging_setup_does_not_recreate_home(self, profile_env, monkeypatch):
        ...
        with pytest.raises(FileNotFoundError, match="Named profile home does not exist"):
            setup_logging(sanremes_home=profile_dir, force=True)

        assert not profile_dir.exists()
        monkeypatch.setenv("SANREMES_HOME", str(profile_env / ".sanremes"))
        assert "worker" not in _named_homes(profile_env)
    ...
    def test_backfill_skips_tombstoned_directory(self, profile_env):
        ...
        _delete("worker")
        profile_dir.mkdir(parents=True)
        (profile_env / ".sanremes" / ".env").write_text(
...
class TestNamedProfileHome:
    def test_logs_under_named_profile_resolve_to_profile_home(self, tmp_path):
        # tmp_path acts as a real SanRemes home (Docker/custom layout): it
    ...
    def test_dot_sanremes_layout_resolves_without_markers(self, tmp_path):
        worker = tmp_path / ".sanremes" / "profiles" / "worker"
        assert named_profile_home(worker / "logs") == worker
        assert named_profile_home(worker) == worker

    def test_default_home_with_profiles_in_path_is_not_named(self, tmp_path):
        default_home = tmp_path / "foo" / "profiles" / "notahome" / ".sanremes"
    ...
    def test_unrelated_profiles_dir_is_not_named(self, tmp_path):
        ...
        # its parent is not a SanRemes home, so logging must keep mkdir-ing.
    ...
    def test_unrelated_profiles_dir_still_mkdirs(self, tmp_path):
        ...
        custom_home = tmp_path / "srv" / "profiles" / "buildcache"
        log_dir = mkdir_under_sanremes_home(custom_home / "logs")
    ...
    def test_setup_logging_under_unrelated_profiles_dir_succeeds(self, tmp_path):
        ...
        log_dir = setup_logging(sanremes_home=custom_home, force=True)
    ...
    def test_tombstone_dir_marks_profiles_root(self, tmp_path):
        ...
        # `sanremes profile delete` — its presence alone anchors recognition,
