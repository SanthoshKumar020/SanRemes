"""Tests for sanremes_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $SANREMES_HOME under tmp_path; no real
...
from sanremes_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running sanremes containers, those
    ...
    """
    monkeypatch.setattr(
        "sanremes_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under sanremes_home/profiles/<name>/."""
    p = sanremes_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on — it's always seeded by
        # `sanremes profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    SANREMES_HOME root (the implicit default profile)."""
    if state is not None:
        (sanremes_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (sanremes_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (sanremes_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        sanremes_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_main_skips_reconcile_in_dashboard_container_s6v3(
    ...
    """
    from sanremes_cli import container_boot
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ...
            "top",
            "/opt/sanremes/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "0.0.0.0",
            "--port",
