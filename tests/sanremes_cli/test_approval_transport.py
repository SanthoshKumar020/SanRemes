...
from sanremes_cli.plugins import PluginContext, PluginManager, PluginManifest
...
def _request():
    from sanremes_cli.approval_transport import ApprovalRequest
...
def test_transport_registry_is_manager_and_profile_isolated(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "profiles" / "work"))
    ...
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "profiles" / "personal"))
    assert first.get_approval_transport("phone") is None


def test_host_accepts_bound_sync_and_async_decisions():
    from sanremes_cli.approval_transport import invoke_approval_transport
...
def test_host_rejects_decision_completed_after_deadline(monkeypatch):
    import sanremes_cli.approval_transport as transport_module
...
def test_host_caps_hung_transport_workers():
    from sanremes_cli.approval_transport import (
...
def test_live_temp_home_fixture_plugin_routes_and_hardline_stays_core_owned(
    ...
    """Real discovery + config + guard path under an isolated SANREMES_HOME."""
    import sanremes_cli.plugins as plugins_module
    from tools import approval
...
def present(request):
    output = Path(os.environ["SANREMES_HOME"]) / "transport-invocations.jsonl"
...
"""
    ...
    token = approval.set_sanremes_interactive_context(True)
    approval.clear_session("local")
    approval._permanent_approved.clear()
    try:
        routed = approval.check_all_command_guards(
            "rm -rf /tmp/sanremes-approval-transport-fixture", "local"
        )
        manager.discover_and_load(force=True)
        reloaded = approval.check_all_command_guards(
            "rm -rf /tmp/sanremes-approval-transport-fixture-reloaded", "local"
        )
        gateway_token = approval.set_sanremes_interactive_context(False)
        monkeypatch.setenv("SANREMES_GATEWAY_SESSION", "1")
        try:
            gateway_routed = approval.check_all_command_guards(
                "rm -rf /tmp/sanremes-approval-transport-fixture-gateway", "local"
            )
        finally:
            approval.reset_sanremes_interactive_context(gateway_token)
        hardline = approval.check_all_command_guards("rm -rf /", "local")
    finally:
        approval.reset_sanremes_interactive_context(token)
