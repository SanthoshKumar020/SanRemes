"""Tests for sanremes backup and import commands."""
...
def _no_real_gateway_service(monkeypatch):
    ...
    assert the wiring."""
    import sanremes_cli.gateway as gateway_mod
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew sanremes_cli.backup's datetime forward instead of sleeping.
    ...
    import sanremes_cli.backup as _backup

    shim = getattr(_backup.datetime, "_sanremes_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _sanremes_test_shim = True
...
def _make_sanremes_tree(root: Path) -> None:
    """Create a realistic ~/.sanremes directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "sanremes_state.db"):
    ...
    # sanremes-agent repo (should be EXCLUDED)
    (root / "sanremes-agent").mkdir(exist_ok=True)
    (root / "sanremes-agent" / "run_agent.py").write_text("# big file\n")
    (root / "sanremes-agent" / ".git").mkdir()
    (root / "sanremes-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_sanremes_agent(self):
        from sanremes_cli.backup import _should_exclude
        assert _should_exclude(Path("sanremes-agent/run_agent.py"))
        assert _should_exclude(Path("sanremes-agent/.git/HEAD"))
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from sanremes_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        sanremes_home.mkdir()
        _make_sanremes_tree(sanremes_home)

        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
        ...
        import sanremes_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = sanremes_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, sanremes_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside SANREMES_HOME."""
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        _make_sanremes_tree(sanremes_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(sanremes_home / "skills" / "outside-link.txt", outside)
        ...
        from sanremes_cli.backup import run_backup
        run_backup(args)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        sanremes_home.mkdir()
        _make_sanremes_tree(sanremes_home)
        with sqlite3.connect(sanremes_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from sanremes_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup

        # Real producer, so the layout under state-snapshots/ is whatever the
        # code actually writes (manifest.json + state.db copy + ...).
        snap_id = create_quick_snapshot(sanremes_home=sanremes_home)
        assert snap_id and (sanremes_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid SanRemes backup."""
        from sanremes_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_auto_installs_gateway_service(self, tmp_path, monkeypatch):
        ...
        from sanremes_cli.backup import run_import
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your SanRemes configuration has been restored." in out
        assert "sanremes gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / ".sanremes"
        (sanremes_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (sanremes_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (sanremes_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            sanremes_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (sanremes_home / "gateway.pid").write_text("4242")
        (sanremes_home / "processes.json").write_text('{"live": true}')
        ...
        assert (sanremes_home / "gateway.pid").read_text() == "4242"
        assert (sanremes_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (sanremes_home / "cron.pid").exists()
        assert not (sanremes_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (sanremes_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".sanremes"
        src_home.mkdir(parents=True)
        _make_sanremes_tree(src_home)

        monkeypatch.setenv("SANREMES_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from sanremes_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".sanremes"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("SANREMES_HOME", str(dst_home))
        ...
        # sanremes-agent should NOT be present
        assert not (dst_home / "sanremes-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from sanremes_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from sanremes_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".sanremes/", "")
            zf.writestr(".sanremes/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_sanremes_home(self, tmp_path, monkeypatch):
        """Backup handles empty sanremes home (no files to back up)."""
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        # Only excluded dirs, no actual files
        (sanremes_home / "__pycache__").mkdir()
        (sanremes_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        (sanremes_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = sanremes_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        (sanremes_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (sanremes_home / "config.yaml").exists()
        assert (sanremes_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`sanremes import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the SANREMES_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (sanremes_home / "config.yaml").write_text(original)
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (sanremes_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(sanremes_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside SANREMES_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        sanremes_home = dst_home / ".sanremes"
        sanremes_home.mkdir()
    ...
    def test_symlinked_target_keeps_its_symlink(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        store = sanremes_home / "store"
        store.mkdir()
        real = store / "config.yaml"
        real.write_text("model: original\n")
        link = sanremes_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        target = sanremes_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo sanremes import`` onto a user-owned (or Docker/NAS volume-owned)
        SANREMES_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "sanremes_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``sanremes import`` is the
        ...
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
        target = sanremes_home / "helper.sh"
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from sanremes_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from sanremes_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from sanremes_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def sanremes_home(self, tmp_path):
        """Create a fake SANREMES_HOME with critical state files."""
    ...
    def test_state_db_safely_copied(self, sanremes_home):
        from sanremes_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(sanremes_home=sanremes_home)
        db_copy = sanremes_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, sanremes_home, monkeypatch, capsys):
    ...
    def test_restore_state_db_live_connection(self, sanremes_home):
        ...
        """
        from sanremes_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(sanremes_home=sanremes_home)

        # Open a live connection (simulating gateway/dashboard).
        live_conn = sqlite3.connect(str(sanremes_home / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, sanremes_home=sanremes_home)
        assert result is True
    ...
    def test_snapshot_includes_pairing_directories(self, sanremes_home):
        ...
        (sanremes_home / "platforms" / "pairing").mkdir(parents=True)
        (sanremes_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (sanremes_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (sanremes_home / "pairing").mkdir()
        (sanremes_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (sanremes_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = sanremes_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (sanremes update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, sanremes_home, capsys):
        ...
        """
        import json
        from sanremes_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", sanremes_home=sanremes_home)
        assert first_id is not None
        first_dir = sanremes_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", sanremes_home=sanremes_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = sanremes_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, sanremes_home=sanremes_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, sanremes_home):
        ...
        board_dir = sanremes_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(sanremes_home=sanremes_home)
        copy = (
            sanremes_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, sanremes_home=sanremes_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, sanremes_home, monkeypatch):
        ...
        import sanremes_cli.backup as bk
        from sanremes_cli.backup import create_quick_snapshot

        board = sanremes_home / "kanban" / "boards" / "work"
        ...
        copy = sanremes_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``sanremes update``
    runs before touching anything."""


    @pytest.fixture
    def sanremes_home(self, tmp_path):
        root = tmp_path / ".sanremes"
        root.mkdir()
        _make_sanremes_tree(root)
        return root


    def test_backup_contents_match_full_backup(self, sanremes_home):
        """Pre-update backup should include the same user data that
        ``sanremes backup`` would, and should exclude the same directories."""
        from sanremes_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(sanremes_home=sanremes_home)
        ...
        # sanremes-agent repo excluded
        assert not any(n.startswith("sanremes-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, sanremes_home):
        """``sanremes update`` in ``full`` mode takes the quick snapshot *before*
        ...
        from sanremes_cli.backup import (
        ...
        snap_id = create_quick_snapshot(label="pre-update", sanremes_home=sanremes_home)
        assert snap_id and (sanremes_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, sanremes_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(sanremes_home=sanremes_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (sanremes_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, sanremes_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside SANREMES_HOME."""
        from sanremes_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def sanremes_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".sanremes"
        root.mkdir()
        _make_sanremes_tree(root)
        # Point SANREMES_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("SANREMES_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve SANREMES_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(sanremes_home, value):
        import yaml
        (sanremes_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(sanremes_home):
        d = sanremes_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(sanremes_home):
        d = sanremes_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, sanremes_home, capsys):
        ...
        self._set_mode(sanremes_home, "off")
        from sanremes_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(sanremes_home)
        assert not self._zips(sanremes_home)



    def test_config_full_mode(self, sanremes_home, capsys):
        self._set_mode(sanremes_home, "full")
        from sanremes_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(sanremes_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (sanremes claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``sanremes claw migrate`` runs before mutating ~/.sanremes/."""
    ...
    def test_restorable_with_sanremes_import(self, sanremes_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid SanRemes
        backup — `sanremes import` should accept it."""
        from sanremes_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(sanremes_home=sanremes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, sanremes_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from sanremes_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(sanremes_home=sanremes_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(sanremes_home=sanremes_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`sanremes update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, sanremes_home: Path, label="pre-update"):
        from sanremes_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, sanremes_home=sanremes_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from sanremes_cli.backup import restore_cron_jobs_if_emptied
        sanremes_home = tmp_path / ".sanremes"
        jobs_path = sanremes_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(sanremes_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, sanremes_home=sanremes_home)
        assert result is not None
...
# location, NOT under SANREMES_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, sanremes_home: Path) -> None:
        sanremes_home.mkdir(parents=True, exist_ok=True)
        (sanremes_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (sanremes_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (sanremes_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / ".sanremes"
        self._make_min_tree(sanremes_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under SANREMES_HOME,
        ...
        # External state did NOT leak into SANREMES_HOME.
        assert not (sanremes_home / "_external").exists()
