...
import sanremes_cli.auth as auth
...
def sanremes_home(tmp_path, monkeypatch):
    """Point SANREMES_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.sanremes/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, sanremes_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, sanremes_home, windows_default_encoding):
        ...
        assert not (sanremes_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, sanremes_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, sanremes_home):
        auth_path = sanremes_home / "auth.json"
...
# --- sibling readers of the same ~/.sanremes/auth.json in other modules -------
#
# _load_auth_store lives in sanremes_cli/auth.py, but several other modules read
# the same ~/.sanremes/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, sanremes_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(sanremes_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_nous_provider_reads_non_ascii_store(self, sanremes_home, windows_default_encoding, monkeypatch):
        ...
        # SANREMES_HOME env from the fixture doesn't reach it — point it at
        # the tmp store explicitly.
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", sanremes_home / "auth.json")
    ...
    def test_read_shared_nous_state_reads_non_ascii_store(
        ...
        """sanremes_cli.auth._read_shared_nous_state must read a non-ASCII store.
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, sanremes_home, monkeypatch, windows_default_encoding
    ):
        """sanremes_cli.main._has_any_provider_configured reads auth.json.
        ...
        # sanremes_cli.auth; patch it on the source module so the local import
        # sees the fake.
    ...
    def test_managed_tool_gateway_reads_non_ascii_nous_state(
        self, sanremes_home, windows_default_encoding
    ):
