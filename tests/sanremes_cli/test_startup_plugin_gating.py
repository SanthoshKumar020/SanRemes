...
``sanremes_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``sanremes --help``, ``sanremes --version``,
``sanremes logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
from sanremes_cli._parser import build_top_level_parser, top_level_value_flag_sets
...
def _live_subcommand_names() -> set[str]:
    """Run ``sanremes --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from sanremes_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["sanremes", "--help"]
    buf = io.StringIO()
    try:
...
def test_reasoning_value_is_not_misclassified_as_subcommand(monkeypatch):
    monkeypatch.setattr(
        sys,
        "argv",
        ["sanremes", "--reasoning", "high", "chat", "hello"],
    )
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``sanremes photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain sanremes-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level sanremes <name> CLI command via PluginContext.
