"""Regression test: `sanremes dashboard --tui` must not hard-crash.

Older SanRemes desktop app shells (<= 0.15.x) spawn the backend as::

    sanremes dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "SanRemes couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real sanremes_cli.main parser in a subprocess.
