"""Tests for the SanRemes plugin system (sanremes_cli.plugins)."""
...
from sanremes_cli.plugins import (
    ENTRY_POINTS_GROUP,
    VALID_HOOKS,
    PluginContext,
    PluginManager,
...
from sanremes_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV
from sanremes_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<sanremes_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<sanremes_home>/plugins/``; we derive
    ``<sanremes_home>`` from it by walking one level up unless *home* is
    given explicitly.

    Pass *home* explicitly whenever the target SanRemes home for this
    plugin isn't necessarily the current ``SANREMES_HOME`` env var — e.g.
    when writing fixtures for two profiles up front and only switching
    ``SANREMES_HOME``/``set_sanremes_home_override()`` per-profile afterwards
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <SANREMES_HOME>/config.yaml.
        # Config is always read from SANREMES_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        if home is not None:
            sanremes_home = Path(home)
        else:
            import os
            sanremes_home_str = os.environ.get("SANREMES_HOME")
            if sanremes_home_str:
                sanremes_home = Path(sanremes_home_str)
            else:
                sanremes_home = base.parent
        sanremes_home.mkdir(parents=True, exist_ok=True)
        cfg_path = sanremes_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_removed_relay_plugin_identity_cannot_be_reloaded(
        ...
        from sanremes_cli import plugins as plugins_mod
        ...
        assert "Relay lifecycle is owned by SanRemes core" in state.error
        assert RELAY_PLUGINS_CONFIG_ENV in state.error
        assert "Refusing to load removed SanRemes Relay plugin" in caplog.text

    def test_enabled_portable_plugin_registers_components(
        self, tmp_path, monkeypatch
    ):
        from sanremes_cli.agent_plugins import MCP_SCHEMA_V1, PLUGIN_SCHEMA_V1
        from sanremes_cli import plugins as plugins_mod
    ...
    def test_disabled_portable_plugin_registers_nothing(self, tmp_path, monkeypatch):
        from sanremes_cli.agent_plugins import PLUGIN_SCHEMA_V1
        from sanremes_cli import plugins as plugins_mod
    ...
    def test_portable_author_object_is_normalized_to_stable_string(
        ...
        bundled.mkdir()
        monkeypatch.setenv("SANREMES_HOME", str(home))
        monkeypatch.setenv("SANREMES_BUNDLED_PLUGINS", str(bundled))
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "sanremes_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "sanremes_test"))
    ...
    def test_middleware_helpers_skip_no_listener_work(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under sanremes_plugins.<name>."""
        plugins_dir = tmp_path / "sanremes_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "sanremes_test"))

        # Clean up any prior namespace module
        sys.modules.pop("sanremes_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "sanremes_plugins.ns_plugin" in sys.modules
    ...
    def test_user_memory_plugin_auto_coerced_to_exclusive(self, tmp_path, monkeypatch):
        ...
        sanremes_home = tmp_path / "sanremes_test"
        (sanremes_home / "config.yaml").write_text(
    ...
    def test_entrypoint_memory_provider_auto_coerced_to_exclusive(
        ...
        ``sanremes_agent.plugins`` entry point but exposing
        ...
        monkeypatch.setattr(
            "sanremes_cli.plugins.importlib.metadata.entry_points",
    ...
    def test_entrypoint_duplicate_does_not_block_directory_provider_activation(
        ...
        # Same-name directory provider under $SANREMES_HOME/plugins/.
        sanremes_home = tmp_path / "sanremes_test"
        plugins_dir = sanremes_home / "plugins"
...
class TestDeliveryParity:
    ...
    def _fresh_manager(self, monkeypatch, register_body):
        ...
        import sanremes_cli.plugins as plugins_mod
...
class TestForceReloadSymmetry:
    ...
    def test_re_register_config_hooks_clears_idempotence_set(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.config.load_config", lambda: {"hooks": {}}
        )
    ...
    def test_hook_timeout_does_not_block_caller(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.plugins._resolve_hook_callback_timeout", lambda: 0.15
        )
    ...
    def test_hook_callback_within_timeout_returns_value(self, monkeypatch):
        monkeypatch.setattr(
            "sanremes_cli.plugins._resolve_hook_callback_timeout", lambda: 1.0
        )
        mgr = PluginManager()
    ...
    def test_hung_callback_suppresses_repeat_fires(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.plugins._resolve_hook_callback_timeout", lambda: 0.1
        )
    ...
    def test_pre_tool_call_timeout_does_not_reach_tool_handler(self, monkeypatch):
        ...
        from sanremes_cli.plugins import _PRE_TOOL_CALL_TIMEOUT_BLOCK_MESSAGE
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "sanremes_cli.plugins.invoke_hook",
...
class TestPreToolCallDirective:
    ...
    def test_first_party_observer_receives_pre_tool_call(self, monkeypatch):
        from sanremes_cli import observability
        from sanremes_cli.plugins import get_pre_tool_call_directive
...
class TestResolvePreToolBlock:
    ...
    def test_approve_gate_receives_tool_observability_context(self, monkeypatch):
        from sanremes_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from sanremes_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from sanremes_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("sanremes_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `sanremes plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_register_command_empty_name_rejected(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="sanremes_cli.plugins"):
    ...
    def test_plugin_manager_scoped_by_sanremes_home_override(self, tmp_path):
        """set_sanremes_home_override() must get its own manager per profile.

        This is the production path used by the gateway multiplexer
        (``gateway/run.py``'s ``_profile_scope`` context manager) and by
        subagent/embedded callers: it swaps ``SANREMES_HOME`` via a
        context-local ContextVar, which — per
        ``sanremes_constants.set_sanremes_home_override`` — deliberately does
        NOT touch ``os.environ``. A regression test that only flips the
        ``SANREMES_HOME`` env var never exercises this path.
        """
        from sanremes_constants import set_sanremes_home_override, reset_sanremes_home_override
        import sanremes_cli.plugins as plugins_mod
        ...
        def write_engine_plugin(home: Path) -> None:
            _make_plugin_dir(
                ...
                register_body=(
                    ...
                    "        def __init__(self):\n"
                    "            self.home = os.environ.get('SANREMES_HOME')\n\n"
                    "        @property\n"
                    "        def name(self):\n"
                    "            return 'home-engine'\n\n"
                    "        def update_from_response(self, usage):\n"
        ...
        # Note: HomeEngine reads os.environ['SANREMES_HOME'] itself (simulating
        # a real plugin like sanremes-lcm capturing its home at registration),
        ...
        # not last-write-wins.
        token_a2 = set_sanremes_home_override(str(home_a))
        try:
            manager_a2 = plugins_mod.get_plugin_manager()
        finally:
            reset_sanremes_home_override(token_a2)
    ...
    def test_relative_import_not_leaked_across_home_switch(self, tmp_path):
        ...
        ``sanremes_plugins.<slug>``, but a relative import inside the
        plugin's ``__init__.py`` (``from .state import STATE``) is cached
        separately in ``sys.modules`` under
        ``sanremes_plugins.<slug>.state``. If a profile switch replaces only
        ...
        def write_stateful_plugin(home: Path, marker: str) -> None:
            ...
            # `sanremes_plugins.stateful_plugin.state`.
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("sanremes_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("sanremes_cli.plugins.PluginContext.dispatch_tool.__module__", "sanremes_cli.plugins"):
...
class TestPluginDebugLogging:
    """SANREMES_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("SANREMES_PLUGINS_DEBUG", raising=False)
        from sanremes_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from SANREMES_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """SANREMES_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """SANREMES_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".sanremes" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SANREMES_HOME", str(prof))
