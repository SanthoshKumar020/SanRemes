"""Regression tests: a failed quarantine restore must never strand `sanremes`.

On Windows the updater renames the live ``sanremes*.exe`` shims aside
(``sanremes.exe.old.<unix-ms>``) so uv can write replacements. Gaps in the
recovery path ended with ``sanremes`` gone from PATH — and, because the command
that repairs it IS ``sanremes update``, unrecoverable without a manual reinstall
(#75584):
...
from sanremes_cli import _early_recovery as er
from sanremes_cli import _install_repair as ir
...
def test_cleanup_rescue_reports_when_it_cannot_recover(tmp_path, capsys):
    ...
    assert "FAILED to restore sanremes.exe" in captured.err
...
def test_cleanup_rescues_newest_by_parsed_stamp_not_lexicographic(tmp_path):
    ...
    """
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "sanremes.exe.old.999").write_bytes(b"MZ-stray-short-stamp")
    (scripts / f"sanremes.exe.old.{_stamp(60_000)}").write_bytes(b"MZ-genuine")

    with _run_cleanup(scripts):
        cli_main._cleanup_quarantined_exes(scripts)

    assert (scripts / "sanremes.exe").read_bytes() == b"MZ-genuine"


def test_cleanup_ignores_names_it_did_not_create(tmp_path):
    """An unparseable suffix is not ours: never rescued, never deleted."""
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "sanremes.exe").write_bytes(b"MZ-live")
    foreign = scripts / "sanremes.exe.old.backup"
    ...
    assert (scripts / "sanremes.exe").read_bytes() == b"MZ-live"
...
def test_cleanup_does_not_rescue_from_a_foreign_name(tmp_path):
    ...
    assert not (scripts / "sanremes.exe").exists()
...
def test_cleanup_leaves_fresh_quarantine_for_concurrent_update(tmp_path):
    ...
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "sanremes.exe").write_bytes(b"MZ-live")
    fresh = scripts / f"sanremes.exe.old.{_stamp()}"
...
def test_cleanup_still_sweeps_genuinely_stale_quarantine(tmp_path):
    ...
    stale = scripts / f"sanremes.exe.old.{_stamp(ancient_ms)}"
...
def test_quarantine_stamp_ms_parses_and_rejects():
    assert cli_main._quarantine_stamp_ms(Path("sanremes.exe.old.1787020473885")) == 1787020473885
    assert cli_main._quarantine_stamp_ms(Path("sanremes.exe.old.backup")) is None
    assert cli_main._quarantine_stamp_ms(Path("sanremes.exe")) is None
...
def test_helper_retries_then_succeeds(tmp_path):
    scripts = _make_scripts_dir(tmp_path)
    quarantined = scripts / "sanremes.exe.old.123"
    quarantined.write_bytes(b"MZ-old-sanremes")
    original = scripts / "sanremes.exe"
    ...
    assert failed == []
    assert original.read_bytes() == b"MZ-old-sanremes"
    assert calls["n"] >= 2
...
def test_helper_reports_failure_and_returns_the_pair(tmp_path, capsys):
    ...
    assert "FAILED to restore sanremes.exe" in captured.err
    assert "sanremes.exe.old.123" in captured.err
