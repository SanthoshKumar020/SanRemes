...
provider's key) in ~/.sanremes/.env won over a valid pool entry and produced
...
class TestMalformedEnvKeySkipped:
    ...
    def test_malformed_openrouter_env_key_falls_back_to_pool(
        self, isolated_sanremes_home, caplog
    ):
        _write_env_file(isolated_sanremes_home, OPENROUTER_API_KEY="not-a-real-openrouter-key")
    ...
    def test_malformed_env_key_with_empty_pool_returns_empty(
        ...
        _write_env_file(isolated_sanremes_home, OPENROUTER_API_KEY="sk-proj-wrong-provider-key")
        pool = _mock_pool()
    ...
    def test_malformed_pool_entry_skipped_for_valid_one(self, isolated_sanremes_home):
...
class TestNoDeclaredPrefixUnaffected:
    ...
    def test_undeclared_provider_env_key_returned_verbatim(self, isolated_sanremes_home):
        _write_env_file(isolated_sanremes_home, DEEPSEEK_API_KEY="totally-unknown-format-key")
...
class TestValidEnvKeyStillWins:
    ...
    def test_valid_openrouter_env_key_wins_over_pool(self, isolated_sanremes_home):
        _write_env_file(isolated_sanremes_home, OPENROUTER_API_KEY="sk-or-v1-env-key-wins")
    ...
    def test_second_env_var_wins_when_first_is_malformed(self, isolated_sanremes_home):
        """Malformed first env var falls through to a valid SECOND env var."""
        _write_env_file(
            isolated_sanremes_home,
