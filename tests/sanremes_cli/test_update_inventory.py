"""Tests for sanremes_cli.update_inventory — the plan phase (#91277 Phase 2)."""
...
import sanremes_cli.update_inventory as ui
...
def fleet(monkeypatch, tmp_path):
    ...
    import re
    monkeypatch.setattr("sanremes_cli.profiles._get_default_sanremes_home", lambda: default_home)
    monkeypatch.setattr("sanremes_cli.profiles._get_profiles_root", lambda: default_home / "profiles")
    monkeypatch.setattr("sanremes_cli.profiles._PROFILE_ID_RE", re.compile(r"^[a-z0-9][a-z0-9_-]*$"), raising=False)
    monkeypatch.setattr("gateway.status._pid_exists", lambda pid: pid in (100, 200))
    monkeypatch.setattr("sanremes_cli.gateway._get_service_pids", lambda all_profiles=False: {100})
    monkeypatch.setattr("sanremes_cli.gateway.supports_systemd_services", lambda: True)
    monkeypatch.setattr("sanremes_cli.gateway.find_profile_gateway_processes", lambda exclude_pids=None: [])
    monkeypatch.setattr(
        "sanremes_cli.build_info.get_code_identity",
        lambda refresh=False: {"sha": "a" * 40, "short_sha": "a" * 8, "version": "1.0", "source": "git"},
    )
    monkeypatch.setattr("sanremes_cli.config.detect_install_method", lambda *a, **k: "git")
    monkeypatch.setattr("sanremes_cli.config.get_managed_system", lambda: None)
    return tmp_path
...
class TestCollectInventory:
    def test_two_profile_fleet(self, fleet):
        ...
        from sanremes_cli.update_inventory import describe_restart_mechanism

        assert "sanremes -p work gateway restart" in describe_restart_mechanism(
            by_profile["work"].restart_via, "work"
        )

    def test_docker_install_not_updatable_in_place(self, fleet, monkeypatch):
        monkeypatch.setattr("sanremes_cli.config.detect_install_method", lambda *a, **k: "docker")
        monkeypatch.setattr(
            "sanremes_cli.config.recommended_update_command_for_method",
            lambda m: "docker pull nousresearch/sanremes-agent:latest",
        )
        plan = ui.collect_runtime_inventory()
    ...
    def test_pid_file_fallback_covers_unstamped_profiles(self, fleet, monkeypatch):
        ...
        from sanremes_cli.gateway import ProfileGatewayProcess
    ...
    def test_never_raises_when_everything_fails(self, monkeypatch):
        ...
        for target in (
            "sanremes_cli.config.detect_install_method",
            "sanremes_cli.build_info.get_code_identity",
            "sanremes_cli.profiles._get_default_sanremes_home",
...
class TestReceiptIntegration:
    def test_plan_recorded_into_active_receipt(self, fleet, monkeypatch, tmp_path):
        import sanremes_cli.update_receipt as ur

        home = tmp_path / "receipt_home"
        home.mkdir()
        monkeypatch.setattr("sanremes_cli.config.get_sanremes_home", lambda: home, raising=False)
        ur._current = None
