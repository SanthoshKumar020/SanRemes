...
sanremes_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `sanremes config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_sanremes_home = get_sanremes_home()`` at import time (module
    singleton), so monkeypatching SANREMES_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SANREMES_HOME.)
    """
    import cli

    cli._sanremes_home = home
