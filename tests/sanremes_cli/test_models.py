"""Tests for the sanremes_cli models module."""
...
import sanremes_cli.models as _models_mod
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("sanremes_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("sanremes_cli.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("sanremes_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        sanremes-agent is tool-calling-first — surfacing a non-tool model leads to
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("sanremes_cli.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("sanremes_cli.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from sanremes_cli.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from sanremes_cli.models import _find_openrouter_slug
        with patch("sanremes_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "sanremes_cli.models.fetch_nous_recommended_models",
...
class TestCheckNousFreeTierCache:
    ...
    @patch("sanremes_cli.nous_account.get_nous_portal_account_info")
...
class TestNousRecommendedModels:
    ...
    def test_fetch_caches_per_portal_url(self):
        from sanremes_cli.models import fetch_nous_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("sanremes_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("sanremes_cli.models.fetch_nous_recommended_models", return_value=payload):
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("sanremes_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("sanremes_cli.models.check_nous_free_tier", side_effect=RuntimeError("boom")),
        ):
...
class TestCodexSoftAcceptPlausibilityGate:
    ...
    def test_unrelated_name_rejected_on_openai_codex(self):
        from sanremes_cli.models import validate_requested_model
...
class TestClaudeSonnet5InCuratedLists:
    ...
    def test_anthropic_native_list_includes_sonnet_5(self):
        from sanremes_cli.models import _PROVIDER_MODELS
...
class TestFormatPricePerMtok:
    ...
    def test_standard_prices_keep_two_decimals(self):
        from sanremes_cli.models import _format_price_per_mtok
...
class TestLocalOllamaModelDiscovery:
    def test_provider_model_ids_uses_ollama_api_tags_from_provider_config(self):
        ...
        from sanremes_cli.models import provider_model_ids

        server, port = _start_fake_ollama_server()
        try:
            with patch(
                "sanremes_cli.config.load_config",
    ...
    def test_native_tags_cache_expires(self, monkeypatch):
        from sanremes_cli.models import fetch_ollama_local_models
        ...
        try:
            ...
            monkeypatch.setattr("sanremes_cli.models.time.monotonic", lambda: 301.0)
    ...
    def test_runtime_error_from_config_load_does_not_escape_ollama_helpers(self):
        ...
        from sanremes_cli.models import _get_ollama_base_url, should_use_ollama_native_catalog

        with patch("sanremes_cli.config.load_config", side_effect=RuntimeError("bad home")), patch(
            "sanremes_cli.models.probe_ollama_local_models",
            return_value=None,
        ):
    ...
    def test_probe_ollama_models_malformed_base_url_returns_none(self):
        ...
        from sanremes_cli.models import probe_ollama_local_models
    ...
    def test_fetch_ollama_models_preserves_probe_failure(self):
        ...
        with patch("sanremes_cli.models.probe_ollama_local_models", return_value=None):
    ...
    def test_ollama_port_detection_requires_working_api_tags(self):
        ...
        with patch("sanremes_cli.models.probe_ollama_local_models", return_value=["qwen3:1.7b"]):
    ...
    def test_provider_model_ids_ollama_cloud_config_uses_generic_catalog(self):
        ...
        ), patch("sanremes_cli.models.fetch_ollama_local_models") as fetch_local, patch(
            "sanremes_cli.models.fetch_api_models",
    ...
    def test_native_ollama_catalog_uses_configured_key_env(self, monkeypatch):
        from sanremes_cli.models import _get_ollama_request_headers
    ...
    def test_provider_model_ids_ignores_active_non_ollama_custom_endpoint(self):
        ...
        with patch(
            ...
            "sanremes_cli.models.fetch_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ) as fetch_local:
            assert provider_model_ids("ollama", force_refresh=True) == ["qwen3:1.7b"]
    ...
    def test_ollama_cache_fingerprint_does_not_probe_custom_endpoint(self):
        from sanremes_cli.models import _credential_fingerprint
        ...
        ), patch("sanremes_cli.models.probe_ollama_local_models") as probe_ollama:
    ...
    def test_clear_provider_models_cache_clears_ollama_native_tags_cache(self):
        import sanremes_cli.models as models
    ...
    def test_picker_user_provider_row_discovers_ollama_api_tags(self):
        ...
        from sanremes_cli.model_switch import list_authenticated_providers
    ...
    def test_picker_user_provider_verifies_ambiguous_ollama_port(self):
        ...
        with patch("sanremes_cli.config.load_config", return_value={"providers": {}}), patch(
            "sanremes_cli.models.should_use_ollama_native_catalog",
            return_value=True,
        ), patch(
            "sanremes_cli.models.fetch_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ), patch("sanremes_cli.models.fetch_api_models", return_value=[]) as fetch_api:
            rows = list_authenticated_providers(
    ...
    def test_named_custom_model_flow_discovers_ollama_api_tags(self):
        ...
        from sanremes_cli.main import _model_flow_named_custom
        ...
        try:
            with patch("sanremes_cli.config.load_config", return_value=config), patch(
                "sanremes_cli.config.save_config"
            ), patch("sanremes_cli.auth._save_model_choice"), patch(
                "sanremes_cli.auth.deactivate_provider"
            ), patch("sanremes_cli.main._save_custom_provider"), patch(
                "sanremes_cli.curses_ui.curses_radiolist",
    ...
    def test_model_validation_ollama_cloud_config_does_not_use_local_tags(self):
        ...
        ), patch("sanremes_cli.models.probe_ollama_local_models") as probe_ollama, patch(
            "sanremes_cli.models.probe_api_models",
            return_value={
                "models": ["qwen3:1.7b"],
    ...
    def test_switch_model_on_current_ollama_custom_endpoint_keeps_base_url(self):
        ...
        from sanremes_cli.model_switch import switch_model
        ...
        try:
            with patch(
                ...
                "sanremes_cli.model_switch.get_model_info",
                return_value=None,
            ):
                result = switch_model(
    ...
    def test_switch_model_on_non_ollama_custom_endpoint_still_resolves_runtime(self):
        ...
        with patch(
            ...
            "sanremes_cli.models.validate_requested_model",
            return_value={
                "accepted": True,
                "persist": True,
                "recognized": True,
    ...
    def test_ollama_root_matching_is_case_insensitive_for_hostnames(self):
        from sanremes_cli.models import _same_ollama_native_root
    ...
    def test_ollama_host_environment_forms_are_normalized(self, monkeypatch):
        from sanremes_cli.models import _get_ollama_base_url, _root_for_ollama_native_api
    ...
    def test_ollama_failed_probe_is_cached_briefly(self):
        ...
        with patch(
            "sanremes_cli.models._urlopen_model_catalog_request",
    ...
    def test_switch_model_direct_ollama_alias_preserves_matching_origin_api_key(self):
        import sanremes_cli.model_switch as model_switch
        ...
        try:
            with patch.object(model_switch, "get_model_info", return_value=None), patch(
                "sanremes_cli.models._get_provider_config_dict",
