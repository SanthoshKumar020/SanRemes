"""ensure_sanremes_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".sanremes"
    second = tmp_path / "b" / ".sanremes"

    monkeypatch.setenv("SANREMES_HOME", str(first))
    cfg.ensure_sanremes_home()

    # Profile switch: SANREMES_HOME moves → the new path is ensured too.
    monkeypatch.setenv("SANREMES_HOME", str(second))
    cfg.ensure_sanremes_home()
