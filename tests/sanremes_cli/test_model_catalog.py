"""Tests for sanremes_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate SANREMES_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from sanremes_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from sanremes_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://sanremes-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/NousResearch/sanremes-agent"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import sanremes_cli.model_catalog as model_catalog
        from sanremes_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in sanremes_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``sanremes model`` CLI — not the live
        ...
        # ``SANREMES_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` SANREMES_HOME directly instead.
        import importlib
        from sanremes_cli import model_catalog
        from sanremes_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from sanremes_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["SANREMES_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("sanremes_cli.models.check_nous_free_tier", return_value=False), patch(
                "sanremes_cli.models.union_with_portal_free_recommendations",
