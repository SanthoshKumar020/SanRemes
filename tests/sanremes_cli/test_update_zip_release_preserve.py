...
(release/win-unpacked/SanRemes.exe) exists only in the live tree. Swapping
...
def test_staged_apps_swap_preserves_live_release_dir(tmp_path, monkeypatch):
    ...
    # live tree: apps/desktop/release/win-unpacked/SanRemes.exe + old source
    root = tmp_path / "install"
    live_apps = root / "apps" / "desktop"
    (live_apps / "release" / "win-unpacked").mkdir(parents=True)
    (live_apps / "release" / "win-unpacked" / "SanRemes.exe").write_bytes(b"MZbuilt")
    ...
    monkeypatch.setattr(sanremes_main, "PROJECT_ROOT", root)
    ...
    exe = root / "apps" / "desktop" / "release" / "win-unpacked" / "SanRemes.exe"
