...
`sanremes curator pin <name>` on an unmanaged skill (curation-eligible but no
...
becomes meaningful after `sanremes curator adopt`) and makes the message say
...
def test_pin_unmanaged_records_flag_and_prints_adopt_hint(monkeypatch, capsys):
    ...
    assert "sanremes curator adopt legacy-skill" in out
