...
from sanremes_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_sanremes_tools_mcp_entry,
    _format_toml_value,
...
# ---- end-to-end migrate(, expose_sanremes_tools=False) ----
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from sanremes_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_sanremes_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(sanremes_cfg, codex_home=tmp_path, expose_sanremes_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside SanRemes' managed block, re-running
        ...
        migrate({"mcp_servers": {"sanremes-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.sanremes-mcp]" in final
    ...
    def test_summary_reports_migration_count(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_sanremes_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``sanremes codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_sanremes_tools=False)
...
# ---- Bug C: SANREMES_HOME tempdir leak into ~/.codex/config.toml ----


class TestSanRemesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_sanremes_tools_mcp_entry()`` read ``SANREMES_HOME``
    ...
    once codex spawned the sanremes-tools MCP subprocess.
    """




    def test_real_sanremes_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate SANREMES_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.sanremes"
        monkeypatch.setenv("SANREMES_HOME", real_path)
        entry = _build_sanremes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("SANREMES_HOME") == real_path

    def test_unset_sanremes_home_omits_env_key(self, monkeypatch):
        """When SANREMES_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever SANREMES_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_sanremes_tools_mcp_entry()
        env = entry.get("env", {})
        assert "SANREMES_HOME" not in env, (
            f"SANREMES_HOME should not be set when env var is unset, got: "
            f"{env.get('SANREMES_HOME')!r}"
        )
