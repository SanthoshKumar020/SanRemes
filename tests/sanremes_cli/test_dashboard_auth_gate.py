...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    # bind (June 2026 sanremes-0day hardening). Non-loopback always requires auth.
    ("0.0.0.0",   True,  True),
    ("0.0.0.0",   False, True),
...
def test_empty_provider_login_page_shows_supported_auth_paths():
    ...
    assert (
        'href="https://sanremes-agent.nousresearch.com/docs/'
...
def test_trusted_container_proxy_controls_https_detection():
    ...
    from sanremes_cli.dashboard_auth.cookies import detect_https
...
def test_public_url_aware_gate_requires_auth_for_loopback_proxy(monkeypatch):
    ...
    from sanremes_cli.web_server import should_require_dashboard_auth

    monkeypatch.setenv(
        "SANREMES_DASHBOARD_PUBLIC_URL",
...
def test_loopback_public_url_fail_closed_message_is_actionable(monkeypatch):
    ...
    assert "sanremes dashboard register" in msg
...
def test_should_require_dashboard_auth_truth_table(
    ...
    if public_url is None:
        monkeypatch.delenv("SANREMES_DASHBOARD_PUBLIC_URL", raising=False)
        monkeypatch.setattr(
            web_server, "_dashboard_public_hosts", lambda: frozenset()
        )
    else:
        monkeypatch.setenv("SANREMES_DASHBOARD_PUBLIC_URL", public_url)
