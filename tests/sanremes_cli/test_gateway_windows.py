"""Tests for sanremes_cli.gateway_windows."""
...
import sanremes_cli.setup as setup
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda sanremes_home: "")
    monkeypatch.setattr("sanremes_cli.config.get_sanremes_home", lambda: str(sanremes_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "sanremes_cli.main"]
    assert cwd == str(sanremes_home.resolve())
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "sanremes_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_build_gateway_argv",
        lambda: (argv, cwd, {"SANREMES_GATEWAY_DETACHED": "1"}),
    )
    monkeypatch.setattr("sanremes_cli.config.get_sanremes_home", lambda: tmp_path)
...
def test_spawn_detached_warns_and_marks_no_breakaway_fallback(
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            argv,
            cwd,
            {"SANREMES_GATEWAY_DETACHED": "1", "SECRET_SENTINEL": "do-not-log"},
        ),
    )
    monkeypatch.setattr("sanremes_cli.config.get_sanremes_home", lambda: tmp_path)
    monkeypatch.setattr(gateway_windows.subprocess, "Popen", fake_popen)
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_sanremes_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".sanremes"
        home.mkdir()
        monkeypatch.setattr("sanremes_cli.config.get_sanremes_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".sanremes"
        project = tmp_path / "checkout"
        monkeypatch.setattr("sanremes_cli.config.get_sanremes_home", lambda: missing)
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "SanRemes_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "SanRemes_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "SanRemes_Gateway_alice")
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\SanRemes\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\SanRemes\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("SanRemes_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "SanRemes_Gateway_alice")
    ...
    assert "SanRemes_Gateway_alice.vbs" in xml_seen["text"]
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "sanremes_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("SANREMES_HOME", "PYTHONIOENCODING", "SANREMES_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
# resume_pending=True) never fires when `sanremes gateway stop` kills the
