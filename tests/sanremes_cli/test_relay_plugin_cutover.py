"""Regression tests for migration from the removed SanRemes Relay plugin."""
...
from sanremes_cli.config import migrate_config
from sanremes_cli.doctor import collect_relay_plugin_cutover_findings
...
def test_doctor_reports_legacy_exporter_env_without_new_config(monkeypatch):
    ...
    findings = dict(
        collect_relay_plugin_cutover_findings(
            {},
            {
                "SANREMES_NEMO_RELAY_ATIF_ENABLED": "true",
                "SANREMES_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S": "30",
            },
        )
    )

    assert "now ignored" in findings["SANREMES_NEMO_RELAY_ATIF_ENABLED"]
    assert RELAY_PLUGINS_CONFIG_ENV in findings["SANREMES_NEMO_RELAY_ATIF_ENABLED"]
    assert "now ignored" in findings["SANREMES_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S"]
    assert (
        RELAY_PLUGINS_CONFIG_ENV
        in findings["SANREMES_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S"]
    )
...
def test_enable_rejects_removed_relay_plugin_without_discovery(name, capsys):
    with (
        patch("sanremes_cli.plugins_cmd._resolve_plugin_key_and_source") as resolve,
        patch("sanremes_cli.plugins_cmd._save_enabled_set") as save_enabled,
    ):
        from sanremes_cli.plugins_cmd import cmd_enable
...
def test_enable_rejects_alias_resolving_to_removed_relay_plugin(capsys):
    with (
        patch(
            "sanremes_cli.plugins_cmd._resolve_plugin_key_and_source",
