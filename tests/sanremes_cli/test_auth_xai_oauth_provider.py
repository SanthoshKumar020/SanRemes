"""Tests for xAI Grok OAuth — tokens stored in SanRemes auth store (~/.sanremes/auth.json)."""
...
def _setup_sanremes_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the SanRemes auth store at the given root."""
    ...
    auth_file = sanremes_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("sanremes_cli.auth.httpx.Client", _factory)
    return holder
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    sanremes_home.mkdir(parents=True, exist_ok=True)
    (sanremes_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_refresh_xai_oauth_tokens_preserves_active_provider(tmp_path, monkeypatch):
    ...
    _setup_sanremes_auth(sanremes_home, access_token=near, refresh_token="rt-old")
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

    auth_path = sanremes_home / "auth.json"
    ...
    monkeypatch.setattr("sanremes_cli.auth.refresh_xai_oauth_pure", _fake_pure)
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
# ``XAI_BASE_URL`` / ``SANREMES_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    sanremes_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    sanremes_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (sanremes_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    sanremes_home = tmp_path / "sanremes"
    _seed_xai_oauth_state(sanremes_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
    ...
    monkeypatch.setattr("sanremes_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((sanremes_home / "auth.json").read_text())
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``sanremes model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``sanremes model``
    ...
    assert "sanremes model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from sanremes_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    SanRemes that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `sanremes model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `sanremes auth list` reflects truth and
    ...
    _setup_sanremes_auth(sanremes_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
...
def test_credential_pool_device_code_seed_respects_suppression(tmp_path, monkeypatch):
    from agent.credential_pool import load_pool
    from sanremes_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``sanremes auth remove xai-oauth 1`` for a
    ...
    from agent.credential_pool import load_pool
    from sanremes_cli.auth_commands import auth_remove_command
    ...
    # Act: the user runs `sanremes auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((sanremes_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every SanRemes invocation."
    )


def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``sanremes model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``sanremes model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``sanremes auth
    ...
    (sanremes_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
    monkeypatch.delenv("SANREMES_XAI_BASE_URL", raising=False)
    ...
    new_access = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    monkeypatch.setattr(
        "sanremes_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
                "access_token": new_access,
    ...
    monkeypatch.setattr(
        "sanremes_cli.auth._update_config_for_provider",
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_sanremes_auth(sanremes_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
    ...
    monkeypatch.setattr("sanremes_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_runtime_provider_uses_pool_entry_for_xai_oauth(tmp_path, monkeypatch):
    ...
    sanremes_home = tmp_path / "sanremes"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_sanremes_auth(sanremes_home, access_token=fresh)
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
    monkeypatch.delenv("SANREMES_XAI_BASE_URL", raising=False)
    monkeypatch.delenv("XAI_BASE_URL", raising=False)
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_sanremes_auth(sanremes_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
    ...
    def _fake_refresh(access_token, refresh_token, **kwargs):
        ...
        (sanremes_home / "auth.json").write_text(json.dumps(raw))
        raise AuthError(
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`sanremes auth add xai-oauth` entries (source='manual:xai_pkce') are
    ...
    _setup_sanremes_auth(sanremes_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``sanremes auth status``, ``sanremes setup``, and the
    ``sanremes`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    sanremes_home = tmp_path / "sanremes"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_sanremes_auth(sanremes_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
