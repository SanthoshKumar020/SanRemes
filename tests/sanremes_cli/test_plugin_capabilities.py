...
from sanremes_cli.plugin_capabilities import (
    CAPABILITY_REGISTRY,
...
def sanremes_home(tmp_path, monkeypatch):
    """Point SANREMES_HOME at a tmp dir with an empty config.yaml."""
...
class TestDeclarationParsing:
    ...
    def test_entrypoint_companion_metadata_declares_capabilities_without_import(
        ...
        capability_ep = SimpleNamespace(
            ...
            value="thread_namer.plugin:register",
            group="sanremes_agent.plugin_capabilities",
            load=load,
        )
        monkeypatch.setattr(
...
class TestConsentPersistence:
    def test_record_consent_persists_grant(self, sanremes_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        cfg = _read_cfg(sanremes_home)
    ...
    def test_record_consent_mirrors_nested_legacy_key(self, sanremes_home):
        record_consent(
            "capplug", ["llm.model_override"], ["llm.model_override"]
        )
        entry = _read_cfg(sanremes_home)["plugins"]["entries"]["capplug"]
        assert entry["llm"]["allow_model_override"] is True

    def test_granted_capabilities_roundtrip(self, sanremes_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        assert granted_capabilities("capplug") == frozenset({"tools.override"})

    def test_grant_is_union_with_previous(self, sanremes_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        record_consent(
            "capplug",
            ["llm.model_override"],
    ...
    def test_capability_granted_after_consent(self, sanremes_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        assert plugin_capability_granted("capplug", "tools.override") is True

    def test_declined_stays_off(self, sanremes_home):
...
class TestUpdateReconsent:
    def test_added_capability_is_pending(self, sanremes_home):
    ...
    def test_unchanged_set_needs_no_reconsent(self, sanremes_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        assert pending_capabilities("capplug", ["tools.override"]) == []
    ...
    def test_no_consent_record_counts_as_changed(self, sanremes_home):
        assert declared_set_changed("capplug", ["tools.override"]) is True

    def test_reconsent_grants_addition(self, sanremes_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        declared_v2 = ["tools.override", "llm.model_override"]
        record_consent(
...
class TestFailClosed:
    def test_missing_config_not_granted(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "nonexistent"))
        assert plugin_capability_granted("capplug", "tools.override") is False
        assert granted_capabilities("capplug") == frozenset()

    def test_corrupt_granted_list_not_granted(self, sanremes_home):
    ...
    def test_corrupt_entry_types_not_granted(self, sanremes_home):
        (sanremes_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    capplug: 42\n", encoding="utf-8"
        )
        assert plugin_capability_granted("capplug", "tools.override") is False

    def test_unknown_capability_denied(self, sanremes_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        assert plugin_capability_granted("capplug", "root.everything") is False

    def test_unknown_ids_in_granted_list_ignored(self, sanremes_home):
        (sanremes_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    capplug:\n"
    ...
    def test_corrupt_consent_hash_counts_as_changed(self, sanremes_home):
        (sanremes_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    capplug:\n"
...
class TestLegacyGateCompat:
    def test_legacy_allow_tool_override_still_works(self, sanremes_home):
        (sanremes_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    oldplug:\n"
    ...
    def test_legacy_nested_llm_key_still_works(self, sanremes_home):
        (sanremes_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    oldplug:\n"
    ...
    def test_legacy_false_stays_denied(self, sanremes_home):
        (sanremes_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    oldplug:\n"
    ...
    def test_tool_override_gate_uses_canonical_path(self, sanremes_home):
        """PluginContext._tool_override_allowed honors capability grant."""
        from sanremes_cli.plugins import PluginContext, PluginManifest, PluginManager
    ...
    def test_tool_override_gate_denies_without_grant(self, sanremes_home):
        from sanremes_cli.plugins import PluginContext, PluginManifest, PluginManager
    ...
    def test_tool_override_gate_legacy_key(self, sanremes_home):
        from sanremes_cli.plugins import PluginContext, PluginManifest, PluginManager
    ...
    def test_bundled_plugin_trusted(self, sanremes_home):
        from sanremes_cli.plugins import PluginContext, PluginManifest, PluginManager
...
class TestHasCapability:
    def test_probe_granted(self, sanremes_home):
        from sanremes_cli.plugins import PluginContext, PluginManifest, PluginManager
...
class TestConsentFlow:
    ...
    def test_consent_yes_records_grant(self, sanremes_home, monkeypatch):
        from sanremes_cli.plugins_cmd import _run_capability_consent
    ...
    def test_consent_decline_leaves_ungranted(self, sanremes_home):
        from sanremes_cli.plugins_cmd import _run_capability_consent
    ...
    def test_non_interactive_fails_closed(self, sanremes_home):
        from sanremes_cli.plugins_cmd import _run_capability_consent
    ...
    def test_already_granted_skips_prompt(self, sanremes_home):
        from sanremes_cli.plugins_cmd import _run_capability_consent
