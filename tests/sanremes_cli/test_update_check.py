"""Tests for the update check mechanism in sanremes_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    from sanremes_cli.banner import check_for_updates
    from sanremes_cli import __version__

    # Create a fake git repo and fresh cache
    repo_dir = tmp_path / "sanremes-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("sanremes_cli.banner.subprocess.run") as mock_run:
...
def test_check_via_local_git_fetch_failure_returns_none(tmp_path, monkeypatch):
    ...
    """
    from sanremes_cli import banner
    ...
    def mock_git_stdout(args, *, cwd, timeout=5):
        if args[:2] == ["remote", "get-url"]:
            return "https://github.com/NousResearch/sanremes-agent.git"
...
def test_check_for_updates_does_not_cache_none(tmp_path, monkeypatch):
    ...
    cache_file = tmp_path / ".update_check"
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
    monkeypatch.delenv("SANREMES_REVISION", raising=False)
    ...
    monkeypatch.setattr(
        "sanremes_cli.config.detect_install_method", lambda root: "git"
    )
    monkeypatch.setattr(
        "sanremes_cli.config.get_project_root", lambda: repo_dir
    )
    ...
    # Simpler: just patch the get_sanremes_home and the repo_dir resolution
    # by making check_for_updates find our fake repo via sanremes_home fallback.
    # The code checks Path(__file__).parent.parent/.git first, then falls
    # back to sanremes_home / "sanremes-agent". We ensure the fallback hits.
    # To do this, we make Path(__file__).parent.parent.resolve() return
    # a path without .git, so it falls through to sanremes_home / "sanremes-agent".
    real_resolve = Path.resolve

    def fake_resolve(self, *args, **kwargs):
        s = str(self)
        if "banner.py" in s or s.endswith("sanremes_cli"):
