"""Tests for sanremes_cli.process_identity — spawn tags, the machine spawn
...
(SANREMES_SPAWN), long-lived processes self-register (pid, create_time,
purpose, spawner) in spawn-ledger.json, and `sanremes update` reaps holders the
...
def test_install_id_stable_and_path_scoped():
    a = pi.install_id(Path("/opt/sanremes"))
    assert a == pi.install_id(Path("/opt/sanremes"))
...
def test_register_self_falls_back_to_desktop_parent_env(tmp_path):
    # Legacy Desktop: no SANREMES_SPAWN, but SANREMES_PARENT_PID + winms marker.
    ledger = tmp_path / "spawn-ledger.json"
    fake = _fake_psutil({999: 50.0})
    env = {
        "SANREMES_PARENT_PID": "888",
        "SANREMES_PARENT_START_MARKER": "winms:1755689000123",
    }
    with patch.dict(sys.modules, {"psutil": fake}), \
         patch.dict(pi.os.environ, env, clear=False), \
...
def _holders(*pids):
    return [(p, "python.exe", f"python.exe -m sanremes_cli.main --profile p{p} serve") for p in pids]
