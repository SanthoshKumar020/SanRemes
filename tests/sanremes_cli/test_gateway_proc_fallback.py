...
See: NousResearch/sanremes-agent#7622
"""
...
_GATEWAY_CMD = "python -m sanremes_cli.main gateway run"
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m sanremes_cli.main",   # own process — excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            ...
            patch("sanremes_cli.gateway._get_ancestor_pids", return_value=set()),
...
class TestPsFallbackBsdCompat:
    ...
    def test_ps_flags_are_bsd_compatible(self):
        ...
        with (
            patch("sanremes_cli.gateway.is_windows", return_value=False),
...
class TestGetServicePidsAllProfiles:
    ...
    def test_default_scope_uses_current_profile_label(self):
        ...
        with (
            patch("sanremes_cli.gateway.is_macos", return_value=True),
            patch("sanremes_cli.gateway.supports_systemd_services", return_value=False),
            patch(
                "sanremes_cli.gateway.get_launchd_label",
                return_value="ai.sanremes.gateway.myprofile",
            ),
            patch(
                "sanremes_cli.gateway._locate_launchd_gateway_service",
        ...
        assert located == ["ai.sanremes.gateway.myprofile"]
        launchctl_calls = [
            c[0][0]
    ...
    def test_all_profiles_enumerates_all_gateway_labels(self):
        ...
        widens the EXCLUDE set with unmapped ai.sanremes.gateway* agents
        (#74075 belt-and-suspenders)."""
        located = []
        label_pids = {
            "ai.sanremes.gateway": 123,
            "ai.sanremes.gateway-profile-b": 456,
        }
        ...
        with (
            ...
            patch(
                "sanremes_cli.gateway.get_launchd_label",
                return_value="ai.sanremes.gateway",
            ),
            patch(
                "sanremes_cli.gateway.launchd_gateway_labels_for_install",
                return_value=["ai.sanremes.gateway", "ai.sanremes.gateway-profile-b"],
            ),
            patch(
                "sanremes_cli.gateway._locate_launchd_gateway_service",
                side_effect=_fake_locate,
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "999\t0\tai.sanremes.gateway-unmapped\n"
    ...
    def test_all_profiles_empty_when_no_gateway_labels(self):
        """When no ai.sanremes.gateway* labels exist, all_profiles returns empty."""
        with (
            patch("sanremes_cli.gateway.is_macos", return_value=True),
            patch("sanremes_cli.gateway.supports_systemd_services", return_value=False),
            patch("subprocess.run") as mock_run,
    ...
    def test_all_profiles_handles_broken_pid_column(self):
        ...
        with (
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "-\t0\tai.sanremes.gateway.broken\n"
                    "  123  \t0\tai.sanremes.gateway.ok\n"
                ),
                stderr="",
            )
            pids = gateway_mod._get_service_pids(all_profiles=True)
    ...
    def test_all_profiles_preserves_systemd_behavior(self):
        ...
        lists every sanremes-gateway* unit unconditionally."""
        with (
            patch("sanremes_cli.gateway.is_macos", return_value=False),
            patch("sanremes_cli.gateway.supports_systemd_services", return_value=True),
            patch("subprocess.run") as mock_run,
        ):
            def _run_side_effect(args, **kwargs):
                ...
                if "list-units" in cmd_str:
                    return MagicMock(
                        returncode=0,
                        stdout="sanremes-gateway-jarvis.service loaded active running\n",
                        stderr="",
                    )
