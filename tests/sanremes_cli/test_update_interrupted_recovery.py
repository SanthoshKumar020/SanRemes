...
``sanremes update`` killed mid-install (Ctrl-C, terminal close, WSL OOM) gets
...
def _stub_install_env(monkeypatch, m, seen):
    ...
    monkeypatch.setattr("sanremes_cli.managed_uv.ensure_uv", lambda: None)
    # The install executor moved to sanremes_cli._install_repair (shared between
...
def test_recovery_self_lock_does_not_clear_core_marker_via_import_probes(
    ...
    monkeypatch.setattr(m, "_sanremes_exe_shims", lambda d: [shim])
    monkeypatch.setattr(
        m,
