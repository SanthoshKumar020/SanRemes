...
def sanremes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_ENABLE_PROJECT_PLUGINS", "0")
    monkeypatch.setenv(
        "SANREMES_BUNDLED_PLUGINS", str(tmp_path / "empty-bundled")
    )
...
class TestV1Regression:
    def test_v1_manifest_parses_with_defaults(self, sanremes_home):
        _write_plugin(sanremes_home / "plugins", "oldie")
        _enable(sanremes_home, ["oldie"])
        mgr = PluginManager()
    ...
    def test_v1_unknown_fields_do_not_warn_loudly(self, sanremes_home, caplog):
        _write_plugin(
            sanremes_home / "plugins", "oldie",
...
class TestV2Parsing:
    def test_v2_fields_parse(self, sanremes_home):
        _write_plugin(
            sanremes_home / "plugins", "modern",
            manifest_extra={
                "manifest_version": 2,
                "api_version": 1,
                "license": "MIT",
    ...
    def test_unknown_field_in_v2_warns_but_loads(self, sanremes_home, caplog):
        _write_plugin(
            sanremes_home / "plugins", "modern",
    ...
    def test_future_manifest_version_warns_but_loads(self, sanremes_home, caplog):
        _write_plugin(
            sanremes_home / "plugins", "fromfuture",
            manifest_extra={
                "manifest_version": SUPPORTED_MANIFEST_VERSION + 5,
            },
        )
        _enable(sanremes_home, ["fromfuture"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
        assert mgr._plugins["fromfuture"].enabled
        assert "newer than this SanRemes" in caplog.text

    def test_malformed_v2_fields_warn_and_degrade(self, sanremes_home, caplog):
        _write_plugin(
            sanremes_home / "plugins", "sloppy",
            manifest_extra={
                "manifest_version": 2,
                "api_version": "banana",
...
class TestDependencyOrder:
    def test_dep_registers_before_dependent(self, sanremes_home):
        ...
        _write_plugin(
            sanremes_home / "plugins", "aaa-consumer",
            manifest_extra={
                "manifest_version": 2,
        ...
        _write_plugin(
            sanremes_home / "plugins", "zzz-base",
            register_body="import sys; sys._m2_order.append('zzz-base')",
        )
        _enable(sanremes_home, ["aaa-consumer", "zzz-base"])
        import sys
    ...
    def test_missing_dep_warns_but_loads(self, sanremes_home, caplog):
        _write_plugin(
            sanremes_home / "plugins", "needy",
            manifest_extra={
                "manifest_version": 2,
                "requires_plugins": [{"id": "ghost-plugin"}],
            },
        )
        _enable(sanremes_home, ["needy"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_cycle_warns_and_falls_back_alpha(self, sanremes_home, caplog):
        _write_plugin(
            sanremes_home / "plugins", "cyc-a",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(sanremes_home, ["cyc-a", "cyc-b"])
        import sys
...
class TestConfigSchema:
    def test_type_mismatch_warns_but_loads(self, sanremes_home, caplog):
        _write_plugin(
            sanremes_home / "plugins", "cfgd",
            manifest_extra={
                "manifest_version": 2,
                "config_schema": {
                    "api_url": {"type": "str"},
    ...
    def test_unknown_declared_type_skips_check(self, sanremes_home, caplog):
        _write_plugin(
            sanremes_home / "plugins", "weird",
            manifest_extra={
                "manifest_version": 2,
...
class TestPythonDependenciesSeam:
    def test_missing_pip_dep_surfaced_with_hint_not_installed(
        self, sanremes_home, caplog, monkeypatch
    ):
        calls = []
        import subprocess
    ...
    def test_satisfied_pip_dep_is_quiet(self, sanremes_home, caplog):
        _write_plugin(
            sanremes_home / "plugins", "pipok",
            manifest_extra={
                "manifest_version": 2,
                "python_dependencies": ["pyyaml>=5,<7"],
            },
        )
        _enable(sanremes_home, ["pipok"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestCtxHasPlugin:
    def test_has_plugin_probe(self, sanremes_home):
        _write_plugin(sanremes_home / "plugins", "probe-target")
        _write_plugin(
            sanremes_home / "plugins", "prober",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(sanremes_home, ["probe-target", "prober"])
        import sys
