"""Tests for the direct SanRemes-to-Relay shared-metrics runtime."""
...
from sanremes_cli import lifecycle, plugins
from sanremes_cli.observability import relay_runtime, relay_shared_metrics
from sanremes_cli.plugins import PluginManager
...
def direct_runtime(tmp_path, monkeypatch):
    fake = _Relay()
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "sanremes-home"))
    monkeypatch.setattr(relay_runtime, "_load_nemo_relay", lambda: fake)
    monkeypatch.setattr(
        "sanremes_cli.config.read_raw_config_readonly",
...
def test_direct_runtime_records_without_enabling_a_plugin(direct_runtime, tmp_path):
    ...
    assert scope_starts[1][1] == "sanremes.task_run"
    ...
    assert tool_starts[0][1] == "sanremes.tool_call"
    ...
    active_marks = [
        ...
        if event[0] == "scope.event" and event[1] == "sanremes.client.active"
    ]
    ...
    root = tmp_path / "sanremes-home" / "telemetry" / "shared_metrics"
    ...
    assert set(metrics) == {
        "sanremes.client.active",
        "sanremes.model_route.count",
        "sanremes.task_run.finished",
        "sanremes.task_run.started",
        "sanremes.tool_approval.count",
        "sanremes.tool_call.count",
    }
    assert metrics["sanremes.client.active"] == {
        "name": "sanremes.client.active",
        "type": "counter",
        "dimensions": {},
        "value": 1,
    }
    assert metrics["sanremes.model_route.count"]["dimensions"] == {
        "model": "claude-sonnet",
        "provider": "anthropic",
    }
    assert metrics["sanremes.model_route.count"]["value"] == 1
    assert metrics["sanremes.tool_call.count"] == {
        "name": "sanremes.tool_call.count",
        "type": "counter",
        "dimensions": {
            "approval_outcome": "approved",
            "latency_bucket": "250ms_to_500ms",
    ...
    assert metrics["sanremes.tool_approval.count"] == {
        "name": "sanremes.tool_approval.count",
        "type": "counter",
        "dimensions": {
    ...
    assert metrics["sanremes.task_run.started"] == {
        "name": "sanremes.task_run.started",
        "type": "counter",
        "dimensions": {
            "entrypoint": "interactive",
            "execution_surface": "cli",
    ...
    terminal = metrics["sanremes.task_run.finished"]["dimensions"]
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    from sanremes_cli.observability.shared_metrics import SharedMetricsStore

    root = tmp_path / "sanremes-home" / "telemetry" / "shared_metrics"
    store = SharedMetricsStore(root / "metrics.sqlite3", root / "outbox")
    tomorrow = datetime.now(timezone.utc) + timedelta(days=1)
    monkeypatch.setattr(
        "sanremes_cli.observability.shared_metrics._utc_now",
        lambda: tomorrow,
    )
    ...
    assert by_metric["sanremes.client.active"][0]["dimensions"] == {}
    assert by_metric["sanremes.client.active"][0]["value"] == 1
    assert len(by_metric["sanremes.task_run.started"]) == 1
    assert by_metric["sanremes.task_run.started"][0]["value"] == 3
    assert len(by_metric["sanremes.model_route.count"]) == 1
    model_counter = by_metric["sanremes.model_route.count"][0]
    ...
    assert {
        ...
        for counter in by_metric["sanremes.tool_call.count"]
    ...
    assert len(by_metric["sanremes.tool_approval.count"]) == 1
    approval_counter = by_metric["sanremes.tool_approval.count"][0]
    ...
    terminal_by_outcome = {
        ...
        for counter in by_metric["sanremes.task_run.finished"]
    }
    ...
    for package in package_payloads:
        assert package["schema_version"] == "sanremes.shared_metrics.v2"
...
def test_real_binding_correlates_plugin_approval_denial_to_tool_metric(
    ...
    tool_metrics = [
        counter
        for counter in snapshot
        if counter["metric_name"] == "sanremes.tool_call.count"
    ]
    ...
    approval_metrics = [
        counter
        for counter in snapshot
        if counter["metric_name"] == "sanremes.tool_approval.count"
    ]
...
def test_core_runtime_is_fail_open_without_a_published_binding(monkeypatch, caplog):
    ...
    assert not relay_runtime.emit_mark("sanremes.probe", session_id="s1")
    assert "SanRemes Relay runtime initialization failed" in caplog.text
    relay_runtime._reset_for_tests()
...
def test_managed_config_cannot_override_shared_metrics_consent(
    ...
    from sanremes_cli import config, managed_scope
    ...
    monkeypatch.setenv("SANREMES_MANAGED_DIR", str(managed))
    config._LOAD_CONFIG_CACHE.clear()
    config._RAW_CONFIG_CACHE.clear()
    managed_scope.invalidate_managed_cache()

    token = set_sanremes_home_override(profile)
    try:
        assert (
...
def test_disabling_shared_metrics_stops_collection_and_shutdown_export(
    ...
    runtime.subscriber(
        SimpleNamespace(
            ...
            category="function",
            category_profile=None,
            name="sanremes.task_run",
            scope_category="start",
            metadata={
                "sanremes.metrics.schema_version": "sanremes.metrics.event.v1",
...
def test_same_request_id_is_isolated_between_tasks(direct_runtime):
    ...
    task_ends = [
        event[2]["output"]
        for event in direct_runtime.events
        if event[0] == "scope.pop" and event[1][1] == "sanremes.task_run"
    ]
...
def test_reused_task_id_starts_a_new_run_for_each_turn(direct_runtime):
    ...
    task_starts = [
        event
        for event in direct_runtime.events
        if event[0] == "scope.push" and event[1] == "sanremes.task_run"
    ]
    task_ends = [
        event
        for event in direct_runtime.events
...
def test_approval_without_tool_context_is_counted_as_unattributed(direct_runtime):
    ...
    [approval] = [
        event
        for event in direct_runtime.events
        if event[0] == "scope.event" and event[1] == "sanremes.tool_approval"
    ]
...
def test_failed_flush_keeps_daily_export_open_for_later_task(
    ...
    metrics = {metric["name"]: metric for metric in package["metrics"]}
    assert metrics["sanremes.task_run.started"]["value"] == 2
    assert metrics["sanremes.task_run.finished"]["value"] == 2
    assert flush_attempts == 2
    assert "SanRemes shared-metrics task flush failed" in caplog.text
...
def test_skill_lifecycle_flows_through_relay_to_a_privacy_safe_package(
    ...
    outbox = tmp_path / "sanremes-home" / "telemetry" / "shared_metrics" / "outbox"
    [package_path] = list(outbox.glob("*.json"))
    package = json.loads(package_path.read_text(encoding="utf-8"))
    skill_metrics = [
        metric
        for metric in package["metrics"]
        if metric["name"].startswith("sanremes.skill.")
    ]
    assert {metric["name"] for metric in skill_metrics} == {
        "sanremes.skill.lifecycle.count",
        "sanremes.skill.load.count",
    }
...
def test_skill_lifecycle_with_only_task_id_uses_unique_task_scope(direct_runtime):
    ...
    [mark] = [
        event
        for event in direct_runtime.events
        if event[0] == "scope.event" and event[1] == "sanremes.skill.lifecycle"
    ]
...
def test_late_skill_lifecycle_is_not_reemitted_at_the_root(direct_runtime):
    ...
    assert [
        event
        for event in direct_runtime.events
        if event[0] == "scope.event" and event[1] == "sanremes.skill.load"
    ] == []
