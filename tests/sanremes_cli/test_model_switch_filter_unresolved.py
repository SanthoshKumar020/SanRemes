...
from sanremes_cli.auth import is_runtime_provider_routable
...
def _rows_with_env(monkeypatch, env_name: str, provider: str) -> list[dict]:
    ...
    with (
        ...
        patch("sanremes_cli.models.cached_provider_model_ids", return_value=["model-a"]),
        patch("sanremes_cli.providers.SANREMES_OVERLAYS", {}),
    ):
