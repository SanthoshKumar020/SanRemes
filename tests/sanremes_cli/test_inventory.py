"""Behavior tests for sanremes_cli.inventory.
...
from sanremes_cli.inventory import (
    ConfigContext,
...
def test_explicit_only_keeps_anthropic_row_with_oauth_credentials():
    ...
    Claude Code (~/.claude/.credentials.json) and SanRemes' own device flow
    ...
    with (
        ...
        patch(
            "sanremes_cli.inventory._anthropic_oauth_credentials_present",
            return_value=True,
        ),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_anthropic_oauth_presence_accepts_pool_only_oauth_entry():
    ...
    """
    from sanremes_cli.inventory import _anthropic_oauth_credentials_present
    ...
    with (
        ...
        patch(
            "sanremes_cli.auth.read_credential_pool",
            return_value=[
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("sanremes_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from sanremes_cli import model_switch

    with patch("sanremes_cli.models.clear_provider_models_cache") as clear:
