...
@pytest.fixture
def client(_isolate_sanremes_home):
    try:
...
class TestApprovalsSaveBroadcast:
    ...
    def test_own_profile_named_default_broadcasts(self, client, broadcast_calls):
        ...
        assert resp.status_code == 200
        assert broadcast_calls, (
            "?profile=default is this process's own SANREMES_HOME; skipping "
    ...
    def test_other_profile_save_does_not_broadcast(self, client, broadcast_calls, monkeypatch, tmp_path):
        ...
        assert resp.status_code == 200
        assert not broadcast_calls, (
            "a profile-scoped save targets a different SANREMES_HOME than this "
...
class TestGatewayBroadcastHelper:
    def test_broadcast_session_info_emits_for_live_sessions(self, _isolate_sanremes_home, monkeypatch):
    ...
    def test_approvals_slash_mirror_broadcasts(self, _isolate_sanremes_home, monkeypatch):
