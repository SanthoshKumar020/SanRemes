"""Tests for ``sanremes gui`` desktop launcher wiring."""
...
from sanremes_cli import main as cli_main
...
def _isolate_xdg_data_home(tmp_path, monkeypatch):
    ...
    call live, and on a Linux dev box it wrote a ``sanremes.desktop`` pointing
...
def _stable_keychain_detection(monkeypatch):
    ...
    monkeypatch.delenv("SANREMES_DESKTOP_PASSWORD_STORE", raising=False)
...
def _make_packaged_executable(root: Path, monkeypatch) -> Path:
    ...
    if sys.platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "SanRemes.app" / "Contents" / "MacOS" / "SanRemes"
    elif sys.platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "SanRemes.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "sanremes"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("sanremes_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("sanremes_cli.main._desktop_build_needed", return_value=True), \
         patch("sanremes_cli.main._write_desktop_build_stamp"), \
         patch("sanremes_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("sanremes_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("sanremes_cli.main._register_linux_desktop_entry"), \
         patch("sanremes_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the SanRemes-managed
    Node ahead of that bare PATH or the install dies with ``node: not found``.
    """
    import os

    from sanremes_constants import iter_sanremes_node_dirs

    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    # A managed Node tree on disk so with_sanremes_node_path() actually prepends it.
    home = tmp_path / "sanremes-home"
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["sanremes"], 0)
    ...
    with patch("sanremes_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("sanremes_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("sanremes_cli.main._desktop_build_needed", return_value=True), \
         patch("sanremes_cli.main._write_desktop_build_stamp"), \
         patch("sanremes_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("sanremes_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("sanremes_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(skip_build=False))

    managed_dirs = [str(p) for p in iter_sanremes_node_dirs() if p.is_dir()]
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    SanRemes.app/Contents/MacOS/SanRemes in place. Re-downloading Electron can't
    ...
    with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("sanremes_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("sanremes_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("sanremes_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("sanremes_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("sanremes_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged SanRemes.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "SanRemes.app"
    _write_info_plist(app, "com.nousresearch.sanremes")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "SanRemes").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "SanRemes Helper.app"
    _write_info_plist(helper, "com.nousresearch.sanremes.helper")
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    SanRemes.app component, silently skipping every .node/.dylib/crashpad
...
def test_setup_tcc_identity_creates_cert_imports_trusts_and_configures(tmp_path, monkeypatch, capsys):
    ...
    identity = "SanRemes Local Signing"
    calls = []
    state = {"trusted": False}
    ...
    monkeypatch.setattr("sanremes_cli.config.set_config_value", lambda key, value: None)
    ...
    assert not list(tmp_path.glob("sanremes-tcc-*"))
...
def test_setup_tcc_identity_fails_when_trust_step_fails(tmp_path, monkeypatch, capsys):
    ...
    assert cli_main._desktop_macos_setup_tcc_identity("SanRemes Local Signing") is False
...
def test_setup_tcc_identity_skips_generation_when_already_valid(tmp_path, monkeypatch, capsys):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(list(cmd))
        if cmd[:4] == ["/usr/bin/security", "find-identity", "-v", "-p"]:
            return _fake_proc(cmd, stdout='  1) ABCD "SanRemes Local Signing"\n     1 valid identities found')
        return _fake_proc(cmd)
    ...
    assert cli_main._desktop_macos_setup_tcc_identity("SanRemes Local Signing") is True
...
def test_cmd_gui_setup_tcc_identity_exits_before_build(tmp_path, monkeypatch):
    """`sanremes desktop --setup-tcc-identity` calls the setup and exits 0/1
    without building or launching the app."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    with patch("sanremes_cli.main._desktop_macos_setup_tcc_identity", return_value=True) as mock_setup, \
         patch("sanremes_cli.main._run_npm_install_deterministic") as mock_install, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(setup_tcc_identity=True, identity="SanRemes Local Signing"))

    assert exc.value.code == 0
    mock_setup.assert_called_once_with("SanRemes Local Signing")
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    """`sanremes desktop` gives the app a launcher presence on Linux."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    packaged_exe = _make_packaged_executable(root, monkeypatch)

    registered: list[Path] = []
    monkeypatch.setattr("sanremes_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr(
        "sanremes_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: registered.append(project_root) or (tmp_path / "sanremes.desktop"),
    )

    launch_ok = subprocess.CompletedProcess([str(packaged_exe)], 0)

    with patch("sanremes_cli.main._desktop_build_needed", return_value=False), \
         patch("sanremes_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("sanremes_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("sanremes_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("sanremes_cli.linux_desktop_entry.install_desktop_entry", boom)
    ...
    with patch("sanremes_cli.main._desktop_build_needed", return_value=False), \
         patch("sanremes_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("sanremes_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("sanremes_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("sanremes_cli.linux_desktop_entry.install_desktop_entry", fail)
...
def test_desktop_launch_options_normalizes_password_store(raw, expected):
    ...
    with patch("sanremes_cli.config.load_config", return_value=cfg):
...
def test_desktop_launch_options_ozone_hint_defaults_auto():
    with patch("sanremes_cli.config.load_config", return_value={}):
...
def test_gui_bridges_ozone_hint_to_launch_env(tmp_path, monkeypatch):
    ...
    with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("sanremes_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("sanremes_cli.main._desktop_build_needed", return_value=True), \
         patch("sanremes_cli.main._write_desktop_build_stamp"), \
         patch("sanremes_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("sanremes_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("sanremes_cli.config.load_config", return_value=cfg), \
         patch("sanremes_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("sanremes_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
    ...
    with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("sanremes_cli.config.load_config", return_value=cfg), \
         patch("sanremes_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("sanremes_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run2, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def _clear_keychain_env(monkeypatch):
    for var in (
        ...
        "SANREMES_DESKTOP_PASSWORD_STORE",
    ):
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("sanremes_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert cli_main._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("sanremes_cli.main.subprocess.run", return_value=ping_fail):
        assert cli_main._detect_linux_password_store() is None
    with patch("sanremes_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert cli_main._detect_linux_password_store() is None
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("sanremes_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("sanremes_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("sanremes_cli.config.load_config", return_value={}), \
         patch("sanremes_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("sanremes_cli.main._detect_linux_password_store", return_value="gnome-libsecret"), \
         patch("sanremes_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SANREMES_DESKTOP_PASSWORD_STORE"] == "gnome-libsecret"
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("sanremes_cli.config.load_config", return_value={}), \
         patch("sanremes_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("sanremes_cli.main._detect_linux_password_store", return_value="kwallet6"), \
         patch("sanremes_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
    ...
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SANREMES_DESKTOP_PASSWORD_STORE"] == "kwallet6"
...
def test_gui_config_password_store_skips_detection(tmp_path, monkeypatch):
    ...
    with patch("sanremes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("sanremes_cli.config.load_config", return_value=cfg), \
         patch("sanremes_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("sanremes_cli.main._detect_linux_password_store") as mock_detect, \
         patch("sanremes_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_explicit_password_store_env_wins_over_config_and_detection(tmp_path, monkeypatch):
    _clear_keychain_env(monkeypatch)
    monkeypatch.setenv("SANREMES_DESKTOP_PASSWORD_STORE", "basic")
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SANREMES_DESKTOP_PASSWORD_STORE"] == "basic"
...
def test_gui_password_store_bridge_is_linux_only(tmp_path, monkeypatch):
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert "SANREMES_DESKTOP_PASSWORD_STORE" not in launch_env
