"""Tests for provider-aware `/model` validation in sanremes_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("sanremes_cli.models.fetch_api_models", return_value=api_models), \
         patch("sanremes_cli.models.probe_api_models", return_value=probe_payload):
...
class TestFetchApiModels:
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("sanremes_cli.models._urlopen_model_catalog_request", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("sanremes_cli.models._urlopen_model_catalog_request", return_value=_Resp()) as mock_urlopen:
...
class TestNormalizeOpencodeBaseUrl:
    ...
    def test_strips_v1_for_anthropic_messages(self):
        from sanremes_cli.models import normalize_opencode_base_url
...
class TestValidateApiFallback:
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("sanremes_cli.models._urlopen_model_catalog_request", return_value=mock_resp):
    ...
    def test_validate_lmstudio_distinguishes_auth_failure(self):
        ...
        with patch("sanremes_cli.models._urlopen_model_catalog_request", side_effect=http_error):
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("sanremes_cli.models.provider_model_ids", return_value=codex_models):
...
class TestValidateCodex900kVariants:
    """`-900k` is a SanRemes picker convention: valid variants come from the
    ...
    def test_catalog_listed_variant_accepted(self):
        with patch("sanremes_cli.models.provider_model_ids", return_value=self._CATALOG):
    ...
    def test_valid_variant_missing_from_catalog_still_accepted(self):
        ...
        with patch("sanremes_cli.models.provider_model_ids", return_value=["gpt-5.6-sol"]):
            result = validate_requested_model("gpt-5.6-sol-900k", "openai-codex")
        assert result["accepted"] is True
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a SanRemes User-Agent.
    ...
    def test_probe_sends_sanremes_user_agent(self):
        ...
        assert ua.startswith("sanremes-cli/"), (
            f"User-Agent must advertise sanremes-cli, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("sanremes-cli/")
...
class TestValidateOpenRouterVariantSuffixes:
    ...
    def test_static_catalog_fallback_accepts_variant(self):
        ...
        with patch("sanremes_cli.models.fetch_api_models", return_value=None), \
             patch(
                 "sanremes_cli.models.provider_model_ids",
