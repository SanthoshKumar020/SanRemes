...
from sanremes_cli.skills_hub import do_check, do_install, do_list, do_update, handle_skills_slash
...
def _capture_update(monkeypatch, results) -> tuple[str, list[tuple[str, str, bool]]]:
    import tools.skills_hub as hub
    import sanremes_cli.skills_hub as cli_hub
...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    """`sanremes skills list` reads the active profile's config via
    SANREMES_HOME (swapped by -p), so it must NOT pass a platform arg to
    ``get_disabled_skill_names`` — otherwise per-platform overrides
    would silently leak in from SANREMES_PLATFORM env."""
...
def test_resolve_does_not_pair_catalog_meta_with_foreign_same_name_bundle():
    ...
    """
    from sanremes_cli.skills_hub import _resolve_source_meta_and_bundle
...
# ---------------------------------------------------------------------------
# Regression: full identifier must be recoverable from `sanremes skills search`
...
def test_do_search_json_flag_emits_full_identifiers(capsys):
    ...
    from sanremes_cli.skills_hub import do_search
