"""
Tests for --yes / -y flag in `sanremes skills uninstall` CLI subcommand.
...
def _run_uninstall_cli(monkeypatch, argv):
    ...
    monkeypatch.setattr("sanremes_cli.skills_hub.do_uninstall", fake_do_uninstall)
...
def test_cli_skills_uninstall_yes_sets_skip_confirm(monkeypatch):
    ...
    captured = _run_uninstall_cli(
        monkeypatch,
        ["sanremes", "skills", "uninstall", "test-skill", "--yes"],
    )
...
def test_cli_skills_uninstall_y_alias_sets_skip_confirm(monkeypatch):
    ...
    captured = _run_uninstall_cli(
        monkeypatch,
        ["sanremes", "skills", "uninstall", "test-skill", "-y"],
    )
...
def test_cli_skills_uninstall_no_flags_keeps_prompt(monkeypatch):
    ...
    captured = _run_uninstall_cli(
        monkeypatch,
        ["sanremes", "skills", "uninstall", "test-skill"],
    )
