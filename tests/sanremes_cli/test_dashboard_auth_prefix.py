...
prefix (e.g. ``mission-control.tilos.com/sanremes/*`` -> local Caddy ->
:9119), injecting ``X-Forwarded-Prefix: /sanremes`` on every request.

The dashboard already honours this for the SPA bundle (rewriting asset
URLs and the bootstrap ``__SANREMES_BASE_PATH__``). The OAuth gate must
honour it too:

  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/sanremes/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/sanremes/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/sanremes`` when behind a prefix so they
...
from sanremes_cli.dashboard_auth import clear_providers, register_provider
from sanremes_cli.dashboard_auth import prefix as prefix_mod
from tests.sanremes_cli.conftest_dashboard_auth import StubAuthProvider
...
class TestForwardedPrefixNormalisation:
    def test_home_assistant_ingress_prefix_with_subpath_is_accepted(
        ...
        the SPA receives the correct __SANREMES_BASE_PATH__ and asset prefix."""
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        ...
        assert r.headers["location"].startswith("/sanremes/auth/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/sanremes/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/sanremes/auth/callback`` — the
        ...
        assert parsed.path == "/sanremes/auth/callback", (
...
# ---------------------------------------------------------------------------
# SANREMES_DASHBOARD_PUBLIC_URL / dashboard.public_url override
# ---------------------------------------------------------------------------


class TestPublicUrlOverride:
    """``dashboard.public_url`` (env override:
    ``SANREMES_DASHBOARD_PUBLIC_URL``) lets an operator force the absolute
    ...
    def patch_config(self, monkeypatch):
        """Replace ``sanremes_cli.config.load_config`` with a stub
        ...
        def _set(public_url) -> None:
            ...
            monkeypatch.setattr(
                "sanremes_cli.config.load_config", lambda: cfg
            )
    ...
    def test_env_overrides_config_public_url(
        ...
        monkeypatch.setenv(
            "SANREMES_DASHBOARD_PUBLIC_URL", "https://from-env.example",
        )
    ...
    def test_malformed_public_url_falls_through_to_reconstruction(
        ...
        for bad in [
            ...
            monkeypatch.setenv("SANREMES_DASHBOARD_PUBLIC_URL", bad)
            redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        of "I set SANREMES_DASHBOARD_PUBLIC_URL but the callback is still
        ...
        patch_config(None)
        monkeypatch.setenv("SANREMES_DASHBOARD_PUBLIC_URL", "sanremes.domain.com")
        ...
        assert any(
            "SANREMES_DASHBOARD_PUBLIC_URL" in m
            and "sanremes.domain.com" in m
            and "scheme" in m
            for m in warnings
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-sanremes_session_pkce=")
        ]
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from sanremes_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("sanremes_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/sanremes, so the next request under
        ...
        (``/sanremes/auth/login``, ``/sanremes/auth/callback``). A cookie
        set with ``Path=/sanremes`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/sanremes/auth/callback``. To
        ...
        pkce_set = next(
            ...
            if "sanremes_session_pkce" in c
        )
        # Parse "__Secure-sanremes_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-sanremes_session_pkce=value"
        state = r1.headers["location"].split("state=")[1]

        # Round-trip the cookie by hand because TestClient's jar won't
        # automatically send a Path=/sanremes cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/sanremes",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-sanremes_session_at=")
        ]
        assert at_cookies, (
            f"session_at missing __Secure- prefix: {cookies!r}"
        )
        assert "Path=/sanremes" in at_cookies[0]
