"""Tests for the auxiliary-model configuration UI in ``sanremes model``.
...
from sanremes_cli.config import DEFAULT_CONFIG, load_config
...
def test_save_aux_choice_persists_to_config_yaml(tmp_path, monkeypatch):
    ...
    (tmp_path / ".sanremes").mkdir(exist_ok=True)
...
def test_reset_aux_clears_delegation_routing_preserves_settings(tmp_path, monkeypatch):
    ...
    from sanremes_cli.config import load_config as _lc, save_config
...
def test_delegation_cfg_as_task_projection():
    ...
    shaped = _delegation_cfg_as_task(
        {"delegation": {"provider": "nous", "model": "SanRemes-4.5"}}
    )
    assert _format_aux_current(shaped) == "nous · SanRemes-4.5"
