...
def test_hosted_auth_start_returns_public_authorization_url(monkeypatch):
    ...
    with patch(
        "sanremes_cli.dashboard_auth.prefix.resolve_public_url",
...
def test_hosted_callback_bypasses_gated_cookie_auth(monkeypatch):
    ...
    flow = DashboardOAuthFlow(
        ...
        server_name="reports",
        profile=None,
        sanremes_home="/tmp/sanremes-test",
...
def test_hosted_auth_allows_same_server_name_in_different_profiles(tmp_path, monkeypatch):
    ...
    existing = DashboardOAuthFlow(
        ...
        server_name="reports",
        profile=None,
        sanremes_home=str(tmp_path / "default"),
    ...
    with patch("sanremes_cli.mcp_config._get_mcp_servers", return_value={"reports": {"url": "https://mcp.example"}}), \
