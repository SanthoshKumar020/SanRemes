...
Regression for #86794: a background/quiet `sanremes chat -c "<title>" -q "..."`
...
def isolated_home(tmp_path, monkeypatch):
    """Point SANREMES_HOME at a temp dir so session creation stays isolated."""
...
def _create_titled_session(title):
    from sanremes_cli.main import _create_titled_session as fn
...
class TestChatCFailLoudlyOnStderr:
    ...
    def test_missing_session_fails_on_stderr(self, isolated_home, monkeypatch):
        ...
        assert not stderr_lines[0].startswith("Use 'sanremes sessions list'")
