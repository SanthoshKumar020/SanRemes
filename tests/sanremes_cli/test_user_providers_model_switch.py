...
import pytest
from sanremes_cli.model_switch import list_authenticated_providers, switch_model
from sanremes_cli import runtime_provider as rp
...
def test_list_authenticated_providers_enumerates_dict_format_models(monkeypatch):
    ...
    (canonical SanRemes write format) should surface every key in the picker.

    Regression: the ``providers:`` dict path previously only accepted
    list-format ``models:`` and silently dropped dict-format entries,
    even though SanRemes's own writer and downstream readers use dict format.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("sanremes_cli.providers.SANREMES_OVERLAYS", {})
...
def test_list_authenticated_providers_accepts_base_url_and_singular_model(monkeypatch):
    """providers: dict entries written in canonical SanRemes shape
    ...
    ``default_model``, so new-shape entries written by SanRemes's own writer
...
def test_list_authenticated_providers_no_duplicate_labels_across_schemas(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.models.fetch_api_models", lambda *a, **k: None)
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("sanremes_cli.model_switch.resolve_alias", return_value=None), \
         patch("sanremes_cli.model_switch.list_provider_models", return_value=[]), \
         patch("sanremes_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("sanremes_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         patch("sanremes_cli.models.detect_provider_for_model", return_value=None), \
         patch("sanremes_cli.model_switch.get_model_info", return_value=None), \
         patch("sanremes_cli.model_switch.get_model_capabilities", return_value=None), \
         patch("sanremes_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.models.fetch_api_models", _fake_fetch)
...
def test_current_custom_model_is_surfaced_in_builtin_provider_row(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.models.cached_provider_model_ids",
