...
reports the state and prints the one-switch setup; `sanremes setup` surfaces
...
from sanremes_cli.setup import _print_macos_fda_tip
...
class TestSetupFdaTip:
    ...
    def test_tip_printed_when_denied(self, monkeypatch, tmp_path):
        ...
        assert "Full Disk Access" in out
        assert "Privacy_AllFiles" in out
        assert "survives every SanRemes update" in out
