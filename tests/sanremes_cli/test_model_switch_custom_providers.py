...
The terminal `sanremes model` flow already exposes `custom_providers`, but the
...
import sanremes_cli.providers as providers_mod
import pytest
import yaml
...
from sanremes_cli.providers import resolve_provider_full
...
def _disable_live_custom_provider_model_probe(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.models.fetch_api_models", lambda *_a, **_kw: None)
    monkeypatch.setattr(
        "sanremes_cli.models.cached_provider_model_ids", lambda *_a, **_kw: []
    )
    monkeypatch.setattr(
        "sanremes_cli.models.provider_model_ids", lambda *_a, **_kw: []
    )
    monkeypatch.setattr(
        "sanremes_cli.models.fetch_ollama_local_models", lambda *_a, **_kw: None
    )


def test_picker_native_probe_failure_falls_back_to_openai_catalog(monkeypatch):
    monkeypatch.setattr(
        "sanremes_cli.models.should_use_ollama_native_catalog", lambda *a, **k: True
    )
    monkeypatch.setattr(
        "sanremes_cli.models._get_ollama_native_headers", lambda *a, **k: {}
    )
    monkeypatch.setattr(
        "sanremes_cli.models.fetch_ollama_local_models", lambda *a, **k: None
    )
    monkeypatch.setattr(
        "sanremes_cli.models.fetch_api_models", lambda *a, **k: ["fallback-model"]
    )
...
def test_picker_generic_discovery_preserves_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.models.should_use_ollama_native_catalog", lambda *a, **k: False
    )
    monkeypatch.setattr("sanremes_cli.models.cached_fetch_api_models", cached)
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "SANREMES_OVERLAYS", {})
    monkeypatch.setattr("sanremes_cli.models.fetch_api_models", lambda *a, **k: [])
...
def test_list_authenticated_providers_can_skip_custom_provider_live_probe(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.models.fetch_api_models", fetch)
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("sanremes_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("sanremes_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_switch_model_does_not_send_ollama_headers_to_unrelated_custom_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.models._get_ollama_request_headers",
    ...
    monkeypatch.setattr("sanremes_cli.models.validate_requested_model", fake_validation)
    monkeypatch.setattr("sanremes_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("sanremes_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_picker_selection_resolves_named_custom_provider_model_id(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr(
        "sanremes_cli.model_switch.get_model_capabilities",
        lambda *a, **k: None,
    )
...
def test_custom_provider_no_key_singular_model_still_probes_live_models(monkeypatch):
    ...
    probed so /model matches the terminal ``sanremes model`` flow. Ollama-native
    ...
    monkeypatch.setattr("sanremes_cli.models.fetch_api_models", fake_fetch_api_models)
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: SanRemes's own writer stores configured models as a dict
...
def test_picker_endpoint_authorization_overrides_inferred_bearer(monkeypatch):
    from sanremes_cli.model_switch import _fetch_picker_live_models
    ...
    monkeypatch.setattr("sanremes_cli.models.should_use_ollama_native_catalog", lambda *a, **k: True)
    monkeypatch.setattr("sanremes_cli.models.fetch_ollama_local_models", fake_native)
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.models.fetch_lmstudio_models", _fake_fetch)
...
def test_custom_providers_uses_live_models_for_multi_model_endpoint(monkeypatch):
    ...
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("sanremes_cli.providers.SANREMES_OVERLAYS", {})
...
def test_resolve_custom_provider_passes_key_env():
    ...
    """
    from sanremes_cli.providers import resolve_custom_provider
...
def test_discovered_models_auto_saved_to_cache(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.models.fetch_api_models", fake_fetch_api_models)
    monkeypatch.setattr(
        "sanremes_cli.model_switch._save_discovered_models_to_config",
...
def test_save_discovered_models_preserves_dict_form(monkeypatch):
    ...
    from sanremes_cli.model_switch import _save_discovered_models_to_config
    ...
    monkeypatch.setattr("sanremes_cli.config.save_config", fake_save)
    monkeypatch.setattr(
...
def test_model_flow_named_custom_persists_discovered_models(monkeypatch):
    """The ``sanremes model`` named-custom-provider flow persists the discovered
    ...
    ``sanremes model`` collapsed to the single ``model:`` default everywhere but
    ...
    monkeypatch.setattr(
        "sanremes_cli.curses_ui.curses_radiolist", lambda *a, **k: 0
    )
    # No-op downstream writes so the test never touches a real config.
    monkeypatch.setattr("sanremes_cli.main._save_custom_provider", lambda *a, **k: None)
    monkeypatch.setattr("sanremes_cli.auth._save_model_choice", lambda *a, **k: None)
    monkeypatch.setattr("sanremes_cli.auth.deactivate_provider", lambda *a, **k: None)
    monkeypatch.setattr(
        "sanremes_cli.config.load_config",
    ...
    from sanremes_cli.model_setup_flows import _model_flow_named_custom
...
def test_custom_provider_context_length_models_dict_still_probes(monkeypatch):
    ...
    ``sanremes model`` writes ``models: {default: {context_length: N}}`` for
...
def _seed_custom_model_cache(monkeypatch, models, *, age_seconds=10):
    ...
    import sanremes_cli.models as models_mod
...
def test_keyless_endpoint_with_saved_catalog_still_reads_cache(monkeypatch):
    """A keyless local server must not be pinned by SanRemes' own auto-save.
...
def test_keyless_endpoint_with_saved_catalog_is_still_not_probed(monkeypatch):
    ...
    The no-key + declared-catalog combination exists to keep SanRemes from
...
def test_legacy_sentinel_catalog_still_resolves_and_migrates(tmp_path, monkeypatch):
    ...
    Pre-fix SanRemes wrote ``__discovered_model_catalog__: true`` (and
