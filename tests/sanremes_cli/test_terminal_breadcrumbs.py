"""Tests for sanremes_cli/terminal_breadcrumbs.py — per-terminal ``sanremes -c``.

Covers terminal id derivation (tty vs env vars vs none), breadcrumb
write/read roundtrip under a temp SANREMES_HOME, stale-session fallback
...
from sanremes_cli import terminal_breadcrumbs as tb
...
def test_breadcrumb_roundtrip(sanremes_home, monkeypatch, no_terminal_env):
    ...
    files = list((sanremes_home / "terminal-sessions").iterdir())
    assert [f.name for f in files] == ["tty-dev-pts-7"]


def test_write_skipped_without_terminal_identity(sanremes_home, monkeypatch, no_terminal_env):
    _fake_no_tty(monkeypatch)
    tb.write_breadcrumb("20260815_120000_abc123")
    assert not (sanremes_home / "terminal-sessions").exists()
    assert tb.read_breadcrumb() is None


def test_two_terminals_do_not_clobber(sanremes_home, monkeypatch, no_terminal_env):
...
def test_stale_breadcrumb_ignored_and_pruned(sanremes_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/1")
    directory = sanremes_home / "terminal-sessions"
...
def test_corrupt_breadcrumb_returns_none(sanremes_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/1")
    directory = sanremes_home / "terminal-sessions"
    directory.mkdir(parents=True)
...
def test_resolve_picks_this_terminals_session(sanremes_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(sanremes_home, "20260815_100000_aaaaaa")
    _make_session(sanremes_home, "20260815_110000_bbbbbb")  # newer, other terminal
    tb.write_breadcrumb("20260815_100000_aaaaaa")
    assert tb.resolve_breadcrumb_session() == "20260815_100000_aaaaaa"


def test_resolve_falls_back_when_session_deleted(sanremes_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(sanremes_home, "20260815_110000_bbbbbb")
    tb.write_breadcrumb("20260815_100000_deleted")  # never existed / deleted
    assert tb.resolve_breadcrumb_session() is None


def test_resolve_projects_through_compression_chain(sanremes_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(sanremes_home, "20260815_100000_parent")
...
def test_config_gate_off_disables_writes_and_resolution(
    sanremes_home, monkeypatch, no_terminal_env
):
