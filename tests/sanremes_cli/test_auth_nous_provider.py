...
from sanremes_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        import ssl
        from sanremes_cli.auth import _resolve_verify
...
def test_resolve_nous_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    ...
    payload = json.loads((sanremes_home / "auth.json").read_text())
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
    monkeypatch.setenv("SANREMES_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.DEBUG, logger="sanremes_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from sanremes_cli.auth import get_nous_auth_status
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `sanremes model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return sanremes_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import sanremes_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from sanremes_cli.auth import PROVIDER_REGISTRY, _login_nous

        sanremes_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `sanremes auth add nous`
    ...
    """
    from sanremes_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    """
    from sanremes_cli.auth import persist_nous_credentials
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", SanRemes must surface an
    ...
    re-authenticate" string led users to report this as a SanRemes persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from sanremes_cli.auth import _refresh_access_token
    ...
    assert "sanremes auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect SANREMES_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("SANREMES_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without SANREMES_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.sanremes/shared/`` across CI runs.
    """
    from sanremes_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("SANREMES_SHARED_AUTH_DIR", raising=False)
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `sanremes auth add nous
    ...
    sanremes_home = tmp_path / "sanremes"
    sanremes_home.mkdir(parents=True, exist_ok=True)
    (sanremes_home / "auth.json").write_text(
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    """
    from sanremes_cli import auth as auth_mod
    ...
    assert result["client_id"] == "sanremes-cli"
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from sanremes_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL
...
class TestNousDeviceAuthTimeoutMessage:
    def test_timeout_message_mentions_captcha_login_and_retry(self):
        from sanremes_cli.auth import _nous_device_auth_timeout_message

        msg = _nous_device_auth_timeout_message("https://portal.nousresearch.com")
        assert "CAPTCHA" in msg
        assert "sanremes portal" in msg
