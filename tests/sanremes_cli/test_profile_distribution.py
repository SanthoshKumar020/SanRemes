"""Tests for sanremes_cli.profile_distribution — git-based profile installs.
...
from sanremes_cli.profile_distribution import (
    DEFAULT_DIST_OWNED,
    DistributionError,
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/sanremes_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_sanremes_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires SanRemes"):
                check_sanremes_requires(spec, cur)
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from sanremes_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_sanremes_requires(self, profile_env, monkeypatch):
        # Pin current SanRemes version to something well below the requirement
        import sanremes_cli
        monkeypatch.setattr(sanremes_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            sanremes_requires=">=99.0.0",
        )
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from sanremes_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from sanremes_cli.profiles import create_profile, list_profiles, get_profile_dir
