...
from sanremes_cli.image_provenance import read_image_provenance
from sanremes_cli.update_contract import (
    UpdateRefusal,
...
def _valid_marker(tmp_path: Path) -> Path:
    ...
    marker.write_text(json.dumps({
        ...
        "manager": "docker",
        "image": "nousresearch/sanremes-agent",
        "version": "1.0.0",
        "revision": "a" * 40,
    }))
    return marker
...
def test_admission_marker_refuses_even_on_git_checkout(tmp_path, monkeypatch):
    ...
    — the marker wins."""
    import sanremes_cli.image_provenance as ip

    monkeypatch.setattr(ip, "IMAGE_PROVENANCE_PATH", _valid_marker(tmp_path))
    monkeypatch.setattr(
        "sanremes_cli.config.detect_install_method", lambda *a, **k: "git"
    )
...
def test_admission_no_marker_falls_back_to_heuristics(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.config.detect_install_method", lambda *a, **k: "docker"
    )
    refusal = evaluate_update_admission(tmp_path)
...
def test_admission_apt_and_nix_refuse(tmp_path, monkeypatch):
    ...
    for method, code in (("apt", "apt"), ("nix", "nix")):
        ...
        monkeypatch.setattr("sanremes_cli.config.detect_install_method", _detect)
        refusal = evaluate_update_admission(tmp_path)
...
def test_refusal_receipt_written_as_refused(tmp_path, monkeypatch):
    ...
    record_refusal_receipt(
        UpdateRefusal(
            code="image-marker",
            message="msg",
            update_command="docker pull nousresearch/sanremes-agent:latest",
        )
    )
