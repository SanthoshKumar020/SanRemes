...
def _write_ledger(entries: list[dict]) -> None:
    from sanremes_cli import process_identity as pid_mod
...
def test_holder_scan_fallback_respects_token_classifier(sleeper, monkeypatch, tmp_path):
    ...
    serve_like = sleeper("-m", "sanremes_cli.main", "serve")
    # Lookalike from the #90778 class — must NOT confer ownership.
    kanban_like = sleeper("-m", "sanremes_cli.main", "kanban", "--preserve-cache")
    ...
    monkeypatch.setattr(
        "sanremes_cli.main._detect_venv_python_processes", fake_holders
    )
    ...
    serve_like.wait()
    monkeypatch.setattr(
        "sanremes_cli.main._detect_venv_python_processes",
