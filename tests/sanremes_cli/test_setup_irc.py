"""Tests for IRC gateway configuration via `sanremes setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh SanRemes install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new SanRemes install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#sanremes")
            monkeypatch.setenv("IRC_NICKNAME", "sanremes-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `sanremes setup gateway` experience with IRC present."""
