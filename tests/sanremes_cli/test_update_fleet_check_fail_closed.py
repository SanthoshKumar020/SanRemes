...
``sanremes update`` exited 0 with zero rows — even when a gateway was
...
from sanremes_cli.main import _fleet_probe_expected_runtimes
...
class TestEmptySnapshotFailClosed:
    ...
    def test_incomplete_when_restart_phase_touched_gateways(self):
        ...
        assert (
            _fleet_probe_expected_runtimes(None, [], None, ["sanremes-gateway"], set())
            is True
        )
