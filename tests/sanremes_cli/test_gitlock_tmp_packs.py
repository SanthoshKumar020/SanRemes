...
from sanremes_cli.gitlock import (
...
def test_removes_old_tmp_pack_debris(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.gitlock._git_proc_running", lambda: False)
...
def test_skips_sweep_while_git_is_running(tmp_path, monkeypatch):
    repo = _mkrepo(tmp_path)
    monkeypatch.setattr("sanremes_cli.gitlock._git_proc_running", lambda: True)
    pack = repo / ".git" / "objects" / "pack"
    p = pack / "tmp_pack_old"
    p.write_bytes(b"x")
    _age(p, STALE_TMP_PACK_MIN_AGE_SECONDS + 60)
