"""Tests for sanremes_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from sanremes_cli import relaunch as relaunch_mod


class TestResolveSanRemesBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/sanremes"
        ...
        assert relaunch_mod.resolve_sanremes_bin() == fake

    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "sanremes"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real 'sanremes' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_sanremes_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/sanremes" if name == "sanremes" else None
        )
        assert relaunch_mod.resolve_sanremes_bin() == "/usr/bin/sanremes"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_sanremes_bin", lambda: "/usr/bin/sanremes")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/sanremes"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/sanremes", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/sanremes", ["/usr/bin/sanremes", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        sanremes).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_sanremes_bin", lambda: r"C:\Users\test\sanremes.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\sanremes.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\sanremes.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_sanremes_bin", lambda: r"C:\sanremes.exe")
...
class TestResolveSanRemesBinWindowsPyGuard:
    """On Windows, resolve_sanremes_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a sanremes.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\sanremes.exe" if name == "sanremes" else None,
        )

        bin_path = relaunch_mod.resolve_sanremes_bin()
        # Must NOT be the .py — must be the sanremes.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\sanremes.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "sanremes"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_sanremes_bin() == str(script)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_sanremes_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND sanremes.exe
        ...
        assert relaunch_mod.resolve_sanremes_bin() is None
