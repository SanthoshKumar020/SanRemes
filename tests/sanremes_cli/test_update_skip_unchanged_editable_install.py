"""``sanremes update`` skips the editable reinstall when the pull can't affect it.
...
that rewrite is the only reason the running ``sanremes.exe`` gets quarantined,
...
from sanremes_cli.update_cmd import _editable_install_is_current
