"""Tests for ``sanremes migrate xai`` — apply path with ruamel round-trip."""
...
from sanremes_cli.xai_retirement import (
    RetirementIssue,
    apply_migration,
...
class TestCrashDurability:
    ...
    def test_symlinked_config_is_replaced_in_place(self, tmp_path: Path):
        ...
        real_dir.mkdir()
        real = real_dir / "sanremes-config.yaml"
        real.write_text(
            "principal:\n"
            "  provider: xai\n"
            "  model: grok-3\n",
    ...
    def test_comments_survive_the_atomic_write(self, trap_config: Path):
        ...
        text = trap_config.read_text(encoding="utf-8")
        assert "# SanRemes config (sample)" in text
