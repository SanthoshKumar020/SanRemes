...
Covers:
  ...
  - Namespace spoofing (sanremes:, foreign, already-colon'd) is rejected
  ...
  - `sanremes plugins show` output includes emits/listens
"""
...
@pytest.mark.parametrize(
    "bad_event",
    [
        "sanremes:x",   # reserved core prefix
...
def test_emit_rejects_namespaced_names(bad_event):
    ...
    for name in (bad_event, f"b:{bad_event}", "sanremes:x", "a:x", "b:x"):
...
def test_subscribe_is_unrestricted():
    """Any plugin may subscribe to any event, including sanremes: and foreign."""
    manager = _fresh_manager()
    ctx_a = _make_ctx(manager, "plugin_a", key="a")
    got = []
    # None of these raise — only emit is namespace-gated.
    ctx_a.subscribe("sanremes:core_event", lambda **p: got.append("sanremes"))
    ctx_a.subscribe("b:ping", lambda **p: got.append("b"))
    assert "sanremes:core_event" in manager._subscriptions
    assert "b:ping" in manager._subscriptions
