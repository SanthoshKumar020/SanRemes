"""Tests for --ignore-user-config and --ignore-rules flags on `sanremes chat`.
...
files. In SanRemes the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.sanremes/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → SanRemesCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("SANREMES_IGNORE_USER_CONFIG", "SANREMES_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``SANREMES_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<sanremes_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With SANREMES_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._sanremes_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("SANREMES_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``SanRemesCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting SANREMES_IGNORE_RULES=1 flips SanRemesCLI.ignore_rules True."""
        monkeypatch.setenv("SANREMES_IGNORE_RULES", "1")

        # Import SanRemesCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of SanRemesCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.SanRemesCLI)
        # Replicate the exact logic from cli.py SanRemesCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("SANREMES_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``sanremes_cli/main.py`` must set
    ...
    def _simulate_cmd_chat_env_setup(self, args):
        ...
        if getattr(args, "ignore_user_config", False):
            os.environ["SANREMES_IGNORE_USER_CONFIG"] = "1"
        if getattr(args, "ignore_rules", False):
            os.environ["SANREMES_IGNORE_RULES"] = "1"

    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("SANREMES_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("SANREMES_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("SANREMES_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("SANREMES_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "SANREMES_IGNORE_USER_CONFIG" not in os.environ
        assert "SANREMES_IGNORE_RULES" not in os.environ
