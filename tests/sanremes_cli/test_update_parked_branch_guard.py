"""Regression tests for the parked-branch guard in ``sanremes update``.
...
memory-limit``, days behind main) left there by earlier tooling. ``sanremes
...
def _no_config(monkeypatch):
    ...
    import sanremes_cli.config as sanremes_config

    monkeypatch.setattr(sanremes_config, "load_config", lambda: {})
...
def test_skip_warning_names_branch_behind_count_and_commands(repo_pair, capsys):
    ...
    assert f"git -C {repo_pair} checkout main && sanremes update" in out
...
def test_print_update_completion_carries_branch_and_sha(
    ...
    monkeypatch.setattr(sanremes_main, "PROJECT_ROOT", repo_pair)
...
def _patch_update_flow(monkeypatch, repo, run_real_git=True):
    ...
    """
    monkeypatch.setattr(sanremes_main, "PROJECT_ROOT", repo)
    ...
    monkeypatch.setattr(sanremes_main, "_discard_lockfile_churn", lambda *a, **k: None)
    ...
    monkeypatch.setattr(sanremes_main, "_record_bytecode_fingerprint", lambda *a, **k: None)
    monkeypatch.setattr(sanremes_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(sanremes_main, "_pause_windows_gateways_for_update", lambda: None)
    monkeypatch.setattr(
        sanremes_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(sanremes_main, "_capture_active_lazy_features", lambda: [])
    monkeypatch.setattr(sanremes_main, "_capture_active_tool_dependencies", lambda: [])
