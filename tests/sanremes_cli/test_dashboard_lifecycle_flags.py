"""Tests for ``sanremes dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``sanremes update``
...
from sanremes_cli.main import cmd_dashboard
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("sanremes_cli.main._scan_dashboard_processes", return_value=[]), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "No sanremes dashboard or serve processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        # couldn't see (#81564).
        processes = [
            (12345, "sanremes dashboard --port 9119"),
            (12346, "python -m sanremes_cli.main dashboard --host 0.0.0.0 --port 9120"),
            (12347, "sanremes serve --host 100.94.65.93 --port 9119"),
        ]
        with patch("sanremes_cli.main._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("sanremes_cli.main._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        # Status is informational — always exits 0.
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "3 sanremes dashboard/serve process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        # for the post-`sanremes update` path).
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("sanremes_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("sanremes_cli.main._kill_stale_dashboard_processes"), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``sanremes dashboard --stop`` must not end up ALSO starting
    a new server."""
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("sanremes_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"sanremes_cli.web_server": fake_ws}), \
...
class TestArgparseWiring:
    ...
    ``sanremes dashboard --stop`` / ``--status`` actually parse."""

    def test_flags_are_registered(self):
        from sanremes_cli.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("sanremes_cli.main")
