...
def test_generic_malformed_open_does_not_attempt_schema_surgery(
    ...
    monkeypatch.setattr(sanremes_state, "apply_wal_with_fallback", _generic_corruption)
    monkeypatch.setattr(
        sanremes_state,
        "repair_state_db_schema",
...
def test_auto_heal_attempted_once_per_process(tmp_path, monkeypatch):
    ...
    _build_healthy_db(db_path)
    _corrupt_duplicate_fts(db_path)
    monkeypatch.setattr(sanremes_state, "_repair_attempted_paths", set())

    calls = {"n": 0}
    real_repair = sanremes_state.repair_state_db_schema
    ...
    monkeypatch.setattr(sanremes_state, "repair_state_db_schema", fake_repair)
    ...
    monkeypatch.setattr(sanremes_state, "repair_state_db_schema", real_repair)
...
def test_fts_read_corruption_repaired_in_place(tmp_path):
    ...
    from sanremes_state import _db_opens_cleanly
...
def test_repair_rebuilds_stale_btree_indexes(tmp_path):
    ...
    reason = sanremes_state._db_opens_cleanly(db_path)
    assert reason is not None
    ...
    assert sanremes_state._db_opens_cleanly(db_path) is None
    raw = sqlite3.connect(str(db_path))
...
# gateway service, the Desktop app's own `sanremes serve` backend, interactive
...
def _lock_held_by_other_process(db_path: Path, hold_seconds: float = 30.0):
    ...
    script = _HOLD_LOCK_SCRIPT.format(
        root=str(Path(sanremes_state.__file__).parent),
...
def test_repair_skips_surgery_while_another_process_holds_the_lock(
    ...
    _build_healthy_db(db_path)
    _corrupt_duplicate_fts(db_path)
    monkeypatch.setattr(sanremes_state, "_REPAIR_LOCK_TIMEOUT_SECONDS", 0.5)
    ...
    assert sanremes_state._db_opens_cleanly(db_path) is not None
...
def test_two_processes_repairing_at_once_perform_surgery_once(tmp_path):
    ...
    script = _REPAIR_SCRIPT.format(
        root=str(Path(sanremes_state.__file__).parent), db=str(db_path)
    )
    procs = [
        subprocess.Popen(
            [sys.executable, "-c", script],
...
def _configure_journal_mode(monkeypatch, tmp_path, mode) -> None:
    ...
    monkeypatch.setattr(
        sanremes_state, "is_sqlite_wal_reset_vulnerable", lambda **kwargs: False,
    )
...
def test_repair_restore_matches_canonical_on_vulnerable_sqlite(
    ...
    _configure_journal_mode(monkeypatch, tmp_path, "wal")
    monkeypatch.setattr(
        sanremes_state, "is_sqlite_wal_reset_vulnerable", lambda **kwargs: True
    )
    _build_healthy_db(db_path)
    conn = sqlite3.connect(str(db_path))
    conn.execute("PRAGMA journal_mode=DELETE")
...
def test_repair_logs_mode_change_when_probe_succeeded(
    ...
    with (
        patch.object(
            sanremes_state, "_probe_journal_mode_for_repair", return_value="delete"
        ),
        caplog.at_level(logging.WARNING, logger="sanremes_state"),
    ):
        report = repair_state_db_schema(db_path)
...
def test_repair_restore_failure_is_nonfatal_and_logged(
    ...
    with (
        patch.object(sanremes_state, "apply_wal_with_fallback", _refused),
        caplog.at_level(logging.WARNING, logger="sanremes_state"),
    ):
        report = repair_state_db_schema(db_path)
