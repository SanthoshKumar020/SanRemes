...
class TestToolsetConsistency:
    ...
    def test_sanremes_platforms_share_core_tools(self):
        """All sanremes-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on sanremes-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["sanremes-cli", "sanremes-telegram", "sanremes-discord", "sanremes-whatsapp", "sanremes-slack", "sanremes-signal", "sanremes-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_sanremes_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("sanremes-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("sanremes-cli")
        second = resolve_toolset("sanremes-cli")
        ...
        assert (
            "sanremes-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("sanremes-cli", include_registry=False)
        second = resolve_toolset("sanremes-cli", include_registry=False)
        assert first == second
