...
def test_upload_evidence_accepts_only_attachment_urls(tmp_path, monkeypatch):
    ...
    assert calls[0][0] == ["gh", "image", "--repo", "NousResearch/sanremes-agent", str(shot)]
...
def test_publish_marks_evidence_upload_failure_in_pr_comment(tmp_path, monkeypatch):
    comment = {
        "id": 123,
        "body": "before\n<!-- sanremes-e2e-evidence:start -->\npending\n<!-- sanremes-e2e-evidence:end -->\nafter",
    }
    updates = []
    ...
    assert updates == [
        (
            "https://api.github.com/repos/NousResearch/sanremes-agent/issues/comments/123",
            "github-token",
            "PATCH",
            {
...
def test_find_review_comment_requires_the_evidence_marker():
    pending = "<!-- sanremes-ci-review-bot -->\n<!-- sanremes-e2e-evidence:start -->\npending\n<!-- sanremes-e2e-evidence:end -->"

    assert _mod._find_review_comment([{"body": "<!-- sanremes-ci-review-bot --> no evidence"}]) is None
