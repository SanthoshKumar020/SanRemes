...
The defect: sanremes_state already knows macOS `fsync()` does not guarantee
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    source = Path(sanremes_state.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(sanremes_state.__file__))
