...
Older versions of ``install.sh`` created ``$command_link_dir/sanremes`` as a
symlink to the pip-generated entry point at ``$SANREMES_BIN`` (i.e.
``venv/bin/sanremes``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/sanremes" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$SANREMES_BIN" "$@"`` then self-recursed and ``sanremes`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/sanremes``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/sanremes\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/sanremes"')
    cat_idx = block.find('cat > "$command_link_dir/sanremes" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/sanremes before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/sanremes        <- pip entry point (the one we must preserve)
          local_bin/sanremes       <- symlink → ../venv/bin/sanremes  (old install)

    Then we run the exact shim-write block from setup_path() with
    ``SANREMES_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    requires that, after the run:

      * ``venv/bin/sanremes`` still contains its original pip-script body
      * ``local_bin/sanremes`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "sanremes"
    ...
    shim_path = command_link_dir / "sanremes"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/sanremes was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/sanremes must be replaced with a regular file, not "
