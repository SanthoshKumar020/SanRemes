...
`scripts/desktop-update/windows.ps1` drives `sanremes update` for the in-app
Desktop updater. It used to invoke the update through the venv's
`venv\\Scripts\\sanremes.exe` console-script launcher. On Windows that launcher is
a real process that keeps `sanremes.exe` mapped as its running image and spawns
`python.exe` as a child. The update ends in `uv pip install -e .`, which rewrites
the console-script shims -- including the `sanremes.exe` the launcher still has
...
Start Menu shortcut points at a `SanRemes.exe` that no longer exists.

Driving the update as `python.exe -m sanremes_cli.main update` puts the inherited
...
hand-off. The invariant it guards is that every `Invoke-SanRemesStep` call site
(the update, its retry, and the desktop rebuild) drives `$pythonExe`, never the
`$sanremesExe` shim. `sanremes.exe` may still be *named* in the file for the
...
def _handoff_source() -> str:
    ...
    fixture runs a synthetic PowerShell step through ``Invoke-SanRemesStep`` to
...
def test_invoke_sanremes_step_calls_drive_python_not_the_shim() -> None:
    source = _handoff_source()

    invocations = re.findall(r"Invoke-SanRemesStep\s+(\$\w+)", source)
    assert invocations, (
        "Expected at least one Invoke-SanRemesStep call in "
    ...
    assert not offenders, (
        "Every Invoke-SanRemesStep call in scripts/desktop-update/windows.ps1 "
        "must drive $pythonExe, not the sanremes.exe shim. Driving the update "
        "through the shim keeps sanremes.exe mapped as a running image, so uv's "
...
def test_update_invocation_uses_module_entrypoint() -> None:
    ...
    assert '@("-m", "sanremes_cli.main", "update"' in source, (
        "The update step must invoke `python.exe -m sanremes_cli.main update ...` "
        "so the inherited image handle lands on python.exe, which uv never has "
        "to replace."
    )
    assert (
        '@("-m", "sanremes_cli.main", "desktop", "--force-build", "--build-only")'
        in source
    ), (
        "The desktop rebuild step must also go through "
        "`python.exe -m sanremes_cli.main desktop ...` for the same reason."
    )


def test_update_no_longer_invokes_the_sanremes_exe_shim() -> None:
    source = _read()

    assert "Invoke-SanRemesStep $sanremesExe" not in source, (
        "scripts/desktop-update/windows.ps1 still invokes the update through "
        "the sanremes.exe shim (`Invoke-SanRemesStep $sanremesExe`). That is the "
