...
def test_managed_node_swap_defines_staged_and_backup_as_siblings() -> None:
    ...
    assert re.search(r'\$staged\s*=\s*"\$SanRemesHome\\node\.new-\$stamp"', swap), (
        "$staged must be defined as a sibling of the live tree "
        '("$SanRemesHome\\node.new-$stamp")'
    )
    assert re.search(r'\$backup\s*=\s*"\$SanRemesHome\\node\.old-\$stamp"', swap), (
        "$backup must be defined as a sibling of the live tree "
        '("$SanRemesHome\\node.old-$stamp")'
    )


def test_managed_node_swap_renames_stay_within_sanremes_home() -> None:
    swap = _swap_block()
