...
SanRemes state stays profile-scoped through SANREMES_HOME. Host subprocesses should
...
See: https://github.com/NousResearch/sanremes-agent/issues/25114
See: https://github.com/NousResearch/sanremes-agent/issues/36144
See: https://github.com/NousResearch/sanremes-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for sanremes_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(sanremes_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("SANREMES_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        sanremes_home = real_home / ".sanremes" / "profiles" / "coder"
        profile_home = sanremes_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_REAL_HOME", str(real_home))

        from sanremes_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".sanremes" / "profiles"
        ...
        monkeypatch.setenv("SANREMES_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("SANREMES_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["SANREMES_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(sanremes_home / "home")
        assert result["SANREMES_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_sanremes_home_unset(self, monkeypatch):
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from sanremes_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from sanremes_cli.profiles import create_profile
