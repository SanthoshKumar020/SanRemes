...
Provides a default ``SANREMES_MODEL`` for cron run_job tests so each one
doesn't have to spell out a model. The global conftest blanks
SANREMES_MODEL hermetically; without this autouse fixture every cron test
that exercises ``run_job`` would hit the fail-fast guard added in
``cron/scheduler.py`` (see issue #23979) and have to be rewritten.

Tests that specifically need ``SANREMES_MODEL`` unset — model-resolution
edge cases — call ``monkeypatch.delenv("SANREMES_MODEL", raising=False)``
...
def _default_cron_test_model(monkeypatch):
    """Pin a default SANREMES_MODEL so cron run_job tests have a resolvable model."""
    monkeypatch.setenv("SANREMES_MODEL", "test-cron-default-model")
    yield
