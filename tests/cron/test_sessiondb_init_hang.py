...
which is already bounded by SANREMES_CRON_TIMEOUT — the worker thread submitted
...
timeout (SANREMES_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_CRON_SESSION_DB_TIMEOUT", "0.2")
        ...
        with patch("cron.scheduler._sanremes_home", tmp_path), \
             ...
             patch("sanremes_state.SessionDB"), \
             patch(
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed SANREMES_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors SANREMES_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("SANREMES_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "SANREMES_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("SANREMES_CRON_SESSION_DB_TIMEOUT", raising=False)
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._sanremes_home", tmp_path), \
                 ...
                 patch("sanremes_cli.env_loader.load_sanremes_dotenv"), \
                 patch("sanremes_cli.env_loader.reset_secret_source_cache"), \
                 patch("sanremes_state.SessionDB", side_effect=_hanging_then_capture), \
                 patch(
                     "sanremes_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
...
class TestSessionDbInitAfterEarlyReturns:
    ...
    def test_wake_gate_false_never_opens_session_db(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._sanremes_home", tmp_path), \
             ...
             patch("sanremes_cli.env_loader.load_sanremes_dotenv"), \
             patch("sanremes_cli.env_loader.reset_secret_source_cache"), \
             patch("sanremes_state.SessionDB") as mock_db_cls, \
             patch(
