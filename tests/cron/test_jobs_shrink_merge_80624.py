...
def test_stale_empty_save_preserves_concurrent_no_agent_create(sanremes_env):
...
def test_remove_other_job_preserves_concurrent_create(sanremes_env):
...
def test_intentional_remove_still_deletes(sanremes_env):
...
def test_replace_flag_allows_wholesale_rewrite(sanremes_env):
    from cron.jobs import create_job, load_jobs, save_jobs
...
def test_jobs_json_on_disk_matches_merge(sanremes_env):
    ...
    save_jobs([])
    payload = json.loads((Path(sanremes_env) / "cron" / "jobs.json").read_text())
    assert [j["id"] for j in payload["jobs"]] == [job["id"]]


def test_stamp_fast_path_skips_merge_when_file_unchanged(sanremes_env, monkeypatch):
...
def test_merge_does_not_mutate_caller_list(sanremes_env):
...
def test_corrupt_disk_file_does_not_break_save(sanremes_env):
...
def test_nested_create_survives_outer_stale_save(sanremes_env):
