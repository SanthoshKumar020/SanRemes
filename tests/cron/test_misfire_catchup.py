...
from cron.jobs import _sanremes_now, create_job, get_job, load_jobs, save_jobs
...
def _park_in_past(job_id, minutes):
    ...
    for j in jobs:
        if j["id"] == job_id:
            j["next_run_at"] = (
                _sanremes_now() - timedelta(minutes=minutes)
            ).isoformat()
    save_jobs(jobs)
...
class TestFireOverdueJobs:
    ...
    def test_claim_advances_next_run_no_refire(self, tmp_cron_dir):
        ...
        assert stamped["next_run_at"] > _sanremes_now().isoformat()
