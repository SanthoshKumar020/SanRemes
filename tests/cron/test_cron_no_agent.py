...
def test_create_job_no_agent_requires_script(sanremes_env):
...
def test_update_job_roundtrips_no_agent_flag(sanremes_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = sanremes_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(sanremes_env):
...
def test_run_job_no_agent_success_returns_script_stdout(sanremes_env):
    """Happy path: script exits 0 with output, delivered verbatim."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = sanremes_env / "scripts" / "alert.sh"
...
def test_run_job_no_agent_reloads_dotenv_before_script(sanremes_env, monkeypatch):
    ...
    def fake_load(*, sanremes_home=None, project_env=None):
        loaded_homes.append(sanremes_home)
        return []

    monkeypatch.setattr(env_loader, "load_sanremes_dotenv", fake_load)

    script_path = sanremes_env / "scripts" / "probe.sh"
    script_path.write_text('#!/bin/bash\necho "ok"\n')
    ...
    assert success is True
    assert error is None
    assert loaded_homes, "load_sanremes_dotenv was not called on the no_agent path"
    assert str(loaded_homes[0]) == str(sanremes_env)


def test_timed_out_no_agent_script_delivery_is_not_mislabeled_as_provider_failure(
    sanremes_env, monkeypatch,
):
    ...
    (sanremes_env / "scripts" / "slow.py").write_text("import time; time.sleep(999)\n")
    job = create_job(
        prompt=None,
        schedule="every 5m",
        script="slow.py",
...
def test_agent_provider_timeout_delivery_keeps_fallback_guidance(sanremes_env, monkeypatch):
...
def test_run_job_script_path_traversal_still_blocked(sanremes_env):
...
def test_run_job_script_nul_path_fails_cleanly(sanremes_env):
...
def test_run_job_script_nul_rejected_before_any_path_call(sanremes_env, monkeypatch):
...
def test_run_job_script_accepts_pathlike_script_path(sanremes_env):
    ...
    script = sanremes_env / "scripts" / "probe.py"
...
@pytest.mark.parametrize(
    "error",
    [
        "Script timed out after 900s: /home/u/.sanremes/scripts/nightly.sh",
