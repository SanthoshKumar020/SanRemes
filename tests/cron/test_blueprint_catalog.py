...
the shared /blueprint command handler (sanremes_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated SANREMES_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("sanremes://blueprint/morning-brief?")
...
class TestCommandHandler:
    def test_bare_lists_catalog(self, isolated_home):
        from sanremes_cli.blueprint_cmd import handle_blueprint_command
