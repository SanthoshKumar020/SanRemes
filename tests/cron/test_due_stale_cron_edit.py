...
These exercise the real store against a temp ``SANREMES_HOME`` (no mocks) per
...
def temp_home(tmp_path, monkeypatch):
    """Isolated SANREMES_HOME so jobs.json doesn't touch the real store."""
...
def test_stale_next_run_on_excluded_dow_does_not_fire(temp_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "cron.jobs._sanremes_now", lambda: _SATURDAY_0700 + timedelta(seconds=30)
    )
...
def test_delayed_manual_trigger_is_not_counted_as_catch_up(temp_home, monkeypatch):
    ...
    monkeypatch.setattr("cron.jobs._sanremes_now", lambda: trigger_time)
    job = create_job(prompt="x", schedule="0 7 * * 1-5", name="delayed")
    trigger_job(job["id"])
    monkeypatch.setattr(
        "cron.jobs._sanremes_now", lambda: trigger_time + timedelta(hours=5)
    )
...
def test_manual_trigger_survives_timezone_change_before_tick(temp_home, monkeypatch):
    ...
    trigger_job(job["id"])
    monkeypatch.setattr("cron.jobs._sanremes_now", lambda: scan_time)
...
def test_stale_next_run_skips_even_inside_catchup_window(temp_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "cron.jobs._sanremes_now", lambda: _SATURDAY_0700 + timedelta(hours=5)
    )
    jid = _write_cron_job("0 7 * * 1-5", _SATURDAY_0700)
...
def test_stale_edit_without_manual_marker_still_reanchors(temp_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "cron.jobs._sanremes_now", lambda: _SATURDAY_0700 + timedelta(minutes=5)
    )
    jid = _write_cron_job("0 7 * * 1-5", _SATURDAY_0700)
