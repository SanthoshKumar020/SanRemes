...
def sanremes_env(tmp_path, monkeypatch):
    """Isolate SANREMES_HOME for each test so jobs/scripts/snapshots don't leak."""
    ...
    # Reload modules that cache get_sanremes_home() at import time.
    import importlib
...
def _install_agent_stubs(monkeypatch, observed: dict):
    ...
    from sanremes_cli import runtime_provider as _rtp
    monkeypatch.setattr(
        _rtp,
    ...
    monkeypatch.setenv("SANREMES_CRON_TIMEOUT", "0")
...
def test_create_job_stores_monitor_script(sanremes_env):
    from cron.jobs import create_job, get_job

    _write_script(sanremes_env, "mon.sh", "echo stable\n")
    job = create_job(
...
def test_create_job_monitor_script_and_url_mutually_exclusive(sanremes_env):
...
def test_create_job_monitor_rejected_with_no_agent(sanremes_env):
    from cron.jobs import create_job

    _write_script(sanremes_env, "w.sh", "echo hi\n")
...
def test_update_job_rejects_no_agent_on_monitor_job(sanremes_env):
...
def test_update_job_rejects_adding_monitor_to_no_agent_job(sanremes_env):
    from cron.jobs import create_job, update_job
...
def test_update_job_rejects_second_monitor_source(sanremes_env):
    from cron.jobs import create_job, update_job
...
def test_update_job_allows_clearing_monitor_then_no_agent(sanremes_env):
...
def test_update_job_unrelated_fields_skip_mode_validation(sanremes_env):
...
def test_hash_is_exact_bytes(sanremes_env):
...
def test_unified_diff_is_capped(sanremes_env):
...
def _make_monitor_job(sanremes_env, script_body: str):
    from cron.jobs import create_job

    _write_script(sanremes_env, "mon.sh", script_body)
    return create_job(
...
def test_first_run_always_runs_agent(sanremes_env, monkeypatch):
    from cron.scheduler import run_job

    job = _make_monitor_job(sanremes_env, "echo 'state A'\n")
    observed: dict = {}
...
def test_unchanged_output_suppresses_agent_run(sanremes_env, monkeypatch):
...
def test_changed_output_injects_diff(sanremes_env, monkeypatch):
    from cron.jobs import get_job
    from cron.scheduler import run_job
    ...
    _write_script(sanremes_env, "mon.sh", "echo 'state B'\n")
    job = get_job(job["id"])
    success, doc, final, error = run_job(job)
    assert success is True
...
def test_hash_persists_across_scheduler_restart(sanremes_env, monkeypatch):
...
def test_monitor_script_failure_is_error_not_change(sanremes_env, monkeypatch):
    from cron.jobs import get_job
    from cron.scheduler import run_job
    ...
    _write_script(sanremes_env, "mon.sh", "echo boom >&2\nexit 3\n")
    job = get_job(job["id"])
    success, doc, final, error = run_job(job)
...
def test_cronjob_tool_create_with_monitor_script(sanremes_env):
    from cron.jobs import get_job
    from tools.cronjob_tools import cronjob

    _write_script(sanremes_env, "mon.sh", "echo hi\n")
    result = json.loads(
        cronjob(
            action="create",
            prompt="React",
...
def test_cronjob_tool_rejects_monitor_script_path_escape(sanremes_env):
    from tools.cronjob_tools import cronjob
...
def test_cronjob_tool_update_clears_monitor_script(sanremes_env):
    from cron.jobs import get_job
    from tools.cronjob_tools import cronjob
