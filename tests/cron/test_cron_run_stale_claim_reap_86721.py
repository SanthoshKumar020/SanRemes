"""Regression for #86721 — a one-shot `sanremes cron run` invocation's
...
def test_recover_interrupted_executions_reaps_the_stale_claim_from_a_dead_process(
    ...
    # exactly what a subsequent `sanremes cron run` invocation would trigger
...
def test_try_dispatch_background_run_calls_recovery_before_claiming(monkeypatch):
    ...
    # caller ("sanremes cron run", tests) early return documented just
