...
- Job store (cron/jobs.py): the field is validated at the storage choke
  point against the canonical SanRemes effort grammar (parse_reasoning_effort
  in sanremes_constants — the SAME parser every other effort surface uses).
...
class TestCronjobToolReasoningEffort:
    ...
    via `sanremes cron create/edit --reasoning-effort` only."""
    ...
    def test_schema_does_not_expose_reasoning_effort(self):
        ...
        keeps the parameter for the CLI lane (sanremes_cli/cron.py), but the
