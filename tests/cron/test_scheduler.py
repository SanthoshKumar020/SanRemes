...
class TestPerJobToolsetMcpMerge:
    ...
    def test_resolver_empty_per_job_falls_through_to_platform(self):
        ...
        with patch("sanremes_cli.tools_config._get_platform_tools",
...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._sanremes_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("sanremes_cli.env_loader.load_sanremes_dotenv"), \
             patch("sanremes_cli.env_loader.reset_secret_source_cache"), \
             patch("sanremes_state.SessionDB", return_value=fake_db), \
             patch(
    ...
    def _run_job_patches(self, tmp_path, extra=()):
        ...
        mock_agent.run_conversation.return_value = {"final_response": "ok"}
        base = [
            patch("cron.scheduler._sanremes_home", tmp_path),
            patch("cron.scheduler._resolve_origin", return_value=None),
            patch("sanremes_cli.env_loader.load_sanremes_dotenv"),
            patch("sanremes_cli.env_loader.reset_secret_source_cache"),
            patch("sanremes_state.SessionDB", return_value=fake_db),
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("SANREMES_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("SANREMES_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("SANREMES_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("SANREMES_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("SANREMES_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("SANREMES_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("SANREMES_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("SANREMES_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("SANREMES_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_heartbeats_oneshot_claim_in_both_wait_modes(
        ...
        monkeypatch.setenv("SANREMES_CRON_TIMEOUT", timeout_value)
    ...
    def test_run_job_resets_secret_source_cache_before_reload(self, tmp_path, monkeypatch):
        ...
        this SANREMES_HOME in ``_APPLIED_HOMES``, so the external-secret pull
        ...
        ``load_sanremes_dotenv`` (which then re-applies external secret sources).
        """
        ...
        with patch("cron.scheduler._sanremes_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("sanremes_cli.env_loader.reset_secret_source_cache", _record_reset), \
             patch("sanremes_cli.env_loader.load_sanremes_dotenv", _record_load), \
             patch("sanremes_state.SessionDB", return_value=fake_db), \
             patch(
                 "sanremes_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("SANREMES_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("SANREMES_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("SANREMES_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigLogging:
    ...
    def test_bad_config_yaml_is_logged(self, caplog, tmp_path):
        ...
        with patch("cron.scheduler._sanremes_home", tmp_path), \
             ...
             patch("sanremes_cli.env_loader.load_sanremes_dotenv"), \
             patch("sanremes_cli.env_loader.reset_secret_source_cache"), \
             patch("sanremes_cli.runtime_provider.resolve_runtime_provider",
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_SANREMES_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_SANREMES_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_SANREMES_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_SANREMES_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_SANREMES_TEST_CRON_UNSET_VAR}"
...
class TestRunJobModelResolution:
    ...
    ``model: null``. At fire time the scheduler must:
      1. fall back to ``SANREMES_MODEL`` env if set,
    ...
    def test_null_job_model_falls_back_to_env(self, tmp_path, monkeypatch):
        """``model: null`` on the job uses SANREMES_MODEL when set."""
        (tmp_path / "config.yaml").write_text("")
        monkeypatch.setenv("SANREMES_MODEL", "env-model")
    ...
    def test_config_model_alias_key_resolves(self, tmp_path, monkeypatch):
        ...
        ``sanremes_cli/oneshot.py``, ``fallback_cmd.py`` and ``prompt_size.py``
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """SANREMES_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("SANREMES_CRON_MAX_PARALLEL", "1")
        call_times = []
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``sanremes cron create --deliver=<platform>`` silently
...
class TestFailureStreakNudge:
    ...
    def test_nudges_at_threshold(self):
        ...
        assert "sanremes cron pause scout" in out
