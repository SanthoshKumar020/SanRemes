...
def cron_store(tmp_path, monkeypatch):
    ...
    (sanremes_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
    monkeypatch.setattr(J, "SANREMES_DIR", sanremes_home)
    monkeypatch.setattr(J, "CRON_DIR", sanremes_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", sanremes_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", sanremes_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_sanremes_now", lambda: now):
            due = J.get_due_jobs()
