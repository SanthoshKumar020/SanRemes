...
def test_run_job_releases_cwd_lock_when_body_raises(tmp_path):
    ...
    with patch("cron.scheduler._sanremes_home", tmp_path), \
         ...
         patch("sanremes_state.SessionDB", return_value=MagicMock()):
...
def test_cwd_lock_timeout_derivation(monkeypatch):
    """The bound tracks SANREMES_CRON_TIMEOUT (+margin) with a floor, and
    ...
    monkeypatch.setenv("SANREMES_CRON_TIMEOUT", "1800")
    assert sched._cwd_lock_timeout_seconds() == 1860.0
    monkeypatch.setenv("SANREMES_CRON_TIMEOUT", "30")
    assert sched._cwd_lock_timeout_seconds() == 180.0
    monkeypatch.setenv("SANREMES_CRON_TIMEOUT", "0")
    assert sched._cwd_lock_timeout_seconds() == 660.0
    monkeypatch.setenv("SANREMES_CRON_TIMEOUT", "bogus")
    assert sched._cwd_lock_timeout_seconds() == 660.0
