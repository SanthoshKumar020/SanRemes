...
@pytest.fixture
def sanremes_env(tmp_path, monkeypatch):
    """Isolate SANREMES_HOME for each test so jobs/scripts don't leak."""
    ...
    importlib.reload(sanremes_constants)
    import cron.jobs
...
def test_create_job_rejects_empty_payload(sanremes_env, prompt):
...
def test_create_job_rejects_blank_script_and_blank_skills(sanremes_env):
    from cron.jobs import create_job
...
def test_create_job_no_agent_error_still_wins(sanremes_env):
...
def test_valid_shapes_are_accepted(sanremes_env):
    from cron.jobs import create_job

    (sanremes_env / "scripts" / "w.sh").write_text("echo hi\n")
...
def test_update_job_rejects_clearing_the_only_payload(sanremes_env):
...
def test_update_job_rejects_dropping_last_skill_from_promptless_job(sanremes_env):
...
def test_update_job_rejects_clearing_script_from_promptless_job(sanremes_env):
    from cron.jobs import create_job, update_job
...
def test_update_job_rejects_toggling_no_agent_on_without_a_script(sanremes_env):
...
def test_update_job_rejects_removing_script_from_a_no_agent_job(sanremes_env, blank):
    from cron.jobs import create_job, get_job, update_job
...
def test_update_job_rejects_swapping_script_for_prompt_on_a_no_agent_job(sanremes_env):
...
def test_update_job_allows_dropping_script_when_no_agent_is_turned_off(sanremes_env):
...
def test_update_job_allows_swapping_the_script_of_a_no_agent_job(sanremes_env):
    from cron.jobs import create_job, get_job, update_job
...
def test_update_job_allows_clearing_prompt_when_script_remains(sanremes_env):
...
def test_update_job_allows_swapping_prompt_for_skill(sanremes_env):
    from cron.jobs import create_job, get_job, update_job
...
def _legacy_empty_job(sanremes_env):
...
def test_run_job_fails_closed_and_never_builds_an_agent(sanremes_env):
...
def test_run_job_pauses_the_job_on_disk(sanremes_env):
...
def test_run_one_job_does_not_resurrect_the_paused_job(sanremes_env):
...
def _legacy_no_agent_scriptless_job(sanremes_env, script_value=None):
...
def test_run_job_pauses_a_legacy_no_agent_job_without_a_script(sanremes_env, script_value):
    """Erroring alone left it enabled, so it re-fired every tick."""
    import cron.scheduler as scheduler
    from cron.jobs import get_job

    job = _legacy_no_agent_scriptless_job(sanremes_env, script_value)
...
def test_run_one_job_does_not_resurrect_the_paused_no_agent_job(sanremes_env):
    """Post-run bookkeeping must not put it back in the due queue."""
    import cron.scheduler as scheduler
    from cron.jobs import get_due_jobs, get_job

    job = _legacy_no_agent_scriptless_job(sanremes_env)
...
def test_run_job_does_not_block_a_valid_no_agent_job(sanremes_env):
    """The guard sits after the no_agent short-circuit, which must still run."""
    import cron.scheduler as scheduler

    script = sanremes_env / "scripts" / "w.sh"
    script.write_text("echo hello\n")

    job = dict(_legacy_empty_job(sanremes_env), script="w.sh", no_agent=True)
    success, doc, final, error = scheduler.run_job(job)
...
def test_tool_update_rejects_the_2026_08_03_destructive_shape(sanremes_env):
...
def test_tool_update_rejects_the_destructive_shape_on_a_script_job(sanremes_env):
...
def test_tool_update_rejects_the_destructive_shape_on_a_no_agent_job(sanremes_env):
...
def test_tool_update_blank_name_is_a_no_op(sanremes_env):
...
def test_tool_update_still_renames_when_a_real_name_is_given(sanremes_env):
    from cron.jobs import create_job, get_job
...
def test_tool_update_blank_scalars_still_clear_workdir_and_context_from(sanremes_env):
    ...
    (sanremes_env / "scripts" / "w.sh").write_text("echo hi\n")
    (sanremes_env / "wd").mkdir()
    job = create_job(
        prompt=None, schedule="0 2 * * *", script="w.sh", name="keeper",
        enabled_toolsets=["file"], workdir=str(sanremes_env / "wd"),
        deliver="local",
    )
    assert get_job(job["id"])["workdir"] == str(sanremes_env / "wd")
