...
profile's SANREMES_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.sanremes/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active SANREMES_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever SANREMES_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    SANREMES_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import sanremes_constants

    monkeypatch.setattr(
        sanremes_constants, "_get_platform_default_sanremes_home", lambda: root
    )
    monkeypatch.setenv("SANREMES_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile SANREMES_HOME (<root>/profiles/<name>), the cron store
    ...
    assert sanremes_constants.get_sanremes_home().resolve() == profile_home.resolve()
    assert sanremes_constants.get_default_sanremes_root().resolve() == root.resolve()

    # cron/jobs.py computes SANREMES_DIR from get_sanremes_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.SANREMES_DIR.resolve() == profile_home.resolve()
        assert (
