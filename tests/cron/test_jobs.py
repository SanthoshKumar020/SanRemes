...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED SanRemes
        ...
        due-check compares against ``_sanremes_now()`` (configured zone), so the
        ...
        monkeypatch.setattr("cron.jobs._sanremes_now", lambda: configured_now)
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured SanRemes timezone differs from the
    ...
    def test_recent_past_oneshot_is_due_under_diverging_tz(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.setattr("cron.jobs._sanremes_now", lambda: now)
...
class TestPauseResumeJob:
    ...
    def test_pause_is_authoritative_due_jobs_do_not_fire(self, tmp_cron_dir):
        ...
        past = (_sanremes_now() - timedelta(hours=2)).isoformat()
...
class TestAdvanceNextRun:
    ...
    def test_advances_interval_job(self, tmp_cron_dir):
        ...
        updated = get_job(job["id"])
        from cron.jobs import _ensure_aware, _sanremes_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _sanremes_now(), "next_run_at should be in the future after advance"
...
class TestGetDueJobs:
    ...
    def test_long_execution_does_not_perpetually_defer(self, tmp_cron_dir, monkeypatch):
        ...
        stale = (_sanremes_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["next_run_at"] = stale
        jobs[0]["last_run_at"] = (_sanremes_now() - timedelta(minutes=1)).isoformat()
        save_jobs(jobs)
    ...
    def test_one_shot_not_redispatched_while_running(self, tmp_cron_dir, monkeypatch):
        ...
        """
        from cron.jobs import _sanremes_now
        t0 = _sanremes_now()
        ...
        for gap in (28, 61, 130):
            monkeypatch.setattr("cron.jobs._sanremes_now",
    ...
    def test_run_claim_heartbeat_keeps_long_run_claimed_past_ttl(
        ...
        monkeypatch.delenv("SANREMES_CRON_TIMEOUT", raising=False)
        from cron.jobs import _sanremes_now, _oneshot_run_claim_ttl_seconds
...
class TestLateEnvRepointScopesStore:
    """A SANREMES_HOME set AFTER cron.jobs import must scope the store even
    ...
    def test_use_cron_store_override_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "env-home"))
    ...
    def test_public_io_after_late_env_repoint_leaves_old_file_untouched(
        ...
        called after a post-import SANREMES_HOME repoint must operate on the NEW
        ...
        monkeypatch.setattr(jobs, "SANREMES_DIR", sim_old_home)
        ...
        monkeypatch.setenv("SANREMES_HOME", str(new_home))
...
class TestJobsJsonIdKeyedMap:
    ...
    or hand edit — SanRemes' own save_jobs() only ever writes a list) made
    ...
    required`` and took down ``sanremes cron list``, the ``cronjob(action=
    ...
    def test_list_jobs_survives_id_keyed_map(self, tmp_cron_dir):
        """The reported traceback path (sanremes cron list / cronjob list tool)."""
        import json
