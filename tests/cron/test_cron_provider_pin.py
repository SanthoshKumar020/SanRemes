...
resolve_runtime_provider against a temp SANREMES_HOME) and the create_job
...
class TestProviderDriftGuard:
    ...
    def test_b_unpinned_snapshot_differs_fails_closed(self, tmp_path):
        ...
        assert "sanremes cron edit pin-test --provider <provider> --model <model>" in blob
        ...
        assert "host running sanremes" in delivered
        assert "sanremes cron edit pin-test --provider <provider> --model <model>" in delivered
    ...
    def test_missing_model_guides_to_user_owned_cli(self, tmp_path, monkeypatch):
        ...
        assert agent_constructed is False
        assert error is not None
        assert "sanremes cron edit pin-test --model <name>" in error
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("sanremes_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
...
def _run_with_current_provider_and_model(
    ...
    *,
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._sanremes_home", tmp_path), \
         patch("cron.scheduler._get_sanremes_home", return_value=tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("sanremes_cli.env_loader.load_sanremes_dotenv"), \
         patch("sanremes_cli.env_loader.reset_secret_source_cache"), \
         patch("sanremes_state.SessionDB", return_value=fake_db), \
