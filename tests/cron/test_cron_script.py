...
- Path resolution (absolute, relative to SANREMES_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp SANREMES_HOME."""
    sanremes_home = tmp_path / ".sanremes"
    sanremes_home.mkdir()
    (sanremes_home / "cron").mkdir()
    (sanremes_home / "cron" / "output").mkdir()
    (sanremes_home / "scripts").mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "SANREMES_DIR", sanremes_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", sanremes_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", sanremes_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", sanremes_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit SanRemes provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _SANREMES_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_SANREMES_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.sanremes/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("SANREMES_SESSION_PLATFORM") is None
        assert os.environ.get("SANREMES_SESSION_CHAT_ID") is None
        assert os.environ.get("SANREMES_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_CRON_SCRIPT_TIMEOUT", "2")
