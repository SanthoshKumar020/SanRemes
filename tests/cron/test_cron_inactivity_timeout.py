...
- Unlimited timeout (SANREMES_CRON_TIMEOUT=0)
- Backward compat: SANREMES_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """SANREMES_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """SANREMES_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("SANREMES_CRON_TIMEOUT", "1200")
...
class TestSysPathOrdering:
    ...
    def test_sanremes_time_importable(self):
        """sanremes_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _sanremes_now
        assert callable(_sanremes_now)
