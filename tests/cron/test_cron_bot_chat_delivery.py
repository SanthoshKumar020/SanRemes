...
def test_named_profile_resolves_when_exists():
    with mock.patch("sanremes_cli.profiles.profile_exists", return_value=True):
...
def test_unknown_profile_resolves_to_none():
    with mock.patch("sanremes_cli.profiles.profile_exists", return_value=False):
...
def test_deliver_runs_canonical_bot_chat_lane():
    ...
    with mock.patch.object(sched.subprocess, "run", side_effect=fake_run), \
         mock.patch.object(sched.shutil, "which", return_value="/usr/bin/sanremes"):
        err = _deliver_to_bot_chat({"id": "j1", "name": "Daily digest"}, "the output", "")

    assert err is None
    argv = calls["argv"]
    assert argv[0] == "/usr/bin/sanremes"
    assert "-p" not in argv  # own profile: subprocess inherits SANREMES_HOME
    assert "chat" in argv
...
def test_deliver_named_profile_uses_p_flag_and_clears_home():
    ...
    with mock.patch.object(sched.subprocess, "run", side_effect=fake_run), \
         mock.patch.object(sched.shutil, "which", return_value="/usr/bin/sanremes"), \
         mock.patch.dict(sched.os.environ, {"SANREMES_HOME": "/tmp/other-profile"}):
        err = _deliver_to_bot_chat({"id": "j1", "name": "n"}, "out", "research")

    assert err is None
    argv = calls["argv"]
    assert argv[1:3] == ["-p", "research"]
    # -p owns resolution; the scheduler's own SANREMES_HOME must not leak in.
    assert "SANREMES_HOME" not in calls["kwargs"]["env"]
...
def test_deliver_timeout_returns_error_string():
    with mock.patch.object(
        ...
        side_effect=subprocess.TimeoutExpired(cmd="sanremes", timeout=600),
    ), mock.patch.object(sched.shutil, "which", return_value="/usr/bin/sanremes"):
        err = _deliver_to_bot_chat({"id": "j1", "name": "n"}, "out", "")
    assert err is not None
...
def test_delivery_targets_include_local_profiles():
    with mock.patch("sanremes_cli.profiles.list_profile_names",
