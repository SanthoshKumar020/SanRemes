...
Field incident: an interrupted `sanremes update` (pull done, restart never ran)
...
had no way to know the fix was one `sanremes gateway restart`. The failure
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "sanremes gateway restart" not in msg
