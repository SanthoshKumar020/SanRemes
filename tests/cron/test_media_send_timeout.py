...
- PR #87967 (@AiwendilInTheWoods): the per-attachment send timeout was a
  ...
  SANREMES_CRON_MEDIA_SEND_TIMEOUT → cron.media_send_timeout_seconds → 300s.
"""
...
class TestMediaSendTimeoutResolution:
    def test_default(self, monkeypatch):
        monkeypatch.delenv("SANREMES_CRON_MEDIA_SEND_TIMEOUT", raising=False)
    ...
    def test_env_wins(self, monkeypatch):
        monkeypatch.setenv("SANREMES_CRON_MEDIA_SEND_TIMEOUT", "45")
        monkeypatch.setattr(
    ...
    def test_invalid_env_falls_back(self, monkeypatch, bad):
        monkeypatch.setenv("SANREMES_CRON_MEDIA_SEND_TIMEOUT", bad)
        monkeypatch.setattr("cron.scheduler.load_config", lambda: {})
