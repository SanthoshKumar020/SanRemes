...
(``sanremes cron list`` / ``sanremes cron status``); the agent path did not.
...
def sanremes_env(tmp_path, monkeypatch):
    """Isolate SANREMES_HOME for each test so jobs don't leak."""
...
class TestCreateSurfacesGatewayLiveness:
    def test_create_with_gateway_running_has_no_warning(self, sanremes_env):
    ...
    def test_create_without_gateway_warns_not_scheduled(self, sanremes_env):
        with (
    ...
    def test_non_builtin_provider_is_exempt(self, sanremes_env):
    ...
    def test_failed_probe_stays_neutral(self, sanremes_env):
...
class TestListSurfacesGatewayLiveness:
    ...
    def test_list_with_gateway_running_has_no_warning(self, sanremes_env):
    ...
    def test_list_without_gateway_warns_jobs_inert(self, sanremes_env):
        _create_job()
    ...
    def test_list_empty_without_gateway_stays_quiet(self, sanremes_env):
    ...
    def test_list_non_builtin_provider_is_exempt(self, sanremes_env):
        _create_job()
        with patch_liveness(provider="chronos", pids=[]):
            result = self._list_jobs()
...
class _LivenessPatches:
    ...
    def __enter__(self):
        ...
        self._stack.enter_context(
            patch(
                "sanremes_cli.cron._active_cron_provider_name",
...
class TestRuntimeLockFirstLiveness:
    ...
    def test_lock_active_reports_alive_despite_empty_pid_scan(self, sanremes_env):
    ...
    def test_lock_inactive_falls_back_to_pid_scan(self):
        ...
        import sanremes_cli.cron as cron_cli

        with (
            patch("sanremes_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("gateway.status.is_gateway_runtime_lock_active", return_value=False),
            patch("sanremes_cli.gateway.find_gateway_pids", return_value=[424242]),
        ):
    ...
    def test_no_lock_no_pids_is_false(self):
        ...
        with (
            patch("sanremes_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("gateway.status.is_gateway_runtime_lock_active", return_value=False),
            patch("sanremes_cli.gateway.find_gateway_pids", return_value=[]),
        ):
...
class TestCronStatusLockFirst:
    """`sanremes cron status` shares the lock-first false-alarm fix (#95947).
    ...
    def _run_status(self, *, pids, lock_active, lock_pid=None):
        ...
        with (
            patch("sanremes_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("sanremes_cli.gateway.find_gateway_pids", return_value=list(pids)),
            patch(
                "gateway.status.is_gateway_runtime_lock_active",
    ...
    def test_lock_active_suppresses_not_running_false_alarm(self, sanremes_env):
    ...
    def test_no_lock_no_pids_still_warns(self, sanremes_env):
