...
A one-shot ``sanremes cron run <job_id>`` used to background-dispatch the run
...
1. ``sanremes_cli.cron._job_action("run", ...)`` declares the channel stateless
...
def _orphan_claimed_row(executions, job_id: str) -> str:
    ...
    Mirrors what a one-shot ``sanremes cron run`` leaves behind: a row stuck in
...
class TestOneShotCliRunIsSynchronous:
    ...
    def test_cli_run_declares_stateless_channel_before_dispatch(self, monkeypatch):
        """`sanremes cron run` must gate off async delivery so the run executes
    ...
    def test_background_dispatch_refused_when_channel_stateless(self, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_SESSION_KEY", "inherited-gateway-session")
