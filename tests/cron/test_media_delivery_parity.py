...
text-only on manual ``sanremes cron run <job-id>``. Same box, same token, same
...
def _install_fake_slack_sender(monkeypatch, result_factory):
    ...
    if entry is None:
        ...
        import sanremes_cli.plugins as hp_boot
    ...
    import sanremes_cli.plugins as hp
...
def slack_platform_config(monkeypatch, tmp_path):
    ...
    # Config caches are process-global; clear them so the temp SANREMES_HOME wins.
    try:
...
class TestMediaPolicyEnvBridge:
    ...
    def test_bridge_helper_exists_and_applies_config(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("SANREMES_HOME", str(home))
        for var in (
            "SANREMES_MEDIA_DELIVERY_STRICT",
            "SANREMES_MEDIA_ALLOW_DIRS",
            "SANREMES_MEDIA_TRUST_RECENT_FILES",
        ):
        ...
        assert os.environ.get("SANREMES_MEDIA_DELIVERY_STRICT") == "1"
        assert str(allow_dir) in os.environ.get("SANREMES_MEDIA_ALLOW_DIRS", "")
        assert os.environ.get("SANREMES_MEDIA_TRUST_RECENT_FILES") == "0"
    ...
    def test_standalone_filter_honors_bridged_allowlist(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("SANREMES_HOME", str(home))
        for var in ("SANREMES_MEDIA_DELIVERY_STRICT", "SANREMES_MEDIA_ALLOW_DIRS"):
            monkeypatch.delenv(var, raising=False)
    ...
    def test_deliver_result_runs_bridge(self, monkeypatch, tmp_path, media_file):
        ...
        Realistic enterprise shape: SANREMES_MEDIA_DELIVERY_STRICT=1 arrives via
