...
use the notepad, and the `sanremes cron notepad` CLI handler.
"""
...
def cron_env(tmp_path, monkeypatch, notepad):
    """Isolated cron environment with temp SANREMES_HOME (mirrors test_cron_context_from)."""
...
class TestPromptInjection:
    def test_nonempty_notepad_rendered_into_prompt(self, cron_env, notepad):
        ...
        assert f"sanremes cron notepad {job['id']} set" in prompt
...
class TestNotepadCli:
    ...
    def test_cli_set_get_list_delete(self, notepad, capsys):
        from sanremes_cli.cron import cron_notepad
    ...
    def test_cron_command_dispatches_notepad(self, notepad):
        from sanremes_cli.cron import cron_command
