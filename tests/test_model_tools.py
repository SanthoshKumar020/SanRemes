...
class TestHandleFunctionCall:
    ...
    def test_post_tool_call_receives_non_negative_integer_duration_ms(self):
        ...
        with (
            ...
            patch("sanremes_cli.plugins.has_hook", return_value=True),
            patch("sanremes_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_no_listener_skips_post_and_transform_emit(self):
        ...
        with (
            patch("model_tools.registry.dispatch", return_value='{"ok":true}'),
            patch("sanremes_cli.plugins.has_hook", return_value=False),
            patch("sanremes_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_tool_request_and_execution_middleware_wrap_registry_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.plugins.invoke_middleware", fake_invoke_middleware)
        ...
        monkeypatch.setattr("sanremes_cli.plugins.has_hook", lambda name: True)
    ...
    def test_registry_exception_emits_terminal_tool_hook(self, monkeypatch):
        from sanremes_cli import lifecycle

        hook_calls = []
        monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", lambda *_args, **_kwargs: [])
...
class TestPreToolCallBlocking:
    ...
    def test_blocked_tool_returns_error_and_skips_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("sanremes_cli.plugins.has_hook", lambda name: True)
        monkeypatch.setattr("model_tools.registry.dispatch", fake_dispatch)
    ...
    def test_relay_rewrite_is_visible_to_pre_tool_authorization(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.observability.relay_runtime.apply_tool_request_intercepts",
            rewrite,
        )
        monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("sanremes_cli.plugins.has_hook", lambda name: True)
...
class TestDisabledToolsetsPlatformBundle:
    """Regression test for #33924: disabling a platform bundle (sanremes-*)
    must not remove core tools from other enabled toolsets."""

    def test_disabling_platform_bundle_preserves_core_tools(self):
        """Disabling sanremes-yuanbao should not strip core tools from sanremes-telegram."""
        from model_tools import get_tool_definitions

        tools_telegram = get_tool_definitions(
            enabled_toolsets=["sanremes-telegram"],
            quiet_mode=True,
        )
        tools_telegram_no_yuanbao = get_tool_definitions(
            enabled_toolsets=["sanremes-telegram"],
            disabled_toolsets=["sanremes-yuanbao"],
            quiet_mode=True,
        )
        ...
        assert names_telegram == names_no_yuanbao, (
            f"Tools lost after disabling sanremes-yuanbao: "
            f"{names_telegram - names_no_yuanbao}"
        )

    def test_disabling_platform_bundle_removes_own_tools(self):
        """Disabling sanremes-discord should remove discord-specific tools."""
        from model_tools import get_tool_definitions

        tools = get_tool_definitions(
            enabled_toolsets=["sanremes-discord"],
            disabled_toolsets=["sanremes-discord"],
            quiet_mode=True,
        )
    ...
    def test_bundle_non_core_tools_unknown_falls_back(self):
        ...
        assert bundle_non_core_tools("sanremes-does-not-exist") == set()
...
class TestDisabledToolsetsPostureToolset:
    ...
    not own -- same non-core-delta subtraction as sanremes-* bundles (#33924) --
