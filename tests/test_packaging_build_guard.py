...
def _build_artifact(kind: str, tmp_path, *, nix_build: bool) -> subprocess.CompletedProcess[str]:
    ...
    if nix_build:
        env["SANREMES_NIX_BUILD"] = "1"
    else:
        env.pop("SANREMES_NIX_BUILD", None)
...
def test_artifact_build_rejects_nix_development_shell_environment(kind, tmp_path):
    ...
    assert "Building wheels or sdists for sanremes-agent is not supported" in result.stderr


@pytest.mark.parametrize(
    ("kind", "artifact_glob"),
    [("sdist", "sanremes_agent-*.tar.gz"), ("wheel", "sanremes_agent-*.whl")],
)
