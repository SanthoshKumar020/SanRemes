...
Browser automation is optional. The installer should not leave SanRemes
...
def test_ensure_browser_no_longer_npm_installs_agent_browser() -> None:
    ...
    `sanremes update`'s npx cache warm or an actual browser-tool call's lazy
...
def test_ensure_browser_no_longer_references_agent_browser_binary_path() -> None:
    ...
    assert "$SANREMES_HOME/node/bin/agent-browser" not in body
