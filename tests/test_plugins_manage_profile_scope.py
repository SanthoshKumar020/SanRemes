...
``set_sanremes_home_override`` / ``reset_sanremes_home_override``. Because
``_plugins_dir()`` keys off ``get_sanremes_home()``, the list action must then
...
def test_plugins_manage_profile_reads_that_profiles_dir(tmp_path, monkeypatch):
    ...
    import sanremes_cli.profiles as profiles
