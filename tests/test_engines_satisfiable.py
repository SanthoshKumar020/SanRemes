...
`npm ci` / `npm install` — the installer's workspace step, `sanremes update`'s
...
`sanremes update` left installs in a mixed state. These tests encode the
...
class TestEnginesAreSatisfiable:
    ...
    def test_managed_node_bundles_an_npm_the_engines_accept(self):
        ...
        11.10–11.16 band — fresh SanRemes-managed installs then die at
        ...
        assert _satisfies_range(stock_npm, npm_range), (
            ...
            "SanRemes-managed install cannot run npm ci."
        )
