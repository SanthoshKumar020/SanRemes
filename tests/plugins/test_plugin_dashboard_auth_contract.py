...
plugin SDK (``window.__SANREMES_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
``authedFetch`` (uploads / blob downloads), and ``buildWsUrl`` /
``buildWsAuthParam`` (WebSockets). These handle BOTH dashboard auth modes —
loopback (``X-SanRemes-Session-Token`` header) and gated OAuth
(``sanremes_session_at`` cookie / single-use ``?ticket=``).

Plugins that hand-roll ``fetch`` / ``WebSocket`` and read
``window.__SANREMES_SESSION_TOKEN__`` directly send an empty token in gated mode
...
_FORBIDDEN = "__SANREMES_SESSION_TOKEN__"
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + sanremes-achievements are bundled today; assert at least one is
