...
_EXPECTED_USER_AGENT = f"openviking-memory-sanremes/{_SANREMES_VERSION}"
...
def test_openviking_provider_config_loader_uses_readonly_config(monkeypatch):
    ...
    config = openviking_module._load_sanremes_openviking_config()
...
""",
    ...
    settings = openviking_module._resolve_connection_settings(
        openviking_module._load_sanremes_openviking_config()
    )
...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(tmp_path, monkeypatch):
    ...
    from sanremes_cli import memory_setup
    ...
    OpenVikingMemoryProvider().post_setup(str(sanremes_home), config)
...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit SanRemes's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the SanRemes
    venv instead of its own, and on Windows locks SanRemes venv DLLs so the
    venv cannot be rebuilt during `sanremes update`.
    """
    popen_calls = []
    ...
    monkeypatch.setattr(openviking_module.shutil, "which", lambda name: "/usr/local/bin/openviking-server")
    monkeypatch.setattr(openviking_module.subprocess, "Popen", fake_popen)
    monkeypatch.setenv("PYTHONPATH", "/opt/sanremes/.venv/Lib/site-packages")
    monkeypatch.setenv("SANREMES_PROFILE", "test-profile")
    ...
    assert child_env.get("SANREMES_PROFILE") == "test-profile"
...
def test_runtime_does_not_autostart_when_local_server_reports_unhealthy(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; SanRemes will retry on a later access "
...
def test_tool_add_resource_rejects_sanremes_credential_file_upload(tmp_path, monkeypatch):
    import agent.file_safety as fs

    sanremes_home = tmp_path / "sanremes_home"
    sanremes_home.mkdir()
    auth_json = sanremes_home / "auth.json"
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "sanremes"
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (sanremes-agent#28296)
# ---------------------------------------------------------------------------
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "sanremes"},
        ]
    }]
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for sanremes-agent#28296 review (M3): once a session is marked
...
def test_concurrent_providers_claim_unlocked_pending_owner_once(
    ...
    for provider in providers:
        ...
        provider._sanremes_home = str(tmp_path)
...
def test_is_available_false_without_any_endpoint(monkeypatch):
    _clear_openviking_env(monkeypatch)
    monkeypatch.setattr(
        openviking_module, "_load_sanremes_openviking_config", lambda: {}
    )
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. SanRemes and
