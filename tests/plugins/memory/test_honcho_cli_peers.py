"""Regression tests for #76414: `sanremes honcho peers` showed "(not set)"
for every non-default profile.

_all_profile_host_configs() built the per-profile host key inline as
f"{HOST}.{profile}" ("sanremes.work") while every other reader/writer —
profile_host_key(), resolve_active_host(), honcho status/enable/sync and
the runtime plugin — uses the underscore form ("sanremes_work"). The lookup
...
a real honcho.json (temp SANREMES_HOME, no network).
"""
import io
import json
import sys
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        "peerName": "alice",
        "hosts": {
            "sanremes": {"peerName": "alice", "aiPeer": "sanremes"},
            "sanremes_work": {"peerName": "alice", "aiPeer": "sanremes"},
            "sanremes_my_profile": {"peerName": "bob", "aiPeer": "sanremes"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.profiles.list_profiles",
        ...
        assert host == "sanremes_work"  # not "sanremes.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'sanremes_my_profile';
    ...
    def test_legacy_dot_form_host_key_still_readable(self, honcho_home, monkeypatch):
        ...
        ("sanremes.work") must keep working — the README promises those keys
        ...
        del cfg["hosts"]["sanremes_work"]
        cfg["hosts"]["sanremes.work"] = {"peerName": "carol", "aiPeer": "sanremes"}
        path.write_text(json.dumps(cfg))
        monkeypatch.setattr(
            "sanremes_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
...
class TestCmdPeers:
    def test_peers_shows_populated_identity_not_host_key_leak(
        ...
        raw malformed key 'sanremes.work' (or '(not set)')."""
        monkeypatch.setattr(
            "sanremes_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
        )
        out = _peers_output(SimpleNamespace())
        assert "sanremes.work" not in out
        ...
        assert "alice" in work_line and "sanremes" in work_line
    ...
    def test_peers_falls_back_cleanly_when_block_missing(
        ...
        out = _peers_output(SimpleNamespace())
        assert "sanremes.new" not in out  # well-formed key, no dot-form leak
