...
(``SANREMES_DISABLE_LAZY_INSTALLS=1``) and lazy installs are redirected to a
writable durable target (``SANREMES_LAZY_INSTALL_TARGET``). honcho/hindsight
...
class TestSupermemoryEnsureCalled:
    def test_client_construction_calls_ensure(self, monkeypatch):
        ...
        _SupermemoryClient(api_key="k", timeout=5.0, container_tag="sanremes")
...
#
# This is the exact hosted-Fly condition: SANREMES_DISABLE_LAZY_INSTALLS=1 seals
# the venv, but SANREMES_LAZY_INSTALL_TARGET redirects installs to a writable
...
class TestSealedVenvDurableTarget:
    ...
    def test_ensure_installs_into_durable_target_on_sealed_venv(
        ...
        monkeypatch.setenv("SANREMES_LAZY_INSTALL_TARGET", str(tmp_path / "lazy"))
        ...
        def fake_install(specs, **kw):
            ...
            captured["target_env"] = os.environ.get("SANREMES_LAZY_INSTALL_TARGET")
    ...
    def test_sealed_venv_without_target_blocks(self, feature, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_DISABLE_LAZY_INSTALLS", "1")
        monkeypatch.delenv("SANREMES_LAZY_INSTALL_TARGET", raising=False)
        monkeypatch.setattr(
            "sanremes_cli.config.load_config",
            lambda: {"security": {"allow_lazy_installs": True}},
        )
