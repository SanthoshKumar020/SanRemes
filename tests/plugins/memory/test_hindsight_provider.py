...
from sanremes_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_sanremes_home", lambda: tmp_path
    )

    provider = HindsightMemoryProvider()
    provider.initialize(session_id="test-session", sanremes_home=str(tmp_path), platform="cli")
    return provider
...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", sanremes_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:sanremes-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:sanremes-agent",
    ]
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between SanRemes Agent and the User"
    ...
    def test_custom_config_values(self, provider_with_config):
        ...
        assert p._retain_source == "sanremes"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight.get_sanremes_home", lambda: sanremes_home)

        save_config = MagicMock()
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("sanremes_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        monkeypatch.setattr("sanremes_cli.config.save_config", save_config)

        provider = HindsightMemoryProvider()
        provider.post_setup(str(sanremes_home), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (sanremes_home / ".env").exists()
        assert not (sanremes_home / "hindsight" / "config.json").exists()
        assert not (user_home / ".hindsight" / "profiles" / "sanremes.env").exists()
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("sanremes_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(sanremes_home), {"memory": {}})
        ...
        profile_env = user_home / ".hindsight" / "profiles" / "sanremes.env"
...
class TestToolHandlers:
    ...
    def test_retain_defaults_item_timestamp_when_no_occurred_at(self, provider, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight._sanremes_now", lambda: event_time)
        result = json.loads(provider.handle_tool_call(
            "hindsight_retain", {"content": "user likes dark mode"}
        ))
        assert result["result"] == "Memory stored successfully."
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config, monkeypatch):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between SanRemes Agent and the User"
        ...
        assert item["metadata"]["source"] == "sanremes"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_sanremes_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", sanremes_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", sanremes_home=str(tmp_path), platform="cli")
...
class TestRetainIndicator:
    ...
    def test_status_callback_wired_from_initialize(self, tmp_path, monkeypatch):
        ...
        p.initialize(session_id="s", sanremes_home=str(tmp_path), status_callback=cb)
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("sanremes") == "sanremes"
    ...
    def test_resolve_empty_template_uses_fallback(self):
        result = _resolve_bank_id_template(
            "", fallback="sanremes", profile="coder"
        )
        assert result == "sanremes"


    def test_resolve_sanitizes_placeholder_values(self):
        result = _resolve_bank_id_template(
            "user-{user}", fallback="sanremes",
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "sanremes-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="sanremes",
        )
        assert p._bank_id == "sanremes-coder"
        assert p._bank_id_template == "sanremes-{profile}"
...
class TestAvailability:
    def test_available_with_api_key(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_sanremes_home",
...
class TestPostSetupEnvEncoding:
    def _run_cloud_post_setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.memory_setup._curses_select",
                            lambda *a, **kw: 0)  # cloud mode
        monkeypatch.setattr("sanremes_cli.config.save_config", lambda c: None)
...
class TestClientAutoUpgradeRoutesThroughLazyDeps:
    ...
    def _init_with_outdated_client(self, tmp_path, monkeypatch, outcome):
        ...
        provider = HindsightMemoryProvider()
        provider.initialize(session_id="s", sanremes_home=str(tmp_path), platform="cli")
        return calls
