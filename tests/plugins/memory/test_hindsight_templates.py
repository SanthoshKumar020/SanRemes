...
_CATALOG = {
    "templates": [
        {"id": "conversation", "name": "Conversation", "integrations": ["litellm", "sanremes"],
        ...
        {"id": "sanremes-gateway-bot", "name": "Gateway Bot", "integrations": ["sanremes"],
         "manifest_file": "templates/sanremes-gateway-bot.json"},
    ]
}


def test_fetch_sanremes_templates_filters_to_sanremes(monkeypatch):
    monkeypatch.setattr(tpl, "_get_json", lambda url: _CATALOG)
    entries = tpl.fetch_sanremes_templates("https://example/templates.json")
    ids = [e["id"] for e in entries]
    assert ids == ["conversation", "sanremes-gateway-bot"]  # coding-agent excluded
...
def test_fetch_manifest_resolves_relative_url(monkeypatch):
    ...
    assert seen["url"] == "https://raw.example/data/templates/sanremes-gateway-bot.json"
...
def test_apply_template_posts_to_import_endpoint(monkeypatch):
    ...
    tpl.apply_template("https://api.hindsight.vectorize.io/", "sanremes", "hsk_abc", {"version": "1"})

    assert captured["url"] == "https://api.hindsight.vectorize.io/v1/default/banks/sanremes/import"
...
def test_apply_template_omits_auth_when_no_key(monkeypatch):
    ...
    monkeypatch.setattr(tpl, "open_credentialed_url", _fake_open)
    tpl.apply_template("http://localhost:8888", "sanremes", None, {"version": "1"})
...
def test_run_template_step_applies_selected(monkeypatch):
    monkeypatch.setattr(tpl, "fetch_sanremes_templates", lambda url=None: [
        {"id": "sanremes-gateway-bot", "name": "Gateway Bot", "manifest_file": "templates/x.json"},
    ])
    ...
    result = tpl.run_template_step(
        api_url="https://api", bank_id="sanremes", api_key="k",
        select=_select_returning(0), cancelled=-1, log=lambda *_: None,
    )
    assert result == "sanremes-gateway-bot"
    assert applied["bank"] == "sanremes"


def test_run_template_step_blank_selection_skips(monkeypatch):
    entries = [{"id": "sanremes-gateway-bot", "name": "Gateway Bot", "manifest_file": "templates/x.json"}]
    monkeypatch.setattr(tpl, "fetch_sanremes_templates", lambda url=None: entries)
...
def test_run_template_step_swallows_fetch_errors(monkeypatch):
    ...
    monkeypatch.setattr(tpl, "fetch_sanremes_templates", _boom)
    # must not raise
...
def test_run_template_step_swallows_apply_errors(monkeypatch):
    ...
    logs = []
    result = tpl.run_template_step(
        api_url="https://api", bank_id="sanremes", api_key=None,
...
def test_probe_existing_customization_true_when_bank_has_config(monkeypatch):
    ...
    assert tpl.probe_existing_customization("https://api", "sanremes", "k") is True


def test_probe_existing_customization_false_when_empty(monkeypatch):
    monkeypatch.setattr(tpl, "open_credentialed_url",
                        _fake_open({"version": "1"}))
    assert tpl.probe_existing_customization("https://api", "sanremes", "k") is False
