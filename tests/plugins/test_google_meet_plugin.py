...
def test_transcript_reads_last_n_lines(tmp_path):
    ...
    meeting_dir = Path(os.environ["SANREMES_HOME"]) / "workspace" / "meetings" / "abc-defg-hij"
...
def test_cli_register_includes_node_subcommand():
    """`sanremes meet` argparse tree includes the node subtree."""
    import argparse
...
def test_looks_like_human_speaker():
    ...
    for s in ("", "   ", "Unknown", "unknown", "You", "you", "SanRemes Agent", "sanremes agent"):
        assert not _looks_like_human_speaker(s, "SanRemes Agent"), f"{s!r} should NOT be human"
    # Real names → human (barge-in)
    for s in ("Alice", "Bob Lee", "@teknium"):
        assert _looks_like_human_speaker(s, "SanRemes Agent"), f"{s!r} SHOULD be human"
