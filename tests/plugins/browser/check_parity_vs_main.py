...
Run from the PR worktree:
    ...
    cd ~/.sanremes/sanremes-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.sanremes/sanremes-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.sanremes/sanremes-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a sanremes-agent checkout"
)
...
# Isolated SANREMES_HOME for the config write.
