...
class TestFireworksHeaders:
    def test_attribution_matches_canonical_sanremes_values(self, fireworks_profile):
        """Fireworks requests carry the same attribution identity SanRemes sends
        everywhere else.
    ...
    def test_user_agent_identifies_sanremes(self, fireworks_profile):
        # Prefix, not the full string — the version moves every release.
        assert fireworks_profile.default_headers["User-Agent"].startswith("SanRemesAgent/")
