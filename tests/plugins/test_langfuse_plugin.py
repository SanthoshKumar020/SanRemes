...
class TestManifest:
    ...
    def test_manifest_fields(self):
        ...
        # Required env vars are the user-facing SANREMES_ prefixed keys.
        assert "SANREMES_LANGFUSE_PUBLIC_KEY" in data["requires_env"]
        assert "SANREMES_LANGFUSE_SECRET_KEY" in data["requires_env"]
...
class TestDiscovery:
    def test_plugin_is_discovered_as_standalone_opt_in(self, tmp_path, monkeypatch):
        ...
        # Isolated SANREMES_HOME so we don't read the developer's config.yaml.
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_returns_none_without_credentials(self, monkeypatch):
        for k in (
            "SANREMES_LANGFUSE_PUBLIC_KEY", "SANREMES_LANGFUSE_SECRET_KEY",
    ...
    def test_get_langfuse_caches_failure_no_config_load(self, monkeypatch):
        ...
        def tracking_get(key, default=None):
            if key.startswith(("SANREMES_LANGFUSE_", "LANGFUSE_")):
                called["n"] += 1
...
# SANREMES_LANGFUSE_PUBLIC_KEY / SECRET_KEY at a template value like
...
# SanRemes logs.  The fix in `_get_langfuse()` validates the documented
...
class TestPlaceholderKeyDetection:
    ...
    def test_validate_langfuse_key_accepts_documented_prefix(self, monkeypatch):
        ...
        assert plugin._validate_langfuse_key(
            "SANREMES_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz"
        ) is None
        assert plugin._validate_langfuse_key(
            "SANREMES_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz"
        ) is None
    ...
    def test_placeholder_public_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("SANREMES_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("SANREMES_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "SANREMES_LANGFUSE_PUBLIC_KEY" in text
    ...
    def test_placeholder_secret_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("SANREMES_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz")
        monkeypatch.setenv("SANREMES_LANGFUSE_SECRET_KEY", "test-key")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "SANREMES_LANGFUSE_SECRET_KEY" in text
    ...
    def test_both_placeholders_one_warning_with_both_keys(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("SANREMES_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("SANREMES_LANGFUSE_SECRET_KEY", "placeholder")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
...
class TestRequestMessageCoercion:
    ...
    def test_messages_for_langfuse_includes_anthropic_system_param(self):
        ...
        out = mod._messages_for_langfuse_input(
            request_messages=[{"role": "user", "content": "hi"}],
            system_prompt="You are SanRemes.",
        )
        assert out[0]["role"] == "system"
        assert out[0]["content"] == "You are SanRemes."
...
# ---------------------------------------------------------------------------
# Capture modes: metadata | sanitized | full  (SANREMES_LANGFUSE_CAPTURE)
# ---------------------------------------------------------------------------
...
class TestCaptureModes:
    ...
    def test_default_mode_is_sanitized(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.delenv("SANREMES_LANGFUSE_CAPTURE", raising=False)
        assert mod._capture_mode() == "sanitized"

    def test_invalid_mode_falls_back_and_warns_once(self, monkeypatch, caplog):
        mod = self._fresh_plugin()
        monkeypatch.setenv("SANREMES_LANGFUSE_CAPTURE", "everything")
        with caplog.at_level(logging.WARNING):
            assert mod._capture_mode() == "sanitized"
            assert mod._capture_mode() == "sanitized"
        warnings = [r for r in caplog.records if "SANREMES_LANGFUSE_CAPTURE" in r.getMessage()]
        assert len(warnings) == 1

    def test_metadata_mode_omits_content(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.setenv("SANREMES_LANGFUSE_CAPTURE", "metadata")
    ...
    def test_sanitized_mode_redacts_secrets(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.setenv("SANREMES_LANGFUSE_CAPTURE", "sanitized")
        samples = {
    ...
    def test_full_mode_keeps_secret_shaped_text(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.setenv("SANREMES_LANGFUSE_CAPTURE", "full")
...
class TestAtexitFinalization(TestTurnTraceIsolation):
    """Short-lived processes (kanban workers, `sanremes chat -q`, cron) can exit
    ...
    def test_atexit_hook_is_registered_on_client_init(self, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_LANGFUSE_PUBLIC_KEY", "pk-lf-0123456789abcdef")
        monkeypatch.setenv("SANREMES_LANGFUSE_SECRET_KEY", "sk-lf-0123456789abcdef")
...
class TestSystemPromptInGenerationInput:
    ...
    and the Responses/Codex API (``instructions``).  SanRemes forwards it to
    ...
    def test_string_system_prompt_prepended(self, monkeypatch):
        ...
        assert captured["input"][0]["content"] == "You are SanRemes."
    ...
    def test_no_duplicate_when_messages_already_carry_system(self, monkeypatch):
        ...
        self._fire(
            mod,
            request_messages=[
                {"role": "system", "content": "You are SanRemes."},
                {"role": "user", "content": "hi"},
            ],
            system_prompt="You are SanRemes.",
        )
    ...
    def test_metadata_records_chars(self, monkeypatch):
        ...
        assert captured["metadata"]["system_prompt_chars"] == len("You are SanRemes.")
...
class TestSystemPromptCrossesHookBoundary:
    ...
    no SanRemes internals mocked (only the Langfuse client is faked)."""
