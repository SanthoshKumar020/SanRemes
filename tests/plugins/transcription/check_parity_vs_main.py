...
the legacy ``SANREMES_LOCAL_STT_COMMAND`` escape hatch exists) or this PR's
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "transcription_tools.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "sanremes-agent-main"
...
assert (PR_DIR / "tools" / "transcription_tools.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a sanremes-agent checkout"
)
...
# Isolated SANREMES_HOME so the config write is hermetic.
home = tempfile.mkdtemp()
os.environ["SANREMES_HOME"] = home
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'sanremes-agent-main' checkout pinned to "
