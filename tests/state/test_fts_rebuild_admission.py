...
Desktop's ``sanremes serve`` backend, CLI sessions, the TUI slash worker). Two
...
sanremes_state_common) and FAILS CLOSED: a process that cannot acquire the
...
import sanremes_state_common
from sanremes_state import FTS_STALE_KEY, SessionDB, _FTS_TRIGGERS
...
def fast_timeout(monkeypatch):
    monkeypatch.setattr(
        sanremes_state_common, "_FTS_REBUILD_LOCK_TIMEOUT_SECONDS", 0.5
    )
...
class TestRebuildFtsAdmission:
    ...
    def test_rebuild_waits_out_a_short_holder(self, db, monkeypatch):
        ...
        monkeypatch.setattr(
            sanremes_state_common, "_FTS_REBUILD_LOCK_TIMEOUT_SECONDS", 10.0
        )
    ...
    def test_admission_yields_true_for_pathless_db(self):
        ...
        with sanremes_state_common.fts_rebuild_admission(None) as admitted:
