...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m sanremes_cli.main serve --port 0"),
    ...
    def test_foreign_holder_detection_includes_deleted_wal(
        ...
        monkeypatch.setattr(sanremes_state, "psutil", FakePsutil)
        monkeypatch.setattr(sanremes_state, "_IS_WINDOWS", False)
        monkeypatch.setattr(sanremes_state.os, "getpid", lambda: 111)
        # Force the macOS/psutil path even on Linux test runners
        monkeypatch.setattr(sanremes_state.sys, "platform", "darwin")
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(sanremes_state, "_IS_WINDOWS", False)
        monkeypatch.setattr(sanremes_state.os, "getpid", lambda: 111)
        monkeypatch.setattr(sanremes_state.sys, "platform", "linux")
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(sanremes_state.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(sanremes_state.os, "readlink", _readlink)
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a SanRemes process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like SanRemes
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(b"python3\x00sanremes_cli.main\x00chat\x00")
        ...
        def _fake_cmdline(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(sanremes_state, "_read_proc_cmdline", _fake_cmdline)

        holders = db._foreign_state_db_holders()
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "sanremes_cli.main" in holders[0][1]
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(sanremes_state_schema.time, "time", lambda: 120.0)
