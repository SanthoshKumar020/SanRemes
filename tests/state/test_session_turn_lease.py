...
from sanremes_state import SessionDB, SessionTurnLeaseLostError
    ...
    def flaky_acquire(*args, **kwargs):
        ...
        if attempts["n"] == 1:
            raise sqlite3.OperationalError(
                "database is locked (another SanRemes process held the "
...
def test_non_expired_turn_lease_from_dead_pid_is_reclaimed(
    ...
    monkeypatch.setattr(
        sanremes_state, "psutil", SimpleNamespace(pid_exists=pid_exists)
    )
