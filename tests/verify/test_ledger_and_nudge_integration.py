...
- ``sanremes verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``sanremes verify --json`` when the workspace
...
from sanremes_cli.verify_cmd import run_verify_command
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        sanremes_dir = project / ".sanremes"
        sanremes_dir.mkdir()
        (sanremes_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(sanremes_home):
    project = _workspace(sanremes_home)
    ...
    assert status["evidence"]["canonical_command"] == "sanremes verify"


def test_record_verify_run_records_failure(sanremes_home):
    project = _workspace(sanremes_home)
...
def test_cli_passing_run_writes_ledger_evidence(sanremes_home, capsys):
    project = _workspace(sanremes_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(sanremes_home, capsys):
    project = _workspace(sanremes_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(sanremes_home, capsys):
...
def test_cli_run_uses_sanremes_session_id_env(sanremes_home, capsys, monkeypatch):
    monkeypatch.setenv("SANREMES_SESSION_ID", "sess-42")
    project = _workspace(sanremes_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> sanremes verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(sanremes_home, capsys):
    project = _workspace(sanremes_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `sanremes verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_sanremes_verify_when_recipe_has_start(sanremes_home):
    project = _workspace(sanremes_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "sanremes verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_sanremes_verify_when_manifest_exists(sanremes_home):
    # No start script, but a saved .sanremes/environment.json qualifies.
    project = _workspace(
        sanremes_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(sanremes_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(sanremes_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "sanremes verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(sanremes_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(sanremes_home, capsys):
    project = _workspace(sanremes_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(sanremes_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(sanremes_home, capsys):
    project = _workspace(sanremes_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
