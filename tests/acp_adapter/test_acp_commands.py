...
from acp_adapter.server import SanRemesACPAgent
...
class FakeAgent:
    def __init__(self):
        ...
        self.enabled_toolsets = ["sanremes-acp"]
...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "sanremes_cli.config",
        mod("sanremes_cli.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,
