"""Tests for sanremes_constants module."""
...
import sanremes_constants
from sanremes_constants import (
    VALID_REASONING_EFFORTS,
    agent_browser_runnable,
    find_sanremes_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_sanremes_root,
    get_sanremes_dir,
    get_sanremes_home,
    get_process_sanremes_home,
    heal_sanremes_managed_node,
    sanremes_managed_node_tree_present,
    iter_sanremes_node_dirs,
    is_container,
    node_tool_runnable,
    ...
    with_sanremes_node_path,
)


class TestGetDefaultSanRemesRoot:
    """Tests for get_default_sanremes_root() — Docker/custom deployment awareness."""

    def test_no_sanremes_home_returns_native(self, tmp_path, monkeypatch):
        """When SANREMES_HOME is not set, returns ~/.sanremes."""
        monkeypatch.delenv("SANREMES_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_sanremes_root() == tmp_path / ".sanremes"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (SANREMES_HOME=<root>/profiles/<name>),
        returns the Docker root, not the profile dir."""
        docker_root = tmp_path / "opt" / "data"
        profile = docker_root / "profiles" / "coder"
        profile.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SANREMES_HOME", str(profile))
        assert get_default_sanremes_root() == docker_root

    @pytest.mark.windows_only
    def test_no_sanremes_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\sanremes, not ~/.sanremes."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("SANREMES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")

        assert get_default_sanremes_root() == local_appdata / "sanremes"

    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; SANREMES_HOME / home changes invalidate.

        get_default_sanremes_root() resolves SANREMES_HOME against the native
        ...
        (native home, SANREMES_HOME) compared for free each call.
        """
        # SANREMES_HOME set to a Docker-profile path: every call resolves the
        ...
        # _get_platform_default_sanremes_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            sanremes_constants, "_default_sanremes_root_memo", None, raising=False
        )

        first = get_default_sanremes_root()
        ...
        # SANREMES_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("SANREMES_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_sanremes_root() == docker_root
        assert resolve_calls["n"] > before, (
            "SANREMES_HOME change must force a fresh resolution"
        )
...
class TestGetSanRemesHome:
    """Tests for get_sanremes_home() platform-aware fallback."""

    @pytest.mark.windows_only
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When SANREMES_HOME is unset on Windows, use %LOCALAPPDATA%\\sanremes."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("SANREMES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(sanremes_constants, "_profile_fallback_warned", False)

        assert get_sanremes_home() == local_appdata / "sanremes"


class TestGetProcessSanRemesHome:
    """Tests for get_process_sanremes_home() — process launch scope.
    ...
    via set_sanremes_home_override().
    """
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_sanremes_home() == home




class TestSanRemesManagedNode:
    @pytest.mark.windows_only
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_sanremes_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_HOME", str(home))
        monkeypatch.setattr(sanremes_constants, "node_tool_runnable", lambda path: True)

        assert find_sanremes_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sanremes_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(sanremes_constants, "heal_sanremes_managed_node", lambda: False)
        monkeypatch.setattr(
            sanremes_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert sanremes_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken SanRemes-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sanremes_constants, "heal_sanremes_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = sanremes_constants._SANREMES_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = sanremes_constants.find_sanremes_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert sanremes_constants.find_sanremes_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert sanremes_constants.find_sanremes_node_executable("node") == str(node)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(sanremes_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(sanremes_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from sanremes_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".sanremes"
    ...
    def test_install_tree_skipped(self, monkeypatch):
        ...
        Regression test for #93050: secure_parent_dir() chmod'd /opt/sanremes to
        0700 because it has 3 path parts and passed the ``< 3`` guard, locking
        out UID 10000 (sanremes user) from traversing the install dir.
        """
        install_root = Path(sanremes_constants.__file__).resolve().parent

        # Directly under the install root (e.g. /opt/sanremes/auth.json)
    ...
    def test_install_tree_siblings_still_hardened(self, monkeypatch):
        ...
        # Prefix-named sibling of the install root (/opt/sanremes-data/...).
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import sanremes_cli._subprocess_compat as subprocess_compat
...
class TestGetSanRemesDir:
    """Tests for ``get_sanremes_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_sanremes_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_sanremes_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert sanremes_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert sanremes_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert sanremes_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert sanremes_constants.windows_path_to_wsl("/home/alex") is None
        assert sanremes_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(sanremes_constants, "is_wsl", lambda: True)
        assert sanremes_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert sanremes_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert sanremes_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def _install_fake_psutil(self, monkeypatch, procs):
        ...
        monkeypatch.setattr(sanremes_constants.sys, "platform", "win32")
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(sanremes_constants.sys, "platform", "darwin")
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path))
        assert sanremes_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert sanremes_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("SANREMES_HOME", str(home))
        monkeypatch.setenv(
            "SANREMES_NODE_TARGET_MAJOR",
            str(sanremes_constants._SANREMES_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(sanremes_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            sanremes_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            sanremes_constants,
        ...
        monkeypatch.setattr(
            sanremes_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(sanremes_constants._SANREMES_NODE_TARGET_MAJOR)
        ...
        result = sanremes_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sanremes_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_sanremes_managed_node() is False
        assert sanremes_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_sanremes_managed_node() is False
        assert sanremes_constants._managed_node_heal_attempted is True
