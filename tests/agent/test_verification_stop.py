...
def clear_verify_env(monkeypatch):
    ...
    set SANREMES_SESSION_SOURCE and the gateway sets SANREMES_SESSION_PLATFORM.
    """
    for var in (
        "SANREMES_VERIFY_ON_STOP",
        "SANREMES_PLATFORM",
...
def test_verify_on_stop_env_can_enable(clear_verify_env):
    ...
    clear_verify_env.setenv("SANREMES_VERIFY_ON_STOP", "1")
    clear_verify_env.setenv("SANREMES_SESSION_PLATFORM", "telegram")
...
def test_verify_on_stop_auto_on_for_interactive_surfaces(clear_verify_env, source):
    ...
    clear_verify_env.setenv("SANREMES_SESSION_SOURCE", source)
...
def test_verify_on_stop_default_path_through_load_config(tmp_path, clear_verify_env):
    ...
    clear_verify_env.setenv("SANREMES_HOME", str(tmp_path / ".sanremes"))
    ...
    clear_verify_env.setenv("SANREMES_SESSION_SOURCE", "cli")
...
def test_ad_hoc_pass_satisfies_no_suite_stop_loop(tmp_path, monkeypatch):
    ...
    script = Path(tempfile.gettempdir()) / f"sanremes-ad-hoc-stop-{tmp_path.name}.py"
