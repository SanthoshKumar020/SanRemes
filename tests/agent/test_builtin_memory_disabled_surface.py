...
``get_tool_definitions``) against a temp ``SANREMES_HOME``, not mocks.
"""
...
class TestBuiltinMemoryToolAvailability:
    def test_tool_hidden_when_both_stores_disabled(self, sanremes_home):
        _write_memory_config(
            sanremes_home, memory_enabled=False, user_profile_enabled=False
        )
        assert "memory" not in _memory_tool_names()

    def test_tool_present_when_only_user_profile_enabled(self, sanremes_home):
        _write_memory_config(
            sanremes_home, memory_enabled=False, user_profile_enabled=True
        )
    ...
    def test_tool_present_when_only_memory_enabled(self, sanremes_home):
        _write_memory_config(
            sanremes_home, memory_enabled=True, user_profile_enabled=False
        )
        definition = _memory_tool_definition()
    ...
    def test_tool_present_by_default(self, sanremes_home):
    ...
    def test_config_flip_updates_tool_without_manual_cache_clear(self, sanremes_home):
        _write_memory_config(
            sanremes_home, memory_enabled=False, user_profile_enabled=False
        )
        assert "memory" not in _memory_tool_names()
    ...
    def test_unreadable_config_fails_open(self, sanremes_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.config.load_config_readonly", _boom, raising=False
        )
...
class TestIndependentStoreWriteGates:
    ...
    def test_user_profile_only_rejects_memory_write(self, sanremes_home):
    ...
    def test_memory_only_rejects_user_profile_write(self, sanremes_home):
        from tools.memory_tool import memory_tool
    ...
    def test_staged_write_cannot_bypass_target_gate(self, sanremes_home):
    ...
    def test_invalid_target_error_is_bounded_and_carries_recovery_hint(self, sanremes_home):
