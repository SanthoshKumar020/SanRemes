...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated SANREMES_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="sanremes_e2e_47967_")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("sanremes_"):
            del sys.modules[mod]
