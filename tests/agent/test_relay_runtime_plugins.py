...
def test_foreign_active_plugin_configuration_is_left_unchanged(
    ...
    try:
        ...
        assert relay.events == []
        assert "already active outside SanRemes native ownership" in caplog.text
...
def test_legacy_exporter_env_without_plugins_toml_warns_and_stays_disabled(
    ...
    monkeypatch.delenv(relay_runtime.RELAY_PLUGINS_CONFIG_ENV, raising=False)
    monkeypatch.setenv("SANREMES_NEMO_RELAY_ATOF_ENABLED", "1")
    monkeypatch.setenv("SANREMES_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S", "30")
    relay = _FakeRelay()
    ...
    try:
        ...
        assert relay.events == []
        assert "no SANREMES_NEMO_RELAY_PLUGINS_TOML was provided" in caplog.text
        assert "SANREMES_NEMO_RELAY_ATOF_ENABLED" in caplog.text
        assert "SANREMES_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S" in caplog.text
    finally:
        host.shutdown()
...
def test_initialization_failure_is_fail_open(explicit_static_config, caplog):
    ...
    try:
        ...
        assert "SanRemes Relay plugin initialization failed" in caplog.text
    finally:
        host.shutdown()
...
def test_session_close_does_not_flush_during_concurrent_managed_publication(
    ...
    event_loop_thread = threading.Thread(
        ...
        name="sanremes-relay-session-close-regression",
        daemon=True,
    )
...
""".strip(),
    ...
    try:
        ...
        assert relay.events == []
        assert "SanRemes [[dynamic_plugins]] records are unsupported" in caplog.text
...
agent_name = "SanRemes Native Test"
agent_version = "test"
""".strip(),
        encoding="utf-8",
    )
    ...
    try:
        for profile in ("profile-a", "profile-b"):
            ...
            monkeypatch.setenv("SANREMES_HOME", str(tmp_path / profile))
