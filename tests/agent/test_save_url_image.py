...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SANREMES_HOME under tmp_path."""
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / ".sanremes"))
    (tmp_path / ".sanremes").mkdir()

    # Force the constants/image cache helpers to re-read SANREMES_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("sanremes_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_sanremes_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SANREMES_HOME — gateway cleanup
