...
class TestKimiEffortVocabulary:
    def test_k3_maps_full_sanremes_ladder(self):
        expected = {
            "minimal": "low",
            "low": "low",
            "medium": "high",
        ...
        for sanremes_level, wire_level in expected.items():
            kw = _kimi_kwargs(
                "kimi-k3", {"enabled": True, "effort": sanremes_level}
            )
            assert kw["reasoning_effort"] == wire_level, sanremes_level
