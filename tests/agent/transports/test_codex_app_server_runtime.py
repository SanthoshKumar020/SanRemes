...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. SanRemes passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("SANREMES_HOME", "/users/alice/.sanremes/profiles/backend-worker")
        monkeypatch.setenv("SANREMES_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "SANREMES_KANBAN_DB",
            "/users/alice/.sanremes/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.sanremes/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through sanremes_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 SanRemes secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        ...
        for var in (
            ...
            "SANREMES_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
