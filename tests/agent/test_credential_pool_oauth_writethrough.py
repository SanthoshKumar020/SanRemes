...
Companion to ``tests/sanremes_cli/test_xai_oauth_writethrough.py``. That file
...
from sanremes_cli import auth as A
...
def profile_and_root(tmp_path, monkeypatch):
    ...
    only refuses the *real* user's ``$HOME/.sanremes/auth.json``; a tmp_path
...
def test_codex_pool_refresh_holds_auth_store_lock_across_post(monkeypatch, tmp_path):
    ...
    Codex refresh tokens are single-use. If two SanRemes processes both read the
