...
def test_load_review_credentials_cfg_missing_section(monkeypatch):
    monkeypatch.setattr(
        "sanremes_cli.config.load_config_readonly", lambda: {"auxiliary": {}}
    )
    assert re_mod._load_review_credentials_cfg() is None
...
def test_collect_skills_from_preloaded_prompt_and_history():
    ...
    parent.ephemeral_system_prompt = (
        ...
        '"sanremes-agent-dev" skill preloaded. Treat its instructions as '
    ...
    msgs = [
        {"role": "assistant", "content": "", "tool_calls": [
            ...
            {"function": {"name": "skill_view",
                          "arguments": '{"name": "sanremes-agent-dev", '
    ...
    assert names == ["sanremes-agent-dev", "github-pr-workflow"]
...
def test_briefing_includes_loaded_skills_instruction():
    ...
    _, context = build_review_task(snap, "", ["sanremes-agent-dev", "xitter"])
    assert "sanremes-agent-dev, xitter" in context
...
def test_start_review_threads_loaded_skills_into_context(monkeypatch):
    ...
    parent = _fake_parent()
    parent.ephemeral_system_prompt = (
        'session with the "sanremes-agent-dev" skill preloaded.'
    )
    msgs = [
    ...
    assert result["status"] == "dispatched"
    assert "sanremes-agent-dev" in built["context"]
