...
an autonomous agent reached through a client shim that cannot carry SanRemes tool
...
So: a client declaring ``SUPPORTS_SANREMES_TOOL_CALLS = False`` skips the fork with
...
def test_fork_is_skipped_when_the_provider_cannot_emit_tool_calls(caplog):
    client = MagicMock()
    client.SUPPORTS_SANREMES_TOOL_CALLS = False
...
def test_fork_is_spawned_when_the_provider_can_emit_tool_calls():
    client = MagicMock()
    client.SUPPORTS_SANREMES_TOOL_CALLS = True
...
def test_the_capability_is_read_off_the_class_too():
    ...
    class _IncapableClient:
        SUPPORTS_SANREMES_TOOL_CALLS = False
