...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_sanremes_home``,
    ``get_default_sanremes_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``SANREMES_HOME`` and no resolver mocks.
    """

    def test_real_sanremes_home_under_profiles_renders_correct_paths(
        ...
        assert get_sanremes_home() == profile_home
        assert get_default_sanremes_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active SanRemes profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """SANREMES_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".sanremes"
        ...
        assert "Active SanRemes profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "SANREMES_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "SANREMES_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_sanremes_home", lambda: Path("/sanremes"))

    expected_profile = (
        "Active SanRemes profile: default. Other profiles (if any) live "
        "under /sanremes/profiles/<name>/. Each profile has its own skills/, "
    ...
    with (
        ...
        patch("sanremes_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("sanremes_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under SANREMES_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from sanremes_cli import config as _cfgmod
