...
calls never come back as pending ``tool_calls`` (SanRemes would re-run finished
work), so two subsystems would otherwise be blind to them:
  ...
  * the skill-review nudge, whose counter only moves on SanRemes tool iterations.
...
def test_projected_rows_are_appended_and_the_nudge_ticks():
    ...
    spliced = splice_provider_projection(
        agent,
        _response(sanremes_projected_messages=_projected_rows(), sanremes_provider_tool_iterations=1),
        messages,
    )
    assert spliced == 2
...
def test_rows_are_stamped_like_every_other_live_transcript_append():
    ...
    splice_provider_projection(
        _agent(), _response(sanremes_projected_messages=_projected_rows()), messages
    )
...
def test_iterations_accumulate_across_calls():
    ...
    splice_provider_projection(agent, _response(sanremes_provider_tool_iterations=3), [])
...
def test_garbage_attributes_cannot_break_the_turn():
    ...
    assert splice_provider_projection(
        agent,
        _response(
            sanremes_projected_messages="not-a-list",
            sanremes_provider_tool_iterations="lots",
        ),
        messages,
    ) == 0
    assert messages == []
    ...
    assert splice_provider_projection(
        agent,
        _response(sanremes_projected_messages=[{"role": "tool", "content": "ok"}, "junk", None]),
        messages,
    ) == 1
...
class _FakeAgentProviderCompletions:
    ...
    def create(self, **_kwargs):
        ...
        return SimpleNamespace(
            ...
            usage=None,
            sanremes_projected_messages=self._projected,
            sanremes_provider_tool_iterations=self._iterations,
        )
