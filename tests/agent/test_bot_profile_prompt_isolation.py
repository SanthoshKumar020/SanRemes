...
thread that did not bind the SANREMES_HOME ContextVar.
...
active-profile line resolved the home via the ambient ``get_sanremes_home()``,
so an unbound build thread fell back to ``~/.sanremes`` (default) and leaked
default's full skills index + "Active SanRemes profile: default" into a bot's
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient SANREMES_HOME, on a
    ...
    def build():
        # No set_sanremes_home_override on THIS thread — ambient resolves to
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    ``get_sanremes_home()/profiles`` as the root would never match and every
    profile would misreport as \"default\". The name must derive from the
    sanremes ROOT (get_default_sanremes_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: SANREMES_HOME env points at the profile dir itself.
    monkeypatch.setenv("SANREMES_HOME", str(bot_home))
...
def test_profile_name_correct_when_ambient_is_another_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(other_home))
