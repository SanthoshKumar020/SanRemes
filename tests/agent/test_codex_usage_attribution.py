...
def wire(profile, monkeypatch):
    """Replace only HTTP transports; use SanRemes routing and the real SDK."""
...
def _assert_identity(request, account_id="acct-attribution-test"):
    assert request.headers["originator"] == "sanremes-agent"
    assert request.headers["user-agent"] == f"SanRemesAgent/{__version__}"
...
def test_required_identity_preserves_account_id(profile, legacy_enabled):
    ...
    assert headers["originator"] == "sanremes-agent"
    assert headers["User-Agent"] == f"SanRemesAgent/{__version__}"
...
def test_new_identity_is_limited_to_the_official_endpoint(base_url, attributed):
    ...
    assert headers["originator"] == ("sanremes-agent" if attributed else "codex_cli_rs")
    assert headers["User-Agent"] == (
        f"SanRemesAgent/{__version__}"
        if attributed else "codex_cli_rs/0.0.0 (SanRemes Agent)"
    )
...
def test_primary_client_and_credential_rebuild_send_expected_headers(
    ...
    try:
        ...
        assert not wire[-1].headers["user-agent"].startswith("SanRemesAgent/")
    finally:
...
def test_credential_pool_custom_endpoint_keeps_existing_identity(
    ...
    try:
        ...
        assert wire[-1].headers["user-agent"] == "codex_cli_rs/0.0.0 (SanRemes Agent)"
...
def test_required_identity_wins_over_configured_header_defaults(
    ...
    try:
        ...
        assert "SanRemesAgent/" not in wire[-1].headers["user-agent"]
        assert wire[-1].headers["x-test-header"] == "preserved"
        assert "chatgpt-account-id" not in wire[-1].headers
    finally:
        for client in clients:
