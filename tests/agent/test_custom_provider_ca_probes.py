...
* ``urllib``-based ``/models`` catalog discovery probe
  (``sanremes_cli.models.probe_api_models`` via ``_custom_provider_ssl_context``).
...
from sanremes_cli.models import _custom_provider_ssl_context
...
class TestResolveRequestsVerifyProviderScoped:
    ...
    def test_no_base_url_does_not_consult_config(self, clean_env, bundle_file):
        ...
        clean_env.setenv("SANREMES_CA_BUNDLE", bundle_file)
        probe = MagicMock(return_value=[])
        with patch("sanremes_cli.config.get_compatible_custom_providers", probe):
