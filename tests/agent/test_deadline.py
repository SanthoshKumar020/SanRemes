...
class TestResolveTimeout:
    def test_default_wins_when_nothing_configured(self, monkeypatch):
        ...
        monkeypatch.delenv("SANREMES_TEST_DEADLINE_X", raising=False)
        assert (
            resolve_timeout("a.b", default=42.0, env_var="SANREMES_TEST_DEADLINE_X")
            == 42.0
        )

    def test_env_var_beats_default(self, monkeypatch):
        monkeypatch.setattr("agent.deadline._timeouts_section", lambda: {})
        monkeypatch.setenv("SANREMES_TEST_DEADLINE_X", "17.5")
        assert (
            resolve_timeout("a.b", default=42.0, env_var="SANREMES_TEST_DEADLINE_X")
            == 17.5
        )
    ...
    def test_invalid_env_value_falls_through_to_default(self, monkeypatch):
        monkeypatch.setattr("agent.deadline._timeouts_section", lambda: {})
        monkeypatch.setenv("SANREMES_TEST_DEADLINE_X", "banana")
        assert (
            resolve_timeout("a.b", default=42.0, env_var="SANREMES_TEST_DEADLINE_X")
            == 42.0
        )
    ...
    def test_broken_config_read_never_breaks_the_protected_path(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.config.load_config_readonly", _boom)
