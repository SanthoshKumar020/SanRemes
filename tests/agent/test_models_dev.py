...
class TestProviderMapping:
    def test_all_mapped_providers_are_strings(self):
        for sanremes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
            assert isinstance(sanremes_id, str)
...
class TestMirrorUrlOverride:
    ...
    def test_mirror_url_used_when_configured(self, mock_get):
        ...
        with patch.object(md, "_disk_cache_age_seconds", return_value=None), \
             ...
             patch.object(md, "_load_etag", return_value=""), \
             patch.object(md, "_save_etag"), \
             patch("sanremes_cli.config.load_config_readonly", return_value=fake_config):
            fetch_models_dev()
...
class TestModelOverrides:
    ...
    def test_provider_key_accepts_either_id_space(self):
        """Override keyed by SanRemes id resolves for models.dev id and back."""
        overrides = {
            "copilot": {
        ...
        with self._setup_overrides(overrides):
            # Caller passes the models.dev id; config keyed by SanRemes id.
        ...
        with self._setup_overrides(overrides):
            # Caller passes the SanRemes id; config keyed by models.dev id.
