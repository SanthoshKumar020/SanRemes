"""Tests for the core Relay-managed SanRemes tool adapter."""
...
def test_request_rewrite_reaches_authorized_callback_once(relay_turn):
    ...
    relay.intercepts.register_tool_request(
        "sanremes-test-tool-request", 1, False, rewrite_request
    )
    relay.intercepts.register_tool_execution(
        "sanremes-test-tool-execution", 1, wrap_execution
    )
    try:
        ...
        relay.intercepts.deregister_tool_execution("sanremes-test-tool-execution")
        relay.intercepts.deregister_tool_request("sanremes-test-tool-request")
