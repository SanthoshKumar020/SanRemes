...
def curator_env(tmp_path, monkeypatch):
    """Isolated SANREMES_HOME + freshly reloaded curator + skill_usage modules."""
    ...
    # save_state() resolves the state path from SANREMES_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.sanremes once monkeypatch
    ...
    # thread here, while SANREMES_HOME is still pinned to this test's tmp home
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from sanremes_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `sanremes model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from sanremes_cli.main import _AUX_TASKS
    from sanremes_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. sanremes_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. sanremes_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
