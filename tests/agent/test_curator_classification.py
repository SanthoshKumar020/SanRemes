...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"sanremes-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "sanremes-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "sanremes-agent-dev"
...
# `sanremes curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `sanremes curator pin <umbrella>` in the rename block so
