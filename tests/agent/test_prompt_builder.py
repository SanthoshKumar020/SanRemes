...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.config.load_config", default_load_config)
        monkeypatch.setattr("sanremes_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("sanremes_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.config.load_config", lambda: {})
        monkeypatch.setattr("sanremes_cli.config.load_config_readonly", lambda: {})
...
class TestBuildContextFilesPrompt:
    def test_empty_dir_loads_seeded_global_soul(self, tmp_path):
        ...
        assert "SanRemes Agent" in result
    ...
    def test_sanremes_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".sanremes.md").write_text("SanRemes-first context.")
        (tmp_path / "AGENTS.override.md").write_text("Override context.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "SanRemes-first context" in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "sanremes_home"))
        sanremes_home = tmp_path / "sanremes_home"
        sanremes_home.mkdir()
        (sanremes_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
    ...
    # --- .sanremes.md / SANREMES.md discovery ---
...
# =========================================================================
# .sanremes.md helper functions
# =========================================================================


class TestFindSanRemesMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".sanremes.md").write_text("rules")
        assert _find_sanremes_md(tmp_path) == tmp_path / ".sanremes.md"



    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".sanremes.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_sanremes_md(sub) == tmp_path / ".sanremes.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .sanremes.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """SANREMES_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("SANREMES_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
