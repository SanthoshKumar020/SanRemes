...
def sanremes_home(monkeypatch):
    """Isolate SANREMES_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``SANREMES_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="sanremes-reload-skills-")
    monkeypatch.setenv("SANREMES_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "SANREMES_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_detects_removed_skill_carries_description(self, sanremes_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(sanremes_home / "skills", "demo", "soon to be gone")
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, sanremes_home):
