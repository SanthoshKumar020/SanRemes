...
class TestPerResponseRunNonceIsolation:
    ...
    SanRemes Studio group chat builds ``gc_run_<room>_<profile>_<name>_<uuid4hex>``
    for every reply and destroys it when the reply completes
    (``groupRuntimeSessionId``), so every conversation-affinity hint SanRemes
    ...
    is an identity, and SanRemes' public session API lets a client choose one
    ...
    def test_parentless_rows_resolve_to_their_own_scope(self, db):
        ...
        an owner SanRemes was never told about must never be guessed. #96811
