...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_moa_unwraps_to_aggregator(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.moa_config.resolve_moa_preset",
            lambda cfg, name: preset,
        )
        monkeypatch.setattr("sanremes_cli.config.load_config", lambda: {"moa": {}})
        monkeypatch.setattr("sanremes_cli.config.load_config_readonly", lambda: {"moa": {}})
...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp SANREMES_HOME
                    ...
                    "moa": {
                        ...
                        "presets": {
                            ...
                            "nous-mix": {
                                "enabled": True,
                                "reference_models": [
                                    {"provider": "nous", "model": "sanremes-4-70b"}
                                ],
                                "aggregator": {
                                    "provider": "nous",
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nous", "sanremes-4", "https://inference-api.nousresearch.com/v1", "max_tokens"),
        ],
    )
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``sanremes auth status`` already reports these as logged in; compression
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))
        monkeypatch.setenv("SANREMES_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``sanremes model``, no per-task override is
...
class TestOpenRouterPaidLaneGuard:
    ...
    def test_free_only_skips_paid_default_model(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("sanremes_cli.config.load_config_readonly", return_value={"auxiliary": {"free_only": True}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            client, model = _try_openrouter()
        assert client is None
        assert model is None
    ...
    def test_free_only_allows_free_model(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("sanremes_cli.config.load_config_readonly",
    ...
    def test_paid_lane_warns_once(self, monkeypatch, caplog):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("sanremes_cli.config.load_config_readonly", return_value={"auxiliary": {}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("sanremes_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_try_nous_refreshes_stale_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=pool),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("sanremes_cli.models.get_nous_recommended_aux_model", return_value=None),
        ):
...
class TestRefreshNousRecommendedModel:
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.models.get_nous_recommended_aux_model", _boom)
    ...
    def test_returns_none_when_no_distinct_alternative(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.models.get_nous_recommended_aux_model",
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through SanRemes auth / credential pool."""
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from sanremes_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("sanremes_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so SanRemes owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("sanremes_cli.config.load_config", return_value=config), patch("sanremes_cli.config.load_config_readonly", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_moa_tasks_reject_task_level_reasoning_effort(self, moa_task, caplog):
        ...
        with patch("sanremes_cli.config.load_config", return_value=config), patch("sanremes_cli.config.load_config_readonly", return_value=config), \
             caplog.at_level(logging.WARNING, logger="agent.auxiliary_client"):
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized SanRemes reasoning.

    MoA slot reasoning is carried through call_llm as a SanRemes
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/NousResearch/sanremes-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/NousResearch/sanremes-agent/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "SanRemesAgent"
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("sanremes_cli.config.load_config", return_value=fake_config), patch("sanremes_cli.config.load_config_readonly", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("sanremes_cli.config.load_config", return_value={"model": {}}), patch("sanremes_cli.config.load_config_readonly", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
...
class TestFastModelTier:
    ...
    def test_catalog_match_prefers_rolling_alias_over_pinned_id(self):
        ...
        with patch("sanremes_cli.models.fetch_models_with_pricing", return_value=catalog):
    ...
    def test_catalog_fetch_is_authenticated(self):
        ...
        with patch(
            "sanremes_cli.auth.resolve_api_key_provider_credentials",
            return_value={"api_key": "sk-test", "base_url": "https://api.example.com/v1"},
        ), patch(
            "sanremes_cli.models.fetch_models_with_pricing", return_value={}
        ) as fetch:
