...
to a generic "Your API key was rejected... run sanremes setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "sanremes setup".
    assert "sanremes portal" in source
