...
1. MULTIPLEX INVERSION (@kshitijk4poor): the messaging gateway hands every
   ...
   via the SANREMES_HOME ContextVar (copy_context into the worker). A bound
...
def test_bound_override_wins_over_shared_db_home(tmp_path, monkeypatch):
    ...
    token = set_sanremes_home_override(str(bot_home))
    try:
...
def test_full_prompt_scoped_to_bot_on_bare_thread(tmp_path, monkeypatch):
    ...
    assert "Active SanRemes profile: mybot" in prompt
    assert "Active SanRemes profile: default" not in prompt
