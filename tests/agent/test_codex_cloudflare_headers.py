...
The official Codex endpoint must receive SanRemes' own harness identity, rather
...
These tests pin:
- the required SanRemes originator
- the versioned SanRemes User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_sanremes_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("run_agent.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "sanremes-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"SanRemesAgent/{__version__}"
