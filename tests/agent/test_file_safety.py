...
class TestCacheFileReadBlocking:
    """Internal SanRemes cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        sanremes_home = tmp_path / ".sanremes"
        cache = sanremes_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._sanremes_home_path", return_value=sanremes_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal SanRemes cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        sanremes_home = tmp_path / ".sanremes"
        hub = sanremes_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_sanremes_home(self, tmp_path):
        """The env basename guard does not depend on SANREMES_HOME resolution."""
        sanremes_home = tmp_path / ".sanremes"
        sanremes_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        sanremes_home = tmp_path / ".sanremes"
        cache = sanremes_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
