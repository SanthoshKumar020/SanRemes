"""Regression tests for NousResearch/sanremes-agent#2765.
...
service that didn't inherit ``~/.sanremes/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "sanremes memory status" in msg
