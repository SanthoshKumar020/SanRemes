"""Tests for SANREMES_HOME credential-file read blocking in file_safety.

Regression for https://github.com/NousResearch/sanremes-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``SANREMES_HOME`` itself,
...
for the credential stores while leaving arbitrary ``SANREMES_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_sanremes_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "sanremes_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_sanremes_home_path", lambda: home)
    return home
...
def test_arbitrary_sanremes_home_file_not_blocked(fake_home):
    """Non-credential files inside SANREMES_HOME stay readable."""
...
def test_identically_named_sanremes_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """SanRemes-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside SANREMES_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside SANREMES_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside SANREMES_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, SANREMES_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_sanremes_home_path", lambda: profile)
