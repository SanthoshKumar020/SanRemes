...
path via ``sanremes chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp SANREMES_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point SANREMES_HOME at a temp dir and turn moa.save_traces on."""
    sanremes_home = tmp_path / ".sanremes"
    sanremes_home.mkdir()
    monkeypatch.setenv("SANREMES_HOME", str(sanremes_home))

    # save_moa_turn reads config via sanremes_cli.config.load_config; stub it to
    ...
    return sanremes_home / "moa-traces"
