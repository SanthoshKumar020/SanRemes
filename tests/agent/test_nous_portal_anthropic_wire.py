...
class TestApiModeRouting:
    ...
    def test_determine_api_mode_honors_the_model_for_nous(self):
        ...
        empty-mode path) must still land Claude on Messages — the SanRemes
        ...
        assert (
            determine_api_mode("nous", PORTAL_URL, model="sanremes-4-405b")
            == "chat_completions"
        )
...
class TestRuntimeResolution:
    ...
    def test_target_model_wins_over_the_persisted_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            rp,
            "_get_model_config",
            lambda: {"provider": "nous", "default": "sanremes-4-405b"},
        )
...
class TestClientShape:
    ...
    def test_portal_bearer_does_not_also_send_env_anthropic_api_key(
        ...
        constructor omits it. SanRemes loads that env from ~/.sanremes/.env, so
...
class TestPortalBodyFields:
    ...
    def test_portal_tags_reach_the_messages_request(self):
        from agent.portal_tags import sanremes_client_tag

        tags = self._build()["extra_body"]["tags"]

        assert "product=sanremes-agent" in tags
        assert sanremes_client_tag() in tags
    ...
    def test_non_anthropic_catalog_model_stays_on_chat_completions(self):
        ...
        with (
            patch(
                ...
                return_value=(plain, "sanremes-4-405b"),
            ),
            patch(
            ...
            client, model = resolve_provider_client("nous", "sanremes-4-405b")

        assert model == "sanremes-4-405b"
