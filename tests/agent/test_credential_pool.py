...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_unmatched_api_key_hint_rotates_without_benching_innocent_key(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "sanremes" / "auth.json").read_text())
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "sanremes" / "auth.json").read_text()
...
def test_load_pool_collapses_duplicate_env_rows_to_active_key(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "sanremes" / "auth.json").read_text())
...
def test_load_pool_persists_bitwarden_origin_metadata_without_secret(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", sentinel)
    monkeypatch.setattr(
        "sanremes_cli.env_loader.get_secret_source",
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    from sanremes_cli.auth import write_credential_pool
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://sanremes/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
    ...
    assert borrowed["source"] == "systemd://sanremes/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "sanremes" / "auth.json").read_text())["credential_pool"]["nous"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.sanremes/.env when
    ...
    # User edited ~/.sanremes/.env with the fresh key
    (sanremes_home / ".env").write_text(
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.sanremes/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (sanremes_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `sanremes setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / SanRemes PKCE
    ...
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    monkeypatch.setattr("sanremes_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_sanremes_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `sanremes_pkce` into auth.json) and later switches to the API key at
    `sanremes setup` would still have those OAuth entries dormant on disk.
    ...
    monkeypatch.setattr("sanremes_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("agent.anthropic_adapter.read_sanremes_oauth_credentials", lambda: None)
    monkeypatch.setattr("agent.anthropic_adapter.read_claude_code_credentials", lambda: None)
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `sanremes setup`.  When ANTHROPIC_TOKEN is set (and
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_sanremes_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_custom_endpoint_pool_seeds_from_config(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "sanremes" / "config.yaml"
    import yaml
...
def test_load_pool_skips_exchange_for_suppressed_copilot(tmp_path, monkeypatch):
    ...
    removed with ``sanremes auth remove copilot gh_cli``.  The gate must run
...
def test_load_pool_skips_resolve_when_all_copilot_sources_suppressed(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "sanremes"))
    from sanremes_cli.copilot_auth import COPILOT_ENV_VARS
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    monkeypatch.setattr("sanremes_cli.copilot_auth.resolve_copilot_token", _boom)
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
def test_persist_preserves_concurrent_disk_only_entry(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool
    from sanremes_cli.auth import read_credential_pool, write_credential_pool
    ...
    final = json.loads((tmp_path / "sanremes" / "auth.json").read_text())
...
def _make_anthropic_claude_code_pool(tmp_path, monkeypatch, *, access_token, refresh_token, expires_at_ms=9_999_999_999_000):
    ...
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {}})
    monkeypatch.setattr("sanremes_cli.auth.is_provider_explicitly_configured", lambda pid: pid == "anthropic")
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_sanremes_oauth_credentials",
        lambda: None,
    )
