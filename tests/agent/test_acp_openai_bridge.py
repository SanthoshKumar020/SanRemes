"""The ACP text bridge is what makes SanRemes' own tools reachable on an ACP provider.
...
def test_allowlist_forwards_only_the_named_tools():
    ...
    make SanRemes re-run finished work, so those clients forward an allowlist."""
...
def test_response_level_extras_survive_the_stream_conversion():
    """SanRemes reads provider extras off the returned object; a plain list would
    drop them and silently disable the projection on stream=True."""
    chunks = completion_to_stream_chunks(
        _completion(sanremes_projected_messages=[{"role": "tool", "content": "x"}])
    )
    assert isinstance(chunks, StreamChunks)
    assert isinstance(chunks, list)
    assert chunks.sanremes_projected_messages == [{"role": "tool", "content": "x"}]
...
def test_copilot_prompt_still_carries_the_contract_and_the_tools():
    ...
    assert "SanRemes requested model hint: gpt-5" in prompt
    assert "hi" in prompt
