...
from sanremes_state import SessionDB, _DB_PERSISTED_MARKER_KEY
...
def test_marker_constant_in_sync() -> None:
    ...
    ``sanremes_state`` and ``agent.turn_finalizer`` import the constant from
    ``agent.context_compressor`` (single source), so the only genuine drift
    surface left is ``run_agent``'s predating copy — sanremes_state cannot
