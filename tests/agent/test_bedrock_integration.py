...
Note: Tests that import ``sanremes_cli.auth`` or ``sanremes_cli.runtime_provider``
...
class TestProviderAliases:
    ...
    def test_aws_alias(self):
        from sanremes_cli.models import _PROVIDER_ALIASES
...
class TestProviderLabels:
    ...
    def test_bedrock_label(self):
        from sanremes_cli.models import _PROVIDER_LABELS
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("sanremes_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("sanremes_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("sanremes_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("sanremes_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
    ...
    def test_bedrock_openai_models_route_to_mantle_responses(self, monkeypatch):
        ...
        for model_id in BEDROCK_OPENAI_RESPONSES_MODEL_IDS:
            with patch("sanremes_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
                 patch("sanremes_cli.runtime_provider._get_model_config", return_value={
                     "provider": "bedrock",
                     "default": model_id,
                 }):
                result = resolve_runtime_provider(requested="bedrock")
...
class TestProvidersModule:
    """Verify bedrock is wired into sanremes_cli/providers.py."""

    def test_bedrock_alias_in_providers(self):
        from sanremes_cli.providers import ALIASES
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "sanremes-agent[bedrock]" not in extras["all"]
...
class TestAuxiliaryClientBedrockResolution:
    ...
    def test_bedrock_mantle_config_region_beats_env_region(self, monkeypatch):
        ...
        with patch("sanremes_cli.config.load_config_readonly",
