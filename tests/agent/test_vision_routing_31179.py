...
def isolated_home(monkeypatch):
    """Temp SANREMES_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="sanremes_test_31179_")
    sanremes_home = os.path.join(test_home, ".sanremes")
    os.makedirs(sanremes_home)
...
def _fresh_modules():
    """Drop cached sanremes modules so each test reloads against current env."""
    _drop_reload_targets()
