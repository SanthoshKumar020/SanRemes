...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("sanremes_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated SANREMES_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="sanremes_api_content_")
    os.makedirs(os.path.join(test_home, ".sanremes"))
    prev_home = os.environ.get("SANREMES_HOME")
    os.environ["SANREMES_HOME"] = os.path.join(test_home, ".sanremes")
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["SANREMES_HOME"] = prev_home
