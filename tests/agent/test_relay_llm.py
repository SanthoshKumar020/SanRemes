...
def test_relay_metadata_preserves_provider_name():
    metadata = {"api_mode": "chat_completions", "sanremes.provider": "explicit"}
...
def test_stream_uses_rewritten_request_and_post_intercept_chunks(relay_turn):
    ...
    try:
        ...
        relay.intercepts.deregister_llm_stream_execution("sanremes-test-stream")
        relay.intercepts.deregister_llm_request("sanremes-test-request")
...
def test_live_stream_defers_runtime_shutdown_until_exhaustion(
    ...
    monkeypatch.setenv("SANREMES_HOME", str(tmp_path / "stream-shutdown-profile"))
    relay_runtime._reset_for_tests()
...
def test_stream_current_primes_lazy_completed_response(relay_turn, monkeypatch):
    """A lazy Relay stream must run once before SanRemes decides its shape."""
    _relay, _turn = relay_turn
