...
def test_pending_response_does_not_mask_later_terminal_exit(
    ...
    monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    agent = _LimitAgent()
...
def test_pending_response_records_kanban_timeout(monkeypatch):
    monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    monkeypatch.setenv("SANREMES_KANBAN_TASK", "task-123")
    record = MagicMock(name="record_task_failure")
    conn = SimpleNamespace(close=lambda: None)
    monkeypatch.setattr("sanremes_cli.kanban_db.connect", lambda: conn)
    monkeypatch.setattr("sanremes_cli.kanban_db._record_task_failure", record)
    agent = _LimitAgent()
...
def test_bounded_fallback_records_kanban_failure_when_interrupted(monkeypatch):
    ...
    """
    monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    monkeypatch.setenv("SANREMES_KANBAN_TASK", "task-456")
    record = MagicMock(name="record_task_failure")
    conn = SimpleNamespace(close=lambda: None)
    monkeypatch.setattr("sanremes_cli.kanban_db.connect", lambda: conn)
    monkeypatch.setattr("sanremes_cli.kanban_db._record_task_failure", record)
...
def test_bounded_fallback_records_kanban_failure_when_failed(monkeypatch):
    ...
    """
    monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    monkeypatch.setenv("SANREMES_KANBAN_TASK", "task-789")
    record = MagicMock(name="record_task_failure")
    conn = SimpleNamespace(close=lambda: None)
    monkeypatch.setattr("sanremes_cli.kanban_db.connect", lambda: conn)
    monkeypatch.setattr("sanremes_cli.kanban_db._record_task_failure", record)
...
def test_bounded_fallback_does_not_fire_when_budget_not_exhausted(monkeypatch):
    ...
    """
    monkeypatch.setattr("sanremes_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    monkeypatch.setenv("SANREMES_KANBAN_TASK", "task-999")
    record = MagicMock(name="record_task_failure")
    conn = SimpleNamespace(close=lambda: None)
    monkeypatch.setattr("sanremes_cli.kanban_db.connect", lambda: conn)
    monkeypatch.setattr("sanremes_cli.kanban_db._record_task_failure", record)
