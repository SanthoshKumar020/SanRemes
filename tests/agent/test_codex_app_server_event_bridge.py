"""Regression tests for the codex_app_server → SanRemes UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_sanremes_tools_mcp_server_emits_bare_tool_name(self):
        """The sanremes-tools MCP server wraps SanRemes' own tools for codex;
        ...
        mcp.sanremes-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "sanremes-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "sanremes-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "sanremes agent docs"
        assert calls[0].args[3] == {"query": "sanremes agent docs"}
