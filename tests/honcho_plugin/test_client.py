...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "sanremes"
        assert config.workspace_id == "sanremes"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"sanremes": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"sanremes": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"sanremes": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="sanremes-agent"
        ):
            result = config.resolve_session_name("/home/user/sanremes-agent/subdir")
        assert result == "sanremes-agent"


class TestResolveConfigPath:
    def test_prefers_sanremes_home_when_exists(self, tmp_path):
        sanremes_home = tmp_path / "sanremes"
        sanremes_home.mkdir()
        local_cfg = sanremes_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: SANREMES_HOME points at ~/.sanremes/profiles/<name>, so
        # _get_default_sanremes_home() must resolve back to ~/.sanremes — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".sanremes"
        ...
        assert _get_default_sanremes_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "sanremes_coder"
        assert profile_host_key("default") == "sanremes"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"SANREMES_HONCHO_HOST": "sanremes.coder"}):
            assert resolve_active_host() == "sanremes.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("SANREMES_HONCHO_HOST", None)
            # Temporarily remove sanremes_cli.profiles to simulate import failure
            saved = sys.modules.get("sanremes_cli.profiles")
            sys.modules["sanremes_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "sanremes"
            finally:
                if saved is not None:
                    sys.modules["sanremes_cli.profiles"] = saved
                else:
                    sys.modules.pop("sanremes_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="sanremes_coder")
        assert config.host == "sanremes_coder"
        assert config.workspace_id == "sanremes"  # shared workspace
        assert config.ai_peer == "sanremes_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"sanremes": {"enabled": True, "aiPeer": "sanremes"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (sanremes.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="sanremes_work",
            workspace_id="sanremes",
            raw={"hosts": {"sanremes.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_sanremes_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("sanremes_cli.profiles.get_active_profile_name", return_value="default"), \
