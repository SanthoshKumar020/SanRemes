...
1. ``load_sanremes_dotenv()`` auto-loads ``~/.sanremes/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_sanremes_dotenv(sanremes_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "sanremes_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
