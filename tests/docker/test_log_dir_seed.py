"""Runtime smoke test for Docker $SANREMES_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the sanremes user after container boot.
...
registered later runs its log service as the dropped sanremes user and
...
def test_logs_gateways_seeded_and_sanremes_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by sanremes after boot."""
    ...
    # Both must be owned by sanremes
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=sanremes" in r.stdout, (
        f"logs/ not owned by sanremes: {r.stdout}"
    )
    assert "gateways=sanremes" in r.stdout, (
        f"logs/gateways/ not owned by sanremes: {r.stdout}"
    )
