...
Build the real image and verify: a config.yaml present in $SANREMES_HOME
is migrated by docker_config_migrate.py on boot, running as the sanremes
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $SANREMES_HOME must be migrated on boot by
    docker_config_migrate.py, running as the sanremes user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $SANREMES_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/sanremes/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by sanremes (migration ran as sanremes)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "sanremes", (
        f"config.yaml not owned by sanremes (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
