...
def test_no_args_starts_sanremes(built_image: str) -> None:
    """``docker run <image>`` should start sanremes cleanly.
...
def test_chat_subcommand_passthrough(built_image: str) -> None:
    """``docker run <image> chat --help`` should exec ``sanremes chat --help``.
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> sanremes batch ...`` returns a
