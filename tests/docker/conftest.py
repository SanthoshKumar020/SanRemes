...
Override the image with ``SANREMES_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SANREMES_TEST_IMAGE", "sanremes-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SANREMES_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SANREMES_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged sanremes user
# ---------------------------------------------------------------------------
#
# Background: every SanRemes runtime path inside the container drops to UID
# 10000 (the ``sanremes`` user) via ``s6-setuidgid sanremes``. ``docker exec``
...
# fails (PermissionError swallowed) for sanremes, so a test that ran as root
...
# helpers below run every probe under ``-u sanremes`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: sanremes).
