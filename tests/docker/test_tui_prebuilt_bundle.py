...
package-lock.json. Without SANREMES_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV SANREMES_TUI_DIR=/opt/sanremes/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the sanremes user, return stdout."""
    inner = (
        "source /opt/sanremes/.venv/bin/activate && "
        "cd /opt/sanremes && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the sanremes user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "sanremes", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_sanremes_tui_dir_env_is_set(built_image: str) -> None:
    """SANREMES_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$SANREMES_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/sanremes/ui-tui", (
        f"SANREMES_TUI_DIR={r.stdout.strip()!r} (expected /opt/sanremes/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from sanremes_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/sanremes/ui-tui')\n"
    ...
    # With SANREMES_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
