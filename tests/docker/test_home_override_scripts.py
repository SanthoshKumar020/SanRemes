...
def test_dashboard_service_resets_home(
    ...
    container with SANREMES_DASHBOARD=1 and verify the dashboard process
    ...
    the process env.
    """
    start_container(built_image, container_name, "SANREMES_DASHBOARD=1", "SANREMES_DASHBOARD_HOST=127.0.0.1")

    # Check if the dashboard process is running and inspect its HOME.
    r = docker_exec_sh(
        container_name,
        # Find the dashboard process (sanremes dashboard) and read its HOME
        # from /proc/<pid>/environ. If not running, verify the run script
        # itself exports HOME=/opt/data by grepping the script source.
        'pid=$(pgrep -f "sanremes dashboard" | head -1); '
        'if [ -n "$pid" ]; then '
        '  tr "\\0" "\\n" < /proc/$pid/environ | grep "^HOME="; '
        'else '
        '  grep -q "export HOME=/opt/data" '
        '    /opt/sanremes/docker/s6-rc.d/dashboard/run && '
        '  echo "HOME=/opt/data"; '
        'fi',
        timeout=15,
    )
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    SANREMES_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "SANREMES_DASHBOARD=1", "SANREMES_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to sanremes:sanremes on every boot.
    ...
    # Verify files are now owned by sanremes.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "sanremes" in r.stdout, (
        f"expected sanremes-owned files after restart, got: {r.stdout!r} — "
