...
Build the real image and verify at runtime:
  ...
  1. /opt/sanremes is not writable by the sanremes user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and SANREMES_DISABLE_LAZY_INSTALLS are set
  3. /opt/sanremes/.install_method contains "docker" (code-scoped stamp)
  4. $SANREMES_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $SANREMES_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_sanremes(
    built_image: str, container_name: str,
) -> None:
    """The sanremes user must not be able to modify /opt/sanremes.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/sanremes as the sanremes user
        "touch /opt/sanremes/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"sanremes user can write to /opt/sanremes (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/sanremes/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"sanremes user can write to /opt/sanremes/.venv: {r.stdout}"
    )


def test_sanremes_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    SANREMES_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$SANREMES_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"SANREMES_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $SANREMES_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $SANREMES_HOME was not healed on boot: "
        f"{r.stdout}"
    )
