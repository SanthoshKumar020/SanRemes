"""Harness: dashboard opt-in via SANREMES_DASHBOARD.

Today (tini): dashboard starts once when SANREMES_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``sanremes`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without SANREMES_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "sanremes dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without SANREMES_DASHBOARD"
    )
...
# whenever `SANREMES_DASHBOARD_HOST` was non-loopback, silently disabling
...
"""
    ...
    probe = (
        "/opt/sanremes/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``SANREMES_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        ...
        "SANREMES_DASHBOARD_HOST=0.0.0.0",
        "SANREMES_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"SANREMES_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and SANREMES_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``SANREMES_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
