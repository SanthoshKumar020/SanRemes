...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("sanremes.gateway.up"), _M("sanremes.gateway.active_agents"),
        _M("sanremes.gateway.busy"), _M("sanremes.gateway.drainable"),
        _M("sanremes.gateway.restart_requested"),
        _M("sanremes.platform.up"), _M("sanremes.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("sanremes.cron.scheduler.heartbeat_age_seconds"),
        _M("sanremes.cron.scheduler.last_success_age_seconds"),
        _M("sanremes.cron.scheduler.catch_up_occurrences"),
        _M("sanremes.cron.jobs.enabled"), _M("sanremes.cron.jobs.running"),
        _M("sanremes.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "SanRemes Agent-owned Relay transport health" in text
