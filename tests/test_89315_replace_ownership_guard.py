...
cannot prove belongs to this SANREMES_HOME.

Design contract (v3, after andrexibiza's second review): ownership is decided
by the persisted identity record ALONE — exact ``_same_sanremes_home`` equality
bound to the live target by exact PID + start-time. A readable live argv
carries no SANREMES_HOME, so it can never prove home ownership; it only feeds a
...
def profile_env(tmp_path, monkeypatch):
    """Isolated SANREMES_HOME mirroring tests/sanremes_cli/test_profiles.py."""
...
def _record(pid=424242, start=111222333, home=None, argv=None):
    return {
        ...
        "argv": argv or ["python", "-m", "sanremes_cli.main", "gateway", "run"],
        "start_time": start,
        "sanremes_home": home,
    }
...
class TestRecordAuthority:
    def test_valid_bound_same_home_record_allows(self, profile_env):
        ...
        with (
            patch(
                ...
                return_value=_record(home=str(profile_env / ".sanremes")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".sanremes" / "gateway.pid",
            ),
            patch(
            ...
            patch(
                "gateway.status._get_process_sanremes_home",
                return_value=profile_env / ".sanremes",
            ),
        ):
        ...
        with (
            patch(
                ...
                return_value=_record(
                    home="/home/other/.sanremes/profiles/timothy"
                ),
            ),
            patch(
                "gateway.status._get_pid_path",
            ...
            patch(
                "gateway.status._get_process_sanremes_home",
                return_value=profile_env / ".sanremes" / "profiles" / "tim",
            ),
        ):
    ...
    def test_legacy_record_without_home_refuses(self, profile_env):
        """A pre-sanremes_home-stamping record cannot prove ownership."""
        from gateway.run import _replace_target_belongs_to_other_profile

        legacy = _record(home=None)
        legacy.pop("sanremes_home")
    ...
    def test_unbound_record_wrong_pid_refuses(self, profile_env):
        ...
        with (
            patch(
                "gateway.status._read_pid_record",
                return_value=_record(pid=999999, home=str(profile_env / ".sanremes")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".sanremes" / "gateway.pid",
    ...
    def test_unbound_record_stale_start_time_refuses(self, profile_env):
        ...
        with (
            patch(
                "gateway.status._read_pid_record",
                return_value=_record(start=1, home=str(profile_env / ".sanremes")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".sanremes" / "gateway.pid",
...
class TestArgvConsistencyCheck:
    ...
    def test_prefix_collision_is_not_a_conflict(self, profile_env):
        ...
        tim_home = Path("/home/x/.sanremes/profiles/tim")
        # Foreign target advertising timothy — NOT ours.
        assert (
            conflict("python -m sanremes_cli.main --profile timothy gateway run", tim_home)
            is True
        )
        # Our own exact name stays consistent.
        assert (
            conflict("python -m sanremes_cli.main --profile tim gateway run", tim_home)
            is False
        )
        assert (
            conflict("python -m sanremes_cli.main -p tim gateway run", tim_home)
            is False
        )

    def test_explicit_home_flag_exact_compare(self, profile_env):
        """SANREMES_HOME= on the argv compares path-exactly, not by prefix."""
        from gateway.run import (
            _looks_like_profile_conflict_from_cmdline as conflict,
        )
        ...
        assert (
            conflict(
                "python -m sanremes_cli.main SANREMES_HOME=/home/x/.sanremes/profiles/timothy gateway run",
                tim_home,
            )
            is True
        )
        assert (
            conflict(
                "python -m sanremes_cli.main --sanremes-home /home/x/.sanremes/profiles/tim/ gateway run",
                tim_home,
            )
    ...
    def test_consistency_contradiction_refuses_even_with_agreeing_record(
        ...
        with (
            ...
            patch(
                ...
                return_value="python -m sanremes_cli.main --profile other-profile gateway run",
            ),
            patch(
                "gateway.status._get_process_sanremes_home",
                return_value=profile_env / ".sanremes",
