...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/sanremes/.overview.md") == "viking://user/sanremes"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/sanremes/memories/profile.md") == "viking://user/sanremes/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_sanremes_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingConfigSchema:
    ...
    def test_recall_config_reads_from_config_yaml(self, monkeypatch, tmp_path):
        ...
        # Populate config.yaml in the temp SANREMES_HOME
        sanremes_home = tmp_path / "sanremes_test"
        sanremes_home.mkdir(exist_ok=True)
        config_yaml = sanremes_home / "config.yaml"
...
""",
    ...
    def test_dashboard_shaped_string_values_are_typed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            openviking_plugin,
            "_load_sanremes_openviking_config",
            lambda: {
                "recall_limit": "12",
                "recall_score_threshold": "0.42",
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/sanremes"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/sanremes/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/sanremes/.overview.md"
        assert result["resolved_uri"] == "viking://user/sanremes"
    ...
    def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
            "viking://user/sanremes/memories/a.md",
            "viking://user/sanremes/memories/b.md",
            "viking://user/sanremes/memories/c.md",
            "viking://user/sanremes/memories/d.md",
        ]
        provider._client = FakeVikingClient(
            {
                (
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/sanremes/.overview.md", "level": "overview"})
            assert False, "Expected summary endpoint error to be raised"
        except RuntimeError:
            pass

        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/sanremes"}),
        ]
...
class TestOpenVikingAutoRecallPrefetch:
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/peers/sanremes/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        monkeypatch.setenv("OPENVIKING_AGENT", "sanremes")
        ...
        assert records["reads"] == ["viking://user/peers/sanremes/memories/e2e-full.md"]
        ...
        assert all(headers.get("x-openviking-actor-peer") == "sanremes" for headers in normalized_headers)
        assert all(
            headers.get("user-agent") == f"openviking-memory-sanremes/{_SANREMES_VERSION}"
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/sanremes/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/sanremes/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/sanremes"}))

        assert result["path"] == "viking://user/sanremes"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/sanremes/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/sanremes/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/sanremes"},
...
class TestEnsureClientReloadsEnv:
    ...
    def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        ...
        class _StubClient:
            def __init__(self, endpoint, api_key, account="", user="", agent="sanremes"):
    ...
    def test_rebuilt_client_resolves_its_own_user_space(self, monkeypatch):
        ...
        assert alice_uri.startswith("viking://user/alice/peers/sanremes/")
        assert bob_uri.startswith("viking://user/bob/peers/sanremes/")
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert instances[1].posts[0][1]["uri"].startswith(
            "viking://user/default/peers/sanremes/memories/"
        )
...
class TestUnavailableWarningsPromiseRetry:
    ...
    reads as "it never recovers" and sends people restarting sanremes for
    nothing (#5721).
    """

    @staticmethod
    def _assert_promises_retry(message: str) -> None:
        assert "for this SanRemes run" not in message, message
