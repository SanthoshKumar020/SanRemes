...
1. `sanremes secrets bitwarden setup --help` works (dispatch path)
2. `sanremes update --check` works (update path)
3. `sanremes secrets bitwarden disable` works (handler execution)
4. `sanremes secrets onepassword status` works (lazy backend loads on demand)
...
def _run_sanremes(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run sanremes CLI as a subprocess from repo root.
    ...
    connectivity — but ``sanremes update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-sanremes-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-sanremes-test-remote/.insteadOf",
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`sanremes secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_sanremes(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`sanremes secrets bitwarden disable` must exit 0."""
        result = _run_sanremes(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`sanremes secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_sanremes(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`sanremes secrets onepassword setup --help` must exit 0."""
        result = _run_sanremes(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `sanremes update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`sanremes update --check` must not load cryptography._rust."""
        result = _run_sanremes(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['sanremes','update','--check']
...
sys.argv = ['sanremes', 'update', '--check']

import sanremes_cli.main as m
...
# resolves the sanremes_cli.main module's lazily re-exported attribute —
# so the patch must land on sanremes_cli.main._cmd_update_check.
with patch('sanremes_cli.main._cmd_update_check', capture_update_check):
    try:
        m.main()
