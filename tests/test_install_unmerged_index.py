...
The ``sanremes update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m sanremes_cli.main gateway run`` — image name
    ``pythonw``, not ``sanremes.exe`` — so the ``taskkill /IM sanremes.exe`` guard
    ...
    # The sanremes.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM sanremes.exe' in text
