...
@patch("run_agent.OpenAI")
def test_opencode_free_sends_sanremes_attribution(mock_openai):
    """Keyless requests still identify as SanRemes (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "SanRemes Agent"
    assert str(headers.get("User-Agent", "")).startswith("SanRemesAgent/")
