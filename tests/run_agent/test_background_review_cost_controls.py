...
def test_routing_auto_inherits_parent_and_downgrades_codex_app_server():
    ...
    with patch("sanremes_cli.config.load_config", return_value=cfg), patch("sanremes_cli.config.load_config_readonly", return_value=cfg):
...
def test_routing_to_different_model_marks_routed_and_resolves_credentials():
    ...
    with patch("sanremes_cli.config.load_config", return_value=cfg), patch("sanremes_cli.config.load_config_readonly", return_value=cfg), \
         patch("sanremes_cli.runtime_provider.resolve_runtime_provider", return_value=fake_rp):
        rt = br._resolve_review_runtime(agent)
...
def test_unrouted_runtime_keeps_parent_pool_and_overrides():
    ...
    with patch("sanremes_cli.config.load_config", return_value={}), patch("sanremes_cli.config.load_config_readonly", return_value={}):
        rt = br._resolve_review_runtime(agent)
...
def test_enabled_defaults_true():
    with patch("sanremes_cli.config.load_config_readonly", return_value={}):
...
def test_enabled_false_disables_automatic_review():
    ...
    with patch("sanremes_cli.config.load_config_readonly", return_value=cfg):
