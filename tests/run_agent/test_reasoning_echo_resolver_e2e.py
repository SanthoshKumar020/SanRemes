...
`sanremes_cli/runtime_provider.py`), and the echo flag must still take effect for
it. A future refactor keying the flag on provider *name* would reintroduce that
miss; this test is the tripwire.

Uses a temp SANREMES_HOME + real `load_config_readonly` (the config cache is
...
def _write_home(tmp_path, monkeypatch, reasoning_echo: bool):
    """Point SANREMES_HOME at a temp profile declaring a named custom provider."""
    ...
    try:
        from sanremes_cli import config as _cfg
