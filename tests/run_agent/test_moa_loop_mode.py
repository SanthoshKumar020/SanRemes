...
def test_reference_messages_drops_system_but_renders_tools_as_text():
    ...
    messages = [
        {"role": "system", "content": "huge sanremes system prompt"},
    ...
    assert all("huge sanremes system prompt" not in m["content"] for m in view)
...
def test_slot_runtime_anthropic_oauth_routes_through_provider_branch(monkeypatch):
    ...
    monkeypatch.setattr(
        "sanremes_cli.runtime_provider.resolve_runtime_provider", fake_resolve
    )
...
def test_reference_messages_flattens_cache_decorated_content():
    ...
    plain = [
        {"role": "system", "content": "sanremes system prompt"},
        {"role": "user", "content": "Can we get codex usage resets into sanremes?"},
    ]
