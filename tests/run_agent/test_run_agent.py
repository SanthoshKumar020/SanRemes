...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._sanremes_home / "logs" / "errors.log").resolve()
...
class TestSessionJsonSnapshotOptIn:
    ...
    def test_traversal_session_id_cannot_escape_logs_dir(self, agent, tmp_path):
        ...
        # originate from the untrusted X-SanRemes-Session-Id API header) must not
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host SANREMES_REDACT_SECRETS state.
...
class TestInit:
    ...
    def test_prompt_caching_cache_ttl_defaults_without_config(self):
        ...
        with (
            ...
            patch("run_agent.check_toolset_requirements", return_value={}),
            patch("run_agent.OpenAI"),
            patch("sanremes_cli.config.load_config", return_value={}), patch("sanremes_cli.config.load_config_readonly", return_value={}),
        ):
            a = AIAgent(
                api_key="test-k...7890",
                model="anthropic/claude-sonnet-4-20250514",
...
class TestBuildApiKwargs:
    ...
    def test_core_responses_preserves_supported_xhigh(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "sanremes_cli.models.github_model_reasoning_efforts",
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.lifecycle.invoke_hook", _capture_hook)
        monkeypatch.setattr("sanremes_cli.lifecycle.has_hook", lambda name: True)
    ...
    def test_interrupt_skips_remaining(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.lifecycle.has_hook", lambda name: True)
        monkeypatch.setattr(
            "sanremes_cli.lifecycle.invoke_hook",
...
class TestConcurrentToolExecution:
    ...
    def test_sequential_registry_tool_forwards_request_middleware_trace(
        ...
        from sanremes_cli.middleware import RequestMiddlewareResult
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tool_call])
        monkeypatch.setattr(
            "sanremes_cli.middleware.apply_tool_request_middleware",
        ...
        monkeypatch.setattr(
            "sanremes_cli.middleware.run_tool_execution_middleware",
            lambda _name, args, callback, **_kwargs: callback(args),
        )
        monkeypatch.setattr(
            "sanremes_cli.plugins._dispatch_pre_tool_call_hooks",
    ...
    def test_managed_tool_pipeline_rejects_second_dispatch(self, agent, monkeypatch):
        ...
        assert duplicate_errors == [
            "SanRemes tool execution callback invoked more than once"
        ]
    ...
    def test_managed_tool_pipeline_allows_one_concurrent_dispatch(
        ...
        assert outcome.result == "ok"
        assert dispatched == [{"command": "true"}]
        assert errors == ["SanRemes tool execution callback invoked more than once"]
        assert outcome.blocked is False
...
class TestHandleMaxIterations:
    ...
    def test_api_sanitizer_repairs_tool_call_with_empty_function_name(self, agent):
        ...
        call, because sanremes' dispatch loop keeps empty-name calls paired with
...
class TestRunConversation:
    ...
    def test_task_start_failure_closes_relay_turn_and_lease(self, agent):
        ...
        with (
            ...
            patch(
                "sanremes_cli.observability.relay_shared_metrics.start_task_run",
                side_effect=start_error,
            ),
            patch(
                "sanremes_cli.observability.relay_shared_metrics.finish_task_run"
            ) as finish_task_run,
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for SanRemes tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "sanremes_cli.lifecycle.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("sanremes_cli.lifecycle.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.lifecycle.has_hook", lambda name: False)
        monkeypatch.setattr("sanremes_cli.lifecycle.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("sanremes_cli.lifecycle.has_hook", _has_hook)
        ...
        with (
            patch("sanremes_cli.lifecycle.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_engine_preflight_fires_below_threshold(self, agent):
        ...
        should_compress_preflight() hook so engines like sanremes-lcm can
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("sanremes_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("sanremes_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when SANREMES_KANBAN_TASK
...
class TestNousCredentialRefresh:
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "sanremes_cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "SanRemes Agent" in agent._cached_system_prompt
