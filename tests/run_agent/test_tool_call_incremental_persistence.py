...
def _make_agent():
    sanremes_home = Path(tempfile.mkdtemp(prefix="sanremes-test-home-"))
    (sanremes_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._sanremes_home", sanremes_home),
