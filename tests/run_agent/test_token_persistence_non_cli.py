...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    sanremes_state = ModuleType("sanremes_state")
    sanremes_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "sanremes_state", sanremes_state)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "SanRemes", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "SanRemes"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "SanRemes", "detail": "full"}),
        ),
    )
