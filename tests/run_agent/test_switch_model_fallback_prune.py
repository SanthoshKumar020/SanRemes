...
Reported: "switched from openrouter provider to anthropic api key via sanremes
...
def _switch_to_anthropic(agent):
    with (
        ...
        patch("sanremes_cli.timeouts.get_provider_request_timeout", return_value=None),
    ):
        agent.switch_model(
...
def test_switch_drops_old_primary_from_fallback_chain():
    ...
    assert agent._fallback_model == {"provider": "nous", "model": "sanremes-4"}
...
def test_switch_within_same_provider_preserves_chain():
    ...
    with patch("sanremes_cli.timeouts.get_provider_request_timeout", return_value=None):
        agent.switch_model(
