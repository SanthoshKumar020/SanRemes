...
AlexKucera reported on Discord (2026-04-16) that after ``sanremes update`` pulled
...
Opt-in — not part of default CI:
    SANREMES_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.sanremes/.env).
"""
...
# Load ~/.sanremes/.env so live runs pick up OPENROUTER_API_KEY without
# needing the runner to shell-source it first. Silent if the file is absent.
def _load_user_env() -> None:
    env_file = Path.home() / ".sanremes" / ".env"
