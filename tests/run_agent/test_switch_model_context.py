...
from sanremes_cli.models import LMStudioLoadResult
...
def _make_direct_start_agent(
    ...
    with (
        patch("sanremes_cli.config.load_config", return_value=cfg), patch("sanremes_cli.config.load_config_readonly", return_value=cfg),
...
def test_lmstudio_switch_uses_destination_context_and_verified_runtime(monkeypatch):
    ...
    monkeypatch.setattr("sanremes_cli.config.get_compatible_custom_providers", fake_compatible)
    monkeypatch.setattr("sanremes_cli.config.get_custom_provider_context_length", fake_provider_context)
