"""Live Fireworks smoke test — exercises the SanRemes runtime, not a raw SDK client.

Opt-in only:
    SANREMES_LIVE_TESTS=1 FIREWORKS_API_KEY=fw_... \\
        pytest tests/run_agent/test_fireworks_live.py -q

Unlike a bare OpenAI() client pointed at the endpoint, this drives SanRemes'
...
LIVE = os.environ.get("SANREMES_LIVE_TESTS") == "1"
FIREWORKS_KEY = os.environ.get("FIREWORKS_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only: set SANREMES_LIVE_TESTS=1"),
...
def _resolve_runtime_client(provider="fireworks"):
    """Build the Fireworks client the way the SanRemes runtime does."""
    from agent.auxiliary_client import resolve_provider_client

    client, model = resolve_provider_client(provider)
    assert client is not None, "SanRemes failed to build a Fireworks client"
    return client, model


def test_sanremes_wires_fireworks_client():
...
def test_fireworks_basic_chat_through_runtime():
    """A single-turn completion via the SanRemes-resolved client returns text."""
    client, model = _resolve_runtime_client()
