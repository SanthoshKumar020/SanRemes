...
class TestFormatFooter:
    ...
    def test_footer_path_not_extracted_by_gateway(self):
        ...
        tmp = tempfile.mkdtemp(prefix="sanremes_footer_")
        try:
...
class TestVerifierEnabled:
    def test_default_is_enabled(self, monkeypatch):
        monkeypatch.delenv("SANREMES_FILE_MUTATION_VERIFIER", raising=False)
        agent = _bare_agent()
    ...
    def test_env_disables(self, monkeypatch, value):
        monkeypatch.setenv("SANREMES_FILE_MUTATION_VERIFIER", value)
        agent = _bare_agent()
    ...
    def test_config_read_once_then_cached(self, monkeypatch):
        ...
        monkeypatch.setenv("SANREMES_FILE_MUTATION_VERIFIER", "0")
        assert agent._file_mutation_verifier_enabled() is False
