...
def _make_agent(max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("sanremes_cli.config.load_config", return_value=config or {}),
...
def test_explanation_persistence_turn_lease_cause_is_specific():
    ...
    assert "sanremes doctor" not in lower
...
def test_explanation_persistence_corrupt_cause_never_says_free_space():
    ...
    assert "sanremes doctor" in lower
...
def test_classify_persistence_error_reuses_disk_full_markers():
    """The disk bucket delegates to sanremes_state.is_disk_full_error, so
...
def test_classify_persistence_error_turn_lease_lost_is_distinct():
    from sanremes_state import SessionTurnLeaseLostError, classify_persistence_error
...
def test_persistence_error_causes_tuple_matches_classifier():
    ...
    from sanremes_state import PERSISTENCE_ERROR_CAUSES, classify_persistence_error
...
def test_explainer_enabled_by_default():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("SANREMES_TURN_COMPLETION_EXPLAINER", None)
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"SANREMES_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
...
def test_explainer_config_read_once_then_cached():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("SANREMES_TURN_COMPLETION_EXPLAINER", None)
        with patch("sanremes_cli.config.load_config", counting_load):
