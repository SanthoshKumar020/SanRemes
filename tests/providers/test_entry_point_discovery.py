"""Tests for pip entry-point provider discovery (sanremes_agent.plugins group).

Verifies that ``providers/__init__.py`` imports provider plugins exposed via a
distribution's ``sanremes_agent.plugins`` entry point, supporting both a
...
class _FakeEP:
    def __init__(self, name, loader):
        self.name = name
        self.group = "sanremes_agent.plugins"
        self._loader = loader
