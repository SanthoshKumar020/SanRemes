...
leaked shell variable, so ``scripts/run_tests.sh``'s ``env -i`` was no help:
  ...
  1. A test drives the ``voice.toggle`` RPC with ``action="tts"``. The
     ...
     ``os.environ["SANREMES_VOICE_TTS"] = "1"``.
  ...
  3. Any later test that drives a turn to completion hits the TTS dispatch in
     ``prompt.submit``, which calls ``sanremes_cli.voice.speak_text`` on a
...
Two independent defences in ``tests/conftest.py``, one test each below:
  ...
  * ``_SANREMES_BEHAVIORAL_VARS`` now blanks ``SANREMES_VOICE`` /
    ``SANREMES_VOICE_TTS`` at every test setup, so step 2 cannot cross a test
    boundary.
...
def test_voice_toggle_still_leaks_the_env_var_but_speech_is_stubbed(monkeypatch):
    ...
    assert os.environ.get("SANREMES_VOICE_TTS") == "1"
    ...
    # calls it (a late `from sanremes_cli.voice import speak_text`), with the
...
def test_voice_env_does_not_leak_into_the_next_test():
    ...
    one above, in the same process, which left ``SANREMES_VOICE_TTS=1`` set.
    Before ``SANREMES_VOICE``/``SANREMES_VOICE_TTS`` were added to
    ``_SANREMES_BEHAVIORAL_VARS``, this assertion failed.
    """
    assert "SANREMES_VOICE_TTS" not in os.environ
    assert "SANREMES_VOICE" not in os.environ
...
def test_guard_can_be_opted_out_of_explicitly():
    ...
    import sanremes_cli.voice as voice
