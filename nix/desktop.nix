# nix/desktop.nix — SanRemes Desktop (Electron) app build + wrapper
#
# `sanremesAgent` is the fully-built `.#default` package — it ships the
# `sanremes` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `SANREMES_DESKTOP_SANREMES` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing SanRemes CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # SANREMES_HOME and SANREMES_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "sanremes-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "sanremes-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = sanremesNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "sanremes-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe sanremesNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/sanremes-desktop $out/bin
    cp -r ${renderer}/* $out/share/sanremes-desktop/
    ...
    substituteInPlace $out/share/sanremes-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/sanremes-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # SANREMES_DESKTOP_SANREMES to the absolute path of the nix-built `sanremes`
    # binary so the desktop's resolver step 4 ("existing SanRemes CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/sanremes-desktop \
      --add-flags "$out/share/sanremes-desktop" \
      --set SANREMES_DESKTOP_SANREMES "${lib.getExe sanremesAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/sanremes.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../sanremes_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/sanremes-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/sanremes.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/sanremes.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for SanRemes Agent";
    homepage = "https://github.com/NousResearch/sanremes-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "sanremes-desktop";
  };
}
