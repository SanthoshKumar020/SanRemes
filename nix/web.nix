# nix/web.nix — SanRemes Web Dashboard (Vite/React) frontend build
{ sanremesNpmLib, ... }:
sanremesNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @sanremes/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../sanremes_cli/web_dist for the
