...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      sanremes-agent = self'.packages.default;
      sanremesVenv = sanremes-agent.sanremesVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "sanremes-check";
                homeDirectory = "/home/sanremes-check";
                stateVersion = "24.11";
              };
            }
            { services.sanremes-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.sanremes-agent = programs;
              services.sanremes-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.sanremes-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.sanremes-agent);
      ...
      configKeys = pkgs.runCommand "sanremes-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${sanremesVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "sanremes-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "sanremes-build-package" { } ''
          echo "PASS: package built at ${sanremes-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "sanremes-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.SANREMES_TEST = "1";
                environmentFiles = [ "/run/secrets/sanremes-env" ];
                sanremesHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "sanremes" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "sanremes" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.sanremesAgentSetup.data;

            failures =
              lib.optional (names != [
                "sanremes-agent"
                "sanremes-backend"
              ]) "expected sanremes-agent + sanremes-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/sanremes gateway" (argvOf "sanremes-agent")
              ) "gateway process does not run `sanremes gateway`: ${argvOf "sanremes-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/sanremes serve" (argvOf "sanremes-backend")
              ) "backend process does not run `sanremes serve`: ${argvOf "sanremes-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "sanremes-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/sanremes-check/.sanremes" (envOf n)) names
              ) "gateway and backend must share one SANREMES_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.SANREMES_HOME or null != "/home/sanremes-check/.sanremes"
              ) "programs.sanremes-agent.enable must export SANREMES_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "sanremes-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/sanremes-check/.sanremes/SOUL.md" activation
              ) "sanremesHomeFiles must install into SANREMES_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads SANREMES_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "SANREMES_MANAGED=home-manager" (envOf n)) names
              ) "processes must report SANREMES_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "sanremes-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "sanremes-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/sanremes-check";
            ...
            cases = [
              ...
              {
                name = "sanremesHomeFiles needs no directory";
                ok = accepts { sanremesHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "sanremes-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one SANREMES_HOME ───────────────
        # `programs.enable` exports SANREMES_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.sanremes
        # while the services use the SANREMES_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/sanremes-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                sanremesHome = "/home/sanremes-check/.sanremes-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "sanremes-desktop") cfg.home.packages;
            ...
            # second SanRemes runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "sanremes-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.sanremes-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.sanremes-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one sanremes-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "SANREMES_HOME" != "/home/sanremes-check/.sanremes-work"
              ) "the launcher must carry SANREMES_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "SANREMES_HOME")})"
              ++ lib.optional (
                setValue "SANREMES_MANAGED" != "home-manager"
              ) "the launcher must report SANREMES_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "SANREMES_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "SANREMES_DESKTOP_SANREMES" != "${lib.head agentPackages}/bin/sanremes"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "SANREMES_DESKTOP_SANREMES")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "SANREMES_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "SANREMES_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "SANREMES_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "SANREMES_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "SANREMES_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "sanremes-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares SANREMES_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set SANREMES_HOME" wrapper
              ) "the launcher must carry SANREMES_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "SANREMES_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "SANREMES_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? sanremes-backend || cfg.launchd.agents ? sanremes-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "sanremes-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `sanremes`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.sanremes-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.sanremes-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "sanremes-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "sanremes-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "sanremes" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."sanremes-agent-setup".text;

            failures =
              lib.optional (names != [
                "sanremes-agent"
                "sanremes-backend"
              ]) "expected sanremes-agent + sanremes-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/sanremes gateway" (execOf "sanremes-agent")
              ) "gateway unit does not run `sanremes gateway`: ${execOf "sanremes-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/sanremes dashboard" (execOf "sanremes-backend")
              ) "backend unit does not run `sanremes dashboard`: ${execOf "sanremes-backend"}"
              ++ lib.optional (
                units.sanremes-agent.environment.SANREMES_HOME != units.sanremes-backend.environment.SANREMES_HOME
              ) "gateway and backend must share one SANREMES_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/sanremes/.sanremes/SOUL.md" activation
              ) "sanremesHomeFiles must install into SANREMES_HOME";
          ...
          in
          pkgs.runCommand "sanremes-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs sanremes, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.sanremes-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/sanremes serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec sanremes directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "sanremes-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "sanremes-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec sanremes, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/sanremes dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "sanremes-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $SANREMES_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                SANREMES_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "sanremes-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'SANREMES_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--sanremes-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "sanremes-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "sanremes-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${sanremes-agent}/bin/sanremes || (echo "FAIL: sanremes binary missing"; exit 1)
          test -x ${sanremes-agent}/bin/sanremes-agent || (echo "FAIL: sanremes-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${sanremes-agent}/bin/sanremes --version 2>&1 | grep -qi "sanremes" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "sanremes-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in sanremes sanremes-agent sanremes-acp; do
            test -x ${sanremes-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "sanremes-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking sanremes --help ==="
          ${sanremes-agent}/bin/sanremes --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${sanremes-agent}/bin/sanremes --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "sanremes-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${sanremes-agent}/share/sanremes-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${sanremes-agent}/share/sanremes-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SANREMES_BUNDLED_SKILLS" ${sanremes-agent}/bin/sanremes || \
            (echo "FAIL: SANREMES_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SANREMES_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${sanremes-agent}/share/sanremes-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${sanremes-agent}/share/sanremes-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "SANREMES_OPTIONAL_SKILLS" ${sanremes-agent}/bin/sanremes || \
            (echo "FAIL: SANREMES_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, SANREMES_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "sanremes-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${sanremes-agent}/share/sanremes-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${sanremes-agent}/share/sanremes-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SANREMES_BUNDLED_PLUGINS" ${sanremes-agent}/bin/sanremes || \
            (echo "FAIL: SANREMES_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SANREMES_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "sanremes-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${sanremes-agent}/share/sanremes-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${sanremes-agent}/share/sanremes-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${sanremes-agent}/share/sanremes-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "SANREMES_BUNDLED_LOCALES" ${sanremes-agent}/bin/sanremes || \
            (echo "FAIL: SANREMES_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: SANREMES_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + SANREMES_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SANREMES_BUNDLED_LOCALES=${sanremes-agent}/share/sanremes-agent/locales \
            ${sanremesVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with SANREMES_BUNDLED_LOCALES set"; exit 1)
        ...
        # SANREMES_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "sanremes-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${sanremes-agent}/share/sanremes-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${sanremes-agent}/share/sanremes-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "SANREMES_OPTIONAL_MCPS" ${sanremes-agent}/bin/sanremes || \
            (echo "FAIL: SANREMES_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: SANREMES_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${sanremes-agent}/bin/sanremes mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: sanremes mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "sanremes-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${sanremes-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${sanremes-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SANREMES_TUI_DIR" ${sanremes-agent}/bin/sanremes || \
            (echo "FAIL: SANREMES_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SANREMES_TUI_DIR set in wrapper"
        ...
        # Verify SANREMES_NODE is set in wrapper and points to Node 26+
        # (SanRemes pins its toolchain to Node 26 everywhere)
        sanremes-node = pkgs.runCommand "sanremes-node-version" { } ''
          set -e
          echo "=== Checking SANREMES_NODE in wrapper ==="
          grep -q "SANREMES_NODE" ${sanremes-agent}/bin/sanremes || \
            (echo "FAIL: SANREMES_NODE not set in wrapper"; exit 1)
          echo "PASS: SANREMES_NODE present in wrapper"

          SANREMES_NODE=$(sed -n "s/^export SANREMES_NODE='\(.*\)'/\1/p" ${sanremes-agent}/bin/sanremes)
          test -x "$SANREMES_NODE" || (echo "FAIL: SANREMES_NODE=$SANREMES_NODE not executable"; exit 1)
          echo "PASS: SANREMES_NODE executable at $SANREMES_NODE"

          NODE_MAJOR=$("$SANREMES_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, SanRemes requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All SANREMES_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SANREMES_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "sanremes-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SANREMES_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SANREMES_MANAGED guards ==="
          check_blocked "config set" ${sanremes-agent}/bin/sanremes config set model foo
          check_blocked "config edit" ${sanremes-agent}/bin/sanremes config edit
        ...
        extra-python-packages = let
          ...
          sanremesWithExtra = sanremes-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "sanremes-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${sanremesWithExtra}/bin/sanremes || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${sanremesWithExtra}/bin/sanremes || \
          ...
          if grep -q "PYTHONPATH" ${sanremes-agent}/bin/sanremes; then
        ...
        extra-dependency-groups = let
          sanremesWithGroups = sanremes-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "sanremes-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${sanremesWithGroups}"
          echo "venv: ${sanremesWithGroups.sanremesVenv}"
        ...
        messaging-variant = pkgs.runCommand "sanremes-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.sanremesVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "sanremes-config-roundtrip" {
          ...
          merge_and_load() {
            local sanremes_home="$1"
            export SANREMES_HOME="$sanremes_home"
            ${configMergeScript} ${nixSettings} "$sanremes_home/config.yaml"
            ${sanremesVenv}/bin/python3 -c '
import json, sys
