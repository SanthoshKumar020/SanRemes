...
#
# `services.sanremes-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  sanremesHome, programs.sanremes-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "sanremes-config-attrs";
    description = "SanRemes YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $SANREMES_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "SanRemes Agent";
      ...
      package = mkOption {
        ...
        description = "The sanremes-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The SanRemes configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `sanremes config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $SANREMES_HOME/.env. SanRemes reads that file at each start, with
          load_sanremes_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."sanremes/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that SanRemes refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          SanRemes reads SOUL.md and memories/ from SANREMES_HOME, so put those
          files in `sanremesHomeFiles`.
      ...
      sanremesHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into SANREMES_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          SanRemes reads SOUL.md and the memory files from SANREMES_HOME and not
          from the working directory. Declare those files here, or SanRemes
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the sanremes plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. SanRemes discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          sanremes_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as sanremes (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in sanremes-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `sanremes gateway`.";
      };
      ...
      # ── The backend: `sanremes serve` or `sanremes dashboard` ──────────────
      # `sanremes serve` and `sanremes dashboard` are the same entry point,
      # sanremes_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that SanRemes Desktop
      ...
      # controls an external gateway, with `sanremes gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that SanRemes
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            SANREMES_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. SanRemes Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."sanremes/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `sanremes` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.sanremes-agent.installPackage was removed. SanRemes now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.sanremes-agent.enable = ${lib.boolToString (value != false)};  # the sanremes CLI, and SANREMES_HOME for your shells
        programs.sanremes-agent.desktop.enable = true;  # the desktop application

      `services.sanremes-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "sanremes-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "sanremes-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "sanremes-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "sanremes-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "sanremes-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # SANREMES_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.sanremesHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and SanRemes make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${sanremesHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. SanRemes
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${sanremesHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${sanremesHome}/config.yaml
            ${run}chmod ${modes.config} ${sanremesHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "sanremes-managed" managedSystem} ${sanremesHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${sanremesHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${sanremesHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${sanremesHome}/auth.json"
        else
          ''
            if [ ! -e ${sanremesHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${sanremesHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree sanremesHome cfg.sanremesHomeFiles}
      ...
      ${run}find ${sanremesHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "sanremes-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${sanremesHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/sanremes"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps sanremes as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "sanremes-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "sanremes-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "sanremes-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          SANREMES_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export SANREMES_DASHBOARD_SESSION_TOKEN

          if [ -z "$SANREMES_DASHBOARD_SESSION_TOKEN" ]; then
            echo "sanremes-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "sanremes-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "sanremes-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "sanremes-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "sanremes-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "sanremes-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "sanremes-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "SanRemes Agent web dashboard and desktop backend"
    else
      "SanRemes Agent backend for SanRemes Desktop";

  # The environment that each SanRemes process needs, from either module.
  #
  # managedSystem gives the value of SANREMES_MANAGED. The CLI reads that
  ...
  processEnvironment =
    ...
    }:
    {
      ...
      SANREMES_MANAGED = managedSystem;
    };
    ...
    [
      {
        ...
        message = ''
          ...
          To give SanRemes an identity and a memory, use
          ${optionPath}.sanremesHomeFiles instead. Those files go to
          SANREMES_HOME. SanRemes reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of SANREMES_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
