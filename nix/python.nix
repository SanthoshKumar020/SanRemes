...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # SanRemes derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            sanremes-agent = prev.sanremes-agent.overrideAttrs (_old: {
              SANREMES_NIX_BUILD = "1";
            });
          })
        ]
      );
  ...
  # $SANREMES_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$SANREMES_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        sanremes-agent = prev.sanremes-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "sanremes-agent-env" {
    sanremes-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "sanremes-agent-editable-env" {
    sanremes-agent = dependency-groups;
  };
}
