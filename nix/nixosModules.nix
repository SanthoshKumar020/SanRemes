# nix/nixosModules.nix — the NixOS module for sanremes-agent
#
...
#
# Container mode: sanremes runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $SANREMES_HOME/.env
# and read by sanremes at startup — no container recreation needed for env changes.
#
# Tool resolution: the sanremes wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      sanremesHome = "${cfg.stateDir}/.sanremes";
      ...
      # SANREMES_HOME via addToSystemPackages, so they can save settings through the
      ...
      # sanremes user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "sanremes-container-entrypoint" ''
        set -eu

        SANREMES_UID="''${SANREMES_UID:?SANREMES_UID must be set}"
        SANREMES_GID="''${SANREMES_GID:?SANREMES_GID must be set}"

        # ── Group: ensure a group with GID=$SANREMES_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$SANREMES_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="sanremes"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$SANREMES_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$SANREMES_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$SANREMES_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$SANREMES_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="sanremes"
          TARGET_HOME="/home/sanremes"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$SANREMES_UID" -g "$SANREMES_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$SANREMES_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$SANREMES_UID:$SANREMES_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure SANREMES_HOME is owned by the target user.
        ...
        if [ -n "''${SANREMES_HOME:-}" ] && [ -d "$SANREMES_HOME" ]; then
          find "$SANREMES_HOME" \! -user "$SANREMES_UID" -exec chown "$SANREMES_UID:$SANREMES_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/sanremes-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/sanremes-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/sanremes ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/sanremes
          chmod 0440 /etc/sudoers.d/sanremes
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$SANREMES_UID" --regid="$SANREMES_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $SANREMES_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "sanremes-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        sanremes_bin=${containerDataDir}/current-package/bin/sanremes
      '';

      # Default: /var/lib/sanremes/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the sanremes group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit sanremesHome; };
    ...
    in
    {
      options.services.sanremes-agent =
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/sanremes";
              description = "State directory. Contains .sanremes/ subdir (SANREMES_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the sanremes CLI to environment.systemPackages and export
                SANREMES_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.sanremes symlink to the service
                  stateDir. These users are automatically added to the sanremes group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the sanremes CLI to system PATH and export SANREMES_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.sanremes/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.SANREMES_HOME = sanremesHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.sanremes-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the sanremes user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.sanremes-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed sanremes
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure sanremes is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${sanremesHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${sanremesHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."sanremes-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  mkdir -p ${sanremesHome}
                  mkdir -p ${cfg.stateDir}/home
                  mkdir -p ${cfg.workingDirectory}
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${sanremesHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${sanremesHome} ${cfg.workingDirectory}
                  ...
                  find ${sanremesHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${sanremesHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${sanremesHome}/$_subdir"
                    chmod 2770 "${sanremesHome}/$_subdir"
                    find "${sanremesHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg sanremesHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${sanremesHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${sanremesHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${sanremesHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.sanremes";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${sanremesHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "sanremes-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.sanremes -> stateDir/.sanremes for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.sanremes-agent = {
              ...
              # $SANREMES_HOME/.env by the activation script. load_sanremes_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.sanremes-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.sanremes-agent = {
              description = "SanRemes Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  SANREMES_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  SANREMES_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env SANREMES_UID="$SANREMES_UID" \
                    --env SANREMES_GID="$SANREMES_GID" \
                    --env SANREMES_HOME=${containerDataDir}/.sanremes \
                    --env SANREMES_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/sanremes gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
