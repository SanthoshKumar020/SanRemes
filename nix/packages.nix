# nix/packages.nix — SanRemes Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./sanremes-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.sanremesTui;
        web = full.sanremesWeb;
        desktop = full.sanremesDesktop;

        update-npm-lockfile = full.sanremesNpmLib.updateNpmLockfile;
      };
    };
}
