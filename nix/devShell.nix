...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      sanremesNpmLib = self'.packages.default.passthru.sanremesNpmLib;
      ...
      sanremesAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "sanremes" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../sanremes} $out/bin/sanremes
          '')
        ...
        shellHook = ''
          ${sanremesAgentDevShellHook}
          ${sanremesNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export SANREMES_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "SanRemes Agent dev shell in $SANREMES_PYTHON_SRC_ROOT"
          echo "Ready. Run 'sanremes' or 'sandbox sanremes' to start."
        '';
      };
    };
}
