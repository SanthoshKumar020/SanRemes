...
# IMPORTANT: sanremes_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See sanremes_bootstrap.py for full rationale.
try:
    import sanremes_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when sanremes_bootstrap isn't registered in the venv
    # yet — happens during partial ``sanremes update`` where git-reset landed
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``sanremes -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("SANREMES_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("SANREMES_SESSION_SOURCE", "")
...
from sanremes_cli.timeouts import (
...
_loaded_env_paths = load_sanremes_dotenv(sanremes_home=_sanremes_home, project_env=_project_env)
...
# (sanremes_state._rows_to_conversation), so any code that mutates a loaded or
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-SanRemes-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.sanremes/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[sanremes-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `sanremes --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from sanremes_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``sanremes chat -q``. In that mode,
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``sanremes chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``SANREMES_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``SANREMES_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("SANREMES_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``SANREMES_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("SANREMES_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        the ``SANREMES_API_CALL_STALE_TIMEOUT`` env var. Reasoning-model floors
        ...
        if get_provider_stale_timeout(self.provider, self.model) is not None:
            return True
        return os.getenv("SANREMES_API_CALL_STALE_TIMEOUT") is not None
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        sanremes-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See sanremes-agent#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use SanRemes against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "SanRemes can't reach the model provider. You may be offline. "
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("SANREMES_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from sanremes_cli import lifecycle as _lifecycle
    ...
    def _redact_message_content(content):
        ...
        Respects ``SANREMES_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.sanremes/sessions/session_{sid}.json``
        ...
        # untrusted input (X-SanRemes-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects SANREMES_REDACT_SECRETS via
    ...
    def interrupt(
        ...
        *,
        ...
        # interrupt event rather than SanRemes' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal SanRemes model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell SanRemes-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``SANREMES_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("SANREMES_FILE_MUTATION_VERIFIER")
            if env is not None:
            ...
            try:
                from sanremes_cli.config import load_config as _load_config
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.sanremes/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``SANREMES_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("SANREMES_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another SanRemes process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another SanRemes process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `sanremes doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.sanremes/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.sanremes/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`sanremes doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (SANREMES_KANBAN_TASK set),
        ...
        if os.environ.get("SANREMES_KANBAN_TASK"):
            try:
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (SANREMES_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset SANREMES_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("SANREMES_DEV_CREDITS"))
        ...
        if _dev:
            # SANREMES_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `sanremes logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when SanRemes YOLO selected unrestricted
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``sanremes auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.sanremes/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``sanremes setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from sanremes_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from sanremes_cli.auth import _resolve_zai_base_url
        ...
        from sanremes_cli.route_identity import normalize_route_base_url
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            self._client_kwargs["default_headers"] = sanremes_xai_default_headers()
        else:
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from sanremes_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from sanremes_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from sanremes_cli.models import ollama_model_supports_thinking
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated sanremes_logging._session_context (a
            ...
            try:
                from sanremes_logging import set_session_context
            ...
            # SANREMES_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        from sanremes_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        ...
        try:
            # Serialize the full load -> run -> flush region across SanRemes
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another SanRemes process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other SanRemes process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another SanRemes process on "
                    ...
                    timeout_msg = (
                        "⏳ Another SanRemes process kept this session busy too "
