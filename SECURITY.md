# SanRemes Agent Security Policy

This document describes SanRemes Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/sanremes-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **SanRemes Agent does not operate a bug
bounty program.**
...
- Environment details (`sanremes --version`, commit SHA, OS, Python
  version).
...
SanRemes Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running SanRemes Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted SanRemes Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in SanRemes Agent's documentation
...
SanRemes Agent supports two OS-level isolation postures. They address
...
SanRemes Agent supports this in two ways:

- **SanRemes Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, SanRemes Agent's in-process heuristics
...
SanRemes Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in SanRemes Agent
itself. Bugs in SanRemes Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   SanRemes Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, SanRemes Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  SanRemes Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  sanremes-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not SanRemes Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in SanRemes Agent. Bugs in SanRemes
...
- SanRemes Agent includes supply-chain guards for MCP server
