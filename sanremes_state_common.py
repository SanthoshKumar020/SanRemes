...
Extracted verbatim from sanremes_state.py so the SessionDB mixin modules
(sanremes_state_search / sanremes_state_schema / sanremes_state_portability) can
reference them without importing sanremes_state (which would be a cycle).
sanremes_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``sanremes sessions optimize-storage``. A legacy DB sits at
...
# `sanremes sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `sanremes db optimize`. This is
...
#
# Several independent SanRemes processes routinely share one state.db (gateway
# service, the Desktop app's `sanremes serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `sanremes_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not sanremes_state)
# because the search/schema mixins cannot import sanremes_state (cycle).
#
