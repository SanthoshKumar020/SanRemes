...
# 5. Symlinks the 'sanremes' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ SanRemes Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/sanremes-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink sanremes into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up sanremes command..."

SANREMES_BIN="$SCRIPT_DIR/venv/bin/sanremes"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SANREMES_BIN" "$COMMAND_LINK_DIR/sanremes"
echo -e "${GREEN}✓${NC} Symlinked sanremes → $COMMAND_LINK_DISPLAY_DIR/sanremes"
...
# ============================================================================
# Seed bundled skills into ~/.sanremes/skills/
# ============================================================================

SANREMES_SKILLS_DIR="${SANREMES_HOME:-$HOME/.sanremes}/skills"
mkdir -p "$SANREMES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.sanremes/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SANREMES_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  sanremes status        # Check configuration"
if is_termux; then
    echo "  sanremes gateway       # Run gateway in foreground"
else
    echo "  sanremes gateway install # Install gateway service (messaging + cron)"
fi
echo "  sanremes cron list     # View scheduled jobs"
echo "  sanremes doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m sanremes_cli.main setup
fi
