...
Edit with `sanremes config edit` or `sanremes config set section.key value`.
Full reference: https://sanremes-agent.nousresearch.com/docs/user-guide/configuration
...
`sanremes config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `sanremes tools` (interactive) or `sanremes tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_SANREMES_CORE_TOOLS` is the default bundle most platforms inherit).
