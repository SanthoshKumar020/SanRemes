...
Set via `sanremes model` (picker) or `sanremes setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://sanremes-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `sanremes auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `sanremes auth add openai-codex` |
| qwen-oauth | OAuth | `sanremes auth add qwen-oauth` |
| minimax-oauth | OAuth | `sanremes auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`sanremes auth`).
Fallback chain when the primary fails: `sanremes fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`sanremes_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   sanremes config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
