...
Author widget apps for the SanRemes TUI (`sanremes --tui`): glanceable ambient
...
- The TUI must be in use (`sanremes --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.sanremes/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.sanremes/logs/tui_gateway_crash.log` if a widget never appears.
