# Themes / Skins — Author a SanRemes Color Theme

Author a SanRemes **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`sanremes_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.sanremes/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for SanRemes itself.
...
- Write access to the SanRemes home dir — `~/.sanremes` by default, or `$SANREMES_HOME`
  / the active profile's dir. Skins live in `<sanremes-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `sanremes config set`).
...
3. `write_file` it to `<sanremes-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `sanremes config set display.skin default` (or they can `/skin default`).
...
```
sanremes skin set <key> <hex>      # e.g. sanremes skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `sanremes skin set background "#08201f"` changes
only the background; `sanremes skin use <name>` / `sanremes skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.sanremes`** when a profile is active — resolve the real home
  from `$SANREMES_HOME` first, falling back to `~/.sanremes`.
...
- **Never hand-edit `config.yaml` to activate.** Use `sanremes config set
...
- **You apply it, not the user.** `sanremes config set display.skin <name>` is
...
- `read_file` the written `<sanremes-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `sanremes config get display.skin` and confirm it reports `<name>`.
