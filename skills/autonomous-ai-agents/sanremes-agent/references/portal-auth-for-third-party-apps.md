...
## Layer 1 — Is this thing a SanRemes plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the SanRemes process**. Its LLM calls go through SanRemes's already-configured provider. | Already uses Portal if user's SanRemes is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside SanRemes. That LLM call is already authenticated
via SanRemes's provider config. The plugin's own server auth (e.g.
...
What SanRemes Agent has that *feels* like OAuth — `sanremes login --provider nous`
opening a browser, user signs in, token lands in `~/.sanremes/auth.json` — is a
**SanRemes-specific browser flow**. Under the hood it produces a credential
SanRemes uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read SanRemes's existing Portal credential out of `~/.sanremes/auth.json`.
...
rides on the credential SanRemes already holds.

Where this could live in SanRemes:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for SanRemes itself.
...
1. First decide: SanRemes plugin (runs inside SanRemes) or separate app? If plugin,
   it already uses Portal via SanRemes's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   SanRemes agent itself") is fully supported. SanRemes treats agent
...
pitch in the Tool Gateway announcement is specifically about SanRemes Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when SanRemes is configured to use Portal as its provider. It is
