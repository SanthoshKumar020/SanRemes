...
Write plugins for the SanRemes desktop app: statusbar items, layout panes,
...
system (`~/.sanremes/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$SANREMES_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$SANREMES_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The SanRemes desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$SANREMES_HOME/desktop-plugins/` (usually
  `~/.sanremes/desktop-plugins/`).

## How to Run

1. Create `$SANREMES_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.sanremes/...` by default, or `~/.sanremes/profiles/<profile>/...` under a
...
The ONLY import surface is `@sanremes/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from SanRemes
  ...
  (`sanremes://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.sanremes/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@sanremes/plugin-sdk`, `react`, and
