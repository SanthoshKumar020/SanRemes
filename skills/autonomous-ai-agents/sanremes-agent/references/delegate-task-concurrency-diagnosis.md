...
**three** code paths in SanRemes that cap a batch. If none of them fired, the
cap came from the **model itself** — not from SanRemes — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in SanRemes
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "SanRemes is
   refusing my value."
...
sanremes config get delegation.max_concurrent_children

# 2. Did SanRemes' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.sanremes/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with sanremes on path)
...
appears, **the cap is the model**, not SanRemes.
