...
SanRemes runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`sanremes config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually SanRemes's env scrubber dropping
...
SanRemes-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every SanRemes tool and most Windows APIs. Prefer forward slashes in code
