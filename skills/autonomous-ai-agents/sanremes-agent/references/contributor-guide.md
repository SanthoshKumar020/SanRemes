...
For occasional contributors and PR authors. Full developer docs: https://sanremes-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.sanremes/config.yaml` (settings), `~/.sanremes/.env` (API keys) — both under `$SANREMES_HOME` when it is set.
...
`_SANREMES_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_sanremes_home()` for paths,
never hardcode `~/.sanremes`. For custom/local-only tools, write a plugin in
`~/.sanremes/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `sanremes_cli/commands.py`
...
- Tests auto-redirect `SANREMES_HOME` to temp dirs — never touch real `~/.sanremes/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/sanremes_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_sanremes_home()` from `sanremes_constants` for all paths (profile-safe)
