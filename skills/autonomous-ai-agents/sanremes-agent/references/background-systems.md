...
the `cronjob` tool, the `sanremes cron` CLI (`list`, `add`, `edit`,
...
User docs: https://sanremes-agent.nousresearch.com/docs/user-guide/features/cron
...
- **CLI:** `sanremes curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `sanremes curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.sanremes/skills/.usage.json` holds
  per-skill `use_count`, `view_count`, `patch_count`,
  `last_activity_at`, `state`, `pinned`.

Config: `curator.*` (`enabled`, `interval_hours`, `min_idle_hours`,
`stale_after_days`, `archive_after_days`, `backup.*`).
User docs: https://sanremes-agent.nousresearch.com/docs/user-guide/features/curator

### Kanban (multi-agent work queue)

Durable SQLite board for multi-profile / multi-worker collaboration.
Users drive it via `sanremes kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `SANREMES_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `SANREMES_KANBAN_BOARD` pinned in env); tenant is a soft namespace
  within a board for workspace-path + memory-key isolation.

User docs: https://sanremes-agent.nousresearch.com/docs/user-guide/features/kanban
