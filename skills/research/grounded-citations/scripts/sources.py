...
Ledger path resolution (first wins):
  --ledger PATH
  $SANREMES_CITATION_LEDGER
  $SANREMES_HOME/cache/citations/ledger.json
"""
...
from _sanremes_home import get_sanremes_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("SANREMES_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_sanremes_home() / "cache" / "citations" / "ledger.json"
