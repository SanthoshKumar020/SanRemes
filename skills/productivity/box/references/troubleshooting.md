...
| remote OAuth browser ends on an unreachable localhost page | expected `--code` redirect or wrong topology | if SanRemes is remote, return the URL's `code` and `state` to the waiting CLI; if SanRemes and the browser are on the same host, stop and restart without `--code` |
