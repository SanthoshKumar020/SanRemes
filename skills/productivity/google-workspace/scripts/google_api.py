#!/usr/bin/env python3
"""Google Workspace API CLI for SanRemes Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing SanRemes-facing JSON contract and falls back to the Python client
...
from _sanremes_home import get_sanremes_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SANREMES_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for SanRemes Agent")
