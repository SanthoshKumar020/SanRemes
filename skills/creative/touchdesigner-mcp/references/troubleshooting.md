...
## 5. SanRemes Configuration

### Config location

`$SANREMES_HOME/config.yaml` (defaults to `~/.sanremes/config.yaml` when `SANREMES_HOME` is unset)
...
Restart the SanRemes session for changes to take effect. The MCP connection is
