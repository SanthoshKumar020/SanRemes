...
SANREMES_HOME_DIR="${SANREMES_HOME:-$HOME/.sanremes}"
SANREMES_CFG="${SANREMES_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure SanRemes config has twozero_td MCP entry ──
if [[ ! -f "$SANREMES_CFG" ]]; then
    echo -e " ${FAIL} SanRemes config not found at ${SANREMES_CFG}"
    manual_steps+=("Create ${SANREMES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SANREMES_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in SanRemes config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to SanRemes config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$SANREMES_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SANREMES_CFG} manually"); }
    manual_steps+=("Restart SanRemes session to pick up config change")
fi
