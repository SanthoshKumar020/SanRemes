#!/usr/bin/env bash
# GitHub environment detection helper for SanRemes Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _sanremes_env="${SANREMES_HOME:-$HOME/.sanremes}/.env"; [ -f "$_sanremes_env" ] && grep -q "^GITHUB_TOKEN=" "$_sanremes_env" 2>/dev/null; then
