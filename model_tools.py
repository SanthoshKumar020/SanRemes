...
def _is_dispatcher_owned_worker() -> bool:
    """False when SANREMES_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_sanremes_home() under the active profile.
...
#   - cli.py, sanremes_cli/*      -> inline on startup (no event loop)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("SANREMES_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by SANREMES_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like sanremes-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("sanremes-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (sanremes-*) include _SANREMES_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("sanremes-")
    ...
    # window). Core SanRemes tools (toolsets._SANREMES_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from sanremes_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to sanremes-agent's coercion layer.
