...
machine as SanRemes itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* SanRemes ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here, unlike SanRemes's own uv call sites: this
    ...
    # local.py puts the SanRemes-managed $SANREMES_HOME/bin there, so a managed-only
