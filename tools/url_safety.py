...
Limitations:
  - DNS rebinding (TOCTOU): an attacker-controlled DNS server with TTL=0
    ...
    connection. SanRemes-owned direct httpx request paths should use
...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for SanRemes-owned URL tools.
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``SANREMES_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
...
def _resolve_allow_private_urls() -> bool:
    ...
    env_val = os.getenv("SANREMES_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``SANREMES_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
...
def ssrf_safe_async_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("sanremes_ssrf_async_origin_schemes")
...
def ssrf_safe_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("sanremes_ssrf_origin_schemes")
...
def _install_ssrf_guard_on_async_transport(transport: Any, schemes_by_origin_var: Any) -> None:
    ...
    if transport is None or state.get("_sanremes_ssrf_guarded", False):
        return
    ...
    transport._sanremes_ssrf_guarded = True
