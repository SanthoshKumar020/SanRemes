...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or SanRemes exit
...
# display_sanremes_home imported lazily at call site (stale-module safety during sanremes update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of sanremes directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "sanremes-*")):
...
def _current_session_key() -> str:
    ...
    Single lookup point for the ``SANREMES_SESSION_KEY`` ContextVar with the
    ...
    return get_session_env("SANREMES_SESSION_KEY", "")
...
def _in_delegated_child_context() -> bool:
    ...
    inherit process-wide interactivity signals (``SANREMES_INTERACTIVE=1`` set
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("SANREMES_GATEWAY_SESSION")
    ...
    for failure in sudo_failures:
        if failure in output:
            from sanremes_constants import display_sanremes_home as _dhh
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (SANREMES_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (SANREMES_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive SanRemes password prompt or the sudo -S password-pipe path.
    ...
    # silently without SanRemes prompting.
    ...
    # SANREMES_INTERACTIVE=1 (and, on a recycled worker thread, potentially a
    ...
    should_prompt_for_sudo = (
        env_var_enabled("SANREMES_INTERACTIVE") or has_sudo_prompt_callback
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``sanremes-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by SanRemes processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      SanRemes processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling SanRemes processes a generous grace
...
def _docker_persistent_profile_scoped() -> bool:
    ...
    ``container_persistent: true`` is ONE long-lived container per SanRemes
...
def _current_session_profile() -> str:
    """Return the active session's SanRemes profile name, or "" when unset.
    ...
    return get_session_env("SANREMES_SESSION_PROFILE", "")
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, SanRemesSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.sanremes/.env or environment variables."
        )
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``sanremes
    ...
    be stale from ``sanremes setup``). Environment values for omitted terminal
    ...
    try:
        from sanremes_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _get_env_config() -> Dict[str, Any]:
    ...
    from sanremes_cli.config import _is_ssh_remote_tilde_cwd
    ...
    return {
        ...
        # Startup orphan reaper for sanremes-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior SanRemes processes that hit SIGKILL / OOM / a closed terminal
        ...
        if provider is not None:
            ...
            try:
                env_obj._sanremes_backend_name = provider.name.strip().lower()
            except AttributeError:
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so SanRemes tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        effective_task_id = _resolve_container_task_id(task_id)
        if _host_local:
            # SanRemes-owned control-plane children must run beside the current
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/sanremes
        # restart|stop|uninstall targeting sanremes-gateway) must never run inside the
        ...
        # never restart. This mirrors the `sanremes gateway restart` guard in
        # sanremes_cli/gateway.py and the cron-path guard in sanremes_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        # Gate on the SUPERVISED-gateway probe, not the raw _SANREMES_GATEWAY
        # marker: gateway.run sets it at import time, so it leaks into every
        # process that merely imports gateway.run (sanremes serve --isolated,
        # CLI, web server) which are NOT the gateway and must be able to
        # restart it. A plain foreground `sanremes gateway run` (env set, PID
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use SanRemes cron for one-shot delayed work, or install an "
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `sanremes gateway restart` from a "
        ...
        if background:
            ...
            try:
                ...
                # sanremes-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in sanremes-agent dev work (PRs #31329, "
                            ...
                            "name='github/sanremes-agent-dev', "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `sanremes -z`, a "
                        ...
                        _gw_platform = _gse("SANREMES_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("SANREMES_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("SANREMES_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("SANREMES_SESSION_USER_ID", "")
                            _gw_user_name = _gse("SANREMES_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("SANREMES_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                can_reprompt = (
                    has_sudo_prompt_callback or env_var_enabled("SANREMES_INTERACTIVE")
                ) and not _in_delegated_child_context()
                if can_reprompt:
                    output += (
