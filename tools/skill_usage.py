...
Tracks per-skill usage metadata in a sidecar JSON file (~/.sanremes/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.sanremes/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.sanremes/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.sanremes/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``sanremes update`` would
...
def list_agent_created_skill_names() -> List[str]:
    ...
    for skill_md in base.rglob("SKILL.md"):
        # Skip SanRemes metadata, VCS, virtualenv/dependency, and cache dirs
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.sanremes/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``sanremes curator
    list-archived`` to help users pass a name to ``sanremes curator restore``.
    """
...
def _is_curator_managed_record(record: Any) -> bool:
    ...
    * management = "may autonomous curation mutate/archive this" — a policy
      decision the user can change at any time via ``sanremes curator adopt``.
...
def list_unmanaged_skill_names() -> List[str]:
    ...
    * It was created by a FOREGROUND ``skill_manage(action="create")`` call,
    ...
    every automatic transition. ``sanremes curator status`` surfaces this count
    so the blind spot is legible instead of silent, and ``sanremes curator
...
def adopt_skill(skill_name: str) -> Tuple[bool, str]:
    ...
    if is_bundled(skill_name):
        ...
        # claim SanRemes' own shipped skill was agent-authored and change nothing
...
def _emit_skill_lifecycle(
    ...
    *,
    ...
    try:
        from sanremes_cli.lifecycle import has_hook, invoke_hook
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.sanremes/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.sanremes/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.sanremes/skills/<skill>/SKILL.md) and category-nested
    (~/.sanremes/skills/<category>/<skill>/SKILL.md) layouts. Uses the gated
