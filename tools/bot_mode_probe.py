...
``ui_meta['sanremes-bots']`` block in its profile.yaml), a bot's canonical
...
def _sanremes_root(home: Path) -> Path:
    """Root ~/.sanremes for both the default profile and named profiles."""
...
def _is_bot_managed(profile_dir: Path) -> bool:
    """True when profile.yaml carries a ui_meta['sanremes-bots'] block.
    ...
    try:
        ...
        if "sanremes-bots" not in raw:
            return False
        import yaml

        data = yaml.safe_load(raw)
        ui_meta = data.get("ui_meta") if isinstance(data, dict) else None
        return isinstance(ui_meta, dict) and isinstance(ui_meta.get("sanremes-bots"), dict)
    except Exception:
        return False
...
def is_bot_mode_managed(home: str | os.PathLike | None = None) -> bool:
    ...
    try:
        resolved = Path(
            str(home) if home else (os.getenv("SANREMES_HOME") or os.path.expanduser("~/.sanremes"))
        )
        root = _sanremes_root(resolved)
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @sanremes.
    return "sanremes" if name == "default" else name


def _profile_role(profile_dir: Path) -> str:
    """A teammate's role line: Bot Mode title, else profile description.

    The ui_meta['sanremes-bots'].title is the name the user gave the bot in
    ...
    try:
        ...
        if isinstance(ui_meta, dict) and isinstance(ui_meta.get("sanremes-bots"), dict):
            title = str(ui_meta["sanremes-bots"].get("title") or "").strip()
            if title:
                parts.append(title)
...
def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``sanremes peer``), for the protocol text.
...
def _remote_paragraph(root: Path) -> str:
    ...
    connected to the user's Desktop (local, remote URL, SSH, SanRemes Cloud,
...
def _peer_paragraph(root: Path) -> str:
    ...
    return (
        ...
        "for the peer's main agent). Run `sanremes peer list` for the live "
        "peer list."
    )


def _build_section(home: Path) -> str:
    root = _sanremes_root(home)
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each SanRemes profile is an agent teammate with "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    not the ambient SANREMES_HOME — build threads can lose the ContextVar
    override and the env var would then name the wrong profile.
    """
    resolved = str(home) if home else (os.getenv("SANREMES_HOME") or os.path.expanduser("~/.sanremes"))
    with _lock:
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    resolved = Path(str(home) if home else (os.getenv("SANREMES_HOME") or os.path.expanduser("~/.sanremes")))
    surface: dict = {}
    try:
        ...
        token = set_sanremes_home_override(str(resolved))
        try:
    ...
    try:
        ...
        surface["peers"] = _peers(_sanremes_root(resolved))
    except Exception:
        surface["peers"] = []
    try:
        ...
        surface["remote_roster"] = sorted(
            ...
            for r in read_remote_roster(_sanremes_root(resolved))
        )
    except Exception:
