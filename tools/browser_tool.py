...
from sanremes_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every SanRemes secret
...
def _build_browser_env() -> dict:
    ...
    Strips SanRemes-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``sanremes_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = sanremes_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    sanremes_home = get_sanremes_home()
    sanremes_node_bin = str(sanremes_home / "node" / "bin")
    sanremes_node_root = str(sanremes_home / "node")
    sanremes_nm_bin = str(sanremes_home / "node_modules" / ".bin")
    return [sanremes_node_bin, sanremes_node_root, sanremes_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
    ...
    if "sandbox" in combined:
        ...
        if _running_in_docker():
            hints.append(
                ...
                "docker pull ghcr.io/nousresearch/sanremes-agent:latest"
            )
        else:
            hints.append(
                "The browser daemon may still be starting, or Chromium may be "
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``sanremes doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.sanremes/plugins/browser/<vendor>/``.
...
def _ensure_browser_plugins_loaded() -> None:
    ...
    """
    try:
        from sanremes_cli.plugins import _ensure_plugins_discovered
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active SanRemes profile."""
    global _cached_cloud_provider, _cloud_provider_resolved
    global _cached_cloud_provider_scope

    scope = sanremes_home_key()
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.sanremes/plugins/browser/<vendor>/``) participate
...
from sanremes_constants import is_termux as _is_termux_environment
...
# comparison sites (four here, plus sanremes_cli/tools_config.py and
# sanremes_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when SanRemes silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — SanRemes-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the SanRemes CLI where sys.stdout and sys.stderr both report
...
_REAL_PROFILE_SESSION = "sanremes-real-profile"
...
def _cdp_http_ready(http_cdp: str) -> bool:
    ...
    try:
        from sanremes_cli.browser_connect import is_browser_debug_ready
...
def _real_profile_cdp() -> tuple:
    ...
    Snapshots the user's default-Chromium profile into a sanremes-owned copy
    ...
    if not _use_real_profile():
        ...
        try:
            from sanremes_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    with _real_profile_cdp_lock:
        ...
        # from a previous sanremes process; if it is driving OUR copy dir, hand it
        ...
        if err or not snap_dir:
            from sanremes_cli.browser_connect import _PROFILE_LOCKED_PREFIX
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-sanremes_…`` the resulting socket path exceeds the
...
# run exactly once, before the cleanup loop started, which meant a sanremes
...
# Hard ceiling for a daemon whose owning sanremes process is still alive but
...
def _emergency_cleanup_all_sessions():
    ...
    crashed sanremes processes — this way every clean sanremes exit sweeps
    ...
    # Sweep orphans from other crashed sanremes processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live sanremes processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current sanremes PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live sanremes process
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    sanremes restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning sanremes process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced sanremes PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent sanremes instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-sanremes_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live sanremes process and must not be touched — but "owner alive"
...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    # browser running on a sanremes-owned SNAPSHOT of their real profile —
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the SanRemes-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy SanRemes-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_sanremes_node_executable`` uses to self-heal a managed Node tree)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, SanRemes-managed
    ...
    # node_modules binary, which disappears on the next ``sanremes update`` and
    ...
    # Build an extended search PATH including SanRemes-managed Node, macOS
    ...
    try:
        from sanremes_cli.dep_ensure import ensure_dependency
        if ensure_dependency("browser"):
            candidates = [
                shutil.which("agent-browser"),
                shutil.which("agent-browser", path=extended_path) if extended_path else None,
                shutil.which("agent-browser", path=str(get_sanremes_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_sanremes_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_sanremes_home() / "node")),
            ]
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``sanremes update`` (or ``sanremes doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential SanRemes holds, while running registry-fetched
    npm code on every ``sanremes update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `sanremes update`/`doctor
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this sanremes PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_sanremes_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = sanremes_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise SanRemes
    ...
    from sanremes_constants import get_sanremes_dir
    screenshots_dir = get_sanremes_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/sanremes-agent:latest")
                else:
