...
def _media_caption_split(text, media_files, *, max_caption_len):
    ...
    across every standalone sender. ``sanremes send`` (and the send_message
    ...
    ``sanremes send --to whatsapp "MEDIA:/x.png This Caption"`` into two parts.
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.sanremes/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.sanremes/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return tool_error(
                ...
                f"or set a home channel via: sanremes config set {home_env} <channel_id>"
            )
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("SANREMES_SESSION_PLATFORM", "cli")
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("SANREMES_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("SANREMES_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("SANREMES_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False, args=None):
    ...
    # drift (credit: @5L-sanremes01 in #67279, @lkz-de in #57929).
    try:
    ...
    if platform == Platform.FEISHU and media_files:
        ...
        from sanremes_cli.plugins import discover_plugins as _dp_feishu
        _dp_feishu()
    ...
    if platform == Platform.SLACK and media_files:
        ...
        from sanremes_cli.plugins import discover_plugins as _dp_slack
        _dp_slack()
    ...
    if platform == Platform.WHATSAPP and media_files:
        ...
        from sanremes_cli.plugins import discover_plugins as _dp_wa
        _dp_wa()
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    is running (standalone cron, ``sanremes send`` CLI).
    """
    media_files = media_files or []
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``SANREMES_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``SANREMES_HOME`` which has no ``gateway.pid``, so the gateway-running check
...
#   - the ``sanremes send`` CLI command (sanremes_cli/send_cmd.py)
