...
def _voice_capture_install_hint() -> str:
    ...
    # If we're running inside a venv (e.g. the bundled SanRemes venv at
    # ~/.sanremes/profiles/<name>/sanremes-agent/venv/), `pip install` on the
...
_TEMP_DIR = os.path.join(tempfile.gettempdir(), "sanremes_voice")
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.sanremes/config.yaml during a
...
class AudioRecorder:
    ...
    def _max_duration_reached(self, elapsed: float) -> bool:
        ...
        recording (see ``SanRemesCLI._voice_start_recording``). A value <= 0
...
# playback-phase barge transcript is treated as a self-capture of SanRemes'
...
def is_tts_echo(
    ...
    SanRemes just spoke using a character-level similarity ratio, which works
    across languages without word-tokenization. A genuine user interjection
    is very unlikely to closely match SanRemes' own words, so a high ratio is
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent SanRemes TTS calls from colliding on
        ...
        if _is_wsl2_env():
            try:
                ...
                if _win_tmp_wsl:
                    ...
                    _wsl_wav = os.path.join(_win_tmp_wsl, f"sanremes-tts-{_unique}.wav")
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("SANREMES_VOICE_DEBUG", "").strip() == "1"


def _vad_log(msg: str) -> None:
    """VAD decision-point diagnostic — always logger.debug, plus stderr when
    SANREMES_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
