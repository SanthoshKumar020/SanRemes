...
def _resolve_sanremes_home() -> Path:
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.sanremes",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``SANREMES_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside SANREMES_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because SANREMES_HOME is exactly where
    ...
    """
    sanremes_home = _resolve_sanremes_home()

    # Reject absolute paths — they bypass the SANREMES_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to SANREMES_HOME)",
            relative_path,
        )
        return False

    host_path = sanremes_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape SANREMES_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, sanremes_home)
    ...
    # inside SANREMES_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.sanremes/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="sanremes-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching sanremes_constants.get_sanremes_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_sanremes_dir()`` for backward compatibility with old directory layouts.
    """
    from sanremes_constants import get_sanremes_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_sanremes_dir(new_subpath, old_name)
        if not host_dir.is_dir():
            ...
            # mount costs the feature. get_sanremes_dir() already resolved
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.sanremes``,
    SSH ``<remote_home>/.sanremes``, etc.) and whether translation is wanted.
...
def to_agent_visible_cache_path(
    ...
    backend's SanRemes cache lands):

    * docker / modal — bind-mounted (docker) or per-file-synced (modal) at
      ``/root/.sanremes`` (the *container_base* default).
    * ssh / daytona / vercel_sandbox — file-synced under the remote user's
      home; ``~/.sanremes`` is shell-expanded by the remote shell, so tool
    ...
    * singularity — NOT translated: Apptainer auto-binds the host home, so
    ...
    if backend in ("docker", "modal"):
        pass  # /root/.sanremes default
    elif backend in ("ssh", "daytona", "vercel_sandbox"):
        container_base = "~/.sanremes"
    else:
