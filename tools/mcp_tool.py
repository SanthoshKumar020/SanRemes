...
transport, discovers their tools, and registers them into the sanremes-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.sanremes/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.sanremes/logs/mcp-stderr.log), tagged with the
...
def _get_mcp_stderr_log() -> Any:
    ...
    with _mcp_stderr_log_lock:
        ...
        try:
            ...
            log_dir = get_sanremes_home() / "logs"
...
def sdk_httpx():
    ...
    side by side with SanRemes' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach SanRemes' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    SanRemes explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so SanRemes and
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            sanremes_home = os.path.expanduser(
                os.getenv(
                    "SANREMES_HOME", os.path.join(os.path.expanduser("~"), ".sanremes")
                )
            )
            candidates = [
                os.path.join(sanremes_home, "node", "bin", resolved_command),
                ...
                # image (which the SanRemes Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → SanRemes MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that SanRemes gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the SanRemes document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`sanremes_cli.mcp_config._unwrap_exception_group` with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active SanRemes profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through SanRemes' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (SANREMES_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit SANREMES_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into SanRemes' logging (agent.log via sanremes_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `sanremes setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this SanRemes process (kill -9, crash, force-quit)
        ...
        try:
            async with stdio_client(server_params, errlog=_errlog) as (
                ...
                if new_pids:
                    ...
                    for _pid in new_pids:
                        try:
                            from sanremes_cli.process_identity import register_child
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"sanremes-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through SanRemes' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until SanRemes is fully
                ...
                if not self._ever_connected:
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``sanremes mcp login``. Parking
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by SanRemes' pinned ``httpx``, and the two define unrelated
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `sanremes mcp login {server_name}` (or delete the tokens "
        f"file under ~/.sanremes/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent SanRemes processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_sanremes_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local SANREMES_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_sanremes_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_sanremes_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local SANREMES_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_sanremes_home()
...
def _filter_suspicious_mcp_servers(servers: Dict[str, dict]) -> Dict[str, dict]:
    ...
    try:
        from sanremes_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
    except Exception:
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the SanRemes config file.
    ...
    ``os.environ`` (which includes ``~/.sanremes/.env`` loaded at startup).
    """
    try:
        from sanremes_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("SANREMES_SAFE_MODE"):
            return {}
        config = load_config()
        ...
        try:
            from sanremes_cli.plugins import discover_plugins, get_plugin_manager
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through SanRemes' MEDIA: tag convention and out to messaging
            ...
            # SanRemes' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves SanRemes's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. SanRemes uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the SanRemes registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``sanremes tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the SanRemes registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple sanremes
