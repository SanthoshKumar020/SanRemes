#!/usr/bin/env python3
"""Bridge desktop-only tools to SanRemes-desktop renderer events.
...
``SANREMES_UI_SESSION_ID`` so the event lands on the window that owns the turn
...
def emit(event: str, payload: dict) -> bool:
    ...
    if fn is None:
        return False
    fn(get_session_env("SANREMES_UI_SESSION_ID", ""), event, payload)
    return True
