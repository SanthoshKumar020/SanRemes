...
-----------------------------------------------------------------------------
    ...
    ~/.sanremes/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/sanremes/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_sanremes_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/sanremes"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (SanRemes convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("SANREMES_CHECKPOINT_TIMEOUT", 30)))
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "SanRemes wrote
...
def _git_env(
    ...
    The shared store is internal SanRemes infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``sanremes checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `sanremes checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "sanremes@local"], store, cfg_wd)
    _run_git(["config", "user.name", "SanRemes Checkpoint"], store, cfg_wd)
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "SANREMES_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``SANREMES_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at SANREMES_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "SANREMES_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file SanRemes just successfully wrote.
        ...
        recorded hash was hand-edited by the user after SanRemes last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what SanRemes last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after SanRemes'
        last write or that SanRemes never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or SanRemes has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # SanRemes never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since SanRemes wrote it: restoring it back is
                # safe — its last content was SanRemes-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has SanRemes changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        SanRemes changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after SanRemes' last write — per the agent-write ledger —
        are left untouched, and only SanRemes-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # SanRemes-created files absent from it (delete to restore state).
            ...
            for rel in restore_paths:
                ...
                if ok_in_commit:
                    ...
                    # SanRemes created it. Deleting it would not restore a prior
...
# ---------------------------------------------------------------------------
# Public helpers for `sanremes checkpoints` CLI
# ---------------------------------------------------------------------------
