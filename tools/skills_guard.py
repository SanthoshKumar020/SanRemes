...
Trust levels:
  - builtin:   Ships with SanRemes. Never scanned, always trusted.
...
_SANREMES_CONFIG_FILES = r'\.sanremes/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.sanremes/\.env|\~/\.sanremes/\.env',
     "sanremes_env_access", "critical", "exfiltration",
     "directly references SanRemes secrets file"),
    ...
    #     For SanRemes/other config files it is high (caution) — legitimate
    ...
    (_prose_modify_re(_SANREMES_CONFIG_FILES),
     "sanremes_config_mod", "high", "persistence",
     "modification language aimed at SanRemes configuration files (verify intent)"),
    (_shell_write_re(_SANREMES_CONFIG_FILES),
     "sanremes_config_mod_shell", "critical", "persistence",
     "shell write (redirect/sed -i/tee/cp/mv) targeting SanRemes configuration files"),
    (r'\.sanremes/config\.yaml|\.sanremes/SOUL\.md',
     "sanremes_config_ref", "low", "persistence",
     "references SanRemes configuration files (informational; only modification intent is scored)"),
...
# scan. `.skillignore` is the SanRemes-native name; `.clawhubignore` is honored
