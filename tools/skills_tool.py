...
from sanremes_constants import get_sanremes_home, display_sanremes_home
import os
import re
...
# Cache validation (mirrors sanremes_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.sanremes/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set SANREMES_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped SANREMES_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from SANREMES_HOME/.env."""
    env_path = get_sanremes_home() / ".env"
    ...
    # the canonical readers in sanremes_cli/config.py).
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set SANREMES_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (SANREMES_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("SANREMES_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("SANREMES_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_sanremes_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.sanremes/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``SANREMES_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``SANREMES_PLATFORM`` environment variable
    3. ``SANREMES_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from sanremes_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("SANREMES_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.sanremes/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``sanremes skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile SANREMES_HOME; the module-level
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from sanremes_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: sanremes plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
                ...
                return json.dumps(
                    {
                        ...
                        "hint": (
                            ...
                            "the repo with `sanremes skills untrust`."
                        ),
                    },
                    ensure_ascii=False,
                )
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.sanremes/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `sanremes skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.sanremes.* first (agentskills.io convention), fall back to top-level
        sanremes_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            sanremes_meta = metadata.get("sanremes", {}) or {}

        tags = _parse_tags(sanremes_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            sanremes_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `sanremes sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
