...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("SANREMES_YOLO_MODE", ""))
...
# SanRemes session id (observability identity, distinct from the gateway
...
# os.environ["SANREMES_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export SANREMES_INTERACTIVE.
_sanremes_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "sanremes_interactive",
    default=None,
)


def set_sanremes_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["SANREMES_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``SANREMES_INTERACTIVE`` env var for legacy callers.
    """
    return _sanremes_interactive_ctx.set("1" if interactive else "")


def reset_sanremes_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_sanremes_interactive_context`."""
    _sanremes_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``SANREMES_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _sanremes_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("SANREMES_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the SanRemes session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("SANREMES_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("SANREMES_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("SANREMES_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("SANREMES_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("SANREMES_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``sanremes chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``SANREMES_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("SANREMES_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("SANREMES_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set SANREMES_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind SANREMES_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds SANREMES_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("SANREMES_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``SANREMES_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $SANREMES_HOME, or by the resolved absolute
# active profile home path such as /home/sanremes/.sanremes/config.yaml. The
# resolved-absolute form is folded into the ~/.sanremes/ patterns at detection
...
# go stale when SANREMES_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_SANREMES_ENV_PATH = (
    r'(?:~\/\.sanremes/|'
    r'(?:\$home|\$\{home\})/\.sanremes/|'
    r'(?:\$sanremes_home|\$\{sanremes_home\})/)'
    r'\.env\b'
)
# ~/.sanremes/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _SANREMES_ENV_PATH; matches the SANREMES_HOME override form as
# well as ~/.sanremes/.
_SANREMES_CONFIG_PATH = (
    r'(?:~\/\.sanremes/|'
    r'(?:\$home|\$\{home\})/\.sanremes/|'
    r'(?:\$sanremes_home|\$\{sanremes_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_SANREMES_ENV_PATH}|'
    rf'{_SANREMES_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_sanremes_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by SanRemes: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY SanRemes
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]sanremes[^\n]*\.env\b', "access to SanRemes secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `sanremes` and `gateway` (e.g. `sanremes -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bsanremes\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart sanremes gateway (kills running agents)"),
    (r'\bsanremes\s+update\b', "sanremes update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `sanremes gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `sanremes ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart sanremes-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart sanremes-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(sanremes|gateway|cli\.py)\b', "kill sanremes/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill sanremes` but not
    # `kill -9 $(pgrep -f sanremes)` because the substitution is opaque
    ...
    # the `sanremes gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.sanremes.gateway`).
    # Catch the operations that stop, restart, or unload it.
    #
    # Order-independent (2026-08-02 incident): the previous version required
    # "sanremes"/"ai.sanremes" to appear AFTER the launchctl verb in the same
    ...
    # 'ai.sanremes.gateway-apollo:...' ...; do label=${item%%:*}; launchctl
    # bootout "$label"; done` — never has the literal text "sanremes" appear
    ...
    # an unrelated "sanremes" mention now also matches) — for an approval gate
    # that's the correct direction to err: an extra approval prompt is
    # cheap, a missed one took down the whole gateway fleet.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:sanremes|ai\.sanremes)\b)', "stop/restart sanremes launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/SanRemes file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.sanremes/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a SanRemes-managed security file (~/.sanremes/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_SANREMES_CONFIG_PATH}|{_SANREMES_ENV_PATH})', "in-place edit of SanRemes config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_SANREMES_CONFIG_PATH}|{_SANREMES_ENV_PATH})', "in-place edit of SanRemes config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_SANREMES_CONFIG_PATH}|{_SANREMES_ENV_PATH})', "in-place edit of SanRemes config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.sanremes/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / SANREMES_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) SanRemes home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\sanremes), so folding the user home
    # first would eat the prefix the SanRemes-home fold needs.
    command = _rewrite_resolved_sanremes_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / SANREMES_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.sanremes``); the matched
...
def _rewrite_resolved_sanremes_home(command: str) -> str:
    """Rewrite the resolved absolute SanRemes home prefix to ``~/.sanremes/``.

    Resolves the active ``SANREMES_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.sanremes/`` so the static ``_SANREMES_CONFIG_PATH`` / ``_SANREMES_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/sanremes/.sanremes/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$SANREMES_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from sanremes_constants import get_sanremes_home
        home = get_sanremes_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.sanremes")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``sanremes send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one SanRemes ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"sanremes-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None


_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = (
    "stop/restart sanremes gateway via shell-spliced verb (kills running agents)"
)
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    ``launchctl kick"start" -k gui/501/ai.sanremes.gateway`` auto-approved
    ...
    anchored on a sanremes-gateway identifier — reusing its prose
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from SanRemes YOLO.
...
def load_permanent_allowlist() -> set:
    ...
    """
    try:
        from sanremes_cli.config import load_config_readonly
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["SANREMES_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "SANREMES_SPINNER_PAUSE" in os.environ:
            del os.environ["SANREMES_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses SanRemes approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``SANREMES_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            SANREMES_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export SANREMES_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set SANREMES_INTERACTIVE or "
                "SANREMES_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set SANREMES_INTERACTIVE or "
            "SANREMES_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("SANREMES_EXEC_ASK"):
        ...
        # can see (SANREMES_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``SANREMES_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from sanremes_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from sanremes_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("SANREMES_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # SANREMES_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from sanremes_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (SANREMES_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # SANREMES_EXEC_ASK (and sometimes a session platform marker) can leak
