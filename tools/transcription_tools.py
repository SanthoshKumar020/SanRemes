...
LOCAL_STT_COMMAND_ENV = "SANREMES_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "SANREMES_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via sanremes_cli.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``SANREMES_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
def _try_lazy_install_stt() -> bool:
    ...
    try:
        ...
        logger.warning(
            ...
            "This is often a permission issue: the SanRemes process user cannot "
            "write to the virtual environment. Try running manually as the "
            "venv owner: `stat -c '%%u' '$(dirname $(dirname $(which python3)))'` "
            "then `su - <owner> -c 'VIRTUAL_ENV=/opt/sanremes/.venv "
...
#
# The single-env-var ``SANREMES_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from SanRemes's remote upload cap."""
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__SANREMES_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"sanremes-cmd-stt-{provider_name}-") as tmpdir:
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set SANREMES_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `sanremes plugins list` to see "
...
def _apply_pre_transcription_hook(
    *,
    ...
    """
    try:
        from sanremes_cli.plugins import has_hook, invoke_hook
...
def _prepare_audio_for_transcription(
    ...
    temp_dir = tempfile.mkdtemp(prefix="sanremes-silk-")
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(
            target=_watch, name="sanremes-stt-idle-unload", daemon=True
        )
...
def _transcribe_local_command(
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="sanremes-local-stt-") as output_dir:
            ...
            # Scrub SanRemes secrets from the child env (sibling path to #56332 /
            # _run_command_stt — this local-whisper path previously inherited
            # the full process environment).
            from tools.environments.local import sanremes_subprocess_env

            child_env = sanremes_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_groq(
    ...
    *,
    ...
    ``stt.language`` (config.yaml) > ``SANREMES_LOCAL_STT_LANGUAGE`` (env).
...
def _transcribe_openai(
    ...
    *,
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="sanremes-stt-") as work_dir:
                try:
    ...
    *,
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `sanremes model` or set XAI_API_KEY",
        }
...
def _transcribe_deepinfra(
    ...
    *,
    ...
    ``sanremes_cli.models`` helpers so every DeepInfra surface resolves the
...
def _trim_silence_for_cloud_stt(
    ...
    work_dir = tempfile.mkdtemp(prefix="sanremes-stt-trim-")
...
def _dispatch_stt_provider(
    ...
    # unavailable). Surface it — with its `sanremes tools` remediation —
