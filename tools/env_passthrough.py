...
def _is_sanremes_provider_credential(name: str) -> bool:
    """True if ``name`` is a SanRemes-managed provider credential (API key,
    token, or similar) per ``_SANREMES_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-SanRemes API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a SanRemes credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated SanRemes-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_sanremes_internal_secret(name):
        return True
    return name in _SANREMES_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are SanRemes-managed provider credentials (from
    ``_SANREMES_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a SanRemes-managed
    ...
    Non-SanRemes third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_sanremes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register SanRemes provider "
                "credential %r (blocked by _SANREMES_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # SanRemes-managed provider credentials must not be passed
                ...
                if _is_sanremes_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register SanRemes "
                        "provider credential %r from config.yaml (blocked "
                        "by _SANREMES_PROVIDER_ENV_BLOCKLIST). Operator "
