...
the get_anchored_view / get_messages_around primitives in sanremes_state.
...
from sanremes_state_common import _RESET_END_REASONS

# Sources that are excluded from session browsing/searching by default.
# Third-party integrations tag their sessions with SANREMES_SESSION_SOURCE=tool;
...
def check_session_search_requirements() -> bool:
    ...
    try:
        from sanremes_state import _default_db_path
...
SESSION_SEARCH_SCHEMA = {
    "name": "session_search",
    "description": (
        "Search past SanRemes sessions (FTS5 over the local session DB), or read/"
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "profile": {
                "type": "string",
                "description": (
                    "Optional. Read sessions from another SanRemes profile's database "
