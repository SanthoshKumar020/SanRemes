...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   ``$SANREMES_HOME/cache/spillover/{tool_use_id}.txt`` — alongside the other
   SanRemes-owned caches (images, audio, documents, ...) instead of littering
...
STORAGE_DIR = "/tmp/sanremes-results"
SPILLOVER_SUBDIR = "cache/spillover"
SPILLOVER_MAX_AGE_HOURS = 24
HEREDOC_MARKER = "SANREMES_PERSIST_EOF"
...
def get_spillover_dir():
    """Return $SANREMES_HOME/cache/spillover as a Path (not created)."""
    from sanremes_constants import get_sanremes_home

    return get_sanremes_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write content host-side to $SANREMES_HOME/cache/spillover.
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/sanremes-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"SANREMES_PERSIST_{uuid.uuid4().hex[:8]}"
...
def maybe_persist_tool_result(
    ...
    # Always persist host-side first: $SANREMES_HOME/cache/spillover is the
    # single canonical home for spilled results (with the other SanRemes-owned
