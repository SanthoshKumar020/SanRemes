"""
Cron job management tools for SanRemes Agent.
...
# uses 30s) — comfortably below the 1800s default SANREMES_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (SANREMES_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with SANREMES_CRON_TIMEOUT=0
...
#      saying "cat ~/.sanremes/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `sanremes-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.sanremes/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("SANREMES_SESSION_PLATFORM")
    origin_chat_id = get_session_env("SANREMES_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        if thread_id and origin_platform == "slack":
            message_id = get_session_env("SANREMES_SESSION_MESSAGE_ID") or None
        ...
        return {
            ...
            "chat_name": get_session_env("SANREMES_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes SANREMES_SESSION_USER_ID to
            ...
            "scope_id": get_session_env("SANREMES_SESSION_SCOPE_ID") or None,
        }
    return None
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``SANREMES_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
    ...
    for part in str(deliver).split(","):
        ...
        if not profile_exists(canon):
            return (
                ...
                "profile that exists here (sanremes profile list), or omit the "
...
def _resolve_cron_context_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    delivery target — the ``SANREMES_CRON_AUTO_DELIVER_*`` contextvars that
    ...
    * Every other element (``local``, ``all``, explicit ``platform:...``)
    ...
    if not is_truthy_value(get_session_env("SANREMES_CRON_SESSION", "")):
        return deliver

    def _creator_target() -> str:
        platform = get_session_env("SANREMES_CRON_AUTO_DELIVER_PLATFORM", "").strip()
        chat_id = get_session_env("SANREMES_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
        if not platform or not chat_id:
            return "local"
        thread_id = get_session_env("SANREMES_CRON_AUTO_DELIVER_THREAD_ID", "").strip()
        if thread_id:
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within SANREMES_HOME/scripts/.
    ...
    # Only relative paths within ~/.sanremes/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.sanremes/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.sanremes/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_sanremes_home() / "scripts"
...
def _latest_job_output_excerpt(job_id: str, max_chars: int = 2000) -> Optional[str]:
    ...
    ``~/.sanremes/cron/output/``. Never raises.
    """
    try:
...
def _try_dispatch_background_run(
    ...
    None
        ...
        ``sanremes -z``, stateless HTTP, Kanban worker, nested cron run).
    ...
    # 'running' by a dead owner process -- e.g. a PRIOR one-shot `sanremes
    ...
    if not session_key:
        # Direct Python callers (`sanremes cron run`, tests) have no agent
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("SANREMES_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
def _gateway_liveness_notice(plural: bool = False) -> dict:
    ...
    Thin adapter over the shared CLI helper ``sanremes_cli.cron._builtin_gateway_liveness``
    ...
    """
    try:
        from sanremes_cli.cron import _builtin_gateway_liveness
    ...
    if _gw is False:
        return {
            ...
            "warning": (
                f"The SanRemes gateway is not running — {subject} "
                "but will NOT fire until the gateway is started "
                "(sanremes gateway install / sanremes gateway start). "
        ...
        if normalized == "create":
            ...
            try:
                job = create_job_with_scheduler_registration(
                    ...
                    # (sanremes cron create --reasoning-effort) ONLY — it is
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional script run each tick; stdout is injected into the agent's prompt as context (with no_agent=True the script IS the job). Relative paths resolve under {display_sanremes_home()}/scripts/; .sh/.bash via bash, else Python. On update, '' clears."
            },
            "monitor": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("SANREMES_INTERACTIVE")
        or env_var_enabled("SANREMES_GATEWAY_SESSION")
        or env_var_enabled("SANREMES_EXEC_ASK")
    )
...
def _cronjob_handler(args, **kw):
    ...
    return cronjob(
        ...
        # `sanremes cron create/edit --model`, or hand-edited jobs). The agent
