...
# ``agent.auxiliary_client`` pulls credential_pool → sanremes_cli.auth → httpx
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("SANREMES_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: SANREMES_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("SANREMES_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
def _normalize_to_supported_image(
    ...
    out_dir = get_sanremes_dir("cache/vision", "temp_vision_images")
...
async def _vision_analyze_native(
    ...
    try:
        ...
        temp_dir = get_sanremes_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $SANREMES_HOME/cache/vision/ and cleaned up
...
async def _materialize_video_from_terminal_backend(video_source: str, task_id: Optional[str]) -> Path:
    ...
    temp_dir = get_sanremes_dir("cache/video", "temp_video_files")
    temp_dir.mkdir(parents=True, exist_ok=True)
