"""
`sanremes computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the SanRemes side — the only
...
def _cua_child_env() -> Dict[str, str]:
    """cua-driver child env with the SanRemes telemetry policy applied.
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with SanRemes provider secrets stripped.
...
def _build_identity(binary: str, report: Dict[str, Any]) -> Dict[str, Any]:
    """SanRemes-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(
    ...
    *,
    ...
    Honors `SANREMES_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if not binary:
        ...
        print("  Run: sanremes computer-use install")
        return 2
    ...
    if json_output:
        ...
        # attach SanRemes identity under sanremes_identity so existing parsers
        # that only read overall/checks keep working.
        payload = dict(report)
        payload["sanremes_identity"] = identity
