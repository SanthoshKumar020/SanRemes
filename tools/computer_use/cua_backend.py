...
etc.) surface as specific blocked checks via `sanremes computer-use doctor`
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and NousResearch/sanremes-agent#67052.
    """
...
# fetches the latest release, so a `SANREMES_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `SANREMES_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "SANREMES_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when SanRemes should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when SanRemes should disable cua-driver telemetry for this user.
...
def _cua_configured_permission_mode() -> str:
    ...
    per-session SanRemes YOLO toggle so a stale config line can never
...
def _cua_grant_existing_profile() -> bool:
    ...
    cua-driver's trusted-launcher grant. SanRemes passes
...
def _standard_runtime_launch_args(
    ...
    *,
    ...
    private_socket = socket_path or os.path.join(
        tempfile.gettempdir(), f"sanremes-cua-standard-{uuid.uuid4().hex[:12]}.sock"
    )
...
def _empty_discovery_reason() -> str:
    ...
    return (
        "window discovery returned no windows; run `sanremes computer-use "
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when SanRemes runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the SanRemes process uses POSIX subprocess spawning inside WSL.
    ...
    if not re.match(r"^[A-Za-z]:[\\/]", path):
        return path
    try:
        from sanremes_constants import is_wsl
    ...
    *,
    ...
    if not resolved_app:
        raise RuntimeError(
            ...
            "Run `sanremes computer-use install` to restore it."
        )
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one SanRemes session's YOLO choice
    ...
    * ``unrestricted`` — explicit SanRemes YOLO; launch-time risk
    ...
    * ``bounded`` — a user-reviewed capability manifest
    ...
    def __init__(
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\sanremes-cua-{token}"
        else:
    ...
    def start(self) -> None:
        ...
        self._stderr_thread = threading.Thread(
            ...
            args=(self._process,),
            name="sanremes-cua-daemon-stderr",
            daemon=True,
        )
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of NousResearch/sanremes-agent#47072: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted SanRemes an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``SANREMES_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    SanRemes Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `sanremes computer-use doctor` succeeds from a login shell.
    """
    ...
    if sys.platform == "win32":
        ...
        candidates.extend([
            ...
            # misses it until every SanRemes process is restarted.
            os.path.join(
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``SANREMES_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host SanRemes' 0.20 integration."""
    ...
    if version < _CUA_DRIVER_RUNTIME_CONTRACT_MIN:
        return {
            ...
            "binary": resolved,
            "version": raw_version,
            "reason": "SanRemes computer use requires cua-driver 0.20.0 or newer",
        }
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `sanremes computer-use install --upgrade`."
    )
...
def _maybe_repair_runtime_contract(contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``SANREMES_CUA_DRIVER_CMD`` override is authoritative even when broken, and
    ...
    try:
        from sanremes_cli.tools_config import install_cua_driver
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `sanremes tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of NousResearch/sanremes-agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of NousResearch/sanremes-agent#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize SanRemes-managed secrets out of the child env.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `sanremes computer-use doctor` and check "
                f"{display_sanremes_home()}/logs/agent.log for the phase timings."
            )
    ...
    def _call_tool_via_cli(self, name: str, args: Dict[str, Any], timeout: float) -> Dict[str, Any]:
        ...
        try:
            for attempt in range(attempts):
                ...
                # SanRemes talks to the direct `cua-driver mcp` runtime
    ...
    def _unknown_transport_outcome(name: str, exc: Exception) -> Dict[str, Any]:
        message = (
            ...
            "unknown, so SanRemes did not replay it. Take fresh state before "
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    NousResearch/sanremes-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of NousResearch/sanremes-agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per SanRemes run, with
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            # An installed-but-incompatible driver (e.g. predating a SanRemes
        ...
        if not contract.get("ready"):
            ...
            if os.environ.get(_CUA_DRIVER_CMD_ENV, "").strip():
                repair = (
                    "Update the binary selected by SANREMES_CUA_DRIVER_CMD or "
                    "remove that override."
                )
            else:
                repair = "Run `sanremes computer-use install` to repair it."
            raise RuntimeError(f"cua-driver is not ready: {reason}. {repair}")
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of SanRemes' minimal core.
    ...
    def capture(
        ...
        Maps sanremes `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of NousResearch/sanremes-agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of NousResearch/sanremes-agent#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See NousResearch/sanremes-agent#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # NousResearch/sanremes-agent#47072). cua-driver-rs gained an explicit
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the SanRemes adapter, and uses cua-driver's
