...
something different on each:
  ...
  * macOS — explicit TCC grants (Accessibility + Screen Recording). cua-driver
    ...
    the installed ``CuaDriver.app``), NOT SanRemes — so no SanRemes entitlement is
...
``sanremes computer-use permissions`` CLI, and ``/api/tools/computer-use/status``.
"""
...
def request_permissions_grant(driver_cmd: Optional[str] = None) -> int:
    ...
    binary = _resolve_driver_cmd(driver_cmd)
    if not binary:
        print("cua-driver: not installed. Run: sanremes computer-use install")
        return 2
