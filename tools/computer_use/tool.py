...
`health_report` (surfaced by `sanremes computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session SanRemes runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-SanRemes-session cached backends. Each backend owns its own cua-driver
...
# See NousResearch/sanremes-agent#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map SanRemes's explicit approval bypass onto Cua's immutable mode.

    SanRemes has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "SANREMES_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown SANREMES_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    SanRemes YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated SanRemes sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the SanRemes process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `sanremes computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (NousResearch/sanremes-agent#67052).
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of NousResearch/sanremes-agent#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    *,
    ...
    The captured base64 PNG is materialised to ``$SANREMES_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_sanremes_dir("cache/vision", "temp_vision_images")
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Save a capture in SanRemes' media cache and return its absolute path.
    ...
    try:
        ...
        cache_dir = get_sanremes_dir("cache/images", "image_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
        try:
            captures = sorted(
...
def _spill_elements_to_file(cap: CaptureResult) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_sanremes_dir("cache/computer_use", "computer_use_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
...
def check_computer_use_requirements() -> bool:
    ...
    `sanremes computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
