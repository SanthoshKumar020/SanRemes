...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from sanremes_state import apply_durability_barriers
    ...
    for name, sql_type in (
        ...
        ("delivery_claim", "TEXT"),
        ("delivery_claimed_at", "REAL"),
        # Raw api_server session id (X-SanRemes-Session-Id) of the ORIGINATING
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        for evt_key, env_name in (
            ("scope_id", "SANREMES_SESSION_SCOPE_ID"),
            ("user_id", "SANREMES_SESSION_USER_ID"),
            ("user_name", "SANREMES_SESSION_USER_NAME"),
        ):
...
def _current_origin_session_id() -> str:
    ...
    The obvious source — ``SANREMES_SESSION_ID`` via ``get_session_env`` — is
    ...
    The request-scoped ``SANREMES_SESSION_CHAT_ID`` binding survives child
    construction: ``_bind_api_server_session`` binds ``chat_id`` to the raw
    ``X-SanRemes-Session-Id``, and its only writer is ``set_session_vars`` —
    ...
    try:
        ...
        if get_session_env("SANREMES_SESSION_PLATFORM", "") != "api_server":
            return ""
        return get_session_env("SANREMES_SESSION_CHAT_ID", "") or ""
    except Exception:
        return ""
...
def dispatch_async_delegation(
    *,
    ...
    try:
        ...
        # get_sanremes_home() under the right profile.
