...
<url>``) is spawned as a direct child of the SanRemes process. SanRemes's own
...
SanRemes process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next SanRemes startup's opt-in
