...
Closes the gap tracked in sanremes-agent#96873: local execute_code holds a
...
"""Auto-generated SanRemes REMOTE session-kernel runner (file cell protocol)."""
import contextlib
import io
import json
import os
...
KDIR = os.environ["SANREMES_KERNEL_DIR"]
...
def _spawn_remote_kernel(env, env_type: str, owner: str, task_env_id: str,
    ...
    kernel_dir = f"{_env_temp_dir(env)}/sanremes_rkernel_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{kernel_dir}/sanremes_tools.py", tools_src)

        env_prefix = (
            f"SANREMES_KERNEL_DIR={q_dir} "
            f"SANREMES_RPC_DIR={shlex.quote(kernel_dir + '/rpc')} "
