...
409), and is driven by:
  ...
  * the ``sanremes sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``sanremes_cli/``) so the
...
is toggled via ``sanremes sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.sanremes/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``sanremes sync enable`` — it is
...
# namespace from sanremes-agent's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the SANREMES_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: SANREMES_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("SANREMES_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a SanRemes Cloud instance can be set
...
# precedence as base_url: the SANREMES_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   SANREMES_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   SANREMES_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   SANREMES_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``SANREMES_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a SanRemes Cloud deployment sets to opt its instances into sync by
    ...
    """
    return _sync_config_bool("SANREMES_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``SANREMES_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``sanremes sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "SANREMES_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``SANREMES_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``sanremes sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    SanRemes Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("SANREMES_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.sanremes/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.sanremes/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (SanRemes Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``SANREMES_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.sanremes/skills/ containing
...
def stable_device_id() -> str:
    ...
    ~/.sanremes/skills/.sync_device_id.
    ...
    Use ``set_device_name()`` / ``sanremes sync device --name`` to set an explicit
    label."""
    ...
    # SanRemes Cloud (and any templated deployment) can seed the label
    # declaratively via SANREMES_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `sanremes sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("SANREMES_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.sanremes/skills/.sync_device_id, overwriting
...
# ~/.sanremes/skills/.sync_state as JSON.
#
...
# path under ~/.sanremes/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.sanremes/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this SanRemes speaks "
            f"{WIRE_VERSION} — update SanRemes to sync with it"
        )
...
def push_skills(
    ...
    *,
    ...
    message: str = "sanremes skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (sanremes sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.sanremes/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "sanremes skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (SANREMES_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``sanremes sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.sanremes/skills/_org/
...
#
# `sanremes sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.sanremes/skills/_org/<org_id>/``.
