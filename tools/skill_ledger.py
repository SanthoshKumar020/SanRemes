...
``~/.sanremes/skills/.curator_ledger.jsonl`` describing who changed what, with
before/after file manifests whose contents are stored content-addressed
(sha256-deduped) under ``~/.sanremes/.curator_backups/blobs/``.
...
Design decisions (Teknium-approved):
  ...
  - The ledger covers ALL actors, tagged ``curator`` / ``agent`` / ``user``.
    ...
    ``sanremes curator rollback <entry-id>``.
...
def ledger_path() -> Path:
    return get_sanremes_home() / "skills" / ".curator_ledger.jsonl"


def blobs_dir() -> Path:
    return get_sanremes_home() / ".curator_backups" / "blobs"
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """All paths in an entry must live under SANREMES_HOME. Defense in depth —
