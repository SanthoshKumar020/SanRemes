...
SanRemes Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own SANREMES root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in sanremes_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "sanremes" if profile == "default" else profile
    if (
...
def _sanremes_cli() -> str:
    """Resolve the sanremes CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``sanremes``
    entrypoint. A bare ``"sanremes"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("sanremes.exe" if sys.platform == "win32" else "sanremes")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("sanremes")
    if found:
        return found
    return "sanremes"
...
# turns concurrently: deliveries spawn separate ``sanremes`` subprocesses, so
