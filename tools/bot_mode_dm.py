...
teammate agent (another SanRemes profile on this install, or an agent on a
...
DM each other via a prompt-injected ``sanremes -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``sanremes -p`` / ``sanremes peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``sanremes -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``sanremes peer dm <peer>[/<name>] < <tmp>``
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'sanremes' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('sanremes' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "sanremes":
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "sanremes-dm-*.txt"),
        (temp_root, "sanremes-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``sanremes -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative sanremes next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("sanremes", "sanremes.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("SANREMES_HOME") or os.path.expanduser("~/.sanremes"))
    return acquire_turn_lock(_sanremes_root(home), argv[2])
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("SANREMES_HOME") or os.path.expanduser("~/.sanremes")
