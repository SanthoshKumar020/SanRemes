#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the SanRemes Skills Hub.
...
Used by sanremes_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _sanremes_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            sanremes_meta = metadata.get("sanremes", {})
            if isinstance(sanremes_meta, dict):
                tags = sanremes_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            sanremes_meta = metadata.get("sanremes", {})
            if isinstance(sanremes_meta, dict):
                raw_tags = sanremes_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "NousResearch/sanremes-agent"
    OPTIONAL_SKILLS_PREFIX = "optional-skills"

    def __init__(self, auth: Optional[GitHubAuth] = None):
        from sanremes_constants import get_optional_skills_dir
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``sanremes update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                sanremes_meta = meta_block.get("sanremes", {})
                if isinstance(sanremes_meta, dict):
                    tags = sanremes_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``sanremes_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# SanRemes centralized index source
# ---------------------------------------------------------------------------

SANREMES_INDEX_URL = "https://sanremes-agent.nousresearch.com/docs/api/skills-index.json"
SANREMES_INDEX_TTL = 6 * 3600  # 6 hours


def _sanremes_index_cache_file() -> Path:
    return _index_cache_dir() / "sanremes-index.json"


def _load_sanremes_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SANREMES_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    sanremes_index_cache_file = _sanremes_index_cache_file()
    if sanremes_index_cache_file.exists():
        try:
            age = time.time() - sanremes_index_cache_file.stat().st_mtime
            if age < SANREMES_INDEX_TTL:
                return json.loads(sanremes_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("SanRemes index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "SanRemes index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("SanRemes index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        sanremes_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        sanremes_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class SanRemesIndexSource(SkillSource):
    """Skill source backed by the centralized SanRemes Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_sanremes_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "sanremes-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "sanremes-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        SanRemesIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "sanremes-index"
