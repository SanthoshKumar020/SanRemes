"""
Lazy dependency installer for opt-in SanRemes Agent backends.

Many SanRemes features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``sanremes-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``sanremes tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``SANREMES_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/sanremes`` read-only), setting
  ``SANREMES_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick SanRemes, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ─── Wake word ("Hey SanRemes") engines ──────────────────────────────────
    ...
    # (SANREMES_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (sanremes trace upload / /upload-trace).
    #
    ...
    # presence, the `sanremes update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "SANREMES_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``SANREMES_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("SANREMES_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``sanremes update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run SanRemes under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``sanremes update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="sanremes-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``sanremes_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = sanremes_subprocess_env(inherit_credentials=False)
        uv_env["VIRTUAL_ENV"] = str(venv_root)

        # Tier 1: uv (preferred — fast, doesn't need pip in the venv)
        # Managed uv first: $SANREMES_HOME/bin is never on PATH, so a bare
        # which() misses the uv SanRemes installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships SanRemes
    ...
    # deployment sets SANREMES_MANAGED=true *and* SANREMES_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from sanremes_cli.config import get_managed_system
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so SanRemes cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of SanRemes instead)."
            )
...
def feature_install_command(feature: str, *, venv_pip: bool = False) -> Optional[str]:
    ...
    (default install, ``SANREMES_HOME`` overrides, profile installs) and
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``SANREMES_DISABLE_LAZY_INSTALLS=1``) and sets
      ``SANREMES_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``sanremes update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``sanremes update``. Never raises; lazy-install failures
