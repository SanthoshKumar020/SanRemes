"""Wake-word ("Hey SanRemes") detection — hands-free session trigger.
...
wake word, SanRemes opens a fresh session and captures voice via the existing
...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey sanremes" model (``tools/wakewords/``) so the wake word
...
# Minimum gap between two consecutive wake fires, so one "hey sanremes" can't
...
# Bundled "hey sanremes" model (tools/wakewords/) — the default, so the wake word
# works out of the box. Config names in _ALIASES resolve to it, not a built-in.
_BUNDLED_MODEL_NAME = "hey_sanremes"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_sanremes", "hey sanremes", "sanremes"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_sanremes model (.onnx/.tflite) for ``framework``."""
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey sanremes"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    try:
        ...
        from sanremes_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return (
            "Microphone delivers only silence. Grant the SanRemes backend "
...
class _OpenWakeWordEngine(_Engine):
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # Default (or explicit "hey_sanremes") → the bundled model; a built-in name
...
# SANREMES_HOME. Keywords are typed phrases tokenized at RUNTIME — no
...
def _sherpa_model_root() -> Path:
    ...
    return get_sanremes_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    model's vocabulary, so "hey sanremes", "hey coder", or any other phrase works
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # wake any profile ("hey sanremes" / "hey coder" / ...). display-name →
        # profile is kept for routing the match back.
        phrase = str(_get(cfg, "phrase") or "hey sanremes").strip()
        ...
        kw = tempfile.NamedTemporaryFile(
            mode="w", suffix=".txt", prefix="sanremes-kws-", delete=False, encoding="utf-8"
        )
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if provider == "porcupine" and not (os.getenv("PORCUPINE_ACCESS_KEY") or "").strip():
        ...
        hint = (f"Wake word needs {missing} configured — run `sanremes tools` "
...
# ---------------------------------------------------------------------------
# Process-wide singleton (mirrors sanremes_cli.voice's continuous API)
# ---------------------------------------------------------------------------
...
def _lock_path() -> Path:
    ...
    return get_default_sanremes_root() / "runtime" / "wake-word.lock"
