...
it is automatically downloaded from GitHub releases to $SANREMES_HOME/bin/tirith.
...
def _get_sanremes_home() -> str:
    """Return the SanRemes home directory, respecting SANREMES_HOME env var."""
    return str(get_sanremes_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_sanremes_home(), ".tirith-install-failed")
...
def _sanremes_bin_dir() -> str:
    """Return $SANREMES_HOME/bin, creating it if needed."""
    d = os.path.join(_get_sanremes_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SANREMES_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_sanremes_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SANREMES_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SANREMES_HOME/bin/tirith
    ...
    sanremes_bin = os.path.join(_sanremes_bin_dir(), "tirith")
    if os.path.isfile(sanremes_bin) and os.access(sanremes_bin, os.X_OK):
        _resolved_path = sanremes_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return sanremes_bin
