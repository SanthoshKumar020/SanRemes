...
# Must match sanremes_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``sanremes-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``sanremes-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. sanremes-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (sanremes-cli,
    # sanremes-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child SanRemes/Qwen subagent onto the parent's Claude Messages wire (or the
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.sanremes/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_sanremes_dir("cache/delegation", "delegation_cache")
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("SANREMES_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from sanremes_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # SANREMES_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-SanRemes-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("SANREMES_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-SanRemes-Session-Id once the batch completes.
            ...
            # SANREMES_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `sanremes -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("SANREMES_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("SANREMES_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and SANREMES_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if configured_base_url and not _is_native_sdk_provider:
        ...
        from sanremes_cli.runtime_provider import _detect_api_mode_for_url
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'sanremes auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active SanRemes config.

    Prefer the shared persistent loader because it follows the active
    SANREMES_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``SANREMES_IGNORE_USER_CONFIG=1`` (``sanremes chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("SANREMES_IGNORE_USER_CONFIG") == "1"
