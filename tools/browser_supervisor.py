...
One ``CDPSupervisor`` runs per SanRemes ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach SanRemes logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "sanremes-dialog-bridge.invalid"
...
(() => {
  if (window.__sanremesDialogBridgeInstalled) return;
  window.__sanremesDialogBridgeInstalled = true;
  const ENDPOINT = "http://sanremes-dialog-bridge.invalid/";
