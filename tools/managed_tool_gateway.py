...
def auth_json_path():
    """Return the SanRemes auth store path, respecting SANREMES_HOME overrides."""
    return get_sanremes_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`sanremes tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a SanRemes release, and
