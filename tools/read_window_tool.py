#!/usr/bin/env python3
"""Read which OS window sits directly underneath the SanRemes desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the SanRemes window as a JSON string."""
    if callback is None:
        return tool_error(
            "read_window_below is only available in the SanRemes desktop app."
        )
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        "Identify the app window directly behind the SanRemes desktop window "
