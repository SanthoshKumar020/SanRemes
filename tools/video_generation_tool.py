...
provider** — turn on a backend by enabling a plugin (``sanremes plugins
enable video_gen/<name>``) and selecting it in ``sanremes tools`` → Video
Generation.
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `sanremes tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `sanremes tools` → "
...
# `sanremes tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`sanremes tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `sanremes tools` → Video Generation."
        )
        return {
            "description": "\n".join(parts),
            "parameters": {
