...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite SanRemes's live source checkout "
        ...
        "checkout, stop SanRemes, run the command externally, then restart "
        "SanRemes."
    )
...
def _scratch_dir_hint() -> str:
    ...
    sanremes_home = os.environ.get("SANREMES_HOME", "").strip()
    base = Path(sanremes_home).expanduser() if sanremes_home else Path.home() / ".sanremes"
