...
- **No server-bound identity ⇒ legacy.** Generic SanRemes callers keep the
...
def extension_controller_available(action: str) -> bool:
    ...
    try:
        ...
        if not browser_control_enabled():
            return False
        session_id = get_session_env("SANREMES_SESSION_ID", "") or None
        principal_id = get_session_env("SANREMES_BROWSER_CONTROL_PRINCIPAL", "") or None
        transport_family = get_session_env(
            "SANREMES_BROWSER_CONTROL_TRANSPORT_FAMILY", ""
        ) or None
...
def routed_browser_handler(
    ...
    *,
    ...
    try:
        ...
        session_id = session_id or get_session_env("SANREMES_SESSION_ID", "") or None
        principal_id = principal_id or get_session_env(
            "SANREMES_BROWSER_CONTROL_PRINCIPAL", ""
        ) or None
