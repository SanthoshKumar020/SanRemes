...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__SANREMES_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__SANREMES_FENCE_" in line or "\x1b]" in line
...
NOT_REGULAR_SENTINEL = "__sanremes_not_regular__"
...
class ShellFileOperations(FileOperations):
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        paths — and SanRemes disables MSYS argument conversion for its bash
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.sanremes-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".sanremes-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.sanremes-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.sanremes-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        snippet = (
            ...
            "    if size > MAX:\n"
            "        print('SANREMES_UTF16:NO'); sys.exit(0)\n"
            "    with open(p, 'rb') as f:\n"
            "        data = f.read()\n"
            "    sample = data[:SAMPLE]\n"
            "    enc = None\n"
        ...
        marker = stdout.find("SANREMES_UTF16:OK")
        if result.exit_code != 0 or marker < 0:
            return None
        payload = stdout[marker + len("SANREMES_UTF16:OK"):].strip()
        try:
    ...
    def write_file(self, path: str, content: str,
        ...
        # clean the temp up on any failure so we never leak a ``.sanremes-tmp``
    ...
    def _check_lint(self, path: str, content: Optional[str] = None) -> LintResult:
        ...
        # globals (`Window.sanremesDesktop` → TS2339) that are defined by the
    ...
    def _search_with_rg(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        # nothing converts back — SanRemes sets MSYS_NO_PATHCONV for its bash).
