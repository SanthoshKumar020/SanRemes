...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``sanremes tools`` repaint, tool-registration scans,
    ...
    2. ``~/.sanremes/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``sanremes auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_sanremes_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.sanremes/.env`` first, then ``os.environ``.

    Wraps :func:`sanremes_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from sanremes_cli.config import get_env_value as _sanremes_get_env_value
    except ImportError:
        return os.environ.get(name, default)

    value = _sanremes_get_env_value(name)
    return value if value is not None else default


def sanremes_xai_user_agent() -> str:
    """Return a stable SanRemes-specific User-Agent for xAI HTTP calls."""
    try:
        from sanremes_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"SanRemes-Agent/{__version__}"


def sanremes_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as SanRemes Agent,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": sanremes_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level SanRemes config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per SanRemes home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_sanremes_home() / "state"
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    Honors ``SANREMES_XAI_BASE_URL`` then ``XAI_BASE_URL`` (the same override
    pair the OAuth branch reads) and pins the origin with
    :func:`sanremes_cli.auth._xai_validate_inference_base_url` so a tampered
    env override can't exfiltrate the bearer; on rejection it falls back to
    the default rather than raising.
    """
    override = str(
        get_env_value("SANREMES_XAI_BASE_URL")
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers SanRemes-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``sanremes_cli.config.get_env_value`` so keys
    stored in ``~/.sanremes/.env`` (the standard SanRemes location) are honored —
    ...
    honors ``SANREMES_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
