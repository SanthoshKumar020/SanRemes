...
CHECKPOINT_PATH = get_sanremes_home() / "processes.json"
...
# ---------------------------------------------------------------------------
# When SanRemes runs as a systemd gateway with MemoryHigh/MemoryMax limits,
...
#
# Wrapping the spawn in ``systemd-run --user --scope --unit=sanremes-worker-<pid>``
        ...
        if not _IS_WINDOWS:
            try:
                ...
                if binary:
                    ...
                    probe_unit = f"sanremes-probe-scope-{os.getpid()}-{uuid.uuid4().hex[:8]}"
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised SanRemes gateway runtime.

    Both supervisor markers and ``_SANREMES_GATEWAY`` are inherited by every
    ...
    """
    if os.environ.get("_SANREMES_GATEWAY") != "1":
        return False
...
def _build_systemd_scope_argv(
    ...
    if binary is None:
        ...
        return shell_argv
    unit_name = f"sanremes-worker-{unit_suffix}"
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from sanremes_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def spawn_local(
        ...
        if use_pty:
            ...
            try:
                ...
                if pty_use_systemd_scope:
                    ...
                    session.systemd_unit = f"sanremes-worker-{session.id}.scope"
        ...
        if use_systemd_scope:
            ...
            session.systemd_unit = f"sanremes-worker-{unit_suffix}.scope"
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/sanremes_bg_{session.id}.log"
        pid_path = f"{temp_dir}/sanremes_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/sanremes_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``sanremes_cli.goals``) to support
    ...
    def wait_for_pending_completions(
        ...
        *,
        ...
        One-shot CLI runs (``sanremes -q/-Q/-z``) exit as soon as their single
        ...
        (#90879): a recipient invoked as ``sanremes -p <bot> chat -Q
    ...
    def _oneshot_completion_wait_seconds() -> float:
        ...
        """
        try:
            from sanremes_cli.config import DEFAULT_CONFIG, cfg_get, read_raw_config
            cfg = read_raw_config()
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `sanremes update` restarting the gateway) is still
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the SanRemes desktop app.",
            }
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            ...
                            # ~/.sanremes/processes.json with the raw command
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'SanRemes'}"
    elif _reason == "lost":
