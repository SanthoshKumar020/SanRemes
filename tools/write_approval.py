...
(see ``sanremes_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<SANREMES_HOME>/pending/{memory,skills}/<id>.json``
...
def _pending_dir(subsystem: str) -> Path:
    return get_sanremes_home() / "pending" / subsystem
