...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.sanremes/.env``.
...
from sanremes_cli.config import cfg_get, load_config, read_raw_config
...
def is_camofox_mode() -> bool:
    ...
    ``sanremes tools``). ``CAMOFOX_URL`` is the server ADDRESS only — its
...
def _managed_persistence_enabled() -> bool:
    """Return whether SanRemes-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    so SanRemes operates in the same browser profile instead of creating a
...
def _adopt_existing_tab_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether SanRemes should recover an existing Camofox tab ID."""
...
def _loopback_rewrite_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    ...
    ``http://127.0.0.1:9377``.  That is correct for SanRemes talking to the
...
def _adopt_existing_tab(session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Some integrations own the visible Camofox tab outside SanRemes. Gateway
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the SanRemes profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if identity_override:
            ...
            session = {
                "user_id": f"sanremes_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_sanremes_home() / "browser_screenshots"
