...
Design contract (matches the SanRemes "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    sanremes_meta = meta.get("sanremes") if isinstance(meta, dict) else None
    if not isinstance(sanremes_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.sanremes.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in sanremes_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.sanremes.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("sanremes", "agent", "sanremes agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'SanRemes Agent' (proper caps) "
