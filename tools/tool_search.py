"""Progressive tool disclosure ("tool search") for SanRemes Agent.
...
``tool_call`` — and surfaced on demand. Core SanRemes tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._SANREMES_CORE_TOOLS`` are *never* deferred.
...
def load_config_readonly() -> ToolSearchConfig:
    ...
    try:
        from sanremes_cli.config import load_config_readonly as _load
        cfg = _load() or {}
        tools_cfg = cfg.get("tools") if isinstance(cfg.get("tools"), dict) else {}
        if not isinstance(tools_cfg, dict):
            tools_cfg = {}
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_SANREMES_CORE_TOOLS)
    except Exception:
        return frozenset()


# Session-gated GUI toolsets. Off ``_SANREMES_CORE_TOOLS`` so non-GUI clients
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is neither in ``_SANREMES_CORE_TOOLS`` nor a session-gated GUI
