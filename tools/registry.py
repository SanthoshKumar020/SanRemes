"""Central registry for all sanremes-agent tools.
...
from sanremes_constants import sanremes_home_key
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (sanremes_constants itself is stdlib-only, so no cycle).
        from sanremes_constants import get_sanremes_home

        return Path(get_sanremes_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``sanremes tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a SanRemes-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("SANREMES_SESSION_ID", ""),
            get_session_env("SANREMES_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("SANREMES_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``sanremes tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved SANREMES_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("sanremes_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``sanremes_plugins.{name}``), not the exact module string.
                # A handler defined in ``sanremes_plugins.pkg.handlers`` is
                # still owned by the ``sanremes_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``sanremes tools enable foo``)
