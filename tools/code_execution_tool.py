...
Lets the LLM write a Python script that calls SanRemes tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `sanremes_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `sanremes_tools.py` with file-based RPC stubs
...
# available on every platform SanRemes itself runs on.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        import hashlib
        from sanremes_constants import get_sanremes_dir
        ...
        cache_dir = get_sanremes_dir("cache/exec", "exec_spill")
...
# match a safe prefix, the operational SANREMES_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports SanRemes
# code can lose the DB-layer Kanban mutation guard while still inheriting
# SANREMES_HOME.
#
# NB: the broad "SANREMES_" prefix was deliberately removed (#27303) — it leaked
# SANREMES_*-named config that lacks a secret substring (e.g. SANREMES_BASE_URL,
# SANREMES_KANBAN_DB, SANREMES_*_WEBHOOK).  The child only needs the few
# location/profile vars in _SANREMES_CHILD_ALLOWED below; SANREMES_RPC_SOCKET /
# SANREMES_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational SANREMES_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set sanremes_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_SANREMES_CHILD_ALLOWED = frozenset({
    "SANREMES_HOME",
    "SANREMES_PROFILE",
    "SANREMES_CONFIG",
    "SANREMES_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational SANREMES_* vars (_SANREMES_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret SANREMES_* vars dropped by the tightened allowlist (#27303). The
    # broad "SANREMES_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like SANREMES_KANBAN_DB / SANREMES_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _SANREMES_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("SANREMES_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped SANREMES_* var.
            _dropped_sanremes.append(k)
    if _dropped_sanremes:
        logger.debug(
            "execute_code: dropped %d non-allowlisted SANREMES_* var(s) from the "
            ...
            len(_dropped_sanremes),
            ", ".join(sorted(_dropped_sanremes)),
        )
...
# ---------------------------------------------------------------------------
# sanremes_tools.py code generator
# ---------------------------------------------------------------------------
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    sanremes_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'sanremes_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_sanremes_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the sanremes_tools.py stub module.
...
"""Auto-generated SanRemes tools RPC stubs."""
...
def _connect():
    ...
    SANREMES_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["SANREMES_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("SANREMES_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (SANREMES_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("SANREMES_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated SanRemes tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("SANREMES_RPC_DIR") or os.path.join(tempfile.gettempdir(), "sanremes_rpc")
_seq = 0
...
def _execute_remote(
    ...
    sandbox_dir = f"{temp_dir}/sanremes_exec_{sandbox_id}"
    ...
    try:
        ...
        # --- Session-kernel path (sanremes-agent#96873) -------------------
        ...
        tools_src = generate_sanremes_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/sanremes_tools.py", tools_src)
        ...
        env_prefix = (
            f"SANREMES_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"SANREMES_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("SANREMES_TIMEZONE", "").strip()
        if tz:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["SANREMES_RPC_SOCKET"] = rpc_endpoint
    child_env["SANREMES_RPC_TOKEN"] = rpc_token
    ...
    # SANREMES_TIMEZONE is an internal SanRemes setting and must not leak
    # into child processes.
    _tz_name = os.getenv("SANREMES_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("SANREMES_TIMEZONE", None)

    apply_subprocess_home_env(child_env)
    # ``sanremes_tools.py`` always lives in the staging directory, so that
    # directory must be importable even when project mode changes CWD.
    # SanRemes's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing SanRemes's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip SanRemes-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-SanRemes-
    ...
    _strip_sanremes_owned_pythonpath(child_env)
    _sanremes_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_sanremes_python_environment(child_python):
        _pp_parts.append(_sanremes_root)
    elif child_python not in _external_env_logged:
        # Import behavior changes silently otherwise — surface it (once
        # per interpreter path) so "import sanremes_constants suddenly
        # fails" reports are diagnosable without log spam.
        _external_env_logged.add(child_python)
        logger.info(
            "execute_code: child interpreter %s is outside the SanRemes "
            "environment; sanremes root omitted from PYTHONPATH",
            child_python,
        )
    if _existing_pp:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.sanremes.gateway`, but the
    ...
    # --- Set up temp directory with sanremes_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="sanremes_sandbox_")
    ...
    # selector from SANREMES_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"sanremes_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated sanremes_tools module.
        ...
        tools_src = generate_sanremes_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "sanremes_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  SANREMES_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.sanremes/.env')).
...
def _load_config() -> dict:
    ...
    ``sanremes --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (sanremes-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the SanRemes environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the sanremes root from every
...
def _uses_sanremes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to SanRemes's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``sanremes tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and sanremes-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.sanremes/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "SanRemes's own python (the common case — stdlib plus SanRemes's "
    ...
    description = (
        "Run Python that calls SanRemes tools programmatically. Use when you "
        ...
        "loses that state.\n\n"
        f"Available via `from sanremes_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from sanremes_tools import {import_str}` "
