...
``sanremes mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the SanRemes desktop app. Use the "
            "terminal instead: `sanremes mcp install <name>` for catalog entries, "
            "`sanremes mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "unanswered, continue without it. Catalog names: `sanremes mcp "
