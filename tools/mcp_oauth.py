...
URL of SanRemes' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``SanRemesTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "SanRemes Agent"
...
from sanremes_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__sanremes_user_skipped__"
...
def _get_token_dir(sanremes_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses SANREMES_HOME so each profile gets its own OAuth tokens.
    Layout: ``SANREMES_HOME/mcp-tokens/``
    """
    from sanremes_constants import get_sanremes_home

    base = Path(sanremes_home) if sanremes_home is not None else Path(get_sanremes_home())
...
def _cached_redirect_port(storage: "SanRemesTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If SanRemes restarts and chooses a
...
def _cached_redirect_uri(storage: "SanRemesTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``sanremes mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `sanremes mcp login <server>` interactively to (re)authorize, "
...
def _write_json(path: Path, data: dict) -> None:
    ...
    # sanremes-agent install tree (#25821, #93050).
...
# ---------------------------------------------------------------------------
# SanRemesTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class SanRemesTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        SANREMES_HOME/mcp-tokens/<server_name>.json         -- tokens
        SANREMES_HOME/mcp-tokens/<server_name>.client.json   -- client info
        SANREMES_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        SANREMES_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """

    def __init__(self, server_name: str, *, sanremes_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._sanremes_home = Path(sanremes_home) if sanremes_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._sanremes_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._sanremes_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._sanremes_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._sanremes_home) / f"{self._server_name}.cimd-off"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # SanRemes records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``sanremes mcp login`` /
        ``sanremes mcp remove``, so a fixed document gets another chance.
        """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._sanremes_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to SanRemes.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://sanremes-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"SanRemes' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `sanremes mcp login <server>` later to "
...
SanRemesOAuthClientProvider: Any = None


def _get_sanremes_oauth_provider_class() -> type | None:
    global SanRemesOAuthClientProvider
    if SanRemesOAuthClientProvider is not None:
        return SanRemesOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _SanRemesOAuthClientProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, token_user_agent: "str | None" = None, **kwargs: Any):
            ...
            self._sanremes_token_user_agent = token_user_agent

        def _stamp_token_user_agent(self, request):
            ua = getattr(self, "_sanremes_token_user_agent", None)
            if ua:
    ...
    _SanRemesOAuthClientProvider.__name__ = "SanRemesOAuthClientProvider"
    _SanRemesOAuthClientProvider.__qualname__ = "SanRemesOAuthClientProvider"
    SanRemesOAuthClientProvider = _SanRemesOAuthClientProvider
    return SanRemesOAuthClientProvider
...
def remove_oauth_tokens(
    server_name: str,
    *,
    sanremes_home: str | Path | None = None,
) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    storage = SanRemesTokenStorage(server_name, sanremes_home=sanremes_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; SanRemes only decides whether a
...
# sanremes-agent.nousresearch.com/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = (
    "https://nousresearch.github.io/sanremes-agent/docs/oauth/client-metadata.json"
)
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port SanRemes picks
...
def _has_cached_client_info(storage: "SanRemesTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "SanRemesTokenStorage | None") -> bool:
    ...
    SanRemes has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "SanRemesTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI SanRemes would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "SanRemes Agent" / "SanRemes" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "SanRemes Agent")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. SanRemes is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: sanremes mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. SanRemes auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older SanRemes is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = SanRemesTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`sanremes mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_sanremes_oauth_provider_class()
