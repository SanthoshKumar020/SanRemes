...
"""Auto-generated SanRemes session-kernel runner. One exec cell per request."""
import contextlib
import io
import json
import os
...
_SENTINEL = os.environ["SANREMES_KERNEL_SENTINEL"]
_CAPTURE_LIMIT = {capture_limit}
_SPILL_DIR = os.environ.get("SANREMES_KERNEL_SPILL_DIR", "")
...
# sanremes-agent#88637 by @z80dev (stable owner id, owner-teardown disposal,
...
def _resolve_owner(task_id: str) -> str:
    ...
    try:
        ...
        if is_delegated_child_context():
            ...
            child_id = get_session_env("SANREMES_SESSION_ID", "") or (task_id or "")
...
def shutdown_kernels_for_owner(owner: str) -> None:
    ...
    (the /new + session-close disposal shape from sanremes-agent#88637).
    """
    if not owner:
        return
    with _KERNELS_LOCK:
...
def _rpc_forever(kernel: SessionKernel, max_tool_calls: int,
    ...
    client stub reconnects on its side (SANREMES_RPC_PERSISTENT).
...
def _spawn(kernel: SessionKernel, *, task_id: str, child_python: str,
    ...
    kernel.tmpdir = tempfile.mkdtemp(prefix="sanremes_kernel_")
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()

    kernel.rpc_token = secrets.token_urlsafe(32)
    kernel.sentinel = "@@SANREMES-KERNEL-" + secrets.token_urlsafe(16) + "@@"
    ...
    if _IS_WINDOWS:
        ...
        kernel.sock_path = os.path.join(_sock_tmpdir, f"sanremes_rpc_{uuid.uuid4().hex}.sock")
    ...
    with open(os.path.join(kernel.tmpdir, "sanremes_tools.py"), "w", encoding="utf-8") as f:
        f.write(tools_src)
    runner_path = os.path.join(kernel.tmpdir, "sanremes_kernel_runner.py")
    ...
    child_env["SANREMES_KERNEL_SENTINEL"] = kernel.sentinel
    ...
    child_env["SANREMES_KERNEL_SPILL_DIR"] = kernel.tmpdir
    ...
    child_env["SANREMES_RPC_PERSISTENT"] = "1"
