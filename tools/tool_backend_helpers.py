...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `sanremes model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``sanremes auth add
    ...
    2. The environment / ``~/.sanremes/.env``. Under a multiplexed gateway turn
       ...
       ``sanremes_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``sanremes auth
    ...
    when omitted, ``sanremes_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `sanremes auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``sanremes auth add openai-api``) via
...
def read_selection(section: str) -> str | None:
    """Return the stored `sanremes tools` provider string for a config section.
    ...
    """
    try:
        from sanremes_cli.config import read_raw_config_readonly
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via sanremes "
        f"tools), but {failure}. Run 'sanremes tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.sanremes/.env`` (via
    ``sanremes_cli.config.get_env_value`` when available) so tool-side
