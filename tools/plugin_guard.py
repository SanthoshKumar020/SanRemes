...
warn / fail). SanRemes already scans hub-installed *skills* via
``tools/skills_guard.py``; this module extends the same static-analysis
engine to ``sanremes plugins install`` and ``sanremes plugins update``, which
...
- Exempts the "reads own env secret" / "HTTP call with key" pattern
  ...
  foreign credential-store access (~/.ssh, ~/.aws, ~/.sanremes/.env),
...
#
# ``sanremes_env_access`` (a reference to ``~/.sanremes/.env``) is the DOCUMENTED
...
def scan_plugin(plugin_dir: Path, source: str = "") -> ScanResult:
    ...
    Args:
        plugin_dir: Path to the plugin directory (typically the temp clone,
            before it is moved into ``~/.sanremes/plugins/``).
