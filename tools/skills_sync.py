...
Copies bundled skills from the repo's skills/ directory into ~/.sanremes/skills/
...
The manifest lives at ~/.sanremes/skills/.bundled_manifest.
"""
...
for _stream in (sys.stdout, sys.stderr):
    if hasattr(_stream, "reconfigure"):
        try:
            ...
            pass
from sanremes_constants import get_bundled_skills_dir, get_sanremes_home, get_optional_skills_dir
...
# cron, kanban workers) import this module once under the launch SANREMES_HOME
# and later scope requests to a different profile via
# set_sanremes_home_override(). Frozen module constants would then resolve —
...
# re-resolve from the live profile-scoped SANREMES_HOME on every call.
_SANREMES_HOME_AT_IMPORT = SANREMES_HOME
_SKILLS_DIR_AT_IMPORT = SKILLS_DIR
_MANIFEST_FILE_AT_IMPORT = MANIFEST_FILE


def _sanremes_home() -> Path:
    """Return the active profile's SANREMES_HOME at call time."""
    configured = Path(SANREMES_HOME)
    if configured != _SANREMES_HOME_AT_IMPORT:
        return configured
...
# Marker file written by `sanremes profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.sanremes profile).
# When present in SANREMES_HOME, sync_skills() is a no-op so neither the
# installer, `sanremes update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# sanremes_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _essential_names() -> frozenset:
    ...
    try:
        ...
        return frozenset({"sanremes-agent"})


def _get_bundled_dir() -> Path:
    """Locate the bundled skills/ directory.

    Checks SANREMES_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.sanremes/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.sanremes/skills/mlops/axolotl
    """
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``sanremes skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `sanremes skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.sanremes/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.sanremes) that wrote the
    ...
    # ``sanremes-agent`` skill is the agent's own operating manual and the
    ...
    essential_only = (_sanremes_home() / NO_BUNDLED_SKILLS_MARKER).exists()
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.sanremes/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `sanremes update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `sanremes skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``SANREMES_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.sanremes/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``SANREMES_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `sanremes skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `sanremes update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``sanremes update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``sanremes update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``sanremes skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `sanremes skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes SANREMES_HOME/.no-bundled-skills so the
    installer, ``sanremes update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``sanremes skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = _sanremes_home() / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            _sanremes_home().mkdir(parents=True, exist_ok=True)
            marker.write_text(
                "This profile opted out of bundled-skill seeding "
                "(`sanremes skills opt-out`).\n"
            ...
            changed = existed
            message = (
                "Opted back in. The next `sanremes update` (or `sanremes skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_sanremes_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.sanremes/skills/ ...")
