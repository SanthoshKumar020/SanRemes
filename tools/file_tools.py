...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``sanremes_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from sanremes_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where sanremes previously hard-rejected an oversized read
...
def _terminal_env_type_for_task(task_id: str = "default") -> str:
    ...
    try:
        ...
        if env is not None:
            ...
            if "daytona" in name:
                return "daytona"
            stamped = getattr(env, "_sanremes_backend_name", None)
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as SanRemes.
...
_sanremes_config_resolved: str | None = None
_sanremes_config_resolved_loaded = False


def _get_sanremes_config_resolved() -> str | None:
    """Return the resolved absolute path of the SanRemes config file (cached)."""
    global _sanremes_config_resolved, _sanremes_config_resolved_loaded
    if _sanremes_config_resolved_loaded:
        return _sanremes_config_resolved
    _sanremes_config_resolved_loaded = True
    try:
        from sanremes_cli.config import get_config_path
        _sanremes_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _sanremes_config_resolved = str(Path(_expand_tilde("~/.sanremes/config.yaml")).resolve())
        except Exception:
            _sanremes_config_resolved = None
    return _sanremes_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the SanRemes config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    sanremes_config = _get_sanremes_config_resolved()
    if sanremes_config and (resolved == sanremes_config or normalized == sanremes_config):
        return (
            f"Refusing to write to SanRemes config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.sanremes/config.yaml directly or use 'sanremes config' instead."
        )
    return None
...
# .cursorrules (or a project-local .sanremes config tree) outlives the current
...
_real_sanremes_home_cached: str | None = None
_real_sanremes_home_loaded = False


def _get_real_sanremes_home() -> str | None:
    """Return the realpath of the authoritative SanRemes home (cached)."""
    global _real_sanremes_home_cached, _real_sanremes_home_loaded
    if _real_sanremes_home_loaded:
        return _real_sanremes_home_cached
    _real_sanremes_home_loaded = True
    try:
        from sanremes_constants import get_sanremes_home
        _real_sanremes_home_cached = os.path.realpath(str(get_sanremes_home()))
    except Exception:
        try:
            _real_sanremes_home_cached = os.path.realpath(_expand_tilde("~/.sanremes"))
        except Exception:
            _real_sanremes_home_cached = None
    return _real_sanremes_home_cached
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from sanremes_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.sanremes home is governed by its own guards
    ...
    # ``.sanremes`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_sanremes_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .sanremes config dirs (e.g. <repo>/.sanremes/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.sanremes`` — matching
        # any ancestor named .sanremes would gate every write inside a
        # checkout that happens to live under ~/.sanremes (e.g. the
        # sanremes-agent repo itself at ~/.sanremes/sanremes-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".sanremes":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side SanRemes mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of SanRemes state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host SanRemes process never reads the mirror, so
    ...
    Returns ``None`` when the write is in-scope or outside SanRemes scope.
    """
    try:
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── SanRemes internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under SANREMES_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == SANREMES_HOME (e.g. "auth.json") still hits the
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with sanremes_tools.write_file() for very "
            "large files."
        )
