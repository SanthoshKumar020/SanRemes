...
~/.sanremes/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `sanremes config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.sanremes/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch SANREMES_HOME and
    ...
    live profile-scoped SANREMES_HOME on every call.
    """
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    ``sanremes-agent``) are treated as permanently pinned: the system prompt
    ...
    try:
        ...
        if name in ESSENTIAL_SKILLS:
            return (
                f"Skill '{name}' is essential to SanRemes (the agent's own "
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`sanremes curator unpin {name}` if they want to delete it. "
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`sanremes curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        # shapes; `sanremes curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `sanremes curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.sanremes/skills/) first, then any
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `sanremes sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `sanremes sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `sanremes sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `sanremes sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER SanRemes profiles.
    ...
    # Default profile (~/.sanremes/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.sanremes/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"profiles (`sanremes -p {other_profile}`) or edit the file "
            ...
            base += (
                ...
                f"Switch profiles (`sanremes -p <name>`) to edit there, or "
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path) -> None:
    ...
    result["lint_hint"] = (
        ...
        "to match SanRemes skill standards."
    )
...
def _patch_skill(
    ...
    if not old_string:
        ...
        # how to recover. Upstream: NousResearch/sanremes-agent#33064.
        return {
            "success": False,
            "error": (
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.sanremes/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `sanremes curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `sanremes curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"{display_sanremes_home()}/skills/; must precede that skill's other "
