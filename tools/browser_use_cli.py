...
_PRIVATE_BROWSER_SENTINEL = "_SANREMES_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# sanremes: pin this named session to its own tab (once per daemon process)
def _sanremes_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "sanremes-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_sanremes_ensure_own_tab()
del _sanremes_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from SanRemes's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at SanRemes's venv site-packages, and a
...
def _read_browser_cfg() -> dict:
    ...
    try:
        from sanremes_cli.config import cfg_get, read_raw_config
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_sanremes_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `sanremes tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """SanRemes' own bin dir ($SANREMES_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from sanremes_constants import get_sanremes_home

        return str(Path(get_sanremes_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: SanRemes' own ``$SANREMES_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    SanRemes-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: SanRemes' managed uv (bootstrapped on demand via
    ``sanremes_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$SANREMES_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical SanRemes-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through sanremes tools).
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_sanremes_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `sanremes tools` → Browser Automation."
        )
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser SanRemes launches on a
    SNAPSHOT of their real profile (see sanremes_cli.browser_connect). Two ways
    in:
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a SanRemes-managed copy of "
