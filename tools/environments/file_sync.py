...
_FORCE_SYNC_ENV = "SANREMES_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.sanremes") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.sanremes to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.sanremes", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``SANREMES_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, sanremes_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.sanremes/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(sanremes_home=sanremes_home)

    def _sync_back_transaction(self, sanremes_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .sanremes/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (sanremes_home or get_sanremes_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="sanremes-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.sanremes/skills/a.md`` →
        ``~/.sanremes/skills/a.md``, a new remote file at
        ``/root/.sanremes/skills/b.md`` maps to ``~/.sanremes/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
