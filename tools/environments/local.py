...
from sanremes_constants import get_process_sanremes_home
...
#
# get_temp_dir() now defaults to SANREMES_HOME/cache/terminal (real storage)
...
#
# Background-process artifacts come in triplets (sanremes_bg_<id>.log/.pid/
...
_BG_GROUP_RE = re.compile(r"^(sanremes_bg_[A-Za-z0-9_-]+)\.(log|pid|exit)$")


def _default_terminal_temp_dir() -> "Path | None":
    """Return SANREMES_HOME/cache/terminal, or None if unresolvable."""
    try:
        from sanremes_constants import get_sanremes_home
        return get_sanremes_home() / "cache" / "terminal"
    except Exception:
        return None
...
def cleanup_terminal_temp_cache(
    ...
    Only prunes the managed default dir (``SANREMES_HOME/cache/terminal``).
    ...
    # Newest mtime per sanremes_bg_<id> group, so a live server's fresh .log
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory SanRemes was already launched from (for example ``sanremes-agent``
    while the process cwd is ``~/.sanremes/sanremes-agent``), passing it through
    unchanged makes the wrapper run ``cd sanremes-agent`` *inside* the project
    ...
    # Common recovery for config values like ``sanremes-agent`` when SanRemes was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./sanremes-agent``; use the current directory instead.
...
# SanRemes-internal env vars that should NOT leak into terminal subprocesses.
_SANREMES_PROVIDER_ENV_FORCE_PREFIX = "_SANREMES_FORCE_"

# SanRemes-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a SanRemes inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from sanremes_cli.auth import PROVIDER_REGISTRY
    ...
    try:
        from sanremes_cli.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "SANREMES_DASHBOARD_SESSION_TOKEN",
    ...
    # SanRemes-managed inference credential — Claude subscription auth is not a
    # working SanRemes provider path.  Stripping it broke agent-spawned
...
_SANREMES_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the SanRemes venv path instead of the project's own
# ``.venv`` — silently clobbering the SanRemes environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# SanRemes venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the SanRemes venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). SanRemes itself treats PYTHONHOME as contamination in its own
...
# _strip_sanremes_owned_pythonpath() which removes only SanRemes-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_sanremes_internal_secret(key: str) -> bool:
    """Return True for SanRemes-internal secrets injected under *dynamic* names.

    ``_SANREMES_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "SanRemes-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`sanremes_subprocess_env` helper all call it, so the
...
def _inject_context_sanremes_home(env: dict) -> None:
    """Bridge the context-local SanRemes home override into subprocess env."""
    try:
        from sanremes_constants import get_sanremes_home_override

        value = get_sanremes_home_override()
        if value:
            env["SANREMES_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (SANREMES_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter SanRemes-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_SANREMES_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_sanremes_internal_secret(key):
            continue
        if key in _plugin_strip:
            continue
        passthrough = _is_passthrough(key)
        if key in _SANREMES_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_SANREMES_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_SANREMES_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_sanremes_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_sanremes_internal_secret(key):
            continue
    ...
    _inject_context_sanremes_home(sanitized)

    from sanremes_constants import apply_subprocess_home_env
    ...
    # the separate SanRemes runtime venv.  The filter validates that relationship
    # against the repo layout before trusting it.
    _strip_sanremes_owned_pythonpath_and_runtime_markers(sanitized)

    # Keep bare ``sanremes`` invocations available to child jobs even when the
    ...
    if path_key is not None:
        sanitized[path_key] = _prepend_sanremes_bin_dir(sanitized.get(path_key, ""))
...
# legitimate child SanRemes spawns needs them, and they are the highest-value
...
# narrow subset of _SANREMES_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in sanremes_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_sanremes_internal_secret, but _ID has no secret suffix, so it must be
...
def sanremes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_SANREMES_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child SanRemes spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_SANREMES_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _plugin_terminal_env_strip_keys():
        env.pop(key, None)
    # Internal routing hints and SanRemes-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_sanremes_internal_secret`.
    for key in list(env):
        if key.startswith(_SANREMES_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_sanremes_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _SANREMES_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_sanremes_home(env)
    from sanremes_constants import apply_subprocess_home_env
    apply_subprocess_home_env(env)

    _strip_sanremes_owned_pythonpath_and_runtime_markers(env)

    _apply_windows_msys_bash_env_defaults(env)

    # Cross-session leak guard, same as the terminal spawn paths: this helper
    # copies os.environ, whose SANREMES_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's SANREMES_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`sanremes_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``SANREMES_HOME`` / subprocess ``HOME`` contract) and the
    SanRemes secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_sanremes_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``SANREMES_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local SanRemes home override into ``SANREMES_HOME`` and apply the
      subprocess HOME contract (``sanremes_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``SANREMES_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs SANREMES_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("SANREMES_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale SANREMES_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\sanremes\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\sanremes\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _sanremes_portable_git = os.path.join(_local_appdata, "sanremes", "git") if _local_appdata else ""
    if _sanremes_portable_git:
        for candidate in (
            os.path.join(_sanremes_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_sanremes_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # SANREMES_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\sanremes\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "SANREMES_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. SanRemes Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set SANREMES_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart SanRemes. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``sanremes`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_SANREMES_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_sanremes_bin_dir() -> str | None:
    """Return the directory holding the ``sanremes`` console-script, or None.
    ...
    sanremes install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``sanremes``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``sanremes`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``sanremes`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("sanremes")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``sanremes`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _SANREMES_BIN_DIR
    if _SANREMES_BIN_DIR is not _SENTINEL:
        return _SANREMES_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("sanremes")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "sanremes" or base.startswith("sanremes."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "sanremes.exe" if _IS_WINDOWS else "sanremes"
    ...
    _SANREMES_BIN_DIR = candidate
    return candidate


def _prepend_sanremes_bin_dir(existing_path: str) -> str:
    """Prepend the sanremes install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_sanremes_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing SanRemes-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes SanRemes installs for
    itself, so on a machine where SanRemes provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$SANREMES_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$SANREMES_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_sanremes_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_sanremes_managed_node``, a first browser install).
    """
    try:
        from sanremes_constants import get_sanremes_home, iter_sanremes_node_dirs

        candidates = [*iter_sanremes_node_dirs(), get_sanremes_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    SanRemes-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  SanRemes
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_SANREMES_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_SANREMES_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_sanremes_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_sanremes_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _SANREMES_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the sanremes install dir is reachable so plugins can shell out
        # to bare ``sanremes`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_sanremes_bin_dir(new_path)

    _inject_context_sanremes_home(run_env)
    ...
    _strip_sanremes_owned_pythonpath_and_runtime_markers(run_env)
...
def _build_sanremes_repo_root_aliases(
    resolved_root: Path,
    lexical_root: Path,
    configured_home: Path,
) -> tuple[Path, ...]:
    """Return exact repo-root spellings emitted by SanRemes launchers.

    ``gateway_windows._preserve_sanremes_home_path`` maps a physical path under
    the resolved SANREMES_HOME back onto the configured SANREMES_HOME spelling.
    Mirror that producer contract here so a junction-backed install is matched
    without treating arbitrary descendants of SANREMES_HOME as SanRemes-owned.
    ...
    # lexically the same way get_default_sanremes_root() does (parent of a
    ...
    # while still never matching arbitrary descendants of SANREMES_HOME.
    ...
    # junction/symlink under the configured root (e.g. D:\sanremes\sanremes-agent
    # -> C:\...\sanremes-agent) while the import spelling (editable install)
    ...
    # strict resolve before accepting it as SanRemes-owned.  Fail-closed: a
...
# --- SanRemes venv / repo-root detection (module-level, computed once) ---

#: The SanRemes repository root - three levels up from this file
#: (``tools/environments/local.py`` -> ``tools/environments`` -> ``tools``
#: -> repo root).  This is the directory the Electron app prepends to
#: PYTHONPATH so the backend can do ``import tools``, ``import sanremes_cli``,
#: etc.  Subprocesses that are NOT the SanRemes backend don't need it and it
#: can shadow local packages.
_sanremes_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that SanRemes launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders SanRemes-owned paths under
#: the configured SANREMES_HOME spelling (which may be a junction to another
#: drive — see ``sanremes_cli/gateway_windows.py::_preserve_sanremes_home_path``).
...
_sanremes_repo_root_aliases: tuple[Path, ...] = _build_sanremes_repo_root_aliases(
    _sanremes_repo_root,
    Path(__file__).absolute().parents[2],
    get_process_sanremes_home(),
)
...
#: subprocess spawn.
_sanremes_site_packages: list[Path] | None = None
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<SanRemes repo>/venv`` layout and a real venv marker; require both before
    ...
    if not any(_same_path(candidate, repo_root / "venv") for repo_root in _sanremes_repo_root_aliases):
        return None
...
def _get_sanremes_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the SanRemes runtime.
    ...
    """
    global _sanremes_site_packages
    if _sanremes_site_packages is not None:
        result = list(_sanremes_site_packages)
    else:
        result = []
        if _in_venv:
            try:
        ...
        _sanremes_site_packages = list(result)
...
def _strip_sanremes_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip SanRemes-owned PYTHONPATH entries, then the runtime marker vars.
    ...
    """
    _strip_sanremes_owned_pythonpath(env)
    for _marker in _ACTIVE_VENV_MARKER_VARS:
        env.pop(_marker, None)


def _strip_sanremes_owned_pythonpath(env: dict) -> None:
    """Remove SanRemes-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the SanRemes repo root and the SanRemes venv's
    ...
    user entries, so only entries proven SanRemes-owned are removed:
    ...
    sanremes_site_packages = _get_sanremes_site_packages(env)
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is SanRemes-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: SanRemes venv site-packages ---
        ...
        for sp in sanremes_site_packages:
        ...
        # resolved and unresolved (SANREMES_HOME/junction) spellings count as
        # SanRemes-owned.
        if not should_strip:
            should_strip = any(
                _same_path(entry_path, repo_root)
                for repo_root in _sanremes_repo_root_aliases
            )
    ...
    if stripped:
        logger.debug(
            "Stripped SanRemes-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, SanRemes trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the SanRemes host itself — controller-side platform
    ...
    def get_temp_dir(self) -> str:
        ...
        **Default (no override set):** a dedicated cache dir under
        ``SANREMES_HOME`` (``~/.sanremes/cache/terminal``) rather than ``/tmp``.
        On several distros (Arch and friends) ``/tmp`` is a small RAM-backed
        tmpfs, and SanRemes session artifacts — background-process logs,
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``SANREMES_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under SANREMES_HOME.  Using
            ...
            try:
                from sanremes_constants import get_sanremes_home
                cache_dir = get_sanremes_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "sanremes_terminal"
        ...
        # Honored ahead of the generic TMPDIR so users can redirect SanRemes' temp
        ...
        # Default: SANREMES_HOME/cache/terminal — real storage, mirroring the
        # Windows branch above. /tmp is only a last-resort fallback now
        # because RAM-backed tmpfs /tmp fills up under SanRemes load.
        try:
            from sanremes_constants import get_sanremes_home
            cache_dir = get_sanremes_home() / "cache" / "terminal"
            cache_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._sanremes_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_sanremes_pgid", None)
                    if pgid is None:
                        raise
