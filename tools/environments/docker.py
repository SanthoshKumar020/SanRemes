...
_EGRESS_LABEL_KEY = "sanremes-egress"
...
def _load_sanremes_env_vars() -> dict[str, str]:
    """Load ~/.sanremes/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active SanRemes profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale sanremes-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=sanremes-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling SanRemes process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=sanremes-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a sanremes process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed SanRemes
    process leaves its container behind permanently because there's no
    subsequent SanRemes process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=sanremes-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=sanremes-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``SANREMES_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("SANREMES_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using SANREMES_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'sanremes'
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``SANREMES_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `sanremes egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `sanremes egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `sanremes egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `sanremes egress setup` to mint provider "
    ...
    container_ca = "/etc/ssl/certs/sanremes-egress-ca.crt"
    ...
    # through the proxy.  Keep the SANREMES_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"SANREMES_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``sanremes-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.sanremes/sandboxes/). Non-persistent
        ...
        if egress_env_overrides:
            try:
                from sanremes_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from sanremes_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_SANREMES_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_SANREMES_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # s6-overlay images (e.g. sanremes-agent:latest) already use /init as PID 1
        ...
        container_name = f"sanremes-{uuid.uuid4().hex[:8]}"
        # Labels make sanremes-created containers identifiable to:
        #   * the orphan reaper (`sanremes-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`sanremes-task-id`, `sanremes-profile`)
        #   * operators running `docker ps --filter label=sanremes-agent=1`
        ...
        label_args = [
            "--label", "sanremes-agent=1",
            "--label", f"sanremes-task-id={task_label}",
            "--label", f"sanremes-profile={profile_name}",
        ...
        self._labels = {
            "sanremes-agent": "1",
            "sanremes-task-id": task_label,
            "sanremes-profile": profile_name,
            _EGRESS_LABEL_KEY: egress_label,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior SanRemes process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # win over the generic SanRemes secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip SanRemes-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_sanremes_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_sanremes_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _SANREMES_PROVIDER_ENV_BLOCKLIST)
        sanremes_env = _load_sanremes_env_vars() if forward_keys else {}
        unset_names: set[str] = set()
        for key in sorted(forward_keys):
            value = os.getenv(key) or sanremes_env.get(key)
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("sanremes-task-id", "")
        profile_label = self._labels.get("sanremes-profile", "")
        existing = self._find_reusable_container(
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"sanremes-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(
        ...
        matches containers that happened to be named ``sanremes-*`` but were
        started by some other tool.
        """
        try:
            filters = [
                "--filter", "label=sanremes-agent=1",
                "--filter", f"label=sanremes-task-id={task_label}",
                "--filter", f"label=sanremes-profile={profile_label}",
            ]
            if egress_label != "off":
                ...
                # sanremes-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "sanremes egress disable",
        ...
        # container) but can happen if a previous SanRemes process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every SanRemes exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        SanRemes process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next SanRemes startup. That covers the
        ...
        # outlives SanRemes processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"sanremes-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``sanremes /quit``
