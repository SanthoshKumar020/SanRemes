...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "sanremes-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.sanremes"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.sanremes directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.sanremes"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="sanremes-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .sanremes/ as a tar archive."""
        # Tar from / with the full path so archive entries preserve absolute
        # paths (e.g. home/user/.sanremes/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.sanremes".lstrip("/")
        ssh_cmd = self._build_ssh_command()
