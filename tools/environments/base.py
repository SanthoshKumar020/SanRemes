"""Base class for all SanRemes execution environment backends.
...
# SANREMES_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("SANREMES_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {SANREMES_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_sanremes_home() / "sandboxes"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    recorded on ``proc._sanremes_stdin_errors`` while stdin is still closed
    ...
    proc._sanremes_stdin_errors = errors
    ...
    proc._sanremes_stdin_thread = thread
    thread.start()
...
def _cwd_marker(session_id: str) -> str:
    return f"__SANREMES_CWD_{session_id}__"
...
# the FIRST session's SANREMES_SESSION_ID into the snapshot makes every LATER
...
# SANREMES_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not SanRemes' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is SANREMES_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (SANREMES_SESSION_|SANREMES_UI_SESSION_ID|SANREMES_CRON_AUTO_DELIVER_|"
    "SANREMES_CRON_SESSION|SANREMES_BROWSER_CONTROL_)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``SANREMES_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!SANREMES_SESSION_*} ${!SANREMES_CRON_AUTO_DELIVER_*} "
        "${!SANREMES_BROWSER_CONTROL_*} "
        # AI_AGENT / SANREMES_AGENT are per-command attribution markers
        ...
        "AI_AGENT SANREMES_AGENT "
        f"SANREMES_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all SanRemes backends.
    ...
    # True only when commands execute on the SAME host as the SanRemes process
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/sanremes-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/sanremes-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__sanremes_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__sanremes_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__sanremes_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__sanremes_fns\" ] && declare -f $__sanremes_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_SANREMES_RUNTIME_PASSTHROUGH_{name}"
        ...
        # under SanRemes via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the SanRemes-specific
        # ``SANREMES_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``sanremes-agent`` — see huggingface.js
        ...
        # inherited from the SanRemes process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (SanRemes running inside another agent's terminal).
        parts.append(
            'export AI_AGENT="${AI_AGENT:-sanremes-agent}" '
            'SANREMES_AGENT="${SANREMES_AGENT:-true}"'
        )
        ...
        parts.append("__sanremes_ec=$?")
        # Restrict SanRemes metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__sanremes_snap_tmp=$(mktemp {_snap_tmp_template}) && "
        ...
        parts.append("exit $__sanremes_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"SANREMES_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_sanremes_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via SANREMES_DEBUG_INTERRUPT=1) -------------
        ...
        stdin_thread = getattr(proc, "_sanremes_stdin_thread", None)
        if stdin_thread is not None:
            stdin_thread.join(timeout=5)
        rendered = output.render()
        result = self._finalize_wait_result(output, rendered, proc.returncode)
        stdin_errors = getattr(proc, "_sanremes_stdin_errors", None)
        if stdin_errors:
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __SANREMES_CWD_{session}__ marker from stdout output.
