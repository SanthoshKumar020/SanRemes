...
`sanremes skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.sanremes.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.sanremes.blueprint`` is a mapping containing
    ...
    sanremes = meta.get("sanremes") if isinstance(meta, dict) else None
    blueprint = sanremes.get("blueprint") if isinstance(sanremes, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.sanremes.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.sanremes.blueprint`` block) they can hand
    to ``sanremes skills publish`` to share. ``body`` is the plain-language
