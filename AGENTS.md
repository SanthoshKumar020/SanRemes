# SanRemes Agent - Development Guide

Instructions for AI coding assistants and developers working on the sanremes-agent codebase.

**Never give up on the right solution.**

## What SanRemes Is

SanRemes is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: SanRemes ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`sanremes tools`, `sanremes setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `SANREMES_HOME`. Mocks hide integration bugs.
...
- **New `SANREMES_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `sanremes tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.sanremes/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `sanremes <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `sanremes webhook`, `sanremes cron`, `sanremes tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.sanremes/plugins/` or a pip package, discovered at runtime.
...
one behind a plain URL + token, or SanRemes Cloud. Only the first two are spawned
by us and carry `SANREMES_DESKTOP=1`. Every env-keyed GUI gate is therefore a
...
SanRemes desktop app."*

The pattern that works:

- **The toolset is the surface gate.** Keep the tools off `_SANREMES_CORE_TOOLS`
...
- **Ask which identity you actually mean.** `SANREMES_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`sanremes --tui` against that same backend) is
...
`$HOME/.sanremes/sanremes-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _SANREMES_CORE_TOOLS list
├── cli.py                # SanRemesCLI class — interactive CLI orchestrator (~11k LOC)
├── sanremes_state.py       # SessionDB — SQLite session store (FTS5 search)
├── sanremes_constants.py   # get_sanremes_home(), display_sanremes_home() — profile-aware paths
├── sanremes_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── sanremes_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── sanremes-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `sanremes --tui`
...
**User config:** `~/.sanremes/config.yaml` (settings), `~/.sanremes/.env` (API keys only).
**Logs:** `~/.sanremes/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_sanremes_home()`.
Browse with `sanremes logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across SanRemes: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`sanremes_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `SanRemesCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.sanremes/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`sanremes_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/sanremes` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `sanremes_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `SanRemesCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `sanremes --tui` or `SANREMES_TUI=1`.
...
npm run dev       # watch mode (rebuilds sanremes-ink + tsx --watch)
npm start         # production
npm run build     # full build (sanremes-ink + tsc)
...
### TUI in the Dashboard (`sanremes dashboard` → `/chat`)

The dashboard embeds the real `sanremes --tui` — **not** a rewrite.  See `sanremes_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `sanremes_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `sanremes --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `sanremes --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@sanremes/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `sanremes serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `SANREMES_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `sanremes` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `sanremes --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known SanRemes built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit SanRemes core.
Use the plugin route instead: create `~/.sanremes/plugins/<name>/plugin.yaml`
and `~/.sanremes/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core SanRemes tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_SANREMES_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_SANREMES_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_sanremes_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `SANREMES_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_sanremes_home()` for the base directory — never `Path.home() / ".sanremes"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `sanremes_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `sanremes_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `sanremes tools`, `sanremes setup`, most CLI subcommands | `sanremes_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`sanremes_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
sanremes_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.sanremes/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic SanRemes gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `sanremes_cli/skin_engine.py`:
...
Users create `~/.sanremes/skins/<name>.yaml`:
...
SanRemes has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.sanremes/plugins/` and pip-installed entry points.

### General plugins (`sanremes_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.sanremes/plugins/`, `./.sanremes/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `sanremes` at startup so
  `sanremes <pluginname> <subcmd>` works with no change to `main.py`
...
bundled, `$SANREMES_HOME/plugins/`, `./.sanremes/plugins/` (opt-in via
`SANREMES_ENABLE_PROJECT_PLUGINS`), and `sanremes_agent.memory_providers` entry
...
`post_setup(sanremes_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `sanremes <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `sanremes --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `sanremes_cli/main.py`, etc.).
...
into `~/.sanremes/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `sanremes memory setup` / `post_setup()` without
...
`~/.sanremes/plugins/` (or via pip entry points). They register through
...
2. User: `$SANREMES_HOME/plugins/model-providers/<name>/`
...
[`sanremes-example-plugins`](https://github.com/NousResearch/sanremes-example-plugins)
companion repo, not in this tree.

### Bot Mode (`apps/desktop/src/plugins/sanremes-bots/`)

The desktop "Bots" experience ships bundled in-tree. Each bot is a SanRemes
...
`ui_meta['sanremes-bots'].chat` and verified it per click; five hardening
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `sanremes skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.sanremes.tags`, `metadata.sanremes.category`,
`metadata.sanremes.related_skills`, `metadata.sanremes.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.sanremes.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native SanRemes tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "SanRemes Agent" is the secondary collaborator. If the
   contributor's commit shows "SanRemes Agent" as author (because they
   used SanRemes to draft the skill), replace it with their actual name
...
lives in the `sanremes-agent-dev` skill at
...
`"messaging"`); `_SANREMES_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `sanremes tools` (the curses UI) or the
...
SanRemes returns a delegation id immediately and the result re-enters the
...
go to `~/.sanremes/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `sanremes_cli/curator.py` wires `sanremes curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.sanremes/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `sanremes cron <verb>`
...
- File lock at `~/.sanremes/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `sanremes kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `sanremes_cli/kanban.py` wires `sanremes kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`sanremes-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `SANREMES_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
## Update Pipeline (`sanremes update`)
...
- **Plan** (`sanremes_cli/update_inventory.py`, `sanremes update --plan`):
...
- **Snapshot** (`sanremes_cli/backup.py`): pre-update quick snapshot for
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE
  (every `sanremes-gateway*` unit / `ai.sanremes.gateway*` LaunchAgent),
...
- **Report**: every run writes a machine-readable receipt to
  `~/.sanremes/logs/update_receipts/` (`latest.json` pointer; steps,
...
`sanremes serve` (control plane, desktop-spawned child) dies with the app
— by design. The messaging gateway (`gateway run`) SURVIVES the app: the
serve backend's `/api/gateway/*` endpoints spawn it detached
(`_spawn_sanremes_action` — `start_new_session` / `DETACHED_PROCESS`), so
...
SanRemes-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `SANREMES_BACKGROUND_NOTIFICATIONS` env var):
...
SanRemes supports **profiles** — multiple fully isolated instances, each with its own
`SANREMES_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `sanremes_cli/main.py` sets
`SANREMES_HOME` before any module imports. All `get_sanremes_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_sanremes_home()` for all SANREMES_HOME paths.** Import from `sanremes_constants`.
   NEVER hardcode `~/.sanremes` or `Path.home() / ".sanremes"` in code that reads/writes state.
   ```python
   # GOOD
   from sanremes_constants import get_sanremes_home
   config_path = get_sanremes_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".sanremes" / "config.yaml"
   ```

2. **Use `display_sanremes_home()` for user-facing messages.** Import from `sanremes_constants`.
   This returns `~/.sanremes` for default or `~/.sanremes/profiles/<name>` for profiles.
   ```python
   # GOOD
   from sanremes_constants import display_sanremes_home
   print(f"Config saved to {display_sanremes_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.sanremes/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_sanremes_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_sanremes_home()`,
   not `Path.home() / ".sanremes"`.

4. **Tests that mock `Path.home()` must also set `SANREMES_HOME`** — since code now uses
   `get_sanremes_home()` (reads env var), not `Path.home() / ".sanremes"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"SANREMES_HOME": str(tmp_path / ".sanremes")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not SANREMES_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".sanremes" / "profiles"`, NOT `get_sanremes_home() / "profiles"`.
   This is intentional — it lets `sanremes -p coder profile list` see all profiles regardless
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  (gateway run), `sanremes_cli.update_cmd._sanremes_holder_subcommand`
  (top-level subcommand of any SanRemes argv). Never hand-roll token scans.
...
### DO NOT hardcode `~/.sanremes` paths
Use `get_sanremes_home()` from `sanremes_constants` for code paths. Use `display_sanremes_home()`
for user-facing print/log messages. Hardcoding `~/.sanremes` breaks profiles — each profile
has its own `SANREMES_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `sanremes_cli/curses_ui.py`. See `sanremes_cli/tools_config.py` for an example.
...
canary ledger, sanremes#85796 / gateway-gateway#210). Violating any of them
...
with actual imports (not mocks) against a temp `SANREMES_HOME`.

### Tests must not write to `~/.sanremes/`
The `_isolate_sanremes_home` autouse fixture in `tests/conftest.py` redirects `SANREMES_HOME` to a temp dir. Never hardcode `~/.sanremes/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_sanremes_home()` resolve within the temp dir.
Use the pattern from `tests/sanremes_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SANREMES_HOME", str(home))
    return home
```
...
subprocess (`--file-retries`, default 1; `SANREMES_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.sanremes/` | Your real config+auth.json                  | Temp dir per test                         |
...
SanRemes supports Linux, macOS and native Windows, and plenty of its behaviour
...
`tests/sanremes_cli/test_venv_holder_windows_live.py` on a real
