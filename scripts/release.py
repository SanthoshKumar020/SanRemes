...
VERSION_FILE = REPO_ROOT / "sanremes_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@sanremes.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.sanremes": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level SANREMES_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (sanremes journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "sanremes.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.sanremes/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows sanremes-shim resolution + prefer --update on recovery; #52378)
    ...
    "sanremes@marian.local": "Schrotti77",
    ...
    "274096618+sanremes-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "sanremes@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on sanremes mcp add)
    ...
    "sanremes@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@sanremes-agent": "teknium1",
    ...
    "github@nadyasanremes.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (SANREMES_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (SANREMES_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: sanremes-eval community link)
    ...
    "sanremesagent26@gmail.com": "sanremesagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+sanremesagent26@users.noreply.github.com": "sanremesagent26",
    ...
    "vigo@sanremes": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.sanremes@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local SanRemes runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level SANREMES_HOME state files)
    ...
    "zhchl@sanremes-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live SanRemes
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "sanremes@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/sanremes-agent",
    ...
    lines.append(f"# SanRemes Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for SanRemes Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="SanRemes Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  SanRemes Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"SanRemes Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"SanRemes Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'SanRemes Agent v{new_version} ({calver_date})' "
