...
When SanRemes runs in TUI mode, the gateway child process communicates with
...
# ``get_sanremes_home() / "plugins"`` (user) and ``./.sanremes/plugins/`` (project,
# gated behind ``SANREMES_ENABLE_PROJECT_PLUGINS``) — see
# ``sanremes_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``sanremes_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import sanremes_constants (this script
    ...
    # plugins in get_sanremes_home()/plugins/ and project plugins in
    # ./.sanremes/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_sanremes_home() / "plugins"]
    if os.environ.get("SANREMES_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".sanremes" / "plugins")
