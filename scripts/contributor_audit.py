...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^SanRemes\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (sanremes-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "sanremes@nousresearch.com",
    "sanremes-audit@example.com",
    "nousbot@nousresearch.com",
    "sanremes@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "NousResearch/sanremes-agent",
                "--state", "merged",
