...
measured in the production session DB) through `sanremes chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    SANREMES_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured SanRemes home with credentials for the
...
(tracker: NousResearch/sanremes-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "SANREMES_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "sanremes_cli.main", "chat", "--query", q,
