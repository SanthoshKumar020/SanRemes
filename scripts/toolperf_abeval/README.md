...
batch (tracker: [#77056](https://github.com/NousResearch/sanremes-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  SanRemes home, same model, same tasks, same reps.
...
1. Create a dedicated SanRemes home with credentials for the models under test:
   ...
   integration at it; no SanRemes observability plugin needs to be enabled.
