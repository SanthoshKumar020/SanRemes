...
Background
----------
A Nous bootstrap session (client_id ``sanremes-cli-vps``) can take a terminal
...
``stage2-hook.sh`` seeds ``auth.json`` from ``SANREMES_AUTH_JSON_BOOTSTRAP`` only
...
``SANREMES_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Nous
...
Design constraints
------------------
- Pure stdlib, no sanremes_cli imports: runs early in the boot hook, before the
...
# from SANREMES_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
...
REBOOTSTRAP_ENV = "SANREMES_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "sanremes-cli-vps"
...
def _nous_entry_is_terminal(nous_state: Any) -> bool:
    ...
    Mirrors the ``terminal`` predicate in ``sanremes_cli.auth.get_nous_session_validity``:
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a SANREMES_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    SANREMES_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             SANREMES_AUTH_JSON_BOOTSTRAP path handle it)
...
def main() -> int:
    ...
    if not auth_path:
        home = os.environ.get("SANREMES_HOME", "")
