...
# tree generates llms.txt — the index every LLM (SanRemes included, via the
# sanremes-agent skill) reads to learn what SanRemes can do — and
...
_MCP_CATALOG_FILES = {"sanremes_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (SanRemes-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``sanremes serve`` backend, the Docker image)
