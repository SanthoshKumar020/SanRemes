# Behavioral test for install.ps1's sanremes launcher staging (PR #92092,
...
#
# The test lifts the real Install-SanRemesCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $SanRemesHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-SanRemesCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-SanRemesCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'sanremes-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'sanremes-agent'
    ...
    $missingThrew = $false
    try {
        Install-SanRemesCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing sanremes.exe fails the launcher stage'
    ...
    $sanremesV1 = [byte[]](77, 90, 1)
    $sanremesV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'sanremes.exe'), $sanremesV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-SanRemesCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $sanremesV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'sanremes.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'sanremes-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'sanremes.exe'), $sanremesV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'sanremes-acp.exe'), $acp)
    Install-SanRemesCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $sanremesV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'sanremes.exe'))) `
        'installer refreshes an existing SanRemes launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'sanremes-acp.exe'))) `
    ...
    Install-SanRemesCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'sanremes.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'sanremes.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'sanremes.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'sanremes.exe')) -and $cmdBody.Contains('%*')) `
