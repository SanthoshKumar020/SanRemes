"""Run a real SanRemes CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_sanremes_executable(sanremes_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "sanremes",
        Path(".venv") / "Scripts" / "sanremes.exe",
    ):
        candidate = sanremes_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("sanremes")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "SanRemes executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="SanRemes source checkout containing .venv/bin/sanremes",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated SANREMES_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["sanremes.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['sanremes.client.active']}"
        )
    [model] = by_name["sanremes.model_route.count"]
    expected_model = {
        "name": "sanremes.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['sanremes.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["sanremes.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['sanremes.task_run.started']}"
        )
    [terminal] = by_name["sanremes.task_run.finished"]
    ...
    [tool] = by_name["sanremes.tool_call.count"]
    ...
    lifecycle = by_name["sanremes.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["sanremes.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The SanRemes development environment requires jsonschema"
        ) from exc
    ...
    if metrics["sanremes.client.active"] != [
        {
            "name": "sanremes.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['sanremes.client.active']}"
        )
    [model] = metrics["sanremes.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['sanremes.model_route.count']}"
        )
    [terminal] = metrics["sanremes.task_run.finished"]
    ...
    [tool] = metrics["sanremes.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["sanremes.skill.lifecycle.count"]
    ...
    loads = metrics["sanremes.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    sanremes_repo = args.sanremes_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    sanremes = _resolve_sanremes_executable(sanremes_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="sanremes-relay-shared-metrics-"))
    home = root / "sanremes-home"
    ...
    try:
        ...
        python_paths = [str(sanremes_repo)]
    ...
    (root / "sanremes.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "sanremes.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"SanRemes exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("SanRemes model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("SanRemes did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("SanRemes did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from sanremes_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "sanremes.shared_metrics.v2.schema.json",
    )

    print("SanRemes -> NeMo Relay shared-metrics smoke test passed")
