# ============================================================================
# SanRemes Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$SanRemesHome = $(if ($env:SANREMES_HOME) { $env:SANREMES_HOME } else { "$env:LOCALAPPDATA\sanremes" }),
    [string]$InstallDir = $(if ($env:SANREMES_HOME) { "$env:SANREMES_HOME\sanremes-agent" } else { "$env:LOCALAPPDATA\sanremes\sanremes-agent" }),
    ...
    # builds apps/desktop into a launchable SanRemes.exe.
    #
    # Why opt-in:
    #   * SanRemes-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched SanRemes.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live SanRemes.exe
    ...
    #     `sanremes desktop` already builds on demand.
...
# SANREMES_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[sanremes] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'SanRemesInstall.LongPath').Type) {
            Add-Type -Namespace 'SanRemesInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [SanRemesInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -SanRemesHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('SanRemesHome')) {
    $SanRemesHome = ConvertTo-LongPath $SanRemesHome
} else {
    $SanRemesHome = ConvertTo-LongPath $(
        if ($env:SANREMES_HOME) { $env:SANREMES_HOME } else { "$env:LOCALAPPDATA\sanremes" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:SANREMES_HOME) { "$env:SANREMES_HOME\sanremes-agent" } else { "$env:LOCALAPPDATA\sanremes\sanremes-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: SanRemesHome=$SanRemesHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:NousResearch/sanremes-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/sanremes-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * SanRemes Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $SanRemesHome)) {
        New-Item -ItemType Directory -Force -Path $SanRemesHome | Out-Null
    }
    $envFile = Join-Path $SanRemesHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `sanremes update`'s npx
    ...
    $prefixDir = Join-Path $SanRemesHome "node"
...
function Install-Uv {
    # SanRemes owns its own uv at $SanRemesHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (sanremes_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `sanremes update` stay in sync.
    $managedUv = Join-Path $SanRemesHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $SanRemesHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $SanRemesHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $SanRemesHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $SanRemesHome "bin"
        ...
        # (sanremes_cli/managed_uv.py looks only at $SanRemesHome\bin\uv.exe).
...
# Put the SanRemes-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone sanremes-setup.exe run, a user typing `npm`) silently
...
# Upgrade the SanRemes-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "SanRemes-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("sanremes-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and SANREMES_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under SanRemes-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun SanRemes setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which SanRemes uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\sanremes\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    SanRemes needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the SanRemes copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\sanremes\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SANREMES_GIT_BASH_PATH`` (User scope) so SanRemes can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a SanRemes-managed PortableGit install instead..."
    }

    # Download PortableGit into $SanRemesHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $SanRemesHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent SanRemes features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  SanRemes needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SANREMES_GIT_BASH_PATH`` (User env scope) so SanRemes can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $SanRemesHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $SanRemesHome\git\usr\bin\bash.exe
    $candidates += "$SanRemesHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$SanRemesHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SANREMES_GIT_BASH_PATH", $candidate, "User")
            $env:SANREMES_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set SANREMES_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- SanRemes may not find Git Bash."
    Write-Info "If needed, set SANREMES_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the SanRemes-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (SanRemes requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy SanRemes requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a SanRemes-managed Node.js installation instead..."

    # Prefer a SanRemes-managed Node from a previous run over a too-old system one.
    $managedNode = "$SanRemesHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$SanRemesHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$SanRemesHome\node"
        Write-Success "Node.js $version found (SanRemes-managed)"
        ...
        Update-ManagedNpm "$SanRemesHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing SanRemes-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $SanRemesHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $SanRemesHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\sanremes-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$SanRemesHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$SanRemesHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$SanRemesHome\node") {
                    try {
                        Rename-Item "$SanRemesHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "SanRemes-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$SanRemesHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $SanRemesHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\sanremes-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `sanremes
                    # update` path (#4735).
                    ...
                    $stashName = "sanremes-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. sanremes-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``sanremes update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if SanRemes behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running sanremes processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/sanremes-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/sanremes-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/sanremes-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\sanremes-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\sanremes-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `sanremes update` (see the notes at the shared clone-path
    ...
    # next `sanremes update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under SanRemes-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running sanremes process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running sanremes processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*SanRemes_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (sanremes.exe) plus its child tree.
            & taskkill /F /T /IM sanremes.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM sanremes.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m sanremes_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not sanremes.exe.
            ...
            # SanRemes children re-exec through .sanremes-runtime, so killing only
            ...
            # user (see sanremes_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-SanRemes " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `sanremes update` / the
...
function Restore-VenvBackup {
    ...
    # venv so SanRemes (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `sanremes.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'sanremes-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install sanremes-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and sanremes.exe broken with
    ...
    # here, not 30 seconds later when the user runs `sanremes`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close SanRemes processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close SanRemes processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves SanRemes and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register sanremes.exe in dist-info/RECORD but fail to
        ...
        # `sanremes` command while sanremes-agent.exe / sanremes-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m sanremes_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `sanremes dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\sanremes_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `sanremes dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `sanremes dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: sanremes_cli/web_server.py"
        }
    }
...
function Install-SanRemesCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the sanremes launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring sanremes.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "sanremes.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the sanremes command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # sanremes_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("sanremes", "sanremes-acp")) {
    ...
    $requiredExe = Join-Path $Destination "sanremes.exe"
    $requiredCmd = Join-Path $Destination "sanremes.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the sanremes command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up sanremes command..."
    
    if ($NoVenv) {
        $sanremesBin = "$InstallDir"
    } else {
        # $SanRemesHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `sanremes update`'s autostash
        ...
        # installer staged under sanremes-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-SanRemesCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $sanremesBin = "$SanRemesHome\bin"
        Install-SanRemesCommandLaunchers -Root $InstallDir -Destination $sanremesBin | Out-Null
    }
    ...
    #   sanremes-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The sanremes-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept sanremes via $sanremesBin)"
        }
    }
    
    if ($currentPath -notlike "*$sanremesBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$sanremesBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $sanremesBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SANREMES_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\sanremes instead
    # of the Unix default ~/.sanremes
    $currentSanRemesHome = [Environment]::GetEnvironmentVariable("SANREMES_HOME", "User")
    if (-not $currentSanRemesHome -or $currentSanRemesHome -ne $SanRemesHome) {
        [Environment]::SetEnvironmentVariable("SANREMES_HOME", $SanRemesHome, "User")
        Write-Success "Set SANREMES_HOME=$SanRemesHome"
    }
    $env:SANREMES_HOME = $SanRemesHome
    
    # Update current session
    $env:Path = "$sanremesBin;$env:Path"
    
    Write-Success "sanremes command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.sanremes-bootstrap-complete which tells the SanRemes
    ...
    # SanRemes-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `sanremes update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".sanremes-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the SANREMES_HOME directory structure ($SanRemesHome, default %LOCALAPPDATA%\sanremes)
    New-Item -ItemType Directory -Force -Path "$SanRemesHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SanRemesHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SanRemesHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SanRemesHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SanRemesHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SanRemesHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SanRemesHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SanRemesHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SanRemesHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$SanRemesHome\.env"
    ...
    # Create config.yaml
    $configPath = "$SanRemesHome\config.yaml"
    ...
    # (the default PS5 behaviour), and SanRemes's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$SanRemesHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in sanremes_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $SanRemesHome"
    
    # Seed bundled skills into $SanRemesHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $SanRemesHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $SanRemesHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$SanRemesHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $SanRemesHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (SanRemes-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'sanremes setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\sanremes-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\sanremes-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\sanremes-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `sanremes tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $SanRemesHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only SanRemes' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside SanRemes' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'sanremes tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `sanremes
...
# paths still lazy-install via install_cua_driver() (sanremes_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `sanremes computer-use install` runs. Bounded
        ...
        # SanRemes' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'sanremes tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: sanremes computer-use install"
            }
        } else {
...
# the final `electron` -> `SanRemes` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable SanRemes.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_sanremes_desktop command
    # resolves apps/desktop/release/win-unpacked/SanRemes.exe directly,
    ...
    # enforces the supported Node lines and prepends the SanRemes-managed Node to
    ...
    # The --dir mode produces an unpacked SanRemes.exe in
    ...
    # are NOT trying to work around). The SanRemes icon + product name are
    # stamped onto SanRemes.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\sanremes-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\SanRemes.exe",
        "$desktopDir\release\win-arm64-unpacked\SanRemes.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no SanRemes.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The SanRemes icon + identity are stamped onto SanRemes.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, sanremes-agent#38216).
    ...
    #    SanRemes.exe. We deliberately do NOT point them at `sanremes desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'SanRemes.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'SanRemes.lnk')
        )
        ...
        # with the SanRemes icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.sanremes/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'sanremes setup'."
        return
    }
    ...
    # Run sanremes setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m sanremes_cli.main setup
    } else {
        python -m sanremes_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $sanremesCmd = "$InstallDir\venv\Scripts\sanremes.exe"
    if (-not (Test-Path $sanremesCmd)) {
        $sanremesCmd = "sanremes"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$SanRemesHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'sanremes whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: sanremes gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$SanRemesHome\logs\gateway.log"
            Start-Process -FilePath $sanremesCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$SanRemesHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: sanremes gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: sanremes gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$SanRemesHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$SanRemesHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SanRemesHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SanRemesHome\sanremes-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   sanremes              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   sanremes setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   sanremes config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   sanremes config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   sanremes gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   sanremes update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning SanRemes repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (SanRemes-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding SanRemes to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://sanremes-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
