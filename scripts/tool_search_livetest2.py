...
def run_one(scenario: Dict[str, Any], mode: str, rep: int, out_dir: Path) -> Dict[str, Any]:
    ...
    sanremes_home = base.setup_isolated_home(enabled, listing=("auto" if mode == "listing" else "off"))
    ...
    try:
        ...
        from sanremes_cli.plugins import get_plugin_manager, discover_plugins
    ...
    shutil.rmtree(Path(os.environ["SANREMES_HOME"]).parent, ignore_errors=True)
    return rec
