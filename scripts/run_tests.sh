#!/usr/bin/env bash
# Canonical test runner for sanremes-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.sanremes/...)
#
# Usage:
...
# (SANREMES_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.sanremes/sanremes-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.sanremes/sanremes-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${SANREMES_PYTHON:-}" ] && [ -x "$SANREMES_PYTHON" ] \
    && "$SANREMES_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: SANREMES_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `sanremes` binary rather
  # than the devShell hook.
  PYTHON="$SANREMES_PYTHON"
  echo "▶ no local venv — using Nix dev venv via SANREMES_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and SANREMES_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.sanremes/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.sanremes"
...
#
#   * SANREMES_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * SANREMES_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     SANREMES_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# SANREMES_RUN_SLOW_PET_TESTS / SANREMES_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no SANREMES_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in SANREMES_TEST_IMAGE SANREMES_TEST_WORKERS SANREMES_TEST_PATHS \
  SANREMES_TEST_FILE_TIMEOUT SANREMES_TEST_FILE_RETRIES SANREMES_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${SANREMES_RUN_SLOW_PET_TESTS:+SANREMES_RUN_SLOW_PET_TESTS="$SANREMES_RUN_SLOW_PET_TESTS"} \
  ${SANREMES_E2E_BROWSER:+SANREMES_E2E_BROWSER="$SANREMES_E2E_BROWSER"} \
