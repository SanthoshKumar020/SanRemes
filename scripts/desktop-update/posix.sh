...
#
# The whole job: wait for the Desktop to exit, run `sanremes update`, tell the
...
# the SANREMES_DESKTOP_CHILD_PID reaper-exclusion dance dies with it.
#
# CONTRACT (keep in sync with apps/desktop/electron/main.ts):
#   bash scripts/desktop-update/posix.sh
#     --install-root <path>    repo checkout (SANREMES_HOME/sanremes-agent)
...
SANREMES_HOME="${INSTALL_ROOT:+$(dirname "$INSTALL_ROOT")}"
SANREMES_HOME="${SANREMES_HOME:-${TMPDIR:-/tmp}}"
MARKER="$SANREMES_HOME/.sanremes-update-in-progress"
LOG_DIR="$SANREMES_HOME/logs"; mkdir -p "$LOG_DIR" 2>/dev/null || true
LOG="$LOG_DIR/desktop-update-handoff.log"
RESULT="$SANREMES_HOME/.sanremes-update-result.json"
STATUS="${TMPDIR:-/tmp}/sanremes-update-status.$$"
...
notify_fallback() { # status message — renderer-free recovery surface.
  ...
  if [ "$(uname)" = "Darwin" ]; then
    /usr/bin/osascript -e "display notification \"$(printf '%s' "$2" | sed 's/"/\\"/g')\" with title \"SanRemes update\"" 2>/dev/null && return 0
  else
    if command -v notify-send >/dev/null 2>&1; then
      notify-send -u critical "SanRemes update" "$2" 2>/dev/null && return 0
    fi
    local p
    if command -v zenity >/dev/null 2>&1; then
      zenity --warning --title="SanRemes update" --text="$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
    if command -v kdialog >/dev/null 2>&1; then
      kdialog --title "SanRemes update" --sorry "$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
...
find_browser() {
  ...
  # email included — inside this window that is titled "SanRemes" (#88410).
...
start_ui() {
  ...
  # teardown TERM killed the shim ~1s into `sanremes update` (2026-08-14 16:44,
  ...
  "$py" -c 'import os, signal, sys; os.setsid(); signal.signal(signal.SIGTERM, signal.SIG_DFL); os.execv(sys.argv[1], sys.argv[1:])' \
    "$browser" --app="http://127.0.0.1:$port/" --user-data-dir="${TMPDIR:-/tmp}/sanremes-update-ui-$$" \
...
stop_ui() { # error/manual outcomes keep the window up briefly so a watching
  ...
  if [ "${1:-}" = "leave-window" ]; then
    sleep "${SANREMES_UPDATE_SHIM_GRACE_SECONDS:-15}"
  fi
...
linux_gate() {
  ...
  GATE=manual GATE_MSG="Update complete, but the rebuilt app can't relaunch itself (its sandbox helper needs root ownership). Reopen SanRemes to finish."
}

mac_swap() {
  local rebuilt="" c
  for c in "$INSTALL_ROOT/apps/desktop/release/mac-arm64/SanRemes.app" \
           "$INSTALL_ROOT/apps/desktop/release/mac/SanRemes.app"; do
    [ -d "$c" ] && { rebuilt="$c"; break; }
  done
  ...
  if [ "$FINAL_CODE" -eq 0 ] && [ -n "$rebuilt" ] && [ -d "$RELAUNCH_TARGET" ] && [ "$rebuilt" != "$RELAUNCH_TARGET" ]; then
    ...
    if ! /usr/bin/ditto "$rebuilt" "$RELAUNCH_TARGET.new"; then
      ...
      if mv "$RELAUNCH_TARGET.old" "$RELAUNCH_TARGET"; then
        ...
        FINAL_CODE=7 FINAL_MSG="The update finished but installing the new app failed and the previous app could not be restored. Reinstall SanRemes (the rebuilt app is at $rebuilt)."
  ...
  if [ -n "$DONE_NOTE" ]; then
    if [ "$(uname)" = "Darwin" ]; then
      ...
      if ! launch_app; then
        ...
        FINAL_MSG="$DONE_NOTE SanRemes also could not reopen itself - open it manually."
        write_result
      fi
    fi
    ...
    FINAL_MSG="Update complete. Reopen SanRemes to finish (it could not restart itself)."
    MANUAL=1
    write_result
    publish "manual" "$FINAL_MSG"; stop_ui leave-window
  fi
...
if [ "$SELF_TEST_UI" -eq 1 ]; then
  ...
  sleep "${SANREMES_SELFTEST_HOLD_SECONDS:-6}"
  RELAUNCH_TARGET=""
  if [ -n "${SANREMES_SELFTEST_FAIL:-}" ]; then FINAL_MSG="self-test error state"
...
# died two seconds after starting `sanremes update`. Re-exec through a one-shot
...
if [ "$HANDOFF_DAEMONIZED" -ne 1 ]; then
  ...
  # EXIT: that would publish a false failure and relaunch SanRemes while the
...
# so the shim server and the later `sanremes update` subprocess both inherit
...
# update_lock.py (the `sanremes update` child adopts it via process ancestry).
# The Desktop supplies one acquisition time for the whole ownership chain.
NOW="$(date +%s)"
STARTED_AT="${SANREMES_UPDATE_STARTED_AT:-$NOW}"
...
if [ "$DESKTOP_PID" -gt 0 ] 2>/dev/null; then
  ...
  if kill -0 "$DESKTOP_PID" 2>/dev/null; then
    FINAL_CODE=4 FINAL_MSG="Update aborted: the SanRemes window (pid $DESKTOP_PID) did not exit within 30s. Nothing was changed. Close SanRemes fully and try again."
...
SANREMES_BIN="$INSTALL_ROOT/venv/bin/sanremes"
[ -x "$SANREMES_BIN" ] || { FINAL_CODE=3 FINAL_MSG="Update aborted: $SANREMES_BIN is missing. The install needs repair (run the SanRemes installer or sanremes doctor)."; log "$FINAL_MSG"; exit 3; }

# Run FROM the install root: `sanremes update` resolves the tree it mutates
...
# "close all SanRemes windows" sentinel.
KEEP_STASH=""
if "$SANREMES_BIN" update --help 2>/dev/null | grep -q -- '--keep-stash'; then
  KEEP_STASH="--keep-stash"
else
  log "installed sanremes predates --keep-stash; running without it"
fi
log "running: sanremes update --yes --gateway $KEEP_STASH --branch $BRANCH"
publish_stage "Updating code and dependencies"
OUT="$("$SANREMES_BIN" update --yes --gateway $KEEP_STASH --branch "$BRANCH" 2>&1)"; CODE=$?
printf '%s\n' "$OUT" >> "$LOG" 2>/dev/null
log "sanremes update exit code: $CODE"

if [ "$CODE" -ne 0 ] && [ "$CODE" -ne 2 ]; then
  # Retry once: update-boundary class (fresh code on disk, stale in memory).
  # Exit 2 ("close all SanRemes windows") is not retryable.
  #
  ...
  if printf '%s' "$OUT" | grep -q "CODE UPDATE SKIPPED"; then
    log "sanremes update skipped (checkout parked on a non-target branch); not retrying"
    FINAL_CODE=8
...
# Truthful completion: `sanremes update` calls a GUI build failure non-fatal
...
if [ "$CODE" -eq 0 ] && printf '%s' "$OUT" | grep -q "Desktop build failed"; then
  log "desktop build failed inside sanremes update; retrying build"
  publish_stage "Rebuilding Desktop"
  "$SANREMES_BIN" desktop --force-build --build-only >> "$LOG" 2>&1 || {
    FINAL_CODE=6 FINAL_MSG="Code and dependencies updated, but the Desktop app rebuild failed - you are running the previous build. Run sanremes desktop --force-build from a terminal to retry."
    exit 6
  }
fi

if [ "$CODE" -eq 0 ]; then FINAL_CODE=0 FINAL_MSG="Update complete."
else FINAL_CODE="$CODE" FINAL_MSG="Update failed (exit $CODE). Run sanremes debug share in a terminal to send a report."; fi
exit "$FINAL_CODE"
