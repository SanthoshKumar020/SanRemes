...
# (%SANREMES_HOME%\sanremes-setup.exe). That binary has no self-update path --
# copy_self_to_sanremes_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `sanremes update` refreshes
...
#     -InstallRoot <path>   repo checkout (SANREMES_HOME\sanremes-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] SanRemes.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .sanremes-update-in-progress in place (tests)
#
...
# .sanremes-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim SANREMES_HOME\.sanremes-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining SANREMES_UPDATE_STARTED_AT from the Desktop hand-off.
# sanremes_cli/update_lock.py's ancestry rule lets our
# `sanremes update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new SanRemes.exe pid.
try {
    Add-Type -Namespace SanRemesHandoff -Name Win32 -MemberDefinition @'
...
$SanRemesHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $SanRemesHome ".sanremes-update-in-progress"
$LogDir = Join-Path $SanRemesHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $SanRemesHome ".sanremes-update-result.json"
$script:Ui = $null
$script:UiStage = "SanRemes will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale sanremes-update-ui-* leftovers from interrupted
    ...
    # update marker (.sanremes-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "sanremes-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-SanRemesStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("sanremes-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [SanRemesHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `sanremes debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"sanremes debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [SanRemesHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen SanRemes themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old SanRemes.exe before writing the replacement,
    ...
    # DO NOT spawn SanRemes.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [SanRemesHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [SanRemesHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [SanRemesHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh SanRemes process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [SanRemesHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `sanremes update` deliberately runs its build steps with stdout
# inherited (sanremes_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:SANREMES_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:SANREMES_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:SANREMES_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:SANREMES_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `sanremes update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (sanremes_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:SANREMES_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:SANREMES_UPDATE_PROGRESS_LOG
}
...
if (-not ("SanRemesUpdateJob" -as [type])) {
...
public static class SanRemesUpdateJob {
...
function Invoke-SanRemesStep([string]$Exe, [string[]]$SanRemesArgs, [string]$Tag) {
    ...
    # afterwards, where `sanremes debug share` picks it up.
    #
    ...
    # .sanremes-update-result.json, clearing .sanremes-update-in-progress,
    ...
    # SanRemes" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($SanRemesArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [SanRemesUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `sanremes update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [SanRemesUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; SANREMES_SELFTEST_FAIL=1 exercises the
# error state, SANREMES_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:SANREMES_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:SANREMES_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:SANREMES_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-SanRemesStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no SanRemes install to
...
#            update log, the shape of every real `sanremes update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:SANREMES_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:SANREMES_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:SANREMES_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:SANREMES_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "sanremes-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "sanremes-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "sanremes-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "sanremes-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "sanremes-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "sanremes-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "sanremes-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "sanremes-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `sanremes update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-SanRemesStep $powershell @(
    ...
    $flood = Invoke-SanRemesStep $powershell @(
    ...
    $stall = Invoke-SanRemesStep $powershell @(
    ...
    try {
        $logstall = Invoke-SanRemesStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-SanRemesStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-SanRemesStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:SANREMES_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for SanRemes to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the SanRemes window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close SanRemes fully and try again."
    ...
    Publish-UiProgress "Preparing SanRemes files"
    $shim = Join-Path $InstallRoot "venv\Scripts\sanremes.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the SanRemes install open (venv\Scripts\sanremes.exe locked after 20s). Nothing was changed. Close other SanRemes windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the sanremes.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\sanremes.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_sanremes_exe must first rename the running sanremes.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that sanremes.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second SanRemes window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a SanRemes.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m sanremes_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the SanRemes installer or `sanremes doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "sanremes_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all SanRemes windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m sanremes_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed sanremes predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-SanRemesStep $pythonExe $updateArgs "update"
    Write-HandoffLog "sanremes update exit code: $($res.Code)"

    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        # One retry for the update-boundary class (fresh code on disk, stale
        # code in memory). Exit 2 ("close all SanRemes windows") is not retryable.
    ...
    # `sanremes update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "sanremes update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-SanRemesStep $pythonExe @("-m", "sanremes_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `sanremes desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `sanremes debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "SanRemes is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. SanRemes was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen SanRemes."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening SanRemes"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen SanRemes to finish (it could not restart itself)."
