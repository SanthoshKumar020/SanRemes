#!/usr/bin/env python3
"""Live test harness for SanRemes Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any sanremes imports.
ORIGINAL_HOME = os.environ.get("SANREMES_HOME")
ORIGINAL_AUTH = Path.home() / ".sanremes" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.sanremes/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.sanremes/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="sanremes_ts_live_"))
    sanremes_home = home_dir / ".sanremes"
    sanremes_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, sanremes_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".sanremes" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, sanremes_home / ".env")
        ...
        load_sanremes_dotenv(sanremes_home=str(Path.home() / ".sanremes"))
    ...
    (sanremes_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new SANREMES_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "sanremes_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SANREMES_HOME"] = str(home)
...
def main():
    ...
    # Restore original SANREMES_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SANREMES_HOME"] = ORIGINAL_HOME
    else:
