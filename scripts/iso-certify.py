...
What it does
------------
1. Spawns a SCRATCH dashboard (``sanremes dashboard``) bound to loopback on a
   free port, with an ISOLATED ``SANREMES_HOME`` (temp dir, minimal seeded state).
   It NEVER touches the live :9119 dashboard / ai.sanremes.dashboard / live
   state.db. Loopback bind ⇒ no auth gate (web_server.should_require_auth).
2. Arms the synthetic GIL-heavy turn seam (``SANREMES_ISO_CERTIFY_SYNTH_TURN=1``,
...
the scratch SANREMES_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
_READY_RE = re.compile(r"SANREMES_(?:DASHBOARD|BACKEND)_READY port=(\d+)")
...
class ScratchDashboard:
    ...
    def __enter__(self) -> "ScratchDashboard":
        ...
        env["SANREMES_HOME"] = str(self.home)
        env["HOME"] = str(self.home.parent) if str(self.home.parent) else env.get("HOME", "")
        env["SANREMES_HOME"] = str(self.home)
        env["PYTHONPATH"] = str(REPO_ROOT) + os.pathsep + env.get("PYTHONPATH", "")
        env["SANREMES_ISO_CERTIFY_SYNTH_TURN"] = "1"
        cmd = [
            python, "-m", "sanremes_cli.main", "dashboard",
...
def run_certify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        with ScratchDashboard(
            ...
            env_extra={"SANREMES_DASHBOARD_SESSION_TOKEN": token},
        ) as dash:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--isolation", choices=["on", "off"], default="on",
                   help="set dashboard.turn_isolation in the scratch SANREMES_HOME")
    ...
    p.add_argument("--keep-home", action="store_true", help="do not delete the scratch SANREMES_HOME on exit")
