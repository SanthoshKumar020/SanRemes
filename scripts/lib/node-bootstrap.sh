...
#   2. ~/.sanremes/node/ from a prior SanRemes-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.sanremes/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$SANREMES_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   SANREMES_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   SANREMES_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   SANREMES_HOME               (default: $HOME/.sanremes)
# ============================================================================

SANREMES_NODE_MIN_VERSION="${SANREMES_NODE_MIN_VERSION:-20}"
SANREMES_NODE_TARGET_MAJOR="${SANREMES_NODE_TARGET_MAJOR:-22}"
SANREMES_HOME="${SANREMES_HOME:-$HOME/.sanremes}"
SANREMES_NODE_AVAILABLE=false
...
# Redirect a SanRemes-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $SANREMES_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$SANREMES_HOME/node/etc/npmrc"
}
...
_nb_npm_range() {
    if [ -n "${SANREMES_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$SANREMES_NPM_TARGET_RANGE"
        return 0
    fi
...
# The Python side recovers through sanremes_cli/npm_engine.py; the installer path
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$SANREMES_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$SANREMES_HOME/node\" npm@\"$range\""
    return 1
}

_nb_have_modern_node() {
    command -v node >/dev/null 2>&1 || return 1
    [ "$(_nb_node_major)" -ge "$SANREMES_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $SANREMES_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$SANREMES_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$SANREMES_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $SANREMES_NODE_TARGET_MAJOR..."
    proto install node "$SANREMES_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $SANREMES_NODE_TARGET_MAJOR..."
    nvm install "$SANREMES_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$SANREMES_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${SANREMES_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${SANREMES_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${SANREMES_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${SANREMES_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${SANREMES_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $SANREMES_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $SANREMES_HOME/node/..."
    ...
    mv "$extracted" "$SANREMES_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # SANREMES_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${SANREMES_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
        ln -sf "$SANREMES_HOME/node/bin/node" "$_link_dir/node"
        ln -sf "$SANREMES_HOME/node/bin/npm"  "$_link_dir/npm"
        ln -sf "$SANREMES_HOME/node/bin/npx"  "$_link_dir/npx"
    fi
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $SANREMES_HOME/node/"
...
# ---------------------------------------------------------------------------
# Heal a broken SanRemes-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$SANREMES_HOME/node/bin/$tool" \
        "$SANREMES_HOME/node/${tool}.exe" \
        "$SANREMES_HOME/node/$tool"; do
...
# The managed node runs but is below SANREMES_NODE_TARGET_MAJOR — an old tree
...
# sanremes_constants.py.
_nb_managed_node_outdated() {
    local probe ver major
    for probe in "$SANREMES_HOME/node/bin/node" "$SANREMES_HOME/node/node"; do
        [ -x "$probe" ] || continue
        ver="$("$probe" --version 2>/dev/null)" || return 1
        major="${ver#v}"; major="${major%%.*}"
        case "$major" in ''|*[!0-9]*) return 1 ;; esac
        [ "$major" -lt "$SANREMES_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
...
# absent. Used by sanremes_constants.find_sanremes_node_executable() and safe
# to call from install reruns.
heal_managed_node() {
    [ -d "$SANREMES_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "SanRemes-managed Node is broken — redownloading to $SANREMES_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        SANREMES_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$SANREMES_HOME/node/bin/node" ]; then
        export PATH="$SANREMES_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (SanRemes-managed)"
            SANREMES_NODE_AVAILABLE=true
    ...
    _nb_try_fnm   && { SANREMES_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { SANREMES_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { SANREMES_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { SANREMES_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { SANREMES_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { SANREMES_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $SANREMES_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
