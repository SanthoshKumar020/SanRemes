...
# changing PATH, installing software, or touching the user's SanRemes home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("sanremes-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$SanRemesHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -SanRemesHome $SanRemesHome -InstallDir $InstallDir
