...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[sanremes] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[sanremes`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[sanremes`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        SanRemesHome = $(if ($paths) { $paths.sanremes_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\sanremes\sanremes-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-sanremes'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-sanremes') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
