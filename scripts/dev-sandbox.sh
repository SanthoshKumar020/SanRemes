...
unprivileged `sanremes` user, giving the layout most people have —
$SANREMES_HOME/sanremes-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  SANREMES_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .sanremes-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `sanremes setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${SANREMES_DEV_SANDBOX_UPSTREAM:-https://github.com/NousResearch/sanremes-agent.git}"
...
GIT_ROOT="${SANREMES_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${SANREMES_DEV_SANDBOX_DIR:-.sanremes-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t sanremes-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t sanremes-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/sanremes-agent.nousresearch.com"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/sanremes-agent.nousresearch.com/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/sanremes-agent.nousresearch.com/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://sanremes-agent.nousresearch.com/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/sanremes-agent.git update-ref refs/heads/main "$next_main"; then
...
if [ "$RUN_AS_USER" = true ]; then
  SANDBOX_UID=1000
  SANDBOX_GID=1000
  SANDBOX_USER=sanremes
  SANDBOX_HOME=/home/sanremes
else
  SANDBOX_UID=0
  SANDBOX_GID=0
  SANDBOX_USER=root
...
# A user-level install writes the `sanremes` launcher to ~/.local/bin and the
# checkout to $SANREMES_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/sanremes-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q sanremes-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t sanremes-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'SanRemes sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/sanremes-sandbox/next"
...
if [ ! -f "$SANDBOX_ROOT/root/certs/ca.pem" ]; then
  if ! ca_error="$(OPENSSL_CONF="$SANDBOX_ROOT/root/certs/openssl.cnf" \
    ...
    -subj '/CN=SanRemes dev sandbox CA' \
