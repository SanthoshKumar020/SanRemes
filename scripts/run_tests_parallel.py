...
Environment:
    SANREMES_TEST_WORKERS  Override worker count (default: os.cpu_count())
    SANREMES_TEST_PATHS    Override discovery roots (colon-sep; on Windows
...
#                        nousresearch/sanremes-agent:test image, via
#                        ``SANREMES_TEST_IMAGE`` so the fixture skips
...
# via --file-timeout or SANREMES_TEST_FILE_TIMEOUT.
#
...
# Set to 0 to disable (env: SANREMES_TEST_FILE_RETRIES).
...
def _split_pathspec(value: str) -> List[str]:
    ...
    ``SANREMES_TEST_PATHS``) into individual paths.
...
def _run_one_file_once(
    ...
    env = os.environ.copy()
    temproot = tempfile.mkdtemp(prefix="sanremes-pytest-tmproot-")
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/sanremes-agent/sanremes-agent/tests/acp/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("SANREMES_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $SANREMES_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("SANREMES_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
        help=(
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("SANREMES_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            f"Default: {_DEFAULT_FILE_TIMEOUT_SECONDS}s ({round(_DEFAULT_FILE_TIMEOUT_SECONDS/60)} min), env: SANREMES_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--file-retries",
        type=int,
        default=int(
            os.environ.get("SANREMES_TEST_FILE_RETRIES", _DEFAULT_FILE_RETRIES)
        ),
        help=(
            ...
            f"Default: {_DEFAULT_FILE_RETRIES}, env: SANREMES_TEST_FILE_RETRIES."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: SANREMES_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "--generate-slices",
    ...
    # Parse --slice (or SANREMES_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("SANREMES_TEST_SLICE")
