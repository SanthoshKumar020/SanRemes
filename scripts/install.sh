...
# Python-driven tool session (e.g. SanRemes terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/sanremes-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/sanremes-agent.git"
SANREMES_HOME="${SANREMES_HOME:-$HOME/.sanremes}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${SANREMES_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SANREMES_INSTALL_DIR"
...
#   code at /usr/local/lib/sanremes-agent, command at /usr/local/bin/sanremes,
#   data still at /root/.sanremes (SANREMES_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$SANREMES_HOME/.no-bundled-skills so future"
            echo "                   'sanremes update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> SanRemes.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.sanremes/sanremes-agent"
            echo "                   default (root, Linux): /usr/local/lib/sanremes-agent"
            echo "  --sanremes-home PATH  Data directory (default: ~/.sanremes, or \$SANREMES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, SanRemes installs the code under"
            echo "  /usr/local/lib/sanremes-agent and links the command into"
            echo "  /usr/local/bin/sanremes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SANREMES_HOME"
            echo "  (default /root/.sanremes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SANREMES_HOME/sanremes-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `sanremes update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (SanRemes-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download SanRemes Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install sanremes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `sanremes` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SANREMES_HOME/sanremes-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SANREMES_HOME/sanremes-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/sanremes-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SANREMES_HOME/sanremes-agent — then preserve it)
#
# Always no-op when the user set --dir or $SANREMES_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SANREMES_HOME.
    if is_termux; then
        INSTALL_DIR="$SANREMES_HOME/sanremes-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SANREMES_HOME/sanremes-agent/.git" ]; then
            INSTALL_DIR="$SANREMES_HOME/sanremes-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/sanremes-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/sanremes-agent"
        ...
        # /usr/local/bin/sanremes wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/sanremes"
        log_info "  Data:    $SANREMES_HOME (unchanged)"
...
# Point a SanRemes-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $SANREMES_HOME/node/bin — which is NOT on
...
# SanRemes's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no SanRemes-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$SANREMES_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$SANREMES_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$SANREMES_HOME/node/etc/npmrc"
}

get_sanremes_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/sanremes" ]; then
        echo "$link_dir/sanremes"
    else
        echo "sanremes"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://sanremes-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # SanRemes owns its own uv at $SANREMES_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (sanremes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `sanremes update` stay in sync.
    local _managed_uv="$SANREMES_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $SANREMES_HOME/bin ..."
    mkdir -p "$SANREMES_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/sanremes-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/sanremes-uv-installer.$$.sh")"
    ...
    # directly into $SANREMES_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$SANREMES_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# replaced with the SanRemes-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing SanRemes-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing SanRemes-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a SanRemes-managed Node from a previous run over a too-old system one.
    if [ -x "$SANREMES_HOME/node/bin/node" ] && [ -x "$SANREMES_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$SANREMES_HOME/node/bin/node" --version)"; then
        export PATH="$SANREMES_HOME/node/bin:$PATH"
        log_success "Node.js $("$SANREMES_HOME/node/bin/node" --version) found (SanRemes-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing SanRemes-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (SanRemes requires Node 22.22+, 24.11+, or 26+) — installing SanRemes-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.sanremes/node/..."
    ...
    # Place into ~/.sanremes/node/ and symlink binaries into the same bin dir
    # the sanremes command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$SANREMES_HOME/node"
    mkdir -p "$SANREMES_HOME"
    mv "$extracted_dir" "$SANREMES_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$SANREMES_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$SANREMES_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$SANREMES_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$SANREMES_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.sanremes/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. SanRemes install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "SanRemes Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `sanremes update` path
                # (#4735).
                ...
                local stash_name
                stash_name="sanremes-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``sanremes update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if SanRemes behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up sanremes command..."

    if [ "$USE_VENV" = true ]; then
        SANREMES_BIN="$INSTALL_DIR/venv/bin/python"
        SANREMES_ENTRYPOINT="$INSTALL_DIR/sanremes"
    else
        SANREMES_BIN="$(which sanremes 2>/dev/null || echo "")"
        if [ -z "$SANREMES_BIN" ]; then
            log_warn "sanremes not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$SANREMES_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$SANREMES_ENTRYPOINT" ]; }; then
        log_warn "SanRemes launcher prerequisites not found"
    ...
    # Create a user-facing shim for the sanremes command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $SANREMES_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$SANREMES_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/sanremes"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/sanremes" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SANREMES_BIN" "$SANREMES_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/sanremes" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/sanremes"
    log_success "Installed sanremes launcher → $command_link_display_dir/sanremes"

    # Also expose `sanremes-agent`. The `sanremes-agent` console script declared in
    ...
    rm -f "$command_link_dir/sanremes-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/sanremes-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SANREMES_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/sanremes-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SANREMES_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/sanremes-agent"
    log_success "Installed sanremes-agent launcher → $command_link_display_dir/sanremes-agent"

    # Also expose `sanremes-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `sanremes-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report SanRemes as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/sanremes-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/sanremes-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SANREMES_BIN" "$SANREMES_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/sanremes-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SANREMES_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/sanremes-acp"
    log_success "Installed sanremes-acp launcher → $command_link_display_dir/sanremes-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "sanremes command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v sanremes' \
                >/dev/null 2>&1; then
        ...
        log_info "sanremes not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# SanRemes Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# SanRemes Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# SanRemes Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so sanremes works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.sanremes directory structure (config at top level, code in subdir)
    mkdir -p "$SANREMES_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.sanremes/.env (top level, easy to find)
    if [ ! -f "$SANREMES_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SANREMES_HOME/.env"
            log_success "Created ~/.sanremes/.env from template"
        else
            touch "$SANREMES_HOME/.env"
            log_success "Created ~/.sanremes/.env"
        fi
    else
        log_info "~/.sanremes/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SANREMES_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.sanremes/config.yaml (top level, easy to find)
    if [ ! -f "$SANREMES_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SANREMES_HOME/config.yaml"
            log_success "Created ~/.sanremes/config.yaml from template"
        fi
    else
        log_info "~/.sanremes/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in sanremes_cli/default_soul.py — the
    ...
    if [ ! -f "$SANREMES_HOME/SOUL.md" ]; then
        cat > "$SANREMES_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.sanremes/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.sanremes/"

    # Seed bundled skills into ~/.sanremes/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `sanremes update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'sanremes update'." \
            > "$SANREMES_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $SANREMES_HOME/.no-bundled-skills"
        log_info "  Future 'sanremes update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.sanremes/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.sanremes/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SANREMES_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$SANREMES_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.sanremes/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$SANREMES_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# SanRemes Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "SanRemes will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# SanRemes Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `sanremes update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `sanremes tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$SANREMES_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside SanRemes' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$SANREMES_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'sanremes tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `sanremes tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `sanremes computer-use install` on their own).
    ...
    # Same upstream installer `sanremes computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the SanRemes install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching SanRemes'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'sanremes tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: sanremes computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'sanremes setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m sanremes_cli.main setup < /dev/tty
    else
        python -m sanremes_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$SANREMES_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for SanRemes to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SANREMES_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'sanremes whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SANREMES_CMD="$(get_sanremes_command_path)"
                $SANREMES_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'sanremes whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'sanremes gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SANREMES_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SANREMES_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: sanremes gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: sanremes gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SANREMES_CMD gateway > "$SANREMES_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.sanremes/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: sanremes gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: sanremes gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.sanremes-bootstrap-complete, which tells the SanRemes
    ...
    local marker_path="$INSTALL_DIR/.sanremes-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SANREMES_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SANREMES_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SANREMES_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}sanremes${NC}              Start chatting"
    echo -e "   ${GREEN}sanremes setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}sanremes config${NC}       View/edit configuration"
    echo -e "   ${GREEN}sanremes config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}sanremes gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}sanremes update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'sanremes' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'sanremes' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'sanremes' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SANREMES_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SANREMES_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SANREMES_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$SANREMES_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`SanRemes` rename
...
# _purge_electron_build_cache() used by `sanremes desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the SanRemes-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `sanremes desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/SanRemes" ]; then
            app="$desktop_dir/release/linux-unpacked/SanRemes"
        elif [ -x "$desktop_dir/release/linux-unpacked/sanremes" ]; then
            app="$desktop_dir/release/linux-unpacked/sanremes"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/SanRemes.app" \
            "$desktop_dir/release/mac/SanRemes.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `sanremes desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading SANREMES_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if SANREMES_HOME="$SANREMES_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from sanremes_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `sanremes update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the SanRemes-managed Node
            # provisioned during prerequisites/node-deps (at $SANREMES_HOME/node/bin)
        ...
        complete)
            ...
            # $SANREMES_HOME. $SANREMES_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'sanremes update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $SANREMES_HOME.
    # $SANREMES_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'sanremes update' on this host install.
