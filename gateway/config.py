...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from sanremes_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (sanremes_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "SanRemes is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_sanremes_home() / "sessions")
    ...
    # gateway behaves exactly as before — single SANREMES_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.sanremes/config.yaml (primary user-facing config)
    3. ~/.sanremes/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_sanremes_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through sanremes_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `sanremes config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `sanremes config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``sanremes config set gateway.streaming.*``
