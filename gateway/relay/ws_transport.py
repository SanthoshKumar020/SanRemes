...
def _normalize_slack_parent_command(
    ...
    """Mirror native Slack ``/sanremes`` routing for authenticated relay text."""
    stripped = text.strip()
    parent_parts = stripped.split(maxsplit=1)
    if not parent_parts or parent_parts[0] != "/sanremes":
...
def _event_from_wire(raw: Dict[str, Any]) -> MessageEvent:
    ...
    source = SessionSource(
        ...
        # The SANREMES profile this event is routed to (multiplex mode). The
    ...
    if platform_enum == Platform.SLACK:
        ...
        # relay, bypassing SanRemes' native Slack command callback. Normalize at
        # the wire boundary so adapter-level active-session gates see the real
        # gateway command rather than the legacy `sanremes` parent name.
