...
Module-level run.py helpers a handler needs (``_sanremes_home``,
...
from sanremes_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: sanremes gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach SanRemes commands.
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            from sanremes_cli.lifecycle import invoke_hook as _invoke_hook
        ...
        try:
            from sanremes_cli.tips import get_random_tip
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        from sanremes_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from sanremes_cli.kanban import run_slash
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`sanremes gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _sanremes_home
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _sanremes_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _sanremes_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running SanRemes Agent version."""
        from sanremes_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _sanremes_home, _load_gateway_config
        ...
        from sanremes_cli.providers import get_label
        ...
        # via the shared single-owner parser (sanremes_cli.model_switch).
        ...
        if force_refresh:
            try:
                from sanremes_cli.models import clear_provider_models_cache
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _sanremes_home) / "config.yaml"
        try:
            ...
            if cfg:
                ...
                try:
                    from sanremes_cli.config import get_compatible_custom_providers
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from sanremes_cli.context_switch_guard import (
                        ...
                        from sanremes_cli.model_switch import format_model_for_display
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from sanremes_cli.route_identity import should_clear_context_pin_async
                        ...
                        from sanremes_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — SanRemes default runtime
        ...
        from sanremes_cli import codex_runtime_switch as crs
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        All resolution/persistence goes through sanremes_cli.personality —
        the single owner of personality state on every surface.
        """
        from gateway.run import _load_gateway_config
        from sanremes_cli.personality import (
        ...
        # Persist the selection only — sanremes_cli.personality never writes
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from sanremes_cli.goals import draft_contract
            ...
            from sanremes_cli.goals import parse_contract
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from sanremes_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `sanremes cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from sanremes_cli.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        SanRemes itself changed. ``--stat`` limits output to the summary.
    ...
    def _save_gateway_config_key(self, key_path: str, value) -> bool:
        ...
        from gateway.run import _sanremes_home
        from sanremes_cli.config import read_user_config_raw
        config_path = _sanremes_home / "config.yaml"
        try:
    ...
    def _reasoning_picker_choices(self, current_effort: str) -> list:
        ...
        from sanremes_constants import VALID_REASONING_EFFORTS
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _sanremes_home
        from sanremes_cli.write_approval_commands import handle_pending_subcommand
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.sanremes/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``sanremes skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _sanremes_home
        from sanremes_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `sanremes skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.sanremes/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> Optional[str]:
        ...
        from sanremes_cli.models import model_supports_fast_mode
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        from gateway.slash_access import policy_for_source
        from sanremes_cli.approval_mode import run_approval_mode_command
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _sanremes_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _sanremes_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_topic_command(self, event: MessageEvent, args: str = "") -> str:
        ...
        if not self._session_db:
            from sanremes_state import format_session_db_unavailable
    ...
    async def _handle_save_command(self, event: MessageEvent) -> str:
        ...
        """
        from sanremes_cli.session_export import (
            SAVE_USAGE,
        ...
        if redact:
            from sanremes_cli.session_export_md import redact_session_data

            export_data = redact_session_data(export_data)

        import tempfile

        temp_dir = tempfile.mkdtemp(prefix="sanremes_save_")
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from sanremes_cli.session_listing import (
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        try:
            await self._session_db.create_session(
                ...
                # path (sanremes_state.py:1994-2009) that searches by the complete
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `sanremes bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``sanremes debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update SanRemes Agent to the latest version.

        Spawns ``sanremes update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``sanremes update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _sanremes_home, _resolve_sanremes_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from sanremes_cli.config import is_managed, format_managed_message
        ...
        if is_managed():
            return f"✗ {format_managed_message('update SanRemes Agent')}"
        ...
        sanremes_cmd = _resolve_sanremes_bin()
        if not sanremes_cmd:
            return t("gateway.update.sanremes_cmd_not_found")

        pending_path = _sanremes_home / ".update_pending.json"
        output_path = _sanremes_home / ".update_output.txt"
        exit_code_path = _sanremes_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `sanremes update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `sanremes update --gateway`
        ...
        try:
            if sys.platform == "win32":
                import textwrap
                from sanremes_cli._subprocess_compat import windows_detach_popen_kwargs

                # Invoke the updater as a module under this interpreter rather
                # than through sanremes_cmd (venv\Scripts\sanremes.exe): the shim
                ...
                subprocess.Popen(
                    [
                        ...
                        sys.executable, "-m", "sanremes_cli.main",
                        "update", "--gateway",
                    ],
                ...
                sanremes_cmd_str = " ".join(shlex.quote(part) for part in sanremes_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {sanremes_cmd_str} update --gateway"
