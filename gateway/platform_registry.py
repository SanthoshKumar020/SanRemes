...
class PlatformEntry:
    ...
    # status displays (``sanremes setup``, ``sanremes status``, the dashboard
    ...
    # Env vars this platform needs (for ``sanremes setup`` display).
    ...
    # built-ins).  Used by ``sanremes gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved SANREMES_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `sanremes`
        # invocation -- including plain `sanremes chat`, which never touches any
        ...
        # `sanremes setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``sanremes setup``/``gateway status``, channel
