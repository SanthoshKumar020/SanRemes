    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if sanremes_home_str:
            takeover_path = Path(sanremes_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(sanremes_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(
    ...
    upgraded sanremes-agent but never re-ran ``sanremes setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../sanremes-gateway.service"
    ...
    # that's the common case for sanremes.
...
def parse_systemd_duration_to_us(raw: str) -> Optional[int]:
    ...
    Public: also consumed by sanremes_cli.gateway's restart-wait sizing.
    """
    if not raw:
        return None
    units = {
