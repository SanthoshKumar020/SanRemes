...
# ``SANREMES_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``SANREMES_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("SANREMES_AUTO_CONTINUE_FRESHNESS")
...
# ``sessions_dir / f"{session_id}.json"`` in sanremes_state, request-dump
...
def _slack_tools_loaded() -> bool:
    ...
    Two independent paths grant Slack capability:
      1. Native `slack` toolset enabled via `sanremes tools` (opt-in, default
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `sanremes tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `sanremes tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_sanremes_home()}/cron/output/)"
    )
...
def build_channel_continuity_note(
    ...
    return (
        f"[System note: This {where} had an earlier SanRemes session "
...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        # ``get_sanremes_home()`` for the turn (its docstring lists "sessions"
    ...
    def _open_session_db_for_active_scope(self):
        ...
        time, and that helper follows the context-local SANREMES_HOME override
        ...
        """
        from sanremes_state import SessionDB, _default_db_path
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.sanremes/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.sanremes/state.db "
                "and are shown by `sanremes sessions list` and `/sessions`. "
    ...
    def _append_to_transcript_serialized(
        ...
        while True:
            try:
                ...
                from sanremes_state import CompressionSessionClosedError
