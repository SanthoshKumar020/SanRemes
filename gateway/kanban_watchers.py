...
def _kanban_dispatch_allowed() -> bool:
    """Return False while the global emergency stop (`sanremes pause`) is engaged.
...
class GatewayKanbanWatchersMixin:
        ...
        while self._running:
            try:
                ...
                def _collect():
                    ...
                    # SANREMES_KANBAN_DB pins the board path; without this guard
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `sanremes kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours SANREMES_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        try:
            ...
            return
        env_override = os.environ.get("SANREMES_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via SANREMES_KANBAN_DISPATCH_IN_GATEWAY env")
            return
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `sanremes kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real SanRemes profile
            ...
            PATH, missing venv, credential loss for a real SanRemes profile).
            """
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            """
            try:
                from sanremes_cli import kanban_decompose as _decomp
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("SANREMES_KANBAN_BOARD")
                try:
                    os.environ["SANREMES_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("SANREMES_KANBAN_BOARD", None)
                    else:
                        os.environ["SANREMES_KANBAN_BOARD"] = prev_env
            return successes
        ...
        while self._running:
            ...
            try:
                # Global emergency stop (`sanremes pause`): skip auto-decompose
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`sanremes kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
