...
SanRemes' long-running gateway status surface into short status lines suitable
for chat surfaces.

Built-in defaults live in ``gateway/assets/status_phrases.yaml``. Users can add
portable, profile-relative phrase catalogs under ``SANREMES_HOME`` either by using
conventional paths::

    ~/.sanremes/status_phrases.yaml
    ~/.sanremes/status_phrases/*.yaml
...
# These are SanRemes UI surfaces, not app/vendor/domain buckets.  Keep this
...
def resolve_status_phrase_catalog(user_config: Mapping[str, Any] | None, platform_key: str | None = None) -> dict[str, list[str]]:
    ...
    _merge_phrase_paths(catalog, list(_CONVENTIONAL_RELATIVE_PATHS), base_dir=sanremes_home)

    display = (user_config or {}).get("display") if isinstance(user_config, Mapping) else None
    if not isinstance(display, Mapping):
        return catalog

    _merge_phrase_config(catalog, display.get("generic_status_phrases"), base_dir=sanremes_home)
    _merge_phrase_config(catalog, display.get("status_phrases"), base_dir=sanremes_home)
    ...
    if platform_key and isinstance(platforms, Mapping):
        ...
        if isinstance(platform_display, Mapping):
            _merge_phrase_config(catalog, platform_display.get("generic_status_phrases"), base_dir=sanremes_home)
            _merge_phrase_config(catalog, platform_display.get("status_phrases"), base_dir=sanremes_home)
    return catalog
...
def classify_status_context(
    ...
    *,
    ...
    """Classify an internal gateway event into a SanRemes UI-surface bucket."""
