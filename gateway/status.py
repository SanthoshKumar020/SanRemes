...
The PID file lives at ``{SANREMES_HOME}/gateway.pid``.  SANREMES_HOME defaults to
``~/.sanremes`` but can be overridden via the environment variable.  This means
separate SANREMES_HOME directories naturally get separate PID files — a property
...
from sanremes_constants import get_sanremes_home, _get_platform_default_sanremes_home
...
_GATEWAY_KIND = "sanremes-gateway"
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_sanremes_home() / "gateway-starts.log"
...
def _get_process_sanremes_home() -> Path:
    """Return the process-level SANREMES_HOME, skipping context-local overrides.
    ...
    ``get_sanremes_home()`` honors ``_SANREMES_HOME_OVERRIDE`` contextvar used for
...
def _canonical_sanremes_home(path: Path | str) -> Path:
    """Return a stable absolute SANREMES_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_sanremes_home(left: Path | str, right: Path | str) -> bool:
    """Compare SANREMES_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_sanremes_home(left))) == os.path.normcase(
        str(_canonical_sanremes_home(right))
    )


# Mirrors sanremes_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (sanremes_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a SANREMES_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.sanremes/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ``"default"`` for the deployment's root home, and ``None`` when no label
    can be inferred.  Never raises — this feeds diagnostics only.
    """
    try:
        canonical = _canonical_sanremes_home(home)
    except Exception:
        return None
    ...
    try:
        ...
        if _same_sanremes_home(canonical, get_default_sanremes_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_sanremes_home(canonical, _get_platform_default_sanremes_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    SANREMES_HOME profile than the caller.  Prefers the explicit ``profile``
    field stamped by :func:`acquire_scoped_lock`; falls back to inferring the
    label from the persisted ``sanremes_home`` for locks written before the
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SANREMES_HOME."""
    home = _get_process_sanremes_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("SANREMES_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "sanremes" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in SanRemes' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the SanRemes gateway lifecycle subcommand from a command line.
    ...
    test also matched ``sanremes_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\sanremes-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- SanRemes's
    ...
    for token in tokens:
        ...
        if basename in ("sanremes-gateway", "sanremes-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "sanremes_cli.main" in joined
        or "sanremes_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("sanremes", "sanremes.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `sanremes gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating SanRemes-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the SanRemes gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a SANREMES_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.sanremes`` or ``$SANREMES_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``sanremes_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``SANREMES_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a SANREMES_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"sanremes_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit SANREMES_HOME= on the argv. SANREMES_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "sanremes_home=" in command_lc and f"sanremes_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # SANREMES_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "sanremes_home": str(_canonical_sanremes_home(_get_process_sanremes_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    ``sanremes_cli``; the helper itself is cached per process. A gateway
    keeps serving the module versions it imported at startup, so stamping
    the identity into ``gateway_state.json`` lets `sanremes update` (and the
    ...
    """
    try:
        from sanremes_cli.build_info import get_code_identity
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `sanremes status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``SANREMES_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``sanremes_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    SANREMES_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different SANREMES_HOME dirs).
    """
    ...
    # is inferable; readers fall back to deriving it from sanremes_home.
    profile = _profile_label_for_home(_get_process_sanremes_home())
    if profile:
...
# enabled in the user's systemd (e.g. ``sanremes.service`` + ``sanremes-
# gateway.service``).
#
...
def _get_takeover_marker_path(sanremes_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``sanremes_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = sanremes_home or _get_process_sanremes_home()
    return _canonical_sanremes_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-SANREMES_HOME --replace while
    ...
    our_home = _get_process_sanremes_home()
    target_home = record.get("target_sanremes_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_sanremes_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_sanremes_home")
        if replacer_home is not None and not _same_sanremes_home(
    ...
    # ``sanremes gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's SANREMES_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_sanremes_home(
            target_home or _get_process_sanremes_home()
        )
        ...
        record = {
            ...
            "target_sanremes_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_sanremes_home": str(
                _canonical_sanremes_home(_get_process_sanremes_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target SANREMES_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("sanremes_home")
    if not isinstance(pid_record_home, str) or not _same_sanremes_home(
