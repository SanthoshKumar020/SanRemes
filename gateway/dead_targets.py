...
The store is a small JSON file under the active profile's SANREMES_HOME so each
...
class DeadTargetRegistry:
    ...
    def __init__(self, path: Optional[Path] = None) -> None:
        ...
        if path is not None:
            ...
            self._path = get_sanremes_home() / "gateway" / "dead_targets.json"
        self._load()
    ...
    def all_dead(self) -> Dict[str, Dict[str, object]]:
        """Snapshot of the current dead set (for diagnostics / `sanremes` CLI)."""
        with self._lock:
