...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-SanRemes-Session-Id header; opt-in long-term memory scoping via X-SanRemes-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-SanRemes-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists sanremes-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible SanRemes sessions
- POST /api/sessions               — create an empty SanRemes session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to sanremes-agent
...
def _prefix_names_served_profile(profile: str) -> bool:
    ...
    """
    try:
        from sanremes_cli.profiles import profile_matches_home
...
_BROWSER_CONTROL_WS_PROTOCOL = "sanremes-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "sanremes-browser-control-ticket."
...
def _sanremes_version() -> str:
    """Return the canonical SanRemes Agent version string.

    ``sanremes_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``sanremes_agent-*.dist-info`` after a source update until the environment is
    reinstalled. Never raises — a version probe must not be able to break the
    health endpoint.
    """
    try:
        from sanremes_cli import __version__
...
# config.yaml accept (sanremes_constants.VALID_REASONING_EFFORTS); wire-level
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from sanremes_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("SANREMES_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``sanremes-agent``)
    ...
    ``direct_model_requests`` config value here, while SanRemes-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously SanRemes-aware and is
    always honored.
    """
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                from sanremes_cli.config import get_sanremes_home
                db_path = str(get_sanremes_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted SANREMES_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # sanremes_state._WAL_INCOMPAT_MARKERS).
        from sanremes_state import apply_wal_with_fallback
...
def _derive_chat_session_id(
    ...
    the same SanRemes session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through sanremes-agent's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the SanRemes-native session-chat
    ...
    def _resolve_max_concurrent_runs() -> int:
        ...
        default = 10
        try:
            from sanremes_cli.config import cfg_get, load_config
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: "sanremes-agent"

        Delegates the tiered fallthrough to
        :func:`sanremes_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from sanremes_cli.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "sanremes-agent")
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from sanremes_cli.auth import has_usable_secret
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            # `sanremes peer dm mini/researcher` answered by the mini's default
        ...
        try:
            from sanremes_cli.profiles import profiles_to_serve
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_sanremes_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
        from sanremes_cli.profiles import get_profile_dir
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-SanRemes-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-SanRemes-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-SanRemes-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-SanRemes-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-SanRemes-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``sanremes sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_sanremes_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_sanremes_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        The advertised virtual model (usually ``sanremes-agent``) means "use
        ...
        400s ("sanremes-agent is not a valid model ID") — the same filter
    ...
    def _session_runtime_request_from_body(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # The virtual model alias (self._model_name, e.g. "sanremes-agent") is
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "sanremes_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "sanremes_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the sanremes-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-SanRemes-Session-Key``).  Unlike ``session_id``
        ...
        # sanremes_cli.model_switch.resolve_effective_model (session /model
        ...
        # (e.g. user ran `sanremes auth add openai-codex` without `sanremes model`),
        ...
        if not model and runtime_kwargs.get("provider"):
            try:
                from sanremes_cli.models import get_default_model_for_provider
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "sanremes-agent", "version": _sanremes_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "readiness": readiness,
            "platform": "sanremes-agent",
            "version": _sanremes_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list sanremes-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return SanRemes provider/model inventory.
        ...
        SanRemes provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
        ...
        try:
            from sanremes_cli.inventory import build_model_options_payload, load_picker_context
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every SanRemes version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "sanremes.api_server.capabilities",
            "platform": "sanremes-agent",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side SanRemes AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-SanRemes-Session-Id",
                "session_key_header": "X-SanRemes-Session-Key",
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<SANREMES_HOME>/plugin-data/.../artifacts``-style controlled root),
        ...
        try:
            ...
            # under the SanRemes home.
            try:
                from sanremes_state import get_sanremes_home

                root = Path(get_sanremes_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    async def _handle_toolsets(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            from sanremes_cli.tools_config import (
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted SanRemes sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        # Exact-title lookup, used by `sanremes peer dm` to resolve a peer's
        ...
        if title_filter:
            ...
            if not sessions:
                ...
                # which would fail `sanremes peer dm` resolution and mint
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty SanRemes session row.
        ...
        # (e.g. "provider::sanremes-agent") are split, and the virtual model
        # alias (self._model_name, e.g. "sanremes-agent") is nulled out
        # there — a bare "sanremes-agent" is not a model_routes alias, so a
        ...
        return web.json_response({"object": "sanremes.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "sanremes.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "sanremes.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "sanremes.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-SanRemes-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-SanRemes-Session-Key"] = gateway_session_key
        runtime = {}
        ...
        return web.json_response(
            {
                "object": "sanremes.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-SanRemes-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-SanRemes-Session-Key"] = gateway_session_key
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "sanremes.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-SanRemes-Session-Key.  This
        # is independent of X-SanRemes-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-SanRemes-Session-Id.
        ...
        provided_session_id = request.headers.get("X-SanRemes-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-SanRemes-Session-Id rejected: "
            ...
            # conversation map to the same SanRemes session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``sanremes.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-SanRemes-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-SanRemes-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["sanremes"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-SanRemes-Completed"] = "false"
            response_headers["X-SanRemes-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + SanRemes-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-SanRemes-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-SanRemes-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-SanRemes-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-SanRemes-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: sanremes.tool.progress`` SSE event so
                frontends can display them without storing the markers in
                conversation history.  See #6972 for the original event,
                #16588 for the ``toolCallId``/``status`` lifecycle fields.
                """
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="sanremes.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-SanRemes-Session-Id header.  Without this,
        ...
        response_headers = {"X-SanRemes-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-SanRemes-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        # `sanremes cron run` / cronjob(action='run', prompt=...) — same length
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # SanRemes agent; they are replayed for structured tool
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-SanRemes-Session-Id header) can track compression-
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_sanremes_api_runtime", {}) or {})
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-SanRemes-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "sanremes.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def _handle_steer_run(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "sanremes.run.steer", "run_id": run_id, "accepted": True})
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local SanRemes-Relay
            ...
            # that is the exact surface the sanremes-0day campaign abused to write
            # ~/.sanremes/config.yaml and plant persistence. Sandboxing (Docker /
            ...
            if is_network_accessible(self._host):
                try:
                    from sanremes_cli.config import load_config as _load_cfg
                    _backend = (
