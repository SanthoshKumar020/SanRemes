...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.sanremes to the active profile home."""
    ...
    if raw.startswith("~/.sanremes/"):
        return sanremes_home / raw.removeprefix("~/.sanremes/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return sanremes_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under SANREMES_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from sanremes_constants import get_sanremes_home

    scripts_root = (get_sanremes_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.sanremes" or raw_text.startswith("~/.sanremes/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__sanremes_ignore__") is True
        ):
            return False, None
