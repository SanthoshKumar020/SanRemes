...
def _get_sanremes_version() -> str:
    """Return the sanremes-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; SanRemes/<sanremes_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; SanRemes/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    sanremes_version = _get_sanremes_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; SanRemes/{sanremes_version})"
