...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.sanremes/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. SanRemes upgrades overwrite this
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "sanremes-agent",
                    "$device": "sanremes-agent",
                },
            },
        }
        try:
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.sanremes/.update_response`` for the
          detached ``sanremes update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``sanremes update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `sanremes setup gateway` / `sanremes gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``sanremes update --gateway`` watcher. Button
        ...
        ``~/.sanremes/.update_response`` so the detached update process
        can read it.
        """
