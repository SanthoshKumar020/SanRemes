...
There are two ways to add a platform to the SanRemes gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.sanremes/plugins/` (or under `plugins/platforms/`
...
**zero changes to core SanRemes code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `sanremes gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `sanremes_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the SanRemes core.
...
And add it to the `sanremes-gateway` composite:

```python
"sanremes-gateway": {
    "includes": [..., "sanremes-your-platform"]
}
```
...
## 12. Status Display (`sanremes_cli/status.py`)
...
## 13. Gateway Setup Wizard (`sanremes_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ sanremes_cli/ toolsets.py \
  --include="*.py" -l | sort -u
