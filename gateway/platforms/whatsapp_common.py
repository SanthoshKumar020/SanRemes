...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *SanRemes Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to SANREMES_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/sanremes), this function
    mirrors the bridge source to a writable SANREMES_HOME location and returns that
    ...
    # Try SANREMES_HOME location first
    sanremes_home = get_sanremes_home()
    sanremes_home_bridge = sanremes_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to SANREMES_HOME if needed
    if sanremes_home_bridge.exists():
        return sanremes_home_bridge

    # Mirror the bridge source to SANREMES_HOME
    try:
        sanremes_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            sanremes_home_bridge,
            dirs_exist_ok=False,
        )
        return sanremes_home_bridge
    except Exception:
