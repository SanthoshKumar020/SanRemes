...
# Audio file extensions SanRemes recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to SanRemes but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through SanRemes' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. SanRemes-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_sanremes_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``SANREMES_SESSION_PLATFORM`` contextvar — that contextvar is
...
from sanremes_constants import get_default_sanremes_root, get_sanremes_dir, get_sanremes_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.sanremes/.env manually."
)
...
IMAGE_CACHE_DIR = get_sanremes_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_sanremes_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_sanremes_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from sanremes_cli.config import load_config_readonly as _load_config
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; SanRemesAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_sanremes_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_sanremes_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_sanremes_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_sanremes_dir("cache/screenshots", "browser_screenshots")
...
_SANREMES_ROOT = get_default_sanremes_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "SANREMES_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "SANREMES_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "SANREMES_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "SANREMES_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _SANREMES_HOME / "image_cache",
    _SANREMES_HOME / "audio_cache",
    _SANREMES_HOME / "video_cache",
    _SANREMES_HOME / "document_cache",
    _SANREMES_HOME / "browser_screenshots",
    ...
    _SANREMES_HOME / "cache" / "images",
    _SANREMES_HOME / "cache" / "audio",
    _SANREMES_HOME / "cache" / "videos",
    _SANREMES_HOME / "cache" / "documents",
    _SANREMES_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared SanRemes root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* SANREMES_HOME's cache, so a gateway running at
    the root (e.g. ``SANREMES_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when SANREMES_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _SANREMES_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    override = os.environ.get("SANREMES_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return [Path(override).expanduser()]
    home_override = os.environ.get("SANREMES_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _SANREMES_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active SanRemes profile and shared SanRemes root both contain control
    ...
    # SANREMES_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.sanremes
    ...
    for sanremes_root in (_SANREMES_HOME, _SANREMES_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(sanremes_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and SanRemes
    secrets (``~/.sanremes/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted SanRemes cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.sanremes/...``
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted SanRemes cache dirs (``/root/.sanremes/...``), the session's
    ...
    # In-process gateways (Desktop backend, `sanremes serve`) may not have
    ...
    # are longer prefixes, so /root/.sanremes/... still translates to the host
    # cache — this only catches stray home writes like /root/out.png.
    if not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.sanremes/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.sanremes"):
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    ``SANREMES_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    SanRemes-managed cache, under an operator-allowlisted root
    (``SANREMES_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the SanRemes root (~/.sanremes/.env,
    ...
    # ``MEDIA:~/.sanremes/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?sanremes\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+sanremes[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``SANREMES_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # SanRemes commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-SANREMES_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("SANREMES_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "SANREMES_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "SANREMES_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: SanRemes sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-SANREMES_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(sanremes --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    def _wire_plugin_handlers(self, native: Any = None) -> None:
        ...
        try:
            from sanremes_cli.plugins import get_plugin_manager
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the SanRemes home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the SanRemes home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.sanremes/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from sanremes_cli.commands import (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          SANREMES_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          SANREMES_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          SANREMES_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("SANREMES_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("SANREMES_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("SANREMES_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_sanremes_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # SANREMES_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _sanremes_run_generation is set on
    ...
    def build_source(
        ...
        per-profile SANREMES_HOME isolation.
        """
