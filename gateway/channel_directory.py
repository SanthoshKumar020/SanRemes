...
~/.sanremes/channel_directory.json.  The send_message tool reads this file for
...
DIRECTORY_PATH = get_sanremes_home() / "channel_directory.json"
...
CHANNEL_ALIASES_PATH = get_sanremes_home() / "channel_aliases.json"
...
def _build_from_sessions_json(platform_name: str) -> List[Dict[str, str]]:
    ...
    sessions_path = get_sanremes_home() / "sessions" / "sessions.json"
...
def format_directory_for_display(platforms: Optional[Dict[str, Any]] = None) -> str:
    ...
    ``sanremes send --list`` to merge in configured-but-undiscovered
