...
# path eagerly freezes it to whatever SANREMES_HOME/profile context existed
# at that exact import moment for the rest of the process's lifetime --
# even if a context-local override (see sanremes_constants.set_sanremes_home_override)
# is established afterward. A freshly-started, short-lived process (e.g. the
# ``sanremes pairing`` CLI) re-imports this module later with the final
...
# working (NousResearch/sanremes-agent#93449).
#
...
def _default_pairing_dir() -> Path:
    ...
    if PAIRING_DIR is not None:
        return PAIRING_DIR
    return get_sanremes_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``sanremes_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from sanremes_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{SANREMES_HOME}/pairing`` while newer code/docs may
    write ``{SANREMES_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own SANREMES_HOME using the same legacy/consolidated layout rules
    as ``sanremes -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current SANREMES_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp SANREMES_HOME
        ...
        if profile:
            ...
            # ``sanremes -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `sanremes` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> sanremes pairing approve ...` as root, "
                    "re-run with `docker exec -u sanremes <container> ...` and "
                    "chown the existing file to the sanremes user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``sanremes
