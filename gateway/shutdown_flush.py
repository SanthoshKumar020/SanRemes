...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<sanremes_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active SANREMES_HOME."""
    from sanremes_constants import get_sanremes_home

    flush_dir = get_sanremes_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<sanremes_home>/pending_messages/``), so a runtime cap rotation no
