"""
Session-scoped context variables for the SanRemes gateway.

Replaces the previous ``os.environ``-based session state
(``SANREMES_SESSION_PLATFORM``, ``SANREMES_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["SANREMES_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("SANREMES_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("SANREMES_SESSION_PLATFORM", "")

    # after
    from gateway.session_context import get_session_env
    platform = get_session_env("SANREMES_SESSION_PLATFORM", "")
"""
...
_SESSION_PLATFORM: ContextVar = ContextVar("SANREMES_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("SANREMES_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("SANREMES_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_TYPE: ContextVar = ContextVar("SANREMES_SESSION_CHAT_TYPE", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("SANREMES_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("SANREMES_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("SANREMES_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_ID_ALT: ContextVar = ContextVar("SANREMES_SESSION_USER_ID_ALT", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("SANREMES_SESSION_USER_NAME", default=_UNSET)
...
_SESSION_SCOPE_ID: ContextVar = ContextVar("SANREMES_SESSION_SCOPE_ID", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("SANREMES_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("SANREMES_SESSION_ID", default=_UNSET)
# In-process UI session/window id for multi-session desktop/TUI hosts. This is
# intentionally separate from SANREMES_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("SANREMES_UI_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("SANREMES_SESSION_MESSAGE_ID", default=_UNSET)

_SESSION_PROFILE: ContextVar = ContextVar("SANREMES_SESSION_PROFILE", default=_UNSET)
_BROWSER_CONTROL_PRINCIPAL: ContextVar = ContextVar(
    "SANREMES_BROWSER_CONTROL_PRINCIPAL", default=_UNSET
)
_BROWSER_CONTROL_TRANSPORT_FAMILY: ContextVar = ContextVar(
    "SANREMES_BROWSER_CONTROL_TRANSPORT_FAMILY", default=_UNSET
)
...
_CRON_SESSION: ContextVar = ContextVar("SANREMES_CRON_SESSION", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("SANREMES_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("SANREMES_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("SANREMES_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("SANREMES_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "SANREMES_SESSION_PLATFORM": _SESSION_PLATFORM,
    "SANREMES_SESSION_SOURCE": _SESSION_SOURCE,
    "SANREMES_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "SANREMES_SESSION_CHAT_TYPE": _SESSION_CHAT_TYPE,
    "SANREMES_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "SANREMES_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "SANREMES_SESSION_USER_ID": _SESSION_USER_ID,
    "SANREMES_SESSION_USER_ID_ALT": _SESSION_USER_ID_ALT,
    "SANREMES_SESSION_USER_NAME": _SESSION_USER_NAME,
    "SANREMES_SESSION_SCOPE_ID": _SESSION_SCOPE_ID,
    "SANREMES_SESSION_KEY": _SESSION_KEY,
    "SANREMES_SESSION_ID": _SESSION_ID,
    "SANREMES_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "SANREMES_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "SANREMES_SESSION_PROFILE": _SESSION_PROFILE,
    "SANREMES_BROWSER_CONTROL_PRINCIPAL": _BROWSER_CONTROL_PRINCIPAL,
    "SANREMES_BROWSER_CONTROL_TRANSPORT_FAMILY": _BROWSER_CONTROL_TRANSPORT_FAMILY,
    "SANREMES_CRON_SESSION": _CRON_SESSION,
    "SANREMES_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "SANREMES_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "SANREMES_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``SANREMES_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("SANREMES_SESSION_ID")`` with an ``os.environ`` fallback,
    ...
    ``SANREMES_SESSION_ID`` for the rest of the process — leaking the child id
    ...
    # parent's session identity. See SanRemesPRDelegationSessionContext task.
    try:
    ...
    os.environ["SANREMES_SESSION_ID"] = session_id
...
def set_session_vars(
    ...
    ``os.environ["SANREMES_CRON_SESSION"]`` fallback, ``"1"`` marks a cron job,
...
def reset_session_vars() -> None:
    ...
    *A's* ``SANREMES_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``SANREMES_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``SANREMES_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("SANREMES_SESSION_*", default)``.
...
# value (``telegram``) to SANREMES_SESSION_PLATFORM, while the CLI, TUI, and
# desktop bind SANREMES_SESSION_SOURCE (``cli``, ``tui``, ``desktop``) and leave
...
# SANREMES_SESSION_PLATFORM but have no attachment channel behind them.
...
def session_is_messaging_surface() -> bool:
    ...
    Resolves ``SANREMES_PLATFORM``, then the session platform, then the session
    source, and reports messaging when any of them names a surface outside
    :data:`NON_MESSAGING_SESSION_SURFACES`.
    """
    import os

    platform = os.getenv("SANREMES_PLATFORM") or get_session_env("SANREMES_SESSION_PLATFORM", "")
...
def declare_stateless_channel() -> None:
    ...
    See NousResearch/sanremes-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    or a one-shot runner that exits after its final response (``sanremes -z``,
    cron — see :func:`declare_stateless_channel`) — and dispatcher-spawned
    Kanban workers (identified by ``SANREMES_KANBAN_TASK``), which are one-shot
