...
* Stateless request/response adapters (the API server,
  ...
  ``X-SanRemes-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-SanRemes-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-SanRemes-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-SanRemes-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "X-SanRemes-Session-Id is rejected (403) on an unauthenticated API "
    ...
    payload = {
        "model": str(getattr(adapter, "_model_name", "") or "sanremes-agent"),
