...
2. An event-loop heartbeat file at ``<SANREMES_HOME>/state/gateway.heartbeat`` so
...
def _process_sanremes_home() -> Path:
    """SANREMES_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<SANREMES_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_sanremes_home()
...
def get_loop_tick_socket_path(
    ...
    ``<SANREMES_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from sanremes_logging import drain_log_queue
...
async def loop_heartbeat_forever(
    ...
    *initiated* by the loop, so a frozen loop still lets the file age.
    ...
    wedged; see ``sanremes_cli.gateway.probe_gateway_loop_liveness`` for the
