...
(the updater, `sanremes serve`/dashboard, the Desktop app) currently discovers
...
- ``identify`` → pid, profile label, sanremes_home, code_sha/code_version
...
- POSIX: Unix domain socket at ``$SANREMES_HOME/gateway.sock``. When the home
  path is too long for ``sun_path`` (~104 bytes on macOS/BSD), the socket is
  bound in the system temp dir and a pointer file
  ``$SANREMES_HOME/gateway.sock.path`` records the real location; clients
  follow the pointer transparently.
- Windows: named pipe ``\\\\.\\pipe\\sanremes-gateway-<home-hash>`` served via
...
Consumers (``sanremes update --plan`` inventory, the post-update fleet version
...
def windows_pipe_name(home: Path) -> str:
    """Per-SANREMES_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\sanremes-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    """
    name = f"sanremes-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if sys.platform == "darwin" and (
        env.get("XPC_SERVICE_NAME", "").startswith("ai.sanremes")
        or env.get("LAUNCHD_SOCKET")
    ):
        return "launchd"
    if env.get("SANREMES_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "sanremes_home": record.get("sanremes_home"),
        "profile": _profile_label_for_home(record.get("sanremes_home") or ""),
...
class GatewayControlServer:
    ...
    its SANREMES_HOME), ``stop()`` on shutdown. All failures are non-fatal —
    ...
    def __init__(
        ...
        *,
        ...
        if home is None:
            from gateway.status import _get_process_sanremes_home
