...
Defenses 1 and 2 (the ``_SANREMES_GATEWAY`` guard on ``sanremes gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.sanremes.gateway")``,
...
State lives in ``<SANREMES_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_sanremes_home() / "gateway" / "restart_loop.json"
