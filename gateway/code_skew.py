...
window before ``sanremes update``'s graceful restart fires), a first-time lazy
...
def _fingerprint() -> str | None:
    ...
    ``sanremes_cli.main`` is always already imported in a gateway process (it's
    the entry point), so this import is free and avoids duplicating the
    worktree-aware ref resolution.
    """
    try:
        from sanremes_cli.main import _read_git_revision_fingerprint
