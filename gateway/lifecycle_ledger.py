...
``<SANREMES_HOME>/state/gateway.lifecycle.json``:
...
def _process_sanremes_home() -> Path:
    """SANREMES_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<SANREMES_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``sanremes_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
