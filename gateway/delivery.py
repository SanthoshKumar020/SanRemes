...
class DeliveryRouter:
    ...
    def __init__(self, config: GatewayConfig, adapters: Dict[Platform, Any] = None,
        ...
        self.output_dir = get_sanremes_home() / "cron" / "output"
    ...
    def _save_full_output(self, content: str, job_id: str) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_dir = get_sanremes_home() / "cron" / "output"
    ...
    def _filter_silence_narration_enabled(self) -> bool:
        ...
        ``SANREMES_FILTER_SILENCE_NARRATION`` env var overrides config when set;
        otherwise the ``gateway.filter_silence_narration`` config flag wins
        (default True).
        """
        env = os.getenv("SANREMES_FILTER_SILENCE_NARRATION")
        if env is not None:
