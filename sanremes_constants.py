"""Shared constants for SanRemes Agent.
...
_UNSET = object()
_SANREMES_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_SANREMES_HOME_OVERRIDE", default=_UNSET
)
...
def set_sanremes_home_override(path: str | Path | None) -> Token:
    """Set a context-local SanRemes home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _SANREMES_HOME_OVERRIDE.set(value)


def reset_sanremes_home_override(token: Token) -> None:
    """Restore the previous context-local SanRemes home override."""
    _SANREMES_HOME_OVERRIDE.reset(token)


def get_sanremes_home_override() -> str | None:
    """Return the active context-local SanRemes home override, if any."""
    override = _SANREMES_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_sanremes_home() -> Path:
    """Return the platform-native default SanRemes home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "sanremes"
    return Path.home() / ".sanremes"


def _sanremes_home_from_env() -> Path:
    """Resolve SANREMES_HOME from the process environment only.

    Reads the ``SANREMES_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_sanremes_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_sanremes_home`
    and :func:`get_process_sanremes_home` so the two never drift.
    """
    val = os.environ.get("SANREMES_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_sanremes_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``SANREMES_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_sanremes_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[SANREMES_HOME fallback] SANREMES_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass SANREMES_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_sanremes_home() -> Path:
    """Return the SanRemes home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_sanremes_home_override`) → ``SANREMES_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``SANREMES_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``SANREMES_HOME`` explicitly (see the systemd
    template in ``sanremes_cli/gateway.py`` and the kanban dispatcher in
    ``sanremes_cli/kanban_db.py``).  See https://github.com/NousResearch/sanremes-agent/issues/18594.
    """
    override = get_sanremes_home_override()
    if override:
        return Path(override)

    if not os.environ.get("SANREMES_HOME", "").strip():
        _warn_profile_fallback_once()

    return _sanremes_home_from_env()


def sanremes_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a SanRemes home/profile directory.
    ...
    """
    candidate = Path(path) if path is not None else get_sanremes_home()
    resolved = candidate.expanduser().resolve(strict=False)
    return os.path.normcase(str(resolved))


def get_process_sanremes_home() -> Path:
    """Return the SanRemes home for the running process, ignoring task overrides.

    Unlike :func:`get_sanremes_home`, this never follows the context-local
    override set by :func:`set_sanremes_home_override`.  It resolves only the
    process ``SANREMES_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_sanremes_root(). The function resolves
# SANREMES_HOME against the native home on every call (~80us of path
...
# Its result depends only on (SANREMES_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates SANREMES_HOME mid-process.
_default_sanremes_root_memo: "tuple[str, str, Path] | None" = None


def get_default_sanremes_root() -> Path:
    """Return the root SanRemes directory for profile-level operations.

    In standard deployments this is the platform-native SanRemes home
    (``~/.sanremes`` on POSIX, ``%LOCALAPPDATA%\\sanremes`` on native Windows).

    In Docker or custom deployments where ``SANREMES_HOME`` points outside
    ``~/.sanremes`` (e.g. ``/opt/data``), returns ``SANREMES_HOME`` directly
    — that IS the root.

    In profile mode where ``SANREMES_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.sanremes/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    global _default_sanremes_root_memo
    native_home = _get_platform_default_sanremes_home()
    env_home = os.environ.get("SANREMES_HOME", "")
    if _default_sanremes_root_memo is not None:
        memo_native, memo_env, memo_result = _default_sanremes_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # SANREMES_HOME is under ~/.sanremes (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — SANREMES_HOME itself is the root
                result = env_path
    _default_sanremes_root_memo = (str(native_home), env_home, result)
    return result
...
# Files whose presence marks a directory as a real SanRemes home. A fresh home
...
_SANREMES_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_sanremes_profiles_root(profiles_dir: Path) -> bool:
    """Return True when *profiles_dir* is a canonical ``<sanremes-home>/profiles``.

    Anchors named-profile recognition so it only fires for directories that
    provably live under a SanRemes home: the classic ``~/.sanremes`` layout, a
    root carrying SanRemes-home marker files (Docker/custom ``SANREMES_HOME``
    like ``/opt/data``), a ``profiles/.deleted`` tombstone directory (only
    ever created by ``sanremes profile delete``), or the process's resolved
    default SanRemes root.
    """
    root = profiles_dir.parent
    if root.name == ".sanremes":
        return True
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir():
            return True
        if any((root / marker).exists() for marker in _SANREMES_HOME_MARKERS):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_sanremes_root().resolve(
            strict=False
        )
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    SanRemes home (see :func:`_is_sanremes_profiles_root`). A default SanRemes home
    whose path merely contains a ``profiles`` segment
    (e.g. ``/tmp/foo/profiles/notahome/.sanremes``) is not a named profile,
    ...
    for candidate in (current, *current.parents):
        if (
            ...
            and _is_sanremes_profiles_root(candidate.parent)
        ):
            return candidate
        # Stop at a default SanRemes home so a coincidental ``profiles/``
        # ancestor is not treated as a named-profile root.
        if candidate.name == ".sanremes":
            return None
    return None
...
def assert_named_profile_home_live(path: str | Path) -> None:
    ...
    Default ``SANREMES_HOME`` (not under ``profiles/``) is unchanged.
    """
...
def mkdir_under_sanremes_home(path: str | Path) -> Path:
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``SANREMES_OPTIONAL_SKILLS``.
    """
    override = os.getenv("SANREMES_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_sanremes_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``SANREMES_OPTIONAL_MCPS``.
    """
    override = os.getenv("SANREMES_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_sanremes_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``SANREMES_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<SANREMES_HOME>/skills`` last-resort
    """
    override = os.getenv("SANREMES_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_sanremes_home() / "skills"


def get_sanremes_dir(
    new_subpath: str,
    old_name: str,
    *,
    home: Path | None = None,
) -> Path:
    """Resolve a SanRemes subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to SANREMES_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to SANREMES_HOME (e.g. ``"image_cache"``).
        home: Optional explicit SanRemes home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local SANREMES_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_sanremes_home()
...
def iter_sanremes_node_dirs(home: Path | None = None) -> list[Path]:
    """Return SanRemes-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\sanremes\\node``. POSIX installs use
    ``$SANREMES_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_sanremes_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with sanremesManagedNodePathEntries() in
...
_SANREMES_NODE_TARGET_MAJOR = int(os.environ.get("SANREMES_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/sanremes_constants.py).
# Used by secure_parent_dir() to skip chmod on the install dir — chmodding it
# 0700 breaks sanremes-user traversal in Docker (UID 10000). See #25821, #93050.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    SanRemes-managed Node trees live under ``$SANREMES_HOME/node`` (or a profile's
    ``SANREMES_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_sanremes_node_executable``
    used to trust file presence alone, so ``sanremes update`` would pick that
...
def sanremes_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any SanRemes-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_sanremes_node_dirs(home):
        for name in names:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ SanRemes-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `sanremes update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%SANREMES_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the sanremes.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_SANREMES_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_SANREMES_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$SANREMES_HOME/node`` on POSIX.
    ...
    ``SANREMES_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
    ...
    try:
        result = subprocess.run(
            ...
            env={
                **os.environ,
                "SANREMES_HOME": str(get_sanremes_home()),
...
def bootstrap_sanremes_managed_node() -> str | None:
    """Install a SanRemes-managed Node tree and return its npm path.
    ...
    SanRemes never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$SANREMES_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_sanremes_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_sanremes_node_dirs():
...
def heal_sanremes_managed_node() -> bool:
    """Redownload SanRemes-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not sanremes_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SANREMES_HOME": str(get_sanremes_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_sanremes_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_SANREMES_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_sanremes_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _SANREMES_NODE_TARGET_MAJOR
    return False


def find_sanremes_node_executable(command: str) -> str | None:
    """Return a SanRemes-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_SANREMES_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_sanremes_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for SanRemes-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy SanRemes-managed installs.

    This is for SanRemes-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_sanremes_node_executable(command)
    if managed:
        return managed
    if sanremes_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_sanremes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with SanRemes-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_sanremes_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``sanremes update`` — leaving a **dangling symlink** that ``which`` still
...
def display_sanremes_home() -> str:
    """Return a user-friendly display string for the current SANREMES_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.sanremes``
        profile:  ``~/.sanremes/profiles/coder``
        custom:   ``/opt/sanremes-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.sanremes``.  For code that needs a real ``Path``, use
    :func:`get_sanremes_home` instead.
    """
    ...
    try:
        ...
        # ``~/AppData\Local\sanremes/skills/`` once callers append
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``SANREMES_HOME`` or other path
    env vars resolve to an unexpected location.

    Also refuses to chmod the sanremes-agent install tree (the directory this
    ...
    See https://github.com/NousResearch/sanremes-agent/issues/25821 and
    https://github.com/NousResearch/sanremes-agent/pull/93050.
    """
    parent = path.parent.resolve()
    # Refuse root and its direct children (/usr, /home, /var, /tmp, …).
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree root. chmodding it 0700 breaks sanremes-user
    ...
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        # A credential file inside the install tree usually means SANREMES_HOME
        ...
        logging.getLogger(__name__).warning(
            ...
            "sanremes-agent install directory (%s). Credential files are "
            "normally stored under the sanremes home directory instead.",
            parent,
            _INSTALL_ROOT,
        )
        return
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{SANREMES_HOME}/home`` when the profile-home directory exists."""
    sanremes_home = get_sanremes_home_override() or (env or {}).get("SANREMES_HOME") or os.getenv("SANREMES_HOME")
    if not sanremes_home:
        return None
    profile_home = os.path.join(sanremes_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("SANREMES_REAL_HOME") or os.getenv("SANREMES_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding SanRemes profile HOME.

    ``SANREMES_HOME`` scopes SanRemes state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={SANREMES_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{SANREMES_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{SANREMES_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply SanRemes' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["SANREMES_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when SanRemes itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/sanremes-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under SANREMES_HOME.

    Replaces the ``get_sanremes_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, sanremes_logging.py, sanremes_time.py, etc.).
    """
    return get_sanremes_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under SANREMES_HOME."""
    return get_sanremes_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under SANREMES_HOME."""
    return get_sanremes_home() / ".env"
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_sanremes_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._sanremes_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``sanremes_cli`` modules using three different Windows
    ...
    A few sites outside ``sanremes_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``sanremes_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of SanRemes itself. An ImportError
...
# `sanremes_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with SanRemes.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("sanremes_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `sanremes update` is exactly
    ...
    return [
        ...
        "    sanremes update",
        "If that also fails, reinstall: https://sanremes-agent.nousresearch.com",
    ]
